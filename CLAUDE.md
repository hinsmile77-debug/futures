...
⑧ 사이징 재설계 — 실전 자본 재설정 **+ `MAX_CONTRACTS` 동반 재검토** (2026-08-01 재작성)
   ...
   > 2026-08-01 MW0601 점검(§9-3)에서 계약수별 실현손익이 **완전 단조**임이 확인됐다:
   > 1계약 173건 **+1,682만원** / 2계약 18건 +138만원 / **3계약 이상 16건 6.2%(1승 15패) -2,642만원**
   > (이항검정 p=5.07e-07, 일자 분산 11일, 최악 2일 제거 후에도 10전 1승 -1,236만원).
   > 계약수 상한 카운터팩추얼도 단조다 — 1계약 고정 +1,274만원 / 상한 2 +865만원 /
   > 상한 3 +388만원 / **상한 없음(실제) -822만원**.
   > 현행 흑자는 목표자본 5천만원이 **우발적으로** 사이즈를 1~2계약에 묶은 결과이며
   > (311차 후속4 `cf2d1b4`, 2026-07-12 경계와 정확히 일치), `SIZING_TARGET_CAPITAL_ENABLED`를
   > 끄면 `MAX_CONTRACTS = 10`(현행) 아래에서 그 이전 3~10계약 체제가 그대로 되살아난다.
   >
   > **따라서 ⑧은 다음 두 가지를 함께 만족해야 해제된다.**
   > 1. 실전 자본으로 `SIZING_TARGET_CAPITAL_KRW` 재설정 (또는 False 전환)
   > 2. **`MAX_CONTRACTS` 재검토** — 인하 판단은 `VALIDATION_CAMPAIGN["sizing_inversion_watch"]`
   >    누적 판정을 근거로 할 것(사후 데이터로 즉시 내리는 것은 313차 위반).
   >
   > 미해결: **왜 3계약 이상에서만 지는가**(사이저의 어느 입력이 역예측적인지)는 미분해다.
   > 상한 인하가 원인 치료인지 증상 억제인지 아직 모른다 — 채널 판정과 함께 볼 것.
   > (근거: `docs/정기점검/매일점검/MW0601-20260731-점검리포트.md` §9-3,
   >  `docs/정기점검/금요일점검/0801_MW0601_최종개선안.md` P0-1)
