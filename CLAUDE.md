...
>
> 🔴 **복원 조건이 지금은 충족 불가능하다 — 섀도 관찰 수단이 배선돼 있지 않다.**
> `strategy/risk/toxicity_gate.py`가 `spread_extreme_shadow`를 매분 계산하지만
> **어디에서도 소비하지 않는다** — 로그·DB·대시보드 어디에도 안 남고, `spread_ticks`
> 자체를 저장하는 컬럼도 없다(`ensemble_decisions`에는 `toxicity_score`·`_ma`·
> `_size_mult`·`_action`·`_reason`만 있다). 2026-07-12 도입 이래 **한 달 넘게 죽은
> 섀도**다. FP-CRITICAL이 "학습분포 저장 함수가 프로덕션에서 호출된 적이 없어 2개월간
> PSI=0.0 고정"이었던 것과 **같은 결함 패턴**이다.
>
> **복원 절차(2단계 — 순서를 지킬 것)**
> 1. **섀도 계측 배선** — `spread_ticks`와 `spread_extreme_shadow`를 영속화한다
>    (`NEXT_TODO.md` F-8). 이게 없으면 아래 2는 영원히 시작되지 않는다.
> 2. 배선 후 관측이 쌓이면 `spread_ticks ≥ 20` 발생 빈도와 그 분봉의 실현 손익을
>    확인하고, 임계 20이 타당한지 재산출한 뒤 `True`로 복원.
>
...
⑨ TOX-SEVERE-SPREAD 재검토 — `TOXICITY_SEVERE_SPREAD_BLOCK_ENABLED` False → 복원 여부 결정
   (2026-08-13 461차 등재. **선행 필수: 섀도 계측 배선** — `spread_extreme_shadow`가 계산만
   되고 저장되지 않아 복원 판단 근거를 만들 수 없다. 절대원칙 §2 TOX-SEVERE-SPREAD 항목 참조)
