    ...
    with _lock:
        with get_conn(TRADES_DB) as conn:
            ...
            additions = {
                ...
                "entry_qty": "INTEGER",
                # [MW0602 465차 P4] 청산 시점에 TP1이 이미 도달돼 있었는가.
                #
                # **왜 필요한가.** `exit_reason`의 `하드스톱`/`하드스톱(틱)`은 정반대
                # 두 사건에 같은 이름을 쓴다 — ① 진짜 손절과 ② TP1 도달 후 보호
                # 스톱(qty=1 ATR-lock 보호전환 / qty≥2 TP1 이후 본전 이동)에 걸린
                # **이익** 청산이다. 0812 실측(레그 단위):
                #     하드스톱[손실] 59건 −9,070,416원  vs  하드스톱[이익] 81건 +4,118,020원
                # 한 버킷으로 집계하면 `하드스톱(틱)` 평균 −65,804 / `하드스톱` 평균
                # +137,060 이라는 무의미한 숫자가 나온다. 0812 6포지션 중 4건이
                # "하드스톱"으로 이익 청산됐다.
                #
                # **왜 라벨을 바꾸지 않는가.** `exit_reason LIKE '%하드스톱%'`가
                # 사전등록 채널 [15]·[26]·[48]과 `scripts/bar_stop_path_watch.py`,
                # `tests/test_439_p2_slippage_judgment.py`의 필터로 쓰인다. 문자열을
                # 갈아치우면 그 채널들의 판정이 조용히 뒤집힌다(캠페인 거버넌스 위반).
                # 그래서 라벨은 그대로 두고 **관측 컬럼을 더해** 소비 측이 스스로
                # 쪼개게 한다.
                #
                # 출처: `PositionTracker.partial_1_done`(TP1 부분청산·보호전환 양쪽
                # 경로에서 True). NULL=구버전 레코드.
                # ⚠ 소급 백필은 하지 않는다 — `pnl_pts>0`로 추정할 수는 있으나
...
_BLOCK_REASON_CATEGORIES = [
    ...
    ("증거금 부족",        "증거금부족"),
    # [MW0602 485차] 방향 무효화 — STEP6 이후 게이트(RegimeOverride·σ미수집·
    # 조건부구간·ATR저변동 등)가 로컬 direction을 0으로 지워 사유 체인이 도달
    # 불가였던 544건(06-02~)이 "차단없음"으로 오집계되던 것을 신설 문구로 분류.
    # 출처 키가 문구에 병기되므로 상세 분해는 entry_gate_json.direction_zeroed_by 로.
    ("방향 무효화",        "방향무효화(신호소거)"),
...
def is_krx_trading_date(date_str: str) -> bool:
    ...
    🔴 **[MW0602 501차] 음성 결과를 영구 캐시하지 않는다 — 그게 결함이었다.**
    종전에는 True/False를 똑같이 `_trading_date_cache`에 박았다. 그런데 이 함수의
    첫 호출은 **장전 08:41 잔고 push**이고, 그 시점에는 당일 `predictions` 첫 행
    (09:00)도 `trades` 행도 아직 없다 — 그래서 `False`가 계산돼 **세션 내내** 굳었다.
    귀결: 493차 F-2가 배선한 `upsert_broker_net`이 2026-08-25 이후 **한 번도
    성공하지 못했다**(0827 로그 실측: `[BrokerNet] state=SKIP_NON_TRADING` 35건 전부).
