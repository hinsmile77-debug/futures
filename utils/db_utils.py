...
def _migrate_trades_db():
    ...
    with _lock:
        with get_conn(TRADES_DB) as conn:
            ...
            # 🔴 [MW0602 497차 / P2 체리픽] **상시** NULL 백필 — 위 1회성 백필
            # (`if "commission_rate_used" not in cols` 안이라 컬럼 신설 때 딱 한 번)
            # 과 별개다. 493차가 라이브 INSERT에 이 컬럼을 빠뜨려, 마이그레이션
            # 이후 새 행이 전부 NULL로 쌓였다(497차 INSERT fix 이전 구간).
            # 그 행들은 **현행 요율 세대**(EFFECTIVE_FROM 이후)에 실제로 그 요율로
            # 계산됐음이 확정이므로 채운다 — **추정이 아니라 기록**이다(4원칙 ④).
            # ⚠ EFFECTIVE_FROM **이전** NULL은 건드리지 않는다 — 위 1회성 백필의
            #   몫이며, 여기서 현행 요율로 채우면 세대가 오염된다.
            # ⚠ v9-dev 기준 EFFECTIVE_FROM은 CYBOS 채널 전환일(2026-08-25)이다
            #   — 495차 후속으로 채널 스펙에서 파생되므로 브랜치마다 다르다.
            conn.execute(
                "UPDATE trades SET commission_rate_used = ? "
                " WHERE commission_rate_used IS NULL AND date(entry_ts) >= ?",
                (FUTURES_COMMISSION_RATE,
...
def init_premarket_levels_db():
    ...
    """)
    # ── [MW0601 542차] 수동 산출 관측 로그 — **별도 테이블** ──────────────
    ...
    execute(PREMARKET_LEVELS_DB, """
        CREATE TABLE IF NOT EXISTS premarket_levels_manual (
            ...
            dist_high   REAL, dist_low REAL,
            sofar_high  REAL, sofar_low REAL,
            rhat_scale  REAL, train_n INTEGER,
            cand_total  INTEGER, cand_above INTEGER, cand_below INTEGER,
...
def save_premarket_levels_manual(date_str: str, clicked_at: str, row: dict) -> None:
    """[MW0601 542차] 수동 산출 1건 — append-only 관측 로그.
    ...
    diag = struct.get("diag") or {}
    execute(PREMARKET_LEVELS_DB,
            """INSERT OR REPLACE INTO premarket_levels_manual
               (date, clicked_at, at_time, model, ref_price, open_price, atr14,
                dist_high, dist_low, sofar_high, sofar_low, rhat_scale, train_n,
                cand_total, cand_above, cand_below, struct_up, struct_down,
                bars, bars_source, note, warnings, payload)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (date_str, clicked_at, row.get("at"), row.get("model"),
             ...
             dist.get("scale"), row.get("train_n"),
             diag.get("total"), diag.get("above"), diag.get("below"),
             json.dumps(struct.get("up"), ensure_ascii=False) if struct else None,
             json.dumps(struct.get("down"), ensure_ascii=False) if struct else None,
