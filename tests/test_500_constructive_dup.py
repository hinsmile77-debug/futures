...
검사 문구와 조건은 **원문 그대로**다(이 브랜치 판 C7b 포함). 블록을 함수로 감싸
그룹별 테스트에서 부르고, 공유 setup(합성 데이터·소스 읽기)은 지연 생성으로 바꿔
임포트 부작용을 없앴다 — 모듈 최상위에 계산을 남기면 O-77 2차층이 재발한다.
"""
...
def _c7():
    ...
    check("C7b 실집행 키(CORE_FEATURES)를 쓴다 — 문서의 CORE 이름이 아니라",
          "from config.constants import CORE_FEATURES" in src,
