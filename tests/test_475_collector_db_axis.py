...
def test_db_failure_is_unmeasured_not_zero():
    ...
    try:
        ...
        shutil.rmtree(root, ignore_errors=True)
    r = out[0]
    # [MW0602 494차] 476차 F-2' 가 판정을 **의도적으로** 갈랐다 —
    # `무기록`(로그 문구 변경/계측 중단 의심, §11 적신호)과 `DB미접속`(수집기 환경
    # 문제 = 미측정)은 처방이 다르다. 그때 이 단언이 함께 갱신되지 않아 이 테스트는
    # 그 이후로 **줄곧 실패 상태**였다(494차 회귀 확인 중 발견 — 이 세션 변경 전에도
    # 실패한다). 상시 붉은 테스트는 그 자체가 죽은 안전장치다.
    check("(5) DB 접근 실패는 `DB미접속`(미측정) — `무기록`과 구분한다(476차 F-2')",
          r["verdict"] == "DB미접속")
