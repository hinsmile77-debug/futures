...
실행: python tests/test_455_multi_hz_feature_eval.py   (DB/브로커 불필요, 읽기 전용)
            또는 pytest tests/test_455_multi_hz_feature_eval.py

🔴 [MW0602 564차 후속3 / `O-77-C` 3단계] **스크립트형 → pytest 전환.**

검사 문구와 조건은 **원문 그대로**다. `# ── Tn: ... ──` 배너를 경계로 블록을
함수로 감싸 그룹별 테스트에서 부른다. 각 블록이 자기 import·setup 을 들고 있어
블록 간 상태 의존은 없다.

⚠ 모듈 최상위의 `sys.stdout.reconfigure(...)` 도 CLI 러너로 옮겼다 — 임포트
  부작용을 남기지 않는다는 전환 원칙. (그 호출 자체는 `O-77` 의 고아 래퍼
  패턴이 **아니다** — 제자리 변경이다.)
"""
import math
import os
...
def test_T1_T2_l4_two_prop_and_analyze():
    """L4 two-proportion z 손계산·대칭성과 analyze 판정/계약 키."""
    _assert_group(_t1_t2)
...
def test_T3_T6_l2_cost_median_and_no_lookahead():
    """L2 비용 산식 정본 일치 · RunningMedian · 미래참조 부재 · 비중복 간격."""
    _assert_group(_t3_t6)
...
def test_T7_n1_ic_cost_hurdle():
    """N1 E[z|top5%] · norm_ppf · IC*(h) ∝ 1/√h."""
    _assert_group(_t7)
...
def test_T8_n3_shuffle_reproducibility():
    """N3 시드 재현성 · 셔플 분포 보존 · phase 길이/NaN 위치 보존 · noise_floor."""
    _assert_group(_t8)
...
def test_T9_T10_n2_decay_classify_and_curves():
    """N2 감쇠 유형 분류 + fit_tau 복원 + analyze_curves peak_h."""
    _assert_group(_t9_t10)
...
def test_T11_n5_bounce_artifact():
    """N5 합성 bounce 가 IC_close 에서만 잡히고 IC_open·IC_mid 는 0 근방."""
    _assert_group(_t11)
...
if __name__ == "__main__":
    ...
    for name, fn in fns:
        try:
            ...
            fail += 1
    print("")
    print("%d passed, %d failed (of %d) — checks %d" % (ok, fail, len(fns), N_CHECKS[0]))
