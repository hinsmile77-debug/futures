...
🔴 [MW0601 체리픽 / 2026-08-26] **원본은 `t1_`~`t8_` 이라 pytest 가 수집하지 못했다**
   — 전체 스위트에서 한 번도 돌지 않는 「죽은 테스트」가 된다(495차와 같은 형태).
   `test_` 로 바꿔 수집되게 했다. 단독 실행 경로는 유지한다.
   블록 앵커도 태그 무관(`실시간잔고 스트립 축 정합`)으로 완화했다 — 이 브랜치는
   체리픽 표기(`/ P1 체리픽`)를 쓰기 때문이다.

⚠ 이 브랜치는 **CYBOS 채널(0.0098104%)** 이라 MW0602(CREON 0.0019%)보다 수수료가
   **5.16배** 크다 — P3(ProfitGuard net 축)의 효과 크기도 그만큼 크다.

⚠ DB 조회는 `trades.db`(수백 행) 단순 집계뿐이다 — CLAUDE.md 456차가 금지한 것은
   `raw_data.db`·`predictions.db` 전수 스캔이며 여기 해당하지 않는다.
"""
import datetime
import io
import os
