...
def test_cost_is_much_higher_than_the_legacy_model():
    ...
    🔴 [MW0602 553차 체리픽 조정] 원본은 `new / old > 5.0` 이었다. 그 5.0 은
      **CYBOS 배수(7.81)를 보고 고른 핀값**이라 CREON 에서는 구조적으로 못 넘는다 —
      이 PC 실측 배수는 **2.54** 다(요율 0.0019% vs 0.0098104%, 3.08배 싸다.
      553차 Phase 0 이 비용을 채널별로 분리한 이유가 바로 그것이다).
      달성 불가능한 문턱은 사전등록이 아니라 알리바이다 — 원본 Phase 0 이 롱 α
      유의성을 요구하지 않은 것과 같은 논리다.
    ⇒ **핀값 대신 파생식으로 고정한다**(이 파일의 「핀값 금지, 493차」 원칙 그대로).
    """
    ctx = CC.cost_context()
    new = CC.roundtrip_cost_pt(1050.0, 1050.0, ctx)
    old = 2 * 1050.0 * CC.LEGACY_KIWOOM_RATE

    # 파생 기대값 — 요율·틱·슬리피지에서 직접 계산한다(사본 금지)
    expect = (2 * 1050.0 * ctx["one_way_rate"]
              + 2 * ctx["slip_ticks_per_side"] * ctx["tick_size"]) / old
    assert abs(new / old - expect) < 1e-9, (
        "배수가 파생식과 어긋난다 — 비용 공식이 바뀌었다: %.4f vs %.4f"
        % (new / old, expect))
    # 채널 무관 하한: 구 모델은 슬리피지가 **아예 없어** 어느 채널이든 2배는 넘는다.
    assert new / old > 2.0, (
        "구 키움 모델과 차이가 사라졌다 — 요율이 되돌아갔는지 확인할 것 "
        "(channel=%s rate=%s)" % (ctx["broker_channel"], ctx["one_way_rate"]))
