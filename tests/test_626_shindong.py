...
D. 차트 — 신동 레이어가 **실제로 그려진다**(픽셀), 실측 색과 안 겹친다, 실측 아래에 깔린다,
   「미배선 · 0건 · n건」 문구가 갈린다, 로더가 `trades` 가 아니라 `shindong.db` 를 읽는다.
   (553차 GP 차트 테스트가 고정하던 원칙을 그대로 옮겼다 — GP 레이어는 이번에 걷어냈다.)
...
def _sd_trade(direction="LONG", entry_i=5, exit_i=25, closed=True):
    ts = lambda i: (_BASE + timedelta(minutes=i)).strftime("%Y-%m-%d %H:%M:00")  # noqa: E731
    up = direction == "LONG"
    legs = []
    if closed:
        legs = [{"exit_ts": ts(exit_i - 8), "exit_price": 350.6 if up else 349.9,
                 "reason": "TP1", "pts": 0.3},
                {"exit_ts": ts(exit_i), "exit_price": 350.8 if up else 349.7,
                 "reason": "TP2", "pts": 0.5}]
    return {"trade_key": "R2|x|%s" % direction, "rule": "R2", "entry_ts": ts(entry_i),
            "direction": direction, "entry_price": 350.2, "status": "CLOSED" if closed else "OPEN",
            "stop_init": 349.6 if up else 350.8, "stop_now": 349.6 if up else 350.8,
            "t1": 350.6 if up else 349.9, "t2": 350.8 if up else 349.7,
            "touch_level": None, "net_krw": 10000.0, "legs": legs}
...
def _color_present(img, hex_color, tol=26):
    ...
    for x in range(0, img.width(), 2):
        for y in range(0, img.height(), 2):
            ...
            if (abs(px.red() - want.red()) <= tol and abs(px.green() - want.green()) <= tol
                    and abs(px.blue() - want.blue()) <= tol):
                return True
    return False


def test_shindong_long_is_actually_painted():
    c = MinuteChartCanvas()
    c.reset_session(_candles(), [], sd_trades=[_sd_trade("LONG")], sd_wired=True)
    assert _color_present(_render(c), MinuteChartCanvas.SD_LONG_COLOR), (
        "신동 매수가 캔버스에 안 나타난다 — 배선만 되고 그려지지 않는 상태다")


def test_shindong_short_is_actually_painted():
    c = MinuteChartCanvas()
    c.reset_session(_candles(), [], sd_trades=[_sd_trade("SHORT")], sd_wired=True)
    assert _color_present(_render(c), MinuteChartCanvas.SD_SHORT_COLOR)


def test_no_shindong_color_without_trades():
    """대조군 — 신동 색이 다른 요소에서 새어 나오면 위 두 검사가 무의미하다."""
    c = MinuteChartCanvas()
    c.reset_session(_candles(), [], sd_trades=[], sd_wired=True)
    img = _render(c)
    assert not _color_present(img, MinuteChartCanvas.SD_LONG_COLOR)
    assert not _color_present(img, MinuteChartCanvas.SD_SHORT_COLOR)


def test_open_trade_draws_plan_lines_without_exit():
    """보유 중 = 청산 없음. 죽지 않고 진입 + 목표/손절 선을 그린다(손익 0 으로 위장 금지)."""
    c = MinuteChartCanvas()
    c.reset_session(_candles(), [], sd_trades=[_sd_trade("LONG", closed=False)], sd_wired=True)
    img = _render(c)
    assert _color_present(img, MinuteChartCanvas.SD_LONG_COLOR)
    # 점선 1.2px 은 안티에일리어싱으로 배경과 섞인다 — 색상 계열로 본다
    assert _color_present(img, MinuteChartCanvas.SD_STOP_COLOR, tol=50)


def test_toggle_off_hides_layer():
    c = MinuteChartCanvas()
    c.reset_session(_candles(), [], sd_trades=[_sd_trade("LONG")], sd_wired=True)
    c.set_overlay("trade_shindong", False)
    assert not _color_present(_render(c), MinuteChartCanvas.SD_LONG_COLOR)


def test_colors_do_not_collide_with_live_trade_colors():
    live = {"#2FBF71", "#FF5D73", "#8B949E"}
    sd = {MinuteChartCanvas.SD_LONG_COLOR.upper(), MinuteChartCanvas.SD_SHORT_COLOR.upper()}
    assert not (sd & {x.upper() for x in live})


def test_shindong_is_drawn_before_live_markers():
    import inspect
    src = inspect.getsource(MinuteChartCanvas.paintEvent)
    assert src.index("_draw_shindong_layer") < src.index("self._draw_markers(")
...
def test_status_separates_unwired_from_zero(trades, wired, expect):
    assert MinuteChartDialog._sd_status_text(None, trades, wired) == expect


def test_status_counts_open_and_decision():
    txt = MinuteChartDialog._sd_status_text(
        None, [_sd_trade("LONG"), _sd_trade("SHORT", closed=False)], True,
        {"pm_sp": 114.0, "bias": -1, "r2_status": "CONFIRMED", "r2_ts": "09:01",
         "product": "wk_thu"})
    assert txt.startswith("신동 2건 (보유 1)")
    assert "+114→하방" in txt and "R2 09:01확정" in txt and "wk_thu" in txt


def test_loader_reads_shindong_db_not_trades_db():
    """🔴 가상거래를 `trades` 에서 읽으면 실거래와 섞인다."""
    import inspect
    src = inspect.getsource(MinuteChartDialog._load_shindong_trades)
    assert "SHINDONG_DB" in src
    assert "TRADES_DB" not in src and "FROM trades" not in src


def test_rows_to_chart_keeps_open_legs_open():
    rows = [{"trade_key": "k", "rule": "R3", "entry_ts": "2026-09-23 11:49:00", "side": 1,
             "entry_px": 1112.52, "status": "OPEN", "stop_init": 1108.5, "stop_now": 1108.5,
             "t1": 1134.5, "t2": 1136.5, "touch_level": 1110, "net_krw": 0.0,
             "leg1_exit_ts": None, "leg2_exit_ts": None}]
    out = MinuteChartDialog._sd_rows_to_chart(rows)
    assert out[0]["legs"] == [] and out[0]["direction"] == "LONG"
