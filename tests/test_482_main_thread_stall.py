...
def test_threshold_ordering():
    from config.settings import (
        ...
        MAIN_THREAD_STALL_ALERT_MS, FREEZE_WATCHDOG_STALL_SEC, CB_PIPE_PAUSE_MS,
    )
    assert MAIN_THREAD_STALL_DETECT_MS < MAIN_THREAD_STALL_WARN_MS
    assert MAIN_THREAD_STALL_WARN_MS < MAIN_THREAD_STALL_ALERT_MS
    # ALERT 는 FZ-1(하드 종료) 이전에 울려야 의미가 있다 — 그 사이가 이 계측의 존재 이유다.
    assert MAIN_THREAD_STALL_ALERT_MS < FREEZE_WATCHDOG_STALL_SEC * 1000
