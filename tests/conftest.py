...
# ── [MW0602 564차 / `O-77` 2차층] 스크립트형 파일 수집 제외 — **현재 비어 있다** ──
#
...
#
# 🔴 [564차 후속3 / `O-77-C`] 해당 6파일을 **전부 진짜 pytest 테스트로 전환**해
#    목록이 비었다. 전환 전에는 그 6파일의 약 140개 검사가 **스위트에서 한 건도
#    돌지 않았다** — 죽은 섀도·죽은 게이트와 같은 형태였다.
#
# 이 목록을 다시 채우기 전에 먼저 물을 것 — **왜 이 파일은 테스트 함수가 없는가.**
# 격리는 전환을 미루는 수단이지 목적지가 아니다. 등재를 잊으면 `test_564` 의
# 규칙 B 가 잡아준다(목록을 소스에서 되읽어 대조한다).
collect_ignore = [
]
