...
# ✅ [MW0602 495차 후속 / 2026-08-26] **CREON 트레이딩 고시로 확정 — 로그인 채널이 정한다.**
#   실측: `commission_rate_recon.py --verify` 이 PC 33거래일 / 약정대금 339.3억원,
#   실효 편도 0.0019000%, R²=1.000000, 최대 잔차 1.5원, 고정비 ≈0. 사용자 고시 화면
#   확인(CREON 트레이딩: KOSPI200/미니 선물 0.0019%)과 일치 — 독립 이중 확인.
#   MW0601(CYBOS 로그인)은 0.0098104% — 채널 간 5.16배 차이가 495차 재보정의 원인.
#   이 테스트는 **이 PC에서 감지될 채널(CREON)의 값**을 고정한다. 채널 감지·매핑
#   자체는 tests/test_495_broker_channel_rate.py가 검증한다.
