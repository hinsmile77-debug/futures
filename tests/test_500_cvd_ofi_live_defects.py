...
실행: python tests/test_500_cvd_ofi_live_defects.py   (COM/브로커 불필요)
            또는 pytest tests/test_500_cvd_ofi_live_defects.py

🔴 [MW0602 564차 후속3 / `O-77-C` 4단계] **스크립트형 → pytest 전환.**

검사 문구와 조건은 **원문 그대로**다. `# ══ Tn ══` 배너를 경계로 블록을 함수로
감싸 그룹별 테스트에서 부른다.

⚠ **DB 부재 시 `pytest.skip` 을 쓰지 않는다.** 이 파일은 이미 자체 `skip()` 으로
  「⏳ 통과가 아니라 미실행」을 출력·집계한다. pytest.skip 으로 바꾸면 그 문구가
  사라져 검사 이름 집합이 달라진다(전환 검증 기준 ①). 미실행은 종전대로
  `SKIPPED` 에 남고 그룹 자체는 통과한다 — 직접 실행 때와 같은 의미다.
"""
import io
import json
import os
...
def test_T1_trendgate_threshold_in_measured_range():
    """TrendGate HARD_BREAK_SPEC 기계판독 선언 + 임계가 실측 범위 안 · 발화율 정상."""
    _assert_group(_t1)
...
def test_T2_T3_dead_keys_and_int_truncation():
    """죽은 키 `cvd_slope` 미사용 + `int()` 절단 부재."""
    _assert_group(_t2_t3)
...
def test_T4_ensemble_shortcircuit_symmetry():
    """앙상블 숏서킷이 방향 대칭인가 — 구조적 LONG 편향 제거."""
    _assert_group(_t4)
...
def test_T5_guard_f1_keys_match_checklist():
    """Guard-F1 이 지키는 키 == 체크리스트 실소비 키."""
    _assert_group(_t5)
...
def test_T6_ofi_identities_frozen():
    """OFI 항등식 고정 — `ofi_imbalance`·`ofi_pressure` 는 `ofi_norm` 의 결정론적 파생."""
    _assert_group(_t6)
