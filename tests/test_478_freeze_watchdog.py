...
# ── 🔴 [MW0602 524차 이식 조정] FZ-4(옵션체인 워커 가드)는 **미이식**이다 ──────────
#
# 아래 두 검사는 FZ-1(메인 루프 워치독)이 아니라 **FZ-4**(OptionChainWorker 가
# COM 수집에 매달려 메인을 굶기는 것을 막는 가드)를 본다. 524차는 사용자 지시
# 범위(Phase 1 = FZ-1)만 배선했으므로 `_COLLECT_ABORT_SEC`·`_RESULT_DISCARD_SEC`
# 가 이 브랜치에 없다.
#
# **지우지 않고 skip 으로 남긴다** — 지우면 공백이 보이지 않게 되고, 그것이
# CB②·CB③-P4·FP-CRITICAL 이 "재검토하기로 했는데 안 함"이 된 경로다.
# FZ-4 를 이식하면 이 두 검사가 **자동으로 되살아난다.**
# 등록: `dev_memory/NEXT_TODO.md` 524차 T-2.
def _fz4_present():
    try:
        from collection.options.option_chain_worker import _COLLECT_ABORT_SEC  # noqa: F401
        return True
    except Exception:
        return False


_FZ4_SKIP = pytest.mark.skipif(
    not _fz4_present(),
    reason="FZ-4(옵션체인 워커 가드) 미이식 — 524차 Phase 1 범위 밖. NEXT_TODO 524차 T-2",
)
