...
⚠ 이 세션은 **라이브 상수를 바꾸지 않았다** — `CB_CONSEC_STOP_LIMIT`은 9999
  그대로다. 여기서 고친 것은 "센다"의 정의뿐이고, 한도 복원은 그 다음 단계다
  (2단계 조건부 복원, 사용자 승인 2026-08-23).
...
(H) `CB_CONSEC_STOP_LIMIT`은 여전히 9999다 (한도 복원은 별도 결정)
...
def test_halt_fires_at_limit():
    ...
    ⚠ 테스트가 임계를 만들지 않는다. 현재 9999라 실발동을 볼 수 없으므로
      인스턴스 상수를 낮춰 **발화 가능성**만 확인한다(위음성 방지).
    """
...
def test_limit_still_dormant():
    """(H) 한도는 아직 복원하지 않았다 — 스테이지2는 별도 결정이다."""
    from config.settings import CB_CONSEC_STOP_LIMIT, CB_CONSEC_STOP_WINDOW_SEC
    check("(H) CB_CONSEC_STOP_LIMIT은 여전히 9999 (라이브 무변경)",
