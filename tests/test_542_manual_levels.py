# -*- coding: utf-8 -*-
"""[MW0601 542차] 수동 맥점 산출(임의 시각) — 회귀 테스트.

534차가 08:50/09:30 **정시** 산출의 불변식을 고정했다면, 여기는 그 사이·이후
아무 때나 누르는 **수동 산출**의 불변식을 고정한다. 지키려는 것은 다섯이다.
  ...
  ③ **지어내지 않는다** — 격자 경로 표본이 없으면 distance=None + 사유.
     (구조 모델은 이력만으로 나오므로 그때도 낸다)
  ④ **굳히기를 오염시키지 않는다** — 수동 산출은 `premarket_levels` 에 행을
     만들지 않는다. 그 테이블은 「하루 2행」이 전제라, 섞이면 장후 채점과 60일
     누적 창(`LIMIT days*2`)이 조용히 망가진다.
  ⑤ **관측 전용** — 결과가 판단으로 새지 않는다.
...
def test_stage2_at_0930_matches_stage2():
    ...
    이 등가가 깨지면 144세션 검증치(MAE 11.0pt · 80% 구간 81%)를 수동 산출의
    근거로 인용할 수 없게 된다.
    """
    ss, _, _ = _history()
...
def test_manual_at_0930_matches_frozen_stage(tmp_path):
    ...
    frozen = LS.compute_stage(
        ...
        LS.prepare_params(ss, bb, "2026-09-07"))["out"]
    man = LS.compute_manual(now=datetime.datetime(2026, 9, 7, 9, 33),
                            today_candles=cand, cache_path=cache, persist=False)
...
def test_manual_refits_at_click_time_not_reusing_0930(tmp_path):
    ...
    ss, bb, _ = _history()
    cache = _cache(tmp_path, ss, bb)
    p0930 = LS.prepare_params(ss, bb, "2026-09-07", at_key="0930")
    p1345 = LS.prepare_params(ss, bb, "2026-09-07", at_key="1345")
...
def test_missing_open_bar_is_reported_not_guessed(tmp_path):
    ...
    man = LS.compute_manual(now=datetime.datetime(2026, 9, 7, 13, 47),
                            ...
                            db_path=str(tmp_path / "no_such.db"), persist=False)
...
def test_db_fallback_failure_returns_reason_not_exception(tmp_path):
    ...
    man = LS.compute_manual(now=datetime.datetime(2026, 9, 7, 13, 47),
                            ...
                            db_path=str(tmp_path / "absent.db"), persist=False)
...
def test_main_manual_hook_is_wired_and_returns_nothing():
    ...
    # 반환값이 없어야 판단에 쓸 수 없다(534차 `_compute_premarket_levels` 와 같은 규약)
    assert "def _on_manual_levels_requested(self) -> None:" in src
    # 어떤 경로로 끝나든 화면을 갱신한다 — 안 그러면 「산출 중…」에 멈춘다
...
def test_entry_panel_manual_button_and_rows(tmp_path):
    ...
    #   거둬 위젯 생성 시 프로세스가 예외 없이 죽는다(pytest 가 점 21개에서 멈춘다).
    ...
    row = LS.compute_manual(now=datetime.datetime(2026, 9, 7, 13, 47),
                            ...
                            cache_path=_cache(tmp_path, ss, bb), persist=False)
