...
### 8. QDQ Phase 2 — 앵커 대조 리포트 (읽기 전용, 소비 0)

`scripts/generate_cvd_anchor_report.py` 신설. Phase 0이 적재한 앵커·섀도 열을
**판정으로 바꾼다.** 답해야 할 질문은 하나다 — `anchor_buy_share`가 0.50 근방인가
0.63 근방인가(전자면 발견 A 라이브 확정 → Phase 3 착수, 후자면 진단 오류 → 취소).

- **대조는 두 쌍**: `legacy(buy_vol) vs anchor`(지금 라이브가 얼마나 틀렸나 — 커야
  정상) / `shadow(buy_vol_flag) vs anchor`(Phase 3 수정안이 정답지와 맞나 — 0에
  수렴해야 함). 후자가 크면 체결구분 분류 자체의 한계이므로 Phase 3을 **앵커 직접
  사용**으로 재설계해야 한다 — 리포트가 그 경고를 자동으로 낸다.
- **크기 축을 순/절대로 분리**: `|Σd|/Σvol`은 봉끼리 상쇄돼 오전 +100/오후 −100이
  0으로 보인다(QDQ §1.3 `anchor_diff` 경고의 집계판). **판정은 `Σ|d|/Σvol`로 한다.**
  둘이 벌어지면 그 자체가 "오차가 방향을 바꿔가며 났다"는 정보.
- **항등식 3종**: I1 앵커합==거래량 / I2 섀도합==거래량(잔차 양수=미분류 정상,
  **음수=이중 집계 버그**) / I3 legacy합==거래량. I1이 깨지면 비중 판정보다 **먼저**
  `⚠ 앵커 신뢰 불가`를 낸다(못 믿을 정답지로 판정하지 않는다).
- **게이팅 없음** — 종료코드 항상 0. CB②·CB③-P4·FP-CRITICAL이 계측이 덜 익은 상태에서
  차단부터 켰다가 상시 발동으로 꺼진 전례(CLAUDE.md §2), 네 번째를 만들지 않는다.
- 출력은 407차 규약(PC폴더·날짜본·stem별 FIFO 4주), EOD 주간 체인에 재학습 **앞**
  배치(410차 규약). `campaign_report_paths._GENERATOR_HINT`에 계열 등록.

**실 DB 첫 실행에서 오진 유발 요소를 하나 잡아 고쳤다.** I3 위반 15봉(2026-08-04)이
검출됐는데 초판 문구가 "수집기 회귀"를 의심하게 만들었다. 실제로는 452차 Phase 1이
이미 규명한 **구 복구 경로 파괴 봉**(발견 C, 전체 55봉)이다. `bar_recovered IS NULL`
(=452차 이전 생성)로 시대를 갈라 "알려진 사안 / 진짜 새 결함"을 분리 출력하도록 수정.

검증: `tests/test_453_cvd_anchor_report.py` 48체크 통과(판정 5분기 + anchor_diff
순/절대 + 항등식 3종 + 산출물·FIFO stem 분리 + EOD 체인 등록 순서). 전체 스위트
**176 passed**. 현재 실 DB는 앵커 계측 **0행**(Phase 0 배포가 2026-08-09 일요일)이라
리포트가 `⏳ 계측 미개시` 판정 — 이게 정상이며, 0%를 계산해 통과시키지 않는 것이
이 리포트의 두 번째 설계 원칙이다.
...
개선안을 제안" → 이어서 "A(정리) → C(장중 프로브는 NEXT_TODO 기록) 순으로 집행 +
재발 방지 D 구현". 사용자 제기 근거는 2026-08-07 raw_features 200분 + DATA.log 393회 실측
(`Dscbo1.CpSvr8111`이 foreign=-650,762만 채우고 개인·기관은 항상 +0).
**성격**: 피처 폐기 3종 + 수집 계층 오라벨 정정 + 상시 감시 도구 신설.

### 1. 사용자 관찰은 맞지만 원인은 "반환값 0"이 아니다 — 필드가 없다

`Dscbo1.CpSvr8111`(프로그램매매 종합매매현황)의 헤더 레이아웃은 idx0~55 전체가
**차익/비차익 × 위탁/자기 × 수량/금액** 축이다. 투자자별(개인·기관·외인) 필드가
이 TR에 **존재하지 않는다** — KRX가 장중 배포하는 프로그램매매 통계의 원래 구조다.

즉 개인·기관이 `+0`을 *반환*한 게 아니라, **초기값 0이 한 번도 갱신되지 않았다.**

3단 연쇄였다:
1. **원천**: TR에 투자자별 축 없음.
2. **커넥터 오라벨**: `api_connector.py:1537`이 `nets = {"foreign": arb + nonarb}` —
   전체 프로그램 순매수를 "외국인"으로 이름 붙였다. individual/institution 키는 생성조차
   안 됐다.
3. **소비 계층 무언 폴백**: `investor_data.py`의
   `_to_int(nets.get(key, self._program_investor.get(key, 0)))`가 없는 키를 초기 0으로
   조용히 유지하면서 **`quality_investor_program_supported = 1`을 함께 보고**했다.

바깥에서 보면 **"수집 성공 + 값 0"** 이라 조용한 장세와 구분되지 않는다. 그래서 2개월 넘게
아무도 못 알아챘다.

**스키마의 유래**: 키움 시절 `opt50008`이 투자자별을 준다는 **미검증 가정**
(`collection/kiwoom/investor_data.py:228` "행이 투자자별 순서라고 **가정**")으로 만든
피처였고, Cybos 전환 때 **스키마만 살아남고 원천이 사라졌다.**

### 2. 신규 발견 아님 — 진단은 있었고 집행이 안 됐다

`docs/미륵이고도화3/Phase0_검증결과_2026-07-27.md` §2-4가 이미 같은 결론("복구 불가,
폐기예정")을 냈고 `scripts/feature_health_report.py:KNOWN`에도 등재돼 있었다.
**폐기가 집행되지 않아** `get_features()`가 매분 계속 emit했을 뿐이다.
§2-4가 함께 지적한 `program_foreign_net_krw` 오라벨 정정도 미집행 상태였다.

### 3. 결정 — 3종 폐기 (A)

| 피처 | 실제 정체 | 처분 |
|---|---|---|
| `program_individual_net_krw` | 원천에 없음 → 상수 0 | 삭제 |
| `program_institution_net_krw` | 원천에 없음 → 상수 0 | 삭제 |
| `program_foreign_net_krw` | **값은 있었다.** 단 외국인이 아니라 전체 프로그램 순매수 = `program_arb_net` + `program_non_arb_net`의 정확한 합 | 삭제(오라벨 + 100% 중복) |

3종 모두 `horizon_feature_sets.json:excluded_from_all_horizons` 등재 상태라 **학습·추론
영향은 이미 0**이었고, 모델·스케일러 마이그레이션 부담 없이 정리 가능한 시점이었다.

- `api_connector.request_program_investor()`: `nets`를 **빈 dict**로 반환(원천이 안 주는
  키를 만들지 않는다). 합계는 `raw["total_net"]`로 분리.
- `investor_data`: `_program_investor` dict 제거 → `_program_total`(이름이 "투자자별이
  아님"을 못 박는다). 패널 키도 `program_total_net_krw` 하나로, `get_stats()`의
  `prog_fi_krw` → `prog_total_krw`.
- `scripts/backfill_features.py:FEATURE_KEYS_ALL`에서 3종 제거 — 남겨두면 백필 행에만
  0.0으로 되살아나 "일부 날짜에만 있는 유령 키"가 된다.

**`excluded_from_all_horizons` 등재는 유지한다** — 과거 DB 행에는 이 키가 남아 있고,
`batch_retrainer._load_from_db`가 DB에서 학습 X를 만들므로 그 등재가 재유입 방어선이다.
...
**(a) 라이브 카나리아** — `collection/provenance.py` 신설(`ProvenanceTracker`).
폴백이 정당한 경우(조회 실패 시 **직전값 유지**)에도 "이번엔 안 왔다"와 "한 번도 온 적
없다"를 구분하지 못하는 게 문제의 핵심이므로, 원천이 채운 키의 합집합을 누적해 후자만
경보한다. 양방향이다:
- `maybe_warn()` — 기대 키가 N회 응답 동안 한 번도 안 오면 **WARNING 1회**
  (`supported=False`면 침묵 — 그건 유령이 아니라 정직한 수집 실패다).
- `notice_new()` — 없던 키가 오기 시작하면 **INFO**. TR을 교체해 투자자별이 실제로
  들어오는 순간을 놓치지 않기 위한 것으로, 프로그램 경로에는 이쪽을 붙였다.

**(b) 오프라인 그물** — `scripts/feature_health_report.py`에 **PHANTOM 판정** 추가.
등급(DEAD/CRITICAL)만으로는 "수집 실패라 0"과 "수집 성공이라면서 0"을 못 가른다.
진짜 신호는 **모순**이다: *가용성 게이트가 켜져 있는데 값이 상수.* 게이트가 꺼진 채 0인
것은 정직한 미수집이라 경보하지 않는다(예: 2026-08-04 `opt_available=0` + `opt_pcr_*=0`,
450차 건).
함께 **KNOWN 부패 점검** 추가 — 해소·폐기된 항목이 KNOWN에 남으면 앞으로의 진짜 이상을
조용히 삼키므로 정리 시점을 알린다.

**검증 중 검출기 자체의 결함 2건을 잡아 고쳤다** (그대로 뒀으면 오검출 도구가 됐다):
1. **행 정렬 붕괴** — 게이트 비중을 전체 행에서 재고 있었다. `macro_vix_abs`는 7,533행 중
   **710행(백필 전용)** 에만 존재하는데 게이트를 7,533행 평균(87%)과 비교해 가짜 모순이
   됐다. → 게이트는 **그 피처가 존재하는 행에서만** 재도록 `collect()`를 행 단위로 재작성.
2. **옵션 게이트 혼동** — `opt_gex_sign`은 OptionChainSnapshot 출신인데 PCRStore의
   `opt_available`에 매핑돼 있었다. → `opt_pcr_*`만 `opt_available`, 나머지 `opt_*`는
   `opt_chain_available`.

**실측 결과(20일/60일)**: 오검출 2건이 위 수정으로 사라지고, **신규 진짜 후보 1건
`opt_gex_sign`** 만 남았다(`opt_chain_available=99%` 켜짐인데 3개 값 중 하나가 97.2%).
20일 창에서 program_* 2종이 유령으로 잡히고 60일 창에서는 안 잡히는 것도 정합적이다 —
07-14 이전에는 `include_program=False`라 게이트가 정직하게 꺼져 있었다.

### 5. 하지 않은 것

- **C(투자자별 원천 탐색)는 장중 프로브가 필요해 미집행** — `NEXT_TODO.md` 451차에 등록.
  `scripts/probe_cybos_program_trade.py`에 **공식 API 목록 기반** 후보
  (`CpSvr7210d`/`7210T`/`7225`/`8116`)를 추가하고 헤더·행·열 샘플 폭을 넓혀
  (40×10×10 → 64×20×20, 8111의 idx55가 잘리고 있었다) 한 번 실행하면 닫히게 해뒀다.
  기존 `check_cybos_investor_candidates.py`의 `CpTd6198~6200`은 **CpTrade(주문)
  네임스페이스의 번호 인접 추측**이라 시장 전체 통계 용도에 맞지 않는다 — 후보로 쓰지 말 것.
- **`opt_gex_sign` 조사** — 검출만 하고 원인은 안 봤다. NEXT_TODO 등록.
- **키움 모듈 정리** — 현재 브로커가 아니라 손대지 않고 경고 주석만 남겼다.
- **`program_arb_net`/`program_non_arb_net`의 단위 라벨 불일치** — `feature_builder.py:531`의
  `_INV_LOG_COLS` 주석은 이 둘을 "계약수"라 부르지만 실제로는 **금액**이다(8111 idx19/37).
  로그압축 `sign*log1p(|x|/1000)` 자체는 결과 스케일이 합리적(−650,762 → −6.48)이라
  **이번에 건드리지 않았다** — 산식을 바꾸면 train/serve skew가 생긴다(317차 원칙).
  주석-실제 불일치로만 등록.

**Why**: 이 사고의 본질은 "값이 0"이 아니라 **"수집 계층이 자기가 모르는 것을 아는 척했다"** 는
것이다. 값 하나만 보면 조용한 장세와 구분되지 않으므로, 값이 아니라 **원천이 그 필드를
응답에 담았는지**를 따로 기록해야 구분이 생긴다. 라이브 카나리아와 오프라인 그물을 둘 다
둔 이유는 전자는 당일에 빠르되 배선한 곳만 보고, 후자는 하루 늦되 전 피처를 훑기 때문이다.

**How to apply**: 수집 계층에서 `.get(key, 기본값)`으로 스키마를 완성하는 코드를 보면,
그 키를 **원천이 실제로 채운 적이 있는지** 먼저 확인할 것. 폴백이 정당한 경우(직전값
유지)에도 `ProvenanceTracker`를 붙여 "한 번도 온 적 없음"을 드러낼 것. 그리고
`quality_*_supported` 류 플래그를 세울 때는 **그 플래그가 하위 피처 전부에 대해 참인지**
확인할 것 — 일부만 참인데 전체를 1로 보고하면 나머지가 유령이 된다.

**검증**: `py_compile` py37_32·py310_64 통과. COM 없이 FakeAPI로 `CybosInvestorData`
전체 경로 스모크(폐기 키 미잔존·차익/비차익 정상·provenance seen/never_provided·
reset 후 이력 유지) + `ProvenanceTracker` 4케이스(경보 1회성·`supported=False` 침묵·
새 키 INFO·비iterable 예외안전) 통과. `feature_health_report.py`는 실제 raw_data.db
20일·60일 두 창에서 실행 확인. **라이브 미검증** — 다음 장중에 `raw_features`에서 3종이
사라지고 `[Provenance]` 경보가 나지 않는지 확인 필요.

**관련**: Phase 0 §2-4(원 진단), 450차(같은 "임계값·스키마는 남고 원천만 바뀐" 드리프트
계열), 318차 hurst_ready(설계 전제와 운영 패턴 불일치 계열), 417차(단위 오독 계열).

### 451차 후속1 — 8111 헤더 실측 대사: B안(미사용 54필드)의 근거를 확정하고 idx17 이상을 발견

사용자 지시("C가 음성이면 대안으로 차익/비차익 데이터 수집·활용을 검토하도록 TODO에 기록").
기록만 하려다 **`[CybosProbe][RAW]` 세션 1회 덤프가 이미 로그에 남고 있다**는 것을 확인해
(`_probe_investor_tr`, `api_connector.py:1335`) 오프라인 산술 대사까지 마쳤다 —
`logs/20260807_PROBE.log`에 2026-08-07 8111 헤더 **56필드 전량**이 있었다.

**확정된 것**: 문서 레이아웃이 실측과 맞는다. 차익·비차익 각각의 **gross 매도/매수**와
**위탁/자기 분해**가 전부 실재하고 합계 대사가 통과한다(비차익 위탁금액 `idx35`=135,080 =
433,693−298,613; 전체 위탁금액 `idx53`=135,010 = 438,595−303,585). 현행 수집이 옳다는 것도
같이 확인됐다 — `idx19`+`idx37` = 109,980이 TR 자신의 `idx55`(전체 순매수 합계금액)와 일치.
즉 **56필드 중 2개만 쓰고 54개를 버리고 있다.**

**발견된 이상**: `idx17`(차익 순매수 위탁금액)만 대사 실패 — 산술상 4,902−4,972 = −70이어야
하는데 실측 −24,518로 `idx19`(합계금액)와 동일하다. 같은 자리의 비차익·전체는 정확히
맞으므로 차익 블록 한정 문제다. 문서 오류인지 당일 데이터 특성인지 미상.
**여러 날짜 덤프 대사 전에는 쓰지 말 것.**

**Why**: 이건 "여분 필드 줍기"가 아니라 **450차와 같은 결함의 피처판**이다. 순매수는
매수·매도가 상쇄된 잔차라 스케일이 100배 요동하는데(450차가 PCR에서 실측), gross는 그렇지
않다. 지금 우리는 그 잔차 2개만 쓰고 안정적인 gross 축을 통째로 버리고 있다.
그래서 B안은 C(투자자별 원천 탐색)가 음성일 때의 **대안**일 뿐 아니라 **C와 독립적으로도
유효**하다.

**How to apply**: Cybos TR을 새로 쓸 때 `[CybosProbe][RAW]` 덤프를 먼저 뒤질 것 — 세션당
1회 자동으로 남고 있어서, 장중 재프로브 없이 오프라인 산술 대사로 필드 매핑을 검증할 수
있다. 매핑을 문서만 믿고 쓰지 말 것: 이번에 문서가 맞는 자리(idx35·idx53)와 안 맞는
자리(idx17)가 같은 TR 안에 공존했다.

**검증**: 오프라인 대사만. 코드 변경 없음(TODO 등록뿐). idx17은 미해결로 등록.

### 451차 후속2 — HTS 화면 8221은 실재한다. 그러나 공식 API에 대응 객체가 없다

**지시**: 사용자가 Creon HTS **화면 8221「프로그램매매 투자자별 종합」** 스크린샷 제시
(코스피/코스닥 × 차익/비차익/전체 × 매도/매수/순매수 × 투자자 12분류, 기준일 2026/08/07,
단위 백만원) + "사이보스 깃허브·커뮤니티·`docs/CyBos ref`에서 수집 방법 조사·적용 검토".

**후속1의 TODO가 걸어둔 "가장 싼 선행 확인"이 양성으로 나왔다** — "HTS에 그 화면이
있는가"에 사용자가 스크린샷으로 답했다. **데이터는 존재한다.** 남은 문제는 경로다.

**조사 3중 확인 — 공식 Plus API에 프로그램 × 투자자 교차 객체는 없다.**

| 경로 | 결과 |
|---|---|
| `docs/CyBos ref` 로컬 문서(백업본 포함, 내용 동일) | 8221 없음 |
| 이 PC 레지스트리 등록 ProgID 전수(289개) | **`CpSvr8221` 미등록.** 프로그램 계열은 8111/8111S/8111KS/**8112**/**8113**/8114/8116/8119/8119S/8119Day뿐 |
| 공식 문서 `cybosplus.github.io` | `8111`·`PgAtime8112`·`8116` 전부 **차익/비차익 축만**. 투자자 계열 `7212`·`7221`·`7222`·`7224`는 **투자자 축만**, 프로그램 구분 입력 없음 |

즉 **451차 본편의 진단("원천에 투자자별 분해가 없다")은 API 범위에서 재확인됐다.**
바뀐 것은 "데이터 자체가 없다"가 아니라 **"HTS에는 있는데 Plus API에 노출되지 않는다"** 는
것이다 — 폐기 결정은 그대로 유효하다.

**다만 종결하지 않았다 — 미문서 등록 객체가 있다.** HTS 화면이 데이터를 받는 이상 그 뒤에
COM 객체가 존재한다. 레지스트리에는 공식 문서에 없는 객체가 다수 등록돼 있다
(`CpSysDib.CpSvr8241`·`8113`·`2221`·`7210d`·`7210T`·`7236`·`7238`·`7240`,
`Dscbo1.CpSvr8081~8083` 등). 이들을 `probe_cybos_program_trade.py`의 후보로 교체 편입했다.
후속1이 넣었던 후보 중 `8116`은 문서로 차익/비차익 전용이 확인돼 제외했다.

**도구 신설**: `probe_cybos_program_trade.py --list-registered [--filter 정규식]`.
레지스트리에서 등록 ProgID를 덤프한다 — **COM 연결도 장중도 32-bit도 불필요**하다.
이번 결론(8221 미등록)이 이걸로 나왔다. 후보를 번호 인접으로 추측해 장중에 찍어보던
방식(`check_cybos_investor_candidates.py`의 CpTd6198~6200)을 대체한다.
⚠ **등록됨 ≠ 사용 가능** — 미문서 객체는 BlockRequest 미지원이거나 권한이 없을 수 있다
(8111S/8111KS가 실제로 그랬다). 등록 조회는 후보를 **좁히는** 용도다.

**부수 수확 — `Dscbo1.CpSvr7225`「시장매매흐름분석」(문서화·지원·등록 확인).**
출력: 시간 / 장내외국인 / 장내기관 / 코스닥외국인 / 코스닥기관 / 프로그램차익 /
프로그램비차익 / **선물외국인** / **선물기관** / **옵션외국인** / **옵션기관**.
`CybosInvestorData._option_flow_supported`가 지금 `mapping pending`으로 남아 있는데
**이 객체가 옵션 외국인·기관을 직접 준다.** 게다가 선물 투자자 + 프로그램을 한 번의
호출로 준다(현재 7221 + 8111 두 번). NEXT_TODO에 별도 기회로 등록.
⚠ 기존 7221 경로를 갈아엎지 말 것 — 병행 기록부터(317차 train/serve skew 원칙).

**미해결 2건 — 이어 붙이기 전에 반드시 해소할 것**
1. **스케일 37배 불일치**: 화면 8221 코스피 비차익 매도 합 ≈11.1조(외국인 10.66조가
   대부분)인데 같은 날 `CpSvr8111` 비차익 매도는 3,001억. 스크린샷 판독 오류 /
   `통합(KRX+NXT)` 범위 / 단위 중 무엇인지 미상.
2. **8221이 장중 갱신되는지 미확인**: 화면에 `기준일` + `조회` 버튼이 있어 일별 마감
   통계일 가능성이 있다. 그렇다면 교차 격자를 얻어도 1분봉 피처로는 못 쓴다.
   → 다음 장중에 화면을 **두 번 조회해 숫자가 변하는지** 보는 게 가장 싼 판정이다.
     안 변하면 프로브도 불필요하다.

**후속1 재검증 결과(같이 수행)**: `[CybosProbe][RAW]` 8111 덤프를 **7거래일**
(07-30~08-07)로 확대 대사. `idx19 + idx37 = idx55`가 **7일 전부 일치** — 후속1의
레이아웃 결론과 현행 수집의 정합성이 재확인됐다. 단 **`idx1`(시간)이 7일 모두 `0`** 이라
이 필드는 의미 있는 시각이 아니거나 첫 호출 시점 특성이다 — 시계열 용도로 쓰지 말 것.

**Why**: "HTS에 보이니 API에도 있겠지"가 이 조사의 함정이었다. Plus API는 HTS 화면의
부분집합이며, 화면 번호와 ProgID 번호는 **대응하지 않는다**(화면 8221의 탭들이
8111·8112·8116·8119 등 서로 다른 객체를 쓴다). 반대로 "공식 문서에 없으니 없다"도
틀린다 — 레지스트리에는 문서보다 많은 객체가 등록돼 있다. 그래서 **문서·레지스트리·
실프로브 세 층을 따로 봐야** 한다.

**How to apply**: Cybos에서 새 데이터를 찾을 때 순서는 ① 공식 문서(있으면 지원 보장)
→ ② `--list-registered`(장외·무료로 존재 여부 확정) → ③ 장중 프로브(사용 가능성)다.
②를 건너뛰면 존재하지 않는 ProgID를 장중에 찍어보게 된다.

**검증**: 문서조사 + 레지스트리 전수조회 + 7일치 로그 재대사. `py_compile` py37_32 통과,
`--list-registered --filter` 실행 확인(44개 반환). **라이브 프로브 미실행** — 장중 필요.

### 451차 후속3 — 공식 CHM 명세 6종 확보: 정정 3건 + 무비용 개선 후보 9건

**지시**: 사용자가 공식 CHM 도움말 전사본 6종(`CpSvr7225`·`CpSvr8111`·`CpSvr8111S`·
`CpSvr8116`·`PgaTime8112`·`CpSvrNew7212`)을 제공하며 "정리해서 관련 데이터 수집 검토".
**산출물**: `docs/CyBos ref/CYBOS_프로그램매매_투자자별_TR_명세.md` 신설(명세 + 수집 대조표
+ 실측 결과 + 우선순위). 지금까지 github 재요약·자료실 seq 번호에 의존하던 것을 1차 자료로 대체.

**후속2의 결론은 유지된다** — 6종 어디에도 **프로그램 × 투자자 교차는 없다.**
`CpSvr7225`가 프로그램 축과 투자자 축을 **나란히** 두지만 교차가 아니다. 폐기 결정 불변.

#### 정정 1 — "37배 불일치"는 착시였다 (후속1·2 기록 무효)

`[CybosProbe][RAW]` 덤프는 **세션당 1회**만 남으므로 전부 **09:02 스냅샷**이다.
그런데 8111은 **일중 누계**다 — 08-07 비차익 `09:02 +134,498 → 15:34 −738,388`.
일일 총계로 다시 대조하면 **HTS 8221 코스피 비차익 순매수 합 −758,233 vs 8111 마감
−738,388 — 2.7% 이내 일치**다. 두 소스는 사실상 같은 데이터였다.
⚠ **차익만 여전히 불일치**(8221 +184,475 vs 8111 +87,626, 약 2.1배) — 미해결로 유지.

#### 정정 2 — `idx17`은 서버측 필드 결함으로 확정 (후속1의 "미상" 해소)

공식 명세는 `idx17 = 차익순매수 위탁체결 금액`으로 정상인데, **7거래일 전부**
`idx17 == idx19`(합계금액)를 반환한다. 즉 문서 오류가 아니라 **데이터 결함**이다.
비차익(`idx35`)·전체(`idx53`)는 7일 모두 정확히 대사된다 — 차익 블록만의 문제.
✅ **우회 확정**: `idx19 − idx18` = `idx11 − idx5` (두 식이 7일 실측에서 일치).
→ B안 진행에 걸림돌이 아니다.

#### 정정 3 — 450차 TODO의 "gross PCR 오프라인 검증 불가"는 틀렸다

450차가 "로그에 gross가 기록된 적이 없어 오프라인 검증 불가"로 라이브 프로브를 선행조건에
걸어놨는데, **`_probe_investor_tr`의 RAW 덤프가 7221 5행×15열을 40거래일치 이미 남기고
있었다.** 콜·외인 실측: **gross 매도 933~5,472 / 매수 529~5,372 vs 현재 쓰는 net −13~−1,469** —
`PCR_MIN_CALL_ABS=1000`이 net에서 상시 탈락하던 이유가 수치로 확인된다(450차 진단과 정합).
→ **라이브 프로브 없이 지금 40일 오프라인 검증 가능.** 단 하루 1건(09:02)이라 일중 분포는
못 보므로 가드 임계 재설정에는 병행 기록이 여전히 필요하다.

#### 핵심 발견 — 두 TR이 같은 병을 앓는다: "순매수만 쓰고 gross를 버린다"

| TR | 제공 | 실사용 | 버림 |
|---|---|---|---|
| `CpSvr8111` | 56필드 | **2** (idx19·idx37, 둘 다 순매수) | **54** — gross·위탁/자기·수량 축 전량 |
| `CpSvrNew7221` | 5투자자군 × 3 = **15열** × 5행 | **5** (순매수만) | gross 매도·매수 전량 + **투자자군 2개 통째로**(열9~14) |

순매수는 매수·매도가 상쇄된 **잔차**라 스케일이 요동한다 — 450차가 PCR에서 실측한
바로 그 구조다. 이 문제는 PCR 하나가 아니라 **수급 수집 계층 전반의 패턴**이었다.

#### 최우선 후보(무비용) — 8111 누계의 **차분(Δ)**

지금 저장하는 건 하루 종일 표류하는 **누계 레벨**(강한 자기상관·비정상)이고, 1분봉 예측이
필요로 하는 **흐름**이 아니다. `_fetch_investor_data`가 이미 60초마다 호출하므로 Δ는 공짜다.
⚠ 기존 피처를 바꾸지 말고 **추가**할 것(317차). 재기동 첫 틱은 **Δ 미정의로 표시**할 것 —
451차에서 폐기한 "모르는데 0으로 채우기"를 반복하면 안 된다.

**부수 확인**: `CpSvr8111S`가 실시간 구독 전용임이 공식 문서로 **확정**(260704 P2의 "추정"
딱지 제거). `CpSvr8116`으로 프로그램 수급 **일 단위 백필 가능** — 신규 피처의 과거 표본
부재 한계를 일부 완화한다. `CpSvrNew7212` 입력코드 **6=선물 만기일 / 7=만기 익일**은
만기 전후 투자자 포지션을 직접 조회할 수 있어 별도 값어치가 있다(현재는 달력 플래그만 있음).

**Why**: 이번 자료가 바꾼 것은 결론이 아니라 **근거의 등급**이다. 폐기·미교차 결론은
그대로지만, 지금까지 "추정"·"미상"으로 남아 있던 항목 3건이 확정됐고 그 과정에서 이전
기록의 오류 2건(37배 착시, gross 검증 불가)이 드러났다. 1차 자료를 확보하면 **기존 기록을
다시 읽는 것**이 새 조사만큼 중요하다.

**How to apply**: 수집 코드를 바꾸기 전에 위 문서 §3 대조표("제공 필드 수 vs 실제 사용")를
먼저 갱신할 것. 451차 사고의 절반은 "이 TR이 무엇을 주는지 적힌 곳이 없어서" 생겼다.
그리고 **누계인지 흐름인지**를 항상 확인할 것 — 8111은 누계인데 피처명(`..._net`)은
그것을 드러내지 않는다.

**검증**: 7거래일 8111 헤더 대사(idx17 결함 7/7 재현, idx35·idx53 7/7 정상), 40거래일 7221
gross 스케일 실측, 08-07 SYSTEM 로그 786회 fetch로 일중 누계 성격 확인(09:02↔15:34 대조).
**코드 변경 없음** — 문서·TODO·로그 기록만.

### 451차 Phase 1-1 — `raw_program_trade` 원천 보존 계층 배포 (추가 COM 호출 0)

**지시**: 구현계획서 Phase 1-1 "꼼꼼하게 구현".
**성격**: **신규 테이블 + 라이브 수집 경로 1줄 추가.** 피처 경로·산식·이름 무변경.

#### 무엇을 했나

`CpSvr8111` 응답 **56필드 전량**을 `raw_data.db:raw_program_trade`에 매분 보존한다.

```sql
raw_program_trade(ts TEXT, market TEXT, fields TEXT(JSON), created_at, PRIMARY KEY(ts, market))
```

| 계층 | 변경 |
|---|---|
| `utils/db_utils.py` | 테이블 DDL(`init_raw_data_db`) + `save_program_trade_raw()` |
| `collection/cybos/api_connector.py` | `request_program_investor()`가 `"fields"`로 56필드 통과 |
| `collection/cybos/investor_data.py` | `_program_fields` 보관 + `get_program_raw_fields()` |
| `main.py` | `_save_program_trade_raw()` 신설, `_fetch_investor_data`에서 1회 호출 |

#### 왜 무비용인가 — 이 항목의 근거 전부

`_probe_investor_tr`는 **이미 매 호출 `GetHeaderValue(0..63)`을 전부 읽는다**
(`api_connector.py:1308`). 56필드는 12개월째 메모리에 올라왔다가 `idx19`·`idx37` 2개만
남기고 버려져 왔다. **못 받은 게 아니라 저장하지 않았을 뿐**이므로 추가 COM 호출이 0이다.
늘어나는 비용은 60초당 `INSERT` 1회(≈1ms)뿐 — `_fetch_investor_data`의 500ms 예산 대비 무시 가능.

#### 설계에서 지킨 4가지 — 전부 451차가 만든 교훈이다

1. **원천은 원천 테이블에.** `raw_features`는 피처 테이블이다. 원천 TR 응답을 섞으면
   스케일러·모델·피처 건강도 리포트가 전부 영향을 받는다.
2. **빈 응답은 저장하지 않는다.** `save_program_trade_raw`가 빈 dict를 거부한다 —
   **"행이 없는 것"과 "값이 0인 것"은 다르다.** 빈 JSON을 남기면 나중에 읽는 사람이
   "그 시각엔 프로그램매매가 0이었다"로 오독한다(451차 유령 피처와 정확히 같은 함정).
3. **조회 실패 시 직전값을 남기지 않는다.** `_program_fields`는 `supported=True`일 때만
   갱신하고 실패·`include_program=False`·`reset_daily`에서 비운다. 안 그러면 "현재 시각 +
   옛 값"이 저장돼 조용히 오염된다.
4. **가공하지 않는다.** `idx17`(서버측 결함, `idx19` 중복 반환)도 **받은 그대로** 저장한다.
   보정은 읽는 쪽에서 `idx19 − idx18`로 한다 — 저장 시점에 고치면 나중에 결함이 고쳐졌는지
   확인할 방법이 사라진다.

#### JSON을 택한 이유

`raw_features.features`와 같은 관례다. 56개를 컬럼으로 펼치면 TR 필드가 바뀔 때마다 스키마
마이그레이션이 필요하고, **그때 담지 못한 필드는 또 영영 사라진다** — 이번에 고치려는 그 문제다.

#### 검증

COM 없이 전 구간을 도는 통합 테스트 **27/27 통과**(2026-08-07 실측 헤더 사용):
테이블 생성·재실행 멱등 / 56필드 통과·빈칸 제외·문자열키+정수값 / `raw.total_net`이 TR 자신의
`idx55`와 일치 / 성공 시만 보관·실패 시 비움·`include_program=False` 비움·`reset_daily` 비움 /
빈 dict 거부·중복행 없음·market 분리 / 왕복 무결성·`idx17` 결함 원형 보존·우회식 일치(−70).
실 DB 적용 완료 — 8번째 테이블 생성, **기존 7개 테이블 행수 변화 없음**, 488MB 유지.

**실측 용량 61.4MB/년**(1행 664B × 370행/일 × 250일). 계획서 추정 55MB보다 약간 크다.

🔴 **라이브 미검증** — 다음 거래일에 `raw_program_trade` 행 적재와
`[ProgramRaw] ... 필드 56개` INFO(세션 1회)를 확인할 것(`NEXT_TODO` 0-1 ④).

**Why**: 이 계층의 값어치는 오늘이 아니라 나중에 나온다. 지금 gross·위탁/자기·차분 파생을
못 만드는 이유가 정확히 "과거에 원시를 안 남겨서"이고, 보존을 시작하는 순간 **파생은 언제든
오프라인에서, 과거 구간까지 소급해** 만들 수 있게 된다. 반대는 불가능하다.

**How to apply**: 수집 계층에서 원천 응답의 일부만 쓰기로 결정할 때, **버리는 나머지를 저장할
비용이 얼마인지 먼저 따질 것.** 이번 경우 그 비용은 0이었는데도 12개월을 버렸다.

**관련**: 451차 본편(유령 피처), 후속1(idx17), 후속3(공식 명세·누계 성격), 후속4(열 절단),
구현계획서 §4.

---

### 451차 Phase 0 실행 — gross PCR 오프라인 검증 통과 + opt_gex_sign 종결

**지시**: 구현계획서(`docs/미륵이고도화3/수급수집_원천보존_구현계획_2026-08-09.md`) **Phase 0 진행**.
Phase 0은 오프라인·주석뿐이라 라이브 경로 변경이 없다.

#### 0-2. `_INV_LOG_COLS` 주석 정정 (완료)

"계약수 단위"라는 서술을 정정했다. 이 집합은 **단위가 섞여 있다** — 7221 계열은 계약수/금액
혼재(입력코드 `ord('1')`=옵션금액/선물계약), `program_arb_net`·`program_non_arb_net`은
**금액(백만원)** 이며 게다가 **일중 누계**다. 산식은 그대로 뒀다(317차).

#### 0-3. gross PCR 오프라인 검증 — 🟢 **가설 지지, 강함**

신규 `scripts/analyze_gross_pcr_offline.py`. 40거래일(2026-06-12~08-07) `[CybosProbe][RAW]`
덤프의 7221 옵션콜/풋 행 × 외인 열3(매도)·열4(매수)·열5(순매수).

| 지표 | net (현행) | **gross** |
|---|---|---|
| 외인 콜 스케일 | 13 ~ 1,853 | **740 ~ 10,844** |
| 구 가드 `\|call\|≥1000` 통과율 | **15.0%** | — |
| 현행 가드 `max≥100` 통과율 | 97.5% | **100.0%** |
| PCR 중앙값 (1.0=중립) | **0.705** | **1.038** |
| **PCR 캡(5.0) 고착률** | **22.5%** | **0.0%** |

**net의 두 병리가 gross에서 사실상 소멸한다.** 캡 고착 22.5% → **0.0%**, 중앙값 편향
0.705 → 1.038(거의 정확히 중립). 이유는 이론대로다 — 순매수는 상쇄 잔차라 분모가 0 근처로
가면 비율이 폭발/붕괴하는데, gross는 그렇지 않다.
**구 가드 통과율 15.0%** 는 450차 진단("절대 임계가 원리적으로 맞을 수 없다")의 독립 재확인이다.

⚠ **캡 고착률 22.5%는 450차가 기록한 21.3%와 일치**하지만, **중앙값은 다르다**
(450차 1.59 vs 이번 0.705). 표본이 다르다 — 이번 것은 **하루 1건(09:02 스냅샷)** 40일이고
450차는 일중 표본이다. **어느 쪽도 틀렸다고 볼 근거가 없으므로 둘 다 남긴다.**
🔴 **그래서 이 결과를 가드 임계 재설정의 근거로 쓰지 말 것** — 일중 분포를 볼 수 없다.
임계 재보정은 병행 기록으로 일중 표본을 쌓은 뒤에.

#### 0-4. `opt_gex_sign` — 🟢 **수집 결함 아님으로 종결**

PHANTOM 검출기가 잡은 **유일한 신규 진짜 후보**였다. 결과: **계측은 정상이다.**

- `opt_gex_bn`(연속값)과 **부호 불일치 0건 / 6,563행**.
- `sign=0`인 59행은 **전부 `opt_chain_available=0`**(미수집) — 정상.
- `sign=-1` 구간의 bn 중앙값 **−264.3**, 최대 **−2.15**. 0 근처에서 흔들려 부호가 뒤집히는
  상황이 아니라 **확고한 음수**다 → KOSPI200 감마 노출이 구조적으로 음수인 시장 특성.

**다만 피처로서는 무가치에 가깝다** — 별도 문제로 등록:
1. **사실상 상수**: `+1`은 **2026-07-15(118행)·07-23(5행) 단 2일**에만 나왔고, 그 뒤
   **11거래일 연속 무변화**다.
2. **정보 100% 중복**: `opt_gex_bn`의 순수 부호 함수다(불일치 0건이 그 증거).
   15m `include_pending_validation`에는 **bn과 sign이 둘 다** 올라 있다.
3. 🔴 **승격 근거를 이 PC 데이터로 재현할 수 없다.** `config/constants.py:233`은
   `opt_gex_sign IC=0.035 p=0.006 (06-01~07-10 재실측)`을 근거로 `DYNAMIC_FEATURES_POOL`에
   등록했는데, **이 PC raw_features에 `opt_gex_sign`은 2026-07-14부터 존재한다**
   (05-26~07-13 전 구간 0행). 그 구간에서 IC를 잴 수 없다.
   ⚠ **단정 금지** — 같은 주석이 "292차에 dev PC(MW0602)에서 편입됐고 이 PC엔 미전파"라고
   적고 있어 **MW0602에는 더 이른 데이터가 있을 수 있다.** MW0602 대조 전까지는
   "이 PC에서 재현 불가"까지만 확정이다.

**현재 학습 영향은 0**: `data/db/shap_feature_registry.json:active_features`(97개)에 미포함이며
6개 호라이즌 `feature_names_*.pkl` 어디에도 없다(2026-08-07 재학습분 실측). `horizon_feature_sets.json`
10m `include`에 이름은 있으나 `get_available_feature_set()`이 마스터와 교집합하며 걸러낸다.

**Why**: PHANTOM 검출기의 첫 실전 결과가 "결함 아님"으로 끝난 것은 **실패가 아니라 정상
동작**이다. 이 검출기의 목적은 "가용하다고 보고하는데 값이 상수"인 **모순을 드러내는 것**이지
원인을 판정하는 게 아니다. 이번엔 원인이 시장 구조였고, 그 판정에 30분이 걸렸다 —
451차의 program_* 유령은 같은 모순을 **2개월** 방치했다.

**How to apply**: PHANTOM이 뜨면 먼저 **연속값 짝(여기선 `opt_gex_bn`)과의 정합성**을 볼 것.
부호·플래그 계열 피처는 거의 항상 연속값에서 파생되며, 둘이 정합하면 계측이 아니라 시장이다.
그리고 정합하더라도 **"연속값이 이미 피처셋에 있으면 부호는 중복"** 이라는 별개 문제가 남는다.

**검증**: `analyze_gross_pcr_offline.py` 40일 실행, `raw_features` 6,563행 전수,
`shap_feature_registry.json` + 6개 pkl 실측. **0-1(라이브 검증 3건)은 미실행** — 2026-08-09는
토요일이라 다음 거래일 필요.

---

### 451차 후속4 — `cybosplus.github.io` 접근성 확정(커버리지 74%) + RAW 덤프 열 절단 수정

**지시**: "`https://cybosplus.github.io/` 접근성을 점검하고, 모든 정보 접근이 가능하면
향후 Cybos·Creon 정보(TR 포함)는 여기를 우선 참조하도록 기억해."

#### 답: 우선 참조처로 채택하되 **"모든 정보"는 아니다 — 커버리지 74%**

| 확인 | 결과 |
|---|---|
| 개별 객체 페이지 | ✅ 입력·헤더·데이터 필드 **전량**. 정확도 높음(8111 56필드가 실측과 일치) |
| 루트·`index.html`을 WebFetch | ❌ **제목만 나온다** — 목차가 JS(`files/*.js`)에 있어서다 |
| 전체 색인 | ✅ `files/helpindexdat.js` = `["객체명","경로"]` **230항목**(기계판독). `files/tree.js`도 있음 |
| 이 PC 등록 ProgID 276개 대조 | 🔴 **문서 있음 205 (74%) / 없음 71** |
| 저장소 최신 커밋 | 🔴 **2014-07-18 — 12년 된 스냅샷** |

없는 것 예: `CpSvr8119`/`8119S`/`8119Day`(종목별 프로그램매매), `CpSvr7818`(금리지표),
`CpSvr7210d`/`7210T`, `CpSvr8113`/`8241`, `OptionGreek`, NXT/ATS 계열 전부
(`StockJpBidNxt`·`CpSvr8119SNxt` — 넥스트레이드 2025년 출범이라 2014년 문서에 있을 수 없다).
반대로 로컬 `CYBOS_투자정보_API_목록.md`에만 있는 것도 있다 — **상보관계지 포함관계가 아니다.**

**URL 규칙(검증됨)**: `/<모듈>/<객체명소문자>.htm`.
`Dscbo1`→`cpdib_rtf_1_` / `CpSysDib`→`cpsysdib_rtf_1_` / `CpUtil`→`cputil_rtf_1_` /
`CpTrade`→`cptrade_new_rtf_1_`(**파일명 뒤에 `_`**: `cptd0723_.htm`).

→ **참조 순서를 3층으로 확정**: ① 문서(무엇을 주는가) → ② `--list-registered`(존재하는가)
→ ③ 프로브·`[CybosProbe][RAW]`(정말 그런가). 문서는 `8111 idx17`을 정상이라 하지만 값은
결함이었다(후속3) — 어느 한 층도 단독으로 충분하지 않다.
기록: `docs/CyBos ref/CYBOS_프로그램매매_투자자별_TR_명세.md` 상단 + 메모리
`reference_cybos_api_docs`.

#### 부수 발견 → 수정: **RAW 덤프가 15열에서 잘리고 있었다**

공식 문서 확인 중 `CpSvrNew7221`이 `GetDataValue` **type 0~51**(투자자 12분류 ×
매도/매수/순매수) × **index 0~22**(주식·코스닥·선물·콜·풋·국채선물·통화선물·상품선물·CME…)임을
확인했다. 그런데 `_probe_investor_tr`의 열 루프가 **`range(15)` 하드코딩**이라
**40거래일 내내 앞 15열만 기록**해 왔다. 행도 `rows[:5]`로 잘라 선물·콜·풋 뒤가 통째로 없었다.
→ 후속3이 "투자자군 2개(열9~14)를 버린다"고 쓴 것은 **과소평가**였다. 실제로는 대부분이다.

**수정**: 넓게 읽는 것은 **세션당 progid별 첫 호출(=RAW 덤프를 남기는 그 호출)** 로 한정
(`field_limit = 64 if is_dump_call else 15`), 행 샘플은 `[:25]`로 확대하고 빈 값은 제거.
**상시 파싱 폭은 15 그대로** — `_fetch_investor_data`는 60초마다 메인 스레드를 점유하고
500ms 초과 시 경고가 찍히므로 라이브 비용을 늘리지 않는다. 실제 사용 열(2·5·8)은 15 안에 있다.

**Why**: 이것도 451차 본편과 같은 계열의 착시다 — **"안 보이는 것"과 "없는 것"을 구분하지
못하는 계측**. 유령 피처는 없는 필드를 0으로 채워 있는 것처럼 보이게 했고, 이번 절단은
있는 필드를 안 보이게 했다. 방향만 반대다.

**How to apply**: 진단용 덤프에 폭·개수 상한을 하드코딩할 때는 **그 상한이 관측 한계라는
사실을 로그에 함께 남길 것**(이번에 `field_limit=%d`를 덤프 줄에 추가했다). 상한이 로그에
없으면 나중에 그 데이터를 읽는 사람이 "이 TR은 15열짜리"라고 결론 내린다.

**검증**: `py_compile` py37_32·py310_64 통과. 저장소 트리·색인 실다운로드로 커버리지 계산,
URL 규칙 6건 HTTP 200/404 실측. **라이브 미검증** — 다음 장중 `[CybosProbe][RAW]`에
`field_limit=64`와 7221의 15열 초과 데이터가 찍히는지 확인 필요.

---

## 2026-08-09 (MW0601 450차 — opt_pcr_* 8종 사망 딥다이브: 가드 스케일 드리프트)

**지시**: "구조적 결함을 보이는 opt_pcr_* 8개 피처가 죽어있는 원인을 딥다이브하고
살릴 수 있는 방안을 수립·검증해서 보고." 사용자 제기 근거는 2026-08-07 raw_features
200분 + DATA.log 393회 fetch 실측(`|call_foreign| ≥ 1000`이 393회 중 3회 = 0.8%).
**성격**: 수집 경로 결함 수정. **라이브 피처 경로 변경 1건**(`collection/options/pcr_store.py`).

### 1. 사용자 진단은 맞다 — 임계값의 유래까지 확정했다

`PCR_MIN_CALL_ABS = 1000`은 **63차**(`d0f8255`, 2026-05-20)가 넣었다. 그날 로그에 원버그가
그대로 남아 있다: `09:00:16 call_foreign=+0 put_foreign=+659` → 구 코드 분모 `+1e-6` →
**PCR = 6.59×10⁸** → `opt_pcr_slope_norm=-5.87`이 20분간 반복. 이걸 막으려 넣은 값이다.

**당시엔 무해했다.** 5월 TR은 `CpSysDib.CpSvrNew7212`였고 `|call|` 스케일이 5,800~39,000이라
1000은 사실상 통과율 99%+였다(5/11~6/2 전 일자 99.2~100%).

**106차**(`eac8cf8`, 2026-06-04)가 TR을 `CpSysDib.CpSvrNew7221`로 교체하면서 스케일 근거가
무효화됐다 — 7221은 `SetInputValue(0, ord('1'))` = **옵션 "금액" 단위**이다
(`collection/cybos/api_connector.py:1372`, 주석에 명시). 그런데 가드 주석은 계속
"최소 유효 **수량**"을 말하고 있었다. **임계값은 그대로 남고 단위만 바뀐 전형적 드리프트.**

### 2. 구조 원인 — 분모가 '순매수'라 어떤 절대 임계값도 원리적으로 맞을 수 없다

순매수는 매수·매도가 상쇄된 잔차다. 방향이 균형이면 하루 종일 수백, 쏠리면 수만이 된다.
62일(2026-05-11~08-07) DATA 로그 23,427행 실측 — **일중앙값이 100배 요동**한다:

| 날짜 | `\|call\|` 중앙값 | `≥1000` 통과율 |
|---|---|---|
| 2026-06-02 | 38,240 | 99.2% |
| 2026-08-04 | 390 | **0.0%** |
| 2026-08-07 | 738 | **0.8%** (3행) |

통합 분포: p05=239 / p25=1,449 / p50=3,853 / p95=24,444. **전체 통과율 81.5%**이고
하위 일자가 통째로 0%가 된다. 사용자가 본 08-07의 3회는 DB `raw_features`에서도
`opt_available=1` 3행으로 정합 확인됐다(200분 표본에 안 잡혔을 뿐 완전한 0은 아님).

### 3. 🔴 정정 — DB "전 구간 0"의 대부분은 이 가드 탓이 아니다

**사용자 표현("사실상 항상 0")을 DB 전 구간으로 확장하면 틀린다.** raw_features 전수
88,542행 실측 결과 `opt_available` 이력은 3구간이다:

| 구간 | 상태 | 진짜 원인 |
|---|---|---|
| ~2026-06-01 | 전멸 | 7212 시대 + 피처 미배선 |
| 06-02~06-08 | **생존**(71~99%) | 정상 |
| **06-09~07-13** | 전멸 | 🔴 **가드 아님 — 백필 덮어쓰기** |
| 07-14~ | 생존(80~95%) | 정상. 단 **08-04 0% / 08-07 0.8%** ← 가드 |

**06-30이 결정적 반례다.** 로그상 그날 `|call|`은 1,791~9,373으로 **전 행이 가드를 통과**하는데
DB는 0이다. 해당 행들의 `feature_quality_score`가 전부 **0.3(백필 행 마커,
`scripts/backfill_features.py:216`)** 로 확인됐다 — 417~418차가 기록한
**"2026-07-13 MW0601 백필 병합이 라이브 수집값을 0.0으로 덮은 사고"** 의 흔적이다.

→ **가드가 실제로 죽인 날은 07-14 이후의 08-04·08-07(전면)과 07-21(44%)·07-29(36%)
같은 부분 사망일이다.** 이 구분을 하지 않으면 이번 수정의 효과를 과대평가하게 된다.

### 4. 조치 — 절대 임계 → 미로드/저활동 2단 가드

`collection/options/pcr_store.py`:

| 목적 | 구 (63차) | 신 (450차) |
|---|---|---|
| 미로드 방어 | `\|call\| < 1000` 스킵 | **`call_abs == 0` 스킵** — 63차 원버그의 실제 형태 |
| 저활동 방어 | (위에 포함) | **`max(\|call\|,\|put\|) < 100` 스킵** |
| PCR 폭주 | `PCR_MAX=4.0` 캡 | 무변경 |

상수명도 `PCR_MIN_CALL_ABS` → `PCR_MIN_ACTIVITY_ABS`로 바꿨다 — 구 이름은 "콜 수량 하한"이라
같은 오해를 다시 부른다. **다른 참조처 없음**(전 코드베이스 grep 확인).

### 5. 검증 — 62일 재생, 수정된 실코드 임포트

`logs/*_DATA.log`의 `[CybosInvestor] futures ... call_foreign=%+d put_foreign=%+d`를
파싱해 **수정된 `PCRStore`를 그대로 import**해 23,427행 재생(스크립트 복제 아님):

- **가용률 81.5% → 99.83%.** 08-04·08-07·06-30 각 **100%**, 06-05 98.9%.
- 상수 고착 해소: 08-07 unique PCR **1개 → 361개**, 08-04 318개.
- 63차 원버그 케이스(`call=0·put≠0`) **33행 전부** ①번 가드가 방어 — 재발 없음.
- 임계 후보 스윕 T ∈ {0,20,50,100}에서 가용률·캡고착·극단발화율이 **사실상 동일** →
  100은 보수적 선택이며 **민감하지 않다**(우연히 맞춘 값이 아님을 확인).

### 6. ⚠ 남은 한계 — 이번 수정으로 해결되지 않는다

1. **캡 고착 21.3%.** 풋이 콜을 압도하는 날은 PCR이 종일 `PCR_MAX=4.0`에 붙는다
   (07-30은 가용 전 분이 4.0 → unique 1). **net 기반 PCR의 본질적 한계.**
2. **중심 편향.** 가용 분 PCR 중앙값이 **1.59**라 `opt_pcr_bearish`(≥1.2) 발화율 ~57%,
   `extreme`(≥1.5) ~52%. **"극단" 플래그의 변별력이 희석돼 있다.**
3. 🔴 **근본 대안은 지금 검증할 수 없다.** 7221 열3(외인매도)·열4(외인매수)를 추가 파싱해
   **gross 기반 고전 PCR**로 가면 1·2가 함께 풀리지만, **로그에 gross가 기록된 적이 없어
   오프라인 재생이 불가능하다.** 라이브 probe로 스케일부터 확인해야 한다(NEXT_TODO 등록).
   ⚠ **임계값(1.2/1.5/0.67) 재보정을 이 수정과 묶어 하지 말 것** — 372차가 PSI에서
   이상치로 임계를 바꿀 뻔한 것과 같은 함정이다. 372차 방법론(일자단위 상관 +
   손익 이상치 분해)을 거칠 것.

**모델 반영은 별도 조치 불요** — 배치 재학습이 최근 창을 쓰므로 살아난 값이 자동 편입된다.
**백필은 건드리지 않았다** — `FEATURE_KEYS_ALL`의 `opt_*` 9종은 백필이 계산할 수단이 없어
0.0인 것이 맞고(`_BACKFILL_COMPUTED_KEYS` 미포함), 3번 사고의 재발원이 바로 그 경로다.

---

## 2026-08-08 (MW0601 449차 — 잔여 47채널 교차 종합 + 주간회의 추인 7건)

**지시**: 2026-08-08 주간회의 확정 4채널(`2457981`: [3-B]·[23-B]·[25]·[25-B])을 **제외한**
잔여 채널의 양 PC 교차 종합 + 추인 필요 채널 별도 정리. **성격**: 거버넌스 결정 기록 +
사전등록 2건. **라이브 상수 변경 0건.**
**산출물**: `docs/정기점검/금요일점검/MW0601/0808_주간회의_검토보고_MW0601_잔여채널.md`

### 0. 이번 대조가 2주 만에 처음 유효하다

MW0602 0807 검토보고 §2-1은 자기 대조를 **스스로 무효 선언**했다 — MW0601 쪽이
`20260801_pre405.md`(31채널)라 **20채널 비대칭**이었다. 448차가 스케줄러를 고쳤고
MW0601 0807 리포트(세션 수동 실행분, 52채널)가 생겨 `only` 줄이 실질적으로 비었다.
**공통 47채널 — 판정 일치 41 / 불일치 6.**

⚠ 두 리포트 모두 440차 세대지만 441~443차 수정 범위는 `exit_replay`·`sim_fidelity_gate`·
기하 3채널 이관에 국한되므로 **제외 4채널 밖에서는 판정에 영향이 없다.**

### 1. 🔴 불일치 6건 중 4건은 불일치가 아니었다 — 이번 대조의 핵심

| 채널 | 겉보기 | 규명 결과 |
|---|---|---|
| **[5]** 레짐 ATR | FAIL vs PASS | **해소 — 청산레그 2개가 전부** |
| **[6]** Hurst 게이트 | FAIL vs PASS | **역전 아님 — 누적이 0선 통과 중** |
| **[4]**·**[21]** | 갈림 | **2주 연속 = 상시.** 0802 판정보류가 옳았다 |
| **[8]** KellySkip×C | INSUFFICIENT vs FAIL | 방향 완전 일치, **표본만 1건 차이** |
| **[42]** 타점의 가치 | REJECTS vs **SUPPORTS** | 🔴 **유일한 성격 미상 정면 충돌** |

#### [5] — DB 전수 재현으로 확정

리포트 수치를 완전 재현한 뒤(n=21 / −3,130,788원, ghost 필터·`_campaign_start` 동일)
분해하니 **2026-07-09 qty=10 −3,192,244원 + 2026-07-10 qty=9 −1,835,349원** 둘이 전부였다.
**빼면 n=19 · +1,896,805원 · 평균 +99,832원** → `trend > 0` 이면서 `trend > neutral`이라
**판정이 PASS로 뒤집히고 방향도 정반대**가 된다.

🔴 **두 레그의 사이징 체제(`MAX_CONTRACTS=10`)는 431차에 이미 제거됐다.** 즉 이 FAIL이
권고하는 대상(현재 trend 배수)과 원인(폐기된 체제)이 다르다.
🔴 **이 채널은 롤오프되지 않는다** — `_campaign_start()`가 고정값 2026-07-05라 28일 창이
아니라 영구 누적이다. **MW0601 [5]는 시장과 무관하게 상시 FAIL로 붙박이고, 라이브 상수
변경 권고를 매주 재출력한다.**

#### [6] — 주간 이력 한 줄이 "최우선 감시"를 해소했다

MW0602 §2-3(c)가 *"🔴 신규 부호 역전, 가장 주의할 것"*으로 올렸으나 이력을 붙이면 다르다:

| PC | 0801 | 0807 | 주간 신규분 | 건당 |
|---|---|---|---|---|
| MW0601 | 236건 / **+37.7244** | 305건 / **+6.8843** | 69건 / −30.84pt | −0.447 |
| MW0602 | 173건 / **+37.3158** | 286건 / **−1.8072** | 113건 / −39.12pt | −0.346 |

지난주 두 PC는 **사실상 같은 값**이었고, 이번 주 신규분은 **양 PC 모두 강한 음수**다
(= 둘 다 "차단이 옳았다" 방향). 판정 임계가 정확히 0이라 누적이 0을 통과할 때
**부호 차이가 곧 판정 차이**가 된 것뿐이다(두 값 거리 8.7pt < 주간 이동폭의 1/4).
덤: 수량가중 참고치는 **양 PC 이미 일치**(−4.74 / −16.07, 둘 다 음수).

### 2. 추인 7건 — 전부 권고대로 승인

| # | 채널 | 확정 | 라이브 |
|---|---|---|---|
| R1 | [5] | **권고 미적용 확정** — `HURST_REGIME_ATR_MULT` trend 1.20 유지 | ➖ |
| R2 | [8] | **차단 보류** + 3관문 사전등록 | ➖ |
| R3 | [42] | **판정보류 등록** (MW0602 D6 전환 자체는 유지) | ➖ |
| R4 | [45] | **(c) 전면 보류 + 트리거 사전등록** | ➖ |
| R5 | [18] | **보류** — `REGIME_EXHAUSTION_GATE_ENABLED` 미전환 | ➖ |
| R6 | [4]·[21]·[6]·[16] | note 추기 3 + **[16] 신규 등록** | ➖ |
| R7 | 인프라 | D1·D5·D7 이행 확인 / **D8 [33] 재실행 승인** | ➖ |

**라이브 상수 변경 0건** — 승인 7건 중 라이브 경로를 건드리는 것은 하나도 없다.

#### R4가 왜 (c)인가 — MW0602 D2를 이행하지 않기로 했다

MW0602 0807 D2는 *"[45] EOD 1회 고정 채택"*을 승인했다(라이브 신호 경로 변경).
**보류 근거 4**: ① 데이터가 1주(6거래일)뿐 — 0801 metrics에 채널 키 자체가 없어 [6]에서
통했던 주간 이력 검증이 불가 ② **조치 트리거가 사전등록된 적이 없다**(게이지 채널,
경보값 4.0·0.10은 표시용) → 지금 배포하면 관측 후 기준 수립(313차 위반)이며 같은 회의
D3이 `atr_lock_0.75`에 정반대로 적용한 원칙 ③ 비용(`CONF_SCALE_BREAKS` 경계 추가)은
확정인데 MW0601 이득은 미측정(점프 중앙값 **0.0162** = 경보의 1/6) ④ 근본 원인
(raw conf AUC 0.489 = 정보 부재)은 이 처방으로 안 없어진다 — **불연속 빈도 관리일 뿐.**

🔴 **PC별 분기 배포 금지를 함께 등록**했다. conf 산출 경로가 갈리면 방금 복구된 PC 대조가
다시 깨진다 — [42]·[21] 불일치가 시장 차이인지 conf 경로 차이인지 구분 불가가 된다.
`replay_regime` PC별 분기와 성격이 다르다(그쪽은 오프라인 재생 경계).
→ **상정 시 선택지는 양 PC 동시 배포 / 전면 보류 둘뿐.**

**트리거(관측 전 확정)**: 양 PC 각각 conf 점프 중앙값 ≥ 0.10 **그리고** 전환 ≥ 4.0회/일을
**2주 연속**. 현재 MW0602 충족 / MW0601 점프폭 미충족 → **다음 주 리포트가 자동 판별.**

⚠ **판독 정정**: 초안이 MW0601 전환 10.67회/일을 "08-05 단일일이 끌어올린 평균"이라
적었으나, **그날을 빼도 6.6회/일로 경보(4.0) 초과**다. 갈리는 축은 횟수가 아니라
**점프 폭 하나**다(MW0601 최대 점프 0.5281은 MW0602와 같다 — 큰 점프가 **드문** 것이지
없는 게 아니다).

### 3. 🔴 R7 — MW0602의 "최우선 도입 권고"는 이미 이행돼 있었다

CLAUDE.md가 경고한 *"이미 이행된 권고를 신규 안건으로 오인"*이 또 재현될 뻔했다.

- **D1** ([47] 재생기에 TP3·트레일링 편입, "1~2세션 공수 최우선") → **이행 완료.**
  `scripts/exit_replay.py`가 4단계 트레일링(`compute_trailing_stop_tier` **라이브 함수 직접
  import**) + TP1/TP2/TP3 분할 + qty=1 보호전환 + 15:10을 전부 모델링한다. 441차가 이것으로
  [47]을 PASS시켰고 442·443차가 3채널을 이관했다.
- **D5** (py310_64 복구, "주말 작업·🔴고위험") → **448차로 해소, 진단도 정정.**
  env 파손이 아니라 **conda 활성화 없는 직접 호출**이었다 → A7의 `conda env export` 백업·
  MKL 재설치 절차는 **불필요**했다.
- **D7** (`slippage_ticks_per_side` 무변경) → **양 PC 지지** ([17] 일자 p 0.7539 동일).

### 4. 배선 — 레지스트리가 요약표에 안 보이던 구멍을 함께 막았다

`_dm()` 인라인 📌 마커는 **9개 키에만** 배선돼 있었다. 신규 6건을 그대로 두면 하단
레지스트리 섹션에만 남아, **매주 FAIL을 보는 사람이 결정을 놓친다** — 이 레지스트리가
막으려던 바로 그 사고다. `[5]`·`[8]`·`[16]`·`[18]`·`[42]`·`[45]` 요약행에 `_dm()` 배선.

⚠ **[16]은 OBSERVE 채널이라 PC 불일치가 `cmp_summary` 판정 대조로 잡히지 않는다** —
A등급 누적 −451,249 vs **+2,099,082원**으로 활성화 근거가 정반대인데 "일치"로 표시된다.
그래서 레지스트리에 남겼다.

### 5. 남은 것

- **[42]가 유일한 미규명 불일치**이고, 하필 MW0602가 연구 우선순위 전환(D6)의 **유일한
  근거**로 삼은 채널이다. ⚠ 어떻게 처리하든 **진입 축은 비어 있다** — [39]·[40] 양 PC 기각,
  `faststop_discovery` 4회 실패, [18]은 표본이 찼으나 그 결정과 충돌(R5).
- **[46]은 정지가 이미 풀려 있다** — 441차가 `no_replay_targets`에 등록했고 0807 리포트의
  SUSPENDED 표시가 낡은 것이다. MW0602 A1이 "정지된 4채널"에 넣었던 항목.
- **[8]은 다음 주 n=20 도달 유력** — 그전에 [37]식 일자단위 검정 이식이 선행돼야 한다.

**방법론 교훈**: 1부는 *"계측을 고치면 수렴하는가"*가 셋을 갈랐다. 잔여 채널에서는
**"주간 이력을 붙이면 수렴하는가"**와 **"몇 건이 그 값을 만들었는가"**가 6건 중 3건을
정리했다. **단일 주차 스냅샷 대조만으로는 셋 다 놓쳤다.**

**관련**: 448차(스케줄러 D6), 2026-08-08 주간회의 1부(기하 4채널 D1~D4, `2457981`),
MW0602 0807 검토보고 §2·§3·§4, 417차(포지션 단위 재계산), 431차(MAX_CONTRACTS 10→3)

---

## 2026-08-08 (MW0601 448차 — D6 스케줄러 BLAS 결함: 진단 정정 + 코드 수정)

**지시**: 주간회의 D6 조치. **성격**: 인프라·실행환경. **라이브 신호·주문 경로 무변경.**
0807 P0로 등록된 뒤 미조치였던 항목 — 08-01·08-07 **주간 리포트 2회 결번**의 원인.

### 🔴 기존 진단 2건을 정정한다

**정정 ① — "작업이 실패했다"가 아니다. 작업은 rc=0으로 성공했다.**
`MireukiEODRetrain` LastResult = **0x00000000**(08-07 15:45:45). 죽은 것은 그 작업이
띄운 **자식 프로세스**(`campaign_steps` → 리포트 생성기)이고, 부모가 그것을 삼켰다.
→ 스케줄러 상태만 보면 정상으로 보인다. **이것이 2주간 못 알아챈 진짜 이유다.**

**정정 ② — 내가 권했던 `os.add_dll_directory()`는 이 문제를 못 고친다(실측).**
...
MKL의 delay-load는 `LOAD_LIBRARY_SEARCH_*` 플래그 없는 평범한 `LoadLibrary`를 쓰므로
add_dll_directory 목록을 **보지 않고 PATH만** 본다. 447차 대화에서 이 방식을 D6
권고안으로 제시했으나 **실측으로 기각**됐다 — 구현 전에 재현부터 한 것이 옳았다.

### 정밀 진단 — `Library\bin` 하나가 필요충분
...
- **numpy import는 성공하고 첫 BLAS 호출(`np.dot`)에서 죽는다.** 그래서 "임포트 되니
  환경은 멀쩡"이 틀린 추론이다(MW0602 0807 §2-1이 같은 취지로 경고).
- **stdout 0바이트의 정체는 버퍼링**이다. `-u`로 돌리면 진행 상황이 보인다 —
  `numpy import OK: 1.26.4` 다음 줄에서 즉사한다.
- MKL 파일명이 base env는 `mkl_core.2.dll`, py310_64는 **`mkl_core.3.dll`**로 달라
  **그림자 충돌이 아니라 단순 부재**다(그래서 append로도 고쳐진다).
- ⚠ **MW0602가 무사한 이유**: 그 PC는 `MireukiEODRetrain` 자체가 **미등록**이라 이
  경로를 타지 않는다. 코드 결함이 아니라 **실행 경로 차이**였다.

### 조치 1 — `utils/dll_bootstrap.py` 신설 (코드로 고친다)

`sys.executable`에서 env 루트를 유도해 `Library\bin`을 **PATH 앞에** 넣는다.
**경로 하드코딩 없음** — py37_32/py310_64, 다른 PC의 다른 설치 경로에서 그대로 맞는다.

**왜 스케줄러 설정이 아니라 코드인가**: 스케줄러 Execute를 conda bat으로 바꾸는 방법도
있으나 그것은 **PC 로컬이라 git으로 공유되지 않는다** — 새 PC에서 반복되고, 수동 실행·
IDE 실행 등 다른 호출 경로는 여전히 무방비다. 코드는 저장소로 퍼진다.
⚠ **자식 프로세스도 자동 보호된다** — `os.environ` 수정을 `subprocess`가 상속한다.
   실제로 죽은 것이 그 자식이었으므로 이 성질이 핵심이다.

배선: `retrain_eod.py`(스케줄러 직접 대상) + `generate_validation_campaign_report.py`
(죽은 당사자, 단독 실행도 잦다). 후자는 **`import numpy`가 `sys.path` 설정보다 위에
있어** 부트스트랩을 끼울 자리가 없었다 — 순서를 바로잡았다.

### 조치 2 — 🔴 조용한 실패 차단 (이쪽이 더 위험했다)

`campaign_steps`가 리포트 스텝이 죽어도 **가장 최근 파일**을 "판정 리포트: …"로
안내했다. 08-07 EOD 로그가 실제로 **6일 묵은 `20260801_pre405`**를 그렇게 찍었다.
**조용한 크래시 + 낡은 파일 안내** 조합이라 알아챌 단서가 없었다.

→ 이제 (a) 그 스텝이 실패하면 **경로를 안내하지 않고 ERROR**로 알리며 0xC06D007F일 때
자가진단 명령까지 안내하고, (b) 스텝이 OK여도 **파일명 날짜가 오늘이 아니면** ERROR로
경고한다(rc=0인데 파일을 못 쓴 경우까지 걸린다).

### 검증

- py310_64 + **py37_32** 양쪽 컴파일 통과.
- **스케줄러 환경 정밀 재현**(base MKL만 PATH, conda 미활성)에서
  `python -m utils.dll_bootstrap` → `BLAS OK`, 리포트 생성 **rc=0 / 152KB**.
  같은 환경에서 수정 전에는 **0xC06D007F 즉사**였다.
- **정상 환경 무회귀** — 두 환경 산출물이 **생성 시각 한 줄 말고 완전히 동일**.
- 요약표 56채널·확정 결정 마커 정상 렌더.

### 남은 것

스케줄러 설정 자체는 **건드리지 않았다**(코드 수정만으로 해결되므로 불필요한 시스템
변경을 피했다). 다만 `retrain_eod.py`가 자식 실패를 종료코드로 전파하지 않는 구조는
그대로다 — 조치 2가 로그로는 시끄럽게 만들었으나, **작업 스케줄러 이력에는 여전히
성공으로 남는다.** 종료코드 전파는 EOD 체인 전체의 실패 정책이라 별도 안건으로 등록.

**관련**: 0807 P0(NEXT_TODO), 0808 주간회의 D6, MW0602 0807 §2-1·§2-4(BLAS 진단·
"복구가 대조 재개의 전제조건"), 432차 후속(최초 발견), 407차(리포트 경로 규약).

---

## 2026-08-08 (주간회의 확정 — 기하 A/B 4채널 D1~D4 승인)

**성격**: 거버넌스 결정 기록 + 사전등록 1건. **라이브 무관·판정식 무변경.**
**근거 문서**: `docs/정기점검/금요일점검/MW0601/0808_주간회의_검토보고_MW0601.md`

### 확정 결정

| # | 채널 | 결정 |
|---|---|---|
| **D1** | [3-B] `quantile_tp_shadow` | **`q_raw` 승격 폐기** |
| **D2** | [23-B] `tp1_geometry_shadow` | **현행 유지** + 불일치를 "거래집합 차이"로 재분류 |
| **D3** | [25] `tp1_protect_offset_shadow` | **미적용 유지** + `atr_lock_0.75` 감시 승격 |
| **D4** | [25-B] (`..._tp2` 키 신설) | **판단 보류** |

### 결정의 뿌리 — 계측 수정 전/후 대조가 불일치의 성격을 갈랐다

두 PC 0807 리포트의 `원 판정`(정지 전 값)과 441~443차 수정 후를 대조했다.

| 채널 | 수정 전 (0601/0602) | 수정 후 (0601/0602) | 판독 |
|---|---|---|---|
| [3-B] | FAIL / FAIL | **PASS / PASS** | 계측이 원인 — **수렴** |
| [23-B] | FAIL / **PASS** | FAIL / **PASS** | 계측 아님 — **불변** |
| [25] | FAIL(4) / FAIL(3) | FAIL(4) / FAIL(2) | 방향 유지 |
| [25-B] | FAIL / FAIL **(일치)** | FAIL(3) / FAIL(1) | **수정이 갈랐다** |

**같은 "PC간 부호 역전"이 셋 다 다른 원인이었다** — 하나는 계측 결함, 하나는 진짜
거래집합 차이, 하나는 소표본 잡음. → **앞으로 역전을 볼 때 "계측을 고치면 수렴하는가"를
먼저 물을 것.** 그 답이 셋을 가른다.

### D1 — [3-B] `q_raw` 승격 폐기

**감사 §5 활성화 순서 4개가 이로써 전부 소진됐다**(1순위 지정가=상태확인 필요,
2순위 Meta-gate=[2] 탈락, 3순위 분위회귀TP=**폐기**, 4순위 TB=[1] 표본 미달).
**승격 대기 후보 0개**라는 사실 자체를 다음 회의 안건으로 승계한다.
⚠ 채널은 유지한다(PASS=현행 유지). 폐기된 것은 **승격안**이다.
⚠ "압도적 열위" 금지 — 확정된 것은 **부호**이지 크기가 아니다(전량 close에서 −0.92pt).

### D2 — [23-B] 불일치 성격 재분류가 핵심이다

계측을 전면 교체했는데도 split이 그대로다(초과 대안까지 동일). **최대 격차 `tp1_x2`는
해소**됐으나(+32.78 vs −58.40 → −11.04 vs −64.01, 양쪽 음수) `stop_1.0`·`sym`은 오히려
벌어졌다. → "계측 의심"에서 **"거래집합 차이 확정"**으로 재분류.
**실무적 함의: 표본이 쌓여도 수렴하지 않을 수 있다** — 기다림의 기대값이 달라진다.

### D3 — [25] `atr_lock_0.75` 감시 승격 + **표본 목표 사전등록**

3중 검사를 모두 통과한 유일한 변형이다(두 PC 공통 +12.62/+10.39 · `close` 체제
강제에서도 +7.27 · MW0602 재도출 체제 +12.66). `bar_range`는 체제 민감도에서 무너지고
(+7.90→+1.00), `atr_lock_0.50`은 MW0602에서 부호 반전(−3.76)이라 제외.

**사전등록**: `watch_candidate="atr_lock_0.75"` / `watch_min_hooks_per_pc=70` /
`watch_min_days_per_pc=13`. **관측 전 확정이며 도달 전에는 어떤 수치가 나와도 적용
안건으로 올리지 않는다**(313차 — 표본이 찬 뒤 기준을 정하면 관측 후 기준 수립이다).
값의 근거: MW0602 현재 표본(76훅/13일)이 **계측 수정 전후로 결론이 흔들리지 않은 유일한
표본**이라 이를 최소선으로 본다. MW0601은 33훅/4일로 절반 미만.
⚠ 도달해도 자동 적용이 아니다 — **클램프 발동률 11/33(33%)**이 단독 기각 사유가 될 수
있다(시장이 그 보호폭을 1/3에서 제시하지 않는다).
⚠ **443차의 "432차 ④ 재검토" 제안은 445차에 자체 철회**됐고 이번 회의가 추인했다 —
**432차 ④("본전은 현행보다 나쁘다")는 유효**로 본다.

### D4 — [25-B] 판단 보류 (레지스트리 키 신설)

`tp1_protect_offset_shadow_tp2` 신설 — [25]와 판정이 완전히 분리돼 있어(§9-4) 결정도
따로 기록한다. 보류 근거 3중: ① 최선 변형 PC 불일치 ② **건별우세 5/33(15%)인데 총합은
최고** = 소수 거래 지배(반대로 `tp2_0.75`는 건별 10/33인데 총합 최악 — **순위가
뒤집혀 있다**) ③ **계측 수정이 불일치를 만들었다**(0807에는 양 PC가 `tp2_2.00`으로 일치).
⚠ 0807 리포트의 "[25-B] FAIL"도 인용 금지 — **정지 누수 버그**(441차 수정)로 표시된
값이며 애초에 정지됐어야 한다.

### 부수 수정 — 요약표 결정 마커 미배선 1건

D4 등록 직후 [25-B] 행에만 📌 마커가 비어 있었다. `_dm()`이 채널 키로 조회하는데
신규 키(`..._tp2`)가 배선되지 않았기 때문. 배선 완료 — 4채널 전부 마커 표시 확인.

### 검증

- py310_64 + **py37_32** 양쪽 컴파일 통과.
- **합격선 무변경 확인**: `min_samples=20` / `min_days=3` / `tp2_min_samples=20` /
  `tp2_min_days=3` — D3의 `watch_*`는 **판정식에 관여하지 않는 감시 지정**이다(§9-4).
- 리포트 재생성 rc=0, 4채널 확정 결정 마커 + 레지스트리 4섹션 렌더 확인.

**관련**: 0808 주간회의 검토보고, 447차(체제 자동점검), 446차([3-B] 체제 견고성),
445차(체제 민감도·443차 자체정정), 443차([25] 이관), 442차([3-B] 이관), 441차([47] 개통),
MW0602 0808 점검리포트 §7·§8, 421차 후속2, 313차, §9.

---

## 2026-08-08 (MW0601 447차 — MW0602 체제 재도출 검토: 경계 적합성 자동 점검 신설)

**계기**: MW0602가 445차 지적을 받아 **자기 로그로 `tick_era_start`를 재도출**하고
4개 채널을 재검증했다(0808 점검리포트 §8). 그 작업이 정확했고, **수동으로 한 것을
자동화**하는 것이 이번 조치다. **라이브 무관·판정 무변경.**

### MW0602 §8 검토 — 방법론이 옳다

- **§9 규율 준수**: settings의 금지 조항("재현이 잘 되는 쪽으로 옮기지 말 것 —
  옮기는 순간 [47]은 검증이 아니라 곡선맞춤")을 명시적으로 지켰고, **도출 과정에서
  재현 성능을 일절 보지 않았다**고 밝혔다. 이것이 이 작업의 신뢰성을 만든다.
- **config 미변경**: 런타임 오버라이드로만 산출하고 사전등록 블록은 안 건드렸다.
  구조적 이유("단일 값이라 이 PC로 덮으면 MW0601이 깨진다")까지 정확히 짚었다.
- **작은 변화의 구조적 설명**: 경계가 07-31 vs 08-03이라 **재분류되는 것은 07-31
  하루(2훅)뿐**이라고 밝혔다 — 델타가 작은 것이 버그가 아님을 자체 검산한 셈이다.

**재도출 결과(MW0602)**: `tick_era_start` **2026-07-31**(2거래일 이르다),
`pre.protect_mode` **atr_profit**(그 PC는 07-20부터 전 구간 atr_profit이라 pre 구간
자체가 없다). **4개 채널 판정 전부 불변**([25] FAIL / [25-B] FAIL·최선 tp2_2.00 /
[3-B] PASS / [23-B] PASS).

### 🔴 추가 발견 — 08-03 13:25/13:38 예외는 **두 PC 공통**이다

MW0602가 §8-2에서 "예외 2건(`2026-08-03 13:25`/`13:38`)은 421차 후속6이 등록한
'TP1 보호모드 장중변경 로그부재' 사건의 실물"이라고 적었다. **MW0601 DB에도
정확히 같은 2건이 있다** — 같은 시각·같은 방향(SHORT).

`protect_mode`는 `_tp1_protect_mode`(대시보드에서 장중 변경 가능한 PC 로컬 상태)를
그대로 기록한다(main.py:12006). 두 PC가 같은 분에 일치한 것은 **운영자가 양 PC
대시보드에서 모드를 바꿨고, 훅 시각은 같은 시장·같은 코드라 진입이 겹쳤기** 때문이다.
→ MW0602의 귀속이 맞고, **로그 부재가 양 PC 공통**이라는 점이 추가로 확인된다
(421차 후속6의 근거로 이 대조를 함께 인용할 것).

### 조치 — 수동 재도출을 **매주 자동 점검**으로

`check_replay_regime_fit()` 신설. `synthetic_partial_exits`를 경계 전후로 갈라
각 구간 **최빈 protect_mode**를 config와 대조하고, 어긋나면 [47] 절에 🔴 경보를 낸다.

- **왜 필요한가**: settings 주석이 "MW0602에서 재확인 필요"라고 경고했지만
  **사람이 기억해야 하는 구조**였고 실제로 두 달 가까이 아무도 안 했다. 자동화하면
  새 PC가 붙어도 그냥 걸린다.
- **최빈값 기준이라 위 08-03 예외 2건에 흔들리지 않는다** — MW0601 실측 `ok=True`
  (pre `breakeven 29` / post `atr_profit 36, breakeven 2`). MW0602라면 pre가
  `atr_profit`이라 즉시 🔴가 뜬다.
- ⚠ **경계를 자동으로 고치지 않는다. 경보만 낸다** — §9 금지 조항 그대로.
- ⚠ 한계: `protect_mode` 축만 DB로 판정 가능하다. **틱 TP 축(`tick_era_start` 자체)은
  TRADE 로그 파싱이 필요해 범위 밖**이다 — MW0602가 한 로그 집계는 여전히 수동이다.

### 지적 — MW0602 §8에 빠진 것: **[47] 게이트를 재검증하지 않았다**

§8-5는 4개 A/B 채널을 재도출 체제로 돌렸으나 **[47] 게이트 자체는 없다.** 게이트는
[23-B]의 `current`를 정본으로 쓰므로 그 기준선이 움직이면 `repro_bias_R`·
`repro_gap_sd`도 함께 움직인다. §8-5가 [23-B] `stop_1.0` +4.44·`tp1_x2` −9.56의
이동을 보고했으니 **게이트 입력도 확실히 바뀌었다.**
→ 그 PC 게이트는 bias +0.007R·격차 0.42SE로 여유가 커 뒤집힐 가능성은 낮지만,
**"판정 불변"의 범위에 게이트가 포함되지 않는다**는 점은 명시돼야 한다. 재확인 요청.

### 검증

- py310_64 컴파일 통과. 리포트 재생성 rc=0, **판정 무변경**
  ([3-B] PASS / [47] PASS / [25]·[25-B] FAIL). 적합성 줄 렌더 확인(MW0601 ✅).

**관련**: 446차([3-B] 체제 견고성), 445차(체제 민감도 발견), 443차([25] 이관),
MW0602 0808 §8, 421차 후속6(장중변경 로그부재), §9.

---

## 2026-08-08 (MW0601 443차 — [25]/[25-B] 정본 엔진 이관: 기하 A/B 정지 전면 해제)

**지시**: 442차가 등록한 다음 차례(=[25]/[25-B]를 `exit_replay`로 이관) 구현.
**성격**: 계측·리포트 계층. **라이브 신호·주문·청산 경로 무변경.** 라이브 상수 변경 0건.

### 결과 — [3-B]와 정반대로 **판정 방향은 유지되고 크기가 바뀌었다**

| 변형 | v1(구) | **v2(정본)** | 비고 |
|---|---|---|---|
| current | +39.74 | +37.95 | 기준선 |
| breakeven | −10.22 | **−0.08** | 사실상 동률로 붕괴 |
| breakeven_plus | −5.42 | −2.21 | |
| atr_lock_0.50 | +6.89 | +6.73 | |
| **atr_lock_0.75** | +6.86 | **+12.62** | 오히려 **강화** |
| bar_range | +7.40 | +7.90 | |
| lock_prop_0.75 | +3.30 | +3.05 | |

[25] 판정 **FAIL 유지**, 현행 초과 대안도 같은 4개다. **[3-B]가 FAIL→PASS로
뒤집힌 것과 대조적** — 인공물의 방향이 채널마다 다르다는 실증이다.

**[25-B]는 최선 변형 자체가 바뀌었다**: `tp2_2.00`(v1 최선 +1.70) →
**`tp2_1.25`(v2 최선 +6.11)**. 현행 초과 대안도 1개 → **3개**(tp2_1.00/1.25/2.00).

**왜 `breakeven`이 붕괴했나** — v1은 보호스톱을 고정으로 뒀지만 v2는 **4단계
트레일링이 그 위로 올라온다**. 초기 lock을 본전에 놔도 트레일링이 이익을 잠그므로
차이가 흡수된다. 즉 432차 ④의 "보호를 본전으로 놓으면 현행보다 나쁘다(−9.23pt,
p=0.021)"는 **v2에서 약해진다** — 재검토 대상으로 레지스트리에 명시.

### 구현 — v1과 v2는 재생 **시작점**부터 다르다

v1: **보호전환 시점(i0)부터** 스톱/TP2만 보고 종료.
v2: **진입부터** 라이브 청산 사다리를 돌리고 offset을 `lock_offset_pts`로 주입.

조건부 타당성은 그대로다 — offset은 TP1 도달 여부에 영향을 주지 않으므로 arm 사건
집합이 전 변형에서 동일하다(§25-B 주석의 논리 그대로).

- **체결가능성 클램프를 정본 안으로 옮겼다** — `exit_replay`에 `lock_clamp_to_bar`
  **opt-in**(기본 False) 신설. offset이 arm 봉의 유리 극값을 넘으면 보호스톱이
  시장이 제시한 적 없는 가격에 놓이고, 다음 봉에서 무조건 히트로 읽힌다(432차가
  v1에서 발견한 결함과 **같은 것**). 실측 발동: atr_lock_0.75 **11건** /
  bar_range 8건 / atr_lock_0.50 3건. current·breakeven류 0건.
  ⚠ 기본 False라 **[23-B]/[3-B]는 영향 0**(라이브 offset ATR×0.25는 TP1 거리
  ATR×0.3~0.7보다 작아 애초에 클램프될 일이 없다) — 무회귀 실측으로 확인.
- **v2 전용 제외 2종**(v1에는 없던 개념, 모집단 36 → **33건**):
  - **원 진입수량 ≠ 1 — 2건.** tier1 축소/TP1 부분청산으로 **잔여 1계약**이 arm된
    케이스인데, 정본은 `stage==1 and qty==1`로 **원 진입수량**을 보므로 부분청산으로
    모델링한다 — 같은 상황이 아니다. (정본 재생기가 "잔여 1계약 arm"을 모델링하지
    않는다는 사실 자체가 별건 발견 — 아래 잔여 과제.)
  - **재생기 arm 미도달 — 1건.** 라이브는 틱으로 TP1을 쳤는데 봉 기준 재생에서는
    못 친 경우. 전 변형이 동일해져 delta를 희석하므로 제외.
  - arm 시각 일치율 **20/33(61%)** — 나머지는 1봉 내외 차이(틱 vs 봉 해상도).
- 리포트: 결말 분포 컬럼 추가. v2에서 `n_stop`이 항상 0인 것은 **보호전환 뒤 털린
  건이 `TP1_ARM_STOP`으로 라벨링**되기 때문이다(결함 아님) — [3-B]와 같은 조치.

### 기하 A/B 채널 정지 전면 해제 — 441차 가드의 소임 완료

| 채널 | 441차 | **443차** |
|---|---|---|
| [23-B] | 정본(FAIL) | 정본(FAIL) |
| [3-B] | SUSPENDED | 442차 이관 → **PASS** |
| [25]/[25-B] | SUSPENDED | **443차 이관 → FAIL** |
| [46] | 재생 없음 | INSUFFICIENT |

`replay_engine_targets`에 승격해 **엔진 커버리지 사유의 정지가 0건**이 됐다.
441차가 신설한 가드는 "이관하라"는 신호였고, 442·443차가 그것을 소진했다.

### 확정 결정 레지스트리 — 결정은 유지, 수치는 무효 표기

[25]의 2026-08-05 결정("현행 유지")은 **v2에서도 판정이 FAIL이고 초과 대안도 같아
유지된다.** 따라서 `date`/`decision`을 건드리지 않았다(새 결정은 주간회의 소관, §9).
대신 note 최상단에 **"아래 모든 수치는 v1 기준"** 부기와 바뀐 크기·[25-B] 최선
변형 변경·④ 재검토 필요를 명시했다.
⚠ [3-B](442차)는 **판정이 뒤집혀** 결정 자체를 교체했다 — 두 처리의 차이를 혼동 말 것.

### 검증

- py310_64 + **py37_32(라이브 런타임)** 양쪽 컴파일 통과.
- **v1 롤백 회귀 재현** (current +39.74 / atr_lock_0.75 +6.86 / lock_prop
  0.35·0.50·0.75 = −2.99·−1.83·+3.30 — 440차·NEXT_TODO 기록값과 일치).
- **무회귀 실측**: [23-B] v2 4/4 동일, [3-B] v2 4/4 동일, [47] PASS 동일
  (bias +0.0146R·격차 0.343SE·시뮬 +18.85 vs 실제 +30.63).
- 리포트 전체 재생성 rc=0 (**`--out-dir`** — EOD 산출물 미훼손, 439차 규율).

### 잔여 과제 (NEXT_TODO 등록)

**정본 재생기가 "잔여 1계약 보호전환"을 모델링하지 않는다.** `replay()`의 arm 조건이
`stage == 1 and qty == 1`(원 진입수량)이라, qty≥2가 tier1/TP1로 1계약이 된 뒤 arm되는
실제 경로가 빠진다. 443차는 그 2건을 제외하는 것으로 우회했으나, **[23-B]·[47]에도
같은 미모델이 들어 있다**(게이트는 통과했으므로 허용 오차 안이지만 알려진 갭이다).

**관련**: 442차([3-B] 이관), 441차([47] 개통·엔진 커버리지 가드), 440차(v2 엔진),
432차(클램프·[25-B] 신설), 404차 후속3([25] 신설), 313차, §9 사전등록.

---

## 2026-08-08 (MW0601 442차 — [3-B] 정본 엔진 이관: "유일한 승격 후보"가 인공물이었다)

**지시**: 441차가 등록한 최우선 후속(=[3-B]를 `exit_replay`로 이관) 구현.
**성격**: 계측·리포트 계층. **라이브 신호·주문·청산 경로 무변경.** 라이브 상수 변경 0건.

### 결론 — 판정이 FAIL → **PASS**로 뒤집혔다 (= 분위 TP1이 현행을 못 이긴다)

| 변형 | TP1 폭(현행 대비) | v1 (구 시뮬) | **v2 (정본 이관)** |
|---|---|---|---|
| current | — | −23.93 | **+0.74** |
| q_x0.7 | +11.8% | +0.56 | **−14.73** |
| q_blend | +29.8% | +6.42 | **−10.99** |
| **q_raw** | +59.7% | **+20.22 ← 최고** | **−9.59** |

건별 우세도 무너졌다(q_raw 67/110 → **15/110**). drop-max도 −13.81로 더 나빠져
"소수 거래가 만든 착시"조차 아니다 — **일관되게 현행이 낫다.**

### 왜 구 수치가 인공물이었나

구 `_simulate`의 규칙은 하나였다 — **"TP1선에 닿으면 그 거리만큼 전부 벌고 즉시 종료"**.
실제로 TP1 도달은 청산이 아니다(1계약이면 **보호전환**이고 포지션은 살아 있다;
2계약 이상이면 33%만 나간다). 즉 구 시뮬은 **"TP1 찍고 반납한 거래"와 "TP2까지 완주한
거래"를 둘 다 만점 처리**했고, 그 오차는 **TP1 거리에 정비례**한다.
→ **TP1을 넓히는 변형일수록 공짜로 점수가 오르는** 구조적 편향.

**지문이 완벽하게 단조였다** — 위 표에서 개선폭 순위가 "분위회귀가 맞았는가"가 아니라
**"얼마나 넓혔는가"**로 전부 설명된다(+11.8%→+0.56 / +29.8%→+6.42 / +59.7%→+20.22).
440차가 [23-B] `tp1_x2`에서 실증한 것과 **같은 인공물**이다(v1 최고 +37.96 → v2 최악 −30.68).
하필 이 채널의 질문이 **"TP1 거리를 무엇으로 정하나"**라 편향의 직격 대상이었다.

### 구현

- `exit_replay.geometry()/replay()`에 **`tp1_pts_abs`** 절대-pt 오버라이드 신설.
  분위 추정치는 배수가 아니라 이미 pt 거리라 배수 역산은 왕복이 생긴다.
  `lock_offset_pts`가 이미 같은 취지의 선례다. **기본 None → [23-B] 무영향**(회귀 확인).
  ⚠ 지정 시 hurst 배수를 곱하지 않는다 — 분위에 이미 변동성이 반영돼 이중 계상이 된다
  (구 `_tp1_for()`도 `current`에만 hurst를 곱했다 — 동작 동일).
- `quantile_tp_shadow`: `compute(engine=)` 스위치([23-B]와 **같은 스위치 공유**),
  `_load_trades`에 qty 추가(entry_qty 우선·레그합 폴백 — 441차 게이트 규약),
  체제 인식(`regime_for`), 결말 분포(`outcomes`) 출력.
- **기하역전 가드 신설** — v1에는 없던 TP2/TP3가 v2에는 있어 분위 TP1이 현행 TP2를
  넘으면 순서가 뒤집힌다. 사전 실측 **110건 전 변형 0건**(근접 1건)이라 표본 손실은
  없지만, 분위는 신호마다 달라 앞으로 생길 수 있으므로 막고 **건수를 리포트에 노출**한다.
  TP2/TP3는 현행 고정 — "**TP1 축만** 바꾼다"는 사전등록 취지 그대로.
- `replay_engine_targets`에 승격 → 441차 엔진 커버리지 가드가 자동 해제.

### 확정 결정 레지스트리 갱신 (CLAUDE.md 규약)

`quantile_tp_shadow` 항목을 2026-08-01 → **2026-08-08**로 교체:
"FAIL이지만 미적용 — MW0601 교차확인 선행" → **"승격 후보 소멸 — 계측 결함이 만든
우위였다"**. 구 결정이 요구한 **MW0601 교차확인은 이제 무의미하다** — 양 PC가 같은
인공물을 공유했을 뿐이다.

⚠ **감사 §5 활성화 순서 4개가 모두 소진됐다.** 1순위 지정가=상태확인 필요, 2순위
Meta-gate=[2] 탈락(0801), 4순위 TB=[1] 표본 미달, **3순위 분위회귀 TP 보조=이번에 소멸**.
0801 결산이 "유일한 승격 후보"로 올린 근거(`q_raw` +20.22pt)는 **인용 금지**.

### 검증

- py310_64 + **py37_32(라이브 런타임)** 양쪽 컴파일 통과.
- **v1 롤백 회귀 4/4 비트 단위 재현** (current −23.93 / q_raw −3.70 / q_x0.7 −23.37 /
  q_blend −17.51) — 구 세대를 되돌려 감사할 수 있다.
- **[23-B]·[47] 무회귀 확인** — 게이트 PASS 유지(bias +0.015R·격차 0.34SE),
  [23-B] FAIL 유지(stop_1.0·sym_1.0_1.0), [25]/[25-B]는 미이관이라 SUSPENDED 유지.
- 리포트 전체 재생성 rc=0 (**`--out-dir`** — EOD 산출물 미훼손, 439차 규율).

### ⚠ PC 대조 주의

리포트의 PC 대조표가 [3-B]를 "부호 역전"으로 표시한다(MW0601 +0.74 vs MW0602 −24.40).
**PC 차이가 아니라 코드 세대 차이다** — MW0602는 아직 미이관(v1)이다.
CLAUDE.md "대조는 같은 코드 세대끼리" 규약대로 **이번 주 [3-B] PC 대조는 무효**다.

**관련**: 441차([47] 개통·엔진 커버리지 가드 신설), 440차(v2 엔진·tp1_x2 인공물 실증),
404차 후속10([3-B] 신설), 0801 W4 결산(승격 후보 선정), 372차(drop-max), 313차, §9.

---

## 2026-08-08 (MW0601 441차 — [47] 게이트 개통: 남은 격차는 재생기 능력이 아니라 계측 결함이었다)

**지시**: 440차 잔여 3안건(오차원 분해 / 부호기준 타당성 / 단위 불일치) 딥다이브 후
구현. **성격**: 계측·판정·리포트 계층. **라이브 신호·주문·청산 경로 무변경.**
라이브 상수 변경 0건.

### 결과 요약 — [47] SUSPEND → **PASS**

| 엔진 | 편향(중앙) | MAE | 격차/SE | 시뮬 | 실제 | 판정 |
|---|---|---|---|---|---|---|
| v1 (435차까지) | +0.018R | 0.373R | **1.250** | −12.27 | +30.63 | SUSPEND |
| **v2 + 441차 수정** | **+0.015R** | **0.165R** | **0.343** | **+18.85** | +30.63 | **PASS** |

**합격선을 완화해서 통과한 것이 아니다** — 아래 ⓐⓑ를 고친 뒤 **구 기준
(`sign_match` 무조건) 그대로 PASS**했고(`verdict_strict_sign`=PASS로 리포트에 병기),
같은 수정을 받은 **v1은 여전히 SUSPEND**다. 기준이 판별력을 잃지 않았다는 대조 증거.

### 440차 진단의 정정 — "틱 경로가 남은 오차원"은 절반만 맞았다

440차는 잔여 격차를 오차원 ①(틱 경로)로 귀속했다. 441차가 **실제 청산 경로별로**
분해하니 그 귀속이 틀렸다:

| 실제 경로 | n | Σ오차(구 단위) | 판정 |
|---|---|---|---|
| 틱스톱만 | 76 | **+0.05** | 틱 재현은 **이미 정확했다** |
| TP 도달 | 33 | −22.73 | 여기에 몰려 있었다 |
| 봉스톱만 | 10 | −10.66 | |

격차는 재생기의 **능력**이 아니라 두 **계측 결함**이었다.

#### ⓐ 판정식 단위 3중 불일치 (−16.15pt) — `sim_fidelity_gate.py`

`_actual_by_entry()`의 `SUM(pnl_pts)`는 **레그별 계약당 pt의 무가중 합**인데 시뮬은
`pts_per_contract`(1계약)였고, 게다가 **시뮬만** 왕복비용 0.10pt/포지션을 뺐다
(실제 `pnl_pts`는 gross — `pnl_pts×50,000×qty == gross_pnl_krw` 전건 검증).

- 비용 비대칭 119포지션 × 0.10 = **−11.90pt** (단일 최대 오염원)
- 레그합 왜곡 **−4.25pt**

레그합 왜곡은 **결과에 비대칭**이라 더 나쁘다(417차와 같은 메커니즘) — 이긴 포지션은
TP 분할로 레그가 여러 개(부풀려짐), 진 포지션은 하드스톱 전량이라 하나다.
**실측: post 세대 TP도달 오차 −18.65pt가 단위 통일 후 0.00pt** — 전부 이 인공물이었다.

→ 양변을 **gross·1계약**으로 통일. 구 입력값은 `*_legacy` 필드로 병기해 감사 가능.
⚠ 440차가 진단 표기하던 `_actual_krw_by_entry()`는 **net**(수수료 차감)이라 gross
기준 판정식에 섞으면 또 다른 불일치다 — 그 병기 수치("계약가중 −23.48 vs +27.69")는
인용 금지.

#### ⓑ 재생기 TP 체결가 (−21.56pt) — `exit_replay.py`

`tp_trigger="close"` 세대(~08-02)는 트리거가 **"종가가 목표가를 넘었다"**인데 체결을
**목표가**로 계상했다 — 자기모순이고, 넘어선 만큼(이익 오버슈트)이 통째로 사라진다.
실측: **TP2 완주 22건이 목표거리를 전건 초과**(합 +18.19pt, 최대 +2.47pt).

→ `close` 세대 TP 체결가를 **트리거 봉 종가**로. 자유 파라미터 도입 0(종가는
데이터이고, 적용 세대는 이미 사전등록된 `replay_regime`이 정한다).
⚠ **스톱에는 적용하지 않는다** — pre 세대에도 `하드스톱(틱)` 레그가 실존하므로
확대 적용 시 −23.40pt가 반대로 얹혀 재현이 **더 나빠진다**(실측 확인).

### 부호기준 견고화 — 1차 설계가 실패했고 그 실패가 기준을 결정했다

`require_sign_match`는 **검정력이 없을 때가 있다**: |Σ실제| = **0.89 SE**,
부트스트랩 **P(Σ실제>0)=0.81**(현실을 다시 돌려도 5번 중 1번 부호가 뒤집힌다),
집중도 **+30.63pt 중 +20.70pt(68%)가 07-21 하루**.

🔴 그래서 "무정보면 일자단위 추적성으로 대체"를 넣었더니 **고장난 v1까지 PASS**했다
(일자상관 0.856·부호일치 88%). 원인 분해: **아무도 수준(level)을 안 보고 있었다** —
중앙 bias는 오차가 소수 포지션에 몰리면 지워지고, 일자 상관은 순위만 본다.

→ **수준 기준 신설**: `|Σ시뮬−Σ실제| ≤ repro_gap_sd_max(1.0) × SE`.
이것은 완화가 아니라 **일반화이자 강화**다 — 정보력 있는 구간에서
`gap ≤ 1SE ∧ |Σ실제| ≥ 2SE ⟹ 부호 일치`가 **수학적으로 따라온다**.
최종 4기준 AND: ①편향 ②수준 ③일자추적 ④부호(정보력 있을 때만 구속).
임계 1.0은 실측 v2=0.343/v1=1.250 사이라 **0.4~1.2 어디든 판정이 같다**(칼날 아님).

### 🔴 게이트를 열다 발견한 별개 결함 — 엔진 커버리지

435차의 간접 추론("정본이 실패하면 공유 가정을 쓰는 그들도 실패")은 **제한 방향으로만
타당하다**. 역방향은 성립하지 않는다 — **그들은 정본과 코드를 공유하지 않는다**.
440차가 고친 것은 `exit_replay.py`인데, 나머지는 자체 `_simulate`를 그대로 쓴다:

| 채널 | 재생 방식 | PASS가 검증하나 |
|---|---|---|
| `tp1_geometry_shadow` [23-B] | **정본** `exit_replay` | ✅ |
| `hurst_threshold_shadow` [46] | **재생 없음**(실제 손익 슬라이스) | ➖ 무관 |
| `quantile_tp_shadow` [3-B] | 자체 `_simulate` — **TP1에서 종료** | ❌ |
| `tp1_protect_offset_shadow` [25]/[25-B] | 자체 `_simulate` — **TP2에서 종료** | ❌ |

가드가 없었으면 441차의 개통이 **검증된 적 없는 계측을 통과시켰을 것**이다 —
[47]이 막으려던 사고를 반대 방향으로 재현하는 셈. `replay_engine_targets` /
`no_replay_targets` 사전등록으로 **PASS의 효력 범위를 제한**했다(SUSPEND 방향 무변경 —
즉 **완화가 아니라 강화**).

🔴 **[3-B]는 특히 위험하다.** 자체 시뮬이 TP1에서 종료하며 TP1 목표가를 확정이익으로
계상하는데, 440차가 [23-B]에서 **"TP1을 넓히는 것만으로 장부가 좋아지는 순수 인공물"**
로 지목한 바로 그 구조다(v1 `tp1_x2` 최선 +37.96 → v2 최악 −30.68). 그런데 [3-B]의
질문 자체가 **"TP1 거리를 무엇으로 정하나"** 라 인공물이 답을 직접 오염시킨다 —
**`q_raw` 우위(+20.22pt)를 이관 전 수치로 인용하지 말 것.**

### 부수 결함 수정 — [25-B]가 정지를 새어나갔다

settings의 targets 주석은 "[25-B]는 [25] 스크립트 안의 평행 축이라 **같은 키로 함께
정지된다**"고 사전등록했는데, 코드는 최상위 `verdict`만 덮어써 중첩 `tp2` 축이
빠져나갔다(0807 리포트에서 [25] SUSPENDED인데 [25-B]만 FAIL로 찍힌 것이 증거).
사전등록과 코드를 일치시켰다 — 기준 변경이 아니다.

### 신설 — [47-B] TP 체결 오버슈트 감시 (`tp_fill_overshoot_watch.py`, OBSERVE 전용)

ⓑ 같은 결함의 **재발 상시 감시**. 첫 관측이 세대 분리 설계를 그대로 확인했다:

| 구분 | n | 중앙(pt) | 양수비율 |
|---|---|---|---|
| 세대 `close` | 18 | **+0.699** | 94% |
| 세대 `envelope` | 15 | **+0.141** | 80% |

⚠ **판정하지 않는다** — 정상 오버슈트 크기의 기준을 지금 만들면 관측을 본 뒤 기준을
세우는 것(313차)이고 post 표본이 얇다(372차 PSI 함정). ⚠ [47] `targets`에 넣지
않았다(게이트가 쓰는 재생기를 감시하므로 순환).

### 판정 변화 (게이트 개통 후)

[47] **PASS** · [47-B] OBSERVE · [23-B] **FAIL**(자력 판정, 정본 검증됨) ·
[46] INSUFFICIENT · [3-B]/[25]/[25-B] **SUSPENDED(정본 재생기 미이관)**

### 검증

- py310_64 + **py37_32(라이브 런타임)** 양쪽 컴파일 통과 (`settings.py`·`exit_replay.py`·
  `sim_fidelity_gate.py`·`tp_fill_overshoot_watch.py`).
- **v1 롤백 회귀: 440차 기록값 4/4 비트 단위 재현**
  (current −24.17 / stop_1.0 −17.49 / tp1_x2 +37.96 / sym_1.0_1.0 +19.69).
- 리포트 전체 재생성 rc=0 (**`--out-dir` 사용** — EOD 산출물 미훼손, 439차 규율).
- 기존 테스트: `test_424_campaign_channels.py` 2건 실패는 **데이터 표류**(‌[37]
  짝지은 거래일 7→10일, 08-04 스냅샷에 하드코딩). 해당 채널은 변경 코드를 **0회**
  참조하며 verdict는 여전히 PASS. `test_424_backfill_marker_and_shadow.py` 통과.

**관련**: 440차([47] 해제작업·v2 엔진), 435차([47]·[48]·[49] 신설), 432차 후속2(러너
프리미엄), 417차(레그 vs 포지션 단위), 372차(이상치 분해·임계 재보정 자제), 313차 원칙,
§9 사전등록.

---

## 2026-08-07 (MW0601 440차 — [47] 해제작업: 재생기를 라이브 청산 사다리로 교체 + 라벨 기하 결함 1건)

⚠ **차수 충돌 주의** — 같은 날 MW0602가 439차/439차 P2/439차 P3를 썼다. 이 세션은
MW0601이고 직전이 438차라 **440차**로 매겼다(PC 접두어 규약, `feedback_devmemory_pc_provenance`).

**지시**: 0807 일일점검 보고의 제안 4·5·6 구현.
**성격**: 계측·리포트 계층. **라이브 신호·주문·청산 경로 무변경.**
설정값 변경 0건 — `TP1_PROTECT_ATR_LOCK_MULT`(0.25) 등 라이브 상수는 그대로다.

### 배경 — 0807 점검이 만든 숙제

방향 11전 10승(+690,825원)인데 MFE30 평균 8.52pt 중 실현 0.66pt(**포착률 7.8%**).
TP1 보호전환 → 청산까지 5·8·69·179·223초. 그런데 이 축을 파고들 수단이 없었다 —
**[47] 게이트가 기하 A/B 4채널의 판정을 전부 정지**시켜 놓았기 때문이다.
[47]이 사전등록한 해제 조건이 "재생기에 TP3·트레일링을 편입해 재현이 회복되면
자동 PASS"였고, 이번 세션이 그 편입을 수행했다.

### 항목4 — `scripts/exit_replay.py` 신설 (정본 재생기 교체)

라이브 `_ts_check_exit_triggers()` 순서를 그대로 미러링한다:
4단계 트레일링 → 하드스톱(종가/봉중 + 423차 유령 가드) → 손절1차(qty>=2) →
TP1/TP2/TP3 분할(qty=1 TP1은 **보호전환**) → 15:10 강제청산.

- 트레일링은 `compute_trailing_stop_tier`를 **라이브에서 import**한다(361차가
  "복붙 금지"로 순수함수 분리해 둔 것을 그대로 씀).
- **새로 도입한 수치 상수 0개.** 배수·비율은 전부 settings와 라이브 함수에서 읽는다
  — 게이트를 통과시키기 위한 튜닝이 원리적으로 불가능해야 회복 판정이 의미를 갖는다.
- `sim_fidelity_gate["engine"]` = "v1"/"v2" 스위치. **v1 코드는 삭제하지 않았다**
  (회귀검증: v1로 되돌리면 435차 수치가 비트 단위로 재현됨 — current −24.17 /
  stop_1.0 −17.49 / tp1_x2 +37.96 / sym_1.0_1.0 +19.69).

#### 🔴 추가 발견 — 재생기만 고쳐서는 안 됐다: **코드 세대(체제) 혼재**

과거를 *지금의 규칙*으로 재생하면 그 자체가 새 오차원이다. MW0601 TRADE 로그
전수 집계에서 경계가 깨끗하게 갈렸다:

```
~2026-07-31 : TickTP1 0건        / protect_mode=breakeven 전량 (16거래일)
2026-08-03~ : TickTP1 상시       / protect_mode=atr_profit  (5거래일)
```

실례(2026-07-21 09:30 LONG qty=1): 실제 +8.82pt(TP2 완주). 그날은 종가 트리거·
breakeven이라 09:36 종가 1040.00에서 본전 스톱 후 완주했다. 지금 규칙(봉고저가·
atr_profit)으로 재생하면 09:32 봉고가에서 조기 arm → 09:33 저가가 lock을 때려
**+1.24pt STOP**. 한 건 격차 **−7.64pt**.
→ `VALIDATION_CAMPAIGN["replay_regime"]`로 사전등록. [25]가 별개 근거로 이미
`mw0601_sample_start: "2026-08-03"`을 등록해 둔 것과 **같은 날짜**라 상호 검증된다.

#### 결과 — 개선은 컸지만 **게이트는 SUSPEND 유지**다

| 엔진 | MAE(개별정확도) | 편향(중앙) | 총합 시뮬 | 총합 실제 | 부호 | 판정 |
|---|---|---|---|---|---|---|
| v1 (435차까지) | 0.4078R | −0.0099R | −24.17 | +34.89 | False | SUSPEND |
| **v2 (사다리+체제)** | **0.2332R (−43%)** | −0.0509R | −14.61 | +34.89 | False | SUSPEND |

**합격선을 사후에 완화하지 않았다(§9).** ②③ 편입은 [47]이 사전등록한 조치이고
그것은 이행했다 — 그런데도 부호가 안 맞으면 그것이 정직한 결과다.

남은 오차원은 ①(틱 경로)이다. 0807 호가로그 **224,812틱**으로 별도 확인:
봉 단위로는 현행 재현이 +62% 어긋나며, 격차는 틱청산 80건(+9.20pt, 건당 +0.115)이
아니라 **TP 도달 29건(−35.75pt, 건당 −1.233)** 에 몰려 있다.

⚠ **부호 판정 자체가 불안정하다는 진단을 함께 남겼다** — |Σ실제| = **0.95 SD**.
실제 총합이 분산 대비 0 근처라 아주 작은 편향으로도 `sign_match`가 뒤집힌다.
post 세대만 보면 틱청산(−46.76 vs −39.59)·비틱청산(+46.52 vs +51.60) **둘 다
부호가 맞는데** 합치면 상쇄돼 −0.24 vs +12.00이 된다. **기준 변경은 하지 않았다** —
주간회의 안건으로만 올린다.

⚠ **단위 불일치(기존 결함) 발견** — 판정식의 `실제`(`SUM(pnl_pts)`)는 레그별
계약당 pt의 단순합이라 계약가중이 아니다. 계약가중으로 맞추면 시뮬 −23.48 vs
실제 +27.69. **판정식 입력은 바꾸지 않고** 리포트가 양쪽을 병기한다.

#### ⚠ v1이 만들던 인공물의 크기 — 과거 결론 인용 금지

[23-B]에서 v1은 `tp1_x2`를 **최선(+37.96pt)**으로 봤는데 v2에서는 **최악(−30.68pt)**
이다. TP1에서 종료하며 `tp1_pts`를 확정이익으로 계상하면 **TP1을 넓히는 것만으로
장부가 좋아지는** 순수 인공물이 생긴다. 432차 후속2가 경고한 바로 그 편향이다.

### 항목5 — [25] §25-C 호라이즌 비례 lock 축 **사전등록** (라이브 무변경)

TP1은 374/387차에 호라이즌별로 분화됐는데(1m .3 / 3m .5 / 5m .7) lock은 339차
상수(ATR×0.25)라 **유지율이 호라이즌에 반비례**한다 — 1m 83% / 3m 50% / 5m 36%
([49]가 실측으로 같은 것을 지적). 설계 의도라는 근거가 없고 두 축을 각각 고친
**부작용**으로 보인다(419차 "죽은 상수", 432차 "3중 정의"와 같은 계열).

`lock_prop_0.35 / 0.50 / 0.75` (offset = TP1거리 × k) 3변형을 **관측 전 확정**해
등록. 양방향 그리드로 **단조성**을 본다. 게이트가 열리는 시점에 자동 판정된다.

⚠ 부수 교정: 비례 변형은 `entry_horizon`이 필요해 `trades` 조인 실패 행이 빠진다.
모집단이 달라진 채 총합끼리 빼면 무의미하므로 **짝지은 부분집합 delta**로 바꿨다
(`delta_paired`). 기존 변형은 전건 일치라 **값이 종전과 동일**하다(회귀 없음).

### 항목6 — 🔴 [41] 라벨러 TP1 기하 결함 (실질 버그)

`exit_outcome_labeler.build_labels()`가 TP1을 `ATR × ATR_TP1_MULT`(=**1.0**,
"entry_horizon 미지정 시 fallback")로 잡고 있었다. 라이브는 호라이즌별 0.3/0.5/0.7
에 hurst 배수까지 곱하므로 **TP1 거리가 1.4~3.3배 과대**였다(표본 호라이즌:
3m 57 / 5m 49 / 1m 11건 — **전건 영향권**). 스톱에도 hurst 배수가 빠져 있었다.

오염 범위: `dist_tp1_atr`·`tp1_done` 피처 + `y_tp1_first` 라벨.
수정 후 `y_tp1_first` 양성률 **42.4% → 53.8%**, 주 라벨 `y_hold_better`는 TP1
기하와 무관해 **36.6% 불변** — 이 불변이 수정 범위가 정확했다는 검산이다.

**왜 표본을 더 기다리기 전에 고쳤나**: [41]은 267포지션(약 41거래일) 부족으로
학습을 거부 중인데, 그동안 쌓이는 것이 **잘못 라벨된 표본**이었다.
기하는 이제 `exit_replay.geometry()` 단일 소스에서 온다.

### 검증

- `py_compile` 7파일 py310_64 전부 통과. `exit_replay.py`·`exit_outcome_labeler.py`·
  `settings.py`는 **py37_32(라이브 런타임)에서도** 컴파일·import 확인.
- 라이브 `main.py`는 라벨러를 import하지 않는다(EOD 전용 경로) — 런타임 영향 0.
- 주간 리포트 전체 재생성 rc=0, [47]/[25]/[41] 3개 절 렌더 확인.
- 회귀: engine="v1" 복귀 시 435차 수치 비트 단위 재현.

**관련**: 435차([47]·[49] 신설), 432차 후속2(러너 프리미엄), 361차
(compute_trailing_stop_tier 분리), 374/387차(TP1 호라이즌 분화), 339차(lock 상수),
417차(레그 vs 포지션), 423차(유령 스톱 가드), 313차 원칙, §9 사전등록.

---

## 2026-08-07 (MW0602 439차 P2 — [17]이 자기 경보를 스스로 억제하고 있었다)

**지시**: 439차 제안 P2 구현. **성격**: 계측·리포트 계층. **라이브 무관.**

### 🔴 결함 — 오염된 풀링 평균이 채널 자신의 경보를 막고 있었다

[17]의 note 조건은 `풀링 평균 > 캠페인 가정(0.02pt)`인데, 풀링 평균이
**봉중(stop_intrabar) 경로에 오염**돼 음수로 끌려가 조건이 **성립할 수 없었다**.

| 구분 | n | 평균 | 가정 초과? |
|---|---|---|---|
| 풀링(구 헤드라인) | 109 | **−0.1816** | ❌ ← 경보 억제 |
| 봉중 (제외 대상) | 13 | **−1.8854** | ← 끌어내린 범인 |
| **유효-hint(틱/종가/목표)** | 96 | **+0.0492** | ✅ **2.5배** |

봉중은 `price_hint`가 봉 고저가에서 유도한 **허구의 스톱가**라 주문(시장가)과
무관하다 — 슬리피지 추정에서 빼는 것이 옳다(P3 [48-T]에서 확정). 경로 분류는
`bar_stop_path_watch._path_of()` **단일 소스**를 재사용해 드리프트를 막았다.

### 판정 승격 — 그리고 결론은 PASS다(오독 주의)

오염을 걷어내도 **일자단위로는 미유의**다: 일자 평균 +0.0133 / 중앙값 +0.0659 /
초과일 **6/10** / 부호검정 **p=0.7539**. → **PASS**.
⚠ **PASS는 "가정이 옳다고 증명됐다"가 아니라 "구분되지 않는다"**이다. 이벤트단위는
가정의 2.5배지만 313차상 판정 단위는 일자다 — 이벤트 수치만 보고 상수를 바꾸면
372차가 이상치로 PSI 임계를 바꿀 뻔한 것과 같은 함정이다.
`event_day_split` 플래그로 세 지표(이벤트평균/일자평균/일자과반)의 불일치를 상시 노출.

### 🔴 구현 중 자기 버그 1건 — 회귀 테스트가 잡았다

초판 FAIL 조건이 `significant AND 일자 **평균** > 가정`이었다. 부호검정은 **양측**이라
"9일이 가정 **미만**이고 1일만 폭등"해도 유의가 뜨고, 그 1일이 평균을 끌어올려
평균 조건까지 참이 되어 **FAIL이 나버린다** — 일자단위로 내려온 이유(이상치 강건성)가
그 자리에서 무너진다. 테스트 (4)가 이걸 잡았다.
→ 방향 판정을 **일자 과반**(`days_over_assumed*2 > n_days`)으로 교체. 이제 부호검정이
세는 통계량과 방향 판정이 **같은 것**이다. 일자 중앙값도 함께 노출.

### 사전등록 정직성

첫 관측치는 이 승격 **전에 이미 봤다**([48]의 고지와 같은 취급). 다만 비교 임계
**0.02pt는 사후에 고른 값이 아니다** — 369차에 고정된 모델 가정
(`slippage_ticks_per_side=1.0 × TICK_SIZE`)을 그대로 쓴다. 움직인 것은 임계가 아니라
**오염 제거와 검정 단위**(이벤트→일자)다.
⚠ FAIL이어도 `slippage_ticks_per_side`를 **자동으로 바꾸지 않는다**(§3 — 캠페인 전
채널의 왕복비용에 쓰이므로 바꾸면 검증 시계 리셋).

### 호환

`avg_slippage_pts`(풀링)는 **의미를 바꾸지 않고 그대로 뒀다** — metrics json 소비처와
과거 주차 대조(`cmp_metrics.py`)가 이 키를 읽는다. 새 키만 추가했다.

### 검증

py_compile 2환경 rc=0 · `tests/test_439_p2_slippage_judgment.py` **28검사 2환경 통과**
(봉중 제외·단일소스 분류·미태깅 재분류·note가 풀링 오염에 안 막힘·**일자 과반이 판정
단위**·일관 초과 시 FAIL·가정 미만 시 PASS·풀링 키 의미 불변·표본미달 INSUFFICIENT·
실 DB 스모크) · 439차 P1/P3·424차 테스트 무회귀 · `--out-dir` 대조에서
**변경 채널 [17] 하나뿐**.

**관련**: 439차 P3([48-T] — 봉중 hint 허구 확정), 369차([17] 신설·가정 고정),
[48](정직성 고지 선례), 372차(이상치로 임계 바꿀 뻔한 전례), 313차·§3·§9 원칙.

---

## 2026-08-07 (MW0602 439차 P3 — [48]은 자기가 내건 질문에 답할 수 없는 상태였다)

**지시**: 439차 제안 P3 구현. **성격**: 계측·리포트 계층. **라이브 무관.**

### 초안 가설 1건 기각 — [48]에 [35] 패턴을 씌우면 틀린다

P3 초안은 "봉중 경로도 [35]처럼 구조적 판정불가"였다. **기각한다.** 봉중은
"구조적으로 불가능"이 아니라 **가드가 유령만 걷어내고 틱 경로가 나머지를 선점**해
희귀해진 것이다 — 08-05의 1건은 `would_suppress=0`, 즉 유령이 아니라 **정상 봉중**이다.
→ `structural_block`을 붙이지 않았다.

### 🔴 진짜 결함 — 사전등록 오염 (풀링이 채널의 존재 이유를 무효화한다)

[48]은 스스로 *"가치는 424차 억제 배포 후의 **회복 판정**에 있다"*고 적어 두고도
`summarize()`가 `start_date`(07-05) 이후를 **통째로 풀링**해 판정한다. 실측:

| 날짜 | 봉중 | 상태 |
|---|---|---|
| 08-03 | **8** | 423차 가드 **장 마감 후** 커밋(`c64efd6`) — 그 세션은 가드 없이 돌았고 진입 21건 전부 qty=1 |
| 08-04 | 4 | qty=1 가드 가동. qty≥2 우회는 424차가 **일부러** 열어 둠(그날 qty=2 진입 5건) |
| 08-05 | 1 | `would_suppress=0` — **정상 봉중** |
| 08-06~07 | **0** | 431차로 qty≥2 급감(08-07 1건) + 틱 경로 선점 |

**13건 중 8건(61.5%)이 가드 이전**이다. n이 25에 도달해도 그 판정은 "억제 후 회복"이
아니라 **혼합**이다. 억제 후 표본은 5건/2거래일뿐.

**조치**: `guard_live_date`(2026-08-04) 경계로 pre/post 분리 집계 + 일자별 발생 추이
(**[48-T]**) + `prereg_contaminated` 경고. **verdict 계산식·`fav_rate_band`·
`min_samples` 전부 무변경** — 439차 P1과 같은 규율(§9, 313차). 가시성만 바꿨다.

⚠ 최근 발생은 **합계가 아니라 일자별 시퀀스**(`8 / 4 / 1 / 0 / 0`)로 낸다 —
합계 하나로 내면 "최근 5일 13건"이 되어 가드 이전 날이 섞인 채 오독된다.
`bar_dry_days`(마지막 발생 이후 무발생 거래일)를 별도로 노출한다.

### 🔴 손익 오독 차단 — `합계pt`를 원화로 환산하지 말 것

이번 세션 초반 내 보고가 봉중 −24.51pt를 "08-03 +548,500원 / 08-04 +596,500원 유리,
08-04 순익의 79%"로 읽었다. **틀렸다.** `price_hint`는 봉 고저가에서 유도한 **허구의
스톱가**이고 주문은 시장가라 손익과 무관하다 — 435차 docstring이 이미 (a)로 명시하고
있었다. 슬리피지 지표만 오염된 값이다.

실제 손익 기여 추정은 **두 개가 3배 이상 어긋난 채 미해결**이라 리포트에 병기했다:
- 424차: 08-04 qty≥2 3건 실현 +8.76pt(+433,609원) vs 반사실 0pt
- 435차: 1분봉 반사실 재생 9건 통산 **≈+3.0pt**, 08-05 억제 5건은 억제가 +0.38pt 유리

둘 다 사전등록 판정이 아니다. **라이브 재판정 경로는 [35-M] 미러뿐**이다.
(여기서 재도출하지 않은 이유: 1분봉 반사실 재생은 [47] SUSPEND의 재현충실도
의심 범위에 든다.)

### 자체 정정 — 439차 P1의 [35] `structural_reason`

초판 문구 "유령 청산은 qty≥2 경로에서**만** 났다" / "qty≥2 진입 **자체가 사라졌다**"는
둘 다 과했다. **08-03 유령 8건은 전부 qty=1**이었고(가드 이전), qty≥2 한정은
**가드 배포 이후**에만 성립한다. qty≥2도 사라진 게 아니라 급감이다(08-07 1건).
→ 문구 수정 + 리포트에 정정 각주를 남겨 [48-T]로 교차참조.

### 부수 — `math.comb` py37 비호환 (435차 잔재)

`_binom_two_sided`가 `math.comb`(3.8+)를 쓰는데 그 주석은 *"scipy 없이도 돌게"*라고
이식성을 의도하고 있었다. py37_32에서 `AttributeError`로 죽어 2환경 테스트가 불가했다.
→ `_comb()` 폴백 추가. **n≤80 전조합 + 경계(k<0, k>n, C(0,0))에서 `math.comb`와
완전 일치 실측 확인** — 3.8+ 동작 무변경.

### 검증

py_compile 2환경 rc=0 · `tests/test_439_p3_bar_stop_split.py` **26검사 2환경 통과**
(pre/post 분리·오염 플래그 참/거짓·표본 0 ZeroDivision·**verdict 불변성**·시퀀스
비풀링·dry_days 3케이스·실 DB 스모크) · 439차 P1·424차 테스트 무회귀 ·
`--out-dir` 검증본 대조에서 **채널 verdict 변화 0** (바뀐 것은 [48] 마커뿐).

**관련**: 439차 P1([35-M]), 435차([48] 신설·손익 주의), 431차(qty≥2 급감), 424차(우회
유보·+433,609원), 423차(가드 `c64efd6`), [47](재현충실도), 313차·§9 원칙.

---

## 2026-08-07 (MW0602 439차 — 0807 일일점검: 채널 2개가 경계값에서 판정을 잃고 있었다)

**지시**: 0807 일일점검(기동~EOD+P8) → 진입 승패 딥다이브(313차) → P1 구현.
**성격**: 계측·리포트 계층. **라이브 신호·주문·가드 경로 무변경.**

### 0807 실적 — 방향은 맞았고 또 청산에서 졌다 (이틀 연속)

지수 **−13.96pt**(988.10→974.14) 하락일에 **17포지션 전량 SHORT**, 11승 6패
(**64.7%**)로 **−0.46pt / −49,358원**. 전량 SHORT는 결함이 아니다 — 5세션 연속
하락장이고 앙상블 신호도 SHORT 144 : LONG 45로 갈렸다.

기동 08:40:01 → 15:40, `ERR-FATAL 0 / CRITICAL 0`. EOD 7단계 전부 rc=0, P8 정상,
피처셋 실이상 0.

### 경로 분해 — 기하는 정상, 진 것은 미전환 손실의 **크기**

07-20~08-07 **125포지션**(417차·435차 규약):
```
P(보호전환)=57.6% · P(TP2|전환)=16.7%(+5.38pt) · 스크래치 +0.68pt · 미전환 −1.39pt
경로 기대값 +0.2493 pt/건 · 손익분기 P(TP2|전환)=7.5% ← 현행 16.7% (2.2배 여유)
```
오늘은 **비율이 오히려 더 좋았다**(전환 64.7%, TP2|전환 18.2%). 나빴던 건 크기 —
미전환 **−2.90pt vs 기준선 −1.19pt(2.4배)**, 기여도 −0.53pt/건으로 지배적.
손절 미준수가 **아니다**(실현/의도 = 95~105%, 전부 qty=1). 고ATR 오전
(09:49~10:45, 985→965)에 ATR 3.3~3.6짜리 손절폭 2건이 그대로 맞았다 —
**설계대로다.** → 기하 변경 제안 없음(436차와 동일 결론, 표본만 125건으로 확대).

### 🔴 결함 A — [35] 채널이 자기 표본을 버리고 있었다 (모집단 소멸)

리포트가 🔴 NO-DATA("배선 확인 필요")를 띄웠으나 **배선은 정상**. 퍼널 실측:
```
적재 16행 → would_suppress=1 : 15 → +live_suppressed=0 : 0 → 판정대상 : 0
16/16 전부 tighten_path='arm_tp1_qty1_mode', quantity>=2 : 0행
```
[35] 주판정은 `live_suppressed=0`(가드가 못 막아 유령 청산이 실제로 난 건)만 센다.
그 경로는 실질적으로 qty>=2의 `tp1_breakeven`뿐이었는데, **431차의 MAX_CONTRACTS
10→3 + 게이트 체인 재구성으로 qty>=2 진입이 사라졌다.** 가드를 켜지 않았는데도
모집단이 소멸한 것이라 **기다린다고 생기지 않는다** — [28]과 같은 구조적 판정불가.

**조치**: (a) 0건의 원인을 "적재 0(=🔴 배선 점검)"과 "모집단 소멸(=⚠구조적판정불가)"로
분리, (b) `_phantom_mirror_subsample()`이 **억제된 쪽**에서 같은 Δ를 반대 방향으로
재어 **[35-M]로 병기**. 주판정 합격선·반사실 정의·verdict 어휘는 **무변경**.

**미러 실측 15건/3일** (부호 규약 통일: (+)면 유령 유리):
| | 합계 | 평균 | 중앙값 |
|---|---|---|---|
| 유령청산했다면 | +24.78pt | +1.65 | +1.56 (**15/15 전부 양수**) |
| 버틴 실제 | +29.08pt | +1.94 | +0.94 |
| **Δ** | **−4.30pt** | −0.287 | **+0.580** (유령 유리 9/15) |

⚠ **평균과 중앙값의 부호가 갈린다** — 평균이 억제 편인 건 오늘 2건(+5.32/+3.64,
가드가 오늘 TP2 두 건을 살렸다) 때문이고 일자합은 +0.38/−1.88/+5.80(3일, p=1.0).
**판정 불가**이며 `mean_median_split` 플래그로 리포트에 상시 노출된다.
→ 424차의 "유령이 유리해 보이니 유보"는 **흔들렸다**(중앙값 기준 억제 유리).

**미러를 주판정과 합치지 않는 이유(리포트에 명시)**: ① 반사실 비대칭(미러 실제값은
억제 이후 TP2·트레일·15:10 전 청산로직과 교락) ② 시점 교락(유령 발생기 vs 억제기 —
436차가 `827bd04` 교락으로 [50]을 PRE/POST로 등록하지 **않은** 것과 같은 함정)
③ `cur_price` 근사는 실측 유령 슬리피지(평균 −2.708pt 유리)만큼 유령을 **과소평가**.
→ verdict를 내지 않는다. 미러용 합격선을 신설하지 **않은 것은 의도**다(사후 기준
이동 금지, 313차). 판정 승격은 §9 주간회의 안건.

### 🔴 결함 B — [1] TB 이월(carry-forward)이 **경계값 0에서만** 뚫려 있었다

384차가 심은 carry-forward는 `n < min_samples_hz` 분기 **안에만** 있었는데,
그보다 앞선 `if not rows: continue`가 **OOS 행이 정확히 0일 때** 먼저 걸려
이월이 통째로 건너뛰어졌다. "표본이 적다"는 살리고 "표본이 없다"는 죽이는 가드다.

**어떻게 드러났나 — 내가 밟았다.** [35] 수정 검증차 리포트를 재생성했더니
[1]이 `FAIL` → `INSUFFICIENT`로 조용히 강등됐다. 원인은 EOD 체인의 **순서**다:
```
15:47:37 재학습 완료 → 15:47:40 리포트(1m n=1844 / 3m n=1194 → FAIL 판정·로그 기록)
                     → 15:48:57 섀도우 TB 재학습 (모델 mtime 이 시각으로 갱신)
```
체인 안에서는 지난주 mtime이 보여 정상 판정되지만, **체인이 끝난 뒤 재생성하면
OOS가 0이 되어 판정을 잃는다.** `tb_verdict_log`(id 48·49)에는 그날의 FAIL이
멀쩡히 남아 있는데도 리포트만 잃는 것이다 — 리포트 재생성은 `--out-dir` 주석이
명시적으로 상정한 정상 작업이므로 언제든 재발할 수 있었다.

**조치**: 이월 적용을 `_tb_apply_carried()`로 뽑아 **두 자리(n=0 / n<min) 공통**으로
호출. 이제 n=0이어도 `FAIL (유지, 2026-08-07 판정)`으로 이월되고 IC 값도 보존된다.
5m~30m은 최근 판정이 없어 기존 그대로 "첫 판정 대기"(동작 무변경).

> ⚠ **자체 정정**: 검증을 `--out-dir` 없이 실제 경로로 돌려 EOD 산출물을 한 번
> 덮었다. `--out-dir`는 바로 이 용도로 있는 플래그다(그 도움말이 "재현·검증용
> 임시 출력에만 쓸 것"이라고 적혀 있다). 결함 B 수정 후 재생성해 [1]은 복구됐고
> IC 값도 원본과 동일하다. 남은 잔차는 `이번 실행 resolve: N건 → 0건`뿐인데
> 이는 EOD 실행이 이미 resolve를 끝냈기 때문이며 **누적 판정 건수는 무변화**다.
> `tb_verdict_log`에는 재실행이 행을 추가하지 않았다(최신 id 48·49 = 15:47:41).

### 그 외 관측 (조치 없음)

- **[17] 청산 슬리피지** — 틱스톱 78이벤트 **+0.0664pt/건 불리 = 모델가정 0.02pt의
  3.3배**, 누적 +5.18pt≈259,000원/9거래일. 단 **일자단위 t=1.161(p≈0.28), 불리일
  7/9로 미유의** → 왕복비용 상수 변경 금지(372차가 이상치로 임계값을 바꿀 뻔한 것과
  같은 함정). 판정 승격은 표본 도달 후.
- **봉중(유령) 하드스톱 경로**는 08-05 이후 **0건**. 그 이전 유리 슬리피지는
  08-03 +548,500원 / 08-04 +596,500원(그날 순익의 79%). [35-M]이 이 부호를
  재판정할 유일한 경로다.
- **438차 경고 ④(ui_prefs 10 vs MAX_CONTRACTS 3)는 라이브 리스크가 아니었다** —
  `position_sizer.py:223`이 `min(raw_qty, MAX_CONTRACTS)`로 먼저 클램프하고
  ui_prefs는 `main.py:7514`에서 추가 min()으로만 걸려 실효 상한은 항상 3이었다.
  ui_prefs는 11:31에 3으로 정정됨 → NEXT_TODO 438차 ③ 종결.

### 검증

py_compile 2환경 rc=0 · `tests/test_439_phantom_mirror.py` **20검사 2환경 통과**
(부호 규약 LONG/SHORT·주판정 모집단 배제·다중레그 합산·판정이전 레그 제외·
mean_median_split 참/거짓·verdict 미생성·실 DB 구조적판정불가) ·
`--out-dir` 검증본 대조에서 **의도 외 변경 0** · 재생성 후 [1] `❌ FAIL` 복구 및
IC 값(0.0004/0.0495, −0.0087/0.0001) 원본 일치 · metrics json에
`phantom_stop_edge.mirror`(15건)·`tb.horizons.{1m,3m}.carried=true` 적재 확인.

**관련**: 436차(경로 분해·[50] 교락 회피), 435차([47]·[49]), 431차(MAX_CONTRACTS
10→3 — 결함 A의 원인), 424차([35] 신설·유보 결정), 423차(가드), 417차(포지션 단위),
384차(TB 이월 — 결함 B의 원본), 357차(NO-DATA 구분), 313차·372차 원칙, [28] 선례.

---

## 2026-08-06 (MW0601 438차 — 자본금 단일 출처화 + 죽은 설정 3건 적발)

**지시**: `SIZING_TARGET_CAPITAL_KRW` 외에 자본금 설정이 필요한 곳이 있는지 조사 → 단일 출처화 구현.
**성격**: 설정 계층. **현재 값에서 동작 무변화**(아래 검증 참조).

### 계기 — 모의계좌 재발급으로 자본이 4.9억 → 5천만으로 바뀌었다

| 날짜 | 계좌 | 잔고 |
|---|---|---|
| ~08-05 | 333042073 | **489,346,422** (4.89억) |
| 08-06~ | 333044256 | **49,880,660** (4,988만) |

**시스템 어디에도 이 사실이 드러나지 않았다** — 사람이 로그를 대조해서야 알았다.
CLAUDE.md ⑧의 전제(`SIZING_TARGET_CAPITAL_KRW=5천만`은 "모의 4.9억이 실전과 너무
달라서" 넣은 보정)가 **무효화**됐다. 이제 실제 잔고와 목표자본이 사실상 일치해
`SIZING_TARGET_CAPITAL_ENABLED`는 no-op에 가깝다. ⑧ 재검토 시 반드시 반영할 것.

### 적발한 결함 3건

**① `daily_loss_pct` 분모가 하드코딩** (main.py 6454/6820/8259)
```
max(-당일손익, 0) / max(잔고, 50_000_000)
```
분모에 5천만 하한이 박혀 있어 **실전 자본이 5천만 미만이면 손실률을 과소평가해
2% 진입차단이 늦게 걸린다.** 모의 잔고가 마침 4,988만이라 증상이 없었을 뿐이다.
또한 사이징은 5천만 기준인데 손실률은 실잔고 기준이라 **내부적으로 불일치**했다
(4.9억 시절 2% 게이트가 실질 9.8배 느슨).

**② `DAILY_LOSS_LIMIT_PCT`는 죽은 설정** — 소비처 **0건**. 실판정은
`checklist.py:231`의 리터럴 `0.02`. 값을 바꿔도 아무 일이 안 일어난다.

**③ `strategy_params.py` 레지스트리가 라이브와 불일치**
| 키 | 레지스트리 | 라이브 | 경위 |
|---|---|---|---|
| `max_contracts` | 10 | **3** | 431차에 settings만 인하 |
| `account_base_risk` | 0.010 | **0.03** | 339차에 settings만 상향 |

`account_base_risk`는 **`high=0.020`을 넘어 허용범위 밖**이라 옵티마이저가 라이브
값을 탐색조차 못 했다. 이 레지스트리는 `param_optimizer`/`hotswap_gate`가 참조하므로
**백테스트·핫스왑이 실제와 다른 값으로 판단**하고 있었다.

### 구현 — `config/capital.py` (신규 198줄)

`sizing_capital()` / `daily_loss_ratio()` / `base_risk_krw()` /
`daily_loss_limit_krw()` / `check_consistency()` / `capital_profile()`.
소비처 **6곳** 교체(position_sizer, main.py 3곳, checklist, 검증리포트 스크립트).

### 🔴 연동하지 않기로 한 것 — 재론 시 근거 필요

- **`MAX_CONTRACTS` 자동 파생 금지.** 431차가 **손익 카운터팩추얼**로 정한 값이다
  (cap=1 +99만 / cap=2 +169만 / cap=3 +53만 / cap=5 −233만 / 무제한 −828만).
  자본 공식으로 파생하면 **실측으로 얻은 안전 상한을 산술식이 덮는다.**
  CLAUDE.md ⑧도 수동 결정으로 규정한다. → `check_consistency`가 **경고만** 한다.
- **`ACCOUNT_BASE_RISK`(3%)·`DAILY_LOSS_LIMIT_PCT`(2%) 연동 금지.** 리스크 "정책"
  (비율)이라 자본과 독립이어야 한다. 자본에 연동하면 `base_risk = 자본 × 비율`에서
  **이중 반영**된다.

### 기동 정합성 로그 + 경고 4종 (신규)

```
[Capital] 기준자본=50,000,000원(target) | 실제잔고=49,803,312원 | 괴리=-0.4% |
          base_risk=1,500,000원(3.0%) | 일일한도=1,000,000원(2.0%) |
          MAX_CONTRACTS=3 최대노출=147,354,000원 (레버리지 2.95x)
```
① 실서버인데 `ENABLED=True` → CRITICAL (437차 ServerType 판정 활용)
② 실제잔고 vs 기준자본 괴리 ≥50% → WARNING (**오늘의 4.9억→5천만이 여기 걸린다**)
③ 레버리지 >5배 → WARNING ④ `ui_prefs.max_entry_qty` 불일치 → INFO
**자동 조치 없음** — 자동 보정은 437차에 제거한 계좌 대체 폴백과 같은 부류다.

### 검증

- 사이저 qty: 잔고 5종 × conf 3종 **전부 동일**. 리포트 `base_risk` 1,500,000 동일.
- `daily_loss_ratio` 구/신: 잔고 ≤5천만 **SAME**, 잔고 >5천만 **의도적 DIFF**
  (신 공식이 사이징 기준과 일관 = 더 엄격 = 안전한 방향).
- 레지스트리 3개 `registry == live` 확인. 대시보드 스모크 3/3. py_compile 6파일.
- 라이브 기동 `[Capital]` 정상 출력.

### 자체 정정 1건

초판은 기동 시점에 파이프라인 가격이 없어 최대노출이 항상 0 → **레버리지 경고가
구조적으로 발동 불가**였다. DB 최근 종가 폴백 추가로 0원 → 147,354,000원(2.95x) 확인.
**"구현했지만 영원히 안 걸리는 경고"가 될 뻔했다** — 첫 기동 로그를 보지 않았으면 못 잡았다.

---

## 2026-08-06 (MW0601 437차 후속 — 런처가 STEP 4 직후 죽었다: 배치 파싱 함정 2종)

**지시**: 작업 스케줄러 미륵이 실행이 안 된다, 점검하라.
**성격**: 🔴 **437차가 만든 회귀.** 배포 직후부터 무인 기동 불가 상태였다.

### 증상 — 재현율 100%

```
[STEP 4] ... 준비 대기
[OK] ... 준비 확인          ← 런처 로그가 여기서 끝(16줄). preflight 로그 없음
```
18:38/18:39/18:48 세 번 모두 동일. 작업 스케줄러 `미륵이` Last Result **255**.

### 원인 2가지 — 둘 다 437차에서 유입

**① 주석 안의 홑따옴 느낌표.** 이 배치는 `SETLOCAL EnableDelayedExpansion`으로 돈다.
**REM 줄이라도** 짝 없는 느낌표가 있으면 CMD가 지연확장 참조로 보고 **다음 느낌표까지
여러 줄을 통째로 삼킨다.** 437차가 `IsConnect!=1`이라고 적은 것이 그것.

**② 한글 주석 40여 줄 추가.** 파일은 UTF-8인데 새 콘솔은 CP949로 시작하고, 3행
`CHCP 65001` 이후 CMD가 멀티바이트 주석에서 **바이트 정렬을 잃는다.** 콘솔 실측:
```
'경로다' is not recognized as an internal or external command,   ← REM 본문이 명령으로 실행
. was unexpected at this time.                                  ← 다음 괄호 블록에서 파서 사망
```

### 🔴 방법론 교훈 — 검증 방식이 실제 기동 경로와 달랐다

437차 드라이런은 PowerShell의 `cmd /c`로 돌렸는데 **거기선 코드페이지가 달라 우연히
통과**했다. 실제 기동(작업 스케줄러/더블클릭 = 새 콘솔 창)과 다른 경로로 검증한 것이
문제였다. **배치 검증은 반드시 `Start-Process cmd`(새 콘솔)로 할 것.**

### 수정

- 437차가 추가한 주석 **전부 ASCII로 교체 + 분량 축소**. 상세 근거는 배치가 아니라
  dev_memory에 둔다. 파일 상단에 재발 방지 경고 명시.
- STEP 4의 CMD 라벨/GOTO 폴링 루프 제거 → `check_cybos_account.py --wait-connect`
  한 번 호출로 대체(파싱 취약면 축소). GOTO는 이번 원인이 **아니었다** — 제거 후에도
  동일 증상이 재현돼 가설이 기각됐고, 그 다음에 콘솔 오류를 잡아 진짜 원인을 찾았다.
- 검증: 실패 재현 방식 그대로(새 콘솔) **16줄 → 28줄**, preflight·RECHECK 통과.

### 미해결 (기존 코드)

`RECHECK` 블록 부근에서 `'x' is not recognized` 1건이 여전히 뜬다. 배치를 죽이지는
않으며 437차가 건드린 구간이 아니다. 같은 뿌리이므로 근본 해결은 **파일 인코딩
재설계**(CP949 전환 또는 전면 ASCII 주석화)이고 별건으로 남긴다.

---

## 2026-08-06 (MW0601 437차 — 잘못된 계좌로 91분 운영: 계좌·서버 오접속 방어)

**지시**: 일일점검 → 계좌 폴백 딥다이브 → 서버 구분 버튼 점검 → 계좌 대조 구현.

### 사고 — 설정 계좌가 아닌 555011215로 91분 운영

```
08:43:50  [Account] ACCNO raw=555011215      ← 설정은 333042073
          구 코드가 조용히 accounts[0]으로 대체
          잔고 0 → 09:30~09:47 신호 8건 전부 MarginBlock
10:15     계좌 정상화(333044256) 후 오후에 진입 재개
```
**BrokerSync가 `97007 모의투자 데이터가 없습니다`를 "정상 무포지션"으로 판정**한 것도
문제였다("계좌가 잘못됐다"와 "보유가 없다"를 구분 못 함).

### 근본 원인 — autologin이 "연결됨"만 보고 성공 판정

`cybos_autologin.py`는 `IsConnect==1`만으로 성공을 판정하고, `autologin()` 진입 시
"이미 연결됨 → 로그인 생략"이라 **잘못된 계좌 세션을 스스로 복구하지 못한다.**

전체 이력 전수조사 결과가 결정적이다:
| 항목 | 횟수 |
|---|---|
| autologin 실행 / 연결 성공 | 43 / 38 |
| **'모의투자 접속' 버튼 정상 클릭** | **0** |
| **blind Enter (무표적 `send_keys`)** | **423** |

**38번의 성공이 전부 blind Enter로 이루어졌다.** 설계된 버튼 클릭 경로는 한 번도
발동한 적이 없다. 같은 파일 1513행에 "비가시 Edit에서 send_keys는 다른 창에 전달될 수
있어 금지"라는 주석이 있는데, blind Enter 경로에는 그 보호가 없다.

당일에는 **autologin 2개 프로세스가 동시 실행**됐다(08:35:10 LAUNCH_API + 08:40:24
start_mireuk). 스케줄 간격 5분 < autologin 최악 6분(3회×120초)이라 **구조적으로 겹친다.**

### ServerType 실측 확정 — 코드베이스가 정반대로 적혀 있었다

실서버 접속 상태에서 직접 조회: **`ServerType=2`**. 따라서 **1=모의 / 2=실서버**.
| 파일 | 판정 |
|---|---|
| `api_connector.py:452` (런타임) | ✅ 맞음 |
| `cybos_plus_preflight.py:44` / `cybos5_preflight.py:49` | ❌ **뒤집힘** |

런처 로그가 **모의를 "실거래"로, 실서버를 "모의투자"로** 기록해 왔다. 후자가 위험한
방향이다(로그만 보고 실서버를 모의로 착각).

### `is_server_match()`는 죽은 코드였다

2026-05월부터 존재했으나 **호출자 0**. 함수 docstring과 화면 라벨(`⚠ 실접속=… · 진입차단`)이
둘 다 차단을 약속하는데 실제로는 빨간 글씨만 뜨고 주문은 나갔다. `_ts_execute_entry`에 배선.

### `TradeInit` 무응답의 정체 — 주문오브젝트 사용 동의 미설정

실서버에서 `TradeInit(0)=-1`. 그때 CYBOS Plus가 띄운 모달:
```
CPTRADE — 주문오브젝트 사용 동의를 하지 않으셨습니다.
```
**이 모달은 클릭 전까지 무한 대기**한다. 같은 날 아침 preflight가 120초를 넘겨 멈춘 것도
동일 원인이었다(무인 기동이라 아무도 확인을 누르지 않았다). **워치독이 필요한 이유가 이것.**

### 배포

- 계좌 `accounts[0]` 폴백 제거 + `_ts_execute_entry` 하드게이트 + **래치 fail-open 수정**
  (`accounts`가 비면 차단이 풀리던 것을 "일치를 확인했을 때만 해제"로 비대칭화 —
  `get_account_list()`는 예외를 삼키고 `[]`를 반환하며 이 함수는 매분 호출된다)
- `ServerType` 상수 단일 출처(`config/constants.py`) + preflight 2개 반전 교정
- `scripts/check_cybos_account.py` 신설 — 2단계 검사(미연결이면 `TradeInit` 미호출)
  + 워치독 20초(`os._exit` — COM 블로킹 중엔 예외/`sys.exit` 무효)
- preflight `CHECK 4/4` 계좌 대조, exit 4=불일치 / 5=타임아웃. **자동 채택 없음**
- `start_mireuk.bat` STEP 4를 준비 대기로 교체(로그인 책임을 LAUNCH_API 한 곳으로),
  STEP 0 제거, 구버전 런처 2개 삭제

### 자체 정정 1건

안내문에 em-dash(U+2014)를 넣었다가 cp949 콘솔에서 `UnicodeEncodeError`. 단순 출력
깨짐이 아니라 **안전 검사의 종료코드가 바뀌는 문제**다(exit 2/4를 돌려줄 자리에서
트레이스백으로 죽으면 런처가 오진). em-dash 제거 + `make_output_safe()` 가드 추가.

---

## 2026-08-06 (MW0602 436차 — 0806 일일점검: 방향은 맞고 청산에서 졌다 + 계측 결함 1건)

**지시**: 0806 일일점검(기동~EOD+P8) → 진입 승패 딥다이브(313차 방법론) → 개선안 구현.
**성격**: 계측·위생 계층. **라이브 신호·주문 경로 무변경.**

### 0806 실적 — 방향은 하루 종일 맞았는데 졌다

지수 **−36.14pt(−3.55%)** 하락일에 **23포지션 전량 SHORT**(전량 qty=1·단일레그),
16승 7패(**69.6%**)로 **−6.34pt / −351,055원**.
...
승리 16건 중 **14건이 본전 긁기**(보유 중앙값 **51초**). 보유구간 내 MFE 33.42pt 대비
**캡처율 −19.0%**. 대표: 09:41:48 SHORT@1005.86 → 진입 6초 뒤 보호전환 → **29초 만에
+0.42pt** 종료, 이후 976.12까지 **추가 29.74pt**.

### 🔴 그러나 "기하가 못 이기는 구조"는 기각 — 108포지션 재검증

07-20~08-06 포지션 단위(417차·435차 규약) 재집계: **총 +31.62pt**.

```
P(보호전환)=62.0% · P(TP2|전환)=19.4%(+5.16pt) · 스크래치 80.6%(+0.70pt) · 미전환 −1.79pt
경로 기대값 +0.293pt/건 · 손익분기 P(TP2|전환)=8.8% ← 현행 19.4%
```

구조는 **손익분기 위**다. 오늘이 진 것은 P(TP2|전환)이 12.5%로 평균 이하였고 고ATR로
미전환 손실이 −2.63pt(평균 −1.79)였기 때문. 435차 [49] edge_gap 양수와 일관.
→ **기하 변경 제안 없음.** [47]이 SUSPEND인 동안 [25]/[25-B]/[46]/[3-B]는 근거가 아니다.

### 가설 3건 전부 탈락 (반증 기록)

1. **"틱 TP1이 러너를 죽였다"** → **판정 불가(교락)**. TP2 완주율 34.8%→11.4%
   Fisher p=0.0472지만 **일자단위 Mann-Whitney p=1.00**(PRE TP2 8건이 3일에 6건 군집 —
   Fisher가 군집을 무시해 p 과소평가), 이상치 2일 제외 시 +1.254 vs +1.120pt로 동일.
   **더 근본적으로**: `TP1_TICK_ENABLED` 배포 커밋 `827bd04`(07-31)는 430차가 지목한
   **conf passthrough 원인 커밋과 같다**. 진입 모집단이 갈렸다(36건 전부 C/conf 0.3611
   → 72건 C63+X9/conf 0.4303, 일평균 1~6 → 11~23건). **처치가 둘이라 표본이 쌓여도
   식별되지 않는다.**
2. **"추세일에 불리"** → **반증**. Spearman(|순변동|,건당실현)=**+0.313**, (레인지)=+0.462 — 부호 반대.
3. **"다빈도 진입일이 손실"** → **미유의**. Mann-Whitney p=0.329, 다빈도 6일 중 4일이 최근 4세션(시점 교락).

### 🔴 신규 계측 결함 — `synthetic_partial_exits.ts` 분 절삭

`main.py`가 `strftime("%Y-%m-%d %H:%M:00")`으로 **초를 리터럴 "00"**으로 박았다.
`entry_ts`는 초 해상도라 **67행 중 10행이 물리적으로 불가능한 음수 지연**(−3~−52초).

**피해는 "부정확"이 아니라 정보 부재다.** 진입은 매분 파이프라인 끝(초 53~58)에 나므로
같은 분 arm은 기록지연이 `60 − 진입초` = 3~7초로 **항상** 나온다 — arm 속도가 아니라
다음 분 경계까지의 잔여시간이다. **"POST 중앙값 8초"는 틱 감지가 8초 걸렸다는 뜻이 아니다.**

⚠ **`ts`를 초로 고치면 안 된다** — `scripts/tp1_protect_offset_shadow.py:240`이
`idx.get(ts)`로 raw_candles **분봉 인덱스 키**로 직접 쓴다. 고치면 [25]·[25-B] 표본이
조용히 0이 된다. → `ts` 보존 + **`arm_ts` 신규 컬럼**으로 분리. 소급 불가(기존 67행 NULL).
417차(레그≠포지션)·435차(파서가 다중레그 누락)와 같은 계열.

### 430차 §5 부분 반증 — 게이트는 수렴, 진입률은 신고점

`base_mc` 0.3971→**0.4151**로 투영(~0.418) **거의 달성**. 그런데 진입률은 되돌기는커녕
5.7→3.0→4.6→**6.2%** 신고점. 또 430차가 지목한 "붕괴행이 진입 예산 잠식"은 반대였다 —
08-03~08-06 진입 72건 중 붕괴행 **1건(1.4%)** vs 전체 붕괴율 21.2% = **리프트 0.07x**로
**강하게 배제**되고 있다. ⚠ 08-11 판정창 이전이므로 **관측 기록이지 판정이 아니다**(430차 §7).

### 구현 4건 (전부 계측·위생, 라이브 무관)

| # | 내용 | 라이브 영향 |
|---|---|---|
| P1-a | `synthetic_partial_exits.arm_ts` 초 해상도 컬럼 (멱등 ALTER + INSERT) | 없음(기록만 1컬럼 추가) |
| P1-b | [44]에 **붕괴행 진입 리프트 + 일자별 진입률** 병기 (`_entry_collapse_lift`) | 없음(verdict 산식 무변경) |
| P2 | **[50] `tp1_arm_latency_watch`** 사전등록 — POST 내부 상관 설계 | 없음(계측) |
| P3 | `crash_fault.log` 크기 로테이션 + **주석 정정** | 기동 시 1회 파일 회전 |

- **P1-a**: `ts`(분)와 `arm_ts`(초)는 **같은 `_arm_now`**에서 파생 — 따로 `now()`를
  부르면 분 경계에서 1분 어긋난다.
- **P1-b**: `_load_conf_rows`는 [45]와 공유하는 5-튜플이라 원소를 추가하면 양쪽 언팩이
  깨진다 → **독립 쿼리로 분리**(회귀 0).
- **P2**: 🔴 **PRE/POST 시간 비교로 등록하지 않았다** — `827bd04` 교락으로 영구 판정
  불가이기 때문. 대신 같은 코드 세대 안에서 (arm 지연)↔(TP2 완주) 상관을 보며,
  역인과(빠른 TP1 도달 = 고변동 국면 대리변수) 방어로 **ATR 3분위 층화를 필수**로 뒀다.
  `data_start=2026-08-07`(arm_ts 배포일), min_samples=30/min_days=10.
  ⚠ SUPPORTS_HYP가 나와도 **`TP1_TICK_ENABLED`를 끄라는 뜻이 아니다** — 403차가 실측
  근거(MFE 캡처율 9.7%)로 넣은 것이고, 조임 **폭** 조정이 더 직접적인 레버([25] 축)다.
- **P3**: 🔴 `dump_traceback_later(30, repeat=True)`는 주석과 달리 **행 여부와 무관하게
  무조건 30초마다** 덤프한다. 실측 확정 — 오늘 세션 6.9시간에 **840회** vs 무조건 주기
  기댓값 **828회** 일치(누적 22,609회 ≈ 97세션 × ~830회). 파일 44.7MB의 정체가 이것이며
  **행이 2만 번 난 것이 아니다**(419차 "죽은 상수"와 같은 주석-실동작 괴리).
  **동작은 그대로 두고**(30초 해상도가 230차의 진단 목적 그 자체) 크기만 관리:
  `CRASH_LOG_ROTATE_MB=8` / `KEEP_GENERATIONS=4` ≈ 5주분, 0 이하 = 킬스위치.
  회전은 **파일을 열기 전에** 해야 한다(faulthandler가 fd에 직접 쓴다).

### 검증

py_compile 2환경 rc=0 · DB 마이그레이션 실행 + **멱등 재실행 OK**(67행/arm_ts NULL) ·
INSERT 컬럼12=플레이스홀더12 + 왕복 실측(`ts` 17:44:**00** / `arm_ts` 17:44:**54**) ·
**[25] 재실행 n=65 무변화**(ts 분봉 조인 무영향) · 리포트 생성 119,508자, [44] 신규 2행
렌더 확인 · `tests/test_436_crash_log_rotation.py` **5/5 통과 2환경**(세대 순서·최고참
탈락·킬스위치·미존재 파일).

⚠ **대시보드 offscreen 스모크는 segfault(exit=139)** — 단 `git stash`로 **HEAD에서도
동일 재현**되므로 이번 변경과 무관한 기존 현상이다(import까지는 exit=0).
`feedback_pyqt_offscreen_testing` 메모가 "생성까지 가능"이라 했는데 지금은 아니다 —
별도 조사 대상으로 NEXT_TODO 등록.

**관련**: 435차([47] 게이트·[49]), 432차 후속2(기하 채널 선행조건), 430차(§5 투영·[44]),
428차(방향 축 무작위), 425차(스톱은 옳았다), 417차(포지션 단위), 403차(`827bd04`),
313차 원칙, 230차(faulthandler).

---

## 2026-08-05 (MW0602 435차 — 계측기를 계측하는 게이트 + 시뮬 무관 채널 2종 신설)

**지시**: 기하 채널 제거/유지 손익 조사 → 딥다이브(①②③) 반영 신규 채널 제안 → 구현.
**성격**: 캠페인 계측 계층 신설. **라이브 신호·주문 경로 무변경.**

### 제거 vs 유지 — 정지가 답이다

- **제거 비용은 작다**: 소스 3테이블(`synthetic_partial_exits`·`exit_fill_slippage`·
  `phantom_stop_shadow`)은 전부 `main.py`가 기록한다. 채널은 읽는 오프라인 스크립트일
  뿐이라 **데이터 손실 0**이고 소급 재실행이 된다.
- **유지 비용은 크다**: 매주 FAIL 4건이 회의 안건화된다. **계측 방식만 바꿔도 결론
  격차 61.51pt = 307만원**이고 [25-B] `tp2_2.00`은 부호가 뒤집힌다(+14.76 → −12.13).
- **그러나 삭제는 §9를 해친다** — 결과를 본 뒤 채널을 지우면 "불리한 결과를 묻었다"로
  읽힌다. → **정지(SUSPEND)**: 사전등록·데이터·수치는 전부 보존하고 verdict만 덮는다.

### [47] `sim_fidelity_gate` — 계측기를 계측하는 메타 게이트 (②)

기하 A/B 채널들은 같은 재생기 가정(1분봉 / TP2 종료 / TP3·트레일링 미모델링)을
공유한다. 정본([23-B]의 `current`)이 실제를 재현하는지 **포지션 단위로 조인**해 잰다.

```
지표: repro_bias_R = median((sim − actual)/R)   ← 372차(중앙값)·R 정규화
      sign_match   = sign(Σsim) == sign(Σactual)
PASS → 하위 채널 정상 / SUSPEND → verdict를 SUSPENDED로 대체
INSUFFICIENT → 게이트를 **열어 둔다**(근거 없이 막지 않는다, 313차)
```

**최초 실측: bias −0.002R(양호)인데 총합 부호 불일치(시뮬 −20.05 vs 실제 +34.66pt)
→ SUSPEND.** [3-B]·[23-B]·[25]·[25-B]·[46]이 정지됐다.
**해제는 자동이다** — 재생기에 TP3·트레일링을 편입해 재현이 회복되면 PASS로 바뀐다.
절대원칙 §2의 CB②·CB③-P4·FP-CRITICAL이 전부 "재검토하기로 했는데 안 함"이었던
패턴에 대한 **구조적 해법**이다.

### [48] `bar_stop_path_watch` — 봉중 하드스톱 경로 건전성 (①)

`exit_fill_slippage`를 경로별로 갈라 슬리피지 **부호 대칭성**을 본다. 시뮬 무관.
현재: 봉중 13/13 유리(p=0.00024) vs 틱 17/42(p=0.28) → **판정은 INSUFFICIENT**
(봉중 n=13<25, 3일<8 — 정직하게 보류). `stop_close`는 hint가 유효하므로 **BAR와 분리**했다.

**🔴 `price_hint`는 지금 고치지 않는다 — 세 가지 이유(재론 시 근거 필요)**:
1. **§9 위반** — 사전등록으로 재려는 지표를 관측 직후 정의만 바꿔 PASS로 만드는 꼴.
   [25] 클램프와 다르다(그건 *체결 불가능한 가격* 계상이라 물리적 오류였다).
2. **손익이 안 바뀐다** — 주문은 `_send_broker_exit_order()`의 **시장가**이고
   hint는 브로커에 전달되지 않는다(main.py:9209 확인).
3. **순수 계측값이 아니다** — Chejan 콜백 유실 시 평균가 폴백으로도 쓰인다
   (main.py:2823 `_flush_pending_exit_agg`). 드물지만 라이브 복구 경로다.
→ 진짜 처방은 424차 억제이고 이미 배포됐다. 이 채널이 **억제가 듣는지**를 검증한다.

### [49] `payoff_geometry_watch` — 문제 자체의 상시 계측 (③의 판단 기준)

**"현행 기하가 애초에 이길 수 있는 구조인가"를 묻는 채널이 하나도 없었다.**
시뮬 무관(`trades` 실측), 포지션 단위(417차), **이중지표**(원화 / 위험조정 R).

### 🔴 자체 정정 — 로그 파서가 다중레그 2번째를 버렸다

[49]를 DB 기준으로 짜자마자 **432차/432차 후속2의 로그 파서 버그**가 드러났다.
`[체결진입]`을 다음 `[체결청산]` 하나와만 짝지어 **다중레그 포지션 10건의 2번째
레그(+12.60pt)를 통째로 누락**했다.
...
→ **"정확히 손익분기선 위에서 돈다"는 서술은 무효다.** 위험조정 기준 +2.61%p,
원화 기준 +5.71%p의 우위가 있다. 다만 두 기준 격차 3.10%p가 임계(3.0)를 넘어
**결과가 소수 고변동일에 집중**됐다는 경고가 함께 뜬다(07-29 2포지션 +21.54pt).

⚠ **영향 없는 것**: §4-3의 기하 산술(하드스톱 1.5ATR vs 보호 0.25ATR = 1:6,
손익분기 85.7%)은 설정 직독이라 무관. 0805 당일 17건은 전부 단일레그라 무관.
[47] 게이트의 결론은 오히려 **강화**된다(괴리 37.47 → 54.71pt).

**교훈 — 로그 파싱은 DB의 대체재가 아니다.** 417차가 "`trades` 행은 청산 레그"를
경고했는데, 이번엔 그 레그를 **읽지도 못하는** 파서를 썼다. 포지션 단위 집계가
필요한 분석은 처음부터 `GROUP BY entry_ts`로 갈 것.

### 검증

py_compile 2환경 rc=0 · %-포매팅 AST 전수 0건 · 3채널 단독 실행 OK ·
리포트 생성 70,428자(신규 3섹션 중복 0) · 대시보드 offscreen 스모크 OK ·
`verdict_raw`/`per_variant` 보존 확인(4·6·4개 변형).

**관련**: 432차·432차 후속2, 423·424차(유령 억제), 417차(레그 단위), 313차·372차 원칙,
MW0601 432차([25-B])·434차([46]).

---

## 2026-08-05 (MW0602 432차 후속2 — 기하 채널은 아직 판정 도구가 아니다 + 자체 정정 2건)

**지시**: "설계기하를 섀도 검증하는 채널을 확인하고, 그 결과 반영으로 승률 58.8%인데
−44만원인 문제를 개선할 수 있는지 조사."
**결론**: **지금은 판정할 수 없다 — 비교의 양쪽이 다 오염돼 있다.**

### ① 채널 4종은 전부 FAIL이지만 합산 불가

[23-B] `sym_1.0_1.0` +4.91 / [3-B] `q_raw` +1.42 / [25] `atr_lock_0.75` +8.04(클램프 후) /
**[25-B] `tp2_2.00` +14.76**(MW0601 432차 신설). 같은 포지션에 겹치는 기하를 각자
시뮬레이션하므로 delta를 더하면 안 된다 — 전 생애주기 통합 시뮬 결과 단일축 합 +22.20 vs
3축 동시적용 **+18.40**으로 **−3.79pt 상호잠식**이 실재한다.

### ② 채널 기준선이 실제를 재현하지 못한다 (괴리 −37.47pt)

```
실제 실현 (109포지션 07-14~08-05)  +19.05pt
통합 시뮬 현행 기하                −18.42pt
[23-B] current (같은 모집단)       −20.05pt
```
봉내 도달순서를 양극단으로 둔 브래킷 **[−58.49, −18.42]가 실제를 감싸지 못하고**
모호봉은 1/109건뿐 — **해상도 문제가 아니다.**

구간 분해: 07-14~26 n=48 괴리 **+10.50**(슬립 미기록) / 07-27~08-05 n=61 괴리 **+26.98**
(유리슬립 +19.35). → 원인이 **둘 겹쳐 있다**: 미모델링 메커니즘(TP3·4단계 트레일링) +
유리 청산 슬리피지.

미모델링 크기: 실제 TP2 청산 14건 평균 **+5.25pt** vs 시뮬 동일건 **+2.64pt** →
**러너 1건당 2.61pt 과소평가**. 이 프리미엄으로 보정하면 순위가 재배열되고
**[25-B] `tp2_2.00`은 +6.17 → −12.13으로 부호가 뒤집힌다**(러너 7건 감소).

### ③ `stop_intrabar` 딥다이브 — 봉중 하드스톱만 계통적으로 유리하다

| 경로 | n | 합계pt | 평균 | 유리비율 | 부호검정 p |
|---|---|---|---|---|---|
| **봉중 하드스톱** | 13 | **−24.51** | −1.885 | **13/13** | **0.00024** |
| 틱 하드스톱 | 42 | +2.12 | +0.050 | 17/42 | 0.280 |
| 목표(TP1/TP2/손절1차) | 13 | −0.38 | −0.029 | 7/13 | 1.000 |

*(음수=유리)* 봉중 vs 틱 Mann-Whitney **p<1e-6**. **틱은 대칭(정직), 봉중만 전건 유리.**
...
price_hint = exit_price ; hint_source = "stop_intrabar" → 시장가 체결
```
08-04 12:04 실례: 12:03:52 진입 SHORT@976.50 → 12:04:26 TP1로 보호스톱 976.50(본전) →
12:04:52 봉 마감 점검이 `bar_high ≥ 976.50`으로 히트 판정(**그 고가는 12:04:26 이전에
발생** — 스톱이 그 자리에 있기 전이다) → 12:04:53 시장가 973.12 체결 **+3.38pt**.
보호스톱이 본전이라 정상이면 0.00pt여야 하고, 기록된 이익은 **전부 유리 체결분과 일치**한다.
423차가 진단한 유령 하드스톱이며 424차 억제(`phantom_stop_shadow`)의 대상이 바로 이것.

### ④ 자체 정정 2건 — 회계 분해를 반사실로 오독했다

**정정 A (이 세션 조사 발표)**: "실현손익의 146%가 슬리피지", "qty=2 사다리 실질
+2.75pt"라고 발표했으나 **틀렸다**. 올바른 반사실은 "체결가가 hint였다면"이 아니라
**"유령이 발동하지 않았다면"**이다 — 억제되면 포지션은 계속 달려 다른 곳에서 청산된다.

1분봉 반사실 재생(08-04 유령 4건): 실현 +12.78 vs **억제 시 +9.42** → **유령 순기여
+3.36pt**(회계상 유리 체결분 +11.93이 아니다). 2건은 억제됐다면 **TP2에서 오히려 더
벌었다**(+3.79 / +4.78).

그리고 **08-05는 억제 배포 후라 직접 관측된다** — 억제 5건에서 유령 발동 시 +6.36 vs
실제 **+6.74** → **억제가 오히려 +0.38pt 유리**. 9개 이벤트 통산 유령 순기여 ≈ **+3.0pt**.
→ **"423/424차 억제가 흑자를 없앤다"는 우려는 근거 없다.** 억제는 옳고 손익 영향은 중립.

**정정 B (0805 리포트 §4-5)**: "08-04 qty=2 5건 +12.08pt로 사다리가 작동했다"의
**크기는 유효**하다(반사실 +11.89, 유령 기여 +0.19pt뿐). 부정확했던 것은 **서술**이다 —
잔량 레그 3건은 "TP2까지 러너가 달린" 것이 아니라 **유령이 중간에 끊은 것**이다.
TP1 부분청산 레그 4건(+1.69/+1.24/+2.28/+1.92)은 슬리피지 ≈0으로 진짜다.

### 무엇이 남았나

**여전히 확립된 것**: 0805 리포트 §4-3의 산술 — 하드스톱 ATR×1.50 vs 보호스톱 ATR×0.25
= 1:6, 손익분기 승률 85.7%. `config/settings.py`와 로그에서 **직접 읽은 값**이라 위
오염과 무관하다.

**우선순위 (계측 먼저, 그다음 배선)**:
1. **봉중 하드스톱 경로의 체결가 산정 재설계** — 424차 억제는 발동을 막을 뿐,
   `_prev_stop_price` + `min(stop, bar_high)`로 체결가를 유도하는 설계는 그대로다.
   틱 경로가 대칭인데 봉중만 13/13 유리하다는 것은 이 경로가 시장을 반영하지 않는다는 뜻 —
   **실투에서는 부호가 반대로 나올 수 있다.**
2. **시뮬레이터에 TP3·4단계 트레일링 편입** — 그래야 채널 판정이 의미를 가진다.
3. 그 둘이 끝난 뒤 기하 변경 논의.

⚠ **표본 한계**: 봉중 경로 13건/8거래일, 유령 반사실 9건. **강한 의심이지 확정 아님**(313차).
다만 경로별 비대칭(틱 42건 대칭 vs 봉중 13건 전건 유리, p=0.00024)은 우연이기 어렵다.

**교훈 — 회계 분해는 반사실이 아니다.** "이 손익의 몇 %가 X에서 왔나"와 "X가 없었다면
얼마였나"는 다른 질문이고, 전자로 후자를 답하면 이번처럼 4배 과장된다. 반사실은
**대안 경로를 실제로 재생**해야 나온다.

**관련**: 423·424차(유령 하드스톱 진단·억제), 432차([25] 클램프), MW0601 432차([25-B]),
313차 원칙, `docs/정기점검/매일점검/MW0602-20260805-점검리포트.md` §7.

---

## 2026-08-05 (MW0602 432차 — [25] 채널이 체결 불가능한 가격을 체결로 계상했다)

**성격**: 계측 결함 수정 + 상수 3중 정의 단일화. **라이브 동작 무변경.**
**계기**: 0805 일일점검에서 [25] `tp1_protect_offset_shadow`가 사전등록 임계를 넘어
FAIL을 냈고, **0801 기각 사유까지 소멸**해 08-08 주간회의 승격 후보가 된 것을 확인.
그 근거를 검산하다 발견.
...
채널 설명서 §9가 *"[25]는 아직 반증된 적이 없다 — MW0602의 drop-max 취약성이
유일한 제동"* 이라 적은 그 유일한 제동이다. 남은 것은 판독규칙 (3)(MW0601 표본
미충족)뿐이었고, MW0601 08-03 스냅샷은 **같은 방향**(+7.76pt·우세 10/14)이라
표본이 차는 순간 규칙 (1) 승격 후보가 되는 상태였다.

### 결함 — offset에 상한이 없었다

`protect_offset`은 진입가에서 **이익 방향으로** 띄우는 거리다. `_offset_for()`에
상한이 없어 보호전환 **시점에 실제로 도달했던 유리이동**을 넘는 값이 나올 수 있었고,
그러면 보호스톱이 당시 가격보다 더 이익 쪽에 놓인다. `_simulate()`는 다음 봉에서
`hit_stop`을 무조건 True로 읽으므로 **시장이 한 번도 제시하지 않은 가격에 체결된
것으로 계상**된다.

**왜 이 변형에서만 터지나 — 기하가 애초에 불가능하다.** 보호전환은 TP1 트리거
(`ATR × {1m 0.3, 3m 0.5, 5m 0.7} × hurst`)가 **닿는 순간** 걸린다. 따라서 그 시점
유리이동은 구조적으로 ATR×0.3~0.7 근방이다 — 실측 **중앙 0.552×ATR**(49건).
`ATR × 0.75` lock은 정의상 그 위이며 **얻을 수 없는 이익을 잠그라는 지시**다.

| 변형 | 체결 불가 | 비율 | 초과폭 중앙 |
|---|---|---|---|
| `current`(ATR×0.25)·`breakeven`·`breakeven_plus` | 0 | 0% | — |
| `atr_lock_0.50` | 5 | 10% | 0.33pt |
| **`atr_lock_0.75`** | **28** | **57%** | 0.55pt |
| `bar_range` | 14 | 29% | 0.19pt |
...
```python
excursion = abs(synthetic_price - entry_price)   # 보호전환 시점 실제 유리이동
off = min(_offset_for(v, atr, bar_rng), excursion)
```

| 변형 | 클램프 無 Δ현행 | **클램프 有** | 허수 기여 | 일자단위 p |
|---|---|---|---|---|
...
일자단위 p=0.255로 확립되지 않는다(11거래일, 그중 4일은 Δ=0.00).
...
### §9 저촉 없음 — 합격선을 바꾸지 않았다

`min_samples`·`min_days`·PASS/FAIL 판정식·변형 6종의 이름과 의미 **전부 무변경**.
바뀐 것은 시뮬레이터가 물리적으로 불가능한 체결을 계상하지 않게 된 것뿐이며, 이는
판정 기준이 아니라 **계측 버그 수정**이다. 전례: 417차(청산레그 단위 오류),
423·424차(PSI 백필 오염) — 모두 관측 후 계측 결함을 고쳤고 시계 리셋 대상이 아니었다.

⚠ **자기유리 편향이 아님을 수치가 보증한다** — 이 수정은 대안의 우위를 **3배 줄이는**
방향이다. 고쳐서 유리해진 것이 아니다.

### 함께 수정 — `TP1_PROTECT_*` 3중 정의 (419차 "죽은 상수"와 동일 계열)

```
main.py:201-202                          TP1_PROTECT_PLUS_ALPHA_PTS / _ATR_LOCK_MULT = 리터럴
dashboard/main_dashboard.py:59-60        같은 이름 리터럴 사본 (툴팁 표시용)
scripts/tp1_protect_offset_shadow.py:73  _LOCK_MULT_CURRENT = 0.25 사본
config/settings.py                       **없음** — 그런데 §25 사전등록 주석은 인용 중
```

`from config.settings import TP1_PROTECT_ATR_LOCK_MULT` → **ImportError**(실측 확인).

**왜 지금 위험했나**: [25]가 바로 이 값의 변경을 회의 안건으로 올리는데, 한 곳만
고치면 ① 대시보드 툴팁이 거짓을 말하고 ② 섀도 `current` 기준선이 라이브와 어긋나
**이후 이 채널의 모든 A/B 판정이 무효**가 된다.
→ `config/settings.py`로 단일화하고 세 곳을 import로 교체. **값 변경 없음(0.20/0.25).**

### 리포트 생성기 — 매주 거짓을 찍고 있었다

`generate_validation_campaign_report.py`의 [25] 상세 섹션에 0801 경고문
("최대 기여 1건만 빼면 부호 역전, 건별 우세 8/23·7/23")이 **하드코딩**돼 있었다.
그 문장은 08-05 기준 전부 거짓이다. 새 제동(일자단위 미유의 + 판독규칙 (3))으로
교체하고, 요약행에 **클램프 발동 건수 태그**를 추가했다 — 요약표만 보고 회의에
올리는 경로가 실제로 있기 때문이다.

**교훈 — "제동이 풀렸다"가 곧 "승격하라"는 아니다.** 0801 기각 사유가 소멸한 것은
사실이지만, 그 사유를 재검산하는 과정에서 판정 재료 자체가 결함이었음이 드러났다.
**제동이 풀린 채널일수록 근거를 먼저 검산해야 한다** — 그때가 가장 위험한 시점이다.

**관련**: 404차 후속3([25] 신설), 406차(모드 통일), 421차 후속2(PC 판독규칙),
419차(죽은 상수), 417차·423차·424차(관측 후 계측결함 수정 전례), 313차 원칙,
`docs/정기점검/금요일점검/0803_Ch25_..._채널설명.md` §10,
`docs/정기점검/매일점검/MW0602-20260805-점검리포트.md` §5.

---

## 2026-08-05 (MW0601 434차 — [46] HurstGate 임계 위치 A/B 신설 + P0-2 근거 2건 자체 정정)

**지시**: 0805 일일점검 P0-2 — "HurstGate 조기 재보정 검토하고 개선방향 제안" → Phase 0~2 구현.
**성격**: ① 내가 제기한 근거의 자체 반증 ② 검증 채널 신설(오프라인, 라이브 무변경).

### 먼저 — P0-2를 제기하며 내가 쓴 근거 2건이 틀렸다

**① "실측 hurst는 임계 0.45를 한 번도 넘지 못한다" → 거짓.**
SIGNAL 로그의 `[HurstGate]` 줄은 **게이트가 발동할 때만**(=hurst<0.45) 남는다. 거기서
분포를 읽은 것이 선택편향이었다. 무조건부 모집단(`ensemble_decisions.features`,
14,185 사이클) 실측:
...
**② "min() 합성 후 이 게이트 하나가 사이징 전체를 결정한다" → 과장.**
발동 시 `quality_min`을 지배하는 것은 맞으나 **결과를 바꾸는 건 일부**다.
`entry_gate_json` 실측 no-op 비율:

```
431차 이전 (~08-04)  applied 132 / no-op 126 = 95%
431차 이후 (08-05)   applied  21 / no-op  15 = 71%
```

431차로 영향도가 오른 것은 사실이지만(95%→71%), 오늘 실제로 수량을 깎은 건 6사이클·
진입 2건이다. "사이징이 상수에 고정된다"는 표현은 과했다.

### 그럼에도 남는 진짜 문제 — 임계가 분포의 중앙에 있다

```
HURST_RANGE_THRESHOLD = 0.45
hurst 중앙값          = 0.452     ← 거의 동일, 발동률 49.4%
```

레짐 분류기의 임계가 추정량 분포 중앙에 있으면 시장 상태와 무관하게 구조적으로 절반을
"횡보"로 라벨링한다. **이건 알려진 편향이다** — `settings.py`가 "진짜 랜덤워크 H=0.5가
평균 0.446으로 찍히는 문제"로 이미 기록하고 있다.

**317차와 겹치지 않는 이유**: 317차는 추정량 **파라미터**(N 60→90, max_lag 20→9)를
재보정하면서 임계는 *"이 파라미터 그대로 검증됐으므로 변경하지 않음"*으로 **명시적으로
손대지 않았다.** 즉 임계는 이 추정량의 실제 분포에 대해 **한 번도 보정된 적이 없다.**

in-sample 스윕(07-14~ 현 사이징 체제, n=101/14일)이 그 공백을 보여준다:
...
차단 구간의 손실이 임계를 낮출수록 깊어진다 — 현행은 진짜 나쁜 하위 20%에 **멀쩡한
40%를 얹어서** 자르고 있다. 5분위로 보면 Q1(0~0.358) 승률 51.4%/-2.30pt는 나쁘지만
Q2(0.358~0.450)는 승률 60.0%/+4.51pt로 멀쩡한데 차단당한다.

⚠ 단, hurst vs 손익 rho=+0.1234(t=+1.64), 일자단위 rho=-0.2977(t≈-1.71)로 **둘 다
유의수준 미달**이다. 방향만 일관될 뿐 이 데이터로 임계를 확정할 수 없다.

### 조치 — [46] `hurst_threshold_shadow` 신설 (Phase 0~2)

**Phase 0 (오프라인 스윕)**: `scripts/hurst_threshold_shadow.py`.
`ensemble_decisions.features.hurst` + `trades`만으로 전부 재구성 — 라이브 회귀 위험 0,
과거 표본에 소급 적용된다.

**측정 지표를 손익 카운터팩추얼이 아니라 "분리도"로 잡은 이유** (설계 핵심):
HurstGate는 333차 후속부터 차단이 아니라 **사이징 ×0.5**다. 임계를 바꾸면 수량이
바뀌므로 손익 재현에 게이트 체인 전체(min 합성·안전군·상한) 재시뮬이 필요하고 재현
오차가 측정하려는 효과보다 커진다. 대신

```
sep(t) = mean_pt_per_contract(hurst >= t) − mean_pt_per_contract(hurst < t)
```

를 잰다. 계약당 정규화라 **사이징과 무관**하고(417차 교훈 — `quantity`는 청산레그
수량), "축소해야 할 구간이 실제로 더 나쁜가"에 직접 답한다.

**분위 후보를 함께 재는 이유**: **371차 선례** — RegimeFingerprint PSI가 균등폭 bin과
실제 분포 불일치로 상시 CRITICAL에 고착했고 분위 비닝으로 해소됐다. 분위 임계는
절대값을 안 쓰므로 추정량 편향과 무관해진다. 롤백은 직전 10거래일이며 **당일 제외**
(당일을 넣으면 실시간 재현 불가 = 룩어헤드).

**Phase 1 (사전등록)**: `VALIDATION_CAMPAIGN["hurst_threshold_shadow"]` —
후보(절대 0.30/0.36/0.39/0.42/0.45 + 분위 10/20/30%), min_samples=40, min_days=10,
`oos_start=2026-08-06`. 판정문 관측 전 고정:
PASS = OOS에서 어떤 후보도 **분리도 우위 + drop-worst 생존**을 동시에 못 만족 /
FAIL = 만족하는 후보 존재 → **즉시 적용 아님, 주간회의 수동 결정**.

**Phase 2 (판정 장치)**: 리포트 `[46]` 요약행 + 상세 섹션 + 판정보조 3종
(drop-worst / 일자단위 / PC 교차). **자동 적용 코드는 없다** — Phase 2의 "구현"은
사람이 결정할 재료를 빠짐없이 같은 화면에 올리는 것까지다.

### in-sample 결과 (⚠ 판정 아님 — OOS n=0)

```
후보        차단n  발동률  차단평균  통과평균   분리도  drop-worst
abs_0.39     45   25.6%  -0.391  +0.945  +1.336     +1.182
abs_0.45★    71   40.3%  -0.021  +1.026  +1.046     +0.945   ← 현행
q20          29   17.2%  -0.642  +0.912  +1.554     +1.322
q30          47   27.8%  -0.430  +1.060  +1.490     +1.344
q10           7    4.1%  +1.071  +0.627  -0.445     -1.320
```

현행 0.45의 **차단구간 평균이 -0.021pt** — 사실상 아무것도 못 가르고 있다.
q10이 음수(-0.445)인 것은 좋은 소견이다: "낮을수록 무조건 나쁘다"가 아니라 비단조
구조가 있다는 뜻이라, 단순한 단조 아티팩트가 아님을 시사한다(단 n=7).

### 26주 WFA 조기 실행은 불필요하다는 결론

내가 P0-2를 제기한 근거가 위처럼 과장이었고, no-op 71%라 영향이 제한적이다. 게다가
WFA 재검증 항목은 **N/max_lag의 FalseBlock 최저구간 확인**이고 이 채널은 **임계 위치**라
질문 자체가 다르다. → **WFA 일정은 그대로 두고 임계 축만 별도 채널로 분리한다.**

### ⚠ 재시도 금지 방향 (코드·주석에 박아둠)

추정량 **편향보정**(상수이동·선형 de-shrinkage)은 317차가 60거래일 실측에서
**FalsePass 14.4% → 30~33% 악화**로 이미 폐기했다. `HURST_DESHRINK_TABLE`이 코드에
남아 있으나 REFERENCE ONLY이며 라이브 미참조다. 이 채널은 추정량을 고치지 않는다.

**검증**: py_compile 양 환경 rc=0. 스크립트 단독 실행 정상(전체표본 8후보 산출, OOS
표본 없음으로 INSUFFICIENT). `summarize()`가 렌더링 payload를 함께 반환하도록 다른
오프라인 채널과 규약 통일. 리포트 `[46]` 구간을 **소스에서 추출해 격리 실행**해
예외 없음·볼드 중첩 0건 확인 — 전체 리포트 실행은 432차 후속(BLAS 파손)으로 여전히
**미검증**이며 환경 복구 후 재확인 필요.

**Why**: 이번 세션에서 내가 만든 근거 오류가 **두 번 다 같은 유형**이었다 —
431차의 "안전군 발동 0건"(집계 대상 미한정)과 내 "hurst가 0.45를 안 넘는다"(발동 시에만
남는 로그에서 분포 추론). 둘 다 **조건부 표본을 무조건부로 읽은 것**이고, 둘 다 그럴듯한
문장으로 기록돼 검증된 것처럼 읽혔다. 로그는 대부분 "무슨 일이 일어났을 때" 남으므로
**로그에서 분포를 읽으면 거의 항상 편향된다** — 분포는 매 사이클 무조건 저장되는
곳(`ensemble_decisions.features`)에서 읽어야 한다.

**How to apply**: (a) "X가 한 번도 없었다/항상 그렇다" 류의 주장은 **모집단을 명시**하고
그 모집단이 조건부인지 먼저 확인할 것. 로그 기반이면 십중팔구 조건부다. (b) 그 확인을
코드 주석에 남길 것 — 이 채널은 `_load_hurst()` docstring에 "SIGNAL 로그에서 뽑지 말
것, 0805에 실제로 그 오독이 났다"를 박아뒀다. (c) 내가 제안한 방향을 내가 검증할 때는
**반대 방향 후보를 함께 넣을 것**(432차 [25-B]의 `tp2_2.00`, 여기서는 q10이 그 역할).

**관련**: 317차(N/max_lag 재보정 + 편향보정 반증), 333차 후속(하드차단→사이징 ×0.5),
371차(분위 비닝 선례), 372차(임계 재보정이 이상치에 무너진 선례 — drop-worst 필수),
431차·433차(min 합성이 이 게이트 영향도를 올림), [3-6] hurst_gate_shadow(다른 모집단),
[23] tp1_geometry_shadow(oos_start 구조 선례), 313차 원칙, §9 사전등록.

---

## 2026-08-05 (MW0601 433차 — 431차 "안전군 발동 0건" 근거 반증 + pre_retrain 품질군 편입)

**지시**: 0805 일일점검 P0-1 — "431차 안전군 근거 정정하고 품질군 편입으로 코드 수정하고
곱셈으로 유지 해제되도록" 구현.
**성격**: ① 근거 정정(431차 오기) ② **라이브 사이징 정책 변경**(사용자 수동 결정).

### 발단 — 431차의 안전 논거가 실측으로 반증됐다

431차는 안전군을 곱셈으로 유지하면서 그 안전성을 **"최근 20거래일 안전군 발동 0건"**
으로 정당화했다. 0805 일일점검에서 "시뮬 2계약+ 37% vs 실측 16%"의 원인을 추적하다
그 주장이 `pre_retrain`에 대해 거짓임을 확인했다:

```
SIGNAL 로그 "[EntryGate] 사이즈 축소 ×0.6"  (2026-07-01~08-05)
  → 160사이클 / 15거래일 발동
  → 07-03 1 · 07-07 25 · 07-08 7 · 07-10 5 · 07-13 19 · 07-15 17 · 07-20 15
     07-21 35 · 07-22 1 · 07-23 1 · 07-27 2 · 07-28 3 · 08-03 6 · 08-04 15 · 08-05 8
  → 그중 24사이클에서 실제 진입 발생
```

나머지 3종(Degraded·VolBurst·L2)은 발동 0건이 **맞다** — 오기는 `pre_retrain` 한 항목이다.

**왜 이게 결정적인가**: 431차의 목표는 "곱셈 중첩으로 계약수가 1에 수렴하는 것"의 해소였다.
그런데 `pre_retrain`이 안전군에 남아 **군 경계를 넘어 이중 축소·이중 반올림이 살아남았다**:

```
08-05 09:49 실측:  Sizer 3계약
  안전군: 3 × 0.6 = 1.8 → round → 2      ← 1차 축소 + 1차 반올림
  품질군: 2 × min(0.90, 0.50) = 1.0 → 1  ← 2차 축소 + 2차 반올림
  결과 1계약 (431차 의도대로면 3 × min(0.6,0.90,0.50) = 1.5 → 2계약)
```

431차가 명시한 설계 원칙 "**단일 반올림**"이 자기 코드 안에서 지켜지지 않고 있었다.

### 왜 pre_retrain은 품질군인가 (분류 타당성)

431차의 분류 기준은 "같은 축의 중복 계상 = 품질군 / 독립 위험 = 안전군"이다.
`_pre_retrain_size_factor`의 실체는 **"오늘 첫 GBM 재학습이 아직 안 끝났다"는 시간 조건**
이다 — 시장 위험이 아니라 **모델 최신성**, 즉 "지금 이 신호를 얼마나 믿을 수 있나"다.
그건 Meta(메타신뢰도)·Hurst(레짐적합)와 **정확히 같은 축**이고 431차 기준으로 품질군이다.
안전군에 남는 Degraded(시스템 헬스)·VolBurst(급변장)·L2(호가 깊이)는 신호 품질과 독립인
진짜 하자드라 그대로 둔다.

### 조치

- `config/settings.py` — `PRE_RETRAIN_SIZE_QUALITY_GROUP: bool = True` 신설.
  **되돌림 플래그**다(False → 431차 원안 곱셈). 코드 수정 없이 즉시 복구할 수 있게
  한 이유는 아래 "노출 증가" 때문 — 손익이 나빠지면 그날 중에 꺼야 한다.
  (CB③-P4·FP-CRITICAL 예외 플래그와 같은 관례)
- `main.py` — `_quality_mults["pre_retrain"] = 0.6` 등록으로 전환. 분류표 주석 갱신 +
  431차 오기에 정정 블록. 로그에 `[품질군 min 합성]` / `[안전군 곱셈]` 병기 — 배포 직후
  "왜 수량이 늘었나"를 추적할 유일한 단서다.
- `dashboard/main_dashboard.py` 툴팁 · `settings.py:VALIDATION_CAMPAIGN_DECISIONS
  ["sizing_prescription_axis"]` 정정.
- **부수 정리**: 로그의 `'조건부 진입 구간'` 분기는 **도달 불가능**이었다
  (`_pre_retrain_size_factor < 1.0` 가드가 `_pre_retrain_done == False`를 함의하므로
  else가 실행될 수 없다). 죽은 문자열을 제거했다 — 09:20~09:29 구간은 `min_conf` 상향만
  하고 사이즈는 안 건드린다(6499행 주석의 "size×0.5"도 미구현 잔재).

### ⚠ 이 변경은 노출을 늘린다 — 손익 근거는 없다

재시뮬(대상 24건, 품질군 배수를 게이트 원로그에서 재구성):

| 지표 | 값 |
|---|---|
| 수량이 바뀌는 건수 | **10/24건 (42%)** — 전부 +1계약 |
| 해당 구간 총노출 | 24 → **34계약 (+41.7%)** |
| 전체 진입 대비 대상 비중 | 24/116 = **19.8%** |

단조성 검사에서 확인했듯 **433차는 431차보다 결코 작아지지 않는다** — 순수한 증량이다.
CLAUDE.md ⑧(실전 자본 재설정)이 미해제인 상태에서 "진입 증가 = 노출 증가" 원칙에
정면으로 걸린다.

**그 구간이 좋은 구간이라는 증거는 없다**:

```
pre_retrain 구간   n=23  승률 52.2%  +14.78pt  +701,171원   8거래일
그 외              n=93  승률 57.0%  -18.14pt  -18,074,359원  16거래일
```

누적은 흑자지만 **이항 p=1.000**(동전과 구분 불가)이고 **일자단위로 8거래일 중 5일이
음수**다 — 소수 대박일이 만든 값이라 313차 원칙상 "증량해도 되는 구간"의 증거가 아니다.
**이 변경의 근거는 분류 타당성 하나뿐이며, 손익은 중립 가정이다.**

### §9 경로에 대한 기록

이건 라이브 사이징 정책 변경이라 원칙적으로 섀도 계측 → 주간회의 경로다. 431차가
`MAX_CONTRACTS` 10→3을 **"사용자 지시로 주간회의 경로 대체"**한 선례(CLAUDE.md ⑧에
명시)와 같은 경로로 처리했다. 대신 되돌림 플래그와 감시 항목(NEXT_TODO 433차)을 함께 뒀다.

**검증**: py_compile 양 환경 rc=0. **위젯 생성 스모크**(`QT_QPA_PLATFORM=offscreen`으로
`EntryPanel()` + `MireukDashboard()` 생성) 통과 — 431차 후속2가 죽었던 바로 그 위젯이라
py_compile만 믿지 않고 실행했다(NEXT_TODO 431차 후속2 항목을 이번에 실제로 이행). 합성
체인 단위 테스트: 실측 10케이스 + 160조합 그리드에서 **차단(0 유지)·하한(≥1)·상한(≤sizer)·
단조(433 ≥ 431) 위반 0건**, 플래그 False가 431차 동작을 정확히 재현함을 확인.
**라이브 미검증** — 다음 거래일 09:00~첫재학습 구간이 첫 관측이다.

**Why**: 431차는 "실측 확인"이라는 표현을 붙인 문장으로 안전성을 정당화했는데, 그 실측이
실제로는 4개 게이트 중 3개만 확인한 것이었다. **집계 대상을 한정하지 않은 "발동 0건"은
가장 위험한 형태의 근거다** — 반증이 나오기 전까지 검증된 것처럼 읽힌다. 그리고 이번에도
그 오기를 잡아낸 건 코드 리뷰가 아니라 **시뮬 예측과 실측의 괴리(37% vs 16%)를 추적한
것**이었다. 예측을 남겨두지 않았으면 영영 몰랐을 것이다.

**How to apply**: 게이트/배수를 군으로 분류하는 변경에서는 (a) **군별로** 발동 실측을
내고 항목별 건수를 함께 적을 것 — 군 전체를 뭉뚱그린 "0건"은 금지. (b) 배포 시 예상
분포를 반드시 남기고 첫 거래일에 대조할 것(431차가 남긴 `{1:57, 2:25, 3:9}`가 이번
발견의 출발점이었다). (c) 노출을 늘리는 변경에는 손익 근거가 없더라도 **되돌림 플래그**를
같이 넣을 것 — 근거가 분류 타당성뿐이면 실측이 반대로 나올 여지가 그만큼 크다.

**관련**: 431차(원 분류·이 정정의 대상), 431차 후속2(위젯 스모크 교훈 — 이번에 이행),
417차(사이징 축 근거 정정 선례), CLAUDE.md ⑧(노출 원칙), 313차 원칙(일자단위 판정),
§9 사전등록.

---

## 2026-08-05 (MW0601 432차 후속 — py310_64 numpy BLAS 파손: 주간 리포트 생성 불가)

**증상**: `scripts/generate_validation_campaign_report.py`가 py310_64에서 **stderr 한 줄
없이** 종료코드 `-1066598273`(= `0xC06D007F`)로 죽는다. 432차 작업 중 리포트 렌더링을
검증하려다 발견.

**근본 원인**: 내 변경과 무관한 **환경 파손**이다. `git stash`로 변경을 빼고 돌려도 같은
코드로 죽는 것을 확인했고, 최소 재현으로 좁혔다:

```
python -c "import numpy as np; a=np.random.rand(4,4); np.dot(a,a)"   → 0xC06D007F
```

`import numpy`는 정상이고 **BLAS matmul 경로만** 프로세스째 크래시한다. faulthandler
역추적: `eval_vol_estimator_watch`(429차 [43] 채널) → `np.corrcoef` → `np.cov` → `dot`.
즉 [43] 채널이 원인이 아니라 **BLAS를 처음 건드리는 지점**일 뿐이다 — 그 채널을 고쳐도
다른 곳에서 같은 곳에 걸린다.

**왜 EOD 재학습은 멀쩡했나**: 오늘 15:45 EOD(HistGBM·RF, 209.8초)는 정상 완료했다.
sklearn의 HistGradientBoosting/RandomForest는 자체 Cython 커널을 쓰고 BLAS matmul을
타지 않는다. **"py310_64가 살아 있다"를 재학습 성공으로 판단하면 안 된다** — 축이 다르다.

**영향 범위**: 주간 캠페인 리포트(금요일 EOD 체인)가 **생성되지 않는다.** MW0601 폴더의
최신 자동생성물이 `validation_campaign_report_20260731.md`인 것과 일치한다(08-01분은
`_pre405` 수동 스냅샷). CLAUDE.md 규약상 리포트는 **두 PC가 서로를 보는 유일한 통로**라
이게 막히면 PC간 대조 자체가 불가능하다.

**결정**: 이번 세션에서 고치지 않는다 — 원인이 코드가 아니라 conda 환경(MKL/OpenBLAS DLL)
이라 환경 재설치가 필요하고, 그건 **EOD 재학습이 의존하는 바로 그 환경**을 건드리는
작업이다(191차 결정으로 py310_64는 재학습 전용 환경). 장중에 손대면 안 된다.
`NEXT_TODO` 432차 후속 항목으로 등록하고 조치 순서를 함께 남긴다.

**Why**: "리포트가 안 보인다"를 표본 부족이나 채널 결함으로 오독하기 딱 좋은 형태다 —
stderr가 비어 있어 로그만 봐서는 실행 자체가 없었는지 죽었는지 구분되지 않는다.
실제로 이번에도 리포트 생성을 시도하기 전까지 아무도 모르고 있었다(마지막 성공이
07-31분인데 08-01·08-08 두 번의 금요일이 그냥 지나갔다).

**How to apply**: 네이티브 크래시(종료코드가 음수/0xC0.. 대역)는 파이썬 예외가 아니라
`faulthandler`로만 잡힌다. 스크립트가 조용히 죽으면 **먼저 `-X faulthandler`로 다시
돌릴 것**. 그리고 "다른 스크립트는 되니까 환경은 멀쩡하다"는 추론을 하지 말 것 —
BLAS처럼 일부 경로만 파손되는 형태가 실재한다.

**관련**: 429차([43] 채널 — 크래시가 드러난 지점이지 원인 아님), 191차(py310_64 재학습
전용 환경 결정), 407차(리포트 PC 폴더 규약), CLAUDE.md "주간 산출물 위치 규약".

---

## 2026-08-05 (MW0601 432차 — [25-B] TP2 거리 A/B 축 신설: 사전등록 + 오프라인 구현)

**지시**: 0805 일일점검 딥다이브 후속으로 "[23]에 TP2 축을 추가"하라는 사용자 지시.
**성격**: 검증 채널 신설(사전등록) — 라이브 코드 무변경, 오프라인 재생 전용.

### 계기 — TP2 거리 ÷ 손절 거리 = 1.00

0805 진입 19건 전부에서 이 비율이 정확히 1.00이었다. 설정상 필연이다:

```
ATR_STOP_MULT = 1.5   ATR_TP2_MULT = 1.5    (hurst 배수는 양쪽에 곱해져 약분)
```

qty=1은 TP1에서 물리적 분할청산이 불가능해 보호스톱 전환만 일어나므로(98%가 qty=1)
실질 페이오프가 3갈래로 갈리고, **손절폭만큼 완주해야 손절 1건을 상쇄**한다:

```
[A] 진입 → 역행 → 손절              −1.5×ATR      0805 6건
[B] 진입 → TP1 → 보호스톱 되돌림    ≈+0.25×ATR    0805 10건 (사실상 0)
[C] 진입 → TP1 → TP2 완주           +1.5×ATR      0805 3건
```

0805는 A:C = 2:1로 겨우 본전권(+1.22pt)이었고, 18거래일 누적은 116포지션 승률 56.0% vs
손익분기 승률 56.4%로 **분기점 아래**다.

### 지시를 [23]이 아니라 [25]에 구현했다 — 편의가 아니라 정확성 문제

**[23] `tp1_geometry_shadow`에는 TP2 축을 넣을 수 없다.** 그 시뮬레이터는 `TP1 도달 =
전량청산`으로 종료해 TP2가 아예 등장하지 않는다(`_simulate()`가 STOP/TP1/TIMEOUT만
반환). 거기에 `tp2_mult` variant를 추가하면 **계산에 아무 영향 없이 표만 늘어난다** —
있으나 마나 한 채널이 생기는 게 더 나쁘다.

반면 [25] `tp1_protect_offset_shadow`의 `_simulate(bars, i0, is_long, ep, stop, tp2)`는
**이미 `보호스톱 vs TP2`를 재생하며 tp2를 인자로 받는다.** 순수함수 재사용만으로 축이
열린다(363차 원칙 — 시뮬 로직 복붙 금지).

더 결정적인 것은 **조건부 추정이 정확히 valid**하다는 점이다. TP2를 바꿔도 stop·TP1은
불변이므로 stage A(진입~TP1)가 전 변형에서 동일하고, 따라서 "TP1 도달 사건 집합"도
동일하다. **TP1 도달 건만 보는 것은 선택편향이 아니라 교란이 제거된 설계다.**

→ [23] config에 "TP2 축은 여기가 아니라 [25-B]에 있다 — 넣지 말 것"을 교차참조로 박았다.
   같은 지시가 반복될 때 다음 세션이 [23]에 죽은 variant를 만드는 것을 막는다.

### 구현

- `config/settings.py` — `VALIDATION_CAMPAIGN["tp1_protect_offset_shadow"]`에
  `tp2_variants` / `tp2_min_samples`(20) / `tp2_min_days`(3) **관측 전 고정**.
  변형: `current`(=ATR_TP2_MULT 1.5) / `tp2_0.75` / `tp2_1.00` / `tp2_1.25` /
  **`tp2_2.00`(대조군 — 넓히는 방향)**.
  `current`를 상수 하드코딩하지 않고 `ATR_TP2_MULT`를 읽는 이유: 라이브 값이 바뀌면
  기준선도 따라가야 `delta_vs_current`가 의미를 유지한다(offset 축의 `_LOCK_MULT_CURRENT`
  관례와 동일).
- `scripts/tp1_protect_offset_shadow.py` — `_tp2_mult_for()` 신설, `compute()`에
  `rows_tp2` 평행 집계(보호 offset은 현행 고정), `_summarize_tp2()` 독립 판정.
  drop-max·건별우세·표준편차를 **처음부터** 계산해 노출한다.
- `scripts/generate_validation_campaign_report.py` — 요약표 `[25-B]` 행 + 상세 섹션.

**양방향 그리드로 만든 이유**: "좁히면 좋다"만 시험하면 우연한 우위를 반증할 기회가
없다. 넓히는 방향을 대조군으로 넣어 결과가 **거리에 대해 단조인지**(진짜 효과) 아니면
특정 값에서만 튀는지(표본 우연)를 구분한다. — 이 설계가 즉시 값을 했다(아래).

**본채널 오염 방지**: verdict를 `tp2` 별도 키로 낸다. [25] 본채널 합격선 무변경이므로
§9-4 검증 시계 리셋 없음. 추가로 **배선 검산**을 넣었다 — TP2축 `current`와 offset축
`current`는 stop·tp2·시뮬이 전부 같아 수치가 일치해야 하고, 어긋나면 축 하나가 잘못
배선된 것이므로 verdict를 건드리지 않고 사실만 노출한다(조용한 오염 방지).

### 초기 관측 — ⚠ 판정 아님 (n=27 / **2거래일** < min_days 3)

```
변형        n   STOP  TP2완주  누적pt   현행대비   건별우세   drop-max
current    27    18      9    +29.17     —          —          —
tp2_0.75   27    14     13    +22.06   −7.11      4/27      −8.39
tp2_1.00   27    14     13    +27.92   −1.25      4/27      −3.16
tp2_1.25   27    16     11    +29.82   +0.65      2/27      −1.90
tp2_2.00   27    19      8    +34.18   +5.01      8/27      +3.60
```

**가설과 반대 방향이 단조로 나왔다.** TP2를 좁히면 완주 건수는 늘지만(9→13) 누적은
줄고, 넓히면 완주는 줄지만(9→8) 누적이 는다. "완주율↑ / 완주 1건당 이익↓" 교환에서
**후자가 이긴다**는 뜻이다. 유일하게 현행을 넘은 `tp2_1.25`(+0.65)는 drop-max에서
−1.90으로 부호가 뒤집혀 이상치 1건이 만든 것이다.

⚠ **313차 원칙상 확정 금지** — 2거래일(08-03·08-05)이고, 08-03은 일중 레인지 3.24%의
극단 휩소일이다. 기하 대안은 변동성 체제에 민감하다. `verdict`도 INSUFFICIENT로 나온다.
**이건 관찰 신호이지 반증이 아니다.**

**Why**: 내가 0805 보고에서 "TP2를 더 가깝게" 방향을 제안했는데, 만약 단방향 그리드로
만들었다면 `tp2_1.25`의 +0.65만 보고 "약하지만 맞는 방향"으로 읽었을 것이다. 대조군과
drop-max가 같은 화면에 있어서 **제안자 본인의 가설이 즉시 의심 대상이 됐다.**

**How to apply**: A/B 채널을 만들 때 (a) 가설 방향만이 아니라 **반대 방향 대조군**을
반드시 넣고, (b) drop-max·건별우세를 사후 요청이 아니라 **처음부터** 같은 표에 낼 것.
그리고 "지시받은 위치"와 "질문에 답할 수 있는 위치"가 다르면 후자에 구현하고 전자에는
교차참조 + 금지 사유를 남길 것 — 지시를 그대로 따르면 죽은 채널이 생겼다.

**검증**: py310_64/py37_32 `py_compile` rc=0. 스크립트 단독 실행 정상, 배선 검산
`baseline_consistent=True`(양축 current 모두 +29.1665). 리포트 렌더링은 **[25-B] 구간만
격리 실행**해 예외 없음·표 정상 확인 — 전체 리포트 실행은 위 432차 후속(BLAS 파손)으로
막혀 있어 **미검증**이며, 환경 복구 후 재확인 필요.

**관련**: 431차(0805 배포), [12] tp1_trail_shadow(TP1 이후 트레일 폭 — 기각),
[25] 본채널(TP1 시점 보호 offset — FAIL이지만 미적용), [3-B] quantile_tp_shadow(TP1
거리 산정 방식), [23] tp1_geometry_shadow(TP1/손절 초기 기하), 313차 원칙, §9 사전등록.

---

## 2026-08-05 (MW0601 431차 후속2 — 기동 전면 실패: 툴팁 `%` 포매팅 오류)

**증상**: 08:40 장전 자동기동이 `TradingSystem.__init__`에서 즉시 크래시, 런처
자동재시작 5회 전부 동일 지점에서 실패 → **09:00 개장 시점에 프로세스 0개**.
...
**근본 원인**: 431차가 `_TIP_QTY` 툴팁 마지막 줄에 `... (%d계약) ..." % MAX_CONTRACTS`를
추가했다. 인접 문자열 리터럴은 **컴파일 시점에 하나로 합쳐진 뒤** `%` 연산이 적용되므로,
포매팅 대상이 마지막 줄이 아니라 **툴팁 32줄 전체**가 된다. 본문에 있던 리터럴 `%`
(`"기본리스크 = 잔고 × 1%\n"`, `"신뢰도배수 = 58%→0.6"` 등)까지 변환지정자로 해석돼
`%` 다음 문자가 개행(`0xa`)인 곳에서 터졌다. index 169 = `잔고 × 1%`의 `%` 위치.
(에러 메시지의 `'?'`는 개행을 출력하지 못한 것일 뿐 별도 문자가 아니다.)

**수정**: `%` 포매팅 제거, 문자열 결합으로만 값 주입 + 재발 방지 주석.
```python
"  상한: MAX_CONTRACTS (" + str(MAX_CONTRACTS) + "계약) / 하한: 1계약"
```

**교훈 — `py_compile` rc=0은 기동 검증이 아니다.**
431차 검증 항목에 `py_compile py37_32/py310_64 rc=0`이 있었고 실제로 통과했다. 이건
문법 오류가 아니라 `_build()` **실행 시점**의 오류라 정적 컴파일로는 원리상 잡을 수 없다.
단위테스트도 사이저·게이트 로직만 덮었고 대시보드 위젯 생성 경로는 비어 있었다.
→ 대시보드를 건드린 커밋은 **위젯 생성까지 가는 스모크 테스트**가 있어야 한다
(`NEXT_TODO` 431차 후속2 항목).

**동일 유형 잔여 점검**: 431차가 건드린 6개 파일(`main.py`·`main_dashboard.py`·
`settings.py`·`position_sizer.py`·`meta_gate.py`·`db_utils.py`)의 모든 `str % args`
노드를 AST로 훑어 변환지정자 유효성을 검사 → **잔여 0건**. 431차가 새로 넣은
`" quality_min=%.2f" %`, `" [ConfShadow: %.1f→%d계약]" %`는 정상이고 `%d%%` 이스케이프도
제대로 돼 있다.

---

## 2026-08-05 (MW0601 431차 — 계약수 자유도 복원: 무효 근거 정리 + 게이트 체인 재구성)

**지시**: "시장상황에 따라 1~3계약 진입" 설정 조정치 제안 → 승인 후 Phase 0/1/2-1 구현.
**성격**: ① 근거 정정(417차 미반영분) ② 사이징 정책 변경(사용자 수동 결정) ③ 섀도 계측 신설.

### 발단 — 417차 정정이 코드에 반영되지 않았다

417차(2026-08-02)는 `CLAUDE.md` ⑧과 리포트 생성기를 정정했으나 **`config/settings.py`를
빠뜨렸다**. 그래서 무효 수치가 두 곳에서 3일간 사이징 축을 계속 봉쇄했다:

- `VALIDATION_CAMPAIGN["sizing_inversion_watch"]` 계기 주석 — "3계약 이상 16건 승률 6.2%
  / p=5.07e-07 / 카운터팩추얼 완전 단조" 그대로
- `VALIDATION_CAMPAIGN_DECISIONS["sizing_prescription_axis"]` — **결정 자체가** 그 수치를
  유일 근거로 "(a) 기본 계약수 상향은 반증됐다"고 확정

→ **교훈**: 근거 무효화 시 `CLAUDE.md`만 고치면 부족하다. 그 근거를 인용하는
**확정 결정 레지스트리**를 함께 갱신해야 한다 — 레지스트리는 정의상 "판정과 무관하게
사람이 내린 결정"이라 리포트 재생성으로 자동 교정되지 않는다.

### 431차 재확인 (160포지션, 2026-06-10~08-04, `entry_qty` 기준)

| qty | n | 승률 | 총손익 | 계약당평균 |
|---|---|---|---|---|
| 1 | 95 | **50.5%** | +294,686 | **+3,101** |
| 2 | 25 | 64.0% | +3,704,105 | +74,082 |
| 3 | 18 | 61.1% | +914,121 | +16,928 |
| >=6 | 9 | 44.4% | **-16,088,217** | -225,212 |

- 진입계약수 vs 계약당pt **rho=+0.026 (p=0.74)** — 무관. 1~5 구간도 +0.088(p=0.28).
- qty=1의 승률이 1~3 구간 **최저**. "1계약 우월"은 부활하지 않는다.
- ⚠ **역방향도 미확정** — qty=1 vs 2~3 승률차 **z=+1.34 p=0.18**, 07-14 이후 qty>=2 n=7.
  주장 가능한 범위는 **"1계약 고정을 정당화하는 근거가 없다"까지**다.

### 무엇이 계약수를 1로 묶고 있었나 (진입 91건 추적)

사이저는 이미 다양했다 — `[Sizer]` 652건 중 **223건(34.2%)이 3계약 이상**, 최대 10.
병목은 그 뒤 **품질군 게이트의 곱셈 중첩**이었다:

| 게이트 | 발동률 | 평균 배수 |
|---|---|---|
| MetaGate reduce | 82% | x0.71 |
| HurstGate soft | 60% | x0.50 |
| ToxicityGate reduce | 36% | x0.70 |

최빈 조합 `Hurst 0.50 x Meta 0.50 = 0.25` → 사이저 3계약이 0.75 → 반올림 → **1계약**.
raw>=3 31건 중 **20건이 1계약으로 체결**, 전체 진입의 85%가 1계약.

### 조치

**① `MAX_CONTRACTS` 10 → 3** (`config/settings.py`)
포지션 단위 카운터팩추얼: cap=1 +99만 / **cap=2 +169만(최대)** / cap=3 +53만 /
cap=5 -233만 / 무제한 **-828만원**. 손실은 qty>=6 9포지션 -1,609만원에 집중.

**② 품질군 게이트 min() 합성 + 단일 반올림** (`main.py:_compose_quality_qty`)
- 품질군(Exec·Meta·Toxicity·Hurst) = **같은 축의 중복 계상** → min() 합성
- 안전군(Degraded·pre_retrain·VolBurst·L2) = **독립 위험** → 곱셈 유지
- 왜 나눴나: 전 게이트 min()이면 급변장에서 오히려 덜 보수적이 된다.
  실측 확인 — 최근 20거래일 **안전군 발동 0건**이라 이 분리는 관측 구간에서 무영향.

> 🔴 **[MW0601 433차, 2026-08-05] 위 두 줄 중 다음이 정정됐다 — 재인용 금지.**
> - (무효) "안전군(Degraded·**pre_retrain**·VolBurst·L2)"
> - (무효) "최근 20거래일 **안전군 발동 0건**이라 이 분리는 관측 구간에서 무영향"
>
> 실측(2026-07-01~08-05 SIGNAL 로그): `pre_retrain ×0.6`은 **160사이클 / 15거래일**
> 발동했고 그중 **24건이 실제 진입**이다(07-07 25회·07-21 35회·08-04 15회·08-05 8회…).
> "발동 0건"은 나머지 3종(Degraded·VolBurst·L2)에 대해서만 참이다.
> **이 오기가 431차 시뮬(2계약+ 37%)과 08-05 실측(16%)의 괴리를 만들었다** — 사이저
> 3계약이 `3×0.6 → 2 → ×min(quality)0.50 → 1`로 **이중 축소·이중 반올림**됐다.
> 431차가 없애려던 바로 그 현상이 **군 경계를 넘어 살아남았다.**
> → 433차에서 `pre_retrain`을 품질군으로 이동. 조치 ①③과 나머지 안전군 분류는 유효하다.
- 부수 수정 2건: (a) MetaGate take 증량(0.9~1.25)이 `_qty_display`에 직접 곱해져
  **순서 의존**이 있었다 → base에 적용 (b) 그 증량이 사이저 클램프 **이후**라
  상한을 넘길 수 있었다(10 x 1.25 → 13) → `MAX_CONTRACTS` 클램프 추가.

**③ MetaGate 무정보 폴백 → 사이징 중립** (`strategy/entry/meta_gate.py`)
reduce 밴드의 `learned["size_multiplier"] or 0.5`는 `_make_result()`가 conf<0.5에서
0.0(falsy)을 내보내 발동한다. 실측 **reduce 1,498건 중 714건(47.6%)이 정확히 0.50** —
축소의 절반이 모델 정보가 아니라 하드코딩 상수다.

> ⚠ **여기서 반증이 하나 나왔다 — 단순 중립화는 위험했다.**
> `size_multiplier`는 사이징 말고 **JointGateBlock의 라이브 차단 기준**
> (`_meta_size x _tox_size < 0.50`)에도 쓰인다. 최빈 조합이 정확히
> `0.50 x 0.70 = 0.35`라, 폴백을 1.0으로 바꾸면 `0.70`이 되어 **PASS 판정 게이트가
> 조용히 무력화**된다([7] 누적 hyp -13.16pt, n=116 — 차단이 손실을 회피).
> → **키를 분리**했다. `size_multiplier`(차단 기준, 무변경) /
> `size_multiplier_sizing`(사이징 전용, 무정보 폴백이면 1.0). main.py는
> `_meta_size`(차단) / `_meta_size_sizing`(사이징)으로 소비처를 나눈다.

**④ [Phase 2-1] `conf_mult_shadow` 섀도 계측 신설** (미적용, 기록 전용)
`CONFIDENCE_MULT_TABLE`의 임계(0.58~0.70)가 라이브 신뢰도 분포와 **정의역이 어긋난다**.
방향 있는 사이클 n=2,540: p50=0.346 / p90=0.402 / p95=0.425 / **max=0.628**,
`conf>=0.58` 0.20% · `>=0.65` **0.00%**. 그래서 `[Sizer]` 신뢰도배수가
**652건 중 649건(99.5%) 0.6 고정** — 사이저의 신뢰도 축은 사실상 상수였다.
417차의 `trades.quantity` 단위 불일치와 같은 계열(정의역 불일치)의 계측 결함.
배수 값은 그대로 두고 임계만 실측 분위수로 옮긴 `CONFIDENCE_MULT_TABLE_SHADOW`를
계산해 `conf_mult_shadow` 테이블에 기록만 한다. **중앙값은 여전히 0.6**이라
중심 경향은 불변이고 상위 절반만 반응한다. 승격은 표본 축적 후 수동 결정(313차).

### 검증

- py_compile **py37_32 / py310_64 둘 다 rc=0**
- flake8 F821/F823/F811/E999 **신규 0건** (main.py F811 2건은 11745/12570 기존 항목)
- 단위검증: `_compose_quality_qty` 7케이스 + 사이저 섀도 키/상한 클램프/재현성 통과
- **최근 20거래일 실제 진입 91건 재시뮬레이션**:

| | 분포 | 2계약+ | 총노출 | 평균 |
|---|---|---|---|---|
| 실제 관측 | {1:77, 2:6, 3:3, 5:2, 9:1, 10:2} | 15% | 137계약 | 1.51 |
| 431차 최종 | **{1:57, 2:25, 3:9}** | **37%** | **134계약** | 1.47 |

**총노출 -2.2%로 사실상 불변, 분포만 재배분**된다. 9·10계약 꼬리(손실 집중 구간)가
사라지고 그만큼이 2~3계약으로 간다. "진입 자유도 확대 = 노출 확대"가 아니다.

### 한계 — 반드시 함께 읽을 것

1. **증상 억제이지 원인 치료가 아니다.** 유의한 유일한 관계는 진입수량 vs
   (실현손실 / 의도손절폭) **rho=+0.318 (p=0.015)** — 손절폭 초과율 1계약 29.4% vs
   3계약+ **68.8%**. 큰 포지션은 방향을 못 맞히는 게 아니라 **손절선을 못 지킨다**
   (호가 깊이 슬리피지 가설, 미검증). 원인 치료는 집행 축([17]·급행풀스톱 라인).
2. **[28] `sizing_inversion_watch`가 즉시 깨어난다.** qty>=3 체결이 이제 생기므로
   표본이 쌓인다 — 감시가 가장 필요한 시점이다. **채널 삭제 금지.**
   ⚠ 상한 3 아래에서 qty>=3 버킷은 "정확히 3"만 담는다.
3. **표본 얇음** — 3계약+ 포지션 39건·손실 16건. 313차 기준 확정 결론 금지 구간.
4. **[0] 표본 기아 STARVED**(주간 진입 9건 < 하한 10)가 겹쳐 판정까지 수 주.

### 발견했으나 고치지 않은 것 (별도 등록)

`_qty_auto` 계산부(`main.py`)가 Degraded 배수를 **두 번 적용**한다 —
`_qty_display`에 이미 `HEALTH_DEGRADED_SIZE_MULT`가 곱해진 뒤
`_qty_auto = _qty_display`에 또 곱한다. 안전군이라 431차 범위 밖으로 두고
`NEXT_TODO.md`에 등록했다. Degraded는 관측 20거래일 발동 0건.

---

## 2026-08-04 (MW0602 430차 — **"앙상블 신뢰도 개선"은 계측 불연속이었다** + [44]·[45] 신설)

**지시**: "앙상블 신뢰도가 최근 드라마틱하게 좋아진 것으로 보인다" — 추세 조사 +
개선 원인 딥다이브. 이어서 도출된 3건의 손익 분석 후 구현.

### 1. 관측은 사실이나 원인은 모델이 아니다
...
두 달간 0.29~0.35 박스였다가 07-31 경계로 **+55%**. 그런데 `confidence_raw`는
0.491→0.515로 거의 그대로다. 갈린 것은 **보정 적용 여부**다.

행 단위 판정(`conf == min(raw, 0.85)` = 보정 미적용):

| 07-29 | 07-30 | **07-31** | **08-03** | **08-04** |
|---|---|---|---|---|
| 0.9% | 2.7% | **30.8%** | **92.2%** | **98.6%** |

`entry_gate_json.cal_degenerate`가 직접 확인해준다 — 07-31 10% → 08-04 **97%**.

**원인은 403차 배포다.** 축퇴 가드·도달불가 폴백이 `learning/calibration.py`에
들어간 커밋이 `827bd04`·`e594cee`, 둘 다 **2026-07-31**이다. 가드가 걸리면
`_fitted=False` → `calibrate()`가 raw를 그대로 반환하고 0.85로만 클리핑된다.
그전까지 Platt(C=0.02)이 모든 입력을 기저율 0.30~0.33으로 뭉개고 있었다.

### 2. 상승분의 65%는 "판단 불가" 구간이 만든다

`weight_collapsed`(실질 가중합 0, 인위적 raw=1.0) 행이 매일 **21%**다.
...
총상승 +0.175 중 **붕괴군 +0.113 (65%)** / 비붕괴군 +0.062 (35%).
07-31 이전엔 보정기가 붕괴행을 0.33으로 눌러 분포 **중앙**에 뒀다 —
이 왜곡은 passthrough 전환이 **새로 만든 것**이다.

### 3. 가드는 오작동이 아니다 — raw에 정보가 없다

보정기 라벨은 1m 적중 여부다(`main.py:4827`). 실측(07-29~08-04, n=1,791):

| raw 구간 | 0.3 | 0.4 | 0.5 | 0.6 |
|---|---|---|---|---|
| 1m 적중률 | 36.6% | 35.7% | 33.9% | 30.8% |

**단조 감소**. 순위 **AUC=0.489**(붕괴 제외 0.495) — 0.5 미만, 역방향.
`auc<0.53` 판정은 옳다. 427차 [39] REJECTS_HYP와 정확히 일치한다.
→ **개선이 아니라 정보 없는 숫자의 압축 해제다.**

### 4. [부수 발견] 축퇴 가드의 AND 조건은 사문(死文)이다

`calibration.py` 주석은 *"두 조건을 AND로 묶어 표본 노이즈로 AUC만 낮게 나온
경우를 배제한다"*고 적혀 있으나, **로그 span 845건 전수 최대 0.01321**로
`DEGENERATE_SPAN_MIN=0.02` 도달이 **0건**이다. 판정은 `auc<0.53` 단독이다.
n=200에서 SE≈0.041이라 08-04 하루에 감지↔해소가 수십 회 반복됐고, 그때마다
같은 신호의 conf가 0.33↔0.85를 오갔다(실측 전환 **9.33회/일**, 점프 중앙값
**0.4456**). **값은 바꾸지 않았다** — 주석만 정정했다.

### 5. [결정적] 진입 급증은 자기소멸 중이다 — DynMC가 백분위 추종이다

`min_conf`는 고정값이 아니라 **최근 conf 분포의 p65**다
(`main.py:_recalibrate_mc` → `update_dynamic_mc`, `MC_PERCENTILE=0.65`).

| 기준일 | 07-30 | 07-31 | 08-03 | 08-04 | 투영 |
|---|---|---|---|---|---|
| base_mc | 0.344 | 0.351 | 0.358 | 0.369 | **~0.418** |

conf가 계단식으로 올랐는데 게이트는 10일 윈도우로 뒤따라간다. **진입 1건→21건은
그 시차가 만든 과도구간**이고 2026-08-11경 되돌아온다. 백분위 게이트는 **스케일
불변**이라, 보정이 켜지든 꺼지든 장기 통과율은 ~35%로 수렴한다.

**여기서 진짜 결함이 드러난다** — 붕괴행이 분포 **최상단**을 점유하므로:

| DynMC 피드 | base_mc | 비붕괴 통과율 |
|---|---|---|
| 현행(붕괴 포함) | 0.4183 | **22.6%** |
| 붕괴 제외 | 0.3989 | **35.2%** ← 설계 의도 |

**판단 불가 구간이 진입 예산의 1/3을 잠식한다.** 안전망은 있으나 빗나갔다 —
ConstOut 필터(동일 반올림값 15% 초과 시 제외)에서 0.85가 **12.7%**로 2.35%p
차이로 통과한다.

### 6. 그래서 무엇을 했나 — 계측·문서만, 집행은 손대지 않음

**손익이 명확히 양수인 것은 계측·문서 축뿐이다.** 집행에 닿는 두 가지(DynMC 피드,
가드 임계)는 어느 쪽도 EV 개선을 보장하지 못한다 — 427차가 확정한 대로 게이트가
무작위 샘플러라, 진입을 늘리든 줄이든 **기댓값이 아니라 분산만 바뀐다.**

| 구현 | 라이브 영향 |
|---|---|
| `settings.CONF_SCALE_BREAKS` 신설 (기계가 읽는 불연속 레지스트리) | 없음 |
| 리포트 상단 자동 경고 + [39] 채널 오염 표기 | 없음 |
| `calibration.py` AND 사문 주석 정정 (**값 불변**) | 없음 |
| `dailycheck_prompt.txt` 상설 경고 블록 | 없음 |
| **[44]** `dynmc_collapse_feed_watch` 사전등록 | 없음(계측) |
| **[45]** `cal_guard_flap_watch` 사전등록(게이지) | 없음(계측) |

⚠ **[44]는 배선을 자동으로 바꾸지 않는다.** 붕괴행 제외는 진입을 22.6%→35.2%로
늘리는데 기대값 부호가 보장되지 않고, CLAUDE.md ⑧ 미해제 상태에서는 **진입 증가 =
노출 증가**다. 회귀 테스트가 그 경고 문구의 존재를 강제한다(문구가 사라지면 다음
세션이 그냥 배선한다).

⚠ **[45]는 임계 튜닝을 정당화하지 않는다.** 근본 문제는 임계가 아니라 정보
부재이고, 임계를 손대면 conf 스케일에 **세 번째 불연속**이 생긴다.

### 7. 판정 창 — 08-11 이전 수치로 판정 금지

`base_mc`가 10일 롤링이라 07-31 전환분이 아직 다 안 채워졌다. [44] `min_days=10`이
그 방어선이다. **지금 conf·진입건수·승률을 기준선으로 삼는 결정(채널 판정, 임계
재보정)은 과도구간을 정상으로 오인한다.**

### 8. 모델축은 이 경계와 무관하다 (확인함)

`predictions.confidence`는 1m/3m/5m 전부 07-27~08-04 구간 0.36~0.41로 **계단 변화
없음**. 호라이즌 보정기도 같은 가드를 쓰지만, 입력이 기저율 근처라 적용/미적용
차이가 작기 때문이다 — **"가드를 안 쓴다"가 아니라 "입력이 극단이 아니다"**.
붕괴 구조가 바뀌면 모델축도 영향권에 들어올 수 있다.

**검증**: `tests/test_430_conf_scale_break.py` 전부 통과 (base_mc 라이브 패리티,
합성 데이터 발화 확인=위음성 방지, 경고 문구 존재 강제). 리포트 실제 생성
스모크테스트 통과(py310_64).

**관련**: 427차([39] conf 판별력 — 같은 결론의 다른 축), 403차(원인 배포),
313차 원칙(일자단위), CLAUDE.md ⑧(진입 증가 = 노출 증가).

---

## 2026-08-04 (MW0602 429차 — **타점도 무작위와 구분 안 됨** [42] + 변동성 추정량 교체 무의미 [43])

**지시**: (1) [42] 타점 가치 채널 신설, (2) 변동성→사이징 섀도 배선.

### 1. [정정] 428차 결론의 절반이 미검증이었다

세션 결론을 *"진입의 방향성은 무의미하고 **체크리스트**와 청산의 기술로 수익"*
으로 읽으려 했는데, **체크리스트 부분은 재본 적이 없었다.** [40]은 진입
**시각을 고정**한 채 방향만 뒤집어 **방향 축만** 격리했기 때문이다.

3팔로 타점 축을 격리했다(`run_three_arm`, trials=400):

| 팔 | 손익 | 5~95% |
|---|---|---|
| A) 실제시각 + 실제방향 | **-22.07pt** | — |
| B) 실제시각 + 무작위방향 | +7.33pt | [-53.26, +60.40] |
| C) **무작위시각** + 무작위방향 | **+8.00pt** | [-53.43, +87.80] |

```
타점 기여 (B-C) = -0.67pt
짝지은 부호검정  B>C 196/400 = 49.0%,  p = 0.7264
```

**타점에도 가치가 없다.** 체크리스트/게이트가 고른 분(minute)이 무작위 분보다
낫지 않다. → **진입 축(방향·타점)이 둘 다 무작위와 구분되지 않는다.**

**짝지은 검정을 쓴 이유**: 두 팔이 같은 시장 데이터를 공유해 양의 상관을 갖는다.
독립 표본으로 다루면 p가 낙관적으로 나온다(세션 중 비짝지음으로 먼저 낸 42%는
그래서 인용하지 말 것 — 짝지으면 49.0%다).

C의 후보 분봉은 **09:05~14:49**로 제한했다 — 라이브도 14:50부터 신규진입
금지(345차)라 그 밖에서 뽑으면 실제로 불가능한 진입과 비교하게 된다.
거래일별 건수도 실제와 맞춰 일중 빈도를 보존했다(후보 5,449분봉).

### 2. [기각] "변동성을 진입에 도입하자" — **사이저는 이미 하고 있었다**

제안을 검토하다 발견: `PositionSizer.compute()`의
`stop_risk = atr × ATR_STOP_MULT × pt_value`. **사이저는 이미 ATR로 변동성
스케일링을 한다.** 그러니 질문은 "변동성을 넣자"가 아니라
**"분위 불확실성(선행 예측)이 ATR(후행 실현) 위에 추가 정보를 갖는가"** 다.

진입 68건(분위불확실성·ATR 동시 보유분) 실측:

| 지표 | 값 |
|---|---|
| corr(ATR×1.5, ATR×1.5) ↔ 30분 실현레인지 | **+0.6996** |
| corr(q90−q10, 실현레인지) | +0.6729 |
| 두 추정량 **공선성** | **0.9556** |
| **부분상관** (ATR 통제 후) | **+0.0211, t=+0.17** |

**ATR 위에 추가 정보가 없다.** 그리고 ATR 자체가 이미 좋은 예측자다(r=+0.70).

**측정할 것이 없는 섀도는 잡음만 만든다** — [41]이 EPV 미달에서 학습을 거부하는
것과 같은 취지로, **사이징 섀도를 만들지 않고 그 판단을 채널로 등록**했다.

### 3. [중요] 그런데 "아무것도 안 바뀐다"가 아니다 — **유해하다**

공선성 0.956인데도 `int()` 절단 때문에 계약수는 **40/68건(58.8%)에서 갈린다.**
즉 교체하면 **근거 없는 사이즈 변동을 59%에 주입**하는 것이다. 무해가 아니라
**유해**다(CLAUDE.md ⑧·425차가 qty 1↔2 경계의 중요성을 문서화했다).

그래서 [43]은 공선성만으로 기각하지 않고 **부분상관으로 판정하되 qty 경계
통과 건수를 함께 보고**한다. 둘을 같이 봐야 "무해/유해"가 갈린다.

### 4. 부수 결함 — 초판이 조용히 건너뛰었다

qty 경계 계산이 `MINI_FUTURES_PT_VALUE`를 `config.settings`에서 import했는데
그 상수는 `config.constants`에 있다. `ImportError`가 `try/except`에 먹혀
리포트에 `—`로만 표시됐다. **회귀 테스트가 잡아냈고**, import 경로 자체를
테스트로 고정했다(`test_constants_import_path`).

### 5. 하지 않은 것

- **사이징 섀도 배선 안 함** — §2가 그 이유다. [43]이 SUPPORTS로 바뀌면 그때 연다.
  그때도 **"교체"가 아니라 "잔차를 더하는" 형태**로 설계할 것(공선성 0.956).
- **체크리스트에 변동성 피처 추가 안 함** — [42]가 REJECTS인 동안 그것은
  *가치가 검증되지 않은 필터에 검증되지 않은 피처를 더하는* 일이다.
- **[42] 결과로 체크리스트를 제거하지 않음** — "가치가 검출되지 않는다"이지
  "해롭다"가 아니다(p=0.726). 그리고 체크리스트는 방향·타점 외에 **차단**
  기능도 한다 — 이 실험은 그 축을 재지 않는다.

**관련**: 428차([40] 방향 축, 이 채널의 짝), 427차([39]), 425차, 313차 원칙,
`docs/미륵이고도화3/호라이즌_개선_타임라인_2026-08-02.md` 6장.

---

## 2026-08-04 (MW0602 428차 — 방향 축이 무작위와 구분되지 않음 확정 + 청산 축 신설 [40]·[41])

**지시**: `호라이즌_개선_타임라인_2026-08-02.md` 학습 → 진행 중 작업 외 추가 묘안
제안 및 구현(A-1·A-2·B-1·B-2·B-3·C-1).

### 1. 계획서의 정보원 조사에는 보탤 것이 거의 없다

`방향신호_발굴계획_2026-07-27.md`는 옵션 포지셔닝(GEX·PCR·IV스큐·옵션체결)·캘린더·
수급/베이시스·실행계층을 전부 훑었고 제외 목록도 근거가 붙어 있다. **새 외부
소스를 발명해 보태는 것은 정직하지 않다.** 대신 발견한 것은 **타깃이 잘못 잡혀
있다**는 것이다 — 계획서의 이월 D·E·G가 전부 **방향 피처 재배치**다.

### 2. [A] 결정적 실험 — 방향 선택은 동전던지기와 구분되지 않는다

**같은 진입 시각·같은 청산 기하**에서 실제 방향과 무작위 방향을 400회 비교했다
(`scripts/random_entry_control.py`, 오프라인·읽기 전용):

```
표본 100진입 / 16거래일
실제 방향   : -22.07pt   승률 58.0%
무작위 400회: 중앙 +4.88pt   5~95% [-52.95, +62.59]   평균승률 61.5%
실제가 무작위 상회 21.0%  →  단측 p = 0.7905
일자단위    : 8/16일 (정확히 동전던지기)
```

**⚠ 절대 수준 인용 금지.** 시뮬은 단순화(TP1 33%+본전이동+TP2+스톱+종가청산;
트레일링·틱청산·계단화·ProfitGuard·비용 없음)라 실제 손익을 재현하지 못한다.
**유효한 것은 두 팔의 비교뿐**이며 같은 시뮬이라 그 비교는 공정하다.
따라서 이 채널은 **"시스템 전체의 엣지"를 재지 않는다. 방향 축 하나만** 잰다.

> 세션 중 임시 스크립트로 먼저 낸 값(+7.53 vs +18.83, p=0.613)은 **진입 봉을
> 포함하는 버그**가 있었다(`ts <= ets[:16]` 문자열 비교). 정식 버전이 진입 봉을
> 제외한다 — 423차·425차 유령 하드스톱과 같은 교훈이다. **정식 값이 옳다.**

이 결과로 §P1(전 호라이즌 15거래일 랜덤)·[39](conf 판별력 -0.0008)와 함께
**방향 축의 무력함이 네 갈래로 수렴**했다.

### 3. [B] 그래서 진짜 공백은 정보원이 아니라 **학습 타깃**이다

| 학습기 | 타깃 | 축 |
|---|---|---|
| GBM/RF 6호라이즌, SGD | 방향 | 진입 |
| `meta_confidence` | `_quality_label(conf, **correct**)` | 진입(방향 적중) |
| `meta_labeling` | take/reduce/skip = 방향 적중 + 추종폭 | 진입 |
| `quantile_regressor` | q10/q50/q90 (섀도, 미연결) | 크기 |
| **청산** | **없음** | **—** |

청산은 전부 규칙이다(ATR 고정배수·트레일링·손절계단화). 그런데 425차는 스톱이
16건 중 14건 옳았음을, [40]은 방향이 무작위와 같음을 보였다 — **돈은 청산에서 온다.**

**신설**: `learning/exit_outcome_labeler.py`(포지션×분 라벨) +
`learning/exit_classifier.py`(섀도 전용) + `exit_model_shadow` 테이블.

주 라벨은 `y_hold_better`(그 시점에 끊는 것보다 계속 보유가 나았나)다.
`y_tp1_first`(TP1 도달률 19%)보다 **결정과 1:1 대응**하고 균형이 낫다.
**방향 예측이 필요 없다** — 이미 진입한 포지션의 조건부 문제다.

**구조적 이점**: 회전율을 늘리지 않는다(진입 수 그대로, 청산 시점만) →
189회 검정을 전멸시킨 왕복비용 0.133pt를 추가 지불하지 않는다.

### 4. [B 핵심] 학습기가 **스스로 학습을 거부**한다

실측: 395행이지만 **포지션당 4.03행**으로 강하게 상관 → **유효표본은 98포지션**.
피처 14개·양성률 32%에서 EPV = 98×0.32/14 = **2.25**로 통상 기준 10의 **1/5**이다.

```
필요 포지션 435건 (337건 부족) · 거래일 16/20
실측 진입속도 6.12건/일 → 잔여 약 55거래일 (≈2.5개월)
```

**이 상태로 학습하면 학습이 아니라 암기다.** 그래서 `train_or_defer()`가 EPV
하한을 넘기 전에는 학습 자체를 거부하고 "얼마나 더 필요한지"만 돌려준다
([33] faststop_rerun_watch와 같은 게이지 방식). DB도 건드리지 않는다(실측 0행 유지).

학습이 열리면 **`GroupKFold(groups=entry_ts)`** 를 쓴다 — 같은 포지션의 행이
학습/검증에 갈리면 누출이라 행 단위 KFold는 쓰지 않는다.

⚠ **라이브 청산 미배선.** 회귀 테스트가 `main.py`에 세 이름이 없음을 고정한다.
배선은 판정 후 주간회의 결정이며, 계획서 2장의 "기준선을 흔드는 변경"이므로
**D-day(08-14) 스냅샷 이후**에 논의한다.

### 5. [C] 보류된 정보원 2종에 재검토 트리거를 못 박았다

계획서가 "타당성 조사 후 재검토"로 **조건 없이** 열어둔 것들이다 — 그러면 영원히
안 한다(CB②·CB③-P4·FP-CRITICAL이 정확히 그 패턴). `FEATURE_SET_DECISIONS`에 등록:

- **`opt_trade_flow_realtime`**(F3′) — 한국 실증 최강(Nam&Kim 2023 +18bp/일 t=7.8)
  이나 신호가 1~2분 내 소멸해 현행 5분 스냅샷으로 구조적으로 불가.
  트리거 3개 중 하나라도 충족 시 주간회의. 그중 (3)이 **[40]이 12거래일 이상
  REJECTS로 굳으면 내부 피처 재배치보다 먼저 올릴 것**이다.
- **`constituent_lead_top_n`** — 통계적 상관이 아니라 **기계적 항등식**(지수 =
  구성종목 가중합). 상위 10종목이 지수의 40~50%라 슬롯 10개로 부분 합성 가능.
  ⚠ 그러나 통상 선물이 현물을 **선행**하므로 리드 방향은 가정이 아니라 검증
  대상이다. **오프라인 리드-래그 검증이 먼저**고, 실패하면 구독 슬롯을 쓸 이유가 없다.

### 6. 검증

`tests/test_428_...py`(43체크). 특히 **위음성 방지** 2종:
- (A1) 시뮬이 진입 봉을 포함하면 스톱이 터지는 합성 케이스로 고정
- (B3) 600포지션 합성 데이터로 **학습 경로가 실제로 동작함**을 확인(AUC 0.87).
  이게 없으면 학습이 깨져도 영원히 "표본 부족"으로만 보인다.

기존 8개 테스트 파일 동반 통과. 리포트 재생성 정상(5.8초).

### 7. 하지 않은 것

- **새 외부 정보원 발명 안 함** — 계획서 조사가 철저하다. C-1은 트리거 등록만.
- **청산 학습기 라이브 배선 안 함** — 표본 미달이고 D-day 전이다.
- **이월 D·E·G 착수 안 함** — [40]이 REJECTS인 동안 소수점 싸움이다.

**관련**: `docs/미륵이고도화3/호라이즌_개선_타임라인_2026-08-02.md`(§5-1 "성공의
정의가 비어 있다" → [40]이 그 공백을 메운다), 방향신호_발굴계획(§3-5 제외목록),
425차(스톱은 옳았다), 427차([39] conf 판별력), 313차 원칙.

---

## 2026-08-04 (MW0602 427차 후속 — 미매칭 19건 딥다이브: **데이터 결손이 아니라 조인 키 결함**)

**지시**: 427차가 "별건"으로 남긴 미매칭 19건(매칭률 81%) 딥다이브.

### 1. 원인 — T−1분 결정의 체결 확인이 T분 `:00~:09`에 도착한다

19건 전수가 `entry_ts` 초 단위 `:00~:03`이었다(정상 진입은 `:5x`). 해당 분 전후를
직접 조회하니 **전부 T−1분에 `entry_executed=1` 결정행이 있었고 방향도 일치**했다.

```
2026-07-15 11:18:00 LONG   ← 11:17 dir=+1 G=C final_ok=1 executed=1 qty=1
2026-07-16 13:05:01 SHORT  ← 13:04 dir=-1 G=C final_ok=1 executed=1 qty=1
2026-07-20 14:18:01 SHORT  ← 14:17 dir=-1 G=C final_ok=1 executed=1 qty=2
2026-07-21 13:03:02 LONG   ← 13:02 dir=+1 G=C final_ok=1 executed=1 qty=2
2026-07-31 11:33:00 SHORT  ← 11:32 dir=-1 G=C final_ok=1 executed=1 qty=1
```
...
파이프라인이 `T:52` 근방에 주문을 내고 체결 확인이 `T+1:00~:09`에 도착하면
`trades.entry_ts`(**체결 시각**)가 결정 분보다 1분 뒤가 된다.

→ **데이터는 완전하다. `entry_ts[:16] == ts[:16]` 조인 키가 어긋난 것이다.**
   `entry_executed` 배선 결함이 아니다(그렇게 의심했으나 반증됐다).

### 2. 같은 결함이 [34]에도 있었다 — 잠재 상태

`eval_meta_size_zero_shadow`가 `substr(entry_ts,1,16)`로 같은 정확 조인을 한다.
`effective_date`(2026-08-04)가 늦어 아직 경계를 넘은 체결이 없어 **발현만 안 했을
뿐** 동일 결함이다. [39]가 그것에 걸려 드러났다.

전수 grep 결과 이 패턴은 리포트 전체에서 **두 곳뿐**이다([34]·[39]).

### 3. 수정 — 공유 헬퍼 `_entry_decision_pairs()`

T → T−1 순으로 찾되 **방향 일치를 요구**한다(인접 분에 반대 방향 결정이 있을 때
엉뚱하게 붙는 것을 막는다). 두 채널을 모두 이 헬퍼로 교체.
...
| 격차 | -0.0015 | **-0.0008** |
| Cohen d | -0.027 | -0.015 |
| 일자단위 | 3/11일 p=0.2266 | 3/12일 p=0.146 |
| verdict | REJECTS_HYP | **REJECTS_HYP** |

승 63 / 패 37은 내가 0804 조사에서 인용한 전체 승률 **63.0%** 와 정확히 맞는다 —
그때 이미 두 수치가 어긋나 있었는데 알아채지 못했다.

### 4. 오프셋 분포를 **지표로 남긴다**

`join_offset_min`을 metrics·리포트에 노출한다. `-1분` 비중은 곧 **체결 확인 지연**
이므로, 갑자기 늘면 집행 품질 신호다. 지금은 19%가 정상 기준선이다.

### 5. 하지 않은 것

- **`trades.entry_ts` 의미 변경 안 함** — 체결 시각이 맞다. 결정 시각을 원하면
  별도 컬럼이지 기존 필드 의미를 바꿀 일이 아니다.
- **다른 채널 일괄 점검 안 함** — grep으로 이 패턴이 두 곳뿐임을 확인했다.

**관련**: 427차(이 채널 신설), 422차 후속([34] 신설), 313차 원칙.

---

## 2026-08-04 (MW0602 427차 — [39] confidence 판별력 채널 + **0804 §P2 방향 정정**)

**지시**: min_conf 재보정의 가치 검토 + 현실화 시 손익 조사 → 그 결과의 채널 신설.

### 1. [정정] 0804 §P2는 **문제 제기의 방향이 틀렸다**

§P2는 *"동적 min_conf가 랜덤 기준선(0.33)까지 내려앉아 사실상 무력화됐다"* 를
**고쳐야 할 결함**으로 적었다. 조사 결과 **되살리면 손해다.**

**(a) confidence는 정보를 담고 있지 않다**

| 축 | 실측 |
|---|---|
| 모델축 (n=15,770 / 15거래일) | 하위절반 32.9% vs 상위절반 33.3% — **격차 +0.4%p, z=+0.56** |
| 앙상블축 (진입 81건) | 승 51건 conf **0.3963** vs 패 30건 **0.3978** — **격차 -0.0015** |

**(b) min_conf 상향은 전 구간 손익 악화**

| min_conf | 잔존/제거 | 잔존 손익 | 델타 |
|---|---|---|---|
| 현행 ~0.33 | 81 / 0 | +581,363 | — |
| 0.40 | 30 / 51 | +173,805 | **-407,558** |
| 0.45 | 16 / 65 | -168,579 | **-749,942** |
| 0.50 | 5 / 76 | -492,863 | **-1,074,226** |
| 0.54(설계 하한) | **1** / 80 | +62,510 | -518,853 |

**직접 증거는 이것이다** — 잘려나간 진입의 승률이 **62~63%** 로 전체 승률
**63.0%** 와 같다. 게이트가 품질 필터가 아니라 **무작위 샘플러**로 작동한다.
수익성 있는 장부에서 무작위로 덜어내면 수익이 줄어드는 것뿐이다.

**(c) "설계값으로 현실화"는 시스템 정지다** — 설계 `_ZONE_PARAMS` 0.54~0.67인데
실제 진입 conf 최댓값이 **0.579**다. 복원하면 진입이 81건 → 1건(0.54)/0건(0.60).
지금 0.33 근방인 것은 `_restore_mc_from_history()`의 DynMC가 실측 분포에 맞춰
내려온 결과이고, **그 적응이 없었으면 시스템은 진입을 못 했다.**

⚠ 일자단위 부호검정은 0.35→p=1.00 / 0.40→p=0.3018 / 0.45→p=0.6072로 **어느
방향도 유의하지 않다.** "올리면 확실히 손해"까지는 아니지만 **올릴 근거는 없다.**

### 2. [자기정정] 내 중간 관찰 하나를 철회한다

조사 중 `conf 0.60~0.70`이 적중률 **20.9%(z=-4.95)** 로 랜덤보다 유의하게 나빠
보였다. **일자단위로는 5/14일·p=0.4240으로 재현되지 않는다** — 표본의 **56%가
07-21 하루**이고 **88%가 퇴역한 30m**(296차)이다. **재인용 금지.**
오늘만 같은 함정에 세 번 걸렸다(지정가 진입 p=0.45 / ProfitGuard 과장 / 이것).

### 3. 그래서 채널은 min_conf가 아니라 **그 선행조건**을 잰다

가설: **"confidence는 판별력을 갖는다."** 게이트는 정보 없는 신호를 걸러낼 수
없으므로, min_conf를 논하려면 먼저 conf가 정보를 가져야 한다. 새 정보원이
들어와 그 순간이 오면 이 채널이 **먼저 반응**한다.

- `SUPPORTS_HYP` = 판별력 출현 → min_conf 재검토를 주간회의 안건으로
- `REJECTS_HYP` = 판별력 없음 확정 → min_conf 조정 무의미 (**현재**)

**두 축을 따로 잰다** — 두께와 의미가 다르다.
- **(B) 모델축** `predictions.confidence` vs `correct`. n=14,770(두껍다).
  ⚠ **30m 제외** — 296차 퇴역인데 predictions엔 계속 쌓이고, `conf≥0.60`의
  88%가 30m이라 포함하면 퇴역 모델이 판정을 끌고 간다(6,861건 제외됨).
- **(A) 앙상블축** min_conf가 **실제로 게이트하는 값**. 진입 81건(얇다).

**임계는 검출력에서 유도했다**(관측 전 고정):
- 모델축: n=14,770 절반분할 격차 SE=0.78%p → 검출하한 1.96SE=**1.520%p**.
  `model_gap_min_pp` = **3.1** (2배 3.04 이상).
  ⚠ 초판 3.0은 2배에 **0.04 미달**이었고 **회귀 테스트가 잡아냈다.**
  값과 근거가 어긋나면 나중에 아무나 바꾼다.
- 앙상블축: 진입 conf sd=0.0556 → `ens_gap_min` = **0.025** (Cohen d≈0.45).

**두 축 모두 효과크기 AND 일자단위를 함께 요구한다** — §2의 전례 때문이다.

### 4. [설계] 위음성이 가장 위험하다

"판별력 없음"과 "탐지 불능"은 **같은 출력**을 낸다. 채널이 버그로 영구
`REJECTS_HYP`에 갇혀도 알 수 없다는 뜻이다. 그래서 회귀 테스트에
**합성 판별표본으로 두 게이트 성분이 실제로 발화하는지**를 넣었다
(격차 20%p → 임계 통과 / 7일 전부 양수 → p=0.0156 유의).

### 5. 부수 발견 — 데이터 무결성

포지션 **100건 중 19건**이 `ensemble_decisions.entry_executed=1` 기록과
매칭되지 않는다(매칭률 81.0%, 07-15/16/20/21/31에 분포). 앙상블축 표본을
직접 깎으므로 채널이 매칭률을 매주 노출한다. **원인 미조사** — 별건.

### 6. 하지 않은 것

- **min_conf 조정 안 함.** 그것이 이 조사의 결론이다.
- **DynMC 손대지 않음.** 하한(`MC_ABS_FLOOR=0.25`)까지 더 내려가거나 진입 conf
  분포 위로 올라가면 진입이 0이 되지만, 지금은 정상 작동 중이다.
- **미매칭 19건 원인 조사 안 함** — 채널이 계측만 하고 별건으로 남긴다.

**관련**: 0804 §P2(정정 대상)·§P1(선행조건), 424차(SUPPORTS/REJECTS_HYP 어휘·
일자단위 도구), 296차(30m 퇴역), 313차·372차 원칙, 425차(min_days 수학).

---

## 2026-08-04 (MW0602 426차 — [38] ProfitGuard-L1 채널 신설 + **0804 "최대 공로자" 과장 정정**)

**지시**: 0804 일일점검 4-3의 ProfitGuard-L1 강화 — 발동 임계와 재개 조건을
캠페인 채널로 계측.

### 1. [정정] 0804 점검의 "오늘 최대의 손실 방어"는 **과장이었다**

내가 0804 보고에서 L1을 "오늘 최대 공로자"로 적었는데, 근거는 *"오후에 시장이
24pt 올랐고 시스템은 SHORT 편향이었다"* 는 **정황 추론**이었다. 차단된 40건이
실제로 졌을지는 **재본 적이 없었다.** 이번에 쟀다:

```
08-04 차단 40건 · 30분창 반사실 · TP1 24 / STOP 16
누적 -1.87pt (-93,492원) · 건당 -0.047pt
```

**박빙이다.** 차단이 미세하게 이득이긴 했으나 "최대 방어"와는 거리가 멀다.
**재인용 금지** — 0804 보고 §3 승리요인 (3)은 이 수치로 대체한다.

### 2. 더 근본적인 사실 — L1은 사실상 거의 발동하지 않는다

| | |
|---|---|
| 캠페인 거래일 | 16일 |
| 피크가 발동금액(100만원)에 닿은 날 | **3일** |
| L1이 발동한 날 | **2일** (07-20 14:56 / 08-04 13:07) |
| 실제로 진입을 막은 날(**구속**) | **1일** — 08-04뿐 |

07-20은 14:56 발동인데 그날 마지막 진입이 14:33이고 14:50부터 신규금지라
**아무것도 막지 못했다.** "발동했다"와 "구속했다"는 다르다 — 그래서 이 채널의
표본 단위를 **구속 발동**으로 잡았다.

### 3. [P0급 발견] 운영 파라미터가 코드에 없다

라이브 값은 `data/profit_guard_prefs.json`에서 온다 — **gitignore 대상이고
대시보드에서 장중 변경이 가능**하다.

| | 발동금액 | 비율 |
|---|---|---|
| **운영(prefs)** | **1,000,000원** | **20%** |
| 코드 기본값(`ProfitGuardConfig`) | 2,000,000원 | 35% |

즉 **판정 기준선이 코드 리뷰로 보이지 않고 세션 중에 바뀔 수도 있다.**
419·420차 `tox_regime_date` 교훈의 더 나쁜 버전이다(그쪽은 최소한 코드 안에
있었다). 리포트가 매주 prefs를 읽어 활성값을 찍고 코드 기본값과 다르면 🔴 경고한다.

### 4. 판정 축 4개 (관측 전 고정)

- **(a) 발동빈도·구속력** — 무해무익 발동은 표본으로 세지 않는다.
- **(b) 차단분 반사실** — hyp_pnl. (+)면 차단이 손해, (-)면 이득.
- **(c) 임계 스윕** — (발동금액 × 비율) 4×4 격자를 **전 거래일에 소급 재현**.
  ⚠ **이 축은 반사실이 아니다** — 발동하지 않았던 날은 그 뒤 진입이 실제로
  체결돼 실현손익이 남아 있다. 표본이 가장 두꺼운 축이다.
  실측: **전 격자에서 차단될 포지션 0건**. 발동금액 150만+는 아예 무발동.
- **(d) 재개 조건** — 현재 재개는 **없다**(`is_halted`가 `reset_daily()`까지 유지).

### 5. (d)에서 나온 유일한 실질 신호 — 15분 지연
...
**15분이 유일한 (−, +) 조합**이다 — 나쁜 첫 11건은 막고 이후 +20.38pt를 되찾는다.
⚠ **단일일이고 신호 수 ≠ 진입 수다**(연속 분봉·포지션 보유·쿨다운 미반영).
상한이지 기대손익이 아니다. 조치 금지, 채널에 축적한다.

### 6. [설계] 구속 판정의 순환논리를 교정했다

초판은 구속을 "발동 후 실제 진입이 있었는가"로 셌는데, **가드가 라이브로 실제
발동한 날은 그 진입이 애초에 없다 — 가드가 막았기 때문이다.** 그래서 유일한
진짜 구속 사건(08-04)이 "비구속"으로 잡혀 사라졌다. 라이브 차단 기록이 있으면
구속으로 인정하도록 고쳤다. 회귀 테스트로 고정.

### 7. 판정 유예 — min_activations=5는 약 4개월이다

실측 발동빈도가 16거래일에 구속 1회이므로 `min_activations=5`는 대략 **4개월**
스케일이다([8] KellySkip 4건/4주와 같은 계열). verdict는 그때까지 **OBSERVE**다.
**그 사이에 판정을 흉내내면 단일일 관측이 정책이 된다 — 0804에 실제로 그럴
뻔했고, 이 채널은 그 재발을 막으려고 만들었다.**

### 8. 하지 않은 것

- **재개 조건 구현 안 함.** 지시는 "계측"이었고, 근거도 단일일 1건이다.
- **임계 변경 안 함.** 스윕 전 격자가 차단 0건이라 조일 근거가 없다 —
  "임계를 얼마로 할까"보다 **"이 레이어가 지금 무엇을 하고 있나"** 가 먼저다.
- **prefs.json을 git에 올리지 않음.** 런타임 산출물이다([[feedback_git_commit_scope]]).
  대신 리포트가 매주 값을 찍어 기록으로 남긴다.

**관련**: 0804 점검 4-3(§3 승리요인 (3) 정정 대상), 424차(일자단위 검정 도구),
419·420차(설정 출처 교훈), 313차 원칙, [8] KellySkip(희귀표본 선례).

---

## 2026-08-04 (MW0602 425차 — 급행 풀스톱 집행축 처방: 후보 4개 중 3개 반증, qty=1 계단화만 생존)

**지시**: 0804 일일점검 4-3. 진입 축에서 못 막으니(421차 Phase A 전멸) 집행 축에서
막을 방안을 마련하고 Phase 0·1·2 전부 구현.

### 1. 모집단 — 급행 풀스톱이 총이익의 74%를 지운다

캠페인(07-05~) 시스템 100포지션 중 **손실 & 보유 ≤150초 16건**:

```
FAST   16건  -2,878,222원 (평균 -179,889)   ← 16건 전부 하드스톱(틱), 보유 36~120초
그 외  84건  +3,878,740원                   ← qty=1이 13건(-2,319,612원, 81%)
```

### 2. 후보 4개를 전부 실측했다 — **3개가 떨어졌다**

| 후보 | 판정 | 실측 |
|---|---|---|
| **진입 후 N초 손절 유예 / 스톱 확대** | ❌ **반증** | 청산 후 10분 내 **14/16이 더 나빠짐**. 07-16 14:10은 -9.38 → **-25.38pt**, 07-22 10:41은 0.00 → -9.30pt. **스톱은 옳았다.** |
| **손절 체결 슬리피지** | ⚪ 대상 아님 | FAST 7건 평균 **-0.0157pt**(오히려 유리). CLAUDE.md ⑧의 호가깊이 가설은 현행 1~2계약에서 미지지 |
| **지정가 진입** | ⚠️ **미지지** | 최선 변형(봉 25% 유리쪽) 근사 +1,558,316원이나 **일자단위 10/16일 p=0.45**, 델타의 **59%가 단일일**(07-09), 08-04은 -707,504원. 4변형 중 최댓값 선택이라 선택편향까지 있다 |
| **qty=1 손절 계단화** | ✅ **생존** | 섀도 +13.98pt·승률 78.6%. FAST 한정 **6/6 전승 +9.12pt** |

부수: 진입봉 추격도(종가가 봉 내 신호방향 극단에 붙은 정도) FAST 0.68 vs 0.52,
일자층화 순열 **p=0.0187** — 421차 245피처 그물에 **없던 변수**다. 다만 내가 4개
지표를 본 뒤 고른 것이라 Bonferroni-4(0.0125) 미통과이고 유효 8일은 421차가
"15일 미만은 근거로 쓰지 말 것"이라 명시한 구간이다. **[33] Phase A 재실행 시
그물에 추가할 것** — 지금 진입 필터로 쓰면 안 된다.

### 3. 왜 이것만 남았나 — 논리가 닫힌다

**스톱이 옳았다면(14/16) 남는 레버는 "더 일찍 끊는 것"이다.** 그리고 FAST의 81%가
qty=1인데 `is_loss_tier1_hit()`가 `quantity <= 1`을 "물리적 분할 불가"로 **원천
제외**해 전액이 풀스톱까지 노출된다. 363차가 섀도만 심어 뒀고 그게 [11]이다.
**새 장치를 만들 필요가 없었다 — 막힌 것을 뚫고 편향을 걷어내는 문제였다.**

### 4. [P0] 그 채널이 급행풀스톱의 40%를 못 보고 있었다

- **틱 경로**(main.py `_on_tick_price_update`)는 `is_loss_tier1_hit()` — **qty≤1 자체 배제**.
- **qty=1 섀도**는 분당 파이프라인(~:52)에서만 평가 → **1분 스트로보 샘플링**.

풀스톱은 매 틱 감시되므로 빠른 역행일수록 다음 샘플 전에 스톱이 먼저 터진다.
실측: 07-22 이후 FAST 11건 중 섀도 기록 6건, **누락 중 qty=1이 4건(-7.82pt)**.
→ 표본이 **느린 사건 쪽으로 편향**돼 있다. 판정해야 할 바로 그 빠른 사건을 덜 본다.

### 5. 구현 (Phase 0·1·2 전부)

**Phase 0 — 계측 정정 (라이브 청산 동작 무변경)**
- 틱 경로에 qty=1 섀도 배선. ⚠ **독립 `if`다 — `elif` 체인에 넣지 않았다.**
  그 체인은 "풀스톱이 잡히면 tier1 미평가"인 상호배타라, 끼우면 **같은 틱이 tier1과
  풀스톱을 함께 뚫은 사건이 통째로 사라진다**(지금 놓치던 것과 같은 종류).
- §4 준수: 콜백에선 flag만 세우고 `QTimer.singleShot(0, ...)`로 예약.
- 섀도 `ts`를 분 절단(`%H:%M:00`) → **초 단위**로. 틱 배선 후에는 같은 분 안의
  tier1↔풀스톱 순서가 핵심 관측인데 분으로 뭉개면 복원 불가.

**Phase 1 — 판정 개정**
- `min_days=6` 신설. **수학적 근거**: 일자단위 양측 부호검정 최소 p는 n=5에서
  **0.0625**로 α=0.05를 **원리적으로** 통과할 수 없다(n=6부터 0.03125).
  실측이 정확히 그 바닥이었다 — **5거래일 전부 양수인데 p=0.0625**.
  종전엔 `min_samples: 20` 하나뿐이라 하루가 표본의 40%를 공급해도 판정이 났다
  (실제로 resolved 14건 중 **12건이 3거래일**에 몰려 있다).
- 채택 조건을 **2겹**으로: ① 누적 hyp > 왕복비용×2(경제성) ② 일자단위 p<α(재현성).
- **모집단 정제 2종** — `live_active`(전환 후 기록: 반사실이 실제와 같아짐) /
  `from_tier1_remainder`(qty=2의 tier1 잔여계약: 이미 보호받은 다른 모집단, [14] 영역).
  구버전 행은 trades의 `손절1차 조기축소` 레그로 소급 보정 — **실측 혼입 1건 검출**
  (08-04 12:21, entry_qty=2가 12:22:18 tier1 체결로 qty 2→1이 된 뒤 이 표에 진입).
  n 14 → 13으로 정정됐다.

**Phase 2 — 실적용 경로 (기본 False)**
- `LOSS_TIER1_QTY1_ENABLED` / `_TICK_ENABLED` / `_EFFECTIVE_DATE` 신설.
- `_ts_execute_loss_tier1_qty1_exit` — qty=1은 쪼갤 수 없으므로 **전량청산**
  (`kind="EXIT_FULL"`). 기존 `_ts_execute_loss_tier1_exit`를 재사용하면
  `get_loss_tier1_exit_qty()`가 `min(cut, quantity-1)`이라 **qty=1에서 0을 돌려주고
  조용히 return한다** — 아무 일도 안 일어난다.
- `hint_source="stop_tier1_qty1"` 태깅(423차 §17 분리 취지 동일).
- **이 정책은 부분 축소가 아니라 qty=1 한정 손절폭 절반**이다. 회복했을 포지션을
  조기 손절하는 비용이 따르며 그 비용은 hyp_pnl_pts에 이미 차감돼 있다(음수 3건).

### 6. 검증

`tests/test_425_loss_tier1_qty1.py`(38체크). 핵심은 **AST로 "독립 if" 불변식을
고정**한 것 — 누가 `elif`로 바꾸면 깨진다. **음성 대조 통과**: 같은 탐지기가
기존 `is_tp1_hit`·`is_loss_tier1_hit`는 elif로 정확히 잡아낸다.
py37_32에서 기존 5개 테스트 파일 동반 통과. 리포트 재생성 정상.

### 7. 하지 않은 것

- **지금 켜지 않았다.** 13건/5거래일이고 p=0.0625다. 켜는 순간 반사실이 실제와
  같아져 채널 판정이 얼어붙는다 — 판정 전 전환은 되돌릴 근거마저 없앤다.
- **지정가 진입 배선 안 함.** 일자단위 미유의 + 선택편향. 하려면 1분봉 근사가
  아니라 HOGA 틱으로 호가 큐 체결 현실성부터 모델링해야 한다.
- **추격도를 진입 필터로 쓰지 않음.** [33] Phase A 재실행 그물에 추가만 권고.

**관련**: 421차 후속9·10(Phase A 전멸, (a)+(c) 결정 — 이 작업이 그 (c)다),
363차([11] 섀도 신설), 360차(계단화), 313차·372차 원칙, 424차(일자단위 검정 도구).

---

## 2026-08-04 (MW0602 424차 후속 — 캠페인 3채널 신설 [35]~[37] + **리포트 생성 크래시 3번째** 발견)

**지시**: 0804 일일점검 4-2(캠페인 채널 신설 3건) 구현.

### 1. 신설 3채널 — 셋 다 "판정 유예, 표본 축적"으로 사전등록

등록 자체가 조치가 아니다. **"지금 움직이면 313차 위반"이라는 판단을 코드에 남겨**
다음 세션이 같은 수치를 보고 성급히 움직이는 것을 막는 장치다(404차 후속11 레지스트리와
같은 취지). 판정 어휘는 PASS/FAIL이 아니라 `SUPPORTS_HYP`/`REJECTS_HYP`를 신설했다 —
셋 다 "조치가 필요한가"가 아니라 **어느 방향으로 필요한가**가 결론이라, FAIL 어휘를
쓰면 "FAIL이니 조여라"로 자동 오독된다([6] Hurst 완화 권고가 3주간 그렇게 오독됨).

| 채널 | 가설 | 등록 시점 판정 |
|---|---|---|
| **[35] `phantom_stop_edge`** | 유령 청산이 반사실(BE스톱 유지)보다 유리 | INSUFFICIENT — 소스 신설일이 08-05 |
| **[36] `grade_x_promotion`** | X→A 승격이 C→A보다 우월 | INSUFFICIENT — 관측 4일 < 5일 |
| **[37] `hurst_meanrevert_drag`** | mean-revert 적자가 **일자단위로도** 유의 | **PASS (미유의)** |

**[35] 반사실 정의(관측 전 고정)**: 억제했다면 포지션은 유지되고 **판정에 실제로 쓰인
스톱(`stop_eval`)** 으로 계속 간다 → 이후 분봉에서 닿으면 그 가격, 15:10까지 안 닿으면
그날 마지막 종가. `delta_pt = 실현 − 반사실`. SUPPORTS면 **버그 수정이 아니라 정책 승격**이
처방이라는 점을 권고문·상세섹션·settings 주석 세 곳에 박았다.

**[37]은 [29] `mean_revert_size_watch`와 모집단이 같다 — 중복이 아니라 브레이크다.**
[29]는 포지션 단위 승률 격차로 판정하는데 0804 실측에서 두 축이 정면으로 갈렸다:

| 축 | 결과 |
|---|---|
| 포지션 누적 | mean-revert 54건 **-1,455,546원** |
| 일자단위 짝지은 검정 | 짝지은 7일, 평균차 -1.63pt, t=-1.133, 부호 2/7, **p=0.4531** |

리포트 실행 결과가 수동 분석과 **소수점까지 일치**했다(p=0.4531·t=-1.133·7일). 판독
규칙을 관측 전에 고정해 상세섹션에 표로 박았다 — **[29] FAIL + [37] PASS = 조치 보류.**

### 2. [범위 밖 P0] `_row_is_live` 미정의 — **리포트 생성 전체가 죽어 있었다**

3채널을 검증하려고 리포트를 돌리자 첫 실행에서 `NameError: _row_is_live`로 죽었다.
419차(`2050437`)가 [30] 채널을 추가하며 **호출만 하고 정의하지 않았다.**
`eval_toxicity_recalib_watch()`의 try/except는 DB 조회만 감싸고 행 순회는 밖이라
첫 행에서 `build_report()`가 통째로 무너진다.

**이번 주에만 리포트가 세 번 죽었고 셋 다 발현 조건이 같다** — `campaign_due()`가
금요일에만 돌아 평일에는 아무도 모른다:

| # | 결함 | 도입 | 수정 |
|---|---|---|---|
| 1 | `[33]` `"BH FDR 10%"` 미이스케이프 → TypeError | 421차 후속10 | 422차 후속 |
| 2 | `[30]` `_row_is_live` 미정의 → NameError | **419차** | **424차 후속(여기)** |
| 3 | `[36]/[37]` `%+,.0f` → ValueError | 424차 후속 자작 | 즉시 |

→ **1번만 고쳐서는 08-07 리포트가 여전히 안 나왔다.** NEXT_TODO의 "08-07이 첫 실전
검증" 항목은 이 전제 위에서 다시 읽어야 한다. **MW0601도 같은 상태다**(419차가 MW0601 코드).

복구한 `_row_is_live()`는 원 의도(채널 독스트링 "백필 생성 행은 제외한다(418차)")대로
`feature_quality_score == 0.3` 마커로 판별한다. **허용오차는 424차 P0-A와 같은 1e-6**으로
맞췄다 — 여기 입력은 JSON dict(float64)라 정확히 일치하지만, 같은 마커를 float32
행렬에서 비교하면 틀린다는 것을 그날 배웠기 때문에 관례를 하나로 통일했다.
복구 후 [30]은 정상 동작: 라이브 770분봉, ceiling=0.42(419차 P0 라이브 확인),
cancel 포화 **100% → 0.6%** (재보정 효과 실측 확인).

3번은 `%`-포매팅이 콤마 플래그를 지원하지 않아서다(`format()`/f-string 전용).
`_fmt_krw()` 헬퍼로 고정했다.

### 3. 회귀 테스트 — "리포트가 죽는 클래스"를 파일 전체에 대해 검사

`tests/test_424_campaign_channels.py` (41체크). 채널 판정 검증 외에 **정적 검사 2종**을
넣었다. 채널이 늘어날수록 가치가 커지고, 깨지면 금요일에 산출물이 안 나온다.

- **(A) 미정의 모듈 헬퍼 호출** — AST로 `_foo(...)` 호출 중 정의 없는 것을 찾는다.
  **음성 대조 통과**: HEAD(수정 전) 파일에 돌리면 정확히 `['_row_is_live']`만 검출된다.
- **(B) `%`-포맷 콤마 플래그** — 실제 `"..." % x` 연산자의 **좌변만** 검사한다.
  초판은 소스 전체를 정규식으로 훑어 `_fmt_krw` 독스트링의 경고 문구까지 잡혔다
  (영원히 빨간불이 되는 검사는 없느니만 못하다).

### 4. 하지 않은 것

- **`[35]` 가드를 켜지 않았다** — 424차 본문 그대로다. 켜면 표본이 안 쌓인다.
- **`[29]`를 [37]로 대체하지 않았다** — 두 축이 갈리는 것 자체가 정보다. 합치면
  0804에서 실제로 발견한 "포지션 누적 vs 일자단위" 괴리를 다시 못 본다.
- **0803 P2 "X 승격 조이기" 제안을 폐기하지 않았다** — [36] 판정까지 **보류**로
  기록만 했다. 실측이 반대 방향이지만 X 경로는 2거래일에만 존재한다(313차).

**관련**: 424차(본문), 419차([30] 도입·결함), 422차 후속([33] 결함), 405차([28]/[29]),
313차·372차 원칙, 404차 후속11(레지스트리 취지).

---

## 2026-08-04 (MW0602 424차 — 423차 수정 2건이 **프로덕션에서 죽어 있었다**: float32 허용오차 + 네 번째 조이기 지점)

**지시**: 0804 일일점검(EOD+P8 흐름·이상점·승패 딥다이브) → 그 결과의 4-1 2항목 구현.

두 결함은 원인이 다르지만 실패 방식이 같다 — **코드가 있는데 아무 일도 일어나지
않았고, 로그만 보면 정상처럼 보였다.** 어제 배포한 수정이 하루 만에 둘 다 그
방식으로 무효였음이 라이브 첫 실행에서 드러났다.

### 1. [P0-A] 백필 제외 필터가 한 행도 못 걸렀다 — float32 유효자리 미달

08-04 EOD 로그: `학습분포 갱신 완료 — 39187행 기준 (전체 39187행 중 백필 0행 제외)`.
같은 창 실측은 **44,125행 중 28,564행(64.7%)이 마커**(`feature_quality_score == 0.3`)다.

원인은 [retrain_eod.py](../retrain_eod.py) 한 줄의 허용오차다.
`X`는 `batch_retrainer._load_from_db()`가 **`dtype=np.float32`** 로 만든다.

```
float32(0.3) = 0.30000001192092896  →  |값 - 0.3| = 1.192e-08
구 허용오차 1e-9 는 그보다 한 자릿수 촘촘 → 모든 백필 행이 필터를 통과
```

**왜 423차 검증에서는 맞았나**: `scripts/scan_backfill_exposure.py`는 같은 `1e-9`를
쓰지만 `raw_features` **JSON dict(float64)** 를 읽어 0.3이 정확히 일치한다.
오프라인 float64 / 프로덕션 float32 — 419·420·421차가 반복 지적한 **"오프라인만
검증"** 패턴이 같은 형태로 재발했다.

→ `_BACKFILL_MARKER_TOL = 1e-6`. py310_64 + 실 numpy 재현 검증:

| 허용오차 | 백필 제외 | 기준선 행수 |
|---|---|---|
| 1e-9 (구) | **0행 (0.0%)** | 44,125 (오염) |
| 1e-6 (신) | **28,564행 (64.7%)** | 15,561 (라이브 전용) |

실사용 품질점수 전수(0.74·0.76·0.78·0.82·0.84·0.86·0.88·0.9·0.94·1.0)는 마커와
**최소 0.44 이격**이라 오분류 여지가 없다.

**회귀 가드 신설**: `_excluded == 0`이면 WARNING. 백필 60%+ 창에서 0 제외는 구조적으로
불가능하다 — 이 줄이 없으면 필터가 또 죽어도 이번처럼 **조용히** 무효가 된다.

⚠ **423차의 "필요조건이지 충분조건이 아니다"는 그대로 유효**하다. 이번 수정으로
비로소 423차가 예측한 값(08-03 기준 1.072 → 0.335)이 실현되는 것이지, CRITICAL이
풀리는 것이 아니다. **실전전환기준 ⑦은 여전히 열리지 않았다.**

### 2. [P0-B] 유령 하드스톱 가드의 **네 번째 조이기 지점** — qty≥2가 통째로 우회

423차는 `stop_updated_at`을 `position_tracker` 3곳(진입/트레일링/`arm_tp1` qty==1)에
심었다. 그런데 **qty≥2의 TP1 손익분기 이동은 [main.py](../main.py)가 `stop_price`를
직접 대입**하므로(`partial_close()`는 스톱을 건드리지 않는다) 조이기 사실 자체가
기록되지 않아 가드가 무력화됐다.

08-04 실측 — 세 건 모두 조이기와 **같은 분봉** 안에서 유령 판정:

| 진입 | 조임(틱) | 판정 | 그 봉 고가 | 판정시 현재가 |
|---|---|---|---|---|
| 11:55 SHORT@978.18 | 11:58:26 982.02→978.18 | 11:58:52 | 979.00 (시가 978.82) | **977.24** |
| 12:03 SHORT@976.50 | 12:04:26 980.75→976.50 | 12:04:52 | 976.88 (시가 976.86) | **973.46** |
| 12:08 SHORT@972.94 | 12:11:29 977.86→972.94 | 12:11:52 | 973.04 (시가 972.88) | **968.84** |

고가는 전부 봉 시가 부근(조이기 이전)에 찍혔고, 현재가는 스톱보다 유리한 쪽에 있었다.
나머지 1건(11:29, qty=1)은 조이기가 **직전 봉**이라 가드가 정상 통과시킨 진짜 히트다 —
**가드 로직은 맞고 배선만 빠졌다.**

### 3. **고치지 않았다** — 손익 부호가 예상과 반대다

| | 건수 | 실현 | 반사실(정상 가드) |
|---|---|---|---|
| 08-03 (423차 실측) | 8 | +1.42pt | 5건 유리/3건 불리 |
| 08-04 (qty≥2 유령) | 3 | **+8.76pt (+433,609원)** | **0pt** (BE스톱이 12:00·12:24·12:34 재히트) |

**+433,609원은 08-04 순익 +752,561원의 57.6%다.** 정상 동작이었다면 그날은
약 +316,000원이었다.

유령 판정은 *"봉 고가가 스톱을 스쳤으나 현재가는 유리하게 되돌아왔다"* 에서만 터져
**시장가가 스톱보다 좋은 국면만 고른다**. 정확성 결함인 동시에 **미발견 청산 알파**일
수 있다. 2거래일 11건으로 확정하는 것은 313차 위반이다.

→ **라이브 청산 동작 무변경 + 섀도 계측**으로 배선했다.

- `Position.stop_shadow_tightened_at` / `stop_shadow_tighten_path` 신설 —
  **가드가 읽지 않는다.** `_mark_stop_tightened(path, live_guard=)`로 두 필드를
  한 곳에 묶었다(423차가 4곳 중 3곳만 심어 생긴 누락을 구조적으로 차단).
- `main.py` 손익분기 지점은 `mark_stop_tightened_shadow("tp1_breakeven_qty2")` —
  섀도만 찍는다. 여기서 `stop_updated_at`을 찍으면 즉시 억제되어 손익이 바뀐다.
- `phantom_stop_shadow` 테이블 신설(계측 전용). 봉중 판정마다 1행 —
  `live_suppressed`(423차 가드가 실제 억제) / `would_suppress`(전 경로 계측 가드였다면)
  두 팔을 다 남겨 비교 가능하게 했다.

**판정 후 처방**: 유리 확정이면 *"스파이크가 스톱을 스쳤으나 현재가가 유리하면
시장가 익절"* 을 **명시적 청산 정책으로 승격**. 불리 확정이면 그때 가드를 qty≥2까지
켠다. 채널 등록(`phantom_stop_edge`)은 4-2 후속 — NEXT_TODO 등록.

### 4. 검증

`tests/test_424_backfill_marker_and_shadow.py` 신설(19체크). py37_32 런타임에서
`test_phantom_hardstop` / `test_meta_gate` / `test_ensemble_decision` 동반 통과.
`main.py` INSERT 18컬럼 ↔ DDL 대조 통과(420차가 경고한 컬럼명 불일치 방지).
⚠ `.venv\Scripts\python`은 numpy가 없어 일부 테스트가 import 단계에서 죽는다 —
**`envs\py37_32\python.exe` 전체경로로 실행할 것**([[feedback_conda_env_python_path]]).

### 5. 같은 날 확인했으나 조치하지 않은 것 (근거·표본 부족)

| 관찰 | 규모 | 313차 판정 |
|---|---|---|
| 전 호라이즌 예측 적중률 랜덤 | 15거래일, 1m 33.7%~30m 32.1% | 일자단위 부호검정 전부 p≥0.118 |
| hurst `mean-revert` 적자 | 54포지션 -1,455,546원 | **짝지은 7거래일 p=0.4531** — 미유의 |
| raw_grade X→A 승격 우수 | 6포지션 5승1패 +587,344원 | 2거래일 — 판정 불가. 단 0803 P2의 "승격 조이기" 제안은 **근거 없음**(오히려 반대) |
| 급행 풀스톱 | 2포지션 -315,354원 | 421차 결론 재확인 — 진입축 판별자는 Phase A에서 전멸, **집행축**으로 |
| 동적 min_conf가 0.33 근방 붕괴 | 370건 중 271건 통과 | 계측 먼저 |

**관련**: 423차(두 수정의 원본), 419·420·421차(오프라인 검증 패턴), 313차 원칙,
417차(집계 단위 교훈), [[feedback_instrument_before_wiring]].

---

## 2026-08-03 (MW0602 423차 — 0803 일일점검 개선 5건 구현: PSI 백필오염·유령 하드스톱 외)

**지시**: 421·422차 커밋 반영 + 금일 로그 재분석 → 수정·개선 제안 5건 구현.

### 1. [P0-1 핵심] RegimeFingerprint PSI 7거래일 100% CRITICAL — 원인은 비닝이 아니라 모집단

371차가 균등폭→분위수 비닝으로 재설계하고 "며칠 라이브 관찰 후 판단"을 남겼다.
**그 관찰이 끝났고 결과는 실패였다** — 07-24~08-03 7거래일 **전부 100% CRITICAL**,
PSI 0.543~1.855로 임계 0.30 아래로 **한 번도** 내려가지 않았다. 371차 오프라인
재현검증(0.02~0.50)과 전 구간 어긋난다.

원인: `retrain_eod.py:354`가 26주 WFA 행렬 `X`를 그대로 PSI 기준선으로 저장하는데,
그 모집단의 **77.6%가 백필 행**(`feature_quality_score==0.3`)이고 백필은 미시구조를
0.0으로 채운다. 실측 — `ofi_norm` 0.0 비율: **백필 100.0% vs 라이브 1.6%**.
0.0 빈 하나의 PSI 기여만 `(0.008-0.633)*ln(0.008/0.633) = 2.73`으로 임계의 9배다.

→ `feature_quality_score == 0.3` 행을 기준선에서 제외. 안전판(필터 후 `_N_BINS*2`
미만이면 전체로 폴백) + 제외 행수 로그 병기.

**⚠ 필요조건이지 충분조건이 아니다 — 실측으로 확인했다.** 26주 창 재현 비교:

| 거래일 | 수정 전 중앙PSI | 수정 후 | 등급 변화 |
|---|---|---|---|
| 07-24 | 1.881 | 0.584 | CRITICAL 유지 |
| 07-27 | 1.201 | 0.260 | → **ALARM** |
| 07-28 | 2.093 | 0.591 | CRITICAL 유지 |
| 07-29 | 1.668 | 0.817 | CRITICAL 유지 |
| 07-30 | 0.797 | 0.211 | → **ALARM** |
| 07-31 | 1.219 | 0.622 | CRITICAL 유지 |
| 08-03 | 1.072 | 0.335 | CRITICAL 유지 |

피처별(08-03 마지막 시점): `ofi_norm` **1.077 → 0.067**(16배, 가설 완전 확증) /
`vwap_position` 1.125 → 0.582 / `cvd_divergence` 0.766 → 0.378.
**`ofi_norm`은 해소됐고 나머지 둘은 절반만 내려왔다** — 잔여 원인이 따로 있다.
따라서 이 수정만으로 **CLAUDE.md 실전전환기준 ⑦은 닫히지 않는다**. NEXT_TODO 등록.

계측 전용이다 — `FP_CRITICAL_GRADE_BLOCK_ENABLED=False`라 라이브 진입 판단 무변경.

### 2. [P0-2] 유령 하드스톱 — 기존 가드가 틱 경로를 못 덮고 있었다

`_ts_check_exit_triggers`의 `_prev_stop_price`는 **그 함수 안의**
`update_trailing_stop()`만 되돌린다. TP1 보호전환은 COM 틱 핸들러가 **함수 밖에서
비동기로** stop_price를 바꾸므로, 다음 파이프라인엔 이미 조여진 값이 들어와
가드가 무력화된다.

08-03 실측: 비틱 `하드스톱` 청산 **8건이 전부** 이 경로. 대표 사례 —
13:49:54 진입 SHORT@988.88(손절 991.00) → 13:50:09 틱 TP1이 988.53으로 조임 →
13:50:55 분봉 13:50 고가 988.94로 히트 판정. **그 고가는 조이기 이전 값이다.**
(`raw_candles.created_at`이 봉 시각 +53초라 파이프라인은 그 봉 전체를 평가한다.)

→ `Position.stop_updated_at` 신설. `is_stop_hit_intrabar(..., bar_start=)`가
스톱이 그 봉 안에서 조여졌으면 봉중 판정을 포기한다. 종가 기준 `is_stop_hit()`은
그대로라 **진짜 히트를 놓치지 않는다**(다음 봉부터 새 스톱으로 정상 판정).
`update_trailing_stop`은 **값이 실제로 바뀐 때만** 시각을 찍는다 — 매분 무조건
찍으면 봉중 판정이 영구 무력화되기 때문(회귀테스트로 고정).

**손익 영향은 오늘 한정 +1.42pt(유리)였다** — 조인 스톱보다 시장가가 좋아 우연히
이득(8건 중 5건 유리/3건 불리). 손실 사건이 아니라 **정확성 결함**으로 다룬다.

### 3. [P1-1] §17 채널 오염 — 그 -1.371pt는 슬리피지가 아니다

`exit_fill_slippage`에 `hint_source` 신설(멱등 ALTER, 기존 35행 무손실).
`stop_close` / `stop_intrabar` / `stop_tick` 3분류.
08-03 실측: 비틱 `하드스톱` 8건 평균 **-1.371pt** vs 틱 13건 **+0.085pt**.
전자의 `price_hint`는 실제로 닿은 적 없는 스톱가라 집행 슬리피지가 아니다 —
그대로 쓰면 왕복비용 가정(0.02pt)을 **60배 이상 과대추정**한다.

### 4. [P1-2] GradeEVGuard 근거 모집단 오염 — 부호가 뒤집혀 있었다

08-03 하루에 `A→B 강등 조건 충족` **59회** 발화(섀도). 근거 "A급 30일 평균순EV
-7,583원"을 분해하면:

| 구간 | n | 합계 | 평균 |
|---|---|---|---|
| entry_source **NULL** (07-09~10, qty=2 시절) | 6 | **-1,069,701원** | -178,283 |
| 급행 풀스톱(손실 & ≤150초) | 12 | -1,870,823원 | -155,901 |
| 나머지 — 정상 시스템 A급 | 46 | **+2,455,218원** | +53,374 |

앞 둘은 **서로소**다. `fetch_ev_by_grade(system_only=True)` 신설 → A급
n=58 평균 **+10,076원**(양수)이 되어 가드 발동 조건 자체가 사라진다.
같은 기간 pt 단위로는 A(+0.358) vs C(+0.239)가 **통계적으로 구분 안 됨**
(Mann-Whitney p=0.80) — 원 단위 차이는 계약수 효과였다(417차 교훈과 동형).

⚠ `VALIDATION_CAMPAIGN_DECISIONS["grade_ev_inversion"]`의 "부결"(2026-08-01)을
**뒤집지 않는다.** 기존 근거(급행 풀스톱 오조준)와 서로소인 **두 번째 근거**를
더할 뿐이다. 기본값은 False라 `daily_exporter` 손익 리포트는 무변경 —
그쪽은 "실제로 얼마 벌었나"라 수동 진입을 빼면 오히려 틀린다.

### 5. [P2] 백필 노출도 스캐너 — 일괄 수정을 처방하지 않은 이유

`scripts/scan_backfill_exposure.py`(읽기 전용). 백필 행은 **균질하게 나쁘지 않다** —
가격계(atr·hurst)는 유효하고 미시구조만 0.0이라 일괄 제외가 정답이 아니다.

백필에서 죽는 피처(백필 0.0 비율 ≥99%) 6종 확정: `ofi_norm` `spread_ticks`
`toxicity_score` `toxicity_atr_stress` `foreign_futures_net` `opt_pcr_norm`.

**소비자별 실제 조회 창을 반영해야 오탐이 안 난다** — 초판은 전 소비자를 26주
창으로 재서 ScalerWarmup을 "노출"로 잘못 찍었다. 실제로는 최신 500행만 읽어
백필 0건이다(라이브 전용 std 0.0847 ≈ EOD 실측 scale 0.0836으로 교차확인).

| 소비자 | 창 | 백필 비중 | 판정 |
|---|---|---|---|
| RegimeFingerprint 기준선 | 26주 | 77.6% | **노출** → P0-1로 해소 |
| GBM 학습 X (26주 WFA) | 26주 | 77.6% | **노출** — 미해결, 아래 등록 |
| ATR ceiling 재보정 | 15거래일 | 0.0% | 무해 |
| ATR multiple 재보정 | 15거래일 | 0.0% | 무해 |
| ScalerWarmup | 최신 500행 | 0.0% | 무해 |

### 6. [부수] 422차 대조 + 테스트 이식성 보강

- **장전 CB 허위 알림: MW0602는 오염 없음**(SYSTEM CRITICAL 0줄). 테스트를 MW0601
  에서만 돌린 결과다. 422차 `conftest.py` 가드는 이미 들어와 있어 추가 조치 불요.
- **리포트 생성 결함([33] `10%` 미이스케이프)**: MW0602도 16:13~17:54 노출됐으나
  `campaign_due()`가 금요일에만 돌아 미발현. 수정본(d7147cd) 반영 완료.
- `test_meta_gate.py`·`test_ensemble_decision.py`에 **sys.path 1줄 추가**. 422차가
  명시한 "각 테스트 파일 상단에도 동등한 2줄" 중 env var만 있고 sys.path가 빠져
  있어 pytest 없이는 실행 불능이었다 — **MW0602엔 pytest가 없다**. 이제 py37_32
  런타임에서 6개 파일 전부 직접 실행 통과.
- 부수 확인: `ensemble_decisions.meta_size_raw`(422차 후속 [34])와
  `exit_fill_slippage.hint_source` 마이그레이션이 이번 DB 초기화에서 함께 적용됨.

### 7. [정정] 앞선 보고의 `meta_size_fallback 80.6%`는 무효

같은 세션에서 내가 `joint_gate_shadow`로 뽑은 80.6%를 보고했는데, 그 테이블은
**차단된 신호 전용**이라 422차 후속이 지적한 선택편향과 동일한 오류다.
진짜 분포는 reduce 밴드 전체(n=1,059)에서 0.500 57.4% / 연속 34.5% / 0.750 8.1%.
**80.6%는 재인용 금지.**

---

## 2026-08-03 (MW0601 422차 후속 — meta "size 0" 계측 사각지대 신설 [34] + 리포트 생성 결함 수정)

**지시**: 0803 정기점검 P2("meta_conf<0.5 상시화") 딥다이브 → 개선안 → 구현.

### 1. [정정] 지시의 전제가 틀렸다 — meta_size는 상수가 아니다

P2 제기 문구는 *"메타게이트가 사실상 상수 0.5로 동작 중"* 이었으나 **오답**이다.
reduce 밴드 전체 모집단(n=1,059) 실측: **0.500 정확 57.4% / (0.5,0.75) 연속 34.5% /
0.750 천장 8.1%** — 3-모드 분포다. 어제 본 26건은 `joint_gate_shadow`(=**차단된**
신호 전용)라 선택편향이 있었다(차단 조건 자체가 `meta_size<0.7143`).

JointGateBlock이 1축인 이유는 meta가 아니라 **tox_size 전수 0.7 상수** 때문이고,
그건 419차 발견 ④·420차 F1에 이미 있다. P2는 새 발견이 아니었다.

### 2. [실측] F2를 전체 모집단에서 처음 정량화

420차 F2는 SIGNAL 로그 82건 표본이었다. 전체 1,059건에서:
**89.1% 차단 / 10.9% 통과, 그리고 통과 115건 중 86건(75%)이 클램프 천장 0.750.**
통과의 3/4은 "모델이 확신해서"가 아니라 천장에 붙어서다.

### 3. [해소] 420차 역산 가정 검증 완료 — NEXT_TODO 항목 종결

`meta_size==0.5 ⟺ meta_size_fallback==1`, **반례 0건**(conf_raw 0.1706~0.4967).
진짜 `conf==0.5`는 0건이라 63% 역산은 폴백 비율로 유효했다. 0803의 76.9%와 63%
차이는 가정 오류가 아니라 표본 변동(n=26 vs 116)이다. → "역산 가정 재조사" 불필요.

### 4. [핵심] "size 0"의 출처와 의미 — 임계 0.5는 임의 상수가 아니다

`_make_result()`는 `conf<0.5`에서 size를 0.0으로 낸다. `conf`는 확률이 아니라
**4급 품질레이블(Q0~Q3, 가중치 0/⅓/⅔/1)의 기대값**이고, 적중률 a·고신뢰비중 h로
환산하면 `conf = ⅓(1−a)(1−h) + ⅔a(1−h) + a·h` 다. **a=0.5를 넣으면 h와 무관하게
정확히 0.5** — 즉 `conf<0.5 ⟺ 적중률<50% 추정`이다. 원리적 손익분기점이다.

출처는 규칙기반 폴백이 아니라 **학습모델**이다: `data/meta_conf_state.pkl` 실측에서
레짐별 LR 4개 전부 `fitted=True`(표본 2186/1614/447/327).

실측 정합: `meta_labels` 87,642건 적중률 **34.57%** → 함의 conf ≈ 0.397, 오늘 실측
conf_raw 0.1706~0.5561. **메타 모델은 예측기 풀이 랜덤에 가깝다는 것을 정확히 감지
하고 있다** — 0803 정기점검의 세 발견(적중률 랜덤 / MFE·MAE 대칭 / 판별자 전멸)과
네 번째 독립 경로로 같은 결론.

⚠ 단 그 "적중률"은 **앙상블이 아니라 호라이즌별 예측기 풀**이다(`for v in verified:`가
6개 호라이즌 전부를 record_outcome에 넘긴다). 호라이즌별 34.0~35.5%.

### 5. [발견] take 밴드가 더 큰 왜곡인데 아무도 안 보고 있었다

```
reduce: learned["size_multiplier"] or 0.5      → 0.0이 falsy → 0.5
take:   max(0.9, min(1.25, size_multiplier))   → 0.0 → 0.9  (거의 풀사이즈)
```
419·420차는 reduce의 `or 0.5`만 계측했다. **take 쪽 왜곡(0.0→0.9)이 더 큰데
계측이 전무**했다 — `size_multiplier_raw`는 420차가 `joint_gate_shadow`에만 넣었고
그건 **차단된** 신호 전용이라, **진입한** 신호의 raw는 어디에도 없었다.

캠페인 실측(진입 79건, 매칭 67, 네 버킷이 손익합계 −943,124원으로 정확히 분할):

| 버킷 | n | 매칭 | 누적 | 승률 | 음수 거래일 |
|---|---|---|---|---|---|
| reduce&0.500 (raw==0 확정) | 20 | 16 | **−1,105,638** | 25% | **4/4** |
| take&0.900 (raw≤0.9 혼재) | 26 | 22 | **−903,986** | 59% | 3/5 |
| reduce&>0.500 (raw>0) | 20 | 18 | **+1,135,909** | **67%** | 2/7 |
| take&>0.900 (raw>0.9) | 13 | 11 | −69,409 | 45% | 2/4 |

raw가 눌린 두 버킷이 손실을 전담하고 raw가 살아있는 reduce>0.5가 유일한 흑자다.
**단 take&0.900은 raw==0과 raw∈[0.5,0.9]가 섞여 있어 −903,986원을 폴백 탓으로
계상할 수 없다** — 그 구분이 이번 배선의 목적이다.

### 6. [기각] 안 A(`or 0.5` 제거 단독)는 현행 체제에서 무효

폴백을 없애면 size는 0.0이 아니라 **클램프 바닥 0.35**가 되고,
`max(1, int(round(qty × size)))`에서 **qty≤2는 결과가 완전히 동일**하다(3부터 갈림).
그런데 문제의 20건은 **전부 qty=1**이고 0803 Sizer 출력의 **91.7%가 qty≤2**다.
→ 419차 P3가 "고치면 라이브 사이징이 바뀐다"며 보류한 그 수정은 **아무것도 바꾸지
못한다.** 419차 P1↔P2의 "반올림에 흡수" 구조가 **세 번째로** 확인됐다.

진짜 결함은 폴백 상수가 아니라 **action(blended_conf)과 size(raw conf)가 서로 다른
conf를 쓰는 것**이다 — 420차가 각주로 처리한 "경로 불일치"가 본체였다.

### 7. [구현] 채널 [34] `meta_size_zero_shadow` — Phase 0, 라이브 무변경

- `utils/db_utils.py` — `ensemble_decisions.meta_size_raw` REAL (멱등 ALTER)
- `learning/prediction_buffer.py` — INSERT **2경로**(`save_ensemble_decision`,
  `save_step9_batch`)에 컬럼·값. `.get()` 그대로 — `or` 금지(0.0 falsy 승격이
  바로 이 채널이 재는 결함이다)
- `config/settings.py` — 판정 규약 사전등록(표본 보기 **전** 확정).
  PASS=현행 유지 / FAIL="안 B"(raw==0 → action skip 강등) **주간회의 안건화**,
  자동 전환 금지. `min_samples=20`, **`min_days=5`**(다른 채널보다 높다 — 교란 (a) 때문)
- `scripts/generate_validation_campaign_report.py` — eval + 요약표 + 상세절

**밴드 분리 집계 강제** — take(0.0→0.9)와 reduce(0.0→0.5)는 왜곡 크기가 달라
합산하면 큰 쪽이 희석된다. 합계 행은 판정용이지 해석용이 아니라고 리포트에 명시.

**교란 3종을 사전등록에 박아 넣었다** — 판정 시 반드시 함께 읽을 것:
(a) `recent_accuracy`가 meta 입력이라 conf는 **자기상관**을 갖는다 — "size 0 구간이
    나빴다"가 사전 판별력인지 나쁜 구간의 뭉침인지 구분되지 않는다(min_days=5 근거)
(b) meta 학습 스트림에 **퇴역한 30m(20.8%)·FLAT 예측(20~45%)이 혼입**된다
(c) LR이 `class_weight='balanced'`라 conf가 실제보다 높게 나올 수 있다 —
    `conf<0.5`는 오히려 **보수적** 추정일 수 있다

소급 백필 없음(결정 시점에만 알 수 있는 값 — 420차와 동일). `effective_date=2026-08-04`.

### 8. [부수 수정·중요] 캠페인 리포트 생성이 통째로 깨져 있었다

`scripts/generate_validation_campaign_report.py`의 `[33]` 절에서
`"... BH FDR 10% 통과 0개" % _fs.get(...)` — **`10%`가 이스케이프되지 않아** `%s`와
함께 변환지정자 2개로 해석돼 `TypeError: not enough arguments for format string`.
**리포트 생성 전체가 죽는다.** 421차 후속10(`8cbe450`) 도입 이래 잠복해 있었다.

`campaign_due()`가 **주 마지막 거래일(금요일)에만** 실행하므로 아직 발현되지 않았고,
**2026-08-07 첫 금요일 실행에서 리포트를 통째로 날렸을 것**이다. 각 스텝이 독립
서브프로세스라(`run_campaign_steps`) 조용히 실패했을 것 — 발견도 늦었을 것이다.
`10%%`로 수정.

> 이것이 NEXT_TODO가 419·420·421차에 걸쳐 반복 경고한 **"수동 실행만 검증, EOD 체인
> 경로 미검증"** 의 실물이다. 이번엔 신규 채널 검증 때문에 우연히 걸렸다.

### 9. [검증] py37_32

```
마이그레이션    실 DB 컬럼 생성, 기존 20,967행 무오염(전부 NULL)
INSERT 왕복     2경로 × (raw=0.0 / raw>0 / 키부재) — 0.0 보존, 키부재만 NULL
판정 분기       합성표본으로 FAIL / PASS(흑자) / PASS(손실이나 승률↑) 3분기 전부 통과
표본 게이트     매칭 12 < min_samples 20 → INSUFFICIENT 정상
리포트 빌드     성공(수정 후) — 요약표 + 상세절 렌더 확인
pytest          20 passed
```

⚠ 검증용으로 생성된 `validation_campaign_report_20260803.md`는 **월요일 산출물**이라
`cmp_summary.py`의 "최신 날짜본 자동 짝짓기"에서 MW0602의 금요일본과 오짝될 수 있어
스크래치패드로 옮기고 커밋하지 않았다(CLAUDE.md 대조 규약).

### 10. [남은 것 — 라이브 미검증]

419·420·421차와 **같은 성격의 미검증 지점**이다: `meta_size_raw`의 라이브 INSERT
경로는 08-04 장중에 처음 돈다. 오프라인·합성만 검증했다.

**하지 않은 것**: 안 A(무효 — P2 양자화와 한 세트여야 의미), 안 B(액션 축 변경 —
주간회의 대상), JointGateBlock 임계 0.50 재선정(방향 증거 없음 t=−0.39, 420차 §6 그대로).

**관련**: 0803 정기점검 P2, 419차 P1·P3, 420차 F1~F3·§6, 313차(min_days),
`docs/정기점검/금요일점검/0803_JointGateBlock_딥다이브_및_419차반영_구현.md`
...
### 1. [구현] 발굴 하네스 — 오프라인 전용, 라이브 무접촉

그물 5축을 진입 분봉 기준으로 조인한다(전부 기존 저장분):
`raw_features` 수치형 / 같은 키의 **직전분 대비 델타**(`d1.`) /
`ensemble_decisions` 수치 필드 / `entry_gate_json` 불리언 / 컨텍스트.
조인 키는 `trades.entry_ts`를 분 단위로 잘라 `raw_features.ts`·
`ensemble_decisions.ts`와 맞춘다(3개 DB 전부 확인).

**핵심 설계 — 일자 내 라벨 순열검정.** FAST 20건 중 15건이 3거래일에 몰려 있어
단순 t검정을 쓰면 *"그날은 변동성이 컸다"* 가 판별자로 위장된다(ATR·스프레드·
toxicity가 전부 그렇게 통과한다). 라벨을 **각 거래일 내부에서만** 셔플해 귀무분포를
만들면 날짜 수준 교란이 구조적으로 제거된다. 질문이 "FAST가 많은 날은 어떤 날인가"
에서 **"같은 날 안에서 FAST와 OK를 가르는 것은 무엇인가"** 로 바뀐다.

대가로 유효표본이 79 → **69건 / 7일**로 준다(FAST·OK가 둘 다 있는 날만 정보를 낸다).
그것이 정직한 표본이며 줄어든 수치를 그대로 출력한다.

### 2. [결과] BH FDR 10% 통과 **0개** — 최강 후보조차 노이즈 기대치다

순열 20,000회 (3,000회 예비 결과와 안정적으로 일치):

```
포지션 79 (FAST 20 · OK 59) / 거래일 12 / 검정 피처 245
일자 층화 유효표본 n=69, 거래일 7
최강 후보 p = 0.0039
검정 245개에서 노이즈가 만드는 최소 p 기대치 ≈ 1/245 = 0.0041
```

**최강 후보의 p가 순수 노이즈 기대치와 사실상 같다.**

### 3. [검증] 상위 항목 3중 점검에서 나온 것

- **상위 항목이 서로 독립이 아니다.** 상위 7개 중 5개가 `opt_pcr_*` 계열이고
  `opt_pcr_norm`↔`opt_pcr_extreme_signed`는 **r=+1.000**(같은 변수), 나머지도
  |r|=0.55~0.94다. "여러 지표가 함께 나왔다"를 독립 증거로 읽으면 안 된다.
- **누출 피처 제거**: `ed.auto_entry`가 초판 12위로 올라왔다(68/11 분포). 이는
  진입 시점 상태가 아니라 **진입 결정의 결과**다. `entry_executed`·`entry_final_ok`와
  함께 `_SKIP_ED`에 넣었다(245개는 그 제거 후 수치).
- **내 강건성 지표가 무력했다**: leave-one-day-out 부호유지가 상위 20개 **전부 7/7**.
  거래일이 7일뿐이라 하루를 빼도 층화 추정치 부호가 좀처럼 뒤집히지 않는다 —
  판별력 없는 열이다. 리포트에 "거래일 15일 이상 전에는 근거로 쓰지 말 것" 명시.

### 4. [해석] 진입측 판별 불가 — 네 번째 확인, 단 확정은 아니다

가격기반 4지표(|t|<1.3) → InstabilityGate(승1/패2 발동·패2 미발동) →
동일 체크리스트가 승·패로 갈린 쌍(11:21 vs 11:25, conf 34.7 vs 34.8%) →
**248개 그물 전멸**. 같은 방향이 네 번 나왔다.

**그러나 확정이 아니다.** 계획서대로 이건 발굴 단계이고 확증창을 거치지 않았다.
남은 가능성 둘:
1. **표본** — 유효 7일·FAST 20건은 중간 크기 효과를 놓칠 수 있다.
2. **해상도** — 정보가 분봉이 아니라 **틱**에 있을 수 있다(계획서 Phase D,
   HOGA 로그 ±30초 미시버스트). 오늘 HOGA 로그만 11.9만 줄이다.

### 5. [미결] Phase B vs Phase D vs 재배치 — 사용자 결정 대기

BH 통과 0개이므로 **Phase B(확증 채널 등록)에 올릴 후보가 없다.** 선택지:
(a) 표본을 더 모아 Phase A 재실행, (b) Phase D 틱 그물 착수,
(c) 부정적 verdict 확정 후 자원을 청산기하·집행 축으로 재배치.
**사용자 결정 전까지 착수하지 않는다.**

---

## 2026-08-03 (MW0601 421차 후속10 — Phase A 결론 (a)+(c) 확정 + 재실행 알림 배선)

**사용자 결정**: Phase A가 BH 통과 0개로 끝난 뒤 제시한 3선택지 중 **(a)+(c) 병행**.
= 자원은 재배치하되 하네스는 남기고 표본이 쌓이면 재실행한다.

### 1. [결정 등록] 잠정 음성 — "확정"이 아니다

`VALIDATION_CAMPAIGN_DECISIONS["faststop_discovery"]`
= *"잠정 음성 — 진입측 판별 시도 중단, 표본 축적 후 재실행"*

**'확정'이 아니라 '잠정'인 이유를 note에 박았다** — 발굴 단계이고 확증창을 거치지
않았으며 유효표본이 69건·7일뿐이라 중간 크기 효과를 놓쳤을 수 있다. (a)와 병행한다는
것 자체가 "아직 닫지 않았다"는 뜻이다.

결정 내용 3가지:
① 신규 진입 차단 게이트 개발 중단 → 자원을 청산기하·집행 축으로 재배치
② 하네스 존치, 표본 축적 후 재실행
③ Phase D(틱 그물)는 **재실행도 전멸일 때만** — 구현이 무겁고 되돌리기 어렵다

재배치 우선순위: `[23]` 기하 재론(OOS 3일+ 후) > 지정가 집행 검증 > `[17]` 스톱 체결.

### 2. [알림 배선] 사람이 기억하지 않아도 되게 만들었다

사용자가 "언제 다시 할지 알림 주는 방안"을 요구. **별도 캘린더를 만들지 않고**
금요일 캠페인 리포트에 얹었다 — CLAUDE.md "주기적 재검증 항목" 절의 근거 그대로
(*"관리 포인트가 늘어날수록 '재검토하기로 했는데 안 함' 상태로 방치될 위험"*).
Slack은 `feedback_slack_notify_deprioritized` 결정대로 건드리지 않았다.

- `faststop_discovery.readiness()` 신설 — **순열 없이 즉시** 유효표본을 계산한다.
- `VALIDATION_CAMPAIGN["faststop_rerun_watch"]` ([33]) 사전등록 — 임계·근거·ETA.
- 리포트 `[33]` 절 — 게이지 표 + 임계 도달 시 🔔 배너. **판정 채널 아님**(verdict 없음).

| 트리거 | 조건 | 실측 ETA |
|---|---|---|
| 중간(재실행 검토) | FAST ≥ 30 & 유효거래일 ≥ 11 | 2026-08-14 안팎 |
| 본(재실행 권고) | FAST ≥ 40 & 유효거래일 ≥ 15 | 2026-08-25 안팎 |

### 3. [정정] 축적 속도 — 내 앞선 추정이 5배 이상 느렸다

세션 중 "min_samples 20 도달까지 8~16주" 류로 추정했으나, SYSTEM_AUTO 진입은
**2026-07-14부터**다(2.9주 구간). 실측:

```
포지션      27.6건/주
FAST         7.0건/주
유효 거래일   2.4일/주   ← 병목은 이쪽이다
```

FAST 건수가 아니라 **유효 거래일**(FAST·OK 공존일)이 병목이라는 점이 임계 설계의
핵심이다. 전체 거래일로 세면 트리거가 실제보다 빨리 켜져 표본 부족 상태로 재실행하게 된다.

### 4. [임계 근거] 관측 전 고정

- interim = 초판(FAST 20·유효 7일)의 약 1.5배 — "다시 돌려볼 만한가" 판단 지점
- full = 약 2배 + **부호유지 지표가 판별력을 갖는 최소 거래일 15일**.
  초판에서 leave-one-day-out 부호유지가 상위 20개 전부 7/7이라 강건성 열이 무력했다
  (421차 후속9). 그 열이 살아나는 지점을 full 트리거로 삼았다.

---

## 2026-08-03 (MW0601 421차 후속5 — 후속4의 "OOS 두 겹 반증" 기록을 정정)

**발단**: 후속4 커밋(008ae41) 직후 검산에서 스크립트 헤더의 `거래일 1일`이 눈에 띄었다.

### [정정] OOS 표본 24건이 전부 하루다 — "반증"이 아니라 관찰 신호다

```
oos_start(2026-07-31) 이후 SYSTEM_AUTO 진입 일자별:
    2026-08-03   24포지션      ← 전부
```

07-31에는 시스템 진입이 **0건**이었다(일별 집계에 07-31 행 자체가 없다). 즉 OOS는
`min_days=3` 미달이고 **313차 "단일일 판정 금지"에 정면으로 걸린다.**

게다가 08-03은 일중 레인지 **3.24%의 극단 휩소** 세션이다. 기하 대안의 성능은
변동성 체제에 민감하므로 단일 이상일 결과를 일반화할 수 없다.

**후속4가 이를 "PC 부호 역전에 이은 두 번째 반증"으로 기록하고 "결정이 강화됐다"고
쓴 것은 과장이다.** 결정을 지탱하는 근거는 여전히 **PC간 부호 역전 하나**다.

정정 반영처 3곳:
- `VALIDATION_CAMPAIGN_DECISIONS["tp1_geometry_shadow"]["note"]` — "두 겹 반증" →
  "관찰 신호", 단일일 한계와 재확인 조건(거래일 3일+) 명시
- `VALIDATION_CAMPAIGN["tp1_geometry_shadow"]` `oos_start` 주석 — 동일
- `docs/정기점검/금요일점검/0803_tp1_geometry_shadow_채널설명.md` §4·§5

### [교훈] 이번 세션에서 같은 유형의 과잉해석이 세 번 나왔다

1. `[25]` "근거가 406차로 무효화됐다" → 반증(23건은 MW0602 atr_profit 표본)
2. 배리어 경주 "기대 75.0% / p=1.3e-05" → 호라이즌 가중 누락, 정정(71.6% / 3.2e-04)
3. `[23]` OOS "두 번째 반증" → 단일일이라 판정 재료 아님

전부 **표본의 구성(모집단·거래일·호라이즌)을 확인하기 전에 결론을 낸 것**이 원인이다.
313차 원칙이 매번 잡아냈다 — 채널 수치를 인용할 때 `n`뿐 아니라 **거래일 수와
모집단 정의를 항상 함께 읽을 것**.

> 스크립트는 이미 `거래일 N일`을 헤더에 찍고 있었다. 출력이 부족했던 게 아니라
> 읽지 않은 것이다.

### [부기] 장중 스냅샷 문제

문서 작성 중 표본이 n=77 → 79로 늘었다(장중 실행). 채널 설명서에 "2026-08-03 14:53
스냅샷이며 재실행이 정본"임을 명시했다.

---

## 2026-08-03 (MW0601 421차 후속4 — [23] OOS 역전 기록 + 채널 설명서 작성)

**발단**: 0803 점검이 "급행 풀스톱의 처방 경로는 [23]뿐인데 근거 없이 방치돼 있다"고
보고했다. 확인 결과 **방치가 아니라 반증이 두 겹인 상태**였고, 그중 하나는 아직
기록되지 않은 것이었다.

### 1. [발견] OOS에서도 역전 — 405차의 우려가 실측으로 확인됐다

405차 P1-4가 `oos_start="2026-07-31"`을 넣은 이유는 *"이 채널의 결과(tp1_x2
+32.78pt)는 403차 P1-6이 55건/11일로 처음 실행하며 **가설을 세운 그 기간의
in-sample**"* 이라는 우려였다. 08-03 실행에서 그 장치가 실제로 발동했다.

| 변형 | 전체 (n=77·12일) | OOS (n=22, 07-31~) |
|---|---|---|
| current | −33.64pt | −16.46pt |
| **tp1_x2** | **−11.51 (현행 대비 +22.13)** | **−27.11 (현행 대비 −10.65)** |
| tp1_x2 승률 | 42.9% | **31.8%** (current 63.6%) |

**부호가 반대다.** 전체표본에서 가장 좋던 변형이 OOS에서는 현행보다 10.65pt 나쁘다.

→ 확정 결정 레지스트리 note와 채널 사전등록 블록 양쪽에 기록했다.
**결정(FAIL이지만 미적용 — 현행 유지)은 바뀌지 않는다 — 오히려 강화됐다.**
verdict·합격선·variants 무변경(§9-4 시계 리셋 없음).

### 2. [정정] 0803 점검이 인용한 배리어 경주 통계 — 효과 크기 과대

점검이 "랜덤워크 기대 75.0% vs 실측 49.2%, 갭 25.8%p, p=1.3e-05"로 보고했으나
**거리비를 1:3 단일로 가정**한 값이다. 실제 거리비는 호라이즌마다 다르고
(1m 5.00:1 / 3m 3.00:1 / 5m 2.14:1) 진입 구성은 **5m 45 / 3m 26 / 1m 5 포지션**이다.

| | 초판 | 정정 |
|---|---|---|
| 랜덤워크 기대 | 75.0% | **71.6%** (호라이즌 가중) |
| 실측 | 49.2% | **53.9%** (41/76) |
| 갭 | 25.8%p | **17.7%p** |
| 유의성 | p=1.3e-05 | **z=−3.41, p=3.2e-04** |

방향(현행 기하가 랜덤워크보다 못하다)은 유지되나 **효과 크기가 초판보다 작다.**
⚠ 이 통계는 아직 **사전등록 채널이 아니므로 [23] verdict에 반영하지 않는다.**

### 3. [정정] "처방 경로는 [23]/[25] 하나뿐"이라는 서술도 틀렸다

확정 결정 레지스트리에 열린 채널이 5개다 — `[23]` tp1_geometry, `[25]`
tp1_protect_offset, `[3-B]` quantile_tp("MW0601 교차확인 선행"), `[4]` signal_decay,
`[21]` direction_ev(뒤 둘은 "판정보류 — PC간 부호 역전").

또한 **`[25]`는 급행 풀스톱을 다룰 수 없다** — `[25]`의 모집단은 TP1 **도달** 건이고
급행 풀스톱은 정의상 TP1 **미도달**이라 서로소다. 손실 축을 다루는 것은 `[23]`이다.
0803 점검이 `[25]`를 "지배적 손실 모드의 유일한 처방"으로 지목한 것은 오류다.

### 4. [산출물] 채널 설명서

`docs/정기점검/금요일점검/0803_tp1_geometry_shadow_채널설명.md` — 구조적 발단
(hurst 약분 → 비율이 호라이즌만으로 결정), qty=1 상방 캡, 변형 4종, 사전등록 합격선,
전체/OOS 실측, 두 겹 반증, 재론 조건, 관련 문서 인덱스.

**이웃 채널과의 혼동 방지**를 문서 앞머리에 명시했다 — `[23]`(차원 A·B, 배리어
위치) vs `[25]`(차원 D, TP1 시점 offset) vs `[12]`(차원 E, TP1 이후 트레일, 기각 완료).

---

## 2026-08-03 (MW0601 421차 후속2 — [25] PC 출처 명기 + 두 PC 불일치 판독 규칙 사전등록)

**발단**: 0803 점검이 *"[25]의 0801 결정 근거 23건은 전부 MW0601 breakeven 표본이며
406차가 판정 모집단을 atr_profit으로 확정하면서 무효화됐다"* 고 보고했고, 사용자가
그 정정문 작성을 지시했다. **구현 전 확증 과정에서 그 진단이 틀렸음이 드러났다.**

### 1. [반증] 23건은 MW0602의 atr_profit 표본이다 — 결정은 유효하다

404차 후속3 본문이 명시하고 있었다:

> `tp1_single_contract_mode`가 **MW0602는 `atr_profit`**(session_state.json·런타임
> 로그·**DB 23건** 세 소스 일치), MW0601은 403차 기록상 `breakeven`이다.
> **MW0602 23건을 분봉 재생해 비교**: 현행 atr_profit 합계 +64.19pt …

즉 23건은 현행 판정 모집단 규칙을 그대로 만족한다. 406차(08-02)가 한 일은
**MW0601**을 breakeven → atr_profit으로 바꾼 것이며, MW0602 기반 결정을 무효화한 게
아니라 **MW0601 자신의 표본을 0에서 출발**시킨 것이다.

MW0601 0801 리포트가 이를 뒷받침한다 — `[25] 보호전환 **0건 (제외 0건)**`.
구 스크립트가 SQL에서 `AND protect_mode='atr_profit'`으로 걸러 breakeven 29행이
**조용히** 사라졌기 때문이고, 406차 주석이 정확히 그 오독("MW0601의 n=0이 '표본이
아직 안 쌓였다'로 오독됐다")을 적어뒀다. 오늘 본 `모드 불일치 제외 N건(breakeven N)`은
**406차가 넣은 투명성 기능이 설계대로 동작한 것**이지 결함이 아니다.

> **이것은 CLAUDE.md가 경고한 "이미 이행된 권고를 신규 안건으로 오인" 그 자체다** —
> 0801 결산 초안이 Hurst 완화를 "승인 1순위"로 잘못 올렸다가 코드 확인 후 철회한 것과
> 같은 실패 유형. 주간회의 안건 등록도 이미 돼 있었다(NEXT_TODO 406차 "MW0601 [25]
> min_samples 도달 ≈ 2026-08-13", 410차 "[25] 기준선이 —를 벗어나는가").
> **다행히 오진이 커밋된 파일에 유출되기 전에 잡혔다**(grep 확인).

### 2. [조치] 오진의 원인을 제거 — note에 PC 출처가 없었다

결정 레지스트리 note는 "건별 우세 8/23·7/23"만 적고 **어느 PC 표본인지 쓰지 않았다.**
`feedback_devmemory_pc_provenance`(모든 항목에 PC 표기)에 어긋나며, 실제로 그 공백이
오독을 만들었다. → note 앞에 **[MW0602 표본 23건 기준]**을 달고, 406차와의 관계
(무효화 아님 / MW0601 표본은 08-03 시작)를 함께 적었다. 리포트 본문에도 같은 문구를
노출한다. **결정·판정식·합격선은 무변경**(§9-4 시계 리셋 없음).

### 3. [사전등록] 두 PC가 어긋나면 어떻게 읽을 것인가 — 데이터 도착 전 확정

MW0601 표본이 08-03에 새로 시작했는데 **첫 표본이 MW0602와 정반대**를 가리킨다:

| 변형 | MW0601 (08-03, n=7) | MW0602 (n=23) |
|---|---|---|
| atr_lock_0.75 | **+5.05pt · 우세 6/7** | +0.92pt (drop-max **−1.33 역전**) · 우세 8/23 |
| bar_range | +3.64pt · 우세 6/7 | +0.81pt (drop-max −0.29 역전) · 우세 7/23 |

게다가 `[23]` tp1_geometry_shadow에서 **이미 PC간 부호 역전 전례**가 있고 그 결정은
"거래집합만 달라 결론이 뒤집히므로 n≈60·12일로는 기하를 결정할 수 없다(313차)"였다.
불일치 가능성이 높은데 판독 규칙이 없었다 — 결과를 본 뒤 규칙을 만들면 사후 합리화다.

**규칙(관측 전 고정)**: (1) 같은 방향 + 둘 다 drop-max 생존 → 승격 후보.
(2) **부호 반대 → 기각 아닌 보류**, 두 값 항상 병기(유리한 쪽만 인용 금지).
(3) 한쪽만 표본 충족 → 단독 판정을 확정으로 쓰지 않고 "단일 PC 표본" 명시.
(4) 어느 경우도 자동 적용 없음(§9). `mw0601_sample_start="2026-08-03"`도 함께 기록.

---

## 2026-08-03 (MW0601 421차 후속 — [31] reduce 연속배수 앵커 재도출, 419차 예고의 실행)

**발단**: 0803 정기점검에서 419차가 NEXT_TODO에 남긴 *"[31] 앵커 재도출 — 현행
HI=0.90/LO=0.45는 재보정 전(cancel 포화) 분포로 노출중립을 맞춘 값이라 분포가
이동하면 평균 배수가 0.70에서 이탈한다"* 항목이 **실제로 발동**한 것을 확인.
사용자가 즉시 재도출·구현을 지시.

### 1. 모집단 정정 — 처음 측정한 0.8205는 앵커와 다른 모집단이었다

점검 중 `joint_gate_shadow` + `toxicity_reduce_shadow` 20건으로 잰 값이 0.8205였다.
그러나 원 앵커는 **reduce 밴드 봉**(n=1,703)에서 도출됐다. 게이트 이벤트는 "진입
시도가 게이트에 닿은 분봉"만이라 필터링된 부분집합이다 — **비교 가능한 값이 아니다.**
재도출은 원 앵커와 같은 봉 모집단으로 다시 했고, 그 기준 이탈은 **0.7894(+12.8%)** 였다.

> 리포트 [31]의 `shadow_mult_mean`은 게이트 이벤트 기준이므로 앵커 도출값과 원래
> 다르다. 절대값 판정이 아니라 **추세 감시용**임을 리포트 본문에 명시했다.

### 2. 재생 방법과 자체검증

`raw_features`에 성분 stress(atr/spread/flow/queue)가 전부 저장돼 있고 P0가 건드린
성분은 cancel 하나뿐이라, ceiling만 갈아끼우면 과거 분봉 score를 정확히 재구성할 수
있다. `score_ma`는 window=20 롤링 + `reset_daily()`를 일자별로 그대로 재현.

**자체검증**: P0가 이미 라이브인 08-03 저장값과 대조해 최대오차 **5.9e-07**.
재생식이 라이브와 동일함을 확인한 뒤에야 결론을 냈다.

밴드 분포 이동도 419차 재생 예측과 부합했다(독립 재확인):

| ceiling | block | reduce | pass | cancel 포화 |
|---|---|---|---|---|
| 0.08(구) | 22.9% | 76.9% | 0.2% | 100.0% |
| 0.42(현행) | 8.0% | 72.0% | 20.0% | 1.2% |
| 419차 재생 예측 | 8.6% | 73.7% | 17.7% | 1.2% |

### 3. 재도출 — span 고정, 수준만 이동

E[t]=0.2458 (reduce 밴드 봉 n=1,807, 2026-07-24~08-03 7거래일).

```
HI = 목표0.70 + span0.45 × E[t]0.2458 = 0.8106 → 0.81
LO = HI − span                              = 0.3606 → 0.36
반올림 후 평균 배수 0.6994 (목표 대비 −0.0006)
```

**왜 span 고정인가**: 이 앵커가 통제하는 것은 *노출 수준*이다. 등급화 강도를 같이
바꾸면 [31]이 나중에 "손익이 왜 변했나"를 분리하지 못한다 — 419차가 노출중립을
설계한 논리와 동일하다. 대안도 함께 계산해 기각했다:
- (a) LO=0.45 고정 → HI=0.7815이나 span 0.331로 **등급화 약화**
- (b) HI=1.00 상한 → LO=**−0.22**로 음수 배수, 성립 불가

**안정성(313차)**: 일자별 HI 권고 0.7718~0.8865, 전체표본 권고 0.8106이 그 안에 있다.
최다기여일(07-30, n=333) 제외 시 0.8080으로 **−0.0026** — 특정일 의존 아님.

### 4. 라이브 영향 — 없음 (섀도 전용)

`evaluate()`의 reduce 밴드 `size_multiplier`는 **상수 0.7 그대로**이고 재도출된 값은
`size_multiplier_shadow`에만 들어간다. 실행 검증:

```
(score,ma,spread)   action   size_mult(실거래)   shadow
(0.30,0.20,3.0)     reduce   0.7                0.7571
(0.36,0.30,4.0)     reduce   0.7                0.5982
(0.44,0.35,5.0)     reduce   0.7                0.3865
```

그래서 장중(13:4x) 반영이 가능했다 — 421차 entry_qty 건과 같은 판단 근거다.

### 5. 재발 방지 — 앵커 신선도 자동 감지

같은 예고가 또 잊히지 않도록 `VALIDATION_CAMPAIGN["toxicity_reduce_mult_shadow"]`에
`anchor_ceiling` / `anchor_date`를 기록하고, 리포트가 현행 `TOXICITY_CANCEL_CHURN_
CEILING`과 다르면 **"⚠ 앵커 낡음"** 을 찍게 배선했다(420차 `tox_regime_date` 불일치
감지와 동일 취지 — 자동 수정은 하지 않는다).

재도출 스크립트 `scripts/toxicity_reduce_mult_anchor_rederive.py`는 재실행 가능하며,
ceiling·가중치·임계를 바꿀 때마다 돌리면 된다.

> ⚠ **[31] 판정은 2026-08-03 이후 표본부터 정상이다.** 그 이전 섀도값은 구 앵커
> (0.90/0.45)로 계산된 것이라 섞어 읽으면 안 된다 — 채널 주석에 명시했다.

---

## 2026-08-03 (MW0601 421차 — entry_qty가 shrink_initial에 깎이던 문제, 진입 계약수 필드 분리)

**발단**: 0803 정기점검에서 08-01·08-02에 등록된 08-03 확인 항목을 전수로 훑던 중,
417차 후속의 *"라이브에서 `entry_qty`가 채워지는가"* 항목이 **불합격**으로 나왔다.

### 1. 실패 재현 (오늘 첫 부분청산 포지션)

```
10:19:55 [Position] 진입 SHORT 3계약 @ 991.1     ← 진입은 분명히 3계약
trades.id=209  quantity=2  entry_qty=2          ← 3이어야 함
trades.id=210  quantity=1  entry_qty=1          ← 3이어야 함
```

사양은 *"레그별 `quantity`는 다르고 `entry_qty`는 전부 같아야 정상"*인데
**감소하는 값의 스냅샷**이 레그마다 찍혔다.

### 2. 근본 원인 — 한 필드를 수명이 다른 두 용도로 썼다

| 필드 | 수명 | 목적 |
|---|---|---|
| `initial_quantity` | 360차 `shrink_initial`이 손절1차 청산분만큼 **깎는다** | `_sync_partial_progress()`가 잔여를 "처음부터 이 수량이던 새 포지션"으로 봐야 TP1 재도달이 가능 |
| `entry_qty`(417차) | 진입 규모가 **보존**돼야 함 | 사이징 축 분석 — `quantity`(청산레그 수량)와의 혼동이 4회 반복된 것을 막으려 신설 |

417차가 `entry_qty` 출처를 하필 `initial_quantity`로 잡으면서 충돌했다.
**360차 쪽은 결함이 아니다** — 의도된 동작이고 그대로 유지했다.

> **부수 발견**: `db_utils`의 구버전 백필은 `SUM(quantity) GROUP BY entry_ts`라
> 3을 정확히 넣는다. 즉 **백필 경로는 맞고 라이브 경로만 틀린** 상태로 두 경로가
> 갈라져 있었다. 백필은 `WHERE entry_qty IS NULL`이라 오늘 행을 고쳐주지도 않는다.

### 3. 수정 — 필드 분리

`entry_quantity`를 신설해 진입 시점 값을 보존한다(진입·추가진입·브로커동기화에서만
갱신, 어떤 청산 경로에서도 감소 없음). `_build_exit_result`의 `entry_qty` 출처를
그쪽으로 교체하고, 폴백 순서(`entry_quantity or initial_quantity or quantity`)로
구버전 상태파일 복원 시 종전 동작을 보존한다.

`shrink_initial` 블록에는 *"entry_quantity는 의도적으로 건드리지 않는다"* 주석과
회귀 시 볼 테스트 경로를 명시했다 — 같은 실수의 재발 방지.

### 4. 검증

- `tests/test_entry_qty_invariant.py` **신설** — 08-03 10:19 사례를 그대로 재현하는
  케이스 포함 13건 전부 통과. 360차 축소가 여전히 일어나는지도 함께 고정한다.
- `scripts/verify_loss_tier1.py` 전부 통과 (360차 무회귀 확인).
- `trades.db` 정정: id 209·210 → `entry_qty=3`. **전수 재검 145포지션 불일치 0건**
  (오염은 오늘 1포지션뿐 — 라이브 기록 경로가 08-02에 들어왔기 때문).

### 5. 라이브 영향 평가 — 계측 전용

`trades.entry_qty`는 **라이브 소비처가 없다**. `main.py:2614`에서 쓰기만 하고
읽는 곳은 오프라인 리포트뿐이다(대시보드 `e_entry_qty`·`ensemble_decisions.entry_qty`
는 이름만 같은 별개 값). 그래서 장중(13:20) v9-dev 반영을 **사용자 승인 하에** 했다
— `project_v9_dev_branch_split`의 "실시간 파이프라인 커밋은 15:40 이후" 규칙에 대한
의도적 예외이며, 근거는 위 소비처 부재다. 실행 중인 프로세스는 08:40에 모듈을 이미
로드했으므로 오늘 남은 거래는 구코드로 돈다.

> ⚠ **따라서 오늘 손절1차가 또 발생하면 그 행은 여전히 잘못 기록된다.**
> 08-04 확인 시 `entry_qty != SUM(quantity)` 재검 후 정정할 것.

---

## 2026-08-03 (MW0601 420차 — JointGateBlock 캠페인 데이터 분석 + 419차 반영 계측/판정 개정)

**발단**: 사용자가 JointGateBlock 채널의 캠페인 도입~07-31 수집데이터를 분석하라고
지시. 이어서 **419차 커밋 내용을 반영해 Action을 수정하고 구현**하라고 지시.

### 1. 원본 116행 재분석에서 나온 것 (분석)

수집 2026-07-15 10:24 ~ 07-31 13:56, 12거래일, **n=116, 미판정 0**.
공식 판정 ✅ PASS (누적 hyp **-13.1632pt**, 승률 69.8% vs 기준선 49.3%).

| # | 발견 | 실측 |
|---|---|---|
| F1 | `tox_size`가 116건 전부 정확히 0.700 → `joint_mult<0.50 ⟺ meta_size<0.7143` | 419차 발견 ④와 동일(독립 재확인) |
| F2 | 차단표본 안에서 임계가 **한 번도 구분자가 아니었다** | 최대 meta_size=**0.7140** → joint=**0.49980**(임계 0.0002 아래). SIGNAL 로그 reduce×reduce 82건 중 통과 6건 전부 meta=0.75(클램프 천장) |
| F3 | meta_size=0.500 **73건(63%)이 학습값이 아니라 falsy 폴백** | 폴백 버킷 hyp=**+3.97pt**(차단이 손해) vs 실학습값 버킷 **-17.14pt**(차단이 이득) — **방향이 정반대** |
| F4 | n=116은 독립 관측치가 아니다 | **59 에피소드**(최대 6연속). 에피소드 첫신호 기준 -12.19pt, 에피소드평균 기준 -16.94pt·승률 59.3% |
| F5 | 통계적으로 0과 구분 안 됨 | 건당 -0.1135pt, sd 3.11, **t=-0.39(p≈0.70)**. 에피소드 단위로도 t=-0.482 |
| F6 | **하루를 빼면 부호가 뒤집힌다** | 07-28 하루가 37건(32%)·-19.66pt → 제외 시 **+6.49pt**. 에피소드 단위로는 07-27 제거 시 -12.19 → **+3.80pt** |
| F7 | 승률-손익 역설 + FAIL 조건 절반은 이미 충족 | 승 81건 평균 +1.78 / 패 35건 평균 -4.50. FAIL의 승률 조건(69.8%>49.3%)은 **이미 통과**, 손익 조건 하나만 남아 PASS |
| F8 | 리포트 고정문구가 MW0601 실측과 모순 | "MAE 평균이 MFE 평균보다 크다"가 **무조건 출력**. MW0601은 MFE 11.73 > MAE 6.68(74건 vs 42건)이라 **거짓**, MW0602(6.63<8.96)에서만 참 |

> **결론**: PASS는 "차단이 옳다"가 아니라 "FAIL이 아니다"일 뿐이다. F5·F6·F4가
> 3중으로 근거를 무너뜨린다. 다만 **완화 근거도 없다** — 방향 증거 자체가 없다.

### 2. 419차를 반영한 Action 수정

**철회**: 원안 P2(a) "tox reduce 밴드 연속화" → 419차 P1(`_reduce_mult_shadow`,
채널 [31])이 이미 구현. F1~F3도 419차 발견 ④가 이미 문서화 — 중복 등록하지 않는다.

**신규(419차가 만든 문제 — 원안에 없던 최우선 안건)**:

- **A. [7] 채널 체제 단절.** 419차 P0(`TOXICITY_CANCEL_CHURN_CEILING` 0.08→0.42)이
  tox 밴드를 이동시킨다(block 23.3→8.6% / reduce 76.4→73.7% / pass 0.27→17.7%).
  JointGateBlock 발동 전제가 `tox_action=="reduce"`라 **[7]의 모집단 자체**가
  08-03부터 바뀐다. 그런데 `resolve_and_eval_joint_gate()`는 `ts >= _campaign_start()`
  하나로 집계 → **구 체제 116건과 신 체제가 조용히 섞인다.** 419차는 [30]에
  `effective_date`를 넣었지만 [7]에는 없었다.
- **B. 419차 P1의 사각지대 = 차단 축.** [31]은 **체결된** reduce 진입만 본다.
  JointGateBlock으로 차단된 신호는 [31]에 아예 없다. 그런데 연속 배수가 실적용되면
  `joint_mult`이 바뀌어 **차단 여부 자체가 뒤집힌다**(스모크 실측: meta=0.714 ×
  tox_shadow=0.90 → 0.643 통과 / meta=0.75 × tox_shadow=0.36 → 0.357 차단).

**419차 P3 보류 근거의 약화**: 419차는 P3(Joint 임계 + meta falsy 폴백)을
"joint가 PASS라 서두를 근거 없음"으로 보류했는데, F5·F6이 **그 PASS 자체가 t=-0.39,
단일일 의존**임을 보였다. 다만 결론은 "게이트를 바꾸자"가 아니라 **"PASS의 신뢰도를
리포트가 드러내게 하자"** — P3 판단 재료를 만드는 것이라 419차 결정과 충돌하지 않는다.
...
| 파일 | 내용 |
|---|---|
| `utils/db_utils.py` | `joint_gate_shadow` 컬럼 10종 추가 + **기동 시 멱등 ALTER** |
| `strategy/entry/meta_gate.py` | 리턴에 `meta_confidence_pre_floor`/`size_multiplier_raw`/`size_multiplier_fallback` (계측 전용, 소비처는 joint 기록뿐) |
| `main.py` | joint 차단 시 meta축·tox체제·차단축 반사실 3축 기록 |
| `config/settings.py` | [7] 채널에 `tox_regime_date`/`episode_gap_min`/`evidence_t_abs`/`jackknife_enabled` **사전등록** |
| `scripts/generate_...report.py` | 체제 분리 판정 / 에피소드 재집계 / 증거강도 / jackknife / 폴백 분리 / P1 차단축 반사실 / MFE 문구 조건부화 / MFE절 위치 교정 |

**기동 마이그레이션이 핵심이다**: `CREATE TABLE IF NOT EXISTS`는 기존 테이블을
갱신하지 않는데 main.py의 INSERT는 컬럼명을 명시한다. ALTER를 리포트에만 두면
**매 차단마다 조용히 실패**해(예외를 삼키고 warning만 남기는 경로) 표본이 통째로
유실됐을 것이다. db_utils에 1차 방어선, 리포트에 백스톱을 둔다.

**판정 규약(사전등록)**: 판정은 **항상 한 체제 안에서만** 낸다. 신 체제가
`min_samples`(20)에 닿으면 그것으로 갈아타고, 닿기 전에는 구 체제로 판정하되
리포트가 "구 체제 기준"임을 명시한다. `verdict`(PASS/FAIL)는 하위호환으로 유지하고
**증거강도**를 별도 축(`evidence`: SUPPORTS_GATE / NO_EVIDENCE / REJECTS_GATE)으로
분리 표기한다. 통계 단위는 **에피소드 첫 신호**(매 분봉 재차단이라 원본 행은
독립 관측치가 아니다).

### 4. 부수 교정 — [7]의 MFE 병기 절이 [29] 뒤에서 렌더링되고 있었다

`jg`(=[7]) 데이터인데 렌더 위치가 `[28]`·`[29]` **뒤**라, 출력물에서 [29] 아래 붙은
H3처럼 보였다(HEAD에도 있던 기존 결함). `_render_joint_mfe()`로 분리해 [7] 본문
바로 아래로 이동 — 순수 이동, 계산 로직 무변경.

### 5. 검증 (py37_32)

- 기동 마이그레이션 실행 → 실제 DB 컬럼 10종 생성 확인, **기존 116행 무오염**
  (신규 컬럼 전부 NULL — 소급 채우지 않는다. meta 폴백 여부·tox 원점수는 차단
  시점에만 알 수 있어 사후 복원이 불가능하고, 억지로 채우면 구 체제 표본이
  신 체제처럼 보인다)
- `resolve_and_eval_joint_gate()` 실 DB 실행 → 수동 분석과 **전 수치 일치**
  (n=116 / -13.1632 / 69.8% / 에피소드 59 / max_run 6 / 첫신호 -12.1901 / t=-0.482)
- 합성 신 체제 30건으로 **post 판정 경로 강제 실행** → `judged_regime="post"`,
  폴백 분리·P1 차단축·tox_ceiling 태그 전 블록 렌더 확인
- main.py INSERT 20컬럼 왕복(스크래치 사본), ToxicityGate 스모크(실배수 전 구간
  0.7 불변 + 섀도 연속성), 전체 리포트 빌드 2회, `pytest 10 passed`

**한계**: 전부 오프라인. **라이브 기록 경로(신규 10컬럼 INSERT)는 미검증** —
419차 `_log_toxicity_reduce_shadow`와 같은 성격의 미검증 지점이다. NEXT_TODO 등록.

---

## 2026-08-03 (MW0601 419차 — ToxicityGate 상수화 딥다이브: 죽은 상수 2개 발견, P0 재보정 + P1 섀도 배선)

**발단**: 사용자가 `toxicity_gate.py:71`의 reduce 밴드 `size_multiplier`가 고정
상수 0.7로 출력되는 흐름을 딥다이브하라고 지시. 확인 결과 그 상수는 **표면**이었고
그 아래 더 큰 상수화가 하나 더 있었다.

### 발견 ① — 0.7은 문자 그대로 상수다 (확정)

`joint_gate_shadow` 116건 전수에서 `tox_size`의 distinct 값이 **`[0.7]` 단 하나**.
예외 0건. `docs/Ref/jointfateBlock.txt`와 `settings.py`가 제기했던 "구조적 의문"은
의문이 아니라 확정이다.

### 발견 ② — 그런데 reduce 밴드가 "거의 전부"다 (더 심각)

라이브 2026-07-24~07-31(380차 재보정 배포 이후, n=2,229분봉) 밴드 분포:
...
일자별로도 일관(pass가 0.0%인 날 5일). `[ToxicityGate]` 로그 10거래일 실측도
reduce 731건 / block 77건. 즉 **0.7은 "독성 구간 축소"가 아니라 상시 적용되는
전역 사이즈 헤어컷**이었다.

### 발견 ③ — 근본 원인: cancel_stress가 100% 포화 (P0)
...
전 분봉이 1.0으로 포화. **중앙값 score 0.3475 중 0.20(58%)이 죽은 상수**이고
실제 변동은 spread/flow만 만들고 있었다. 임계선 전체가 +0.20 오프셋된 좌표계 위에
놓인 셈.

**이건 380차가 스스로 예고한 미완 작업이다** — `toxicity.py`에 *"ceiling=0.08은
신규 피처라 과거 데이터로 재현검증 불가한 잠정치 — 라이브 섀도 관찰 몇 주 후
재보정 필요"* 라고 적혀 있었고 그 재보정이 실행되지 않았다. 380차 백테스트가
07-14~23 데이터로 임계값(0.45/0.28)을 골랐는데 그 구간엔 `cancel_churn_ratio`가
**아예 없었다**(신규 피처) — 즉 임계값은 cancel_stress를 뺀 분포로 정해졌는데
라이브에선 그 항이 상수 +0.20으로 들어왔다.

### 발견 ④ — JointGateBlock은 tox 축 정보가 0이라 meta 단일 임계와 동치

`tox_size ≡ 0.7`이므로 `joint_mult < 0.50 ⟺ meta_size < 0.7143`. 실측
**116/116(100%)** 이 그 조건을 만족 — 2축 조건이 아니라 meta 단일 임계였다.
덤으로 `meta_size`도 **73/116(62.9%)이 정확히 0.500**인데, 원인은
`meta_confidence.py:264`가 conf<0.5에서 `size_mult=0.0`을 내고
`meta_gate.py:206`의 `learned["size_multiplier"] or 0.5`가 falsy로 잡아 0.5로
**승격**시키기 때문(의도와 반대 방향). → `joint_mult`은 다수 케이스에서
0.5 × 0.7 = **0.35 이중 상수**.

### 발견 ⑤ — 양자화 체인이 배수를 다시 뭉갠다

`main.py`에 `max(1, int(round(q*m)))`가 **8단 연쇄**(6974/6980/6993/7006/7026/
7062/7076/7200). `[SizerMatch]` 실측: `3→1`(43건) `2→1`(30건) `4→1`(16건)
`5→1`(5건) `6→2`(2건). ×0.7의 실효 절감률이 유입 수량에 따라
**0%(q=1) / 50%(q=2) / 33%(q=3)** 로 비단조다.

### 그럼에도 버려지는 정보는 실재한다

상수화를 고칠 가치가 있는지 검증 — **reduce 밴드 내부만**(n=1,614):

| 5분위 | score 범위 | 15m 실현레인지 | 향후 15m 평균스프레드 |
|---|---|---|---|
| Q1 | 0.200~0.294 | 9.52pt | 2.8틱 |
| Q5 | 0.382~0.449 | **13.70pt** | **3.8틱** |

Spearman(score, 향후스프레드) **rho=+0.319 t=13.48** / (score, 15m레인지)
rho=+0.260 t=10.81 — 완전 단조, 강하게 유의.

⚠ **단, ATR로 정규화하면 레인지 상관은 rho=-0.08로 소멸**한다. 레인지 정보는
상당 부분 "변동성 수준"이고 ATR에 이미 들어 있다. **연속화의 근거는 레인지가
아니라 스프레드(집행비용) 축으로 잡아야 한다** — 그쪽은 정규화 대상이 아니다.

### 결정 — 3단계로 나누고 이번엔 P0 + P1섀도만

**P0(실적용): `TOXICITY_CANCEL_CHURN_CEILING = 0.42`** (실측 p99. 380차가 다른 4개
성분에 쓴 "p90~p99 실측 기준" 관례와 동일). 실제 `ToxicityCalculator`에 라이브
원입력을 다시 흘려보낸 재생 검증 — **구값 재생이 저장된 실측과 일치**해
경로 신뢰성 확인(block 23.5/reduce 76.4/pass 0.2 vs 저장값 23.3/76.4/0.27):
...
**임계값(0.45/0.28)은 건드리지 않았다** — 재보정 후 이동한 분포를 라이브로 본 뒤
결정할 사안이라 [30] 채널로 사전등록.

**P1(섀도 전용): reduce 밴드 연속 배수를 `size_multiplier_shadow`로 병기.**
실제 `size_multiplier`는 **상수 0.7 그대로** — 실거래 사이징 무변경.
앵커 HI=0.90@reduce_thr / LO=0.45@block_thr는 **노출 중립**으로 골랐다(실측 밴드
분포 n=1,703에서 평균 0.6932 = 현행 0.70 대비 **-1.0%**). 이게 설계 핵심 —
총 노출이 함께 움직이면 나중에 손익이 바뀌었을 때 그게 *등급화* 덕인지
*노출 증가* 탓인지 분리할 수 없다.

**P2(양자화 체인 단일화)·P3(JointGateBlock 임계 재선정 + meta falsy 폴백)은
이번에 하지 않았다.** P2는 사이징 축 전체를 건드려 CLAUDE.md ⑧ 미해제 상태에서
별도 승인 대상이고, P3은 `joint_gate_shadow`가 현재 **PASS(존치)** 라 서두를
근거가 없다(116건 resolved, 누적 hyp_pnl **-13.16pt**, TP1 도달 69.8% — 차단이
실제로 손실을 회피 중).

### 사전등록 채널 2종 (§9 원칙 — 합격선을 배포 전에 확정)

- **[30] `toxicity_recalib_watch`** — P0 후 밴드 분포가 정상 구간으로 돌아왔는지.
  PASS: pass ∈ [15%, 92%] ∧ block ≤ 12% ∧ cancel포화 ≤ 30%.
  FAIL → 임계값 재선정을 **주간회의 안건으로** (자동 변경 금지 — 임계값은 전 채널
  판정에 영향).
  ⚠ 재생 기준으론 세 조건 다 통과(17.7% / 8.6% / 1.2%)하나 **pass 하한 15%까지
  여유가 2.7%p뿐**이다. 라이브가 재생보다 낮게 나오면 그건 의미 있는 신호다.
- **[31] `toxicity_reduce_mult_shadow`** — 연속 배수를 썼다면 수량이 달라졌을지.
  1차 질문은 손익이 아니라 **실효성**: PASS(수량 상이 < 20%) = "양자화에 흡수되니
  현행 상수 유지", FAIL(≥ 20%) = 실적용 검토(단 앵커 재도출 + P2 동반 검토 안건화).
  ⚠ **PASS가 "좋다"는 뜻이 아니다** — 상수가 정보를 폐기하는 건 이미 확정이고,
  살릴 여지가 없다면 복잡도만 늘리는 변경은 안 하는 게 맞다는 뜻이다.

### 변경 파일

- `features/technical/toxicity.py` — ceiling 파라미터화(기본 0.42) + 근거 주석
- `features/feature_builder.py` — config 주입
- `strategy/risk/toxicity_gate.py` — `_reduce_mult_shadow()` + 3개 분기에
  `size_multiplier_shadow` 병기 (**실배수 무변경**)
- `config/settings.py` — `TOXICITY_CANCEL_CHURN_CEILING`,
  `TOXICITY_REDUCE_MULT_SHADOW_HI/LO`, VALIDATION_CAMPAIGN 채널 2종
- `utils/db_utils.py` — `toxicity_reduce_shadow` 테이블
- `main.py` — `_log_toxicity_reduce_shadow()` + 진입 2경로 배선
- `scripts/generate_validation_campaign_report.py` — [30]/[31] 평가·요약행·상세절

### 검증

py37_32에서 실행: 게이트 스모크(실배수 전 구간 0.7 불변 확인) / 성분 역산
오차 max=6e-07 / 실제 Calculator 재생 신구 대조 / 섀도 기록→평가 왕복(합성 21건,
FAIL 경로 확인) / 전체 리포트 빌드 / `pytest tests/` 10 passed.
⚠ **전부 오프라인**이다 — 라이브 경로(P0가 실제 밴드를 옮기는지, `_log_toxicity_
reduce_shadow`가 실거래에서 기록되는지)는 **미검증**. NEXT_TODO 참조.

---

## 2026-08-02 (MW0601 417차 — `trades.quantity`는 청산레그 수량: 계약수 축 계측 3중 수정)

**발단**: 사용자가 "호라이즌별 피처 성능"에서 시작해 기대값 → 계약수별 손익으로 내려온
딥다이브. **성격**: 계측 정합성. 매매 로직·진입 판단 무관, **판정식·합격선·`start_date`
전부 무변경** — 409차 `ef3ee59`가 [13]에 적용한 논리와 동일(집계 단위 정정은 계측 버그
수정이지 기준 완화가 아님)이므로 §9-4 검증 시계 리셋 대상이 아니다.

### 확정된 사실 — `quantity`는 진입 수량이 아니다

`trades.quantity`는 **청산 레그별 계약수**다. 편향 방향이 한쪽으로만 생긴다:
이익 포지션은 TP1/TP2/TP3로 쪼개져 **작은 수량 여러 행**, 손실 포지션은 하드스톱
전량청산이라 **큰 수량 한 행**. 실측(전체 207행):

| 청산 사유 | n | 평균 레그수량 | 평균 pt |
|---|---|---|---|
| TP2(전량) | 30 | 1.00 | +5.85 |
| TP1 부분청산 33% | 35 | 1.11 | +3.48 |
| 하드스톱(틱) | 44 | 1.89 | −2.61 |
| 하드스톱 | 30 | 2.07 | −2.86 |

→ **"큰 quantity = 패배"는 인과가 아니라 정의상 성립하는 동어반복이다.**

포지션(entry_ts) 단위로 재집계하면 CLAUDE.md ⑧이 인용하는 "3계약 이상 16전 1승"이
**21승 16패(승률 56.8%)**가 되고, 진입수량 vs 계약당손익 상관은 **rho=+0.004 (p=0.96)**
로 무관하다. 유일하게 유의한 관계는 진입수량 vs (실현손실/의도손절폭) **rho=+0.318
(p=0.015, 이상치 제거 후 +0.269 p=0.045)** — 큰 포지션은 방향을 못 맞히는 게 아니라
**손절을 못 지킨다**(손절폭 초과 비율 1계약 29.4% vs 3계약+ 68.8%).
⚠ 3계약+ 시스템 포지션 37건·손실 16건으로 표본이 얇다. 확정 결론 금지(313차).

### 수정 3건

**③ [28] `sizing_inversion_watch` 구조적 판정불가 — 2건**

`entry_source='SYSTEM_AUTO'` 필터는 문자 그대로는 출처를 거르지만, 그 컬럼이
2026-07-12(311차 후속4 `cf2d1b4`)에 신설돼 이전 129행이 전부 NULL이라 **실제로는
"07-14 이후만"이라는 날짜 필터로 작동**했다. 3계약+ 진입이 존재하던 유일한 구간
(06-10~07-10)이 통째로 빠져 qty≥3 버킷이 **표본 0건** — 감시 대상 현상을 못 보는 채널.
NULL 129행의 grade는 A=109/B=8/C=7/공백=5로 **MANUAL 0건**이고(MANUAL은 전부
`GHOST_PENDING_MISS`로 라벨) 수동 진입이 섞일 경로가 없어, `grade<>'MANUAL'`을 함께
걸어 하위호환 복원했다(`_SYSTEM_AUTO_SQL`).

추가로 `structural_block` 진단을 신설했다. qty≥3이 안 쌓이는 이유가 (a) 아직 모이는 중
(b) 계통적으로 안 생김 — 두 경우가 똑같이 `INSUFFICIENT`로만 보이는데 실제는 (b)다.
CLAUDE.md ⑧이 이 채널을 MAX_CONTRACTS 인하 근거로 **명시 지정**하고 있으므로 (b)를
(a)처럼 표시하면 ⑧ 해제가 무기한 대기한다. **verdict는 INSUFFICIENT 유지**
(합격선 무변경, 313차), 바꾼 것은 사유 가시성뿐.

> **[417차 내 자체 정정] 최초 진단 "현행 사이징 자본 때문에 3계약+가 안 생긴다"는 틀렸다.**
> 사용자의 "[28]을 제거할지" 질문을 조사하다 반증했다 — TRADE 로그 실측
> (2026-07-14~07-31): `[Sizer]` 호출 379건 중 **86건(22.7%)이 3계약 이상을 출력**했고
> **최대 6계약**까지 나왔다. 목표자본 5천만원이 사이즈를 묶고 있다는 설명은 사실이
> 아니다. 진짜 병목은 **사이저 뒤의 곱셈 게이트 체인**(`main.py`의
> `_qty_display = max(1, int(round(qty × mult)))` 연쇄 — health·pre_retrain·exec·meta·
> tox·L2·hurst)이다. 진입까지 간 69건이 전부 1~2계약으로 깎였고 **qty≥3 체결 0건**,
> 최빈 조합이 `meta≈0.50 × tox=0.70 ≈ 0.35`라 사이저 3계약이 1계약이 된다.
> → 함의가 정반대다. "영원히 죽은 채널"이 아니라 **"눌려 잠든 채널"** 이고,
> ⑧ 해제로 base_risk가 오르면 즉시 깨어난다. 코드 주석·`structural_reason` 문구·
> CLAUDE.md ⑧을 전부 이 진단으로 교체했다.

**① `_merged_positions()` 수량 `max` → `sum`**

405차가 헬퍼를 만들며 `max`를 채택하고 근거를 "부분청산으로 줄어들기 전 원래 진입
규모"라 적었으나, 409차가 실측으로 반증했다(`2026-07-10 11:37:04` = TP1 2 + 하드스톱 3
= 5계약, max면 3). 409차는 [13]만 고쳐 **한 파일 안에서 [13]=sum / [28]·[29]=max로 규칙이
엇갈린 상태**였다. `max`의 편향이 특히 나쁘다 — 큰 계약수 구간에 손실만 남겨 [28]이
탐지하려는 역전을 스스로 만든다(캠페인 구간 qty≥3 격차 max 1,313,295원 → sum 700,485원,
**87% 부풀림**).

**② `trades.entry_qty` 컬럼 신설 — 근본 수정**

같은 사고가 네 번 반복됐다(311차 인지만 → 402차 후속5 승률 65.5% 과대 → 405차 max 채택
→ 409차 [13]만 수정). 분석 측 관례로는 못 막으므로 기록 측에 값을 남긴다.
`PositionTracker.initial_quantity` → `_build_exit_result()` → `_record_trade_result()`.
구버전 행은 `(entry_ts, direction)` 레그 **단순 합**으로 백필.

> ⚠ **중복 제거를 하려다 반증됐다 — 반복 금지.** `stuck_exit_flat`/`stuck_exit_remainder`가
> 같은 청산의 이중 기록으로 보여 (exit_ts, quantity, net_pnl_krw) 중복을 접으려 했으나,
> TRADE 로그 `[Position] 진입 N계약`과 대조하니 **단순 합이 맞았다**:
> 06-26 11:01:58 로그 6 = 2+2+1+1(중복제거 5 ✗) / 07-01 13:00:01 로그 6 = 5+1 /
> 07-03 13:20:01 로그 8 = 1×8(중복제거 7 ✗). flat/remainder는 진짜 별개 레그이고,
> 1계약씩 같은 가격에 나가면 값이 우연히 같아질 뿐이다. **정상 레그가 동일값을 갖는
> 경우가 실재**하므로 값 기반 중복 제거는 안전하지 않다.

### 검증

- py_compile py37_32·py310_64 rc=0, flake8 F821/F823/F811/E999 신규 0건
  (main.py F811 2건은 11402/12099로 수정 범위 밖 기존 항목)
- 백필: 로그 `[Position] 진입 N계약`과 **6/6 일치**, NULL 잔여 0건. 적용 전 백업
  `data/db/trades.db.bak_410_20260802_181418`
- **동일 데이터로 수정 전/후 리포트 재생성 → 33채널 verdict 변동 0건**
- [28] 포지션 55→63, 일수 11→13, **qty3+ 0건 → 7건**(승률 42.9% 평균 -670,606원),
  계약수 출처 전부 `entry_qty` 기록값
- [29]는 verdict PASS 유지하나 `neutral_trend` 평균이 +56,323 → **-108,594원으로 부호
  반전**(추가된 8포지션이 06-10~07-10 대형 손실). 판정 기준은 승률 격차라 무영향이지만
  이 채널 수치를 인용할 때 주의할 것.

### CLAUDE.md ⑧ 근거 정정 (사용자 지시로 후속 수행)

**먼저 원 수치를 완전 재현해 산출 방법을 확정했다** — 추정으로 고치지 않기 위해서다.
행 단위 집계로 §9-3의 값이 그대로 나온다: 1계약 173건 +16,825,435 / 2계약 18건
+1,380,597 / 3계약+ 16전 1승 -26,422,808, 이항검정 5.00e-07(원문 5.07e-07, p_win 반올림
차이), 최악2일(07-09·07-10) 제외 **10전 1승 -12,360,677원 정확 일치**, 카운터팩추얼
**+12,740,217 / +8,654,999 / +3,879,482 / -8,216,776 세 값 모두 완전 일치**.
→ 원 산출은 행 단위임이 확정됐다.

| 원 서술(행 단위) | 포지션 단위 재계산 |
|---|---|
| 3계약+ **16건 6.2%(1승 15패)** -2,642만원 | **39포지션 22승 17패(56.4%)** -1,174만원 |
| 이항검정 **p=5.07e-07** | **p=0.9312** — 유의성 소멸 |
| 최악2일 제거 후 **10전 1승 -1,236만원** | **31전 19승(61.3%) +45만원** 흑자 |
| 카운터팩추얼 **완전 단조** | **단조 아님** — 1계약 +103 / 상한2 **+157(최대)** / 상한3 +60 / 상한5 -226만원 |

**변하지 않는 것**: 총 실현손익 -8,216,776원(집계 단위 무관), 구간 분해
(~07-10 75포지션 -881만 / 07-14~ 58포지션 +60만), **"어떤 상한이든 흑자·상한 없음만
적자"**. → ⑧의 두 조건은 유지. 죽은 것은 "완전 단조"와 "사이저가 역예측적"이라는 해석.

**추가 분해**: 손실이 "3계약 이상"에 고르지 않다 — 3계약(11승/17, +145만)·5계약
(3승/3, +313만)은 흑자이고 **6계약 이상 9포지션에 -1,609만원이 몰려** 있다.
진입수량 vs 계약당손익 rho=+0.004(p=0.96)로 무관하고, 유의한 것은 진입수량 vs
손절폭 초과 배율 rho=+0.318(p=0.015)뿐이다. 원인은 방향이 아니라 **집행(손절 체결)**.

**조치**: 416차가 SHAP 근거에 한 것과 동일 — **결정 유지, 근거 문구 정정.**
- `CLAUDE.md` ⑧에 무효 수치 4건을 표로 명시하고 대체값·재분해 결과·표본 한계 병기.
  ⑧ 2번이 지정하던 근거 채널([28])이 구조적 판정불가임도 함께 적어, 그 PASS/FAIL을
  기다리는 무기한 대기를 막았다.
- 원 출처 §9-3에는 🔴 정정 배너만 달고 **본문은 보존**했다(원본 훼손 방지).
  두 문서가 어긋나 다음 세션이 §9-3을 직접 인용하는 것을 막는 것이 목적이다.

---

## 2026-08-02 (MW0601 416차 — 414차 잔여 2건 확정: 범위 18일로 정정 + CORE 강등 근거 1건 무효)

**발단**: 414차가 범위 밖으로 남긴 2건(05-18~06-15 양상 분해 / 오염 창의 SHAP 인용 대조)을
사용자가 확정 지시.

### [1] 05-18~06-15는 **정상**이다 — 오염 범위를 22 → **18거래일**로 정정

414차가 "22거래일"이라 쓴 것은 **과다 계상**이었다. 직접 증거(비영 집합이 이름 배열의 앞
K칸 부분집합인가)로 전수 판정한 결과:

| 구간 | 거래일 | 판정 | 근거 |
|---|---|---|---|
| 2026-05-18~06-15 | 19 | **정상** | 비영이 25~76개로 **매일 변동**하고 프리픽스가 아니다 |
| 2026-06-17~07-10 | **18** | 오염 | 비영이 pkl 앞 12칸(06-29~30은 121배열 앞 14칸)의 부분집합으로 고정 |
| 2026-07-13~07-31 | 14 | 정상 | 기록수 = 모델 차원, 전부 비영 |

**왜 05-18~06-15는 정상인가 — 그때는 호라이즌 슬라이싱이 없었다.** 슬라이싱 로그
(`1m 호라이즌 피처 슬라이싱`)는 **2026-06-17에 처음 등장**한다. 그 전에는 1m 모델이 전체
피처셋으로 학습됐고, 그래서 `ShapTracker.feature_names` 길이 = 모델 차원이 매일 일치했다
(LEARNING 로그의 `managed feature set 적용: N개`와 그날 SHAP 기록 피처수가 91/89/105/101/97로
날짜별 전부 일치). 즉 **오염은 슬라이싱 도입(06-17)과 함께 시작해 337차 가드(07-13)로
끝났다** — 시작과 끝이 둘 다 코드 변경 시점과 정확히 맞는다.

### [2] 오염 창의 SHAP 인용 — **CORE 강등 근거 1건이 무효**

`DECISION_LOG` 전수 스캔에서 오염 창 날짜의 SHAP 언급은 1줄(288차 SGD 재설계의 지나가는
서술)뿐이었으나, **`CLAUDE.md` 절대원칙 §3에 2026-06-25자 CORE 강등 2건이 SHAP을 근거로
기록돼 있다.** 06-25는 오염 창 한복판이다.

> `macro_vix`는 2026-06-25 CORE 강등. … **SHAP 기여 ≈ 0** …
> `macro_risk_off`는 2026-06-25 CORE 해제. … (GBM gain=0, **SHAP=0**)

실측: 그날 두 피처의 SHAP은 353행 전부 **정확히 0.0**이다. **그러나 그날 비영은 12개뿐이고
나머지 85개가 전부 0.0이었다** — 앞 12칸(cvd_*/ofi_*/vwap 블록) 밖의 어떤 피처를 조회해도
0이 나왔다는 뜻이다. 따라서 **"SHAP=0"은 그 피처에 대해 아무 정보도 담고 있지 않다.**
진짜 중요도가 얼마였든 0으로 읽혔을 것이다.

**단, 결정 자체는 뒤집히지 않는다 — 독립 근거가 남아 있다.**

| 피처 | 무효가 된 근거 | 살아있는 독립 근거 | 실측 재확인 |
|---|---|---|---|
| `macro_vix` | "SHAP 기여 ≈ 0" | 일봉 VIX라 **분봉 구간 상수**, 임계 27.5가 평상시 항상 통과(게이트 무발동) | 현재 10m·30m 피처셋에 보조 피처로 **유지 중** — 강등은 "CORE에서 뺀다"였지 제거가 아니었고 그대로다 |
| `macro_risk_off` | "SHAP=0" | **모든 호라이즌 feature_names_hz 미포함** — 모델이 애초에 쓸 수 없다. 체크리스트·SGD 경로에도 없음 | 6개 호라이즌 pkl 직접 로드 결과 **전 호라이즌 미포함** 재확인 |

특히 `macro_risk_off`의 주 근거("모델 입력에 아예 없다")는 SHAP과 **논리적으로 독립**이고
지금도 참이다. `macro_vix`의 "일봉 → 분봉 상수"도 데이터 구조상의 사실이라 SHAP과 무관하다.

→ **조치: 결정 유지, 근거 문구만 정정.** `CLAUDE.md` §3의 두 줄에 "SHAP 근거는 414차에서
무효 확인(오염 창)"을 병기한다. 결정을 되돌리는 것이 아니라, **다음 사람이 그 SHAP 수치를
재인용하지 못하게** 막는 것이 목적이다.

### 다른 곳은 깨끗하다

`dev_memory/NEXT_TODO.md`·`docs/미륵이고도화2/`에서 오염 창 날짜를 명시한 SHAP 근거 인용은
없었다. 07-13 딥다이브 보고서의 SHAP 언급(F1)은 `active_features` 레지스트리 대조에 관한
것이지 중요도 수치 인용이 아니다. **`replace_log`가 비어 있어 자동 교체가 없었다는 414차
확인과 합쳐, 오염이 실제 피처셋 변경으로 이어진 경로는 끝까지 없었다.**

---

## 2026-08-02 (MW0601 415차 — SHAP 정렬 하드닝: "우연히 맞음"을 구조적 보장으로)

**발단**: 414차가 남긴 취약점("현재 정상인 이유가 가드가 아니라 우연한 일치")을 사용자가
구현 지시. **성격**: 계측 정합성 하드닝. 매매 로직·진입 판단 무관, 판정식·합격선 무변경.

### 착수 전 확인 — 지금 살아있는 결함은 없다

3m/5m은 `get_available_feature_set()`(json 명세 ∩ 레지스트리)로 라벨을 만드는데, json
명세 13개 중 레지스트리에 없는 1개가 빠져 12개가 되고 그게 배포 pkl의 12개와 **순서까지
일치**함을 실측 확인했다(1m도 8/8 일치). 즉 오염은 현재 없다 — 다만 그 일치는 구성상의
우연이지 보장이 아니다. 명세나 레지스트리가 바뀌어 "개수는 같고 순서만 다른" 조합이
나오는 순간 값이 조용히 잘못된 이름에 붙는다. 그 창을 닫는 것이 이번 변경이다.

### 변경 4건

1. **`main.py:_refresh_shap_horizon` — 라벨 출처를 배포 pkl 우선으로**
   (`self.model.horizon_feature_names[hz]` → 없을 때만 `get_available_feature_set`).
   337차가 1m(`_ensure_shap_tracker`)에만 적용한 수정을 3m/5m으로 확장한 것이다.
   `horizon_feature_sets.json`은 "다음 재학습 계획"이지 배포 스펙이 아니라는 337차·395차·
   413차의 반복 확인을 라벨 경로에도 반영한다.
2. **`main.py:_shap_names_ok()` 신설** — 두 경로 공통 사전 게이트.
   `len(names) != model.n_features_in_`이면 계산 자체를 하지 않는다.
   추가로 3m/5m은 저장 직전 `len(score_map) != len(names)`도 본다(최종 방어).
3. **`shap_tracker._calc_importance()`에 선행 가드(경로 0) 추가** — 경로 1~3의 길이
   검사는 전부 `self._n_features`(이름 배열 길이)만 봐서 모델 차원과의 어긋남을
   구조적으로 못 잡았다. 337차가 경로 4에만 넣은 가드를 앞단으로 끌어올려 1~4를 한 번에
   막는다. **414차 오염이 통과한 바로 그 구멍이다.**
4. **`compute_horizon_importance()`에 차원 가드 + zip 전 길이 검사** —
   `zip(feature_names, imp)`은 길이가 다르면 조용히 짧은 쪽에서 잘린다. 그게 정확히
   22거래일 오염의 실패 유형이라 명시 검사를 넣었다.

### 설계 원칙 — 건너뛰되 조용하지 않게

SHAP은 STEP6 임계경로에서 도는 표시·심사용 계측이므로 **예외를 올리지 않고 건너뛴다**
(진입 판단에 쓰이지 않으므로 스킵의 대가가 없다). 다만 조용한 스킵은 337차의 "동적피처
탭이 몇 시간째 갱신 안 됨" 혼란을 낳으므로 경고를 남기되, 불일치는 재학습 전까지 계속
유지되고 이 경로는 매분 돌기 때문에 **조건별 하루 1회로 스로틀**한다
(`_shap_guard_logged`, `_last_dim_warn_date`).

### 검증

- 가드 차단: 이름 12 vs 모델 8 → `_calc_importance` None + 경고 1회,
  **2회차는 무로그(스로틀 동작)**. `compute_horizon_importance`도 None.
- 정상 통과: 이름 12 vs 모델 12 → 가드 통과 후 계산 경로 진행.
- **실배포 회귀 없음**: 실제 `gbm_{1,3,5}m.pkl` + 실제 pkl 이름으로 1m 8/8, 3m 12/12,
  5m 12/12 전부 게이트 통과하고 score_map 이름 집합이 입력 이름과 완전 일치.
- py_compile py37_32·py310_64 rc=0. `main.py`의 `datetime` 사용 형태가 기존 용례와
  동일함을 AST로 확인(py_compile이 못 잡는 런타임 NameError 방지).

### 남은 것

414차가 등록한 두 항목은 이번 범위 밖이다 — 05-18~06-15(15일) 양상 분해, 그리고 오염된
22거래일 SHAP 순위를 인용한 사람 판단이 있었는지 세션 로그 대조.

---

## 2026-08-02 (MW0601 414차 — SHAP 이름↔값 정렬 어긋남 **확정**, 22거래일 기록 무효)

**발단**: 413차가 정황으로만 남긴 "06-29~30 SHAP 정렬 어긋남"을 사용자가 딥다이브 지시.
**결론**: 정황이 아니라 **확정**이다. 그리고 **범위가 2일이 아니라 22거래일**이다.
성격상 계측 오염이며 매매 로직·실거래에는 영향이 없다(SHAP은 표시·심사·DB 관측 전용).

### 확정된 불변식 — "비영 중요도가 이름 배열의 앞 K칸을 정확히 차지한다"

413차는 "비영이 알파벳 첫 14개"라고 봤지만 그건 **06-29~30 이틀에만 성립하는 우연**이었다.
인접일을 비교하니 진짜 불변식이 드러났다.

| 날짜 | 기록 피처수 | 비영 | 비영 집합의 정체 |
|---|---|---|---|
| 2026-06-17~06-26, 07-01~07-10 (20일) | 97 | **12** | `feature_names.pkl` **앞 12칸과 집합 완전 일치** (cvd_divergence·cvd_direction·cvd·cvd_slope·cvd_exhaustion·cvd_exhaustion_signal·vwap_position·above_vwap·ofi_norm·ofi_pressure·ofi_imbalance·ofi_reversal_speed) |
| 2026-06-29~06-30 (2일) | 121 | **14** | 그 주에 배열이 121개로 바뀌었고 그 배열은 앞부분이 알파벳순 → **앞 14칸**(= 알파벳 첫 14개로 보였던 것) |
| 2026-07-13~07-31 (14일) | 8~12 | 8~12 | 기록수 = 모델 피처수, **전부 비영 → 정상** |

즉 K는 그때그때 **모델의 실제 피처 수**이고, 값은 길이 97/121짜리 **이름 배열의 0..K-1
위치**에 얹혔다. 06-26의 비영 12개는 CVD/OFI 블록인데 이는 레지스트리 선두 블록일 뿐이고,
같은 시기 1m 모델이 실제로 쓰던 피처셋과 무관하다(현재 1m 실피처는 cvd_delta_norm·
toxicity_score·cancel_add_ratio·spread_ticks·vwap_position·is_open_volatile·bb_position·
poc_distance로 전혀 다르다).

우연일 확률은 사실상 0이다 — 진짜 모델의 비영 집합이 하필 배열 앞 12칸과 정확히 일치할
확률은 1/C(97,12) ≈ 10⁻¹⁵이고, 그것이 **20거래일 연속** 같은 집합으로 반복됐다.

### 무엇이 어긋났나

`ShapTracker.feature_names`가 **모델의 호라이즌 피처셋이 아니라 전체 레지스트리 배열**
(97/121)이었고, 중요도 벡터는 모델 차원(12/14)이었다. `_calc_importance`의 길이 검사는
`self._n_features`(= 이름 배열 길이) 기준이라 이 어긋남을 잡지 못했다.

**경계가 수정 시점과 정확히 일치한다** — 07-13부터 증상이 사라지는데, 337차가 넣은 두 가드가
바로 그 무렵이다: ① `_ensure_shap_tracker()`가 `self.model.horizon_feature_names`(실배포
모델의 피처셋)를 최우선으로 쓰도록 변경 ② `_permutation_importance_fallback()`에
`X.shape[1] != model.n_features_in_` 스킵 가드 추가. 337차 주석이 적은 증상("동적피처 탭이
몇 시간째 갱신 안 됨")과 여기서 관측한 오염은 **같은 뿌리의 다른 발현**이었다.

### 영향 — 의사결정으로 번지지는 않았다

- `data/db/shap_tracker_history.json`: `replace_log: []`, `last_replace: null` —
  **SHAP 심사가 피처를 교체한 적이 한 번도 없다.** 오염된 순위가 교체를 유발하지 않았다.
- 히스토리 deque에 남은 것은 2026년 29·30·31주뿐이고 셋 다 `len=8`(정상 구간).
  즉 오염 구간 배열은 이미 롤오프돼 현재 상태에 남아 있지 않다.
- 남은 영향은 **그 22거래일 동안 대시보드 동적피처 탭과 주간심사 rank_table이 잘못된
  이름-값 쌍을 표시**한 것. 그 기간 SHAP 순위를 인용해 내린 판단이 있다면 재검토 대상이다.
- **05-18~06-15(15일)은 다른 양상**이다(기록 89~105, 비영 25~76). 앞 K칸 불변식이 성립하지
  않아 같은 결함으로 단정할 수 없다 — 호라이즌 슬라이싱 이전이라 모델이 실제로 다피처였을
  가능성이 있다. **미분해로 남긴다.**

### 남은 취약점 (코드 변경은 하지 않음 — 사용자 판단 대기)

현재 경로는 정상이지만 그 이유가 "가드"가 아니라 **"우연히 일치하기 때문"**인 지점이 있다.
`_prep_shap_xy()`는 `h_idx = [model.feature_names.index(n) for n in h_names]`로 열 순서를
`h_names` 순서에 맞추는데, 길이만 같고 **순서가 학습 시점과 다르면 아무 가드도 걸리지
않는다**(`n_features_in_` 검사는 개수만 본다). 지금은 `h_names`가 `feature_names_1m.pkl`
그대로라 일치하지만, 이는 구조적 보장이 아니라 구성상의 우연이다.
→ 권고: `save_shap_scores()` 직전에 `len(score_map) == model.n_features_in_`
어서션을 추가하거나, `h_names`와 pkl 순서 동일성을 명시 검증할 것.

---

## 2026-08-02 (MW0601 413차 — 395차 "07-27 배포 확인" 기록 반증 + 배포 결정 경로 확정)

**발단**: 412차가 미분해로 남긴 "07-27 pkl에는 어떻게 들어갔는가"를 사용자가 분해 지시.
**결론 2줄**: ① **395차의 배포 확인 기록은 사실이 아니다** — 재역전은 없었고 07-27에도
제외돼 있었다. ② **"active_features에 없으면 json에 뭐라고 적든 pkl에 못 들어간다"는
확정** — 단 필터가 통째로 미적용되는 예외 경로 3개가 있다.

### [반증] 395차 기록 — 독립 증거 3개가 전부 반대를 가리킨다

395차는 `horizon_feature_sets.json` 1m note에 *"feature_names_1m.pkl(07-27 15:45 재학습)
직접 로드로 실제 배포 확인 — need_add→in_pkl. shap_tracker.db에도 오늘 하루(270분봉)
매분 정상 스코어링, 평균 중요도 0.0069(10개 중 9위)"*라고 적었다. 셋 다 어긋난다.

| 증거 | 395차 주장 | 실측 |
|---|---|---|
| **재학습 로그** | 07-27 재학습본에 배포됨 | 그 재학습(15:45:39)이 `[FeatureReg] 1m: 2개 피처 미가용 (need_add) → 제외: ['queue_directional_depletion', 'micro_regime_code']`를 **명시적으로 찍었다** |
| **슬라이싱 카운트** | 1m 10피처 | 07-27 `97 → 8개`. 07-31과 **동일**. 전 기간 유니크값은 12/14/8뿐 — **10이었던 적이 없다** |
| **shap_tracker.db** | 07-27 하루 270분봉 매분 스코어링, 0.0069 | 07-27 두 피처 행수 **0**. 07-27 1m은 모델과 같은 8피처·각 169분봉이고 0.0069/0.0050에 해당하는 값도 없다(근접값은 poc_distance 0.0059·cvd_delta_norm 0.0053) |

**실제 배포 이력은 2026-06-29~30 이틀뿐이다.** 그 이틀만 `managed feature set 적용:
121개`였고(06-26·07-01은 97개) 1m 슬라이싱이 `121 → 14개`, `FeatureReg 미가용` 줄도
없었다. shap_scores의 두 피처 기록도 정확히 그 이틀(각 640행)이 전부다. 즉 395차가
인용한 SHAP 정황은 **한 달 전 창의 데이터**이며, 07-27 pkl은 처음부터 8피처였다.

교훈은 337차·394차와 같은데 한 겹 더 깊다 — 텍스트 로그뿐 아니라 **"직접 로드로
확인했다"는 기록조차 재검증 대상**이다. 이번엔 retrain 로그가 반증의 근거가 됐다.

### [확정] 배포 피처셋을 결정하는 경로

```
raw_features 전 구간 키 합집합                     (현재 153)
  └→ shap_feature_registry.json:active_features 로 교집합   ★관문1  (현재 97)
       └→ get_available_feature_set(hz, ...) = json include ∩ 위 결과   ★관문2
            └→ result["replaced"] 일 때만 feature_names_{hz}.pkl 저장   ★관문3
```

**"active_features에 없으면 json 명세만으로는 pkl에 들어갈 수 없다" — 참이다.** 관문1이
관문2보다 위에 있어서, json에 아무리 적어도 레지스트리에 없으면 후보 집합 자체에서
빠진다. 1m이 `97 → 8`인 것은 json include 10개 중 **정확히 그 2개가 탈락**한 결과다.

**단, 무조건 참은 아니다 — 필터가 통째로 미적용되는 예외 경로 3개**
(`learning/batch_retrainer.py:_load_from_db`):

1. 레지스트리 파일 부재·파싱 실패 → `managed feature set load 실패` 경고 후 **전체 합집합**
2. `active_features`가 빈 리스트 → `if active_features:` 미진입
3. `active_features ∩ raw키`가 공집합 → `if managed:` 미진입

이 경우 학습 행렬이 raw 전체(153)가 되므로 **json 명세가 그대로 통과한다.** 06-29~30의
121개도 같은 축의 사건이다(그때는 레지스트리 자체가 121이었음). 이번 조사에서 07-27·
07-31 로그에 `managed feature set 적용: 97개`가 정상으로 찍힌 것을 확인해 예외 경로가
아니었음을 먼저 배제했다.

**관문3(별도 축)**: pkl은 모델이 실제 교체된 경우에만 저장된다. 즉 레지스트리에서 피처가
빠져도 그날 모델가드가 교체를 거부하면 **옛 pkl이 그대로 남는다** — "레지스트리 상태"와
"pkl 상태" 사이에 최대 며칠의 지연이 구조적으로 존재한다. 배포 확인은 여전히 pkl 직접
로드가 유일한 진실이지만, **그 pkl이 언제 쓰인 것인지(mtime)를 함께 봐야 한다.**

### [부수 발견] 06-29~30 SHAP 이름↔값 정렬 어긋남 정황

그 이틀 1m SHAP은 **121개 이름 전부**에 값을 기록했는데, 0이 아닌 것이 정확히 14개이고
그 14개가 **알파벳 순 첫 14개**(above_vwap·atr·…·cvd)와 일치한다. 모델은 14피처였다.
길이 14짜리 중요도 벡터를 길이 121짜리 이름 목록에 zip한 것으로 보인다 — 그렇다면 그
기간 SHAP 기록의 **이름-값 대응이 전부 틀렸다**. 07-27은 8이름/8값으로 일치해 현재
경로에는 이 증상이 없다. **정황 증거이고 SHAP writer 코드는 아직 읽지 않았다** —
`NEXT_TODO.md`에 검증 항목으로 등록한다.

### 정정한 산출물

412차가 근거로 옮겨 적은 "395차 07-27 배포 확인 → 07-31 재역전" 서술이 반증됐으므로
`config/settings.py:FEATURE_SET_DECISIONS["queue_directional_depletion"]` 노트와
`horizon_feature_sets.json` 1m 2종 note를 정정했다. **결정 자체는 유지된다** — 근거 ①
(1m 영구 퇴역)·②(감쇠곡선상 타 호라이즌 여지 없음)는 그대로이고, 오히려 "배포된 적이
거의 없다"는 사실이 종결 근거를 강화한다.

---

## 2026-08-02 (MW0601 412차 — 1m 갭 2종 사용자 결정 + json `pkl` 표기 11건 정렬)

**발단**: 411차가 "권고이지 결정이 아니다"라며 보류한 1m 갭 2종을 사용자가 확정.
**관련**: 411차(레지스트리), 07-30 실행계획 항목 F, 337차·394차·395차(계획문서≠배포스펙)

### 조사 중 발견 — 배포 상태가 다시 뒤집혀 있었다

사용자 질문("두 피처가 무엇이고 제외 이유는")에 답하려 pkl을 직접 열어보니, 395차가
2026-07-27 재학습본에서 **배포 확인**했다고 기록한 두 피처가 07-31 재학습본(8피처)에는
없었다. `active_features`(97)에도 없다.

메커니즘은 확정했다 — `batch_retrainer`가 `get_available_feature_set()`으로 호라이즌
명세를 **학습 행렬에 실제 존재하는 피처와 교집합**하는데
(`features/horizon_feature_registry.py:115`), 학습 행렬은 레지스트리 기준이다. 즉
**레지스트리에 없는 한 json 명세만으로는 pkl에 들어갈 수 없다.** 07-27에 어떻게
들어갔었는지는 이번에 확정하지 못했다(다른 경로 추정) — 미분해로 남긴다.

### 사용자 결정 2건 — 같은 갭인데 처방을 달리했다

`suppress=True`를 통째로 적용하면 후보 재고에서 **전 호라이즌 통틀어** 빠지는데,
항목 F가 말한 제외는 "1m 편입 작업을 하지 않는다"라 범위가 어긋난다. 그래서 갈랐다.

- **`queue_directional_depletion` → `suppress=True` (종결)**. ① 1m은 331차 후속2로
  영구 퇴역(`ENSEMBLE_WEIGHTS['1m']=0.0`, 퇴역 근거 방향적중률 47.75% z=-2.82)이라
  피처셋을 고쳐도 거래 결과가 안 바뀐다. ② **타 호라이즌 승격 여지가 없다** —
  `features/feature_decay.py` 감쇠곡선이 (1.0, 0.8, 0.5, 0.1, 0.0, 0.0)로 1m 최대·10m
  이후 0으로 사전 정의돼 있다(호가 큐 방향별 고갈 강도, 성격상 초단기 전용).
  ③ 07-27 배포됐을 때조차 SHAP 중요도 0.0069로 10개 중 9위.
- **`micro_regime_code` → `suppress=False` (1m만 제외)**. 1분 lag를 허용하는 레짐
  코드(0=횡보~4=급변)라 초단기 전용이 아니고, 10m·30m include 명세에도 같은 이름이
  올라 있다(셋 다 미배포). 타 호라이즌 유효성은 검증된 적이 없으므로 월간 L1'
  스크리닝이 계속 평가하게 둔다. **1m 하나 때문에 전 호라이즌 후보 자격까지 닫는
  것은 과잉이다.**

### `horizon_feature_sets.json` `pkl` 표기 11건 정정

1m 정렬을 지시받아 전 호라이즌을 대조하니 **같은 유형이 11건**이었다(전부
`in_pkl`이라 적혀 있으나 실제 pkl에 없음): 1m 2종, 5m `opt_chain_pcr`, 10m
`opt_gex_sign`·`micro_regime_code`, 30m 6종. 공통 원인은 위와 같은 교집합 탈락이다.

**동작 변화 0을 먼저 확인하고 손댔다** — `pkl` 필드는 `get_feature_set()`이 `name`만
읽으므로 코드가 소비하지 않는 **문서 메타데이터**다. 정정 후 `get_feature_set()`
반환 개수가 전 호라이즌 동일함(1m 10 / 3m 13 / 5m 13 / 10m 15 / 15m 14 / 30m 18)을
재확인했다. JSON은 무손실 왕복(개행 1자 차이)이라 프로그램 정정으로 diff를 최소화했다
(24 insertions / 23 deletions).

`opt_chain_pcr`에는 "⚠ 30m 그룹 CORE — 체크리스트 게이트로는 계속 소비된다. 미배포는
'모델 입력이 아니다'라는 뜻이지 CORE 지위와 무관하다"를 병기했다. 이 정정을 CORE
강등으로 오독하면 절대원칙 §3 위반이 된다.

### 검증

py_compile py37_32·py310_64 rc=0. 레지스트리 16건 필수필드 누락 0. json 재대조
불일치 **0건**. 후보 재고 32 → **31건**(queue_directional_depletion만 제외).
orphan 0건.

---

## 2026-08-02 (MW0601 411차 — 피처셋 주기점검 Phase B: 피처 확정 결정 레지스트리)

**계획**: `docs/Spec for feature/피처셋_주기점검_자동리포트_구현계획_2026-08-02.md` Phase B
**관련**: 410차(Phase A), 404차 후속11(`VALIDATION_CAMPAIGN_DECISIONS` — 같은 문제·같은 해법)

### 왜 필요했나

410차 §4 후보 현황판이 매주 POOL/pending을 다시 훑어 상태를 **재계산**하므로, 이미 판단이
끝난 피처도 매주 "축적중/검증가능"으로 다시 뜬다. 기록이 없으면 다음 세션이 같은 검정을
반복하고, 우연히 유의가 나오면 반증된 피처가 재승격된다. 410차 첫 산출의 후보 36건에
`basis_pt`·`cvd_direction`이 그대로 섞여 있었던 것이 실제 사례다.

### `config/settings.py:FEATURE_SET_DECISIONS` 신설 — 14건

스키마는 `VALIDATION_CAMPAIGN_DECISIONS`를 따르되 3개 필드를 더 뒀다.
`suppress`(재고 제외 여부) · `re_eval`(재론 조건) · **`source`(근거 위치, 필수)**.

**등록 원칙 — 넣지 말아야 할 것이 넣어야 할 것보다 중요하다.** 근거 없는 등록은 실제로
기각된 적 없는 후보를 영구히 숨기는 것과 같다. 그래서 세 가지를 명시적으로 배제했다.

1. **수급 계열(`foreign_futures_net` 등)을 등록하지 않았다.** 394차가 비용차감 후 생존
   신호 0/189를 확인했지만(IC t=-3.51인데 일중 -1.087pt/일), 그것은 **단독 게이트로 쓸 때**의
   결론이지 GBM 입력에서 빼라는 결론이 아니다. "단독 신호로 돈이 안 된다 ≠ 모델 입력으로
   무용하다"를 레지스트리 헤더에 규약으로 못 박았다.
2. **1m 레지스트리 갭 2종(`micro_regime_code`·`queue_directional_depletion`)을 등록하지
   않았다.** 07-30 실행계획 항목 F가 "명시적 제외 결정 **권고**"라고 썼을 뿐 결정은 아직
   내려지지 않았고(0단계 완료 조건 4에 미결로 남아 있다), 권고를 결정으로 승격시키는 것은
   레지스트리의 권한 밖이다. **사용자 판단 대기 항목으로 NEXT_TODO에 올렸다.**
3. **CORE 피처는 어떤 경우에도 `suppress=True`로 올리지 않는다** — CORE 교체는 사용자
   승인 사안이다(CLAUDE.md §3). `opt_chain_pcr`·`vwap_position`은 마커만 붙였고, 노트
   첫 문장에 "이 항목을 '기각'으로 읽지 말 것"을 박았다.

**`suppress=True` 5건** — `basis_pt`·`basis_change_pt`(386차 표본확대 후 소멸) ·
`cancel_ratio`(계좌등급 제한, 구현불가 확정) · `cvd_direction`(2026-06-25 cvd_delta_norm
교체 완료, 395차가 "폐기 대상 잔존 항목"으로 판정) · `threshold_feasibility`(F4 부호 반전).

**`suppress=False` 9건**(마커만, 재고 유지) — magnitude 계열 3종(sign/ratio로 대체됐으나
표현형 다양성 사례라 추적 유지) · `opt_chain_pcr`·`vwap_position`(CORE) · exhaustion 4종
(`EXHAUSTION_RESTORE_MODE='shadow'`, live 전환 조건 ⓐⓑⓒ가 이미 사전등록돼 있다).

### 리포트 반영 3건

- `suppress` 이분 처리 — True는 상태를 덮고 재고에서 빼고, False는 마커만 덧붙이고 재고에
  남긴다. **보류·조건부채택을 재고에서 빼면 오히려 추적이 끊긴다.**
- §5에 `근거` 줄 + 동일 사유 그룹핑(exhaustion 4종이 같은 문단을 4번 반복하던 것 해소).
- **이름 불일치(orphan) 검출** — 레지스트리 키가 배포 피처·후보 명부·raw 어디에도 없으면
  경고한다. `suppress=True` 오타는 "기각했다고 믿었는데 후보로 계속 살아있는" 최악의
  실패라 조용히 넘기면 안 된다. 없는 이름을 주입해 경고가 뜨는 것까지 실측 확인했다.

### 정정 — 410차 기록의 부정확한 표현

410차 항목에 "**기각 확정**된 basis_pt"라고 썼는데, 원문 확인 결과 386차의 실제 표현은
**"승격 보류"**였다. 표준절차 Phase 6의 기각(반증) 요건은 충족하지만 그렇게 종결 선언된
적은 없다. 레지스트리에는 원문 그대로 옮기고 노트에 그 차이를 명시했다 — **결정을
기록하는 곳에서 결정을 만들어내면 안 된다.**

### 검증

py_compile py37_32·py310_64 rc=0. 필수필드 6종 누락 0건. 후보 재고 36 → **32건**
(suppress 5건 중 `cancel_ratio`는 애초에 미배선이라 재고 밖). 410차의 "레지스트리 미정의"
생성 경고 소멸 확인. orphan 검출 실측 확인.
...
후보 36건에 이미 판단이 끝난 `basis_pt`·`cvd_direction`이 섞여 있다 — **Phase B
(`FEATURE_SET_DECISIONS`) 미구현 탓**이며 리포트가 생성 경고로 그 사실을 명시한다.

> **[411차 정정]** 위 문장을 최초에 "**기각 확정**된 basis_pt·cvd_direction"이라고 썼는데
> 부정확하다. 411차에 원문을 확인한 결과 386차의 실제 표현은 **"승격 보류"**이고, 표준절차
> Phase 6의 "기각(반증)"으로 종결 선언된 적은 없다. 요건은 충족하지만 그렇게 부른 사람이
> 없었다 — 레지스트리에는 원문 그대로 "승격 보류"로 올렸다.
...
**관련**: 406차(R1·R4), 407차(경로 규약 — R3의 전제), 313차 원칙, 404차 후속11(레지스트리)

전부 계측·거버넌스 축이며 **판정식·합격선 무변경** — 33채널 verdict 무변경 diff 확인.

### [R2] `[4]`·`[21]`을 확정 결정 레지스트리에 "PC간 부호 역전 — 판정보류"로 등록

0802 대조에서 판정이 갈린 7건 중, 표본 임계선 문제(4건)도 PC 로컬 설정 차이(1건)도
아닌 **진짜 부호 역전** 2건이다. 313차 원칙상 두 PC가 반대 부호를 내면 어느 쪽도
단일 근거로 쓸 수 없다.

- `signal_decay` — saved가 MW0601 −2.0516pt(FAIL) vs MW0602 +12.9656pt(PASS).
  **양쪽 다 min_samples=10 충족**이라 표본 핑계가 불가한 가장 강한 사례.
- `direction_ev_watch` — LONG만 역전(+16,138 vs −52,786원). SHORT는 양쪽 양수로 일치.

등록 이유는 0801 결산 D5가 지적한 그 위험이다 — "FAIL이지만 일부러 적용하지 않기로
했다"가 기록되지 않으면 다음 세션이 재시도한다. 특히 `[4]`는 **MW0602 리포트만 보면
PASS**라서 "신호소멸청산은 잘 돌고 있으니 유지/확대"로 읽히는데 MW0601에서는 손해다.

부수적으로 `[4]`·`[21]` 요약행에 `_dm()` 마커 호출이 **빠져 있던 것**도 확인해 배선했다
(404차 후속11이 레지스트리를 만들 때 이 두 행을 빠뜨렸다 — 등록해도 안 보였을 것이다).

### [R3] A/B 채널 기준선(current.total_pt) PC 대조 섹션 — 표시 전용

`[3-B]`·`[23-B]`·`[25]`의 FAIL은 전부 `delta_vs_current`로 내리므로 **기준선 자체의
부호가 PC간 반대면 "현행 초과 대안" 목록이 구조적으로 갈린다.** 실측 −16.94 vs +6.03
(quantile_tp), −17.18 vs +3.64(tp1_geometry) — 그 결과 tp1_x2·q_x0.7·q_blend가 601에서만
등재됐다. 교차검토 §2-4는 이걸 tp1_x2 1건으로 봤지만 전수로는 A/B 채널 3개 전체의 문제다.

**407차가 리포트를 PC별 폴더 날짜본으로 옮긴 덕에** 상대 PC 파일을 읽을 수 있게 됐다 —
R3는 407차 없이는 구현 불가였다. `_peer_ab_baselines()`가 금요일점검/ 하위 PC 폴더를
훑어 최신 metrics의 `per_variant.current.total_pt`를 모으고, 부호가 갈린 채널에 ⚠를 단다.

실패해도 리포트를 막지 않는다(상대 파일 없음이 정상인 상황이 많다: 새 PC, pull 전,
단일 PC 운용). 상대 PC 날짜를 헤더에 병기해 **다른 주차 리포트를 같은 주로 착각하는 것**을 막는다.

### [R5] `[13]` 등급 × 계약수 교차 분해 — **원래 서술이 틀렸다**

R5는 "8.2배 차이는 3계약 이상 손실 1건 지배"라고 적었으나 실측하니 두 가지가 달랐다.

**(1) 계약수 집계 방식이 틀려 있었다.** `trades.quantity`는 청산 행별 계약수이지 포지션
크기가 아니다 — 부분청산은 여러 행에 나뉜다(실측 `2026-07-10 11:37:04`: TP1 부분청산 2 +
하드스톱 3 = 5계약). `max`로 세면 5계약이 3계약으로 잡힌다. **`sum`이 맞다.**
첫 임시 조회에서 `max`를 써 A등급 qty 분포를 잘못 읽었고, 구현 전에 잡았다.

**(2) "손실 1건 지배"가 아니라 계약수 구간 전체가 갈린다** (캠페인 구간, 포지션 단위):

    A: qty<=2  48건 +1,311,796원(평균 +27,329) / qty>=3 4건 -5,298,131원(4전 1승)
    C: qty<=2  11건   -352,072원(평균 -32,007) / qty>=3 3건   +603,890원(3전 2승)

→ **qty<=2로 한정하면 A(+27,329) > C(-32,007)로 등급 역전이 사라진다.**

함의가 R5의 예상보다 크다. `[13]`의 FAIL은 GradeEVGuard(등급 강등) 활성화를 유도하는데,
이 분해가 맞다면 **그것은 잘못된 처방이다** — A가 적자인 이유가 등급의 역예측성이 아니라
A등급이 큰 계약수를 더 많이 받았기 때문이라면, 등급을 낮춰도 계약수 배분이 그대로면
문제가 남는다. 0801 결정 "부결 — GradeEVGuard 활성화하지 않음"을 강하게 지지한다.

⚠ 그러나 qty>=3 표본이 A 4건 / C 3건으로 **극히 얇다**(313차: 확정 결론 금지). C의 qty>=3이
흑자인 것은 CLAUDE.md 전환기준 ⑧이 인용하는 §9-3 "3계약 이상 16전 1승"과 방향이 다른데,
그쪽은 **전체 기간**이고 이쪽은 **캠페인 구간**이라 모집단이 다르다 — 같은 것으로 읽지 말 것.
**판정식은 무변경**(A 평균 순EV < 0 → FAIL). 계약수 축 판정은 [28] sizing_inversion_watch가 맡는다.

### 검증

- py_compile py310_64(리포트) / py37_32(settings) rc=0, flake8 F821/F823/F811/E999 0건
- 리포트 재생성 rc=0, **33채널 verdict 완전 무변경** — §9-4 시계 리셋 없음
- `[4]`·`[21]` 요약행에 📌 마커 렌더링 확인
- R3 섹션: `[3-B]`·`[23-B]` ⚠부호 역전 표시, `[25]`는 MW0601 표본 0이라 `—`(역전 아님)로
  정확히 처리됨. MW0602 날짜(20260801) 헤더 병기 확인
- R5 표: A/C 두 행 렌더링 + "역전이 사라진다" 경고 분기 동작 확인

---

## 2026-08-02 (MW0601 408차 — 새 PC 폴더 자동생성 보강 + 주간 리포트 FIFO 4주 보관)

**관련**: 407차(산출물 PC 폴더 · 날짜본 이전)

### [보강] 폴더 생성은 이미 동작했다 — 진짜 빈틈은 **PC 이름**이었다

407차의 `os.makedirs(out_dir, exist_ok=True)`가 새 PC에서 이미 폴더를 만든다.
문제는 그 폴더의 **이름**이다 — `pc_id()`는 호스트명에서 `MW####`를 못 찾으면
호스트명 원문을, 그것도 실패하면 `"UNKNOWN"`을 돌려준다. 그대로 쓰면
`금요일점검/UNKNOWN/`이 **조용히** 생겨 다음 주간회의에서야 발견된다.
→ `_pc_dir_name()` 신설: 경로 불가 문자 치환 + `MW####`가 아니면 stderr 경고.

### [결정] 주간 리포트 FIFO 4주 보관

407차가 날짜본으로 바꿔 덮어쓰기 유실은 막았지만, 이번엔 **무한히 쌓인다**
(주 2파일 × PC × 커밋되는 위치). `VALIDATION_REPORT_KEEP_WEEKS = 4`.

**왜 4주인가**: 캠페인 판정 창이 28일(4주)라 그보다 오래된 리포트는 이번 주
판정과 겹치는 구간이 없다. 주차간 비교에도 4주면 충분하다.

**삭제 안전장치 — 설계 원칙**:
- 삭제 대상은 `validation_campaign_(report|metrics)_YYYYMMDD.(md|json)` **정확히 그
  형태만**이다. 접미사가 붙은 수동 스냅샷(`..._20260801_pre405.md`)과 주간회의
  검토보고 md는 건들지 않는다 — **사람이 이름을 붙여 남긴 것은 자동 삭제
  대상이 아니다**는 원칙.
- 날짜 단위로 묶어 md/json을 **함께** 지운다(한쪽만 남으면 대조가 깨진다).
- 쓰기가 끝난 뒤에만 돌아 중간에 죽어도 이번 주 파일은 이미 디스크에 있다.
- `--out-dir`는 **절대 정리하지 않는다** — 임의 폴더를 지우면 안 된다.
- 자기 PC 폴더만 정리한다(다른 PC 보관은 그쪽 EOD가 집행).
- 삭제 실패(잠김·권한)는 삼킨다 — 보관정리가 리포트 생성을 막으면 안 된다.
- `keep <= 0` 킬스위치, `--keep`/`--no-prune` 런타임 오버라이드.

### 검증

- 샌드박스 6주차 + 보호대상 4개 → keep=4: **자동생성 2주차만 삭제**,
  `_pre405`·검토보고·최종개선안 전부 생존, md/json 짝 유지, dry_run==실제
- `keep=0` 삭제 0개 / 4개 이하에서 재실행 삭제 0개(멱등)
- 호스트명 4종 시뮬레이션: `DeskTop-MW0601`→`MW0601`(무경고),
  `DESKTOP-ABC123`·`my pc:2`→`my_pc_2`·`''`→`UNKNOWN` 전부 경고 출력
- py_compile py310_64/py37_32 rc=0, flake8 F821/F823/F811/E999 0건
- 기본경로 end-to-end → "보관정리: 삭제 대상 없음(보관 4주)",
  `--out-dir` → "임의 폴더는 정리하지 않는다" — 둘 다 확인 후 일요일 산출물 제거

---

## 2026-08-02 (MW0601 407차 — 주간 리포트 산출물을 PC별 폴더 + 날짜본으로 이전)

**관련**: 0802 MW0601×MW0602 항목별 대조(수동 복사 의존), 405차 P0-5 `pc_id()`

### [결정] `data/` 고정파일명 → `docs/정기점검/금요일점검/<PC명>/*_YYYYMMDD.*`

두 사고가 각각 근거다 — **둘 다 실제로 발생했다.**

1. **다른 PC의 리포트를 볼 수 없었다.** `data/`는 gitignore 대상 PC 로컬 디렉터리라
   0802 31채널 대조에서 MW0602 리포트를 사용자가 수동 복사해줄 때까지 작업이 막혔다.
2. **지난주 리포트가 남지 않았다.** 고정 파일명이 매주 덮어써 2026-07-31분이 08-01
   재생성에 덮였고, 405차 세션 스크래치패드에 우연히 백업이 없었으면 영영 잃었다.

→ 커밋되는 위치 + 날짜본으로 둘 다 해소. PC 식별은 405차 P0-5의 `_pc_id()` 재사용
(호스트명에서 `MW####`). 폴더가 PC를 나타내므로 파일명에는 날짜만 넣는다.

### [예외 명시] "런타임 산출물 커밋 금지"의 의도적 예외다

`feedback_git_commit_scope`는 모델·DB·세션 등 PC별 런타임 산출물의 커밋을 금지한다.
주간 검증 리포트는 형식상 런타임 산출물이지만 **두 PC가 서로를 보는 유일한 통로**이므로
예외로 둔다. 다음 세션이 "이게 왜 커밋되지?" 하고 되돌리지 않도록 `CLAUDE.md`
"주간 산출물 위치 규약" 절에 근거와 함께 못박았다. 대상은 리포트 md/json 2종뿐이며
모델·DB·session_state 등 나머지 금지 원칙은 그대로다.

### 변경

| 파일 | 내용 |
|---|---|
| `scripts/generate_validation_campaign_report.py` | 출력 경로 이전 + `--out-dir` 오버라이드(재현·검증용). `BASE_DIR` import 추가 |
| `scripts/campaign_report_paths.py` | **신설** — 경로 규약 공용 헬퍼(`latest()`/`list_reports()`). 날짜는 파일명 8자리 우선, 없으면 mtime |
| `scripts/cmp_summary.py`·`cmp_metrics.py` | 하드코딩 경로 제거 → PC별 최신 날짜본 자동 짝짓기. `--pc1/--pc2/--date1/--date2` |
| `scripts/campaign_steps.py` | EOD 로그의 리포트 경로를 새 규약으로(옛 경로를 찍으면 파일을 찾다 헤맨다) |
| `CLAUDE.md` | "주간 산출물 위치 규약" 절 신설 — 커밋 예외 근거 포함 |
| `docs/정기점검/금요일점검/weekly_prompt.txt` | 검토보고 파일명·위치 규약, cmp 사용법, "only 줄 확인" 주의 |

기존 보존본도 새 규칙으로 개명했다(`..._MW0601_20260731_1547.md` →
`validation_campaign_report_20260731.md` 등). MW0602 수동 복사본은 생성시각
2026-08-01 18:30에 맞춰 `_20260801`을 붙였다.

### 검증

- py_compile py310_64 5파일 / py37_32 2파일(EOD 체인) rc=0, flake8 F821/F823/F811/E999 0건
- `--out-dir` 스모크 → 임시 폴더에 날짜본 2개 생성 확인
- 기본 경로 end-to-end 실행 → `docs/정기점검/금요일점검/MW0601/`에 생성 확인 후
  **일요일 산출물이라 제거**(금요일 산출물이 아니며 cmp 기본 짝짓기를 어긋나게 한다)
- cmp 2종 재실행 → 하드코딩 시절과 **동일 결과**(판정 일치 24/불일치 7, 부호 역전 33건,
  공통 수치키 579) + 선택된 파일명을 상단에 출력

**주의** — `--days` 기본 28일 창은 그대로다. 날짜본은 "생성일"이지 "분석 창"이 아니다.

---

## 2026-08-02 (MW0601 406차 — TP1 보호전환 모드 실거래 통일 + breakeven 재생감시 전환)

**상세**: `docs/정기점검/금요일점검/0802_TP1보호모드_통일_및_재생감시_구현계획.md`
**발단**: `docs/정기점검/금요일점검/0802_MW0601xMW0602_리포트_항목별대조.md` §2-B / R1·R4
**관련**: 405차 P0-5([23] PC 분리), 404차 후속3([24]/[25] 신설), 사전등록 원칙 §9

### [결정] 실거래는 `atr_profit`으로 통일, `breakeven`은 카운터팩추얼 재생으로만 감시

MW0601만 `data/session_state.json`의 `tp1_single_contract_mode`가 `breakeven`이었다
(코드 기본값·MW0602는 `atr_profit`). 근거:

- MW0602 23건 재생 실측에서 `breakeven`은 현행 대비 **건별 우세 0/23**(22/23 열위).
- `[25]`의 `variants`에 `breakeven`이 **이미 사전등록돼 있다** — 실거래를 통일해도
  재생 감시는 그대로 유지된다. 잃는 대조군이 없다.
- 두 PC의 라이브 모드 차이를 A/B로 쓰는 것은 **교란**이다 — 모델·거래모집단이
  다르고(진입 55 vs 60건), A/B 채널의 기준선 `current.total_pt`조차 부호가 반대다
  (−17.18 vs +3.64). 같은 거래 위 재생이 비교할 수 없이 강하다.
- MW0601 `[25]`가 n=0으로 죽어 있었다 — 통일하면 자동으로 살아난다.

`[25-B]` 신설은 하지 않았다. 통일만으로 목적이 달성된다.

### [발견] `[25]`의 ATR 역산은 두 경우에 깨진다 — 하나는 `atr_profit` 행에도 걸린다

`[25]`는 ATR을 `|stop_after - entry| / 0.25`로 **역산**했다.

1. **`breakeven` 행은 원리적으로 복구 불가** — offset=0이라 ATR=0이 돼 버려진다.
   MW0601 29건 중 25건이 여기 해당 — **`[25]` n=0의 진짜 원인**이며,
   리포트는 그걸 "표본 부족"으로 찍어 "곧 차겠지"로 오독되게 두고 있었다.
2. **나머지 4건의 offset은 ATR이 아니다** — `position_tracker.py:749-751`의
   `if mult*(prev_stop - protected_stop) > 0: protected_stop = prev_stop` 때문에
   기존 트레일링 스톱이 남은 것이다(0.8685/0.8579/1.1993/2.6164pt).
   이걸 `/0.25` 하면 ATR과 무관한 숫자가 **조용히** 나온다.
   **이 함정은 모드와 무관하므로 `atr_profit` 행에도 걸린다** — MW0602의 `[25]`도
   해당 행에서 오염된 ATR 위에 6개 변형을 쌓고 있을 수 있다. **몇 건인지는 MW0602 DB가
   없어 미확인** — MW0601에서 4/29(13.8%)였다는 사실만 안다.

→ `atr`·`protect_offset_pts`를 **영속화**해 역산 자체를 폐기했다. 둘 다
`arm_tp1_single_contract_with_mode` 호출부에 이미 있던 값이다 — **버리고 있었을 뿐**이다.
`protect_offset_pts ≠ |stop_after-entry|`가 곧 (2)의 식별자 역할을 겸한다. 소급 불가 — 기존 29행은 NULL.

### [수정 범위 축소] `[25]` 모드 필터 제거를 철회했다

처음엔 `protect_mode='atr_profit'` 필터를 없애 breakeven 행까지 판정 모집단에
넣으려 했으나, **그러면 판정식이 깨진다**. `current` 변형이 `ATR×0.25`로 고정 정의돼
있어 "실제 일어난 일"과 일치해야 `delta_vs_current`가 의미를 갖는데, breakeven 행에서는
실제 일어난 일이 offset=0이다. → 필터는 유지하고 **빠진 건수와 사유만 노출**하도록
범위를 좁혔다(`n_other_mode`/`n_backout_legacy`/`n_excluded_override`).

### [R1] `[24]`는 mode별 분리 집계로 전환 — 축은 PC가 아니라 **모드**다

`[24]`는 행마다 `mode`를 갖고 `by_mode` 분포까지 출력하면서도 평균·중앙값·상관은
두 모드를 섞어 낸다. 보호스톱을 거는 자리가 모드마다 달라 "반납"의 기준선 자체가
다른데도 그렇다 — 실측으로 중앙값 반납이 breakeven −0.1pt vs atr_profit +1.0pt로
**부호가 반대**다. 이 수치는 MW0602 검토보고 §1-4에서 "더 들고 있어라"의 근거 두 축 중
하나로 인용됐다 — 라벨 없이 두면 안 된다.

405차 P0-5가 `[23]`에 한 `pc` 분리와 **같은 원리, 축만 다르다**. 지금은 PC와 모드가
우연히 1:1이지만, 본 차수의 통일로 **08-03부터 MW0601 DB 안에서 실제로 섞이기 시작한다**.
DB 스키마 변경 불필요 — 집계 함수 1곳만 그룹핑했다. 최상위 키는 하위호환을 위해
풀링 값 그대로 두고 `by_mode_stats`에 병기했다. `[24]`는 verdict가 항상 OBSERVE라 판정 무영향.

### 검증

- py_compile py37_32(main·db_utils) / py310_64(스크립트 2종) rc=0
- flake8 F821/F823/F811/E999 신규 0건 (F811 2건은 HEAD에도 있는 기존 항목)
- 리포트 재생성 rc=0, **33채널 verdict 완전 무변경** — §9-4 검증 시계 리셋 없음
- INSERT 11-placeholder DB 사본 드라이런 성공, 오버라이드 판별식 동작 확인

**라이브 미검증** — `atr_profit` 보호전환·`atr` 컬럼 INSERT·오버라이드 행 포착은
2026-08-03(월)이 첫 실행이다. **MW0602에도 B·C를 배포해야** 그쪽 오염된 ATR 역산이 해소된다.

---

## 2026-08-01 (MW0601 405차 — 0801 최종개선안 P0·P1 구현 11건 + py310_64 리포트 크래시 블로커 수정)

**상세**: `docs/정기점검/금요일점검/0801_MW0601_최종개선안.md`
**관련**: MW0602 404차~후속11, MW0601 0731 점검리포트 §9(2차 재검증), 교차검토 §2·§6·§7,
313차·372차 원칙, 317차 Phase 1(py310_64 corrcoef 크래시).

### [사건] 404차 후속3의 [24] 채널이 MW0601 EOD 리포트를 통째로 크래시시키고 있었다

구현 착수 직후 baseline 리포트를 뽑으려다 발견했다. `generate_validation_campaign_report.py`가
py310_64에서 **네이티브 크래시**(exit 0xc06d007f DELAYLOAD_FAILURE)로 즉사한다 —
traceback 없이 프로세스가 죽어 try/except로도 못 잡는다. faulthandler로 지점을 특정:

```
File numpy/lib/function_base.py line 2747 in cov
File numpy/lib/function_base.py line 2889 in corrcoef
File scripts/generate_validation_campaign_report.py line 1656 in eval_tp1_protect_giveback_watch
```

**이미 알려진 제약이었다.** `scripts/hurst_nmin_search.py`·`hurst_oos_validation.py`·
`hurst_stability_check.py` 3종이 docstring에 *"py37_32에서 실행 — py310_64는 np.corrcoef
크래시, 317차 Phase 1에서 확인"* 을 달아두고 있다. 404차 후속3(MW0602)이 [24] 채널에
`np.corrcoef`를 넣으면서 그 제약이 **EOD 체인으로 유입**됐고, MW0602에서는 재현되지 않아
발견될 수 없었다 — 전형적인 PC 환경차다.

실측 경계: `np.dot`은 정상(5.0), `np.cov`가 크래시. py37_32는 `np.corrcoef` 정상(1.21.6).
**수정**: `_pearson_r()` 신설 — 합/평균만 쓰는 정의식. py37_32 `np.corrcoef`와 값 일치
확인(0.9933992677987828). 이 스크립트에서 `np.corrcoef`/`np.cov` 사용 금지를 docstring에 명시.

> **파급**: 이 크래시가 없었다면 08-03 EOD에서 MW0601의 캠페인 리포트 생성이 실패했을 것이다
> (07-31 EOD는 [24] 배포 전이라 rc=0이었다). 리포트 미생성은 조용히 지나가기 쉬운 실패다.

### [구현] P0 7건 — 전부 판정식 무변경

| # | 항목 | 파일 |
|---|---|---|
| P0-0 | `np.corrcoef` → `_pearson_r` (위 사건) | `generate_validation_campaign_report.py` |
| P0-1 | `CLAUDE.md` 실전전환 ⑧ 재작성 + 목표자본 1억→5천만 정정 | `CLAUDE.md`, `settings.py` |
| P0-2 | D1 방향 (b) 확정 — `sizing_prescription_axis` 레지스트리 | `settings.py` |
| P0-3 | pt 채널 `[1계약·TP1상한·미실현 시뮬]` 배지 + qty 분포 부기 | 리포트 |
| P0-4 | [13] `grade_ev_inversion` 포지션 단위 병합 | 리포트 |
| P0-5 | [23] `guard_shadow` PC 식별자 분리 (`pc_id()`) | `db_utils.py`, 리포트 |
| P0-6 | `joint_gate_shadow` 기록조건 점검 → **결함 없음, 코드 무변경** | — |

**P0-1 근거**: 후속11이 `CLAUDE.md`를 열어 ⑤(CB②)를 고치면서 ⑧은 그대로 뒀다. ⑧은
(a) 목표자본을 "1억원"으로 적고 있었고(실제 5천만원, 339차 후속 `66f696f`),
(b) *"False로 전환"* 을 단독 지시로 남겨 §9-3에 따르면 `MAX_CONTRACTS=10` 아래
3~10계약 체제 복원을 의미했다. "실전 자본 재설정 **+ MAX_CONTRACTS 동반 재검토**"로 재작성.

**P0-2 근거**: MW0602 D1이 사이징 완화 처방 91% no-op을 확인하고 (a) 기본 계약수 상향 /
(b) 처방 축 전환 중 선택을 요청했다. (a)는 MW0601 §9-3으로 반증 — 계약수별 실현손익이
완전 단조이고(1계약 +1,682만 / 3계약↑ 16전 1승 -2,642만, 이항검정 p=5.07e-07),
캠페인 구간만 포지션 단위로 다시 세도 유지된다(qty=1 +204만 / qty 8~10 -1,355만).

**P0-4 실측 왜곡**: 73 청산이벤트 = **66 포지션**. C등급 승률 47.1%→**42.9%**,
표본 A 56→52 / C 17→14. verdict는 INSUFFICIENT 유지(뒤집히지 않음).
[21]은 이미 병합하며 docstring에 "402차가 실제로 이 함정에 걸렸다"까지 적어뒀는데
자기가 "동일 구조의 미러"라 부르는 [13]에는 적용이 누락돼 있었다.

**P0-5 근거**: `missed_upgrade_rate`가 MW0601 **0%**(6개 호라이즌 전부 공정비교로도 HOLD,
하락폭 0.11~0.23 vs 허용 0.025~0.05) vs MW0602 **50%**로 정반대다. [23-B] tp1_x2
(+32.78 vs -19.04)에 이은 **두 번째 PC간 부호 역전**. 합산하면 상쇄돼 무의미해진다.
`pc` 컬럼 + `pc_id()`(호스트명 `DeskTop-MW0601`에서 정규식 추출) 신설,
verdict는 이 PC 행만으로 계산(pc=NULL은 405차 이전 기록이므로 이 PC로 간주).

### [기각] P0-6 — `joint_gate_shadow` 기록 결함 의심은 실측으로 해소

교차검토 §7-2에서 *"`tox_size=0.5`가 분기 모집단의 10.1%인데 섀도 116건 전수가 0.7 —
402차 후속3과 같은 계열의 기록조건 결함일 수 있다"* 고 의심했다. **그 의심은 틀렸다.**

1. `main.py:8038`의 INSERT는 `if _joint_blocked:` 블록 안에 있어 **차단 시 100% 기록**된다.
2. 체크리스트를 통과해 실제 이 경로에 도달한 모집단(`entry_final_ok=1` & 양쪽 reduce)은
   **n=200 전부 `tox_size=0.7`** 이다 — 0.5는 기록 누락이 아니라 **실제 미발생**이다.
   (근거로 썼던 2,930건은 등급 X·체크리스트 탈락까지 포함한 전체 모집단이었다.)
3. `entry_block_reason` 159건 vs 섀도 116건의 43건 갭은 **전부 2026-07-14** — 327차가
   섀도를 배선하기 전날이다. 07-15 이후 13거래일은 **100% 일치**(5/5, 1/1, 11/11 …).

→ **[7] JointGate PASS 판정은 온전하다.** 코드 변경 없음. 의심과 기각을 함께 기록으로 남긴다(D5 취지).

### [구현] P1 4건 — 전부 섀도/사전등록, 라이브 무변경

**P1-1 EOD 공정 홀드아웃 섀도** (`batch_retrainer._measure_fair_holdout`)
`_measure_incumbent_acc`는 docstring이 스스로 *"완전히 공정한 비교는 아니다 … 진짜 공정한
비교는 두 모델 모두 학습에 쓰지 않은 최근 홀드아웃이 필요하며 그건 별건으로 남긴다"* 고
적어뒀고, 404차는 그 값을 DB에 영속화했을 뿐이다. 결과가 **12거래일 연속 교체 0건**이다.
→ 최신 1,850봉(≈5거래일)을 학습에서 완전히 뺀 도전자를 따로 학습해 둘 다 그 구간으로 채점.
`[GuardFair]` 로그 + `guard_shadow_log` 4컬럼(`fair_new/fair_old/fair_hold_bars/fair_note`).
**판정은 acc.txt 그대로.** 3거래일 관측 후 주간회의 수동 결정(§9).
⚠ 여전히 완전 공정은 아니다 — 현행 pkl은 intraday가 덮어써(스모크 실측 mtime **07-31 14:05**)
홀드아웃을 이미 학습했을 수 있다. 측정 격차는 **하한**이며 pkl mtime을 로그에 병기한다.
비용: 호라이즌당 fit 1회 추가(07-31 실측 1.3~7.1초) → EOD +25초 안팎.

**P1-2 `sizing_inversion_watch` [28] 신설** — 등록 채널 30여 개 중 최상위 이상점을 보는
채널이 하나도 없었다. qty 1/2/3+ 구간별 포지션 단위 실현 순EV. `min_samples_per_bucket=20`,
`min_days=5`, `streak=3`. **MAX_CONTRACTS 인하의 정규 경로** — 자동 조치 없음.
첫 실행 **INSUFFICIENT (qty1=54, qty3+=0)** — 캠페인 구간에 3계약 이상이 아예 없다.
이것 자체가 목표자본 5천만원의 "우발적 봉합"이 작동 중이라는 증거다.

**P1-3 `mean_revert_size_watch` [29] 신설** — 313차 z검정 5개 중 유일한 Bonferroni 생존축.
**"진입 차단"이 아니라 사이즈/등급 축 가설**로 등록(317차 FalseBlock 교훈).
첫 실행 **PASS** (MR 47.1% n=34 vs neutral+trend 52.4% n=21, 격차 5.3%p < 15%).
§6-3의 45.7% vs 69.1%보다 훨씬 약하다 — 그쪽은 207 이벤트 비병합 전체 이력이고
이쪽은 캠페인 구간 병합 포지션이다. **지금 조치하지 않기로 한 판단이 옳았음을 지지한다.**

**P1-4 `tp1_geometry_shadow` OOS 부분집계** — `oos_start="2026-07-31"` 사전등록.
누적 표는 가설을 세운 그 기간(403차 P1-6, 55건/11일)의 in-sample이다. 이후 신규분만
따로 세어 병기하되 **판정 미반영**. `rows_ts` 평행 리스트로 기존 튜플 구조 무변경(회귀 위험 0).
첫 실행: 07-31 진입 0건이라 "OOS 표본 없음"이 정상 출력.

### [검증]

- `py_compile` py310_64·py37_32 양쪽 5파일 rc=0 / `flake8 --select=F821,F823,F811,E999` 0건
- 리포트 재생성 rc=0 — **기존 31개 채널 verdict 완전 무변경**(§9-4 시계 리셋 없음).
  신설은 [28] INSUFFICIENT / [29] PASS 2건뿐.
- 홀드아웃 스모크 10항목 ALL PASS — 6개 호라이즌 실제 pkl 로드·채점, 표본부족 스킵,
  피처셋 불일치 시 도전자만 산출, 킬스위치, `save_guard_shadow` 시그니처 정합, `pc_id()=MW0601`.

**라이브 미검증** — `[GuardFair]`·`pc` 컬럼·[28]/[29] 누적은 **08-03 EOD가 첫 실행**이다.

---

## 2026-08-01 (MW0602 404차 후속11 — W4 결산 확정 결정 + 캠페인 상시 전환)

사용자 승인으로 0801 주간회의 검토보고(`docs/정기점검/금요일점검/0801_주간회의_검토보고_MW0602.md`)
의 결정이 확정됐다. 라이브 로직 무변경 — 설정 상수 1개(`mode`), 결정 레지스트리 신설,
리포트 렌더링, 문서 반영이 전부다.
...
| 안건 | 확정 |
|---|---|
| [6] Hurst 완화 | **추가 조치 없음** — 권고는 2026-07-15(333차 후속)에 이미 이행. 그 자리를 대신한 오늘의 실행 항목이 [3-A]·[3-B] 구현·연장(404차 후속10) |
| [13] GradeEVGuard | **부결** — 활성화하지 않음 |
| [2] Meta-Gate | **탈락 처리** — 승격 포기, 감사 §5 2순위 공식 삭제 |
| 나머지 FAIL 3건 | **"FAIL이지만 미적용"** 기록 — [23-B]·[25]·[3-B] |
| CB② 복원 | **4주 연기 → 재검토 2026-08-29** |
| 캠페인 | **상시(standing) 전환** |

나머지(D3·D6·D7·D9~D12·D14)는 권고안으로 캠페인에 편입하고 운영은 상시 유지.

### [설계] 캠페인 상시 전환 — 합격선은 건드리지 않는다

`VALIDATION_CAMPAIGN["mode"] = "standing"` (+ `mode_changed_on`).

- 왜: §3-1(TB)은 표본 미달로 4주 안에 시험조차 못 했고(3m OOS 584/800),
  `min_samples=20` 채널 다수가 실측 도달속도 9~16주 스케일이다([8] 4건/4주 → 약 16주).
  §8이 상정한 "W4에 활성화/폐기 확정"이라는 전제가 표본 현실과 맞지 않았다.
- 바뀌는 것: 종료 기한 없음. 채널별 `min_samples` 도달 시점에 개별 판정([1]이 384차부터
  이미 그 방식).
- **바뀌지 않는 것: 모든 합격선·판정문·`start_date` 무변경.** 상시 전환은 거버넌스
  결정이지 판정 기준 변경이 아니므로 §9-4의 시계 리셋 대상이 아니다. 이 구분을
  흐리면 "운영 편의로 기준을 바꿨다"가 되므로 명시적으로 못박는다.

### [설계] 확정 결정 레지스트리 — 판정과 결정의 분리

`VALIDATION_CAMPAIGN_DECISIONS` 신설(채널키 → date/decision/note 6건). 리포트가 요약표
📌 인라인 마커 + 하단 전용 섹션으로 렌더링한다(`_dm()`).

**왜 개별 수정이 아니라 레지스트리인가** — 이 문제는 양방향으로 다 터졌다:

1. **이미 이행된 권고를 신규 안건으로 오인**: [6]은 2026-07-15에 사이징 ×0.5가 배포돼
   라이브 중(`HURST_SOFT_BLOCK_ENABLED=True`, main.py:7298)인데 리포트가 조치 여부를
   모른 채 3주째 "완화 권고"를 찍었고, **0801 결산 초안이 그것을 승인 1순위 안건으로
   잘못 올렸다**(코드 확인 후 철회). 사람이 매번 코드를 열어 대조해야만 알 수 있었다.
2. **미적용 결정이 어디에도 안 남음**: FAIL은 매주 재계산되므로 "보고 나서 일부러
   적용하지 않기로 했다"는 사실이 없으면 다음 세션이 같은 FAIL을 보고 재시도한다.

판정 로직에는 일절 관여하지 않는다 — verdict는 사전등록 기준으로만 계산되고 레지스트리는
"그래서 사람이 무엇을 결정했나"만 덧붙인다.

### [결정 근거 요약] 왜 그렇게 결정했나

- **[13] 부결**: A등급 누적 −488,009원에서 [15] 급행 풀스톱 11건 −1,060,617원을 빼면
  나머지 42건은 +572,608원(평균 +13,633원). 부분집합 관계는 [15] C등급 1건 −210,640원이
  [13] C등급 최대손실과 정확히 일치하는 것으로 확인. 등급 문제가 아니라 **급행 풀스톱이라는
  별개 실패 모드**이며 A등급 전체 강등은 대상 오조준(잘려나갈 42건이 수익 구간).
  재론 조건: 급행 풀스톱을 빼고도 A등급 평균 순EV가 음수일 때.
- **[2] 탈락**: 분리도 −0.0809(필요 +0.1457)로 **부호가 반대**, 분위당 n=402라 표본 핑계
  불가. [17] 슬리피지 실측(0.0379pt vs 가정 0.02pt)을 반영하면 더 나빠지므로 뒤집힐 여지 없음.
- **[23-B] 미적용**: MW0601 +32.78 vs MW0602 −19.04로 **PC간 부호 역전**(n≈60·12일 한계).
- **[25] 미적용**: 최대기여 1건 제거 시 부호 역전(atr_lock_0.75 +0.92→−1.33), 건별우세 8/23.
- **[3-B] 미적용**: drop-max는 견디나(+9.36→+7.76) 변형 간 비단조. MW0601 교차확인 선행.
- **CB② 연기**: 감사 §7의 트리거가 "캠페인 종료 시"였는데 상시 전환으로 그 시점이
  사라졌다. 게다가 복원은 당일 정지를 늘려 진입 표본을 직접 줄이므로, min_samples 미달
  채널이 다수인 지금은 검증 속도를 더 늦춘다. **무기한 유예 아님** — Phase 5 조건 ②·⑤가
  걸려 있어 2026-08-29에 반드시 재론.

### [구현]

- `config/settings.py` — `VALIDATION_CAMPAIGN["mode"/"mode_changed_on"]`,
  `VALIDATION_CAMPAIGN_DECISIONS` 신설, `GRADE_EV_GUARD_ENABLED`·`CB_CONSEC_STOP_LIMIT`
  결정 주석.
- `scripts/generate_validation_campaign_report.py` — `_dm()` 마커 + 헤더 상시모드 표기 +
  "확정 결정 레지스트리" 섹션. FAIL 6채널 전부에 마커 부착 확인.
- `CLAUDE.md` — "검증 캠페인 운영 모드 — 상시" 섹션 신설, 절대원칙 §2 CB② 연기,
  실전 전환 기준 ⑤ 주석. **멀티PC 컨벤션상 git 공유 문서에 적어야 MW0601 세션도 따른다**
  (402차가 이 원칙을 어겨 같은 날 다른 세션이 몰랐던 사례).

---

## 2026-08-01 (MW0602 404차 후속10 — [3-A]·[3-B] 분위회귀 채널 확장)

0801 캠페인 W4 결산 중 사용자 제안. **전부 오프라인** — 라이브 무변경.

### [배경] §5 활성화 순서에서 유일하게 남은 승격 후보가 미검증이었다

감사 §5 승격 순서 4개 중 3개가 탈락/미시험이다 — 1순위 지정가(상태 확인 필요),
2순위 Meta-gate([2] 분리도 −0.0809로 탈락), 4순위 TB([1] 3m 584/800 표본 미달).
남은 3순위 "분위회귀 TP 보조"가 유일한 후보인데, **정작 그 승격 형태(q90 기반 TP
거리 조정)가 한 번도 시뮬레이션된 적이 없었다.** [3] PASS는 캘리브레이션(커버리지
0.8101 / 상관 0.3274)이지 "TP에 써서 돈이 되는가"가 아니다.

### [판단] 캠페인 종결을 기다릴 이유가 없다 — §9 저촉 없음

당초 "캠페인 종결(D13) 이후 착수"로 잡았으나 사용자가 *"기존 캠페인 결과에 영향을
주는 로직이 아니므로 기존 채널에 반영해 연장하라"* 고 지적했고, 확인 결과 옳았다:

- 라이브 신호 경로 무변경(오프라인 재생 전용) → §9-1 해당 없음.
- [3] 본채널 합격선(coverage/unc_corr) 무변경 → §9-4 해당 없음.
- 동일 패턴 전례 3건이 이미 캠페인 중 신설됐고 시계 리셋이 없었다 —
  [23-B](403차 P1-6), [25](404차 후속3), [26]/[27](404차 후속5).

### [정정] "진입 시점 ATR 저장이 선행조건"은 틀렸다

세션 중 제시했던 선행조건인데 오판이었다. `tp1_geometry_shadow.py:_load_atr_map()`이
이미 `ensemble_decisions.features` JSON에서 ATR을 뽑아 쓴다([23-B]가 "ATR 결측 0건"인
이유). 신규 DB 컬럼도 라이브 계측도 불필요.

### [정정] KPI ①은 "사후 추가"가 아니라 "사전등록됐는데 미렌더링"이었다

감사 §3-3은 KPI를 **3종**으로 정의했으나(① q50 방향 적중률 ② 폭-실현 상관
③ 커버리지) 합격선이 ②③만 채택해 ①이 한 번도 표시되지 않았다. 표시 복원은
§9-4가 금지하는 판정기준 사후 변경이 아니다.

### [설계] q90이 아니라 "방향 정합 분위"를 쓴다

`quantile_q90_pt`는 가격변동의 상위 90분위다. LONG의 이익 방향은 상방이라 q90이
맞지만 **SHORT는 |q10|을 써야 한다** — SHORT에 q90을 쓰면 손실 방향 꼬리를 TP로
삼는 꼴이다. 감사 §3-3 "q90 기반"의 취지는 '이익 방향 꼬리까지의 거리'이므로
방향별로 favorable quantile을 고른다.

### [설계] 스톱을 전 변형 고정해 [23-B]의 교란을 제거했다

[23-B] `sym_1.0_1.0`은 TP1 확대와 스톱 축소를 동시에 바꿔 기여 분해가 불가능했다
(stop_1.0 단독 +0.53 vs sym +1.38). 이 채널은 TP1 축만 바꾼다.
추가로 **paired 보장** — 한 변형이라도 시뮬 불가면 그 진입을 전 변형에서 함께
제외한다([23-B]는 변형별 개별 skip이라 이론상 unpaired가 될 수 있었다).

### [결과] [3-A] q50에는 방향 정보가 없음이 확정됐다

n=3,551 / 적중률 **51.0%** / |q50| 상위 30% 적중률 **50.5%** / q50 양수 비율 47.9%.
크기에도 정보가 없다(상위 30%가 전체와 동일). → **q50을 방향·사이징에 쓰면 안 된다.**
[2] Meta-Gate가 확신도-성과 역상관으로 탈락한 것과 같은 실패의 예방선이다.

### [결과] [3-B] FAIL — q_raw가 현행 초과, 그리고 drop-max를 견딘다

진입 후보 60건 → 시뮬 36건(분위결측 24건은 2026-07-20 이전 미적재) / 9거래일.

| 변형 | TP1 | STOP | 누적pt | 현행대비 | drop-max | 건별우세 |
|---|---|---|---|---|---|---|
| current | 28 | 8 | +6.03 | +0.00 | — | 0/36 |
| q_raw | 26 | 10 | +15.39 | **+9.36** | **+7.76** | **25/36** |
| q_x0.7 | 26 | 10 | −2.10 | −8.13 | −9.07 | 15/36 |
| q_blend | 26 | 10 | +5.68 | −0.35 | −1.15 | 25/36 |

**[25]와 결정적으로 다르다** — [25]는 최대기여 1건 제거 시 부호가 역전됐으나
q_raw는 +7.76로 살아남고 건별 우세도 25/36(69%)다.

**그러나 즉시 적용은 금지한다.** 결과가 단조롭지 않다 — q_x0.7이 현행보다도
나쁜데 q_raw는 크게 낫다. 실측상 방향정합 분위는 35/36건에서 현행 TP1보다 넓고
(중앙값 2.23 vs 1.40pt), current→q_x0.7 구간에서 2건이 TP1→STOP으로 뒤집히며
손실이 발생한 뒤 q_raw 구간에서 TP 폭 확대분이 이를 뒤집는 구조다. 즉 **폭 자체보다
"몇 건이 뒤집히나"가 결과를 지배**하며, n=36에서 그 2건은 얇다.
[23-B]가 같은 코드·같은 기간에 PC간 부호 역전(tp1_x2 +32.78 vs −19.04)을 낸 전례가
있으므로 **MW0601 교차확인 전에는 판단 보류**(313차).

### [구현]

- `config/settings.py` — `VALIDATION_CAMPAIGN["quantile_tp_shadow"]` 사전등록
  (관측 전 고정: min_samples=20 / min_days=3 / variants 4종 / 판정문 + 필독 3종).
- `scripts/quantile_tp_shadow.py` 신설 — [23-B]·[25]와 동일 `compute()`/`summarize()`
  규약. `summarize()`가 `delta_drop_max`·`sign_flips_on_drop`을 자동 산출한다
  (372차 이상치 분해의 자동화 — 종전에는 세션마다 수작업이었다).
- `scripts/generate_validation_campaign_report.py` —
  `_quantile_direction_stats()` 신설, `eval_quantile_channel()`이 `out["dir"]`로 병기
  (**verdict 계산에는 미반영**), `eval_offline_geometry_channels()`에 3번째 채널 편입,
  요약표 `[3-A]`·`[3-B]` 행 + 상세 섹션.

### [주의] 후임 세션이 오독하지 말 것

- **[3-B]의 FAIL은 [3]의 PASS와 무관하다.** [3] 합격선은 커버리지·상관 두 개 그대로다.
- [12](TP1 **이후** 트레일 폭, 기각) · [25](TP1 **시점** 보호 offset, 현행 유지) ·
  [3-B](TP1 **거리를 무엇으로 정하나**) 는 서로 다른 질문이다.
- [23-B]가 기각한 `tp1_x2`는 "일괄 2배"다. q_raw는 신호별 가변이며 중앙값 기준
  현행의 약 1.6배 — 같은 방향이지만 같은 처방이 아니다.

---

## 2026-08-01 (MW0602 404차 후속9 — P1-D: JointGateBlock 섀도에 MFE/MAE 병기)

0731 정기점검 §4-B 순위 3. 전부 오프라인(리포트 스크립트 + DB 컬럼) — 라이브 무변경.

### [설계 정정] 리포트의 "섀도 1곳"은 불가능하다 — resolve 시점 계측이어야 한다

리포트는 비용을 "섀도 1곳"으로 적었지만, 30분 MFE/MAE는 **본질적으로 사후 값**이라
진입 시점(`main.py`의 INSERT)에 기록할 수 없다. 캠페인 리포트의 **resolve 루프**에서
채워야 한다 — 그 루프는 이미 분봉을 걷고 있어 계산은 사실상 공짜다.

또 resolve 루프는 스톱/TP에 닿으면 `break`로 조기 종료하므로 **창 전체를 도는 별도
walk**가 필요하다(`_mfe_mae()`). 청산 시점과 무관하게 "얼마나 갈 수 있었나"를 물어야
하기 때문이다.

### [구현] 소급 백필이 핵심 — 표본 대부분이 이미 확정된 행이다

- `joint_gate_shadow`에 `mfe_30m`/`mae_30m` 컬럼(`utils/db_utils.py` 스키마 +
  `_ensure_shadow_mfe_columns()` 멱등 ALTER). **스키마만 고치면 기존 DB는 안 바뀐다** —
  `CREATE TABLE IF NOT EXISTS`는 이미 있는 테이블을 갱신하지 않고, 두 PC가 각자 로컬
  DB를 갖고 있어서 ALTER가 필수다.
- `_backfill_shadow_mfe()` — resolve 루프는 `resolved=0`만 처리하므로 컬럼 신설 이전에
  확정된 행은 영원히 빈다. 캠페인 78건 **전부**가 그쪽이라 소급이 필수. 멱등.
- 거래불능 분봉은 `exclude_untradeable=True` 맵으로 이미 제외(후속5) — 체결 불가한 분의
  고저로 "갈 수 있었다"를 주장하면 안 된다.

### [교차검증] §2-E 표를 독립 계산으로 재현했다

0731 리포트 §2-E가 손으로 뽑은 값과 이 구현의 출력이 정확히 일치한다:

```
07-31 13:16 LONG   MFE +17.72 / MAE −0.12 / TP1 기준 hyp +1.01
```

### [반전] MFE만 보면 정반대로 읽힌다 — MAE가 더 크다

n=78 집계 결과가 §2-E의 서사를 **뒤집는 방향**이다:

| 지표 | 값 |
|---|---|
| MFE 평균 | 6.63 pt (중앙값 5.35) |
| **MAE 평균** | **8.96 pt** |
| MFE>MAE | 33건 |
| **MAE≥MFE** | **45건** |
| ΣMFE / ΣMAE | 517.16 / **698.50** pt |

차단된 신호들은 **유리하게 간 폭보다 불리하게 간 폭이 더 컸다.** §2-E가 지목한
"추세일 차단 비용 과소평가"는 개별 추세 건(13:16 등)에서는 사실이지만, **표본 전체로는
반대 방향 증거가 더 크다** — 차단이 옳았던 쪽이다.

MFE만 병기하고 끝냈으면 "게이트가 비싸다"는 잘못된 인상을 줬을 것이다. 리포트 출력에
MFE·MAE를 나란히 놓고 경고를 달았다.

### [한계 명시] MFE/MAE는 순서를 모른다

`07-27 09:49 SHORT`는 MFE +23.96인데 cf=STOP(−5.7)이다. 스톱이 **먼저** 맞은 뒤에
유리하게 갔다는 뜻일 수 있다. MFE는 "이만큼 벌 수 있었다"의 **상한**이지 실현 가능한
값이 아니다. 이 경고를 리포트에 박아 뒀다.

### [주의] 포착률 Σhyp/ΣMFE 는 음수다 — 비율로 읽지 말 것

−9.4%. Σhyp가 음수(−48.42)라 그렇다. "TP1 가정의 순손익이 마이너스"라는 뜻이지
"MFE의 −9.4%를 포착"이 아니다. 리포트에 그 주석을 병기했다.

### [범위] joint_gate_shadow 한정

`hurst_gate_shadow` 등 같은 패턴의 다른 섀도도 동일하게 확장 가능하도록 헬퍼를
테이블명 인자로 만들었으나, 이번엔 §2-E가 지목한 [7] 채널만 적용했다.

---

## 2026-08-01 (MW0602 404차 후속8 — 가격제한 3단계 규정 반영: 스냅샷 캐시 → 기준가 유도)

근거 문서 편입: `docs/CyBos ref/코스피200_미니선물_가격제한_규정.md`(KRX 공시·증권사
거래설명서 2025.11~2026.5 기준).

### [확정] 후속7이 "미확인"으로 남긴 비대칭은 규정 그대로였다

> §2-① "확대는 **한 방향으로만** 적용됩니다. 상승 쪽 제한폭이 열려도 하락 쪽은 기존
> 단계 유지."

2026-08-01 실측의 상한 +20% / 하한 −8% 비대칭이 정확히 이것이다. 추론이 맞았고,
`is_at_daily_limit()`의 초과 방어는 이제 "필수 보정"이 아니라 **다중 방어**로 남는다.

### [발견·수정] 스냅샷 1회 캐시는 상한가 날에 조용히 무력화된다

규정 §1·§2: 가격제한폭은 고정이 아니라 **장중에 ±8%→±15%→±20%로 확대**된다
(상·하한가 도달 후 5분 경과 시, 또는 주식시장 CB 재개 시). 그런데 이 시스템은
FutureMst 스냅샷을 `_prime_from_snapshot()`에서 **기동 시 1회**만 읽는다. 기동 시각
(08:45~08:55)은 항상 1단계이므로:

- 장중 확대되면 캐시(±8%)가 낡는다 → 실제 상한(+20%)을 모른다
- 초과 방어가 작동해 오차단은 없지만, **정작 상한가 날에 기능이 무력화**된다

COM 재조회는 `BlockRequest`가 최대 30초 파이프라인을 막아 매분 돌릴 수 없다(후속7에서
연속 호출 타임아웃 실측). 대신 **기준가격에서 산술 유도**로 해결했다 — 기준가격은
하루 동안 고정이고 단계 비율은 규정 상수라 **추가 COM 조회가 0**이다.

- `market_state.PRICE_LIMIT_STAGES = (0.08, 0.15, 0.20)`
- `market_state.effective_price_limits(base, price, session_high, session_low)`
- `CYBOS_FUTUREMST_BASE_PRICE_IDX = 13` (6종목 교차검증에서 [13]×1.20 = [17] 일치)
- `daily_limits`가 `base`를 함께 반환, `is_at_daily_limit()`이 **base를 우선**

### [함정] 단계가 역행해 과잉 차단이 났다 — 래칫 필요

첫 구현은 **현재가**로 단계를 재계산했다. 07-31 분봉 재생 결과:

```
09:00 1단계 → 09:05 2단계 → 09:11 3단계 → 09:17 3→2 역행(!) → 09:18 다시 3
LONG 차단 38/353 분봉 (10.8%)
```

한 번 확대된 제한폭은 **당일 중 축소되지 않는데**, 가격이 되돌리자 단계가 내려가
상방 여지가 충분한 구간(실제 상한 1036.30)을 993.12로 잘못 막았다.

→ **세션 고가/저가로 단계를 판정**하도록 교체했다. 확대는 "제한선 **도달**"로 발동하므로
세션 고가가 그 도달 이력을 보존한다 — 별도 상태 없이 래칫이 성립한다. 하한은 세션
저가로 독립 판정(방향별 확대 규정과 일치).

### [실측] 07-31 재생 — 오차단 0

```
단계 전이: 09:00 1단계 → 09:05 2단계 → 09:11 3단계 (역행 없음, 단조)
LONG 차단 37/353 분봉 — 종가 1036.28(3단계 상한) 33건 + 932.66(1단계 상한) 4건
                        ※ 전부 상한선 1틱 이내 = 진짜 상한 접촉, 오차단 0
정상일 07-30(-2.54%) 차단 0/370
```

이전 방식(캐시된 [17])보다 **엄격히 낫다**: 기동 캐시는 932.67이라 초과 방어가 발동해
1036.28 구간을 통째로 놓쳤을 것이고, 09:00~09:04 1단계 고착도 못 잡았다.

### [등록] 규정에서 확인한, 아직 모델링하지 않은 제약

- **실시간 가격제한(직전 약정가격 ±α)** — 착오주문 방지용으로 1단계 범위 안에서만
  작동하고 2단계 확대 시 해제된다(§3). α는 상품별 시행세칙 값이라 미확보. 우리 게이트는
  일일 제한폭만 본다.
- **야간거래는 1단계(±8%)만, 확대 없음**(§4). 이 시스템은 오버나이트 금지(절대원칙 §1)라
  해당 없으나, 기동 스냅샷을 야간 시간대에 읽으면 값이 다를 수 있다.
- **주식시장 CB 발동 시 선물도 중단**(§5). 그 구간은 거래불능 감지기(후속5)가 사후로
  잡는다.

---

## 2026-08-01 (MW0602 404차 후속7 — 가격제한선 인덱스 실측 확정: [17]/[18])

### [확정] `Dscbo1.FutureMst` 상한가=[17] / 하한가=[18]

크레온 로그인 후 프로브 실행. **6개 종목 교차검증**에서 예외 없이:

```
[17] = 기준가[13] × 1.20   (전부 정확히 +20.00%)  → 상한가
[18] = 기준가[13] × 0.92   (전부 정확히  -8.00%)  → 하한가
```

**결정적 확증**: 2026-07-31은 전 종목이 상한가로 시작(시가[72]·저가[74] = 기준가
+8.00% = 1단계 상한)해 +20%까지 확대된 날이고, **고가[73]가 [17]과 정확히 일치**한다
(A0568000 1036.28, A0169 1041.10). 이 1036.28은 404차 후속5가 raw_candles 독립 분석으로
찾아낸 "하루 종일 한 번도 초과되지 않은 천장"과 같은 값이다 — 서로 다른 두 경로가
같은 숫자에 도달했다.

### [함정 1] 휴리스틱이 정답을 후보에서 빼버렸다

프로브 초안의 후보 필터에 `v != cur`(현재가와 다른 값만)이 있었는데, **상한가에 닿은
날에는 현재가 == 상한가**라 정답 [17]이 통째로 배제됐다. 초안이 "가장 유력"으로 내놓은
[89]/[14]는 오답(이론가·전일종가 계열)이었다.

**자동 확정을 막아둔 설계가 실제로 사고를 막았다.** 스크립트가 [89]/[14]를 그대로
기입했다면 진입 게이트가 이론가를 상한가로 알고 오작동했을 것이다.

### [함정 2] 단일 종목으로 확정했으면 틀렸다 — [116]/[117] 탈락

근월물 A0568000만 보면 [116]=1036.28, [117]=794.50으로 [17]/[18]과 **값이 완전히 같아**
구분이 안 된다. 6종목으로 넓히자 [116]/[117]은 **4개 종목에서 0.00**이었다(파생/불안정
필드). 그쪽을 골랐다면 대부분 종목에서 조용히 0.0을 읽어 게이트가 항상 통과했을 것이다
— B51(2026-05-10 잘못된 인덱스)과 같은 계열의 사고를 교차검증이 막았다.

### [발견·방어] 상한과 하한이 비대칭이다 (+20% / −8%)

프로브 초안은 "상한·하한은 기준가 중심 대칭"이라고 가정했는데 **실측은 비대칭**이었다.
닿은 쪽(상한)만 3단계까지 확대되고 안 닿은 쪽은 1단계에 머문 것으로 보이나 **확인되지
않았다.** 만약 하한이 −8%로 고정이라면 07-28(−11.64%) 같은 폭락일에 종가가 하한선
아래에 놓여 **SHORT가 하루 종일 잘못 막힌다.**

→ `is_at_daily_limit()`에 **초과 방어**를 넣었다: 현재가가 제한선을 **넘어서** 있으면
값 신뢰 불가로 보고 차단하지 않는다. 진짜 제한선은 정의상 넘을 수 없으므로, 넘었다면
그 값이 오래됐거나 안 맞는다는 뜻이다. 이 가드 덕분에 **하한 필드의 동적 여부를 몰라도
안전하다** — 07-28 시나리오를 회귀 테스트로 고정했다.

또한 프로브의 판별식을 대칭 검사 → **기준가 대비 고정 비율 매칭**(×1.20/1.15/1.08,
×0.92/0.85/0.80)으로 교체하고, 고가/저가 일치 확증과 "단일 종목 확정 금지" 경고를 넣었다.

### [실측] 종단 검증 (A0568000)

```
스냅샷: price=1036.28 high=1036.28 low=932.66 upper=1036.28 lower=794.50
모순검증 채택 ✓ | 고가==상한가 True
LONG  @1036.28 → 차단 ✓     SHORT @1036.28 → 통과 ✓ (되돌림 베팅은 상한선과 무관)
정상가 915.39  → LONG/SHORT 모두 통과 ✓
```

### [운영 주의] COM 권한과 레이트리밋

- **클라이언트 프로세스도 관리자 권한이어야 한다.** 크레온이 상승 권한으로 돌 때
  비관리자 파이썬은 `CpCybos.IsConnect=0`을 받는다(프로세스는 떠 있는데 연결 실패로
  보여 오진하기 쉽다). 이번에도 이 때문에 처음 프로브가 막혔다.
- 근월물 자동 탐색(`get_nearest_futures_code()`)이 **빈 문자열**을 반환했다. 코드를
  인자로 넘기면 정상 동작한다. 이 함수의 결함은 별건으로 남겨 둔다.
- `Dscbo1.FutureMst`를 연속 호출하면 30초 타임아웃이 난다(레이트리밋 추정). 운영은
  기동 시 1회 호출이라 무관하지만 프로브를 여러 종목에 돌릴 때는 간격이 필요하다.

---

## 2026-08-01 (MW0602 404차 후속6 — 당일 가격제한선 배선: FID 305/306은 배선 불가 판정)

### [정정] FID 305/306을 배선하라는 지시는 그대로 수행할 수 없다 — 브로커가 다르다

404차 후속5가 "`FID_UPPER_LIMIT(305)`/`FID_LOWER_LIMIT(306)`이 정의만 되고 미사용"으로
등록했고 그 배선을 착수했으나, **그 상수들은 배선 대상이 아니다.**

| | |
|---|---|
| 라이브 백엔드 | **Cybos** (`BROKER_BACKEND` 기본값 "cybos", 이 PC env 미설정 확인) |
| FID 305/306 위치 | `config/constants.py`의 "실시간 FID" = **키움** 섹션 |
| 실제 소비처 | `collection/kiwoom/*` 뿐 — **휴면 경로** |
| Cybos 필드 접근 | `GetHeaderValue(index)` — **FID 개념 자체가 없음** |

즉 305/306을 문자 그대로 배선하면 휴면 키움 경로에 코드를 넣는 것이라, 고치려던
"정의만 되고 안 쓰이는 상수" 문제를 방향만 바꿔 반복하게 된다. `constants.py`에
**[키움 전용/휴면]** 표기와 Cybos 대체 경로 안내를 달아 재발을 막았다.

### [설계] Cybos 등가 경로는 `Dscbo1.FutureMst` — §4 제약 밖이라 안전

이미 쓰고 있는 `CybosAPI.request_futures_snapshot()`이 그 TR이다. **BlockRequest(TR)라
COM 실시간 콜백이 아니므로** 절대원칙 §4(콜백 내 dynamicCall·emit 금지)와 무관하다.
새 구독을 만들지 않고 기존 스냅샷에 필드 2개를 더 읽는 것으로 끝난다.

### [보류] 헤더 인덱스는 추측하지 않는다 — 프로브 선행

상한가/하한가의 GetHeaderValue 인덱스가 알려져 있지 않다. 기존 인덱스(71=현재가,
80=미결제약정, 115=market_state …)도 **2026-05-10 라이브 스냅샷 실측으로 교정**된
값이다(B51 — 그 전엔 잘못된 인덱스로 엉뚱한 값을 읽고 있었다). 잘못된 인덱스로
상한가를 읽으면 진입 게이트가 엉뚱한 가격에서 차단하므로 추측 배선은 금지한다.

이번 세션에는 Cybos가 미로그인(`CpCybos.IsConnect=0`)이라 프로브를 돌리지 못했다.
그래서 **계측 먼저, 그다음 배선** 원칙대로 프로브와 소비 경로를 전부 구현해 두고
인덱스만 비워 뒀다 — 다음 거래일에 프로브 1회 실행 → 상수 2개 기입이면 끝난다.

### [구현] 인덱스만 채우면 동작하는 완전 배선

- `scripts/probe_cybos_limit_price.py` **신설** — FutureMst 헤더 전수 덤프 + 상한/하한
  후보 추출 + **대칭성 점검**(상한·하한은 기준가 중심 대칭, KRX 제한폭 ±8/15/20%).
  후보를 제시만 하고 **자동 확정하지 않는다**(B51 재발 방지).
- `config/settings.py` — `CYBOS_FUTUREMST_UPPER_LIMIT_IDX` / `_LOWER_LIMIT_IDX`
  (기본 **None** = 기능 비활성, 나머지 동작은 그대로).
- `api_connector._read_opt_idx()` — 인덱스가 None이면 **아예 읽지 않는다.** 추측값이
  흘러드는 경로 자체를 없앤다.
- `realtime_data.daily_limits` — 스냅샷에서 1회 확보. **모순 검증**(상한>하한,
  하한≤현재가≤상한) 통과 시에만 채택하고, 실패하면 WARNING + 미채택.
- `market_state.is_at_daily_limit()` — 방향별 판정. 상한가에서 LONG만 막고 SHORT는
  통과(되돌림 베팅은 상한선과 무관). 하한가는 반대. 정보 없으면 절대 차단 안 함.

### [메움] 후속5가 남긴 구멍을 정확히 겨냥한다

후속5의 `is_untradeable_now()`는 체결이 붕괴한 **뒤에야**(연속 5분) 발동해서, 아직
유동적인 상태의 상한가 진입은 못 막았다 — 07-31 14:17 LONG이 그 사례(vol 109로 멀쩡,
게이트는 14:26부터). 그때 진입했다면 TP1(1037.07)이 상한(1036.28) 밖이라 채워질 수
없고 스톱만 맞는 구조였다(섀도 실측 −2.366pt). `is_at_daily_limit()`이 그 지점을 막는다.

### [함정] 역전 모드에서 정반대로 차단할 뻔했다

진입 경로에는 `_resolve_entry_direction()`의 역전 로직이 있어 `raw_dir_str`과
`final_dir_str`이 다를 수 있다. 처음에 `raw_dir_str`로 판정하게 짰는데, 역전 모드에서는
**상한가에서 SHORT 체결을 막고 LONG 체결을 통과시키는 정반대**가 된다. 실제 체결
방향인 `final_dir_str`로 교체했다(`_execute_entry`에 넘기는 값과 동일).

---

## 2026-08-01 (MW0602 404차 후속5 — P0-B: 거래불능(가격상한) 구간 계측·진입보류)

### [전제 정정] 07-31 고착은 상한가가 맞지만, 리포트의 시간 서술은 틀렸다

0731 리포트 §1-C 이상점 3은 "14:20~15:06 가격 고착 46분"이라고 썼다. **틀렸다.**
14:34~14:37은 거래량 499/315/250/225에 저가 1032.80까지 내려간 **정상 거래 구간**이다.
시간범위 블랭킷 제외는 오답이고 분봉 단위 판정이어야 한다.

상한가라는 성격 자체는 데이터로 확인된다 — 1036.28을 초과한 분봉이 **하루 종일 0개**
이고 42개 분봉이 정확히 그 가격을 터치한다. 07-31만 유일하게 "시가=일중저가,
종가=일중고가(+18.55%)" 패턴이다.

### [기각] 리포트가 지목한 MFE·캡처율 오염은 실측상 존재하지 않는다

고착 분봉이 MFE를 오염시키려면 **그 극단을 고착 분봉만이 만들었어야** 한다. 그런데
1036.28은 유동 분봉 13개(10:06 vol 470, 10:09 379, 14:11 642 …)가 이미 만들었다.
29개 고착 분봉을 전부 지워도 MFE는 한 틱도 안 변한다.

> **구조적 이유**: 가격이 상한에 붙으려면 먼저 유동적으로 그 가격에 도달해야 하고,
> 붙어 있는 동안은 `high == low`다. 즉 고착 분봉은 **새로운 극단을 만들지 못하고
> 기존 극단을 반복**할 뿐이라, max/min 연산인 MFE·MAE에는 원리적으로 무해하다.
> 예외는 갭 상한가 직행(유동 분봉 없이 극단 생성) — 이번 표본엔 없다.

리포트 전문 before/after diff로 확인했다: 필터를 켜도 **단 한 글자도 바뀌지 않았다.**

### [기각] 체결 방향 비대칭 가정 — 데이터로 판단 불가

"상한가에서는 매도만 체결된다"는 가설을 세웠으나 근거를 만들 수 없었다. 고착 분봉의
`sell_vol`은 전부 0이지만 **13개 분봉은 buy/sell 둘 다 0(미분류)** 이고, 호가는
**역전**돼 있다(bid1 1036.28 > ask1 1036.26, 12/36 분봉 null). 열화된 피드에
과적합한 방향 규칙을 만드는 대신 방향 무관 전면 보류로 갔다.

### [발견] 진짜 오염 후보는 counterfactual의 목표가 쪽 — 단, 이것도 결국 기각

3개 채널 공통 행(07-31 14:17 LONG)이 `entry=1036.28`(상한가) / `tp1=1037.07`(상한
초과)로 기록돼 있었다. 처음엔 "이길 수 있는 경우의 수가 0인 가상거래"로 보고 판정
자동 제외(resolved 1→2)를 구현했으나 **되돌렸다.**

되돌린 이유: 14:17 시장은 유동적이었고(vol 109) 실제로 진입했다면 **체결됐을** 것이며,
TP1이 상한 밖이라 **정말로** 스톱만 맞았을 것이다. counterfactual은 모델링 결함이
아니라 나쁜 거래를 충실히 시뮬레이션한 것이고, 게이트의 손실 회피는 진짜다. 이런 행을
빼면 게이트의 정당한 공로를 지우게 된다.

남는 질문은 측정 오류가 아니라 **공로의 출처**다(게이트 알파인가, 신호가 우연히
가격상한에서 나온 시장 구조인가). "옳았지만 이유는 달랐다"는 해석 문제라 판정을
조용히 바꾸지 않고 **민감도만 병기**해 주간회의로 넘긴다(§9).

> ⚠ **[18] RegimeExhaustionGate는 3건에 판정이 뒤집힌다** — SUPPORTS_GATE(누적
> hyp −1.75) → REJECTS_GATE(+6.85). 표본 3건이 권고를 뒤집는다는 사실 자체가 그
> 채널 결론의 근거가 얼마나 얇은지를 보여준다(372차 이상치 분해와 같은 교훈).
> [6] 37.32→39.68, [7] −48.42→−46.05(판정은 둘 다 불변).

### [구현] 5단계 — 기존 리포트 출력은 100% 보존(순수 추가)

- **Phase 0** `utils/market_state.py` 신설. 오프라인 `detect_limit_pin_bars()`(최종
  일중극단 기반, 상한가 판별)와 라이브 `is_untradeable_now()`(연속 N분 zero-range +
  거래량 붕괴, 거래가능성 판별)로 **개념을 분리**했다. 라이브가 물어야 할 것은
  "규제 상한가인가"가 아니라 "지금 체결이 가능한가"이기 때문이다.
- **Phase 1** 캠페인 `[26]` 채널. 핵심 열은 고착 분봉 수가 아니라 **극단 유동터치**다
  (0이면 그때가 진짜 오염).
- **Phase 2** `_load_candle_maps(exclude_untradeable=)`. **호출부별 opt-in**이다.
- **Phase 3** `[27]` 도달불가 목표가 민감도 채널(읽기 전용, 판정 미반영).
- **Phase 4** 라이브 진입 보류 `LIMIT_PIN_ENTRY_BLOCK_ENABLED=True`.

### [함정] 블랭킷 필터가 레이블을 망가뜨렸다 — 실측으로 잡음

Phase 2를 처음엔 `_load_candle_maps` 안에서 무조건 제외하도록 짰다. 그러자 [1]
Triple-Barrier 채널의 OOS 표본이 **3m 584→572 / 5m 346→339 / 10m 170→166 /
15m 107→104 / 30m 50→48**로 깎였다. TB는 이 맵을 청산 시뮬이 아니라 **레이블
생성**(N분 뒤 가격)에 쓰는데, 고착 분봉이라도 그 시각 가격은 진짜다.

> **구분 기준은 "그 분봉을 무엇에 쓰는가"다** — 체결 시뮬(스톱/TP 도달, MFE)이면
> 제외, 가격 조회·레이블이면 제외 금지. 10개 호출부 중 TB 1곳만 끄고 9곳을 켰다.

### [함정] 라이브 감지기의 날짜 혼입

`fetch_recent_raw_candles(limit=60)`은 **날짜를 넘나든다.** 세션 필터가 시각만 보면
전일 장마감 고착 분봉이 당일 판정에 이어붙는다. 판정 함수가 최근 일자만 남기도록
수정하고 합성 표본 회귀 테스트를 넣었다.

### [실측] 발동 실적 — 캠페인 6,886 세션분봉 재생

- 라이브 게이트 발동 **23분봉(0.33%)**: 07-13 12:36~12:37, 07-31 14:26~15:06
- 07-13 발동은 상한가가 아니다(1108.88이 12:38에 뚫림). 그러나 그 6분간 vol 4~21의
  zero-range라 **차단 자체는 타당**하다 — 이 게이트는 상한가 판별기가 아니다.
- 07-31 실거래 3건(11:33·14:07×2)과 차단 구간 **겹침 0** — 이번 표본에서 실거래를
  막지 않았다.

> **한계 명시**: 이 게이트는 14:17 LONG을 **막지 못한다**(그때는 vol 109로 유동적,
> 발동은 14:26부터). 즉 "거래불능 구간 진입"은 막지만 **"아직 유동적인 상태에서
> 가격상한에 진입"** 은 못 막는다. 후자는 상한가를 라이브에서 알아야 하고, 그러려면
> `FID_UPPER_LIMIT(305)`/`FID_LOWER_LIMIT(306)` 배선이 필요하다(아래).

### [발견] FID 305/306이 정의만 되고 미사용 — 정답을 받을 수 있는데 추론 중

`config/constants.py:26-27`에 `FID_UPPER_LIMIT=305`/`FID_LOWER_LIMIT=306`이 있는데
**코드 어디에서도 참조되지 않는다.** 브로커가 당일 상한가/하한가를 실시간으로
내려주는데 수집하지 않아, 지금은 분봉 형태로 추론하고 있다. 배선하면 감지가
추론→확정이 되고 위 한계도 해소된다. COM 구독 경로 변경이라 절대원칙 §4 리스크
구간이므로 이번엔 손대지 않고 등록만 한다(`NEXT_TODO.md`).
...
0731 정기점검이 "지배적 병리는 예측이 아니라 청산 기하"로 결론 낸 뒤, **그 기하를
지금 무엇으로 감시하고 있는가**를 전수조사했다. 차원별 매핑 결과:

| 차원 | 채널 | 상태 |
|---|---|---|
| A 초기 손절 거리 / B 초기 TP1 거리 | §23 `tp1_geometry_shadow` | ⚠ **리포트 미노출**(별도 실행 필요) |
| C TP1 도달 처리 / F TP2·TP3 배분 | [10] tp2_hold | 🔴 n=0 |
| **D TP1 보호전환 offset 폭** | **없음** | 🔴 **통째로 미커버** |
| E TP1 이후 트레일 폭 | [12] tp1_trail | ✅ PASS (n=17) |
| G 손절 계단화 | [11] n=6 / [14] n=0 | 🔴 표본 부족 |
| H 시간 기반 청산 | [4] signal_decay | ✅ PASS |
| I 체결 슬리피지 | [17] | 👁 n=14 |
| J 감지 지연 | [15] 급행 풀스톱 | 👁 n=19 |

### [발견] 데이터는 쌓이는데 소비처가 없는 테이블 — 402차 후속3과 같은 계열

`synthetic_partial_exits`(339차 신설)에 TP1 보호전환 **23건**이 진입가·TP1 도달가·
보호스톱·모드까지 온전히 기록돼 있었다. 그런데 캠페인 리포트에서 이 테이블을
읽는 곳이 **하나도 없었다**(주석에 "동일 계열"로 언급만 됨). 402차 후속3의
`toxicity_block_shadow`가 "기록이 안 되던" 사각지대였다면, 이번은 **"기록은 되는데
아무도 안 읽는"** 사각지대다.

실제로 계산해보니 차원 D의 병증이 그대로 드러났다(n=23):
- 반납 **15건** / 더 달림 8건
- 반납 **중앙값 1.00pt** vs **평균 0.21pt** — 0729 폭락일 2건이 평균을 5배 끌어올려
  평균만 보면 "8% 반납"으로 무해해 보이지만 중앙값 기준은 그 몇 배다(372차 원칙).
- 보호폭 vs 반납 상관 **−0.295**(좁을수록 더 반납 — 가설과 방향 일치, 단 n=23)

### [정정] 두 PC A/B 차이는 protect mode 때문이 아니다

조사 도중 "MW0601(breakeven) vs MW0602(atr_profit)의 설정 차이가 §4의 tp1_x2 부호
반전(+32.78 vs −19.04)을 설명할 수 있다"는 가설을 세웠으나 **틀렸다.**
`tp1_geometry_shadow`는 protect mode를 **전혀 입력으로 쓰지 않는다**(4개 변형 전부
"TP1 도달=전량청산" 동일 규칙, 입력은 진입가·ATR·분봉뿐). 부호 반전의 원인은
원 설명대로 **두 PC의 거래집합 차이**이며 §4 결론은 그대로 유효하다.

### [실측] breakeven vs atr_profit — MW0602는 현행이 22/23 우세

`tp1_single_contract_mode`가 MW0602는 `atr_profit`(session_state.json·런타임 로그·
DB 23건 세 소스 일치), MW0601은 403차 기록상 `breakeven`이다. 이 파일은 gitignore
대상 **PC 로컬 설정**이라 두 PC가 서로 다른 청산 정책으로 돌고 있다.

MW0602 23건을 분봉 재생해 비교: 현행 `atr_profit` 합계 +64.19pt(승률 100%) vs
`breakeven` +60.35pt(승률 65.2%), **건별 우세 0/23**. breakeven은 스톱이 진입가라
더 멀어 버티는 여지는 크지만, 대부분(19건) 결국 본전으로 되돌아와 **0.00으로 끝난다.**
403차가 breakeven을 문제로 지목한 것(*"상방이 사실상 0으로 캡"*)이 수치로 확인됐고,
**MW0602를 breakeven으로 바꾸는 것은 반대**다(방향이 반대 — MW0601을 atr_profit으로
바꾸는 쪽이 검토 대상, 단 MW0601 실제 설정 확인 선행 필요).

### [구현] 3건 — 전부 라이브 코드 무변경

- **[24] TP1 보호전환 반납 관찰**(신설, OBSERVE) — `synthetic_partial_exits`를 읽어
  반납 중앙값·건수비·보호폭 상관을 낸다. 차원 D의 눈을 뜨는 것이 목적이며 판정은
  하지 않는다(처방 검증은 [25] 담당). 소급 23건 즉시 확보.
- **[25] TP1 보호전환 offset A/B**(신설, 판정) — `scripts/tp1_protect_offset_shadow.py`.
  **오프라인 스크립트**다(403차 P1-6과 동일 설계): `synthetic_partial_exits` +
  `raw_candles`만으로 재구성되므로 진입 경로 섀도 INSERT가 불필요 → 라이브 회귀
  위험 0 + 과거 표본 소급 적용. 변형 6종(current/breakeven/breakeven_plus/
  atr_lock_0.50/0.75/bar_range), 사전등록은 `VALIDATION_CAMPAIGN`.
- **[23-B] `tp1_geometry_shadow` 리포트 편입** — 계산부를 `compute()`/`summarize()`로
  순수 추출(출력 바이트 동일 확인)해 캠페인 리포트가 직접 호출. 403차 신설 이후
  별도 실행해야만 보여 주간회의에 노출되지 않던 상태를 해소.

### [판정] [25]가 즉시 FAIL을 냈으나 — 적용 금지 권고

`atr_lock_0.75` **+0.92pt**, `bar_range` **+0.81pt**로 현행 초과 → 사전등록 기준상
FAIL(변경 검토). 그러나 372차 이상치 분해 결과:

| 대안 | 합계 | 최대 기여 1건 제외 | 건별 우세 | 중앙값 차 |
|---|---|---|---|---|
| atr_lock_0.75 | +0.92 | **−1.33 (역전)** | 8/23 | 0.000 |
| bar_range | +0.81 | **−0.29 (역전)** | 7/23 | 0.000 |

**단 1건만 빼면 부호가 뒤집히고**, 건별 우세는 소수이며 중앙값 차이는 0이다.
FAIL 표기는 사전등록 기준을 정직하게 적용한 결과로 남기되, **현행 유지가 합리적**
이라는 판단을 리포트 본문에 명시했다. 표본 축적 후 재판정 대상.

### [사전등록 정직성 고지 — 의도적으로 남김]

`breakeven` 변형의 결과는 이 채널을 **만들기 전** 조사에서 이미 1회 측정됐다.
따라서 breakeven 항목은 재확인용이며 **사전등록된 검증이 아니다.** 이 채널의
사전등록 가치는 아직 측정된 적 없는 `atr_lock_0.50/0.75`·`bar_range`에 있다 —
이 사실을 settings 주석과 리포트 본문 양쪽에 명시했다(§9 자기기만 방지).

### [검증]

- `py_compile` 통과. `tp1_geometry_shadow.py` 리팩터링 후 CLI 출력이 종전과 **바이트
  동일**함을 확인(순수 추출 검증).
- 캠페인 리포트 전체 생성 성공. **기존 24개 채널 판정 전부 무변화**, 신설 3개만 추가
  (총 27개). 오프라인 스크립트 호출은 try/except로 격리해 실패해도 리포트가 죽지 않는다.

### [남은 것]

- **MW0601 `session_state.json` 확인 필요** — 이 PC에서는 불가. 두 PC 청산 정책이
  실제로 다른지 확정해야 §4 교차검증의 의미가 정리된다.
- [DONE 2026-08-01] **잘못된 주석 2곳 정정 완료** — `config/settings.py`
  (`TP1_TICK_ENABLED`)·`main.py`(`_process_tick_tp1` docstring)가 MW0601 로컬 설정
  (`'breakeven'`)을 프로젝트 공통값처럼 단정형으로 기술하던 것을 "PC마다 다름 —
  각자 `data/session_state.json` 확인" + 양 PC 실측값·[25] 결과 병기로 교체.
  402차 "공유 문서에는 PC 구분 명시" 원칙 준수.
- [10]/[11]/[14] 표본 기아는 채널 결함이 아니라 **qty=1 편중**(MW0602 88%) 때문 —
  사이징이 바뀌지 않는 한 대기해도 판정 불가. 이번엔 손대지 않았다.

**관련**: 0731 정기점검(§3 청산 기하 결론), 403차 P1-6(오프라인 A/B 선례),
402차 후속3(계측 사각지대 패턴), 372차(이상치 분해), 313차·§9 원칙.

---

## 2026-08-01 (MW0602 404차 후속2 — intraday 학습창 20,000→4,800 정정 + 시간감쇠 반감기 학습창 비례로 전환)

### [정정] 404차 후속 §1의 "시간감쇠 미구현" 진단은 오독이었다

404차 후속이 `_make_sample_weight`를 "docstring은 시간감쇠를 표방하는데 per-sample
가중치에 감쇠가 실리지 않는다 = 결함"으로 보고했다. **틀렸다.** 도입 커밋
(165차 `88cf7fd`)이 명시한 설계는 *"동적 역빈도+시간감쇠 **class weight**"* 이고,
커밋 본문도 *"재학습 시마다 최근 500봉 분포 자동 반영"* 이라 적었다. 즉 decay는
처음부터 **클래스 빈도를 최근 구간 기준으로 추정**하기 위한 것이고, 반환되는
per-sample 배열이 라벨별 클래스 가중치인 것은 sklearn `sample_weight`의 올바른
사용법이다. 구현은 의도와 일치한다.

### [진짜 결함] 반감기 70봉 = 유효표본 101봉으로 수천~수만 봉의 가중치를 정한다

`_DYN_HALFLIFE=70`(165차 100 → 186차 70)은 학습창 크기와 무관한 절대값이다.
기하급수 합으로 유효표본(ESS) ≈ hl/ln2 ≈ **101봉**에 불과한데, 그 101봉에서
추정한 클래스 가중치가 학습창 전체(20,000봉)에 적용된다. 가장 오래된 표본의
감쇠 가중치는 EOD 기준 `2^-560`으로 언더플로해 0이다.

연속 재학습 5시점(58봉 간격) 실측 — 클래스 가중치 변동폭:

| 호라이즌 | 현행 hl=70 | 수정 후 hl=n/4 | 감소 |
|---|---|---|---|
| 1m DOWN | 0.562 | 0.037 | 15배 |
| 5m DOWN | 0.925 | 0.060 | 15배 |
| 5m UP | 0.686 | 0.152 | 4.5배 |

재학습마다 클래스 가중치가 ±38%씩 요동쳤고, 이는 404차 후속이 측정한
"연속 재학습 간 예측 불일치 16~22%"의 원인 중 하나다.

### [워크포워드 A/B] 40시점 × 6호라이즌 풀링(n=2,320/호라이즌, SE≈1.0%p)

**항목2 단독**(창 고정 W=4,800, 반감기만 변경) — 현행 대비:
`1m +0.09 / 3m -1.64 / 5m -0.65 / 10m -0.26 / 15m +1.72 / 30m +1.90 (평균 +0.19%p)`
→ **6개 전부 2SE(2.0%p) 이내 = 정확도 구분 불가.** 사전 등록 판정규칙 ①
("정확도가 구분되지 않으면 안정성으로 결정")에 따라 채택 — 안정성 지표
(클래스가중치 변동폭 합)는 22.23 → 3.47로 **6.4배** 개선.

**항목1 단독**(반감기 고정 hl=70, 창만 변경) — 현행 대비:
`1m -0.34 / 3m -2.72 / 5m -0.73 / 10m -0.99 / 15m +3.92 / 30m +2.67 (평균 +0.30%p)`
→ **호라이즌별로 방향이 갈린다.** 장기(15m/30m)는 뚜렷이 개선, 단기(3m)는 악화.

거래 관점 지표(비FLAT 방향예측만의 적중률):
`1m +0.69 / 3m -1.96 / 5m -2.08 / 10m -0.38 / 15m +8.61 / 30m +5.42 (평균 +1.72%p)`
→ 같은 갈림. **5m은 최근 30일 진입 58건으로 거래량이 가장 많은 호라이즌인데
2.08%p 악화**한다 — 이 점은 반드시 관찰이 필요하다.

**중요**: D(W=20,000 + hl=n/4)·F(W=20,000 + hl=2,000)는 명확히 악화(평균 -3.79 /
-1.55%p)했다. 즉 반감기 연장은 **작은 창과 짝지을 때만** 유효하다 — 큰 창에서
반감기를 늘리면 10주 전 레짐의 클래스 분포를 끌어와 오히려 해롭다. 두 항목이
독립이 아니라 결합해야 성립한다는 뜻이며, 이번에 둘을 함께 바꾼 것과 정합적이다.

### [부수효과] 186차 FLAT 편향 장치와의 상호작용 — 개선 방향

예측 FLAT 비율: `3m 66.8%→46.8%(-20.0%p)` / 15m +12.9 / 30m +8.4 / 5m +6.9 /
10m +2.6 / 1m +2.1. **3m의 FLAT 고착(186차가 잡으려던 바로 그 문제 — "09:00~10:00
상승장에도 UP 0건")이 크게 완화**됐고, BiasReset 임계 0.70에 근접해 있던 3m(66.8%)이
임계에서 멀어졌다. 나머지 호라이즌은 FLAT이 다소 늘었으나 0.70 근처는 없다.
3m의 전체 정확도 하락(-2.72%p)은 FLAT 과잉예측 감소와 같이 움직인다 — FLAT이
평가구간 최다 클래스라 FLAT을 많이 찍으면 정확도가 올라가는 구조이므로,
전체 정확도만으로 우열을 판정하면 안 된다.

### [구현] 3건 — 이 중 2건은 검증 과정에서 발견한 필수 동반 수정

- **`MAX_TRAIN_BARS_INTRADAY` 20,000 → 4,800** (12.5거래일 × 384봉, 최근 15거래일
  중앙값 기준). 20,000봉은 실측 54거래일(10.8주)로 주석 의도(2.5주)와 4배 어긋나
  있었다 — 데이터 누적으로 같은 상수가 뜻하는 기간이 늘어난 것.
- **`MAX_CV_FOLD_BARS = 20,000` 신설(동반 필수)** — 같은 상수가 "장중 학습창"과
  "32-bit OOM 방어용 CV 폴드 절단 상한" 두 목적에 재사용되고 있었다. 분리 없이
  4,800으로 낮추면 PreRetrain(08:55, `intraday=False`·`full_cv=False`)의 CV 폴드
  학습셋이 20,000→4,800으로 **조용히 축소**된다. 실측으로 폴드3이 29,407→20,000
  으로 종전과 동일하게 절단됨을 확인(회귀 없음).
- **`retrain_now`의 `MIN_TRAIN_BARS` 검사를 intraday 인지형으로 정정(동반 필수)** —
  `_load_from_db`가 절단 후 4,800행을 반환하는데 `retrain_now`가 15,000행을 요구해
  **모든 intraday 재학습이 `ok=False`로 전량 실패**했다(실측). 종전에는 20,000 >
  15,000이라 우연히 통과하고 있었을 뿐이다. 이 수정이 없으면 DriftRetrain·
  WarmupRetrain·EKS해제·ExchangeCB 재학습이 전부 죽는다.
- **`_resolve_halflife(n) = max(n × 0.25, 70)` 신설** — `batch_retrainer` ·
  `multi_horizon_model` 양쪽 동기화(후자는 `fit()` 라이브 호출자가 없어 사실상
  휴면 코드이나 "동일 로직 유지 필수" 규약대로 함께 수정).

### [검증]

- `py_compile` py37_32·py310_64 통과, 기존 unittest 10건 전부 통과(변경 전후 동일).
- 두 `_make_sample_weight` 구현이 n=0/1/50/279/280/4,800/20,000에서 **비트 동일**
  출력임을 확인. n<280에서는 하한 70이 적용돼 종전 동작 그대로(소표본 회귀 없음).
- **라이브 회귀 검증**: 모델 6종 백업 후 `retrain_now(intraday=True)` 실제 실행 →
  1차 `ok=False`(위 MIN_TRAIN_BARS 결함 발견) → 수정 후 `ok=True`,
  `n_samples=4800` 전 호라이즌, 소요 28.3초. `acc.txt` 6개 md5 불변(intraday는
  갱신하지 않는 기존 동작 유지). 검증 후 모델 파일 전량 원복.
- EOD 경로 무영향 확인: 로드 39,209행·`full_cv=True`라 `MAX_CV_FOLD_BARS` 미적용.

### [남은 위험 — 의도적으로 열어둠]

- **3m/5m 단기 호라이즌 성능 저하**(전체 -2.7/-0.7%p, 방향 -2.0/-2.1%p)는 실측된
  비용이다. 특히 5m은 거래량 최다 호라이즌이라 라이브 관찰이 필요하다.
  근본 해법 후보는 **호라이즌별 학습창 차등화**로, 이미 Phase 2 경로에
  `TRAINING_WINDOW_BARS` 인프라가 있다(Phase 1/intraday 경로는 미적용).
  이번엔 사용자 지시 범위(단일 상수 정정)를 넘지 않기 위해 착수하지 않았다.
- A/B 표본은 40시점 × 58봉 = 6거래일 남짓이고 일중 자기상관이 있어 실효 SE는
  표기값(1.0%p)보다 크다. **단일 A/B로 확정하지 말 것**(313차) — 라이브
  `guard_shadow_log`(404차 후속 [23-부속])에 intraday cv_acc가 쌓이므로 그
  추세로 사후 확인할 것.

**Why**: "주석대로 고쳐라"는 단순 상수 변경으로 보였으나, 실제로는 (a) 같은 상수의
이중 용도, (b) 하류 최소표본 검사와의 결합, (c) 진단 자체의 오독까지 얽혀 있었다.
라이브 실행 검증을 넣지 않았다면 intraday 재학습 전량 실패를 커밋할 뻔했다.

**관련**: 404차 후속(§1 실측·오독 정정 대상), 165차 `88cf7fd`(decay 설계 의도),
186차 `5aadd83`(halflife 100→70, FLAT 편향), 313차·§9 원칙.
...
**배경**: MW0601 403차 종합(커밋 `827bd04`)이 P0-1로 보정기 복원을
`_scaler_warmup_worker` 내부에서 `__init__`(`_restore_ensemble_calibrator`)으로
옮겨 **항상 실행**되게 만들었다. 동시에 P0-2로 축퇴 가드를 신설했으나, 커밋
메시지와 NEXT_TODO에 *"MW0602에서 P0-1을 켜기 전 `auc=` 값을 먼저 확인할 것 —
0.53 이상이면 가드가 통과시켜 09:00부터 봉쇄된다"* 를 미확정 항목으로 남겼다.

**측정**: MW0602 `data/ensemble_calibrator.pkl`(2026-07-30 15:40 저장, n=296)에
프로덕션 코드(`PredictionCalibrator.load()`)를 그대로 적용.

```
auc = 0.5359   span = 0.01321   out_max = 0.3012
기준: auc < 0.53  AND  span < 0.020
  auc  조건: 0.5359 < 0.53 → False   ← AND 불성립
  span 조건: 0.01321 < 0.020 → True
→ is_degenerate = False,  _fitted = True   (가드 통과)
```

**결론**: 경고했던 시나리오가 MW0602에서 **실현될 상태였다.** 출력상한 0.3012 <
`ENS_CONF_FLOOR_FOR_AUTO`(0.33)이므로, 07-31 09:00부터 보정이 전면 적용되어
자동진입이 **산술적으로 0건** 확정. 07-30 실적 재현으로도 확인 — 그날 방향신호
164분봉 중 `conf>=0.33`이 29개였는데, 복원이 켜졌다면 0개가 되고 11:20 실거래
진입(conf 34.7%)도 차단된다.

### [설계결정] AUC 임계 상향이 아니라 "도달가능성"으로 판정한다

**왜 `DEGENERATE_AUC_MIN`을 0.54로 올리지 않았나**: n=200·양성률 0.30에서 AUC
표준오차는 약 0.047(Hanley-McNeil). 0.53은 "정보 없음(0.5)"에서 겨우 0.65 SE
떨어진 값이라 임계로서 변별력이 없다. **MW0601 0.491과 MW0602 0.536은 둘 다
0.5의 ±1 SE 안**이며, 어느 쪽이 걸리고 어느 쪽이 통과하는지는 사실상 표본
잡음이 정한다. 합성 검증에서도 라벨과 완전히 무관한 순수 랜덤 표본이 AUC
0.5318을 냈다(가드 통과). 임계를 0.54~0.55로 올리면 이번엔 진짜 약신호까지
축퇴로 오탐한다 — 313차 원칙상 이 방향은 막다른 길이다.

**대신 한 것**: `CAL_UNREACHABLE_FALLBACK_ENABLED`(기본 True) 신설.
보정기 출력상한 < `ENS_CONF_FLOOR_FOR_AUTO`이면 보정을 적용하지 않고 raw를
통과시킨다. 근거는 통계가 아니라 **지배관계(dominance)** 다 — 적용하면 자동진입
확률이 0으로 확정되고, 적용하지 않으면 raw가 그대로 나가 종전과 같다. 따라서
표본 크기·임계값 가정 없이 "미적용"이 "적용"을 지배한다.

**MW0601 축퇴 가드는 무수정**. 이 폴백은 AUC/span 판정과 **독립적으로 OR** 동작하며,
`_degenerate`와 `_unreachable`을 **따로** 노출한다 — 원인이 다르기 때문이다
(순위정보 상실 vs 스케일 불일치). AND→OR 연산자 변경도 검토했으나 기각:
합성검증에서 약신호(span 1.95pp)를 축퇴로 오탐했다.

**Why**: 402차 후속3의 `toxicity_block_shadow` 교훈("차단이 없었다"와 "계측이
죽었다"가 구분되지 않으면 원인이 뭉개진다)을 그대로 적용했다. 축퇴와 도달불가를
한 플래그로 합치면 다음 세션이 "왜 보정이 꺼졌는가"를 다시 추적해야 한다.

### [구현]

- `config/settings.py`: `CAL_UNREACHABLE_FALLBACK_ENABLED = True` 신설
  (사전등록 근거·롤백 방법 주석 포함).
- `learning/calibration.py`: `_evaluate_unreachable()` 신설,
  `fit()`·`load()` 양쪽에 축퇴 가드 **뒤**로 적용, `_unreachable` 상태 +
  `is_unreachable` 프로퍼티 노출. 상태 무변화 시 WARNING 억제(debug 강등).
- `main.py:_restore_ensemble_calibrator()`: 복원 로그에 `unreachable=` ·
  `out_max=` 추가(기존 `n/fitted/degenerate/span/auc`에 이어).

### [검증]

- `py_compile` py37_32 · py310_64 양쪽 통과.
- 가드 테스트 4군 **ALL PASS**: ①MW0602 실제 저장본 → 도달불가 발동·raw 통과·
  `calibrate(0.42)=0.4200` ②무신호 합성표본(AUC 0.5318, MW0601 가드 미탐) →
  폴백이 대신 차단 ③강신호(AUC 0.897, out_max 0.4381) → 정상 적용(오탐 없음)
  ④플래그 False → 403차 종합 원본 거동 재현(롤백 확인).
- flake8/pyflakes는 MW0602에 미설치라 400차 회귀 유형(F821)은 AST 검사 + **실제
  런타임 경로 실행**으로 대체 확인(load 도달불가·fit 도달불가·fit 정상·롤백 4경로 전부).
- **라이브 미검증** — 07-31이 첫 검증일. 확인 항목은 `NEXT_TODO.md` 동일 날짜 항목.

### [한계 — 의도적으로 남김]

폴백은 **봉쇄를 피할 뿐 스케일 불일치를 고치지 않는다.** 기저율 ~0.30인 한
정상 보정기도 출력이 0.33에 못 미치는 경우가 잦다 — 합성검증에서 AUC 0.861의
진짜 강신호 보정기조차 `out_max=0.2798`로 폴백 대상이 됐다. 즉 현재 설정에서
보정은 "대부분 꺼진 채" 운영된다. 근본 해결(하한 인하 또는 보정 함수형 교체)은
§9 사전등록 대상이라 코드가 임의로 결정하지 않는다 — 403차 종합의 동일 결론
(*"원인은 축퇴가 아니라 스케일 불일치"*)과 일치한다.

**관련**: MW0601 403차 종합(`827bd04`), MW0602 403차 리포트(`e26bedf`),
313차(소표본 판정 금지), 402차 후속3(계측 사망 vs 미발생 구분), §9 사전등록 원칙.
...
## 2026-07-31 (MW0601 403차 종합 — MW0601+MW0602 교차검증 후 픽스 8건 구현)

**상세 리포트**: `docs/정기점검/매일점검/MW0601+MW0602-20260730-종합점검리포트.md`
**관련**: MW0602 403차 리포트(`3e2a781`), 402차 후속1~7, 313차·372차 원칙, 297차·396차 패턴.

### [교차검증] 두 PC를 겹쳐 보지 않았으면 놓쳤을 것 2건 · 잘못 결론 냈을 것 3건

**양쪽이 독립 재현 (확정)**
1. 앙상블 보정기 복원이 9거래일 연속 dead code — 저장 `n`이 매일 300대로 리셋
   (MW0601 368/367/369/369/369/195/318/300/296, MW0602 거의 동일 수열).
2. 보정기 축퇴 — MW0601 `coef_=0.002502` 출력폭 **0.05pp**(0.3047~0.3052)로
   기저율 0.3050과 소수 4자리까지 일치. MW0602(1.3pp)보다 25배 납작.
3. 청산 채널 손익 누출 — 틱 하드스톱이 MW0601 34건 PF **0.07**(-409만원),
   MW0602 46건 PF **0.09**(-533만원)로 TP2가 번 돈을 거의 전부 상계.

**MW0602 결론을 정정한 것**
4. `toxicity_block_shadow` — MW0602는 "채널 고유 결함 없음, 손대지 말라"로 끝냈으나
   MW0601 2건은 `atr_ok=True`·`grade≠X`이고 **`_hurst_ok` 하나만 실패**했다. 이 조건은
   라이브 마스터게이트(`_hurst_ok or HURST_SOFT_BLOCK_ENABLED`)보다 엄격해, 진입을
   전혀 막지 않는 조건이 섀도에서만 모집단의 39.7%(147/370분봉)를 삭제하고 있었다.
5. 손익비 3:1의 원인 — MW0602는 `HURST_REGIME_ATR_MULT["trend"]` 탓으로 진단했으나,
   hurst 배수는 stop·tp1에 **동일하게 곱해져 비율에서 약분**된다. 실측 검산: 비율은
   호라이즌만 결정하며 **1m 5.00:1 / 3m 3.00:1 / 5m 2.14:1로 전 버킷 상시**.
   MW0601 #207(hurst=mean-revert·3m)이 stop 7.13 / TP1 2.38 = 3.00:1로 동일 확인.
6. EOD 모델가드 — MW0602는 "정상 작동"으로 봤으나, `old_acc`는 `acc.txt`(최대 8일 전
   다른 분포)이고 intraday 재학습이 가드를 건너뛰며 pkl만 덮어쓴다. 07-30 파일시스템
   실측: 모델 mtime 6개 전부 **07-30 14:52**인데 acc.txt는 3m **07-22** / 15m **07-27**.
   즉 가드는 이미 8회 덮어써져 존재하지 않는 모델의 점수와 비교했다.

**MW0601 코드 + MW0602 통계가 맞물린 것**
7. 리스크/리워드 비대칭 — MW0602의 틱스톱 누출은 일자클러스터 p=0.093으로 미확립이었는데,
   `main.py:3225-3270` 틱 콜백이 **하드스톱·손절1차만** 감시하고 TP1/TP2/TP3는 매분
   STEP8(:33초)에서만 평가한다는 기전을 MW0601이 코드로 확정. 인과 주장이 p-값에
   의존하지 않게 됐다. MW0601 캡처율 실측 **9.7%**(MFE 197.39pt vs 실현 19.15pt).

**정반대로 나온 것 (313차 원칙 사례)**
8. JointGateBlock 오늘 카운터팩추얼이 MW0601 **+5.51pt**(8승2패, 완화 방향) vs
   MW0602 **-0.66pt**(존치 방향). 같은 날 같은 게이트인데 부호가 반대이고, 시뮬
   파라미터마저 달랐다(MW0601 캠페인 resolver / MW0602 자체 1.4×ATR·45분).
   → **어느 쪽도 임계 0.50 조정 근거로 쓸 수 없다.** 표본으로만 편입.

### [설계 결정] P0-2 축퇴 가드 — 초안의 "출력폭 5pp" 기준을 실측으로 폐기

**최초 제안**: fit 후 출력폭이 5pp 미만이면 축퇴로 보고 raw 통과.
**반증**: py310_64 실측(n=200, C=0.02, raw 0.24~0.80 합성표본)

| 신호세기 | coef | span | AUC |
|---|---|---|---|
| 무정보(acc .33 상수) | .026 | 0.55pp | .556 |
| 중간(.25→.55) | .059 | 1.45pp | .610 |
| 강함(.20→.75) | .118 | 2.88pp | .722 |
| 매우 강함(.10→.90) | .134 | **3.33pp** | .753 |
| MW0601 실제 저장윈도 | .006 | **0.13pp** | **.491** |

`C=0.02` 규제 하에서는 **매우 강한 신호조차 3.33pp**에 그친다 — 5pp로 잡으면 정상
보정기까지 전부 오탐으로 걸러 보정 기능을 영구 비활성화한다.
**결정**: 주 판정을 AUC(순위변별력)로 교체하고 span은 보조 조건(AND)으로만 사용.
`DEGENERATE_AUC_MIN=0.53`, `DEGENERATE_SPAN_MIN=0.02`.
**Why**: AUC≈0.5는 "raw conf가 적중/오답을 전혀 가르지 못한다"는 뜻이라, 어떤 단조
보정을 씌워도 Platt이 잡을 신호가 없음을 직접 의미한다. span은 규제 강도에 좌우되는
파생 지표라 단독 기준으로 부적합.
**검증**: 5개 테스트군 ALL PASS — 실제 축퇴 pkl 포착 / 중간·강함·매우강함 오탐 없음 /
순증 성분 없는 U자 포착 / 축퇴→정상 자동 복구 / MultiHorizonCalibrator 무영향.
**한계(코드에 명시)**: 비단조라도 순증 성분이 남으면 잡지 못한다(합성 톱니 AUC 0.547
통과). 부분 비단조 대응은 Isotonic/구간별 보정으로 함수형을 바꾸는 별건.

### [설계 결정] 축퇴 가드는 "진입 봉쇄"를 해결하지 못한다 — 별개 문제

기저율이 ~0.30인 이상 **정상 보정기도 출력이 0.30 근처에 모이므로**
`ENS_CONF_FLOOR_FOR_AUTO`(정적 0.33)를 넘지 못한다. 즉 봉쇄의 원인은 축퇴가 아니라
"하한이 보정된 3-클래스 확률의 도달 범위 밖에 있다"는 **스케일 불일치**다.
스모크 테스트로 확인: `min_conf`(동적)만 낮춰도 `max(0.33, min_conf)`의 정적 항이
binding이라 도달 불가가 해소되지 않는다.
**결정**: 하한 인하 vs 보정 함수형 교체는 §9 사전등록 대상이므로 코드가 임의로 내리지
않는다 — `_check_conf_floor_consistency()`가 **경보만** 남기고 자동 조치는 없다.
**Why**: 여기서 raw로 우회시키면 6/29 분석이 억제하려던 과신(conf 0.7+ tail이 실측
acc 29~31%)이 그대로 복원된다. 정직한 보정 결과를 무시하는 것이 답일 수 없다.

### [구현] 픽스 8건 — 파일·검증

| # | 항목 | 파일 | 검증 |
|---|---|---|---|
| P0-1 | 보정기 복원 → `__init__` | `main.py` `_restore_ensemble_calibrator()` | AST: `self.ensemble`(316) < 호출(338) |
| P0-2 | 축퇴 가드 + ConfFloorGuard | `learning/calibration.py`, `model/ensemble_decision.py` | 테스트 5군 ALL PASS |
| P0-3 | 섀도 조건식 정합 | `main.py:7461` | 마스터게이트 7225와 표현 일치 |
| P0-4 | old_acc 재측정(섀도) | `learning/batch_retrainer.py` `_measure_incumbent_acc()` | 실제 pkl 6종 로드·shape 정합 확인 |
| P1-5 | TP1 틱 감지 | `main.py` `_process_tick_tp1()`, `TP1_TICK_ENABLED` | elif 순서 stop<tier1<tp1, §4 준수 |
| P1-6 | 기하 A/B | `scripts/tp1_geometry_shadow.py` + 캠페인 사전등록 | 실행 확인 55건/11일 |
| P1-7 | conf_raw 관측필드 | `main.py` `entry_gate_json` | 필드 5종 추가 |
| P1-8 | (미구현, 절차만) | — | 선행조건 `NEXT_TODO.md` 등록 |

**공통 검증**: py37_32·py310_64 양쪽 `py_compile` 통과, `flake8 --select=F821,F823`
(400차 `UnboundLocalError` 회귀 유형) **0건**, 런타임 스모크 22항목 ALL PASS.
**라이브 미검증** — 07-31이 첫 검증일이며 확인 항목은 `NEXT_TODO.md`에 등록.

### [주의] MW0602에 P0-1을 켜기 전 확인할 것

MW0601 저장본은 AUC 0.491로 확실히 가드에 걸리지만, MW0602(coef 0.0636 / span 1.3pp)는
AUC를 측정하지 않아 걸릴지 모른다. **가드가 통과시키면 P0-1이 오히려 해롭다** —
어제 축퇴한 보정기를 09:00부터 되살려 종전보다 이른 시각부터 봉쇄된다.
MW0602는 `[Calibration] 보정기 복원 완료` 로그의 `auc=` 값을 먼저 확인할 것.
...
현재는 **개발 단계**라 사용자가 매일 **장전·장중·장후로 미륵이 작동흐름을 직접
모니터링**하고 있다. 그 상태에서 Slack 알림은 우선순위가 아니며 오히려 잡일만 늘린다.

**이 항목을 이슈로 다시 제기하지 말 것.**

### [배경] 330차 이후 반복 재등장하던 항목이다

`message_limit_exceeded`는 2026-07-13~14에 처음 터져 330차가 "무음 실패 재발방지"로
다뤘고, 07-17 워크스페이스 교체로 한 번 해소됐다가 07-27부터 재발했다. 그 뒤로
400차·402차 후속2가 각각 "관측성 공백"으로 다시 등록했고, 402차 후속2는 캠페인 [20]
채널을 만들면서 "판정을 밀어서 알려주는 경로는 Slack뿐이라 사람이 파일을 열어야만
한다"를 남은 구멍으로 적었다.

**그 전제가 틀렸다.** 사용자가 이미 매일 직접 모니터링하고 있으므로 "사람이 파일을
열어야 한다"는 결함이 아니라 현재 운영 방식 그 자체다. 복구 작업(워크스페이스 한도·
토큰 점검)은 순수한 추가 부담이다.

**Why**: 관측성 개선을 설계할 때 "푸시 알림 채널이 없다"를 자동으로 결함 취급했는데,
운영자가 상시 관측 중인 개발 단계에서는 그 가정이 성립하지 않는다. 관측성의 목적은
"정보가 사람에게 도달하는 것"이지 "푸시 채널이 존재하는 것"이 아니다.

**How to apply**: 알림·관측성 항목을 제안할 때 Slack 복구를 올리지 말 것. 캠페인
리포트·EOD 로그·대시보드처럼 **사용자가 직접 열어보는 경로**로 설계하면 충분하며,
"밀어서 알려주는 경로가 없다"를 결함으로 기술하지 말 것. 실전 전환(Phase 5)으로 넘어가
사용자가 상시 모니터링을 못 하게 되면 그때 사용자가 먼저 꺼낼 것이다.

**구현**: 코드 변경 없음. `NEXT_TODO.md`의 활성 항목 2건(400차·402차 후속2)을
`[보류 2026-07-29, 재론 금지]`로 전환. 세션이 바뀌어도 재제기되지 않도록 사용자 메모리
(`feedback_slack_notify_deprioritized`)에도 함께 등록했다 — dev_memory는 열어봐야
읽히지만 메모리 인덱스는 매 세션 로드되기 때문이다.
**관련**: 330차, 400차, 402차 후속2.
