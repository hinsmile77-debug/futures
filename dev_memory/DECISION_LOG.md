...
## 2026-07-28 (MW0601 로컬 400차 — 일일점검: EOD/P8 흐름·승패 딥다이브 + "399차 수치 불일치"는 오류가
아니라 MW0601/MW0602 두 PC의 서로 다른 trades.db를 비교한 것으로 정정)

### [배경] 사용자 요청으로 7/28 EOD+P8 동작·오늘 진입 승패를 이 PC(MW0601, v9-dev)에서
독립적으로 재점검하며, 당일 399차가 실행했다고 기록한 두 분석 스크립트(`hurst_softblock_
noop_analysis.py`, `p4_cvd_ofi_demotion_analysis.py`)를 이 PC의 로컬 `trades.db`/
`predictions.db`로 재실행했다. 결과가 DECISION_LOG 399차 항목의 수치와 표본 크기·승률·
손익 부호까지 전혀 달라, 처음에는 "399차가 재현 불가능한 수치를 기록했다"고 결론짓고 이
자리에 정정 항목을 적었었다. **그러나 사용자 확인 결과 399차의 근거 데이터는 MW0602에서
나온 것**이었다 — 즉 수치 불일치는 399차의 오류가 아니라, **서로 다른 두 PC(MW0601/
MW0602)가 각자 독립적으로 돌리는 실거래의 `trades.db`를 비교한 데서 온 당연한 차이**였다.

### [핵심 교훈] dev_memory는 git으로 PC 간 공유되지만 DB/로그는 PC별 로컬이라 "출처 PC"가
안 적혀 있으면 재현 시도 자체가 오판을 낳는다

`dev_memory/*.md`는 코드·config와 함께 커밋되어 MW0601↔MW0602 사이에 cherry-pick으로
공유되지만(393차가 이미 이 패턴으로 커밋 번호 충돌을 겪은 바 있음), `trades.db`/
`predictions.db`/`logs/`는 "코드/config/dev_memory만 커밋, 모델·DB·세션 등 PC별 런타임
산출물은 커밋 금지" 원칙(멀티 PC pull 호환성)상 PC별 로컬 런타임 산출물로 커밋되지 않는다.
그 결과 "어느 PC의 DB에서 뽑은 수치인지" 표기 없이 dev_memory에
구체 수치를 적으면, 다른 PC에서 그 항목을 검증하려는 사람(또는 나)이 자신의 로컬 DB로
같은 스크립트를 재실행해 다른 숫자를 얻고 "기록이 틀렸다"고 오판하기 쉽다 — 오늘 이 세션이
정확히 그 함정에 빠졌다.

**앞으로**: dev_memory에 DB/로그 기반 구체 수치(n=, 승률, 손익 등)를 적을 때는 출처 PC명
(MW0601/MW0602)을 반드시 함께 기록한다. 과거(2026-07-28 이전) 항목은 출처 PC 표기가
없을 수 있으므로, 다른 PC에서 재현이 안 될 때 곧바로 "오류"로 단정하지 말고 PC 차이
가능성부터 배제할 것.

**이 PC(MW0601)에서 직접 관측한 오늘(7/28) 데이터는 아래 [승패 딥다이브]/[313차 카운터
팩추얼] 섹션대로 별도로 유효하다** — MW0601의 `trades.db` 기준 오늘 실체결은 4건(3승1패,
전부 grade='A', 순손익 +635,183원)이며, 이는 399차(MW0602 기준)의 관찰과 별개의 유효한
관측치다. 두 PC 모두 같은 종목(A05/A0568)을 각자 독립 계좌로 운용 중이므로 서로 다른 거래
결과가 나오는 것 자체는 정상이다.

### [발견] 오늘(7/28) EOD GBM 재학습에서 6개 호라이즌 중 4개(1m/3m/5m/15m) 모델가드 보류
(MW0601 기준)

`logs/retrain_eod_20260728.log`(MW0601): 신규 학습 정확도가 구모델 대비 1m -4.84%p,
3m -14.99%p, 5m -11.61%p, 15m -12.92%p 하락해 EOD 모델가드(허용 2.5~3.0%p)가 전부 발동,
구모델 유지(10m/30m만 교체). 하루 단위로 4/6이 한꺼번에 가드 발동한 것은 이례적 — 같은 날
장중 내내 `RegimeFingerprint PSI=0.82~0.85 CRITICAL`(감시전용, 판정 비활성) 고착,
`day_regime_shadow=CHAOS`(신뢰도 0.45~0.95) 관측과 시점이 일치해, 오늘 학습 윈도우에 포함된
구조변화/변동성 급등 구간이 신규 GBM 적합을 일시적으로 오염시켰을 가능성이 높다. 가드 자체는
설계대로 정상 작동(악화된 모델을 실전에 내보내지 않음)했으므로 실거래 영향은 없음 — 다음
EOD에서 1m/3m/5m/15m이 정상 교체율로 복귀하는지 관찰 필요. (MW0602도 같은 시장 데이터를
보므로 동일 현상이 있었는지 대조해볼 가치가 있음 — 다음 교차점검 시 확인.)

**Why**: 멀티 PC(MW0601/MW0602) 병행 실거래 구조에서, dev_memory에 남긴 DB 기반 수치가
출처 PC 표기 없이는 "재현 불가능한 오류"처럼 보일 수 있다는 것을 실제로 겪은 사례. 313차
원칙(소표본 결론 금지)과는 별개로, 멀티 PC 환경 고유의 새로운 함정.

**How to apply**: 앞으로 dev_memory에 DB/로그 기반 구체 수치를 적을 때는 출처 PC명
(MW0601/MW0602)을 반드시 함께 기록할 것. 다른 PC 기준 항목을 이 PC에서 재현하려 할 때는
먼저 출처 PC 표기를 확인하고, 표기가 없으면 PC 차이 가능성부터 배제한 뒤에 "오류"로
단정할 것.

**구현**: 코드 변경 없음(읽기 전용 점검) — `dev_memory/DECISION_LOG.md`,
`dev_memory/NEXT_TODO.md`(400차 항목), `scripts/p4_cvd_ofi_demotion_analysis.py`
docstring(399차 관련 서술을 "PC 차이"로 정정) 갱신.
**검증**: MW0601 로컬 `trades.db`/`logs/`로 직접 조회·재실행(2026-07-28 저녁).
**관련**: 399차, 398차, 313차·372차 원칙.
...
## 2026-07-19 (358차, MW0601 세션) — 357차 주간회의 결론을 라이브 운영 PC(MW0601) 자체
데이터로 독립 재현·검증 + [2]/[3] 채널 MW0601 재학습 + 주간 체인 최초 소급 실행

> 전체 분석: `docs/정기점검/금요일점검/0719_주간회의_검토보고_MW0601.md`.
> 357차(MW0602 세션)의 자매편 — 같은 7/5 안건 6개를 MW0601(라이브 Cybos 운영,
> v9-dev 브랜치) 자체 trades.db/predictions.db로 재판독.

### [핵심 발견] git reflog 실측 — 341~355차가 이 PC엔 7/17 00:29에야 pull됨

333차 후속~338차는 7/15 09:52~12:43, 339·340차는 7/16 08:21·15:17, **341~355차(유령
하드스톱·틱스톱 즉시화·추격필터·사이징 상향 5천만/3% 등 15건)는 7/17 00:29**에
cherry-pick됨. 즉 7/16까지 MW0601의 모든 실거래는 구코드로 실행됐고, **7/20(월)이
이 PC의 341~355차·340차의 첫 라이브 검증일**.

### [실측] 안건① 순EV(유령 GHOST_PENDING_MISS 2건 제외) — 캠페인 누적 -3,643,302원

W1(7/9~10) -433만원(13건, 평균 3.5계약, 최대 10계약) → W2(7/14~16) +69만원(9건,
전부 1계약). 손실의 몸통은 W1 대형 사이즈 하드스톱(틱) 3건(-654만원, pnl -6.35pt·
-4.04pt·-2.01pt)에 집중 — 341차(유령 하드스톱)·348차(틱스톱 60초 지연)가 정확히
겨냥한 패턴이고 **당시 미수정 코드로 발생**. W2 손실 정지는 수정 효과가 아니라
켈리 슬럼프로 사이즈가 1계약에 눌린 결과(교란변수) — "이미 나았다"는 근거로 쓸 수
없음, 7/20 이후 실측 필요. 등급 역전(A -469만/승54% vs C +105만/승67%)은 MW0602와
동일 방향으로 독립 재현 — 357차 P1(등급-사이징 수술 보류, 실측 후 재평가) 확장 적용.

### [진단·수정] [2] Meta-Gate·[3] 분위회귀 — MW0601은 "버전 불일치"가 아니라
"모델이 한 번도 학습된 적 없음"

MW0602(357차)는 sklearn≥1.1 pkl을 1.0.2 런타임이 로드 실패한 경우였으나, MW0601은
`model/horizons/meta_gate/`·`quantile/` **디렉토리 자체가 부재** — 학습이 이 PC에서
한 번도 실행되지 않았던 상태(`ensemble_decisions` 16,976행 non-null 0건 실측,
357차 NO-DATA 생존점검이 정확히 🔴로 구분 표기 — 계측 개선의 첫 실전 검증 성공).

**수정**: `py310_64` 전체경로로 §4-1 체인 순서 그대로 수동 소급 실행:
① 게이트 ablation(7일, MW0601 최초 생성) ② 캠페인 판정 리포트(재학습 **전** 실행 —
§3-1 OOS 순서 준수, [2]/[3] NO-DATA 상태를 정확히 캡처) ③ 섀도우 TB 재학습(6/6
champion 대비 우위) ④ 분위회귀 재학습(6/6, pinball 0.37~2.28) ⑤ 메타라벨 분류기
재학습(6/6, AUC 0.50~0.57). 전 스텝 rc=0.

**세션 중 사고 및 즉시 수습**: 체인이 서브프로세스로 분위회귀 재학습을 실행하는
도중, 동일 스크립트(`train_quantile_regressor.py`)를 본 세션에서 별도로 한 번 더
수동 실행해 같은 출력 pkl에 쓰기 경합 발생 — 발견 즉시 중복 프로세스 종료
(TaskStop), 원본 체인만 완주시킴. **사후 검증**: py37_32 런타임에서 두 스코어러
직접 로드+스코어링 — 6개 호라이즌 전부 정상 float/dict 반환(ALL_OK, 손상 없음),
양쪽 환경 `py_compile` 통과.

### [MW0601 고유 발견] 안건② [0] 표본 기아 🚨 STARVED (MW0602는 정상)

주간 진입 9건(하한 10 미달), MW0602는 24건으로 정상 — 같은 캠페인 기간인데 PC별로
상반된 상태. 7/15~16 최대 차단 사유가 "사이저 산출 수량 0"(켈리 슬럼프)과 진입후보
분수 급감(180→58→37분, conf 저하)으로, 둘 다 7/20 첫 라이브인 340차(사이징 상향)·
353차(ConfStuckBoost)가 직접 겨냥 — **사다리 2단계(MetaGate take_ceil 완화) 적용은
보류하고 자연 회복 관찰 후 7/24 재판정 권고**(357차 P1과 동일 논리: 교란변수 회피).

### [제안, 결정 대기] M-P0-2: 7/20 조건부 롤백 기준 사전 등록

W1 손실 구조(대형 사이즈 × 틱스톱 결함)를 고친 수정(341·348차)과 사이즈를 다시
키우는 수정(340차: 5천만/3%)이 **7/20에 동시에 첫 라이브** — 이 PC 최대의 단일
리스크로 판단. 제안: 7/20~22 중 하드스톱(틱) 청산에서 `|pnl_pts|`가 스톱폭+2pt를
초과하는 사례가 1건이라도 재발하면 348차 수정 실패로 간주, `SIZING_TARGET_CAPITAL_KRW`
5천만→2.5천만 임시 롤백 + 원인 재수술. 재발 없이 20건 누적 시 340차 확정. **사용자
승인 필요** — 사전 등록 여부만 결정, 코드 변경 없음.
