...
## 2026-08-08 (MW0601 446차 — [3-B] 체제 민감도: 결론은 견고, 점검을 상시화)

**지시**: 445차가 등록한 후속(=[3-B] 체제 민감도 점검) 수행.
**성격**: 계측·리포트 계층. **라이브 무관·본판정 무변경.**

### 결론 — [3-B]는 체제 지정에 **견고하다** ([25]와 대조적)

`tp_trigger`·`protect_mode`를 바꿔가며 5개 시나리오를 실측했다(MW0601 110건):

| 시나리오 | n | q_raw Δ현행 | q_x0.7 | q_blend | 판정 |
|---|---|---|---|---|---|
| ① 사전등록(혼재) | 110 | −9.59 | −14.73 | −10.99 | PASS |
| ② └ `close` 부분집합 | 46 | −2.54 | −10.42 | −7.71 | PASS |
| ③ └ `envelope` 부분집합 | 64 | −7.05 | −4.32 | −3.28 | PASS |
| ④ 전량 `envelope+atr_profit` 강제 | 110 | −19.53 | −4.98 | −2.84 | PASS |
| ⑤ 전량 `close+breakeven` 강제 | 110 | **−0.92** | −5.32 | −3.14 | PASS |

**부호가 한 번도 뒤집히지 않는다.** 442차 결론("모든 대안이 현행 이하")은
`replay_regime` 경계 설정과 무관하게 성립한다 — 445차가 [25]에서 발견한 체제
민감도(`atr_lock_0.50` +6.73 → +0.24, MW0602 −3.76)가 여기서는 관측되지 않는다.

**왜 [25]와 다른가**: [25]의 질문(보호 offset)은 **arm이 언제 걸리느냐**와 직결되고
`tp_trigger`가 바로 그것을 정한다. 반면 [3-B]의 인공물(TP1 도달 = 폭에 비례하는
공짜 이익)은 **v2 사다리 자체가 제거**하므로 체제와 독립이다.

⚠ 크기는 여전히 체제 민감하다(−0.92 ~ −19.53). ⑤가 가장 약하므로 **"압도적 열위"로
서술하지 말 것.** 확정된 것은 부호(방향)이지 크기가 아니다.
⚠ 부수 확인: v2에서는 "폭이 넓을수록 좋아 보이는" 단조성이 사라졌다(④에서 q_blend
−2.84 > q_x0.7 −4.98인데 q_blend가 더 넓다). 인공물 제거의 방증이다.

### 조치 — 일회성 확인이 아니라 **상시 점검**으로 만들었다

체제별 부분집합 판정을 채널에 내장했다(`_by_regime()`). 추가 재생 없이 기존 실행
결과를 체제로 분할하므로 **비용 0**이며, 리포트가 매주 자동으로 찍는다.

- 체제별 판정이 풀링과 **일치하면 ✅**, **갈리면 🔴 "풀링 수치를 단독 근거로 쓰지
  말 것"** 경고를 띄운다.
- MW0602처럼 경계가 어긋난 PC에서 풀링 판정이 조용히 오염되는 것을 막는 장치다 —
  445차가 [25]에서 겪은 사고를 구조적으로 재발 방지한다.
- ⚠ **본판정에 반영하지 않는다**(사전등록 합격선은 풀링 기준 그대로, §9-4).
  ⚠ 부분집합은 표본이 쪼개지므로 313차상 단독 근거 금지 — 리포트가 함께 경고한다.

### 검증

- py310_64 + **py37_32** 양쪽 컴파일 통과.
- 내장 계산이 스크래치패드 프로브와 **일치**(close −2.54 / envelope −7.05).
- 리포트 재생성 rc=0, **판정 무변경**([3-B] PASS / [47] PASS / [25]·[25-B] FAIL).
  마크다운 구분선 깨짐 1건 발견·수정(변형 수에 맞춰 동적 생성).

### 남은 것

[25]에도 같은 `by_regime`를 넣는 것이 **더 가치 있다** — 실제 불일치가 거기서 났고
MW0602는 체제가 혼재한다. MW0601은 [25]가 `envelope` 균일이라 분할이 무의미해
이번에는 넣지 않았다. NEXT_TODO 등록.

**관련**: 445차(체제 민감도 발견), 443차([25] 이관), 442차([3-B] 이관),
MW0602 0808 §7, 421차 후속2, 313차, §9-4.

---

## 2026-08-08 (MW0601 445차 — MW0602 [25] 교차검토 검증: 불일치의 유력 원인은 체제 지정)

**계기**: MW0602가 443차를 cherry-pick해 [25]/[25-B]를 교차검토(0808 점검리포트 §7).
그쪽이 **MW0601 해석 2건이 재현되지 않는다**고 지적했고, 그 지적이 옳았다.
검증하다 **전달 누락 2건**을 함께 발견해 조치했다. **라이브 무관·판정 무변경.**

### 🔴 443차 서술을 정정한다 — "432차 ④ 재검토"는 과대 주장이었다

443차는 `breakeven` 붕괴(−10.22 → −0.08)를 근거로 *"432차 ④ '본전은 현행보다
나쁘다'가 v2에서 약해진다"*고 적었다. MW0602는 같은 코드로 **−16.52 → −15.25**
(거의 불변)를 얻었다. 그쪽 표본이 **76훅/13일**로 MW0601(**33훅/4일**)보다 훨씬
두껍다 — 313차 원칙상 **MW0601 관측을 일반화한 것이 잘못이었다.**
→ 그 서술은 **MW0601 전용 관측**으로 격하한다. 421차 후속2 판독 규칙대로 PC별로
따로 읽고 주간회의에서 병치할 것. (레지스트리 note는 이미 "재검토 필요"로만
적혀 있어 결정 자체에는 영향 없다.)

### 유력 원인 규명 — `replay_regime` 경계는 **MW0601 캘리브레이션**이다

settings가 이미 경고하고 있었다("PC별로 다르다 … MW0602에서 판정할 때 재확인이
필요하다"). MW0601 atr_profit 훅은 **전량 08-03 이후**라 체제가 균일(envelope)인데,
MW0602는 원래 atr_profit이라 훅이 **08-03 이전까지 뻗는다**(76훅/13일). 그 구간은
`regime_for()`가 `pre`(`tp_trigger="close"`)를 준다.

**민감도 실측** (MW0601 33훅, `tp_trigger`만 교체 — 다른 조건 동일):

| 변형 | envelope | close | MW0602 실측 |
|---|---|---|---|
| breakeven | **−0.08** | **−2.53** | −15.25 |
| atr_lock_0.50 | **+6.73** | **+0.24** | −3.76 |
| atr_lock_0.75 | +12.62 | +7.27 | +10.39 |
| bar_range | +7.90 | +1.00 | +10.62 |

**MW0602가 불일치로 지목한 두 변형(`breakeven`·`atr_lock_0.50`)이 정확히 체제
민감도가 가장 큰 것들**이고, 방향도 그쪽 결과와 일치한다(close → breakeven 더 열위,
atr_lock_0.50 우위 소멸). 전부를 설명하지는 못하지만(MW0602 −15.25는 더 극단),
**표본 차이만으로 돌리기 전에 체제 지정을 먼저 확인해야 한다.**
arm 실패도 1건 → 8건으로 늘어 모집단까지 바뀐다.

### 두 PC 공통 생존자 — 0802 대조 규칙 적용

| 변형 | MW0601 | MW0602 | 공통 |
|---|---|---|---|
| **atr_lock_0.75** | +12.62 | +10.39 | ✅ (체제 무관하게도 양수: close에서도 +7.27) |
| bar_range | +7.90 | +10.62 | ✅ (단 close에서 +1.00으로 약함) |
| atr_lock_0.50 | +6.73 | **−3.76** | ❌ 탈락 |

0802 대조 §3 규칙("양쪽에서 공통으로 살아남은 대안만 후보")대로면 후보는 2개이며,
**체제 민감도까지 견디는 것은 `atr_lock_0.75` 하나**다. ⚠ 적용 근거는 아니다 —
표본·유의성은 여전히 미달이고 결정은 주간회의 소관이다.

### 조치 1 — 🔴 전달 누락 2건 (443차·442차가 만든 것)

`summarize()`가 **새 dict를 만들어 반환**하는데 `compute()`에만 키를 넣어,
리포트에 **한 번도 렌더된 적 없는** 줄이 둘 있었다:

- [25] `engine`/`n_qty_gt1`/`n_no_arm` (443차) — v2 전용 제외 2종이 안 보였다.
- [3-B] `engine` (442차) — "재생기 엔진" 줄이 처음부터 안 나왔다.

MW0602가 §7-2에서 v1/v2를 **직접 실행해** 대조한 덕에 드러났다(리포트만 봤으면
어느 엔진 수치인지 알 수 없었다). 이관 채널에 키를 더할 때는 **compute와 summarize
양쪽을 함께 볼 것.**

### 조치 2 — 체제 분포를 리포트에 노출

[25]·[3-B] 상세에 `재생 체제 분포`를 찍고, `close`가 섞이면 🔴 경고를 단다.
MW0601 실측: [25] `envelope 34건`(균일) / **[3-B] `close 46건` + `envelope 64건`(혼재)**.
→ **[3-B]도 이미 체제가 섞여 있었다.** 442차 결론(q_raw 인공물)은 두 PC에서 부호가
같아 견고하지만, 그 채널 역시 체제 민감도 점검 대상이라는 사실이 이번에 드러났다.

### 검증

- py310_64 + **py37_32** 양쪽 컴파일 통과. 리포트 재생성 rc=0, **판정 무변경**
  ([3-B] PASS / [47] PASS / [25]·[25-B] FAIL). (`--out-dir` — EOD 산출물 미훼손)
- 민감도 스크립트는 스크래치패드 전용(저장소 미포함) — 재현 방법은 위 표와
  `tp_trigger` 인자 교체로 충분하다.

**관련**: 443차([25] 이관), 442차([3-B] 이관), 441차([47] 개통), MW0602 0808 점검리포트 §7,
421차 후속2(두 PC 불일치 판독), 0802 PC 대조 §3, 313차, §9.

---

## 2026-08-08 (MW0601 444차 — MW0602 교차검토 수용: CLI 표기 정리 + §47 번호충돌 고지)

**계기**: MW0602가 441·442차를 cherry-pick해 자기 데이터로 교차검토
(`docs/정기점검/매일점검/MW0602-20260808-점검리포트.md`). 그 리포트를 검토하다
**우리 쪽에 남은 미완 2건**을 발견해 조치했다. **라이브 무관·판정 무변경.**

### 🎯 가장 중요한 것 — [3-B] 인공물이 **두 PC 독립 데이터에서 재현**됐다

| PC | q_raw v1 → v2 | 표본 |
|---|---|---|
| MW0601 | +20.22 → **−9.59** | 110건/13일 |
| MW0602 | +8.20 → **−36.66** | 125건/14일 |

**부호가 같은 방향으로 뒤집혔다.** 442차가 "TP1을 넓힐수록 공짜 점수"라고 특정한
지문(개선폭 순위 = TP1 폭 순위)도 MW0602에서 재현됐고, drop-max −41.27·건별우세
9/125로 이상치 착시도 아니다. → **`q_raw` 승격 근거 소멸이 교차확인으로 확정**됐다.
0801 결산의 "감사 §5 유일한 승격 후보"는 두 PC 모두에서 무효다.

⚠ 단, MW0602 리포트 §3의 "TP1 폭(현행 대비) +59.7%/+29.8%/+11.8%"는 **MW0601이
측정한 값**이다(442차 문서에서 인용). 그 PC 표본에서 재측정한 값이 아니므로
`feedback_devmemory_pc_provenance` 관점에서 출처를 병기하는 게 옳다 — 결론에는
영향 없다(순위 자체는 정의상 폭 순서와 같다).

### [47] 게이트 — 두 PC 모두 PASS, 그런데 **잔차 방향이 반대다**

| PC | 편향(중앙) | 격차 | 시뮬 vs 실제 | \|Σ실제\| |
|---|---|---|---|---|
| MW0601 | +0.015R | 0.343SE | +18.85 vs **+30.63** (시뮬<실제) | 0.89SE |
| MW0602 | +0.007R | 0.42SE | **+29.32** vs +13.12 (시뮬>실제) | **0.34SE** |

두 가지가 확인된다.
1. **잔차가 한 방향의 계통 편향이 아니다** — PC마다 부호가 반대다. 441차가 남긴
   "pre 세대 −11.78pt 잔여"를 계통 결함으로 읽으면 안 된다는 뒷받침이다.
2. **441차의 부호기준 조건부화가 옳았다** — MW0602는 |Σ실제|가 **0.34SE**로 MW0601
   (0.89SE)보다 더 무정보다. 총합 부호를 합격 조건으로 강제하면 그 PC에서는
   사실상 동전던지기를 요구하는 셈이었다.
   ⚠ 다만 MW0602는 부호가 우연히 일치해 **구 기준으로도 PASS**였다 — 즉 이번에도
   조건부 경로가 판정을 바꾸지 않았다(`verdict_strict_sign` 동일). 441차 설계 의도대로다.

### 조치 1 — CLI 표기가 리포트와 어긋나 있었다 (실사용 확인됨)

442·443차가 **리포트**는 결말 분포로 바꿨으나 **CLI `main()`은 구 컬럼**이 남아
있었다. v2에서 그 컬럼들은 구조적으로 항상 0이다:

- `quantile_tp_shadow` — `TP1`·`만료` 컬럼 (TP1은 청산이 아니라 보호전환이라 결말이 못 됨)
- `tp1_protect_offset_shadow` — `STOP`·`강제` 컬럼 (보호전환 뒤 털리면 `TP1_ARM_STOP`)

**MW0602가 실제로 그 CLI 출력을 인용하며 컬럼을 수동으로 걸러냈다**(§3 코드블록).
가설이 아니라 실사용에서 드러난 오독 유발 지점이라 둘 다 결말 분포로 통일했다.
[3-B] CLI에는 `재생기 엔진` 줄도 추가(어느 세대 수치인지 출력에서 바로 보이게).

### 조치 2 — 🔴 `[47]` 번호 충돌 고지

**리포트에 `[47]`이 둘 있다.** MW0602 435차가 §47로 *시뮬 재현충실도 게이트*를,
MW0601 436차가 §47로 *진입 맥락별 손익*을 각각 등록했다 — 두 PC가 같은 주에
같은 번호를 쓴 사고이며 `settings.py` 사전등록 주석에도 그대로 있다.
441차가 파생 라벨 `[47-B]`를 붙이면서 혼동 위험이 커졌다(그것은 **게이트 쪽** 파생이다).

→ 양쪽 절 머리에 상호 참조 고지를 넣었다. **번호 재정리는 하지 않았다** — 두 PC와
과거 문서가 공유하는 식별자라 일방 변경이 더 위험하다. 주간회의 안건으로 등록.

### MW0602 리포트에서 유효성이 만료된 항목

§6의 "`[25]/[25-B]`는 여전히 정본 미이관(SUSPENDED) — 자동 해제 금지"는 **443차로
해소됐다**. 443차는 자동 해제가 아니라 **실제 이관 후 해제**이므로 그 경고의 취지
(근거 없는 자동 해제 금지)에 부합한다. MW0602는 443차(`103acfb`)를 pull하면
[25] FAIL·[25-B] FAIL(최선 변형 `tp2_1.25`)로 자기 데이터 교차검토가 가능하다.

### 검증

- py310_64 + **py37_32** 양쪽 컴파일 통과.
- CLI 2종 실행 확인 — 결말 분포가 의미 있는 값을 찍는다
  ([3-B] `STOP=43 TP1_ARM_STOP=34 TP2=33`, [25] `TP1_ARM_STOP=19 TP2=14`).
- 리포트 전체 재생성 rc=0, **판정 무변경**([3-B] PASS / [47] PASS / [25]·[25-B] FAIL).
  번호충돌 고지 2곳 렌더 확인. (`--out-dir` — EOD 산출물 미훼손)

**관련**: 443차([25] 이관), 442차([3-B] 이관), 441차([47] 개통), MW0602 0808 점검리포트,
`feedback_devmemory_pc_provenance`, §9.
...
## 2026-08-05 (MW0601 436차 — [47] 진입 맥락별 손익 채널 신설: 사후 슬라이스 3종의 자기검증)

**지시**: 0805 일일점검이 "사전등록 후보(⚠ 사후 슬라이스 — 확정 금지)"로 올린 관찰
3종을 캠페인 채널로 등록.
**성격**: 검증 채널 신설(오프라인, 라이브 무변경) + **원 보고 수치·해석 2건 정정**.
...
### 등록하면서 원 보고의 문제 4가지가 드러났다

**① 사후 슬라이스 + 다중비교.** 0805에 진입 맥락을 여러 축(재진입 간격 4구간 ·
동일/반대 방향 · 직전 승/패 · 시간대 6구간)으로 갈라본 뒤 **나빠 보이는 것만 골라**
보고했다. 가지가 최소 15갈래인데 개별 수치를 그대로 제시했다.

**② 관측 창을 바꾸면 부호가 뒤집힌다** (포지션합 pt 기준):
...
**③ 지표가 계약수에 오염돼 있었다.** 원 보고는 **포지션합 pt**였는데 이는 계약수가
섞인 값이다(417차 교훈 — 06-10~07-10 대형 계약수 구간이 통째로 들어간다).
계약당으로 정규화하면 **S2는 −0.000(보완 +0.743)으로 효과가 사실상 사라진다.**

**④ 해석 하나가 창 의존이었다.** *"S2는 승률이 평균 이상인데 손실만 크다 — 방향이
아니라 페이오프 문제"*라고 적었으나, 06-10~ 기준 승률은 **51.5% vs 보완 57.3%로
오히려 낮다.** 그 해석은 07-01~ 창에서만 성립한다. 사용자가 이를 "P2와 같은 축"으로
받아 후속 사고를 전개했으므로 **정정이 필요한 오류**다.

### 그리고 등록 즉시 드러난 사실 — 발견된 그 표본에서조차 유의하지 않다

계약당 pt + 순열검정(20,000회) 실측:

```
슬라이스              n   거래일  승률Δ   평균ptΔ  drop-worst  페이오프Δ  p(순열)  음수일%
S1_hour09           21    12  -15.2%   -0.879    -0.424     +0.05   0.3329    50%
S2_reverse_reentry  33    18   -5.8%   -0.743    -0.594     -0.23   0.3308    50%
S3_same_dir_le5min  45    16   -9.9%   -1.169    -0.945     -0.39   0.0849    75%
```

**Bonferroni 보정(α=0.01667) 이전에도 셋 다 유의하지 않다.** 즉 "발견된 그 데이터"
에서조차 우연과 구분되지 않는다. 다만 **평균차 부호는 세 창에서 모두 음수로 일관**
하므로(창 민감도 표 참조) "효과 없음"으로 단정하지도 않는다 — 그래서 폐기가 아니라
등록이 맞다.

### 설계

**1채널 3슬라이스** (3채널 아님). 363차 후속 원칙 — "제안 하나당 새 트랙"이 늘면 §9의
관리 포인트 억제 효과가 역전된다. 더 중요한 건 **한 채널로 묶어야 다중비교 보정이
강제**된다는 점이다(따로 등록하면 각자 α=0.05를 쓰게 된다).

- 주 지표 **계약당 pt** — 사이징 무관(417차). ③의 재발 방지.
- 검정 **순열검정** — 표본이 얇고 꼬리가 두꺼워 정규 근사가 위험하고 scipy 의존도 피한다.
  ⚠ numpy BLAS가 이 PC에서 파손 상태라(432차 후속) 행렬연산 미사용 — 순수 파이썬.
  `perm_seed` 고정: 실행마다 p가 흔들리면 판정이 흔들린다.
- **Bonferroni** α/k. 등록 슬라이스는 3개지만 실제 탐색 가지가 더 많았으므로
  **이 보정은 하한**이며 통과해도 "약한 증거"다 — 리포트에 그대로 표기.
- **방향 축(승률)과 페이오프 축을 분리 계측** — ④의 가설이 어느 축인지 OOS에서
  다시 확인할 수 있게.
- **창 민감도**(3개 창)를 리포트에 상설 노출 — 등록 사유 ②를 매주 눈에 보이게.
- FAIL 조건은 4개 동시 충족: 보정 후 유의 **∧** 평균차 음수 **∧** drop-worst 후에도
  음수 **∧** 일자단위 과반 음수.

### ⚠ FAIL이 나와도 "진입 차단"은 처방이 아니다 (사전등록에 명시)

(a) 427·428차가 방향 축이 무작위와 구분되지 않음을 확정했으므로 진입 필터의 기대값이
보장되지 않는다(430차 [44] 주석과 동일 논리). (b) 감사 §2-2 ③ "차단형 게이트는 이미
충분히 많다". → 처방은 **사이징 또는 집행(청산) 축**에서 찾는다. 특히 S2가 페이오프
축에서만 유의하다면 그건 P2(집행) 안건이다.

**검증**: py_compile 양 환경 rc=0. 스크립트 단독 실행 정상(3슬라이스 산출, OOS 표본
없음으로 INSUFFICIENT). 리포트 `[47]` 구간을 소스에서 추출해 격리 실행 — 예외 없음,
볼드 중첩 0건. 전체 리포트 실행은 432차 후속(BLAS 파손)으로 여전히 미검증.

**Why**: 0805 보고에서 나는 이 관찰들에 "⚠ 사후 슬라이스 — 확정 금지"를 붙여 제시했다.
경고를 붙인 것 자체는 맞았지만 **경고와 함께 제시한 수치가 이미 오염돼 있었다**
(포지션합 pt). 경고문은 "이 수치를 확정으로 쓰지 말라"고만 하지 "이 수치가 다른
지표로는 다르게 나온다"고는 말하지 못한다. **주의 문구는 계측 결함을 대체하지
못한다.**

**How to apply**: 사후 슬라이스를 보고할 때는 경고문을 붙이는 것으로 끝내지 말고
(a) **정규화 지표(계약당)로 한 번 더 계산**하고, (b) **관측 창을 최소 2개** 써서 부호
안정성을 확인한 뒤 제시할 것 — 둘 다 몇 줄이면 되고, 이번에는 둘 다 결과를 바꿨다.
그리고 사후 슬라이스를 채널로 등록할 때는 **슬라이스를 개별 채널로 쪼개지 말 것** —
다중비교 보정이 구조적으로 빠진다.

**관련**: 363차 후속(채널 증식 억제 원칙), 417차(포지션합 vs 계약당 — 같은 오염의
선례), 427·428차(방향 축 무작위 — 차단 처방을 배제하는 근거), 425차·P2(집행 축),
[23]·[46](oos_start 구조), 313차 원칙, §9 사전등록.

---

## 2026-08-05 (MW0601 435차 — P3 운영 위생 3건 + CB⑤ 발동 원인이 SHAP임을 확정)

**지시**: 0805 일일점검 P3(운영 위생) 항목들 구현.
**성격**: 회귀 테스트 신설 + 오탐 제거 + 비판단 경로 스로틀. 매매 판단 로직 무변경.

### [P3-1] 대시보드 위젯 생성 스모크 테스트 — `tests/test_dashboard_smoke.py`

431차 후속2가 남긴 NEXT_TODO 항목의 실제 이행. 431차 툴팁 `%` 포매팅 오류가
`py_compile` rc=0을 통과한 채 배포돼 **08:40 장전 기동이 5회 재시작 전부 실패**,
09:00 개장 시점에 프로세스 0개였다(08:57 수동 복구).

**변이 테스트로 유효성을 확인했다** — 회귀 테스트는 "그 버그를 실제로 잡는가"를
증명하지 않으면 의미가 없다. 431차 버그를 프로덕션 코드에 재주입하고 돌린 결과:

```
py_compile rc=0                                          ← 431차의 함정 재현
test_entry_panel_builds  FAIL
  ValueError: unsupported format character '?' (0xa) at index 169   ← 실제 사고와 바이트 동일
```

되돌린 뒤 3/3 통과 확인. 툴팁에 **미치환 변환지정자**가 남는 역방향 사고도 함께 막는
테스트를 넣었다(`%%`는 정상이므로 제거 후 검사).

### [P3-2] DailyClose EOD 마커 경보 — 구조적 오탐이었다

`daily_close()`는 15:40에 도는데 `retrain_eod.py`는 py310_64 장외 스케줄러가 15:45에
띄워 ~15:48에 마커를 쓴다. **즉 그 시점에 오늘 마커가 없는 것이 정상이고, 검사가
원리상 항상 실패한다.** 실측으로 07-30·07-31·08-03·08-04·08-05 **전 거래일** 이
WARNING이 찍혔고, 그때마다 다음 날 아침 PreRetrain은 `[PreRetrain] EOD 마커 파일 직접
확인 → 스킵`으로 정상 동작했다. **기능은 멀쩡한데 경보만 매일 늑대를 외치고 있었다.**

검사를 지우지 않고 **시각을 인지해 등급을 나눴다**:
- 예정시각(`EOD_RETRAIN_SCHEDULE_HM=1545`) 이전 → INFO(정상)
- 그 이후인데도 마커 없음 → WARNING(진짜 미실행/실패 의심)

마커가 **있는** 경우의 `session_state` 기록은 재기동 등으로 EOD가 먼저 끝난 경우에
여전히 유효하므로 그대로 뒀다. 판정의 정본은 다음 날 아침 PreRetrain의 마커 파일
직접 확인이며 이 경로는 관측용이다.

### [P3-3] CB⑤ 발동 원인이 SHAP임을 확정 — 슬로우 스로틀 도입

0805 점검에서 "CB⑤ 42건 경고 + 12:34 실제 발동(5117ms), 병목 S0·S5"라고 적었는데
**S0·S5는 틀렸다.** `[S6Detail]` 370건을 전수 분해한 실측:

| 구간 | 평균 | p50 | p90 | p99 | 최대 |
|---|---|---|---|---|---|
| **shap** | **268ms** | 332ms | 545ms | 858ms | **4,481ms** |
| meta_gate | 6ms | 4ms | 6ms | 39ms | 194ms |
| checklist_pre | 8ms | 8ms | 12ms | 47ms | 190ms |

**SHAP이 S6 시간의 92.6%**(하루 99.2초/107.1초)다. 그리고 당일 유일한 CB 발동
(12:34:07, total 5,117ms)의 내역은 `shap=4481ms` 단독이다:

```
[S6Detail] ensemble=1ms checklist_pre=11ms meta_gate=194ms gates=0ms
           imp=0ms shap=4481ms corr=4ms dash_ui=0ms tail=1ms
[PipePerf] total=5117ms | S0=3ms S1=69ms S2=133ms S4=73ms S5=138ms S6=4695ms
```

1000ms 초과 20건의 병목 분포도 **S6 14건(70%)**이고, 그중 `[GBM재학습중]` 태그는
**0/20** — 재학습 창과 무관하다. (S0 스파이크 4건은 10:08·11:01·13:14·14:29로
intraday 재학습 완료(10:07·11:00·13:13·14:28)의 **다음 분**과 정확히 일치 —
모델 리로드 비용이며 별건.)

**SHAP은 그 분의 매매 판단에 쓰이지 않는다** — `_refresh_shap_state()` docstring이
"대시보드 표시·주간 심사·DB 관측용"이라 명시한다. **판단에 안 쓰이는 계산이 진입을
멈추게 하는 것은 설계 결함이다.**

402차 후속7 P0-1b가 이미 라운드로빈으로 **평균**을 1/3로 줄였으므로, 남은 **꼬리**를
자른다: 직전 SHAP이 `SHAP_SLOW_MS`(900ms, p90 545ms 위)를 넘으면
`SHAP_SLOW_COOLDOWN_MIN`(5분)간 건너뛴다. **라운드로빈 인덱스를 건너뛴 분에
전진시키지 않으므로 호라이즌은 유실되지 않고 밀릴 뿐**이다(주 단위 dedup이라 심사
표본에도 영향 없음). 일일 리셋에 쿨다운 초기화도 넣었다.

### 하지 않은 것 (2건) — 이유 포함

**① CB⑤ 입력에서 SHAP 시간 제외** — 더 근본적인 해법이다(판단 경로만 계측하면
비판단 계산이 CB를 흔들 수 없다). 그러나 CB⑤는 **CLAUDE.md 절대원칙 §2**의 안전장치
5종 중 하나이고 그 입력 정의를 바꾸는 것은 안전장치의 의미를 바꾸는 일이다.
**사용자 결정 사항으로 남긴다**(NEXT_TODO 435차 등록).

**② 앙상블 보정기 degenerate 조사 — 철회.** 0805 점검에서 "기동 시부터 사실상
무보정(auc=0.491, span=0.00053)"이라고 개선 항목으로 올렸으나, 확인해보니 **430차
[45] 채널이 이미 같은 현상을 다루고 있고** 그 결론이 "근본 문제는 임계가 아니라
**정보 부재**(raw conf 순위 AUC 0.489)이며 **AUC 임계·보정 함수형은 건드리지 말 것**
— 손대면 conf 스케일에 세 번째 불연속이 생긴다"이다. 내 제안이 중복이자 이미
반증된 방향이었다. 철회한다.

**검증**: py_compile 양 환경 rc=0. 신규 스모크 3/3 통과 + **변이 테스트로 431차 버그
검출 확인**. 기존 테스트 회귀 없음(phantom_hardstop·circuit_breaker·meta_gate·
log_isolation OK / entry_qty_invariant는 전 항목 통과하고 마지막 print의 cp949
인코딩만 실패 — `git stash`로 내 변경 없이도 동일 재현, 기존 이슈). SHAP 스로틀
제어흐름은 배포 코드와 동일한 시뮬로 불변식 3종 확인(호라이즌 순서 보존 / 쿨다운
분 수 정확 / OFF 시 종전 동작). **라이브 미검증** — 다음 거래일 관측.

**Why**: P3 5개 항목 중 **2개가 내 오진**이었다 — CB⑤ 병목을 S0·S5로 지목한 것(실제
S6/SHAP)과 보정기 항목(이미 다뤄진 사안). 둘 다 **로그를 요약 수준에서만 보고 적은
것**이 원인이다. `[PipePerf]`의 단계 라벨만 보고 `[S6Detail]`을 안 열었고, 보정기는
기동 로그 한 줄만 보고 캠페인 채널을 확인하지 않았다. 반면 실제로 파고든 SHAP은
**전수 분해로 92.6%라는 확정 수치**가 나왔다.

**How to apply**: (a) 성능 병목을 지목할 때는 **가장 세분화된 계측까지 내려갈 것** —
이 시스템은 `[PipePerf]` → `[S6Detail]` 2단 계측이 이미 있고 상위만 보면 오진한다.
(b) "개선 제안"을 올리기 전에 **기존 캠페인 채널 목록을 먼저 검색**할 것 — 이미
판정났거나 "건드리지 말 것"으로 결론난 사안일 수 있다. (c) 회귀 테스트는 **변이
테스트로 검출력을 증명**할 것 — 통과하는 테스트는 아무것도 보장하지 않는다.

**관련**: 431차 후속2(스모크 테스트 원 지시), 402차 후속7 P0-1b(SHAP 라운드로빈 —
이번 스로틀의 앞단), 430차 [45](보정기 — 철회 근거), 273차(S6Detail 계측 도입),
191차(EOD 스케줄러 py310_64 분리 — 마커 타이밍의 배경), CLAUDE.md 절대원칙 §2(CB⑤).
...
## 2026-08-02 (MW0601 418차 — `--update-features`가 라이브 29거래일을 덮어쓴 것 확정)

**발단**: MW0602 417차 후속(`fa642cd`)이 "MW0601 관찰창 안의 백필 행이 INSERT인가
UPDATE인가"를 확인 항목으로 등록. **결과: UPDATE 확정 — MW0602가 경고한 최악의
시나리오가 MW0601에서는 이미 2026-07-13에 발생해 있었다.**

### 판정 근거 — `created_at`이 결정적이다

MW0602의 판별 스니펫(`feature_quality_score == 0.3` 마커)을 전 구간으로 확장하고
`created_at`을 함께 뽑았다. 혼재일 **15일**(MW0602는 0일), 그리고 `ts ≤ 2026-07-13`인
정규장 행의 `created_at`이 **전부 2026-07-13 한 시점**이었다.

| 날짜 | backfill(0.3) | live | backfill 행 created_at |
|---|---|---|---|
| 2026-06-15 ~ 06-19 | 341~369 | 0 | **2026-07-13** |
| 2026-06-22 ~ 07-13 | 313~379 | 3~15 | **2026-07-13** |
| 2026-07-14 ~ 07-31 | 0 | 355~385 | 해당 거래일 |

317차 NEXT_TODO "[배포 필수] MW0601에서 backfill 재실행" 절차 4번(`--from` 없는
`--update-features`)이 그날 실행된 것이다. 살아남은 3~15행은 전부 `08:45~08:59`
장전 행 — 백필이 정규장 분(09:00~15:20)만 계산하기 때문이다.

**마커 신뢰성 검증**: 라이브 경로가 낸 `feature_quality_score`는 1.0/0.94/0.86/0.82/
0.78/0.74뿐이고 **정확히 0.3은 0건**이다(`feature_builder.py:604`가 페널티 차감식으로
계산해 0.3이 나오려면 페널티 0.7이 필요). 마커는 백필 전용으로 확정.

### 소실 규모 — 백업본 대조로 "원래 라이브"만 분리

`data/db/raw_data.db.bak_20260608_151648`(2026-06-08 백업)과 대조해, "원래부터
백필이었던 날"과 "라이브였는데 덮인 날"을 갈랐다. **이 분리가 없으면 피해가 2배로
과대평가된다.**

- **2026-04-28 ~ 06-01 (22일)** — 백업본에서도 이미 0.3 → **결측일 INSERT, 무해**.
  MW0602가 깨끗하다고 보고한 바로 그 구간과 동일하다.
- **2026-06-02 ~ 07-13 (29거래일 10,448행)** — 백업본에 `live=100%`로 존재 →
  **덮어쓰기 손상 확정**.

손상행은 키가 136 → 115개로 줄었다. **완전 소실 30종**: `opt_chain_pcr`·`opt_gex_bn`·
`opt_atm_pcr`·`vkospi`·`vpin`·`kyle_lambda`·`basis_pt`·`multi_timeframe_5m/15m`·
`micro_regime_code`·`toxicity_spread_stress`·`trend_efficiency`·`hurst_ready` 등.
남은 키도 0.0으로 깔렸다(07-10 vs 07-14: `ofi_pressure` 15/385 → 381/385,
`foreign_futures_net` 0/385 → 370/385).

**복구 가능 1,446행**(06-02·06-04·06-05·06-08, 백업본에 온전, `ts` 100% 일치).
**복구 불가 9,002행 / 25거래일**(06-09 ~ 07-13, 백업 이후 구간).

### 주 재학습 경로는 무사 — 과잉대응 방지

`raw_features_horizon`은 **손상되지 않았다**. 백필은 `raw_features`만 UPDATE한다
(`backfill_features.py:434`). EOD 재학습은 `EOD_RETRAIN.bat` → `eod_retrain.py
--phase2` → `_retrain_phase2()`로 이 테이블만 읽는다(07-10 3m 실측: `opt_chain_pcr`
123/123, `vkospi` 123/123 정상). `ScalerWarmup`(최근 500행)·`_shap_feature_window`
(240)·`_param_corr_history`(120)도 07-14 이후 무손상 구간만 본다.

### 실제로 새는 곳 3건

**① 섀도우 TB 1m이 손상 데이터로 학습됨 (검증 캠페인 [1] 채널 오염)**
1m은 설계상 `raw_features_horizon`에 없어 `batch_retrainer.py:1624`가 `raw_features`로
폴백한다. `gbm_1m_shadow_tb.pkl`이 **07-31 15:48**(손상 이후) 생성이고
`RETRAIN_WEEKS_BACK=26`이라 학습창 대부분이 0.0 구간이다. 나머지 5개 호라이즌은
07-24 생성 + horizon 테이블 사용이라 무관.

**② 피처셋 건강도 리포트가 체계적으로 왜곡됨 — 이번 발견의 단서였던 그 리포트**
20거래일 관찰창(07-03~07-31, 7,539행) 중 **7일 2,620행(34.7%)이 손상행**.
§2-b의 `bear_reversal_signal`·`macro_vix_abs`·`microprice`·`feature_recoverable_errors`
"DEAD 100%"는 죽은 피처가 아니라 **백필 전용 키가 손상행에 심어진 것**이다
(수정 후 재생성에서 4건 전부 소멸 — 원 증상 해소 확인).

§4 후보 현황판은 **축적일수가 부풀려져** 있었다. 백필 행이 `FEATURE_KEYS_ALL`의
모든 키를 0.0으로 채우는데, presence 집계가 "값이 None이 아니면 그날 관측됨"으로
세기 때문에 **한 번도 실제로 수집되지 않은 날까지 축적일로 계상**됐다. 수정 후:
`foreign_futures_net` 60일 → **17일**, `cvd_direction` 60일 → **32일**,
`imbalance_slope`·`microprice_slope`·`mlofi_norm` 60일 → **32일**.
즉 실제보다 **검증 준비가 된 것처럼 보이고 있었다**.

> ⚠ 조사 중간에 세웠던 "축적이 정상인데 13/20일로 **승격이 부당하게 지연**된다"는
> 가설은 **틀렸다 — 방향이 반대였다.** `kyle_lambda`·`basis_pt`·`multi_timeframe_*`의
> "13일"은 손상과 무관하게 정확한 값이다: 이 피처들은 **2026-07-14부터 수집이 시작된
> 신규 피처**이고, 07-03~07-13에 살아남은 라이브 행은 장전(08:45~08:59) 행이라
> 키가 78~82개뿐이어서 애초에 이 피처들을 담고 있지 않았다(07-14 행은 88키).
> 13이 무손상 라이브 일수와 우연히 같아 생긴 오독이었다.

**③ RegimeFingerprint PSI 워밍업 — 거의 자연 치유됨**
`main.py:1310`이 `limit=_LIVE_WIN_MINS=5000`으로 읽는데 무손상 행이 4,919행이라
현재 손상행은 81행(1.6%)뿐. 다만 **07-14~07-25에는 워밍업 버퍼가 CORE 피처 0.0으로
대량 오염**돼 있었다 — 371차 PSI 재설계의 "라이브 관찰 후 판단" 근거가 그만큼 흐리다.

### 결정 (사용자 승인 2026-08-02)

1. **복구 불가 25거래일은 `raw_features` 소비처에서 제외한다.** 0.0 상수 구간은
   "값 없음"이 아니라 **"0이라는 잘못된 정보"** 라 학습에 능동적으로 해롭다.
2. **재발 방지(Phase 1)를 복구(Phase 2)보다 먼저 한다.** 복구 작업 중 같은 명령을
   다시 밟는 것을 먼저 막는 게 순서상 맞다.

### Phase 4 재평가 — 317차 백필 지시는 목적 달성이 불가능했다

사고를 부른 그 지시가 **애초에 얻을 것이 없었다.** 목적은 "hurst 재보정을 재학습
데이터에 소급 반영"인데:

- `hurst`가 실제 배포 학습 피처인 곳은 **10m 하나뿐**이다
  (`model/horizons/feature_names_10m.pkl`에 존재, 1m(8개)·3m(12개)에는 없음).
- **10m은 `raw_features_horizon`에서 학습한다** — EOD가 `--phase2`로 돈다.
- 그런데 `backfill_features.py`는 **`raw_features`만 UPDATE하고
  `raw_features_horizon`은 건드리지 않는다.**

→ 백필이 완벽히 성공해도 10m 학습 입력의 hurst는 바뀌지 않는다. 남은 소비처도
실질이 없다: 전역 `feature_names.pkl`(97개, hurst 포함)을 쓰는 1m Phase-1 경로는
`--phase2`라 재학습되지 않고, ScalerWarmup은 최근 500행만 읽어 항상 무손상이다.

**결론: 지시 폐기 권고.** 이득 0, 실증된 위험 1건(10,448행 손상). 굳이 소급하려면
대상은 `raw_features_horizon`이고 별도 설계가 필요하다. NEXT_TODO 317차 항목에
이 판단을 붙였다(지우지 않고 근거와 함께 남김).

### 조치 결과 (Phase 1~3)

| Phase | 내용 | 결과 |
|---|---|---|
| 1 | `backfill_features.py` 안전장치 3종 | 라이브행 스캔 차단(exit 2)·병합 갱신·자동 백업. 복제본 E2E 검증에서 **백필 계산 키 밖 변경 0건**, `opt_chain_pcr`·`vkospi`·`vpin`·`kyle_lambda` 전부 보존 |
| 2 | 백업본에서 부분 복구 | **1,446행 복구 완료**(06-02·04·05·08). 06-08 잔여 18행은 백업 시각(15:16:48) 이후라 대상 밖. 9,002행은 복구 불가 확정 |
| 3 | 오염 산출물 정리 | 섀도우 TB 1m 폴백에서 백필행 제외 → 가용 6,522행 < MIN 15,000이라 **당분간 스킵**(약 22거래일 후 자동 재개). 오염된 pkl 3종은 `quarantine_418/`로 격리. 건강도 리포트 기본 제외 + 헤더 고지 → **§2-b 유령 DEAD 4건 소멸 확인** |

Phase 1 검증에서 실값이 0.0이 된 6건(`in_value_area` 5, `poc_above` 1)은 백필이
정당하게 재계산하는 볼륨프로파일 이진 플래그로, 데이터 소실이 아니다. 다만 백필과
라이브의 VP 계산이 385행 중 6행에서 갈린다는 **기존 불일치**를 드러낸 것이므로
별도 관찰 대상으로 남긴다(이번 범위 밖).

### ⚠ `raw_data.db.bak_20260608_151648`은 삭제 금지

1,446행의 **유일한 복구원**이다. gitignore 대상이라 다른 PC에도 사본이 없다.
정리 대상으로 보이더라도 지우지 말 것.
