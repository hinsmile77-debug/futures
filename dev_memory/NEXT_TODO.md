...
### ★ 사용자 결정 대기 (레지스트리가 대신 정할 수 없는 것)

- [ ] **1m 레지스트리 갭 2종을 "명시적 제외"로 확정할 것인가** —
      `micro_regime_code`·`queue_directional_depletion`. 07-30 실행계획 항목 F가
      *"1m은 331차 후속2 영구 퇴역 호라이즌이므로 개선 대상이 아니다. '안 하기로
      했다'를 기록해 재론을 막는다"*고 **권고**했으나 결정은 아직 없다(그 계획의
      0단계 완료 조건 4에 미결로 남아 있다). 권고를 결정으로 승격시키는 것은
      레지스트리의 권한 밖이라 411차는 등록하지 않았다.
      → 확정 시 `FEATURE_SET_DECISIONS`에 `suppress=True`로 추가하면 재고에서 빠진다.
      현재 둘 다 "검증가능(28일)"로 후보 재고에 계속 잡힌다.
