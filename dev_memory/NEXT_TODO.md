...
- [ ] 🆕 **O-3(491차) (매 장중)** 개장 후 최초 `D_FORCE`/`B_OPEN` 축퇴 개수.
  08-24 09:02 = **0**(해소). 풀리지 않는 날이 나오면 **P1 승격**.
...
- [ ] **F-1(491차) P1** — 스킬 문서 3곳의 **죽은 CB② 기한 「2026-08-29」** 교체 (1-3)
...
- [ ] **F-5(491차) P1** — `sizing_trace`에 `margin_cap_qty`·`margin_binding` + **불변식 테스트** (1-8)
...
- [ ] **F-4(491차) P1** — 장중 재학습 조건 **도달 가능성** 매일 계측 (1-7)
  - 전일 `[DriftRetrain]` **369/369 `cond=False`** · **max(n)=4**(요구 15) · `acc5m` 369/369 `0.0%`
  - ⚠ **임계 변경 금지**(사전등록 ④). 계측만
  - 소요 20분
- [ ] **F-7(492차) P1 🆕** — **SGD 학습률 조절기의 「상한 포화」를 매일 보이게** (1-13)
...
- [ ] **F-6(491차) P2** — `O-3(491차)` 판정 기준 「최초 1점」 → 「개장 후 전 구간」 (1-10). 소요 10분
- [ ] **F-8(492차) P2 🆕** — EOD 두 DB 정리 경로 구분 (1-14)
  ...
  - 소요 15분. 검증 `O-13(492차)`
- [ ] **F-2·F-3(491차) P2** — 리포 루트 산출물 2건 정리 + 26주 WFA 산출물 커밋 (1-6, **5건 → 7건**)

**적용 순서**: `F-1 → F-5 → F-4 → F-7 → F-6 → F-8 → F-2·F-3`
