...
**사용자 조작 필요 (이 기록이 MW0601 전달의 유일한 경로 — 예약은 git 공유 안 됨)**

- [ ] **A1 장전 예약 08:50 → 09:02** — 0814에 08:50 실행이라 08:55 매크로·PreRetrain·
      **O-1의 F-1 해제 로그**가 다이제스트에 하나도 안 잡혀 사람이 원본 로그로 메웠다.
      09:02면 08:58 레짐 확정 + 09:00 첫 파이프라인 1사이클까지 포함된다.
      ⚠ claude.ai 루틴이라 세션에서 변경 불가. **MW0601에도 따로 등록할 것**
- [ ] **A2 장후 예약을 EOD 완료 발화로** (차선: 16:10 고정) — 0814 장후 점검이 15:51에
      돌았는데 EOD 체인은 16:02:32에 끝나 캠페인 판정 리포트(165KB)를 1차가 못 열었고
      점검이 1·2차로 쪼개졌다. `data/eod_retrain_done_{d}.txt` + 캠페인 요약 라인 폴링
      (17:00 타임아웃). ⚠ **MW0601에도 따로 등록할 것**
