...
- [ ] **다음 장중 실제 하드스톱 발동 시 라이브 검증(최우선)** — (1) `[TickStop-S0C]`
  로그 직후 `PendingOrder set {kind: EXIT_FULL}`이 찍히는지, (2) 이후 실체결이
  `pending_matched=True`로 정상 매칭돼 "미추적체결(pending_miss)" 로그가 더 이상
  발생하지 않는지, (3) `[청산 완료]` PnL이 실제 체결가 기준으로 정상 기록되는지,
  (4) 유령 포지션 재발 없이 브로커 잔고와 내부 상태가 항상 일치하는지 확인.
