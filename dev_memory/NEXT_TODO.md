...
- [ ] **entry_horizon(ATR 기반) 선택 로직에 호라이즌 스킬 정보 반영 검토** — 최초
  ...
  분석에서 5m 자체가 conf≥0.55에서 유의하게 역행(-1.93pt, t=-3.25)하는 무스킬
  호라이즌으로 확정됐으므로, `select_entry_horizon()`(변동성 크기만으로 1m/3m/5m
  선택, 스킬 무관)이 5m을 선택했을 때 TP1 폭(ATR×0.7, 최대)이 여전히 적절한지는
  별도 검토 가치 있음.
