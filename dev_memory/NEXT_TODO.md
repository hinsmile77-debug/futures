...
- [ ] **P0-2 가드가 MW0602 케이스를 잡는지 미확인** — MW0601 저장본은 AUC 0.491로
      확실히 걸리지만, MW0602(coef 0.0636 / span 1.3pp)는 AUC를 측정하지 않아 모른다.
      **MW0602에서 P0-1을 켜기 전에 반드시 `[Calibration] 보정기 복원 완료` 로그의
      `auc=` 값을 먼저 확인할 것** — 0.53 이상이면 가드가 통과시켜 09:00부터 봉쇄된다.
