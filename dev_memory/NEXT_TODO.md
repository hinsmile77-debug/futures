...
> 🔴 Phase 2 폴백은 **1m 을 학습하지 않는다**(`raw_features_horizon` 에 1m 행이
> 사상 0개 — 설계상 기록되지 않는다). 폴백이 걸린 날마다 1m 이 하루씩 늙는다.

- [ ] **F2-1 (2026-09-15 장후)** EOD 로그에 `[EODFallback]` 이 또 떴는가.
      떴으면 1m 은 그날도 안 늙었는지 확인 — `model/horizons/gbm_1m_meta.json`
      의 `trained_at`. (09-14 시점 값 = `2026-09-11 15:50:48`)
- [ ] **F2-2 (2026-09-16~17)** 예측대로 Phase 1 이 15,000 을 넘어 **1m 이 교체**되는가.
      산술: 부족 445행 ÷ 순증 약 338행/거래일 ≈ **1.3 거래일**.
      ⚠ 안 넘으면 산술 전제(순증 338)를 먼저 재측정할 것 — **임계를 내리지 말 것**(458차 D6).
- [x] **F2-3ⓑ** ✅ **566차 구현 완료** — 폴백 시 미교체 호라이즌 + 1m 구조적 불가 사유
      + 현행 모델 나이를 `[EODFallback]` 경고로 승격. 가드 `test_566_eod_fallback_skip_visibility.py`.
      - [ ] **F2-3ⓑ-확인 (2026-09-15 장후)** 폴백이 또 걸렸다면 그 경고 2줄이 실제로
            EOD 로그에 찍혔는가. 안 찍혔으면 배선이 아니라 **조건**을 잘못 잡은 것이다.
- [ ] **F2-3ⓐ (P2)** 3거래일 넘게 안 풀리면 그때 안건화 — 1m 만 `raw_features` 로
      학습하는 전용 경로를 폴백에 추가. ⓑ 가 먼저였던 이유는 **조용히 늙는 것이
      늙는 것보다 나쁘기** 때문이다. ⓐ 는 학습 경로 신설이라 비용이 다르다.

### 🟡 566-F5 — 백필 필터 범위와 418차 결정문의 3.1배 괴리 (미판정)

근거: `dev_memory/DECISION_LOG.md` 2026-09-14(566차 후속 ③).

    실제 배선 27,718행 / 78 거래일   vs   418차 결정문 9,002행 / 25 거래일

- [ ] **F5-1** 418차 결정 1 의 의도가 **「오염 25일만」** 인가 **「백필 전량」** 인가를
      원 기록(418차 항목)에서 확정한다. 코드를 먼저 고치지 말 것.
- [ ] **F5-2** 「백필 전량」이 의도면 **문서를 배선에 맞춘다**(코드 변경 없음).
      「25일만」이 의도면 그때 비로소 필터 범위가 안건이 된다.
- [ ] **F5-3** 어느 쪽이든 **Phase 1 게이트에 미치는 영향**을 함께 적는다 —
      이 필터가 15,000 임계를 구속하는 유일한 변수다(566차 후속 ② 산술).
⚠ **좁히는 방향으로 먼저 가지 말 것.** 백필 행은 라이브 미시구조가 없어 학습에서
  빼는 쪽이 맞을 가능성이 높다. 317차 FalseBlock·539차 ATR 교훈과 같은 순서다.


- [x] **F2-4** `old_acc` 정체 값 — ✅ **조사 종결, 결함 아님**(566차 후속 ①).
      `acc.txt` 는 모델 교체 시에만 쓰이고 장중 재학습은 일부러 안 쓴다.
      재론 시 `acc.txt` 가 아니라 사이드카 `gbm_{hz}_meta.json` 을 볼 것.

### 🎫 T-BOOK-1 인수 판정 (2026-09-15 장후) — 566차 구현 완료, **판정 미완**

구현은 끝났고 **실측이 남았다.** `python scripts/defect3_collection_check.py`
한 번으로 세 항목이 함께 나온다.

- [ ] **566-1 (다음 거래일 장후)** `book_*_max 적재율` ≥ 98% (인수조건 1)
- [ ] **566-2** `불변식 tot<=max` 위반 **0** — 위반이면 max 누적이 틀렸다 (인수조건 2)
- [ ] **566-3** `불변식 avg<=max` 위반 **0** (인수조건 3)
- [ ] **566-4** 기존 3거래일(09-10~09-14)은 `max` **NULL** 유지 — 점검기가 그날을
      FAIL 이 아니라 `n/a(566차 배포 이전)` 로 찍는지 함께 확인 (인수조건 4)

🔴 **566-0 (선행) — 내일 장 시작 전에 본체가 반드시 재기동돼야 한다.**
현재 프로세스(PID 9296)는 **2026-09-14 17:32:03 기동**이라 566차 코드를 모른다.
그대로 살아 넘어가면 `init_raw_data_db()` 의 ALTER 가 안 돌아 **컬럼 자체가 없고**,
있더라도 구 `_book_depth_cols`(5열)를 쓴다 ⇒ `_max` 전량 NULL, 566-1 자동 실패.
평소 런처가 매일 기동하므로 정상 경로지만, **인수 판정 전에 기동 시각을 먼저 볼 것**
(`logs/YYYYMMDD_SYSTEM.log` 의 `[Runtime]` 줄).

- [ ] **566-5 (T-BOOK-1d)** `_tot` 실제 제거 — **아직 아니다.** 552~566차 구간
      (2026-09-10~09-14)이 분석에서 빠진 뒤에 한다. 그전에 지우면 그 구간의
      봉내 극단값을 볼 수단이 사라진다.
- [ ] **566-6** `scripts/defect3_collection_check.py` 의 「5단 ÷ 1단 잔량비」는
      **의도적으로 `_tot`** 이다(`bid_qty` 와 같은 스냅샷이라야 불변식이 성립).
      T-BOOK-1d 착수 시 이 항목의 `bid_qty` 쪽 대응을 **먼저** 정할 것.
- [ ] **566-7 (MW0602)** `dev` 에도 동일 적용 — 계측 설계 문제라 데이터와 무관한
      공통 결함이다.

### 564차 — log_manager 지연 포매팅 (2026-09-14) — 🔴 **회귀 수정**

근거: `DECISION_LOG.md` 2026-09-14(564차). 가드 `tests/test_564_log_manager_lazy_format.py`.

- [ ] **564-1 (다음 거래일)** `[LogFmt]` 경고가 **뜨지 않아야** 정상이다. 뜨면 아직
      남은 오용 호출부가 있다는 뜻 — 경고에 찍힌 `msg=` 로 위치를 찾을 것.
- [ ] **564-2** `[ERR-FATAL] minute_pipeline` 이 재발하지 않는지. 오늘 1건(14:25).
- [ ] **564-3 (P2)** 같은 계열 점검 — `log_manager` 외에 「logger 처럼 생겼지만
      아닌」 래퍼가 더 있는지. 5/22·564차가 같은 함정을 두 번 밟았다.

### 563차 후속 — EOD 폴백 구현 후 확인 (2026-09-14)

근거: `DECISION_LOG.md` 2026-09-14(563차 후속). 가드 `tests/test_563_eod_fallback_and_marker.py`.

- [ ] **563-F1 (다음 EOD)** 폴백이 자동으로 도는가 — `[EODFallback]` 로그와
      마커의 `phase2_fallback: true` 확인. Phase 1 풀이 15,000 을 넘기면 폴백 없이
      정상 경로로 돌아간다(둘 다 정상).
- [ ] **563-F2 (P1) 1m 모델이 갱신되지 않는다** — `raw_features_horizon` 에 1m 행이
      0개라 Phase 2 가 건너뛴다. 1m 은 설계상 `raw_features` 폴백인데 Phase 2 경로엔
      그 폴백이 없다. 현재 1m 모델은 **2026-09-11자**. 폴백이 상시화되면 1m 만
      계속 낡는다 — Phase 2 에 1m 폴백을 넣을지 결정 필요.
- [ ] **563-F3 (P2) 정확도 추세는 다음 EOD 부터** — 오늘 0.36→0.54 는 Phase 1↔2
      원천 차이라 직접 비교 불가(461차 분모 불일치와 같은 유형).
- [ ] **563-F4** 내 559차 기록의 「복구 불가 25거래일(9,002행)」을 **재인용하지 말 것** —
      563차 실측은 79거래일 28,099행이다. 마커가 사고 오염행과 일반 백필행을 구분하지
      않는 것으로 보인다(563차 F-1 과제와 연결).

### 🎫 T-BOOK-1 — 호가깊이 **추정량 정리** (사용자 제안, 2026-09-14) — 🔴 **장 마감 후 착수**

> 사용자 제안 원문: 「`book_*_tot` 2종을 스키마에서 폐기하거나 컬럼명에 `_last_snapshot`
> 을 박는 게 좋겠다. 이름이 「합계」로 읽혀 오용을 부른다 — 주석에 경고가 있었는데도
> 내가 그대로 밟았으니, 다음 사람도 밟는다.」
>
> 🔴 **이 제안의 근거는 추정이 아니라 실증이다.** 552차 원래 주석은 이미
> `-- 봉 마지막 스냅샷 5단 매수잔량 합` 이라고 **정확히** 적고 있었다. 그런데도
> 2026-09-14 에 오독이 발생했다. **이름이 주석을 이긴다.**
> 그리고 561차가 한 조치(주석 3곳 보강)는 **같은 실패를 더 큰 목소리로 반복한 것**이다.
>
> 같은 계열 선례: `cvd_divergence`(다이버전스 아님, 500차) · `mdd_pct`(분모 2종, 461차) ·
> `regular_candles.cum_sell`(실제로는 누적 **매수**, 559차).

**범위는 「개명」이 아니라 「추정량 정리」로 잡는다.**
`_last_snapshot` 은 *언제 찍혔는지*만 알려주고 *대표성이 없다*는 건 여전히 안 알려준다.
봉별 last 를 여러 봉에 걸쳐 평균 내면 통계적으로 틀리지도 않아서 **더 헷갈린다**.
의미 없는 표본을 **의미 있는 통계**로 바꾸는 것이 근본 해법이다.

**이행 순서 — 461차 선례를 따른다**(옛 이름을 즉시 지우지 않고 강등 → 소비 0 → 제거)

- [x] **T-BOOK-1a** ✅ 566차 구현 완료 — `book_bid_max` / `book_ask_max` 신설
      · `_handle_hoga` 에 `max` 누적 한 줄씩 (이미 `_sum` 을 누적 중이라 같은 자리)
      · `_book_depth_cols` 반환 확장 · DDL + ALTER · **양쪽 writer**(가드가 강제)
      · `session_bars` 도 동일
      ⚠ **min 은 넣지 않는다** — 실측상 5단 합의 하한이 5에 붙어 있어(전 스냅샷 min=5,
        중앙 8) 정보가 거의 없다. 범위를 넓히지 말 것.
- [x] **T-BOOK-1b** ✅ 566차 — `_tot` 을 **폐기 예정**으로 강등 — 스키마 주석에 명시, 신규 소비 금지.
      판정 경로에서는 561차가 이미 뺐다(`book_depth_duplication_check` → `_avg`).
- [x] **T-BOOK-1c** ✅ 566차 — 소비처 이동 — `defect3_collection_check` 의 「tot 대표성」을
      `max` 기반으로 바꾸고, 분석 스크립트 2종(`whipsaw/extract_ws.py`,
      `whipsaw/rebuild_panel.py`)의 `book_bid_tot` 조회를 `_avg`/`_max` 로.
- [ ] **T-BOOK-1d (소비 0 확인 후)** `_tot` 2종 제거. **← 유일한 미이행 항목**(565-5). 그때까지는 남긴다 —
      지금 지우면 봉내 극단값을 볼 수단이 사라진다(`_avg` 는 뭉갠다).

**인수 조건 (다음 거래일 장후)** — 판정 절차는 위 「T-BOOK-1 인수 판정」 항목 참조.
⚠ `scripts/defect3_collection_check.py` 가 1~3 을 자동으로 찍는다(566차 신설).
  1. `book_bid_max` 적재율 ≥ 98% (양 테이블)
  2. 불변식 `book_bid_tot <= book_bid_max` **위반 0** ← max 누적이 맞는지 직접 검증
  3. 불변식 `book_bid_avg <= book_bid_max` 위반 0
  4. 기존 3거래일은 `max` **NULL** — 0 으로 채우지 말 것(계측 4원칙 ②)

**위험·주의**
  · 🔴 **쓰기 경로 변경이라 장중 금지**(`project_v9_dev_branch_split` 규약).
  · writer 누락은 `tests/test_552_book_depth_and_gp_range.py::
    test_every_raw_candles_writer_includes_book_columns` 가 잡는다 — 552차 결함1
    (writer 둘 중 하나만 패치 → 영구 NULL)에서 나온 가드다. **신규 열도 그 목록에 넣을 것.**
  · MW0602(`dev`)에도 **같이** 적용한다 — 계측 설계 문제라 데이터와 무관하게 공통이다.
  · 참조처 실측: 코드 8곳 + 문서 다수(`grep -rn book_bid_tot`).
...
- [ ] **561-1 (장 마감 후 · 미륵이 본체 종료 상태에서)**
      `python scripts/probe_futurejpbid_fields.py --snaps 20`
      — `FutureJpBid` 12~18·29~ 대역에 **총잔량·건수**가 있는지 본다.
      있으면 우리 5단 합을 총잔량과 **직접 대사**해 단위 검증이 완결된다.
      동시에 「5단 너머에 얼마나 더 있는가」도 알 수 있다.
      🔴 장중 금지 — 새 COM 구독이고 본체가 같은 객체를 구독 중이라 간섭 위험.
      ⚠ 장 마감 후엔 호가가 안 움직여 스냅샷 0개일 수 있다 → **15:35 직후**를 노릴 것.
- [ ] **561-2** 프로브 결과가 나오면 딥다이브 §1 「남은 미확인」을 닫고,
      총잔량이 5단 합보다 크면 **5단 절단이 얼마나 정보를 버리는지** 별도 평가.
- [ ] **561-3 (Phase 3-0 적립 대기)** 20거래일 도달 시 판정. 현재 3/20.
      ⚠ 이제 판정 입력은 `_avg` 다. `_tot` 으로 재서 「독립」이 나오면 그건 잡음이다
      (561차 실측: tot +0.178 vs avg +0.902).
- [ ] **561-4 (파생 설계 시)** 신규 축은 **절대 깊이만** 후보다
      (`book_bid_avg + book_ask_avg`, `depth_bias` 와 rho=+0.068 독립).
      불균형 축은 만들지 말 것 — 326차가 이미 기각했고 561차가 재확인했다.
...
## 2026-09-13 (MW0601 559차 — 데이터 결함 3건 구현) — **매매 정책 무변경**

> 근거: `DECISION_LOG.md` 2026-09-13(559차),
> `docs/Spec for feature/Feature integrity/데이터결함3건_상태파악_및_수집구현계획_MW0601-20260913.md`

### 559차 후속3 (2026-09-14) — 장중 재학습 회귀 수정 후 확인

- [ ] **559-10** 다음 장중 재학습이 **성공**하는가 — `logs/retrain_intraday_*.log` 에
      `재학습 완료` 가 찍히는가. `학습 데이터 부족 (... < 15000)` 이 다시 보이면 수정이
      안 먹은 것이다. ⚠ 이벤트 구동(ConstOut·Drift)이라 안 도는 날도 정상이다.
- [ ] **559-11** **오늘 밤 EOD 가 559차 변경 후 첫 EOD** 다 — Phase 2 경로 성공 여부와
      `[UnitMismatch]` 제외 행수(>0 이어야 정상)를 함께 볼 것.
- [ ] **559-12** 라이브 행이 쌓이면 Phase 1 풀도 15,000 을 넘는다(약 385행/거래일,
      2026-09-14 기준 778행 부족). 넘은 뒤에도 **장중 경로는 4,800 기준을 그대로 쓴다** —
      이 수정은 되돌릴 필요가 없다.

### 🔴 다음 거래일 장후 — **결함 3건이 정상 수집되는가** (최우선)

> 여기서 안 보면 계측이 죽은 채로 굳는다. 이 프로젝트가 반복해서 밟은 함정이
> 「만들어 놓고 안 돌린 계측」이다(FP-CRITICAL 2개월 · TOX 섀도 1개월 · 552차 결함1).

- [ ] **559-1 (한 줄로 끝난다)** `python scripts/defect3_collection_check.py`
      — 세 결함의 수집 정상성을 한 번에 판정한다. **FAIL 0 이어야 통과.**
      ⚠ 장 마감 후에만(456차). 종료코드 1 = FAIL 있음 · 2 = 장중 차단.

      이 스크립트가 보는 것(손으로 볼 거면 이 목록을 그대로 볼 것):

      | 결함 | 항목 | 정상 |
      |---|---|---|
      | ① | 앵커·섀도 적재율 | ≥98% (지금 100%) |
      | ① | **미분류 `unk_vol` 적재율** | ≥98% — 559차 신설, **첫 적재일이다** |
      | ① | 항등식 I4 `flag_buy+flag_sell+unk == volume` | 위반 0 |
      | ① | 앵커 매수비중 | 45~55% |
      | ① | legacy 매수비중 | ≥55% — **편향이 남아 있어야 정상**(전환 전) |
      | ① | 섀도 vs 앵커 괴리 | ≤2%p |
      | ② | 값이 상수가 아닌가 | distinct > 1 |
      | ② | 단위 정상 | `|max|` ≤ 10 |
      | ② | **`*_measured` 플래그** | 프리장 0.0 → 정규장 1.0 (559차 신설) |
      | ② | TR 지원율 | 정규장 > 50% |
      | ③ | `book_*` 적재율 | ≥98% (양 테이블) |
      | ③ | 5단 ÷ 1단 잔량비 | ≥1.05 (지금 7.37) |
      | ③ | 적립 진척 | 2/60 거래일 — 끊기면 여기서 보인다 |
...
- [ ] **559-2** EOD 로그에 `[FeatureEpoch]` · `[InvestorMeasured]` · `[UnitMismatch]` ·
      `[BackfillFilter]` 4종이 찍히는가. 특히 `[UnitMismatch]` 는 **제외 행수 > 0**
      이어야 정상이다(4거래일이 26주 창 안에 있다). 0 이면 필터가 배선에서 빠진 것이다.
      ⚠ MW0602(dev)는 **반대로 0 이 정상**이다 — 별개 DB다.

- [ ] **559-3** N분봉 회귀 확인 — `bar_aggregator` 가 섀도·앵커·미분류를 올리도록
...
- [ ] **559-4 (Phase 3 선행)** 정규화 대안 확정 — 실측상 **V3(창 진폭)** 가 유일하게
      포화 0.0%. V1(현재값 제외)은 **효과 0 으로 탈락**했다. V2(σ 기반)는 k=3·클립 때문에
      불리하게 나왔으므로 재보정 후 재비교할 것. 채택은 **Phase 3 승인 사항**.
- [ ] **559-5 (P1-2 재개 조건)** 섀도 **60거래일**(≈2026-11 중순) 또는 Phase 3 승인 시
      legacy vs 섀도 A/B. 기존 `validate_feature_set_purged_cv.py` 로는 안 된다
      (피처셋 OLD/NEW 비교 도구다) — **전용 하네스** 필요: 같은 봉을 두 원천으로 재계산 →
      동일 라벨·동일 분할. ⚠ 24거래일로 돌려서 판정하지 말 것(458차 D6).
- [ ] **559-6 (주간회의 안건)** 정규 10100 체결 방향 정답지(533거래일)를 미니 편향 구간의
      **대체 신호**로 쓸 것인가. 선행: `scripts/regular_flow_field_verify.py` 검증
      거래일 20일 누적(현재 **1일**) + 미니·정규 분봉 매수비중 상관 측정.
- [ ] **559-7** `FutOptChart` **필드11** 정체 규명 — 하루 220회 하락하므로 누적량이 아니다.
      `cybosplus.github.io` 필드표로 확인 전까지 **사용 금지**(컬럼명 `cum_buy` 는 오기).
- [ ] **559-9 (P1-7 · Phase 3 전환 후 20거래일)** 우회책 3건 재검토 — **원인이 사라지면
      근거도 사라진다.** 전환 전에는 건드리지 말 것(지금 되돌리면 편향이 그대로인 채로
      방어만 없어진다).
      ⓐ 2026-06-25 체크리스트 CORE 키 교체 `cvd_divergence` → `cvd_delta_norm`
        (`CORE_FEATURES_BY_GROUP["short"]`) — 편향 때문에 바꾼 것이므로 원복 검토 대상.
      ⓑ 394차 detrend (`cvd_exhaustion`) — 단조증가 상쇄용으로 만든 축.
      ⓒ AutoMask 면제 (`_CORE_MASK_EXEMPT_BY_HZ`) — 상수 고착 방어용.
      판정 기준: 전환 후 20거래일 `cvd_*` 분포(포화율·음수비)가 정상 구간이면 ⓐ~ⓒ 순으로
      하나씩 되돌리고 각 1주 관찰. 한꺼번에 되돌리지 말 것.

- [ ] **559-8 (별건, 이번 범위 밖)** `batch_retrainer` 의 `rec[1].get(f, 0.0)` —
      학습 X 구성에서 없는 키를 **0.0 으로 채운다**. 수집 계층에서 금지한 바로 그 패턴이
      학습 계층에 남아 있다(451차 계열). 고치면 NaN 처리·스케일러 영향이 커서 별도 안건.

### 완료

- [x] **P0-1** 필드10 라벨 정정 + `regular_flow_1m` 뷰 + `--status` 단조성 검사 + 검증기
- [x] **P0-2** 역변환 가드 3곳 배선 (`inv_unit_guard.py`)
- [x] **P0-3** 552-6 잔여 확인 — ONESIDE **0건** · 5단/1단 비 5.72·7.37 · `book_snaps=0` **0봉**
      → **552-6 종결**
- [x] **P0-4** CLAUDE.md 「98.6%」 3지표 대조표
- [x] **P1-1** 포화 분해 실측 (`cvd_saturation_probe.py`)
- [x] **P1-3** 전환 배선 (`CVD_FLOW_SOURCE_MODE`, 기본 legacy)
- [x] **P1-4** `unk_vol` 미분류 버킷 + 항등식 I4
- [x] **P1-5** N분봉 섀도·앵커 전달
- [x] **P1-6** 세대 마스크 (관측만)
- [x] **P1′-1** 수급 `*_measured` (481차 F-1 구현)
- [x] **P1′-2** 단위 불일치 4일 제외 (**기본 활성** — 스케일러 std 5.7만 배 왜곡 실측)
- [x] **P1′-3** 418차 백필 제외를 학습 로더 전부에 배선
- [x] **P1′-4** 건강도 리포트 단위 일관성 WARN
- [x] **곁가지** `test_532` 자기참조 결함 → 532차 커밋(`3c2f17c`)에 고정

---

## 2026-09-10 (MW0601 556차 — 장후 점검, 종합 완성본)

- [x] **554-1 (F-1) [DONE 2026-09-10]** ProfitGuard Tier4 판정에서 entry_source 미상
      포지션 제외 — `bd33401`(09:38)로 배포 확인. `PROFIT_GUARD_SYSTEM_SOURCES`·
      `ENTRY_SOURCE_REGISTRY`가 EOD 마감 시점 실제로 정상 동작(`sys_pnl_krw=0.0` 확인).
- [x] **554-2 (F-2) [DONE 2026-09-10]** entry_source 없는 포지션 복원 시 경고 로그 —
      `c08128f`(14:35, 555차 후속) `[출처축] 미분류 entry_source` 경고로 구현 확인.
- [ ] **F-1 (갱신, P1)** ProfitGuard Tier4 래치를 "당일 영구"가 아니라 "정정 가능한
      재평가"로 바꿀지 검토 — `strategy/profit_guard.py:_TierGate.check()`. 오늘 사고처럼
      `current_pnl` 계산 자체가 오염됐다가 나중에 정정된 경우, 래치가 하루 종일 안 풀리는
      설계 특성이 확인됨(1-8). **회귀 위험 큼 — 섀도 선행 + 사용자 승인 필수.**
- [ ] **F-5 (신규, P2)** `[Armistice]` 재시작 유예 — 12:20:01 정상 해제 후 14:42:02
      `sync=2/2`인데도 재고착 경보(1-9). `_restart_armistice_until` 재연장 원인을
      `main.py` 대입 지점 전수 grep으로 조사할 것.
- [ ] **G-4 (고도화)** ProfitGuard Tier 판정에 래치 트리거 시점 `current_pnl` 구성
      거래 ID 스냅샷을 로그로 남긴다 — 다음 유사 사고 시 원인 거래 즉시 특정 가능.
- [ ] **F-3 (장중 신규, 이월 — P0)** 장중 브로커 잔고 주기적 재대사(5~10분) — 오늘
      552~555차 커밋 중 관련 변경 없음, 아직 미착수. 1-5(3시간32분 무방비 노출)의
      근본 대책.
- [x] **F-4 (범위변경) [재확인 2026-09-10]** `[UnreconciledExit]` 발동 조건 재검토 —
      코드 확인 결과(`main.py:11588`) 12:17~12:18 청산은 `_broker_position_reconciled`가
      이미 True였던 정상 케이스로, 경보 미발동이 결함이 아니었음이 확인됨(O-i4). 우선순위
      하향.
- [ ] **554-4 (G-1, 미착수)** `position_state.json` 회전 백업 도입 — 오늘도 코드 변경 없음.
- [ ] **554-5 (G-2, 미착수)** 외부 진입 감지를 "엔진 기동 전 이미 존재하던 포지션"까지 확장.
- [ ] **O-t1 (내일 장전 판정)** `[SessionStateDrop]` 재발 여부 + F-1(538-4) 승인 여부.
- [ ] **O-t2 (다음 세션)** 1-8 재latch 경로 — `_TierGate._halted`가 재기동 시 어떤
      경로로 즉시 복원되는지 `strategy/profit_guard.py`·`main.py` 재기동 경로 대조.
- [ ] **O-t3 (내일 장후 판정)** `[ConfFloorGuard]` 오실레이션 3거래일 연속 재현 시 채널
      등록 검토(09-09·09-10 각 2회 발생·회복).
- [ ] **O-t4 (이번 주 내)** F-3 실제 착수 여부 확인.
- [ ] **O-t5 (다음 세션)** 1-9 `_restart_armistice_until` 재연장 원인.
- [ ] **로드맵 제안**: 실전 자본 전환 전, ProfitGuard 래치가 오염된 입력으로 오발동했을
      때의 복구 절차(수동 해제 UI 등) 마련 여부를 실전 전환 검토 항목으로 등록할지 결정.

---

### 554차-A~F — 유령 포지션 사고: 구현·정정 (MW0601, 2026-09-10) — **매매 정책 무변경**

> 근거: `DECISION_LOG.md` 2026-09-10(554차 · 554차 후속).
> 회귀 가드: `tests/test_554_position_state_isolation.py`(12건). 커밋 `bd33401`.
>
> 🔴 **번호가 A~F 인 이유 — 같은 날 다른 세션과 `554-N` 이 충돌했다.**
> 이 파일 아래쪽의 `554-1`~`554-5`(P0/P1/P2 · F-2 · G-1/G-2)는 **같은 사고를 조사한
> 다른 세션**이 먼저 등록한 것이고, 이 블록은 그 뒤에 구현 세션이 쓴 것이다.
> 273차(두 PC 가 같은 "273차"를 붙여 dev_memory 가 충돌)와 같은 유형이며, 이번엔
> **같은 PC 의 동시 세션**이라 PC명 태그로도 갈리지 않는다. 번호를 덮어쓰지 말고
> 갈래를 분리한다(461차 `mdd_pct` 교훈 — 식별자를 재사용하면 과거 기록이 어긋난다).

- [x] **554-A** `tests/conftest.py` autouse 격리 + `_save_state()` 운영경로 가드
- [x] **554-B** `load_state(reference_price=…)` 괴리 가드(5.0%) + `.rejected_*` 격리
- [x] **554-C** `_broker_position_reconciled` 신설 + `[UnreconciledExit]` CRITICAL
- [x] **554-D** [DONE 2026-09-10 12:28] `trades` id=572,573 의 `entry_source` 를
      `SYSTEM_AUTO` → `PHANTOM_STATE_ARTIFACT` 정정. 재계산 `sys=+0원(0레그)`.
      백업 `trades.db.bak_20260910_pre_554_4` · 마커 `strategy_events` id=96.
      ⚠ id=574,575(12:17 회수분)는 이미 `BROKER_SYNC_RECOVERY` 라 **손대지 않았다** —
        518차·552-11 배선이 정상 동작했다.
- [x] **554-E** [DONE 2026-09-10] 커밋 `bd33401` (사용자 지시로 장중 커밋).
- [ ] **554-F** 전수 조사 — 다른 운영 상태파일도 같은 경로로 뚫리는가.
      `data/*.json` 중 테스트가 쓰는 것이 또 있으면 conftest 에 같은 방식으로 묶을 것.
      422·473·554 세 번 다 **사고가 난 뒤에** 그 파일 하나만 격리했다 — 전수 조사가
      한 번도 없었다. 이번이 세 번째이므로 이제는 목록으로 훑을 것.
- [ ] **554-G** 🔴 **재시동 필요** — 라이브 세션이 `_sys_daily_net_krw=6,921,594` 를
      메모리에 들고 있어 554-D 만으로는 L2-Tier4 래치가 풀리지 않는다(그 값은 청산
      때마다 누적에 더하는 방식이라 DB 정정이 소급되지 않는다).
      `session_state.json` 에 `profit_guard_state` 가 없으므로 재시동하면 DB 에서
      `sys=0` 으로 재계산된다. 포지션 FLAT 확인 완료 — 재시동 안전.
...
- [x] **553-4** Phase 4 — 수익 판넬 「GP(가상)」 구분 완료. **표시 전용 · 기본 해제**.
      분리 합성(`실거래분 + GP분`)으로 브로커 net 대사를 보존 — 해제 시 종전과 값 동치를
      테스트가 고정한다. 배너·셀 마커(🟣)·「미배선/0건/n건」 분리. 반사실 탭에서는 제외.
      원천 `fetch_gp_shadow_positions()`(challenger.db). 테스트 20건.

- [ ] **553-5** Phase 5 — 무개입 관찰 **60거래일**(개시 2026-09-10). 위 553-OBS 참조.

### 552차 후속 — 호가깊이 적재 검증·결함수정 (MW0601, 2026-09-09) — **매매 정책 무변경**

> 근거: `DECISION_LOG.md` 2026-09-09(552차 후속).
> 계획서: `docs/미륵이고도화3/호가깊이/552차_호가깊이수집_구현계획.md`(개정본).

- [x] **552-1** 결함1 — `save_candle_and_features()` 에 `book_*` 5열 배선.
      프로덕션 raw_candles 쓰기가 `save_candle()` 이 아니라 이쪽이라 초판은 영구 NULL 이었다.
      구조 가드 `test_every_raw_candles_writer_includes_book_columns` 동반.
- [x] **552-2** 결함3 — 양변 유효 스냅샷만 계수 + 직전값 캐시 제거 +
      `[BOOK][ONESIDE]` / `oneside_skipped` 가시화.
- [x] **552-3** 결함4 — `_book_depth_cols()` 공개 평균 컬럼 폴백(복구봉 왕복 무손실).
- [x] **552-4** 계획서 정정 + byte-identical 중복 문서 삭제.

- [ ] **552-5 (별건, 552차 범위 밖)** `tests/test_500_*.py` 5개가 pytest 로
      **수집 불가**다 — 모듈 최상단에서 `print` 후 `sys.exit(0)` 하는 스탠드얼론
      스크립트라 `SystemExit` 로 수집이 깨지고 pytest capture 까지 무너진다
      (`ValueError: I/O operation on closed file`). 그래서 `pytest tests/` 전수 실행이
      **이 저장소에서 성립하지 않는다**(`--ignore`/`--continue-on-collection-errors` 로도
      `no tests ran`).
      대상: `test_500_constructive_dup` · `_cvd_ofi_live_defects` ·
      `_known_unresolved_visible` · `_stage3_decisions` · `_warmup_measured`.
      ⚠ **500차의 계측 4원칙 회귀 가드 5개가 전수 실행에서 죽어 있다** —
      TOX 죽은 섀도·FP-CRITICAL 죽은 게이트와 같은 「있는데 안 도는」 계열이다.
      조치안: pytest 함수형으로 변환하거나 `scripts/` 로 옮기고 얇은 pytest 래퍼를 둘 것.
      그때까지 검증 문구는 **「파일 지정 실행」** 으로 쓴다.

- [x] **552-8** Phase 3-0 중복 축약 신설 — 계획서 §1·§2-1·§2-2·Phase 3 정정 +
      `scripts/book_depth_duplication_check.py` + `tests/test_552b_...`(11) +
      `config/constants.py` 326차 주석 보강. 근거: `DECISION_LOG.md` 552차 후속2.
      ⚠ **Phase 3(사전등록 검증)은 Phase 3-0 통과 후에만 착수한다.**
      판정 흐름: DUPLICATE → `book_net_ratio` 등록 금지(`book_rel` 만) /
      PARTIAL → 잔차만 / INDEPENDENT → 초판대로 2종.

- [x] **552-9 [DONE 2026-09-10]** `test_504::test_unfiltered_view_keeps_broker_measurement`
      실패 원인 규명·수정 — **코드가 아니라 테스트 전제**였다. 테스트가 무필터 상태를
      세우지 않고 `data/ui_prefs.json`(운영자가 대시보드에서 마지막으로 클릭한 상태)을
      읽었다. 실측 `pnl_cb_origin_manual=False`·`unknown=False` → `_active_rows()`
      557행 중 247행 → 부분 선택된 날은 `_day_is_whole()` 이 브로커 일단위 값을
      **정상적으로 거부**. `_force_unfiltered()` 헬퍼 + 짝 테스트
      `test_partial_filter_refuses_broker_day_value` 추가. 30 passed.
      근거: `DECISION_LOG.md` 2026-09-10(552-9).

- [x] **552-10 [DONE 2026-09-10]** 브로커 net 롤오버 오염 — **가드 수정 + 재산출 완료**.
      ⚠ 원인은 「가드가 뚫림」이 아니라 **기준점 소실**이었다 —
      `_broker_dep_base_today` 가 프로세스 상태라 21:58:29 재기동이 지웠고, 그 세션에겐
      21:58:45 롤오버 판독이 「그날 첫 판독」이라 불변식이 성립했다.
      조치: `fetch_broker_dep_base()` DB 승계 + `BROKER_NET_SESSION_END`(15:35) 이후
      무기준 판독 `SKIP_NO_BASE` + `commission_rate_recon.py --date` + 감사도구에
      **라이브 경로 점검** 추가(종전엔 소급 파서만 봐서 ✅ 를 찍고 있었다).
      재산출: 2026-09-07 net **-386,630 → -1,432,630**. D1 오염 없음 / D2 10→8건.
      마커 `strategy_events` **id=95** · 백업 `trades.db.bak_20260910_pre_552_10` ·
      CLAUDE.md 전환기준 ① 등재. 회귀 `tests/test_552c_broker_net_restart_guard.py`.

- [x] **552-11 [DONE 2026-09-10]** `entry_source` 오귀속 — **출처 복원 완료**.
      `PositionTracker.entry_source` 신설(포지션 상태 파일에 실어 재시작을 넘긴다) +
      `main._entry_source` 프로퍼티 setter 미러링 + `__init__` 복원 승계 +
      읽기부 `getattr` 폴백 제거. 검출기
      `scripts/entry_source_attribution_watch.py` 신설 — 현재 **3레그/1포지션/+656,935원**.
      회귀 `tests/test_552d_entry_source_restart.py`.
      ⚠ **소급 정정은 하지 않았다**(518차 전례). 위 3레그를 `GHOST_PENDING_MISS` 로
      고칠지는 **별도 승인 사항** — 고치면 `trades.entry_source` 시계열이 2026-09-02 에
      이어 두 번째로 흔들린다.

- [ ] **552-12 (별건 · 541차 커밋에서 유입)** 비용식 핀 누수 —
      `scripts/generate_validation_campaign_report.py:7930` 이 `FUTURES_COMMISSION_RATE`
      (라이브)를 써서 `test_493::test_cost_formulas_do_not_use_live_rate` 가 실패한다.
      GP 교차 채널만 **다른 비용 기준**으로 판정돼 리포트 안에서 기준이 섞인다
      (배너는 「핀됨」이라 말하는데 그 채널은 아니다).
      조치안: `COST_MODEL_COMMISSION_RATE` 로 교체. 원 커밋 `3f2b3c6`(541차).

- [ ] **552-13 (별건 · 달력 롤오버 flake)**
      `tests/test_456_wave1_stats_and_shs.py::test_trend_sql_counts_positions_not_legs`
      가 하드코딩 날짜 **2026-08-10** 을 쓰는데 `fetch_trend_daily(30)` 창 밖으로
      나갔다(2026-09-10 기준 31일 전). **2026-09-10 부로 항상 실패한다.**
      코드 결함이 아니라 테스트가 오늘 날짜에 의존하는 것이다 — 552-9(테스트가
      `ui_prefs.json` 을 읽던 건)와 같은 계열. 조치안: 삽입 날짜를 `today` 기준
      상대일로 만들 것.

- [x] **552-6** `book_*` 첫 적재 확인 **[DONE 2026-09-13 / 559차 P0-3]** —
      ①② 적재율 100%(383/383·384/384) · ③ ONESIDE 0건 · ④ 5단/1단 비 5.72·7.37(비≤1.0 0건).
      원문:
      ① `raw_candles`·`session_bars` **양쪽** 에 `book_snaps > 0` 행이 있는가
      ② `book_snaps` 봉당 분포, 0 인 봉이 개장 직후에만 몰리는가
      ③ `[BOOK][ONESIDE]` 발생 여부 · `oneside_skipped` 누계
      ④ `book_bid_tot ÷ bid_qty`(1단) 비 — 1.0 이면 2~5단이 0 이거나 파싱 어긋남.
      ⚠ 장 마감 후에만(456차).

- [ ] **552-7 (Phase 4 전제, 잊지 말 것)** `gp_buy_20`/`gp_sell_20` 과
      `gp_range_20`/`gp_dir_20` 이 **4개 공존** 중이다. 명세 §4.3 은 선형종속이므로
      **둘만** 등록하라고 한다(권장 `gp_dir`+`gp_range`). 기록 전용인 지금은 무해하나
      소비 전환 시 반드시 줄일 것.

---

### 548차 — 장후 자동조치: 산출물 커밋 + 스테일 락 회수 (MW0601, 2026-09-08)

> 근거: `DECISION_LOG.md` 2026-09-08(548차). 리포트 제10부.
> **코드 변경 0건** — 오늘 조치 제안 전량이 C등급(승인 대기) 또는 사용자 손 작업이었다.

- [x] **548-1** `.git/index.lock`(16:18:29, 0바이트) 스테일 회수 완료 —
      `python scripts/git_lock_guard.py --reclaim`(Windows PowerShell). 547-5 해소.
      ⚠ 원인 제거가 아니라 뒤처리다. 하루 2회 재현(544-5 12:27:38 · 547-5 16:18:29)이며
      원인 확정은 **544-6**(수집기 내부 `git diff` 호출 추정)에 그대로 남아 있다.
- [x] **548-2** 09-08 점검 산출물 6종 커밋 — 리포트 1 · evidence 3 · DECISION_LOG · NEXT_TODO.
      547차가 락 때문에 커밋하지 못한 것을 이어받음.
- [x] **548-3** 547차 리포트가 주장한 545/546차 회귀테스트 통과를 독립 재확인 —
      profit_guard 관련 4파일 **55 passed**(156s). 전체 스위트는 코드 변경 0건이라 미실행.
- [ ] **548-4 (자동조치 판정 근거 기록, 참고용)** 오늘 자동 구현 0건의 사유 —
      F-1(543-1)=브로커 어댑터 파일 + 승인 대기 + 결정사항 미결 / 538-4=승인 대기 /
      F-2(547-1)=DB 쓰기 + 전환기준① 판정 원천 + 범위 미결 / 547-2=사용자 결정 /
      G-2=리포트 자신이 조치 불필요 / 로드맵 2건=주간회의 안건 / 제5부=신규 0건.
      ⚠ **F-1 의 1단계(소요시간만 로깅)는 등급이 갈릴 수 있다** — 섀도 계측만 보면
      B등급이나 변경 파일이 자동조치 제외 범위(브로커 어댑터)라 보류했다. 사용자가
      "이 파일도 섀도면 허용"으로 정하면 다음 자동조치부터 즉시 구현 가능하다.

---

### 547차 — 장후 점검: ProfitGuard 원인 통합 기록 + NetRecon 회계오염 발견 (MW0601, 2026-09-08)

> 근거: `DECISION_LOG.md` 2026-09-08(547차). 리포트: `docs/정기점검/매일점검/MW0601-20260908-점검리포트.md` 제1p~9p.

- [ ] **547-1 (P1, Fix F-2, 사용자 승인 필요)** `daily_broker_pnl` 2026-09-07 행 롤오버
      정산 오염 백필 — `python scripts/broker_net_chain_audit.py`(읽기전용) 실측:
      09-07 장중 실제 net -1,432,630원 vs 기록 -386,630원(약 104만원 과소). D2 체인
      연속성 검사에서 09-07→09-08 구간 +386,630원 불연속으로 교차확인. dev 브랜치
      501차가 규명한 D1 패턴이 v9-dev에 독립 재현된 것으로 보임. 조치:
      `python scripts/commission_rate_recon.py --backfill --force`(미실행, 승인 대기).
      ⚠ D2가 5월~9월 사이 총 10건 불연속을 나열했다 — 백필 범위를 09-07 단건으로
      좁힐지 전체를 검토할지 결정 필요.
- [ ] **547-2 (확인 필요, P2)** 2026-09-02 NetRecon 발동 건(엔진 -5,265,803원 vs
      브로커 -5,564,446원, 잔차 +298,643원)이 547-1과 같은 D1 계열 오염인지 —
      D2 감사 결과 목록에 이 구간은 없어 이번 세션에서 확정하지 못함. 547-1 백필과
      함께 재검토할 것.
- [ ] **547-3 (관측 예정, O-t1~O-t4)** 다음 거래일(09-09) 확인 — ⓐ ProfitGuard 헤더
      배지가 정상 렌더링되는가(`PG 정상 +N원` 등, `PG ——` 고착이나 `⚠ 갱신실패` 아님)
      ⓑ `[DebugPnL]` 4축(sys/외부/engine/broker)이 로그에 찍히는가, 차단 줄에
      `src=engine(net·시스템진입만)` 토큰이 붙는가 ⓒ 09-09 EOD `[NetRecon]`이 다시
      허용오차를 초과하는가(초과 시 547-1 백필로도 미해결이거나 신규 오염) ⓓ
      `data/profit_guard_prefs.json`의 `profit_tiers` 5단계가 여전히 전부 500,000원인가
      (545-1 여전히 미결이면 그대로).
- [x] **547-4** 이월 처리 — 544-2(이월 레그 확정) ✅ 해소: `trades.db` 조회 결과
      09:20:44 청산 레그의 `entry_ts`는 2026-09-08 09:20:36(오늘)로 확정 — 이월
      포지션이 아니었다. 사용자조치4(`.git/index.lock`) ✅ 해소: `git_lock_guard.py
      --check` 재실행 결과 "정상 — 락 없음"(단, 이후 547-5로 재발 확인됨). 1-2
      (외부진입) 손익 수치는 로그 재구성(5건 승3·60%·+72,991) → DB 실측(6건
      승3패3·50%·+67,967, 총액 동일)으로 정정.
- [x] **547-5 ✅ 해소(548차 자동조치가 회수)** `.git/index.lock` 이 세션 자신의
      `collect_evidence.py --phase post`(16:17:52) 실행 중 재생성(16:18) —
      `git_lock_guard.py --check` 는 "스테일 확정"이나 `--reclaim` 이 리눅스 샌드박스
      마운트 제약으로 `Operation not permitted` 실패. Windows PC에서 직접
      `del .git\index.lock` 필요. 544-6(수집기 git diff 호출이 원인 추정)과 같은
      계열 — 대상 파일 수가 552건으로 클 때 재현되는지 다음 세션에서 관찰할 것.

---

### 546차 — ProfitGuard 판정 손익 = 시스템 자동매매 한정 (MW0601, 2026-09-08) — 🔴 **매매 정책 변경**

> 근거: `DECISION_LOG.md` 2026-09-08(546차). 회귀 가드
> `tests/test_546_profit_guard_system_only_pnl.py`(17건).
> 킬스위치: `config/settings.py:PROFIT_GUARD_SYSTEM_ONLY_PNL` → `False` 면 종전 복귀.

**관측 확인 (다음 거래일 — 재기동 후부터 적용된다)**
- [ ] **`[DebugPnL]` 4축이 찍히는지** — `source=engine_system_only used=… | sys=…
      외부=… | engine=… broker=…`. `sys` 와 `engine` 이 **같으면 그날 외부매매가
      없었다는 뜻**이고, 크게 벌어지면 그 차이가 곧 제외분이다.
- [ ] **차단 줄의 원천 토큰** — `src=engine(net·시스템진입만)` 인지. 다른 토큰이면
      플래그가 꺼졌거나 폴백 경로다.
- [ ] **첫 시스템 청산 이후 배지 손익이 오르는지** — 그 전까지는 `PG 정상 +0원`이
      정상이다(툴팁에 `시스템 청산 0레그` + 제외분이 함께 뜬다).
- [ ] **재시작 복원** — 장중 재기동 시 `[Restore] ProfitGuard 시스템손익 복원:
      sys=… 외부=… 출처미기록=…` 이 찍히는지. 안 찍히면 그 세션은 0원에서
      시작하므로 보호가 느슨해진다.
- [ ] **`출처미기록` 레그가 0인지** — 0이 아니면 `entry_source` 배선이 다시 샌 것이다
      (518차 F-3 계열). 그 레그는 시스템으로 세지 않으므로 판정이 과소해진다.

**dev 이식 — 완료**
- [x] **dev 이식 완료 (2026-09-08)** — `d64e10d`(dev). 기계적 체리픽이 아니라
      **적응**이다: dev 에는 477차 GR-3 `pnl_source` 배선이 없어 최소한만 들여왔고
      (인자 미제공 시 종전과 같은 문구 — 하위호환), dev 에 없는 490차 F-G 블록은
      제외했으며, dev 의 468차 G-3 청산 2축 분류는 유지했다.
      검증(dev): 45 passed · 변경 7파일 py_compile 통과.
      ⚠ `import main` 계열 테스트는 격리 워크트리에 `config/secrets.py`·`data/` 가
      없어 제외했다 — **dev 를 실제로 쓰는 PC 에서 한 번 더 돌릴 것**(dev 쪽
      `NEXT_TODO` 에 등록).
      ⚠ dev 는 MW0602 운영 브랜치다. `data/profit_guard_prefs.json` 이 PC 로컬
      (gitignore)이라 그쪽 `profit_tiers` 값이 다르면 체감 변화도 다르다.

**후속 안건**
- [ ] **546-1. 새 축에서 `profit_tiers` 임계 재보정** (545-1 승계).
      5단계 전부 500,000원은 그대로 뒀다 — 축과 임계를 같이 바꾸면 사후에 어느 쪽
      효과인지 분리할 수 없다(313차). 새 축은 실현 전용이라 **같은 값이라도 더 늦게
      닿는다.** 며칠 `sys` 실측치 분포를 본 뒤 정할 것.
      ⚠ 재보정 시 `strategy_events` 에 불연속 마커를 남길 것.
- [ ] **546-2. L1 트레일링 발동금액도 같이 볼 것** —
      `trail_activation_krw=300,000` 역시 계좌 전체 기준으로 잡힌 값일 가능성이 크다
      (2026-09-08 피크 +684,928원은 거의 전부 외부분이었다). 546-1과 한 묶음.
- [ ] **546-3. `entry_source` 미기록(NULL) 레그의 처분** — 현재 **제외**다.
      311차 이전 구간이며 423차 실측에서 이 구간 6건이 A급 30일 합계의 부호를
      뒤집었다. 라이브에서 새로 NULL 이 생기면 그건 결함 신호이므로, 위 관측
      항목이 0을 확인하는 것으로 감시한다.
- [ ] **546-4. 전환기준 ① 과의 관계 확인(문서 안건)** — ① 은 여전히 **브로커 실측
      net(계좌 전체)** 으로 판정한다(493차). ProfitGuard 축만 좁혔으므로 두 축이
      공존한다. 주간회의에서 "실전 전환 판정도 시스템 한정으로 볼 것인가"를 별도로
      정할 것 — **이번 변경은 ① 을 건드리지 않았다.**

**하지 말 것**
- ⚠ ProfitGuard 축이 좁아진 것을 근거로 「가드가 느슨해졌으니 임계를 낮추자」로
  가지 말 것. 시스템이 스스로 벌면 **같은 임계에서 그대로 발동한다**(테스트로 고정).
  먼저 새 축의 실측 분포를 볼 것.
- ⚠ 2026-09-08 앞뒤로 「ProfitGuard 가 본 일일손익」을 직접 비교하지 말 것(불연속).

---

### 545차 — ProfitGuard L1~L4 배지 (MW0601, 2026-09-08) — **매매 정책 무변경 · 관측 전용**

> 근거: `DECISION_LOG.md` 2026-09-08(545차). 회귀 가드
> `tests/test_545_profit_guard_badge_status.py`(19건).

**관측 확인 (다음 거래일)**
- [ ] **헤더 배지가 실제로 갱신되는지** — `PG 정상 +N원` 또는 `PG 감시 …`가
      뜨고 칩 4개(L1~L4)가 색을 갖는지. `PG ——`(초기값)가 장중 내내 유지되면
      호출부가 안 도는 것이고, `PG ⚠ 갱신실패`(보라·깜빡임)면 `guard_status`가
      예외를 낸 것이다 — 그때는 SYSTEM 로그에 `[ProfitGuard] 배지 갱신 실패`가
      **세션당 1회** 찍힌다(종전처럼 침묵하지 않는다).
- [ ] **래치 시 깜빡임 확인** — 래치가 실제로 걸리는 날, 본라벨과 구속 칩이
      700ms로 깜빡이고 라벨에 `🔒 <레이어> 중단 <당일손익>`이 뜨는지.
      ⚠ 래치를 만들려고 일부러 조건을 맞추지 말 것 — 관측될 때까지 기다린다.

**매매 정책 안건 — 사용자 결정 대기 (이번 세션에서 고치지 않았다)**
- [ ] **545-1. `profit_tiers` 5단계 전부 500,000원이 의도인가.**
      `data/profit_guard_prefs.json` 실측이며 코드 기본값(Tier4 = 4,000,000)의
      **1/8**이다. Tier0~4 임계가 같아 50만원을 넘는 순간 중간 단계 없이
      Tier4(당일 중단)로 점프한다. 의도면 그대로 두고, 아니면 Tier0~3 임계를
      되돌린다. ⚠ prefs는 gitignore 대상이고 **장중 대시보드에서 바뀔 수 있다**
      (426차 §38 주석의 "판정 기준선이 코드 리뷰로 보이지 않는다"와 같은 문제).
- [ ] **545-2. ProfitGuard 판정에서 외부(수동) 매매 손익을 분리할 것인가.**
      현재 원천은 `broker_net_est` = 브로커 실현손익(gross) − 엔진 당일 수수료라
      **계좌 전체**다. 2026-09-08 09:54 실측 `used=+513,967 engine=+67,967` —
      시스템이 벌지 않은 44.6만원으로 시스템이 멈췄다. 계측 4원칙 ①(축 명시)
      관점의 축 혼재다. ⚠ 엔진 net으로 되돌리는 것은 **답이 아니다** — 493차가
      브로커 원천으로 옮긴 이유(수수료 축 미대사)가 그대로 살아 있다.
      선택지: (A) 시작값 델타(545-3)로 흡수 (B) `entry_source` 기준 분리 집계
      (C) 현행 유지하고 배지·툴팁에 원천만 명시(**이미 반영됨**).
- [ ] **545-3. 장 시작 시 `실현손익`이 0이 아닐 때의 기준선 처리.**
      09-03·09-04·09-07은 최초값 `0`인데 **09-08만 09:04:58에 이미 +446,000**
      이었다(출처 미확인 — 야간세션 체결 가능성). 시작값을 빼고 **당일 델타**로
      판정하지 않으면 "54,000원 여유로 개장"이 재발한다.
      ⚠ 먼저 **446,000의 출처를 확정**할 것 — 야간세션이면 델타가 맞고, 전일
      미정산 이월이면 `daily_broker_pnl` 롤오버 판독 오염(dev 501차 D1)과 같은
      계열일 수 있다. 장 마감 후 `scripts/broker_net_chain_audit.py`(읽기전용)로 확인.

**부수 확인**
- [ ] **`ProfitGuard.status_dict()` 호출부 없음** — 545차에 되살아났지만 소비처가
      없다. `profit_guard_panel`이 `guard_status`로 옮겨탈지, `status_dict`를
      폐기할지 다음에 정리. 지금은 무해(테스트가 무예외만 고정).
- [ ] **기존 실패 2건은 545차와 무관** — `test_493::test_cost_formulas_do_not_use_live_rate`
      (지목 대상 `scripts/generate_validation_campaign_report.py:7930`) ·
      `test_504::test_unfiltered_view_keeps_broker_measurement`(`PnlHistoryPanel`,
      라이브 DB 의존). 별건으로 처리할 것.

---

### 542차 — 수동 맥점 산출 버튼 (MW0601, 2026-09-07) — **매매 정책 무변경 · 관측 전용**

> 근거: `DECISION_LOG.md` 2026-09-07(542차).

- [ ] **다음 EOD 로그(2026-09-08)** — `[LEVELS] 이력 캐시 갱신 … 격자경로 N세션` 이
      찍히고 N ≥ 30 인지. 30 미만이면 수동 산출의 **거리 모델이 나오지 않는다**
      (구조 모델만 뜬다). ⚠ 2026-09-07 저녁에 전량 재생성을 1회 수동 실행해
      244/250 세션을 채워 두었다 — 다음 EOD 는 증분이라 `(전량 재생성)` 이 **안 찍히는
      것이 정상**이다.
- [ ] **라이브 첫 클릭** — 로그 `[LEVELS 수동] 봉원천=buffer` 인지 확인.
      `db_fallback` 이면 종일 봉 버퍼가 죽은 것이다(534차 F-2 와 같은 계열).
      산출 소요도 함께 찍힌다 — 100ms 를 크게 넘으면 캐시가 커진 것이다.
- [ ] **09:00 이전 클릭 1회** — 모델이 `M1` 로 뜨고 「09:00 이전」 경고가 붙는지.
      여기서 `P@…` 가 뜨면 격자 스냅이 깨진 것이다(훈련 시점 밖 외삽).
- [ ] **수동 산출 채점 여부 결정(안건)** — 정시 2행과 달리 채점 경로가 없다.
      `premarket_levels_manual` 이 쌓이면 「클릭 시각별 커버리지」를 잴 수 있지만,
      **클릭 시점이 사용자 선택이라 표본이 자기선택 편향**을 갖는다(움직임이 큰 날
      더 누른다). 채점을 붙일지, 붙인다면 편향을 어떻게 다룰지는 주간회의 안건.
      ⚠ 그 전까지 「수동 산출이 잘 맞더라」를 성과로 인용하지 말 것.
- [x] **dev 이식 완료 — 「적응」으로 끝냈다 [DONE 2026-09-07]** (`532d941`)
      기계적 체리픽은 못 쓴다: dev 는 같은 계측 5건을 `91fb5a9`(MW0602 538차)로
      **독립 구현**했고 API 가 다르다 — `rhat_trace` ↔ **`rhat_diag`** ·
      `structure_diagnostics`/`structure_note` **부재** · 대시보드 `_fmt(items)` 는
      diag 를 안 받는 534차 원형 · `prepare_params(…, db_path, excluded)`.
      `a21e270` 체리픽 실측은 자동병합 4파일 + 충돌 3곳인데 **자동병합된 쪽이 dev 에
      없는 함수를 부른다** ⇒ 빌드는 되고 런타임에 NameError(오늘 고친 GP 결함과 같은
      유형). 그래서 dev 어휘에 맞춰 옮겼다:
      빈 쪽 사유는 dev 의 `stage_warnings()` 가 내고, 대시보드는 `_fmt` 를 그대로 쓰며
      사유는 툴팁으로 붙인다(정시 행과 **같은 말**을 쓰기 위해). `bars_source` 는 dev
      스키마에 없어 **수동 테이블에만** 남겼다 — 남의 스키마 무변경.
      검증(dev): test_542(23) + test_534 + test_502·540·541 = **92 passed 1 skipped**,
      `main.py` 참조 46파일은 이식 **전후 동일**(15 failed / 44 passed — 워크트리에
      `data/` 가 없어 나는 기존 실패).
      ⚠ **같은 기능의 변종이 두 갈래에 생겼다.** 앞으로 이 기능을 고칠 때는 양쪽을
      함께 볼 것 — 짝짓기는 번호가 아니라 **키 문자열**(`compute_manual` ·
      `PATH_GRID_ID` · `premarket_levels_manual`)로 한다(489차 B-3 원칙).
      · dev 반영 커밋: `76d1a9b`(GP NameError) · `532d941`(수동 산출 적응판).
      · ⚠ dev 의 첫 클릭은 거리 모델이 안 나온다 — 격자 경로를 EOD 가 채워야 한다.
...
- [ ] **V5 (다음 장중 재학습)** — `logs/retrain_intraday_*.log` 첫머리에
      `[DLL] BLAS 경로 보강 N개: [... Library\bin]` 이 찍히고 재학습이
      **성공 유지**되는지. ⚠ `보강 0개`가 아니라 **`bootstrap 실패`** 가 찍히면
      모듈 import가 깨진 것이며 BLAS를 밟는 순간 즉사한다.
      ※ 「이미 충족」이 뜨면 F-2(부모가 env를 넘김)가 먼저 작동한 것 — 정상이다.
- [ ] **V6 (다음 EOD 재학습)** — `logs/retrain_eod_*.log` 가 현행과 동일하게
      성공(호라이즌 6/6 교체)하는지. F-2가 라이브 spawn 경로를 건드렸으므로
      **회귀가 없다는 것을 실측으로 확인**해야 한다.
- [ ] **다음 26주 WFA 착수 시** — L1(`core_feature_discovery`) 실행이 즉사하지 않는지
      확인. 종전에는 L2만 보호돼 있어 L1이 먼저 죽었을 것이다(미실행이라 드러나지 않았다).
- [ ] **회귀 가드 상시화 검토** — `python scripts/audit_dll_bootstrap.py --fail-on-gap`
      을 EOD 체인이나 주간 점검에 편입할지. 지금은 `tests/test_537_...` 가 pytest 경로로만
      막는다. ⚠ 편입하면 실행 시간이 늘어난다(전 저장소 AST 스캔 약 10초).

### 534차 후속 — 맥점 계측 결손 5건 (MW0601, 2026-09-07) — **매매 정책 무변경 · 계측 전용**

> 근거: `DECISION_LOG.md` 2026-09-07, `docs/정기점검/매일점검/MW0601-20260907-맥점계측-딥다이브.md`.
> ~~🔴 **커밋 보류 중** — 장중 실시간코드 커밋 금지. 장 마감(15:35) 후 커밋할 것.~~

- [x] **장 마감 후 커밋** — `main.py`·`dashboard/main_dashboard.py`·`utils/db_utils.py`·
      `features/levels/*` + 딥다이브 문서. 커밋 전 `tests/test_534_premarket_levels.py`
      재실행(32 passed). **[DONE 2026-09-07]** — 문서는 537차·538차 후속에 이미 실려
      있었고 코드만 남아 있었다. 장 마감 후 542차 세션이 커밋.
- [ ] **09-08 08:50 첫 계측 관측** — 새 로그 1줄
      `[LEVELS 08:50] 봉원천=… N봉 · 기준봉=08:45 · 구독지연=N.Ns`.
      정상 = `봉원천=buffer`. **`db_fallback`이면 메모리 버퍼가 죽은 것**이며
      「장중 DB를 읽지 않는다」는 설계 전제가 깨진 상태다(로그 레벨 WARNING).
- [ ] **구독 지연 추이** — 2026-09-07 실측 **7.0s**(08:45:07). 이 값이 커지면 거리
      모델 기준점 O의 오차도 커진다. **533차 09-08 프로브(08:45 봉 형태)와 함께**
      볼 것 — 맥점 단독으로는 O 오차를 못 잰다.
- [ ] **R̂ `floor_hit` 빈도 축적** — 09-07 실측 `raw=0.3898 → clip 0.85`.
      갭 큰 날마다 하한에 붙는지 `rhat_trace` JSON으로 누적.
      ⚠ **60일 전에는 판정하지 않는다**(가이드 §8, 313차 — 하루 표본으로 파라미터
      변경 금지). 지금은 하한 0.85를 올릴 근거가 **아니다**.
- [ ] **구조 상방/하방 0 은 결함 지표가 아니다** — 갭 큰 날의 정상 현상.
      진짜 결함 지표는 **`cand_total = 0`**(이력 봉 부족)이다. 혼동하지 말 것.

### 535차 — 장전 점검 (MW0601, 2026-09-07)

> 리포트: `docs/정기점검/매일점검/MW0601-20260907-점검리포트.md` 장전(pre) 절.
> 상세: `DECISION_LOG.md` 2026-09-07(535차).

- [ ] **F-1 (P1, 승인 시 즉시 적용 가능 — 매매정책 무변경)** `strategy/runtime/session_recovery_service.py`
      `increment_session()` 120~129행에 `p8_last_success_date`·`eod_retrain_ok_date`
      이어받는 두 줄 추가. 원인 **확정**(3거래일 재현 — 09-03·09-04·09-07). 장후 재기동 시 적용.
      검증: 09-08 첫 기동 시 두 키 보존 확인 + `tests/test_5XX_session_state_marker_carryover.py` 신설.
- [ ] **G-1 (P2, F-1과 함께)** `increment_session()` 날짜 전환 자체를 `[SessionRollover]` INFO 로그로 남긴다.
- [ ] **O-p1 (최우선, 장중·장후)** `[ExternalEntry]` 정체불명 외부 진입 재발 여부 — 09-04까지
      4거래일 연속(최종 62건/66계약), 오늘 08:57~09:05 구간 0건이나 판정 불가(개장 직후).
- [ ] **O-p2 (09-08 장전)** F-1 적용 후 다음 날짜 전환 시 `session_state.json` 마커 재소실 여부.
- [ ] **O-p3 (매 장전)** 09:00대 메인 스레드 정지 최댓값 추이 — 오늘 28,797ms(482차 F-3 그림자
      계측 신규 최댓값, 종전 08-25 21,781ms 대비 +32%). 26주 재검토 전 중간 점검 권고.
- [ ] **P2 (문서)** `CLAUDE.md` 운영 환경 표의 `joblib 1.1.1`을 실제 설치·`requirements.txt`
      기준 `1.1.0`으로 정정.

### 사용자 몫 (자동조치 범위 밖)

- [ ] **최우선** — 정체불명 외부 진입(09-01~09-04, 4거래일 62건/66계약) 계좌 본인 직접거래
      여부 확인. 아니라면 계좌 보안(비밀번호·공동인증서·API 키) 점검 필요.
- [ ] F-1 적용 승인(코드 버그 수정, 매매 정책 무변경 — 재기동만 필요).
- [ ] `.git/index.lock` 수동 삭제 — `git_lock_guard.py --check`가 스테일 확정(실행 중
      git 0개)했으나 이 점검 세션(리눅스 마운트)에서는 `Operation not permitted`로
      회수 실패. Windows 작업 PC에서 직접 삭제할 것.

---

### 534차 — 당일 맥점 예측 거리·구조 모델 (MW0601, 2026-09-06) — **매매 정책 무변경 · 관측 전용**

> 배선 완료. 근거: `DECISION_LOG.md` 2026-09-06(534차),
> `docs/미륵이고도화3/당일맥점예측_거리모델_구조모델_구현가이드_2026-09-06.md`.

- [ ] **첫 라이브 산출 관찰(다음 거래일)** — SYSTEM 로그에 `[LEVELS 08:50]` 2줄,
      `[LEVELS 09:30]` 2줄. 정상 = 미산출 사유 없음 + `[PipePerf]` 최장 블로킹
      무변화(맥점 산출은 수십 ms 예산). 대시보드 진입탭 좌(거리)·우(구조)에 값 표시.
      ⚠ **EOD 캐시가 없으면 「이력 캐시 없음」으로 미산출된다** — 09-07 EOD가 한 번
      돌아야 09-08 08:50이 나온다. 이미 수동으로 캐시를 만들어 뒀으므로(249세션,
      ~2026-09-04) 다음 거래일은 그대로 나와야 한다.
...
- [ ] **누적 60일 도달 시 R̂ 채택 재판정**(가이드 §5·§8) —
      ...
      기대: 08:50 83% vs 78% / 09:30 81% vs 74%. 역전되면 §5 채택을 되돌린다.
      ⚠ **현재 백필 표본은 46~47일뿐이고 2026-06-23 이후만이다** — `raw_candles`
      첫 봉이 08:45가 된 날. 그 이전은 08:45 시가가 없어 전부 미산출이며, 이는
      결함이 아니라 규칙대로 지어내지 않은 결과다.
- [ ] **구조 모델 판정은 「무작위와 같으면 정상」이다** — 백필 실측 ±0.5% 안착
      43%/41%(기대 ~47%). 이 값이 낮다고 개선 과제로 올리지 말 것. 터치율은
      지표가 아니다(레벨이 그날 범위 안이면 반드시 터치, 무작위도 78%).
- [ ] **`session_bars` 안정화 후(533차 Phase 2~3) 실제 고·저 원천 확장 검토** —
      현재 채점은 `raw_candles`(08:45~15:08)를 쓴다. 15:09~15:44가 들어오면 그날
      고·저가 바뀔 수 있으므로 **원천 전환은 불연속 마커**(`strategy_events`
      `METRIC_REDEFINITION`)와 함께 해야 한다(461차 `mdd_pct` 유형).
- [ ] (선택) 옵션 OI 원천 — 가이드 §9 Phase 4. 검증에서 극값 안착이 44.3%→41.8%로
      **내려갔으므로 후순위**. Cybos `OptionMst` 헤더 99가 선행 조건.
...
5. 미륵 피처를 넣지 않는다 — 넣으면 MAE 11.0 → 16.5.
6. 장중에 `raw_candles` 전수 스캔을 하지 않는다(2026-08-10 CB⑤ 자가유발).

### 550차 — 풀타임 수집 첫 3거래일 점검 후속 (MW0601, 2026-09-09) — **매매 정책 무변경**

- [ ] **09-10 장후 `python scripts/session_bars_recon.py --date 2026-09-10`** — 550차 플러시 첫 라이브.
      정상 = `POST_FORCE_EXIT 25`(15:34 포함) · 로그 `[CybosRT-FLUSH] … ts=15:34` 1줄(15:36:xx) ·
      `auction_code` 전부 0(40 아님) · Phase 1 **PASS**. 15:34 가 여전히 없으면 스케줄러 창(15:36) 확인.
- [ ] **프로브 4종 — 아직 한 번도 안 돌았다(09-08 예정분 미실행).** 09-10 실행 순서(py37_32·관리자):
      08:43 `python scripts/probe_cybos_session_edge.py --until 08:48` (P0-2 개장 체결 28=10)
      장중 5분 `… --until +5m --coexist` (P0-4 병행 구독)
      15:30 `… --until 15:47` (P0-1 마감 체결 28=30)
      15:55~ `… --chart 20260910` (P0-3 차트 TR)
      요약 `logs/session_edge_*_summary.txt` 를 그날 리포트에 첨부. ⚠ 프로브 요약의 「auction ticks」는
      550차 이전 파서와 무관하게 헤더 28 원값을 그대로 찍는다 — **40 은 장중(연속매매)** 로 읽을 것.
- [ ] **Phase 2 배선 조건**(변경 없음): P0-1 에서 15:45 체결틱 28=30 관측. 스케줄러 15:50 은 완료.
- [ ] **안 C 결정(사용자)**: P0-4 결과 확인 후.
- [ ] MW0602 가 533차를 체리픽했다(dev `05f75d9`) — 550차 정정 2건도 같은 결함을 갖는다.
      가이드 §7 추가분 참조(가져갈지는 MW0602 가 정한다).

### 533차 — 풀타임 수집 Phase 0·1 (MW0601, 2026-09-06) — **매매 정책 무변경 · 저장 전용**

- [ ] ~~**09-08 프로브 4종 수동 실행**~~ → **미실행(550차 확인)**, 09-10 로 이월(위 549차 항목)(py37_32·관리자) — 순서·명령은
      `docs/미륵이고도화3/풀타임수집_검토및구현계획_2026-09-06.md` §7-1.
      요약 파일 `logs/session_edge_*_summary.txt` 를 그날 점검 리포트에 첨부.
- [x] **09-08 장후 Phase 1 검증** `[DONE 2026-09-09 550차]` — 3일치 실행: OHLCV 불일치 0, 15:09 회수 ✓, **15:34 유실·코드 40 오판 2건 발견·정정**. — `python scripts/session_bars_recon.py --date 2026-09-08`.
      정상 = `PRE_MARKET 15 / REGULAR 370(15:09 포함) / POST_FORCE_EXIT 25` ·
      raw_candles 교집합 OHLCV 불일치 0 · `[SessionBars] 적재 실패` 로그 0건.
      ⚠ 첫날 `auction_code` 가 전부 NULL 이면 헤더 28 인덱스가 문서와 다른 것 —
      `[CybosRT-AUCTION] 헤더 28 읽기 실패` WARNING 확인.
- [ ] **09-08 종료 후 결정(사용자)**: 별도 수집 프로세스(안 C) — P0-4 `--coexist` 결과.
- [ ] **Phase 2 배선 조건**: P0-1 요약에 `15:45 종가단일가 체결틱(28=30): 관측됨`.
      배선 항목: 구독 종료 15:46(만기일 15:21) · 자동종료 그 뒤 · FZ-1 창 15:47 ·
      08:44:30 구독 시작. 스케줄러 15:50 이동은 **완료**.
- [ ] Phase 3(장후 FutOptChart 대사·보충) — P0-3 결과(08:45·15:45 봉 형태) 확인 후.
...
> A-1·A-2·B-1·B-2·B-3·[8] 구현 완료. 상세: `DECISION_LOG.md` 2026-08-23(MW0601 489차).

**🔴 CB② 스테이지 2 — 기한 2026-08-29(토), 마지막 회의 08-28(금)**

- [ ] **CB2-1 (08-24 월, 첫 거래일) 스테이지1 라이브 검증** — 손절이 난 날
      `[CB] 연속 손절 N회 (300초 창, 포지션 단위)` 형식으로 찍히는가.
      **같은 포지션의 계단식 손절이면** `[CB] 같은 포지션의 추가 손절 레그 —
      카운트하지 않는다` 가 함께 떠야 한다. `position_key 가 없다` WARNING이
      뜨면 호출부 중 하나가 키를 못 넘긴 것이다(즉시 조사).
- [ ] **CB2-2 재기동 왕복 확인** — `session_state`에 `stop_events`가 남고 복원
      로그가 뜨는가. 구버전 상태 복원 경고(`stop_events 없음`)는 **첫 재기동 1회만**
      정상이다(그 뒤로는 새 포맷이 저장된다).
- [ ] **CB2-3 (08-28 금) 한도 복원 결정** — CB2-1·CB2-2가 확인되면
      `CB_CONSEC_STOP_LIMIT` **9999 → 3**. ⚠ **2가 아니라 3** (표본 비용 최소화).
      ⚠ 이것은 **라이브 매매 정책 변경**이다 — 되돌림은 상수 한 줄이지만 당일 정지가
      늘어 진입 표본을 직접 줄인다(현재 58채널 중 13채널이 표본 미달).
      ⚠ 확인이 안 됐으면 **복원하지 말고 사유와 새 기한을 레지스트리에 박을 것** —
      2026-08-01 연기가 "트리거 소멸"로 흐지부지된 전례가 있다.

**다음 금요일(08-28) 리포트에서 볼 것**

- [ ] **[8] 3관문** — ②의 평균차 부호가 계속 (+)인가. (+)면 *"KellySkip이 더
      벌었다"*는 뜻이고 차단 도입은 **방향 자체가 틀린** 처방이다. 누적 손익만
      보고 재론하지 말 것(이번 실측: 누적 −262,228원인데 일자단위는 +86,733원).
- [ ] **[46] 판정보조 ③** — 반쪽 **차단n**이 두 자릿수로 올라오는가.
      🔴 지금은 1~9건이다. ③ PASS를 채택 근거로 인용하려면 그 수부터 볼 것.
- [ ] **[45] 플래핑** — 재등록된 단독 트리거(점프 중앙값 ≥0.10 **그리고**
      전환 ≥4.0회/일, **2주 연속**) 충족 여부. 0821 실측은 점프 0.0335로 미충족.
- [ ] **[25] atr_lock_0.75** — 70훅/13일 도달 여부(0821 실측 52훅/9일).
      도달해도 자동 적용이 아니다(클램프 발동률 33%가 단독 기각 사유일 수 있다).

**하지 말 것**

- 🔴 **`CB_CONSEC_STOP_LIMIT`을 CB2-1 확인 전에 바꾸는 것** — 2단계 조건부 복원의
      "조건"이 그 확인이다. 계측이 틀린 채로 한도를 조이면 **정상 거래를 끊는다**.
- 🔴 **`record_stop_loss()`를 인자 없이 호출하는 것** — 레그 단위 폴백으로 돌아가
      이 수정이 무력해진다. `tests/test_489_...py` (G)가 AST로 막는다.
- 🔴 **[46] ③ PASS 를 "채택 가능"으로 읽는 것** — 2분할은 표본을 절반으로 쪼갠다.
      차단n 한 자릿수 위의 분리도이며, 임계를 낮추면 **노출이 는다**(CLAUDE.md ⑧ 미해제).
- 🔴 **"양 PC 수렴"을 해제조건으로 새로 쓰는 것** — B-2가 그것을 폐기했다.
      대조가 필요하면 번호가 아니라 **채널 키 문자열**로 할 것(B-3).

---

### 488차 — [35] 유령 하드스톱 판정 준비 (MW0601, 2026-08-23) — **매매 정책 변경 0건**

> 439차의 *"모집단 소멸"* 서술은 **MW0601 에 적용되지 않는다.** 주판정 표본이
> 12건/5거래일로 살아 있고 `min_samples=20` 까지 **약 9거래일**이다. 상세:
> `DECISION_LOG.md` 2026-08-23(MW0601 488차).

**판정 시 읽는 법 — 도달 전에 고정했다 (313차 · 372차)**

- [ ] 🔴 **`delta_pt_sum` 을 단독 인용하지 말 것.** 2026-08-21 기준 −45.70pt 는
      **1건(2026-08-18 10:40:58, −52.84pt)이 만든 값**이고, 그 1건을 빼면
      **+7.14pt 로 부호가 뒤집힌다**. 리포트가 `drop-max` 줄로 매주 실측한다.
- [ ] **사전등록 관문은 일자단위 부호검정 하나다** — drop-max 는 보조이고
      **합격선이 아니다**(§9-4 판정기준 사후 변경 금지). 도달했다고 문턱을
      바꾸지 말 것(458차 D6).
- [ ] **이 채널의 delta 는 구조적으로 비대칭이다** — 유령은 이익을 소액 확정
      (상방 = 실현 pt 근방), 반사실은 본전 보호스톱이라 **상방 무제한**.
      누적이 추세일 1건에 지배되는 것은 **결함이 아니라 설계상 예정된 성질**이다.
- [ ] **현재 궤적이면 PASS 가 유력하다**(유의성 관문이 먼저). 그 권고문은
      *"손익 중립이면 정확성 우선 — 가드를 `qty>=2` 까지 활성화"* 이며
      **라이브 청산 동작이 바뀌는 변경**이라 §9 주간회의 수동 결정이다.

**다음 주 관측**

- [ ] **PH-1 (매 금요일)** [35] `n` 이 20 에 도달했는가. 도달 시 위 4줄을 먼저 읽고
      상정 여부를 판단할 것 — 자동 적용 금지.
- [ ] **PH-2** `drop-max` 줄의 `부호 역전` 표시가 사라지는가. 사라지면 결론이
      1건 의존에서 벗어났다는 뜻이라 증거 강도가 실제로 올라간다.
- [ ] **PH-3** 리포트 [35] 절에 *"소멸했다"* 문구가 다시 뜨지 않는가.
      뜨면 `pse["n"]` 이 0 이 됐다는 뜻이므로 **모집단이 정말 끊긴 것**이니
      배선(`tp1_breakeven` 훅)을 확인할 것.

**파생 — 라이브 데이터에 고정된 테스트 4건 → [DONE 2026-08-23, 488차 후속]**

- [x] **4개 파일 전부 4축 재설계 완료** (425·427·428·429 — FAIL 12건 → 0건,
      실 DB eval 테스트 8개 파일 전부 rc=0). 창 의미 선행 확인 결과 대상 6채널
      전부 `_campaign_start()` 고정 누적이라 ③ 단조 하한이 성립했다.
      상세·파일별 교체표: `DECISION_LOG.md` 2026-08-23(MW0601 488차 후속).
- [ ] **앞으로 캠페인 테스트를 새로 쓸 때** — 실 DB 산출값을 `==` 로 핀하지 말 것.
      ① 공식은 합성 입력 ② 실 DB 는 내부 정합(재계산 일치) ③ 단조 하한(등록 시점
      실측 이상 — 고정 시작 누적 채널만) ④ 판정 매핑(사전등록 임계로 재계산해 대조).
      권고 문구 단언은 **갈래 조건과 묶을 것**("얹지 말라"·"유해" 류는 갈래 전용이다).

**하지 말 것**

- 🔴 **가드를 `qty>=2` 까지 켜는 것** — 판정 전에 켜면 이 표본이 더 안 쌓인다
      (사전등록 블록의 경고). 현재는 `mark_stop_tightened_shadow()` 로 계측만 한다.
- 🔴 **439차 서술을 다른 문서에 복사하는 것** — *"431차 이후 `qty>=2` 진입이
      사라졌다"* 는 MW0601 실측과 다르다(08-18 7건 · 08-12 4건 · 08-07 3건).
      431차 재시뮬의 *"2~3계약 15%→37% 재배분"* 이 정본이다.
- 🔴 **파일 편집 시 원본을 `"w"` 로 직접 여는 것** — 이 세션이 그 방식으로
      `generate_validation_campaign_report.py` 를 0바이트로 만들었다(복구 완료,
      실질 손실 0). **임시 파일 → 구문 검증 → `os.replace`** 로 할 것.

---

### 483차 후속5 — 주간회의 ① 결정 반영 (MW0601, 2026-08-23) — **매매 정책 무변경**

> 진입 호라이즌 경계 **동결** + 경보 참고용 강등 + `entry_band_watch` 캠페인 편입.
> 상수는 한 글자도 안 바뀌었다 — 바뀐 것은 "바꾸지 않기로 했다"는 기록의 유무다.

- [ ] **EB-1 (다음 금요일 08-28) 새 스텝이 실제로 도는가** — EOD 로그 요약에
      `라우팅 밴드 성과 [D9-B]=OK` 가 뜨는지. ⚠ **이 채널은 자동 경로에서 판정을 낸
      적이 한 번도 없다**(474차 사전등록 이후 발화 지점 부재). `monthly_cleanup` 과
      같은 유형이므로 첫 자동 실행을 반드시 확인할 것
- [ ] **EB-2 판정이 `BAND_ANOMALY` 로 바뀌는지 감시** — 바뀌면 **동결 재개 조건 충족**이다.
      ⚠ 문턱을 낮춰 판정하지 말 것(458차 D6). 표본 24는 문턱 20의 1.2배로 검정력이 낮아
      `BAND_UNIFORM` 은 "이상 없음이 입증됐다"가 아니다
- [ ] **EB-3 경보 문구 확인** — 08-28 `[EntryHorizonRecal]` 줄 끝이
      `참고용 경보(판정 아님)` 인지. 앞머리 형식은 무변경이어야 한다(로그 파서 계약)

**하지 말 것**

- 🔴 `ENTRY_HORIZON_B1/B2` 를 재보정기 권고값으로 바꾸는 것 — 실제 진입 220 포지션 중
      **79.5%가 재배정**되고, 그중 142건은 유일한 흑자 구간(3m, 계약당 +17,538원)이
      적자 구간(1m, −20,018원) 설정으로 내려간다. 재개 조건 충족 전에는 금지
- 🔴 재보정기의 **버킷비중을 진입 분포로 읽는 것** — 그것은 **전 분봉** 기준이다
      (08-21 전 분봉 5m 86% vs 그날 실제 진입 5m **0건**)

---

### 483차 후속2 — `.git/index.lock` 스테일 잔존 (MW0601, 2026-08-23) — **미륵이 런타임 변경 0건**

> 2026-08-21 08:59:53에 생긴 **0바이트 `index.lock`이 53.5시간** 남아 저장소가 커밋
> 불가였다. 읽기(`git status`)는 rc=0으로 조용히 통과해 **어떤 계측에도 안 걸렸다.**
> 기전·범위·정정 전문은 `dev_memory/DECISION_LOG.md` 483차 후속2.

- [x] **P0-1 인덱스락 상태 병기 + §11 적신호** `[DONE 2026-08-23]` — 3중 조건
      (0바이트 · 나이>10분 · git 프로세스 0개) 전부 충족 시에만 stale 판정
- [x] **P0-2 「커밋 0건」을 "안 했다/못 했다"로 분리** `[DONE 2026-08-23]` (계측 4원칙 ②)
- [x] **P1-1 읽기 전용 git 호출에 `--no-optional-locks`** `[DONE 2026-08-23]`
- [x] **P1-2 `run_git` 타임아웃 시 자식 종료** `[DONE 2026-08-23]` — 종전엔 고아 git이 남았다
- [x] **fuoption 잔존 락 제거** `[DONE 2026-08-23 15:06]` — 제거 후 `.git/index` mtime
      갱신으로 회복 확인. 저장소 7개 전수 재점검 결과 남은 락 0건

**다음 거래일 확인**

- [ ] **L-1 정상 경로 문구** — 다음 장전 다이제스트 §2가 `· 인덱스락 없음` 과
      `(당일 커밋 없음 — 커밋 가능 상태였음)` 으로 나오는가. **§11에 락 항목이 뜨면**
      진짜 잔존이므로 3중 조건을 확인하고 제거할 것
- [ ] **L-2 `--no-optional-locks` 부작용 없음** — §2의 HEAD·브랜치·미커밋 건수가
      종전과 같은 값인가(옵션은 인덱스 쓰기만 막는다). 다르면 즉시 롤백
- [ ] **L-3 장중 점검 소요시간** — 인덱스 쓰기가 사라졌으므로 §2 구간이 느려질 이유는
      없다. 오히려 짧아졌는지 관찰만 한다(판정 아님)

**미구현 이월**

- [x] **P1-3 커밋 경로 프리플라이트** `[DONE 2026-08-23 483차 후속3]` — `scripts/git_lock_guard.py`
      신설(종료코드 0/2/3). git hook 으로는 못 막는다(훅은 락을 잡은 **뒤에** 돈다) —
      `SKILL.md` 커밋 규약에 절차로 박았다. 원안:
- [ ] ~~**P1-3 커밋 경로 프리플라이트**~~ — git 쓰기 전에 3중 조건 검사 → 경고 또는 회수.
      ⚠ **무조건 자동 삭제 금지** — 진짜로 도는 git을 죽이면 인덱스가 깨진다
- [x] **P2-1 형제 저장소 공통화** `[DONE 2026-08-23 483차 후속3]` — 정본 1개 + fuoption
      바이트 동일 사본 + 드리프트 감시 테스트. 수집기의 인라인 판정은 걷어내 정본 호출로
      바꿨다(판정 두 벌이 되면 한쪽만 고쳐져 갈라진다). ⚠ 사본 동일성 테스트는 형제
      저장소가 없는 PC에서 **skip** 되며 **skip 은 "같다"가 아니다**. 원안:
- [ ] ~~**P2-1 형제 저장소 공통화**~~ — 같은 결함이 fuoption에도 났다.
      **fuoption `messiah-daily-check` 수집기에는 이번 4건이 반영되지 않았다**(별도 저장소).
      저장소별 처방이 아니라 점검 스킬 공용 유틸로 갈 것

**재인용 금지 (이번에 만든 수치)**

- ⚠ **"스테일 락이면 status 8배 느림"** — 8.3배는 **실험실 12,000파일** 값이다.
  실측은 fuoption **약 1.5배**(0.131 → 0.085s, 447파일). futures는 783파일.
  실질 피해는 지연이 아니라 **커밋 봉쇄**다
- ⚠ **락은 「미커밋 463건 CRLF 착시」와 무관하다** — 락이 남아 있던 fuoption이 24건으로
  정상이었다. **P2-B를 463 기준으로 설계하지 말 것**(재측정 필요)

---

### 483차 — 0821 리포트 P1-A 구현 후속 (MW0601, 2026-08-23 배포) — **매매 정책 변경 0건**

> 계측 전용. 확인할 것은 하나다 — **15:40 경보의 ConfFloorGuard 3칸이 드디어 실측값을
> 남기는가.** 482차 O-8은 이 수정 전까지 값 자체가 없었으므로 08-21까지의 관측은
> 무효이며 여기서 다시 시작한다.

- [ ] **P1A-1 3칸 부활 확인** — 다음 거래일 15:40 SIGNAL 로그 `[mc-conf gap]`(또는
      `[경보] mc-conf 괴리`) 줄 끝에 `| ConfFloorGuard 도달가능 N분 · 도달불가 M분 ·
      재지않음 K분`. **세 값이 전부 0이면 수정이 회귀한 것이다**
- [ ] **P1A-2 합 = 분모 대조** — `N + M + K`가 그날 파이프라인 분수와 같은지.
      같은 줄에 분모가 없으므로 `[ModelHealth]` 줄의 `파이프라인 N분`과 대조한다.
      **어긋나면 카운트 지점(`main.py:7005`대)이 파이프라인 분모와 다른 자리로 밀린 것**
- [ ] **P1A-3 `재지않음` 비율 첫 해석** — 482차 O-8이 남긴 판정 유보를 이어받는다.
      압도적이면(예상) 앞의 두 수치는 **아직 해석하지 말 것** — 보정기 미fit이 상시라는
      403차 축퇴 가드의 귀결이며 그 자체가 별도 안건이다. **첫날 값 하나로 판단 금지**(313차)
- [ ] **P1A-4 `scaler_daily` 스키마 무변경 확인** — 3키는 로그 전용이다.
      `PRAGMA table_info(scaler_daily)`에 `cfg_*` 컬럼이 생겼으면 잘못 구현된 것이다
      (테스트가 막지만 마이그레이션 코드가 따로 붙으면 뚫린다)

**미적용 이월 (0821 리포트)** — 지시가 있을 때 별도 세션에서 구현한다.

- [x] P2-C + P2-G + P2-J **문서 묶음** `[DONE 2026-08-23 483차 후속1]` — 한 커밋으로 적용
- [x] P2-H `monthly_cleanup.py` stdout UTF-8 강제 `[DONE 2026-08-23 483차 후속4]` —
      두 겹으로 처방: ① `_force_utf8_stdio()` in `main()` 첫 문장 ② `campaign_steps.py` 가
      **12스텝 전부**를 `PYTHONIOENCODING=utf-8` 로 기동(근본 처방). 세 경로 rc=0 검증.
- [ ] **P2H-1 (관측) 2026-08-28 EOD 에서 `월간 로그 정리=OK` 확인** — 그리고
      `data/monthly_cleanup_last_run.txt` 에 `202608` 이 찍히는지. ⚠ **이 스텝이 자동
      경로에서 성공한 적이 아직 한 번도 없다**(2026-05 신설, 08-21 첫 시도 실패).
      실패하면 로그 tail 을 그대로 인용해 새 이상점으로 올릴 것 — 같은 자리인지 다른 자리인지가 핵심.
      ⚠ 08-28 은 `--apply` 자동 실행이다. 08-23 DRY-RUN 계획은 `[1a]` 7번들 28파일
      (2026-07-20~23) 압축 후 원본 정리뿐이고 DB 행 삭제는 비활성이었다 — 결과가 그보다
      크면 그 자체가 점검거리다
- [x] P2-I `trades.exit_stage` 컬럼 신설 (청산 사유에서 손절 ↔ TP1후 트레일 분리) `[DONE 2026-08-24 491차 — F-I 로 구현]`
- [ ] P2-K 수집기 미커밋 건수에 실질 변경 수 병기 (P2-B와 묶어 구현 권고)
- [ ] P2-A · P2-E · P2-F (장전·장중 이월분, 전부 미적용)

---

### 482차 — 0820 리포트 §2·§3 구현 후속 (MW0601, 2026-08-20 배포) — **매매 정책 변경 0건**

> 배포분은 전부 **계측 전용**이다. 다음 거래일(08-21)에 확인할 것은 하나로 요약된다 —
> **새로 붙인 계측이 실제로 값을 남기는가.** FP-CRITICAL이 2개월, TOX-SEVERE-SPREAD가
> 1개월 "죽은 섀도"였던 전례가 있으므로 배선 확인 없이 넘어가지 말 것.

- [ ] **O-1 `[DBG-CB]` 새 필드** — `logs/20260821_DEBUG.log`에 `n=%d/30 ready=Y|N`과
      `resets=`·`dropped=`가 **전 행에** 찍히는가. 장 초반(09:00~)에는
      `acc30m=—(표본없음) n=0/30 ready=N`이 나와야 정상이다.
      `acc30m=0.0%`가 다시 보이면 F-1이 회귀한 것이다
- [ ] **O-2 CB③ ready 분** — EOD `[ModelHealth]`(15:40, SIGNAL 로그) 줄 끝에
      `CB③ ready N분/M분 (P%) (리셋 R회, 표본손실 D건)`이 붙는가.
      `scaler_daily`에도 같은 값이 들어갔는지 대조:
      `select date,cb3_ready_minutes,cb3_buffer_resets,cb3_samples_dropped from scaler_daily order by date desc limit 3`
      ⚠ **08-20 이전 행은 NULL이 정상**이다(미측정). 0이면 마이그레이션이 잘못된 것
- [ ] **O-3 CB③ ready 비율 첫 관측** — G-1의 판정선은 *3거래일 연속 ready < 50%면
      §11 적신호*다. **첫날 값 하나로 판단하지 말 것**(313차). 기록만 한다
- [ ] **O-4 `cb3_triggered` 부활 확인** — CB HALT가 0회인 날은 여전히 0이 정상이다.
      HALT가 나는 날에 1이 찍히는지는 그때 확인한다. 지금 확인할 것은
      `_daily_halt` getattr이 되살아나지 않았는가뿐이며 테스트가 지킨다
- [ ] **O-5 `[MainStall]` 필드** — 블로킹 2초 초과 시 줄 끝에
      `[MainStall] stall_ms=… band=… since_pipe_s=…`가 붙는가.
      `since_pipe_s=NA`가 장 초반에만 나오는지(파이프라인 첫 실행 전) 확인.
      장중 내내 NA면 `notify_pipeline_ran()` 기록 지점이 죽은 것이다
- [ ] **O-6 ALERT 밴드 오탐 0건** — `stall_ms ≥ 15,000` (ERROR)이 정상일에 나오면
      안 된다. 나오면 `MAIN_THREAD_STALL_ALERT_MS` 재검토 대상이며, FZ-1(180초)과의
      간격도 함께 본다
- [ ] **O-7 수집기 §5 CB⑤ 잔차 표** — 5초 초과 건이 있으면 잔차가 계산되는가.
      `**미측정**`이 뜨면 `PipePerf` 짝짓기가 깨진 것이다(`hhmm`이 HH:MM:SS라
      앞 5글자로 잘라야 한다 — 구현 중 실제로 겪은 함정)
- [ ] **O-8 ConfFloorGuard 3상태 첫 실측** — 🔴 **[483차 무효] 이 관측은 값 자체가
      없었다.** 경보가 카운터를 리셋 **뒤에** 읽어 08-20·08-21 둘 다 `0분 · 0분 · 0분`
      이었다(0821 이상점 1-9). 483차 P1-A가 고쳤으므로 **P1A-1~P1A-3으로 다시 관측한다.**
      아래 원문은 판정 유보 취지를 P1A-3이 이어받으므로 보존한다 —
      EOD `[mc-conf gap]` 줄에
      `도달가능 N분 · 도달불가 M분 · 재지않음 K분`. **`재지않음`이 몇 %인가가 핵심이다.**
      압도적이면(예상) 앞의 두 수치는 **아직 해석하지 말 것** — 보정기 미fit이
      상시라는 403차 축퇴 가드의 귀결이며, 그 자체가 별도 안건이다
- [ ] **O-9 수집기 §5 포지션 표 2일차** — 08-21에도 포지션 단위 합계가
      `trades.db` 포지션 합과 일치하는가. 정합성 줄이 `OK · OK`인지.
      **불일치가 뜨면 그날 리포트의 손익 수치를 인용하지 말 것**
- [ ] **O-10 §11 적신호 칸 수** — 브랜치 범위 밖 5칸이 사라졌는지(0820 실측 15 → 11칸).
      다시 15칸이 되면 F-2가 회귀한 것이다

#### 사전등록 판정 대기 / 이월

- [ ] **G-3 2단계 (판정: `재지않음` 비율 5거래일 관측 후)** —
      `ensemble_decisions`에 분당 `conf_floor_state`를 적재해
      `fetch_entry_candidate_gap()`의 5거래일 롤링까지 분리할지 결정한다.
      ⚠ **`재지않음`이 대부분이면 DB 컬럼을 늘려도 아무것도 판정할 수 없다.**
      먼저 비율을 보고 결정할 것 — 순서를 뒤집지 말 것
- [ ] **F-3 2주 섀도 (판정 2026-09-03 이후)** — `[MainStall]` 잔차 분포를 모아
      `MAIN_THREAD_STALL_ALERT_MS`를 실측 기반으로 재설정한다.
      **차단 승격은 그 다음이며 주간회의 안건**이다. 지금 플래그를 만들지 말 것
- [ ] **O-10 문언 폐기 (480차 등록분)** — *"`_tick_header` 5,000ms 초과 1건이라도 나오면
      CB⑤ 실발동 — 사유 추적 필수"* 는 **전제가 틀렸다**. CB⑤는 파이프라인 경과시간에
      걸리고 그 값은 메인 스레드 전체 정지시간이라 단위가 다르다.
      **CB⑤ 미발동이 설계상 옳다.** 482차 F-3 잔차 표가 이 문언을 대체한다
#### 주간회의 안건 (코드로 처리하지 않았다 — 거버넌스 소관)

- [ ] **전환기준 ⑥ 문언 확장 검토** — 0820 리포트 §3 로드맵 표의 제안:
      *"CB③ ready 시간이 장중 50% 이상일 것"* 을 판정 전제로 추가할지.
      482차는 그 값을 **잴 수 있게만** 했고 기준은 건드리지 않았다.
      ⚠ `CLAUDE.md` 실전 전환 기준을 바꾸는 일이라 세션 단독으로 수정하지 말 것.
      먼저 O-3으로 실측 분포를 몇 거래일 모은 뒤 상정한다
- [ ] **F-3 차단 승격 여부** — 2주 섀도 후. 지금은 `MAIN_THREAD_STALL_BLOCK_ENABLED`
      **자체가 없다**(일부러). 승격하려면 소비 코드와 함께 넣고
      `config/dailycheck_targets.json`의 `documented_disabled_flags`에 등록할 것
- [ ] **`ZONE_ENTRY_BAN_SHADOW_ENABLED` 양 PC 배선** — 0820 리포트 1-3에서 등록된 안건.
      482차 F-2는 **수집기가 브랜치를 구분하게** 했을 뿐, 5개 상수를 v9-dev에
      이식할지는 별개다. 462차 "집행과 무관하게 위반 계측은 항상 켜져 있어야 한다"가
      MW0601에서 성립하지 않는 상태는 그대로다

#### 기타

- [ ] **`accuracy_30m` 소비자 이관 (선택, 우선순위 낮음)** — 현재 float + `_measured`
      동반으로 원칙 준수는 끝났다. 소비자 5곳(`main.py:6227`·`7724`·`10592`·`10607`,
      `dashboard/main_dashboard.py:5101`)이 `_measured`를 함께 읽게 할지는 별건이다.
      **지금 `None` 반환으로 바꾸지 말 것** — 회귀 위험만 늘고 얻는 것이 없다

---

### 480차 — 0819 리포트 §2·§3 구현 후속 (MW0601, 2026-08-19 배포) — **매매 정책 변경 0건**

> 다음 거래일(08-20) 라이브에서 확인할 것. 전부 **관측**이며 임계는 지금 고정한다.

- [ ] **O-1 하트비트 파일 생성** — `data/heartbeat_MW0601_20260820.json`이 08:40 기동 직후
      생기고 30초마다 갱신되는지. `beat_age_sec`가 항상 **5초 안팎**(5초 QTimer)인가.
      `null`이 계속 찍히면 `_main_beat` 갱신 지점이 죽은 것이다(FZ-1 회귀)
- [ ] **O-2 F-2 사이드카 기동** — 런처 로그에 `[GUARD-FLAT] 15:12 FLAT 가드 사이드카 기동`
      1줄 + 15:12에 `logs/force_flat_guard_20260820.log`에 판정 1줄.
      **정상 = `OK` (포지션 FLAT · 프로세스 생존)**. `UNKNOWN`이면 하트비트 경로가 안 맞는 것
- [ ] **O-3 F-2 오탐 없음** — `data/force_flat_alert_20260820.txt`가 **생기지 않아야** 정상.
      생겼는데 실제로는 정상이었으면 `FORCE_FLAT_GUARD_HEARTBEAT_STALE_SEC` 재검토
- [ ] **O-4 `[PositionFallback]` 미출현** — 정상일에는 0건이어야 한다. 뜨면 그 포지션이
      TP1 배수 폴백으로 열린 것이므로 `ensemble_decisions.meta_gate_horizon`과 대조할 것
- [ ] **O-5 `trades.entry_horizon` 공란 0건** — F-5 적용 후 자동진입 레그에 공란이 남으면
      승계 경로가 또 있는 것이다(현재 커버: `open_position` + `apply_entry_fill` 2호출부)
- [ ] **O-6 수집기 §7 새 표** — "로그 종료시각 — 직전 5거래일 대조"가 장후 다이제스트에
      뜨고, 정상일 델타가 **±5분 이내**인가. 08-17(17:58)처럼 늦게 끝난 날이 중앙값에
      섞이면 기준선이 흔들리므로 관측할 것
- [ ] **O-7 EOD 마커** — `data/eod_retrain_done_20260820.txt`에
      `daily_close_seen: true` / `wait_dc_timeout: false`가 찍히는가(정상일 기대값)

#### 사전등록 판정 대기

- [ ] **F-3 5거래일 관측 (판정 2026-08-26 이후)** —
      `python scripts/ofi_norm_distribution_probe.py --from 2026-08-03`
      현재 축소 배율 **×1.55(보류 구간)**. 최근 5거래일이 전부 08-14 이후로 채워지면
      배율이 ×2를 넘을 가능성이 높다. **판정문·임계를 사후에 움직이지 말 것**(458차 D6).
      ⚠ ①(수집)로 판정되면 다음 조사축은 **OFI 계산 입력(틱 수집량·호가 갱신 빈도)**이지
      CORE 지정이 아니다 — CLAUDE.md 절대원칙 §3은 이 조사 단독으로 바뀌지 않는다
- [ ] **FZ-1 라이브 왕복 리허설 — ⚠ 사용자 실행 필요.** 전환기준 ②의 선행 ⓑ.
      `python scripts/freeze_watchdog_rehearsal.py --live-instructions`.
      F-1R(15:10 리허설)보다 **먼저** 하는 것이 순서다 — F-1R이 프로세스 생존을 전제한다

#### 다음 세션 착수 후보

- [ ] **G-3 진입 3경로 공통 헬퍼 (P2로 강등 검토)** — 선행 F-5①② 완료, G-5(AST 테스트)는
      `tests/test_480_entry_param_completeness.py`로 대체 완료. 남은 편익은 `[Position] 진입`
      로그 형식 통일뿐이므로 **라이브 진입 경로를 건드릴 값어치가 있는지 먼저 판단**할 것
- [ ] **F-2 2단계(브로커 직접 청산) 여부 — 주간회의 안건.** 이중 청산·수량 불일치 위험이
      있어 1단계는 알림 전용으로 두었다. 승격하려면 라이브 프로세스와의 배타 제어
      (락 파일 또는 브로커 잔고 재확인)를 먼저 설계해야 한다
- [ ] **F-6(STEP 1 채점 커버리지) · G-7(IntradayRegime 플랩)** — 478차 장중분 미착수.
      이번 세션 범위(§2·§3) 밖이었다
...
**다음 거래일 (F-8 Phase B 배선 게이트) — [DONE 2026-08-20] 3건 전부 통과**

> 검증 데이터: 2026-08-18(첫 거래일) · 2026-08-19. 개장 전 07:50 읽기전용 조회.
> 배선 이후 475~480차가 진행됐으므로 **그 후속 차수들이 배선을 깨지 않았다**는 확인도 겸한다.
>
> | date | n | spread_ticks | shadow | sizing_trace | 로그 구간 |
> |---|---|---|---|---|---|
> | 08-13 | 370 | 0 | 0 | 0 | 배선 전 |
> | 08-14 | 369 | 0 | 0 | 0 | 배선 전 |
> | **08-18** | 370 | **370** | **370** | **59** | 09:00~15:09 |
> | **08-19** | 282 | **282** | **282** | **8** | 08:59~13:40 |

- [x] ~~**F1** — `ensemble_decisions.spread_ticks` 적재 확인~~
      **[DONE 2026-08-20] 통과 — 08-18 370/370 · 08-19 282/282 = 100%.**
      섀도도 실제 발화했다: 08-18에 `spread_extreme_shadow=True` **1건**
      (spread 28.003틱). "죽은 섀도"가 살아났다는 실증이다.
- [x] ~~**F2** — 컬럼 × JSON 대조~~
      **[DONE 2026-08-20] 통과 — 652행 전량 대조, 값불일치 0 · 플래그불일치 0.**
      원본 `features` JSON이 살아 있어 가능한 검증이며, `sizing_trace`는 이 대조를
      할 수 없다(원본 없음). 새 컬럼을 만들 때 원본을 남기는 설계의 값어치.
- [x] ~~**F3** — `sizing_trace` 0행 여부~~
      🔴 **[DONE 2026-08-20] 해소 — 그리고 473차의 경고 전제가 틀렸다.**
      473차는 *"471차 `sizing_trace`가 배포 후 0행이니 F-8도 같은 방식으로 죽을 수
      있다"* 를 F1의 근거로 삼았는데, **배선이 죽은 게 아니었다** — 배포일(08-14)이
      장 마감 후였고 그 다음이 휴장(08-17)이라 **거래일이 없었을 뿐**이다.
      08-18부터 59행/8행 정상 적재. **471차 패턴은 정상 작동한다.**
      ⚠ 교훈은 남는다: "배포 후 N행"을 볼 때는 **그 사이에 거래일이 있었는지**를
      먼저 확인할 것. 거래일 0인 구간의 0행은 결함 증거가 아니다.

**부수 관측 (조치 불요)**

- 08-19가 13:40에 끊긴 것은 배선 문제가 아니다 — `logs/20260819_SYSTEM.log`가
  **13:41:21**에 멈췄고 480차가 등재한 동결 사고(메인 Qt 스레드 네이티브 스핀)와
  일치한다. 그날 로그는 17:02:49 자동 종료까지 뛴다.
- ⑨ 표본은 거의 안 늘었다 — 대조군 195→206, **처리군(>=20틱 진입)은 6포지션 그대로**.
  적립 0.091건/거래일로 ETA 여전히 7개월대다. 473차 결론("기다려도 닫히지 않는다")이
  이틀치로 재확인됐다 → F4는 그대로 살아 있다.
...
### 458차 — 0812 이상점 3건 조치 배포 후속 (MW0601, 2026-08-12 배포)

> 상위: `docs/정기점검/매일점검/MW0601-20260812-이상점3건-딥다이브.md`
> 배경: `DECISION_LOG.md` 2026-08-12(458차). 전체 293 passed. **매매 정책 변경 0건.**

**다음 EOD (P1-A / P1-B / P1-C 게이트)**

- [ ] **D1** — `[GuardClean]` 로그가 6/6 호라이즌에 뜨는지. 형식:
      `깨끗한꼬리 new=… vs old=… | 격차=… | n=…`
      ⚠ **n이 0이면** 현행 cutoff가 홀드아웃 전체를 덮은 것이다(장중 재학습이
      15:0x까지 돌았거나 홀드아웃 폭이 짧아진 경우) — 그날은 표본 적립 0이다.
      실측 기준선: **159봉/일**(08-12).
- [ ] **D2** — `guard_shadow_log`의 `clean_new/clean_old/clean_n`이 NULL이 아닌지.
      `SELECT date(ts),horizon,clean_n,clean_new,clean_old FROM guard_shadow_log
       WHERE source='eod' ORDER BY ts DESC LIMIT 6`
- [ ] **D3** — `[ModelLive]` 1줄 + `live_acc/live_n` 적재 확인. 08-12 기준선:
      1m .3496(n=369) / 3m .3510 / 5m .2808 / 10m .3194 / 15m .2128 / 30m .2206.
- [ ] **D4** — ⑤안 교체 로그에 `최신OOS=` 가 함께 찍히는지. cv와 12pp 갈리면
      그 자체가 관측 대상이다(08-12 3m: cv .4215 / OOS .3000).
- [ ] **D5** — `[FeatureReg]` 로그 끝에 `| 만성도: …` 가 붙는지.
      첫날은 `최초관측`이 정상이고, **둘째 날부터 `2거래일째`** 로 올라가야 한다.
      `data/feature_exclusion_state.json` 생성 여부도 함께 확인.

**표본 누적 대기 (P1-A 판정)**

- [ ] **D6** — `EOD_GUARD_CLEAN_TAIL` 도달 시 주간회의 상정:
      **min_days=10 且 min_bars=1350** (159봉/일 기준 약 9거래일 = 2026-08-26 근방).
      도달 전까지 `EOD_GUARD_VERDICT_SOURCE`는 **`acc_txt` 유지**.
      판정 내용: ① clean 기준 판정과 acc.txt 판정의 불일치율 ② `guard_fair`
      전환 여부 ③ 456차 전환조건 ①(달성 불가)의 **공식 폐기**.
      ⚠ **`min_days`·`min_bars`를 낮추지 말 것** — 1일 CI ±7.3pp / 5일 ±3.3pp로
      가드 허용폭 2.5pp를 분해하지 못한다. 사전등록 위반이다(457차 G4 동일 규율).

**B1 재조사 (G9가 회부 — 457차 규칙대로)**

- [x] ~~**D7** — 선로드 워커가 메인 파이프라인을 막는 문제~~
      **[DONE 2026-08-12] ②안(시각 이동) 구현 완료.** `MODEL_RELOAD_QUIET_WINDOW`
      (target=:15 / latest_start=:45 / max_wait=45s), 롤백 `enabled=False`.
      스폰 :01초 → 14.0s 대기 → 시작 :15.0 → 2.1s 로드 종료 :17.1(양쪽 파이프라인 회피).
      `(S0 비블로킹)` 로그도 대기·시작초 병기로 정정. 15 tests.
- [ ] **D7-G (다음 거래일 게이트)** — **선로드 분의 S0 < 200ms 인가.**
      이것이 B1의 진짜 합격선이다(457차가 세운 목표 그대로).
      · 로그 단서: `[Model] 워커 선로드 완료 NNNNms (대기 N.Ns, 시작 :15초)`
      · **시작 초가 :15가 아니면** 스폰 시각 가정이 깨진 것 → `target_sec` 재검토
      · 함께 볼 것: 그 분의 **S1·S4·S5·S6가 평상시로 돌아왔는가**(19/56/111/29ms).
        S0만 내려가고 나머지가 여전히 높으면 원인이 GIL 경합이 아닌 다른 것이다.
      · **CB⑤ 경고 0건 · Degraded `cause=S0` 0건**이 목표(08-12: 경고 5 / S0 3건).
      ⚠ 아직 발생하면 ①안(별도 프로세스, GIL 회피)으로 승격 검토.

**다음 거래일 (P3 게이트)**

- [ ] **D13** — `loss_tier1_qty1_shadow.elapsed_sec`가 NULL이 아닌 신규 행이 생기는지.
      tier2 행이 생기면 `ts`가 `:00`이 아니라 **초까지** 찍히는지도 함께 확인.
      ⚠ 기존 35행은 NULL이다 — 소급하지 않았다. 이 축 집계에서 **제외**할 것
      (NULL을 0으로 읽으면 "즉시 발동" 구간이 통째로 오염된다).
- [ ] **D14** — `loss_tier1_elapsed_watch` 표본 적립(min_samples=25 · min_days=6,
      `data_start=2026-08-13`). 도달 시 주간회의에서 **① 구간 분할 ② 합격선을 먼저
      정하고 그 다음 집계**한다 — 지금 채널에 문턱이 없는 것은 의도다(사후적합 방지).
      ⚠ 표본이 답답하다고 문턱을 데이터 본 뒤에 정하지 말 것.

**주간회의 상정 (미결)**

- [ ] **D8 — P2-B 피처 온보딩 절차 신설.** 97→N 슈퍼셋 확장. need_add로 잠긴
      피처가 **11종**이고, 455차 배터리가 앞으로 발굴할 피처도 전부 이 문에서 막힌다
      (= B-4-1 처방의 배포 통로가 없다). 요건: ①확장 스크립트(합집합+순서보존+diff)
      ②전 호라이즌 EOD 전체 재학습(py310_64, 비거래일) ③스모크(ShapTracker shape
      337차 · `_restore_analysis_buffers` 332차 · P8 ScalerRefresh · Canary)
      ④**양 PC 동시 적용** ⑤기존 97 스냅샷 롤백 경로.
- [ ] **D9 — F9(30m CORE 처분).** `CLAUDE.md` §3에 상태는 명기됨(P2-A). 선택지:
      강등 / 온보딩(D8 선행) / 장기 그룹 자체 재정의. 근거: IC −0.0066(n=209),
      장기 그룹 체크리스트 발동 **사상 0건**, 296차 앙상블 퇴역.
- [ ] **D10 — 455차 배터리 미드호라이즌 조기 재실행 여부.** 정규 주기는 26주
      WFA 편입이나, P(적중|DN예측)=베이스레이트(무정보) 실측이 앞당길 근거가 된다.

**하지 말 것 (근거와 함께 기각됨 — 재시도 금지)**

- ❌ **A1 후보 ①②③을 정확도 회복 수단으로 재시도** — 30거래일 walk-forward
      카운터팩추얼에서 **기각**(이득 ±0.8pp 잡음, DN 예측 오히려 증가).
      편향은 절편이 아니라 **조건부 구조**에 있다. (③은 conf 스케일 위생 목적으로만 존치)
- ❌ **방향 반전** — 457차 기각 재확인.
- ❌ **SHORT 감쇠 게이트 신설** — G4 사전등록 채널의 누적 판정을 기다린다.
- ❌ **opt_chain_pcr 긴급 온보딩** — IC 무정보, 서두를 근거 없음(D9로 회부).
- ❌ **홀드아웃을 cutoff 이후로 이동 / T+1 지연 판정 / 장중 cutoff 앞당김** —
      딥다이브 §A-5에서 각각 기각(검정력 붕괴 / 통계력 동일한데 비용 큼 / 적응성 훼손).
- ❌ **"Purged CV 재사용으로 홀드아웃 재설계"(오전 리포트 §5 P1) — 전제가 틀렸다.**
      CV는 **454차 ATB-A1a부터 이미 퍼징돼 있다**(`_run_cv_folds`의 `train_idx[:-h_min]`
      + LeakGuard). 그런데도 cv가 최신 OOS 대비 3m −12.2pp 부풀려지므로 원인은
      라벨 누출이 아니라 **레짐 혼합**(폴드가 26주를 섞음)이다. 퍼징은 지렛대가 아니며,
      올바른 처방은 이미 배포된 **P1-A + P1-C**다. 미구현 안건으로 되살리지 말 것.

**MW0602 대조 필요**

- [ ] **D11** — P1-A·P1-B·P1-C·P2-C는 **공통 코드**다. 그쪽도 동일하게 6/6
      ghost_bypass 상태일 것이다. pull 후 EOD 로그·`guard_shadow_log` 확인.
- [ ] **D12** — D8(97→N 확장)은 **양 PC 동시 적용이 요건**이다. 일정을 묶을 것.

---

### 457차 — 0811 Fix 파동 W-A~W-E 배포 후속 (MW0601, 2026-08-11 배포)

> 상위: `docs/정기점검/매일점검/MW0601-20260811-점검리포트.md` §2-1
> 배경: `DECISION_LOG.md` 2026-08-11(457차)

**다음 거래일 장중 (W-B / W-E 게이트)**

- [ ] **B1** — 장중 재학습 직후 `[PipePerf]` S0 < 200ms 인지. 종전 2,979~4,381ms.
      로그 단서: `[Model] 워커 선로드 완료 NNNms — 메인 스레드 스왑 대기`
      → 다음 분 `[Model] 재학습 완료 모델 교체 적용 (S0) — 스왑 N ms`
- [ ] **B2** — `[S6Detail]`의 `shap=` 이 매분 0~5ms인지(종전 460~521ms).
      S6 중앙값 < 100ms · **CB⑤ 0건**이 목표.
- [ ] **B1+B2 부작용 점검** — 워커가 겹쳐 스킵되는 빈도.
      `[SHAP] 직전 워커 진행 중 — 이번 분 스킵`이 하루 수십 건이면 라운드로빈
      갱신 주기가 늘어난 것이므로 SHAP_SLOW_* 임계 재검토.
- [ ] **W-E** — `ensemble_decisions.meta_gate_horizon` 분포가 `[ATR-Horizon]` 로그
      분포와 일치하는지(08-11 기준 3m 82% / 5m 10% / 1m 8%).
      **370/370건 '1m'** 이 재현되면 C3 수정이 실효 없는 것이다.
      ⚠ 이 컬럼은 의미가 바뀌었다 — 캠페인 [2]의 08-11 **이전 데이터와 혼용 금지**.

**다음 EOD (W-A / W-C 게이트)**

- [ ] **C5** — `daily_stats`의 `verified_count > 0` 그리고 `sgd_accuracy ≠ 0.5`.
      ⚠ 과거 8행은 소급 수정하지 않았으므로 추이 패널에 **불연속선이 생기는 것이 정상**.
- [ ] **C6** — `[FeatureReg] 30m: 7개 피처 미가용` 뒤에 **7개 전량**이 찍히는지
      (종전 5개만). 은닉됐던 2개가 무엇인지 확인해 F9(30m CORE 정정) 입력으로 쓸 것.
- [ ] **C1** — `[GuardFair]`가 "학습행 타임스탬프 미상"이 아니라
      **정량 오염 메시지**(`오염 홀드아웃 1850봉 중 N봉(P%)…`)를 내는지.
      로그 단서: `[Retrain] 주입 X와 직전 DB 로드 행 수 일치(N) — 학습행 타임스탬프 유지`
      ⚠ 정상화되면 **진짜 오염이 드러날 가능성이 높다**(intraday 4,800봉 ⊃ 홀드아웃
      1,850봉). 그 경우 판정 보류는 **정당한** 보류이고 C2가 유일한 출구다.
- [ ] **C2** — **3m EOD 교체가 성립하는지**(7거래일 연속 보류의 해소).
      로그 단서: `[GuardGhost] 3m 가드 미적용 — 현행 배포본이 CV 미검증 intraday 모델…`
      `guard_shadow_log.verdict_source = 'ghost_bypass'` 행이 남는지도 확인.
- [ ] **C7** — 장전 08:55에 `🌡 Canary 이상 감지` 알림이 **재적합으로 해소되는 경우
      발송되지 않는지**. 판정 로그는 종전대로 남아야 한다(억제 대상은 알림뿐).
- [ ] **C8** — 08:41 `[WarmupRetrain] 장전/장외 기동 … 08:55 PreRetrain에서 소진 예정`
      → 08:55 `[PreRetrain] … | WarmupRetrain 예약 소진` 연결이 보이는지.

**W-C +5거래일 (W-F)**

- [ ] **A2** — 3m ConstOut 발생 횟수 재측정. 목표 ≤ 2회/일(5거래일 평균, 종전 6회/일).
      5거래일 후에도 3일 이상 발생하면 **모델 자체 문제**로 승격해 `gbm_3m`
      하이퍼파라미터·피처셋(97→12 슬라이싱)을 별건 조사.

**표본 누적 대기**

- [ ] **A3** — `cost_edge_watch` 사전등록 채널. `data_start=2026-08-12`,
      min_samples=40 · min_days=10 도달 시 판정.
      실행: `python scripts/cost_edge_shadow.py --days 30`
      ⚠ 소급 참고치는 이미 `REJECTS_HYP` 방향(rho=0.156 t=0.96, 3분위 격차 −2.4%p) —
      그 경우 처방은 게이트 신설이 아니라 **분위회귀 재설계**이고, 고도화 **G6의
      전제가 무너진다**. 주간회의에 G2-C와 함께 올릴 것.

**주간회의 상정 (미결)**

- [ ] **A1 조치안 — 절편 보정 축**. 진단은 완료(원인 = 모델 절편, 라벨 아님).
      후보 ① `make_sample_weight` 클래스 가중 재산출 ② 호라이즌별 독립 Platt
      ③ 앙상블 보정기 축퇴(403차 가드) 해제 조건 재검토.
      ⚠ **방향 반전은 후보가 아니다** — 역방향 검정에서 적중률이 떨어졌다
      (3m 0.347→0.337). 재확인: `python scripts/direction_bias_diagnosis.py --days 60`
- [ ] **G4** — 방향 편향 상시 감시 채널 신설 여부. 오늘 8:1 편향을 DriftDetector·
      Brier·CB③ 어느 것도 잡지 못했다. A1 하네스의 상시화 버전.
- [ ] **G7** — 진입 호라이즌 라우터에 모델 건강도 결합(ConstOut 활성 호라이즌 제외 +
      F6 사이드카 `cv_acc=null` 감점). F10-A와 동일 배치 여부.
- [ ] **G8** — 계측 규약 ④ "폴백 가시화" `CLAUDE.md` G3-A 확장 승인.
      457차 하루에 같은 패턴 4건(C3·C4·C5×2). 자동 검출 가능:
      `grep -n 'getattr(self, "_' *.py` 로 할당부 없는 속성 색출.

**MW0602 대조 필요**

- [ ] **C1·C3·C4·C5는 공통 코드** — 그쪽도 동일 결함이다. 배포 전 대조할 것.
      특히 C3(`meta_gate_horizon` 오조인)은 캠페인 [2] 채널 데이터에 영향.
- [ ] **G8 검출기를 MW0602에서도 돌릴 것** — `pytest tests/test_457_fallback_visibility.py`.
      같은 패턴(할당부 없는 getattr 폴백)이 그쪽에만 있을 수 있다.

---

### 457차 후속 — 고도화 G4~G9 배포 후속 (MW0601, 2026-08-11 배포)

> 상위: `docs/정기점검/매일점검/MW0601-20260811-점검리포트.md` §3-0
> 배경: `DECISION_LOG.md` 2026-08-11(457차 후속)
> ⚠ 전부 **계측·규약**이다 — 매매 정책 변경 0건. 축소 결정 3건(G6·G7·G9)의 근거는
>   DECISION_LOG에 있으니 "왜 안 했나"를 다시 묻기 전에 그쪽을 볼 것.

**다음 거래일**

- [ ] **G5** — EOD 로그에 `[ModelHealth] date=… 앙상블유효가동률=…` 1줄이 뜨는지.
      `scaler_daily`의 7개 신규 컬럼이 채워지는지(NULL/0이면 배선 실패).
      8/11 기준선 **74.3%** 대비 추이를 볼 것.
- [ ] **G7** — `router_health_shadow` 신규 행 발생 확인. 특히 `chosen_degraded=1`
      빈도. 로그 단서: `[RouterHealth] 라우터가 ConstOut 활성 호라이즌 선택 …`
      ⚠ 8/11 실측은 **0건**이었다. 며칠 계속 0이면 `degraded_rate_rejects`(2%)에
      걸려 REJECTS_HYP로 종결되는 것이 정상 — 코드를 늘릴 이유가 없다는 뜻이다.
- [ ] **G9** — `[HealthPolicy] Degraded 선제차단: … | cause=SN(NNNms)` 태그 확인.
      **핵심 관측점: B1 배포 후에도 Degraded가 발생하는가.**
      · 발생 0건 → G9 정책 분기는 **불필요**로 종결(관측 태그만 유지)
      · 발생하는데 `cause=S0` → B1이 부족한 것 → 재조사
      · 발생하는데 `cause=S1/S4` → 외부·DB 기인 → 그때 분기 정책을 주간회의로

**다음 금요일 EOD**

- [ ] **G4** — 캠페인 리포트에 `| [50] 방향 편향 상시 감시 |` 행이 뜨는지.
      ⚠ 현재 캠페인 창(26거래일)에서는 `PASS`다(3m만 생존, 문턱 2 미달).
      A1의 60거래일 진단은 3m·5m 생존이었다 — **창이 짧아 검정력이 낮은 것**이지
      편향이 사라진 것이 아니다. **결과가 답답하다고 `alert_min_horizons`나
      `t_threshold`를 낮추지 말 것** — 사전등록 위반이다.

**주간회의 상정**

- [ ] **G6 → A3 판정 후 결정.** `cost_edge_watch`가 `REJECTS_HYP`로 확정되면
      처방은 게이트 신설이 아니라 **분위회귀 재설계**다. G2-C(손익비 명시화)와
      **함께** 올릴 것 — 둘 다 "무엇을 근거로 진입 크기·자격을 정하는가"의 두 면이다.
- [ ] **G7 정책 승격 여부** — `router_health_shadow` min_samples 60·min_days 10 도달 후.
- [ ] **G8 규약 추인** — `CLAUDE.md` "계측 4원칙" 신설분을 주간회의에서 정식 추인.
      456차 G3-A가 계획서에만 있고 CLAUDE.md에 승격된 적 없던 것을 이번에 해소했다.

---

### 456차 — Wave 0 배포 후속 + Wave 1~5 대기 (MW0601, 2026-08-10 배포)

> 상위: `docs/정기점검/매일점검/0810_Fix_고도화_통합구현계획_MW0601.md`
...
- [x] ~~**Wave 3 (F6+F7)**~~ **[DONE 2026-08-10] 배포 완료 — 판정 동작 무변경.**
      ⑤/⑥ 택일을 폐기하고 **3안**(F6로 GuardFair를 정직하게 만든 뒤 판정 소스를
      그리로 이전)으로 재구성. `EOD_GUARD_VERDICT_SOURCE="acc_txt"` 기본값 유지.
      26주 실데이터 E2E로 오염 100%(1850/1850) 실측 확인. 전체 228 passed.
      상세: `DECISION_LOG.md` 456차 Wave 3.
- [ ] 🔴 **[F6] 다음 EOD가 첫 실측** — 오늘까지는 사이드카가 없어 오염이 `-1`(미상)이다.
      ① 내일 EOD가 `gbm_{h}_meta.json` 6개를 만드는가
      ② **모레 EOD부터** `fair_contaminated_bars`가 실제 값으로 찍히는가
      ③ 첫날 `[GuardFair] 판정 불가 — 오염 …` 6/6은 **정상**이다(놀라지 말 것)
      ④ `[GuardGhost]` 경고가 3m에 뜨는가(4거래일 연속 유령 비교 중이었다)
- [ ] 🔴 **[F6 선결 판단] 오염이 영구적일 수 있다** — intraday 재학습(하루 4회,
      n=4,800봉)이 홀드아웃 1,850봉을 **항상 포함**한다. 그렇다면 전환 조건 ①
      ("오염 0봉 5거래일")은 **영원히 안 찬다.**
      → 5거래일 관측 후 무작정 기다리지 말고 다음 중 하나를 먼저 결정할 것:
        **(가)** `holdout_bars`를 intraday 학습창 **밖**으로 이동(예: 4,800봉 이전 구간)
        **(나)** EOD 직전 intraday 재학습 스킵
        **(다)** intraday 모델을 EOD 비교 대상에서 제외하고 직전 EOD 모델과 비교
- [ ] **[F7] 판정 소스 전환은 별도 커밋** — 조건 ① 오염 0봉 5거래일 연속
      ② fair_verdict이 6/6 HOLD로 쏠리지 않을 것 ③ 주간회의 승인.
      `EOD_GUARD_VERDICT_SOURCE`를 `"guard_fair"`로.
- [ ] **[F6/F7] MW0602 대조** — 공통 코드.
- [x] ~~**Wave 4 사전등록 + 계측/리포트/테스트**~~ **[DONE 2026-08-10] 배포 완료.**
      3제안이 전부 기존 채널과 중복이라 **신규 채널이 아니라 하위 축**으로 붙였다
      (승인 수치는 그대로): [TP1-M]/[17-Q]/[36-P]. 라이브 동작 무변경, 244 passed.
      상세: `DECISION_LOG.md` 456차 Wave 4.
- [ ] 🔴 **[`_spearman` 정정] 계측 불연속 — 경계 2026-08-10.** 동률 처리 버그를 고쳤다
      (서수순위 → 평균순위). **이 헬퍼를 쓰는 기존 채널의 rho가 바뀐다** — 이산
      소범위 축(수량·pass_count·플래그)일수록 크게. 이 날짜를 걸치는 rho 비교는 무효.
      과거 리포트의 rho를 인용할 때 반드시 경계를 확인할 것.
- [ ] **[17-Q] qty>=2 병목 관찰** — 1.3건/일 → 20건까지 약 3주(09-01 경).
      ⚠ **미달이어도 min_samples를 낮추지 말 것**(사후 조작). 채널 설계를 재검토한다.
- [ ] **[17-Q] 🔴 사전관측 채널** — 첫 관측치를 등록 전에 봤다(qty1 −0.3162 /
      qty2 −0.6875). 최초 판정은 "이미 본 결과의 코드화"이며 가치는 **재현**에 있다.
- [ ] **[36-P] 08-11부터 적립** — 오늘까지 C→A 42건은 pass_count 미기록(소급 불가).
      내일 첫 진입에서 `trades.checklist_pass_count`가 채워지는지 확인할 것.
- [ ] **[TP1-M] arm_ts 표본** — 08-07 이후 14건/2일. 40건·10일까지 약 2주.
- [ ] **MW0602 대조** — `_spearman` 수정은 공통 코드다.
...
직접 호출한다 → `Library\bin`이 PATH에 없어 **모든 BLAS 호출이 프로세스를 즉사**
...
- [ ] **조치 택1** (①권장)
      ...
      ② `retrain_eod.py` 진입점에서 `os.add_dll_directory(...Library\bin)`
...
### 444차 — MW0602 교차검토 후속 (MW0601, 2026-08-08)

- [ ] **🔴 `[47]` 번호 충돌 정리 — 주간회의 안건** — 리포트에 `[47]`이 둘이다
      (MW0602 435차 *시뮬 재현충실도 게이트* / MW0601 436차 *진입 맥락별 손익*).
      444차는 상호 참조 고지만 넣고 **번호는 안 바꿨다** — 두 PC·과거 문서가
      공유하는 식별자라 일방 변경이 더 위험하다. 파생 `[47-B]`는 **게이트 쪽**이다.
      ⚠ 재정리 시 `settings.py` 사전등록 주석·DECISION_LOG·양 PC 리포트를 함께 볼 것.
- [ ] **MW0602에 443차(`103acfb`) 전달** — 그쪽 0808 §6의 "[25]는 여전히 미이관"은
      만료됐다. pull 후 [25]/[25-B]를 자기 데이터로 교차검토하면
      **`atr_lock_0.75` 강화(+6.86→+12.62)와 `breakeven` 붕괴(−10.22→−0.08)가
      재현되는지**가 핵심 확인점이다(후자는 432차 ④의 근거를 흔든다).
- [ ] **[25-B] 최선 변형 변경의 교차확인** — `tp2_2.00` → `tp2_1.25`. MW0601 단독
      33건/4일이라 313차상 확정 불가. MW0602 표본과 합쳐야 판단 가능.
- [ ] **PC간 잔차 방향이 반대라는 사실 기록해 둘 것** — [47]에서 MW0601은
      시뮬<실제(+18.85 vs +30.63), MW0602는 시뮬>실제(+29.32 vs +13.12). 잔차를
      **계통 편향으로 읽지 말 것**(441차 "pre 세대 −11.78pt"도 마찬가지).
...
### 436차 — [47] 진입 맥락별 손익 판독 (MW0601, 2026-08-05 신설)

사전등록 완료(S1 09시대 / S2 반대방향 재진입 / S3 동일방향 ≤5분, min_samples=25
**슬라이스별**, min_days=10, `oos_start=2026-08-06`, Bonferroni α=0.01667).
**라이브 무변경** — 오프라인 재생 전용.

- [ ] **⚠ 이 채널은 "관찰을 믿지 말자"가 목적이다** — 전체표본에서 셋 다
      **보정 이전에도 유의하지 않다**(p=0.3329 / 0.3308 / 0.0849). 발견된 그
      표본에서조차 우연과 구분되지 않는다. OOS에서 뒤집힐 것으로 기대하지 말 것.
- [ ] **OOS 표본이 차면 판독** — 슬라이스별 n≥25가 필요해 S1(09시대)이 가장 늦다.
      진입 페이스상 **수 주** 걸릴 수 있다. 서둘러 판정하지 말 것.
- [ ] **부호 일관성은 남아 있다** — 세 창(06-10/07-01/07-14) 모두 평균차 음수다.
      그래서 폐기가 아니라 등록이다. 다만 **크기가 유의하지 않다**는 게 현재 사실.
- [ ] **⚠ FAIL이어도 진입 차단은 처방이 아니다** — 427·428차(방향 축 무작위) +
      감사 §2-2 ③(차단 게이트 과다). 사이징/집행 축에서 찾을 것.
      **S2가 페이오프 축에서만 유의하면 그건 P2(집행) 안건으로 넘길 것.**

**원 보고(0805) 수치 정정 — 재인용 금지**

- 🔴 "S1 -23.90pt / S2 -45.18pt / S3 -15.78pt"는 **포지션합 pt**이며 계약수가 섞여
  있다(417차 교훈). 계약당으로는 -0.879 / -0.743 / -1.169이고 **S2는 -0.000**으로
  효과가 사실상 사라진다.
- 🔴 "S2는 **승률이 평균 이상**인데 손실만 크다 — 방향이 아니라 페이오프 문제"는
  **07-01~ 창에서만 성립**한다. 06-10~ 기준 승률 51.5% vs 보완 57.3%로 **오히려 낮다.**
  이 해석을 근거로 P2와 묶는 논의는 OOS 확인 전까지 보류할 것.

---

### 435차 — P3 운영 위생 배포 후속 (MW0601, 2026-08-05)

- [ ] **SHAP 슬로우 스로틀 발동 확인** — `[SHAP] 슬로우 감지 ...ms — 다음 5분 건너뜀`이
      찍히는지. 08-05 실측 기준 하루 3건(≥1000ms) 예상. **하루 10건을 넘으면**
      임계(900ms)가 너무 낮은 것이니 올릴 것 — SHAP 갱신이 과하게 희박해진다.
- [ ] **CB⑤ 발동이 사라지는가** — 08-05는 12:34 1건(원인 shap=4481ms). 스로틀은
      **첫 스파이크는 못 막는다**(사후 감지) — 연속 발생만 억제한다. 단발로 또
      발동하면 아래 "사용자 결정 대기" 항목을 재론할 것.
- [ ] **DailyClose 마커 경보가 INFO로 내려갔는가** — 15:40 daily_close 로그에서
      `EOD 재학습 마커 없음 — 정상 (스케줄러 15:45 예정...)` INFO 확인.
      **WARNING이 그대로면** `EOD_RETRAIN_SCHEDULE_HM`이 실제 스케줄러 시각과
      안 맞는 것이다(작업 스케줄러 트리거를 확인할 것).
- [ ] **대시보드 커밋 시 스모크 필수** — `python tests/test_dashboard_smoke.py`.
      py_compile은 `_build()` 런타임 오류를 원리상 못 잡는다(431차 후속2).

**사용자 결정 대기 — CB⑤ 입력에서 SHAP 시간 제외**

SHAP은 그 분의 매매 판단에 쓰이지 않는데(대시보드·주간심사·DB 관측용) 파이프라인
총시간에 포함돼 **CB⑤ 진입정지를 실제로 발동**시켰다(08-05 12:34, shap 4,481ms /
total 5,117ms). 판단 경로만 계측하면 구조적으로 재발하지 않는다.

- [ ] **다만 CB⑤는 CLAUDE.md 절대원칙 §2의 안전장치**라 입력 정의 변경은 안전장치의
      의미를 바꾸는 일이다. **임의 변경 금지 — 사용자 결정 필요.**
      결정 시 고려: 총시간은 `[PipePerf][DBG]`로 계속 관측되므로 가시성은 유지된다.

### 435차 발견 — S0 스파이크는 intraday 재학습 직후 모델 리로드 (미조치)

1000ms 초과 20건 중 S0 병목 4건(3,062~3,877ms)이 10:08·11:01·13:14·14:29로
intraday 재학습 완료(10:07·11:00·13:13·14:28)의 **바로 다음 분**과 정확히 일치한다.
`[GBM재학습중]` 태그는 0/20 — 재학습 **창**은 이미 끝났고 그 직후 리로드 비용이다.

- [ ] **완화 창을 리로드까지 덮을지 검토** — `_gbm_retrain_active`는 CB⑤ 임계 완화
      **그리고 Degraded 판정**에 함께 쓰인다(`main.py:_is_gbm_retrain_window`).
      Degraded는 사이징에 직결되므로 **창을 넓히면 사이징에 영향**이 간다.
      로그 표시만 고칠지, 완화 창을 넓힐지 먼저 정할 것. 급하지 않다(4건/일).

### 435차 발견 — SHAP 단발 13배 스파이크 원인 미확정 (미조치)

평균 268ms인 SHAP이 12:34에 4,481ms(13배)로 튀었다. 재학습 창과 무관하고 그 분의
다른 단계는 정상이었다. `permutation_importance` 내부 GC 또는 데이터 증가로 추정하나
**미확정**이다. 435차 스로틀은 증상 억제일 뿐 원인 치료가 아니다.

- [ ] **재발 시 그 분의 표본 크기·GC를 함께 찍어 원인 좁히기** — 지금은 소요시간만
      남아 원인을 가를 재료가 없다.
...
## 2026-08-02 (MW0601 418차 — backfill UPDATE 손상 대응, Phase 1~4)

> 상세·근거: `dev_memory/DECISION_LOG.md` 2026-08-02(MW0601 418차).
> 손상 확정 구간 **2026-06-02 ~ 07-13 (29거래일 10,448행)**.

### ⚠ 삭제 금지 자산

- `data/db/raw_data.db.bak_20260608_151648` — 1,446행의 **유일한 복구원**.
  gitignore 대상이라 다른 PC에도 사본이 없다. 정리 대상으로 보여도 지우지 말 것.

### Phase 1 — 재발 방지 (backfill_features.py) ★ 최우선

- [ ] `--update-features` 실행 전 **라이브 행 포함일 스캔 → 명시적 플래그 없으면 중단**
- [ ] UPDATE를 **병합 갱신**으로 전환 (기존 features JSON 로드 → 백필 계산 키만 덮어쓰고
      나머지 보존). MW0602 `fa642cd`가 제안한 해법이며, 이게 있었으면 사고가 없었다.
- [ ] `--update-features` 실행 시 대상 DB 자동 백업

### Phase 2 — 부분 복구 (1,446행)

- [ ] `scripts/restore_raw_features_from_backup.py` 신설 (dry-run 우선) —
      백업본에서 06-02·06-04·06-05·06-08 라이브 행 복원. `ts` 100% 일치 확인됨.
- [ ] 06-09~07-13 25거래일 9,002행은 **복구 불가로 확정 기록**

### Phase 3 — 오염된 산출물 재생성

- [ ] 섀도우 TB 1m 재학습 시 손상 구간 제외 (`gbm_1m_shadow_tb.pkl` 07-31 15:48 생성분이
      오염 — 검증 캠페인 [1] TB 채널에 그대로 반영돼 있다)
- [ ] `generate_featureset_health_report.py`에 손상행 제외 + 헤더에 "관찰창 내 백필행 N/M"
      표시. §2-b의 유령 DEAD 4건 제거, §4 축적일수를 무손상 기준으로 재계산 —
      백필 행이 `FEATURE_KEYS_ALL`을 0.0으로 채워 축적일수가 **부풀려져** 있었다
      (`foreign_futures_net` 60→17일 등). 실제보다 검증 준비가 된 것처럼 보였다.

### Phase 4 — 317차 원래 목적 재확인

- [ ] 손상행에 `hurst`는 들어갔으나 `hurst_ready`는 백필·라이브 양쪽에 혼재. 317차
      hurst 소급 반영이 실제로 무엇을 달성했고 지금도 필요한지 재평가 — 불필요하면
      해당 NEXT_TODO 항목 자체를 폐기해 함정을 제거할 것.

---

## 2026-08-02 (MW0602 417차 — backfill FEATURE_KEYS_ALL 갱신, **MW0601 확인 완료 → UPDATE 확정**)

> 상세: 커밋 `6a795ab`. 라이브 미기록 9종 제거(116→107). 이 변경은 **앞으로의 백필**
> 에만 적용되며 이미 DB에 들어간 행은 그대로다 — 그래서 아래 확인이 필요했다.

> ### ✅ 2026-08-02 MW0601 418차에서 확인 완료 — **UPDATE 손상 확정**
>
> 아래 두 항목은 모두 해소됐다. 결론만 옮긴다(전체 근거·수치는
> `dev_memory/DECISION_LOG.md` 2026-08-02 MW0601 418차):
>
> - **판정: 혼재일 15일 → `--update-features`가 라이브 행을 덮어썼다.** 결정적 증거는
>   `created_at` — `ts ≤ 2026-07-13`인 정규장 행 전체가 **2026-07-13 한 시점에** 재기록됐다.
>   317차 절차 4번(`--from` 없는 전체 실행)이 그날 MW0601에서 실행된 것이다.
> - **손상 범위 2026-06-02 ~ 07-13, 29거래일 10,448행.** 2026-04-28~06-01(22일)은
>   백업본 대조 결과 **원래부터 백필 INSERT라 무해** — MW0602가 깨끗하다고 본 구간과 같다.
>   이 분리를 안 하면 피해가 2배로 과대평가된다.
> - **주 재학습 경로 `raw_features_horizon`은 무사.** 백필은 `raw_features`만 UPDATE하고
>   EOD는 `--phase2`로 horizon 테이블만 읽는다. 실제 오염은 섀도우 TB 1m(raw_features
>   폴백)·피처셋 건강도 리포트·PSI 워밍업 3곳.
> - **복구**: 백업본 `raw_data.db.bak_20260608_151648`에서 1,446행(06-02·04·05·08)
>   복구 가능, 9,002행(06-09~07-13 25거래일)은 **복구 불가**.
>
> **사용자 결정(2026-08-02)**: 복구 불가 25거래일은 `raw_features` 소비처에서 **제외**
> 한다(0.0 상수는 "값 없음"이 아니라 "0이라는 잘못된 정보"). 재발 방지를 복구보다 먼저 한다.

### ~~★ MW0601에서만 확인 가능 → MW0601 세션이 실행할 것~~ (완료)

- [x] **MW0601의 07-03~07-31 관찰창에 있는 백필 행이 INSERT인가 UPDATE인가** —
...
- [x] (위가 UPDATE로 판명될 때만) **MW0601 GBM 재학습 창에서 해당 날짜 제외 여부 결정** —
      **결정: 제외한다.** 단 조사 결과 이 항목의 전제가 일부 빗나갔다 — GBM 주 재학습
      (`--phase2`)은 `raw_features_horizon`을 읽고 그 테이블은 **무손상**이라 애초에
      제외할 것이 없다. 제외가 실제로 필요한 곳은 `raw_features`를 읽는 **섀도우 TB 1m**
      과 **피처셋 건강도 리포트**다(아래 MW0601 418차 항목의 Phase 3).
...
- [ ] **[배포 필수] MW0601(라이브 운영 PC)에서 backfill 재실행** — 위 219일·81,762행
  ...
  **절차**:
  ...
  4. `python scripts/backfill_features.py --update-features` 실행 — 개발 PC 기준
     ...
     > 🚨 **[418차] MW0601에서는 이 4번이 2026-07-13에 이미 실행됐고, 아래 경고가
     > 예측한 손상이 그대로 발생했다 — 2026-06-02~07-13 29거래일 10,448행.**
     > 다시 실행하지 말 것. 대응은 위 "MW0601 418차" 항목 Phase 1~4 참조.
     >
     > **더 중요한 것: 418차 재평가 결과 이 항목은 애초에 목적을 달성할 수 없다.**
     > 이 절차의 목적은 "hurst 재보정을 **재학습 데이터**에 소급 반영"인데,
     > `hurst`가 실제 배포 학습 피처인 곳은 **10m 하나뿐**이고
     > (`model/horizons/feature_names_10m.pkl`에 있음, 1m/3m엔 없음),
     > **10m은 `raw_features_horizon`에서 학습한다**(EOD가 `--phase2`로 실행).
     > 그런데 `backfill_features.py`는 `raw_features`만 건드리고
     > **`raw_features_horizon`은 절대 쓰지 않는다.** 즉 이 백필이 아무리 성공해도
     > 10m 학습 입력의 hurst는 1비트도 바뀌지 않는다.
     >
     > 남은 소비처도 실질이 없다 — 전역 `feature_names.pkl`(97개, hurst 포함)을 쓰는
     > 1m Phase-1 경로는 EOD가 `--phase2`라 재학습되지 않고, ScalerWarmup은 최근
     > 500행만 읽어 항상 무손상 구간이다.
     >
     > **→ 권고: 이 항목 폐기.** 얻을 것이 없고 실행 위험만 실증됐다. 굳이 hurst를
     > 소급 반영하려면 대상은 `raw_features`가 아니라 **`raw_features_horizon`**이며,
     > 그건 이 스크립트가 하는 일이 아니라 별도 설계가 필요하다.
     > (근거: `dev_memory/DECISION_LOG.md` 2026-08-02 MW0601 418차 Phase 4)
     >
...
- [x] **F-8 STEP 3 재학습 주기 문서 정정 (P2)** `[DONE 2026-08-23 483차 후속1]` —
  ⚠ **이 항목은 2026-08-14 470차 후속(장중)에 이미 등록돼 있었다.** 0821 리포트가
  같은 사안을 이상점 1-16(O-13 승계)·**P2-J 신규**로 다시 올렸다 — *"이미 있는 것을
  새 안건으로 올리지 않는다"* 함정에 걸린 것이다. 실제 구현은 F-8 원안이 지시한
  **3개 파일 전부**(CLAUDE.md STEP 3 + `phases.md` B-2 + `invariants.md` §3-B)에 적용했다.
  P2-J 원안은 앞의 둘만 적었다. 원안:
...
## 2026-08-18 (MW0601 475차 — 장전 점검) 신규 항목

### Fix — 08-18 장 마감(15:10) 이후 적용

- [ ] **F-1 장전 점검 예약 시각 조정 (P1) — ⚠ 사용자 결정 필요, 코드 아님.**
  오늘 예약이 08:57이 아니라 **13:43**에 돌았다(O-7 첫 판정 = 실패). 08-17에도 3국면이
  16:48/16:49/16:55에 몰렸으므로 휴장일 요인은 배제된다.
  권고: **08:57 → 08:50**으로 앞당기고 3국면을 10분 이상 이격(pre 08:50 / intra 12:30 /
  post 15:50). ⚠ 08:45 이후는 라이브 DB 금지 구간이라 DB 확인은 어차피 포기 — 로그 확보
  우선으로 08:50이 합리적. 확인할 것: 예약 등록 시각·타임존·최근 실행 이력.
  검증: 08-19 리포트 헤더 `점검 시각`이 08:50±5분인가.

- [ ] **F-2 수집기 국면-시각 검증 + `--pc` 인자 + CRLF 정규화 (P1) — 기존 F-5와 통합, 중복 착수 금지.**
  `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py`:
  ① `main()` 진입부에 국면별 기대 창 상수(`pre` 08:40~09:05 / `intra` 09:00~15:10 /
     `post` 15:35~). 벗어나면 다이제스트 첫 줄 `⚠ 국면-시각 불일치: pre 기대 08:40~09:05,
     실행 13:43` 배지 + §11 **적신호 1번**으로 승격.
  ② `pc_id()` — `--pc` 인자 추가(우선순위 `--pc` > `MIREUK_PC_ID` > 호스트명).
     ⚠ 현 리포 판본은 `--pc`를 받지 않아 오늘 `platform.node()` 몽키패치로 우회했다.
  ③ git 호출부에 `--ignore-cr-at-eol`(적신호 15 오탐 제거).
  ⚠ 표준 라이브러리 전용 유지 — 외부 의존 금지.
  ⚠ **적신호 6·7(커버리지·공백)도 ①이 함께 해소한다** — 장전을 오후에 돌리면 구조적으로 뜬다.
  검증: `--phase pre`를 오후에 강제 실행 → 배지·적신호 출현. `--pc MW0601` → 헤더
  `MW0601 (override · host=…)`.

- [ ] **F-3 FaultHandler "행감지" 문구 정정 (P1) — 동작 코드 0줄 변경.**
  `main.py` 5개소: `16790` 주석 / `16792~16793`("30초 이상 블로킹되면 덤프" — 436차가
  무효라 한 서술) / `16801` / **`16814` 운영 로그 문자열 `"행감지=30s"` → `"주기덤프=30s(행감지 아님)"`**.
  근거: 오늘 616덤프 vs 30초 주기 기댓값 606(경과 303분) 일치, 마지막 덤프 전 스레드 대기 상태.
  ⚠ `dump_traceback_later(30, repeat=True)`는 **그대로 둔다**(436차 결정 — 30초 해상도가
  크래시 직전 진단 가치).
  런타임 `py37_32`, COM 콜백 밖 → 절대원칙 §4 무관. **장후 즉시 적용 가능**(EOD 위험 없음).
  검증: 다음 기동 로그에 `주기덤프=30s(행감지 아님)` 출현.

- [ ] **F-4 CLAUDE.md 운영환경 표 분봉 TR 행 정합화 (P2).**
  `CLAUDE.md` 운영환경 표 `선물 분봉 TR | OPT50029` 행을 **삭제하지 말고** 교체:
  `선물 분봉 | Cybos 실시간 틱 집계(Dscbo1.FutureCurOnly → [BAR-CLOSE][CYBOS]) —
  Kiwoom OPT50029/SetRealReg 경로는 2026-05-04 SUPERSEDED`.
  함께: `references/phases.md` A-2 항목 교체 + `references/invariants.md` §3-B 표 **3번째 행 추가**.
  ⚠ **삭제 금물 — 계측 4원칙 ③.** `collection/kiwoom/`이 통째로 남아 있다는 사실이 숨는다.
  근거: 당일 로그 `OPT50029`·`OPT10080` 각 0건, `broker=cybos`, `NEXT_TODO.md:11765`
  `[V1] … [SUPERSEDED 2026-05-04]`.
  검증: 08-19 장전 점검에서 A-2가 `N/A`가 아니라 `✅`.

- [ ] **F-5 매크로 2단계 중복 호출 섀도 계측 (P2) — 조사만, 차단 금지.**
  `main.py` `[System] 매크로 수집 완료` 호출부 전수 식별 → `caller=` 출처 태그.
  `collection/macro/macro_fetcher.py` → `[MacroStage2] 중복 호출 감지 prev=<ts> gap=<n>s`
  (WARNING, **차단하지 않음**).
  ⚠ 두 번째 호출이 첫 번째 실패를 덮는 **안전망일 가능성 미배제** — 지금 차단 넣지 말 것.
  ⚠ `macro_fetcher._loop`은 별도 스레드(`crash_fault.log` `Thread 0x000054f4` 실측) —
  COM 콜백 아님, 절대원칙 §4 무관.
  검증: O-14(3거래일).

### 고도화

- [ ] **G-1 장전 P0 프리플라이트 스크립트 (이번 주) — 선행: F-3·F-4.**
  `scripts/premarket_preflight.py`(신규, 표준 라이브러리 전용). **08:59에 미륵이 프로세스와
  무관하게** 실행. 6항목만 판정: ① 브랜치 `v9-dev` ② `[Model] * 로드 성공` 6/6
  ③ `[CybosSub] subscribe end` 존재 ④ 직전 거래일 `retrain_eod_*.log` 완주 마커
  ⑤ 설정 불변식 `불일치` 0 ⑥ `[Regime] 레짐 확정` 존재.
  `rc=0/1/2`, `rc=2`면 알림. **아무것도 차단하지 않는다**(알림만) → 섀도 단계 생략 가능.
  근거: 오늘 장전 P0 탐지가 실행 시각에 종속돼 4시간 44분 지연됐다. A-0~A-4 20항목 중
  19개가 로그 문자열 매칭으로 기계 판정 가능하다.
  로드맵: **실전 전환 기준 ②의 선행 확인사항 등재 검토**(471차가 이미 15:10 실집행 확인을
  붙인 전례).

- [ ] **G-2 `crash_fault.log` 덤프 시각 인덱스 (이번 주) — 선행: F-3.**
  `scripts/fault_dump_index.py`(신규, 읽기 전용 오프라인). `[START]` 헤더로 세션 분할 →
  30초 주기 성질로 `추정시각 = START + 30s×n` 부여 → `--at 13:28:36 --window 60` 질의.
  ⚠ **`추정`을 출력에 명시**(계측 4원칙 ④). 30초 주기 가정이 깨지면 어긋나므로
  **판정 도구가 아니라 후보 제시 도구**로만 쓴다.
  근거: 오늘 13:28:36 `_tick_header 37,875ms`(역대 최대)에 대응하는 스택을 616개 덤프 속에서
  특정할 수 없었다. `_tick_header` 채널(O-7/N-4/O-2/O-6 계열)이 6개월째 건수·최댓값만 세고
  원인을 못 짚은 이유가 이것이다.

- [ ] **G-3 레짐 이중 판정의 승자 명시 — 선행: F-5 3거래일 관측 완료.**
  이중 호출 출처 확정 후 레짐 확정 로그에 `stage2_seq=1/2` · `final=Y/N` 추가.
  **값이 갈린 경우에만 WARNING 승격.**
  ⚠ F-5 결과에 종속 — 이중 호출이 안전망이면 설계가 달라진다.

### 다음 거래일(08-19) 관측 — 본 세션 추가분

- [ ] **O-13 리포트 자기 시각 검증** — F-1 미조치여도 08-19 장전 리포트가 실행 시각 이탈을
  스스로 §1 P1으로 올리는가.
- [ ] **O-14 `[Regime] 레짐 확정` 건수와 두 판정의 값** — 08-19·08-20·08-21 3거래일.
  2건 유지 여부 + **두 값이 갈리는가. 1건이라도 갈리면 [5] P2 → P1 승격 + G-3 착수.**
  오늘 기준: 2건, 값 동일(VIX=14.2 / SP500=-0.52% / KRW=+0.26% / NEUTRAL).
- [ ] **O-15 `crash_fault.log` 당일 `Timeout` 덤프 수 ÷ 세션 분수** — 30초 주기 기댓값 대비
  **+5% 초과면 진짜 정체 발생 신호**. 오늘 616 / 기댓값 606 = +1.7%(정상 범위).
- [ ] **O-16 `_tick_header ≥5,000ms` 건수·최댓값** — 오늘 **4건 / 37,875ms**.
  건수 15건 초과면 G-3(기존, 수집기 적신호 분리) 상향. **최댓값 40,000ms 초과면 P1 승격.**

### 오늘(08-18) 안에 답이 나오는 이월 항목 — 🔴 최우선

- [ ] **O-1 EOD 재학습 완주 확인 (오늘 15:45~16:10)** — `logs/retrain_eod_20260818.log`가
  ≥10KB **且** `[검증 캠페인] 요약:` 행 존재. **미달이면 연속 2일 실패 = P0 승격.**
  08-17은 1,040B / 11행에서 `[WaitDC]` 대기 중 15:46:34에 끊겼다.
- [ ] **O-2 `session_state.json` P8 키 재생성 (오늘 EOD 직후)** — 오늘 13:33 스냅샷 기준
  `p8_last_success_date`·`eod_retrain_ok_date` **여전히 없음**(현재 키 8개 전량 확인).
- [ ] **O-3 P8 키 잔존 (08-19 08:41 기동 후)** — 재생성됐다면 기동 후에도 남는가.
- [ ] **F-1H 하트비트 1일차 (오늘 15:10)** — `[SchedForceExit] … bar_pass=N회` 출현.
  08-17은 본체가 15:11에 부재해 판정 불가였다. **오늘이 진짜 1일차.**
- [ ] **F-4M 스키마 마이그레이션 1일차 (오늘 EOD 후)** — 471차 후속2 신규 행.
  오늘 진입 7건 발생했으므로 **신규 행이 있어야 한다**(08-17은 0건이라 판정 불가).
- [ ] **F-1R 15:10 강제청산 리허설 — ⚠ 사용자 실행 필요.** 15:05~15:09 모의 1계약 진입 후
  15:10 관측. 정상 = `[ForceExitPass]` → `[TimeExit]` → `[ExitAttempt]`가 15:10:00~05,
  `[SchedForceExit] … 안전망 발동`(ERROR) **미출현**. 비용 왕복 수수료 약 1,600원.

### 문서·운영

- [ ] **커밋 대기 — 475차 산출물 4종(코드 변경 0건).**
  `docs/정기점검/매일점검/MW0601-20260818-점검리포트-pre.md` ·
  `docs/정기점검/매일점검/evidence_MW0601-20260818_pre.md` ·
  `dev_memory/DECISION_LOG.md` · `dev_memory/NEXT_TODO.md`.
  ⚠ 실질 미커밋 변경은 이 4종 + 기존 문서 2종(`weekly_prompt.txt`·`dailycheck_prompt.txt`)뿐.
  **수집기가 표시한 "454건"은 샌드박스 CRLF 아티팩트 — 재인용 금지**(08-17 [5] 재확인,
  실측 `git diff --ignore-cr-at-eol --stat` = 2파일 / 15+ / 1−).
- [ ] **CB② 복원 08-29 주간회의 상정** — 기한 **11일** 남음. 오늘 `9999` 유지 확인.
  오늘 CB 이벤트 `연속 손절 1회`(13:08:37)·`2회`(13:19:39) 발생 — **카운터는 오르고 정지는
  안 하는 설계대로의 동작**.
- [ ] **브랜치 격차 처분을 주간회의 안건으로** — `v9-dev`↔`origin/dev` = **320/315**(기존).
  설정 불변식 `미발견` 5종의 근본 원인. 점검 세션 단독 결정 사안 아님.
- [ ] **실전전환 ⑨ TOX-SEVERE-SPREAD 처분** — 473차 F-8 Phase A `INSUFFICIENT`(ETA 7.1개월).
  **주간회의 안건** — 선행조건 유지 vs 26주 WFA 이관(기존).

### 완료 처리 (475차)

- [x] **커밋 대기 — 473차 실질 변경 10파일 + untracked 2종** → 474차 커밋 `7dc14bc`에 포함 확인.
  오늘 실측 실질 미커밋은 문서 2파일뿐.
- [x] **O-8 08-18 08:40 자동 기동 여부** → **정상.** 런처 `[GUARD] 장전 [0840] — 10초 후 자동 Y
  (스케줄러 자동 기동)`, `08:40:47 [FaultHandler] 활성화 … PID=21660`.
  ⚠ 08-17 17:57 기동분이 밤새 살아 있었고 런처 가드가 정상 종료 처리 — 이중 실행 방지 설계대로.
- [x] **O-7 예약 점검 시각** → **판정 = 실패**(13:43). 상세는 [1]·F-1.
- [x] **O-2 P8 키 재생성** → **판정 = 안 됨**(예상된 결과 — 08-17 EOD 미완주). 진짜 판정은
  오늘 EOD 후 재관측.
...
- [ ] **F-1 수집기 §5 포지션 단위 집계 (P1) — 08-18 장후 점검 *종료 후* 적용** —
  `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py` 거래일 요약 생성부.
  `[Position] 진입` 시각을 키로 포지션 버킷을 만들고 `[TP1 부분청산]`·`[손절1차 조기축소]`·
  `[청산 완료]` 레그를 전부 담는다. 표 머리글을 **포지션 n건 · 승 n · 포지션손익 합계**로,
  기존 레그 표는 **`(참고) 청산 레그 n건`** 으로 단위를 박아 **남긴다**(계측 4원칙 ①·③).
  승패는 `exit_reason`이 아니라 포지션 합계 부호로. `하드스톱`에 `tp1_reached`가 섞였으면
  `하드스톱(TP1보호)` 별도 표기. `[ProfitGuard][DebugPnL] engine=` 값과 자동 대조해
  `✓ engine 일치` 출력.
  ⚠ **08-18 장후 점검 전에 넣지 말 것** — 같은 날 두 국면 형식이 갈린다.
  ⚠ 수집기는 **표준 라이브러리 전용**.
  검증: `--date 20260818 --phase intra` 재실행 → **+661,668원** + `✓ engine 일치`.
  08-14(1포지션)로도 회귀 확인.
  근거: 08-18 P2-1. **08-14 동일 관측이 fix 미등록으로 재발·12배 확대된 건이다.**

- [ ] **F-2 CB③ HALT 비활성 사실을 문서 3곳에 등재 (P1) — 08-14 잔여 F-2·F-8과 같은 커밋** —
  ⚠ **코드는 건드리지 않는다. 문서만.**
  · `CLAUDE.md` 절대원칙 §2 CB③ 항목: *"HALT 트리거는 296차(2026-07-06)부터 코드에서 비활성
    (`safety/circuit_breaker.py:315~334` — 임계 미달 시 `logger.debug`만). 임계 `0.28`은
    살아 있으나 집행되지 않는다. `accuracy_buf`·P4 stage·Contrarian 입력은 유지."*
  · `CLAUDE.md` 전환기준 ②: *"CB①~⑤ 라이브 발동 확인"* → *"CB①·②·④·⑤ 확인 /
    **CB③은 HALT 비활성이라 발동이 구조적으로 불가능**"*.
  · `CLAUDE.md` 전환기준 ⑥: **CB③ HALT 복원 여부를 명시 편입**(현재 P4 등급차단만 가리켜 누락).
  · `references/invariants.md` §3-B 표에 한 행 + §1 절대원칙 ②의 CB③ 줄에 `(HALT 비활성)`.
  · `references/phases.md` A-1: *"모델 미교체 → CB③ HALT 유발"* →
    *"→ **정지 없이 열화 모델로 계속 매매**(CB③ HALT는 296차부터 비활성)"*.
  근거: 08-18 P1-1. acc30m 12:00~13:00 **0.0~5.3%** 체류에도 `state=NORMAL`.

- [ ] **F-3 `_tick_header` 크기 기준 적신호 + 포지션 보유 여부 인라인 (P1) — F-1과 한 커밋** —
  `collect_evidence.py` 적신호 생성부(약 1526행). 현행 `5초 초과 n건`에 더해
  **`최대값 ≥ 20,000ms`** 를 **독립 적신호**로. 문구에 *"CB⑤는 이것을 보지 않는다
  (별도 계측) — 크기만 문제"* 를 박아 G-3이 분리한 것을 다시 합치지 않게 한다.
  발생 시각이 `[Position] 진입`~`[청산 완료]` 사이면 `⚠ 포지션 보유 중` 태그.
  ⚠ 임계 20,000ms는 사전등록 — **이후 움직이지 않는다**(313차 ④).
  검증: `--date 20260818` → `최대 37,875ms` 적신호 출현 + 4건 모두 `포지션 보유 중` **미출현**.
  08-14(최대 9,125ms)에서는 **미출현**.

- [ ] **F-4 `PIPE_LATENCY_EXCLUDE_MODEL_SWAP` 체리픽 심사에 S5 변종 첨부 (P2)** —
  코드 변경 없음. 08-17 장전 F-1B 항목에 실측 한 줄 추가:
  *"⚠ 이 스위치는 08-18 12:18 `cause=S5(1630ms)` 건을 막지 못한다 — 당일 선제차단 4건 중
  1건(25%)이 사정거리 밖."* 주간회의 브랜치 격차 논의의 근거로 승계.

### 고도화

- [ ] **G-1 ProfitGuard L1-Trail 기회비용 섀도 채널 (이번 주 — 우선)** —
  래치는 **그대로 둔다**(수익 보존이 설계 의도). `strategy/profit_guard.py:_block()`이
  차단 시 `pg_layer`·`pg_peak_krw`·`pg_current_krw`·`grade`를 `ensemble_decisions`
  차단사유 경로에 남기고, `config/settings.py:VALIDATION_CAMPAIGN`에
  `profit_guard_latch_watch`(`min_samples=20`, `min_days=10`) 신설.
  차단된 A/B등급 신호의 잔여 구간 반사실 손익을 적립.
  **판정문 사전등록**: *"반사실 합계가 유의하게 양(+)이면 `trail_ratio` 재보정 검토,
  음(−)이거나 무의미하면 현행 유지."*
  ⚠ **집행 무변경 — 순수 섀도**(스킬 §7-7). ⚠ **문턱 낮추기 금지**(458차 D6·473차 F-8 전례).
  ⚠ 표본 속도 위험: L1 발동이 **7주에 1회**(2026-06-30 → 08-18)라 `min_days=10`은 수개월 스케일.
  **우선순위 이번 주** — 오늘 데이터를 놓치면 다음 표본이 7주 뒤다.
  근거: 08-18 13:19:59 래치 → 82건 차단(A등급 최소 3건), 세션의 30%(약 110분) 구조적 무거래.

- [ ] **G-2 `_tick_header` 블로킹 경고에 포지션 상태 인라인 (이번 주 — F-3보다 먼저)** —
  `dashboard/main_dashboard.py:_tick_header()`(약 10448~10466행) 경고 로그에
  `| pos=FLAT` 또는 `| pos=SHORT 2ct stop=1095.75` 추가.
  ⚠ **Qt 메인 스레드다 — DB 조회 금지, `position_manager` 메모리 스냅샷만.**
  ⚠ COM 콜백 아니므로 절대원칙 ④ 무관(`dynamicCall`·`emit` 넣지 않는다).
  기대: `블로킹 5초 초과 × 포지션 보유`를 **일별 1개 숫자**로. 0이 아닌 날이 진짜 P0다.
...
## 2026-08-18 (MW0601 476차 — 장후 점검 / post) 신규 항목

### Fix — 즉시 적용 가능 (라이브 프로세스 15:40:38 종료 확인)

- [ ] **F-1 DriftAdjuster 포화 가시화 (P1)** — `learning/self_learning/drift_adjuster.py:_adjust_alpha()`.
  `new == old`이면 액션을 `SATURATED_MAX`/`SATURATED_MIN`으로, 로그를
  `alpha 0.01000 유지 — ALPHA_MAX 포화 (연속 N일)`로. `_saturated_days`를
  `data/drift_adjuster_state.json`에 영속, `get_state()`에 `saturated`·`saturated_days` 추가.
  ⚠ `main.py:11245` 소비부가 `DRIFT_UP` 문자열 하드코딩 비교하지 않는지 확인.
  ⚠ **`ALPHA_MAX`·`RECOVERY_THRESHOLD` 값은 이 커밋에서 바꾸지 않는다**(F-2).
  검증: `tests/test_476_drift_saturation.py` + 다음 거래일 15:40 로그 `포화 (연속 7일)`.
  근거: 476차 [1] — 08-10~08-18 **6/6 거래일** `alpha 0.01000→0.01000`.

- [ ] **F-3 TP1 보호전환 섀도 qty≥2 확장 (P1)** — `main.py:_ts_record_tp1_trail_shadow()`
  호출을 qty=1 분기(현 `:13067`) 외에 부분청산 stage==1 완료 지점에도 추가.
  `utils/db_utils.py`에 `tp1_trail_shadow.entry_qty` · `.hook_path`(`qty1_static`/`qty2plus_partial`)
  ALTER TABLE, 기존 68행 `qty1_static` 백필. `config/settings.py:VALIDATION_CAMPAIGN["tp1_trail_shadow"]`
  판정을 `hook_path`별 분리(**섞지 않는다** — 청산 메커니즘이 다르다, 계측 4원칙 ①).
  근거: 476차 [2] — 두 테이블 모두 **08-12 이후 0행**, 그 사이 8포지션 TP1 도달.

- [ ] **F-4 `daily_broker_pnl` 단위 명시 (P2)** — `utils/db_utils.py`에
  `pnl_gross_krw`/`commission_krw`/`pnl_net_krw` 분리(기존 `pnl`은 하위호환 별칭, **판정 미사용** —
  461차 `mdd_pct` 관례). `strategy/ops/daily_exporter.py`에 3행 표기 + `gross − commission = net`
  자동 assert 로그. 근거: 476차 [3] — 23,332원이 수수료 합계와 원 단위 일치.

- [ ] **F-5 RegimeFingerprint PSI 매분 계측 (P2)** — `main.py:6059~6098`에 레벨 무관 5분 하트비트
  `[RegimeFingerprint] PSI=%.3f %s (heartbeat)` 추가 + `except`의 `logger.debug` → `logger.warning`
  (5분 스로틀). `ensemble_decisions`에 `fp_psi REAL`·`fp_level INTEGER` 컬럼(473차 F-8 Phase B와 같은 방식).
  ⚠ **`FP_CRITICAL_GRADE_BLOCK_ENABLED`는 `False` 유지 — 집행 무변경.**
  검증: 다음 거래일 heartbeat 74±5줄, `fp_psi` 비-NULL 370행.
  근거: 476차 [4] — **실전 전환 기준 ⑦의 선행 계측**. 오늘 PSI=0.008 CLEAR.

### 안건 (코드 변경 아님)

- [ ] **F-2 SGD 학습률 정책 재검토 — 08-29 주간회의 상정** — `ALPHA_MAX`(0.01) 상향 또는
  `RECOVERY_THRESHOLD`(0.58) 현실화. **F-1의 `saturated_days` 2주 시계열이 쌓인 뒤** 근거를 붙인다.
  사전등록 없는 임계 변경 금지(458차 D6). CB② 복원 안건과 동일 회차.

- [ ] **`entry_mode` 실전 목표값을 전환 체크리스트에 신설 제안 — 주간회의** —
  최소 08-03 이래 `manual`(A/B/C 전면 허용) 고정, `entry_mode_fallback` 0(실제 설정값).
  오늘은 최종등급 7건 모두 A라 실해 없음. 근거: 476차 [8].

### 고도화

- [ ] **G-1 `position_mfe_shadow` 신설 (이번 주)** — 포지션별 MFE/MAE를 +5/+10/+20분 시점으로 기록
  (entry_ts, horizon_min, mfe_pt, mae_pt, realized_pt_per_ct, regime, intraday_range_pt).
  `VALIDATION_CAMPAIGN`에 **min_samples=30 · min_days=15**로 사전등록.
  **판정문 사전 고정**: *"추세일(당일 레인지 ≥ 5일평균×1.5) 부분집합에서 realized/MFE10 중앙값
  < 0.35 이면 TP1 배수·보호전환 재설계를 주간회의 안건으로 올린다."*
  ⚠ MFE는 **사후 최적 상한** — 절대수준이 아니라 레짐별 상대 비교로만 해석(판정문에 명시).
  근거: 476차 [7] — 오늘 realized 6.85pt/ct vs MFE10 43.22pt/ct(비율 **0.158**).
  ⚠ **n=7 · 1거래일 · overlap 심각 — 오늘 수치로 어떤 임계도 바꾸지 않는다**(313차).

- [ ] **G-2 ProfitGuard L1-Trail 해제 조건 축 추가 (기등록 G-1에 병합)** —
  `strategy/risk/profit_guard.py` L1 보호선에 경과시간 감쇠를 **섀도 필드로만** 평가.
  근거: 476차 [10] — 자기잠금 구조(차단 중엔 피크 갱신 불가), 오늘 110분/29.6% 차단.
  ⚠ 08-10(-244,067원) 같은 날 L1이 손실을 막았는지 **반대 방향도 확인**. 판정 전 집행 변경 금지.

- [ ] **G-3 `[ModelLive]` 라이브 적중률 DB 승격 (이번 주)** —
  `predictions.db`에 `model_live_daily(date, horizon, live_acc, n, cv_acc, oos_acc)` 신설,
  `scripts/retrain_eod.py`가 로그와 동시 INSERT. 471차 후속7 `[51]` 채널과 조인 가능해진다.
  근거: 476차 [7] — `[ModelLive]`는 08-13부터만 존재하고 텍스트 로그에만 있다.

### 오늘 답이 나온 것 (완료 처리)

- [x] **O-1 EOD 재학습 완주** → **성공.** `horizons_replaced: 6/6` · `t_total_s: 340.2` · py310_64
- [x] **O-2 P8 키 재생성** → `15:50:50 [P8] session_state p8_last_success_date + eod_retrain_ok_date 기록 완료`
- [x] **O-A / F-1H 하트비트 1일차** → `15:11:21 [SchedForceExit] … status=FLAT … bar_pass=1회 →
  청산 대상 없음(정상)`. 안전망 ERROR **미출현**. ⚠ 경로 **생존**만 확인 — 주문 발주는 미증명(F-1R 필요)
- [x] **O-B `[Bias⚠] 1m FL편향`** → 13:55:59 79% 도달 후 `[BiasReset] … uniform fallback 적용`.
  streak 10분에서 리셋돼 30분 미도달 → `[CB-FLBias]` 0건이 **정상**. P5 경보 경로 점검 불필요
- [x] **O-C FP PSI file 로그** → **P1 아님.** 임계(>0.20) 미만이라 미출력. 죽은 섀도가 아니다 → F-5로 범위변경
- [x] **O-D broker−engine 23,332원** → **수수료 합계와 원 단위 일치.** gross vs net 문제(F-4)
- [x] **O-F ≥20,000ms 블로킹 · 포지션 보유 여부** → 5건 전부 **FLAT 구간**. P0 아님. CB⑤ 미발동이 정상
- [x] **F-4M 스키마 마이그레이션 1일차** → `entry_block_axes`·`health_preblock`·`sizing_trace` 370행 비-NULL

### 이월 (다음 거래일)

- [ ] **F-1R 15:10 강제청산 리허설 — ⚠ 사용자 실행 필요.** 15:05~15:09 모의 1계약 진입 후 15:10 관측.
  **실전 전환 기준 ②의 유일한 해소 경로** — 오늘도 15:10 FLAT이라 미시험(누적 미시험 계속)
- [ ] **O-3 P8 키 잔존** — 08-19 08:41 기동 후에도 남는가
...
- [ ] **[9] 실전전환 ⑨ TOX-SEVERE-SPREAD** — 오늘 `spread_ticks ≥ 20` **1분 / 진입 0건**(최대 28.0틱).
  473차 F-8 Phase A 적립속도 0.094건/거래일과 정합 — **진전 없음**

### 기한

- [ ] **CB② 복원 08-29 주간회의 상정** — 기한 **11일** 남음. 오늘 `9999` 유지 확인.
  표본 진전: 연속 손절 카운터 1·2회(13:08:37 · 13:19:39)
...
- [ ] `dev_memory/DECISION_LOG.md` · `dev_memory/NEXT_TODO.md` (append)
- ⚠ **475차 산출물 4종(pre/intra 리포트·다이제스트)과 08-17 세션 미커밋 10파일이 여전히 대기 중**이다.
  F-1~F-5 구현 커밋보다 **먼저** 정리해야 이력이 섞이지 않는다.


### 477차 — post 확인필요 3건 딥다이브 후속 (MW0601, 2026-08-18 분석만·코드 0건)

> 상위: `docs/정기점검/매일점검/MW0601-20260818-확인필요3건-딥다이브.md`

- [x] ~~**DD-1 (G-3 확장)**~~ **[DONE 2026-08-18 477차 후속]** — G-3 자체 + 4열 동반 구현.
      `utils/db_utils.py: summarize_live_prediction_rows()`(순수 계산부) ·
      `fetch_live_daily_detail()` · `save_model_live_daily()`(model_live_daily 테이블,
      PK=(date,horizon), predictions.db) 신설, `learning/batch_retrainer.py` [ModelLive]
      블록에서 EOD마다 INSERT. n_eff는 **겹치지 않는 h분 창의 그리디 최대 개수**
      (버킷 나눗셈은 :x9/:x0 쌍을 다른 버킷으로 갈라 기각). 채점 표본 없는 날은
      행 없음(미측정 ≠ 0). **08-04~08-18 10거래일 60행 백필 완료** — 0818 실측과
      원 단위 일치(10m .208/n_eff36/σ.129/clean_old.243) 확인.
      tests/test_477_model_live_detail.py 7건. ⚠ 라이브 배선 검증: **다음 EOD**
      로그에 `[ModelLive] DB 승격 저장 실패` 미출현 + model_live_daily 6행 증가.
- [x] ~~**DD-2 (q_exp 방향정렬)**~~ **[DONE 2026-08-18 477차 후속]** — 파생 **저장은
      하지 않았다**(설계 변경): loss_tier1_qty1_shadow에 direction이 같은 행에 있어
      읽기 시점 정렬(`q50×방향부호/unc`)이 중복 없는 형태다. 리포트가
      `aligned_edge_corr`/`n_edge_opposed`를 병기(기존 |exp|/unc 유지, 시계열 단절
      방지). 실측: 35행 중 **역행예측 진입 11건(31%)**. 부호 규약 주석은
      db_utils 2곳 + postmortem.md §3 인용문에 등재. 판정선 변경 없음.
- [ ] **DD-3 (D6 안건 편입 — 2026-08-26 주간회의, CB② 복원과 동일 회차)** —
      D6 논의에 ⓐ guard_fair 전환의 ghost_bypass 분기 포함 여부 ⓑ ghost 분기 한정
      최소 clean_n 사전등록(후보 100) ⓒ 08-18 5m 실사례(clean 0.183<0.317 무비교 교체)
      첨부. ⚠ D6 문턱(min_days=10·min_bars=1350) 인하 금지 — 사전등록 위반.
      ※ DD-4 구현으로 서류가 자동 생성된다 — 14일 창 실측: ghost_bypass 15건 중
      clean 측정 가능 9건, **반대 방향 4건**(1m·15m n=214 / 3m n=8 / 5m n=164).
- [x] ~~**DD-4 (주간 리포트 1행)**~~ **[DONE 2026-08-18 477차 후속]** —
      `eval_guard_shadow_channel()`에 `ghost_bypass_clean` 관측(표본부족 조기반환보다
      앞이라 INSUFFICIENT 주간에도 출력) + [23] 렌더링 1행. PC 필터 채널 본체와 동일.
      판정 무관여. 스모크 실측은 DD-3 항목에 기재.
- [ ] **DD-5 (10m/15m 5거래일 관측 계속)** — 476차 §6 항목 그대로. 08-18 라이브 20.8%와
      clean_old 0.243이 상호 확인됐고 그 모델은 EOD 교체됨(clean_new 0.428) — 08-19 반등
      여부가 "모델 문제였다" 가설의 1차 검증.


### 477차 후속2 — 476차 Fix 계획 검토 결과 (MW0601, 2026-08-18 · 검토만, 코드 0건)

> 상위: `docs/정기점검/매일점검/MW0601-20260818-Fix계획검토.md`
> ⚠ 아래 F번호는 **476차 post §2** 기준 — 475차 F-1~F-3(수집기/CB③문서/_tick_header)과 별개.

- [x] ~~**FR-1 (F-1 구현 시 반영)**~~ **[DONE 2026-08-18 커밋 710c1c5]** — `get_status()`(get_state 아님)에 saturated 키 ·
      dashboard `_DRIFT_LABELS`에 SATURATED_MAX/MIN 라벨 · `:4880` 툴팁 1줄.
- [ ] **FR-2 (F-2 안건 문안)** — "DriftAdjuster 입력 = 롤링 100분×6호라이즌(블렌드
      비활성 1m/15m/30m 포함) → 3m/5m/10m 한정 재정의 여부" + "10~35%는 3클래스
      라벨 기준(무정보 ~33%)" 맥락 추가.
- [x] ~~**FR-3 (F-3 재설계 — 원안대로 구현 금지)**~~ **[DONE 2026-08-18 커밋 ae5c29b]** — ⓐ synthetic_partial_exits에
      hook_path·entry_qty 확장(qty≥2 TP1 스테이지 기록) ⓑ [12] 리졸버 qty=1 한정
      필터 + "모집단 소멸" 사유 표기 ⓒ [25]/[25-B] hook_path별 분리 판정
      ⓓ G-1 position_mfe_shadow와 같은 커밋. 근거: exit_replay.replay()가 qty≥2
      사다리 기지원(stage_plan) — 검토 문서 F-3 절.
- [x] ~~**FR-4 (F-4 동반 필수)**~~ **[DONE 2026-08-18 커밋 a5f4b4c — 유령 15행 삭제 포함]** — ⓐ upsert 거래일 가드(휴장일 유령 행 방지)
      ⓑ 기존 유령 행 4건(08-08/09 723,000 · 08-16/17 32,000) 처리 — **삭제 vs
      is_phantom 플래그, 사용자 결정 필요** ⓒ pnl==0 스킵 재검토 ⓓ "추정자산" 키
      이름≠의미 주석 ⓔ net/commission은 EOD daily_close에서 trades 합산 UPDATE.
      ⚠ 전환기준 ① 판정을 daily_broker_pnl SUM으로 하기 전에는 ⓐⓑ가 선행조건
      (8월 실측 +151만원 이중가산).
- [x] ~~**FR-5 (F-5 구현 시 반영)**~~ **[DONE 2026-08-18 커밋 710c1c5]** — PSI 스태시는 sizing_trace 관례(예외 시 None 갱신) ·
      하트비트 로그 레벨 **INFO**(WARN 금지 — 수집기 §11 오염).


### 477차 후속1~3 — 476차 Fix 구현 완료 (MW0601, 2026-08-18 · 커밋 3건)

> 검토: `docs/정기점검/매일점검/MW0601-20260818-Fix계획검토.md`
> 커밋 순서는 검토 재안대로 ① F-1+F-5 → ② F-4 → ③ F-3 재설계+G-1.
> 전체 스위트 **477 passed**(신규 23건). 매매 정책·판정선 변경 0건.

**다음 거래일(08-19) 라이브 게이트 — 이 5개가 구현 검증이다**

- [ ] **G-A (F-5 하트비트)** — `logs/*_SYSTEM.log`에
      `[RegimeFingerprint] PSI=... level=... (heartbeat)` **INFO**로 5분 간격 출현
      (하루 약 74줄). ⚠ WARNING으로 찍히면 배선 오류 — 수집기 §11이 오염된다.
- [ ] **G-B (F-5 영속)** — `ensemble_decisions.fp_psi` 비-NULL이 당일 행수만큼.
      NULL 행이 있으면 그 분 `update_live` 예외(같은 시각 WARNING 5분 스로틀 대조).
- [ ] **G-C (F-1 포화)** — 15:40 `[DriftAdjuster]` 로그가
      `alpha 0.01000 유지, ALPHA_MAX 포화 (연속 N일)` 형식인지. 종전
      `0.01000→0.01000` 조정문이 다시 보이면 미배포다. `data/drift_adjuster_state.json`에
      `saturated_days` 키 생성 확인.
- [ ] **G-D (F-4 EOD)** — 15:40 `[BrokerPnl] EOD 확정 — gross ... − 수수료 ... = net ...`
      1줄 + `daily_broker_pnl` 당일 행의 `commission_krw`/`pnl_net_krw` 비-NULL.
      ⚠ `gross 불일치` WARNING이 뜨면 브로커 미정산 또는 체결 누락 — 원인 확인.
- [ ] **G-E (F-3 훅)** — TP1 도달 시 `synthetic_partial_exits`에
      `hook_path='qty2plus_partial'` 행 생성(08-13 이후 첫 신규 행). 같은 날
      `python scripts/tp1_protect_offset_shadow.py`의 판정 모집단이 **늘지 않아야**
      한다(qty1_static만 읽는 필터가 사는지 — 늘면 사전등록 위반).

**08-29 주간회의 상정 (기존 CB②·브랜치격차·⑨와 같은 회차)**

- [ ] **[안건] F-2 SGD 학습률 정책** — `ALPHA_MAX`(0.01) 상향 또는
      `RECOVERY_THRESHOLD`(0.58) 현실화. 근거는 F-1이 만드는 `saturated_days`
      시계열(08-19부터 적립). 문안에 FR-2 2건을 반드시 포함할 것:
      ⓐ **입력 모집단** — `record_accuracy`가 받는 값은 "당일 전체 정확도"가 아니라
        `online_learner.recent_accuracy()` = **롤링 100분 × 6호라이즌 평균**이고,
        그 절반이 블렌드 비활성(1m·15m·30m, 288차 "신호 부재")이다. alpha의 실효
        대상(3m·5m·10m)과 판정 입력이 어긋나 있다 → 3m/5m/10m 한정 재정의 여부.
      ⓑ **임계의 의미** — 실측 10~35%는 **3클래스** 라벨 기준(무정보 ≈33%)이라
        회복 임계 0.58이 무엇을 뜻하는지부터 재정의가 필요하다.
      ⚠ 값 변경은 예측 분포 → 앙상블 → DynMC → CB③으로 전파된다. 사전등록 없는
        임계 변경 금지(458차 D6).
- [ ] **[안건] 진입모드(`entry_mode`) 실전 목표값** — 476차 §1-5. 최소 08-03 이래
      `manual`(A/B/C 전면 허용) 고정이고 `entry_mode_fallback` 370/370행 `0`이라
      **실제 설정값**이다. 실전 전환 시 무엇이어야 하는지가 어느 체크리스트에도 없다.
      → 전환 체크리스트에 신설할지 결정. 오늘은 최종등급 7건 전부 A라 실해 없음.
- [ ] **[안건] DD-3 ghost_bypass 범위** (477차 등록, 재확인) — D6 clean 판정 전환이
      ghost_bypass 분기도 대체하는가 + ghost 한정 최소 `clean_n`(후보 100).
      서류는 DD-4가 매주 자동 생성한다(14일 창 실측: ghost 15건 중 clean 반대 4건).


### 477차 후속5 — 476차 §3 고도화 방안 조사 결과 (MW0601, 2026-08-18 · 조사만)

> 상위: `docs/정기점검/매일점검/MW0601-20260818-고도화방안검토.md`
> G-1·G-3은 이미 구현 완료(`ae5c29b`·`cf0f803`) — 신규 작업 없음.

- [x] ~~**476차 G-1 추세일 조기절단 감시**~~ **[DONE `ae5c29b`]** `position_mfe_shadow`로 구현.
- [x] ~~**476차 G-3 ModelLive DB 승격**~~ **[DONE `cf0f803`]** 4열 확장 + 10거래일 백필.
- [ ] **GR-1 `scripts/profit_guard_latch_watch.py` 신설 (이번 주)** — 읽기 전용 소급.
      입력: SIGNAL 로그(1차 원천 — `entry_block_axes`가 08-14 이후만 존재) +
      `ensemble_decisions` + `raw_candles`. 반사실은 `exit_replay.replay()` 재사용.
      **필수 출력**: 깔때기(로그콜 → 유니크분 → 자격생존 → **binding**) · 등급 분포 ·
      overlap 경고(30분 내 중복 자리). 산출은 금요일점검 PC 폴더 날짜본.
      ⚠ 집행 무변경. ⚠ 판정문 재등록 전까지는 **관측 출력만**(verdict 표기 금지).
...
- [ ] **F-1 `session_state` EOD 인계키 소멸 — 원인 규명 + 폴백 가시화 (P1, 장후)**
      ① 15:51 / 익일 기동 직전 / 08:42 **3시점 스냅샷 diff**로 주체 특정
         (오늘은 `.py`만 grep했다 — `.bat`·`scripts/`까지 확장할 것)
      ② `main.py:_read_session_state()` 기본값 반환 시 **INFO 로그 필수**(지금은 조용하다, 4원칙 ④)
      ③ `main.py:5057~5072` 1차 실패 → 폴백 전환을 **WARNING**으로
      ④ `retrain_eod.py:184` 쓰기 직후 **read-back 검증** 1줄
      ⚠ `_read_session_state` **반환 계약은 바꾸지 않는다**(`_restore_*` 4함수 영향)
      런타임: `main.py`=py37_32 / `retrain_eod.py`=py310_64
...
- [ ] **F-2 수집기 불변식 감시표 브랜치 스코프 분리 (P1, 장후)** — 08-14·08-17에 이어 **3번째 등록**.
      `collect_evidence.py` 스펙 dict에 선택 키 `"branch": ["dev"]` → `해당없음(dev 전용)` 렌더 +
      §11 제외. ⚠ **키 부재 = 전 브랜치 적용**이 기본값(기존 22행이 조용히 빠지면 새 은폐 수단).
      ⚠ 표 하단 `타브랜치 미적용 N종` 필수. `invariants.md` §2-B에 `적용 브랜치` 열.
...
- [ ] **F-5① 진입 레이스 폴백 가시화 (P1, 장후 즉시)** — `main.py:16361-16365` ERROR
      로그에 `| 유실=entry_horizon=… hurst=… extra_stop_mult=… pass_count=…` 추가 +
      `main.py:16517` `apply_entry_fill()` 호출부에 **`entry_horizon=pending.get("entry_horizon")`
      인자 추가**(범용 경로 `main.py:14171`과 시그니처 일치) +
      `position_tracker.py:_recalculate_levels()`에 폴백 WARNING 1회.
      계측 4원칙 ④의 직접 이행. 회귀 위험 사실상 없음(기본값 인자 1개 + 로그)
- [ ] **F-5② `apply_entry_fill()` 파라미터 확장 (P1, 다음 세션)** —
      `position_tracker.py:458` 서명에 `hurst_bucket`·`extra_stop_mult`·`checklist_pass_count`
      **선택 인자**(기본값 유지) 추가, FLAT 분기(506-520)에서만 대입. 호출부 3곳 전수 확인
- [ ] **F-5③ 콜백 큐잉으로 레이스 제거 — 보류** ⚠ **절대원칙 ④에 인접.**
      착수 전 호출 스택이 COM 콜백 내부가 아님을 반드시 확인. 확인 미완이면 손대지 않는다
...
- [x] **FZ-1 (P0) 메인루프 동결 워치독 스레드** ✅ 2026-08-19 배포 (`utils/freeze_watchdog.py`) — `main.py`에 순수 threading.Thread(daemon)
      신설. STEP 9 말미·`_scheduler_tick` 진입부에서 `self._main_beat` 갱신, 워치독은
      30초마다 검사해 장중(거래일 09:00~15:45) 180초×2회 무갱신이면 crash_fault.log에
      타임스탬프 CRITICAL 직접 기록 후 `os._exit(43)` → 런처 RESTART_LOOP 재기동.
      **QTimer 금지**(이번에 죽은 계층). 킬스위치 `MIREUK_FREEZE_WATCHDOG=0`.
      검증: sleep(400) 주입 리허설 + 하트비트 갱신 지점 불변식 테스트
- [ ] **FZ-2 (P0, 운영) 동결 프로세스 수동 종료** — `Stop-Process -Id 21612 -Force`.
      내일 08:40 자동기동 전 필수(이중 인스턴스 방지). EOD·P8 완료·FLAT·런처 무재시작 확인됨
- [x] **FZ-3 (P1) COM 타이머 위상 분리** ✅ 2026-08-19 배포 (`main.py:12726`) — `main.py` 타이머 초기화부(12584~12623)에서
      `_option_chain_timer` 기동을 +17초 오프셋 — 매분 :21의 수급 2건+지수 2건+옵션 워커
      CoInitialize/Dispatch 동시 발생 해체. 검증: 다음 거래일 워커 완료 로그 :38초대 이동
- [x] **FZ-4 (P1) 워커 이상 가드** ✅ 2026-08-19 배포 — `option_chain_worker.py` `_collect_snapshots` 누적
      60초 초과 시 조기 중단, `_execute` elapsed>30s면 피처 폐기(빈 dict 반환)+WARNING.
      오늘의 PCR 0.103/GEX 169B는 메인 동결 덕에 우연히 미저장 — 부분 지연 상황이면 저장된다
- [x] **FZ-5 (P2) crash_fault.log 타임스탬프** ✅ 2026-08-19 배포 (워치독에 통합) — FZ-1 워치독이 30초마다 `[TS] ISO시각`
      1줄 append. 이번 분석에서 덤프 블록 무시각으로 시각 역산 필요했음
- [x] **FZ-6 (P2) 수집기 §11 "장중 로그 침묵" 적신호** ✅ 2026-08-19 배포 — `collect_evidence.py`에
      장중 실행 시 SYSTEM.log 마지막 기록이 5분 이상 낡으면 적신호 출력
- [x] **FZ-7 (관찰)** ✅ 2026-08-19 17:01 **장후 재기동으로 선행 해소** — ① 잔존 프로세스 0
      ② 세션 복원 정상(당일 거래 6건 / 누적 +92,785원) ③ 누락됐던 daily_close가 이 재기동으로
      **복구 완료**(마감통계·캘리브레이터·전략리포트·WAL·마커 전부 생성). 모델 mtime 16:0x
      유지로 EOD 재학습 미훼손 확인. 상세: 딥다이브 §9
- [ ] **FZ-7b (관찰, 08-20) 다음 기동 정상 여부** — 08:40 자동기동이 평소대로 뜨는지,
      `[FreezeWatchdog] 기동` 로그가 있는지, `[TS]` 줄의 `beat_age`가 장중 0~5초를 유지하는지
      (30초 이상 튀면 임계 조정 전에 **그 블로킹의 정체를 먼저 밝힐 것**)
- [x] **FZ-7c 동결구간 분봉 백필 — 하지 않기로 결정** ✅ 2026-08-19. 네 테이블이 13:40에서
      일관되게 끊겨 부분기록 없음. `scripts/backfill_features.py`가 *"불가 피처(호가/수급/
      매크로/옵션 없음)"*라 OHLCV 소급은 **라이브와 구조가 다른 행을 정상처럼** 만든다
      (451차가 폐기한 패턴, 계측 4원칙 ② 위반). 미채점 39건 `correct=NULL` 유지가 옳다.
      ⚠ 내일 `prev_day_same_hour_ret` 13:41 이후 결측은 **정상** — 결함으로 보고 재시도 금지
- [ ] **FZ-8 (선택) 풀 덤프 WinDbg 분석** — `c:\tmp\mireuk_freeze_20260819_pid21612.dmp`
      메인 스레드 네이티브 스택으로 스핀 DLL 특정(Cybos OCX vs Qt/pywin32) → FZ-3 근거 확정
- [x] **FZ-9 (문서) 453차 D2 docstring 정정** ✅ 2026-08-19 배포 — "분봉·틱이 전부 죽어도 이 타이머는 돈다"는
      QTimer인 한 거짓임을 명기하고, 생존 계층은 FZ-1 워치독임을 참조로 연결

### 478차 후속2 — 장후 점검 (MW0601, 2026-08-19 · 분석만, 코드 0건)

리포트 `docs/정기점검/매일점검/MW0601-20260819-점검리포트-post.md`

#### 즉시 (P0)

- [ ] **F-0 좀비 프로세스 수동 종료 — ⚠ 사용자 실행 필요.** 13:41:21에 정지했으나 종료되지
      않은 라이브 프로세스가 **점검 시각(16:30)에도 살아 있다**(`logs/crash_fault.log` 30초마다 갱신).
      PID는 `logs/20260819_SYSTEM.log` 첫 줄 `[FaultHandler] … PID=21612`.
      **권고: `procdump` 또는 작업관리자 덤프 1개 확보 후 종료** — 종료하면 원인 증거가 사라진다.
      미조치 시 08-20 08:40 기동에서 COM 중복접속·pkl/DB 잠금 충돌 위험(관측 P-1).
- [ ] **F-1 라이브니스 워치독 신설 (P0, 장후 즉시)** —
      `scripts/mireuk_liveness_watchdog.py`(신설, 표준 라이브러리만, py 버전 무관) +
      `main.py:_scheduler_tick` 말미 `data/heartbeat_{PC}.txt` 1줄 쓰기(**COM 콜백 밖** — 절대원칙 ④ 무저촉) +
      `start_mireuk.bat` RESTART_LOOP에서 `start /B` 동반 기동.
      판정: 장중(09:00~15:20) SYSTEM 로그 mtime **180초** 정지 + PID 생존 → `STALLED`.
      ⚠ **1단계는 알림만.** 자동 kill·재기동 승격은 주간회의 안건(포지션 보유 중 재기동 위험).
      **사전등록 판정문**: *"섀도 5거래일 `STALLED` 오탐 0건이면 자동 조치로 승격. 1건이라도
      오탐이면 임계 180초를 재산출하고 5거래일을 다시 센다."* ⚠ 임계는 지금 등록하고 움직이지 않는다(313차 ④·458차 D6)
- [ ] **F-2 15:10 프로세스 밖 최후 방어선 (P0, F-1 다음)** —
      `scripts/force_flat_guard.py`(신설, Windows 작업 스케줄러 15:12 단발) +
      `config/settings.py:FORCE_FLAT_GUARD_ENABLED = False`(섀도 신설, 311차 관례).
      ⚠ **1단계는 주문을 내지 않는다** — 이중 청산·수량 불일치 위험. 브로커 직접 청산 승격은 주간회의 안건.
      ⚠ **F-1R(15:10 리허설)보다 먼저 배선할 것** — 리허설이 프로세스 생존을 전제하기 때문

#### 이번 주

- [ ] **F-3 `ofi_norm` 원값 분포 프로브 (P1)** — `scripts/ofi_norm_distribution_probe.py` 신설(읽기 전용,
      `utils/analysis_db.guard_intraday()` 필수). `raw_features` 08-11~08-19 일자별 std·0비율·유니크값.
      **사전등록 판정문**: *"08-17 이전 대비 std가 한 자릿수 배율로 줄었으면 수집 원천 문제,
      std 불변인데 identity 강제만 늘었으면 스케일러 판정 임계 문제. 5거래일 관측으로 확정하며
      그 전에는 CORE 지정을 건드리지 않는다."* ⚠ **절대원칙 §3 CORE 지정 변경은 이 조사만으로 하지 않는다**
- [ ] **F-4 EOD `WaitDC` 폴백 마커 가시화 (P2)** — `scripts/eod_retrain.py`가
      `data/eod_retrain_done_{d}.txt`에 `wait_dc_timeout: true` / `daily_close_seen: false` 기록.
      계측 4원칙 ④. ⚠ **`py310_64` 전용 파일**(191차) — py37_32 언급 금지
- [ ] **G-1 하트비트를 별도 파일로 분리 (F-1과 같은 커밋)** — 오늘 `data/session_state.json` mtime은
      **16:08**(EOD가 쓴 것)이라 라이브 정지(13:41)를 전혀 반영하지 못했다.
      쓰는 주체를 라이브 프로세스로 단일화한 `data/heartbeat_{PC}_{date}.txt`
- [ ] **G-2 수집기 §7에 "로그 생존구간 vs 직전 5거래일 중앙값" 대조 상설 편입** —
      `collect_evidence.py`. 당일 종료시각이 중앙값보다 30분 이상 이르면 적신호.
      DB 접근 없음(장중 안전). 오늘 사고를 **15:00 이전 장중 점검에서** 잡을 수 있었다
- [ ] **G-3 `[Position] 진입` 로그를 포지션 개시 3경로 공통으로 승격** —
      `open_position()` / `apply_entry_fill()`(FLAT) / `_ts_manual_position_restore()`가 공통 헬퍼를 거쳐
      진입 로그 + 위험 파라미터 5종을 세팅. **선행: F-5① → G-5(AST 테스트)** 순서 엄수.
      ⚠ 진입 경로 리팩터링은 라이브 위험이 있다
- [ ] **G-7 착수 조건 성립** — IntradayRegime 전이 **51회(반일)** vs 08-18 22회(종일).
      `scripts/intraday_regime_flap_watch.py` 신설(계측만, 집행 연결 금지)

#### 08-20 관측 예정

- [ ] **P-1** — 08:40 기동 정상 완료(`[FaultHandler] 활성화` → `[System] Qt 이벤트 루프 진입`).
      COM 중복접속·pkl 잠금 오류 시 **P0**
...
- [ ] **R-후보(5거래일 누적 후 판정)** — 등급 인플레(원시 C → A급) 축. 오늘 2건 승1 패1이고
      **패 쪽(CASE-02 −286,570원)이 당일 손실의 전부**다. ⚠ 관측일 1일 — 확정 결론 금지(313차)

#### 주간회의 상정 (기존 안건에 추가)

- [ ] **[실전전환기준 ②에 선행 확인사항 추가 제안]** — *"장중 프로세스 정지 감지·조치 경로 1회 실측"*.
      471차가 추가한 *"15:10 경로 실집행 1회 확인"*과 같은 이유 — 프로세스가 멈추면 그 확인 자체가 무의미해진다
- [ ] **[로드맵] 26주 WFA 재검증 항목에 라이브니스 워치독 임계(180초) 편입 제안** —
      오늘 실측 1건으로 잡은 값이라 파이프라인 지연 프로파일과 함께 드리프트한다
- [ ] **[08-29 CB② 보강]** — 오늘 **CASE-02(11:11 SHORT 3계약)가 한 포지션으로 `연속 손절 1회`·`2회`를
      둘 다 만들었다**. 478차 후속이 등록한 *"시간창 없음 + 레그 단위 카운트"*의 라이브 실례

#### [정정] 478차 후속2 — 미종료 딥다이브가 정본이다 (중복 구현 금지)

`docs/정기점검/매일점검/MW0601-20260819-미종료-딥다이브.md` §5가 P0 대응의 정본이다.

- [x] ~~**F-0 좀비 프로세스 수동 종료**~~ → **철회(중복).** 딥다이브 **P0-2**로 대체.
...
- [ ] **F-1 `batch_retrainer.py:508-510` X/y 가드 정밀화 — G-1 부활.**
...
- [ ] **F-1 회귀 `tests/test_476_eod_label_scheme_roundtrip.py` 신설** —
  2케이스(자기 `_load_from_db` 산 X → `"fixed"` / 출처 불명 X → `None` 유지).
- [ ] **F-2 수집기 `db_rows()` WAL 폴백 체인** —
...
- [ ] **F-3 `learning/model_meta.py:54-56` 진단 문구 분리** —
...
- [ ] **`references/invariants.md` §2-B 각주 "G-1 최초 검증 기회 = 2026-08-17(월)" → 2026-08-18 로 정정.**
...
- [ ] **`references/invariants.md` §2-B `MODEL_LABEL_STATE_UNLOCK_ENABLED` 행
  `⏳ 미검증` → `🔴 미충족 — 구조적 결함 확인(476차)` 로 격상.**
...
### 🔵 기한 — 주간회의(~~2026-08-22 금~~ → **2026-08-21 금**) 상정

> ⚠ **[483차 P2-C 정정]** 08-22는 **토요일**이다 — 실제 회의일은 **08-21(금)**.
> MW0602 476차(`0215f6c`)·477차(`44e2652`) 표기 오류다(0821 리포트는 "481차"라
> 적었으나 `git log -S` 실측은 476·477차다). 코드는 요일로 건다
> (`scripts/eod_retrain.py` — 금요일에만 자동 실행). **그 기한은 이미 지났으므로
> 아래 미이행 안건은 08-28(금)로 이월된다.**
...
- [ ] **F-5 수집기 §2 를 `git --no-optional-locks status` 로 교체** —
  인덱스 갱신을 시도하지 않아 `index.lock` 자체를 만들지 않는다. F-2와 같은 뿌리
  (예약 점검이 샌드박스에서 도는 한 매일 재발). `collect_evidence.py` §2 · 커밋 상태 수집부.
...
- [ ] **F-6 (Phase A) 09:20~09:29 게이트의 표기를 사실에 맞춘다 — 임계는 건드리지 않는다.**
...
- [ ] **F-6 회귀 `tests/test_476_grade_reachability.py` 신설** —
...
- [ ] **F-2′ F-2 범위 정정 — 실패 조건은 "샌드박스"가 아니라 "라이브 프로세스 가동 중"이다.**
...
- [ ] **F-7 수집기 §5 배너 검산 패턴 교체** —
  ...
  `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py`
- [ ] **F-8 ConfFloorGuard 샘플 축 정정 (docstring + 수집기 §12)** —
...
- [ ] **G-4 §12 `threshold_reachability` 축 신설 — "임계 미도달"을 매일 자동 탐지.**
...
- [ ] **G-5 도달성 테스트 명명 규약** — `tests/test_*_reachability.py` 를 `AGENTS.md` 에
  한 줄로 정하고, 새 게이트·등급·임계 신설 시 같은 형식의 **정의 대조 테스트**를 동반.
  분포 의존 단언은 G-4(점검 도구)에 두고 테스트는 정의만 본다.
- [ ] **G-6 `[SchedForceExit] … bar_pass=N회` 를 §12 `benign` 으로 등록** —
...
- [ ] **`references/invariants.md` §6 ① 의 "0818 기준 20거래일 +836,870원" 에 날짜 병기 필수.**
...
- [ ] **`references/phases.md` C-5 에 "검증 캠페인은 금요일 전용" 명시** —
...
- [ ] **P-4** 장후 §12 `ConfFloor` 관측률 **≥0.95**(F-8 적용 후). 오늘 원시 0.88 / 앙상블 축 1.00.
...
- [ ] **G-2 수집기 §12에 `state_file_keys` 축 신설** — `data/session_state.json`의
...
- [ ] **P-4 (장후)** §12 `ConfFloor` 관측률 **≥0.95**(앙상블 축). (이월)
...
- [ ] **F-1 `ensemble_decisions`에 실제 진입 계약수 축 신설** —
...
- [ ] **`tests/test_477_entry_qty_attribution.py`** 3종 — ① `qty_display != qty_auto` 스냅샷에서
  `entry_qty_final == qty_auto` ② `entry_qty`는 종전과 동일 ③ 컬럼 부재 DB에서 마이그레이션 후
  INSERT 성공.
...
- [ ] **`NEXT_TODO` P-4 판정 기준 문구 정정** — *"§12 `ConfFloor` 관측률 ≥0.95"* →
  *"§12 `ConfFloor` **당일** 관측률 ≥0.95"*. 현행 문구로는 오늘 실제 1.00인데 FAIL로 읽힌다.

### 🟡 P2 — Fix (이월, 변동 없음)

- [ ] **F-3 `git push origin dev`** (사용자, 윈도우에서 라이브 종료 후) — **ahead 5 유지, 6거래일째**.
  미푸시 `8972dcd`·`7a335ac`·`8a158a4`·`172395e`·`77fe6e8` 전부 `[MW0602] 476차`.
  🔴 **우선순위 상승** — `7a335ac`가 오늘 P-1로 **효과를 실증했다**. 푸시 전까지 MW0601(`v9-dev`)은
  같은 사이드카 결함을 그대로 안고 돈다.
- [ ] **F-4 `TASK_CLAUDE_WAKE_INSTALL.bat` 실행** (사용자 1회, MW0602) — **P-7 5거래일 연속 미충족**.
- [ ] **477차 장전 F-1·F-2 (`increment_session()` 키 보존 + EKS 마커 폴백)** — 이월.
  🔴 **오늘 밤이 마지막 적용 기회다** — 지금 `session_state.json`은 10키 정상이나
  **내일 08:55 롤오버에서 3키가 다시 사라진다**(O-1·O-2가 그 예측을 판정한다).
...
- [ ] **G-3 `MainThread` 스택 1프레임 덤프** — `[Shutdown] 잔존 스레드` 직후
  `faulthandler.dump_traceback()`(이미 활성, `logs/crash_fault.log`)로 한 번만.
  ⚠ **처방이 아니라 관측 강화다.** 기존 결정(*"코드 변경 없음, 진단만 먼저"*)은 유지.
  COM 콜백 밖(종료 경로) — 절대원칙 ④ 저촉 없음.
- [ ] **26주 WFA 재검증 목록에 「계측 배포일 마커 갱신」 행 추가** (`invariants.md` §5) —
  G-1의 `measured_since`가 낡는 것을 잡는다. **새 관리 포인트를 만들지 않고 기존 26주 주기에
  흡수한다**(317차 원칙, 468차 G-2와 같은 뿌리).
...
- [ ] **`evidence_map.md` §8-2 `ensemble_decisions` 컬럼 설명 보강** —
  *"`entry_qty` = **산출 수량**(증거금 캡 이전). 실제 진입은 `entry_qty_final`(477차~) 또는
  `sizing_trace.qty_auto`(08-14~) 또는 `trades.entry_qty`"*. F-1과 같은 커밋.
- [ ] **`CLAUDE.md` 전환기준 ⑧ [28] 문단에 선행조건 1줄 추가** —
  *"[28] 판정 전에 `entry_qty` 귀속(477차 F-1)과 `MarginCap` 상태 샘플(G-2)이 배선돼 있어야 한다."*
...
전문: `docs/정기점검/매일점검/MW0601-20260820-점검리포트-pre.md`
증거: `docs/정기점검/매일점검/evidence_MW0601-20260820_pre.md`
**P0 없음.** 아래 전 항목 **적용 시점 = 15:10 강제청산 이후**(라이브 프로세스 PID 13140 가동 중).

#### Fix

- [ ] **F-2 (P1, 최우선 · 코드 변경 없음) `ofi_norm` 분포 프로브 실행** —
      `python scripts/ofi_norm_distribution_probe.py --days 20 --out docs/정기점검/`.
      2026-08-19 신설 후 **한 번도 실행된 적 없다**(산출물 부재) — FP-CRITICAL·
      TOX-SEVERE-SPREAD와 같은 "만들어 놓고 안 돌린 계측"이 되기 직전.
      판정: raw σ가 구간 경계에서 **계단식**이면 코드/수집경로, **완만**하면 미시구조.
      ⚠ `guard_intraday()`가 장중이면 rc=2로 막는다 — **우회하지 말 것.**
- [ ] **F-1 (P1) 프리장 수급 미측정 플래그 + 스케일러 제외 섀도** —
      `collection/cybos/investor_data.py` 반환 dict에 `institution_futures_net_measured` ·
      `foreign_futures_net_measured` · `retail_futures_net_measured`(bool) 동반(값 0은 하위호환 유지) →
      `model/multi_horizon_model.py` 미측정 행을 **해당 피처에 한해** 제외 + `[ScalerRefresh] … 제외 n행(미측정)` →
      `config/settings.py:INVESTOR_UNMEASURED_SCALER_EXCLUDE_ENABLED = False`(**섀도 우선**).
      ⚠ pickle 포맷 변경 금지(py310_64 재학습 protocol=4 호환). COM 콜백 밖이라 절대원칙 §4 저촉 없음.
      검증: `tests/test_481_unmeasured_investor.py` / 다음 거래일 09:01 `max_z` |z|<8 / 제외 로그 08:45~08:59 한정.
- [ ] **F-3 (P2, 0819 2-1 이월 · 2일차) CybosProbe CoInitialize + 실패사유 3분류** —
      `collection/cybos/api_connector.py` 프로브 워커에 `pythoncom.CoInitialize()` / `try-finally CoUninitialize()`
      (기존 `:176`·`:386` 패턴 복사), 결과를 `미시험(COM 초기화 실패)` / `미발견(-2147221005)` / `ok` 로 분리.
      `collection/cybos/investor_data.py` `reason=` 을 "프리장 미제공(09:00 이후 재시도)" vs "TR 미발견" 으로 분리.
      ⚠ **콜백 안에 `dynamicCall`·`emit` 금지**(절대원칙 §4). 1거래일 `crash_fault.log` 0xC0000409 감시 필수.
      검증: 다음 거래일 08:58 PROBE 로그 `-2147221008` **0건**.
- [ ] **F-4 (P2, 0819 F-1 이월 · 7일차) 수집기 브랜치 스코프 분리** —
      `collect_evidence.py` 불변식 기대표에 `branches: ["v9-dev"|"dev"|"*"]` 추가.
...
리포트: `docs/정기점검/매일점검/MW0601-20260820-점검리포트-intra.md`
🔴 **전 항목 장후 적용.** 장중 라이브 프로세스(PID 13140) 가동 중이라 코드·커밋 0건.

#### Fix

- [ ] **F-1 (P1, 장후) 수집기 §5 부분청산 레그 합산** —
      `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py:133`
      `PATTERNS["exit"]` 를 `체결(부분)?청산` 으로 확장 + `partial` 그룹.
      `:911` 렌더러를 **2단화**: `청산 이벤트 n건(레그)` / **`포지션 n건 · 포지션 합계 ±원`**.
      컬럼 머리글에 `_per_leg` / `_per_position` 접미사(계측 4원칙 ①).
      §5 안내문의 "손절 준수율"에 **포지션 단위 사용**을 명시.
      §5 상단에 `[집계 개정 2026-08-20]` 마커(461차 MDD 개정과 동일 — 과거 날짜본 불연속).
      **검증**: 08-20 재실행 기대값 = **포지션 1건 · -269,884원**(현행 -147,628원).
      08-19·08-18 로그로 익절 분할 포지션이 1건으로 묶이는지 확인.
- [ ] **F-2 (P2, 장후 · F-1과 같은 커밋 가능) 스킬 참조문서를 CLAUDE.md 461·474차에 정합** —
      `references/phases.md` B-1의 *"T-30은 퇴역 대상"* 을 **뒤집는다** →
      *"30m 채점 유지가 정상(`main.py:5539`가 CB③ `record_accuracy()` 입력으로 사용,
      474차). **30m 채점이 끊긴 것**이 점검거리"*.
      `references/invariants.md` §1-②-③ *"30분 정확도 < 35%"* →
      *"30분 **방향성** 정확도 < **28%**(`CB_ACCURACY_MIN_30M`, FLAT 제외). ⚠ 35% 재인용 금지"*.
      phases.md B-4에 "conf 절대값을 구 기준표에 대지 말 것"을 체크박스로 승격.
- [ ] **F-3 (P1, 장후 · 조사만 — 코드 변경 없음) CB③-P4 판정 입력 결정** —
      `main.py:7598`이 원시 등급을 체크리스트 등급으로 덮은 뒤 `:7720` P4가
      `_final_grade == "C"` 만 본다 → **등급 인플레가 P4의 유일 사정권을 비운다.**
...
상세: `docs/정기점검/매일점검/MW0601-20260820-점검리포트-post.md`
당일 성적(포지션 단위): **4포지션 2승 2패(50.0%) net −348,018원** · P0 0건

#### Fix

- [ ] **F-1 (P1, 승격 — 선행 O-9 해소) `[DBG-CB]`에 표본 수·판정가능 여부 병기** —
      `main.py:7388~7393` 포맷에 `n=%d ready=%s` 추가(`n=_cb["cb3_samples"]`,
      `ready=(n>=CB_ACC30M_MIN_SAMPLES)`), `n==0`이면 `acc30m=—`.
      2단계로 `safety/circuit_breaker.py:618` 빈 버퍼 `None` 반환 + `accuracy_30m_or_zero`
      하위호환 별칭(461차 `mdd_pct` 패턴). 수집기 §5에 `CB③ ready 분 / 장중 분`.
      **왜**: 오늘 `acc30m<0.28`이 236분(64.0%)인데 HALT 0회 — 표본 부족인지 통과인지
      로그로 구분 불가. `max(len,1)`이 빈 버퍼를 0.0으로 위장(계측 4원칙 ②·④).
      ⚠ **판정 로직 무변경**(임계 0.28 그대로) — 계측만.
      **검증**: `tests/test_457_fallback_visibility.py` 확장 + 익일 369행 전부 `n=`·`ready=` 출현.
- [ ] **F-2 (P1, 승격 · 8일차) 수집기 설정 불변식 표를 브랜치별로 분기** —
      (구 F-4 / 0819 F-1 이월) 기대표 각 항목에 `"branches": [...]` 추가, 현재 브랜치와
      교집합 없으면 표에서 제외하되 `_이 브랜치 범위 밖 N건: …_`로 개수·이름 잔존
      (계측 4원칙 ③). 기본값 `["dev","v9-dev"]`. §11 승격 대상에서도 제외.
      **근거**: 5개 상수는 repo 전체에서 수집기 기대표에만 존재하고 `origin/dev`에만 실재
      (`git grep -c ZONE_ENTRY_BAN_ENFORCE origin/dev -- config/settings.py` = 2,
      `origin/v9-dev` 없음). **회귀 아님.**
      **검증**: v9-dev `미발견` 0건 / dev 5행 유지 / §11 15칸 → 10칸.
- [ ] **F-3 (P2, 신규) 메인 스레드 정지 전용 섀도 계측** —
      `config/settings.py` 신설 `MAIN_THREAD_STALL_WARN_MS=5_000` · `_ALERT_MS=15_000` ·
      `MAIN_THREAD_STALL_BLOCK_ENABLED=False`(**차단 없음**).
      `main.py` `_tick_header` 계측부 `[MainStall]` 태그에 `gap_ms`·`pipe_elapsed`·
      `in_pipeline`·`position_status` 병기. 수집기 §5 파이프라인 **내/밖 분리 표기**.
      **왜**: 오늘 ≥5,000ms 4건(최대 8,375ms) 전부 `pipe_elapsed=0` — CB⑤(파이프라인 5초,
      당일 최대 3,183ms)도 FZ-1(180초)도 못 본다. **5초~180초 구간에 주인이 없다.**
      ⚠ **CB⑤ 임계·판정 무변경**(절대원칙 §2). `_tick_header`는 Qt 타이머 슬롯 = COM 콜백 밖.
      ⚠ 게이트 성격이므로 **섀도 2주 선행 후** 차단 승격 여부는 주간회의.
- [ ] **F-4 (P1, 기등록 유지 · 2일차) 수집기 §5 포지션 단위 집계 + 정합성 등식** —
      (481차 후속 F-1 그대로) 추가할 것은 **정합성 등식 자동 체크**:
      `§5 합계 + 은닉 레그 합 = DB 포지션 합 = 전략 리포트 net`
      (오늘 검산: `−230,004 + (−118,014) = −348,018` ✓).
      **오늘 장후에서도 재현** — 은닉 3레그가 당일 순손실의 **33.9%**, 승률 25% vs 실제 50%.
- [ ] **F-5 (P2, 기등록 유지) 수집기 CRLF 내성** — 코웍 리눅스 샌드박스에서 `461건` 착시.
      실측은 `--ignore-cr-at-eol` 기준 **2 files +416**. 적용 전까지 매 리포트에서 정정할 것.

#### 고도화

- [ ] **G-1 (이번 주, 선행 F-1) CB③ 판정 가능 시간을 일일 지표로 승격** —
      `[ModelHealth]`에 `CB③ ready=N분/369분`, `daily_stats.cb3_ready_minutes` 신설.
      **3거래일 연속 ready < 50% → §11 적신호.**
      **왜**: "CB③이 실제로 살아 있는 시간"이 처음 시계열이 된다 → 전환기준 ⑥ 논의 근거.
      ⚠ 계측 전용, 판정 무관여.
- [ ] **G-2 (다음 주, 선행 G-1) 스케일러 재적합 ↔ CB③ 버퍼 상호작용 계측** —
      `[ConstOut] … → acc30m 버퍼 리셋` 로그에 **리셋 직전 버퍼 크기 + 직전 ready 지속 분**
      병기, EOD에 `재적합 N회 → CB③ 표본 손실 M건` 집계.
      **왜**: 오늘 3m 재적합 6회가 CB③ 리셋 판단 6회를 유발(실제 리셋 2 / 기아방지 스킵 4).
      **두 계측이 서로를 침식하는데 그 관계가 어디에도 기록되지 않는다.**
- [ ] **G-3 (다음 주) 진입 후보 시간에서 `ConfFloorGuard` 구간 분리 카운트** —
      `후보 62분 (그중 도달불가 N분)` 형태.
      **왜**: 오늘 `[ConfFloorGuard] 자동진입 하한 도달 불가 — 보정기 출력상한 0.3528 <
      필요 0.3790` 1회(09:06). 지금은 "기회가 없었다"와 "구조적으로 불가능했다"가 같은
      숫자에 섞여 5일 평균 하한 경보(53분 < 60분)의 오탐/정탐을 구분할 수 없다.
...
> 브랜치 `v9-dev` · HEAD `0be0eaa` · ahead **0**(push 완료).
> 리포트: `docs/정기점검/매일점검/MW0601-20260821-점검리포트.md` (**하루 한 파일 첫 적용일**).
> 근거 전문: `dev_memory/DECISION_LOG.md` 2026-08-21 483차.

#### 🔴 Fix

- [ ] **P2-A (신규) 런처 단일 인스턴스 가드 진단 출력을 로그 파일로 영속화** —
      `start_mireuk.bat:435` 인라인 파이썬의 `print('[GUARD] 실행 중 main.py: N')` +
      `print('  PID={} cmd={}')` 가 **콘솔로만** 나가고 `!_BLOG!`에 안 남는다
      (로그 헬퍼는 610행 `:L` 뿐, `2>NUL`은 stderr만 죽인다).
      실측: 오늘 런처 로그 `grep "실행 중 main.py\|PID="` → **일치 0건**.
      재발 **6거래일 6/6**(0814·0817·0818·0819·0820·0821).
      ⚠ **"어제 자동종료 실패"로 설명 안 된다** — 08-20 15:40:22 `[System] 자동 종료 실행` +
      `[Shutdown] 정상 종료 플래그 기록 (auto_shutdown)` 이 **있는데도** 08:40에 죽일
      대상이 있었다. ①저녁 재기동 ②종료 실패 **구분 불가**가 이 항목의 본질이다.
      (NEXT_TODO:13542의 "08-17 저녁 수동 기동분" 설명은 **08-20에 전용 불가** —
       그날은 저녁 런처 로그가 없다.)
      **How**: 435행을 `> "%TEMP%\_mireuk_guard.txt" 2>NUL` 로 받고
      `FOR /F "usebackq delims=" %%G IN (…) DO CALL :L "%%G"` 로 흘린다 +
      대상 PID의 `psutil.Process(pid).create_time()` 병기.
      ⚠ `sys.exit(1 if procs else 0)` 반환 규약 불변(436행 분기). `%TEMP%` 폴백 `logs\Mireuk_batch\`.
      ⚠ stdout 리다이렉트가 `ERRORLEVEL` 전달을 막지 않는지 cmd.exe 실측 필요.
      **Why**: 계측 4원칙 ③(탈락 가시화). 480차 전환기준 ② ⓑ가 **런처 재기동의 관측
      가능성**을 전제하는데 그 창구가 비어 있다.
      **적용: 장후.** **검증**: 다음 거래일 로그에 `PID=… started=…` 출현 → 2거래일 후 O-6 판정.

- [ ] **P2-B (기등록 F-5 승계) 수집기 CRLF 내성** — `미커밋 463건` 중 **463건 전부 착시**
      (`git diff --ignore-cr-at-eol --shortstat` 출력 없음 · `--cached` 도 없음).
      착시율 **100%** · **3일 연속**.
      **How**: `scripts/collect_evidence.py` §2 건수를 `git diff --ignore-cr-at-eol --numstat`
      (+`--cached`) 합산으로 교체하되 **원 건수를 지우지 않고 병기**
      (`미커밋 463건 (CRLF 제외 실측 0건)` — 계측 4원칙 ②·④).
      §11 자동 적신호는 **실측치 > 0일 때만** 올린다.
      ⚠ `--numstat` 은 rename/binary를 `-`로 내므로 파싱 방어 필요.
      **검증**: 코웍 리눅스 × 윈도우 양쪽 대조(윈도우는 두 값이 같아야 한다).

- [x] **P2-C (신규) NEXT_TODO 주간회의 기한 요일 정정** `[DONE 2026-08-23 483차]` —
      ⚠ 원 표기 4곳의 **행 번호가 밀려 있었다**(14642·14778·14895·15013 → 14675·14817·14940·15064).
      귀속도 정정: `git log -S` 실측 결과 **481차가 아니라 MW0602 476차(`0215f6c`)·477차(`44e2652`)**.
      정정 표기(취소선 + 사유 인용문)로 적용했고 원문은 지우지 않았다. 원안: 14642·14778·14895·15013행
      `주간회의(2026-08-22 금)` → **08-22는 토요일**, 실제 금요일은 **08-21(오늘)**.
      코드는 요일로 건다(`scripts/eod_retrain.py:63` "금요일에만 자동 실행").
      **덮어쓰지 말고 정정 표기**:
      `~~2026-08-22 금~~ → **2026-08-21 금**(08-22는 토 — 481차 표기 오류, 483차 정정)`.
      ⚠ **CB② 재검토 기한 2026-08-29도 토요일**이다. 실무 판정은 08-28(금)에만 가능하나
      CLAUDE.md 절대원칙 §2 한시예외 문구 수정이므로 **주간회의 안건**(아래).

#### 🟢 고도화

- [ ] **G-1(483) 아침 CORE 스케일러 폴백을 "어느 피처가 며칠째"로 시계열화** —
      08:45 EarlyWarmup 창 실측이 `24건(4피처) → 12건(2피처) → 6건(1피처)`로
      3거래일 단조 감소했는데 **오늘 손으로 세어서 처음 보였다.**
      수집기 §6은 표본만 인용하고 §11은 건수만 센다.
      **How**: 수집기에 `core_scaler_fallback` 절 신설 —
      `{피처명 → (당일 건수, 최근 5거래일 건수, raw_std)}` 표. **판정 없음, 표만.**
      **Why**: 26주 WFA L1은 IC 축이라 "분산이 사라지는 시간대"를 평균에 녹인다.
      두 축은 서로를 대체하지 않는다.
      ⚠ **처방 아님** — `raw_std` 임계 변경 제안이 아니다(462차 P4 "처방이 아니라 등록이다").

- [ ] **G-2(483) 런처 가드 이벤트를 하트비트 JSON에 편입** —
      `data/heartbeat_MW0601_YYYYMMDD.json`에 `launcher_killed: [{pid, started_at, killed_at}]`
      append-only 키 추가(FZ-1이 이미 PID를 안다).
      **Why**: 전환기준 ② ⓑ의 검증 자료가 지금 반쪽이다. P2-A가 최소 조치, 이것이 기계 판독본.
      ⚠ **하트비트는 FZ-1의 생존 판정 입력이다.** `beat_age_sec`·`strikes`·`fired` 읽기 경로를
      건드리면 장중 `os._exit(43)` 오발동. **`scripts/freeze_watchdog_rehearsal.py` 로
      발화 판정 재확인 후 배포.**

- [ ] **G-3(483) 장전 점검이 "오늘 금요일"을 스스로 인지** —
      `scripts/collect_evidence.py --phase pre` 에
      `오늘은 금요일이다 — EOD 체인이 검증 캠페인 주간 리포트를 생성할 예정
      (직전: …_20260814.md, 7일 전)` 배너 1줄. `weekday()==4` + 폴더 glob.
      **DB 미접근이라 장전에도 안전하다.**
      **Why**: 캠페인 상시 전환 뒤 남은 유일한 리듬이 금요일인데, 그 리듬을 사람의 기억과
      손으로 적은 기한 문구에만 맡긴 것이 P2-C의 구조적 원인이다.
...
- [ ] **P2-E 메인 스레드 정지 일별 집계 승격 (P2 · 이상점 1-5)** —
      `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py` §5 「메인 스레드 블로킹」에
      **직전 5거래일 동시간대 대조 열** 추가(`| 일자 | 전체 | 5초초과 | 최대 | band=WARN 비율 |`).
      기존 `stall_ms` 파서·CB⑤ 잔차 표 로직 재사용, 날짜 토큰으로 파일 목록만 확장.
      ⚠ 로그 없는 날짜는 **`—`(미측정)** 으로 — **0으로 채우지 말 것**(계측 4원칙 ②).
      검증: `--phase post --pc MW0601 --out-auto` → 0821 행 `21/9/9922ms`, 0817 행 `1/0/2781ms`.
      **왜**: 417차 `[Sizer]` "379건 중 86건(22.7%)"이 손으로 한 번 센 값인 채 매주 재인용된
      전례가 정확히 이 경로다(471차 후속6/G-1이 `sizing_trace`를 만든 이유).
- [ ] **P2-F `_acc30m_stage` 변경에 사유 로그 필수화 (P2 · 이상점 1-7 · 계측 4원칙 ④)** —
      `safety/circuit_breaker.py` 상태 대입 3곳(`:344` 정상전환 / `:579` 버퍼리셋 /
      `:650` 일일리셋) 중 **로그는 :344 하나뿐**. 단일 헬퍼 `_set_acc30m_stage(new, cause)`로
      모으고 `cause`(`accuracy` / `buffer_reset(scaler_refit)` / `daily_reset`)를 로그와
      `to_dict()`(`:687`)에 **같은 행으로** 병기.
      문구 예: `[CB③-P4] acc30m 단계 전환: RESTRICTED → NORMAL (사유=버퍼리셋 · 표본 0/30 — 정확도 회복 아님)`
      ⚠ `:579`는 반드시 `_accuracy_buf.clear()` **뒤** 유지(리셋 전 스냅샷 관례,
      `main.py:daily_close()` `_ccf_today`와 동일 이유).
      검증: ①유닛(리셋 직후 `cause=buffer_reset` 로그) ②`tests/test_457_fallback_visibility.py`
      포함 CB 테스트 전량 ③라이브(다음 `ConstOut` 재적합 시 `[CB③] 버퍼 리셋` + `[CB③-P4] … 사유=버퍼리셋`
      **같은 초** 출현).
      🔴 **전환기준 ⑥(`CB3_P4_GRADE_BLOCK_ENABLED` 복원)의 선행 작업으로 등록.**
      복원 전 이 구분이 없으면 [2]가 잰 「판정 불가 158분」이 "정상이라 NORMAL"로 오인돼
      C등급 자동진입이 무근거로 열린다.
- [x] **P2-G `references/phases.md` B-1 「T-30은 퇴역 대상」 문구 정정 (P2)** `[DONE 2026-08-23 483차]` —
      B-1 외에 **B-3 「(30m 제외 상태 확인)」도 함께 정정**했다(같은 오독을 부르는 문구 —
      원안 범위 밖이나 같은 결함이다). 원안:
      CLAUDE.md 절대원칙 §3 말미(*"30m 학습 중단 금지 — CB③의 유일한 입력원"*, `main.py:5539`)와
      **정반대**다. 그대로 두면 다음 세션이 30m 채점 180건을 이상점으로 올린다(461차 CB③ "35%"와
      같은 문서 지체 패턴). → *"T-30 채점은 유지되는 것이 정상. **끊겼다면** 그것이 점검거리"* 로 교체.
      **장전 P2-C(NEXT_TODO 요일 정정)와 같은 커밋으로 묶을 것.**
- [ ] **테스트 축 확장 검토 (P2, 설계 미착수)** — `tests/test_457_fallback_visibility.py`는
      *"할당이 없는 속성"* 은 잡지만 *"할당에 로그가 없는 것"* 은 못 잡는다(`_acc30m_stage`가
      :107 초기화 + 3곳 할당으로 **통과**했다). 상태 변수 대입 지점 대비 로그 부재를 검사하는
      축을 추가할지 **주간회의 안건**. 이번엔 등록만.

**고도화 (사전등록 필수 · 라이브 변경 금지)**

- [ ] **고도화 ④ CB③ 판정 가능 시간 × 스케일러 재적합 스케줄 (G-2 후속)** —
      실측 근거: `ready=Y` 51/209분(24.4%), 첫 `ready=Y`가 개장 **100분 뒤**, 50분 만에
      11:31:01 재적합이 30건 전량 폐기. 09:37은 4건이라 기아 방지가 막음 → **타이밍 우연**.
      또한 재적합 `scope`가 **3m·5m인데 30m 버퍼를 버린다**(무효화 근거 약함).
      섀도 제안: `_reset_accuracy_buffer()`에 ①`scope`에 `30m` 미포함 시 리셋 스킵
...
- [x] **O-13 (장후) GBM 배치 재학습 「30분마다」 조건 확인** `[DONE 2026-08-23 483차 후속1]`
      — 판정: **정기 30분 스케줄은 코드에 없다.** 트리거 6종(`ConstOut` 후속 ·
      `DriftRetrain` · `WarmupRetrain` · `PreRetrain` · `periodic` 주간/월간) 전부 조건부.
      문서 3개 정정 완료(F-8·P2-J). 원문:
...
- [ ] **P1-A `ConfFloorGuard` 3상태 카운터를 리셋 전에 읽는다 (P1 · 이상점 1-9)** —
      `main.py:11667` `_reset_model_health_counters()` 가 `main.py:11717~11720` 읽기보다
      **먼저** 실행돼 세 칸이 매일 `0분 · 0분 · 0분`으로 찍힌다. 482차 후속3(`74191d6`)
      배포 후 **첫 온전한 거래일에 즉시 재현**. 형제 카운터 `_mh_cb3_ready_minutes`는
      `_snapshot_model_health()`(`main.py:2276`대)로 리셋 전에 빠져나가 정상
      (`CB③ ready 111분/369분`) — **세는 코드는 옳고 읽는 자리만 틀렸다.**
      권고안: `_snapshot_model_health()` 반환 dict에 `cfg_reachable_min` /
      `cfg_unreachable_min` / `cfg_unmeasured_min` 3키 추가(로그 전용 — `daily_stats`
      스키마 무변경), 경보가 그 dict를 읽게 한다. **`cb3_ready_minutes`와 같은 자리·같은 규약.**
      동반: **`tests/test_483_daily_close_snapshot_order.py` 신설** —
      `daily_close()` 소스에서 `_reset_model_health_counters()` 호출 줄보다 **뒤에서**
      `self._mh_` 속성을 읽으면 실패. `test_457_fallback_visibility.py`가 못 잡는 축
      (그쪽은 `getattr` 미할당 속성, 이쪽은 **호출 순서**)의 일반 검출기다.
      검증: 다음 거래일 15:40 경보 세 칸 합 = 그날 파이프라인 분수.

- [ ] **P2-H `monthly_cleanup.py` 표준출력 UTF-8 강제 (P2 · 이상점 1-10)** —
      `scripts/monthly_cleanup.py:365~366` 배너 `print`의 em-dash(U+2014)가 cp949 파이프에서
      `UnicodeEncodeError` → **정리 로직 실행 전 즉사**(`rc=1`). 479차(`59c516a`)가 만든
      EOD 발화 지점의 **첫 자동 실행이 오늘이었고 첫 시도에서 실패**.
      ⚠ **em-dash 한 글자 치환 금지** — 같은 파일 cp949 불가 문자 `— ≈ ═ ⚠ 🔴` **5종 359회**.
      ① `main()` 첫 줄 `sys.stdout/stderr.reconfigure(encoding="utf-8", errors="replace")`
      (try/except 감싸기) ② **근본 처방** — `scripts/campaign_steps.py` 서브프로세스 실행부에
      `env={**os.environ, "PYTHONIOENCODING":"utf-8"}` (나머지 11스텝 예방).
      ⚠ 수정 후 첫 실행은 **반드시 `--apply` 없이** 삭제 대상 목록 육안 확인.
      검증: `PYTHONIOENCODING=cp949` DRY-RUN 완주 → 마커 여전히 비어 있음 →
      2026-08-28 EOD `월간 로그 정리=OK` → 마커 `202608`.
      실측 압박: `logs/` **2.0GB · 880파일**, 단일 최대 62.7MB.

- [x] **P2-I `trades.exit_stage` 컬럼 신설 — 손절과 TP1-후 트레일을 가른다 (P2 · 이상점 1-11 · 계측 4원칙 ①)** — `[DONE 2026-08-24 491차 — F-I 로 구현]`
      오늘 4포지션 전부 `exit_reason='하드스톱(틱)'` 인데 **3건이 TP1 보호전환 후 이익 확정**
      (오염률 75%). 진짜 손절은 13:53:08 1건뿐(초과율 6.4% = 0.12pt = 6틱, **표본 1건 —
      확정 결론 금지**). ⚠ **`exit_reason` 문자열은 바꾸지 않는다** — 소비처
      (`[ExitCooldown]` · CB② 연속손절 카운터 · 캠페인 다수)가 많고 소급 분석이 깨진다.
      대신 컬럼 추가: `INITIAL_STOP` / `TRAIL_AFTER_TP1` / `TIER1_EARLY` / `TP1` …
      과거 행 NULL = **미측정**(계측 4원칙 ②).
      동반: **수집기 자동 적신호 2번 문구 정정**(오늘 75% 오탐) + §5 청산사유 분포에
      `exit_stage` 소계.
      검증: 오늘 4포지션 재생 → `TRAIL_AFTER_TP1`×3 · `INITIAL_STOP`×1 /
      CB② 연속손절 카운터가 오늘과 동일(1,1,2) 유지.
      확인 필요: 캠페인 [15]([TP1미도달] 조건)·[24](`mode` 분리)는 **현재 안전해 보이나 미확인**.

- [x] **P2-J CLAUDE.md STEP 3 「30분마다」 문구 정정 (P2 · 이상점 1-16 · O-13 승계)** `[DONE 2026-08-23 483차]` —
      원안이 빠뜨린 **주간(월 08:50)·월간(1일 07:00) `periodic` 경로**를 트리거 표에 함께 적었다.
      `phases.md` B-2도 같이 고쳤다. 원안:
      정기 30분 스케줄 상수·타이머가 **코드에 없다**(`GBM_RETRAIN_INTERVAL` 등 전수 0건).
      오늘 장중 재학습 4회 전부 `ConstOut` 조건부(09:36:59 3m / 11:29:59 5m / 13:04:00 3m /
      14:57:00 3m, 간격 113·95·113분). 다른 경로도 전부 이벤트 구동
      (`[WarmupRetrain]` · `[PreRetrain]` · `[DriftRetrain]`).
      → `STEP 3: GBM 배치 재학습 (조건부 — ConstOut 확정 / 방향 드리프트 / 세션 재시작 /
      08:55 사전. 정기 스케줄 아님)`. 461차 F-3 형식의 정정 주석 병기.
      ⚠ **절대원칙 6종 본문 무변경** — STEP 3은 절대원칙이 아니다.
      `references/phases.md` B절 동일 문구 함께.
      **P2-C(주간회의 기한 요일)·P2-G(phases.md B-1 T-30 문구)와 한 커밋으로 묶어 구현 권고**
      — 셋 다 문서, 회귀 위험 0.

- [ ] **P2-K 수집기 §2에 실질 변경 파일 수·파일명 병기 (P2 · 이상점 1-14 · F-5/P2-B 강화)** —
      `git status --short` 원시 474건 옆에 `git diff --ignore-cr-at-eol --numstat` 기반
      실질 변경(오늘 **4건**: dev_memory 2 append + FIFO 자동삭제 2)을 병기하고 파일명을
      나열. 절단 시 `… 외 N개`로 잔여 개수 명시(계측 4원칙 ③).
      ⚠ **착시율 분모를 ` M` 추적 수정분으로 한정할 것** — 오늘 실측
      ` M` 444 / ` D` 2 / `??` 29 이며 착시율은 **442/444 = 99.5%**다.
      「474건 중 100%」는 분모에 미추적·삭제가 섞인 잘못된 표현이었다(계측 4원칙 ①).
      **기등록 F-5/P2-B와 묶어 구현 권고.**

**고도화**

- [ ] **고도화 ⑦ CORE 피처 건강도 일별 1행 (`core_feature_health`)** —
      오늘 처음 **같은 날 두 독립 계측이 같은 결론**을 냈다: ①분포 —
      `08:45:18 [ScalerRefresh] CORE 'cvd_divergence' raw_std≈0(0.0225) → identity(0,1) 강제`
      6 호라이즌 전부 ②기여도 — `shap_tracker.db` 오늘 `cvd_divergence` **62행 전부 0.0**
      (`vwap_position`만 1m/3m/5m 전부 존재) ③진입 — `cvd❌` 2건, `[차단]` `4_cvd` 16건.
      CLAUDE.md 371차 잔여 이슈(*"91% 점질량 … CORE 피처 자체 조사 필요"*)가 두 달째 열려 있다.
      제안: CORE 3종 × 호라이즌별 `raw_std` · `identity_fallback` · `shap_value` ·
      `scaler_floor_hits` · `checklist_fail_count` 5값 적재. **네 값 다 이미 찍히고 있다** —
      흩어져 있어 "며칠째 어느 CORE가"를 못 볼 뿐. O-3의 수작업 자동화이기도 하다.
      ⚠ **절대원칙 §3 — CORE 교체 불가.** 이것은 교체 판단이 아니라 계측이며, 결과가
      나빠도 처방은 **피처 정의 재설계**(371차 방향)다. O-14에서 확인했듯 SHAP 교체후보
      산출은 `if not e["is_core"]`로 CORE를 원천 배제하므로 교체 경로를 열지 않는다.
      ⚠ 오늘 SHAP=0.0은 **재인용 금지 대상 아님**(416차가 무효화한 것은 정렬 오염 구간
      2026-06-17~07-10). **단 표본 1일 — 확정 결론 금지.**

- [ ] **고도화 ⑧ 캠페인 [28]에 `binding_gate` 빈도표 추가 (판정 미관여 게이지)** —
      오늘 `sizing_trace` 실측: 진입 4건의 `binding_gate` = `hurst`×3 · `meta`×1,
      `tox_size` 전부 1.0(무구속), `max_contracts=3` 접촉 0건.
      417차 서술 *"meta·tox 배수에 눌림"* 구도가 **431차 min() 합성 전환 이후 바뀌었을
      가능성**이 처음 데이터로 보였다. [28]은 현재 `qty` 버킷만 보고 `binding_gate`
      분포를 안 본다. 주간·누적 빈도표 + 게이트별 평균 삭감 계약수.
      ⚠ **합격선 무변경 · 판정 미관여**(§9 사전등록).
      ⚠ `sizing_trace`는 2026-08-14 이후 행에만 존재 — 이전 NULL = **미측정**(계측 4원칙 ②).
      ⚠ 417차 `[Sizer]` 379/86(22.7%)은 **재인용 금지** — 비교에 쓰지 말 것.

- [ ] **고도화 ⑨ `RECALIBRATOR_DECISIONS` 레지스트리 신설** —
      재보정기 3종(`ThresholdRecal` · `ATRCeilingRecal` · `EntryHorizonRecal`)에는
      캠페인의 `VALIDATION_CAMPAIGN_DECISIONS` 같은 「확정 결정」 장치가 없다.
      그래서 `EntryHorizonRecal` **5주 연속 UPDATE**(07-24 +38/+68% → 07-31 +278/+289% →
      08-07 +115/+131% → 08-14 +150/+174% → 08-21 **+79/+95%**)인데 상수는
      387차 이후 `3.2`/`4.4` 무변경이고, **미이행인지 의도적 동결인지 확정 불가**다.
      제안: 구조는 캠페인 레지스트리와 동일. 로그·리포트가 **현행값 · 권고값 ·
      마지막 결정 · 연속 권고 주수**를 함께 찍는다 —
      `[EntryHorizonRecal] UPDATE (5주 연속) — 현행 3.2/4.4 · 권고 5.72/8.57 · 마지막 결정: 없음`.
      **「결정 없음」이 명시적으로 보이면 그 자체가 안건이 된다.**

- [ ] **고도화 ⑩ `[MainStall]` 섀도 분석에 판별 질의 2개 **사전등록**** —
      오늘 5초 초과 15건이 5분 격자에 몰렸고(초 단위 :04~:09), 그중 3건(12:22·12:27·12:37)이
      장중 점검 세션(12:26~12:40, 수집기 12:27:02)과 겹친다. **그러나 10:19~10:49의 5건은
      점검 세션이 없다** — 가설 A(주기 작업)와 가설 B(점검 자기유발) 어느 쪽도 확정 불가.
      **처방이 정반대이므로 원인을 가르기 전에 손대면 안 된다**
      (A → 주기 작업 스레드 분리 / B → 장중 IO 제한을 **로그 파일 읽기까지** 확장).
      2주 뒤 데이터를 앞에 두고 질의를 설계하면 **사후 슬라이스**가 된다([47] 판독규칙) →
      **지금 등록한다**: ①`evidence_*.md` 생성 시각 ±10분 창 안팎의 5초 초과 발생률 대조
      ②`stall` 시각의 `분%5`·`초` 분포 → 편중 시 `S6Detail`·`[SHAP]`·`[ScalerMonitor]` 교차.

**🔵 주간회의(2026-08-22 토 — ⚠ 실제 금요일은 08-21 오늘) 상정 — 483차 후속2 신규 3건**

- [x] **① 진입 호라이즌 경계 `ENTRY_HORIZON_B1/B2`** `[DONE 2026-08-23 483차 후속5]` —
      **동결 확정**(사용자 결정). 경보는 참고용으로 강등하고 판정은 `entry_band_watch` 가
      한다. 그 채널은 474차 사전등록 이후 **발화 지점이 없어 판정이 한 번도 안 나왔었고**,
      캠페인 스텝에 편입했다(스텝 11개). 근거·재개조건은 `config/settings.py:
      ENTRY_HORIZON_B1` 주석과 `VALIDATION_CAMPAIGN_DECISIONS["entry_band_watch"]`.
      **6주째 재등장은 이것으로 끝난다.** 원안:
- [ ] ~~**① 진입 호라이즌 경계 `ENTRY_HORIZON_B1/B2` — 갱신 / 동결 / 재검토 중 하나를 명시 결정하고 기록**~~ —
      5주 연속 UPDATE 권고. **최소한 "그대로 둔다"라도 기록해야** 여섯 주째 같은 안건이
      반복되지 않는다. 양쪽 근거가 다 있다 — 갱신 쪽: 5m 버킷 5주 내내 81~99.6% 점유
      (*"버킷 고착 조짐"*). 동결 쪽: 오늘 델타가 5주 최소, 5m 비중 99.6%→86.0% **하락 추세**,
      그리고 **오늘 실제 진입 4건의 `entry_horizon`은 3m×3 · 1m×1로 5m 0건**
      (버킷비중=전체 분봉 / 진입=필터 통과분 — 같은 것을 재지 않는다).
      ⚠ **매매 정책 변경**이며 `ATR_HORIZON_TP1_MULT`(1m 0.3/3m 0.5/5m 0.7)를 통해
      청산 기하까지 이어진다. 자동 반영 금지.
- [ ] **② 캠페인 [46] HurstGate 임계 위치 — 첫 FAIL 판정에 대한 결정 등록** —
      `⏳INSUFFICIENT`(08-14) → **`❌FAIL`(08-21)**. 현행 `abs_0.45`의 OOS 분리도 **+0.633이
      8개 중 최하위**, 최상 `abs_0.30` +1.751. **같은 날 오늘 진입 4건 중 3건의
      `binding_gate`가 `hurst`** 였다 — 우연이 아닌 겹침.
      ⚠ **지금 바꾸면 안 된다** — `abs_0.30`의 OOS 차단 표본 **n=3**. 확정 결론 금지(313차).
      "다음 주에 다시 본다"로 충분하되 **그 결정을 레지스트리에 남길 것**.
      ⚠ [6]의 📌결정(*"권고는 2026-07-15에 이미 이행됨"*)을 [46]에 옮겨 쓰지 말 것 — 다른 질문.
      ⚠ CLAUDE.md 26주 재검증의 Hurst 항목은 `HURST_WINDOW_N/MAX_LAG`(추정기)이고
      [46]은 `HURST_RANGE_THRESHOLD`(판별 경계) — 대상이 다르다.
- [ ] **③ `ATRCeilingRecal` 첫 UPDATE 격상 처분** —
      `CLEAR`(07-24) → `WATCHLIST`(08-07) → **`UPDATE`(08-21)**.
      floor 3.5→2.5(-28.6%) / ceiling 6.0→**4.0**(-33.3%). 실현 변동성이 설계 상하한
      아래로 내려앉았다는 뜻. ⚠ 이 재보정기는 매주 돌지 않는다(07-24·08-07·08-21만 기록) —
      **주기 조건을 코드에서 확인할 것**.

**관측 (다음 거래일·다음 금요일이 닫는다)**

- [ ] **N-1 (다음 장후) `ConfFloorGuard` 3상태** — P1-A 미적용이면 또 `0분·0분·0분`인지
      확인해 **매일 재현됨을 기록**. 적용 후면 세 칸 합 = 그날 파이프라인 분수.
- [ ] **N-2 (다음 장후) 메인 스레드 정지** — 5초 초과 건수·최대·`band=WARN` 비율.
      **오늘 15건이 일시 스파이크인지 새 기준선인지**가 관건. 2일 연속 10건 이상이면
      **P1→P0 격상 검토**(단독 판단 금지, 사용자 보고). 5분 격자 적중률 함께 기록.
      6거래일 기준선: 08-14 6 / 08-17 0 / 08-18 5 / 08-19 4 / 08-20 4 / **08-21 15**.
- [ ] **N-3 (다음 장후) CB③ ready 종일 비율** — 3거래일 연속 50% 미만이면
      기등록 항목 「전환기준 ⑥에 ready ≥ 50% 전제 추가」를 **주간회의 정식 안건으로 승격**.
      오늘 111/369=30%, 리셋 3회·표본손실 90건.
- [ ] **N-4 (다음 장후) `cvd_divergence` 이중 축** — ①`identity 강제` 건수·피처 구성
      ②`shap_tracker.db` 값이 또 전부 0.0인지. **2일 연속 두 축 동시면 371차 잔여 이슈
      P1 격상 검토.**
- [ ] **N-5 (다음 장후) 3m 호라이즌 / R-1** — `model_live_daily` 3m `live_acc`와 `ConstOut`
      분포. 오늘 최저는 **1m 0.2935**(3m 0.3771 회복). 3m이 다시 최저면 R-1 우선순위 재검토.
- [ ] **N-6 (2026-08-28 금) `[EntryHorizonRecal]` 6주차** — 하락 추세 지속이면
      「자연 수렴 중」, 재상승이면 「갱신 필요」. **두 해석을 가르는 회차.**
- [ ] **N-7 (2026-08-28 금) 월간 로그 정리** — P2-H 미적용이면 **또 `FAIL(rc=1)`이 나와야
      정상**(마커 미기록 = 자동 재시도 생존 증거). `data/monthly_cleanup_last_run.txt` 상태 확인.
- [ ] **N-8 (2026-08-28 금) 캠페인 [9]** — 오늘 차단 10건 resolve 후 n=37→47 근처에서
      **PASS 유지 여부**. 뒤집히면 이상점 1-8 재개.
- [ ] **N-9 (2026-08-28 금) 캠페인 [46]** — 2주 연속 FAIL인지, OOS 표본 증가 시
      `abs_0.30` 우위 유지 여부. **n=3짜리 우위는 아직 근거가 아니다.**
- [ ] **N-10 (매 장후 누적) 15:10 강제청산 실집행** — 누적 **0회** 유지.
      `[ForceExitPass]`→`[TimeExit]`→`[ExitAttempt]` 3연쇄. **`[SchedForceExit] … 안전망 발동`
      (ERROR)이 뜨면 P0.** F-1R 리허설 없이는 자연 발생을 기다리는 수밖에 없다.
      전환기준 ② ⓐ가 여기 걸려 있다.
- [ ] **N-11 (다음 장후) 등급 인플레 일자단위** — 오늘 원시C→최종A **3건**
      (1승 2패 -185,238원) / C 유지 1건(1승 +5,355원). **손익 결론 재론 금지** —
...
리포트 `docs/정기점검/매일점검/MW0601-20260826-점검리포트.md` (하루 한 파일 · 장전이 생성)
증거 `docs/정기점검/매일점검/evidence_MW0601-20260826_pre.md`
**장전 자격 판정 ✅ 정상** — 12관문 통과. P0 0건 · P1 2건 · P2 1건 · 확인필요 2건.

### 🔴 최우선 — 기한 임박

- [ ] 🔴 **CB② 복원 재검토 — 기한 2026-08-29(토) ⇒ 마지막 거래일 2026-08-28(금)**
      `config/settings.py:CB_CONSEC_STOP_LIMIT = 9999` (현재값 확인됨).
      오늘 포함 거래일 **3일**(08-26·27·28). **이번 주 금요일 주간점검 안건으로 반드시 상정.**
      근거 CLAUDE.md 절대원칙 §2 — "무기한 유예가 아니다 … 2026-08-29에 반드시 재론할 것".
      실전 전환 기준 ②·⑤가 여기 걸려 있다. ⚠ 이 항목은 **결정이 아니라 상정 의무**다(함정 ①).

### 장후 적용 (오늘, 사용자 지시 시) — 코드 변경은 15:10 이후

- [ ] **F-1** (P1) 점검 세션이 repo에 잠금 시체를 남기지 않게 (이상점 1-1)
      ① `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py` git 호출부에
         **`--no-optional-locks`** 부착 (`git --no-optional-locks status --short` / `-sb`)
      ② **자동 적신호 1번 문구 조건 분기** — `0바이트 AND git 프로세스 0개 AND 60초 경과`
         3중 충족 시 `🟢 시체 락 — 지워도 안전하다: del <repo>\.git\index.lock`,
         미충족 시 현행 유지. **판정 근거 3개 같은 줄 병기**(계측 4원칙 ④)
      ③ `SKILL.md` §2에 "수집기 실행 전 repo 쓰기 유발 git 명령 금지" 1줄
      ⚠ **스킬 원본과 예약 실행 로드 사본은 별개**(0825 1-6) — 두 곳 다
      검증: 샌드박스 재실행 후 `ls .git/index.lock` 없음 / 락 강제생성 직후엔 현행 문구 유지 /
            §2 `미커밋 N건 · 실질 변경 M건` 전후 동일

- [ ] **F-2** (P1) 런처 단일 인스턴스 가드 **재배선** (이상점 1-2)
      `start_mireuk.bat:457` 부근 `[GUARD]` 블록 → `scripts/guard_single_instance.py` 호출.
      **새로 설계할 것 없음** — 스크립트·유닛·리허설이 `35ed037`에 이미 있다. 되돌린 것은
      배치 배선 한 조각뿐.
      요구 동작: ① 종료코드 분리 `0`=없음/`1`=발견·종료성공/`3`=프로브실패 (4원칙 ②)
                ② `종료 완료`는 **실제 terminate() 성공 시에만**, 없으면 `기존 main.py 없음`
                ③ `p.pid == os.getpid()` 비교를 파이썬 소스 안에서 (cmd.exe에 `!` 미노출)
      ⚠ 회귀 중 — 런처는 08:40 자동기동 입구. ① `start_mireuk.bat.bak_494` 백업
        ② `--dry-run` 리허설(493차 후속7 동일 절차) ③ **리허설에서 terminate() 실호출 제외**
      발효: **내일 08-27 08:40 기동부터**(오늘 인스턴스 무영향)
      검증: O-3

- [ ] **F-3** (P2) 설정 불변식에 비용 모델 3행 추가 (이상점 1-3)
      `config/dailycheck_targets.json` — `FUTURES_COMMISSION_RATE`(0.000098104) ·
      `COST_MODEL_COMMISSION_RATE_PINNED`(True) · `FUTURES_COMMISSION_RATE_EFFECTIVE_FROM`("2026-08-25")
      Why: 오늘 §3 불변식 21행에 수수료율이 **없다**. 6개월간 아무 계측에도 안 걸린 값을
           고친 뒤에도 감시 목록에 안 넣었다 — FP-CRITICAL·TOX 섀도와 같은 계열의 재발
      검증: 내일 장전 다이제스트 §3에 3행 `일치`

### 고도화 (당일 관측 근거)

- [ ] **G-1** 비용 모델 **세대 배지** — 손익 표출 지점마다 `rate_gen` 병기.
      P&L 패널 `net … [rate_gen=2026-08-25]` · 캠페인 리포트 비용 항 머리글
      `(rate_gen=LEGACY_KIWOOM — 라이브와 다름)` · `trades.commission_rate_used`(493차 F-AD ④)를
      **표시까지 승격**(현재 소비처가 DB 안쪽에 머묾).
      **F-3′과 독립 — 승인 전에도 안전**(표시만, 판정 무관). 승인 전에 넣어야 전후 대조가 쉽다.
      근거: 오늘부터 라이브 0.0098104% vs 계측 0.0015%가 표시 없이 섞인다(461차 `mdd_pct` 반복 조건)

- [ ] **G-2** 개장 첫 분 「하한 vs 보정기 상한」 괴리 **시계열 적재** —
      매 거래일 09:00 첫 파이프라인에서 `cal_out_max` · `min_conf_raw` ·
      `min_conf_effective`(FQAdj 후) · `gap` · `guard_fired`(0/1)를 **경보 유무와 무관하게** 1행.
...
## MW0601 494차 후속 (2026-08-26 12:30 — 장중 점검)

### 🔴 장후 적용 (오늘, 사용자 지시 시) — 코드 변경은 15:10 이후

- [ ] **F-5 (P1)** 장중 설정 편집 감지·경고 — ① `main.py` `HealthPolicy` 리로드 실패를
      최초 1회 ERROR + 5분 요약 + *"라이브 설정이 {기동시각} 판본에 고정"* 병기
      (계측 4원칙 ④) ② `collect_evidence.py` §2에 **`config/*.py` mtime 장중 여부** 1행
      ③ `references/phases.md` B-7에 *"파일 mtime으로 확인"* 명기. **회귀 위험 없음(계측만)**
- [ ] **F-4 (P1) — 섀도까지만** 시간대 진입 금지 하드 게이트.
      `main.py:9136` `is_new_entry_allowed()` 분기 **위**에 `is_entry_zone(_tz)` 분기 추가,
      `TIME_ZONE_ENTRY_BLOCK_ENABLED=False`로 신설 + `ensemble_decisions` 1컬럼 적재.
      ⚠ **착수 전 `dev` 브랜치 `ZONE_ENTRY_BAN_SHADOW_ENABLED`/`ZONE_ENTRY_BAN_ENFORCE`를
      먼저 읽을 것** — 이름 다른 같은 기능 중복 생성 금지(489차 B-3 계열).
      ⚠ `is_entry_zone`는 이미 `main.py`에 임포트돼 있다(7081·7142). 새 상수 만들지 말 것.
      ⚠ **승격(=True)은 오늘 하지 않는다** — O-8 손익 대조 후 별건.
- [ ] **F-6 (P2)** `[ZeroDiag] 진입X 원인` 문자열이 `actual_min_conf`(전 게이트 적용 후)를
      인용하도록 교체 + 전/후 병기(`conf미달(0.443<mc0.440→진입)`). **판정 로직 무변경.**
      ⚠ **F-4 섀도 적재보다 뒤에** — 구조체 없이 문자열만 고치면 값이 굳는다(471차 F-3→G-1).
- [ ] **F-7 (P2)** `_run_effect_report_script` 장중 회피 또는 비블로킹화.
      ⚠ **482차 F-3 섀도 2주 종료 후 착수** — 지금 바꾸면 섀도가 재는 대상이 바뀐다. 택1은 주간회의.
- [ ] `tests/test_496_zone_entry_hard_gate.py` 신설 (F-4 도달성 불변식,
      474차 `test_473_core_group_reachability.py` 형태)

**적용 순서**: F-1 → F-2 → F-3(장전분) → **F-5 → F-4(섀도) → F-6** → (2주 뒤) F-7.

### 494차 후속 관측 항목

- [ ] **O-8** 🆕 점심 블랙아웃(11:50~13:00) 진입 **9건 / 6일**(08-03 3 · 08-04 1 · 08-05 2 ·
      08-24 1 · 08-25 1 · 08-26 1)의 **포지션 단위** 실현손익·승패·손절 준수율.
      ⚠ 표본 9건 — **확정 결론 금지**(313차). 목적은 F-4 승격 근거 축적이지 승격 자체가 아니다.
      **DB 접근 필요 → 장후.**
...
- [ ] **O-11** 🆕 절대원칙 §1 — 종가 포지션 보유 시 `[ForceExitPass]`→`[TimeExit]`→
      `[ExitAttempt]`가 15:10:00~05에 출현하는가. `[SchedForceExit] 안전망 발동`(ERROR)
      **미출현이 정상**. ⚠ 12:31 FLAT이라 미검증 이월 가능(471차 F-1R — 6개월 실행 0회)
- [ ] (승계) **O-3** 내일 장전 · **O-4·O-5·O-6·O-7** 장후

### 고도화 (당일 관측 근거)

- [ ] **G-3** 「선언-실효 대조」 정기 점검 신설 — 정책 딕셔너리 불리언 키의 소비처를 세어
      **대시보드/테스트/자기 모듈뿐인 키** 목록을 날짜본 적재. 26주 WFA 편입 제안.
      ⚠ 판정 무관 · 사전등록 합격선 무변경(458차 D6)
- [ ] **G-4** `TIME_ZONES` 미정의 공백(11:50~13:00) 제거 — ① `LUNCH_BLACKOUT` 명시 존
      (동작 무변경, 안전) 또는 ② `LUNCH_RECOVERY` 11:50 시작. **②는 매매 정책 변경 → 주간회의**
- [ ] **G-5** `dev`의 `PIPE_LATENCY_EXCLUDE_MODEL_SWAP` 이식 여부 → 주간회의.
      이식 전이라도 `Degraded 선제차단` 로그에 「직전 N초 내 모델 교체」 태그 병기는 즉시 가능

### 로드맵·실전전환 기준 반영 제안 (주간회의)

- [ ] 실전 전환 기준 ②에 **「차단 게이트가 선언대로 실제로 차단하는가」** 축 추가 검토
      — 오늘 1-4가 그 계열 **네 번째** 사례(FP-CRITICAL · TOX 섀도 · 474차 mid/long CORE · 오늘)
- [ ] 26주 WFA 주기 항목에 **G-3** 편입 검토
- [ ] `dev`→`v9-dev` 이식 안건 2건: `PIPE_LATENCY_EXCLUDE_MODEL_SWAP` · `ZONE_ENTRY_BAN_*`.
      ⚠ 2026-08-23부로 멀티PC 조율 의무 폐기 — **이 갈래의 독립 결정**으로 처리
- [ ] (승계, 최우선) **CB② 복원 재검토 기한 2026-08-29(토) → 마지막 거래일 08-28(금)**

### 점검 규약 메모

- 장중 절은 대원칙 B대로 **같은 파일 끝에 append**했고 `-intra` 파일을 만들지 않았다.
  절 번호는 장전 `1p-*`에 맞춰 `2i-*`를 썼다(494차 장전 메모의 절충안 계승).
- 장전 절 본문은 고치지 않았다. `C-1`·`C-2` 헤딩 아래에 **포인터 한 줄씩만** 추가
  (`⬆️ 12:30 격상 — 제2부 2i-0 … 참조` / `✅ 12:30 해소 — …`).
- 이상점 번호는 장전 마지막 `1-3` 다음 `1-4`부터 이어 받았다. 지속 항목(1-1·1-2·1-3)에
  새 번호를 붙이지 않았다.

### 커밋 대기 (오늘 커밋하지 않았다)

```
docs/정기점검/매일점검/MW0601-20260826-점검리포트.md        (제2부 append)
docs/정기점검/매일점검/evidence_MW0601-20260826_intra.md   (신규 · 미추적)
docs/정기점검/매일점검/evidence_MW0601-20260826_pre.md     (신규 · 미추적)
docs/정기점검/매일점검/evidence_MW0601-20260825_pre.md     (어제분 · 미추적 잔존)
docs/정기점검/매일점검/evidence_MW0601-20260825_intra.md   (어제분 · 미추적 잔존)
docs/정기점검/매일점검/evidence_MW0601-20260825_post.md    (어제분 · 미추적 잔존)
dev_memory/DECISION_LOG.md                                 (494차 후속 절)
dev_memory/NEXT_TODO.md                                    (이 절)
```

🔴 **선행: `del C:\Users\82108\PycharmProjects\futures\.git\index.lock`** (이상점 1-1).
⚠ **`git add .` 금지** — 나머지 509건은 EOL 착시. 위 8개 경로만 명시 add.

### MW0601 494차 정정 (2026-08-26 14:55)

- [x] ~~**F-1** (P1) 수집기 `--no-optional-locks` + 적신호 3중 조건 분기~~
      🔴 **철회 — 이미 코드에 있다** (`collect_evidence.py:446~460` · `scripts/git_lock_guard.py`
      483차 후속3 · `tests/test_483_git_lock_guard`). **함정 ① 위반 오늘 3회째**
- [ ] 🔴 **F-1′** (P2 · **선행순서는 F-2·F-3보다 앞**) 예약 실행 로드 SKILL.md 사본 ↔ repo 원본
      **동기화** — 기존 0825 **P2-E / 이상점 1-6**에 흡수(신규 아님, **근거 승격**: 이론 → 실측 1건).
      원본 `:62`(`git_lock_guard.py --check` 프리플라이트) · `:75`(`git --no-optional-locks`)가
      **로드 사본에 없어** 장전 세션이 맨손 git → 08:59:22 락 → 오진 + 불필요한 사용자 조치.
      보조: 수집기 §11 적신호 1번에 `python scripts/git_lock_guard.py --check` 병기
      ⚠ **보조도 현행 문구 재확인 후 낼 것**(함정 ① 재발 방지)
      검증: **O-8**
- [ ] ⚠ **F-3 설계 변경** — `5c54496`(12:24:59)이 `FUTURES_COMMISSION_RATE`를
      **고정 상수 → 채널 감지 파생**으로 바꿨다(`config/constants.py:BROKER_CHANNEL_SPECS
      [detect_broker_channel()]`). 불변식 3행을 **상수값 대조가 아니라 `BROKER_CHANNEL_SOURCE`·
      감지 결과 대조**로 재설계. 착수 전 `config/constants.py` 선독
- [ ] **F-AE′** (P2, 기존 F-AE에 흡수) 뒤 국면이 자기 절 시작 **전에**
      ① `git log --since=<오늘>` ② `docs/정기점검/매일점검/` 당일 mtime 목록 확인을 SKILL.md 절차에.
      근거: 오늘 병행 세션 **4개**가 서로를 몰랐고, 장중 절이 "커밋 불가"라 쓴 시점에
      이미 `5c54496`이 커밋돼 있었다
...
- [ ] **장후 숙제** `MW0601-20260826-청산로그갭-딥다이브.md`(11.7KB, 11:58) **내용 열람 후**
      리포트에 결론 1줄 + 그날 판정에 미치는 영향 역링크 (F-AE 취지). 이 세션은 미열람

**정정 요지** — 이상점 1-1 ⬇️완화 **P1 → P2**. 봉쇄 실구간은 08:59:22 ~ 12:24 이전뿐이고
그 뒤 커밋 2회 성공(12:24:59 · 14:36:30), 14:52 현재 락 없음. 사용자 `del`은 대상 없음으로 종료.
**사용자 조치 1(잠금 삭제) 취소.** 커밋 대기는 유효(장 마감 뒤 권장).

### MW0601 494차 후속 (2026-08-26 15:10) — F-1′ 적용 완료

- [x] **F-1′ 조치 1** 앱 저장 스킬 교체 (`save_skill overwrite=true`) —
      정본 433줄(2026-08-24 `91c6120`) 기반 + 보강 8건.
      교체 전 앱 사본은 **2026-08-20 23:36 · 417줄**(6일 낡음, `44e2652` 세대).
      사본에 없던 것: 🔴 **git 호출 규약**(`e7a5b53` 2026-08-23) · 🔴 **장후 제4·5부 전체**
- [x] **F-1′ 조치 2** 예약작업 3종(`mireuk-premarket/intraday/postmarket-check`) 프롬프트에
      `## 0. 시작 전`(정본 Read 선행 · `--no-optional-locks` · 병행 세션 확인) 주입.
      **스킬이 또 낡아도 프롬프트가 정본을 강제**하는 이중화
- [ ] **O-8** (내일 08:57) 장전 첫 bash git 호출이 `git --no-optional-locks …` 인가 ·
      세션 종료 시 `.git/index.lock` 부재인가
...
리포트: `docs/정기점검/매일점검/MW0601-20260826-점검리포트.md` 제3부~제7부.
당일 2포지션 2승 net **+189,902원**. 전환기준 ① 4주 창 **−224,296원 미충족**.
**커밋하지 않았다.** 근거 전문은 `DECISION_LOG.md` 2026-08-26(494차 후속3).

#### Fix (우선순위 순 — 사용자 지시 대기)

- [ ] 🔴 **F-10 (P1) 동결 감시가 정상 마감을 동결로 오판** — 이상점 1-11.
      `scripts/freeze_sentinel*.py` 판정 함수: ⓐ `data/daily_close_done_<date>.txt` 존재를
      정상 종료의 **독립 증거**로 추가 ⓑ 마감 후 구간은 `CRITICAL` → `INFO(마감 후 정지 —
      정상)` 강등하되 감시 유지 ⓒ 상태 전이 시 1회만 발행.
      ⚠ **회귀 위험 높음(안전장치를 무디게 하는 방향) → F-8·F-9와 별도 커밋.**
      🔴 **전환기준 ② ⓑ 왕복 실측의 선행조건이다** — 오탐 상태로 왕복하면 사고 재현.
      검증: `freeze_watchdog_rehearsal.py` 2시나리오 + 내일 15:45 alert 미생성(O-t4)
- [ ] 🔴 **F-8 (P1) 브로커 net 축 EOD 확정 기입** — 이상점 1-9.
      `main.py: daily_close()`에서 `update_daily_broker_pnl_net()` **직전**에
      마지막 예탁현금 헤더 스냅샷으로 `upsert_broker_net()` 1회 더 호출.
      `_last_deposit_header`를 FLAT 가드 **밖에서** 갱신하고 `__init__`에서 `None` 명시
      초기화(⚠ `getattr` 폴백 금지 — 계측 4원칙 ④).
      **FLAT 가드는 유지**(main.py:16624 주석 근거 — 보유 중 미실현 오염).
      `deposit_snapshot_ts` 컬럼 신설 + 15:10 이후 푸시 없으면 `source="stale_snapshot"`.
      NULL일 때만 쓰도록 해 실측 기입을 덮지 않는다
- [ ] **F-9 (P1) gross 대사 자기참조 분리** — 이상점 1-10.
      `main.py:12028~12043`에서 `fetch_broker_daily_pnl_map()` 조회를
      `update_daily_broker_pnl_net()` **앞으로** 이동. 로그에 비교 축·원천 명시.
      `update_daily_broker_pnl_net()`에 `created: bool` 반환 추가.
      검증: `tests/test_49X_gross_recon_not_self_referential.py` — **현행 코드로는 반드시
      실패해야 정상**(도달 불가 분기 존재 증명). ⚠ F-8과 같은 블록이라 함께 손댄다
- [ ] **F-2 (P1) 런처 단일 인스턴스 가드 재배선** — 이상점 1-2 (장전 등록, 계속 이월).
      오늘 `35ed037`(07:20) 제목이 「런처 배선은 되돌림」. 오늘 08:40에도 허위
      「감지 → 종료 완료」가 찍혔다
- [ ] **F-11 (P2) 관측 번호 국면 접두 유일화** — 이상점 1-12.
      `O-p*`/`O-i*`/`O-t*` + 각 국면 절 끝 「발급 번호 범위」 1줄 강제.
      ⚠ **세 곳 동시 갱신** — 저장소 정본 · 앱 저장 스킬(`save_skill overwrite=true`) ·
      예약작업 3종 프롬프트(SKILL.md §9)
- [ ] **F-12 (P2) 참조 문서에 브랜치별 DB 스키마 차이 명시** — 이상점 1-13.
      `references/evidence_map.md §8-3` · `postmortem.md §3-1`:
      `tp1_reached` 옆에 `(dev 전용 — v9-dev에는 없다. exit_stage 사용)` 병기 ·
      `v9-dev` 38컬럼 표 추가 · `TRAIL_AFTER_TP1`=보호 트레일(이익 청산) 규칙 ·
      상단에 `pragma table_info` 선확인 경고. **`dev` 서술을 지우지 않는다**
- [ ] **F-4~F-7 (장중 등록분)** — 계속 이월.
      ⚠ **F-4(시간대 하드 게이트 승격)는 보류한다** — 오늘 그 구간 진입이 순손익의
      **89%**를 만들었다. O-t7 표본 도달 후 사전등록 판정

#### 고도화

- [ ] **G-6** 「대사/일치/recon/match」 로그 전수 감사 — ① 비교 대상 ② 원천 독립성
      (자기참조 여부) ③ 결측 시 표기(0 vs 미측정). `tests/`에 인벤토리 스냅샷 고정.
      근거: 하루에 대사 결함 2건(1-9·1-10), 둘 다 로그는 정상으로 보였다
- [ ] **G-7** 진입후보 시간을 변동성으로 정규화해 **섀도 병기**(C-3).
      ⚠ 판정식 변경은 §9-4 검증 시계에 걸린다 — 5거래일 병기 후 주간회의
- [ ] **G-8** `meta=0.50<fallback>` 차단 건수를 `ensemble_decisions`에 별도 플래그 적재.
      ⚠ 431차 「차단 기준 `size_multiplier` 무변경」은 **기록된 결정** — 뒤집자는 것이
      아니라 폴백 차단 기회손실의 크기를 재자는 것(계측 4원칙 ④ 확장)

#### 제5부 방안 (전부 표본 미달 — 계측 신설 제안)

- [ ] **방안 1** `horizon_ev_inversion_watch` 채널 신설 — 3m만 30일 순EV **−6,900원(124건)**
      인데 오늘 진입 2건이 전부 3m. 사전등록 `min_samples=40 且 min_days=15`, 일자단위
      부호검정 p<0.05, drop-worst 후 부호 유지. ⚠ 라우터·`ENTRY_HORIZON_B1/B2` 무변경.
      ⚠ `VALIDATION_CAMPAIGN_DECISIONS`에 「계측 전용·조치 없음」을 **처음부터 등록**(함정 ①)
- [ ] **방안 2** `binding_gate`별 손익 귀속 — [28] 하위 분해. `min_samples=25 且 min_days=12`.
      ⚠ `sizing_trace`는 2026-08-14 이후 행에만 있다 — NULL 구간 분리 표기(계측 4원칙 ②)
- [ ] **방안 3** TP1 비중·보호 트레일 오프셋 섀도 병렬 계산 — 기존 `tp1_trail_shadow` ·
      `tp2_hold_shadow` · `synthetic_partial_exits`가 오늘 두 케이스를 담았는지 먼저 확인.
      `min_samples=30 且 min_days=15`. ⚠ 섀도가 좋아도 바로 라이브 반영 금지
- [ ] **방안 4** 블랙아웃 진입 손익 양방향 적재(O-t7) — `min_samples=20 且 min_days=10`.
      음수+유의면 F-4 승격, 아니면 **시간대 항목 하향도 함께 검토**(313차 ④ 양방향)
- [ ] **방안 5** `docs/정기점검/수익률향상_누적대장.md` 신설 — 방안 ID · 최초 제안일 ·
      사전등록 판정식 · 현재 표본(n, 일수) · 다음 판정 예정일. 장후는 표본 칸만 갱신

#### 관측 (다음 거래일)

- [ ] **O-t1** (내일 장전) 런처 로그에 프로브 출력 줄 실재 · 허위 「종료 완료」 소멸
- [ ] 🔴 **O-t2** (내일 장후) `[NetRecon]`이 `NO_BROKER`가 아닌가 ·
      `daily_broker_pnl` 08-27 행 `broker_net_krw` NULL 아닌가.
      **F-8 미적용이면 재발이 정상** — 재발 여부가 곧 원인 진단의 확인이다
- [ ] **O-t3** (내일 장후) 08-26 행이 내일 푸시로 **사후 충전되는가**.
      채워지면 「결손」이 아니라 「지연」이며 F-8 설계가 달라진다
- [ ] **O-t4** (내일 장후) ① `freeze_sentinel_20260826.log`가 밤새 계속 자라는가
      ② 내일 15:45 `freeze_sentinel_alert_20260827.txt`가 또 생기는가
- [ ] **O-t5** (내일 장전) 재기동 후 **첫 설정 핫리로드 성공**하는가.
      **해소되지 않으면 1-5 원인 진단이 틀린 것**
- [ ] 🔴 **O-t6** (포지션이 남는 날) 15:10 강제청산 경로 실집행 —
      `[ForceExitPass]`→`[TimeExit]`→`[ExitAttempt]` 15:10:00~05 출현,
      `안전망 발동`(ERROR) 미출현. **471차 F-1R 이래 미도래 지속. 전환기준 ② ⓐ 미충족**
- [ ] **O-t7** (표본 도달 시) 블랙아웃 진입 포지션 단위 누적 n·순EV·일자 부호
- [ ] **O-t8** (08-28 주간점검) 2026-08-20~25 장후 리포트의 **제4·5부 소급 품질 점검** —
      언급만 있고 절차(3원 대사·요인 태그·313차 다섯 갈래)가 없으면 소급 보완 대상.
      ※ 2D-4의 「6일간 누락」 우려는 과대였다(0825 2회·0824 1회·0821 1회 흔적 확인, 0820은 파일 없음)
- [ ] **O-t9** (09-02) 진입후보 시간 3축 병기 — ① 분/일 ② 당일 레인지 pt ③ `재지않음` 비율.
      **변동성 보정 후에도** 하한 미달이 지속되는가
- [ ] **O-t10** (5거래일 후) CB③ 판정 가능 시간 추이. 오늘 **41%(152/370분)**.
      30% 미만이 나오면 전환기준 ⑥ 논의에서 **임계보다 가용시간이 먼저**다

#### 커밋 대기 (오늘 커밋하지 않았다)

⚠ **`git add .` 금지** — 리눅스 샌드박스 `git status`는 CRLF/LF로 520건을 「변경」으로
표시한다. 실질 변경은 **3건**(코드 0건)이다. 경로를 명시해 add 할 것.

```
.claude/skills/mireuk-daily-check/SKILL.md          ← 사용자가 수동 복사한 정본(rev: 2026-08-26)
dev_memory/DECISION_LOG.md
dev_memory/NEXT_TODO.md
docs/정기점검/매일점검/MW0601-20260826-점검리포트.md
docs/정기점검/매일점검/evidence_MW0601-20260826_post.md
```
...
**구현 완료(커밋됨)**: F-10 · F-8 · F-9 · F-2 · F-12 · F-11 · F-3 · G-6 · 제5부 방안5.
근거·회귀위험·검증은 `DECISION_LOG.md` 2026-08-26 498차 항목.

#### 라이브 검증 대기 — 다음 거래일에 닫는다

- [ ] **O-p1 (F-2, 08:40)** 런처 로그에 `[GUARD] 기존 main.py 없음` 또는 rc 동반 문구가
      뜨는가. 🔴 **`[GUARD] 기존 프로세스 종료 완료` 무조건 출력이 사라졌는가**가 판정
      기준이다. `terminate()` 경로는 리허설에서 제외했으므로 **실제 이중기동 때 관측**한다.
      되돌리려면 `start_mireuk.bat.bak_498`(로컬 백업, 커밋 대상 아님).
- [ ] **O-p2 (F-3, 장전)** 다이제스트 §3 불변식 표에 비용 모델 4행이 `일치`로 나타나는가.
      ⚠ `FUTURES_COMMISSION_RATE` 기대값은 **숫자가 아니라 파생식**이다
      (`_BROKER_SPEC["one_way_commission_rate"]`) — 495차 후속 파생 구조를 지키는지 보는 것.
- [ ] **O-t1 (F-10, 15:45)** `data/freeze_sentinel_alert_<date>.txt` 가 **생성되지 않는가** ·
      `logs/freeze_sentinel_<date>.log` 에 `NORMAL_CLOSE` 가 **1회만** 찍히는가(매분 반복 아님).
      🔴 반대로 **장중 동결이 나면 여전히 CRITICAL 이어야 한다** — 미탐이 오탐보다 나쁘다.
- [ ] **O-t2 (F-8, EOD)** `[NetRecon]` 이 `NO_BROKER` 가 아닌 `일치`/`MISMATCH` 를 찍는가 ·
      `daily_broker_pnl.broker_net_source` 값이 `live`인가 `stale_snapshot`인가.
      `stale_snapshot` 이 반복되면 잔고 푸시 주기 자체를 봐야 한다.
- [ ] **O-t3 (F-9, EOD)** `[BrokerPnl]` 이 `broker gross[TR수신 …] vs engine gross` 형태로
      **비교 축과 원천**을 밝히는가.

#### 🔴 사용자 조치 (자동조치가 할 수 없는 것)
...
#### Fix (장후 적용 — 장전은 코드 변경 금지)

- [ ] **F-1 (P1) 브로커 채널·요율 기동 로그 1줄.** `main.py` 기동 배너 직후
      `[BrokerChannel] channel=%s source=%s rate=%.6f%% fee_source=%s`.
      `source=="fallback"` 이면 **WARNING 승격**. + `scripts/collect_evidence.py:124`
      `_ALWAYS_QUOTE` 에 `"BrokerChannel"` 추가(매일 장전 다이제스트 §6 자동 인용).
      ⚠ COM 콜백 안 금지(절대원칙 §4) · 배너보다 앞에 두지 말 것(순환 임포트).
      **이미 있는가 확인 완료** — 로거 호출 0건 / `.bat` 고정 0건.
      검증: 다음 거래일 08:40 로그 출현 + `MIREUK_BROKER_CHANNEL=CREON` 강제 시 `source=env` +
      `tests/test_495_broker_channel_rate.py` 단언 1케이스 추가.
...
> 상세: `DECISION_LOG.md` 2026-08-30(500차). 사용자 승인으로 3단계(주간회의 안건)
> 까지 이번 세션에서 결정·집행한다.

**0단계 — 문서 정정 [진행]**
- [x] `CLAUDE.md` §3 각주 3종 — 단기 CORE 실소비 키 / `cvd_divergence` 정체 / OFI 항등식
- [x] `DECISION_LOG.md` 500-A~G
- [x] 이 항목

**1단계 — 라이브 결함 4건 (P0)**
- [ ] **D-1 TrendGate 하드브레이크** `_CVD_SLOPE_HARD_BREAK_DN/-UP` (-300/+200) vs
      실측 `cvd_slope` [0.0000, 0.9743] → 판정 근거를 `cvd_delta_norm`으로 교체.
      ⚠ 임계를 -0.3 식으로만 바꾸면 안 된다 — `cvd_slope`가 음수 0건이라
...
**🔴 P0 — 장후(15:45 이후) 적용. 장전·장중 적용 금지**

- [ ] **F-1 blank-rows 폴백 재설계** `main.py:17107~17132`
      ⓐ `len(rows)==0` 판정을 `_is_mock` 앞에 두고 **FLAT 강제**
      ⓑ 유지 분기도 `block_new_entries=True` (현행 3번째 인자 `False`)
      ⓒ `_ts_verify_position_before_exit(expect_side, expect_qty)` 신설 →
        `TickTP1`·`TickStop-S0C`·`SchedForceExit` 3곳에서 청산 주문 **직전** 호출.
        불일치 2회 확인 시 차단 + `CRITICAL`. `[ExitPrecheck]` 태그로 매일 계측,
        1주 섀도 후 승격
- [ ] **F-1 테스트 선행** `tests/test_505_broker_sync_blank_rows.py` —
      08-28 15:30:02 원문 응답 픽스처(`rows=[]`, `summary={'총매매': '49838525', ...}`)
- [ ] **F-2 `BrokerDirectExit` 재시도** `main.py:16271~16283` —
      `ret != 0` 3회(0.5/1/2s) + 실패 시 다음 분 재시도 플래그 + **15:44 하드 캡**
      🔴 **재시도 전 잔고 재조회 필수(F-1 ⓒ 재사용)** — 이중 청산 위험.
      CLAUDE.md 전환기준 ② 주석이 이미 주간회의 안건으로 지정. **잔고 재확인 없이
      재시도만 넣는 것은 금지**
- [ ] **F-2 테스트** `tests/test_505_direct_exit_retry.py` — 재시도 전 잔고 재조회
      호출 확인 · 15:44 이후 재시도 없음
- [ ] `config/settings.py` 신설 — `BROKER_DIRECT_EXIT_RETRY = 3` ·
      `BROKER_DIRECT_EXIT_RETRY_BACKOFF_S = (0.5, 1.0, 2.0)`

**P1**

- [ ] **F-3 15:10 미체결 진입 주문 일괄 취소** — `SchedForceExit` 경로에
      `broker.cancel_order`(`main.py:10821`) 배선. **`kind`가 `ENTRY_*`인 것만**
      (청산 주문 취소하면 정반대 사고). `[SchedForceExit] pending_cancel=N건` +
      취소 실패 건수 명시(계측 4원칙 ③)
...
- [ ] **F-7 (P1) PSI 기준선 키 불일치 흡수** `strategy/regime_fingerprint.py`
      ...
      — **ⓐ와 함께 넣는다**(ⓐ만이면 다음 집합 교체 때 재발) ⓒ 기준선 파일에
      `core_features` 목록·코드 리비전 동봉 ⓓ `_training` 공백 시 `get_psi()` → `None`
      (0.0 위장 금지 — 4원칙 ②)
- [ ] **F-7 테스트** `tests/test_506_fingerprint_key_mismatch.py` — 08-28자 실제 파일
      픽스처 고정(`cvd_divergence`/`ofi_norm`), 무예외, 공백 시 None
- [ ] **F-8 (P1) 수집기 진입 정규식** `scripts/collect_evidence.py:133` +
      `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py` **양쪽**
      `[Position] 체결진입` 형식 추가. ⚠ **`[체결진입보정]`(`main.py:17770`) 배제 필수**
      (현행 주석 `:1147~1148`이 "정규식이 `\[체결진입\]`로 닫혀 매칭 안 됨"에 의존).
      함께 ⓐ 포지션 행 **출처 칸**(엔진/외부/추정) ⓑ §11 적신호 `진입 0건` →
      **`엔진 진입 0건 / 계좌 진입 N건`** 분리
- [ ] **F-8 테스트** `tests/test_506_collector_fill_entry_regex.py` — 오늘 TRADE 로그로
      포지션 14건·레그 25행·**-6,477,882원** 재현 / 보정행 미매칭 / 08-27 이중계상 0
- [ ] **F-9 (P2) 외부 진입 표기·격리** 🔴 **주간회의 안건 · 자동조치 등급 밖 ·
      사용자 승인 필요** — `apply_entry_fill()` 외부진입 속성 보존 +
      `trades.entry_source`(`engine`/`external`/`unknown`).
      ⚠ 전환기준 ① 표본 정의를 바꾼다 · 504차 「거래 출처 필터」와 **정의 일치 필수**
      (두 곳에서 다르게 정의하면 461차 `mdd_pct`·500차 CORE 3중 정의와 같은 사고) ·
      `strategy_events` `METRIC_REDEFINITION` 마커 동반

**고도화 — 섀도만, 차단 아님 (오늘 n=1 / 계열 n=3, 313차 원칙: 확정 결론 금지)**

- [ ] **G-3 `entry_gate_stuck_shadow`** — 진입 차단 사유별 **연속 지속시간** 매분
      `predictions.db` 적재, 개장 후 30분 이상 연속 시 경보 등급 상향.
      `_entry_block_reason`(`main.py:9210~9250`) 재사용 → **새 계산 0회**.
...
**Fix (계획만 — 코드 변경 0 · 커밋 0)**

- [ ] **F-10 (P1) `partial_1_done`에 사유 축 추가** `strategy/position/position_tracker.py:1509~1513`
      `_sync_partial_progress(reason)` — TP 계열 사유일 때만 승격. `[ExitStage]` INFO 병기.
      ⚠ **선행조건**: `is_tp1_hit` 재발동 가드(`main.py:4574`)가 같은 플래그를 쓴다 →
      **TP 재발동 가드를 별도 플래그로 분리**해야 한다. 소급 재분류 금지 + `METRIC_REDEFINITION` 마커.
      근거: 오늘 손실 5포지션(−6,977,391원)이 `TRAIL_AFTER_TP1`로 오분류(이상점 1-19)
- [ ] **F-10 테스트** `tests/test_507_exit_stage_multifill.py` — ⓐ 4계약 1→2→1 하드스톱 최종레그
      `INITIAL_STOP` ⓑ TP1 경유 시 `TRAIL_AFTER_TP1` 보존 ⓒ **오늘 5건 픽스처 뒤집힘**
      ⓓ `is_tp1_hit` 재발동 0회
- [ ] **F-11 (P1) 승패 표기 축 병기** `main.py:11657` + `strategy/ops/daily_exporter.py`
      `승=12 패=8 (gross pt) | net 9승 11패` · `WR(gross)/WR(net)` · `daily_stats`에
      `wins_net`·`losses_net` **추가**(기존 키 무변경). **판정 축은 바꾸지 않는다**
- [ ] **F-11 테스트** `tests/test_507_win_axis_dual.py` — ⓐ 오늘 20포지션 `wins=12/wins_net=9`
      ⓑ 기존 `wins` 불변 ⓒ 동일값 날에도 병기 유지
- [ ] **F-12 (P1) 미측정 PSI를 `0.000 CLEAR`로 렌더링하지 않는다**
      `strategy/regime_fingerprint.py:97·204` + `strategy/ops/daily_exporter.py:143~152`
      `_current_psi` 초기값 `None` · `get_psi() -> Optional[float]` · 렌더러 `미측정` ·
      하루 `update_live` 성공 0회면 마감 WARNING 1회.
      ⚠ **착수 조건**: `grep -n "get_psi()"` 소비처 전수 확인(반환 타입 변경).
      ⚠ **F-7과 같은 파일 — F-7 → F-12 한 묶음으로 처리**
- [ ] **F-12 테스트** `tests/test_507_psi_unmeasured_render.py` — ⓐ 미실행 시 `get_psi() is None`
      ⓑ 리포트에 `0.000`·`CLEAR` 미출현 ⓒ 정상 계산 후 종전 문자열
- [ ] **F-13 (P2) 🔴 주간회의 안건 · 자동조치 등급 밖** — `dev`의
      `PIPE_LATENCY_EXCLUDE_MODEL_SWAP` **이식 여부 결정**. CB⑤ 계측 범위 축소이므로 정책 결정.
      ⚠ 브랜치 갈라짐으로 `git pull` 불가 — **파일 직접 전달** 전제. `dev` 구현이
      「모델 교체 플래그가 선 분 한정」인지 「S0 전체 제외」인지 **원문 확인 선행**.
      이식하지 않기로 정하면 **그 결정 자체를 기록**할 것(함정 ①)
- [ ] **F-13 대안(등급 안)** CB 발동 로그에 `cause=` 병기 + 자가유발 카운터 분리. 차단 무변경.
      `tests/test_507_cb5_selfinduced_tag.py` — 오늘 13:58 리플레이로 1건 계상
- [ ] **F-14 (P2) `[NetRecon]` 진단문을 실측 축 분해로 교체** — 고정 문구
      *"gross가 일치하는데 net만 어긋나면 …"* 제거, gross/수수료 기여율 + 지배 축 출력.
      `[BrokerPnl]`과 같은 초에 다른 결론을 말하는 상태 해소.
      ⚠ `scripts/commission_rate_recon.py --verify` 판정식은 **무변경**(사전등록)
- [ ] **F-14 테스트** `tests/test_507_netrecon_axis_message.py` — ⓐ 오늘 값 「지배 축=gross」
      ⓑ 합성 케이스 「지배 축=수수료」 ⓒ 고정 문구 조건부화
- [ ] **F-15 (P2) 🔴 주간회의 안건 · 자동조치 등급 밖 — 장중 F-9를 대체한다.**
      `fetch_daily_net_for_verdict()`(`utils/db_utils.py:3259`)가 읽는
      `daily_broker_pnl.broker_net_krw`는 예탁금 차이라 **`entry_source`로 거를 수 없다** ⇒
      전환기준 ①의 판정 원천에 출처 필터가 **구조적으로 부재**. 선택지 A/B/C는 그날 리포트 F-15.
      ⚠ 504차 「거래 출처 필터」와 정의 일치 필수 + `METRIC_REDEFINITION` 마커

**장전·장중 항목 갱신 (새 번호 부여 금지 — 원 항목에 근거만 추가)**

- [ ] **F-4 🔄 범위변경** — 「일일 마감 손익을 브로커와 대사」는 **손익 축은 이미 있다**
      (`[BrokerPnl]`·`[NetRecon]`, 오늘 정상 발화). 남은 것은 **포지션 축** 대사다.
      제목을 「포지션 축 대사」로 다시 쓸 것
- [ ] **F-9 🔄 범위변경 → F-15로 대체** — `entry_source` 컬럼·집계 필터는 **311차 후속에
      배포 완료**(함정 ① 위반 직전에서 잡음). 상세는 DECISION_LOG 507차 「함정 ① 정정」
- [ ] **F-6 ⬆️ 최우선** — 1-15가 P0로 격상됐다(09:21~15:08 **5시간 47분** · 최근 6거래일 0건 =
      오늘 첫 발생). 자동 진입 경로가 하루 전부 죽는다
- [ ] **F-2 설계 선행조건 미충족** — O-p2(`ret=4` 의미)가 **표본 n=1로 판정 불가**.
      「재시도 + 실패 지속 시 에스컬레이션」 **양쪽을 갖춘 형태**로 설계할 것
- [ ] **F-3 적용 범위 질문 잔존** — O-p3 판정으로 「엔진 아님」이 확정됐다. **수동 주문까지
      취소할 것인가**는 사용자 결정(사용자가 건 주문을 시스템이 말없이 지우면 그 자체가 새 사고)
- [ ] **F-5 우선순위 하향 가능** — 08-28 브로커 실측 net(+114,274원)이 이미 있어
      **전환기준 ① 판정은 오염되지 않았다**. 오염 대상은 엔진 net 시계열과 캠페인 채널
- [ ] **F-7 ⚠ EOD 자가 치유됐어도 그대로 적용** — 치유는 「이번 한 번」, F-7은 「다음 집합 교체」를 막는다

**고도화 (섀도만 — 차단 아님)**

- [ ] **G-5 `exit_stage_recon_shadow`** — 일일 마감에서 `exit_stage='TRAIL_AFTER_TP1'` 대
      「그 구간 TP1 이벤트 로그 유무」 대사, 1줄 요약. **새 계산 0회**(둘 다 이미 기록됨).
      승격 최소 10거래일 뒤. 근거: 오늘 13건 중 5건 미대응
- [ ] **G-6 `selfinduced_cb_shadow`** — CB 발동 직전 60초의 재적합/모델 교체/`cause=S0`로
      `selfinduced=1` 태그(`predictions.db` 컬럼 1개). CB 동작 무변경. 4주 관찰 후 승격 판단
- [ ] **G-3 근거 강화** — Armistice 연속 차단이 3시간 5분이 아니라 **5시간 47분**이었다

**관측 (다음 국면이 판정/보류/이월 중 하나로 닫는다 — 미처분 금지)**

- [ ] **O-t1 (다음 거래일 장후)** net 대사 잔차의 지배 축 — gross인가 수수료인가.
      **2일 이상 gross 지배면 체결 대사 문제로 확정**. 후보 ⓐ 스냅샷 15:05:11 vs 최종 체결
      15:05:09(2초차) ⓑ 요율 5% 과다
- [ ] **O-t2 (다음 거래일 장전 · 최우선)** Restart Armistice 재현 여부 —
      09:00~09:30 `[차단] Restart Armistice` 유무 + `sync=` 값. 재현 시 F-6 즉시 최우선
- [ ] **O-t3 (다음 거래일 장중)** EOD 치유 실효 — `[RegimeFingerprint] update_live 예외` 소멸 ·
      `ensemble_decisions.fp_psi` non-NULL 출현
- [ ] **O-t4 (다음 거래일 장후)** 15m EOD 교체 보류 반복 —
      `15:47:12 [Retrain] 15m 교체 보류(EOD 모델가드) acc 하락 0.0372 > 허용 0.0300`.
      **2일 연속이면 구모델 고착 안건**
- [ ] **P-1 (다음 거래일 장중)** 엔진 자동 진입 재개 여부 — `entry_executed=1` 1행 이상.
      **0이면 이틀 연속 = 자동매매 사실상 정지, 사용자에게 보고**
- [ ] **P-2 (다음 거래일 장중)** 외부 진입 지속 여부 — `entry_source='GHOST_PENDING_MISS'`
      신규 행. **주체 확인(사용자 조치 1)의 답과 대조**
- [ ] **O-p2 이월** `[CybosOrder] ret=` 코드 의미 — 보유 로그 전체 1건. n≥2 또는 API 문서 확인 시 판정
- [ ] **O-i5 이월 · 근거 반전** `entry_horizon` 폴백과 하드스톱 편중의 인과 —
      **오늘 집행(E) 축은 정상**(스톱 대비 −0.17~+0.26pt). 엔진 진입 재개 후
      `entry_horizon` 실값 20포지션 · 10거래일로 재판정
- [ ] **O-p6 부수 기록(결함 아님)** — 「전날 P8 성공 여부」의 유일 원천이 마커 파일이다.
      `session_state.json`은 당일 롤오버로 전날 값을 남기지 않는다

**수익률향상 누적 대장 (`docs/정기점검/수익률향상_누적대장.md` — 오늘 갱신 완료)**

- [ ] **P5-06** 분할체결 손절을 손절률 분모에서 되찾는다 — n=5/1거래일. G-5 섀도 10거래일 판정
- [ ] **P5-07** 0.2pt 이하 이익의 구조적 net 적자 계측 — n=3/1거래일. 손익분기 0.204pt/1계약.
      **임계 변경은 제안하지 않는다**(계측만). `min_samples=20` 且 `min_days=10`
- [ ] **P5-08** 자가유발 안전장치 발동의 진입공백 비용 환산 — n=1(계열 2). 4주 · `min_days=10`
- [ ] **P5-05 판정 유예 → 2026-09-04 재판정** — 08-27·08-28 장후 미실행으로 2거래일 결장(1-23)

**사용자 조치 (리포트 `## 사용자 조치`와 동일 — 8항목)**
...
- [ ] **5. CB② 복원 결정** (기한 2026-08-29 경과) — 오늘 「2회」 2번 도달.
      ⚠ 오늘 손절은 전부 외부 진입이라 예측력 낮음. **주간회의 안건**
...
`mireuk-postmarket-autofix` 가 A/B등급 7건(F-7·F-8·F-11·F-12·F-14·G-4·G-5)을
구현·커밋했다. 아래는 **자동조치 등급 밖**이라 남긴 것들이다.

### C등급 — 주문·청산 경로 (사용자 지시 없이는 착수 금지)

- [ ] **F-1 (P0)** blank-rows 폴백을 "유지"에서 "잠금"으로 — `main.py:17107~17132`
      · `rows=0` 과 `all_blank_rows` 분리 / `block_new_entries=True` /
        청산 주문 직전 잔고 재확인 게이트(`_ts_verify_position_before_exit`)
      · 🔴 청산 주문 경로 + 리포트 스스로 "1주 섀도 관찰 후 승격"
      · 검증안 `tests/test_505_broker_sync_blank_rows.py`
- [ ] **F-2 (P0)** `BrokerDirectExit` 주문 실패 재시도 — 🔴 **주간회의 안건**
      (이중 청산 위험). CLAUDE.md 전환기준 ② 주석이 이미 못박고 있다.
      착수 시 **F-1의 잔고 재확인 헬퍼 재사용이 선행조건** — 그 설계 없이 재시도만
      넣는 것은 금지.
- [ ] **F-3 (P1)** 15:10 미체결 **진입** 주문 일괄 취소 — 주문 경로.
      `kind`가 `ENTRY_*`인 것만. 청산 주문까지 취소하면 정반대 사고.
- [ ] **F-6 (P0)** Restart Armistice 고착 해소 — `main.py:8537~8556`
      · 🔴 **진입 허용 여부를 바꾼다.** 리포트가 사용자에게 "내일 아침 9:30에
        재현되는지 보고 지시해 달라"고 물어둔 상태(O-t2).
      · 재현되면 **최우선**. 승격 조건 3중 AND(`_armistice_time_ok` ·
        `_broker_sync_verified` · `block_new_entries is False`) + 세션당 1회 제한.
      · ⓒ항(30분 고착 ERROR)만은 순수 로그라 분리 구현 가능 — G-3와 묶는 편이 낫다.
- [ ] **F-10 (P1)** `partial_1_done` 에 사유 축 — `position_tracker.py:1509~1513`
      · 🔴 `is_tp1_hit` 재발동 가드(`main.py:4574`)가 **같은 플래그**를 쓴다.
        TP 재발동 가드를 **별도 플래그로 분리하는 것이 선행조건**이다.
      · 승인 조건(P5-06 사전등록): 오늘 붙인 **G-5 섀도 10거래일** 중 3일 이상에서
        「미대응」 1건 이상. → 2026-09-01부터 매일 마감 `[ExitStageRecon]` 누적.

### C등급 — 판정 기준·표본 (사용자 승인 필요)

- [ ] **F-5 (P2)** 08-28 손익 기록 정정 + `METRIC_REDEFINITION` 마커
      · 🔴 실전 전환 기준 ①의 4주 판정 표본을 건드린다.
- [ ] **F-15 (P2)** 외부 진입을 브로커 net 판정 경로에서 분리 — 🔴 **주간회의 안건**
      · `fetch_daily_net_for_verdict()` 원천(`broker_net_krw` = 예탁현금 차)에는
        `entry_source` 필터가 **구조적으로 존재할 수 없다**. 안 A/B/C는 리포트 F-15 표.
      · 504차 「거래 출처 필터」와 **정의를 맞춰야 한다**(461차 mdd_pct 유형 방지).
- [ ] **F-13 (P2)** `PIPE_LATENCY_EXCLUDE_MODEL_SWAP` `dev` → `v9-dev` 이식 여부
      · 🔴 CB⑤(절대원칙 §2) 계측 범위를 좁히는 결정. 자동조치 등급 밖.
      · 이식 안 하기로 정해도 **그 결정을 기록**할 것(미기록 FAIL은 재시도를 부른다).
      · 대안(섀도만: CB 로그에 `cause=` 병기)은 자동조치 등급 안 = 아래 G-6.
- [ ] **승률의 정의** — 실전 전환 기준 ③ 「승률 ≥ 53%」가 gross 인지 net 인지
      문서에 없다. 오늘 값은 정의에 따라 **60%(통과) / 45%(미달)** 로 갈린다.
      F-11이 두 축을 병기해 두었으니 **판정 축만 정하면 된다.** 🔴 주간회의.
...
## 2026-09-01 (MW0601 510차 — 장중 점검) 등록/이월

### 오늘 판정 완료 — 이월 목록에서 제거
- [x] O-p1 — `[HealthPolicy] Degraded 선제차단`(09:01) 실제 진입 저지 여부 →
      **영향 없음(이중 차단)**. 같은 분 `[ZeroDiag] conf미달(0.000<mc0.424)`로 확인.
- [x] O-p2(오늘) — `[ConfFloorGuard]` OK 복귀 시각 → **10:42:00 최초 복귀,
      10:55~11:13 재차단·재복귀**.
- [x] P-1 — 엔진 자동 진입 재개 여부 → **재개됨**(11:38:01 SHORT 1계약 3m +10,560원).
- [x] P-2 — 외부 진입 재유입 여부 → **대규모 재유입 확인**(7건 -1,216,234원) →
      이상점 1-1로 등록, 아래 F-16 참조.

### 부분 판정 — 계속 이월
- [ ] O-t3 — exception 축(`update_live` 0건)은 확인 완료. **DB 축(`ensemble_decisions.fp_psi`
      non-NULL)은 라이브 DB 스캔 금지 원칙상 미확인 — 장후 판정.**
- [ ] O-t1·O-t4 — 정의대로 장후 판정 대상, 변동 없음.
- [ ] O-p2(08-31 이월) `[CybosOrder] ret=` 코드 의미 — 오늘도 미발생, 발생 시 판정.

### 신규 — 이상점 1-1·1-2 (장중 등록)
- [ ] **F-16(신규, P1) 🔴 주간회의 안건 — 09:09~09:33 원인불명 외부진입 7건
      (-1,216,234원) 발생원 규명.** order_no 462·700·746·752·845·955·1035·1052·
      1071·1075를 `predictions.db`·`trades.db`(`entry_source`)와 대조. **F-15와
      통합 검토** — F-15가 이미 "외부진입을 브로커 net 판정 경로에서 분리"를
      다루므로 중복 스크립트 만들기 전에 `grep -n "entry_source" main.py
      utils/db_utils.py`로 기존 구현 확인 우선(함정①).
- [ ] **P2 — `[Health]` 로그에 `auto_entry_blocked` 필드 추가** —
      `main.py:_emit_runtime_health()`(2723~2799행). exceptions_10m 급증 →
      degraded ON → 자동진입 차단의 인과가 지금은 로그 3종 사후 대조로만
      재구성된다(계측 4원칙④).
...
## 2026-09-01 (MW0601 511차 — 장중 재점검 13:40) 등록

### P0

- [ ] **F-17 청산 주문 실패에 단계적 대응 핸들러** — 2026-09-01 13:28~13:30 하드스톱
      청산이 브로커에서 163회 전량 거부(94025 주문가능금액 부족 142 / ret=4 21)됐고,
      **동일 형태가 15:10 강제청산·15:18 FINAL_CLOSE에도 있다**(절대원칙 §1 위반 경로).
      대상: `main.py:4553`(틱 하드스톱)·`14585`·`14628`(손절1차)·`14896`(하드스톱)·
      `15033`(시간청산)·`_ts_broker_direct_force_exit`의 else 분기 → 공통 핸들러로 통일.
      단계 ①TRADE 기록 ②`closable_qty` 기반 수량 축소 재시도 ③지수 백오프(1s/2s/4s)·
      상한 3회 ④(**승인 후**)미체결 주문 취소 재시도 ⑤경보.
      ⚠ pending 선등록 순서(305차)를 깨지 말 것 · 절대원칙 4(COM 콜백 밖에서만).
      검증: `tests/`에 `send_order` 실패 주입 스텁 — 상한·TRADE 기록·pending 누수 0.
      근거: 점검리포트 09-01 제2부-B 이상점 1-3, DECISION_LOG 511차.

- [ ] **F-18 (🔴 주간회의 안건 — 자동조치 C등급) 청산 거부 시 미체결 주문 자동 취소
      허용 여부** — F-17의 4단계. `CYBOS_FUTURES_CANCEL_PROGID = "CpTrade.CpTd6833"`은
      이미 정의돼 있으나(`api_connector.py:88`), 취소 대상이 **사용자의 수동 주문**일
      수 있다. 승인 전에는 F-17을 3단계까지만 구현한다.

### P1

- [ ] **F-19 청산 주문 실패를 TRADE 로그에 남긴다** — 현재 은닉률 97.6%(163/167).
      else 분기 5곳에 `log_manager.trade(f"[주문실패] {kind} 청산 {direction} {qty}계약
      ret={ret} 사유={msg}")` 1줄씩. `msg`는 `api_connector._last_order_error`에 이미 있다.
      회귀위험 거의 없음(로그 추가). 계측 4원칙 ③·④.

- [ ] **F-20 `[TickStop-S0C] … → 주문 전송` 문구를 결과 기준으로** — 전송 **전** 로그는
      `→ 주문 시도`로. **선행**: `grep -rn "주문 전송" scripts/ docs/`로 소비처 확인
      (기존 집계가 이 문자열을 세고 있으면 깨진다).

- [ ] **F-21 실패 후 재무장 쿨다운** — `_process_tick_stop()` else 분기가 쿨다운 없이
      매 틱 재발동해 144초에 163회를 쐈고, 그 폭주가 `ret=4`(요청 제한 추정) 21건을
      2차로 유발했다. F-17의 ③단계와 함께 구현.

### P2

- [ ] **F-22 BlockRequest 반환코드 매핑 상수** — `collection/cybos/api_connector.py`에
      `ORDER_RET_MEANING = {0:…,1:…,2:…,3:…,4:…}` 신설 + 로그에 의미 병기.
      ⚠ **선행: 대신 API 공식 문서 확인** — 이 저장소에 근거가 0건이다
      (`docs/CyBos ref/`·코드 전체 grep 0). 08-31 O-p2의 후속.

### 고도화 (섀도 → 승격 절차 준수)

- [ ] **G-6 `closable_qty` 상태 승격** — `[ChejanFlow]`가 매 체결마다 이미 실어 보내는데
      (오늘 실측 13:22:05 `1→2`, 13:26:41 `0`, 13:31:40 `0→1→2`) **아무도 보관하지 않는다.**
      보관만 하면 스톱 발동 **전에** "지금 청산 가능한가"를 알 수 있다.
      FP-CRITICAL 죽은 게이트·TOX 죽은 섀도·CpTd6197 예탁현금과 같은 계열
      (*없던 데이터가 아니라 아무도 안 본 데이터*). 계측 4원칙 ⑤.
      변경: `main.py` Chejan 처리부 + `ensemble_decisions` 컬럼 1개. 회귀위험 없음.
      검증: 추가 후 1주 적재율 100%.

- [ ] **G-7 외부 미체결 주문 경보 채널** — `[ChejanMatch] pending_matched=False`로
      **이미 100% 검출**되는데 사후 동기화만 하고 경보를 안 낸다. "외부 미체결 주문이
      청산가능수량을 선점 중"을 대시보드에 표시. 검증: 13:26:41~13:30:50 재생 시 경보 출현.
      ⚠ 오늘 1건 — **확정 결론 금지**, 섀도 관측부터.

### 판정 기준 갱신 (문서)

- [ ] **F-1R 리허설 판정 기준에 "주문 거부"를 추가** — 현행 기준은
      `[ForceExitPass]`→`[TimeExit]`→`[ExitAttempt]` 출현 + 15:11 ERROR 미출현이다.
      오늘 사례는 **`[ExitAttempt]`는 뜨고 주문만 거부되는** 경우를 보여준다.
      → `[ExitSendOrderResult] ret==0`을 판정 기준에 **명시 추가**할 것
      (`dev_memory/NEXT_TODO.md` F-1R 항목 + `CLAUDE.md` 26주 재검증 절).
...
- [ ] **O-i3** 원인 가설 확정 — 13:26:41~13:30:50 구간 주문가능금액·청산가능수량
      (`CpTd6197`·`trades.db` 대조)
- [ ] **O-i4** 외부 주문 22레그 발생원 — 프로그램 후보 배제 완료(미륵이 1프로세스 ·
      메시아 G2 전 사이클 NO_TRADE·페이퍼). 남은 후보: HTS 수동 / 타 기기 접속 → **사용자 확인**
...
## 2026-09-01 (MW0601 — 일일 점검 장후: 이상점 1-6 P0·1-7 P2)

### P0

- [ ] **P0-2 (🔴 주간회의 안건 — 자동조치 C등급) `daily_close()` 진입 전 잔여 포지션
      확인 후 강제 청산 경로 동기 완주** — 1-6. 승인 전에는 구현하지 않는다.
      `main.py: daily_close()` 진입부 + `_ts_time_exit_pass()`(또는 동등 함수) 호출
      순서 확인부터. 재시도에는 511차 백오프(`EXIT_REJECT_BACKOFF_SEC` 등) 재사용,
      상한·타임아웃 필수(무한 대기 방지).

### P1

- [ ] **P1-3 (승인 불요 — 즉시 구현 가능) `daily_close()` 진입 시 잔여 포지션 있으면
      즉시 경보** — 1-6 임시 안전판. `main.py: daily_close()` 진입부 3줄.
- [ ] **`force_flat_guard.py` 15:12 단발 → 15:10~15:39 반복 스케줄로 확장** —
      `FORCE_FLAT_GUARD_AT` 단일값을 배열로. 알림 전용 성격 유지(주문 없음).
- [ ] **외부/MANUAL 진입 실시간 경보 훅** — `main.py` `[체결동기화] 외부진입` 로그
      지점에 알림 추가. F-15/F-16과 함께 검토.
- [ ] **⚠완료주장-커밋없음 — F-23(종료 마커)·F-24(센티넬 4번째 축)** — 513차
      DECISION_LOG는 "구현·테스트 완료"로 적었으나 2026-09-01 커밋 3건 중 해당
      커밋이 없다. 다음 세션에서 `git diff`로 실재 확인 후 커밋 또는 재작업.
- [ ] **`docs/정기점검/수익률향상_누적대장.md` 갱신 미이행** — 장후 세션이 신규
      방안(외부/MANUAL 실시간 경보)을 리포트 본문에만 기록하고 대장 파일 자체는
      수정하지 않았다. 다음 세션에서 대장에 신규 행 등록.

### P2

- [ ] **P2-2 `exit_stage='TRAIL_AFTER_TP1'` 오판정 3건 원인 확인** — 1-7.
      `grep -n "TRAIL_AFTER_TP1" main.py`로 판정 지점 확인 후 단일계약 설계 케이스와
      TP1 미도달 케이스 구분 플래그 추가.
- [ ] **레그합-포지션합 불일치(-54,538원) 원인 특정** — 귀속 실패 레그 3행
      (evidence §5). 신규 이상점으로 등록하기엔 근거가 옅어 관측만 등록.
...
## 2026-09-02 (MW0601 517차 — 장후 점검 결과)

> 상세: `DECISION_LOG.md` 2026-09-02(517차 — 장후 점검: 종합). 리포트
> `docs/정기점검/매일점검/MW0601-20260902-점검리포트.md` 장후(post) 절 + 종합.

### 완료 처리 (이월 항목)

- [x] **O-p2** — DB 확인 완료: `ensemble_decisions` 09-01 15:08:00 이후 판단
      행 0건, 진입(15:34:46) 대응 행 없음 → 시스템 자동진입 경로 아님 확정.
      "누가·어떻게"는 여전히 미확정(사용자 확인 필요, 아래 사용자 몫 참조).
- [x] **1-6 / O-i1** — `ConfFloorGuard` 원인 가설 판정 완료(시장 저신호 쪽 우세,
      확정 아님 — 313차 1일 대조 한계).
- [x] **1-2** — `PositionFallback`을 1-9(entry_source 세션 고착)로 재정의·승계.
- [x] **1-4** — 증거 다이제스트 불일치는 도구 한계(이월 포지션 미인식)로 재규정,
      데이터 손실 아님 확인. 고도화 G-4(다이제스트 이월 포지션 인식) 등록.

### 신규 — 장후 발견 3건

- [ ] **F-3 (P1)** — `main.py:self._entry_source`가 `__init__`에서만 `"SYSTEM_AUTO"`로
      초기화되고 이후 리셋되지 않는다. 정상 체결 지점(`grep -n "\[주문요청\]"`로
      위치 확인)에 `self._entry_source = "SYSTEM_AUTO"` 재세팅 추가. 검증:
      `tests/test_5XX_entry_source_reset.py` 신설. 오늘 실측: 10:19·10:49·13:34
      정상 진입 3건이 전부 `BROKER_SYNC_RECOVERY`로 오표기됨(`trades.id=469~473`).
- [ ] **O-t1** — 1-8 net 대사 잔차의 gross 축 차이(-315,000원) 원인 규명. 08-31과
      거래량 대비 차액 방향이 반대라 단순 수수료율 가설 배치 — 다음 사례 대기 또는
      `commission_rate_recon.py --verify` 재실행(판정식 불변).
- [ ] **O-t2** — F-10(exit_stage 다중체결 오분류) 3번째 사례 발생 여부 관찰.
      2/필요 "3일 이상" 도달 — 다음 발생 시 F-10 승인 검토 즉시 착수.
- [ ] **O-t3** — `entry_source` 오표기의 과거 영향 범위 회고 조사(코드 변경 불요,
      `trades.db` 전체 스캔으로 지금도 가능). 검증 캠페인 SYSTEM_AUTO 필터 표본
      누수 규모 추정.

### 지속 — 누적 대장 재검증 (1-10, P2)

- [ ] **P5-01** `3m` 호라이즌 승률 대비 기대값 역전 계측 — 09-01·09-02 미검증.
- [ ] **P5-02** `binding_gate` 별 손익 귀속 적재 — 09-01·09-02 미검증.
- [ ] **P5-03** TP1 비중·보호 트레일 오프셋 섀도 병렬 계산 — 09-01·09-02 미검증.
- [ ] **P5-04** 점심 블랙아웃 진입 손익 귀속 — 08-31 기준 표본 도달에 가장 근접
      (약 10건 부족) — 다음 세션 우선 확인.
- [ ] **P5-05** 대장 자체 갱신 여부 — 2026-09-04 재판정 예정일 임박.
- [ ] **P5-07** 「0.2pt 이하 이익」 구조적 net 적자 계측 — 09-01·09-02 미검증.
- [ ] **P5-08** 자가유발 안전장치 발동 진입공백 비용 — 09-01·09-02 미검증.

### 재이월

- [ ] **O-p4** — 사용자가 09-01 저녁 지시대로 포지션을 정리하려 시도했는지.
      응답 대기 지속.

### 사용자 몫 (자동조치 범위 밖 — 오늘 리포트 "사용자 조치" 절과 동일)

- [ ] 어제 저녁 "즉시 매도" 요청 실행 여부·미반영 사유 확인
- [ ] 정체불명 매매(09-01 38건 -163만원 + 오늘 손실 포지션) 본인 여부 확인 —
      DB로 "시스템 경로 아님"까지는 확정, "누구인지"는 사용자만 답할 수 있음
- [ ] 대신증권 계좌 정산 내역(오늘 실현손익 -556만원 vs 시스템 계산 -527만원,
      차액 30만원) 직접 재확인
- [ ] (참고) F-1(`daily_close()` 잔여 포지션 자동청산) 승인 여부 — 이번 주 3번째
      상정

## 2026-09-02 (MW0601 518차 — 장후 자동조치 구현 결과)

> 상세: `DECISION_LOG.md` 2026-09-02(518차). 리포트 제4부:
> `docs/정기점검/매일점검/MW0601-20260902-점검리포트.md`.

### 구현 완료 (커밋됨 · 재기동 대기)

- [x] **F-3** `_entry_source` 세션 고착 — `_ts_execute_entry()` 게이트 통과 후 ·
      주문 전송 전 `SYSTEM_AUTO` 복귀 (`main.py`)
- [x] **G-1(=G-3)** 재기동 시 브로커 잔량 경보 문구 — 조건부 ERROR 추가
      (훅은 이미 있었다 · 함정① 적발, `main.py`)
- [x] **G-4** 이월 포지션 손익 가시화 (`collect_evidence.py` §5)
- [x] 세션 번호 자리표시자 `51X차` → **517차** 확정 (dev_memory 2파일)

### 라이브 검증 대기 (다음 재기동 이후)

- [ ] 🔴 **미륵이 재기동** — F-3·G-1은 `main.py` 변경이라 재기동 전까지 실효 없음
- [ ] **F-3 실측**: 재기동 후 첫 자동진입의 `trades.entry_source = 'SYSTEM_AUTO'` 확인.
      **기한 2026-09-03 장후.** 특히 브로커 동기화/외부진입 이벤트가 **먼저 있은 날**에
      그 뒤 진입이 SYSTEM_AUTO로 찍히는지가 판정점이다
- [ ] **G-1 실측**: 재기동 시 잔량이 있으면 「2 경보」 탭에 새 ERROR 줄 실물 확인.
      ⚠ 잔량 없으면 **미발화가 정상** — 미발화를 결함으로 읽지 말 것
- [ ] **G-4 회귀**: 다음 장후 다이제스트에서 이월분이 있으면 블록이 뜨는지,
      없으면 안 뜨는지 양방향 확인

### C등급 — 승인 필요 (자동조치 손댈 수 없음)

- [ ] **F-1** `daily_close()` 진입 전 잔여 포지션 강제청산 — **주간회의 승인 대기
      (3번째 상정)**. 주문·청산 실행 경로 변경. 09-01·09-02 사고가 근거.
      회귀 위험: 마감 절차 지연 · 청산 재거부 시 무한 대기 → **상한 시도 횟수·타임아웃 필수**
- [ ] **CB② 복원**(`CB_CONSEC_STOP_LIMIT` 9999 → 2~3) — 재검토 기한 2026-08-29
      **초과 5일**. 3일 연속 동일 권고. 실전 전환 기준 ⑤
...
- [ ] **F-2** `[ConfFloorGuard]` 장시간 미복구 원인 규명 — 장후 `predictions` 테이블에서
      ...
      패턴이면 주간회의 안건 상정 검토
- [x] **G-2** `[ConfFloorGuard]` 발동 로그에 그 시점 `[Calibration]` auc·span 스냅샷
      동봉 — 장중에도 DB 조회 없이 레짐 탓/보정기 결함 1차 판단 가능하게
      **[DONE 2026-09-03 523차]** ⚠ `span`은 **이미 있었다**(482차 G-3 이래) — 실제
      결손은 `auc` 하나였다. WARNING·복구 INFO 양쪽에 동봉. 재기동해야 반영된다.
...
## 2026-09-03 (MW0601 522차 — 장후 점검)

### 승격 처리

- [x] **1-3 신규 등록** — `exit_stage=TRAIL_AFTER_TP1` 라벨 오분류(F-10) 3번째 사례,
      사전등록 승격검토 조건(3일 이상) 오늘 충족. 누적대장 P5-06 갱신 완료
      (🕒표본미달 → 🔬판정진행).

### 신규 등록

- [ ] **F-3(장후)** F-10(청산 라벨 오분류) 승격 여부 사용자 결정 — 승격 시
      `strategy/position/position_tracker.py: _sync_partial_progress()`/
      `classify_exit_stage()` 변경 필요. 회귀 위험(시계열 불연속) 검토 후
      `tests/test_507_exit_stage_multifill.py` 실행 + 과거 7건 소급 재분류 대조.
- [x] **G-3(장후)** `collect_evidence.py`(또는 `evidence_map.md` always_quote_patterns)에
      `ExitStageRecon` 패턴 추가 — 다음부터 수동 grep 없이 매일 자동 게재.
      **[DONE 2026-09-03 523차]** 🔴 `config_dailycheck_targets.json`에는 **넣지 않았다** —
      `load_config()`가 키 단위 통째 교체라 거기 쓰면 기본 패턴 30여 개가 사라진다.

### 이월 (09-02 519차 장후에서 승계, 오늘도 미해결)

- [ ] **CB② 발동 1회 관측** — 실전 전환 기준 ⑤의 남은 조건. 오늘도 카운터 3건
      발생했으나 매번 300초 창 밖이라 리셋(당일정지 미발동). 인위적으로 만들지 말 것
- [ ] **O-t1** net 대사 gross 축 차이(-315,000원) 원인 규명 — **09-02분은 여전히
      미규명.** 09-03분은 브로커 net(-165,630)과 엔진 net(-165,617)이 13원 차로
      일치함을 오늘 확인, 09-03분은 재발 없음
- [x] **O-t2 / 1-10 관련** F-10(exit_stage 다중체결 오분류) 3번째 사례 — **오늘
      이상점 1-3으로 정식 편입, 사전등록 승격검토 기준 충족.** 사용자 결정 대기로
      이관(위 F-3 참조)
- [ ] **O-t3** `entry_source` 오표기 과거 영향 범위 회고 스캔(F-3(구)은 미래만 고침 —
      번호 혼동 주의: 이 O-t3의 "F-3"은 오늘 신규 등록한 F-3(장후)와 다른 항목)
- [ ] **1-10** 누적 대장 P5-01~05·07·08 재검증 6건 — 오늘도 시간 제약으로 미실시,
      P5-06만 갱신됨(3거래일 연속 미실시 — 다음 장후가 반드시 수행할 것)

### 관측 예정

- [ ] **1-1** `session_state.json` P8 마커 소실 재발 여부 — 오늘 15:49 이후
      정상 기록 확인(`p8_last_success_date=2026-09-03`). 내일(09-04) 아침
      기동 직후 재확인 필요(표본 여전히 1일)
- [ ] **1-2** `[ConfFloorGuard]` 3거래일째(09-04) 재발 여부 — 오늘 DB 조사로
      가설(a) 레짐 탓에 무게 실림(확신도 자체는 6일 중 최고치), 확정 결론 보류.
      3일 연속이면 주간회의 안건 상정 검토
...
- [ ] 정체불명 매매 38건 · 09-02 손실 포지션 본인 여부 확인
- [ ] 대신증권 계좌·정산(-556만원) 직접 확인
- [ ] **신규**: F-10(청산 라벨 오분류) 승격 여부 결정 — 이번 주 검토 권고
      (`docs/정기점검/매일점검/MW0601-20260903-점검리포트.md` §1p-1-3·§5p 참조)

## 2026-09-03 (MW0601 523차 — 장후 자동조치)

> 예약작업 `mireuk-postmarket-autofix` 자동 실행. 522차가 낸 G-1·G-2·G-3 구현.
> 상세: `DECISION_LOG.md` 2026-09-03(523차), 리포트 제4부.

### 닫힌 항목

- [x] **G-1(장전)** `session_state.json` 3키(`date`·`p8_last_success_date`·
      `eod_retrain_ok_date`)를 증거 다이제스트 §9에 매일 게재 —
      `collect_evidence.py: state_snapshot_section()` 신설.
      「파일 없음」·「키 없음」·「null」·「값+일치여부」를 서로 다른 문구로 쓴다.
- [x] **G-2(장중)** `[ConfFloorGuard]` 로그에 보정기 `auc` 동봉(WARNING·복구 INFO 양쪽).
- [x] **G-3(장후)** `[ExitStageRecon]`을 `always_quote_patterns`에 등록.
- [x] 회귀 고정 `tests/test_523_conf_floor_auc_and_state_snapshot.py` 16건 신설.
- [x] 오늘 점검 산출물(리포트·증거 3종·누적대장·dev_memory) 커밋 — 522차가 남긴 몫.

### 신규 등록

- [ ] **523-1** 나머지 대사 로그 3종(`NetRecon`·`BrokerPnl`·`StateRecon`)도
      `always_quote_patterns`에 넣을지 검토. G-3과 **같은 실패 양식**(계측은 도는데
      다이제스트에 안 실려 수동 grep에 의존)이 그대로 적용되며,
      `tests/test_498_recon_log_inventory.py`가 이미 4종을 전부 등록해 두고 있다.
      ⚠ 오늘은 리포트 G-3의 명시 범위(ExitStageRecon)만 반영했다 — 범위 확대는
      다음 장후 세션 또는 사용자 판단.
- [ ] **523-2** `[ConfFloorGuard]` 로그의 `auc`가 실제로 유효한 최신값인지 확인 —
      `rank_auc`는 `_is_degenerate()` 안에서만 갱신되므로, 그 경로가 안 도는 구간에서는
      **직전 fit 시점의 값**일 수 있다. 내일 이후 라이브 로그에서 `auc=미측정` 비율과
      값 변화 주기를 관찰할 것(계측 4원칙 ④ — 낡은 값이 최신값처럼 보이는 형태).

### 관측 예정 (내일 09-04 장전에 자동으로 확인 가능해진 것)

- [ ] **1-1** `session_state.json` P8 마커 소실 재발 — **이제 손으로 열어보지 않아도
      된다.** 증거 다이제스트 §9의 「기동 마커 스냅샷」 표에서 `p8_last_success_date`가
      「키 없음」이거나 「아니오」면 재발이다.
- [ ] **1-2** `[ConfFloorGuard]` 3거래일째 재발 — 재기동 후에는 발동 로그 한 줄에
      `span`·`auc`가 함께 찍히므로, DB 조회 없이 가설 (a)레짐 / (b)보정기 결함을
      1차로 가를 수 있다.

### C등급 — 손대지 않음 (사용자·주간회의 결정)

- [ ] **F-3(장후) = F-10 승격** — `strategy/position/position_tracker.py`
      (`_sync_partial_progress()` / `classify_exit_stage()`) 변경. **청산 트리거 경로**라
      자동조치 범위 밖이다. 사전등록 승격검토 조건(3거래일 이상)은 오늘 충족
      (7포지션/3거래일)이나 **충족 = 자동 승격이 아니다**(313차 원칙 ④).
      승격 시 필요: 시계열 불연속 마커(461차 mdd_pct 유형) +
      `tests/test_507_exit_stage_multifill.py` 실행 + 과거 7건 소급 재분류 대조.

### 사용자 몫 (자동조치가 할 수 없음)

- [ ] 🔴 **미륵이 재기동** — G-2(auc 동봉)는 재기동해야 반영된다.
      G-1·G-3은 점검 도구 쪽이라 재기동과 무관하게 내일 바로 적용된다.
- [ ] **F-10 승격 여부 결정** — 이번 주 검토 권고.
- [ ] 09-01 "매수 3계약 매도" 지시 미반영 경위 확인
- [ ] 정체불명 매매 38건 · 09-02 손실 포지션 본인 여부 확인
- [ ] 대신증권 계좌·정산(-556만원) 직접 확인
- [ ] **CB② 발동 1회 관측** — 실전 전환 기준 ⑤의 남은 조건. 인위적으로 만들지 말 것.
...
- [ ] **531-3 (P1)** 1-3(ConfFloorGuard 3거래일 연속) — 장후 `predictions` 테이블 09:00~12:28
      ...
      이탈이면 주간회의 상정 검토.
- [x] **O-i(15:10 재확인)** — 재발 없음. 마지막 유입 15:01:09, 15:11:17 `[SchedForceExit]`
      로 FLAT 확인. 오버나이트 위험 회피(절대원칙① 준수).

## 2026-09-04 (MW0601 532차 — 장후 점검)

- [x] **531-1 / F-2** — 레이스 가설(ⓑ) 검증 완료: `[ChejanMatch]` 66건 전수가
      `pending='NONE'`(대기 주문 자체가 없었음, 매칭 실패 흔적 0건) — ⓑ 약화, ⓐ(외부
      접근)에 무게. **사용자 확인은 아직 미답변** — 다음 세션이 재확인.
- [x] **531-3** — DB 비교 완료: 09:00~12:28 confidence 분포(weight_collapsed 제외) 오늘
      0.3664, 최근 4거래일 0.3541~0.3721 — 뚜렷한 이탈 없음. 판정 유보 유지(313차, 표본
      1일 비교로 확정 불가). `min_conf` 임계 시계열 비교는 아직 못 함(G-3 고도화 제안).
- [ ] **532-1 (P0, 최우선 지속)** 정체불명 외부 진입 — 오늘 최종 62건/66계약. 사용자에게
      "이 계좌를 HTS/MTS 등으로 직접 조작한 적이 있는지" 재확인 요청(2026-08 하순부터
      미답변 지속). 답변 확보가 최우선.
- [ ] **532-2 (P2, 고도화)** G-3 — EOD 마감 로그에 그날 `min_conf`(DynMC 산출) 시간대별
      요약을 남긴다. 다음 ConfFloorGuard 장기화 시 confidence 분포와 함께 즉시 대조하기 위함.
- [ ] **532-3 (C등급, 사용자 승인 필요)** F-10 적용 승인 여부 — 사전등록 기준(10거래일 중
      3일 이상 미대응) 09-03에 충족, 09-04로 4일째 재확인(누계 13건/-13,160,648원). 승인
      시 `position_tracker.py:1509~1513` 작업 착수(선행조건: `main.py:4574` TP 재발동
      가드 분리).
- [ ] **532-4 (P2)** F-15(외부진입을 브로커 net 판정 경로에서 분리, 주간회의 안건) 재확인
      — 오늘 Live 20일 MDD(자본대비) 26.9%가 1-2 외부진입 손익과 섞여 있을 가능성. F-15
      처리 전에는 UNDERPERFORM 판정이 엔진 자체 열화인지 구분 불가.

## 2026-09-04 (MW0601 532차 후속 — 장후 자동조치)

- [x] **530-2 / G-1** — `_write_session_state()` 쓰기 계측 구현 완료.
...
- [x] **537-1 (해소)** 정체불명 외부 진입 — **사용자 확인으로 원인 규명 완료**:
      5거래일간(09-01·09-02·09-03·09-04·09-07) 전부 사용자 본인의 모의투자 병행 수동
      테스트 거래였다(계좌 보안 사고·주문추적 결함 아님). O-t2(6거래일째 재발 관측)
      취소, F-2 종결. 상세: `DECISION_LOG.md` 2026-09-07(537차) 항목 5.
      → 후속 정책 제안(G-4, 아래 537-8) 등록.
- [ ] **537-2 (P2, 신규)** 이상점 1-4 — F-1("session_state 마커 유실") "확정" 판정이
      532차 자신이 사전등록한 보조 확인 수단(`[SessionStateDrop]` 로그, 3거래일 연속
      미발동)과 어긋난다. **F-1 코드 적용은 보류 권고** — `increment_session()` 새
...
- [ ] **554-3 (확인 필요, 장후)** 554-1 이월 포지션의 실제 출처 규명 — 장중 라이브 DB
      ...
      🔵 **[해결 2026-09-10 12:2x / 554-A~D 세션] 출처 규명 완료 — 장후 조사 불필요.**
      엔진 진입 기록이 없는 이유는 **엔진이 진입한 적이 없기 때문**이다. 08:02:01 에
      pytest(`tests/test_493_exit_stage_all_builders.py:_opened()`)가 운영
      `data/position_state.json` 을 덮어썼다(`open_position(price=1040.0, quantity=2,
      atr=1.5)` → `_save_state()`). 파생값이 한 자리도 안 어긋난다 —
      `stop=1040−1.5×1.5=1037.75`, `tp1=1040+1.5×1.0=1041.50`.
      ⚠ 계좌에는 **유령 SHORT 2계약 @1109.42 가 실제로 있었다**(08:45 허구 청산이
      브로커에는 신규 매도였다). 12:17 재시동에서 `[BrokerSync] synced SHORT 2 @
      1109.42` 로 회수·청산(+463,232원). 상세: `DECISION_LOG.md` 554차 · 554차 후속.
...
> 커밋 `5f8e40e`. 구현 완료분은 리포트 제4부 4-1, 정정 2건은 4-2 참조.

- [x] **556-A (F-5)** Armistice 「고착」 ERROR 오탐 제거 — 경보를 `sync`/`time` 두
      갈래로 분리 + 워밍업 유예 900초. 차단 판정 무변경. `tests/test_556` F5-1~5.
- [x] **556-B (G-4)** ProfitGuard L2 래치 사유 스냅샷(`[LatchSnapshot]`) + 구성 레그
      원장(라이브·재기동 복원 양쪽). 라이브 반영 0 불변식 고정. `tests/test_556` G4-1~6.
- [x] **556-C (G-1)** `position_state.json` 회전 백업 3세대(포지션 동일성 변경 시에만).
      `tests/test_556` G1-1~6.
- [x] **556-D** 장후 F-5·O-t2 조사 종결 — 리포트 제3부 1-8·O-t2·§4-1 표 결론 3건 정정.
...
- [ ] 🔴 **F-1 (P0, 신규, 승인 대기) EOD 재학습 학습 데이터 풀 부족 조사·조치** —
      2026-09-13(559차) 배포된 `BackfillFilter`(신규 배선) + `UnitMismatch`(신규
      활성) 두 필터가 결합해 26주 EOD 학습 풀을 45,601 → 14,555행까지 줄여
      `MIN_TRAIN_BARS=15000` 미달로 EOD 재학습이 6개 호라이즌 전부 실패했다
      (09-01~09-11 8거래일은 무재현 — 오늘이 이 필터 조합의 첫 EOD 실행일).
      직접 DB 조회로 `BackfillFilter`의 실제 제외 범위(`feature_quality_score==0.3`)가
      79거래일·28,099행에 걸쳐 있어, 문서상 서술("복구 불가 25거래일·9,002~9,526행")
      보다 훨씬 넓다는 것도 확인했다 — 마커가 "07-13 사고 오염"과 "라이브 수집
      이전 일반 백필"을 구분하지 않을 가능성. 조사 우선순위: ① 백필 생성
      파이프라인에서 `feature_quality_score=0.3`이 찍히는 정확한 조건 확인
...
- [x] O-i3 — `.git/index.lock` 이 이 세션 시작 시점에 이미 해소되어 있었음
      (`git_lock_guard.py --check` = "정상 — 락 없음"). 누가 지웠는지는 불명.
- [x] O-p2 — EOD 1m 모델 교체 여부: `horizons_replaced: 6/6` 확인, 긍정 판정.
- [x] O-p3 — SessionBackfill inserted/open_fixed: 원인 규명됨 — 옛 보충 방식은
      다음날 아침에만 09-15분을 보충(inserted=5, mismatch=14). 589차가 「당일
      15:46~48 보충」을 배포해 근본 원인을 해결 — O-t3으로 이어받아 관찰.
...
## 2026-09-17 (MW0601 장중 점검 — 거래 0건 딥다이브)

> 근거: `docs/정기점검/매일점검/MW0601-20260917-점검리포트.md` 「장중(intra)」 절 ·
> `dev_memory/DECISION_LOG.md` 2026-09-17 장중 항목.
> **장중이라 코드 미변경. 아래 F-4~F-6은 장후 적용 후보다.**

- [ ] **F-4 (P1, 신규) EKS 발동 로그에 「보정기 상한」과 「지연 판정」 동반 기록** —
      `safety/system_health.py:evaluate_early_kill_switch()` 4개 분기 + `main.py:9046`
      호출부. ① 그 분의 `ConfFloorGuard` 상태(출력상한·span·auc·도달가능 여부)를 인자로
      받아 병기 — `발동 (conf_max=34.4% < 발동선=40.1% | 보정기 상한=34.8% ⇒ 구조적
      도달불가)`. ② `_gap_pipe_delayed` 판정 결과와 경과 ms를 **발동 분기에서도 항상**
      인쇄(현재 미인쇄라 2026-09-17 조사에서 `PipePerf` 교차대조가 필요했다).
      **판정 로직 무변경 · 로그 인자 추가만.** 검증: 다음 거래일 09:05 로그에 두 값 동반 출현.

- [ ] **F-5 (P1, 신규) `_gap_pipe_delayed` 제외 봉을 「미측정」으로 분리 계상** —
      `safety/system_health.py:record_gap_open_bar()` · `evaluate_early_kill_switch()`.
      `conf_max`가 한 번도 갱신되지 않은 경우를 `None`으로 구분해 `_is_cold_start` 근거에
      "conf 측정 0건"을 명시. 현재는 폴백값 `0.0`이 cold-start의 **양성 근거**로 쓰인다
      (계측 4원칙 ②·④).
      ⚠ **발동 조건식은 건드리지 않는다**(459차 선례 — 안전장치 라이브 발동 특성은
      근거 없이 바꾸지 않는다).
      ⚠ **회귀 테스트 선행 필수**: 09-08~09-17 8일치 실측 입력(delayed/policy/conf)으로
      판정이 기존과 100% 일치함을 고정한 뒤에만 착수.

- [ ] **F-6 (P2, 신규) 점검 수집기 크래시 패턴 오탐 제거** —
      `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py:131`.
      `Traceback` 부분문자열 매칭이 `[MainStallTrace] … → logs/mainstall_traceback_*.log`
      **파일명**에 걸린다(2026-09-17 §11-3 「Traceback 3건」 = 실제 0건). 480차 이후 상시
      동작하므로 **모든 미륵이 점검 리포트가 노출**. 제외 조건은 `mainstall_traceback_`
      접두사로 **좁게** 잡아 진짜 트레이스백을 놓치지 말 것.
      ⚠ SKILL.md §실행-9의 3곳 갱신 규약(저장소 정본·앱 저장 스킬·예약작업 3종) 준수 +
      **MW0602 몫은 파일로 전달**(브랜치 분기로 `git pull` 전파 안 됨).

- [ ] **G-3 (P2, 신규, 섀도 전용) EKS 판정 전에 「판정 가능성」을 먼저 묻는다** —
      `evaluate_early_kill_switch()` 진입부에서 "그 분의 보정기 상한 < 발동선"이면
      `UNMEASURABLE`로 분류해 **섀도 로그만** 남긴다(실제 발동 여부 무변경).
      `[SHS-EKS-Shadow] verdict=UNMEASURABLE|MEASURABLE` 일별 집계.
      현재 추정 100% — 참이면 EKS는 시장 신호를 한 번도 본 적이 없다는 뜻.
      **사전등록 필요 · 승격 전 최소 10거래일 관측.**

- [ ] **G-4 (P2, 신규, 주간회의 안건 — 결정 아님) EKS 회복 마감·조건 재설계** —
      `EKS_RECOVERY_DEADLINE=11:30` · `EKS_RECOVERY_INTERVAL_MIN=30` ·
      `EKS_RECOVERY_CONF_MIN_HITS=3`. 2026-09-17은 `conf_hits`가 0→0→0→1→2로 **단조
      증가** 중 11:30 마감으로 종료(필요 3에서 1 부족).
      논점 ⓐ 마감 연장(청산 창 압박과 상충) ⓑ 추세 상승 시 1회 연장 ⓒ 간격 단축
      ⓓ **1-4가 고쳐지면 발동 자체가 줄어 논점이 사라지는가**.
      ⚠ **매매 정책이므로 세션이 임의 변경 금지. ⓓ 때문에 F-4·F-5 적용 후 재관측이 선행.**
      표본 3건 — 확정 결론 금지(313차 원칙).

- [ ] **459차 F2 — 사전등록 트리거 도래 기록(2026-09-17)** —
      `safety/system_health.py:record_gap_open_bar()` 도크스트링이
      "`측정 z봉`의 z가 지속적으로 0에 가까우면 발동 조건 재설계를 결정할 것"이라
      사전등록했고, **`z=0`이 09-15·16·17 3일 연속**으로 충족됐다.
      ⚠ 표본 3일 — **재설계 방향은 확정하지 않는다**(313차 원칙). F-5가 분리 계상을
      먼저 넣고, 표본이 쌓인 뒤 방향을 정한다.

### 관측 등록 (O-i1~O-i5)

- [ ] **O-i1 (오늘 장후 판정)** 15:10까지 `[SHS-EKS]` 해제 로그 출현 여부 —
      설계상 11:30 이후 재평가 없음 → **0건이 정상**. 1건이라도 있으면 설계 이해가 틀린 것.
- [ ] **O-i2 (오늘 장후 판정)** 15:10까지 진입 0건 유지 여부 — 자동 0건 예상.
      수동 진입은 `[체결동기화] 외부진입`으로 **구분 집계**할 것.
- [ ] **O-i3 (오늘 장후 판정)** `[ConfFloorGuard]` 오후 도달가능/불가 전환 횟수 —
      오전은 로그 5줄 · 전환 4회(불가→가능→불가→가능→불가). 오후 패턴이 다른지.
- [ ] **O-i4 (다음 장중 판정 — 1-4의 반증 가능한 예측)** 다음 거래일 09:00
      `[PipePerf] total − (S7+S8) ≥ 1000ms` → **미발동(cold-start)** /
      `< 1000ms` → **발동**. **어긋나면 1-4의 인과가 불완전하므로 정정할 것.**
      2026-09-17 비교값 ~928ms로 경계에서 72ms — 재현 가능성 높음.
- [ ] **O-i5 (오늘 장후 판정)** `MainStallTrace` 스냅샷 누적(12:31 현재 3/20) —
      20 도달 시 상한 소진으로 이후 정지가 계측되지 않는다. 도달 여부 확인.

### 이월 처리 (장전 항목)

- [x] 1-1 (CYBOS 접속 끊김·재로그인 1차 실패) — **장중 미재현으로 해소**(09:00 이후 0건).
- [x] 1-2 (재기동 5회·access violation) — **장중 미재현으로 해소**(09:00 이후 0건).
      ※ F-2 조사 항목 자체는 "다음 장전 판정"으로 지속.
- [x] O-p1 (ConfFloorGuard 장중 해소 여부) — **판정 완료**: 하루 종일 고정이 아니라
      경계선 왕복(09:00 불가 → 10:49 가능 → 10:55 불가 → 11:07 가능 → 11:26 불가).
      그 왕복이 EKS 해제 실패(1-6)와 직결된다.
...
- [ ] **F-4·F-5 적용 승인** (장후) — 매매 정책·임계 무변경, 로그·계상만. 미적용 시
      내일도 09:00 계산이 1초보다 빠르면 하루 종일 거래 정지 재발.
- [ ] **G-2 승인** (2026-09-15 등록, **3일째 대기**) — EKS 미해제 확정 시 대시보드
      "오늘 자동 재개 없음 — 수동 진입만 가능" 상시 배지.
- [ ] `.git/index.lock` — 12:28 생성분은 **HOLD(판정보류)**. **지우지 말 것.**
      장후 세션이 재판정한다. (장전 §7-B의 STALE 건은 09:51 커밋 성공으로 해소됨.)

### 실전 전환 기준 영향

- [ ] **기준 ⑤(CB② 발동 1회 관측)** — 진입 0건 3일 연속이라 **관측 기회 자체가 생기지
      않는다.** 1-4 미해소 시 ⑤는 무기한 열린 채로 남는다. 두 항목이 독립이 아님을
      전환기준 판정 시 명시할 것.
- [ ] **기준 ①(4주 통산 수익률 양수)** — 거래 없는 날이 4주 창에 누적되면 분자·분모가
      모두 얇아진다. 판정 시 **거래일 수와 진입 0일 수를 병기**할 것.

## 2026-09-17 (MW0601 장중 점검 — 예약작업, 병행 세션 후속)

- [x] 리포트 파일에 "0i-D. 병행 세션 충돌 확인" 문단 추가 완료 — 장후 세션이 EKS 관련
      최종 판정 시 윈도우 세션 쪽(이상점 1-4~1-7, F-4~F-6, G-3~G-4)을 채택하도록 명시.
- [ ] **(장후 확인)** 이 세션의 1-5(09:40:07 메인 스레드 7,734ms 정지, CB⑤ 사각 4,738ms/61%)
      — 윈도우 세션이 다루지 않은 별개 관측이므로 당일 이상점 통합 대장에 독립 항목으로
      포함할 것. `exc_tags=[LEVELS 09:30]×1`과의 인과는 미확정(표본 1건).
- [ ] **(장후 확인)** `.git/index.lock`(12:28 생성) 최종 처리 여부 — 이 세션 종료 시점
      (12:47)까지 계속 HOLD(판정보류) 상태였다. 두 세션 모두 손대지 않았음.


## 2026-09-17 (MW0601 599차 — F-4·F-5·G-2 구현 완료, 미커밋)

- [x] **F-4 (2026-09-17 장중 등록)** EKS 발동 로그에 보정기 상한·지연 판정 동반 기록 —
      **구현 완료.** `main.py` 봉별 `[SHS-EKS-Bar]` 신설 + 호출부 보정기 상태 전달 +
      `safety/system_health.py` 판정 4분기 전부에 병기 + `to_dict()` `gap_open_delayed`.
      판정 로직 무변경. 38건 테스트 통과.
- [x] **F-5 (2026-09-17 장중 등록)** `_gap_pipe_delayed` 제외 봉을 「미측정」으로 분리
      계상 — **구현 완료.** `_gap_open_conf_measured_count` 신설(4곳).
      ⚠ **`_is_cold_start` 계산식은 의도적으로 무변경** — 아래 후속 항목 참조.
- [x] **G-2 (2026-09-15 등록, 3일 대기 후 2026-09-17 사용자 승인)** EKS 11:30 마감 경과 시
      대시보드 "오늘 자동 재개 없음 — 수동 진입만 가능" 배지 — **구현 완료.**
      `is_eks_recovery_closed()` 단일 출처 + 배지 4번째 상태 + 툴팁 범례 2줄 분리.

- [ ] 🔴 **사용자 조치: 커밋** — 이 세션은 커밋하지 않았다(SKILL.md §6).
      대상: `main.py` · `safety/system_health.py` · `dashboard/main_dashboard.py` ·
      `tests/test_599_eks_gap_open_instrumentation.py`(신규) ·
      `docs/정기점검/매일점검/MW0601-20260917-점검리포트.md` ·
      `docs/정기점검/매일점검/evidence_MW0601-20260917_intra.md`(신규) ·
      `dev_memory/DECISION_LOG.md` · `dev_memory/NEXT_TODO.md`.
      제목 예: `[MW0601] 599차: EKS 판정은 시장이 아니라 1초 경주를 재고 있었다 — 계측만 고친다`
      ⚠ `git add .` 금지 — 경로를 명시할 것(CRLF 파생 변경 다수).

- [ ] **599-A (P1) 재기동 후 라이브 확인** — 장 마감 후 정규 재기동으로 반영된다.
      **다음 거래일 09:05 로그**에 `conf측정=`·`delayed=`·`보정기 상한=`·`⇒ 구조적
      도달불가|도달가능`이 찍히는지 확인. 안 찍히면 배선이 끊긴 것이다.
      **11:30 이후 대시보드 배지가 `⛔ 오늘 종료`로 바뀌는지**도 함께(EKS 발동일 한정).

- [ ] **599-B (P2) `_is_cold_start` 판정식 재설계 — 표본 대기** (459차 F2 연장선)
      현재 `conf_max == 0.0` 을 본다. `not conf_measured` 로 바꾸는 것이 자연스러워
      보이나 **동치가 아니다**("산입됐는데 conf 가 정확히 0.0"에서 갈린다).
      F-5 가 넣은 `gap_open_conf_measured` 로 그 경우의 **라이브 빈도를 먼저 센다.**
      판정 기준(사전등록): 10거래일 관측 후 "conf측정>0 이면서 conf_max==0.0"인 날이
      **0건**이면 두 식이 실질 동치이므로 교체 안전, 1건 이상이면 교체가 판정을 바꾸므로
      별도 근거 필요. `test_cold_start_formula_untouched` 가 무단 변경을 막는다.

- [ ] **F-7 (P1, 신규) `peter_paste` DB 폴백에 `*_measured`/`*_fallback` 동반 컬럼** —
      `dashboard/main_dashboard.py` `peter_paste` INSERT 3곳이 `or ''`·`or 0.0` 으로
      폴백값을 쓰면서 폴백 여부 플래그가 없다. `test_457_fallback_visibility::
      test_db_fallback_columns_have_measured_flag` **현재 FAIL**.
      **계측 4원칙 ④ 정면 위반**이며 `test_457` 이 바로 그것을 막으려고 만든 전수 가드다 —
      가드는 울렸는데 아무도 안 들었다(2026-09-13 `O-77` 과 같은 계열).
      ⚠ 599차 세션과 **무관한 선행 결함**이다(내 diff 에 `peter_paste` 0건,
      `git show HEAD:` 판에 이미 3건 — 592~598차 피터 사료 작업).
      조치 선택지: ⓐ 동반 컬럼 추가 ⓑ `_DB_FALLBACK_PINNED` 에 **근거와 함께** 등록.
      근거 없는 등록은 규약 위회이므로 ⓐ 우선.

- [ ] **O-i6 (장후 판정, 신규)** `test_456_wave1_stats_and_shs::
      test_trend_sql_counts_positions_not_legs` FAIL(`KeyError: '2026-08-10'`) 이
      데이터 의존(30일 창 이동)인지 진짜 회귀인지 — **장중 DB 조회 금지라 보류**.
      장후에 `fetch_trend_daily(30)` 반환 날짜 범위를 확인해 판정.
      데이터 의존이면 테스트를 창 이동에 견디도록 고칠 것(F-8 후보).

### 이 세션이 지킨 제약 (다음 세션이 확인할 것)

- [x] EKS 발동/해제 **판정 무변경** — 실측 7거래일 회귀 테스트로 고정.
- [x] 매매 정책 상수(`EKS_RECOVERY_DEADLINE`·`INTERVAL`·`MIN_HITS`·`TRIGGER_MARGIN`·
      `MIN_BARS`) **무변경** — `test_recovery_deadline_and_constants_untouched` 로 고정.
      G-4(회복 마감 재설계)는 여전히 **주간회의 안건**이다.
- [x] 라이브 프로세스 **재기동 안 함** · 커밋 안 함 · 장중 DB 스캔 안 함.
- [x] `.git/index.lock`(12:28 수집기 생성분) **HOLD 판정이라 지우지 않음.**

### 테스트 작성 교훈 (재발 방지)

- [ ] 🔴 **이 저장소에서 로그 검증에 `caplog` 를 쓰지 말 것** — 프로젝트 로거가 루트로
      **전파되지 않아** 빈 목록이 돌아온다(2026-09-17 실측). 그대로 두면 **항상 통과하는
      가짜 테스트**가 된다. `logging.Handler` 를 대상 로거에 직접 달고, **헬퍼가 정말
      잡는지 검사하는 테스트를 하나 먼저 둘 것**
      (`test_599_…::test_capture_helper_actually_captures` 가 본보기).
      기존 테스트 중 `caplog` 로 로그 부재를 단언하는 것이 있으면 전수 점검 대상.

### 15:45 후속 — 병행 세션 번호 충돌

- [예약] 장중 절(12:33)과 [대화] 장중 절(12:40)이 `1-4`·`1-5`·`O-i1`·`O-i2` 를 **각각 다른 뜻으로** 발급했다.
  대조표: 리포트 「제2부-C. 병행 세션 번호 대조」. **번호만으로 인용 금지** — `1-4[예약]`/`1-4[대화]` 또는 내용으로 부를 것.
  최종 심각도는 **P1**([대화]가 [예약]에 없던 실측을 더한 것이지 뒤집은 것이 아니다).
- [예약] 절이 최상위 `#` 헤더를 다시 써서 하루 한 파일 규약상 `#` 가 2개다 — **고치지 않고 남김**(의미 변경이라 append 규약 저촉). 다음 세션 판단.
- `.git/index.lock` **STALE 확정**(0바이트·3.3시간·git 프로세스 0). `--reclaim` 권한 거부로 실행 불가 → **사용자 수동 삭제 필요.**

### 16:20 채점기 딥다이브 (사용자 지시, 장 마감 후)

- [ ] 🔴 **F-9 (P1, 신규) 앙상블 보정기 35일 동결 — 저장 실패가 조용하다**
      `learning/calibration.py:save()` 가 `_fitted=False`(축퇴/도달불가 시 해제)에서
      **조용히 False** 를 돌려주고, `main.py:13062` 가 `if save():` 로 **성공만** 로깅한다.
      결과: 장 마감 시 축퇴 상태면 그날 학습이 통째로 버려지고 **로그에 흔적이 없다**.
      실측 — 복원값이 35일째 동일(`n=888 span=0.0063 auc=0.550 out_max=0.3479`),
      아티팩트 mtime 2026-09-15, 저장 성공 로그 35일간 6회(뒤 4회 전부 n=888),
      그동안 `daily_close()` 는 매일 정상 실행.
      **동결 상태의 out_max=0.3479 < 진입 하한 0.42** → 매일 아침 구조적 진입 불가.
      ⇒ **이상점 1-4(1초 경주)와 같은 뿌리다.** 1000ms 경계는 이 동결이 만든
      「항상 불합격」을 가리고 있던 우연한 차폐막이었다.
      계측 4원칙 ④ 위반. FP-CRITICAL 죽은 게이트·TOX 죽은 섀도와 같은 계열.
      **수정 순서 고정: A(저장 실패 가시화, 위험 0) → 관측 → B(버퍼 분리 저장) /
      C(축퇴 임계 재설계, 매매 정책 — 주간회의).** C 를 먼저 건드리지 말 것.

- [ ] **F-9-A (P1) 저장 실패 가시화** — `save()` 가 실패 사유(`not fitted` / `sklearn 없음` /
      예외)를 WARNING 으로 남기고, 호출부가 성공·실패 양쪽을 로깅한다. **로그 전용, 위험 없음.**
      검증: 다음 거래일 15:40 에 성공이든 실패든 한 줄이 반드시 남는지.

- [ ] **G-4 보강 (주간회의 안건에 정량 근거 첨부)** — EKS 회복 창(09:20~11:30)이
      **합격률 최저 구간과 겹친다**. 45거래일 실측 합격률: 09시 27.1% · 10시 37.0% ·
      11시 31.7% · **13시 46.9% · 14시 44.2%**(12시·15시는 진입금지존이라 제외).
      회복 조건이 `conf_hits>=3/10` 인데 09~11시 기저율 27~37% → 기대 히트 2.7~3.7 로
      **임계 3과 거의 같아 판정이 동전 던지기**다. 11:30 이전 합격 건수:
      09-15 **0건**(해제가 수학적으로 불가) · 09-16 17건 · 09-17 7건.
      ⚠ 마감 연장은 **매매 정책 변경** — 여전히 주간회의 몫이다.

- [ ] **O-t1 (다음 거래일)** F-9-A 배포 전이라면, 15:40 에 저장 성공 로그가 뜨는지만
      맨눈 확인 — 안 뜨면 그날도 학습이 버려진 것이다(아티팩트 mtime 으로 교차확인).

**근거**: 리포트 「제2부-D. 채점기(앙상블 보정기) 딥다이브」.
원천 `data/db/predictions.db:ensemble_decisions`(45거래일, 붕괴행 제외 n=9,462) ·
`logs/2026091*_*.log` · `learning/calibration.py` · `main.py:13062`.
핵심 실측: **conf>=0.40 인 분의 91.2% 가 「보정 미적용(raw 통과)」** —
점수가 합격선을 넘는 것은 채점기가 정상이어서가 아니라 **꺼졌기 때문**이다.
...
- [ ] 🔴 **F-10 (P1, 신규) `2356820` 체리픽 — 보정기 영속화 복구 (dev → v9-dev)**
      ...
      (2026-08-23 14:13:42, dev). **v9-dev 에 네 조각 전부 미적용**:
      ① `calibration.py:save()` 게이트에서 `_fitted` 제거
         (dev `if not _SKLEARN_OK:` / v9-dev `if not _SKLEARN_OK or not self._fitted:`)
      ② `calibration.py:load()` `state.get("fitted", True)` (축퇴 저장본 부활 방지)
         — v9-dev 는 `self._fitted = True` 무조건
      ③ `main.py` 저장 호출부 `else` 분기 WARNING `저장 건너뜀`
      ④ `tests/test_484_calibrator_persistence.py`
      ⇒ **F-9-A(저장 실패 가시화)는 F-10 에 포함되므로 흡수한다.** 자체 구현하지 말고
        검증된 원 커밋을 가져올 것.
      ⚠ 줄 위치가 크게 다르다(`save()` dev 626행 / v9-dev 396행) — 599차처럼
        **격리 worktree 시험 적용 → 테스트 → 확정** 순서 준수.
      ⚠ 체리픽 시 CLAUDE.md 요구대로 DECISION_LOG 에 **원 sha · 원 PC · 이유** 기록.

- [ ] **O-t2 (F-10 적용 후 관측, 사전등록)** **F-10 이 1-4(EKS)를 자동 해소한다고 쓰지 말 것.**
      `dev` 기록상 수정 **이틀 뒤(2026-08-25)에도** `09:00:01 [ConfFloorGuard] …
      out_max=0.3479 < 필요 0.4420` 이 관측됐다(그쪽 O-4 로 관측만 등록).
      즉 영속화가 복구돼도 **아침 출력상한이 진입 하한을 넘는다는 보장이 없다.**
      판정 기준: F-10 배포 후 5거래일 09:00 `out_max` 추이 —
      ⓐ 상승해 `min_conf` 를 넘으면 1-4 해소 경로 확보 ⓑ 0.3479 부근 고착이면
      **보정기 함수형(Platt 축퇴) 자체가 별개 문제**로 확정되고 G-4/C갈래로 넘어간다.

- [ ] **F-11 (P2, 신규) `[Calibration]` 로그에 보정기 인스턴스 이름 접두**
      `learning/calibration.py` 의 `logger` 가 모듈 단위라 `ensemble_calibrator` ·
      `horizon_calibrator` · `extremity_corrector` 가 **같은 태그를 공유**한다.
      그래서 2026-09-17 제2부-D §3 이 축퇴 전이 1,591회를 앙상블 보정기 단독인 것처럼
      집계하는 오류를 냈다(제2부-E §7 에서 자체 정정). **보정기별 분리 집계가 현재 불가**이며
      이는 계측 결함이다 — 계측 4원칙 ③(탈락·출처 가시화) 계열.

- [ ] **G-5 (주간회의 안건 — 결정 아님) 멀티PC 조율 폐기 결정에 「안전·계측 결함 픽스」 예외 신설 검토**
      2026-08-23 결정(487차)으로 상호 반영 의무가 사라졌고, **같은 날(485차)** 나온
      보정기 영속화 픽스가 **26일간 v9-dev 로 오지 않았다.** 그 공백이 이상점 1-9→1-4 의
      뿌리다. ⚠ **조율 폐기 결정을 되돌리자는 것이 아니다** — 폐기 근거(조율 비용)는 유효하다.
      묻는 것은 **"안전장치·계측 결함 픽스만은 양방향 통지 대상으로 둘 것인가"** 하나다.
      비용 최소안: 픽스 커밋 제목에 태그(예: `[XFIX]`)를 달고 **주간회의 때 상대 브랜치에서
      `git log --grep=XFIX` 한 번 훑기**. 상시 조율이 아니라 **주 1회 단방향 조회**다.
