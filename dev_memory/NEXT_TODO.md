...
- [ ] **Slack 워크스페이스 조치(사용자, 코드 무관)** — 요금제/메시지 한도 확인 후 유료
  전환 또는 다른 워크스페이스로 봇 이전. 이게 해결되기 전까지는 `utils/slack_queue.py`
  수정과 무관하게 실제 알림은 계속 실패함(단, 이제는 WARN.log/대시보드에 보임).
- [ ] **라이브 검증** — 다음 기동(내일 08:45) 시 Slack이 여전히 실패 중이면 08:4x분 내
  `WARN.log`/대시보드 SYSTEM 탭에 `[SlackQueue] Slack 알림 전송 실패 지속 중` 경고가
  뜨는지 확인. Slack 쪽이 해결됐다면 이 경고 자체가 안 뜨는 게 정상(성공 시 경고 없음).
