...
- [ ] 🔴 **장중 프로브 — 화면 8221의 COM 원천 찾기** (P1, 장중 1회)
      ...
      **그래도 프로브할 값어치가 있는 이유**: HTS 화면 8221이 데이터를 받는 이상 그 뒤에
      ...
      📌 **스케일 미해결**: 화면 8221 코스피 비차익 매도 합(≈11.1조, 외국인 10.66조가
      대부분)이 같은 날 `CpSvr8111` 비차익 매도(3,001억)와 **약 37배** 차이 난다.
      스크린샷 판독 오류·`통합(KRX+NXT)` 범위·단위 중 무엇인지 미상.
      **두 소스를 이어 붙이기 전에 반드시 해소할 것.**

- [ ] 🟢 **[신규 기회] `Dscbo1.CpSvr7225` 검토 — 옵션 flow 미매핑의 직접 해답 후보**
      (P2, 451차 후속2 문서조사 중 발견)
      「7225 시장매매흐름분석」, **공식 문서에 있고 지원되는 객체**이며 이 PC에 등록돼 있다.
      입력 = 최종시간(0=현재). 출력 = 시간 / 장내외국인 / 장내기관 / 코스닥외국인 /
      코스닥기관 / **프로그램차익** / **프로그램비차익** / **선물외국인** / **선물기관** /
      **옵션외국인** / **옵션기관**.
      **왜 중요한가**: `CybosInvestorData._option_flow_supported`는 지금
      `"Cybos option investor-flow mapping pending"` 상태로 남아 있는데, 이 객체가
      옵션 외국인·기관을 직접 준다. 게다가 선물 투자자 + 프로그램까지 **한 번의 호출**로
      온다(현재는 7221 + 8111 두 번).
      ⚠ 검증 필요: "시간대별"의 실제 granularity(시간 단위면 1분봉엔 거칠다), 값이
      누적인지 구간인지, 7221과의 정합성. **기존 7221 경로를 갈아엎지 말고 병행 기록부터.**
      (317차 원칙: 산식·소스 교체는 train/serve skew를 만든다)
...
- [ ] **B안 — 8111 미사용 54필드(차익/비차익 gross·위탁/자기) 수집·활용 검토**
      ...
      **실측 검증 완료(2026-08-07 `logs/20260807_PROBE.log` `[CybosProbe][RAW]` 덤프).**
      ...
      🔴 **단 `idx17`(차익 순매수 위탁금액)은 대사에 실패했다 — 쓰기 전에 확인할 것.**
      산술상 4,902−4,972 = **−70**이어야 하는데 실측은 **−24,518**로 `idx19`(합계금액)와
      **똑같다.** 비차익(`idx35`)·전체(`idx53`)는 같은 자리에서 정확히 맞으므로 차익
      블록만의 문제다. 문서 레이아웃 오류인지 그날의 데이터 특성인지 미상 —
      **여러 날짜의 `[CybosProbe][RAW]` 덤프를 대사해 확정한 뒤 쓸 것.**
      (반대로 `idx19`·`idx37`은 대사 통과했고, 둘의 합 109,980이 TR 자신의
       `idx55`(전체 순매수 합계금액)와 일치해 현행 수집이 옳음도 함께 확인됐다.)

      **진행 순서**
      1. 여러 날짜 PROBE 로그 덤프로 idx17 포함 전 필드 대사 (오프라인, 무위험)
      ...
      ⚠ 신규 피처는 과거 DB에 없다 — 백필 불가이므로 **표본이 앞으로 쌓이는 만큼만**
        검증된다. 이번에 폐기한 3종처럼 "있는데 안 쓰는" 상태로 오래 두지 말 것.
      ⚠ 단위는 **백만원 추정**이다(전체매수 439,555 → 4.4조가 아니라 4,395억으로 읽힘).
        피처화 전에 확정할 것 — `_INV_LOG_COLS` 주석이 이미 같은 축에서 "계약수"로
        틀려 있다(아래 항목).
...
- [ ] **gross PCR 승격 조사** (P2, 라이브 probe 필요)
      `CpSvrNew7221` 행3(옵션콜)·행4(옵션풋)의 **열3=외인매도 / 열4=외인매수**를 파싱하면
      net의 두 한계(캡 고착 21.3%, 중앙값 1.59 편향)가 함께 풀린다.
      🔴 **로그에 gross가 기록된 적이 없어 오프라인 검증이 불가** — 먼저 `_probe_investor_tr`
      RAW 덤프로 열3/열4 스케일을 확인하고, **며칠 병행 기록한 뒤** 교체를 판단할 것.
