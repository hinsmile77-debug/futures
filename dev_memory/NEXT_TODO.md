...
### 554차 — 유령 포지션 사고 뒷수습 (MW0601, 2026-09-10) — **매매 정책 무변경**

> 근거: `DECISION_LOG.md` 2026-09-10(554차). 회귀 가드: `tests/test_554_position_state_isolation.py`(12건).

- [x] **554-1** `tests/conftest.py` autouse 격리 + `_save_state()` 운영경로 가드
- [x] **554-2** `load_state(reference_price=…)` 괴리 가드(5.0%) + `.rejected_*` 격리
- [x] **554-3** `_broker_position_reconciled` 신설 + `[UnreconciledExit]` CRITICAL
- [ ] **554-4** 🔴 **장 마감 후** — `trades.db` 2026-09-10 08:45 허구 2레그
      (+6,921,594원) 처리. 전환기준 ①은 브로커 실측 net 기준이라 무오염이지만,
      엔진 net·승률·`exit_stage` 집계는 오염된다. 지우지 말고 사유 컬럼으로
      배제 표기하는 방법을 먼저 결정할 것(461차 불연속 마커 관례).
      ⚠ `broker gross vs engine gross` 대사가 오늘 크게 어긋나는 것은 이 때문이다.
- [ ] **554-5** 커밋 — 장 마감 후(v9-dev 장중 실시간코드 커밋 금지).
      ⚠ 그 전에 장중 재기동이 걸리면 수정된 코드가 로드된다(테스트는 통과 상태).
- [ ] **554-6** 전수 조사 — 다른 운영 상태파일도 같은 경로로 뚫리는가.
