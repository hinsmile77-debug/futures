...
- [ ] **① conf 하락(06-29~) 근본 원인 특정** — `predictions` 5m 일평균 confidence가
      0.41~0.50대(06-19~06-30)에서 0.39대(07-01~07-02)로 떨어진 시점 전후 EOD 재학습 로그·
      canary z경고 피처·scaler refit 로그를 대조해 무엇이 모델 신뢰도를 깎았는지 확인
- [ ] **② CB③(30분 정확도<35%→당일 정지) 오늘 발동 여부 확인** — `calibration_report.md`
      기준 30m 호라이즌 누적 accuracy가 32.88%로 이미 CB③ 임계(35%) 밑인데, 오늘 실제
      CB③ HALT 로그가 있었는지 확인. 없었다면 CB③이 30분 이동평균 윈도우와 누적 통계
      중 어느 걸 보는지, 왜 상시 발동하지 않는지 코드 재확인 필요
- [ ] **③ conf_inversion 임계값(3%p) 근소 미달 관찰** — 현재 gap=2.85%p로 발동 임계 바로
      아래. 273차 EKS 마진 이슈와 동일 패턴이므로 1~2주 관찰 후 임계를 2%p대로 낮출지
      히스테리시스를 넣을지 판단
- [ ] **④ drift_adjuster 최소 표본수 가드 검토** — `acc_history` 최근 10개가
      0.156~0.5로 요동(표본 부족 구간 노이즈). alpha=0.0076로만 완만히 적응 중인데
      최소 표본수 미달 구간은 업데이트를 스킵하는 가드가 필요한지 검토
