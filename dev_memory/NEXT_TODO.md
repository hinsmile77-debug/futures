...
- [ ] **[31] 앵커 재도출** — [30]이 새 밴드 분포를 확정한 뒤.
      현행 HI=0.90/LO=0.45는 **재보정 전(cancel 포화) 분포**로 노출중립을 맞춘 값이라
      분포가 이동하면 평균 배수가 0.70에서 이탈한다. 리포트 [31] 상세절의
      `shadow_mult_mean`이 그 감시 지표 — 0.70에서 크게 벗어나면 재도출 신호.
