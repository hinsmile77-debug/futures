...
- [ ] **05-18~06-15(15일) 양상 분해** — 기록 89~105 / 비영 25~76으로 앞 K칸 불변식이
      성립하지 않는다. 호라이즌 슬라이싱 이전이라 모델이 실제로 다피처였을 가능성이
      있어 같은 결함으로 단정할 수 없다. 오염이면 그 기간 SHAP 순위도 무효다.
- [ ] **그 22거래일 SHAP 순위를 인용한 판단이 있는지 확인** — `replace_log`가 비어
      있어 **자동 교체는 없었음이 확정**됐으나, 사람이 대시보드 동적피처 탭이나 주간심사
      rank_table을 보고 내린 결정이 있다면 근거가 무효다(2026-06-17~07-10 세션 로그 대조).
