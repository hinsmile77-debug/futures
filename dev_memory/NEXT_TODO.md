...
- [ ] **`meta`·`tox` 배수가 사이즈를 계통적으로 누르는 것이 의도인가** (417차 신규 등록)
      ...
      결정하고 있는 것**인지 미확인. 후자라면 ⑧의 "MAX_CONTRACTS 재검토"는 상한이
      아니라 **게이트 배수 축**으로 다시 써야 한다.
      ⚠ 이 배수들은 66건 표본에서 손익 예측력이 없었다(rho: tox +0.030 p=0.81 /
      meta +0.048 p=0.70) — 즉 **예측력 없는 배수가 사이즈를 지배**하고 있다.
...
- [ ] **`CONFIDENCE_MULT_TABLE` 죽은 파라미터** — 311차 후속(07-12)에 등록됐고
