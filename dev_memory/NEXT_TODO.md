...
- [ ] **Phase B/D/재배치 중 택일 — 사용자 결정 대기 (최우선)**
      BH 통과 0개라 **Phase B에 올릴 후보가 없다.** 선택지:
      (a) 표본 축적 후 Phase A 재실행 — 유효표본이 7일뿐이라 중간 효과를 놓쳤을 수 있음
      (b) Phase D 틱 그물 — 정보가 분봉이 아니라 틱에 있을 가능성(HOGA 로그 일 11.9만 줄)
      (c) 부정적 verdict 확정 → 청산기하·집행 축으로 재배치
      ⚠ 결정 전 착수 금지. 특히 (b)는 구현이 무거워 되돌리기 어렵다.
