...
### 483차 — 점검 체계 이관 (MW0602, 2026-08-21) — **사용자 조치 1건**

근거: `dev_memory/DECISION_LOG.md` 2026-08-21(483차),
지침 `docs/프롬프트/MW0602_미륵이_점검체계_이관지침.md` §3.

- [ ] 🔴 **예약작업 3종 프롬프트 갱신 — 사용자만 할 수 있다** (세션에서 조회·수정 불가.
      `CronList` 는 세션 cron만 보고 Claude 앱 로컬 등록분은 안 보인다. 예약은 PC를
      건너가지 않는다 — 지침 §0 「함정 ③」).
      대상: MW0602에 등록된 미륵이 장전·장중·장후 3개.
      넣을 것: 지침 §3 「세 프롬프트 공통 블록」(리포트 대원칙 A·B) + 국면별 `## 산출` 절 교체
      + 자체검증 12항목.
      ⚠ **MW0601 프롬프트를 그대로 복사하지 말 것** — 리포 루트(`C:\Users\pc1\PycharmProjects\futures`),
      운영 브랜치(`dev`), 커밋 태그(`[MW0602]`)가 다르다. **예약 시각은 현재 등록값을 유지**한다.
      안 하면: 리포트가 계속 옛 형식(`§0`에 전문용어·「영향」 한 줄)으로 나오고,
      장중·장후가 `-intra`/`-post` 파일을 새로 만들 수 있다.
- [x] ~~`references/report_template.md` 「영향 2줄」 보강~~ **[DONE 2026-08-21]**
      정본 `SKILL.md:71-75` 와 어긋나 있던 것을 맞춤(3곳).
- [x] ~~`references/phases.md` 확인~~ **[DONE 2026-08-21]** 이미 반영돼 있어 무변경.
- [x] ~~파일명 기대 코드 legacy 패턴 도입(지침 §4)~~ **[불요 2026-08-21]**
      `grep "점검리포트"` 15건이 전부 주석의 근거 인용이라 고칠 감시 코드가 없다.
      도입하면 감시 대상 없는 죽은 코드만 는다(468차 G-2 계열).

---

### 470차 후속6 — 0814 장전 재점검 (MW0602, 2026-08-14 19:35) — **P0 0건 · P1 2건**

근거: `dev_memory/DECISION_LOG.md` 2026-08-14(470차 후속6),
`docs/정기점검/매일점검/MW0602-20260814-점검리포트.md` `## 장전(pre) 재점검` 절.

> ⚠ **핵심**: 468차 G-1은 EOD 주경로에서 구조적으로 해제되지 않는다. 08-14 EOD가 사이드카
> 6개를 새로 썼는데 `label_scheme`이 전부 `null`이다. **라이브 위험은 없다**(F-1이 같은
> 배수를 이미 풀고 있고 폴백이 보수 방향) — 막히는 것은 **실전전환기준 ⑧**이다.

#### F — Fix (승인 후 착수, 현재 코드 변경 0건)

- [ ] **F-pre2-1 (P1) EOD 경로에서 `label_scheme` 보존** — ⚠ **먼저 착수할 것**
      `learning/batch_retrainer.py:retrain_now()`에 `label_scheme`/`label_params` 인자 추가
      (기본 `None` → 기존 호출부 무영향), `:507-509` 초기화 분기를 "명시하면 신뢰, 아니면
      종전대로 지움"으로 교체. `retrain_eod.py:320` 호출부에서 `_load_from_db()` 직후 값 전달.
      ⚠ `retrain_intraday.py`는 **건드리지 않는다**(X/y를 안 넘겨 이 분기를 안 탄다).
      ⚠ `py310_64` 전용(191차) — `py37_32`로 돌리지 말 것.
      검증: `pytest tests/test_468_label_state_unlock.py` 6개 그대로 통과 + 신규 2건 →
      08-17 EOD 후 사이드카 `['fixed']*6` → **08-18(화) 08:55 `불일치 0/6`**.
      ⚠ **①안(호출부 저장·복원)은 비권고** — 가드 우회라 다음 호출자가 생기면 재발.
- [ ] **F-pre2-2 (P1) 반증된 예고 3곳 정정** — F-pre2-1과 **같은 커밋**
      `config/settings.py:479-480` · `.claude/skills/mireuk-daily-check/references/invariants.md`
      §2-B · 본 파일 I-3. ⚠ `invariants.md` §2-B **관측란은 08-18 라이브 확인 후에** 채운다
      (469차 O-1 선반영 사고 재발 금지).
- [ ] **F-pre2-3 (P2) 수집기 §12에 `since`(배포일) 필드** —
      `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py:scan_stuck_indicators()`.
      관측 가능일 0이면 `무기록`이 아니라 `신규(미관측)`, §11 적신호에서 제외.
      초기 등록: `ConfFloor`·`CORE준비도` → `since: 2026-08-15`.
      ⚠ 수집기는 표준 라이브러리만·**py3.7 호환**(f-string·walrus 금지).
- [ ] **F-pre2-4 (P2) C1' `edge_only_indicators`에 명시 페어 등록부** —
      `config/dailycheck_targets.json`. 이름 규칙은 후보 발굴용으로만 쓰고 판정은 등록부만.
      첫 등록: `[WarmupRetrain] … 예약` ↔ `[PreRetrain] … 스킵`. **C1' 구현 전 설계 입력이다.**
- [~] **F-pre2-5 (P2) 점검 잔여물 삭제** — **절반 처리됨.** 사용자가 `.gitignore:120`에
      `collect_stderr.txt`를 staged로 추가(세션 중 확인). 남은 것은 **파일 실물 삭제뿐**
      (리포 루트 368B, 샌드박스 권한 거부). 향후 수집기 stderr는 `%TEMP%`로 리다이렉트.

#### G — 고도화

- [ ] **G-pre2-1 (즉시) EOD 사이드카 자기점검** — `retrain_eod.py` 종료 직전 6개 사이드카를
      다시 읽어 `label_scheme`/`train_end_ts`/`acc` 결손 수를 `WARNING`으로.
      판독은 `learning/model_meta.py`를 **import**(복제 금지 — 런타임과 갈라진다).
      → 오늘 같은 조용한 결손이 **당일 저녁에** 드러난다. F-pre2-1의 검증 도구도 된다.
- [ ] **G-pre2-2 (이번 주) 수집기 국면·시각 정합성 경고** — `--phase`와 KST 실행 시각 대조,
      어긋나면 다이제스트 첫 줄에 경고. ⚠ **차단하지 않는다**(의도적 소급 점검이 유효할 수 있다).
- [ ] **G-pre2-3 (이번 주) `scripts/check_label_state.py`** — 사이드카로 G-1 판정을 오프라인
      재현해 판정 주기를 1거래일 → 즉시로. `model_meta.py` import.
      ⚠ **사이드카 상태만 검증한다** — I-3의 "F-1 마커 없이도 해제되는가"는 여전히 라이브 관측 필요.
      선행: F-pre2-1.

#### O — 다음 거래일(2026-08-17) 관측

- [ ] **O-p1 원인 진단 검증** — `[EntryGate] 불일치 N/6`. F-pre2-1 미적용이면 **6/6 재현**이
      정상이다(진단이 옳다는 증거). 적용 후 첫 EOD를 거쳤으면 `0/6`. 08-17 08:55
- [ ] **O-p2** EOD 사이드카 `label_scheme` 6개 → `['fixed']*6`. 08-17 15:47 이후
- [ ] **O-p3 I-3 재정의** — `불일치 0/6` **＋** F-1 마커 없이도 해제. **둘 다** 충족해야 한다.
      ⚠ 한쪽만으로 "G-1 검증 완료"라고 쓰지 말 것. 최단 08-18(화)
- [ ] **O-p4** `[CORE준비도] 축퇴 N/M` 첫 관측(470차 B4) — **찍히는지만** 확인. 값 판정은 20거래일 후
- [ ] **O-p5** `[ConfFloorGuard] state=` 첫 관측(470차 L2) — §12 `무기록` 해소 여부
- [ ] **O-p6** `[전략 상태 경보]` 배너 MDD 분모가 자본 기준인지(470차 R1 후 첫 배너).
      ⚠ **1일로 고착 해소 판정 금지**
- [ ] **O-p7** `[유령 프로세스]` EOD 체인 점검(470차 A3 `47e5301`) 첫 실행 — 08-17 16:0x

- [ ] **F-pre2-6 (P2) 수집기 §2에 측정 시각 명기** — 470차 S2가 측정 **방식**(환경 중립)은
      고쳤으나 **시점**은 표기하지 않는다. 점검 세션과 사람 작업이 겹치면 `git status`
      스냅샷이 리포트 작성 도중 낡는다(본 세션 실제 발생 — 19:35 6건 → 19:5x 5건, 구성 변경).
      G-pre2-2와 같은 계열.

#### 확인 필요 (판정 보류)

- [ ] **예약작업이 왜 19:35에 `--phase pre`로 돌았는가** — repo 로그로 판정 불가.
      데스크톱 앱 Cowork 예약 목록에서 등록 시각(cron)과 `--phase` 인자를 직접 확인할 것.
      지연 실행인지 슬롯 오등록인지가 갈린다. (470차 후속4 `a71da04`와 같은 축)
- [ ] **08-17 장전 `above_vwap` CORE 축퇴 재발 여부** — O-p4로 관측.
      ⚠ **1일 표본으로 결론 금지**(313차)

#### 갱신 처리 (완료 아님)

- [~] **I-3 (G-1 최초 검증)** — 판정일 **08-17 → F-pre2-1 배포 후 첫 EOD의 다음 거래일
      (최단 08-18)** 로 **조건 변경**. 완료 처리가 아니다.

---

### 470차 — 0814 장전 점검 (MW0602, 2026-08-14) — **라이브 코드 무변경 · P0 0건**

근거: `dev_memory/DECISION_LOG.md` 2026-08-14(470차),
`docs/정기점검/매일점검/MW0602-20260814-점검리포트.md`.
**장중이라 코드 변경은 보류 — F-1·F-2·F-5(문서·수집기)만 즉시 가능, F-3·F-4는 15:10 이후.**

> 🔴 **커밋은 반드시 Windows(MW0602)에서.** 샌드박스 git에는 `core.autocrlf`가 없어
> CRLF 바이트가 그대로 들어간다 — `DECISION_LOG.md` 하나만 커밋해도 **+22,796/−22,641줄**
> 가짜 diff가 생긴다(실측). **470차 세션은 파일만 써 두고 커밋하지 않았다.**
> 커밋 전 확인: `git -c core.autocrlf=true status --short` → **7건 근방**이어야 정상.

**즉시 (문서)**

- [ ] **F-1 `invariants.md` §2-B의 O-1 선반영 서술 철회** — `MODEL_LABEL_STATE_UNLOCK_ENABLED`
      행의 `2026-08-14 O-1에서 0건 확인`은 **오늘 07:13 커밋(469차) 시점 서술**로,
      장 시작 2시간 전이라 **관측이 불가능했다.** 실제 오늘 실측은
      `F-1 경로로 해제 / G-1은 사이드카 구버전이라 미해제(정상) / 사이저 0회라 O-1 미판정`.
      함께 §2-B에 규약 신설: **"관측란에는 관측된 것만 쓴다. 예정은 NEXT_TODO에 쓴다."**
      ⚠ 이 줄을 믿고 O-1을 종결 처리하면 검증 없이 CLAUDE.md ⑧ 선행조건이 충족된 것으로 오독된다.
- [ ] **F-5 CLAUDE.md 멀티PC 컨벤션에 체리픽 기록 규약 추가** — *"타 PC 브랜치 커밋을
      cherry-pick 하면 원 커밋 sha·원 PC·가져온 이유를 `DECISION_LOG.md`에 남긴다.
      코드 변경이 없는 문서 체리픽도 예외가 아니다."* (현 HEAD `a86b238`이 무기록 사례)

**장후 (코드)**

- [ ] **F-4 `전략판정` MDD 분모를 자본으로** — `config/strategy_registry.py:601`
      `mdd_krw / abs(peak)` → `mdd_krw / SIZING_TARGET_CAPITAL_KRW`.
      자본 미설정 시 `None` 반환 → `_compute_verdict`가 `INSUFFICIENT`
      (**모르는 것을 UNDERPERFORM으로 단정하지 않는다**).
      추가로 `:788-796`에서 WFA 기준선이 2026-05-07 seed일 때 `INSUFFICIENT` 반환.
      ⚠ **임계값(`_NORMAL_SHARPE_DELTA` 등)은 건드리지 않는다** — 단위 버그 수정이지 기준 완화가
      아니다(313차 원칙 ④ 사전등록).
      회귀 확인: `dashboard/strategy_dashboard_tab.py:71,77`에 `INSUFFICIENT` 매핑 존재 여부.
      **진입·사이징 경로 무영향은 코드로 확인함**(경보·표시 계층 전용).
      검증: `tests/test_470_verdict_mdd_unit.py` — ① 08-12 `mdd_pct ≤ 1.0`
      ② 08-13 `NORMAL` 또는 `INSUFFICIENT` ③ seed 기준선 시 `INSUFFICIENT`.
- [ ] **F-3 1단계 — 종료 시 잔존 스레드 덤프(진단 전용)** — `main.py:_auto_shutdown()` 말미에
      `[Shutdown] 잔존 스레드: {[(t.name, t.daemon) for t in threading.enumerate() if t.is_alive()]}`.
      **원인 없이 종료 로직을 건드리지 않는다.** 2단계(`stop()`/`join()` 또는 `os._exit(0)`)는
      덤프 결과 확인 후. COM 콜백 경로가 아니므로 절대원칙 ④ 무관.
      판정 기준: 다음 거래일 08:40 런처에 `[WARN] 이미 실행 중인 main.py` **미출력**.
- [ ] **F-2 수집기 미커밋 집계를 환경 중립으로** — `collect_evidence.py` 미커밋 집계를
      `git -c core.autocrlf=true status --short`로 교체 + §2에 `측정: core.autocrlf=true 강제
      (host=…)` 한 줄. 검증: 오늘 워킹트리에서 `520` → `7`, Windows에서도 `7`.
      ⛔ **`.gitattributes`·`git add --renormalize`는 하지 않는다** — 리포지터리는 정상이다.
      초안이 518파일 정규화를 권고했으나 **철회**했다(아래 참조).
      → 라이브 코드 아님, 즉시 적용 가능.
- [ ] **0813 리포트 정정 주석** — `MW0602-20260813-점검리포트.md:4`의
      "미커밋 517건(대부분 CRLF/개행 차이로 추정)"은 현상은 맞으나 **원인을 리포지터리 쪽에
      두었다.** 원인은 측정 환경(샌드박스 git에 `core.autocrlf` 없음)이다. 한 줄 정정 추가.

**관측 숙제 (검증 기한 명시)**

- [ ] **O-1 종결 — 오늘 장후** — 판정 기준(불변): 전일 EOD 성공일에
      `[EntryGate] 사이즈 축소 ×0.6` **0건**. 09:01 현재 0건이나 **사이저 호출 0회라 분모가 0**이다.
      대조값: 0813 = ×0.6 18건 / `[Sizer]` 18회 = 100%, 해제 0건.
- [ ] **G-1 검증 — 2026-08-17(월) 장전이 최초 기회** — 오늘 EOD가 사이드카 6개에
      `label_scheme`을 새로 쓴 다음이라야 `[EntryGate] 모델 레이블 체계` 불일치 6/6이 풀린다.
      **오늘 장중 ×0.6이 0건이어도 그것은 F-1의 검증이지 G-1의 검증이 아니다.**
- [ ] **P1-2 `above_vwap` 개장 후 해소 확인 — 오늘 09:00~09:10** — `trigger=B_OPEN`
      리프레시에서 `CORE 'above_vwap' raw_std≈0`이 사라지는지. 0813은 사라졌다(B_OPEN 3회 중 0건).
      **재발하면 P1 → P0 승격 검토**(절대원칙 ③ CORE, 미통과 시 강제 X).
- [ ] **2-4 `session_state.json` 키 소실 구간 좁히기 — 오늘 EOD** — 15:47 EOD 직후와 15:50에
      각각 파일을 떠서 `p8_last_success_date`·`eod_retrain_ok_date`가 언제 사라지는지 확인.
      유령 프로세스(F-3) 가설과 함께 검증.
- [ ] **08:58 레짐 확정 로그 확인 — 장중** — 08:55는 `seed fetch`까지이고 레짐 확정은 08:58 2단계로
      연기된다(`레짐=NEUTRAL` 유지). CLAUDE.md 9단계는 "08:55 매크로 수집 → 시장 레짐"이라
      **문서-코드 미세 드리프트**. 확인 후 문구 정정 여부 판단.

**고도화 (리포트 §3)**

- [ ] **G-1 경보에 "며칠째"를 붙인다** — `[전략 상태 경보] 판정: UNDERPERFORM (10거래일 연속 ·
      직전 전환 2026-07-31 OUTPERFORM→)`. `strategy_events` 역순 스캔이면 되고 새 저장소 불필요.
      `CB_state`·`CORE안전`·`degraded`에도 적용. **선행: F-4** (안 그러면 고장을 예쁘게 표시할 뿐).
      우선순위 이번 주.
- [ ] **G-2 장전 CORE 준비도 섀도 계측** — 09:00 직전 마지막 워밍업 후
      `[PreMarket] CORE 준비도: 축퇴 1/3 (above_vwap ×6hz) | 정상 cvd_divergence, opt_chain_pcr`.
      ⚠ **차단은 붙이지 않는다**(317차 HurstGate 오판차단 72.3% 전례 — invariants §4-2 ④).
      매일 무조건 찍히는 상태 샘플이라 수집기 §12 `stuck_indicators` 등록 가능(468차 G-2 규약 충족).
      20거래일 축적 후 "장전 CORE 축퇴일의 09:00~09:30 정확도"를 **일자단위**로 비교해 판정.
- [ ] **G-3 장전 예약작업 08:50 → 09:02** — 오늘 08:50 실행 탓에 08:55 매크로·PreRetrain·
      EntryGate 해제가 다이제스트에 하나도 안 잡혔다(phases.md A-2는 장전 핵심 항목).
      09:02면 08:58 레짐 확정 + 09:00 첫 파이프라인 1사이클까지 포함돼 오히려 강해진다.
      ⚠ **예약작업은 git 공유 대상이 아니다 — MW0601에도 따로 등록.** 우선순위 즉시.
- [ ] **G-4 종료 완결성을 당일 저녁에 드러내기** — `EOD_RETRAIN.bat` 체인 말미에
      프로세스 생존 확인(`tasklist /FI "IMAGENAME eq python.exe"`)을 EOD 로그에 기록.
      살아 있으면 경고 → 다음날 아침이 아니라 당일 저녁에 드러난다.
      ⚠ `EOD_RETRAIN.bat`은 **`py310_64` 전용** — 환경 언급을 건드리지 말 것.
- [ ] **로드맵 반영 3건** — ① 실전전환기준 ③ 옆에 "현 `WFA sharpe=1.42/mdd=0.142`는 2026-05-07
      QA seed이며 실측이 아니다" 명기(F-4와 세트) ② 기준 ⑧ 선행조건을 "G-1 확인"에서
      "G-1 확인(**F-1 해제와 구분**)"으로 정밀화 ③ 26주 WFA 재검증 항목에 **"판정식의 입력 단위 점검"**
      추가 — 오늘 발견은 감시목록이 낡은 문제가 아니라 **지표 자체의 단위가 틀린** 새 유형이라
      고착 감시(468차 G-2)로는 원인까지 못 간다.

**문서 보수**

- [ ] **469차 DECISION_LOG 항목이 헤더만 있고 본문이 없다** — `dev_memory/DECISION_LOG.md`의
      469차 헤더 바로 다음 줄이 456차 헤더다. 470차 세션은 **작성자 의도를 모르므로 임의로 채우지
      않았다.** 커밋 `4abf7c4` 메시지를 근거로 본문(증상→원인→결정→Why→How→검증) 보완 필요.
- [ ] **`CURRENT_STATE.md:403-405` 정정** — *"UNDERPERFORM 판정 자체는 버그가 아니라 처음 드러난
      진짜 신호"* 는 470차에 반증됐다. 인용된 `Live MDD=58.6%`도 같은 분모 결함의 산물 —
      **재인용 금지** 표기 추가(`invariants.md` §3에도 등재 검토).

---
### 474차 — D9 딥다이브 후속 (MW0601, 2026-08-17) — **매매 정책 변경 0건**

근거: `dev_memory/DECISION_LOG.md` 2026-08-17(474차). 전체 447 passed / 0 failed.

**주간회의 상정 (사용자·회의 결정)**

- [ ] **G1 — §3 CORE 그룹 처분.** mid(10m·15m)·long(30m)이 **도달 불가**인 것은
      태생이다(124차 라우터 → 179차 그룹, 8일 차). 선택지:
      **A** mid/long 행 폐지 → §3을 단기 단일로 축소
      **B** "예약(도달 불가)"로 명기 유지 ← **현재 상태**(474차가 명기만 함)
      **C** 라우터 상위 경계 신설 — ⚠ **매매 정책 변경**이고 전제가 약하다
        (tf 밴드별 적중률 **비단조**: 6.0~8.0에서 5m 46.8%≈15m 47.6%,
         **≥8.0에선 1m 42.2%로 단기 우위**). TP1 배수·메타게이트 스코어러·라벨 설계가
         **전부 없다** — `ATR_HORIZON_TP1_MULT`에 10m 이상 항목이 존재한 적이 없다.
      ⚠ §3은 절대원칙이라 개정은 사용자 결정이다. 처분이 바뀌면
        `tests/test_473_core_group_reachability.py`의 선언 상수도 함께 갱신할 것.

- [ ] **G2 — 30m 처분은 CB③ 기준 교체와 한 묶음이다.** 🔴 30m 학습을 중단하면
      **CB③이 유일한 입력원을 잃는다**(`main.py:5539` `if v["horizon"]=="30m"` →
      `record_accuracy()`, `main.py:6203` → Contrarian). 가중 0.0이라 "낭비"로 보이지만
      아니다. 전환기준 ⑥이 *"30m 재도입 **또는** CB③ 기준 호라이즌 교체"* 인 이유다.
      → 30m EOD 학습 중단을 단독 안건으로 올리지 말 것.

**표본 누적 대기 (D9-B 채널)**

- [ ] **G3 — `entry_band_watch` 재판정.** 현재 **BAND_UNIFORM**이나 감시밴드 n=21로
      문턱(20)을 겨우 넘겼다(**저검정력 주석 동반**). 표본이 2배(40) 근방이 되면
      재판정할 것. ⚠ `BAND_UNIFORM`은 "이상 없음이 입증됐다"가 아니라 **"사전등록
      기준에서 안 잡혔다"**이다.
      ⚠ **`band_edges`·`focus_band`·문턱을 데이터를 본 뒤 바꾸지 말 것** — 이 채널은
        애초에 사후적합 위험이 높은 관측에서 출발했다(4.4-6.0 승률 38.1%가 계기).
        실제로 일자단위에서 소멸했다(p=1.0000, 우세 8일/열위 9일).

- [ ] **G4 — 캠페인 리포트에 [52]·[53] 배선 여부 판단.**
      `spread_extreme_watch`(473차)·`entry_band_watch`(474차) 둘 다 아직 스탠드얼론
      스크립트다. `_safe_channel` 등록 시 **`_fmt_verdict`에 신규 판정어 4종
      (`BLOCK_JUSTIFIED`/`BLOCK_UNJUSTIFIED`/`BAND_ANOMALY`/`BAND_UNIFORM`)을 함께
      등록**할 것 — 미등록 verdict는 **조용히 INSUFFICIENT로 표시된다**(471차 G-2 경고).

**하지 말 것 (근거와 함께 기각 — 재시도 금지)**

- ❌ **30m 예측을 진입 역필터로 쓰기** — `VALIDATION_CAMPAIGN_DECISIONS
      ["reverse_30m_entry_filter"]`에 기각 등록. 6×3 칸 중 최극단 1칸의 사후 선택이고
      다중비교 보정·일자단위·drop-worst가 없다. 방향 반전은 457·458차에 이미 두 번 기각.
      그 수치(53건 45.3% vs 50건 68.0%)를 **근거로 인용하지 말 것.**
- ❌ **4.4-6.0 밴드 승률 38.1%를 근거로 B2 경계 조정** — 일자단위 p=1.0000으로 소멸했다.

### 473차 — 구조적 교착 해소 후속 (MW0601, 2026-08-17) — **매매 정책 변경 0건**

근거: `dev_memory/DECISION_LOG.md` 2026-08-17(473차). 전체 434 passed / 0 failed.

**다음 거래일 (F-8 Phase B 배선 게이트) — 이게 제일 중요하다**

- [ ] **F1** — `ensemble_decisions.spread_ticks` 적재 확인.
      `SELECT COUNT(*) FROM ensemble_decisions WHERE spread_ticks IS NOT NULL`
      ⚠ **471차 `sizing_trace`가 배포 후 지금까지 0행**이다. 같은 패턴으로 배선했으므로
      같은 방식으로 죽을 수 있다. **배선 테스트 통과 ≠ 라이브 적재**다.
- [ ] **F2** — `python scripts/spread_extreme_watch.py` 의 "F-8 Phase B 컬럼 대조" 절이
      `값불일치 0 · 플래그불일치 0 → 일치`인지. 불일치면 배선이 틀린 것이다.
      (원본 `features` JSON이 살아 있어 대조가 가능하다 — sizing_trace가 못 했던 것)
- [ ] **F3** — `sizing_trace`도 함께 확인. 여전히 0행이면 471차 배선이 죽은 것이므로
      별건으로 조사 회부.

**주간회의 상정 (미결 — 전부 사용자/회의 결정)**

- [ ] **F4 — ⑨(TOX-SEVERE-SPREAD)의 거처.** 손익 축 판정은 ETA **7.1개월**
      (진입 6포지션/4거래일)이라 실전 전환 선행조건으로 실효가 없다. 선택지:
      ① 실전 전환 기준에서 빼고 26주 WFA 재검증 항목으로 이관
      ② 손익 대신 **집행 비용 축**(슬리피지 실측)으로 판정 기준을 재설계
      ③ 그대로 두고 7개월 대기
      ⚠ **`min_samples`를 낮춰 판정하는 선택지는 없다**(458차 D6과 동일 위반).
- [ ] **F5 — D9 CORE 그룹 처분.** 30m만이 아니라 **중기(10m·15m)도 도달 불가**다.
      A(30m만 폐지+학습 중단) / B(중기도 함께, §3을 단기 단일로 축소) /
      C(라우터에 B3·B4 추가 — **매매 정책 변경**, 별도 검증 필요).
      ⚠ §3은 절대원칙이므로 개정은 사용자 결정이다.
      해소되면 `tests/test_473_core_group_reachability.py`의 선언 상수도 함께 갱신할 것.
- [ ] **F6 — D8: 현재 결론은 「대기」다. 열려면 거버넌스 승인이 필요하다.**
      2026-08-17 조사로 세 안이 정리됐다 — 상세는 `DECISION_LOG.md` 473차 [5-후속].
      · **학습창 단축 = 실행 불가**. opt_* 96%를 얻으려면 8,716봉으로
        `MIN_TRAIN_BARS`(15,000)의 58%. 게다가 97개 전부·6 호라이즌 전역 변경이고
        전환기준 ③의 26주 근거를 무효화한다.
      · **대기 해소일 = 2026-12-25**(롤링 26주 창에서 opt_* 90% 도달).
      · **2종 즉시 = 알파 근거 없음**. 기록 rho와 **부호가 반대**다
        (`cvd_monotone_ratio` +0.031 → **−0.0115**; `opt_pcr_extreme_bearish` 15m은
        기록 자체가 None). `threshold_feasibility`·`basis_pt` 선례와 같은 패턴 —
        **레지스트리 rho 전반이 낡았다.**
      ⚠ **"인프라 리허설" 논거는 철회됐다** — 292차가 MW0602에서 97→105를 **이미**
        수행했고(기계적 성공), 후속으로 ScalerWarmup 0-패딩 → long 정확도 14~20%
        급락 등 사고가 이어졌다. 미검증이 아니라 **기지의 위험**이다.
      ⚠ `--apply`는 331차 관례(`DYNAMIC_FEATURES_POOL` 경유, `active_features` 직접
        편집 금지 — CLAUDE.md §6) 위반이라 `--governance-approved` 없이는 거부된다.

- [ ] **F6-a — 🔴 피처 슈퍼셋은 git으로 전파되지 않는다. D8의 어떤 선택지보다 먼저다.**
      MW0601은 **한 번도 105가 된 적이 없다**. 원인이 실수가 아니라 **구조**임을
      2026-08-17에 확정했다:
      · `data/db/shap_feature_registry.json`은 `.gitignore:20 data/` 대상이라
        **git으로 전파될 수 없다**(런타임 산출물 커밋 금지 규약의 정상 귀결 —
        `feedback_git_commit_scope`).
      · 브랜치 분기점은 **291차 `5e1426c`**, 그 **직후 292차**가 97→105를
        `dev`(MW0602)에서 수행했다. 즉 분기와 확장이 맞물렸다.
      ⇒ 두 PC가 **구조적으로 다른 X 공간**에서 학습·추론하고 있을 수 있고,
        D8 요건 "양 PC 동시 적용"(D12·F9)은 **충족될 방법 자체가 없다** —
        고쳐야 할 동기화 실패가 아니라 **없는 절차**다.
      ⚠ 주간 대조 규약도 이 차이를 못 잡는다(채널 집합이 같아도 X가 다를 수 있다).
      **할 일**: ① MW0602의 `active_features` 길이 확인(한 줄)
      ② 길이가 다르면 그 사실을 캠페인 대조 리포트 상단에 명시할 방법을 정할 것
      ③ 슈퍼셋 전파 절차를 신설할지(커밋 예외 vs 수동 배포 체크리스트) 결정.

- [ ] **F6-b — 레지스트리 rho 전수 재측정을 26주 WFA 배터리 안건으로 등록.**
      오늘 잰 2종이 전부 부호 반전이었고 이는 F4·basis_pt 선례와 같은 패턴이다.
      `horizon_feature_sets.json`의 rho 값을 근거로 인용하지 말 것.

**D8 실행 (F6 승인 시에만 — 비거래일에 한 묶음으로)**

- [ ] **F7** — `--snapshot`(완료: `data/superset_snapshots/20260817_160444`, 33파일)
      → `--plan --coverage-min <결정값>` → `--apply --governance-approved`
      → **즉시 이어서** `<py310_64> python scripts/eod_retrain.py --weeks 26 --force`
      → `python scripts/feature_superset_expand.py`로 3-튜플 정합 재확인.
      🔴 **중간에 멈추면 registry/pkl(N) vs scaler(97)로 갈려 `validate_and_resync()`가
      6/6 호라이즌을 끈다 = 예측 전면 중단.** 롤백은 `--rollback <스냅샷경로>`.
      💡 **전환 거동만 보고 싶으면 프로덕션을 쓰지 말 것** —
      `BatchRetrainer(model_dir=, scaler_dir=)`가 둘 다 파라미터라(346차 선례)
      스크래치 디렉터리 재학습으로 위험 0으로 관찰할 수 있다.
- [ ] **F8** — 재학습 후 스모크: `validate_and_resync()` 빈 리스트 / ShapTracker
      shape(337차) / `_restore_analysis_buffers`(332차) / `_shap_names_ok`(414차) /
      Canary / SGD 리셋 1회 후 정상 / `[FeatureReg]` 미가용 건수 감소.
      ⚠ 292차 사고 목록을 체크리스트로 쓸 것(ScalerWarmup 0-패딩·Phase2 pkl 덮어쓰기).
- [ ] **F9** — **양 PC 동시 적용**(407차 규약). pkl은 protocol=4라 복사 가능하나
      **스케일러는 PC별 P8 재실행**이 필요하다. F6-a 결과에 따라 MW0602 쪽 정합
      복구가 선행될 수 있다.

**만성도 계측 (1번 조치 후속)**

- [ ] **F10** — `data/feature_exclusion_state.json`이 다음 EOD에 재생성되고
      **`최초관측`으로 시작**하는지. 오염분은 백업 후 삭제했다(진짜 이력은 이미 소실).
      둘째 날부터 `2거래일째`로 올라가야 458차 P2-C가 되살아난 것이다.

### 472차 — Phase 5 게이트 배지 교체 후속 (MW0601, 2026-08-17) — **매매 정책 변경 0건**

근거: `dev_memory/DECISION_LOG.md` 2026-08-17(472차).
판정기 `strategy/ops/phase5_gate_status.py` / 레지스트리 `config/settings.py:PHASE5_GATE_DECISIONS`.

**다음 실 UI 기동 (라이브 미검증분)**

- [ ] **E1** — 좌상단 배지가 `Phase 5 게이트 0/9`로 뜨는지. 헤드리스로는 확인했으나
      실제 해상도(`S.f(9)`)에서 **제목 옆 폭이 충분한지**는 못 봤다. 깜빡임 시 텍스트가
      `● Phase 5 게이트 0/9 · CB2복원 D-7`로 길어지므로 그때 잘리지 않는지 함께 볼 것.
- [ ] **E2** — 툴팁 54줄이 화면 세로에 들어가는지. 넘치면 `_MAX_NOTE_LINES`(현재 2)를
      1로 낮춘다. ⚠ 줄일 때도 `…`와 전문 위치 안내는 남길 것(계측 4원칙 ③).

**2026-08-22 (깜빡임 자동 개시 예정일)**

- [ ] **E3** — 배지가 실제로 깜빡이기 시작하는지 확인. ⑤ CB②의 `due=2026-08-29`가
      D-7에 진입하는 날이다. **안 깜빡이면** 1분 갱신(`_refresh_cycle_badge` → 
      `_refresh_phase5_badge`)이 끊긴 것이므로 그쪽을 볼 것.
      ⚠ 이 날짜는 **CB② 재검토 그 자체와 다른 항목이다** — 재검토는 08-29다.

**게이트 상태가 바뀌면 반드시 함께 할 것**

- [ ] **E4** — ①②③④를 실측으로 판정했으면 `PHASE5_GATE_DECISIONS[n]["status"]`에
      기록할 것. 기록하지 않으면 배지는 영원히 `미측정`이다(그게 설계다 — 조용히
      충족으로 넘어가지 않는다). 특히 **F-1R 리허설을 돌리면 ②를 갱신**할 것.
- [ ] **E5** — ⑥⑦⑨에서 "복원" 대신 **"재설계"** 경로를 택하면 플래그는 False로 남으므로
      배지가 계속 `미충족`으로 뜬다. 그 경우 레지스트리에 `status="met"`과 근거를 적을 것.

**선행 결함 (이번 변경과 무관 — 회부)**

- [ ] **E6 — `tests/test_457_guard_meta_and_scope.py`가 pytest에서만 실패한다.**
      `_bind()`가 `__main__` 블록에서만 호출돼 pytest 수집 시 `_Stub`에 메서드가
      안 붙는다(`AttributeError`). 직접 실행(`python tests/...py`)은 통과.
      → 전체 스위트에서 **계속 빨간불이면서 457차 P4 가드 유효성 판정을 아무것도
      지키지 않는 상태**다. 픽스처(`autouse`)나 모듈 임포트 시점 바인딩으로 옮길 것.
      ⚠ 고치면 **그동안 가려져 있던 진짜 실패가 드러날 수 있다** — 붙인 뒤 내용도 볼 것.

### 469차 — 일일 점검 스킬 갱신 (MW0602, 2026-08-14) — **라이브 코드 무변경**

근거: `dev_memory/DECISION_LOG.md` 2026-08-14(469차). `RUN_ON_MW0602.md` STEP 1·2 완료분.

- [ ] **다음 장후 점검에서 제4·5부(승패 사후검증)를 실제로 한 번 돌려본다** —
      `/dailycheck post`. 확인할 것: ① 3원 대사가 포지션 단위로 맞는가
      ② `하드스톱` 건을 `tp1_reached`로 승패 가르기가 실제로 되는가
      ③ `report_template.md` §4·§5 양식이 실전에서 채워지는가.
      막히는 칸이 있으면 그 칸을 지우지 말고 **왜 못 채웠는지**를 리포트에 적을 것.
- [ ] **수집기 DB 조회 절 신설 여부 판단** — 현재 수집기는 로그 전용이라 진입 건수·승률·
      손절 준수율이 자동 집계되지 않는다(제4부에서 매번 수동 조회). 417차 결론대로 진짜
      신호가 손절 준수율에 있다면 §13으로 넣을 값어치가 있다. **위 실행 1회 후에 판단** —
      먼저 돌려보지 않고 만들면 쓰지도 않을 절이 늘어난다.
- [ ] **MW0601 pull 후 `--discover` 1회** — 스킬은 PC를 안 가리지만 로그 적재량·보관
      정책이 다를 수 있다. `config/dailycheck_targets.json`의 `scan_dirs`가 그 PC에서도
      맞는지 확인(SKILL.md §1-B).
- [ ] **`MIREUK_DAILYCHECK_HANDOFF.md`(리포 루트, untracked) 처리 결정** — 설치 인계용
      임시 문서이고 내용은 스킬 본문에 흡수됐다. 커밋할지 지울지 결정할 것.
...
- [x] **F-2 (P1) 보호트레일 분리 — A안(표시 계층)으로 배포** — **[DONE 2026-08-14 · 커밋 ④]**
      사용자 판단: **A안**(DB `exit_reason` 무변경 → 465차 결정 보존).
      `strategy/position/position_tracker.py` 최종 청산 **로그 줄**에만
      `[TP1보호]`/`[TP1미도달]` 태그(손절 계열에만, 출처는 465차 `tp1_reached`와 동일한
      `partial_1_done`). 수집기에 `exit_stop_kind()`/`exit_stop_counts()` 신설 —
      §5는 "진짜 손절·보호트레일·태그없음" 3분해, §11 적신호는 보호트레일을 제외하고
      태그없음은 "468차 이전 로그"로 명시. **사전등록 채널·테스트 전부 무영향.**
      ⚠ 대시보드는 이미 `pnl_pts` 부호로 WIN/LOSS를 내므로 손대지 않았다.
      아래는 보류 시점 기록(보존):
...
- [x] **G-1 사이즈 배수 해제를 이벤트→상태로 재작성** — **[DONE 2026-08-14 · 커밋 ⑧]**
      `learning/model_meta.py:read_label_state()` 신설(순수 함수 — COM/PyQt 없이 테스트 가능),
      `main.py:_model_label_scheme_current()`는 설정을 읽어 넘기는 어댑터.
      사이드카에 `label_scheme`·`label_param`을 **학습 시점 실제 사용값**으로 기록
      (batch_retrainer **두 경로 모두** — Phase 2 EOD 주경로를 빼먹으면 무력화된다).
      호출: 장전(08:55) + **장중 첫 확인**(하루 1회 — 08:55 이후 재시작 사각지대를 덮는다).
      킬스위치 `MODEL_LABEL_STATE_UNLOCK_ENABLED`.
      ⚠ `_pre_retrain_done`은 **폐기하지 않았다** — 소비처(사이징·일간리셋·재학습 콜백)가
      여럿이라 플래그는 두고 **설정 방법만** 상태 기반으로 바꿨다. F-1(마커)도 독립 증거로
      유지(사이드카에 새 필드가 없는 구버전 구간을 덮는다).
      ⚠ **배포 직후~첫 EOD 전까지는 G-1이 해제하지 못한다**(사이드카 구버전) — 정상이며
      그 구간은 F-1이 덮는다. 다음 EOD가 6개를 새로 쓰면 작동.
      회귀 테스트 `tests/test_468_label_state_unlock.py`(21단정).
      판정 기준: 전일 EOD 성공일에 `사이즈 축소 ×0.6` 로그 **0건**(O-1)
- [x] **G-2 수집기 §12 "고착 지표" 신설** — **[DONE 2026-08-14 · 커밋 ⑤]**
      `scan_stuck_indicators()` — 최근 10거래일 상태값 분포를 세어 `고착`(한 값 100%)·
      `무기록`(표본 0 = 계측 중단/문구 변경)·`정상고착`(benign 등록분)·`표본부족`·`변동`으로 판정.
      ⚠ **조건부 로그는 표본으로 쓰지 않는다** — 구현 중 `[RegimeFingerprint] PSI=…`
      (PSI>0.20에서만 출력)를 넣었다가 "PSI 100% CRITICAL 고착"이라는 **가짜 적신호**를
      만들어 봤다. 매 주기 무조건 찍히는 상태 샘플만 등록할 것(SKILL.md에 규약 기재).
      **첫 실행 실적 2건**: `CORE안전` ⚠️ 100%(101건/10일 — F-3 이전 상태를 소급 포착),
      `전략판정` UNDERPERFORM 100%(9건/9일 — 신규 발견)
      → **CLAUDE.md 26주 WFA 재검증 항목에 "고착 지표 점검" 추가 검토**(G-2 의존)
- [ ] **[G-2 첫 수확] `전략판정` 9거래일 연속 UNDERPERFORM** — 배너 판정이 9/9일 한 값
      고착. 일일 리포트가 매일 경보로 올리지만 **"며칠째인지"는 아무도 세지 않았다.**
      판정식이 무의미해진 것인지(고착) 진짜 저성과인지 구분 필요 — 주간회의 상정
- [x] **G-3 청산 라벨 2축 스키마** — **[DONE 2026-08-14 · 커밋 ⑦]**
      `trades.exit_trigger`(하드스톱/보호트레일/하드스톱?/손절1차/TP1~3/시간마감/안전망/
      수동/복구/기타) + `exit_outcome`(이익/손실/본전). `exit_reason` **무변경**(465차 P4 보존).
      `utils/db_utils.py:classify_exit()`이 **유일한 생성자** — 기록 경로(main.py INSERT +
      `_build_exit_result`)와 분석이 같은 함수를 쓴다.
      ⚠ **트리거는 `tp1_reached`로만 가른다** — `pnl_pts>0` 추정을 쓰면 트리거 축이 결과 축과
      상관돼 2축으로 나눈 의미가 사라진다. `tp1_reached=None`(구버전)은 `하드스톱?`(모른다).
      멱등 ALTER 2중화: db_utils `additions` + 리포트 `_ensure_trades_exit_axis_columns()`
      (420차 joint_gate_shadow 사고 형태 방지). **소급 백필 없음.**
      회귀 테스트 `tests/test_468_exit_axes.py` 신설(21단정).
- [ ] **[G-3 후속] 손절 준수율 채널 신설** — 2축이 생겼으니 이제 라벨만으로 계산 가능하다.
      `exit_trigger IN ('하드스톱','손절1차')` 행의 `실현손실 ÷ 의도손절폭(ATR×1.5)`을
      캠페인 채널로 등록 검토 — 417차 재분해에서 **유일하게 유의**했던 축
      (rho=+0.318 p=0.015, 1계약 29.4% vs 3계약+ 68.8%). ⚠ 표본은 2026-08-14 이후 행만
      (그 이전은 `exit_trigger` NULL) — min_samples 도달까지 수 주 걸린다
- [x] **G-4 파이프라인 지연 STEP 단위 분해** — **[DONE 2026-08-14 · 커밋 ⑥] 원인 확정.**
      `scripts/pipeperf_step_decomposition.py` 신설(읽기 전용).
      **결론: S6(앙상블진입판단) → 그 안의 `shap`(SHAP 심사).**
      3거래일(08-11·12·13) 전부 증가, rho +0.93~+0.98, 총 증가분의 96~118%가 S6이고
      S6 증가의 97~98%가 `shap`. **형태는 폭주가 아니라 계단형 + 포화**:
      11:00까지 0ms → 11:30에 140~170ms(라벨 `SHAP_MIN_DATA_POINTS=100` 충족 시점) →
      13:00 전후 2차 계단 → 180~260ms에서 평탄(`_shap_labeled_window` deque **maxlen=240** 상한).
      🔴 **기존 가설 반증**: "`pred_select` 5~12초 병목(S1) — predictions.db 풀스캔"은
      이 증상의 원인이 **아니다**(S1은 3일 평균 **+2ms**). predictions.db 851MB는 사실이나
      지연 상승과는 무관 — 별건으로 남긴다.
- [ ] **[G-4 후속] SHAP 심사 비용 처방 결정** — 원인은 확정됐고 **폭주가 아니므로 긴급하지 않다**
      (총 지연 최대 568ms vs CB⑤ 임계 5,000ms, 9배 여유). 선택지:
      ① 방치(상한 있음) ② `permutation_importance` 표본 축소(현행 `sample_n=min(120, …)`)
      ③ 라운드로빈 주기 확대(현행 매분 1호라이즌) ④ 장중 심사 중단·EOD 전용.
      ⚠ ②~④는 SHAP 심사 표본을 줄이므로 F-3로 막 되살린 `CORE안전` 지표와 교체후보 품질에
      영향 — **주간회의 안건**. 판단 근거는 `scripts/pipeperf_step_decomposition.py` 재실행
...
- [ ] **CB② 복원 재검토 2026-08-29** — ~~`CB_CONSEC_STOP_LIMIT` 9999 → 2~3~~
      → **[470차 E1 재기술] `C1(포지션 단위 집계) 배포 확인 후` 9999 → 2~3.**
      · **왜 재기술했나**: 0814에 실제 손실 **포지션 2건**에 카운터가 **3**까지 올라간 것이
        라이브에서 처음 실측됐다(360차 예견의 확정). 보정 전 숫자로 임계 2를 정하면
        **실질 임계가 1~1.5**가 된다.
      · **C1 은 배포됐다**(470차, `safety/circuit_breaker.py` `is_partial_leg` 인자).
        재검토 자리에서 **라이브 재현 확인부터** 할 것 — 부분청산이 있는 날
        `[CB] 손실 레그(부분청산) N회 … 카운터 미증가` 가 찍히는지.
      · **실측 근거**(470차 C2, `scripts/verify_cb2_kelly_leg_counting.py`, 전 기간
        포지션 192/레그 222): 최대 연속 손절 **레그 6 vs 포지션 5**,
        손실 레그 2회 이상 포지션 **8건**, 승률 레그 62.2% vs 포지션 63.0%.
      · Kelly 도 같은 4개 지점에서 레그를 세지만 승률 차가 **-0.9%p**로 작아
        서둘러 바꿀 근거가 없다 — 사이징 직결이라 주간회의 안건으로 분리.
...
- [ ] **08:49 장전 예약이 실제로 발화했는가** — 코웍 "Scheduled" 패널의 `lastRunAt`.
      앱이 꺼져 있었으면 다음 실행 시로 밀린다(정상 동작)
- [ ] **산출물 이름이 `evidence_MW0602-20260814_pre.md`인가** —
      `UNKNOWN`이 붙어 있으면 프롬프트에서 `--pc MW0602`가 누락된 것
- [ ] 12:40 장중 / 16:22 장후도 동일 확인
- [ ] **장중 실행이 코드 변경·재기동·git 커밋을 하지 않았는가** — 프롬프트에 금지를
      명시했으나 라이브 미검증. 어겼으면 프롬프트 문구 강화
- [ ] 첫 실행이 도구 승인 프롬프트에서 멈추지 않는가 — 멈추면 "지금 실행"으로
      한 번 수동 승인해 두면 이후 자동 실행에 적용된다
- [ ] 세 리포트가 같은 날짜본을 덮어쓰지 않고 국면별로 분리·append 되는가

**미조치로 남긴 것 (467차 범위 밖)**

- [ ] `.claude/scheduled_tasks.lock` — 2026-07-05 pid 12572 스테일 잠금.
      코웍 스케줄러와 무관해 이번엔 안 건드렸다. Claude Code 쪽 예약을 쓸 거면 삭제 필요
- [ ] **MW0601도 코웍 예약을 걸 거면** 그 프롬프트에 `--pc MW0601`이 필요하다
      (repo pull만으로는 예약이 공유되지 않는다 — 467차 DECISION_LOG 참조)
- [ ] `.claude/mireuk_dailycheck_v4.zip` — 코웍 설치용으로는 안 먹는다
      (`.skill` 확장자 + 루트에 SKILL.md 필요). 배포용으로 쓸 거면 재패키징
- [ ] 코웍 스킬 사본은 repo와 **별도로 저장**된다. SKILL.md를 고치면 코웍 사본도
      함께 갱신해야 두 경로가 갈라지지 않는다
...
- [ ] **`tests/test_425_loss_tier1_qty1.py` — 판정 스냅샷 만료 (2026-08-14 확인)**
      `"현재는 표본/거래일 미달로 INSUFFICIENT"` 단정이 425차 시점 데이터 상태를 박아둔
      것인데, 지금 실측은 **`verdict=PASS` (econ_ok=True, repro_ok=False)** 다.
      🔴 **이건 테스트 부채이자 캠페인 이벤트다** — 사전등록 채널이 판정을 내기 시작했다는
      뜻이므로 **주간회의 상정**. 사전등록 원칙상 **판정식·기준은 건드리지 않았고**
      테스트 단정도 임의로 고치지 않았다(고치면 판정 전환을 조용히 덮는다).
- [x] [DONE 2026-08-14] `tests/test_465_retrain_scope_and_tp1_flag.py` — `tp1_reached`
      단정이 `count(*) where not null == 0`이라 **배선이 동작하는 순간 깨지는** 형태였다
      (08-13 라이브 6행 적재로 실패). "배포일 이전 행만 NULL" + "이후 행은 실제로 채워짐"
      두 단정으로 정정 — 후자를 함께 넣은 이유는 "컬럼만 있고 아무도 안 쓰는 상태"가
      영구 통과하는 것을 막기 위함(292·303차 반복 실패 형태)
...
- [NEXT 다음 거래일] **P5-a 실동작 확인**
  ...
  - `[ConstOut]` 감지가 뜬다면 그건 진짜 상수출력이다 — 08-10 패턴(dir=-1,
    range 0.002)과 대조해 모델 축퇴 여부 별도 조사
  - 장중 재학습 횟수가 줄었는지 (0811은 3m 3회 + 5m 2회)

- [NEXT 다음 EOD] **`유령 기준 HOLD` 경고 출력 확인** — 3m·5m HOLD 시
  `[Retrain] ⚠ 유령 기준 HOLD` WARNING이 나오는지 (잠금 해제 전까지 매일 떠야 정상)

- [NEXT 주간회의] **가드 로직 개정 검토** — meta source=intraday면 콜드스타트 취급
  (acc.txt 비교 무효화). [22] guard_shadow가 이 모드를 측정 못 하는 문제
  (fair_valid=0 행 제외)와 함께 채널 스펙 개정으로 논의

- [NEXT 조사] **3m DN편향 반복 자체의 원인** — BiasReset이 3m에서 매일 발동하는 것
  (DN 70%+ 저적중)은 오탐이 아니라 실제 편향이다. 잠금 해제 후 EOD 모델에서도
  재현되는지 관찰 — 재현되면 레이블·클래스 불균형 축 조사(455차 배터리 연계)

---

### 463차 — C3·C4·C5 배선 (MW0602, 2026-08-12 배포, **라이브 미검증**)

> 배경: `DECISION_LOG.md` 2026-08-12(463차). 진입 판단 무변경 — 기록·표시 계층만.

- [NEXT 다음 거래일] **C3 배선 실동작 확인 (최우선)**
  - `ensemble_decisions.meta_gate_horizon` 분포가 `'1m'` 고정에서 벗어나는지
  - ATR-Horizon 로그 분포와 일치하는지 (배포 전 봉 기준 1m 43% / 3m 51% / 5m 6%)
  - ⚠ 여전히 100% `'1m'`이면 배선이 안 먹은 것 — `threshold_feasibility`가
    STEP4에서 결측(기본값 1.0 폴백)이 아닌지 확인할 것

- [NEXT 다음 거래일] **C5 확인** — `daily_stats`의 `sgd_accuracy`가 0.5가 아닌 값,
  `verified_count`가 0이 아닌 값으로 저장되는지. 둘 중 하나라도 그대로면 캡처 지점 재확인
  - ⚠ 2026-08-12 **이전 30행은 영구 결측**이다. 대시보드 추이에서 그 구간을
    "미계측"으로 읽을 것 — 0.5를 실제 정확도로 해석하지 말 것

- [NEXT 다음 거래일] **C4 확인** — 대시보드 SHAP 탭 CORE 그룹 표시가 진입 호라이즌을 따라
  바뀌는지 (종전 단기 고정)

- [NEXT 주간] **[2] meta_gate 재측정 표본 축적**
  - `VALIDATION_CAMPAIGN_DECISIONS["meta_gate"]` 재개 조건: **2026-08-12 이후** 데이터로만
    분위당 n≥300. 리포트 배지 `[스코어러 경계 …: 이전 N건 / 이후 M건]`의 M이 900을 넘으면 착수
...
### 462차 — 0811 점검 P1~P4 (MW0602, 2026-08-11 배포, **라이브 미검증**)

> 배경: `DECISION_LOG.md` 2026-08-11(462차). **라이브 무변경** — 집행형 플래그
> `ZONE_ENTRY_BAN_ENFORCE`는 기본 OFF, 나머지는 계측·경보뿐이다.

- [NEXT 다음 거래일] **461차 + 462차 신규 컬럼 2종 생성 확인 (최우선)**
  - 461차 커밋은 16:35 — 0811 장중에는 미적용이었다. 아직 한 번도 라이브를 돌지 않았다.
  - 기동 후 `PRAGMA table_info(ensemble_decisions)`에 `cal_applied`·`min_conf_effective`
    두 컬럼이 실제로 생겼는지(멱등 ALTER 동작) 확인
  - `[CleanShadow]`·`[FloorShadow]`(461차) 로그가 나오는지 확인

- [NEXT 다음 거래일] **P1-b: `min_conf` vs `min_conf_effective` 괴리 실측**
  - TrendGate 발동 분에서 두 값이 실제로 갈리는지(0811 기준 TrendGate 17회)
  - 갈린 행 중 진입 성사분이 있으면 그것이 462차가 규명한 경로다
  - ⚠ 이 컬럼이 채워지기 전 데이터로 "conf 여유(conf−mc)" 분석을 하지 말 것 —
    기록 `min_conf`는 판정값이 아니다

- [NEXT 다음 거래일] **P2: CB⑤ 오탐 소멸 확인**
  - 장중 재학습 직후 사이클의 `[PipePerf]`에 `[모델교체 …ms 차감 / raw=…ms]` 태그 확인
  - 그 사이클에서 `CB⑤ 파이프라인 지연 경고`·`HealthPolicy Degraded 선제차단`이
    **더는 뜨지 않는지** — 0811엔 5회씩 떴다
  - ⚠ 차감 후에도 지연이 남으면 그건 진짜 지연이다. 태그 없는 CB⑤는 정상 동작

- [NEXT 다음 거래일] **P3: 09:30 조기경보 동작 확인**
  - conf 분포가 낮은 날 `[경보] mc 도달불가(조기 09:30)`이 뜨는지
  - 진입금지 존(11:50~13:00)에서는 뜨지 않는지 (404차 후속4 오탐 재발 방지)
  - 당일 1회만 뜨는지 (매분 반복 아님)

- [NEXT 주간] **P1-a 집행 결정 — [53] `zone_ban_breach_watch` 판정 대기**
  - 현재 위반 코호트 7건 < min_samples 20 → INSUFFICIENT
  - `[ZoneBanShadow]` 로그로 위반이 계속 쌓이는지 관찰
  - ⚠ **FAIL(위반 코호트가 열위)일 때만 `ZONE_ENTRY_BAN_ENFORCE=True` 승격 후보**다.
    지금 실측은 반대 방향(7건 +596,858원)이라, PASS가 유지되면 처방은 집행이 아니라
    **`TIME_ZONES`에 11:50~13:00이 없는 것이 의도인지 재검토**하는 쪽이다

- [NEXT 주간] **[51]/[52] 채널 — 표본 축적만, 처방 금지**
  - 둘 다 첫 실행 PASS이고 [52]는 **가설과 반대 방향**으로 나왔다(저가중이 오히려 우위)
  - min_days=10·버킷당 25건 도달 전까지 어떤 처방도 올리지 말 것 (313차)

- [NEXT 조사] **3m 모델 상수출력 반복**
  - 0811 `ConstOut` 5회 중 3m 3회 / 5m 2회. 3m은 EOD 교체도 보류(0.4756→0.4138)
  - `cvd_monotone_ratio` 미가용으로 3m 피처가 105→12개. 진입 비중 최대 호라이즌이다

- [NEXT 조사] **`[ZeroDiag]`가 완화 전 min_conf로 판정하는 잔여 오진**
  - 462차 발견: 08-03 12:14 행이 진입 성사인데 ZeroDiag는 "conf미달(0.579<mc0.620)"
  - 456차가 고친 오진과 같은 계열. `_checklist_min_conf`를 쓰도록 정정 검토

---

### 461차 — 보정기 표본 위생 P-A~P-D (MW0602, 2026-08-11 배포, **라이브 미검증**)

> 배경: `DECISION_LOG.md` 2026-08-11(461차). **라이브 신호 경로 무변경** —
> 새 스위치 2종 전부 기본 OFF. 섀도·계측만 배포했다.

- [ ] 🔴 **다음 거래일 `[CleanShadow]` 로그 확인 (P-A)** — SIGNAL/LEARNING 로그에
      `live(...) vs clean(...) 오염행=N건 축퇴판정 live=X clean=Y` 가 찍히는지.
      **오염행 비중이 20% 안팎이면 실측(저장본 21.0% / 20일 23.4%)과 일치**한다.
      `축퇴판정 live != clean`이 잦으면 "지금의 축퇴 판정은 오염이 만든 것"이라는
      뜻 → §46 채널이 그것을 집계한다.
- [ ] 🔴 **`[FloorShadow] 판정 불일치` 로그 확인 (P-B)** — 0811 오전 같은
      "정적하한 통과 + 실효하한 미달" 구간이 실제로 얼마나 자주 나오는지.
      이 로그가 **자주 뜨는데 진입이 0이면** P-B를 켤 근거가 쌓이는 것이다.
- [ ] **EOD 후 `cal_applied` 적재 확인 (P-C)** — NULL이면 마이그레이션 또는
      decision 키 전파가 안 된 것.
      ```
      sqlite3 data/db/predictions.db "SELECT date(ts), cal_applied, COUNT(*) FROM ensemble_decisions WHERE date(ts)=date('now','localtime') GROUP BY 2"
      ```
      기대: 1(보정 적용)과 0(raw 통과)이 **둘 다** 나온다. 한쪽만 나오면 그날
      가드가 안 뒤집혔다는 뜻이므로 [45] 플립 카운트와 대조할 것.
- [ ] **오프라인 연구 재실행 (P-D)** — 표본이 늘면 결론이 바뀔 수 있다.
      ```
      C:/Users/pc1/anaconda3/envs/py310_64/python.exe scripts/cal_functional_form_study.py --db-days 30
      ```
      ⚠ **읽기 전용**이다. 여기서 좋아 보이는 C·함수형을 바로 배포하지 말 것.
- [ ] **(주간회의 안건) `CAL_COLLAPSE_EXCLUDE_LIVE` / `CAL_UNREACHABLE_USE_EFFECTIVE_FLOOR`
      를 켤 것인가** — 🔴 **449차 R4(EOD 고정) 트리거와 한 안건으로 묶을 것.**
      셋 다 보정기 적용 상태를 건드려서 따로 켜면 [45] 계측 해석이 오염된다.
      켤 때 `CONF_SCALE_BREAKS`에 **네 번째 경계**를 등록하고 §9-4 시계 리셋 판단.
- [ ] (읽는 법) **P-A를 "span 붕괴 해결"로 인용 금지** — 오염행을 빼도 C=0.02에서
      span은 0.0074 → **0.0038로 더 납작**해진다. 직접 원인은 C다. 다만 오염이
      있으면 **C를 500배 올려도 span이 안 풀린다**(20일 표본 C=10에서 0.0062 vs
      clean 0.2112) — 두 원인은 곱해진다.
- [ ] (읽는 법) **430차 "정보 부재" 진단은 아직 유효** — clean AUC 이득이 전부
      SE 안이다(+0.0135/SE 0.041, +0.0096/SE 0.010). 뒤집힌 게 아니라
      **재측정 대상이 된 것**이다. `DEGENERATE_AUC_MIN`·함수형 금지선 유지.
- [ ] (알려진 함정) 회귀 테스트는 **`PYTHONIOENCODING=utf-8`로 실행**할 것 —
      cp949 콘솔에서 em-dash 출력 시 `UnicodeEncodeError`로 죽는다(로직 실패 아님).
      456·459차 테스트도 같은 함정이 있다.

---

### 459차 — F1 승패 집계 단위 + F2 SHS CORE 미측정 (MW0602, 2026-08-10 배포, **라이브 미검증**)

> 배경: `DECISION_LOG.md` 2026-08-10(459차). 계측 결함 수정 — 진입·청산 판정 무변경.

- [ ] 🔴 **다음 거래일 SHS 로그 확인 (F2)** — 관망·포지션 보유 분에서 SHS가 더 이상
      상시 75점(−25 전액 감점)에 붙박이지 않는지. **SHS 분포가 위로 이동하는 것이
      정상이다** — 과거 시계열과 불연속(전환일 2026-08-10). 슬랙 경보 빈도도 함께
      줄어드는 것이 기대 동작.
- [ ] 🔴 **다음 거래일 첫 부분청산 포지션에서 승패 1건만 세는지 확인 (F1)** —
      `daily_stats`의 trades가 레그 수가 아니라 포지션 수여야 한다. 장중 재기동이
      끼면 `position_state.json`에 `pos_acc_pnl_pts`가 남아 있는지도 함께 볼 것.
- [ ] **EOD 후 `daily_stats.win_formula_version = 2` 기록 확인** — NULL이면
      `init_daily_stats_db()` 마이그레이션이 안 돈 것.
      ```
      sqlite3 data/db/trades.db "SELECT date, trades, wins, win_formula_version FROM daily_stats ORDER BY date DESC LIMIT 5"
      ```
- [ ] **EKS 로그의 `(측정 z봉)` 관찰 — 조치는 아직 하지 말 것.** GAP_OPEN 5봉 중
      측정 봉이 지속적으로 0에 가까우면 "CORE 통과 조건이 사실상 상시 참"이라는
      뜻이다. **발동 조건은 일부러 안 바꿨다**(안전장치). 표본이 몇 주 쌓인 뒤
      발동 조건 재설계 여부를 판단할 것.
- [ ] **(주간회의 안건) SHS `entry_blocked`를 실제 게이트로 승격할 것인가** —
      현재 **표시 전용으로 명문화**했다(코드 주석·docstring). 권고는 표시 전용 유지:
      감점계수 8/2.5/25/5가 전부 미캘리브레이션이라 지금 승격하면 상시 차단이 된다
      (303차 FP-CRITICAL·292차 CB③-P4가 같은 실패 유형). 승격하려면 F2 이후 라이브
      SHS 분포를 수 주 관찰해 임계(60)를 먼저 재캘리브레이션할 것.
- [ ] (읽는 법) **F1을 "승률 개선"으로 인용 금지** — 2026-08-10 실DB 재집계에서
      레그 61.6% → 포지션 62.4%, 뒤집힘은 패→승 4건 / 승→패 0건이었다. 종전 방식이
      승률을 부풀리지 않았다. 이번 수정의 근거는 **정밀도**(건별 금액이 크다)다.
- [ ] (알려진 기존 실패) `tests/test_424~429` 6종은 이번 수정 전부터 실패 중 —
      DB 스냅샷 실측치를 고정한 테스트가 데이터 증가로 드리프트한 것. 별건.

### 458차 — 손익 원천 이전: 3-A·3-B·3-C 구현 완료 (MW0602, 2026-08-10 배포)

> 제안서: `docs/미륵이고도화3/손익원천이전_제안_2026-08-10.md` (승인됨)
> 구현: `DECISION_LOG.md` 458차 후속. **라이브 매매 경로 무변경 — 전부 오프라인 채널.**

- [ ] **다음 금요일 EOD 체인에서 신규 스텝 2개 rc=0 확인** — `[검증 캠페인]` 요약에
      `조기청산 반사실 [49]=OK`·`방향 처분 실험 [40-B]=OK`가 보여야 한다.
      MISSING/FAIL이면 campaign_steps 경로·인코딩부터 볼 것.
- [ ] 🔴 **[49] 판정 개시 임박 (56/60건, 1주 내)** — 첫 실측이 제안 가설과 **반대**다:
      주팔(원 스톱 유지) 건당 **-1.056pt** = 트레일의 이 발동 지점이 옳았다는 방향.
      판정이 PASS(현행 유지)로 나오면 "부스러기 회수" 논의를 데이터로 닫는다.
      ⚠ 표본 미달 상태의 이 수치로 어떤 조치도 하지 말 것.
- [ ] **[40-B] 표본 대기 (160/300건, ~35일)** — 어떤 결과도 역방향 매매 금지.
      처분 분기(재학습 격하/리서치 철수/존치)는 관측 전 선언돼 있다(settings 주석).
- [ ] **3-C 지도 월간 재실행 + MW0601 대조** — 첫 지도(92건/6일): 현행 셀이 골짜기
      근방, `tp1=0.3` 행이 전 스톱 최상. **소표본 + D2 경고 — 인용·안건화 금지.**
      MW0601이 같은 스크립트를 돌려 표면이 재현되는지가 선행 조건.
      다음 실행: `python scripts/exit_expectancy_map.py` (py310_64, 월 1회).
- [ ] (읽는 법) **방향 축 "해롭다" 인용 금지** — 3팔 점추정 전부 음이나 비유의.
      [40] 공식 판정("무작위와 구분 안 됨")이 정본. [40-B] 일자단위 9/20 = 정확히 절반.
- [ ] (별건 후보) 기대값 지도의 트레일 지연·TP1 비율 축 — `exit_replay.replay()`가
      파라미터화하지 않아 현 지도에 없다. [49]가 FAIL로 갈 때만 확장 가치가 생긴다.

### 457차 — P4·P5·P6 라이브 확인 (MW0602, 2026-08-10 배포, **라이브 미검증**)

> 배경: `DECISION_LOG.md` 2026-08-10(457차)

- [ ] 🔴 **다음 거래일 ConstOut 발생 시 스코프 확인** (P5)
      `[GBM-64] ... | scope=3m` 과 `[Retrain] 교체 스코프 제한: ['3m'] 만 재학습`이
      함께 보여야 한다. `scope=ALL`이 뜨면 `CONST_OUT_RETRAIN_SCOPED`가 안 먹은 것.
      동시에 **비트리거 호라이즌의 pkl mtime이 EOD 이후 안 바뀌는지** 확인할 것 —
      이게 이 변경의 목적이자 부작용이다(당일 중 갱신 없음, EOD까지 대기).
- [ ] 🔴 **다음 EOD에서 `gbm_*_meta.json` 6개 생성 확인** (P6)
      교체된 호라이즌만 생긴다(HOLD면 미생성 — 정상). `acc_kind`가 `cv`인지,
      `train_end_ts`가 그날 마지막 학습봉인지 확인.
      ```
      for f in model/horizons/gbm_*_meta.json; do echo "$f"; cat "$f"; echo; done
      ```
- [ ] **다음 EOD `[GuardFair]` 로그에 유효성 문구 확인** (P4)
      `ok (구모델 pkl mtime=...) | 유효 — 현행 train_end=... < holdout_start=...`
      또는 `무효 ... | 현행이 홀드아웃 학습함`. **457차 시점 예측은 "무효"** 다 —
      EOD 직전까지 장중 재학습이 돌기 때문. 유효가 뜨려면 그날 ConstOut이 없어야 한다.
      → P5(스코프 제한)로 장중 교체가 줄면 유효 표본이 생기기 시작할 것. 그 인과를 관찰할 것.
- [ ] **[23] guard_shadow 채널 `fair_holdout_invalid_rate` 추이 감시** (매주)
      현재 **1.0(36/36)**. 이 값이 내려가야 GuardFair 승격 논의가 가능하다.
      **유효 표본이 쌓이기 전에는 fair_* 수치를 성능차로 인용하지 말 것** —
      457차 이전 "6/6 신모델 열위"는 in-sample 프리미엄이었다.
- [ ] (읽는 법) **P6는 판정을 바꾸지 않았다.** acc.txt 의미론·EOD 가드 로직 무변경.
      `missed_upgrade_rate=0.0714 → PASS`(404차 사전등록)가 여전히 유효하므로
      "acc.txt가 왜곡됐으니 기준을 바꾸자"를 이 커밋 근거로 삼지 말 것.
- [ ] **acc.txt 영구 동결 자체는 미해결로 남았다** (P2, 조사)
      3m은 최근 CV 0.4239 vs 고정 old_acc 0.4756 — 허용 -0.025 안에 못 들어
      구조적으로 교체 불가다(08-01 이후 9거래일 EOD 교체 0회). 메타 사이드카로
      **가시화만** 했다. 해소하려면 404차 채널이 FAIL로 뒤집히거나 별도 안건이 필요하다.

### 456차 — P1-c·P2·P3 라이브 확인 (MW0602, 2026-08-10 배포, **라이브 미검증**)

> 배경: `DECISION_LOG.md` 2026-08-10(456차)

- [ ] 🔴 **다음 거래일 로그 확인 3종** (P1)
      ① `[ZeroDiag] 진입X 원인:` 줄에 `| 참고:` 구분이 보이는지. 차단이 있었던 분봉에서
         `⚠원인미상`이 잦으면 아직 목록에 없는 차단자가 있다는 뜻 — 그 분봉의 SIGNAL
         블록을 통째로 읽어 무엇이 막았는지 찾고 `checklist_reason`에 기록되는지 확인.
      ② `[MCFloor]` 출현 여부. 발동하면 그날 진입이 실제로 줄었는지 대조할 것
         (12거래일 재생 기준 예상 0~1회/일, 진입 영향 0건).
      ③ `[JointGateBlock] (섀도) meta는 무정보 폴백` 출현 여부 — 폴백 차단이 나면 찍힌다.
- [ ] **`joint_gate_shadow.meta_neutral_pass` 적재 확인** (P3, 다음 차단 발생일)
      ```sql
      SELECT COUNT(*), SUM(meta_neutral_pass) FROM joint_gate_shadow
      WHERE ts >= '2026-08-11';
      ```
      NULL만 쌓이면 INSERT가 조용히 실패하는 것 — 420차와 같은 사고다. 즉시 조사.
- [ ] **[7] 채널에 `meta_neutral_pass` 절 추가** (표본 누적 후)
      `generate_validation_campaign_report.py`는 지금 이 컬럼을 **소비하지 않는다**
      (컬럼 보장만 함). 폴백 차단 표본이 [7] `min_samples`에 닿으면 "중립화했다면
      풀렸을 신호"의 누적 hyp_pnl을 절로 붙일 것.
- [ ] 🔴 **`JOINT_GATE_META_FALLBACK_NEUTRAL`을 단독으로 켜지 말 것** — 431차가 거부하고
      456차가 재확인한 결정이다(차단 158건 반사실 -72.85pt). 켜려면 위 절의 판정이
      먼저이고, 켤 때 `VALIDATION_CAMPAIGN_DECISIONS`·`DECISION_LOG`를 함께 갱신할 것.
- [ ] (읽는 법) **`MC_RELAX_FLOOR_ENABLED`를 손익 개선으로 소개하지 말 것.**
      12거래일 재생에서 **차단됐을 진입 0건** — 불변식 보강이지 수익 조치가 아니다.
      초안이 "14:49 -162,926원의 패인"이라고 잘못 적었던 항목이다(conf 0.495 > 강화선 0.46).

### 456차 파생 — 미조치 등록건 (MW0602, 2026-08-10)

- [ ] **P8 스케일러 창(500봉)이 짧아 수급 계열이 상시 이상값으로 잡힌다** (P2, 조사)
      `python scripts/check_scaler_feature_drift.py --days 5` 실측:
      `program_non_arb_net` z=+3.07·격리유발 **35%**, `foreign_call_net` **36%**,
      `program_foreign_net_krw` 25%, **`foreign_retail_divergence`(15m 실사용) 22%**.
      대부분 비실사용이라 예측 오염은 제한적이나, `last_extreme_features` 상위 5칸을
      잠식해 **AutoMask가 진짜 이상값을 놓칠 수 있다**. 창 길이 조정은 P8 전체에
      영향을 주므로 단독 변경 금지 — 먼저 AutoMask 선정 로직이 실사용 피처를
      우선하는지부터 확인할 것.
- [ ] 🔴 **`acc.txt`가 교체 성사 호라이즌만 갱신돼 EOD 가드가 교착된다** (P1, 조사)
      실측 mtime: `gbm_3m_acc.txt` **2026-08-01**(9일 정체), `gbm_5m_acc.txt` 08-06,
      나머지 4개는 08-10. 3m·5m은 EOD 가드에 계속 보류돼 `old_acc`가 고정되고,
      고정된 값(3m 0.4756) 대비 -0.025 안에 못 들면 **영구 동결**된다(최근 CV 0.4239).
      그 사이 장중 무검증 재학습이 `gbm_3m.pkl`을 계속 덮어써서(mtime 08-10 14:16)
      **가드가 지키려는 "검증된 구모델"이 실재하지 않는다.**
      346차 가드 설계 의도와 어긋나는지 먼저 확인할 것 — 의도된 동작일 수 있다.
- [ ] **`GuardFair`가 6/6 신모델 열위인데 4개가 교체됐다** (P2, 주간회의 안건)
      2026-08-10 EOD: 공정 홀드아웃 격차가 1m -0.067 / 3m -0.175 / 5m -0.128 /
      10m -0.047 / 15m -0.144 / 30m -0.086로 **전부 음수**인데 판정은 `무영향(섀도)`.
      섀도 승격은 사전등록 후 4주 관찰이 먼저다(313차 — 관측 후 기준 수립 금지).
- [ ] **`DB pruning 실패: database table is locked`가 6주 연속** (P2)
      06-29·07-06·07-13·07-20·07-27·08-03·08-10 — 한 번도 성공한 적 없다.
      `predictions.db` 856MB / `raw_data.db` 481MB. EOD 15:40:08에 WAL 체크포인트와 경합.
- [ ] **`daily_stats.sgd_accuracy`·`verified_count`가 죽은 열** (P2)
      최근 8거래일 전부 `0.5` / `0`. 실제 오늘 SGD 50분정확도 25.6%, 검증 1,253건.
      toxicity_block_shadow 3개월 0건 방치와 같은 유형(계측이 조용히 죽음).
- [ ] **청산사유 라벨 과부하** (P3) — 비틱 `하드스톱` 19건이 **19승 0패**(트레일링
      보호청산)인데 손절과 같은 이름을 쓴다. 청산사유 집계가 오독된다.
      `exit_fill_slippage`의 `stop_intrabar` 경로도 평균 슬리피지 **-2.708pt**로
      `price_hint`가 실제 스톱가가 아닐 정황(439차 P3 경고와 연결).

---
- [ ] **다음 거래일 장중 — F8 가드 실효 확인**: 분석 스크립트를 장중에 실행해
      rc=2로 차단되는지 1회 실측. 동시에 그날 CB⑤ 자가유발이 0건인지.
- [ ] **다음 금요일 EOD — F3/F8이 캠페인 체인에서 무해한지**: `[검증 캠페인]` 요약에
      `SKIP(장중차단)`이 **없어야** 정상(EOD는 15:45 = 장 마감 후).
      피처셋 건강 리포트의 후보 수가 62 → 82 근방으로 늘었는지도 함께 확인.
- [x] ~~**Wave 1 착수 (F1 승패집계 · F2 SHS)**~~ **[DONE 2026-08-10] 배포 완료.**
      실데이터 검증: 오늘 13레그 → 10포지션 7승3패(70.0%), 연간 194포지션 115승
      (59.28%) — 파이썬 누적기와 SQL 두 독립 경로가 일치. SHS 미측정 84.5점(구 59.5).
      전체 214 passed. 상세: `DECISION_LOG.md` 456차 Wave 1.
- [ ] **[F1/F2 후속] 다음 거래일 라이브 확인** — ① `daily_close` 로그의 승/패가
      포지션 단위인지(다레그 발생일에), ② SHS 경보가 뜨면 문구가 "경고 전용,
      진입 차단 아님"이고 CORE가 "미측정"으로 나오는지, ③ 대시보드 추이 탭의
      오늘 승률이 마감 로그와 일치하는지.
- [ ] 🔴 **[F1/F2] MW0602 대조** — 둘 다 **공통 코드**다. 그쪽 `daily_close` 승패와
      SHS 점수 분포가 같은 방식으로 어긋나 있을 것이다. pull 후 확인할 것.
- [ ] **[F1/F2] 시계열 불연속 2건 — 경계 2026-08-10**. 이 날짜를 걸치는 **승률**
      비교와 **SHS** 비교는 무효다(승패 단위 변경 / SHS 분포 상향).
      430차 `CONF_SCALE_BREAKS`와 같은 성격 — 점검 시 경계 인지할 것.
- [x] ~~**Wave 2 (F5 opt_pcr) Step 1 진단**~~ **[DONE 2026-08-10] 가설 반증.**
      450차 무관(07-22부터 발생) · 스케일러 최신(당일 15:48). 진짜 원인은
      **간헐 이진 플래그**(`bearish` 고유값 2, `extreme` 3, 일별 점화율 12~100% 진동)
      **× P8 500봉(≈1.3거래일) StandardScaler** = 산술적 필연.
      **코드 변경 0건.** 상세: `DECISION_LOG.md` 456차 Wave 2.
- [ ] 🔴 **[F5 Step 2] 주간회의 결정 — 3안 택1**. 셋 다 10m·15m·30m 모델 입력을 바꾼다.
      **A** 플래그 계열 피처셋 제외(IC 근거상 최단순) / **B** 플래그 스케일링 제외 /
      **C** 간헐 피처만 스케일러 창 연장(500봉 → 20거래일).
      판단 재료: L1 실측 IC가 연속형 3종 **전부 |t|<1**, 플래그 5종은 `MIN_DISTINCT`
      미달로 스크리닝 진입 불가. **A안이면 L2/L3 검증 후 Wave 5로.**
- [ ] **[F5 파생, 별건] 스크리너와 프로덕션의 기준 불일치** — L1은 `opt_pcr_bearish`·
      `extreme`를 "분석 불가한 플래그"(고유값<20)로 제외하는데 **프로덕션 모델은
      그대로 소비**한다. 다른 피처에도 같은 불일치가 있는지 전수 확인할 것.
- [ ] **Wave 3 (F6+F7)** — **동반 배포 강제**. F7 단독은 대체 기준 없이 교체정책만
      바꾸는 것이라 더 위험하다. 주간회의 ⑤안/⑥안 택일 선행.
- [ ] **Wave 4 사전등록** — F4-A(TP1 지연 섀도)·G2-B(손절 슬리피지)·F10-B(C→A 승격)
      3채널 합격선을 **데이터 수집 시작 전에** `VALIDATION_CAMPAIGN`에 박을 것
      (캠페인 §9 사후 기준 변경 금지). F4-A의 `would_be_tp2`는 **실틱 경로**로
      재구성 — 봉 재생을 쓰면 [25]가 SUSPENDED된 이유를 그대로 반복한다.
- [ ] **(별건, 기록만)** `validate_feature_set_purged_cv.load_data()`가 `raw_features`
      전체를 WHERE 없이 읽고 파이썬에서 최근 3만행만 남긴다 — 4종 중 IO 최대.
      쿼리 의미 변경이라 Wave 0 범위 밖으로 뒀다.
...
- [ ] 🔴 **[최우선] 다음 거래일 장 마감 후 아래 1줄 실행** — 이 결과가 Phase 3 착수
      여부를 가른다. **Phase 0은 이 판정을 위해 존재한다.**

      ```
      python scripts/generate_cvd_anchor_report.py --days 1 --stdout
      ```

      [453차 Phase 2] 이 리포트가 아래 수동 쿼리를 **대체한다** — 같은 지표를 계산하되
      판정문·근거·다음 조치까지 함께 낸다. 판정이 `⏳ 계측 미개시`로 나오면 SYSTEM
      로그의 `[CVD-ANCHOR][UNAVAIL]`을 확인할 것(원천이 22/23 미제공 = 가설 반증).
      수동 쿼리는 교차 확인용으로 남긴다:
...
- [x] ~~**[Phase 1] `main.py` 복구 경로 데이터 파괴 차단**~~ **[DONE 2026-08-09] 배포 완료.**
      `_bar_from_raw_candle_row()`가 `row.keys()`를 순회해 **열 목록을 하드코딩하지 않는다**
      (네 번째 재발 차단). `bar_recovered` 열 신설(NULL=452차 이전/0=정상/1=복구).
      회귀 테스트 8/8 — 구버전 재현 시 **9열 파괴** 확인(Phase 0 앵커 4열 포함).
      **라이브 미검증** → 아래 확인 항목.

- [ ] **[452차 Phase 1] 라이브 확인 3종** — 다음 복구 발생일에:
      ① `[복구 시도]` 로그의 ts를 잡아 그 봉의 `buy_vol`/`bid1`/`oi`/`anchor_buy` 보존 확인
...
- [ ] **[452차 Phase 1 파생, 별건] 백필과 라이브가 다른 `override_horizon_features`를 쓴다**
      ...
      | 백필(`scripts/aggregate_and_backfill.py`) | 자체 로컬 함수 `:53` | 1분봉 값 그대로 |

      즉 호라이즌 모델의 학습행과 추론행이 **같은 이름의 피처를 다르게 채운다**.
      정렬하면 학습 데이터 의미가 바뀌므로 영향 분석 후 진행할 것.
      (452차는 백필의 `buy_vol: 0` 하드코딩만 제거 — 위 로컬 함수가 그 값을 소비하지
      않아 산출물을 직접 오염시킨 적은 없으나, 두 함수를 정렬하는 순간 밟을 지뢰였다.)

- [x] ~~**[Phase 2] 앵커 대조 리포트**~~ **[DONE 2026-08-09, 453차]**
      `scripts/generate_cvd_anchor_report.py` 신설 + EOD 주간 체인 편입(재학습 앞).
      판정 5분기·anchor_diff 순/절대 분리·항등식 3종. 게이팅 없음(종료코드 항상 0).
      테스트 48체크 + 전체 176 passed. 산출물은 다음 EOD 체인(금요일) 또는 위 수동
      실행에서 처음 생성된다.

- [ ] **[Phase 3, 조건부] CVD 전환** — 위 판정이 `✅ 발견 A 확정`으로 나오고 섀도
      **20거래일** 누적 후에만. 정규화 포화(`cvd.py:104`, 원인 ②)를 **동시에** 고쳐야
      하며, 틱 원본이 없어 2026-06-08~전환일 구간은 **영구 편향**(백필 불가) — 학습 창
      분리 필요. 우회책 3건(2026-06-25 CORE 교체 / 394차 detrend / AutoMask 면제 제외)
      재검토 동반.
      ⚠ 리포트가 "섀도 vs 앵커 괴리 > 2%" 경고를 내면 전환 대상을 **앵커 직접 사용**으로
      바꿔 설계할 것 — 올바른 체결구분 파싱만으로는 정답지에 도달하지 못한다는 뜻이다.
...
- [ ] **[0-3 후속] gross PCR 병행 기록** (P2, Phase 1 이후)
      ...
      중앙값 0.705 → **1.038**(중립), 구 가드 통과율 net 15.0%.
      🔴 **그러나 덤프는 하루 1건(09:02)이라 일중 분포를 볼 수 없다** — 이 결과를
      **가드 임계 재설정 근거로 쓰지 말 것.** 일중 표본이 필요하고, 그것은 7221 전량
      보존(계획서 Phase 1-2)이 되면 자동으로 쌓인다.
      ⚠ 450차 기록의 중앙값 1.59와 이번 0.705가 다르다 — 표본(일중 vs 09:02)이 달라서다.
        **둘 다 남길 것.** 어느 쪽도 틀렸다는 근거가 없다.

- [ ] **[0-4 후속] `opt_gex_sign` 승격 근거 MW0602 대조** (P2, 교차검토 필요)
      `config/constants.py:233`의 승격 근거 `IC=0.035 p=0.006 (06-01~07-10 재실측)`을
      **이 PC에서는 재현할 수 없다** — `opt_gex_sign`이 raw_features에 **2026-07-14부터**
      존재하고 그 이전 구간은 0행이다.
      ⚠ **단정 금지** — 같은 주석이 "292차에 MW0602에서 편입, 이 PC엔 미전파"라고 적고
      있어 MW0602에는 더 이른 데이터가 있을 수 있다. **MW0602 대조 전까지는
      "이 PC에서 재현 불가"까지만 확정이다.**
      → 대조 결과 MW0602에도 없으면 `DYNAMIC_FEATURES_POOL` 등록 근거가 사라진다.
      💡 별개로, 재현되더라도 `opt_gex_sign`은 `opt_gex_bn`의 **순수 부호 함수**(불일치 0건)라
        15m `include_pending_validation`에 **bn과 함께 올라 있는 것은 완전 중복**이다.
        계측 결함은 아니므로(451차 종결) 서두를 일은 아니다.

### 451차 후속4 — RAW 덤프 열 확대 후속 (MW0601, 2026-08-09)

- [ ] 🔴 **라이브 1일 확인 — 7221의 15열 너머를 처음 본다** (P1, 다음 거래일)
      `[CybosProbe][RAW]`에 `field_limit=64`가 찍히고 `CpSvrNew7221` 행에 **열 15 이상**이
      실제로 오는지 확인. 공식 문서상 type **0~51** × index **0~22**다.
      → 오면: 투자자 12분류 × 매도/매수/순매수 전량 + 국채/통화/상품/CME 선물 행이
        새로 보인다. `docs/CyBos ref/CYBOS_프로그램매매_투자자별_TR_명세.md` §3 대조표를
        **그 실측으로 갱신할 것**(지금 표의 "15열"은 우리 프로브 한계였지 TR의 한계가 아니다).
      → 안 오면: 2014년 문서와 현행 서버 응답이 다른 것이다 — 그것도 기록할 것.
      ⚠ 상시 파싱 폭은 15로 남겼다(라이브 500ms 예산). 넓은 읽기는 **세션 첫 호출 1회뿐**이다.
      ⚠ 열이 늘어도 **의미를 모르는 열을 피처로 만들지 말 것** — 451차가 그래서 났다.

- [ ] **(읽는 법) 과거 `[CybosProbe][RAW]` 로그의 7221은 15열까지만 있다**
      2026-08-09 이전 40거래일치 덤프는 전부 절단본이다. 그 로그로 "7221은 15열"이라고
      결론 내리지 말 것. gross PCR 검증(450차)에 쓰는 열3·열4는 절단 범위 안이라 **유효하다.**
...
- [x] ~~**HTS에 프로그램매매 투자자별 화면이 있는가**~~ **[DONE 2026-08-09] 있다.**
      사용자가 Creon HTS **화면 8221 「프로그램매매 투자자별 종합」** 스크린샷으로 확인.
      코스피/코스닥 × 차익/비차익/전체 × 매도/매수/순매수 × 투자자 12분류
      (개인·외국인·기관계·금융투자·보험·투신·은행·기타금융·연기금등·국가지자체·
      기타법인·사모펀드), 기준일 2026/08/07, 단위 백만원.
      → **데이터는 존재한다. 남은 문제는 API 경로다(아래).**

- [ ] 🔴 **장중 프로브 — 화면 8221의 COM 원천 찾기** (P1, 장중 1회)
      ```
      conda run -n py37_32 python scripts/probe_cybos_program_trade.py --ensure-login
      ```

      **선행 문서조사 결과(451차 후속2, 완료) — 공식 API에는 교차 객체가 없다.**
      | 계열 | 객체 | 축 |
      |---|---|---|
      | 프로그램 | `CpSvr8111`·`PgAtime8112`·`CpSvr8116`·`8119` | 차익/비차익 **only** |
      | 투자자 | `CpSvrNew7212`·`7221`·`7222`·`7224` | 투자자 **only** |
      | 혼합 | `Dscbo1.CpSvr7225` | 둘을 **나란히** 두지만 교차 아님 |

      **그래도 프로브할 값어치가 있는 이유**: HTS 화면 8221이 데이터를 받는 이상 그 뒤에
      COM 객체가 있다. 이 PC 레지스트리에 **공식 문서에 없는 등록 객체**가 다수 존재하며
      (`CpSvr8241`·`8113`·`2221`·`7210d`·`7210T`·`7236`·`7238`·`7240` 등), 프로브에
      후보로 심어뒀다. 전부 **레지스트리 등록 확인을 거친 실재 ProgID**다.
      ```
      python scripts/probe_cybos_program_trade.py --list-registered --filter "8[0-2][0-9]{2}"
      ```
      (COM 연결·장중 불필요. `CpSvr8221`은 **등록돼 있지 않음**을 이걸로 확인했다.)

      ⚠ **등록됨 ≠ 사용 가능.** 미문서 객체는 `BlockRequest` 미지원이거나 권한이 없을 수
      있다(8111S/8111KS가 실제로 그랬다 — 260704 P2).
      ⚠ **판정 기준은 둘 다다: ① 투자자 × 차익/비차익 교차 격자인가 ② 장중에 갱신되는가.**
      화면 8221에 `기준일` + `조회` 버튼이 있는 것으로 보아 **일별 마감 통계일 가능성이
      있다.** 그렇다면 응답이 와도 1분봉 피처로는 못 쓴다.
      💡 **가장 싼 다음 확인(무료·5초)**: 다음 장중에 화면 8221을 **두 번**(예: 10시,
      14시) 조회해 숫자가 변하는지 볼 것. **안 변하면 그 자리에서 종결** — 프로브도 불필요.
      ⚠ `scripts/check_cybos_investor_candidates.py`의 `CpTd6198~6200`은 CpTrade(주문)
      네임스페이스 번호 인접 추측이다 — **후보로 쓰지 말 것.**
      → **교차 격자 + 장중 갱신 둘 다 확인되면** 수집 재설계.
      → **어느 하나라도 실패하면** 폐기를 최종 확정하고 **곧바로 아래 B안으로 넘어갈 것.**
        투자자별이 없다고 해서 8111이 정보가 없는 TR인 게 아니다 — 우리가 56필드 중
        2개만 쓰고 있다(B 항목 참조).

      📌 ~~**스케일 37배 미해결**~~ → **[451차 후속3 해소] 착시였다.**
      `[CybosProbe][RAW]` 덤프는 전부 **09:02(세션 첫 호출) 스냅샷**이지 일일 총계가 아니다
      — 8111은 **일중 누계**다(08-07 비차익: 09:02 +134,498 → 15:34 **−738,388**).
      마감값으로 다시 대조하면 **8221 코스피 비차익 순매수 합 −758,233 vs 8111 −738,388,
      2.7% 이내 일치**다.
      ⚠ **차익은 여전히 안 맞는다**: 8221 +184,475 vs 8111 +87,626 (약 2.1배).
      `통합(KRX+NXT)` 범위 차이 가능성. **두 소스를 이어 붙이기 전에 이것만은 해소할 것.**

- [ ] 🔴🟢 **[최우선·무비용] 8111 폴링의 차분(Δ) 저장 — 지금 누계 레벨만 쌓고 있다**
      (P1, 451차 후속3. **신규 TR 불필요, 이미 매분 받는 데이터**)
      `program_arb_net` / `program_non_arb_net`은 `CpSvr8111 idx19/idx37`인데, 이 값은
      **일중 누계**다 — 실측: 2026-08-07 비차익 09:02 **+134,498** → 15:34 **−738,388**.
      즉 지금 피처는 하루 종일 느리게 표류하는 **레벨**(강한 자기상관·비정상)이고,
      1분봉 예측이 필요로 하는 **흐름(Δ)** 이 아니다.
      `_fetch_investor_data`가 **60초마다** 이미 호출하고 있으므로 직전값과의 차분은 **공짜**다.
      ⚠ **기존 피처를 바꾸지 말고 Δ를 추가할 것** — 산식 교체는 train/serve skew(317차).
      ⚠ 재기동 시 직전값이 없으므로 첫 틱은 Δ 미정의로 처리할 것(451차에서 폐기한
        "모르는데 0으로 채우기"를 반복하지 말 것 — `quality_*` 플래그로 표시).
      💡 이게 성립하면 후보 #6(`PgAtime8112` 시간대별)은 불필요해질 수 있다.

- [ ] 🟢 **[신규 기회] `Dscbo1.CpSvr7225` 검토 — 옵션 flow 미매핑의 직접 해답 후보**
      (P2, 451차 후속2 발견 / 후속3에서 공식 명세 확보)
      「7225 시장매매흐름분석」, **공식 문서 확인·지원 객체**이며 이 PC에 등록돼 있다.
      입력 `SetInputValue(0, (short)최종시간)` — **0이면 현재시간**, 시간 지정 시 그 **바로
      전 시간부터** 수신. 헤더 0=최종시간, 1=데이터수. **연속 O**.
      출력(행=시간대) 0 시간 / 1·2 장내 외국인·기관 / 3·4 코스닥 외국인·기관 /
      5·6 **프로그램 차익·비차익** / 7·8 **선물 외국인·기관** / 9·10 **옵션 외국인·기관**.
      **왜 중요한가**: `CybosInvestorData._option_flow_supported`는 지금
      `"Cybos option investor-flow mapping pending"` 상태인데, 이 객체가 옵션 외국인·기관을
      직접 준다. 선물 투자자 + 프로그램까지 **한 번의 호출**로 온다(현재는 7221 + 8111 두 번).
      ⚠ **개인(individual)이 없다** — 외국인·기관만이다. `7221`을 **대체 못 하고 보완**한다.
      ⚠ 검증 필요: "시간대"의 실제 granularity, 값이 누적인지 구간인지, 7221과의 정합성.
      **기존 7221 경로를 갈아엎지 말고 병행 기록부터.**(317차)

- [ ] **B안 — 8111 미사용 54필드(차익/비차익 gross·위탁/자기) 수집·활용 검토**
      (P2, C 프로브가 음성일 때의 대안. 단 **C와 독립적으로도 유효**하다)

      **추가 TR 비용 0.** 이미 매분 받아서 버리고 있다. `request_program_investor()`가
      읽는 것은 `idx19`(차익 순매수 합계금액)·`idx37`(비차익 순매수 합계금액) **단 2개**이고,
      나머지 54개는 `_probe_investor_tr`이 헤더로 다 가져온 뒤 폐기된다.

      **실측 검증 완료(2026-08-07 `logs/20260807_PROBE.log` `[CybosProbe][RAW]` 덤프).**
      세션당 1회 자동 덤프가 이미 남고 있어서 **오프라인으로 산술 대사가 가능했다** —
      문서 레이아웃이 실제 값과 맞는지 다음과 같이 확인했다(단위는 백만원으로 추정):

      | 블록 | idx | 2026-08-07 실측 | 대사 |
      |---|---|---|---|
      | 차익 매도 (위탁/자기/합계 금액) | 5·6·7 | 4,972 / 24,491 / 29,463 | 합계 ✓ |
      | 차익 매수 | 11·12·13 | 4,902 / 43 / 4,945 | 합계 ✓ |
      | 비차익 매도 | 23·24·25 | 298,613 / 1,499 / 300,112 | 합계 ✓ |
      | 비차익 매수 | 29·30·31 | 433,693 / 917 / 434,610 | 합계 ✓ |
      | 비차익 순매수 **위탁금액** | **35** | 135,080 | 매수−매도 = 433,693−298,613 ✓ |
      | 전체 순매수 **위탁금액** | **53** | 135,010 | 438,595−303,585 ✓ |

      → **gross 매도/매수와 위탁/자기 분해가 전부 실재하고 산술이 맞는다.** 지금 쓰는
        `idx19`·`idx37`은 그 gross의 잔차 하나일 뿐이다. 450차가 PCR에서 겪은 것과
        같은 구조 — **순매수는 상쇄된 잔차라 스케일이 100배 요동**하는 반면 gross는
        그렇지 않다. 그래서 이건 "여분 필드 줍기"가 아니라 **같은 결함의 다른 피처판**이다.

      🔴 **`idx17`(차익 순매수 위탁금액)은 필드 결함이다 — [451차 후속3 확정]**
      7거래일(07-30~08-07) **전부** `idx17 == idx19`(합계금액)를 반환한다. 참값은 다르다
      (08-07: 참 −70 vs 반환 −24,518). 공식 명세는 정상이므로 **문서 오류가 아니라 서버측
      필드 결함**이다. 비차익(`idx35`)·전체(`idx53`)는 7일 모두 정확히 대사된다.
      ✅ **우회 확정(두 방법이 7일 실측에서 일치)**: `차익순매수위탁금액 = idx19 − idx18`
      = `idx11 − idx5`. → **B안 진행에 걸림돌 아님.**
      (`idx19`·`idx37`도 대사 통과했고 둘의 합이 TR 자신의 `idx55`와 일치해 현행 수집이
       옳음도 함께 확인됐다.)

      **진행 순서**
      1. ~~여러 날짜 덤프로 전 필드 대사~~ **[DONE 2026-08-09] 7일 완료.** §4 참조:
         `docs/CyBos ref/CYBOS_프로그램매매_투자자별_TR_명세.md`
      2. 후보 피처 산출 — 예: `program_arb_gross`(매도+매수, 활동량),
         `program_prop_ratio`(자기/합계, 프랍 개입도),
         `program_arb_imbalance`(매수−매도 ÷ 매수+매도, 스케일 무차원화)
      3. `scripts/ic_probe_pending_features.py` → `pending_validation` 등재 →
         purged WFA 게이트 (**기존 절차 준수, 건너뛰지 말 것**)
      ⚠ 신규 피처는 과거 DB에 없다 — 다만 **`CpSvr8116`(프로그램 일별매매종합)으로 일 단위
        백필은 가능**하다(451차 후속3에서 확인). 분 단위는 여전히 앞으로만 쌓인다.
      ✅ 단위는 **백만원 확정**(공식 명세 "금액", 전체매수 439,555 → 4,395억으로 규모 정합).
        → `_INV_LOG_COLS` 주석의 "계약수"는 확정적으로 틀렸다(아래 항목). 산식은 그대로 둘 것.
...
> 근거: `dev_memory/DECISION_LOG.md` 2026-08-09(450차).

- [ ] **라이브 1일 확인** (P1, 다음 거래일)
      `opt_available=1`이 장중 유지되고 `[PCRStore] 저활동 스킵` DEBUG가 **장초반 몇 분에만**
      찍히는지. 장중 내내 찍히면 `PCR_MIN_ACTIVITY_ABS=100`이 과대한 것이다.
      ⚠ 62일 재생은 **오프라인**이다 — 371차 PSI 수정 때와 같은 구조(재생 OK / 라이브 미검증).
- [ ] **gross PCR 승격 조사** (P2) — 🔴 **[451차 후속3 정정] 차단 사유가 해소됐다**
      `CpSvrNew7221` 행3(옵션콜)·행4(옵션풋)의 **열3=외인매도 / 열4=외인매수**를 파싱하면
      net의 두 한계(캡 고착 21.3%, 중앙값 1.59 편향)가 함께 풀린다.
      ~~🔴 로그에 gross가 기록된 적이 없어 오프라인 검증이 불가~~ → **틀렸다. 이미 있다.**
      `_probe_investor_tr`가 **세션당 1회** 7221 5행×15열을 통째로
      `logs/YYYYMMDD_PROBE.log`(`[CybosProbe][RAW]`)에 남기고 있었고, **40거래일치** 확인됨.
      실측(콜·외인, 2026-07-27~08-07): **gross 매도 933~5,472 / 매수 529~5,372**인데
      현재 쓰는 **net은 −13~−1,469** — `PCR_MIN_CALL_ABS=1000`이 net에서 상시 탈락하던
      이유가 수치로 확인된다(450차 진단과 정합).
      → **라이브 probe 없이 지금 바로** 40일 오프라인 스케일·안정성 검증 가능.
      ⚠ 한계: 덤프는 **하루 1건(09:02 스냅샷)** 이라 일중 분포는 못 본다. 일간 스케일
      판정에는 충분하지만 가드 임계 재설정에는 병행 기록이 여전히 필요하다.
      ⚠ 교체는 산식 변경이므로 train/serve skew를 만든다(317차 원칙) — 즉시 갈아타지 말 것.
- [ ] **임계값 재보정은 별건으로 분리** (조건부)
      `PCR_BEARISH 1.2` / `EXTREME 1.5` / `EXTREME_BULLISH 0.67`은 교과서값이며 이 시스템
      데이터로 캘리브레이션된 적이 없다. **가드 수정과 묶어서 바꾸지 말 것** — 372차가 PSI에서
      이상치로 임계를 정당화할 뻔한 것과 같은 함정이다. 재시도 시 372차 방법론
      (일자단위 상관 + 손익 구간별 이상치 분해)을 먼저 적용할 것.
      ⏰ 선행조건: gross 승격 여부 확정(승격하면 임계 자체가 다시 정의된다).
- [ ] (읽는 법) **"opt_pcr_*가 DB 전 구간 0"을 이 가드 탓으로 다시 진단하지 말 것**
      06-09~07-13 전멸은 **백필 덮어쓰기**(417~418차 사고) 흔적이다 — 06-30은 가드를
      전 행 통과하는데도 0이고 `feature_quality_score=0.3`(백필 마커)이 붙어 있다.
      가드가 죽인 날은 **08-04·08-07(전면)과 07-21·07-29(부분)** 이다.

### 🔴 449차 — 2026-08-08 주간회의 추인 7건 후속 (MW0601, 2026-08-08)

> 추인 자체는 완료(레지스트리 6건·사전등록 2블록·📌 마커 6채널). 아래는 **남은 작업**이다.
> 근거: `docs/정기점검/금요일점검/MW0601/0808_주간회의_검토보고_MW0601_잔여채널.md`

- [ ] **R2 선행 — [8]에 일자단위 짝지은 부호검정 이식** (P1, 다음 주 전까지)
      `VALIDATION_CAMPAIGN["kelly_skip"].block_gate_day_sign_p`는 등록됐으나 **계측이 없다.**
      `[37]`(`hurst_meanrevert_drag`)의 일자단위 검정을 그대로 가져올 것.
      ⚠ **이식 전에는 어떤 n에서도 차단을 안건으로 올리지 말 것.**
      ⏰ MW0601이 **다음 주 n=20 도달 유력**(현재 19)이라 그때 FAIL이 뜬다 —
      검정 없이 FAIL만 보면 곧장 차단으로 갈 위험이 바로 그 시점에 생긴다.
- [ ] **R7 D8 — [33] 급행풀스톱 판별자 Phase A 재실행** (P2, 오프라인·무위험)
      양 PC 중간 트리거 도달(MW0601 FAST 34/30·유효 11일 / MW0602 32/30·유효 12일).
      마지막 실행 2026-08-03 결과는 BH FDR 10% 통과 **0개**.
      ⚠ 또 0개면 `faststop_discovery` "잠정 음성" **유지** + **본 트리거(FAST 40/유효 15일)
      까지 재론 금지**가 사전등록된 조건이다. **Phase D(틱 그물)로 넘어가지 말 것** —
      그것은 재실행도 전멸일 때만이고 구현이 무겁다.
- [ ] **R4 감시 — [45] 트리거 2주 연속 여부 확인** (매주, 리포트만 보면 됨)
      양 PC 각각 conf 점프 중앙값 ≥ 0.10 **그리고** 전환 ≥ 4.0회/일.
      현재 MW0602 충족 / MW0601 점프폭 **0.0162**로 미충족.
      ⚠ 도달해도 **PC별 분기 배포 금지** — 양 PC 동시 배포 / 전면 보류 둘 중 하나다.
      ⚠ 🔴 `DEGENERATE_AUC_MIN`·보정 함수형은 **건드리지 말 것**(430차 금지선).
- [ ] **R1 감시 — [5]는 상시 FAIL이 정상이다** (매주, 조치 없음)
      `_campaign_start()` 고정으로 롤오프가 없어 07-09·07-10 두 레그가 영구히 trend
      버킷을 지배한다. **매주 뜨는 trend 1.20→1.10 후퇴 권고를 실행하지 말 것.**
      재론 트리거는 `start_date` 이동 또는 **이상치 제외 기준으로도** trend 평균 음수.
- [ ] **[42] 해제 조건 감시** — 양 PC가 같은 방향으로 수렴하면 판정보류 해제 검토.
      그전까지 "타점에 가치가 있다"를 근거로 새 진입 피처·게이트를 얹지 말 것.
- [ ] (확인만) **[46] 정지 해제 상태** — 441차가 `no_replay_targets`에 등록해 이미 풀렸다.
      0807 리포트의 `SUSPENDED` 표시는 440차 세대 잔재다. 다음 리포트부터 정상 판정
      (현재 실질 INSUFFICIENT: OOS MW0601 18건/2일 · MW0602 40건/2일, 기준 40건/10일).
      **"정지된 채널"로 다시 안건화하지 말 것.**
...
### ✅ 0807 P0 — 스케줄러 BLAS 결함 **[DONE 2026-08-08, 448차]**

`utils/dll_bootstrap.py` 신설 + 진입점 2곳 배선 + 조용한 실패 차단으로 해결.
스케줄러 환경 정밀 재현에서 리포트 생성 **rc=0 / 152KB** 확인(수정 전 0xC06D007F 즉사).
정상 환경 산출물과 **생성 시각 한 줄 말고 완전히 동일** — 무회귀.

🔴 **아래 원 진단 2건은 틀렸다 — 재인용 금지.**
- ~~"스케줄러 작업이 rc=3228369023으로 실패"~~ → 작업은 **rc=0으로 성공**했다
  (LastResult 0x00000000). 죽은 것은 **자식 프로세스**이고 부모가 삼켰다.
  **스케줄러 상태만 보면 정상으로 보인다 — 이것이 2주간 못 알아챈 진짜 이유다.**
- ~~"`os.add_dll_directory`로 고칠 수 있다(조치안 ②)"~~ → **실측 기각.** MKL delay-load는
  `LoadLibrary`(PATH만 참조)를 써서 add_dll_directory 목록을 보지 않는다.
  **PATH 수정만 유효**하다(append·prepend 둘 다 동작).

- [ ] **`retrain_eod.py`의 자식 실패 종료코드 전파** — 448차 조치 2가 **로그로는**
      시끄럽게 만들었으나 **작업 스케줄러 이력에는 여전히 성공(rc=0)으로 남는다.**
      EOD 체인 전체의 실패 정책이라 별도 안건. 스케줄러 이력만 보는 사람은 아직도
      결번을 놓친다.
- [ ] **MW0602에 448차 전달** — 그 PC는 `MireukiEODRetrain` 미등록이라 지금은 무사하나,
      **등록하는 순간 같은 사고가 난다.** 코드가 먼저 가 있어야 한다.

### 🔴 (원 항목 보존) 0807 P0 — 스케줄러 PATH 결함 (MW0601, 2026-08-07)
...
### ✅ 2026-08-08 주간회의 확정 — 기하 A/B 4채널 (D1~D4 승인 완료)

D1 [3-B] 승격 폐기 / D2 [23-B] 현행 유지(거래집합 차이로 재분류) /
D3 [25] 미적용 유지 + `atr_lock_0.75` 감시 승격 / D4 [25-B] 판단 보류.
레지스트리 4건 갱신 완료. 상세는 `DECISION_LOG.md` 2026-08-08 주간회의 항목.

- [ ] **D3 표본 감시 — `atr_lock_0.75`** 사전등록 목표 **PC별 70훅/13거래일**
      (`watch_min_hooks_per_pc` / `watch_min_days_per_pc`). 현재 MW0601 **33훅/4일**.
      ⚠ **도달 전에는 어떤 수치가 나와도 적용 안건으로 올리지 말 것**(313차).
      ⚠ 도달 시 3종 함께 제출: (a) 두 PC Δ현행·건별우세 (b) 체제별 부분집합 판정
      (c) **클램프 발동률**(현재 11/33=33% — 단독 기각 사유가 될 수 있다).
- [ ] **D4 재검토 조건** — MW0601 [25-B] 표본이 MW0602 수준(70훅/13일) 도달 후.
- [ ] **승격 대기 후보가 0개다** — 감사 §5 활성화 순서 4개가 전부 소진됐다(D1).
      **이 사실 자체가 다음 회의 안건**이다. 새 후보를 어디서 찾을지 결정 필요.
- [ ] **D5 `replay_regime` PC별 분기 — 아직 미승인.** 아래 445차 항목 참조.
- [ ] **🔴 D6 스케줄러 PATH 결함 — 아직 미조치.** 아래 0807 P0 항목 참조.
      이것이 해소되지 않으면 **다음 주 PC 대조가 또 불가능하다**(MW0601 리포트 결번).
...
- [ ] **[주간회의 안건] CB② 복원 시 손절1차 이벤트 이중계상 감안** — 포지션당 손절
  ...
  처리되기 전엔 삭제하지 말 것.
  - **2026-08-14 라이브 최초 실측 — 상세 470차.** 실제 손실 포지션 2건에 카운터 3
    (13:07:27 손절1차 → 1회 / 13:07:31 하드스톱 → 2회 **같은 포지션** / 14:41:43 → 3회).
  - **2026-08-14 470차 C1 로 CB② 쪽은 조치됨** — `record_stop_loss(is_partial_leg=True)`
    는 카운터를 올리지 않는다(`record_win` 도 대칭으로 리셋하지 않는다).
    회귀 테스트 `tests/test_470_cb2_position_unit.py`.
  - **남은 안건은 Kelly 축이다** — `kelly.record()` 는 여전히 레그를 센다.
    470차 C2 실측상 승률 차가 -0.9%p로 작아 **의도적으로 바꾸지 않았다**(사이징 직결,
    313차 ④ 사전등록). 표본이 쌓이면 재론할 것.
...
## 2026-08-14 (MW0602 470차 후속 — 0814 장후 2차 점검)

리포트: `docs/정기점검/매일점검/MW0602-20260814-점검리포트.md` §장후(post) — 2차
근거: `dev_memory/DECISION_LOG.md` 2026-08-14 470차 후속 항목

### F — Fix (승인 후 착수, 코드 변경 0건 상태)

- [ ] **F-p2-1 (P1) CLAUDE.md 실전전환기준 ⑧ 본문 정정 — [28]은 ⑧ 해제 전엔 안 켜진다**
  - ⑧의 *"431차 조치로 qty≥3 체결이 다시 발생한다 … 지금부터 표본이 쌓인다"* 를
    **취소선 + 정정 블록**으로 교체(468차 CB③ 병기 형식, **원문 삭제 금지**)
  - 정정 내용: 431차 이후 8거래일 **74포지션 qty≥3 0건** / 마지막 qty≥3은 **2026-07-31** /
    실효 상한은 `MAX_CONTRACTS=3`이 아니라 **브로커 증거금 2**(`main.py:15757`)
  - ⑧ 해제 순서 명시: `실전 자본 재설정 → 증거금 상한 상승 → [28] 표본 축적 → 판정`
  - `config/settings.py:MAX_CONTRACTS` 정의부 주석에 **"현재 무발동(죽은 파라미터)"** 병기
  - `scripts/generate_validation_campaign_report.py` [28] 절 `⚠구조적판정불가` 에 사유 1줄
  - 코드 동작 무변경. 절대원칙 저촉 없음

- [ ] **F-p2-2 (P1) 수집기 §5 를 포지션 단위로 렌더링** (1차 Fix ③ 흡수)
  - `collect_evidence.py` — `체결부분청산` 패턴 추가 + §5를 `GROUP BY 진입시각`으로
    (`포지션 / 레그수 / 합계pt / 합계원 / 청산사유 체인`), 레그표는 `<details>` 안으로
  - **표 머리글에 `(포지션 단위)` 를 박아 넣을 것** — 417차가 `n(이벤트)` 표기를 했는데도
    읽는 쪽이 놓쳤다. 단위를 제목에 넣는다
  - 전략경보 `오늘 PnL`과 **자동 검산**, 불일치 시 §11 적신호 승격
  - 검증: 0814 로그 재실행 → `4포지션 / 7레그 / -93,449원`

- [ ] **F-p2-3 (P2) 다이제스트 덮어쓰기 방지** — 오늘 실제 발생(15:53본이 16:22에 덮임)
  - `--out-auto` 경로 존재 시 기존본을 `..._<HHMM>.md` 로 rename 후 stderr 경고
  - 같은 날 같은 국면 보존본 FIFO 3개

- [ ] **F-p2-4 (P2) `[SizeCut]` 통합 로그** (1차 Fix ④ · 장중 G-intra-3 흡수)
  - `main.py` — `[SizeCut] 사이저=N → 최종=M (사유=margin|quality_gate|max_contracts|none, …)`
    **축소가 없어도 `사유=none` 을 남긴다**(부재를 부재로 관측)
  - ⚠ 이것이 **G-p2-2의 선행 조건** — `[MarginCap]`은 조건부 로그라 §12에 못 넣는다
    (`invariants.md` §5)
  - 적용 시점: 다음 기동. `py37_32` 라이브 경로. COM 콜백 밖(절대원칙 ④ 무관)

### G — 고도화

- [ ] **G-p2-1 실효 상한 분해표** — `min(MAX_CONTRACTS, MarginCap, 품질게이트)`
  - EOD 리포트 또는 수집기 §5에 `사이저 산출 / MarginCap 상한 / 품질게이트 min() / 최종 qty`
    분포와 **그날의 binding constraint** 를 일자별로
  - 목적: ⑧ 해제 시 "자본을 얼마 올리면 [28]이 켜지는가"를 역산
  - 선행: F-p2-4. 우선순위: 이번 주(배포 후 1주 관측)

- [ ] **G-p2-2 수집기 §12 고착 감시에 사이징 축 3종 추가**
  - `sizer_conf_mult` · `sizer_safe_mult` · `margin_cap_qty`
  - ⚠ `신뢰도배수`는 431차가 이미 규명·계측 중 → **`benign`(정상고착·기록됨)으로 등록**.
    목적은 발견이 아니라 **"규명된 고착도 계속 보이게 두는 것"**(규명 사실이 잊히면 재발견한다)
  - 선행: F-p2-4(무조건 로그). 26주 WFA 갱신 대상으로도 등록

- [ ] **G-p2-3 장후 점검을 EOD 체인 완료에 걸어 발화**
  - 오늘 장후 1차 15:51 vs EOD 완료 16:02:32 → 캠페인 판정 리포트(165KB)를 1차가 못 열었다
  - `data/eod_retrain_done_{d}.txt` + 캠페인 요약 라인 폴링(상한 17:00). 차선책은 **16:10**
    (EOD 총 소요 오늘 17분 30초)
  - ⚠ **예약작업은 git으로 공유되지 않는다**(함정③) — **MW0601에도 따로 등록할 것.**
    이 줄이 유일한 전달 경로다

### O — 다음 거래일(2026-08-17) 관측

- [ ] **O2-1** `[EntryGate] … 사이즈 제한 해제 (×0.6 → ×1.0)` 재출현 + `사이즈 축소 ×0.6` 0건
  → **2일 연속이면 468차 G-1/F-1 라이브 검증 완료 선언**, `invariants.md` §2-B에서 관측 대기 해제
- [ ] **O2-2** 사이저 3계약 산출 시 `[MarginCap]` 동반 여부 → 안 찍히면 C-3의 (a) 로그 누락 확정
- [ ] **O2-3** `trades.db` qty≥3 포지션 발생 여부 → **1건이라도 나오면 발견①의 "구조적" 서술 완화**
- [ ] **O2-4** `[TickLossTier1Qty1] … 실적용=False` 건수 + 해당 포지션 손절 준수율 → [11] 일자 표본 +1
- [ ] **O2-5** `[GuardFair] … 무효` 비율 → 3거래일 연속 6/6이면 §12 감시 항목 승격 검토
- [ ] **O2-6** WeightCollapse 비율(O-8 밴드 20.3%±5%p) + `진입후보 분` → [44] 진입률 추이 갱신

### [470차 후속2] 0814 리포트 종합검토 후 일괄 구현 — 잔여 항목

**사용자 조작 필요 — 예약 위치 확정: `Claude 데스크톱 앱의 Cowork 예약 태스크`**

> 세 후보를 전부 확인했다. Windows 작업 스케줄러 ❌(`Maitreya`·`Maitreya_EODretrain`만 있음) /
> claude.ai 클라우드 루틴 ❌(`RemoteTrigger list` → 비어 있음) /
> **데스크톱 앱 Cowork ✅** — 증거는 다이제스트 헤더의 리포 경로
> `/sessions/charming-gracious-mccarthy/mnt/futures` (로컬 폴더 마운트).
> 이 세션에서는 조회·수정 API가 없어 **데스크톱 앱에서 직접** 바꿔야 한다.
> ⚠ 예약은 git으로 공유되지 않는다 — **MW0601에도 따로 등록할 것.** 이 기록이 유일한 전달 경로다.

- [x] ~~**A1 장전 예약 08:50 → 09:02**~~ → **[2026-08-14 사용자 적용 완료]**
      0814에 08:50 실행이라 08:55 매크로·PreRetrain·**O-1의 F-1 해제 로그**
      (`08:55:02 [EntryGate] ×0.6 → ×1.0`)가 다이제스트에 하나도 안 잡혀 사람이 원본
      로그로 메웠다. `phases.md` A-2(08:55 매크로)는 장전 점검 필수 항목인데 구조적으로
      관측 불가였다. 09:02면 08:58 레짐 확정 + 09:00 첫 파이프라인 1사이클까지 포함된다.
- [x] ~~**A2 장후 예약 → 16:30**~~ → **[2026-08-14 사용자 적용 완료]**
      0814 장후 점검이 15:51에 돌았는데 EOD 체인은 16:02:32에 끝나 캠페인 판정 리포트
      (165KB)를 1차가 못 열었고, [44] 판정 조건 도달을 늦게 발견했으며 점검이 1·2차로
      쪼개졌다. 16:30은 0814 EOD 총 소요 17분 30초(15:45→16:02) 대비 여유 있는 값이다.
      · **남은 개선(선택)**: 고정 시각 대신 `data/eod_retrain_done_{d}.txt` + 캠페인 요약
        라인 폴링으로 발화(17:00 타임아웃). 재학습이 30분을 넘기는 날이 생기면 그때 전환

- [ ] ⚠ **MW0601에도 같은 시각 등록했는가 확인** — 예약은 git으로 공유되지 않는다.
      MW0602만 바꾸면 두 PC의 점검 관측 범위가 달라져 리포트 대조가 어긋난다

**[470차 후속7] 동시 세션 충돌 — 2026-08-14 저녁에 실제로 3건 터졌다**

> 이 세션(VSCode)과 Cowork 예약 세션이 **같은 워킹트리를 공유**하면서 생긴 사고들이다.
> 셋 다 데이터 유실은 없었으나, 몰랐으면 유실됐을 것들이다.

- [ ] **`reset` 이 남의 커밋을 잘랐다** — 3차 점검 세션이 자기 커밋(`9ba1eb7`) 직후
      `reset HEAD~1` 을 했는데 그 사이 이 세션의 `68d1413` 이 위에 얹혀 있어
      **남의 커밋이 잘렸다**(reflog `HEAD@{0}: reset: moving to HEAD~1`).
      내용은 워킹트리에 남아 `483d41a` 로 재커밋했다.
      → **규약 제안**: 공유 워킹트리에서 `reset` 전에 `git log -1` 로 HEAD 가 자기 커밋인지
        확인할 것. 점검 스킬 문서(SKILL.md/phases.md)에 넣을지 검토
- [ ] **샌드박스 세션이 쓰면 줄바꿈이 뒤집힌다** — 3차 세션(리눅스, autocrlf 미설정)이
      `dev_memory/*.md` 를 **LF 로 재작성**해 `9ba1eb7` 이 **+35,940/−35,746** 가짜 diff가 됐다.
      되돌리며 `483d41a` 도 같은 크기가 났다(현재는 repo 원래 규약 CRLF 로 복귀).
      실제 내용 변경은 189줄뿐이었다(`--ignore-cr-at-eol`).
      → **470차 S1 이 읽기 측(수집기 미커밋 집계)을 고쳤다면 이건 쓰기 측이다.**
      → **결정 필요**: `.gitattributes` 에 `dev_memory/*.md text eol=crlf` 류를 넣어
        환경과 무관하게 고정할 것인가. 두 PC 모두 Windows 라 안전하지만
        **최초 1회 재정규화 커밋이 또 크게 난다** — 주간회의 안건으로 올릴 것.
        ⚠ 470차가 거부한 "리포 전체 `--renormalize`"(520건 오탐 대응)와는 **다른 건**이다.
        그건 없는 문제를 518파일로 고치는 것이었고, 이건 실재하며 반복되는 churn 이다
- [ ] **git 락이 하루 3회** — `index.lock` 08:50:53 / 19:35:53, `HEAD.lock` 19:47:35.
      전부 0바이트·git 프로세스 없음(스테일). 08:50 락은 **그날 종일 로컬 git 쓰기를 막았다.**
      동시 접근원이 셋이다: 이 세션 · Cowork 예약 세션 · **VSCode git 확장(프로세스 13개)**.
      → 예약 점검 스크립트 시작 시 스테일 락 정리 단계를 넣을지 검토

**08-17(월) 장전 — A1 변경의 첫 검증이자 G-1 최초 검증일이 겹친다**

- [ ] **A1 효과 확인**: 다이제스트에 `08:55:02 [EntryGate]` · `08:58 레짐 확정` ·
      `09:00 첫 파이프라인`이 **자동으로** 잡히는가 (사람 보강 없이)
- [ ] **I-3 G-1 최초 검증**: 사이드카 `label_scheme` **불일치 0/6** + F-1 마커 없이도 해제.
      ⚠ 08-14에 해제한 것은 **F-1**이지 G-1이 아니다 — 두 축을 구분해 읽을 것
      (`invariants.md` §2-B 470차 D1 규약).
      **A1을 08:50 그대로 뒀다면 이 검증에 필요한 로그를 또 놓쳤을 것이다**

**다음 거래일(2026-08-17) 관측 — 470차 배포분 라이브 검증**

- [ ] **진입 건수 재확인** — L1(`[ZeroDiag]` 존금지 1급화)은 로그만 바꿨지만 조립부가
      **진입 판단과 같은 함수 안**에 있다. 회귀가 나면 여기서 난다
- [ ] `grep -c "ConfFloorGuard] state=" ` ≈ 파이프라인 분수(370±5) — L2 상태 샘플 동작
- [ ] `[CORE준비도] 축퇴 N/M` 이 매 refit 마다 찍히는가 — B4. §12에서 `무기록` 탈출 확인
- [ ] 11:50~13:00 구간 `[ZeroDiag]` 에 `존금지` 접두 100% — L1
- [ ] `[TP비율] TP1 설정=33% 실효=50%` 출현 (부분청산 발생 시) — L3
- [ ] `[Shutdown] 잔존 스레드` + `[EodGhostCheck]` 대조 → 유령 프로세스 원인 스레드 특정 — L4·A3
- [ ] `[JointGateEdge]` 누적 — **최소 30건까지 어떤 결론도 내지 않는다** (D1')
- [ ] **`.git/index.lock` 재발 여부** — 0814 08:50 예약 점검이 0바이트 스테일 락을 남겨
      **그날 종일 git 쓰기가 불가능**했다(470차 후속2에서 제거). 샌드박스 점검이
      락을 남기는 구조면 매일 재발한다 — 재발 시 예약 스크립트에 락 정리 단계 추가 검토

**주간회의로 넘긴 것**

- [ ] **Kelly 레그 계상** — CB②(C1)는 포지션 단위로 고쳤으나 `kelly.record()` 는 여전히
      레그를 센다. 470차 C2 실측 승률 차 **-0.9%p**(레그 62.2% / 포지션 63.0%)로 작아
      **의도적으로 바꾸지 않았다**(사이징 직결, 313차 ④). 표본 축적 후 재론
- [x] ~~**손절 오버슛 대조군**~~ → **[470차 후속3 완료] 가설 반증. 조치 없음.**
      매칭 대조군(같은 거래일·같은 방향·**같은 거리 D**, 처치 ±N분 제외) 구현 후 실측:
      · 신호단위 15분 — 처치 **59.6%** vs 대조 **66.1%** = **−6.4%p** (처치가 오히려 낮다)
      · **일자단위 부호검정(주판정)** — 처치 우세 6일 / 대조 우세 10일, **p=0.4545 (비유의)**
      · 드롭-워스트(313차 ③) 후에도 부호 유지(−4.6%p, p=0.6072)
      · 시드 3종(470/1/99)·대조 50개에서도 −4.5~−6.0%p로 안정
      **→ 최초의 80.2%는 손절 지점의 특성이 아니라 이 시장의 기본 되돌림 성질이다.**
      원 모집단으로 재현하면 처치 80.8% vs 대조 **80.1%** = **+0.7%p (p=0.6291)** —
      대조군이 거의 정확히 같은 값을 낸다.
      ⚠ **"손절폭을 넓혀라"의 근거로 쓸 수 없다.** 371·372차에 이은 3번째
      "숫자는 커 보였지만 아니었다" 사례로 기록.
- [ ] **`trades.tp1_reached` 소급 백필 검토** — 470차 후속3에서 드러난 별건.
      최근 20거래일 손절 라벨 131건의 `tp1_reached` 분포가 **{None: 129, 0: 2}** 다
      (컬럼이 2026-08-13부터 적재). `_is_real_stop` 류의 `if tp1_reached:` 필터가
      NULL 에 대해 **아무 일도 하지 않아** 보호 트레일 이익 청산 79건이 "진짜 손절"로
      통과했다. 465차 P4 계열 채널·사후검증이 같은 결측을 공유하는지 점검할 것.
      ⚠ 백필은 과거 판정을 바꿀 수 있으므로 **사전등록 절차를 거칠 것**(313차 ④)

### 주간회의 안건 (일일 점검에 결정 권한 없음)

- [ ] **[44] DynMC 붕괴행 잠식 — 안건 등재.** 판정 조건 도달(이탈 +10.08%p, 일자 11/11일 p=0.001)
  했으나 `VALIDATION_CAMPAIGN_DECISIONS` 에 항목 없음. ⚠ **자동 배선 금지** —
  배선 시 통과율 24.9%→35.0%, [39]가 `min_conf`를 무작위 샘플러로 확정해 기대값 부호 무보장.
  **⑧ 미해제 상태에서 진입 증가 = 노출 증가**
- [ ] **CB② 복원 재검토(2026-08-29, D-15) 안건을 "Fix ① 적용 후 9999→2~3"으로 재기술**
  — 장후 1차 1-post-1(연속손절 카운터 이중계상 라이브 최초 실측) 반영. NEXT_TODO:290·3028 두 항목 모두
- [ ] **26주 WFA 재검증 목록에 `GuardFair 무효율 추세` 신설** — 오늘 6/6 무효(1일 표본, 판정 보류)

### 완료 처리

- [x] **O-1 (468차 G-1 라이브 검증)** — **[충족 2026-08-14 · 1일]** `사이즈 축소 ×0.6` 0건 확인.
  ⚠ **2일 연속 확인(O2-1) 후 종결** — 그 전에는 CLAUDE.md ⑧ 해제를 진행하지 말 것

## 2026-08-14 (MW0602 470차 후속5 — 0814 장후 3차 점검)

> 3차는 **코드 변경 0건**. 아래는 전부 승인 후 착수. 2차(O2-1~6)는 그대로 유효하되
> **O2-1의 판정 기준만 3차가 정정**했다(`×0.6` 0건 → `불일치 0/6`).

### F — Fix (승인 후 착수)

- [ ] **F-p3-1** G-1 검증 상태 정정 전파 — P1. `dev_memory/DECISION_LOG.md:23086` 에 포인터
      한 줄(**완료**, 후속5에서 적용) + 점검리포트 §7-post3 무효 선언(**완료**).
      **잔여**: 08-17 장전 점검이 세 문서에서 같은 결론(⏳미검증)을 읽는지 확인. 원문 삭제 금지.
- [ ] **F-p3-2** 런처 GUARD 진단을 로그에 남긴다 — P1. `start_mireuk.bat:457` 출력을
      임시파일→`CALL :L` 재출력(⚠ `ERRORLEVEL`을 리다이렉션 **직후 즉시** 보관 — `FOR`가 덮어쓴다)
      + `scripts/eod_ghost_check.ps1` 매칭을 `-match '(?i)python'` 으로 확대(WARN에 cmdline 전문 병기).
      🔴 **매일 08:40 자동기동 진입점** — **주말에만** 적용 · 드라이런 2회(유령 없음/더미 있음) 필수.
      A3 계약(**항상 exit 0**) 유지. → O3-1
- [ ] **F-p3-3** `scripts/aggregate_and_backfill.py:33-48` 로깅 초기화를 모듈 최상위 →
      `__main__`/`_setup_logging()` 으로 이동 — P2. import만으로 `logs/<날짜>_BACKFILL.log` 0B가
      생겨 당일 인벤토리를 오염. 적용 전 호출자 전수 확인(현재 `tests/test_452_…:251` 1곳).
      런타임 py37_32(3.7 문법 유지). 검증: 해당 테스트 실행 후 로그 파일 **미생성**.
- [ ] **F-p3-4** 리포 루트 정리 + push — P2. `.gitignore` 에 `collect_stderr.txt` 추가 /
      `MIREUK_DAILYCHECK_HANDOFF.md` 커밋 또는 삭제 / `evidence_*_{pre_0850,post_1622}.md`
      보존본 커밋 / **`git push origin dev`(ahead 9 해소)**. → O3-4

### G — 고도화

- [ ] **G-p3-1** **정정 전파 체크리스트** — `references/correction_targets.md` 신설:
      판정을 뒤집을 때 반드시 함께 훑을 5곳(① `CLAUDE.md` ② `invariants.md`
      ③ `DECISION_LOG` 해당 항목 포인터 ④ 당일 리포트 절 ⑤ `VALIDATION_CAMPAIGN_DECISIONS`).
      + **수집기 기계 검사 1개**: `invariants.md` §2-B의 `⏳미검증`/`✅검증완료` 토큰 ×
      `DECISION_LOG` 최근 30일 `[확인]` 헤딩 대조 → 엇갈리면 §11 적신호.
      ⚠ 오탐 대비 `benign` 화이트리스트 동반. 우선순위 **이번 주**.
      근거: 오늘 하루에 같은 실패 2회(469차 선반영 → 한 파일만 철회 / 2차 §0 정정이 1,000줄 아래).
- [ ] **G-p3-2** 종료 완결성을 양 끝에서 재고 매칭 폭을 하나로 —
      ① `scripts/proc_scan.py`(py37_32 무의존) 신설해 GUARD·A3가 같은 규칙 호출
      ② A3를 **EOD 직후 + 심야 23:30** 2회로 늘려 유령 생성 구간(현재 15시간 창)을 좁힘
      ③ `main.py:_auto_shutdown()` L4 잔존 스레드 덤프와 **같은 파일**에 시간순 기록.
      **선행조건 F-p3-2** — 그게 없으면 ①이 대조할 대상이 없다. ②·③은 O3-1 결과 후 착수.

### O — 다음 거래일(2026-08-17) 관측

- [ ] **O3-1** `logs/Mireuk_batch/launcher_20260817_*.log` 의 `[GUARD] 실행 중 main.py: N`·`PID=… cmd=…`
      → cmdline이 `main.py` 실체면 **(b) 유령 실재** / 다른 프로세스면 **(a) GUARD 오탐**. 1일이면 갈린다
- [ ] **O3-2** `logs/eod_ghost_check.log` 2번째 행 — 08-17 저녁도 `잔존 0` 이고 O3-1이 (b)면
      **유령은 야간 생성** → G-p3-2 ② 착수
- [ ] **O3-3** 수집기 §12 `ConfFloor`·`CORE준비도` — **기록 발생(무기록 해소)이 정상.**
      08-17에도 0건이면 그때 비로소 P1(L2/B4 배선 실패). ⚠ 그 전엔 결함으로 올리지 말 것
- [ ] **O3-4** `git status -sb` → `[ahead 0]` · 미추적 0건
- [ ] **O2-1 정정** — G-1 판정 기준은 `×0.6` 0건이 아니라 **`[EntryGate] … 불일치 0/6`**.
      첫 기회 2026-08-17 08:55(오늘 EOD `horizons_replaced 6/6` 로 재료는 갖춰짐)

### 26주 WFA 재검증 목록 — 추가 제안

- [ ] **C6' 종료 완결성 점검 2종의 매칭 규칙 동기화** — 런처 GUARD와 `eod_ghost_check.ps1` 이
      각자 진화하면 조용히 어긋난다. 오늘이 첫 사례(psutil `'python' in name` vs CIM `-eq 'python.exe'`).
      468차 G-2(죽은 지표)·470차 C1'(열린 채 끝난 지표)와 같은 계열 — **"같은 질문에 다른 자"** 를 잡는다.

### 기한 (변동 없음, 재확인)

- [ ] **CB② 복원 재검토 2026-08-29 (D-15)** — 2차 H-1대로 "Fix ① 적용 후 9999→2~3" 으로 안건 재기술.
      오늘 `[CB] 연속 손절 3회` 까지 오른 것은 **9999 하에서 정상**(카운터만 상승, 정지 없음)

### [470차 후속5→**후속7 개명 필요**] 3차 점검 자체가 만든 문제 2건 — 최우선

- [ ] **부기-1 세션 차수 충돌** 🔴 — `470차 후속5` 가 둘이다(`43ffcdc` A1/A2 예약 / `9ba1eb7`
      장후 3차 점검). 같은 PC의 **동시 세션** 충돌이라 PC명 태그로는 못 막는다.
      → 3차 절과 `DECISION_LOG:23273`·본 NEXT_TODO 항목을 **`470차 후속7`** 로 개명(본문 정정 커밋).
- [ ] **G-p3-3** CLAUDE.md 멀티PC 컨벤션에 **"같은 PC의 동시 세션"** 항 추가 —
      차수 확정 직전 `git fetch && git log -1` 재확인. 예약작업은 **커밋 직전**에 차수 재확인.
- [ ] **F-p3-5 개행 재인코딩 되돌리기** 🔴 P1 — `9ba1eb7` 이 LF→CRLF 전면 재인코딩으로
      12파일 40,273+/39,151- 를 찍었다(실 내용 변경은 133줄). 내가 편집하지 않은 9파일 포함.
      **MW0601(`v9-dev`)이 보면 전 파일 충돌.**
      → ① **Windows 세션에서** `git -c core.autocrlf=input add -- <12파일>` + 정규화 커밋 1건
         (히스토리 rewrite 불필요) ② **`.gitattributes` 신설**(`* text=auto` / `*.bat`·`*.ps1 eol=crlf`)
         — 현재 repo에 `.gitattributes` **없음**. 이것이 근본 해법
         ③ `SKILL.md` §6에 "샌드박스 커밋 시 `-c core.autocrlf=input`" 명시.
      ⚠ 샌드박스에서 `git reset --soft` 는 권한 오류(`HEAD.lock` unlink 불가)로 **적용 안 됐다.**
        동시 세션이 인덱스를 쓰고 있어 히스토리 재작성은 시도하지 않았다.
- [ ] 리포 루트 미추적 `MIREUK_DAILYCHECK_HANDOFF.md` 처리(커밋 또는 삭제) — F-p3-4 잔여
...
> 리포트 `docs/정기점검/매일점검/MW0601-20260814-점검리포트-pre.md` · **라이브 코드 무변경 · 미커밋**
> ⚠ 차수 470은 잠정(원격 `dev`가 469차까지 사용). 사용자 확인 후 확정.

### Fix — 전부 **장후 적용**

- [ ] **F-1 불변식 감시표를 브랜치 인식형으로 (P1)** — `collect_evidence.py` 불변식 스펙 dict에
  `branches: ("dev",) / ("v9-dev",) / None` 키 추가. 현재 브랜치에 없으면 `미발견`이 아니라
  **`해당없음(타브랜치)`** 로 렌더링하고 §11 적신호에서 제외.
  ⚠ **표 하단에 `타브랜치 미적용 N종`을 반드시 남긴다** — `해당없음`이 새 은폐 수단이 되면
  계측 4원칙 ③을 그대로 위반한다.
  **근거**: 462·468차 스위치 5종이 `v9-dev`에 없어 §11 적신호 1~5번을 상시 점유.
  469차(`a581231`)만 체리픽되고 코드(`c63b4ee`·`f2332be`·`ce3d9d9`)는 `origin/dev` 전용.
  **검증**: `v9-dev` 실행 시 적신호 7→2건, `dev` 체크아웃 시 5행 `일치` 복귀.

- [ ] **F-1B 462·468차 스위치 5종의 `v9-dev` 체리픽 여부 심사 (P1, 별건)** —
  🔴 **F-1과 묶어서 자동 진행하지 말 것.** 라이브 동작을 바꾼다.
  특히 `PIPE_LATENCY_EXCLUDE_MODEL_SWAP`(모델 교체 구간을 CB⑤ 판정 지연에서 차감)은
  MW0601에도 값어치가 있으나, `ZONE_ENTRY_BAN_ENFORCE`는 `False`여야 하고
  `MODEL_LABEL_STATE_UNLOCK_ENABLED`는 `dev` 쪽에서도 **라이브 미검증**이다(§2-B).
  **결정 필요 — 주간회의 안건.** merge-base `5e1426c`(2026-07-05, 291차) 이후 310/300 커밋 분기.
...
- [ ] **F-5 `[SizerMatch]` 축소원인 오귀속 수정 (P1)** — `main.py` `[SizerMatch]` 출력부(현행 9002~9010):
  `meta={_meta_size}` → **`_meta_size_sizing`**(사이징 실사용값), 두 값이 갈리면
  `(gate_raw=…)` 병기(431차 `[MetaGate]` 관례). **`quality_min` + `quality_src`(argmin 게이트명) 추가.**
  `kelly=` 뒤에 `(사이저 내 반영)` 표기.
  **근거**: 11:48:00 진입에서 3→2 축소의 진짜 원인은 `hurst=0.50`인데 로그는 `meta=0.50`을 찍는다.
  `main.py:7784/7881`이 쓰는 값은 1.00(meta는 안 깎음), `main.py:8096`의 hurst가
  `_compose_quality_qty`(`main.py:267`) min()을 지배 → `round(3×0.50)=2`.
  실전 전환 기준 ⑧의 근거 채널 [28] `sizing_inversion_watch`가 읽는 값이 이것이다.
  ⚠ `_quality_mults` 빈 dict → `min()` ValueError. `main.py:8105` 기존 가드 관례를 따를 것.
  ⚠ **단독 커밋**으로 분리(라이브 본체 진입경로 인근 유일 항목).
  **검증**: 다음 거래일 첫 `sizer != actual` 진입에서 `round(sizer × quality_min) == actual` 항등.
  **결정 필요**: [28] 과거 축적분 — 권고는 삭제 대신 `meta_attr_suspect=1` 플래그 부착.

- [ ] **F-6 안전장치 로그 이중기록 제거 (P2)** — 호출부를 `log_manager` 단일 경로로 통일.
  확인된 지점: `collection/cybos/api_connector.py:110-115 _system_info()` /
  `:118-123 _system_warning()` / `main.py:13530-13531`(ExitCooldown).
  **전수 조사 대상**: `PipePerf` `CB⑤` `CB③` `CB③-P4` `CybosDailyPnl` `Capital` `ShadowSession` `MarketDNA`.
  **근거**: 수집기 §4 "CB⑤ 8건"의 실제 사건은 4건. 어제도 동일(만성).
  ⚠ 회귀 위험 중 — `system_logger`에만 붙은 별도 핸들러 존재 여부를 **먼저 확인**.
  대안(dedup)은 "같은 초에 진짜 두 번 일어난 사건"을 지우므로 차선.
  **검증**: 하루 로그 인접 동일행 0건. `tests/`에 인접중복 회귀 테스트 추가 권고.

- [ ] **F-7 CB③-P4 상태 전이 양방향 기록 (P2)** — `RESTRICTED → NORMAL` 복귀도 같은 형식으로 로그.
  버퍼 리셋이 stage를 되돌리는 경로면 **리셋 전 스냅샷**(`daily_close()`의 `_ccf_today` 관례).
  `stage_since` 추가 → `[CB③-P4] RESTRICTED 지속 N분 후 NORMAL 복귀`.
  **근거**: 오늘 `NORMAL → RESTRICTED` 2건(10:56:59 acc=13.3% / 12:20:59 acc=0.0%) 사이에
  복귀 전이 로그가 없다. 실전 전환 기준 ⑥ 재판정에 지속시간이 필요한데 지금은 못 낸다.
  ⚠ `CB3_P4_GRADE_BLOCK_ENABLED=False` — **판정 로직·임계 무변경**(절대원칙 ②).

- [ ] **F-8 STEP 3 재학습 주기 문서 정정 (P2)** — `CLAUDE.md` 매분 파이프라인
  `STEP 3: GBM 배치 재학습 (30분마다)` → 실제 트리거 4종(DriftRetrain 조건A/B · ConstOut ·
...
- [x] **F-1 15:10 강제청산 1차 경로 도달성 복구 (P1)** — ✅ **2026-08-14 471차 커밋 `76211c3`.**
  `_ts_run_force_exit_pass()` 신설 + 가드 직전 호출. 유닛테스트
  `tests/test_471_force_exit_reachability.py`(30항목) 통과. ⚠ **라이브 미검증 — F-1R 참조.**
  ~~`main.py:4401~4403` `_on_bar_close()`의~~
  `if is_force_exit_time(now): return` **직전에** `_ts_run_force_exit_pass(self, candle)` 호출 삽입.
  신설 함수는 `_ts_run_maintenance_pass()`(`main.py:14521`) 패턴을 따라 **STEP 8만 평가**하고
  `notify_pipeline_ran()`·`_last_real_pipeline_dt`는 갱신하지 않는다(228차 취지).
  **근거**: 15:09 봉 콜백이 정확히 15:10:00에 와서(`20260814_SYSTEM.log:5284`) 가드가 항상 먼저 걸린다.
  마지막 STEP 8은 15:09:00(15:08 봉)이고 그때 `should_force_exit()`는 False.
  `[TimeExit]`·`[ExitAttempt]` 라이브 0건, `trades.exit_reason='15:10 강제청산'` 전 기간 0건.
  ⚠ **P0 아님** — 453차 D2 스케줄러 안전망이 §1을 커버한다. 다만 최후 방어선이 상시 1차가 된 상태.
  ⚠ **확정 아님(구조적 추론)** — 검증: 시각 주입 유닛테스트 + **모의 1계약 리허설(사용자 판단 필요)**.
  회귀 위험: 15:18 FINAL_CLOSE와 이중 주문(→ `_has_pending_order()` 가드 확인 필수).

- [x] **F-2 강제청산·안전망 하트비트 로그 (P2)** — ✅ **2026-08-14 471차 커밋 `76211c3`**(F-1과 동일 커밋).
  하트비트에 `bar_pass=N회`(F-1 패스 당일 호출 수)를 함께 실었다 — **0이면 F-1 회귀**다.
  수집기 §11 판정 교체 + `always_quote_patterns`에 `[ForceExitPass]`·`[SchedForceExit]`
  추가(**`"안전망"`보다 앞에** — 첫 매치만 채택하므로 순서가 판정을 바꾼다).
  ~~`main.py:14618~14622`~~
  `_ts_scheduler_force_exit_net()`의 `if not _pos_open and _broker_cached <= 0: return False` 분기에서
  **당일 최초 1회만** INFO: `[SchedForceExit] 15:11 점검 — status=FLAT … → 청산 대상 없음(정상)`
  (`self._sched_force_exit_heartbeat_date`로 1일 1회 제한 — 30s마다 찍으면 로그가 죽는다).
  동반: `collect_evidence.py` §11 적신호 6번을 "`강제청산` 문자열 부재" → "**하트비트 또는 실집행
  로그 중 하나가 있는가**"로 교체. **근거**: FLAT인 날 강제청산 경로가 한 줄도 남기지 않아
  "대상 없음"과 "코드 사망"이 로그상 동일하다 — 어제·오늘 연속으로 사람이 수동 판별했다.
  **F-1과 같은 커밋**(같은 뿌리).

- [x] **F-3 `[SizerMatch]` 품질군 전량 출력 + `binding=` 명시 (P1)** — ✅ **2026-08-14 471차 커밋 `8be4048`.**
  `tests/test_471_sizermatch_binding.py`(15항목) 통과. **[28] 채널 파서 수정 불필요 확인** —
  `eval_sizing_inversion_watch()`는 이 문자열이 아니라 DB 포지션(`entry_qty`)을 읽는다.
  ~~기존 F-5의 **범위 확장**.~~
  `main.py` `[SizerMatch]` 출력부(현행 9002~9010): `_quality_mults` 전량 순회 + `min()`을 만든 키를
  `binding=<key>`로, 값은 **사이징 경로 실투입값**으로.
  **근거**: 11:48:00 세 줄(`20260814_SIGNAL.log:3165~3167`) — `MetaGate`는 스스로
  `size_mult_sizing=1.00(무정보폴백→중립)`이라 적었는데 `[SizerMatch]`는 `meta=0.50`을 찍고,
  실제 binding인 **HurstGate(quality_min=0.50)는 필드 자체가 없다**. `3 × 0.50 = 1.5 → 2계약`.
  ⚠ **G-1의 선행** — 오귀속을 고치기 전에 구조체를 만들면 틀린 값이 굳는다.
  ⚠ [28] 채널이 이 문자열을 파싱하면 파서 동시 수정 필요.

- [x] **F-4 Degraded 선제차단 vs 최종 차단사유 이름 불일치 정합 (P2)** — ✅ **2026-08-14 471차 커밋 `82e7554`.**
  `ensemble_decisions`에 `entry_block_axes TEXT`(동시 성립 축 전량, 안정 키 24종) +
  `health_preblock INTEGER` 추가. `tests/test_471_block_axes.py`(18항목) 통과.
  ⚠ **원안 대비 1점 변경**: 목록을 `entry_block_reason`에 누적하지 **않고** 새 컬럼에 담았다 —
  그 컬럼을 부분문자열로 읽는 분류기 3곳(`_categorize_block_reason` ·
  `generate_gate_blocking_report.py` · 캠페인 `LIKE`)의 집계가 조용히 재정의돼
  과거 시계열과 불연속이 생기기 때문(461차 `mdd_pct` 유형). 원안 복귀 시 3곳 동반 수정 + 
  `strategy_events`에 `METRIC_REDEFINITION` 마커 필수.
  ~~차단 사유를 단일 문자열이~~
  아니라 목록으로 누적(`;` 구분) + 선제차단을 별도 컬럼 `health_preblock`(0/1)로 분리.
  `utils/db_utils.py:402` 근방 스키마에 컬럼 추가. **근거**: 09:39·11:24는 로그가
  `Degraded 선제차단`인데 DB 최종 사유는 `등급X — 2_confidence`다(10:39만 Degraded로 남음).
  ⚠ **스키마 변경 — EOD 창(15:40~15:45)에서만.** 장중 절대 금지(2026-08-10 전례). **F-1~F-3보다 후순위.**

### 고도화

- [x] **G-1 사이징 축소 원인을 구조체로 저장 (이번 주)** — ✅ **2026-08-14 471차 후속6 커밋 `ca954b8`.**
  `ensemble_decisions.sizing_trace`(JSON) + [28] `sizer_pressure` 매주 실측화.
  417차 하드코딩 수치(379건 중 86건)를 대체한다. **판정 무관여**(합격선 불변).
  `tests/test_471_sizing_trace.py`(37항목) 통과. ⚠ 2026-08-14 이후 행에만 존재 —
  이전 구간 NULL은 미측정이지 압력 0이 아니다.
  ~~기존 G-4 승계, 근거 보강.~~
- [ ] **G-1 사이징 축소 원인을 구조체로 저장 (이번 주)** — 기존 G-4 승계, 근거 보강.
  `_quality_mults` 전량 + `_meta_size_sizing` · `_qty_sizer_raw` · `_qty_auto` + `binding_gate`를
  `ensemble_decisions.sizing_trace`(JSON)로. [28] 채널 판정을 문자열 파싱 → 구조화 질의로.
  **근거**: 오늘 1건에서 문자열 파싱이 원인 게이트를 잘못 지목했고 실제 원인은 문자열에 없었다.
  **선행: F-3.** ⚠ 스키마 변경 — EOD 창에서만.

- [x] **G-2 `ConstOut(호라이즌)` 일별 카운트를 호라이즌 건강지표로 승격 (이번 주)** —
  ✅ **2026-08-14 471차 후속7 커밋 `211246d`.** 캠페인 채널 **[51]** 신설.
  🔵 **함정 ① 정정**: "일별 테이블 영속화"는 **457차 G5가 이미 구현**했다
  (`scaler_daily.const_out_by_horizon`, 08-12부터 실데이터 3거래일). 그대로 구현했으면
  중복 테이블을 만들 뻔했다 — 남은 절반(진입 성적 결합)만 만들었다.
  🔴 미측정일(NULL)은 heavy·clean 어디에도 넣지 않는다 — 실측 제외 138포지션.
  ⚠ 표본 미달로 당분간 INSUFFICIENT가 정상. `tests/test_471_const_out_horizon_watch.py`(23항목).
  ~~기존 G-6 승계,~~
- [ ] **G-2 `ConstOut(호라이즌)` 일별 카운트를 호라이즌 건강지표로 승격 (이번 주)** — 기존 G-6 승계,
  종가값 확정. `[ModelHealth]`가 이미 내는 `ConstOut 5회/7분 {"3m": {"events":5,"minutes":7}}`를
  DB 일별 테이블로 영속화 + N회 이상인 날의 해당 `entry_horizon` 진입을 별도 버킷 **태깅**(차단 아님).
  **근거**: 오늘 `['3m']` 확정 **4회** → 재학습 4회 → S0 스파이크(최대 2,570ms) → Degraded 4회 연쇄.
  오늘 유일한 진입의 호라이즌도 3m. ⚠ **EOD에서만 집계.**

- [x] **G-3 15:10 강제청산 리허설을 26주 WFA 주기에 편입** — ✅ **2026-08-14 471차 후속8.**
  `CLAUDE.md` "주기적 재검증 항목"에 등재(관측 패턴·정상/실패 판정·비용 포함) +
  실전 전환 기준 **②에 선행 확인사항**("15:10 강제청산 경로 실집행 1회 확인") 추가.
  ⚠ **1회차 리허설 자체는 아직 미실시** — `F-1R`이 그 실행 항목이다.
  ~~별도 캘린더 신설 금지~~
- [ ] **G-3 15:10 강제청산 리허설을 26주 WFA 주기에 편입** — 별도 캘린더 신설 금지
  (CLAUDE.md "주기적 재검증 항목" 도입 취지 — 관리 포인트가 늘면 방치된다).
  **근거**: `exit_reason='15:10 강제청산'` 2026-06-10~08-14 **0건**, `[TimeExit]`·`[ExitAttempt]`·
  `[SchedForceExit]` 라이브 출현 0건. 절대원칙 §1 집행 경로가 6개월간 미실행이다.
  **선행: F-1.** 비용: 리허설 1건 왕복 수수료(오늘 실측 2계약 3,270원 수준).

### 문서·운영

- [x] **실전 전환 기준 ②에 "15:10 강제청산 경로 실집행 1회 확인"을 선행 확인사항으로 추가** —
  ✅ **2026-08-14 471차 후속8** — `CLAUDE.md` ②에 등재. 충족은 F-1R 리허설로만 가능하다.
- [ ] **실전 전환 기준 ②에 "15:10 강제청산 경로 실집행 1회 확인"을 선행 확인사항으로 추가** —
  CB③~⑤는 확인됐으나 절대원칙 §1 집행 경로는 라이브 0회 실행이다(F-1·G-3).
- [ ] **`evidence_MW0601-20260814_pre_1622.md` 수동 삭제** — 수집기 `--pc` 부재로 생긴 부수 산출물.
  근본 해결은 기존 **F-3(수집기 `--pc` / `MIREUK_PC_ID`)** — 오늘로 재발 2회째다.
- [ ] **CB② 복원 8/29 주간회의 상정** — 기한 15일 남음. 오늘 캠페인 `[0] 표본 기아 = OK(주간 32건)`
  이나 `[28]`은 여전히 `⏳INSUFFICIENT`(qty1=110 · qty3+=15, 각 20 필요).

### 다음 관측 (판정 근거)

- [ ] **O-1 EOD `[GuardFair]` `사이드카=현행 train_end_ts 없음` 건수** — 오늘 **6/6**.
  내일 0~1건이면 457차 사이드카 정상 자기치유. 그대로 6건이면 FP-CRITICAL(2개월 PSI=0.0)·
  TOX-SEVERE-SPREAD(한 달 죽은 섀도)와 **동일 결함 패턴 확정**. 판정 2026-08-18(월)
- [ ] **O-2 `WeightCollapse` 분봉 단위 비율** — 오늘 **79/369=21.4%**(`[ModelHealth]` 라인 사용).
  캠페인 [20] 목표 20%±7%p. **3거래일 연속 27% 초과**면 앙상블 가중 딥다이브.
  ⚠ **문자열 등장 횟수로 세지 말 것** — 그렇게 세면 2배가 된다(43.7% 재인용 금지)
- [ ] **O-3 `3m ConstOut` 확정 횟수** — 오늘 **4회**. 3거래일 연속 3회 이상이면 3m 피처셋 딥다이브
- [ ] **O-4 `IntradayRegime` 일일 전이 횟수** — 오늘 **32회**(NORMAL↔DAY_RISK_OFF 18 ·
  DAY_RISK_OFF↔CRASH 10 · NORMAL↔CRASH 4). intra 12:32 19회 → 종가 32회.
  **5거래일 누적 후 G-5(히스테리시스) 판정** — 판정 2026-08-21
- [ ] **O-5 Degraded 선제차단 분봉의 `conf` vs `min_conf`** — 오늘 **4/4 모두 `conf < min_conf`**
  (기회손실 0). 5거래일 중 1건이라도 `conf ≥ min_conf`면 F-1B 체리픽을 손익 근거로 승격.
  ⚠ 1일차 — 313차 원칙 확정 결론 보류
- [ ] **O-6 `_tick_header ≥ 5,000ms` 일일 건수** — 오늘 **6건**(최대 9,125ms @09:01:07 장시작 버스트).
  15건 초과면 G-3(구 항목) 상향. ⚠ **CB⑤ 판정 대상 아님** — CB⑤는 `[PipePerf] total`(오늘 최대 2,830ms)
- [ ] **O-7 RegimeFingerprint `PSI`·`PSI/feat`** — 오늘 **0.007 CLEAR**(cvd=0.151 / vwap=0.007 /
  ofi=0.002). 10거래일 중 CLEAR~WATCH를 오르내리면 실전 전환 기준 ⑦ 복원 판단 근거 성립.
  하루라도 CRITICAL 고착이면 재설계 재검토. 판정 2026-08-28
- [ ] **O-8 `[50]` 방향 편향** — 오늘 앙상블 방향 **SHORT 142 / LONG 33 / FLAT 194**(4.3:1).
  캠페인 [50] 🟠 ALERT_BIAS(3m·5m Bonferroni 생존). 3거래일 연속 3:1 초과면 레이블/학습창 딥다이브
...
- [x] **F-9 `entry_mode` 예외 폴백이 가장 관대한 값으로 조용히 떨어진다 (P2)** —
  ...
  전 행 NULL이면 배선 실패다.
  ~~(원 등록 내용)~~
- [ ] **F-9 `entry_mode` 예외 폴백이 가장 관대한 값으로 조용히 떨어진다 (P2)** —
...
## 2026-08-17 (MW0601 473차 후속 — 장전 점검 / 휴장일) 신규 항목

> 근거: `docs/정기점검/매일점검/MW0601-20260817-점검리포트-pre.md`
> ⚠ 오늘은 KRX 휴장일이라 08-14 배포분(471·472차)의 **라이브 1일차 검증이 08-18로 이월**됐다.

### Fix — 적용 시점 명시

- [ ] **F-1 불변식 감시표를 브랜치 스코프와 분리 (P1)** — `collect_evidence.py` 불변식 항목에
  선택 키 `"branch": ["dev"]` 도입. 현재 브랜치가 목록에 없으면 `미발견`이 아니라
  `해당없음(dev 전용)`으로 찍고 **§11 적신호에서 제외**. **키 부재 = 전 브랜치 적용**이 기본값.
  `references/invariants.md` §2-B에 `적용 브랜치` 열 신설 + 08-14 관측치에 `(MW0602 관측)` 병기.
  **근거**: 462·468차 상수 5종이 `origin/dev`에만 있는데 469차 감시표만 `v9-dev`에 편입
  (`git log --oneline v9-dev | grep -cE "46[28]차"` = 0). 적신호 5건 상시 오점등.
  **검증**: 재실행 시 §11 7건 → 2건. **적용: 다음 거래일 장 마감 후.**

- [ ] **F-2 `[WaitDC]` 조기이탈·하트비트 + EOD 휴장일 스킵 (P1)** —
  `retrain_eod.py:_wait_for_daily_close()`에 ① 대기 전 본체 생존 1회 확인(마커 부재 +
  launcher `Mireuk 종료` → 즉시 `return False`), ② 폴링마다
  `data/eod_retrain_alive_YYYYMMDD.txt` 하트비트. `scripts/eod_retrain.py:main()`에
  `is_trading_day` 게이트(휴장일 INFO + **rc=0**).
  🔴 `py310_64` 전용(191차). ⚠ **`sys.exit(1)` 금지** — `EOD_RETRAIN.bat`이 `pause`로 멈춘다.
  ⚠ **게이트 발동 시 반드시 INFO 로그** — 조용한 스킵이 되면 거래일 재학습 소실 위험.
  **적용: 08-18 15:35 이후** (그 전에 O-2로 상시 결함 여부를 먼저 관측).

- [ ] **F-3 473차 변경분 커밋 (P2)** — 커밋 ① 코드(`config/settings.py` ·
  `learning/prediction_buffer.py` · `utils/db_utils.py` · `tests/conftest.py` ·
  `tests/test_457_guard_meta_and_scope.py` · `scripts/spread_extreme_watch.py` ·
  `scripts/feature_superset_expand.py` · `tests/test_473_*.py` 4종) /
  ② 문서(`CLAUDE.md` · `dev_memory/*` · `docs/…앙상블진입판단…md` · `weekly_prompt.txt`) /
  ③ 점검 산출물 untracked 16종. **적용: 사용자 확인 후 즉시, 08-18 기동 전 권장.**

- [ ] **F-4 `TimeRouter` 거래일 인식 (P2)** — 휴장일엔 세션 `OTHER` 고정 + 로그 1회(매분 금지).
  ⚠ 착수 전 `grep -rn "class TimeRouter" --include=*.py .`로 파일·함수 특정 —
  이번 점검은 로그로만 확인했다. `DynMC` 세션별 임계 조회에 영향 없는지 확인.
  **적용: 다음 거래일 장 마감 후.**

### 고도화

- [ ] **G-1 수집기에 휴장일 배지 (이번 주)** — `is_krx_holiday(day)`로 헤더에 `거래일/휴장일`
  표기, §5 0건을 `0 (휴장일 — 정상)`, §7 커버리지를 `해당없음`으로.
  ⚠ **§11 적신호는 억제하지 않는다**(진짜 이상을 덮지 않기 위해).
  **근거**: 오늘 §5·§7·§11이 만든 오탐 최소 3곳.

- [ ] **G-2 재학습 "완주 마커" (이번 주, F-2와 동시)** — `scripts/eod_retrain.py` 정상 종료
  직전 `data/eod_retrain_done_YYYYMMDD.txt`(완료 시각 + 호라이즌별 AUC) 기록,
  수집기 §1에 "전일 EOD 완주 마커" 행 신설. 마커 부재 = 적신호.
  ⚠ **마커 부재는 적신호이지 확정 실패가 아니다**(계측 4원칙 ②).
  ⚠ **휴장일 스킵은 `eod_retrain_skipped_*.txt`로 파일명을 분리** — "스킵"과 "완주"를 같은
  파일명으로 쓰면 미측정≠0과 같은 계열의 실수가 된다.

- [ ] **G-3 브랜치 격차를 수집기 §2에 상시 노출 (이번 주 후반)** — `merge-base` · 양방향 커밋 수 ·
  상대 브랜치 전용 `*_ENABLED` 목록 3줄. **선행: F-1**(순서를 뒤집으면 적신호와 중복돼 노이즈).
  ⚠ `git show origin/dev:…`는 fetch 상태에 의존 — **fetch 시각을 함께 찍어** 신선도를 드러낼 것.

### 다음 거래일(08-18) 관측 — 판정 근거

- [ ] **O-1 예약 장전 점검 실행 시각** — `evidence_MW0601-20260818_pre.md` 헤더가 08:5x면
  오늘 지연(16:48)은 1회성. 또 오후면 스케줄러 결함 확정.
- [ ] **O-2 `logs/retrain_eod_20260818.log` 완주 여부** — `[검증 캠페인] 요약: … OK` 도달 +
  **>20KB**. 미도달이면 F-2를 **P1 → P0 승격**.
- [ ] **O-3 F-1H 하트비트** — `[SchedForceExit] 15:11 점검 … bar_pass=N회` 1건 + `bar_pass>0`.
  (08-14 배포 후 첫 거래일 — 휴장으로 이월)
- [ ] **O-4 F-4M 스키마** — `entry_block_axes`·`health_preblock`·`entry_mode_fallback` 존재 +
  당일 행 적재. ⚠ 471차 이전 행 NULL은 **미측정**이지 0이 아니다.
- [ ] **O-5 F-8M 스키마(신규)** — `spread_ticks`·`spread_extreme_shadow` 생성 + 적재.
  **전 행 NULL이면 배선 실패.** ⚠ 473차 이전 행 NULL = 미측정.
- [ ] **O-6 `[TimeRouter]` 정상 전환** — 1-3이 휴장일 한정 현상임을 반증적으로 확인.
- [ ] **O-7 수집기 §11 적신호 건수** — 오늘 7건(5건이 F-1 오탐 + 1건 CRLF 착시).
- [ ] **O-8 F-1R 15:10 강제청산 리허설(모의 1계약) — 사용자 실행 필요.** 휴장으로 오늘 실행
  불가 → 이월. 정상 판정: `[ForceExitPass]`→`[TimeExit]`→`[ExitAttempt]`가 15:10:00~05,
  `[SchedForceExit]` ERROR **미출현**.

### 주간회의 안건 (점검 세션이 단독 결정하지 않음)

- [ ] **`v9-dev` ↔ `dev` 기능 정렬 여부** — 분기 320/315커밋, 공통조상 291차.
  ⚠ 특히 `MODEL_LABEL_STATE_UNLOCK_ENABLED`는 **실전전환 ⑧의 선행조건**인데 MW0601에 코드가
  없다 — **MW0601 관측만으로 ⑧을 판단하면 틀린다.**
- [ ] **CB② 복원 8/29 상정** — 기한 **12일** 남음. 현재값 `9999` 정상.
- [ ] **실전전환 ⑨ TOX-SEVERE-SPREAD 처분** — 473차 F-8 Phase A `INSUFFICIENT`(ETA 7.1개월).
  선행조건 유지 vs 26주 WFA 이관. ⚠ **문턱을 낮춰 판정하지 말 것**(458차 D6과 같은 사전등록 위반).

## 2026-08-17 (MW0601 473차 후속 — 장중 점검 / 휴장일) 신규 항목

> 근거: `docs/정기점검/매일점검/MW0601-20260817-점검리포트-intra.md`
> ⚠ **장전 세션 F-1~F-4 · G-1~G-3과 중복되지 않게 번호를 F-5 / G-4부터 이어 붙였다.**
> 장전이 이미 등록한 것(불변식 브랜치 스코프 · EOD 휴장일 게이트 · TimeRouter 거래일 인식 ·
> 473차 커밋 · 수집기 휴장일 배지 · 완주 마커 · 브랜치 격차 노출)은 **재등록하지 않았다.**

### Fix — 적용 시점 명시

- [ ] **F-5 수집기 git 호출부 CRLF 정규화 + `--pc` 인자 (P1)** —
  `collect_evidence.py` git 서브프로세스 전량에 `-c core.autocrlf=true`
  (읽기 전용, 작업트리 무해. 실측 **453 → 35건**). §2는 수정(` M`)/미추적(`??`)을
  **분리 표기**하고 §11 적신호는 **수정분만** 센다(미추적은 점검 산출물이 매일 늘어 상시 점등).
  `pc_id()` 우선순위 `--pc` > `MIREUK_PC_ID` > 호스트명 + 헤더에 `(override · host=…)` 표기.
  **근거**: `utils/time_utils.py` `231+/231-`(행수 동일=전문 재기록) · 작업트리 CRLF vs 블롭 LF ·
  `core.autocrlf` 미설정. 장전 1-4가 "거짓"까지 적었으나 Fix 미등록이었다.
  🔴 **장전 F-1과 같은 파일·같은 함수군 — 반드시 한 커밋으로 묶어라.**
  ⚠ **윈도우 직접 실행 교차 확인 필요**(샌드박스 한정 여부 미검증).
  **적용: 장후, 장전 F-3(473차 커밋) 이후.**

- [ ] **F-6 백필 스크립트 기동·종료 흔적 로깅 (P2)** —
  `scripts/aggregate_and_backfill.py` 모듈 최상단(`basicConfig` 직후)에
  `[Start] pid=… argv=…` 1행 + `__main__`을 try/except로 감싸 `logger.exception("[Fatal] …")`·`[Done]`.
  **근거**: `20260817_BACKFILL.log`가 **15:11:13 생성 · 0바이트** — 파일 생성(임포트)과
  첫 로그(line 124 `DB 스키마 초기화 완료`) 사이에서 죽으면 흔적이 남지 않는다.
  트레이스백은 `StreamHandler(sys.stdout)`로만 나가 사라진다.
  ⚠ **0바이트가 아니게 되는 것만으로 성공이 아니다** — `[Done]`/`[Fatal]` 중 하나가 필수.
  🔴 py37_32 경로(`features.bar_aggregator` 임포트). **적용: 장후.**

- [ ] **F-7 런처 정상 종료 문구 정정 (P2)** — `start_mireuk.bat` 재시작 판정 블록에서
  `user_close` 경로가 `일시적 크래시, 카운터 초기화`를 쓰지 않게 **출력만** 분기.
  🔴 **재시작 판정 로직 불가침** — 오늘 동작은 옳았다(RestartCnt=0, 재시작 안 함).
  **근거**: 진짜 크래시 사후 검색 시 오탐. 오늘도 `crash_fault.log` 말미
  `[CLEAN EXIT] 2026-08-17T12:54:57 PID=22184`를 확인해야 크래시 아님이 확정됐다.
  **적용: 장후(최저 우선순위).**

### 고도화

- [ ] **G-4 `Calibration` 기동 복원 스윕 로그 레벨 하향 (이번 주)** —
  스윕 구간(부트스트랩 n 증가 루프)의 `축퇴 감지`·`하한 도달불가`를 DEBUG/INFO로 내리고
  **스윕 종료 후 최종 상태만 WARNING**. 또는 1행 집계
  (`축퇴 감지 n=80~85 3건 / 해소 n=90 → 최종 degenerate=False`).
  **근거**: 오늘 `LEARNING` WARNING **143건 전부**가 08:41:00~08:41:09 기동 스윕이고
  `보정기 복원 완료 (… degenerate=False unreachable=False)`로 정상 종료 —
  **정상 절차가 WARNING 143건을 남기고 두 세션이 독립적으로 같은 소음을 걸러냈다.**
  🔴 **임계·판정 로직 불가침**(403차 축퇴 가드는 conf 스케일에 직접 영향 — 판정 변경은 별건).
  ⚠ 복원 실패를 조용히 넘기지 않도록 **최종 상태 WARNING은 반드시 남긴다**(계측 4원칙 ④).

- [ ] **G-5 `references/phases.md` D절 "휴장일" 신설 (이번 주, 장전 G-1과 같은 커밋)** —
  ⓐ `daily_close_done_{d}.txt` **부재** 확인(있으면 그게 이상) ⓑ EOD 재학습이 스킵 1행으로
  끝났는가(장전 F-2 적용 후 판정 가능) ⓒ 백필·캠페인 체인이 직전 거래일로 이월됐는가
  ⓓ `model/horizons/*.pkl`·완료 마커 날짜가 **직전 거래일 유지**인가
  (🔴 **오늘 날짜면 휴장일 재학습이 실제로 일어난 것 = P0에 가깝다.** 오늘은 08-14 유지 정상)
  ⓔ 유휴 대기 중 COM 세션·DB 핸들 점유 ⓕ 각 배치 로그가 0바이트가 아닌가(F-6이 만든 항목).
  ⚠ **D절은 점검 축소가 아니라 대상 교체**임을 문서에 명시(장전 G-1이 §11 적신호를
  억제하지 않기로 한 것과 같은 취지).
  **근거**: 오늘 실질 이상점 전부가 **거래일엔 구조적으로 가려지는 것**이다.

### 다음 거래일(08-18) 관측 — 본 세션 추가분

- [ ] **O-9 `20260818_BACKFILL.log` 크기** — 0바이트면 F-6을 **P2 → P1 승격**(휴장일 한정이
  아니라 상시 결함). 15:11 기동 주체(스케줄러 등록 여부)도 함께 확인 — **사용자 확인 필요.**
- [ ] **O-10 `LEARNING` WARNING 중 `Calibration` 비율** — 오늘 143/143(100%).
  거래일에도 기동 스윕이 대부분이면 G-4를 이번 주 착수. 장중 발생분이 섞이면 그쪽을 먼저 본다.
- [ ] **O-11 예약 3국면 실행 시각 3개 전부** — 오늘 16:48/16:49/16:55.
  `O-1`을 3국면으로 확장해 본다. `pre` 다이제스트 헤더가 `PC UNKNOWN`이면 F-5 미적용 상태.
- [ ] **O-12 `[SHAP] … CORE안전=⚠️` 상세** — 오늘 1행만 있고 대상 피처 불명.
  주간 심사 산출물에서 하락피처 1개·교체후보 3개의 실체 확인. 휴장일 기동 시 심사가 도는 것이
  의도인지도 함께(신규 표본 0인데 전일과 같은 결론이 나왔을 가능성).
...
> 리포트: `docs/정기점검/매일점검/MW0601-20260817-점검리포트-post.md`
> ⚠ 같은 날 장전 점검이 등록한 항목과 **중복 착수 금지** — F-1~F-4(장전)와
> PF-1~PF-3(장후)는 축이 겹친다. 특히 **장전 F-2 ↔ 장후 PF-1**, **장전 G-1 ↔ 장후 PF-3**은
> 같은 것이다. 착수 전 양쪽 리포트를 함께 볼 것.

### 🔴 최우선 — 다음 거래일(2026-08-18)에 답이 나오는 것

- [ ] **O-1 EOD 재학습 완주 확인** — `logs/retrain_eod_20260818.log`가 10KB 이상 +
  `horizons_replaced: 6/6` + `data/eod_retrain_done_20260818.txt` 생성.
  **하나라도 빠지면 오늘의 무기록 정지가 거래일에 재현된 것 → 즉시 P0.**
  오늘 실측: 1,040B / mtime 15:46:34 / 마커 미생성 / `daily_stats`에 2026-08-17 행 없음.

- [ ] **O-2 `session_state.json` P8 키 재생성** — 08-18 EOD 완주 후
  `p8_last_success_date`·`eod_retrain_ok_date` **둘 다** 존재하는지.
- [ ] **O-3 P8 키 잔존(C-2 2단계)** — 08-19 08:41 기동 **후에도** 두 키가 남아 있는지.
  사라지면 **`main.py` 기동 경로가 범인**으로 확정된다. 남으면 08-17 단발 사건.

- [ ] **F-1R 15:10 강제청산 리허설 — 08-18로 이월(사용자 실행 필요).**
  오늘 휴장으로 물리적 불가였다. 절차·판정 기준은 471차 등록분 그대로.
  `trades.exit_reason LIKE '%15:10%'`는 오늘도 **전기간 0건**(실측 재확인).
- [ ] **F-1H 하트비트 1일차 — 08-18로 이월.** 오늘 본체가 15:11에 부재해
  `[SchedForceExit]` 관측 자체가 불가능했다. `bar_pass=0`이면 F-1 경로 회귀.
- [ ] **F-4M 스키마 마이그레이션 1일차 — 08-18로 이월.** 오늘 신규 행 0건.
  확인 대상에 **473차 신규 `spread_ticks`·`spread_extreme_shadow` 2컬럼 추가**.
  ⚠ 이전 행 NULL은 **미측정**이지 0이 아니다(계측 4원칙 ②).

- [ ] **O-8 08-18 08:40 자동 기동 여부** — 오늘 12:55 `user_close` 수동 종료 후
  `logs/Mireuk_batch/launcher_20260818_*.log`가 생기는지. 없으면 사용자 수동 기동 필요 —
  **당일 거래 전면 불가로 이어지는 경로**다.
- [ ] **O-7 예약 점검 시각(C-3)** — 장전 점검이 08:57±5분에 도는지.
  오늘은 pre 16:48 / intra 16:49 / post 16:55로 몰렸다. 2일 연속이면 재현성 확정.
  **1일 관측이므로 313차 원칙에 따라 현재는 확정 결론 보류.**

### Fix — 08-18 장 마감 후 적용 (오늘 밤 금지: 08-18 EOD가 미검증 코드로 돈다)

- [ ] **PF-1 `retrain_eod.py` 거래일 게이트 + 완주/중단 흔적 (P1)** —
  ① `main()` 진입부에 비거래일 게이트: **로그 1줄 남기고** `sys.exit(0)`
  (조용한 종료는 이 이상점 자체의 재생산이다).
  ② `_wait_for_daily_close()`(224~239행): 본체 생존을 `data/_exit_normally` 또는
  `session_state.json:auto_shutdown_done_date`로 확인해, 부재면 20분 대기 생략하고
  `[WaitDC] 본체 부재 확인 — 대기 생략, 강제 진행` WARNING 후 즉시 반환.
  ③ `main()`을 `try/finally`로 감싸 **완주 여부와 무관하게**
  `[EOD] 종료 code=… elapsed=…s marker={생성|미생성}` 1줄.
  ⚠ **`py310_64` 전용**(191차). 거래일 판정 import가 32-bit COM을 끌어오면
  `config/krx_holidays.py` 직접 파싱으로 대체.
  검증: `tests/test_474_eod_gate.py` 신설 + 라이브 O-1.

- [ ] **PF-2 `session_state` 완료 스탬프 가시화 (P1)** —
  ① `main.py:_read_session_state()`(2887~2896): 두 키 부재 시
  `[SessionState] EOD 스탬프 부재 — 마커 폴백 사용` **상태 변화 시 1회만**
  (⚠ 매분 호출 함수 — 471차 후속4 `entry_mode` 관례).
  ② `main.py` PreRetrain 폴백(5069~5085): `_days_ago`와 **5일 창 잔여** 병기.
  ③ `retrain_eod.py:p8_scaler_refit()`(184~196): 쓰기 **직후 되읽어 검증**,
  실패 시 `[P8] session_state 기록 검증 실패` WARNING.
  ⚠ `main.py`=`py37_32` / `retrain_eod.py`=`py310_64` — 각 런타임에서 컴파일 확인.
  ⚠ **이 fix를 근본 원인 조사(C-2)로 갈음하지 말 것.**
...
## 2026-08-18 (MW0602 475차 — 장전 점검 / pre) 신규 항목

리포트: `docs/정기점검/매일점검/MW0602-20260818-점검리포트-pre.md`
**P0 0건.** 절대원칙 6종 준수 · 설정 불변식 24행 전부 `일치`.

### 🔴 Fix — 08-19 장 시작 전 적용 (라이브 종료 15:40:31, 적용 가능 시간대)

- [ ] **F-1 EOD 경로 `label_scheme` 소실 복구 (P1)** — `retrain_eod.py:320` 이
  `retrain_now(X=…, y_dict=…)` 로 X/y를 직접 넘겨 `batch_retrainer.py:507-509` 가
  `_last_label_scheme=None` 으로 리셋 → 사이드카 6/6 `null` → 장전 `불일치 6/6` 고정.
  `retrain_now()` 에 `label_scheme`/`label_params` 키워드(**기본 `None` 유지**) 추가 +
  `retrain_eod.py` 가 실제 쓴 규칙 전달 + `model_meta.py:56` 메시지를
  "키 부재" vs "값 null" 로 분기(현재 `구버전` 으로 오귀속).
  ⚠ `py310_64`+`py37_32` **양쪽 컴파일 확인**. 반환 시그니처 불변(457차).
  ⚠ 기존 호출부(`retrain_intraday.py:92`·`scripts/eod_retrain.py:95`) 무변경 동작 유지.
  **검증**: O-1 → O-2. ⚠ **`불일치 0/6` 만으로 G-1 검증 완료 아님** — `invariants.md` §2-B
  *"F-1 마커 없이도 해제되는 것"* 확인 전 **실전전환 ⑧ 해제 금지.**

- [ ] **F-2 ConfFloorGuard 상태샘플 calibrator 오참조 (P1)** —
  `model/ensemble_decision.py:416` `getattr(self,"calibrator",None)` →
  `getattr(self,"ensemble_calibrator",None)` (1줄).
  **함께**: `:437-438` `except` 의 `logger.debug` → **`logger.warning`** 승격 + 예외 타입 출력.
  (선택) 두 함수가 공통 `_resolve_conf_calibrator()` 사용 → 재발 구조 차단.
  근거: 오늘 `state=` 샘플 80건 전부 `ZONE_BLACKOUT`(금지 존 80분과 정확 일치),
  진입 허용 290분 **0건**. `[Ensemble] dir=` 는 364건이라 `compute()` 는 매분 돌았다.
  ⚠ 예외 발생 지점은 코드 대조로 특정(`output_max` 유력)했으나 런타임 스택 미확인 —
  **원인 문구를 단정 인용하지 말 것.** 둘 다 같은 수정으로 해소된다.
  **검증**: O-3 / O-4.

- [ ] **F-3 미푸시 12건 + `[MW0601]` 11건 이관 기록 (P1)** —
  `git rev-list --left-right --count origin/dev...dev` = **0/12**.
  (1) `git fetch --all` → `origin/v9-dev` 대조로 원 sha 확정
  (2) CLAUDE.md 470차 규약 3요소(원 sha·원 PC·이유)를 DECISION_LOG에 기록 — **즉시**
  (3) `git push origin dev` — **주간회의 확인 후**(브랜치 격차 처분이 이미 안건).
  ⚠ 471~474차는 `ensemble_decisions` 스키마 변경 포함 → push 후 상대 PC 마이그레이션 확인.

- [ ] **F-4 O-9 판정 기준 정정 (P2)** — *"0바이트면 F-6 P1 승격"* →
  **"0바이트이거나 파일 부재면"**. 오늘 실측이 파일 부재라 문자 그대로면 판정 불가다.
  **계측 4원칙 ② — "미측정"과 "0"은 다르다.**

### 고도화 (이번 주)

- [ ] **G-1 "도달 불가 안전장치" 자동 탐지 — 수집기 §12 세 번째 축** —
  `scan_branch_asymmetry()`: 상태 샘플 태그의 관측건수를 §7 매분 커버리지(371)·
  §5 진입 가능/금지 예산(290/80)과 대조. `관측/기대 < 0.5` 이고 값 분포 단일 → `⚠️ 분기편향`.
  회고 검증: 08-14(0/364)·08-18(80/370) 둘 다 탐지되어야 한다.
  ⚠ `benign` 화이트리스트 필수(조건부 발동이 설계인 태그).
  **⚠ 470차 C1'·C2'와 같은 커밋으로 — 중복 구현 금지.** 선행: F-2.

- [ ] **G-2 EOD 사이드카 완전성 검사** — `retrain_eod.py` P8 직후 6개 `gbm_*_meta.json` 의
  `label_scheme`·`train_end_ts`·`acc_kind`·`source` non-null 검사 → 결손 시 마커에
  `meta_incomplete: …` + `[EOD_RETRAIN] WARNING`. 수집기 §4가 마커 전문을 인용하므로
  다음 날 장전에서 자동으로 보인다. 선행: **F-1**. `py310_64`. `except` 는 `logger.warning`.

- [ ] **G-3 예약 시각 복구 + 국면 직렬화** — (a) MW0602 `pre` 트리거 **08:57 KST** 재설정.
  ⚠ 예약작업은 PC별 등록이며 **git 공유 안 됨** — 등록 사실을 이 파일에 남길 것.
  (b) 473차 후속2 등록 `dev_memory/.dailycheck.lock` 구현(스테일 mtime 10분 초과 무시).
  **(b)는 신규 안건이 아니라 미착수 확인이다.** 오늘 3국면이 **같은 분(16:32)** 에 겹쳤다.

### 다음 거래일(08-19) 관측 — 판정 기준 포함

- [ ] **O-1** `model/horizons/gbm_*_meta.json` 6개 `label_scheme` → **6/6 non-null** 이면 F-1 성공 (08-19 EOD 후)
- [ ] **O-2** `[EntryGate] … 불일치 N/6` → **0/6** 이면 G-1 1단계 통과 (08-20 08:55)
- [ ] **O-3** `grep -c "ConfFloorGuard] state=OK\|BLOCKED"` → **≥200** 이면 F-2 성공 (08-19 장후)
- [ ] **O-4** `[ConfFloorGuard] 상태 샘플 실패`(WARNING 승격 후) → 0건 정상. 출현 시 예외 타입이 원인 특정
- [ ] **O-5** `eod_retrain_done_20260819.txt` 의 `meta_incomplete` 줄 → 부재 = 사이드카 완전(G-2 적용 시)
- [ ] **O-6** 다이제스트 생성 시각 → **08:57±5분** 이면 O-7 해소
- [ ] **O-7** `evidence_MW0602-20260819_*.md` 3종 생성 시각 → 서로 다른 분이면 동시 실행 해소
- [ ] **O-8** `logs/20260819_BACKFILL.log` → 존재 + 0B 아님이면 F-6 불요. 부재/0B면 **F-6 P1 승격**
- [ ] **O-9** `[SchedForceExit] … bar_pass=N회` 매 거래일 출현 · `안전망 발동`(ERROR) 미출현 (상시)
- [ ] **O-10** `ensemble_decisions.sizing_trace` non-null 일별 증가(오늘 65) → F-4M 정착
- [ ] **O-11** §12 `CORE안전` ⚠️ 재출현 여부 — 오늘 종일 `✅`(468차 F-3 개선 유지 확인)
- [ ] **O-12** `[MarginCap]`·`[SizerMatch]` 와 `qty≥3` 체결 — 0건 지속 예상. 발생하면 470차 결론 재검토

### 이월 (미완)

- [ ] **F-1R 15:10 강제청산 리허설 — 08-19로 재이월 (사용자 실행 필요).**
  오늘도 15:10 이전 전량 청산(최종 14:45:51)이라 실집행 기회 없음.
  절차: 15:05~15:09 모의 **1계약** 진입 → 15:10 관측.
  정상 = `[ForceExitPass]` → `[TimeExit]` → `[ExitAttempt]` 가 **15:10:00~05** 출현 +
  `[SchedForceExit] … 안전망 발동`(ERROR) **미출현**. 비용 약 1,600원.
  ⚠ 모의투자 계좌에서만. **실전전환 기준 ② 선행 확인사항.**

### 종결 처리 (오늘 판정 완료)

- [x] **F-1H 하트비트 1일차 — ✅ 성공.** `15:11:15 [SchedForceExit] … bar_pass=2회 →
  청산 대상 없음(정상)`. 471차 F-1·F-2 라이브 최초 확인.
  ⚠ *"경로가 살아 있다"* ≠ *"주문이 나간다"* — F-1R 여전히 필요.
- [x] **F-4M 스키마 마이그레이션 1일차 — ✅ 성공.** `ensemble_decisions` 56컬럼 ·
  08-18 신규 369행 · `sizing_trace` 65 · `spread_ticks`/`spread_extreme_shadow` 각 369.
  ⚠ `binding_gate` 는 별도 컬럼이 아니라 `sizing_trace` JSON 내부.
- [x] **O-7(구) 예약 점검 시각 — ❌ 미충족 판정.** 16:32 실행. → G-3(a)로 승계.
- [x] **O-11(구) 3국면 동시 실행 — 재발 확인.** 같은 16:32. → G-3(b)로 승계.
- [x] **O-12(구) `[SHAP] … CORE안전` — 종일 `✅`.**
- [x] **O-9(구) BACKFILL 로그 — 기준 정정 필요 판정.** → F-4.
- [x] **O-1(구) EOD 재학습 완주 — ✅** (17.4KB · 6/6 · 161.2s).
  ⚠ **산출물은 반쪽이다** — 위 F-1 참조.
- [x] **O-2(구) `session_state.json` P8 2키 — MW0602 는 `2026-08-18` 존재.**
  ⚠ 08-17 소실은 **MW0601 관측**이다. **이 데이터로 MW0601 항목을 닫지 말 것.**

### 기한

- [ ] **CB② 복원 08-29 주간회의 상정 — 남은 기한 11일.** 오늘 `9999` 유지 확인.
  ⚠ 오늘 `[CB] 연속 손절 3회` 가 실제로 찍혔다 — **복원하면 오늘 같은 날은 당일 정지**다.
  복원 판단 시 08-18(진입 18건 · 진짜 손절 5건 · TP1 보호 8건)을 표본으로 쓸 것.
- [ ] **실전전환 ⑧ 진행 금지** — 선행조건 G-1이 F-1 적용 전까지 **도달 불가**(오늘 확정).

---

## 2026-08-18 (MW0602 475차 — 장후 점검) 신규 항목

### 🔴 최우선 — 다음 거래일(2026-08-19)에 답이 나오는 것

- [ ] **O-3 P8 키 잔존 (C-2 2단계 종결 조건)** — 08-19 08:41 기동 **후에도**
  `data/session_state.json` 에 `p8_last_success_date` · `eod_retrain_ok_date` 두 키가
  남아 있는지. 오늘 15:47:44에 정상 기록됨(O-2 완료). **소실하면 P0 승격** —
  08-14→08-17 사이와 같은 소실이 재현된다는 뜻이다.
- [ ] **O-1 ConfFloorGuard 상태 샘플 수** — F-1 적용 후
  `grep -c "ConfFloorGuard\] state=" logs/20260819_SIGNAL.log` **≥300** 이고 값 **2종 이상**.
  오늘 기준선 **80건·1종(`ZONE_BLACKOUT` 100%)**. F-1 미적용이면 80 안팎이 재현되는지만 확인.
- [ ] **O-4 `[SchedForceExit]` 하트비트 2일차** — 15:11 출현 + `bar_pass≥1`.
  미출현이면 471차 F-2 회귀다. 오늘 `15:11:15 … bar_pass=2회` 정상.
- [ ] **O-5 `sizing_trace` 행수 · `binding_gate` 분포** — 오늘 65행 ·
  `tox`45% / `hurst`20% / `meta`12% / `None`23%. **`tox`가 2일 연속 최다면 G-2 승격.**
- [ ] **O-7 `[MarginCap]` 건수 · `qty≥3` 체결** — 오늘 MarginCap 1건(3→2), `qty≥3` **0건**.
  `qty≥3`이 나오면 [28] `sizing_inversion_watch` 각성 신호(⑧ 해제 전에는 안 나오는 게 정상).
- [ ] **O-8 `degraded=ON` 재발 여부** — 오늘 14:31~14:32 2분(08-12는 0건).
  2일 연속이면 §12 `degraded` 행(`✅ 변동`) 재평가.
- [ ] **O-10 `[Brier] 과신` 이동평균** — 오늘 최대 **0.367**(임계 0.35). 08-12는 0.352.
  **0.40 접근 시 P1 승격.**
- [ ] **O-6 `spread_ticks ≥20` 진입 건수** — 오늘 분봉 1건 · **진입 0건**.
  누적 진입이 6→7이 되면 ⑨ ETA(7.1개월) 재산출.
- [ ] **O-9 점검 세션 다이제스트 생성 시각 3개** — 오늘 16:31:54 / 16:31:59 / 16:32:16.
  **G-3(세션 직렬화) 재현 근거**로 계속 적립.

### Fix — 오늘 밤 적용 가능 (본체 15:40:31 정상 종료 · EOD 15:47:44 완주 확인 후)

- [ ] **F-1 `[ConfFloorGuard] state=` 존 무관 기록 (P1)** — 상태 샘플 emit을
  `zone_entry=False` 분기 **바깥**의 매분 훅으로 이동. 상태값 `OK`/`BLOCKED`/`ZONE_BLACKOUT`
  3분류 유지, 허용 존에서 `출력상한`·`필요` 실값 기록(현재 `N/A`).
  ⚠ **로그 문구 변경 금지** — `collect_evidence.py:275`
  `r"\[ConfFloorGuard\] state=(?P<v>\w+)"` 가 물려 있다.
  구현부: `grep -rn "ConfFloorGuard\] state=" --include=*.py` 로 확정. `py37_32`, COM 콜백 밖.
  회귀 위험: 하루 +290행(≈+30KB). 게이트 판정 로직 불변.
  근거: 오늘 80샘플 = 금지 존 체류 80분과 **정확히 일치**, 허용 290분 0건.
  커밋: `[MW0602] 475차: ConfFloorGuard 상태 샘플 존 무관 기록 — 470차 L2 조건부 고착 해소`

- [ ] **F-2 일일 전략 리포트 배너 자본대비 MDD 병기 (P1)** —
  `strategy/ops/daily_exporter.py:70`·`:79` 를
  `MDD=%.2f%%(자본대비) [peak대비 %.1f%%]` 로. 분모는 `SIZING_TARGET_CAPITAL_KRW`.
  ⚠ **`tests/test_465_retrain_scope_and_tp1_flag.py:306~307` 을 같은 커밋에서 갱신**할 것 —
  현재 문자열(`"MDD=%.1f%%(누적손익peak대비)"` · `"누적손익peak대비%s"`)을 정확히 검사한다.
  분리 커밋하면 CI가 깨진 채 남는다.
  근거: 오늘 배너 `MDD=143.3%(누적손익peak대비)` = 자본 대비 **2.89%**.
  461차 결정("판정·**리포트 배너**·대시보드는 `mdd_pct_of_capital`만")의 **잔여 경로**.
  ⚠ **교체가 아니라 병기** — 계측 4원칙 ③(탈락 가시화).
  커밋: `[MW0602] 475차: 일일 전략 리포트 배너 자본대비 MDD 병기 (461차 잔여 경로)`

- [ ] **F-4 `docs/정기점검` untracked 산출물 추적 편입 (P2)** —
  **기존 항목 "정기점검 산출물 untracked 16종 추적 편입" 과 동일. 중복 착수 금지.**
  오늘 2건 합류: `MIREUK_DAILYCHECK_HANDOFF.md` · `0816_주간회의_검토보고_MW0602.md`.
  CLAUDE.md "주간 산출물 위치 규약"이 리포트 커밋을 **의도적 예외**로 정한 근거가 있다.

### 조사 (fix가 아니다)

- [ ] **F-3 수집기 §12 `전략판정` 원천 정합 (P1) — 조사 선행, 패턴 먼저 고치지 말 것** —
  §12가 `UNDERPERFORM×7`로 🔴 고착 판정(적신호 13번)했는데 당일 리포트 판정은
  **`INSUFFICIENT`** 다. `grep "UNDERPERFORM\|INSUFFICIENT" logs/20260818_*.log` = **0건**
  (로그 원천 아님). 패턴 정의 `collect_evidence.py:265`.
  조사 대상: §12 원천(`strategy_registry.db:strategy_live_snapshots` 추정) vs
  `daily_exporter` 출력의 **시점 차**.
  ⚠ 471차 F-3→G-1 순서 교훈 — "문자열을 고치기 전에 구조체를 만들면 틀린 값이 굳는다".
  검증: 08-18 날짜로 `--phase post` 재실행 시 §12에 `INSUFFICIENT`가 포함되는지.

- [ ] **O-2 `ofi_norm` 축퇴 ↔ `ofi❌` 인과 (판정 예정 2026-08-25)** —
  오늘 `[CORE준비도] 축퇴 1/6 — ofi_norm×6hz` 가 10:15:58~14:57:00 **연속**(42샘플 중 29건),
  진입 18건 체크리스트 미통과에서 `ofi`×14 **최다**. raw_std 궤적 08:45 `0.0294` →
  10:15 `0.0417` → 11:32~14:16 `0.0307~0.0317` 고착. EOD 15:47:44에도 미해소.
  ⚠ **신규 결함이 아니다** — CLAUDE.md 371차 문단이 "cvd_divergence·ofi_norm 학습표본의
  91%가 정확히 0.0인 점질량"을 이미 잔여 이슈로 등록했다. 오늘 470차 B4 계측이 처음
  **일중 지속시간**으로 가시화한 것.
  방법: `축퇴 1/6` 구간 vs `0/6` 구간의 진입건 `ofi` 통과율을 **일자단위 5거래일** 비교
  (313차 ① 필수). **관측일 1일 — 확정 결론 금지.**

- [ ] **degraded 발동의 지연 원인 (P2)** — 14:20~14:35 `[PipePerf]` 원값(`raw=…ms`,
  462차 P2 `PIPE_LATENCY_EXCLUDE_MODEL_SWAP`)과 차감분 대조.
  오늘 `14:31:00 [HealthPolicy] Degraded 선제차단: streak=1.75+0.35 ≥ 2
  (latency=745ms quality=1.00 cache=0s exc10m=8)` — `quality=1.00`인데 지연 축만으로 발동.
  `config/settings.py:5367~5388`이 경계한 "exceptions_10m 단독 발동" 계열인지 확인.
  (오늘은 latency도 높아 단독 발동은 아님)

- [ ] **`mc-conf 괴리` 5일 평균 49분의 분모 (P2)** — `15:40:15 [경보] mc-conf 괴리:
  최근 5거래일 평균 진입후보 49분/일 < 하한 60분 — 금일 103분`.
  08-15·08-17 휴장으로 실제 3일치 평균인지 확인.

### 고도화

- [ ] **G-1 수집기 §5에 gross/수수료/net 3열 상설 (이번 주)** —
  오늘 gross **+8,000원** · 수수료 **-33,084원** · net **-25,084원**.
  승률 66.7%인데 적자이며, 수수료/총이익 = **413%** 다.
  `trades` 스키마에 `gross_pnl_krw`·`commission_krw`·`net_pnl_krw` 세 컬럼이 **이미 있다**
  (신규 배선 불필요). 합계 줄에 비율 병기.
  기대: "방향은 맞혔는데 비용에 졌다" 와 "방향을 틀렸다" 를 그날 즉시 구분.

- [ ] **G-2 §12 고착 지표에 `binding_gate` 축 추가 (이번 주)** —
  오늘 처음 실측: `tox`×29(45%) · `hurst`×13(20%) · `meta`×8(12%) · `None`×15(23%).
  **사이즈를 가장 많이 구속한 것은 ToxicityGate**인데 §12 감시 목록에 없다.
  원천이 로그가 아니라 **DB(`ensemble_decisions.sizing_trace` JSON)** 이므로 §12에 DB 경로를
  여는 첫 사례가 된다. ⚠ 수집기는 표준 라이브러리 전용 — `sqlite3`·`json`은 표준이라 무방.
  ⚠ `sizing_trace`는 **2026-08-14 이후 행에만** 있다. 그 이전은 NULL이며 **미측정이지
  "압력 0"이 아니다**(계측 4원칙 ②).
  기대: 431차 min() 합성 이후 "무엇이 실제로 구속하는가"를 매일 관측. 한 게이트 100% 고착
  시 316차 HurstGate(63% 차단) 방식으로 조기 발견.

- [ ] **G-3 존 체류시간 × 계측 샘플수 자동 대조 (26주 WFA 주기)** —
  §12에 보조 열 `샘플/거래분` 추가. 매분 샘플러를 표방하는 지표의 이 값이 **1.0에서 크게
  벗어나면** `⚠조건부 의심` 표기. 오늘 `ConfFloor` = 80/371 = **0.22** → 즉시 걸린다.
  근거: P1-1을 발견한 방법이 *"80샘플 = 금지 존 80분"* 이라는 **산술 일치**였고, 그건
  계측이 아니라 사람이 우연히 한 것이다.
  ⚠ 오탐 가능(설계상 조건부인 지표) → `benign` 화이트리스트 병행.
  **선행: F-1**(먼저 고쳐야 기준선이 생긴다). 468차 G-2 · 470차 C1'~C5'와 같은 목록.

- [ ] **R-1 TP1 보호트레일 `protect_offset` 감도 — 섀도 계측만, 집행 변경 금지** —
  오늘 보호트레일 8건 건당 **+0.44pt** vs TP2 도달 5건 **+2.87pt**(6.5배).
  `protect_offset_pts` 현행 `ATR×0.25`(14:25:43 실측 `atr=1.2429 → 0.3107`).
  `main.py` `[SingleContractTP1Arm]` 산출부에 **섀도 컬럼만** 추가:
  `trades.protect_offset_pts` + `offset×0.5 가정 시 청산가`.
  ⚠ **채택 보류** — 313차 가드 ③에서 부호가 뒤집힌다(최대손실 제거 시 **+215,558원**,
  최대이익 제거 시 **-193,461원**). n=8, **관측일 1일**.
  회귀 위험: offset을 넓히면 TP1 도달 후 이익이 손실로 뒤집히는 건이 생긴다 —
  오늘 최소 이익 +0.26pt(14:19)는 offset 0.62pt면 적자 전환했을 수 있다.
  검증: 섀도 **20거래일** 누적 → 일자단위 부호 검정(`_sign_test_p()`), 합격선 사전등록 후 불변.
  462차 P4 — **"처방이 아니라 등록이다."**

### 기한·안건

- [ ] **CB② 복원 08-29 주간회의 상정 — 기한 11일 남음.** 오늘 `9999` 유지 확인 ✅.
  **오늘 처음 반사실 표본이 생겼다**: `[CB] 연속 손절 1→2→3회`(10:10:58 · 13:05:30 ·
  13:36:20)가 정상 증가했고, CB②가 2~3이었다면 **13:36:20에 당일 정지**돼 이후 진입
  **10건**(TP2 **5건 전부** 포함) **순 +657,691원**을 놓쳤을 것이다.
  ⚠ **1일 표본 — 확정 결론 금지.** 한 근거로만 상정할 것.

- [ ] **실전전환 ⑨ TOX-SEVERE-SPREAD 처분 (주간회의)** — 오늘 표본 진전 **0**
  (분봉 `≥20틱` 1건, **진입 0건**). 진입 18건은 `<8`×17 · `8-12`×1.
  473차 F-8 Phase A `INSUFFICIENT`·ETA 7.1개월 그대로. 선행조건 유지 vs 26주 WFA 이관.

- [ ] **브랜치 격차 처분 (주간회의)** — `v9-dev` ↔ `origin/dev` = **320/315**.
  오늘 MW0602 `dev`의 설정 불변식 **`미발견` 0건**이 473차 후속2 [5]의 "브랜치 분기가 원인"
  진단을 뒷받침한다(어제 `v9-dev`는 5종 미발견).

- [ ] **G-3(473차) 점검 세션 직렬화 `dev_memory/.dailycheck.lock` — 미착수, 오늘 재현.**
  pre 16:32:16 / intra 16:31:59 / post 16:31:54 세 다이제스트가 같은 분에 생성됐고
  **pre·intra는 대응 점검리포트도 dev_memory 갱신도 없다.** 본 세션은 append만 수행.

- [ ] **⑧ 사이징 재설계 — G-1 라이브 확인 여전히 미충족.**
  ⚠ 오늘 `[Sizer]` 65회 전부 `신뢰도배수=0.6` 인데 **이것은 G-1의 `_pre_retrain_done`
  ×0.6이 아니라 별개의 신뢰도 배수**다 — 혼동 금지(`references/invariants.md` §2-B).
  ⑧ 해제 진행 금지.

### 완료 처리 (2026-08-17 473차 후속2 항목)

- [x] **O-1 EOD 재학습 완주 확인** — ✅ 08-18 15:47:44 `호라이즌 교체=6/6 가드보류=0
  합계=161.2s`, `Python 3.10.20 64-bit`(정상), rows 39438 × 105열. 로그 17.4KB(>10KB 기준 충족).
- [x] **O-2 `session_state.json` P8 키 재생성** — ✅
  `[P8] session_state p8_last_success_date + eod_retrain_ok_date 기록 완료` 15:47:44.
- [x] **F-1H 하트비트 1일차** — ✅ `15:11:15 [SchedForceExit] 15:11 점검 — status=FLAT
  engine=0ct broker_cached=0ct bar_pass=2회 → 청산 대상 없음(정상)`.
  `bar_pass=2회`로 471차 F-1 1차 경로 도달성도 라이브 확인.
- [x] **F-4M 스키마 마이그레이션 1일차** — ✅ `ensemble_decisions` **56컬럼**, 당일 369행.
  `sizing_trace` 65 · `spread_ticks` 369 · `entry_mode_fallback` 369 · `health_preblock` 2행.
  HEAD `90c66cb`(플레이스홀더 54개 정정) 이후 첫 거래일 — **정합 확인**.

### 이월

- [ ] **F-1R 15:10 강제청산 리허설 — 08-19로 재이월 (사용자 실행 필요).**
  오늘 15:05~15:09 진입이 없어 기회가 없었다(마지막 청산 14:45:51, `14:50 이후 신규 진입
  금지` 6건). `trades.exit_reason LIKE '%15:10%'` 는 **전기간 0건** 재확인.
  절차·판정 기준은 471차 F-1R 항목. 전환기준 ②의 선행 확인사항.

- [ ] **G-3 범위 확장 — 세션 차수 충돌 회피 (473차 후속2 G-3에 합류, 중복 착수 금지)** —
  2026-08-18에 같은 PC(MW0602)의 pre·post 두 세션이 원격 git 기준(최대 474차)으로
  독립 산출해 **둘 다 `475차`** 를 얻었다. `DECISION_LOG.md`·`NEXT_TODO.md` 양쪽에
  같은 날 `475차` 헤더가 둘 있다(국면 라벨로만 구분). 유실은 없다.
  → `.dailycheck.lock` 획득 시 같은 날·같은 PC의 기존 헤더를 스캔해 `475차-2` 접미 또는
  국면 라벨을 차수의 일부로 승격. **소급 수정하지 않는다.**
  근거: 2026-07-02 두 PC "273차" 충돌과 같은 형태이나 이번은 **한 PC 안에서** 발생.

### 🔴 475차 마감 시 실제 발생 — 커밋 미실행 (G-3(b) 즉시로 승격)

- [ ] **475차 산출물 커밋 (미실행 — 다음 세션/사용자가 마무리)** —
  `.git/index.lock` 이 **0바이트 · mtime 16:32**(세 국면 다이제스트와 같은 분)로 남아 있어
  본 세션이 커밋하지 못했다. 16:50:44 기준 **18분 경과**, 45초 뒤 재확인에도 잔존.
  471차 전례(**mtime 10분 초과 = 스테일**)로는 제거 가능하나, 다른 세션 생존 가능성을
  배제할 수 없어 **강제 제거하지 않았다.** 산출물 3종은 **디스크에 기록 완료**, 커밋만 미실행.
  ```
  ...
  ```
  ⚠ **push 는 별개다** — F-3(주간회의 확인 후).

- [ ] **G-3(b) `.dailycheck.lock` 우선순위 이번 주 → 즉시.**
  동시 실행이 "충돌 가능"이 아니라 **실제로 `.git/index.lock` 을 남겨 커밋을 막았다.**
  락 설계 시 **스테일 판정(mtime 10분)을 반드시 포함**할 것 — 오늘 잠금이 정확히 그 형태다.

### G-3(a) 예약 시각 복구 — 등록 스크립트 배포 (475차 후속)

- [ ] **`TASK_CLAUDE_WAKE_INSTALL.bat` 실행 (사용자 1회, MW0602)** —
  Windows 작업 스케줄러에 `\Mireuk\Mireuk_ClaudeWake_0847` 등록.
  **평일(월~금) 08:47** 에 Claude Desktop 을 기동해 08:57 Cowork 장전 점검 예약을 깨운다.
  구현: `scripts/claude_wake_task.ps1`(탐색·등록·검증), `TASK_CLAUDE_WAKE_INSTALL.bat`(런처).
  설정: `WakeToRun`(절전 해제) · `StartWhenAvailable`(놓친 실행 복구) · `LogonType=Interactive`
  (GUI 앱이라 SYSTEM/S4U 로는 창이 안 뜬다) · `ExecutionTimeLimit=0`.
  관리자 권한 불필요. `Claude.exe` 경로는 6단계로 자동 탐색(LOCALAPPDATA · Squirrel `app-*` ·
  Programs · ProgramFiles · 시작메뉴 `.lnk` 역추적 · **실행 중 프로세스**).
  실패 시 `-ExePath "C:\...\Claude.exe"` 로 명시 가능. 해제는 `-Uninstall`.

  **근거**: 08-17·08-18 이틀 연속 3국면 예약이 앱 기동 시각에 몰려 실행
  (08-18 다이제스트 `16:32:06.9 / 16:32:07.6 / 16:32:23.6` — **17초 내 3건**).
  그날 PC는 깨어 있었고(미륵이 08:40~15:40 매분 371/371) 트레이에도 클로드가 있었다 —
  **"트레이 상주"는 예약 실행의 충분조건이 아니다.**

  ⚠ **예약작업은 PC별 등록이며 git 으로 공유되지 않는다**(CLAUDE.md 멀티PC 컨벤션).
  스크립트는 커밋되지만 **등록 행위는 각 PC에서 따로** 해야 한다 — MW0601 에도 필요하면
  그 PC에서 같은 .bat 을 한 번 실행할 것. 등록한 PC와 날짜를 이 항목 아래에 남길 것.
  - [ ] MW0602 등록일: ____________
  - [ ] MW0601 등록일: ____________ (필요 여부 먼저 판단 — MW0601 예약 시각은 미확인)

  ⚠ **`WakeToRun` 은 전원 옵션의 "절전 해제 타이머 허용"이 켜져 있어야 실제로 깨운다.**
  등록 후 `powercfg /waketimers` 로 확인할 것(스크립트 출력 안내에 포함).

  **검증(O-6 승계)**: 08-19 `evidence_MW0602-20260819_pre.md` 헤더 생성 시각이
  **08:57±5분**. 여전히 앱 기동 시각이면 (a) 작업이 실행됐는지 `schtasks /query /tn
  "\Mireuk\Mireuk_ClaudeWake_0847" /v /fo LIST` 의 `Last Run Time`·`Last Result` 확인 →
  (b) 작업은 돌았는데 예약이 안 깼으면 **Cowork 스케줄러가 창 활성까지 요구**한다는 뜻이므로
  장전 점검 자체를 작업 스케줄러로 이관하는 안을 검토한다(미륵이 `Maitreya_EODretrain` 전례).

### G-3(a) 후속 — 실측 확정: 이 PC의 Claude 는 **MSIX 패키지 앱**이다 (475차, 2026-08-18)

- [x] **`WindowsApps` 경로 직접 실행 불가 확정.** 작업 스케줄러 실측:
  ```
  Exe  : C:\Program Files\WindowsApps\Claude_1.30096.1.0_x64__pzs8sxrjxfjjc\app\Claude.exe
  Rslt : 0x80070005          <- ACCESS DENIED. 액션이 아예 실행되지 않았다
  Proc : (동일 경로)          <- 탐색은 정확했으나 '읽기 가능 != 실행 가능'
  ```
  `WindowsApps` 는 TrustedInstaller 소유라 일반 사용자 경로 실행이 막힌다.
  → **패키지 앱은 AUMID 로 띄운다**: `explorer.exe shell:AppsFolder\<PFN>!<AppId>`
  이 PC 실측 AUMID = **`Claude_pzs8sxrjxfjjc!Claude`** (`Get-StartApps` 로 회수).

  ⚠ **교훈 — "경로를 찾았다"와 "그 경로로 실행된다"는 다르다.**
  v1 스크립트는 실행 중 프로세스에서 경로를 정확히 회수하고도 실패했다.
  `[EntryGate] label_scheme` (§F-1) · FP-CRITICAL · 15:10 1차 경로와 **같은 계열** —
  *"등록/배선은 됐는데 도달 불가"*. v2 는 `WindowsApps` 경로를 발견하면
  **채택을 거부하고 중단**한다(조용히 등록해 두는 실패를 만들지 않는다).

- [ ] **리허설 미완 — 판정 대기.** `schtasks /run` 후
  `LastTaskResult` 와 **창이 전면에 나오는지** 둘 다 확인해야 한다.
  - `0x0` + 창 나옴  → 해결. 08-19 08:47 자동 검증(O-6).
  - `0x0` + 창 안 나옴 → **"앱 재실행이 Cowork 스케줄러를 깨운다"는 전제가 반증된 것.**
    다음 후보: (a) 숨겨진 창 핸들 `ShowWindow` 복원 (b) 08:47 종료 후 재기동
    (c) **장전 점검을 Cowork 에서 떼어 작업 스케줄러로 이관**(가장 견고, `Maitreya_EODretrain` 전례).
  - `0x80070005` 잔존 → AUMID 회수 실패. 설치 로그 `[1/4] launch :` 줄 확인.

- [ ] **미해소(경미) `NextRunTime` 이 `08:47:47`** — 트리거 초를 `[datetime]::Today.AddHours/AddMinutes`
  로 0 고정했고 스크립트도 `08:47:00` 을 출력했으나 결과는 `:47` 그대로.
  **원인 미규명**(RandomDelay 추정했으나 미확인). 08:57 대비 10분 여유라 실해 없음 —
  작업 XML(`schtasks /query /xml`)로 확인 가능. 우선순위 낮음.

### G-3(a) 판정 — v2(AUMID) 작동 확인 · `0x1` 은 실패 아님 (475차, 2026-08-18 18:27)

증거: `logs/claude_wake_task_verify.txt` (`scripts/claude_wake_diag.ps1` 생성)

- [x] **등록 내용 정상 확인**
  ```
  Execute      : C:\Windows\explorer.exe
  Arguments    : shell:AppsFolder\Claude_pzs8sxrjxfjjc!Claude
  StartBoundary: 2026-08-18T08:47:00+09:00     <- 초 00 (v2 수정 반영됨)
  DaysOfWeek   : 62  = Mon|Tue|Wed|Thu|Fri
  WakeToRun/StartWhenAvailable/Interactive/RunAs=pc1 : 전부 의도대로
  ```

- [x] **`LastResult = 0x1` 은 실패 신호가 아니다.** `explorer.exe` 는 기존 셸 인스턴스에
  활성화를 넘기고 빠지면서 1을 반환한다. **실제 실행은 두 증거로 확인됐다.**
  1. `LastRunTime 18:19:19` 직후 **18:19:35~42 에 Claude 프로세스 11개가 새로 생성**됐다
     (생존 중이던 것은 16:30:34 의 pid 14868 하나뿐). 16초 간격의 시간적 연결.
     ⚠ 사용자 수동 재시작 가능성을 완전히 배제하진 못했다.
  2. TEST-A(18:27:33) 전후로 pid 14132 의 **MainWindowHandle 이 789168 → 919994 로 변경**
     (StartTime 동일 18:19:35). 프로세스는 그대로인데 주 창 핸들이 바뀌었다 =
     **활성화 요청이 앱에 도달해 창이 재생성/전면화됐다.**
     TEST-B(`Start-Process shell:AppsFolder\...`)에서는 추가 변화 없음 — 이미 활성 상태.

  ⚠ **한계**: 이 시험은 `before` 스냅샷에 이미 `hWnd=789168 title="Claude"` 가 있었으므로
  **"트레이로 완전히 접힌 상태"에서의 시험이 아니다.** 그 조건은 08-19 08:47 이 최초 검증이다.

- [x] **`NextRunTime 08:47:47` 원인 좁혀짐 — 트리거는 정상이다.**
  `StartBoundary` 가 `T08:47:00` 이고 `RandomDelay` 는 비어 있는데 `NextRunTime` 만 `:47` 이다.
  → 트리거 정의가 아니라 **스케줄러의 다음실행 계산 쪽** 문제이며,
  `StartWhenAvailable=True` 가 최대 1분의 지연을 붙이는 동작으로 추정된다(**미확인**).
  08:57 대비 10분 여유라 실해 없음. **추가 조사 불요.**

- [ ] **미확인: `powercfg /waketimers` 는 관리자 권한 필요** — 진단에서 확인 실패.
  다만 **미륵이가 08:40 에 기동하므로 08:47 시점에 PC는 이미 깨어 있다.**
  `WakeToRun` 은 예비 안전장치이지 주 경로가 아니다. 우선순위 낮음.
  필요하면 관리자 cmd 에서 `powercfg /waketimers` 1회 확인.

- [x] **`\Maitreya` · `\Maitreya_EODretrain` 작업 존재 확인** — 장전 점검을 작업 스케줄러로
  이관하는 대안(플랜 B)의 전례가 실재함을 확증.

### 🔎 부수 발견 — `futures` 사본이 두 곳이다 (확인 필요)

- [ ] **`G:\내 드라이브\futures` 에 repo 사본이 있다.** 탐색기 즐겨찾기의 `futures` 바로가기가
  **Google Drive 쪽을 가리킨다**(작업 경로 `C:\Users\pc1\PycharmProjects\futures` 아님).
  `.git`·`config`·`dev_memory`·`docs` 등 동일 구조이고, 내부에
  `C  Users82108PycharmProjectsfuturesd…` 라는 이름이 뭉개진 폴더가 하나 보인다.
  **의도된 백업인지, 과거 동기화 잔재인지 미확인.**
  ⚠ 위험: 즐겨찾기를 눌러 **엉뚱한 사본을 편집**하면 커밋이 갈린다. 멀티PC 브랜치 격차
  (`v9-dev` ↔ `origin/dev` 320/315)와 겹치면 추적이 매우 어려워진다.
  → 주간회의에서 처분 결정. 백업 목적이면 즐겨찾기 이름을 구분되게 바꿀 것.

#### 후속 — 즐겨찾기 `futures` 정정 완료 (475차, 2026-08-18 18:35)

- [x] **`C:\Users\pc1\PycharmProjects\futures` 를 즐겨찾기에 고정(pin)했다.**
  실측 확인: 고정된 `futures` → `MW0602 > PycharmProjects > futures` ✅
- [x] **원인 규명 — G: 쪽은 '고정'이 아니라 '자주 사용하는 폴더'였다.**
  핀 아이콘이 없었다. 즉 누군가/무언가가 `G:\내 드라이브\futures` 를 **자주 열어서**
  자동 등재된 것이다. 실측: 그 항목 → `내 PC > Google Drive (G:) > 내 드라이브 > futures`
- [ ] **남은 질문 — 왜 G: 사본이 자주 열리는가.** 항목을 지워도 접근이 계속되면 다시 뜬다.
  후보: (a) Google Drive 가 repo 를 동기화 중 (b) 과거 작업 잔재 (c) 다른 도구가 그 경로를 참조.
  ⚠ **동기화 중이라면 `.git` 이 함께 동기화돼 두 사본의 이력이 갈릴 수 있다** —
  멀티PC 브랜치 격차(`v9-dev` ↔ `origin/dev` 320/315)와 겹치면 추적 불가에 가까워진다.
  **주간회의 안건.** 확인 방법: Google Drive 설정에서 동기화 폴더 목록 확인.
- [ ] (선택) 잔여 '자주 사용하는 폴더' 항목 제거 — 우클릭 → 즐겨찾기에서 제거.
  또는 폴더 옵션에서 '즐겨찾기에 자주 사용되는 폴더 표시' 해제(전역).

---

## 2026-08-18 (MW0602 475차 후속 — 장후 §2 Fix 구현) 신규 항목

세 국면 세션의 §2 Fix 를 구현하고 커밋했다(상세: `DECISION_LOG.md` 같은 날 `475차 후속`).
**push 는 하지 않았다** — 장전 F-3(주간회의 확인 후) 그대로.

### ✅ 완료 — 오늘 배포

- [x] **F-1(장후) = F-2(장전) ConfFloorGuard 상태 샘플** — 원인은 emit 위치가 아니라
  **calibrator 오참조**(런타임 실측 `AttributeError … 'output_max'`). `_resolve_conf_calibrator()`
  단일 접근자 + `except` **debug → warning(예외 타입 병기)**. 로그 문구·emit 위치 무변경.
  ⚠ **두 리포트가 같은 항목을 다르게 적었다** — 장후 리포트의 "zone 분기 안쪽" 가설은
  **틀렸다. 재인용 금지.**
- [x] **F-2(장후) 자본 대비 MDD 병기** — `daily_exporter` 배너 2줄, 분모는 registry
  단일 헬퍼(`_live_mdd_vs_capital`). `test_465` 문자열 검사 동반 갱신(취지 무변경).
- [x] **F-3(장후) §12 `전략판정` 원천 교체** — 조사 완료: 로그 배너는 **조건부**
  (`main.py:11517`). 원천을 `strategy_report_*.txt`(무조건 생성)로 교체. 실측 재현에서
  `🔴 고착` → `변동(UNDERPERFORM×7, INSUFFICIENT×1)`.
- [x] **F-3′(신규) INSUFFICIENT 사유 구체화** — `verdict_reason` 배선. `live_days=60`
  인데 "데이터 부족 (60일)" 이던 자체 모순 해소.
- [x] **`tests/test_475_conffloor_sample_and_verdict_source.py`** 신설(불변식 5종).
- [x] **`.git/index.lock` 스테일 제거** — 0바이트·mtime 16:32·6시간 경과·git 프로세스 0건.

### 다음 거래일(08-19) 관측 — 판정 기준

- [ ] **O-1(장전 O-3 계승)** `grep -c "ConfFloorGuard\] state=" logs/20260819_SIGNAL.log`
  → **≥300** 이고 값 **2종 이상**(`OK`/`BLOCKED`/`RAW`/`ZONE_BLACKOUT`)이면 F-1 성공.
  0818 실측(80건·1종)이 판정 기준선이다.
- [ ] **O-2(장전 O-4 계승)** `[ConfFloorGuard] 상태 샘플 실패` → **0건**이 정상.
  출현하면 **예외 타입이 그대로 찍히므로** 원인 특정이 바로 된다(승격의 목적).
- [ ] **O-3** `strategy_report_20260819_*.txt` 배너에
  `MDD=…%(자본대비) [peak대비 …%]` 와 `사유 : 판정 불가: WFA 기준선이 QA seed…` 가
  찍히는지. 자본 미상 폴백(`N/A(자본미상)`)이 뜨면 `SIZING_TARGET_CAPITAL_KRW` 로딩 실패다.
- [ ] **O-4** 수집기 §12 `전략판정` → `변동` 유지(무기록으로 떨어지면 리포트 파일 생성
  실패이므로 §4 마커 확인).
- [ ] **O-5** 대시보드 전략 탭 권고 배지 사유가 배너와 **같은 문장**인지(두 화면이 다른
  설명을 하면 확인 비용이 매번 든다).

### 이월 (미착수 — 의도)

- [ ] **F-4 `docs/정기점검` untracked 산출물 추적 편입** — 어제 항목과 동일. 중복 착수 금지.
  ⚠ 오늘 실측 untracked 는 리포트 3종 외에 `TASK_CLAUDE_WAKE_*.bat` 2 ·
  `scripts/claude_wake_*.ps1` 2 · `MIREUK_DAILYCHECK_HANDOFF.md` ·
  `0816_주간회의_검토보고_MW0602.md` — 합류 시 이 목록으로 갱신할 것.
- [ ] **F-5 = G-3(b) `.dailycheck.lock`** — 473차 후속2 등록 항목. **스테일 판정
  (mtime 10분 초과) 필수** — 오늘 `.git/index.lock` 이 정확히 그 형태로 커밋을 막았다.
  차수 충돌 회피(`475차-2` 접미 또는 국면 라벨 승격)를 범위에 포함.
- [ ] **G-1(장전) 분기편향 자동 탐지 `scan_branch_asymmetry()`** — 선행 F-2 가 오늘
  해소됐으므로 **착수 가능**. 회고 검증 기준: 08-14(0/364)·08-18(80/370) 둘 다 탐지.
  ⚠ 470차 C1'·C2'와 **같은 커밋** — 중복 구현 금지.

---

## 2026-08-18 (MW0602 475차 후속2 — 장후 §3 고도화 구현) 신규 항목

§3 G-1~G-3 구현·커밋 완료(상세: `DECISION_LOG.md` 같은 날 `475차 후속2`).
**push 안 했다** — 장전 F-3 그대로.

### ✅ 완료 — 오늘 배포

- [x] **G-3(장후) = G-1(장전) 분기편향 탐지** — 한 커밋(`79aa8a6`). 옵트인
  (`sample_axis: "minute"`), 분모는 "그 지표가 사는 로그가 살아 있던 분", 표본 없는
  날은 분모에서 제외. 현재 대상 `ConfFloor`·`CB_state` 2종.
  ⚠ **장전이 적은 회고 기준 "08-14(0/364)도 탐지"는 틀렸다 — 재인용 금지.**
  그날 0건은 편향이 아니라 미배포(470차 L2 커밋 08-14 19:47)이고 정답은 `무기록`이다.
- [x] **G-1(장후) 비용 3열** — §5에 gross/수수료/net + 부호역전 경고. 포지션 귀속은
  진입 시각(HH:MM). 0818 실측 gross +8,000 / 수수료 33,084 / net -25,084(414%).
- [x] **G-2(장후) `binding_gate` 축** — §12 첫 DB 지표. 판정은 `stuck_verdict` 공용.
  0818 `tox×29 · None×15 · hurst×13 · meta×8` (1일 65건 → 표본부족).
- [x] **수집기 read-only DB 계층(`db_rows`)** — G-1·G-2 공통 선행조건이었다.
  ⚠ `ensemble_decisions` 는 `predictions.db` 에 있다. `data/db/ensemble_decisions.db`
  는 **0바이트 유령 파일**이다(실측).
- [x] **실전 전환 게이트 ①④ 실측 배선** — 충족은 자동 선언하지 않는다(반증만).
  ④는 **합격선 부재로 영구 미측정**임을 코드가 말한다.
- [x] 회귀 3종 신설(`test_475_branch_asymmetry` · `_collector_db_axis` ·
  `_phase5_gate_measurement`). 합성 픽스처라 실로그·실DB 에 의존하지 않는다.

### 다음 거래일(08-19) 관측

- [ ] **O-6** §12 `ConfFloor` → F-1 수정이 들으면 관측률 **≈1.0** 으로 올라가고
  `분기편향`이 사라져야 한다. 안 사라지면 F-1이 안 들은 것이다(O-1과 교차 확인).
- [ ] **O-7** §12 `binding_gate` 관측일 **2일**로 증가(3일차 = 08-20에 첫 판정).
  값 집합이 `tox/hurst/meta/None` 그대로인지 — 늘어나면 게이트가 추가된 것이다.
- [ ] **O-8** §5 비용 축이 매일 렌더되는지. `비용 축 미측정`이 뜨면 DB 경로 문제다.
- [ ] **O-9** 대시보드 Phase 5 배지 툴팁 ①④에 `[자동]` 수치가 뜨는지(1분 타이머,
  TTL 300s 캐시).

### 주간회의 안건 (신규)

- [ ] 🔴 **④ 합격선을 정해야 한다.** "일일 수익률 변동성 안정적"은 수치 기준이 없어
  **영구 미측정**이다. 0818 실측: 20거래일 평균 +41,844원 · 표준편차 441,747원
  (**평균의 10.6배**) · 양수일 **7/20(35%)** · 최소 -473,373 / 최대 +1,074,209.
  → ①(통산 +836,870원)은 **소수의 큰 날**이 만든 흑자다. ①만 떼어 "충족"으로 읽지 말 것.
  ⚠ 기준은 **사전등록**이어야 한다(사후적합 금지) — 실측을 보고 통과하도록 정하면 안 된다.

---

## 2026-08-18 (MW0602 475차 후속3 — §5 R-1 조사) 신규 항목

### 🚫 폐기 — 구현 금지 (재제안 방지)

- [x] **R-1(TP1 보호트레일 protect_offset 감도 섀도) — 함정 ① 확정, 구현하지 않음.**
  캠페인 [25] `tp1_protect_offset_shadow`가 동일 질문을 더 강한 형태(소급 재생,
  95훅/16일)로 이미 측정했고, 📌 주간회의 확정 결정(2026-08-08 "미적용 유지 —
  atr_lock_0.75만 감시 후보")까지 끝났다. **이 질문을 다시 여는 것은 주간회의 소관.**
  상세: `DECISION_LOG.md` 같은 날 `475차 후속3`.

### ✅ 완료 — 오늘 배포 (재발 방지)

- [x] **수집기 §13 "확정 결정 레지스트리 (재론 주의)"** — 최신 주간 리포트에서
  결정 헤딩 추출·표 렌더(0818 실행 18건). 부재 시 미측정 렌더.
- [x] **관문 보강** — phases.md C-6 · report_template.md §5: R-* 쓰기 전 §13 대조.
- [x] `tests/test_475_decision_registry_visibility.py` 신설(불변식 5종 통과).

### 다음 거래일(08-19)~금요일(08-21) 관측

- [ ] **O-10** 08-19 다이제스트 §13이 정상 렌더되는지(18건±). `미측정`이 뜨면
  주간 리포트 FIFO 삭제 또는 경로 문제 — 원인 확인.
- [ ] **O-11 (08-21 금요일)** [25] watch 진행 확인 — **MW0602는 목표 도달**
  (95훅/16일 ≥ 70/13). MW0601 축적(08-08 기준 33훅/4일)이 게이트.
  양 PC 도달 시 주간회의 상정 가능 — ⚠ 도달해도 자동 적용 아님(클램프 33%
  단독 기각 사유 가능, `VALIDATION_CAMPAIGN_DECISIONS` note 참조).

---

## 2026-08-19 (MW0602 476차 — 장전 점검) 신규 항목

### 🔴 P1 — Fix (코드 변경은 **장후**. 라이브 프로세스 가동 중)

- [x] **[DONE 2026-08-19 장후 커밋 `7a335ac`] F-1 `batch_retrainer.py:508-510` X/y 가드 정밀화 — G-1 부활.**
  `_load_from_db()` 반환 직전 표식(id+shape+scheme+params) → `retrain_now()` 가드가
  "모르는 X/y만 지움". 오탐 방향 None(보수) 유지. **라이브 검증은 P-1(08-20 EOD)·P-2.**
  `retrain_eod.py:298` 이 만든 X(= 같은 인스턴스 `_load_from_db()` 산물)까지 지워서
  사이드카 `label_scheme=null` → `[EntryGate] 불일치 6/6` 이 **2거래일 연속**(08-18·08-19).
  "모르는 X/y만 `None`" 으로 좁힌다. **오탐 시 `None`(제한 유지)로 떨어지는 쪽이 기본.**
  ⚠ `_load_from_db` **반환 시그니처 변경 금지**(`batch_retrainer.py:2205` 주석).
  ⚠ **`py310_64` 서브프로세스 코드다** — `py37_32`로 되돌리지 말 것(191차).
  ⚠ **적용 시점 = 15:47 EOD 완료 이후.** 재학습 중 교체하면 그날 EOD가 구코드로 돈다.
  ⚠ **⑧ 해제와 같은 커밋에 섞지 말 것.** 실효 배수는 이미 F-1 폴백으로 ×1.0이라
     이 수정 자체는 노출을 늘리지 않는다 — 늘리는 것은 ⑧이다.
  상세: `DECISION_LOG.md` 2026-08-19 476차 [1].
- [x] **[DONE 2026-08-19 커밋 `7a335ac`] F-1 회귀 `tests/test_476_eod_label_scheme_roundtrip.py` 신설** —
  불변식 4종(아는 X 보존 / 모르는 X·id불일치·shape불일치 삭제 / 표식 수명 / F-3 문구 3분화).
  py310_64 PASS + 기존 `test_468_label_state_unlock.py` 통과 유지.
- [x] **[DONE 2026-08-19 커밋 `8972dcd`, F-2′로 범위 정정] F-2 수집기 `db_rows()` WAL 폴백 체인** —
  ① `mode=ro` → ② 스냅샷 복사(db+`-wal`+`-shm`) → ③ `immutable=1`(WAL 미반영 경고 동반).
  §12에서 **접속 실패(`⬛ DB미접속(미측정)`)와 표본 0(`🔴 무기록`) 분리**, §11 문구 교체.
  ⚠ `predictions.db` **917MB** — ① 실패 시 **1회만 복사해 세션 내 재사용**.
  ⚠ 수집기 표준 라이브러리 원칙 유지(`shutil`·`tempfile`은 표준).
- [x] **[DONE 2026-08-19 커밋 `7a335ac`] F-3 `learning/model_meta.py:54-56` 진단 문구 분리** —
  `키없음`(진짜 구버전) / `label_scheme=null`(기록경로 결함) / `빈값`.
  현재 셋 다 `"(구버전)"` 이라 어제 새로 쓴 사이드카를 "구버전"이라 부른다.
- [ ] **F-4 `TASK_CLAUDE_WAKE_INSTALL.bat` 실행 (사용자 1회, MW0602)** —
  장전 점검이 오늘 **09:07**(개장 7분 후)에 돌았다. **권고 목표 08:58**
  (`[PreRetrain]`·`[EntryGate]`·레짐 확정 후 · 개장 전). 이어서
  `TASK_CLAUDE_WAKE_VERIFY.bat` 로 `NextRunTime` 확인 후 아래 칸 기입.
  - [ ] MW0602 등록일: ____________ / 확인된 NextRunTime: ____________

### 🟡 고도화

- [ ] **G-1 EOD 재학습 자체 사후검증** — `retrain_eod.py` 가 마커를 쓰기 **전에**
  방금 저장한 사이드카 6개를 `model_meta.read_label_state()` 로 되읽어
  `[SelfCheck] label_state ok=… detail=…` 을 로그·마커에 남긴다. `ok=False` 는 **WARNING**
  (ERROR 쓰지 않는다 — 이 프로젝트는 ERROR를 안 남긴다).
  ⚠ **재학습을 실패로 만들지 않는다** — 모델 교체는 진행, 관측만 남긴다.
  선행: F-1(안 고치면 매일 `ok=False`만 찍힌다). 우선순위: **이번 주**.
- [ ] **G-2 §12 `CORE축퇴_피처` 축 신설** — 현재 `CORE준비도`는 **개수만** 센다
  (`1`×35). 어느 피처인지 알 수 없어 470차가 사전등록한 20거래일 판정 때 로그를 다시
  파싱해야 한다. `[CORE준비도] … 축퇴 N/6 — <피처>×6hz` 에서 **피처명 집합**을 값으로.
  ⚠ **차단 승격 아님 — 섀도 관측 축 추가일 뿐이다.**
- [ ] **G-3 장전 국면 관문 배너** — `--phase pre` 인데 생성 시각이 09:00 이후면
  다이제스트 최상단에 `⚠ 장전 점검이 개장 후 N분에 실행됨 — 이 시점까지 진입 M건` .
  F-4가 해결되면 자연히 사라진다 → **해결 여부의 지표**가 된다.

### 📄 문서 정정 (근거 오류 — 다음 세션이 오독한다)

- [x] **[DONE 2026-08-19] `references/invariants.md` §2-B 각주 "G-1 최초 검증 기회 = 2026-08-17(월)" → 정정 완료** (F-1 배포 반영: 최초 검증 기회 = 2026-08-20 EOD).
  **08-17은 휴장일이다** (`logs/20260817_SYSTEM.log` 48행: `공휴일·휴장일 — 다음 KRX 거래일
  08:45 재개`, `retrain_eod_20260817.log`·`eod_retrain_done_20260817.txt` 부재).
- [x] **[DONE 2026-08-19] `references/invariants.md` §2-B `MODEL_LABEL_STATE_UNLOCK_ENABLED` 행
  구조적 결함 확인(476차) + F-1 배포 사실 병기로 갱신 완료.**
  "관측을 못 했다"가 아니라 **"현 코드로는 관측될 수 없다"** 다. `⏳`를 "기다리면 된다"로
  읽으면 F-1이 영영 안 나온다.
- [ ] **CLAUDE.md 절대원칙 §2 ⑤ 실값 병기 제안** (문서만, 코드 무변경) —
  `CB_API_LATENCY_LIMIT=5.0`(Kiwoom 레거시, Cybos 미사용) vs 실질 `CB_PIPE_PAUSE_MS=5_000`
  (파이프 지연 → 5분 진입 정지). 468차 CB③ 병기와 같은 방식. `invariants.md` §3-B
  "발견하면 계속 보고" 지정 항목이며 2회 이상 반복 보고됐다.
- [ ] **26주 WFA 재검증에 C6' 신설 제안 — "사이드카·마커 계약 검증"**:
  쓰는 쪽(EOD)과 읽는 쪽(런타임)이 같은 필드를 같은 의미로 쓰는가.
  476차 [1]이 정확히 그 계약 파손이었다. C1'(열린 채 끝난 지표)·C2'(사이징 축) 계열.

### 🔵 기한 — 주간회의(2026-08-22 금) 상정

- [ ] 🔴 **CB② 복원 재검토 — 기한 2026-08-29 (남은 거래일 8일).**
  `CB_CONSEC_STOP_LIMIT` 9999 → 2~3. 2026-08-01 결산이 4주 연기하며 **"무기한 유예가
  아니다 — 반드시 재론"** 이라고 못박았다. Phase 5 조건 ②·⑤가 걸려 있다.
- [ ] **전환기준 ⑨(TOX-SEVERE-SPREAD) — "선행조건 유지 vs 26주 WFA 이관" 결정.**
  473차 F-8: 손익축 판정 표본 ETA **7.1개월**(진입 6포지션/4거래일). ⚠ 문턱 인하 금지(458차 D6).
- [ ] **⑧ 사이징 재설계 — 선행조건이 막혀 있음이 확인됨(476차 [1]).**
  F-1 → O-2(`불일치 0/6`) 통과가 선행. 그전에는 ⑧ 논의 진행 금지.

### 다음 거래일(08-20)~ 관측

- [ ] **O-1** F-1 적용 후 첫 EOD: `gbm_*_meta.json` **6/6 `"label_scheme":"fixed"`**.
  `null` 이면 F-1이 안 든 것.
- [ ] **O-2** 그 다음 장전: `[EntryGate] … 불일치 0/6` 또는 "모델 레이블 체계 미확인" 소멸.
  ⚠ **`×0.6` 소멸은 판정 근거가 아니다** — F-1 폴백만으로도 사라진다(`invariants.md` §2-B).
- [ ] **O-3** F-2 적용 후 §12 `binding_gate` 실제 값 분포(0818 기준 `tox/hurst/meta/None`).
- [ ] **O-4** §5 비용 3열(gross/수수료/net) 렌더 — 기존 O-8 계승.
- [ ] **O-5 (오늘 장후)** `[ConfFloorGuard] state=` 하루 전체 관측률.
  09:00~09:11 구간 **7/12(0.58)**. **≥0.9** 면 `bd3bbba` 완전 성공, 0.5~0.9면
  "복구 전 분기 무샘플" 가설 채택.
- [ ] **O-6** `[CORE준비도]` 의 `ofi_norm` 포함률 — 오늘 7/8(87.5%), 0818 29/42(69%).
  **3거래일 연속 ≥60%** 면 개수 축과 별개 안건으로 승격.
- [ ] **O-7** 장전 점검 실행 시각이 **09:00 이전**인가(F-4 적용 후). G-3 배너 미출현.
- [ ] **O-8** `[CybosProbe]` 초기 실패 — 오늘 08:58 10건 → 09:04 복구. 매일 같은
  패턴이면 정상(장전 TR 미개시), 09:10 넘겨도 실패면 별건.

### ✅ 완료 처리 (0818 후속2·후속3 등록분)

- [x] **O-10** 다이제스트 §13 확정 결정 레지스트리 — **18건 정상 렌더 확인**
  (원천 `validation_campaign_report_20260814.md`).
- [x] **O-2(0818)** `[ConfFloorGuard] 상태 샘플 실패` — **0건 확인**(`bd3bbba` 효과).
- [~] **O-6(0818)** ConfFloor 관측률 — **부분 충족.** §12 2거래일 합산 0.22이나
  오늘 구간은 0.58. 완전 판정은 위 O-5(장후)로 이월.

### 🔴 476차 추가 — 커밋 미실행 (사용자 조치)

- [x] **[DONE 2026-08-19 — 사용자가 락 해소, 산출물은 커밋 `385c616` 으로 반영됨]
  `.git/index.lock` 삭제 후 476차 산출물 커밋 (사용자, 윈도우에서).**
  샌드박스 마운트가 **unlink 불가**라 09:07 `git status` 가 만든 0바이트 락을
  스스로 못 지웠고, 이후 `git add` 가 `fatal: Unable to create '.git/index.lock': File exists`
  로 실패했다. ⚠ **다른 git 프로세스 때문이 아니다 — 라이브 프로세스를 죽이지 말 것.**
  ```bat
  ...
  ```
  이월 미커밋 7건(475차 산출물 · `TASK_CLAUDE_WAKE_*` · `0816_주간회의_검토보고_MW0602.md`)도
  함께 정리 권고 — 특히 주간회의 검토보고는 커밋 전에는 MW0601이 볼 수 없다.
- [x] **[DONE 2026-08-19 커밋 `8972dcd`] F-5 수집기 `run_git()` 에 `--no-optional-locks`** —
  인덱스 갱신을 시도하지 않아 `index.lock` 자체를 만들지 않는다(모든 git 호출 공통 적용).

---

## 2026-08-19 (MW0602 476차 — 장후 점검) 신규 항목

> 장전 등록분 F-1~F-5 는 **당일 커밋 0건이라 전부 미착수 그대로**다. 아래는 장후 신규분이며
> **중복 착수 금지**.

### 🔴 P1 — Fix

- [x] **[DONE 2026-08-19 장후 커밋 `8a158a4`] F-6 (Phase A) 09:20~09:29 게이트의 표기를 사실에 맞춘다 — 임계는 건드리지 않는다.**
  `main.py:7084-7091` 의 `[EntryGate] 조건부 구간 — A등급만 허용 (09:30까지)` 문구에
  `⚠ A는 conf≥0.70 요구이며 2026-06 이후 라이브 도달 0건 — 실질 전면 금지` 병기.
  `CLAUDE.md` 「확률 판단 기준」표에 468차 CB③ 병기 방식의 각주(라이브 방향성 conf
  최근 30거래일 p50 0.3478 / p100 0.5970, 상위 2행 도달 불가). **코드 로직·임계 무변경.**
  ⚠ `main.py` = **py37_32** 런타임, **COM 콜백 밖**.
  ⚠ **임계 인하·존 재정의는 이 항목에 포함하지 않는다** — 매매 정책 변경이라 주간회의 소관.
  상세: `DECISION_LOG.md` 2026-08-19 476차 [장후 발견 P1].
- [x] **[DONE 2026-08-19 커밋 `8a158a4` — 4/4 PASS] F-6 회귀 `tests/test_476_grade_reachability.py` 신설** —
  474차 `tests/test_473_core_group_reachability.py` 방식의 **불변식 고정**.
  ① `ENTRY_GRADE` 에 A/B 존재 ② 앙상블 A 임계 = 0.70 ③ 정의가 바뀌면 테스트가 깨져
  문서 갱신을 강제. ⚠ **라이브 분포 단언은 테스트에 넣지 말 것**(CI 불안정) — 분포는 G-4.
- [x] **[DONE 2026-08-19 커밋 `8972dcd`] F-2′ F-2 범위 정정 — 실패 조건은 "샌드박스"가 아니라 "라이브 프로세스 가동 중"이다.**
  장후 16:32 실행에서 `file:…?mode=ro` 가 **그대로 성공**했다(§12 `binding_gate` 실제 분포
  `hurst6/meta2/tox1/None1` 렌더). 폴백 체인은 **장전·장중에만** 필요하다.
  ⚠ `predictions.db` **929MB** — ① 실패 시 세션당 **1회만** 복사.
  §12 에서 `⬛ DB미접속(미측정)` 과 `🔴 무기록` 분리는 그대로.

### 🟡 P2 — Fix

- [x] **[DONE 2026-08-19 커밋 `8972dcd` — `daily_close` 패턴 + 폴백, 하위호환 유지] F-7 수집기 §5 배너 검산 패턴 교체** —
  찾는 토큰이 `오늘 PnL` 인데 라이브 문구는
  `일일 마감 | 승=(\d+) 패=(\d+) PnL=([-\d,]+)원` 이다(오늘 실측 `승=3 패=7 PnL=-711,815원`,
  DB −711,816원과 1원 차). 정규식 추가 + 기존 토큰 하위호환 유지, **둘 다 없을 때만** 경고.
  `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py`
- [x] **[DONE 2026-08-19 커밋 `8972dcd` — 스모크 실측 관측률 1.16(≥1 정상)] F-8 ConfFloorGuard 샘플 축 정정 (docstring + 수집기 §12)** —
  `model/ensemble_decision.py:403-424` docstring "**매분 무조건**" → "**앙상블이 계산된
  매 분(`compute()` 진입 시) 무조건**". **코드 무변경**(건너뛴 분에는 판정 대상이 없다).
  수집기 §12 `ConfFloor` 의 `sample_axis` 를 `"minute"` → `"ensemble_minute"`,
  분모를 `[Ensemble] dir=` 출현 분 수로.
  실측 근거: 09:00~15:10 생존 347분 · ConfFloor 306분 · Ensemble 306분 — **교집합 306,
  차집합 양방향 0** → 앙상블 축 **1.00**.
  ⚠ 축을 늘리면 다른 지표가 잘못 편입될 수 있다 — **`ConfFloor` 하나만 옵트인**하고
  `references/invariants.md` §5 "분기편향 옵트인 목록"에 축 종류가 늘었다는 사실을 병기.

### 🟢 고도화

- [x] **[DONE 2026-08-19 커밋 `172395e` — §12b. 실측: 앙상블A/B `미도달 10일 연속` max 0.597, TOX 표본 2일. known(규명 안건)은 적신호 제외] G-4 §12 `threshold_reachability` 축 신설 — "임계 미도달"을 매일 자동 탐지.**
  §12 는 *한 값 고착*(죽은 지표), 470차 C1' 은 *열린 채 끝남*, 475차 G-1 은 *분기편향* 을
  잡는다. **네 번째 형태 = "로그는 정상인데 그 로그가 지키려던 분기가 도달 불가"** 는
  아무도 안 본다(1-9 가 정확히 그것 — `[EntryGate] 조건부 구간` 은 **54회 정상 출력**됐다).
  `(임계, DB컬럼, 조건)` 레지스트리로 `max(관측) < 임계` 가 **N거래일 연속**이면
  `⚠️ 임계 미도달(N일)`. 최초 등록 후보 3쌍:
  ...
  ③ `(20.0, ensemble_decisions.spread_ticks, entry_executed=1)` — `TOXICITY_SEVERE_SPREAD_BLOCK_TICKS`
     (473차 F-8 의 "표본이 오지 않는다"가 자동으로 보인다. ⚠ **판정 기준 무변경** — 사전등록)
  ⚠ `benign` 화이트리스트 필수(조건부 임계는 미도달이 정상).
  선행: **F-2′**. ⚠ **475차 G-1·470차 C1'/C2' 와 같은 커밋 계열 — 중복 구현 금지.**
- [x] **[DONE 2026-08-19 커밋 `8a158a4` — AGENTS.md '테스트' 절 신설(낡은 "프레임워크 없음" 서술도 정정)] G-5 도달성 테스트 명명 규약** — `tests/test_*_reachability.py` 를 `AGENTS.md` 에
  한 줄로 정하고, 새 게이트·등급·임계 신설 시 같은 형식의 **정의 대조 테스트**를 동반.
  분포 의존 단언은 G-4(점검 도구)에 두고 테스트는 정의만 본다.
- [x] **[DONE 2026-08-19 커밋 `172395e` — `강제청산_경로`, value_map 으로 0(경로사망)/≥1(생존) 정규화. 실측 `≥1(생존)`×4] G-6 `[SchedForceExit] … bar_pass=N회` 를 §12 `benign` 으로 등록** —
  오늘 `15:11:16 … bar_pass=2회 → 청산 대상 없음(정상)` 이 정상 출현했으나 **어디에도
  집계되지 않는다.** 470차 C2' 가 `신뢰도배수` 를 benign 으로 등록한 것과 같은 취지
  (*발견이 아니라, 규명된 것도 계속 보이게 두는 것*). `bar_pass=0` 이면 1차 경로 재사망.
  Phase 5 조건 ② 선행 확인(F-1R 리허설)까지의 공백을 매일 관측으로 메운다.

### 📄 문서 정정 (근거 오류 — 다음 세션이 오독한다)

- [x] **[DONE 2026-08-19] `references/invariants.md` §6 ① 의 "0818 기준 20거래일 +836,870원" 에 날짜 병기 필수.**
  **2026-08-19 기준은 −810,390원(부호 반전)** 이고 `_chk_paper_profit` 이 `OPEN`(미충족)을
  반환했다. 롤링 창이라 날짜 없는 인용은 다음 세션을 오도한다.
  ⚠ 분해: 창 이동 −1,647,260원 중 오늘 손실 43% / **07-20(+935,444원)이 창 밖으로 밀려난
  효과 57%** — "오늘 나빠져서 뒤집혔다"로 쓰지 말 것.
- [x] **[DONE 2026-08-19] `references/phases.md` C-5 에 "검증 캠페인은 금요일 전용" 명시** —
  08-18·08-19 는 P8 에서 정상 종료했는데 체크리스트만 보면 "캠페인 리포트 없음"이
  결함처럼 읽힌다. 08-14(금)만 10/10 rc=0.
- [ ] **26주 WFA 재검증 항목에 "앙상블 등급 임계(0.70/0.60)" 편입 제안** —
  `invariants.md` §5 드리프트 항목(`ENTRY_HORIZON_B1/B2`·`CB_ACCURACY_MIN_30M`)과 동일 성격.
  라이브 conf 분포가 2026-05→06 에 급변했는데 임계만 고정됐다.

### 🔵 기한 — 주간회의(2026-08-22 금) 상정

- [ ] 🔴 **CB② 복원 재검토 — 기한 2026-08-29 (남은 거래일 7).** (이월, 변동 없음)
- [ ] **전환기준 ⑨(TOX-SEVERE-SPREAD) — 선행조건 유지 vs 26주 WFA 이관.** (이월)
- [ ] **⑧ 사이징 재설계 — 선행조건 F-1→O-2 미통과. 그전에는 논의 진행 금지.** (이월)
- [ ] 🆕 **1-9 등급 임계 재설계 — 3안 중 택일.**
  (A) 임계를 라이브 분포에 맞춰 인하하고 09:20~09:29 조건부 허용을 실질 복원
  (B) `TIME_ZONES` 에 09:20~09:29 를 **명시적 진입 금지 존**으로 편입
  (C) 현행 유지 + 표기만 정정(F-6 Phase A — 오늘 바로 적용 가능)
  ⚠ (A)(B)는 **노출을 바꾼다** — 섀도 계측·표본이 선행. 손익 함의는 **미확정**이다
  (통과시켰다면 이겼을지 알 수 없다).

### 다음 거래일(08-20)~ 관측

- [ ] **P-1** `model/horizons/gbm_*_meta.json` **6/6 `"label_scheme":"fixed"`**.
  `null` 이면 F-1 미적용 — 오늘로 **3거래일 연속 null**.
- [ ] **P-2** 장전 `[EntryGate] … 불일치 0/6`. ⚠ **`×0.6` 소멸은 판정 근거가 아니다.**
- [ ] **P-3** 장전·장중 §12 `binding_gate` 값 분포 렌더(장후 실측 `hurst6/meta2/tox1/None1`).
- [x] **P-4** 장후 §12 `ConfFloor` **당일** 관측률 **≥0.95**(F-8 적용 후). 오늘 원시 0.88 / 앙상블 축 1.00.
  → **0820 당일 1.000(330/330) 통과.** ⚠ 문구 정정(477차 후속 F-2): §12 렌더 합산값(0.72)은
  08-18(배포 전=미측정)이 섞인 값이라 판정 기준이 아니다 — **당일값**으로 판정한다.
- [ ] **P-5** §5 배너 검산 줄 출현(F-7 적용 후). 오늘 차 1원.
- [ ] **P-6 (2026-08-20)** `[CORE준비도]` `ofi_norm` 포함률 — **3거래일 연속 ≥60%** 판정.
  실적 08-18 71.4% · 08-19 **100%**.
- [ ] **P-7** 장전 점검 실행 시각 **09:00 이전**(F-4 사용자 조치 후). 오늘 09:07 — 3거래일 연속 개장 후.
- [ ] **P-8 (매 장후)** 롤링 20거래일 통산 net 부호. ⚠ **단일일로 판정 금지**(창 구성 효과).
- [ ] **P-9 (매 장후)** `[SchedForceExit] … bar_pass=N회` 가 `N≥1` 인가. `0` 이면 1차 경로 재사망.
  ~~G-6 등록 전까지는 수동 grep~~ → **G-6 등록 완료(0819)** — §12 `강제청산_경로` 로 매일 자동 표시.

### ✅ 완료·종결 처리 (장전 등록 O-* )

- [x] **O-5 종결** — ConfFloor 관측률. 가설 *"복구 전 분기 무샘플"* **기각**.
  앙상블 축 **306/306 = 1.00**, `bd3bbba` **완전 성공**. 후속은 F-8(계측 축 정정).
- [x] **O-8 종결** — `[CybosProbe]` 798건 중 786건 `ok`, 실패 12건 전부 **08:58 기동 시
  1회성**. 09:02 부터 정상 — **매일 같은 패턴이면 정상**이라는 장전 판정 확정.
- [x] **O-4 충족** — §5 비용 3열(gross/수수료/net) 정상 렌더.
- [~] **O-3 원인 정정** — DB 축 실패는 **라이브 프로세스 WAL 점유** 조건. → F-2′ 로 이관.
- [ ] **O-1 미충족(3거래일 연속)** — 사이드카 6/6 `label_scheme=null`. → P-1 로 계승.
- [ ] **O-7 미충족** — 장전 09:07 실행. → P-7 로 계승.

### 🔴 476차 장후 추가 — 커밋 미실행 (사용자 조치, 장전에서 이월)

- [x] **[DONE 2026-08-19 — 커밋 `385c616` (3국면 리포트·dev_memory·이월분 포함)]
  `.git/index.lock` 삭제 후 476차 3국면 산출물 커밋 (사용자, 윈도우에서).**
  ...
  `scripts\claude_wake_*.ps1`)도 함께 정리 권고 — **주간회의 검토보고는 커밋 전에는
  MW0601이 볼 수 없다.**

---

## 2026-08-20 (MW0602 477차 — 장전 점검)

### 🔴 P1 — Fix (⚠ 전부 **장후·오늘 EOD 완료 후** 적용. 오늘 EOD는 476차 F-1의 P-1 판정일이다)

- [ ] **F-1 `increment_session()` 날짜 롤오버 키 전량 소실 수정** —
  `strategy/runtime/session_recovery_service.py:101-121`. 화이트리스트(보존키 나열) →
  **블랙리스트(당일 종속키만 리셋)로 반전**.
  `_DAILY_RESET_KEYS = ("count","auto_shutdown_done_date","today_open",
   "circuit_breaker_state","profit_guard_state")` 외 전부 `dict(data)`로 보존.
  🔴 **CB/ProfitGuard 상태를 반드시 리셋 목록에 넣을 것** — 빠뜨리면 전일 CB 상태가 날짜를
  넘어 복원돼 절대원칙 §2 판정이 오염된다(현행 동작과 동일하게 유지하는 것이 목표).
  런타임 `py37_32`, COM 콜백 밖. 절대원칙 6종 저촉 없음. 섀도 계측 불요(노출 무영향).
- [ ] **F-2 EKS 원인 태깅 마커파일 폴백** — `main.py:7643` 부근.
  `_ss_eks.get("p8_last_success_date","")`가 빈 값이면 `data/eod_retrain_done_{어제}.txt`
  존재로 폴백(08:55 PreRetrain이 이미 쓰는 같은 원천). 발동 조건·차단 동작 무변경, **원인
  태그 문자열만** 바뀐다. F-1과 같은 커밋.
- [ ] **`tests/test_477_session_state_rollover.py`** 불변식 4종 —
  ① `p8_last_success_date`·`eod_retrain_ok_date` 생존 ② `gbm_total_retrain_count` 생존
  ③ `count=0`·`auto_shutdown_done_date=""` 리셋 ④ `circuit_breaker_state`·
  `profit_guard_state` **부재**.

### 🟡 P2 — Fix

- [ ] **F-3 `git push origin dev`** (사용자, 윈도우에서 라이브 종료 후) — 현재 **ahead 5**.
  미푸시 `8972dcd`·`7a335ac`·`8a158a4`·`172395e`·`77fe6e8` 전부 `[MW0602] 476차`.
  **푸시 전에는 MW0601이 476차 후속(F-1 사이드카·G-4 §12b·G-5 도달성 테스트)을 볼 수 없다** —
  CLAUDE.md 멀티PC 규약의 목적이 푸시 전에는 달성되지 않는다.
- [ ] **F-4 `TASK_CLAUDE_WAKE_INSTALL.bat` 실행 (사용자 1회, MW0602)** — 이월, 변동 없음.
  **P-7 4거래일 연속 미충족**(08-18·19·20 + 오늘 09:07). 미실행 시 장전 점검이 계속 개장
  후에 돌아 장전/장중 표본이 한 파일에 섞인다.

### 🟢 고도화

- [ ] **G-1 상태 영속 파일 키 계약 불변식 테스트** — `tests/test_477_state_contract.py`.
  `session_state.json`에 **쓰는 곳**(`retrain_eod.py:190-191`, `main.py:4553-4554`,
  `main.py:4179`, `main.py:11092`)과 **읽는 곳**(`main.py:7643`, PreRetrain)의 키 집합을
  소스에서 정적 추출해 **"쓰는 키 ⊆ 롤오버 생존키 ∪ 당일리셋키"** 강제.
  `test_*_reachability.py` 규약(AGENTS.md)의 자매 규약으로 `test_*_contract.py` 추가.
  **선행: F-1 배포**(대조 대상 목록이 코드에 명시돼야 한다). 우선순위 이번 주.
- [ ] ⏸ **[보류 2026-08-23 488차 — 도입 명분 소멸, 폐기 아님]** **G-2 수집기 §12에 `state_file_keys` 축 신설** — `data/session_state.json`의
  `p8_last_success_date`·`eod_retrain_ok_date`·`gbm_total_retrain_count` 3키 존재 여부를
  매일 `있음/없음`으로 렌더. F-1 **배포 전에** 넣어야 전/후 대비가 관측된다 →
  **F-1 검증 그 자체**가 된다(476차 G-6 15:10 하트비트와 같은 형태). read-only, 경합 없음.
  → **488차 보류 사유**: 핵심 명분(F-1 전/후 대비)은 F-1 08-20 배포·08-21 P-2 통과로 소멸.
  코드 측 회귀는 `test_477_state_contract.py`(488차 계획 E)가 커밋 시점에 잡는다.
  **재평가 조건**: 계획 E 배포 후에도 O-3(유령 프로세스) 지속 시.
  근거: `MW0602-20260823-점검리포트.md` 부록 D-4.
  ⚠ 오늘 규명을 잡아낸 것은 자동 계측이 아니라 사람이 파일을 직접 연 것이다.
- [ ] **G-3 (오늘 장후) `_dump_surviving_threads()` 출력 실독** — `main.py:11772`.
  전일 프로세스 잔존 **8거래일 연속**인데 덤프를 아무도 읽지 않았다면 471차 F-2 하트비트와
  같은 "죽은 계측"이다. **새 조치 제안 아님** — 기존 결정("코드 변경 없음, 진단만 먼저")이
  전제한 진단 수집이 실제로 되고 있는지만 확인.
- [ ] **26주 WFA `C6'`(사이드카·마커 계약 검증)에 `session_state` 키 계약 흡수 제안** —
  이미 등록 제안이 있는 항목과 같은 뿌리다. **새 관리 포인트를 만들지 않는다**(317차 원칙).

### 📄 문서 정정 (근거 오류 — 다음 세션이 오독한다)

- [ ] **`DECISION_LOG.md` `### [발견] session_state.json이 전일 EOD 기록을 잃는다` 항목에
  477차 규명 링크 병기** — 그 항목의 *"유령 프로세스가 낡은 스냅샷으로 덮어썼을 가능성"*
  가설은 **반증됐다**(원인은 `increment_session()`). 다음 세션이 그 가설을 근거로 종료
  로직을 건드리지 않도록 반증 사실을 원 항목에 병기할 것. **유령 프로세스 자체는 별개
  현상으로 여전히 미해결**(8거래일 연속)이라는 점도 함께 적을 것.

### 🔵 기한 — 주간회의(2026-08-22 금) 상정

- [ ] 🔴 **CB② 복원 재검토 — 기한 2026-08-29 (남은 거래일 7).**
  **이번 주 금요일이 기한 전 마지막 주간회의다.** 절대원칙 §2가 *"무기한 유예가 아니다 —
  2026-08-29에 반드시 재론할 것"* 이라 못박았다.
- [ ] **전환기준 ⑨(TOX-SEVERE-SPREAD) — 선행조건 유지 vs 26주 WFA 이관.** (이월)
  §12b 최근 2일 진입 max **9.003틱 < 20** — 표본 여전히 안 옴.
- [ ] **1-9 등급 임계 재설계 — 3안 중 택일.** (이월) §12b **11일 연속 미도달** max 0.597.
- [ ] **⑧ 사이징 재설계 — 선행조건 P-1→P-2 미통과. 그전에는 논의 진행 금지.** (이월)
  ⚠ 재산출 시 실제잔고 괴리 추이를 병기할 것: 08-10 **-40.5%** → 08-20 **-42.3%**
  (실제잔고 28,874,174원 / 목표 5천만원). 목표자본이 실잔고의 1.73배라 `MarginCap`이
  사이저보다 먼저 자른다(470차 실측).

### 다음 거래일(08-21)~ 관측

- [ ] **P-1 (오늘 08-20 EOD)** `model/horizons/gbm_*_meta.json` **6/6 `"label_scheme":"fixed"`**.
  `null`이면 476차 F-1 미적용 — **4거래일 연속 null**.
- [ ] **P-2 (08-21 장전)** `[EntryGate] … 불일치 0/6`.
  ⚠ **`×0.6` 소멸은 판정 근거가 아니다** — 오늘 08:55:15에 `전일 EOD 모델 적재 확인 —
  사이즈 제한 해제 (×0.6 → ×1.0)`가 이미 찍혔으나 그것은 **F-1 축**이다.
- [ ] **P-3** §12 `binding_gate` 값 분포 렌더. (이월)
- [ ] **P-4 (장후)** §12 `ConfFloor` **당일** 관측률 **≥0.95**(앙상블 축). (이월 — 문구 정정
  477차 후속 F-2: 합산 렌더값이 아니라 §12 관측률 3값 중 **당일** 열로 판정한다)
- [ ] **P-5 (장후)** §5 배너 검산 줄 출현. (이월)
- [ ] **P-6 (오늘 장후)** `[CORE준비도]` `ofi_norm` 포함률 — 장전 잠정 71.4%/100%/**100%(7/7,
  부분표본)**. **장후 전량으로 재계산 후 종결**할 것(313차 — 부분 표본 확정 금지).
- [ ] **P-7 (08-21 장전)** 장전 점검 실행 시각 **09:00 이전**. 오늘 09:07 — **4거래일 연속 미충족**.
- [ ] **P-8 (매 장후)** 롤링 20거래일 통산 net 부호. ⚠ **단일일 판정 금지**.
- [ ] **P-9 (매 장후)** `[SchedForceExit] … bar_pass=N회` ≥1. 전일 실측 **2회** ✅.
- [ ] 🆕 **O-1** F-1 배포 다음 장전: `[PreRetrain] EOD 마커 파일 직접 확인 … (session_state
  미기록 보완)` **소멸**하고 1차 경로 문구로 전환.
- [ ] 🆕 **O-2** F-1 배포 다음 장전: `data/session_state.json`에
  `p8_last_success_date`·`eod_retrain_ok_date`·`gbm_total_retrain_count` **3키 생존**.
  ⚠ **동시에 `circuit_breaker_state`·`profit_guard_state`는 부재**여야 한다(회귀 가드).
- [ ] 🆕 **O-3 (매 장전)** 런처 `[GUARD] 기존 프로세스 종료 완료` 연속일. 오늘 **8/8**.
...
> 리포트: `docs/정기점검/매일점검/MW0602-20260820-점검리포트-post.md`
> ⚠ 본 세션은 예약 점검(자동 실행)이라 **코드를 변경하지 않았다.** 아래는 전부 미착수다.

### 🔴 P1 — Fix

- [x] **F-1 `ensemble_decisions`에 실제 진입 계약수 축 신설** — **[완료 2026-08-20 커밋
  `e8b664a`]** `entry_qty_final` 신설(마이그레이션+INSERT 55컬럼 arity 검증). ⚠ 실제 INSERT
  위치는 `learning/prediction_buffer.py:save_step9_batch`였다(아래 `main.py:3008`은 trades
  INSERT — 리포트 §2 F-1의 위치 지목 오류, 구현에서 정정). 라이브 판정은 O-6.
  `main.py:9643` 에 `decision["entry_qty_final"] = int(_qty_auto)` **한 줄 추가**.
  🔴 **기존 `entry_qty`는 절대 건드리지 말 것** — 의미를 바꾸면 2026-08-20을 경계로 같은 컬럼이
  다른 것을 세게 된다(461차 `mdd_pct` 불연속과 동일 유형, 계측 4원칙 ①).
  동반: `main.py:3008/:3050` INSERT 컬럼·튜플 + `ALTER TABLE ensemble_decisions ADD COLUMN
  entry_qty_final INTEGER`(IF NOT EXISTS 가드).
  근거: 140포지션 중 **6건(4.3%)**이 `trades.entry_qty`와 어긋나고 **전부 DB 쪽이 1계약 크다**
  (07-31·08-04·08-14×2·08-18·08-20 — 전부 `[MarginCap]` 발생일).
  런타임 `py37_32`, COM 콜백 밖, 절대원칙 6종 저촉 없음, 매매 정책·게이트 임계 무변경.
- [x] **`tests/test_477_entry_qty_attribution.py`** 3종 — **[완료 2026-08-20 `e8b664a`]**
  T1~T4 전부 통과(py37_32).
- [x] **F-2 수집기 §12 관측률 일자축 분리** — **[완료 2026-08-20 커밋 `e84101d`]**
  `합산/당일/최소일` 3값 렌더 + 분기편향 판정 당일값 전환. 0820 재실행 검증:
  ConfFloor `1.00/**1.00**/1.00 (계측 08-19~)`, §11 신규 오탐 없음. (원 계획) —
  `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py:scan_stuck_indicators()`.
  `sum(분자)/sum(분모)` 단일값 → **`합산 / 당일 / 최소일` 3값** 렌더.
  🔴 **분기편향 판정(`branch_ratio_min=0.5`)을 당일값 기준으로 바꿀 것** — 합산은 참고 표시.
  근거: `ConfFloor` §12 렌더 **0.72** vs 일자별 **08-18 0.223(배포 전) / 08-19 1.000 /
  08-20 1.000**. 08-18은 476차 F-8 배포 이전 = **미측정**인데 합산에 섞였다(계측 4원칙 ②).
  동반 문서: §12 각주 · `invariants.md` §5 분기편향 칸.
- [x] **`NEXT_TODO` P-4 판정 기준 문구 정정** — **[완료 2026-08-20 `e84101d`]**
  두 P-4 항목(0819·0820 관측 목록) 모두 *"**당일** 관측률 ≥0.95"*로 정정, 0820 통과 병기.

### 🟡 P2 — Fix (이월, 변동 없음)

- [x] **F-3 `git push origin dev`** — **[완료 확인 2026-08-20 밤]** `git status -b` ahead 0
  (476차 5커밋 + 477차 `b4fd2d3` 포함 동기화 확인). 477차 후속 커밋분은 재푸시 필요(§아래).
- [ ] **F-4 `TASK_CLAUDE_WAKE_INSTALL.bat` 실행** (사용자 1회, MW0602) — **P-7 5거래일 연속 미충족**.
- [x] **477차 장전 F-1·F-2 (`increment_session()` 키 보존 + EKS 마커 폴백)** —
  **[완료 2026-08-20 커밋 `b4fd2d3`]** 회귀 테스트 포함 배포. O-1·O-2(08-21 장전)가 판정한다.
- [ ] **477차 장중 F-1i·F-2i·F-3i** — 이월(CB③ 억제 계측 승격 · CB③-P4 이중로그 제거 · 문서 정정 3건).

### 🟢 고도화

- [x] **G-1 지표별 `measured_since` 마커** — **[완료 2026-08-20 커밋 `e84101d`]**
  ⚠ 구현 위치는 `dailycheck_targets.json`이 아니라 수집기 `DEFAULT_CONFIG.patterns`의
  지표별 선택 필드다 — json의 같은 키는 목록을 **통째로 대체**하므로(load_config 규약)
  json 등록은 오히려 기본 패턴을 지운다. 초기 등록 5종 + 계측 시작 전 지표 `표본부족`
  처리 + 26주 갱신 행(invariants §5). (원 계획) — `config/dailycheck_targets.json`에 선택 필드
  `measured_since: "YYYY-MM-DD"`를 두고, 수집기가 그 이전 일자를 **분모·분자에서 제외**한 뒤
  관측률·표본을 낸다. 표에 `(계측 2026-08-19~)` 병기.
  초기 등록: `ConfFloor`=2026-08-19(476차 F-8) · `binding_gate`=2026-08-14(471차 후속6) ·
  `강제청산_경로`=2026-08-14(471차 F-2) · `CORE준비도`=2026-08-14(470차 B4).
  **선행: F-2**(같은 함수). 목적 = 「미측정」과 「측정했는데 낮다」를 **도구가** 가르게 하는 것.
- [x] **G-2 `[MarginCap]` 무조건 상태 샘플** — **[완료 2026-08-20 커밋 `dfc4a3d`(main.py)
  + `e84101d`(§12 `MarginCap_state` 축, measured_since=08-21)]** 형식은
  `state=OK|CAP|BLOCK {dir} 산출=N 상한=M`(가용은 상한과 동일값이라 생략). 기존 축소/차단
  로그 무변경(파서 호환). 라이브 첫 관측 = 08-21. (원 계획) — 증거금 조회가 성공한 매
  사이클에 `[MarginCap] state=OK|CAP 산출=N 상한=M 가용=K` 1줄. 이후 수집기 §12
  `stuck_indicators.patterns`에 `margin_cap_state` 축 추가.
  ⚠ **증거금 조회 자체를 늘리지 말 것** — 이미 `_margin_check_eligible`로 걸러진 사이클에만 붙인다
  (COM `BlockRequest` 추가 비용 0).
  근거: **470차 C2'가 이미 등록한 항목**이며(*"조건부 로그라 §5 규약 위반 — 무조건 상태 샘플을
  먼저 만들 것"*), 오늘 이상점 1-1이 **그 조건부성이 실제 오귀속을 만든 사례**다.
  🔴 **전환기준 ⑧의 직접 입력** — 470차의 사슬(*실전 자본 재설정 → 증거금 상한 상승 → qty≥3
  발생 → [28] 표본 축적*)에서 **첫 칸이 지금 측정되지 않고 있다.**
- [x] **G-3 `MainThread` 스택 1프레임 덤프** — **[완료 2026-08-20 커밋 `dfc4a3d`]**
  `[SHUTDOWN-MARKER]` + `dump_traceback(all_threads=False)` 1회를 crash_fault.log에 append.
  판독법: **마커 이후의 30초 주기 Timeout 덤프 MainThread 프레임 = 루프 종료 후 잔류 지점**
  (dump_traceback_later는 루프 종료 후에도 계속 돈다). 종료 로직 무변경. 첫 판독 = 다음
  유령 발생일 아침 crash_fault.log.
- [x] **26주 WFA 재검증 목록에 「계측 배포일 마커 갱신」 행 추가** (`invariants.md` §5) —
  **[완료 2026-08-20 `e84101d`]** 분기편향 칸 ⑤ 병기 포함.

### 📄 문서 정정 (근거 오류 — 다음 세션이 오독한다)

- [ ] **`invariants.md` §2-B `MODEL_LABEL_STATE_UNLOCK_ENABLED` 관측란에 판별 규칙 병기** —
  `main.py:4921-4938` 실측: `_refresh_pre_retrain_state`(G-1)가 **먼저** 돌고 F-1 마커 경로는
  `if not self._pre_retrain_done` 안에 있다. **⇒ G-1 성공 시 F-1 줄은 구조적으로 침묵한다.**
  따라서 두 축은 로그만으로 판별된다 — `모델 레이블 체계 확인(장전)`이 뜨면 G-1,
  `전일 EOD 모델 적재 확인`이 뜨면 F-1. §2-B가 경고한 *"×0.6 소멸을 G-1 검증으로 읽지 마라"*가
  이제 **기계적으로 확인 가능**해졌다는 사실을 적을 것.
- [x] **`evidence_map.md` §8-2 `ensemble_decisions` 컬럼 설명 보강** — **[완료 2026-08-20
  `e8b664a`]** 컬럼 목록 + 경고 블록 병기.
- [x] **`CLAUDE.md` 전환기준 ⑧ [28] 문단에 선행조건 1줄 추가** — **[완료 2026-08-20
  `dfc4a3d`]** 배선 완료 사실 + O-6 라이브 대사 예정까지 병기.

### 🔵 기한 — 주간회의(2026-08-22 금) 상정

- [ ] 🔴 **CB② 복원 재검토 — 기한 2026-08-29 (남은 거래일 6). 이번 금요일이 기한 전 마지막
  주간회의다.** 절대원칙 §2가 *"무기한 유예가 아니다"*로 못박았다.
  **오늘 관측이 안건 무게를 더한다**: CB②는 카운터만 오르고(연속 손절 2회) 정지 안 함,
  CB③은 억제 계측 사망(477차 장중 P1-i-1), CB③-P4는 차단 비활성.
  → **전환기준 ②는 현재 CB①·④·⑤로만 충족 가능**하다는 사실을 안건 설명에 병기할 것.
- [ ] **전환기준 ⑨(TOX-SEVERE-SPREAD) — 선행조건 유지 vs 26주 WFA 이관.** (이월)
  §12b 오늘 진입 스프레드 **max 9.003틱 < 20** — 표본 여전히 안 옴(ETA 약 7.1개월).
- [ ] **1-9 등급 임계 재설계 — 3안 중 택일.** (이월) §12b **10일 연속 미도달** max 0.597.
- [ ] **⑧ 사이징 재설계 — 선행조건 P-1 통과. 남은 관문은 P-2(08-21 장전).** (이월)
  ⚠ **오늘 선행조건이 하나 늘었다** — 위 F-1·G-2(문서 정정 3번째 항목 참조).
  ⚠ 재산출 시 잔고 괴리 추이 병기: 08-10 −40.5% → 08-19 −42.3% → **08-20 −42.1%**
...
### ✅ 2026-08-20 장후 — 사용자 지시로 조치 완료 (예약작업 3종 시각 교정)

- [x] **`F-4(bat)` `TASK_CLAUDE_WAKE_INSTALL.bat` 실행 → 보류(목적 상실).**
  P-7의 진짜 원인은 `.bat` 미실행이 아니라 **예약작업 cron이 `2 9`(09:02)라 지터 253초를
  먹으면 구조적으로 개장을 넘기는 것**이었다. `mireuk-daily-check-pre` cron을
  **`55 8 * * 1-5`(→ 08:59:13 발화)** 로 교정해 직접 닫았다.
  ⚠ **항목을 삭제하지 말 것** — cron 교정이 실패하면 다시 유일한 경로다. **O-5 판정까지 보류.**
- [x] **`mireuk-daily-check-intra`·`-post` — cron 무변경, description만 실제값으로 정정.**
  둘은 기능 정상이고 낡은 것은 설명 텍스트뿐이었다(intra 12:40 · post 16:32).
  고칠 것 없는데 건드리면 475차 renormalize 오탐과 같은 사고가 된다.
- [x] 세 작업의 **프롬프트(SKILL.md 본문) 무변경** 확인 — `--pc MW0602`·산출 경로·함정 3종 경고문 그대로.

- [ ] 🆕 **O-5 (매 장전)** 다이제스트 생성 시각이 **09:00 이전**인가.
  **3거래일 연속이면 P-7 종결**하고 `F-4(bat)` 항목도 함께 폐기한다.
  최초 판정 **2026-08-21 장전**. ⚠ 마진 47초 — ① `jitterSeconds=253` 고정 여부
  ② 레짐 확정이 08:58대에 끝나는지, 둘 중 하나가 흔들리면 재발한다.
  **재발 시 판단**: cron을 더 앞당기지 말 것(A-2 매크로·레짐 증거를 잃는다) →
  `F-4(bat)` 복원 또는 지터 회피 수단을 먼저 검토.

- [ ] 📄 **문서 정정** — 0820 리포트 **`§9p 사용자 조치` 표 2번(`.bat` 실행)은 무효**다.
  같은 표 **3번(477차 F-1 적용 결정)은 유효**하다. 리포트 §10에 대체 사실을 명시해 두었으나,
  다음 세션이 §9p만 읽고 `.bat`을 실행할 위험이 남는다.
  ⚠ **리포트 안에 `F-4`가 두 개다** — `F-4(bat)`(장전 §2·§7) vs `F-4p`(장후 §2p, 477차 F-1·F-2 코드 수정).

---

## 2026-08-21 (MW0602 484차 — 장전 일일 점검)

### 🔴 P1 — Fix (⚠ **장후·15:10 강제청산 이후** 적용. 장중 적용 금지)

- [ ] **F-1 앙상블 보정기 영속화 복구** — 이상점 1-1. `data/ensemble_calibrator.pkl`이
  **08-11 15:40 이후 6거래일 무저장**, 저장 실패가 **무로그**.
  1. `learning/calibration.py:626-629` `save()` — 게이트를 `not self._fitted` →
     `self._model is None` 으로 분리. payload에 `fitted`/`degenerate`/`unreachable` 3키 추가.
  2. `learning/calibration.py:658` `load()` — `self._fitted = True` →
     `state.get("fitted", True)` (구버전 하위호환). `degenerate`·`unreachable`도 동일.
     → **이상점 1-7(축퇴 저장본 부활 위험)을 같은 커밋에서 닫는다.**
  3. `main.py:11150-11158` — `if save(...)` → `_ok = save(...)` + `else` WARNING
     `[Calibration] 앙상블 보정기 저장 건너뜀 — fitted=%s degenerate=%s unreachable=%s n=%d`.
     🔴 **3번이 핵심이다** — 이 로그만 있었어도 6거래일이 아니라 하루 만에 발견됐다.
  - 🔴 **[45] `cal_guard_flap_watch`의 히스테리시스를 재제안하지 말 것** —
    확정 결정 「전면 보류」(2026-08-08). 축퇴 판정 임계·전환 설계는 **건드리지 않는다.**
  - 런타임 `py37_32`, COM 콜백 밖. 절대원칙 6종 저촉 없음. 섀도 계측 불요(노출 무영향).
  - `joblib protocol=4` 유지(191차).
- [ ] **`tests/test_484_calibrator_persistence.py`** 불변식 4종 —
  ① `_fitted=False`에서 `save()`가 `True` 반환 + 파일 생성 ② 그 파일 `load()` 시
  `is_fitted is False` ③ 구버전 payload(3키 없음) `load()` 시 `is_fitted is True`
  ④ `_model is None`이면 `save()` `False`. F-1과 같은 커밋.

### 🟡 P2 — Fix

- [x] **F-2 `NEXT_TODO.md` 표기 정정 3건** — 이상점 1-4. **[본 세션 반영]**
  ① `O-5` 이중 등록 → 장전판을 **`O-10`**으로 개명(477차 `O-5(장후)` 보존)
  ② `주간회의(2026-08-22 금)` → **08-22는 토요일**, 금요일은 08-21
  ③ CB② *"이번 금요일이 기한 전 마지막 주간회의"* → **틀렸다.** 08-21·08-28 두 번 남았다
- [x] **F-3 어제 장후 미커밋 3파일 커밋** — 이상점 1-2. **[본 세션 반영]**
  `DECISION_LOG.md`(+54) · `NEXT_TODO.md`(+24) · `MW0602-20260820-점검리포트.md`(+70).
  ⚠ 그 안에 *"`F-4(bat)` 실행은 무효, 진짜 원인은 cron `2 9`"* **정정이 들어 있었다** —
  미커밋 상태로 두면 MW0601 세션이 `.bat` 실행을 다시 요청한다.
- [ ] **O-3 유령 프로세스 — 10거래일 연속.** 이상점 1-3. 가드 정상 작동(이중 실행 0).
  477차 G-3: 비-데몬은 `MainThread` 하나뿐 → 원인 후보 "이벤트 루프 종료 후 MainThread
  미반환(COM 해제 유력)". ⚠ **313차 — 3거래일이라 확정 금지.** O-5(장후) 추적 계속.

### 🟢 고도화

- [ ] **G-1 수집기 §12에 `snapshot_identity` 축 신설** — 오늘 1-1을 §12가 **감시목록에
  없어서** 못 잡았다. 고착·무기록·분기편향 어디에도 안 걸리는 **네 번째 형태**다:
  로그는 매일 정상 출력되고 값도 "정상"이며 **다만 어제와 똑같다.**
  다중 필드 캡처를 튜플로 묶어 일자 간 동일성 비교 → `연속 동일일수 ≥ N`이면 `🟠 스냅샷 정체(n일)`.
  초기 등록 3종: `앙상블보정기_스냅샷`(`[Calibration] 앙상블 보정기 복원 완료 …`, N=8) ·
  `MetaConf_스냅샷` · `앙상블가중_스냅샷`.
  ⚠ `N`은 실측 갱신 주기의 2배로 **사전등록**, 사후 조정 금지(313차 ④).
  ⚠ `benign` 화이트리스트 동반. ⚠ `measured_since` 마커 등록.
  → **26주 WFA `invariants.md` §5 「고착 지표 감시목록」 행과 나란히 등록할 것**(468차 G-2 취지).
- [ ] **G-2 예약 점검 발화 마진 상시 기록** — O-10 마진 **예측 47초 → 실측 12초**.
  다이제스트 §1 아래에 `발화 마진 = 09:00:00 − 생성시각` + 최근 5거래일 추이.
  판정: `< 30초` 2거래일 연속 → P2 자동 등록.
  🔴 **재발해도 cron을 더 앞당기지 말 것** — `phases.md` A-2(08:55 매크로 → 레짐) 증거를
  잃는다. 오늘 실측 레짐 확정은 **08:58:19**이므로 08:58:30 이전 발화는 A-2 관측 불가.
  O-10 3거래일 판정(예정 08-25)보다 **먼저** 들어가야 의미가 있다.

### 📄 문서 정정

- [ ] **`invariants.md` §2-B `MODEL_LABEL_STATE_UNLOCK_ENABLED` 관측란 갱신** —
  `⏳ 라이브 미검증` → **`✅ 2026-08-21 장전 실측(P-2 통과)`**.
  근거 두 줄 병기: `6/6 호라이즌 현행 레이블(fixed)` 출현 **and** `전일 EOD 모델 적재 확인`
  미출현. ⚠ **`×0.6` 소멸을 근거로 적지 말 것**(§2-B가 금지한 오독 경로).
  ⚠ 같은 표의 `PRE_RETRAIN_DONE_BY_EOD_ENABLED`(468차 F-1)도 O-1로 함께 확인됐다.
- [ ] **`invariants.md` §6 ⑧ 행 갱신** — "G-1 라이브 확인이 선행조건" → **선행조건 충족**.
  남은 것은 **조건 1(실전 자본 재설정) 하나**이며 **사용자 결정 사항**임을 명기.
  ⚠ `MAX_CONTRACTS=3`도 그때 함께 재산출.

### 🔵 기한 — 주간회의 (오늘 2026-08-21 금 · 다음 2026-08-28 금)

- [ ] 🔴 **CB② 복원 재검토 — 기한 2026-08-29(토), 남은 거래일 6(08-21·24·25·26·27·28).**
  ⚠ **정정**: 기한 전 금요일 주간회의는 **오늘과 08-28 두 번**이다. 종전 기재
  *"이번 금요일이 마지막"* 은 틀렸다(F-2 ②③). **오늘 상정은 필수, 결론 강요는 불필요.**
  상정 시 병기: CB②가 9999인 동안 **전환기준 ②는 CB①·③·④·⑤로만 충족 가능**,
  CB③-P4는 차단 비활성.
- [ ] **전환기준 ⑨(TOX-SEVERE-SPREAD) — 선행조건 유지 vs 26주 WFA 이관.** (이월)
  §12b **3일 연속 미도달**(진입 max **9.003틱** < 20). 473차 F-8 `INSUFFICIENT`, ETA 약 7.1개월.
  ⚠ **문턱을 낮춰 판정하지 말 것**(458차 D6 계열 사전등록 위반).
- [ ] **1-9 등급 임계 재설계 — 3안 중 택일.** (이월) §12b **9일 연속 미도달** max **0.5878**.
  ⚠ 오늘 기동 `out_max=0.3897`은 **방증일 뿐 신규 발견이 아니다.**
- [ ] **⑧ 사이징 재설계** — ⬆️ **선행조건 P-2 오늘 통과.** 남은 관문은
  **실전 자본 재설정(사용자 결정)** 하나. ⚠ 재산출 시 잔고 괴리 추이 병기:
  08-10 **−40.5%** → 08-19 −42.3% → 08-20 −42.1% → **08-21 −42.1%**
  (실제잔고 28,930,730원 / 목표 5천만원, 전일 대비 +8원).

### 오늘(08-21) 장중·장후 관측

- [ ] 🆕 **O-11 (오늘 장중)** `[MarginCap] … state=OK|CAP|BLOCK` (477차 후속 G-2,
  조회 성공 사이클마다 무조건 1줄, `measured_since=2026-08-21`).
  **1건 이상이면 계측 생존.** ⚠ **장전 0건은 정상**이다(주문가능수량 조회가 없다) —
  수집기 §11이 `🔴 무기록`으로 올린 것은 배포 첫날의 예상된 표시다.
  **장중에도 0건이면 그때가 진짜 계측 실패(P1).**
- [ ] 🆕 **O-12 (오늘 장후 15:40)** `[Calibration] 앙상블 보정기 저장 완료 n=` 또는
  (F-1 적용 후) `저장 건너뜀` 중 **하나는 반드시** 출현.
  🔴 **오늘은 F-1 미적용이므로 무로그가 예상값**이며, 그 무로그 자체가 1-1의 재확인이다.
- [ ] 🆕 **O-13 (오늘 장후)** `data/ensemble_calibrator.pkl` mtime.
  **여전히 08-11이면 1-1 지속 확정(7거래일).**
- [ ] **1-5 (오늘 장중)** `[CORE준비도] 축퇴 N/M` 이 09:00 이후 `0/6` 유지.
  장전 08:45:15에 `2/6`(`cvd_divergence`×6hz `opt_chain_pcr`×6hz) → 08:48:03부터 4회 연속 `0/6`.
  ⚠ `opt_chain_pcr`은 **장기(30m) 그룹 CORE**이고 474차 각주대로 중기·장기 그룹은
  `select_entry_horizon()`이 `1m/3m/5m/None`만 반환해 **구조적 도달 불가** — 실집행 영향 없다.
  실질 대상은 `cvd_divergence`(단기 CORE) 하나. **판정 보류 — 1일 표본**(313차).
  닫는 조건: 20거래일 축적 후 "축퇴일의 09:00~09:30 정확도" **일자단위** 비교(317차 교훈).
- [ ] **O-10 (매 장전, 구 `O-5` 장전판)** 다이제스트 생성 시각 09:00 이전 + **발화 마진**.
  **오늘 1/3 (08:59:48, 마진 12초).** **3거래일 연속이면 P-7 종결**하고 `F-4(bat)`도 폐기.
  ⚠ 마진이 어제 예측(47초)의 1/4이다 — G-2가 이 값을 매일 찍게 한다.
- [ ] **O-3 (매 장전)** 런처 `[GUARD] 기존 프로세스 종료 완료` 연속일. 오늘 **10/10**.
- [ ] **O-4 (매 장전)** `[Capital]` 괴리율 추이만. 오늘 **−42.1%**. 단일일 판정 금지.
- [ ] **P-8 · P-9 · O-5(장후) · O-6 ~ O-9** — 장후 소관, 이월(기준 무변경).

### ✅ 완료·종결 처리 (484차 장전)

- [x] **P-2 (08-21 장전)** ✅ **통과** — `08:55:16 [EntryGate] 모델 레이블 체계 확인(장전) —
  사이즈 제한 해제 (×0.6 → ×1.0) | 6/6 호라이즌 현행 레이블(fixed) 학습본` **출현** +
  `[EntryGate] 전일 EOD 모델 적재 확인` **전 로그 0건**. 468차 G-1 라이브 검증 종결.
  ⚠ `×0.6` 소멸은 근거로 쓰지 않았다(§2-B 금지 경로).
- [x] **O-1 (08-21 장전)** ✅ **통과** — `[PreRetrain] EOD 마커 파일 직접 확인 …` **0건**(소멸).
  대신 `08:55:16 [PreRetrain] EOD 재학습 날짜 복원: 2026-08-20 (1일 전)` — session_state 경로 정상.
  🔴 0820 장전이 *"소멸하지 않을 것으로 예측(477차 F-1 미배포)"* 이라 적었으나 그 뒤
  **같은 날 장후에 `b4fd2d3`로 배포**됐다 — 예측이 낡았던 것이지 관측이 틀린 게 아니다.
- [x] **O-2 (08-21 장전)** ✅ **통과(불변식 4/4)** — `data/session_state.json`
  `{"date":"2026-08-21","count":1,"auto_shutdown_done_date":"","gbm_total_retrain_count":2,
  "p8_last_success_date":"2026-08-20","eod_retrain_ok_date":"2026-08-20","today_open":1068.4}`.
  3키 생존 · `circuit_breaker_state`/`profit_guard_state` 부재 · `count` 리셋 후 증가 ·
  `auto_shutdown_done_date` 공백. **477차 F-1 첫 롤오버 실사용 검증 종결.**
- [x] **P-7** ⬇️ **완화 P1→P2, 종결 진행** — 다이제스트 **08:59:48** 생성(5거래일 연속 09:07 → 개장 전).
  0820 장후 cron 교정(`2 9` → `55 8`)의 효과 실측. **O-10으로 승계**(3거래일 연속 시 종결).
- [x] **`F-4(bat)` 최종 확인** — 오늘 실측으로 **목적 상실 확정**. ⚠ **항목은 삭제하지 않는다** —
  cron 교정이 실패하면 다시 유일한 경로다. **O-10 3/3 판정까지 보류 유지.**

### 📄 문서 정정 (484차 자체검증 — 본 세션 반영 완료)

- [x] **리포트 `main.py` 행번호 11150 → 11151** (11150은 import 줄). `else` 부재 주장은 무변경.
- [x] **"8거래일" → "7거래일(기동 8회)"** — **2026-08-17은 공휴일·휴장일**이다
  (`logs/20260817_SYSTEM.log 18:11:56 대기 중 | 공휴일·휴장일`).
  ⚠ `logs/20260809_SYSTEM.log`도 **일요일**이다 — `n=887` 구간 계수에서 제외할 것.
  🔴 **로그 파일이 있다는 사실을 거래일로 읽지 말 것** — 휴장일·주말에도 프로세스가 뜬다.
- [x] **G-1 `N` 8 → 6** — 도출 규칙을 「과거 실측 정체 구간 최댓값 × 1.5」 하나로 고정.
  거래일 기준 정체 이력 `n=296`(2) · `n=592`(4) · `n=887`(2) → 4 × 1.5 = **6**.
  초판의 *"임계 8 → 08-19에 떴을 것"* 은 산술 오류였다(정체 7거래일 < 8).
  ⚠ `MetaConf_스냅샷`·`앙상블가중_스냅샷`의 `N`은 **정체 이력 실측 후** 같은 규칙으로.
  ⚠ 소급 계산은 예시이지 근거가 아니다(313차 ④).

---

## 2026-08-23 (MW0602 485차 후속 — 0821 장후 산출물 커밋 + F-1·F-4·F-5·F-6·F-7·F-10 구현)

> 0821 리포트 장후 절(485차)의 커밋 계획을 사용자 지시("F-1 · F-4 · F-5 · F-6 · F-7 ·
> F-10 구현해")로 집행. 오늘은 **일요일(장외)** — F-1의 "장후 적용" 조건 충족.
> F-2·F-3은 484차가 이미 반영. **F-8(A/B)·F-9는 미착수** — 사용자 지시 범위 밖이고
> F-8은 (A)/(B)/(C) 택일(권고 C), F-9는 주간회의 합의 후(양 PC 동시 적용) 사안.

### ✅ 구현 완료 (본 세션 — 커밋 4건 분리: 문서 / F-1 / F-4·F-5·F-7 / F-6)

- [x] **F-1 앙상블 보정기 영속화 복구** (1-1 · 1-7 동반) — `learning/calibration.py`
  `save()` 게이트 `not self._fitted` → `self._model is None` 분리 + payload
  `fitted`/`degenerate`/`unreachable` 3키, `load()` `state.get("fitted", True)`
  복원(구버전 하위호환 — 재평가는 fitted를 **내리기만** 하므로 축퇴 저장본 부활 없음).
  `main.py` daily_close 저장 호출 `else` 분기 WARNING `저장 건너뜀 — fitted=… n=…` 신설.
  `tests/test_484_calibrator_persistence.py` 불변식 4종 **4/4 PASS** ·
  py37_32/py310_64 `py_compile` 통과. [45] 히스테리시스·축퇴 임계 무변경(확정 결정 보존).
- [x] **F-4 DriftRetrain 무조건 상태 샘플** (1-10) — `main.py` `_drift_trigger` 산출
  직후 매분 **INFO** 1줄 `[DriftRetrain] state=FIRE|SKIP acc5m=… n=… mins_since_rt=…
  halted=… running=… cooldown_ok=… cond_a=… cond_b=…`. 임계·조건식 무변경(사전등록).
- [x] **F-5 수집기 `[MarginCap]` 정규식 신·구 양식** (1-11) —
  `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py` `margin_cap` 패턴
  alternation + §5 「조회 N · 축소 M」 2축 + 실효 상한 분해 표·발동 경고를
  축소(`state=CAP`) 기준으로 정정. **0821 다이제스트 재생성 검증: §5 `조회 27 · 축소 3`
  == §12 `MarginCap_state` 27(`OK`×24 `CAP`×3) 일치.**
  ⚠ 스킬 캐시 사본은 이 PC에 없음(repo 원본 단일) — 탐색으로 확인.
  ⚠ §5 시계열 불연속: 08-21 재생성분부터 「증거금 상한」 칸이 2축이다.
- [x] **F-6 `phases.md` B-1·B-2 정정** (1-15) — B-1 *"T-30 퇴역 대상"* → **"T-30 채점은
  계속돼야 한다(CB③·Contrarian 유일 입력원, 474차 각주)"**, B-2 *"30분마다"* →
  **조건부 트리거 3종**. MW0601 F-8의 `dev` 브랜치 반영(커밋 메시지 명시).
- [x] **F-7 `monthly_cleanup.py` stdout 인코딩** (1-16) — reconfigure 블록 삽입(캠페인
  체인 공통 관용구 복제, 문자 삭제 방식 배제). **py310_64 dry-run rc=0 완주** +
  `tests/test_485_monthly_cleanup_encoding.py` 2종 PASS(cp949 파이프 생존 포함).
  라이브 검증 **O-20(08-28 금 EOD)**.
- [x] **F-10 관측 ID 정정** (1-19) — 0821 장중 절이 `O-8`(정본: 손절 준수율)로 오라벨한
  WeightCollapse 밴드 관측을 **`O-22`로 재부여** (아래 신규 항목).

### 신규·갱신 관측

- [ ] 🆕 **O-22 (매 장후)** `[WeightCollapse]` 종가 비율 — 밴드 **20.3~20.5% ±5%p**
  이탈 시 원인 조사 (구 0821 장중 절 `O-8` 오라벨 승계 — 1-19). 08-21 실측
  **75/365 = 20.5%** ✅ 밴드 내. ⚠ 문자열 등장 횟수로 세지 말 것(2배 오독 전례 —
  `[ModelHealth]` 분봉 단위로 셀 것).
- [ ] **O-12 (다음 거래일 08-24 장후 15:40) — F-1 적용 후 첫 관측.**
  `[Calibration] 앙상블 보정기 저장 완료 n=`(**1183보다 큰 값** 기대) 또는
  `저장 건너뜀` WARNING 중 **하나는 반드시** 출현. **무로그 = fix 실패.**
- [ ] **O-13 (다음 거래일 장후)** `data/ensemble_calibrator.pkl` mtime **당일 갱신**.
- [ ] 🆕 **O-24 (다음 거래일 장중) — F-4 검증.** `[DriftRetrain] state=` ① 줄 수 ==
  정규장 분수(약 370) ② `state=SKIP` 사유 항이 실제 값과 정합 ③ FIRE 조건 성립 시
  `state=FIRE` 출현 — 안 찍히면 1-10 원인 후보 1(`_gbm_retrain_running` 고착)로
  좁혀지고 그 줄의 `running=` 값이 답을 준다.

### 🆕 부수 발견 (F-7 검증 중 — 다음 점검 세션에 이관)

- [ ] **`utils/analysis_db.py`가 `dev` 브랜치에 없다** — `monthly_cleanup.py:387`
  `guard_intraday()` 임포트가 실패해 **장중 가드가 경고만 내고 우회**된다
  (dry-run 실측: `[경고] 장중 가드 불가(utils.analysis_db 임포트 실패…)`).
  479차(MW0601) 가드 설계가 소비부만 오고 생산부(모듈)가 안 온 상태 —
  **1-17([50]/[51] 생산부 결손)과 같은 브랜치 비대칭 계열**이다. F-8 논의에 합류시킬 것.
  ⚠ 발화 지점이 EOD 체인(15:45, 장 마감 후)뿐이라 실위험은 낮다 — P2.

### ⏭ 미착수(이월) — 사용자/주간회의 결정 대기

- [x] **[DONE 2026-08-23 487차 후속 `f8faa12` — 488차 확인] F-8 캠페인 채널 [50]/[51] 생산부 결손 해소** —
  (B) 채택: `NOT_AVAILABLE_ON_THIS_BRANCH` 감지형 표기(`generate_validation_campaign_report.py:7852-7861`).
  검증 채널 O-21(08-28 금 EOD)은 유효.
- [x] **[DONE 2026-08-23 487차 후속 `f8faa12` — 488차 확인] F-9 캠페인 채널 번호 `[51]` 중복 해소** —
  ConstOut `[51]→[54]` 재배정(`:7891-7893`). 멀티PC 정책 폐기(487차)로 "양 PC 합의" 전제 소멸.

---

## 2026-08-23 (MW0602 486차 — 휴장일 점검)

> 오늘은 **일요일**이라 거래가 없었다. 아래 P-*는 전부 **다음 거래일 2026-08-24(월)** 소관이다.
> 리포트: `docs/정기점검/매일점검/MW0602-20260823-점검리포트.md`

### 🟡 P2 — Fix

- [ ] **F-1(486차) 수집기 휴장일 판정 배선** — 이상점 1-1.
  `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py` 날짜 확정 직후
  `is_trading_day()` 분기. **새 판정식 금지** — `config/krx_holidays.py:87 is_krx_holiday()`
  임포트 + `scripts/campaign_steps.py:40-41`과 **동일한 식**(`weekday()>=5 or is_krx_holiday(d)`).
  임포트 실패 시 `weekday()>=5` 폴백 + 그 사실을 다이제스트에 명시(표준 라이브러리 원칙).
  ⚠ py37_32/py310_64 양쪽 호환 — 3.8+ 문법 금지.
  - [ ] §1 「가능성 (a)~(d)」에 **`(e) 오늘이 거래일이 아니다`** 추가
  - [ ] §11 적신호 **1~6번만** `휴장(정상)`으로 강등(**명시 열거**). 🔴 **전부 끄지 말 것** —
    미커밋 변경·PC명 태그 위반·설정 불변식 `불일치`는 휴장일에도 유효한 사실이다.
    **`억제된 적신호 N건`을 반드시 출력**해 은폐가 아니라 이동임을 보이게 할 것.
  - [ ] **`tests/test_486_collector_holiday.py`** — 주말·공휴일·거래일 3케이스 +
    **억제 목록이 6개로 고정**돼 있는지(불변식). 목록이 늘면 테스트가 깨져 재검토를 강제한다.
  - [ ] 회귀 검증: **08-21 날짜로 재실행 → 거래일 산출물 바이트 동일**.
    🔴 **과잉 억제가 이 fix의 최대 위험**이다 — 휴장 플래그가 잘못 서면 진짜 결함이 있는
    거래일에 적신호가 통째로 사라진다.
  - 적용 시점: **08-24(월) 장 마감 이후 권고**(거래일 회귀를 실산출물로 한 번 더 확인).

### 🟢 고도화

- [ ] **G-1(486차) 다이제스트 「거래일 문맥」 머리줄 상설** — F-1(486차)과 같은 커밋.
  §1 머리에 `대상일 · 거래일 여부(주말/공휴일/거래일) · 직전 거래일과 그 로그 존재 여부` 3줄.
  ⚠ **1단계는 표시까지만.** §12(10거래일)·§12b(14거래일)·수기 계수의 **정의 통합은
  26주 WFA 경계로 분리**한다 — 지금 바꾸면 관측일 수가 달라져 시계열이 끊긴다
  (461차 `mdd_pct` 유형). `invariants.md` §5 갱신 대상에 편입.

### 오늘(08-23) 등록 관측 — 다음 거래일(08-24 월) 소관

- [ ] 🆕 **P-1 (08-24 장후) — F-1(0821 보정기) 라이브 검증.**
  15:40에 `[Calibration] 앙상블 보정기 저장 완료 n=` **또는** 신설된 「저장 건너뜀」
  WARNING이 **사유와 함께** 출현. 🔴 **둘 중 하나는 반드시 나와야 한다** —
  *부정 분기에 로그가 없는 것*이 1-1(0821)의 핵심 결함이었다. 둘 다 없으면 **P0 승격.**
- [ ] 🆕 **P-2 (08-24 장후)** `data/ensemble_calibrator.pkl` mtime이 **2026-08-24로 갱신**.
  현재 **2026-08-11 15:40**(7거래일 정체 — 08-12·13·14·18·19·20·21).
  ⚠ **08-17은 거래일이 아니다**(484차 정정). 기동 횟수로 세지 말 것.
- [ ] 🆕 **P-3 (08-24 장중) — F-4 라이브 검증.** `[DriftRetrain] state=…` 매분 1줄.
  🔴 **미발동 분에도 나와야 한다** — 그것이 F-4의 목적이다(0821 1-10: 안 돈 분에
  로그가 한 줄도 없어 "조건 미충족"과 "조건 충족인데 못 돎"을 영영 구분 못 했다).
- [ ] 🆕 **P-4 (08-24 장후) — F-5 라이브 검증.** 다이제스트 §5 `[MarginCap]` 건수
  **==** §12 `MarginCap_state` 표본 수. 0821에는 `3` vs `27`로 갈렸다.
- [ ] 🆕 **O-1 (재실행 발생 시)** 예약 점검 `lastRunAt` vs 수집기 대상일 정합.
  오늘 1-2(판정 보류, 관측 1건) — `lastRunAt 2026-08-21T07:32:09Z`인데 수집기는
  08-23을 대상으로 잡았다. **스케줄 cron은 정상**(`30 16 * * 1-5`)이므로 **변경하지 말 것.**
  F-1(486차) 적용 후에는 휴장일 배지로 자동 노출된다.

### 이월 (08-21 등록분 — 기준 무변경)

- [ ] **O-17 (08-24 장후)** `entry_horizon` 소비처 전수. 청산 기하 전용이면 정상 종결,
  방향 판단에도 쓰이면 **P1 승격**. 하루치 코드 조회로 닫힌다.
- [ ] **O-18 · O-19 · O-22 · O-23** Tier1 준수율 / 진입 방향 편향 / WeightCollapse 비율
  (밴드 20.3~20.5%±5%p) / `sizing_trace.margin_cap_qty` — 매 장후, 누적.
  ⚠ **O-19는 F-8 완료 후 [50] 채널의 사전등록 기준으로만 판정.** 문턱을 낮추지 말 것.
- [ ] **O-16 · O-5(장후) · O-7 · O-8 · O-9 · P-8 · P-9** — 5m 정확도 / 비데몬 스레드 /
  `meta_action` 승패 / 진짜 손절 준수율 / GuardFair 무효율 / 롤링20일 net 부호 /
  `bar_pass≥1`. 기준 무변경, 누적.
- [ ] **O-20 (08-28 금 EOD)** `월간 로그 정리=OK` + `data/monthly_cleanup_last_run.txt` = `202608`.
- [ ] **O-21 (08-28 금 EOD)** `direction_bias_watch.error` / `const_out_horizon_watch.no_data`.
  ⚠ **F-8 미착수이므로 현 상태 유지가 예상값**이다 — 변화 없음을 실패로 읽지 말 것.
- [ ] **O-3 · O-4 · O-10 (08-24 장전)** 런처 GUARD 연속일 / `[Capital]` 괴리율 /
  다이제스트 발화 마진. ⚠ **O-10은 3거래일 연속 09:00 이전이 조건, 현재 1/3**(08-21).
  **휴장 이틀(08-22·23)은 카운트하지 않는다.**

### 🔵 기한 — 주간회의 (다음 2026-08-28 금)

- [ ] 🔴 **CB② 복원 재검토 — 기한 2026-08-29(토), 남은 거래일 5**(08-24·25·26·27·28).
  ⚠ **08-21 1-14 확정으로 전제가 하나 좁아졌다** — 실전 전환 기준 ②를 채울 수 있는
  후보는 **CB①·④·⑤ 셋**이다(CB③ HALT는 `safety/circuit_breaker.py:365-383` 코드 비활성).
- [x] **[DONE — 487차 후속 `f8faa12`가 (B)로 종결, 488차 확인] F-8 (채널 [50]·[51] 생산부 결손).** 주간회의 안건에서 제외.
- [x] **[DONE — 487차 후속 `f8faa12`, 488차 확인] F-9 (채널 번호 `[51]` 중복) — ConstOut `[54]` 재배정.**
  멀티PC 정책 폐기(487차)로 양 PC 동시 적용 전제 소멸. 주간회의 안건에서 제외.
- [ ] **멀티PC 컨벤션 보강 — "소비부만 옮기지 않았는가" 한 줄 추가.** (0821 등재, 이월)
- [ ] **1-9 앙상블 등급 임계 재설계 — 3안 중 택일.** (이월) §12b **9일 연속 미도달**,
  관측 max **0.5862** < 0.60.
- [ ] **전환기준 ⑨(TOX-SEVERE-SPREAD) — 선행조건 유지 vs 26주 WFA 이관.** (이월)
  §12b **4일 연속 미도달**(max 9.003 / 임계 20). ⚠ 문턱을 낮춰 판정하지 말 것.

### ✅ 완료·종결 처리 (486차)

- [x] **485차 후속 Fix 6종 코드 실재 검증** — F-1(`2356820`) · F-4·F-5·F-7(`81b325d`) ·
  F-6(`54bff4a`) · F-10(`567cc86`). 완료주장-커밋없음 **0건**, 범위변경 **0건**.
  🔴 **라이브 미검증 — 검증 기한 2026-08-24**(P-1~P-4).
- [x] **F-8·F-9 미착수 사유 확인** — 08-21 §2p 「적용 순서」 5번(*"주간회의 후"*)과 일치.
  **지연이 아니라 계획대로다.** 신규 안건으로 재등록하지 말 것(함정 ①).

### 🔴 즉시 (사용자 소관) — 커밋 차단 해제

- [ ] 🔴 **`.git/index.lock` 잔존 — 486차 커밋 ① 미실행.**
  `Remove-Item C:\Users\pc1\PycharmProjects\futures\.git\index.lock -Force` (PowerShell).
  샌드박스에서는 `Operation not permitted`로 삭제 불가였다.
  해제 후: `git add "docs/정기점검/매일점검/MW0602-20260823-점검리포트.md" dev_memory/DECISION_LOG.md dev_memory/NEXT_TODO.md`
  → `git commit -m "[MW0602] 486차: 08-23 휴장일 점검 — 수집기 휴장일 미인지 발견(1-1) + 485차 후속 Fix 6종 배포 검증"`
  ⚠ **08-24 장전 점검 전에 해제할 것** — 남아 있으면 다음 세션도 같은 자리에서 막힌다.
  ⚠ 파일은 전부 저장돼 있어 **무손실**이다. 미반영은 커밋뿐.

### 🆕 채널 [55] 신설 후속 (486차 · 1-9 처분)

- [x] **채널 [55] `early_window_gate_shadow` 사전등록 + 스크립트 + 리포트 배선 + 테스트**
  — 판정 **`BLOCK_JUSTIFIED`**(57건/17일, 관문 3/6 미충족). 노출 변화 0.
- [ ] 🔵 **주간회의(2026-08-28) — 1-9 처분 4안 중 택일. 권고 (E).**
  (A) 전역 임계 인하 = 관할 밖·무효 / (B) 금지 존 편입 = 16일 +45pt 포기 /
  (D) 전면 제거 = **상정 자격 미달** / **(E) 등급 조건 → 변동성 조건 교체**.
  ⚠ (E)는 **사전등록 선행 필수** — ATR 단조 관계는 **사후탐색**이라 지금 기준을 만들면
  313차 ④ 위반이다. ⚠ 채택 시에도 `size×0.5`는 **분리 존치** 권고.
- [ ] 🆕 **O-24 (2026-08-28 금 EOD)** 캠페인 요약표에 `[55]` 행 출현 +
  `🛡 BLOCK_JUSTIFIED` + headline 정상. `INSUFFICIENT`면 배선 실패(1-17과 같은 형태).
- [ ] 🆕 **O-25 (매주 금)** `[55]` 표본 증가 추이(차단 건수·거래일).
  ⚠ **판정은 사전등록 기준 그대로** — 문턱을 움직이지 말 것(313차 ④).
- [ ] ⚠ **`2026-07-01`을 재인용할 때 반드시 단위를 병기할 것** — `−35.228pt`는
  **1계약·TP1상한·미실현 시뮬**이고 실현손익이 아니다. 원(₩) 채널과 더하지 말 것.
- [ ] **커밋 대기** — `.git/index.lock` 잔존으로 미커밋(위 「즉시」 항목 참조).
  대상: `config/settings.py` · `scripts/early_window_gate_shadow.py`(신설) ·
...
## 2026-08-23 (MW0602 488차 — 고도화 백로그 8건 구현손익 전수 조사 + 구현계획 A~E)

> 사용자 지시 수행분. **코드 변경 0건 — 조사·계획까지.** 전문:
> `docs/정기점검/매일점검/MW0602-20260823-점검리포트.md` **부록 D**.
> 판정: 구현이익 ➕ 7건 / 보류 ⏸ 1건(G-2/477차 `state_file_keys` — 위 477차 절에 병기).
> 8건 전부 관측 계층 — **직접 매매손익 효과 0**, 이익은 전례 사고 재발 방지로 측정.

### 🟢 구현계획 (사용자 지시 대기 — "계획 X 구현해")

- [ ] 🔴 **계획 A (기한: 08-25 O-10 판정 전)** — 발화 마진 상시 기록(G-2/485차) +
  지각 배너(G-3/476차) 병합. `collect_evidence.py` pre 한정, `_fire_margin()` 순수 함수 +
  최근 5거래일 추이 + `<30초 2거래일 연속 → P2` + `마진<0 → 최상단 배너`.
  `tests/test_488_pre_margin.py`. **휴장일인 오늘 배포 가능·권고.** 예상 40분.
- [ ] **계획 B** — 거래일 문맥 머리줄(G-1/486차 1단계). **F-1(486차)과 같은 커밋.**
  §1 머리 3줄(대상일·거래일 여부·직전 거래일+로그 존재). 1단계는 표시까지 —
  §12/§12b 계수 무변경을 테스트로 고정. 08-24 장후. 예상 +10분.
- [ ] **계획 C** — EOD SelfCheck(G-1/476차). `model_meta.wants_from_settings()` 신설(어댑터
  이관, main.py:1113-1126 축약 — **유일한 라이브 접점**, 기존 테스트 2종 커버) +
  `retrain_eod.py` 마커 직전 `[SelfCheck] label_state ok=…`(False→WARNING, ERROR 금지,
  재학습 비치명). `tests/test_488_eod_selfcheck.py`. py310_64. 08-24 장후. 예상 50분.
- [ ] **계획 D** — §12 `snapshot_identity` 축(G-1/485차, 초기 3종·N 사전등록·benign·
  measured_since) + `CORE축퇴_피처` 축(G-2/476차, 병렬 추가). 검증: 보정기 정체 구간
  (08-11~21) 소급 검출 확인(계측 유효성용 — 판정 근거 아님, 313차 ④). 이번 주 후반. 예상 80분.
- [ ] **계획 E** — `tests/test_477_state_contract.py` 신설(G-1/477차, 등록명 유지).
  소스 정규식 스캔(라인 고정 금지) + `_ss["key"]` 관용구 가드. AGENTS.md에
  `test_*_contract.py` 규약 1줄. 차주. 예상 60분. **⑧(state_file_keys) 재평가 선행조건.**

### ✅ 완료·종결 처리 (488차)

- [x] **고도화 백로그 8건 미구현 실측 확인(함정 ①)** — `[SelfCheck]`·`snapshot_identity`·
...
## 2026-08-23 (MW0602 488차 후속2 — 사용자 결정 ① + 테스트 실패 12건 전량 처리)

> 전문: `MW0602-20260823-점검리포트.md` **부록 F**. **매매 경로 변경 0건.**
> 최종 검증: `439 passed · 2 skipped · 0 failed` + 단독 5파일 OK + 신설 5종 = **444 통과.**

### ✅ 완료 (488차 후속2)

- [x] **[사용자 결정] `opt_chain_pcr` 두 축 분리** — CORE 체크리스트 게이트는 도달 불가
  유지 / `active_features` 편입·수집은 계속. `CLAUDE.md` §3 각주 신설 + 표 「장기」 행
  `⚠ 미발동(도달 불가)` 표기. **경로 오류 정정**(`collection/option/option_chain.py`는
  존재하지 않는 파일 → `collection/options/option_chain_worker.py`).
  테스트 단언 반전·개명(`test_long_group_core_feature_stays_a_plain_feature`) —
  **양방향 잠금**(편입이 빠져도, 장기 그룹이 도달 가능해져도 깨진다).
- [x] 🔴 **`_conn()` DB 연결 90개 누수 수정** — `@contextlib.contextmanager` 전환.
  호출부 90곳 무변경, **안쪽 `with conn:` 유지로 트랜잭션 의미 보존**(섀도 테이블
  UPDATE 유실 방지). 실패 8건 동시 해소. 신설 `tests/test_488_conn_lifecycle.py` **5종**.
- [x] 🔴 **전환기준 ⑨ 채널 침묵 수정** — `eta_status`/`eta_reason` 신설, 출력 조건을
  `trading_days_observed` 기준으로. 테스트는 **정밀화**(숫자 강요 → "숫자 또는 사유").
- [x] 🔴 **브랜치 비대칭 명시화** — 458차 만성도 계측 생산부 미이관.
  `raising=False` 탓에 **초록불로 보이던** 상태를 명시적 skip + 사유로 전환.
- [x] **전환기준 ① 판정 노출** — `test_472_phase5_gate_badge` 를 모듈 설계(반증 전용)에
  맞춤. ①은 `MET` 불가 단언으로 대체. ③·④ `source` 분리(`pending` vs `auto`).
- [x] **`ens_row` 매직 넘버(48) → 관계식** — 실측 55와 어긋나 있었다.
  `ens_row 길이 == 최대 INSERT 컬럼수`. 다시 낡지 않는다.

### 🔵 주간회의 (2026-08-28) — 갱신·신규

- [x] **[해소] `opt_chain_pcr` 안건** — 사용자 결정으로 종결(위). 재론 불필요.
- [ ] 🔴 **실전 전환 기준 ① 현재 미충족 — 확인 필요.**
  최근 20거래일 통산 **−178,004원**(양수일 8/20). ④ 표준편차 426,585원(평균의 47.9배).
  ⚠ **새 결함이 아니다** — 원래 그랬던 것을 도구가 이제 말한다(488차 후속2 전까지는
  테스트가 ①을 `unmeasured` 로 고정해 가려져 있었다).
- [ ] **전환기준 ⑨ — 표본이 오지 않는다는 것이 실측으로 확정됐다.**
  66거래일 · `≥20틱` 진입 **0건** · 진입 스프레드 **최댓값 19.0002**.
  ⚠ **문턱을 낮춰 판정하지 말 것**(458차 D6). 선행조건 유지 vs 26주 WFA 이관 결정.
- [ ] ⚠ **473차 `≥20틱` 진입 6포지션/4거래일 수치와 현재 0건의 불일치 — 미규명.**
  `12-20` 도 10→9로 감소(총 포지션은 201→222 증가). 스크립트는 474차 이후 무변경이라
  코드 탓이 아니다. **어느 쪽이 옳은지 단정하지 말 것.** ⑨ 처분 시 함께 확인.

### ⏭ 사용자 판단 대기

- [ ] **458차 만성도 계측 생산부 이관 여부** — `features/horizon_feature_registry.py` 에
  `_chronic_suffix()`·`_CHRONIC_PATH` 가 없다(`dev` 는 178차 이후 무변경).
  현재 `tests/test_473_state_file_isolation.py` 2종이 **명시적 skip**.
  선택지: (A) 생산부 이관 (B) 소비부(테스트·conftest 격리) 제거.
  ⚠ (B)를 고르면 `conftest` 의 autouse 격리도 함께 지울 것 — 남겨두면 다음 이관 때
  **지킨다고 믿는 초록불**이 다시 생긴다.

### 📌 계측 규율 — 이번에 재확인된 것 (등록만, 조치 불요)

- [ ] **매직 넘버는 관계식으로.** `ens_row 48` 이 7컬럼 뒤처져 있었다.
  같은 형태가 더 있는지는 확인하지 않았다 — 26주 WFA 때 `tests/` 전수로 볼 것.
- [ ] **`monkeypatch(..., raising=False)` 는 없는 것을 만들어 준다.**
  격리 테스트에 쓰면 대상 부재를 통과로 바꾼다. conftest 신규 격리 추가 시 주의.
- [ ] **pytest 캡처와 충돌하는 5파일**(`test_461`~`test_465`, import 시 `sys.stdout`
  감쌈)은 전체 스위트에서 **보이지 않는다** — `test_461` 의 부패가 그래서 오래 남았다.
  전량 실행 시 `--ignore` + 단독 실행 병행이 현재 절차.

---

## 2026-08-23 (MW0602 489차 — 0823 주간회의 결정 9건 전량 반영)

> 전문: `dev_memory/DECISION_LOG.md` 489차 · 원안
> `docs/정기점검/금요일점검/MW0602/0823_주간회의_검토보고_MW0602.md`.
> **매매 경로 변경 0건.** 검증: `442 passed / 0 failed` + 단독 5파일 OK + 신설 21종.

### ✅ 완료 (489차)

- [x] **D1 [2] Meta-Gate 최종 탈락 확정** — 재개 조건(경계 이후 분위당 n≥300) 충족 후
  재측정 실행: 분위당 **411건**, 분리도 **−0.0136**(필요 +0.1409). 레지스트리 갱신.
  ⚠ **재개하려면 새 사전등록 필요** — 이 항목의 재개 조건은 **소진**됐다.
- [x] **D2 CB② 기한 날짜 → 사건** — 복원 시점 = **⑧ 해제와 동일 커밋**.
  채널 **[56] `cb2_restore_shadow`** 신설(스크립트·사전등록·리포트 배선·판정 어휘 4종).
- [x] **D3 [46] Hurst 미적용 · 감시 승격** — `promotion_watch` 사전등록
  (`abs_0.39` 단일 후보 · 판정창 08-24~ · **신규 통과분 평균 > 왕복비용**).
- [x] **D4 [33] Phase A 재실행** — `--perm 20000`. **초판 이래 첫 BH 통과**(`ctx.qty`).
  `last_run_date` → 2026-08-23.
- [x] **D5 [44] 보류 사유 교체** — "MW0601 대기"(487차로 소멸) → "손익 축 근거 없음".
- [x] **D6 ①·④ 미충족 기록 + ⑧ 동결** — CLAUDE.md 전환기준 ⑧에 선행조건 명기.
- [x] **D7 ⑨ TOX-SEVERE-SPREAD → 26주 WFA 이관** — 라이브 `False` 무변경.
- [x] **D8 [55] (E)안 사전등록** — 앵커 `ATR_MAX_ENTRY`(사후탐색 경계 미사용).
- [x] **D9 만성도 격리 소비부 제거** — 테스트 파일 삭제 + conftest fixture 제거 +
  `invariants.md` §0-C 신설(이관 시 복원 조건).

### 🆕 등록 — 관측 (다음 거래일 08-24 소관)

- [ ] 🆕 **O-26 (08-24 EOD)** 캠페인 요약표에 **`[56]` 행 출현** + 판정창
  `2026-08-24~` 표기. `INSUFFICIENT`가 **정상**이다(표본 0). 행 자체가 없으면
  배선 실패(1-17·O-24와 같은 형태).
- [ ] 🆕 **O-27 (08-24 EOD)** `[44]`·`[46]` 요약표 행에 **📌 결정 마커** 출현.
  마커가 없으면 다음 세션이 SUPPORTS_HYP/FAIL을 미조치로 오독한다.
- [ ] 🆕 **O-28 (매주 금)** `[56]` 표본 진행 — 정지일·제거 포지션.
  ⚠ **판정은 사전등록 기준 그대로**(313차 ④). 문턱을 움직이지 말 것.
- [ ] 🆕 **O-29 (매주 금)** `[46] promotion_watch` 신규 통과분 누적.
  ⚠ 본 채널 FAIL은 매주 재출력된다 — **📌 마커가 있으면 이미 처분된 것**이다.

### 🔵 주간회의 (2026-08-28) — 신규 안건

- [ ] 🔴 **D4 후속 — `ctx.qty` 후보를 집행(체결) 축 안건으로 상정.**
  [33] Phase B/C(진입 판별자 확증)로 보내면 **안 된다** — `qty`는 진입 정보가 아니라
  사이저 출력이고, 처방은 손절 체결 축이다. 417차 재분해(rho=+0.318)와 **같은 결론에
  독립 방법으로 닿은 것**이므로, 물어야 할 것은 *"큰 포지션의 손절이 왜 밀리는가"*
  (미니선물 호가 깊이 슬리피지 가설 — **여전히 미검증**)다.
  ⚠ 관측 구간이 431차·MarginCap 이후라 `qty`는 사실상 1~2뿐 — **3 이상 외삽 금지.**
- [ ] **[46]·[55]·[56] 표본 진행 보고** — 판정 창이 열린 첫 주다. 판정이 아니라
  **표본이 오고 있는가**만 확인한다.

### ⏭ 이월 (변동 없음)

- [ ] **멀티PC 컨벤션 보강 — "소비부만 옮기지 않았는가" 한 줄 추가.** (0821 등재)
  🔴 **489차 D9가 이 항목의 실례를 하나 더 만들었다** — 458차 만성도 계측은 반대로
  *"생산부 없이 소비부만 있었던"* 경우다. 두 방향을 다 적을 것.
- [ ] **458차 만성도 계측 생산부 이관 여부** — D9로 **(B) 소비부 제거**를 택했으므로
  이 항목은 *"이관할 것인가"* 만 남았다. 이관하면 `invariants.md` §0-C대로
  **격리를 함께 되살릴 것**(테스트가 강제한다).

### 📌 계측 규율 — 이번에 재확인된 것 (등록만, 조치 불요)

- [ ] **집계 단위를 먼저 확인하라.** CB② 반사실 초판이 레그 단위 로그
  (`[CB] 연속 손절 N회`, 2026-08-14 이전)에 오염돼 있었다. 470차 C1이 문서화해 둔
  덕에 구현 중에 잡혔다 — **문서화된 괴리가 실제로 사고를 막은 사례다.**
- [ ] **사후탐색 표는 문턱의 재료가 아니다.** D8이 486차 B-4의 ATR 3분위 경계
  (4.242/5.297)를 쓰지 않고 무관한 기존 상수 `ATR_MAX_ENTRY`에 앵커했다.
  테스트가 그 두 값과의 일치를 즉시 잡는다.
- [ ] **보류 사유가 소멸하면 재심사다.** D5. 침묵은 승인으로 읽힌다.

### 🆕 489차 후속 — [2] Meta-Gate 스코어러 딥다이브 (2026-08-24)

> 전문: `docs/미륵이고도화3/MetaGate_스코어러_딥다이브_및_숙제_2026-08-24.md`
> **코드 변경 0건.** D1 결론 무변경 — 다만 "왜 못 맞히는가"가 한 겹 내려갔다.

- [ ] 🔴 **H1 — 스코어러 AUC를 노출할 것.** 실측 0.4890~0.5477(5m·30m은 0.5 미만).
  매주 EOD 로그에 찍혀 왔는데 **캠페인 리포트에도 §12에도 없어 아무도 읽지 않았다.**
  → [2] 절 병기 **또는** 수집기 §12 등록. 무조건 상태 샘플이라 §5 규약에 저촉되지 않는다.
- [ ] 🔴 **H2 — `feat_names` 스키마 고착.** 모델 144키 vs 서빙 141키, 차이 3종
  (`program_*`, 451차 폐기분)이 **서빙 시 항상 0.0**. 매주 재학습해도 자동으로 안 낫는다
  (최다 키 행에서 스키마를 뽑으므로). ⚠ 영향도 **미측정** — 순열 중요도 필요.
- [ ] 🔴 **H3 — 학습 행 86.6%가 0 패딩** → 시점 대리변수 위험(371차 PSI 계열).
  가장 싼 처방은 **한 줄**: 결측을 `0.0` → `np.nan`(HGB는 네이티브 처리).
  고친 뒤 **H1의 AUC가 움직이는지**로 검증한다.
- [ ] 🟡 **H4 — 채점 경로 flat `SIGMA_K`.** `prediction_buffer`가 0.41을 전 호라이즌에
  쓰는데 `SIGMA_K_PER_HORIZON`은 15m 0.38 / 30m 0.33이다. 🔴 **30m은 CB③ 유일 입력원.**
  ⚠ **코드부터 고치지 말 것** — git 이력으로 의도/드리프트 판별 선행. 밴드를 바꾸면
  CB③ 임계 0.28도 재설계 대상이 된다(전환기준 ⑥).
- [ ] 🟡 **H5 — `conf≥0.70` OR 분기 실효 2.9%.** 유지/제거 판단. 제거 시 검증 시계 리셋.
- [ ] 📋 **H6 — 재설계 사전등록 순서**: ① 학습 파이프라인 → ② 레이블 정의 → ③ 피처 축.
  🔴 **"호라이즌을 맞게 채점하면 나아진다"는 이미 반증됐다** — 재시도 금지.
  ⚠ ①을 고친 뒤 AUC를 보고 **그 결과를 보기 전에** 합격선을 박을 것(313차 ④).

### 🆕 490차 — 스코어러 AUC 개선 P0~P2 배포 (2026-08-24)

> 전문: `dev_memory/DECISION_LOG.md` 490차 · 딥다이브 문서 **부록 A·B**.
> **매매 경로 변경 0건.** 검증 480 passed / 0 failed + 신설 17종.

- [x] **P0 학습 위생 배포** — NaN 패딩 · 스키마=마지막 거래일 교집합 · 스케일러 제거 ·
  Purged CV(엠바고=호라이즌 분). 🟢 **H2 해소 확인**: `feat_names` 144→141,
  451차 폐기 `program_*` 3종 소멸. 서빙 6/6 정상.
- [x] **P0-① AUC 소비 경로** — 사이드카 `training_metrics.json` + 리포트 **[2-A]** 절.
  종전엔 EOD 로그에만 찍혀 **아무도 못 봤다**(292·303·371·468차 계열).
- [x] **P1 재개 게이트** — `auc_net_directional` ≥ 0.53(앵커) **4주 연속**.
  현재 ⏳ INSUFFICIENT(판정창 08-25~).
- [x] **P2 섀도 게이지** — `auc_strong_directional` 매주 기록(**판정 미반영**).
- [x] **H3 부분 해소** — NaN 채택. ⚠ 학습창 절단은 **변형 E가 반증**해 채택하지 않았다.

### 🆕 등록 — 관측 (다음 거래일 08-25~)

- [ ] 🆕 **O-30 (08-25 EOD)** 캠페인 리포트에 **[2-A] AUC 표**와 **[2-B] 게이트** 출현.
  게이트는 **🔒 CLOSED**가 정상이다(순이익 AUC 0.4955~0.5152 < 0.53).
  `⚠ UNKNOWN`이면 앵커 상수 또는 사이드카 경로가 깨진 것이다.
- [ ] 🆕 **O-31 (매주 금)** 사이드카 `history` 누적 + 순이익 AUC 추이.
  ⚠ **문턱을 움직이지 말 것**(313차 ④).
- [x] ✅ **py37_32 서빙 검증 완료 (2026-08-24, 배포 당일)** — 라이브 런타임
  (py3.7 / sklearn 1.0.2)에서 사이드카 판독 OK · `scaled=False` 인식 OK ·
  6/6 호라이즌 정상 스코어 · **결측 다수 입력(NaN 경로)도 정상**.
  점수가 py310_64와 **소수 4자리까지 일치**(1m 0.3896 · 3m 0.5346 · 15m 0.2317).
  ⇒ 배포의 유일한 라이브 인접 리스크는 **사전에 닫혔다**.
- [ ] 🆕 **O-32 (08-25 장중)** 그래도 **실파이프라인 1회 확인** —
  `ensemble_decisions.meta_entry_quality_prob`이 계속 채워지는가(NULL 급증 없음).
  ⚠ 위 검증은 스크립트 직접 호출이다. 라이브는 `MetaGate.evaluate()` 경유라
  **호출 경로가 한 겹 더 있다**(계측 4원칙 ② — 검증한 것만 검증된 것이다).

### 🟡 남은 숙제 (490차 이후)

- [ ] **H2 영향도 미측정** — 폐기 피처 3종의 기여도. HGB는 `feature_importances_`가 없어
  **순열 중요도**가 필요하다. 스키마는 고쳤으나 "얼마나 해로웠나"는 모른다.
- [ ] **H4 `SIGMA_K` 스칼라** — git 이력으로 의도/드리프트 판별 **선행**.
  🔴 30m은 CB③ 유일 입력원이라 섀도 이야기로 끝나지 않을 수 있다.
- [ ] **H5 `conf≥0.70` OR 분기** — 유지/제거. 제거 시 §9-4 검증 시계 리셋.
- [ ] 👁 **15m 이상치** — 방향성 0.5834 · 강한추종 0.6214가 왜 홀로 높은가.
  **판정 금지**(사후탐색), 관찰만.
- [ ] 📋 **본질** — 위생을 고쳐도 순이익 AUC는 0.50이다. 병목은 파이프라인이 아니라
  **피처에 담긴 정보 그 자체**이며 26주 WFA 피처셋 재검증(L1~L3)의 몫이다.

### 🔵 주간회의 (2026-08-28) — 신규 안건

- [ ] **P2 처분** — `auc_strong_directional`(0.53~0.62)을 **청산 축** 채널로 승격할지.
  ⚠ 승격하려면 **새 채널 사전등록 필요**. 왕복비용을 이길지는 별개 문제다.
  0816 D4 우선순위(**청산 기하 > 집행 > 진입**)와는 방향이 맞다.

---

### 🆕 491차 — 장전 점검 (2026-08-24 · MW0602)

> 전문: `dev_memory/DECISION_LOG.md` 491차 · `docs/정기점검/매일점검/MW0602-20260824-점검리포트.md`
> **장전 국면이므로 라이브 코드·설정 변경 0건.** 아래는 전부 **장후 이후** 적용 대상이다.

**Fix (P1·P2)**

- [ ] 🔴 **F-1(491차) — 죽은 CB② 기한 「2026-08-29」 3곳 교체** (P1 · 대응 1-3)
  489차 D2가 기한을 **날짜 → 사건(⑧ 해제와 동일 커밋)** 으로 바꿨는데 정본
  (`references/invariants.md` 153·379행)만 갱신됐다. 남은 세 곳:
  · `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py` **168행** (`why` — 매일 §3 렌더링)
  · `.claude/skills/mireuk-daily-check/references/phases.md` **45행** (장전 A-3 **판정 문항 자체**)
  · `.claude/skills/mireuk-daily-check/references/report_template.md` **255행** (§7 예시 행)
  ⚠ **CB② 값(`9999`)은 손대지 않는다** — 기한 문구만.
  ⚠ `invariants.md`의 「효력 종료」 문맥 2행은 **역사 기록이므로 지우지 않는다.**
  ⚠ `config/dailycheck_targets.json`에는 이 문자열이 **없다**(실측 확인) — 파이썬 상수만.
  검증: `grep -rn "2026-08-29" .claude/skills/mireuk-daily-check/` → 2행만 잔존 · O-1(491차).
  🔴 **기한: 2026-08-25 장전 다이제스트 전.** 늦으면 죽은 날짜가 한 번 더 인쇄된다.
- [ ] **F-2(491차) — `gate_blocking_*` 산출 경로를 리포 루트 밖으로** (P2 · 대응 1-6)
  생산부 특정 선행: `grep -rn "gate_blocking_metrics" scripts/ --include=*.py`.
  ⚠ **소비부를 먼저 전수 확인**(`grep -rn "gate_blocking" --include=*.py .`) — 「소비부만
  옮기지 않았는가」의 역방향. ⚠ `docs/정기점검/금요일점검/`으로 보내지 말 것(성격이 다르다).
  권고 위치: `docs/미륵이고도화3/`.
- [ ] **F-3(491차) — 26주 WFA 피처셋 재검증 산출물 보존** (P2 · 대응 1-6)
  `docs/미륵이고도화3/26주WFA_20260824/` · `26주WFA_피처셋재검증_2026-08-24.md` ·
  `scripts/loo_feature_ablation_purged_cv.py`.
  ⚠ **보존이지 판정이 아니다** — 455차 「호라이즌 방향예측 피처셋 재검증」의 L1→L2→L3
  완료 여부와 판정은 **주간회의 소관**. 사후 데이터로 기준을 움직이지 않는다(313차 ④).
  ⚠ `loo_feature_ablation_purged_cv.py`는 재학습 계열 → **`py310_64` 전용** 확인(191차 OOM).
  ⚠ EOD 체인에 연결돼 있지 않은지 확인 — 연결돼 있으면 코드 변경이라 별건.

**고도화**

- [ ] 🔵 **G-1(491차) — §12에 `CORE축퇴_개장시점` 파생 축 신설** (이번 주)
  값 = **09:00 직전 마지막 `A_WARMUP` 로그의 축퇴 개수**(08-24 = `1` / 08-21 = `0`).
  `sample_axis` 는 분 단위가 아니라 **거래일 1샘플** — 관측률 판정 대상 아님.
  피처 이름 축(`CORE축퇴_피처`)도 개장시점 값을 별도로 남긴다.
  **왜**: 현행 `CORE준비도`는 하루 분포만 봐서 「축퇴한 채로 개장했는가」를 답하지 못한다.
  470차 B4의 판정 절차(20거래일 후 축퇴일 09:00~09:30 정확도 **일자단위** 비교)가 요구하는
  **코호트를 현행 분포로는 만들 수 없다.**
  🔴 **오늘이 코호트 1일차다 — 지금 배선해야 처음부터 쌓인다.**
  ⚠ **이 축을 근거로 차단을 신설하지 말 것**(317차 HurstGate 전례).
  ⚠ 26주 WFA 「고착 지표 감시목록」(468차 G-2)·「관측 계층 5종」(470차)에 **동시 등재**할 것.
- [ ] **G-2(491차) — 수집기 §14 「문서 내 미래 날짜 스캔」 신설** (이번 주, **F-1 다음**)
  `.claude/skills/mireuk-daily-check/**` + `CLAUDE.md`에서 `20\d\d-\d\d-\d\d` 를 훑어
  ① 30일 이내 도래 ② 이미 지난 날짜를 파일·행번호·주변 20자와 함께 나열.
  **판정하지 않고 나열만 한다.** ⚠ 오탐 다수(역사 기록 날짜) → **적신호로 올리지 않고 참고 표로만.**
  🔴 **선행: F-1.** 지금 켜면 자기가 고칠 3건을 적신호처럼 뱉는다.
- [ ] **G-3(491차) — 장전 발화 마진 블록에 `부팅~수집 소요 시간` 추가** (이번 주)
  마진 악화가 ① 스케줄 트리거 지연인지 ② 수집기 자체 지연인지를 가른다.
  분산이 크다는 것(−454초 ~ +12초)이 곧 원인이 스케줄이 아니라는 신호다.
  🔴 **cron 을 08:58:30 이전으로 앞당기는 처방과 함께 제안하지 말 것** —
  `phases.md` A-2(08:55 매크로 → 레짐 확정) 증거를 잃는다. 오늘 레짐 확정 실측 **08:58:06**.

**관측 (491차 신규)**

- [ ] 🆕 **O-1(491차) (08-25 장전)** 다이제스트 §3 `CB_CONSEC_STOP_LIMIT` 행에
  「2026-08-29」 **미출현** + 「⑧ 해제와 동일 커밋」 출현. F-1 적용 후.
- [ ] 🆕 **O-2(491차) (매 장전 · 누적)** `[CORE준비도]` 개장 직전 마지막 `A_WARMUP` 축퇴 개수.
  08-24 = **1**(`ofi_norm×6hz`) · 08-21 = **0**. **20거래일 축적 후** 470차 B4 절차로 판정.
  🔴 **그 전에 확정 결론·차단 승격 금지**(313차).
- [ ] 🆕 **O-3(491차) (매 장중)** — 🔧 **F-6(491차)로 판정 기준 재정의(2026-08-24 장후)**.
  종전: 「개장 후 **최초** `D_FORCE`/`B_OPEN` 축퇴 개수」 → **폐기**.
  신규: 「개장 후 **전** refit 의 ① **축퇴율**(축퇴≥1 refit / 총 refit) ②
  **최장 연속 축퇴 구간**(시각 범위)」.
  - **왜 바꿨나**: 「최초」는 1점 표본이라 **재발에 구조적으로 눈이 멀다.**
    08-24 에 정확히 그 사고가 났다 — 09:01 `0/6` 이라 종전 기준으로는 ✅ 였는데
    실제로는 **21/28 = 75%** · 최장 연속 **09:22~12:38(3시간 16분)** 고착이었다
    (0824 이상점 `1-10` 이 `1-1` 을 정정).
  - 기준선(08-24 실측): 축퇴율 **75%** · 최장 연속 **3시간 16분**.
  - ⚠ **O-2(491차)와 축이 겹치지 않게 할 것** — O-2 는 **장전** 마지막 값(`A_WARMUP`),
    O-3 은 **개장 후** 분포(`D_FORCE`/`B_OPEN`)다.
  - ⚠ 축퇴율이 높은 날이 나와도 **그 자체로 P1 승격하지 않는다** — 20거래일 축적 후
    470차 B4 절차로 판정한다(313차 ④: 사후 데이터로 기준을 움직이지 않는다).
- [ ] 🆕 **O-4(491차) (08-25 EOD)** `[Calibration] 축퇴 감지` 건수 · FloorShadow `판정 불일치` 비율.
  08-24 **24/24** · 08-21 **28/28**. **490차 [2-B] 재개 게이트 판정창(08-25~)과 함께** 읽는다.
  ⚠ 단독으로 확정 결론 금지 — 임계 `DEGENERATE_AUC_MIN=0.53`은 그 채널의 앵커다.
- [ ] 🆕 **O-5(491차) (오늘 장후)** `[CybosProbe]` 경고 총건수. 08-19·20·21 각 **798건**과
  같은 수준이면 **상시 조건 확정**(「선물 투자자 TR 미제공」 확인 필요 항목 종결).
  ⚠ 오늘 장전 18건은 로그가 09:02까지여서다 — **미측정 ≠ 감소**(계측 4원칙 ②).

**앞 국면 항목 처분 (491차 장전이 닫은 것)**

- [x] ✅ **O-1(486차) 해소** — 예약 점검 대상일 `2026-08-24` = 실제 거래일. 486차 F-1
  (휴장일 판정) 배선 후 첫 거래일이며 「🗓 거래일 문맥」 블록 정상 출력.
- [x] 🔴 **O-10(0821) 판정 — 실패(리셋 1/3 → 0/3)** — 오늘 발화 마진 **−36초**(개장 후).
  → 1-4로 이상점화 + G-3 등록.
- [ ] ➡ **P-1·P-2·P-4(486차) · O-17(0821) · O-16/O-5/O-7/O-8/O-9/O-18~O-23/P-8/P-9 → 장후 이월**
- [ ] ➡ **P-3(486차) → 장중 이월**
- [ ] ➡ **O-26~O-29(489차) → 오늘 EOD** (판정 창 08-24~ · **1일차**)
- [ ] ➡ **O-30~O-32(490차) → 08-25** (원 등록대로. 앞당기지 않는다)
- [ ] ⏸ **O-3(0821 런처 GUARD)** — 08-14·18·19·20·21·24 **6거래일 연속** `기존 프로세스 종료 완료` 1회.
  ⚠ **오늘 판정하지 않는다** — 무엇이 정상인지 기준이 등록돼 있지 않다. **판정 기준 사전등록이 선행.**
- [ ] ⏸ **O-4(0821 `[Capital]` 괴리율)** — −40.6%(08-14) → −40.7% → −40.8% → −42.3% →
  −42.1%(08-21) → **−41.5%(08-24, 실제잔고 29,274,631원)**. **횡보 — 추세 반전 아님.** 판정 기준 미도달.

**📌 계측 규율 — 이번에 재확인된 것 (등록만, 조치 불요)**

- [ ] **결정을 고칠 때는 사본을 세어라.** 489차가 정본만 고쳤고 사본 셋이 남았다.
  `grep -rn`으로 **개수를 먼저 세는 것**이 갱신 절차 1단계여야 한다.
- [ ] **판정 문항 자체가 낡으면 점검자가 성실할수록 틀린다.** `phases.md:45`가 그 형태다.
- [ ] **분포는 순간을 지운다.** 판정에 쓸 코호트를 정의할 수 없는 계측은 아직 계측이 아니다.
- [ ] **로그의 부재가 정보인 경우가 있다.** `_refresh_pre_retrain_state()`는 이미 해제됐으면
  무로그 조기 반환 — **로그가 찍혔다는 사실 자체가 G-1 경로였다는 증거**다.
- [ ] **같은 0이라도 분모를 밝혀라.** 오늘 진입 0건은 **「개장 36초 시점의 0」** 이다.

**🔵 주간회의 (2026-08-28) — 491차가 확인한 이월 안건 (신규 상정 아님)**

- [ ] **1-9 앙상블 등급 임계 재설계** — §12b **9일 연속 미도달**(관측 max **0.5862** < 0.60).
  0819 등재 → 0822 안건 → 0823 미처분 이월. ⚠ 재계산되는 판정이지 미조치의 증거가 아니다.
- [ ] **멀티PC 컨벤션 보강**(「소비부만 옮기지 않았는가」) — 0821 등재. 491차 1-3이 그
  **역방향 변종**(정본만 고치고 사본 셋 잔존)을 실증했으므로 문구에 양방향을 넣을 것.

---

## 🆕 492차 — 장중·장후 점검 일괄 (2026-08-24 · MW0602)

> 예약 점검(자동 실행). **라이브 코드·설정·커밋 변경 0건.** 장중 세션이 dev_memory 갱신을
> 장후로 미뤘으므로 이 절이 장중분을 함께 담는다.
> 당일: 포지션 24건 승 17(70.8%) · **net +68,089원**(자본 대비 +0.14%) · **절대원칙 6종 전부 준수** ·
> P0 0건. 🔴 **상위 1포지션 제거 시 −99,311원 · 롤링 20거래일 −107,232원** — 하루 흑자를 추세로 읽지 말 것.

### 🔴 Fix — 미적용 (491차 이월 + 492차 신규)

- [x] ✅ **F-1(491차) P1 — 2026-08-24 장후 구현** — 스킬 문서 3곳의 **죽은 CB② 기한 「2026-08-29」** 교체 (1-3)
  - `collect_evidence.py:168` `why` 문자열 · `phases.md:45` 판정 문항 · `report_template.md:255` 예시 행
  - 정본은 `invariants.md:153·379`(489차 D2 반영 완료). **오늘 다이제스트가 세 번째로 죽은 날짜를 인쇄했다.**
  - 소요 10분. 검증 `O-1(491차)` — 2026-08-25 장전
- [x] ✅ **F-5(491차) P1 — 2026-08-24 장후 구현** — `sizing_trace`에 `margin_cap_qty`·`margin_binding` + **불변식 테스트** (1-8)
  - 원인 `main.py:9750` `binding_gate = min(_sz_q, …)` — `_sz_q`에 증거금 축이 없다
  - 🔴 **오늘 5행 추가 → 계측 전구간 15행. `meta` 34건 중 8건(23.5%) 오귀속**
  - **⑧ 해제의 선행조건으로 명시할 것을 제안** — 470차 순서에서 오귀속률 상승 시점과 [28] 각성 시점이 같다
  - 소요 30분. 검증 `O-7(491차)` 미설명 축소행 **0행**
- [x] ✅ **F-4(491차) P1 — 2026-08-24 장후 구현** — 장중 재학습 조건 **도달 가능성** 매일 계측 (1-7)
  - 전일 `[DriftRetrain]` **369/369 `cond=False`** · **max(n)=4**(요구 15) · `acc5m` 369/369 `0.0%`
  - ⚠ **임계 변경 금지**(사전등록 ④). 계측만
  - 소요 20분
- [x] ✅ **F-7(492차) P1 — 2026-08-24 장후 구현** — **SGD 학습률 조절기의 「상한 포화」를 매일 보이게** (1-13)
  - 🔴 **`ALPHA_MAX`·`ALPHA_UP_FACTOR`·`DRIFT_THRESHOLD`·`RECOVERY_THRESHOLD`·
    `MIN_SAMPLES_REQUIRED` 전부 무변경.** 계측만
  - ① `drift_adjuster.py:136~146` — `old == new` 면 `(상한 포화 — 무연산)` 병기 + `WARNING`→`INFO`
  - ② 액션명 `DRIFT_UP` / **`DRIFT_UP_SATURATED`** 분리
  - ③ `main_dashboard.py:4873`·`:5113` 을 `startswith("DRIFT_UP")` 로 (정확 일치면 미표시)
  - ④ `record_accuracy()` 로그에 **`n=%d src=online_learner.recent_accuracy`** 병기
    ← ⚠ **①②③보다 중요하다.** 오늘 `acc=0.0%`가 진짜 0%인지 「미측정 0」인지 구분 안 됨
  - ⑤ 수집기 §12에 `sgd_alpha`·`drift_action` 2행 (⚠ `sgd_alpha`는 **`benign` 아님**)
  - ⑥ `tests/test_492_drift_adjuster_saturation.py` 신설 — **이 테스트가 깨지는 날이 「하향 경로가 살아난 날」**
  - 소요 25분(회귀 포함 40분). 검증 `O-12(492차)`
- [x] ✅ **F-6(491차) P2 — 2026-08-24 장후 구현** — `O-3(491차)` 판정 기준 「최초 1점」 → 「개장 후 전 구간」 (1-10). 소요 10분
- [x] ✅ **F-8(492차) P2 — 2026-08-24 장후 구현** — EOD 두 DB 정리 경로 구분 (1-14)
  - `[Retrain] DB pruning 실패: database table is locked` 에 **대상 테이블명·삭제 예정 행수** 추가
  - 락 실패 시 1회 재시도(1초) 후 실패면 그때만 `WARNING`
  - ⚠ **재시도 루프 금지** — 15:40:03은 저장이 몰리는 구간이고 15:45 EOD 체인(`[WaitDC]` 20분) 여유를 갉는다
  - 소요 15분. 검증 `O-13(492차)`
- [x] ✅ **F-2·F-3(491차) P2 — 2026-08-24 장후 구현** — 리포 루트 산출물 2건 정리 + 26주 WFA 산출물 커밋 (1-6, **5건 → 7건**)

**적용 순서**: `F-1 → F-5 → F-4 → F-7 → F-6 → F-8 → F-2·F-3` → ✅ **2026-08-24 장후 전량 구현 완료**(사용자 지시). 상세·정정 3건은 `DECISION_LOG.md` 2026-08-24 「492차 후속」 참조.
> 🔴 그 과정에서 **계획에 없던 사실 3건**이 드러났다 — ① CB② 기한 사본이 3곳이 아니라 **4곳**이었고 4번째(`PHASE5_GATE_DECISIONS[5]["due"]`)가 **유일하게 라이브에서 소비**되던 것(대시보드 배지) ② **1-14 의 「서로 다른 두 정리 경로」 진단은 틀렸다** — 같은 호출의 안/밖이며, 커밋 실패분 3,810행을 **성공으로 보고**하고 있었다 ③ §12b 는 DB 원천만 볼 수 있어 로그 원천 임계쌍을 **등록조차 못 하는** 반쪽이었다(F-4 에서 신설).
⚠ **F-4·F-5·F-7은 전부 「계측만 붙이고 임계는 안 건드린다」** — 316차 HurstGate가 진짜 신호의
72.3%를 오판 차단한 전례. 게이트·차단 신설은 섀도 계측 후 승격.

### 🟡 고도화 — 미적용

- [ ] **G-1·G-2·G-3(491차)** — 장전 CORE 축퇴 별도 축 / 스킬 문서 날짜 기한 기계 스캔 / 장전 발화 부팅 지연
- [ ] **G-4(491차)** — 「도달 불가 분기」를 §12b가 자동으로 찾게. **오늘 1-13이 세 번째 사례**
- [ ] **G-5(491차)** — 「미측정 0.0%」와 「진짜 0.0%」 구분. **오늘 1-13 ⚠②가 같은 문제**
- [ ] **G-6(491차)** — 진입 방향 편향을 「그날 시장」과 나눠 보는 축. **오늘 숏 100%(24/24)로 표본 확대**
- [ ] **G-7(492차) 🆕** — `THRESHOLD_REACHABILITY_REGISTRY` (가칭). §12b 하드코딩 3쌍 → **레지스트리 순회**
  - 오늘로 **다섯 번째**: 1-7(`_min_conf_sgd` 0.52 / 통과율 4.57%) · 1-9(앙상블 0.70·0.60 / max 0.5862) ·
    **1-13(`RECOVERY_THRESHOLD` 0.58 / 10일 max 0.4048)** · 471차 F-1(6개월) · 474차(6개월)
  - ⚠ **판정 기준을 만들지 않는다.** ⚠ **임계 인하 근거로 읽지 말 것**
  - **선행: G-4 먼저 → G-7로 확장.** 소요 G-4 이후 40분
- [ ] **G-8(492차) 🆕** — 수집기 §13-b「사전등록 판정 창 — **표본 진행만**」 (1-15)
  - `채널 · 판정창 시작일 · 경과 거래일 · 현재 표본 / min_samples` **네 열만**
  - 🔴 **판정·권고·합격선 절대 출력 금지** — 일일 점검이 판정하면 **458차 D6과 같은 사전등록 위반**
  - 비용 크면 「원천 행수」만 세는 축소판부터. 소요 축소판 30분 / 전량 90분

### 🆕 관측 등록 (492차)

- [ ] **O-12(492차)** — `[DriftAdjuster]` 액션·alpha. F-7 배포 후 `DRIFT_UP_SATURATED`·`n=`·`src=` 출현.
  배포 전엔 alpha 분포만(오늘 `0.01000` 단일값 · **9거래일 연속**) → **F-7 배포 다음 거래일**
- [ ] **O-13(492차)** — `[Retrain] DB pruning 실패` 재현 여부. 3거래일 중 2일↑ 이면 상시 → **2026-08-27**
- [ ] **O-14(492차)** — `[Bias⚠] 5m … UP=n` 의 UP 건수. **오늘 UP=0**(30건 중).
  방향 편향이 예측 단계인지 진입 단계인지 가른다(`O-11`의 상류 축) → **20거래일 누적**
- [ ] **O-15(492차)** — TP1 도달 코호트. 오늘 도달 19건 89.5% +44,223원 / 미도달 5건 **0.0% −154,431원**.
  ⚠ **`tp1_*` 확정 결정 3건이 이미 있는 질문 — 신규 계측 제안 금지, 표본만 누적** → **매 장후**

### ⏭ 이월 (기준 무변경)

- [ ] `O-1`(491차) → **2026-08-25 장전** (F-1 미적용)
- [ ] `O-2`(491차) 장전 축퇴 개수 → 누적 **2일차, 오늘 `1`**
- [ ] `O-4`(491차) `[Calibration]` 축퇴·FloorShadow → **2026-08-25 EOD** (전일 174건)
- [ ] `O-6`(491차) `max(n)` → 누적 **1일차, 오늘 `4`**
- [ ] `O-7`(491차) 미설명 축소행 → F-5 배포 후 (오늘 **5행** · 전구간 **15행**)
- [ ] `O-8`(491차) `acc30m` 쌍 → 20거래일 (오늘 쌍 1건)
- [ ] `O-9`(491차) 30m SGD 리셋 후 CB③ 접촉 → 20거래일 (오늘 리셋 **7회** · CB③ 0회)
- [ ] `O-10`(491차) 개장 후 축퇴율 → 누적 **2일차, 오늘 29/36 = 80.6%**
  (08-18 68% / 08-19 100% / 08-20 72% / **08-21 0%** / 08-24 80.6%)
- [ ] `O-11`(491차) 진입 방향 편향 → 누적. **오늘 숏 100%(24/24 포지션 · 30/30 레그)**
- [ ] `O-17`(0821) `entry_horizon` 소비처 전수 → **2026-08-25 장후** (미착수)
- [ ] `O-23`(0821) `sizing_trace.margin_cap_qty` → F-5 배포 후 (필드 부재 재확인 · 181행 중 0행)
- [ ] `O-20`·`O-21`(0821) → **2026-08-28(금)** 금요일 전용 체인
- [ ] `O-26~O-29`(489차) `[56]`·`[44]`·`[46]` → **2026-08-28(금)**.
  ⚠ 판정 창은 **오늘** 열렸으나 렌더링 수단이 금요일 전용(**1-15**)
- [ ] `O-30~O-32`(490차) → **2026-08-25** (원 등록대로, 앞당기지 않는다)
- [ ] ⏸ `O-3`(0821 런처 GUARD) — **7거래일 연속**. ⚠ **판정 기준 미등록 — 사전등록이 선행**
- [ ] ⏸ `O-4`(0821 `[Capital]` 괴리율) — **−41.5%**. **횡보, 추세 반전 아님**

### ✅ 완료 처리 (492차에 닫은 것)

- [x] **`1-2` 앙상블 확률 보정기 스냅샷 9거래일 정체 — ✅ 해소.**
  pkl mtime **2026-08-24 15:40:03** · 3,008→**5,299바이트** · `[Calibration] 저장 완료 **n=1478**`.
  **0821 F-1(저장 성공/실패 양방향 로그) 라이브 검증 완료**
- [x] `P-1`·`P-2`(486차) ✅ 판정 통과 — 위와 동일 근거
- [x] `P-3`(486차) ✅ 통과 — `[DriftRetrain]` **369줄 / 371분**
- [x] `P-4`(486차) ✅ 통과 — §5 `조회 53 · 축소 5` == 로그 `OK`×48 + `CAP`×5 = **53**
- [x] `P-8`(0821) ✅ 기록 — 롤링 20거래일 **−107,232원** (배너 −107,234원, 2원 차)
- [x] `P-9`(0821) ✅ 통과 — `15:11:02 [SchedForceExit] … bar_pass=2회 → 청산 대상 없음(정상)`
- [x] `O-6`(0821) ✅ 통과 (대사 3/3) — **477차 후속 F-1(`entry_qty_final`) 라이브 검증 완료**
- [x] `O-22`(0821) ✅ 밴드 내 — `75 / 364 = 20.6%` (사전등록 20.3~20.5%±5%p)
- [x] `O-5`(491차) ✅ 종결 — `[CybosProbe]` **798 == 798**. **상시 조건 확정**
- [x] 선물 투자자 수급 TR ✅ 해소 — 09:20:02 `supported=True`. **상시 제약 아님**
- [x] `1-4`(491차) ✅ 해소(국면 조건). `O-10`(0821) 카운터 재시작은 **내일 장전**부터

### 🟠 판정 보류 (확정 결론 금지 — 313차)

- [ ] `1-11` `acc30m` 두 원천 — 쌍 표본 **1건**. 닫는 조건 `O-8` 20거래일
- [ ] `1-12` 30m SGD 붕괴 — 전일 **7회**, 08-21 6회와 같은 수준 = **상시**. 닫는 조건 `O-9` 20거래일
- [ ] `1-14` DB pruning — **1거래일**. 두 경로(`[Retrain]` 실패 / `[GBM]` 성공)이며 **실해 없음**. 닫는 조건 `O-13`
- [ ] `1-5` GuardFair 무효율 — **9거래일 54표본 100% 무효**, 사유 6/6 동일.
  섀도라 채널 [23] PASS — **죽은 것 아님**. ⚠ **판정 기준 미등록** — 470차 C3'(26주 WFA)
- [ ] 사후검증 §4p 전 축 — 등급 역전·계약수(n=6)·KellySkip·5m(n=7).
  **1거래일 · 일 내 층화 = 일자단위 검정 아님.** 등급 축은 **오늘과 30일이 반대 부호**.
  🔴 `grade_ev_inversion`·`kelly_skip`·`tp1_*` 3계열 **전부 §13 확정 결정** — **신규 계측 제안 금지**

**📌 계측 규율 — 이번에 재확인된 것 (등록만, 조치 불요)**

- [ ] **「액션을 기록했다」와 「값이 바뀌었다」는 다르다.** 상태 전이 로그는 전·후 값을 둘 다 찍고,
  같으면 같다고 말해야 한다 (1-13: `DRIFT_UP` 6일 / alpha 9일 불변)
- [ ] **적응형 제어기는 포화 시점을 스스로 알려야 한다.** 상·하한에 붙는 것은 정상이지만
  **며칠째인지**는 계측 없이 아무도 모른다
- [ ] **로그 파일 범위가 다르면 같은 태그도 다른 수를 낸다.** `CybosProbe` 18 vs 798 —
  감소가 아니라 `PROBE.log`가 대상 밖이었다. **분모를 밝혀라**(계측 4원칙 ②)
- [ ] **하루 흑자에도 이상치 분해를 하라.** +68,089원 → 상위 1건 제거 시 −99,311원.
  489차 D6이 캠페인 전체에서 본 구조가 **하루 안에서도 나타난다**
- [ ] **「청산 로그 없음」과 「청산할 것이 없었음」은 다르다.** 하트비트(`bar_pass=2회`)는
  **경로 생존**만 증명하지 **주문 발사**를 증명하지 않는다 —
  **실전 전환 기준 ②의 선행 확인은 2026-06-10 이래 여전히 미충족**

**🔵 주간회의 (2026-08-28) — 안건 (491차 이월 + 492차 추가)**

- [ ] ① **1-9 앙상블 등급 임계 재설계** — §12b **10일 연속 미도달**(max **0.5862** < 0.60).
  0819 등재 → 0822 안건 → 0823 미처분 이월. ⚠ 재계산되는 판정이지 미조치의 증거가 아니다
- [ ] ② **F-8 채널 생산부 결손** 잔여 확인 (487차 후속으로 (B) 확정·(A) 폐기)
- [ ] ③ **멀티PC 컨벤션 보강**(「소비부만 옮기지 않았는가」) — 491차 1-3이 **역방향 변종**
  (정본만 고치고 사본 셋 잔존)을 실증했으므로 문구에 양방향을 넣을 것
- [ ] ④ **`auc_strong_directional` 청산 축 승격 여부** (490차 등재)
- [ ] ⑤ 🆕 **「상수 고착」 3건(1-7·1-9·1-13)을 개별이 아니라 한 묶음으로 처분할지** — D7·G-7 참조.
  다섯 사례가 각각 3~6개월씩 아무도 몰랐고, 전부 **사람이 우연히** 발견했다
## 2026-08-23 (MW0602 487차 — 멀티PC 정책 폐기 + F-8 MW0602 한정 권고)
...
- [ ] **P1 F-A `core_feature_health` 일별 1행 (이상점 1-3)** — `utils/db_utils.py`에 테이블
      신설(일자·호라이즌·피처명·`raw_std`·`identity_forced`·`trigger`), `daily_close()` 배치 1회 기록.
      🔴 **매분 경로에 DB 쓰기 추가 금지** — CB⑤ 여유 잠식(2026-08-10 7,619ms 전례).
      기등록 `고도화 ⑦ CORE 피처 건강도 일별 1행`과 **동일 항목** — 중복 등록 아님, 근거 보강.
      검증: 오늘 로그 재생 → 장전 48건·종일 집계가 테이블과 일치.
- [x] **P2 F-B `main.py` 기동 시 `[Runtime]` 1행 (이상점 1-5 · N-12 영구 해소)** — `[DONE 2026-08-24 491차]`
      라이브 `[Runtime]` 1행 배선 완료(`main.py:_log_runtime_versions()`, `[Capital]` 뒤 1회). 🔴 **실측
      드리프트 1건** — 이 PC `joblib=1.1.0` vs CLAUDE.md 표 `1.1.1`. 크래시 위험 축(scipy·비트수)이 아니라 WARNING
      대상에는 넣지 않았고 값은 매 기동 로그에 보인다. 문서 정정 여부는 사용자 판단(아래 별도 항목).
      `sys.version` / `platform.architecture()[0]` / scipy·sklearn·joblib·numpy `__version__`.
      위치: `[Capital]` 로그 근방(`main.py:12727`). try/except로 감싸 기동 차단 금지
      (`[Capital] 자본 프로파일 로그 실패(무해)` 관례). 기대값 불일치 시 **WARNING만, 자동 조치 없음**.
      ⚠ py37_32에서 실행 — f-string 중첩·walrus 등 3.8+ 문법 금지.
      검증: 다음 장전 `grep -n "\[Runtime\]" logs/{날짜}_SYSTEM.log` 1건 → **N-12 영구 종결**.
- [ ] **P2 F-C Cybos 투자자 탐침 COM 초기화 후 섀도 재실행 (이상점 1-6)** —
      `collection/cybos/investor_data.py` 워커 진입점에 `pythoncom.CoInitialize()`/`CoUninitialize()`
      후 5개 ProgID 재탐침, 결과는 `[CybosProbeShadow]` 별도 태그.
      🔴 피처 경로·`futures_supported` 판정·학습 입력 **무변경**(섀도 선행 규약).
      🔴 절대원칙 §4 확인 — COM **콜백 내부가 아니다**. 콜백에 `dynamicCall`/`emit` 넣지 말 것.
      회귀 위험 중간(아파트먼트 모델) → **장 마감 후 단독 기동 시험 선행**, 라이브는 다음 장전.
      검증: ProgID 중 **하나라도 성공하면 "TR 미발견"이 오판** → 별도 안건 승격.
- [x] **P2 F-D 수집기 §2·§11에 실질 변경 수·EOL 상태 병기 (이상점 1-1)** — `[DONE 2026-08-24 491차]`
      §2 에 실질/코드/EOL 분해 + 파일명(12개 절단 시 `… 외 N개`), §11 은 실질 0건이면 미출력·코드 섞이면 무조건·**미측정이면 출력**. ⚠
      구현 중 초판의 분모 불일치(porcelain ` M` vs diff numstat)를 발견해 둘 다 `git diff --numstat` 에서 뽑도록
      고쳤다(461차 `mdd_pct` 계열).
      `collect_evidence.py`. `미커밋 486건 (실질 4건 · 코드 0건 · EOL 차이 482건)` 형식.
      §11 적신호는 **실질 변경 0이면 미발화, 코드 파일 섞이면 무조건 발화**.
      기등록 `P2-K`와 동일 항목 — 근거 보강.
- [ ] **P2 F-E `CLAUDE.md:654` "모의 잔고(4.9억)" 정정** — 실측 `[Capital] 실제잔고=49,955,870원
      괴리=-0.1%`. 결정은 **438차(2026-08-06, DECISION_LOG 5220행)에 이미 남아 있다**
      ("4.9억 → 5천만, ⑧의 전제 무효화, ⑧ 재검토 시 반드시 반영할 것"). 문서만 갱신.
      ⚠ **⑧의 해제 조건은 무변경** — 남은 조건은 여전히 "실전 자본 재설정" 1건.
      461차 F-3 · 483차 P2-J와 같은 계열(코드는 옳고 문서가 낡음).

### 고도화 (당일 관측 근거)

- [ ] **고도화 ① Canary 임계 여유 로깅 (근거 1-4)** — `z경고피처=12개 (임계 12개, 여유 0개 ·
      초과율 0.0%)` + 재적합 후 값 + **잔존 피처의 호라이즌** 병기.
      ⚠ 임계 12를 **지금 바꾸자는 제안이 아니다** — 바꿀 근거를 만들자는 제안.
- [ ] **고도화 ② `[PreflightSummary]` 장전 자격 1행 (근거 1-B)** — branch/origin/eod_retrain/
      broker/regime/rt_sub/optchain/invariants/gates_off/blocking_max/capital_dev.
      ⚠ **판정 미관여 게이지** — 어떤 값이어도 진입을 막지 않는다.
      Why: 471차 F-1(15:10 경로 6개월 구조적 도달 불가가 어떤 계측에도 안 걸림) 재발 방지.
- [ ] **고도화 ③ 다이제스트 §0 「관측 지점 프로파일」 (근거 1-1)** — 셸 종류 · `core.autocrlf` ·
      워크트리/인덱스 EOL · 라이브 DB 접근 가부 · 런타임 버전 확인 가부.
      F-D가 숫자를 고친다면 이것은 **숫자를 읽는 법**을 고친다.
- [ ] **고도화 ④ CB② 카운터팩추얼 표 — 08-27(목) 장후 선제 작성 (근거 1-2)** —
      한도 2/3/5/9999별 ① 당일 정지 발동 일수 ② 차단됐을 진입 건수 ③ **포지션 단위** 실현손익 합.
...
- [x] **P1 F-F 점검 세션 git 호출이 락을 남기지 못하게 한다 (이상점 1-7)** — `[DONE 2026-08-24 491차]`
      ① `SKILL.md` 「git 호출 규약」 신설(`--no-optional-locks` 를 **세션 명령까지** 확대,
      `diff`·`log`·`rev-list`·`ls-files` 명시) ② 수집기 락 자가점검(시작 스냅샷 vs 종료 비교 → §2·§11 양쪽) ③
      `SKILL.md` §7 체크박스.
...
## 2026-08-24 (MW0601 490차 후속 — 장중 점검) 등록분

> ⚠ 전부 **장후(15:10 이후) 적용 대상**. 장중 코드 변경 0건.
> 함정 ① 확인 완료 — CB③ HALT 비활성 · `tp1_protect_offset` · JointGateBlock 임계/합성은
> **전부 확정 결정(재상정·재론 금지)**이므로 아래 항목은 **계측·가시화만** 다룬다.

### P1

- [x] **F-G — CB③ 「조건 성립 분수 + 그 창 안 진입·손익」 계량 (이상점 1-8)** `[DONE 2026-08-24 491차]`
      `cb3_acc30m`(표본 없으면 None) · `cb3_would_halt` 프로퍼티 + 카운터 3종 + 15:40 `[CB③계측]` **무조건** 1줄.
      ⚠ mc-conf 경보에 얹지 않았다(경보 발화 여부에 종속되지 않게). 🔴 HALT 경로 무변경.
      오늘 `10:55:00~11:24:00` 30분 연속 `acc30m=0.0% n=30/30 ready=Y state=NORMAL`,
      그 창 안 진입 2포지션 **-128,195원**(자본 대비 -0.26%). 그런데 이 사실이 **운영 로그
      어디에도 없다** — `[DBG-CB]`(DEBUG 채널) 한 줄뿐이고 `[CB③ 비활성]`은
      `logger.debug` × `LOG_LEVEL=INFO`로 전 로그 0건(477차 P1-i-1 기등록).
      482차 G-1 `_mh_cb3_ready_minutes`는 **가용 분수만** 센다.
      **변경**: `main.py:997` `__init__`에 `_mh_cb3_would_halt_minutes` /
      `_mh_cb3_would_halt_entries` / `_mh_cb3_would_halt_pnl_krw` **명시 초기화**
      (계측 4원칙 ④ — `getattr(self,"_x",default)` 금지) · `main.py:7011` 부근 증분 ·
      진입/청산 확정 지점에서 **포지션 단위** 누산(계측 4원칙 ①) ·
      `main.py:11748` 15:40 로그에 `| CB③ 조건성립 %d분 · 그 창 진입 %d포지션 · 손익 %+,d원` ·
      `safety/circuit_breaker.py`에 `acc30m_ready`/`acc30m_value` 읽기전용 프로퍼티.
      🔴 **HALT 경로(`circuit_breaker.py:409~424`)는 손대지 않는다 — 재상정 금지.**
      ⚠ **리셋 전 스냅샷** 필수(`daily_close()`의 `_ccf_today` 관례).
      **검증**: 오늘 로그 재생 → `조건성립 30분 · 진입 2포지션 · -128,195원`.
      `tests/test_457_fallback_visibility.py` 신규 속성 3종 통과.
      **회귀 위험**: 낮음(판정 경로 무관여).

- [ ] **F-J — 5~180초 정지 구간 스택 스냅샷, 섀도 한정 (이상점 1-10)**
      `11:16:22` 메인 스레드 **20,985ms** 정지(`band=ALERT`, 6거래일 중 첫 ALERT).
      잔차 20,314ms(97%)가 S0~S8 **밖**. CB⑤(5초·파이프라인)와 FZ-1(180초·하트비트) 사이
      **설계상 공백**이며 `FaultHandler` 행감지=30s라 덤프도 없다.
      **변경**: `main.py` `_tick_header` 블로킹 감지부 — `band=ALERT` 발화 시
      `faulthandler.dump_traceback(file=logs/mainstall_<date>.log, all_threads=True)` 1회,
      **하루 5회 상한**, **정지가 끝난 직후**에 기록(덤프가 메인을 추가로 막지 않게).
      ⚠ 절대원칙 §4(COM 콜백 내 `dynamicCall`·`emit` 금지) 저촉 위치가 아닌지 확인 필수.
      **검증**: 인위 20초 지연 주입 → `logs/mainstall_*.log` 프레임 기록.
      ⚠ **482차 F-3 섀도 2주 관찰을 대체하지 않는다**(오늘 2일차).

### P2

- [x] **F-I — 기등록 P2-I(`trades.exit_stage`) 근거 보강 (이상점 1-9)** `[DONE 2026-08-24 491차]`
      P2-I 를 **원안 그대로 구현**했다(근거 보강에서 구현으로 승격 — 사용자 조치 6번). 🔵 구현 중 확인: 오늘 TP1 보호전환 8건 중
      `trades` 에 행을 남긴 것은 **1건**뿐이다(qty=1 `arm_tp1_single_contract*` 는 청산 레그를 만들지 않는다). ⇒ **DB
      만으로는 이 구분을 영영 복원할 수 없다** — 컬럼이 필요한 이유의 결정적 근거.
      **새 fix가 아니다.** P2-I 표본이 4→**8포지션**으로 증가, 오염률 75%→**66.7%(4/6)**.
      오늘 1계약 TP1 후 즉시 청산 4연속(경과 22초/4초/7초/68초, 평균 25초, 평균 +0.5875pt).
      원안 그대로: `exit_reason` 문자열 **무변경**, 컬럼 추가
      (`INITIAL_STOP`/`TRAIL_AFTER_TP1`/`TIER1_EARLY`/`TP1`), 과거 행 NULL=**미측정**.
      **검증**: 오늘 6포지션 재생 → `TRAIL_AFTER_TP1`×4 · `INITIAL_STOP`×2 ·
      CB② 연속손절 카운터 (1,1,1) 유지.
      🔴 **`tp1_protect_offset` 조정 미포함** — 캠페인 [25] 2026-08-08 주간회의 확정 결정.

- [x] **F-K — `[JointGateBlock 차단]` 무정보 폴백 가시화 (이상점 1-11)** `[DONE 2026-08-24 491차]`
      로그·`entry_block_reason` 3곳 `<fallback>` + `ensemble_decisions.meta_size_fallback`. ⚠
      컬럼명은 리포트의 `meta_fallback` 이 아니라 `joint_gate_shadow` 와 맞춘 **`meta_size_fallback`** 이다(두
      테이블 조인 성립). 🔴 임계·합성 무변경.
      차단 12건 중 8건이 `meta=0.50`(무정보 폴백 추정)인데 **로그만으로 실측 0.50과 구분 불가**
      — 확정하려면 같은 분 `[MetaGate]` 로그와 교차대조해야 했다(계측 4원칙 ④).
      **변경**: 차단 로그를 `(meta=0.50<fallback> tox=0.70 joint=0.350)` 형태로 ·
      `ensemble_decisions.meta_fallback` 불리언 동반(473차 `spread_extreme_shadow` 선례).
      🔴 **임계·합성 방식 무변경** — 431차 확정 결정, 캠페인 [7] PASS 게이트가 걸려 있다.
      **검증**: 오늘 12건 재생 시 8건에 `<fallback>`.

### 고도화 (당일 관측 근거)

- [ ] **고도화 ⑤ — 08-28 CB② 결정용 카운터팩추얼 표에 CB③ 열을 합본**
      장전 고도화 ④의 확장. 08-21·08-24 두 거래일 `[DBG-CB]` 재집계로
      「CB② 한도별(2/3/9999) 정지 시각 · 차단됐을 진입·손익」 + 「CB③ 조건성립 분수 ·
      창 안 진입 손익」을 **한 표**로. 오늘값: CB② 카운터 3회 전부 1회에서 끊김(정지 0) /
      CB③ 30분 성립 · 2포지션 · **-128,195원**.
      ⚠ **표본 2거래일 — 확정 결론 금지.** 표는 결정을 대신하지 않는다.
      🔴 **CB③ HALT 재활성화 제안이 아니다.**

- [ ] **고도화 ⑥ — 「TP1 보호전환 → 청산」 경과초 라이브 계측 (사전등록 문턱 명시)**
      매 EOD에 `[TickTP1]`→`[TickStop-S0C]` 쌍의 **경과 초** + 그 사이 **최대 유리 이동폭(pt)**
      히스토그램 1행. 오늘값 4건 · 4/7/22/68초 · 평균 25초 · 평균 +0.5875pt.
      캠페인 [25]는 **소급 재생** 기반이라 라이브 경과 시간은 재지 않는다 — 겹치지 않는다.
      **사전등록 판정 문턱**: 20거래일 누적 후 `경과초 중앙값 < 60초 && 최대 유리 이동폭
      중앙값 > TP1 폭 × 2` 이면 **주간회의 상정**. 아니면 계속 관찰.
      ⚠ **문턱을 사후에 낮추지 않는다**(458차 D6 위반 금지).

- [ ] **고도화 ⑦ — `[IntradayRegime]` 로그에 z 산출 표본 봉 수 `n=` 병기**
      `09:02:00 … day=-0.09% ATR=1.00 z=5`가 개장 2분차 얕은 표본인지 진짜 급변인지
      **로그만으로 판정 불가**해 5거래일 대조가 필요했다. 표기만 추가, **레짐 판정 로직 무변경**.

### 다음 국면(오늘 장후)이 닫을 관측 항목

- [ ] **O-2′** 이상점 1-3 ② 축 — `cvd_divergence`·`ofi_norm` 피처 구성 기여.
      15:40 SHAP 심사 산출물로 판정. ⚠ 2026-06-17~07-10 SHAP 정렬 오염 구간 수치 **재인용 금지**.
- [ ] **O-5** `[Capital]` 2행 중복 — `main.py:12727` 호출 경로 (P2 미만).
- [ ] **N-10** (누적) 15:10 강제청산 실집행 — 누적 0회. `[ForceExitPass]`→`[TimeExit]`→
      `[ExitAttempt]`가 15:10:00~05에 뜨는가. 15:11에 `[SchedForceExit] … 안전망 발동`(ERROR)이
      뜨면 **1차 경로가 다시 죽은 것**(471차 F-1).
...
> 근거: `docs/정기점검/26주WFA_피처셋재검증_L1L2L3_MW0601-20260824.md`,
> `DECISION_LOG.md` 2026-08-24 490차 후속2. 원시 산출물:
> `docs/정기점검/26주WFA_MW0601-20260824/` (다음 사이클 2027-02 대조용 기준선).
> **피처셋은 무변경 결정** — 교체할 후보가 없다(L2 984셀 전멸, L1 통과자 2개 중 1개는 계측 불가).

### A. 계측 신뢰 회복 — 피처 변경보다 **먼저**

- [ ] **A-1** L1·L2에 `--live-only`(백필 행 제외) 신설 + 출력 헤더에 백필 비율 1줄.
      🔴 **이번 창의 74.6%(83/130거래일)가 백필 행**인데 L1·L2·L3 어디에도 표시되지 않는다.
      주간 「피처셋 건강 리포트」는 **418차부터 이미 기본 제외**(`BACKFILL_QUALITY_MARKER=0.3`,
      `--include-backfill`로 복원)한다 — 같은 마커를 26주 배터리에 **전파만** 하면 된다.
      세 스크립트의 `feature_quality_score` 참조는 현재 **0건**.
      ⚠ 합격선·판정문 무변경 → 사전등록(§9-4) 대상 아님.
...
## MW0601 490차 장후 — 일일 점검 (2026-08-24) 신규 항목

> 전부 **미조치**. 코드 변경 0건. 적용은 사용자 지시 후 별도 세션.
> 근거: `dev_memory/DECISION_LOG.md` 2026-08-24(490차 장후),
> `docs/정기점검/매일점검/MW0601-20260824-점검리포트.md` 제3부.

### P0

- [x] **F-L** `_on_gbm_retrain_done()`의 GUI 접근을 메인 스레드로 되돌린다 (이상점 1-12) `[DONE 2026-08-24 491차]`
      `_dashboard_call()` 신설 + GUI 접근 3곳 위임 + docstring 정정. 🔵 **반례 실증**: 신설 검출기를 수정 전 HEAD
      `4dbdf80` 에 돌리면 3건을 잡고 수정 후 0건이다.
      - `main.py:4733` 진입부에서 스레드 판정 → `self.dashboard.*` 호출 전량을
        `_daily_close_ui_sig.request.emit(...)` 경유(`main.py:321`/`:641`/`:12034` — **304차 후속이
        만든 기존 통로 재사용**, 새 기전 금지).
      - 🔴 **`main.py:11239`의 `_poll_gbm_retrain_subprocess()` 호출을 되돌리지 않는다**
        (396차 `33e0e60`, 381차 20분 지연의 정당한 해결책).
      - `_gbm_retrain_done_event.set()`·CB⑤ 임계 복원·193차 모델 교체 예약은 **현행 위치 유지**
        (GUI가 아니며 옮기면 `daily_close` 대기 해제가 깨진다).
      - docstring "메인 스레드에서 실행" 갱신 — 더 이상 참이 아니다.
      - 신설: `tests/test_490_no_gui_from_worker_thread.py` (457차 정적검사 방식).
      - 검증: 워커 스레드 유닛 / 서브프로세스 완료 15:10 이후 지연 리허설
        (`daily_close_done_<date>.txt` · `_exit_normally` 생성 확인) / 정상일 무변화.
      - **절대원칙 6종 무변경 · 매매 정책 무변경.**

- [x] **F-M** 동결 감시를 프로세스 밖으로 — **섀도(알림 전용) 한정** (이상점 1-14) `[DONE 2026-08-24 491차]`
      `scripts/freeze_sentinel.py`(FZ-2) 신설 + 런처 사이드카(6-C) + `--gil` 리허설. **알림 전용 — 하드 종료
      없음**(승격은 주간회의 안건). 2026-08-24 실제 로그 재생 `--at-time 15:50` → CRITICAL rc=3 / `13:00` → OK
      rc=0.
      - 🔴 **FZ-1 제거 금지** — 「메인 루프 지연형」에는 유효하다. **2층 추가**다.
      - `scripts/force_flat_guard.py` 확장 또는 `scripts/freeze_sentinel.py` 신설.
        3신호 동시 정체로 판정: 하트비트 `written_at` / `crash_fault.log` 최신 `[TS]` /
        `{date}_SYSTEM.log` 최종 기록. 감시 창 **09:00~16:30**
        (현행 `FREEZE_WATCHDOG_WINDOW=("09:00","15:45")`는 마감 구간을 덮지 않는다).
      - 🔴 **하드 종료 권한 없음.** 승격은 **주간회의 안건**(F-2 브로커 청산 승격과 같은 성격).
      - 리허설에 「GIL 점유형 동결」 시나리오 추가. **오늘 `crash_fault.log` 87덤프를 재생 입력으로 사용.**

### P1

- [x] **F-N** `daily_close` 마커 2단화 (이상점 1-15) — ⚠ **F-L 적용 후에 붙일 것** `[DONE 2026-08-24 491차]`
      `daily_close_started_*.txt` 신설(진입 직후) + `done` 위치·의미 무변경 + EOD 2신호 AND 정체 판정 + 마커
      `daily_close_stalled: true|false|unmeasured`. ⚠ 파일은 `retrain_eod.py`(리포 루트) —
      `scripts/eod_retrain.py` 아니다.
      - `daily_close_started_{date}.txt` 신설(`main.py:13150` 진입 직후) +
        `daily_close_done_{date}.txt` **현행 위치 유지**(완료의 뜻 보존).
      - `scripts/retrain_eod.py` `[WaitDC]`: `started` 존재 + `SYSTEM.log` 정체 → 즉시 진행하고
        마커에 **`daily_close_stalled: true`** 기록. 과거 마커 부재 = **미측정**(계측 4원칙 ②).

- [ ] **CLAUDE.md 갱신 제안 (사용자 승인 필요)** — 실전 전환 기준 **② ⓑ** 문구를
      「라이브 미확인」 → 「현행 인프로세스 설계로는 이 유형에서 충족 불가」로 갱신하고,
      충족 경로를 **프로세스 밖 센티넬 왕복**으로 바꿀지 결정. 「주기적 재검증 항목 — 동결
      워치독 임계(480차)」의 재검증 축도 `STALL_SEC` 적절성 → **감시 위치**로 확장.
      🔴 **오늘은 제안만 기록했고 CLAUDE.md를 수정하지 않았다.**

### P2

- [ ] **고도화 ⑧** `daily_close()` 마지막에 **마감 절차 체크리스트 1행** 로그
      (`[DailyClose] 완료 12/12 | cb_reset=Y shs=Y broker_pnl=Y phase5=Y dbqueue=Y wal=6/6
      marker=Y exit_flag=Y`). 중간에 죽으면 줄이 없으므로 **"없음 = 미완"** 이 즉시 읽힌다.
      절차 순서·내용 무변경, **관측만 추가**.
- [ ] **고도화 ⑨** `collect_evidence.py` §7에 **앵커별 필수 문자열 있음/없음 표** 추가
      (`15:11 [SchedForceExit]` · `15:40 일일 마감` · `15:40 🏁 마감 완료` · `15:40 [WAL] 체크포인트` ·
      `15:45 [WaitDC] 완료 확인` · `16:0x 재학습 완료: 호라이즌 교체`).
      **1개라도 결측이면 자동 적신호**(사전등록 — 사후에 문턱을 낮추지 않는다, 458차 D6).
      Why: 오늘 SYSTEM 로그는 **정상처럼 보이는 줄로 끝났고**, 종료 「시각」 대조(-82분)만으로는
      08-18·08-21과 같은 15:40이라 정상으로도 읽힌다. 종료 「내용」을 봐야 잡힌다.
- [ ] **고도화 ⑩** 장중 재학습의 **완료 감지 지연**을 매일 1행 적립
      (`[GBM] 완료감지 지연 n=7 중앙값 60s 최대 1859s(15:09분)`).
      판정 문턱: 20거래일 누적 후 **지연 > 600초가 월 1회 이상**이면 F-N 우선순위를 올려
      주간회의 상정. ⚠ 문턱을 사후에 낮추지 않는다.
- [ ] **점검 인프라 정정** — `.claude/skills/mireuk-daily-check/SKILL.md` 의
      「이 PC = MW0602 / 브랜치 `dev` / 리포 루트 `C:\Users\pc1\...` / `--pc MW0602`」를
      **MW0601 / `v9-dev` / `C:\Users\82108\PycharmProjects\futures` / `--pc MW0601`** 로 정정.
      🔴 예약 실행이 문서를 그대로 따랐다면 산출물이 다른 PC 이름으로 저장될 뻔했다(함정 ③).
- [ ] **점검 인프라 정정** — `references/report_template.md` 가 `-pre`/`-intra`/`-post` 접미사
      방식을 말하고 있으면 **대원칙 B(하루 한 파일 · append)** 에 맞춘다.
- [ ] **F-B 범위 축소 가능** — 라이브 런타임이 `py37_32` 인 것은 오늘 `crash_fault.log` 덤프
      프레임 경로로 실증됐다. 남은 미확인은 **32비트 여부 하나**뿐이다.

### 다음 거래일(2026-08-25) 관측 항목

- [ ] **O-2′** (장후) 단기 CORE 2종의 **피처 구성 기여** — 오늘 15:40 SHAP 심사 미실행으로 이월.
      ⚠ 2026-06-17~07-10 SHAP 정렬 오염 구간 수치 **재인용 금지**.
- [ ] **O-10** (장후) EOD가 교체한 **30m 정확도 하락**(0.4158 → 0.3786, -3.7%p). 2거래일 연속
      하락이면 이상점 승격. ⚠ **30m 학습 중단은 선택지가 아니다** — CB③의 유일한 입력원(CLAUDE.md §3).
- [ ] **O-11** (장중·장후) **1-12 재발 조건** — `logs/retrain_intraday_<date>_15*.log` 존재 +
      `[GBM-64] subprocess 완료` 시각이 15:10 이후인가. 있으면 장중에 미리 경보.
- [ ] **O-12** (장전) **동결 프로세스 잔존** — 08:40 `[GUARD] 기존 main.py 프로세스 종료 완료`가
      찍히는가. 안 찍히면 이중 인스턴스 위험.
- [ ] **O-13** (장전) **굳은 모델 교체 예약** — `15:40:20 [Model] 재학습 완료 — 다음 파이프라인
      시작 전 모델 교체 예약`이 남아 있다. 재기동 없이 이어가면 EOD 새 pkl 위에 덮일 수 있다
      (193차 race 변형). `[RF] 로드 완료` 시각과 `model/horizons/gbm_*.pkl` mtime 대조.
- [ ] **N-10** (장후 누적) 15:10 강제청산 실집행 — 누적 **0회 유지**. F-1R 리허설로만 닫힌다.
- [ ] **1-2 / CB② 기한** — 실무 판정일 **08-28(금)**, 남은 거래일 4.
      오늘 실측 1일치: 연속 손절 카운터 **4회 전부 「1회」에서 끊김** ⇒ 한도 2~3 복원해도
      오늘은 정지 없음. 고도화 ④·⑤ 카운터팩추얼 표를 **08-27까지** 준비.

### 2026-08-24 491차(fix 9건 구현)가 남긴 것

- [ ] **CLAUDE.md 운영환경 표의 `joblib 1.1.1` 실측 대조 (P3 · 문서)** —
      F-B 가 배선한 `[Runtime]` 로그의 이 PC 실측이 **`joblib=1.1.0`** 이다
      (`scipy=1.5.4` · `sklearn=1.0.2` · `numpy=1.21.6` 은 표와 일치).
      크래시 위험 축(32비트 DLL 충돌 = scipy·비트수)이 아니라 기동 WARNING 대상에는
      **넣지 않았다** — 매 기동 로그에 값이 그대로 보이므로 은폐가 아니다.
      ⚠ 두 갈래다: (a) 표가 낡았다 → CLAUDE.md 정정 / (b) 이 PC 환경이 밀렸다 →
      `conda list joblib` 확인 후 맞춘다. **어느 쪽인지 확인 전에는 고치지 말 것**
      (461차 F-3·483차 P2-J 와 같은 「문서-코드 드리프트」인지, 진짜 환경 드리프트인지
      가르는 것이 먼저다).

- [ ] **FZ-2 하드 종료 승격 여부 (주간회의 안건)** —
      `FREEZE_SENTINEL_KILL_ENABLED` 는 자리표시이며 구현이 없다(켜져 있어도 스크립트가
      stderr 경고 후 알림 전용으로 간다). F-2 브로커 직접 청산 승격과 **같은 성격**이다 —
      안전장치가 새 사고를 만들 수 있다. ⚠ 특히 15:10 이후 하드 종료는 런처 재기동을
      유발하지 않으므로(오버나이트 금지 정책) 이득 없이 마감 절차만 더 확실히 죽인다.
      승격 판단은 **FZ-2 섀도 관찰 결과를 본 뒤** 한다.

- [ ] **FZ-2 라이브 첫 발화·오탐 관찰 (다음 거래일부터)** —
      `logs/freeze_sentinel_<date>.log` 에 `ARMED` 줄이 매 거래일 남는가(사이드카 배선
      확인), 정상일에 CRITICAL 이 뜨지 않는가(오탐), 그리고 `UNKNOWN` 이 상시로 뜨지
      않는가(3신호 중 하나가 구조적으로 없다는 뜻).
      ⚠ **`ARMED` 는 있는데 종료 줄이 없으면 센티넬이 장중에 죽은 것이다** — 감시자가
      스스로 「조용한 부재」에 빠지는 형태이므로 그 자체가 이상점이다.

- [ ] **F-L·F-N 라이브 왕복 확인 (다음 15:40)** —
      정상: `[GBM-64] subprocess 완료` 가 15:10 이후에 와도 `daily_close()` 가
      `[Notify] 🏁 미륵이 일일 마감 완료` 까지 도달하고
      `data/daily_close_started_*.txt` · `daily_close_done_*.txt` · `_exit_normally`
      **셋 다** 생긴다. `started` 만 있고 `done` 이 없으면 F-L 이 덜 잡은 GUI 접근이
      더 있다는 뜻이다(검출기 범위를 넓혀야 한다 — 그 테스트의 「한계」 절 참조).

- [ ] **F-I·F-K·F-G 첫 라이브 값 확인 (다음 거래일 장후)** —
      ① `trades.exit_stage` 가 NULL 이 아닌가(NULL 이면 `result` dict 경로가 안 탄 것)
      ② `ensemble_decisions.meta_size_fallback` 3상태가 다 나오는가
      ③ 15:40 `[CB③계측]` 1줄이 조건 성립 0분인 날에도 찍히는가
      (**찍혀야 한다** — 계측은 사건이 없을 때 「0분」이라고 말할 수 있어야 한다).

---

## MW0601 491차 후속 — 순수 라이브 전용 WFA 손익 조사 후속 (490차 후속2의 연속)

> 근거: `docs/정기점검/26주WFA_피처셋재검증_L1L2L3_MW0601-20260824.md` **§12**,
> `DECISION_LOG.md` 2026-08-24 491차 후속. 산출물:
> `docs/정기점검/26주WFA_MW0601-20260824/live_only.* · live_null.*`.
> **피처셋 무변경 판정 유지** — 라이브 전용에서도 채택 후보 0개(노이즈 하한선 미달).

- [ ] **A-6** L2(`horizon_signal_tradability.py`) 출력에 **노이즈 하한선 병기**.
      실피처 각각의 **위상 무작위 쌍둥이**를 같은 검정에 태워 "우연 합격 기대치"를 같이 찍는다.
      🔴 **근거**: 순수 라이브 31일에서 실피처가 9셀 합격했는데 **노이즈도 6셀 합격**했고,
      노이즈 최고치(`net 18.53pt · t=2.92`)를 넘는 실피처 합격 셀은 **0개**였다.
      455차 N3(`scripts/noise_benchmark.py`) 방법론 그대로 쓰고 **규모만 확대**하면 된다
      (참고 구현: `docs/정기점검/26주WFA_MW0601-20260824/live_null.py`).
      ⚠ **사전등록 합격선은 바꾸지 않는다**(§9-4). 하한선을 *함께 출력*하자는 것뿐이다 —
      `|t|>=2`에 다중비교 보정이 없다는 사실을 **보이게** 만드는 조치다.
- [ ] **A-7** L1·L2 출력 헤더에 관찰창의 **일중/오버나이트 드리프트** 1줄.
      **근거**: 07-14~08-24 28일이 일중 **−6.10pt/일** · 오버나이트 **+6.01pt/일**이었다.
      창의 방향 편향을 모르면 ±방향 손익을 오독한다(항상 SHORT가 +5.98pt/일로 보이지만 t=0.96).
- [ ] **B-6** 라이브 **SHORT 건당 손익이 LONG의 1/12**인 이유 조사.
      **근거**: 같은 28일 171포지션에서 LONG 48건 건당 **+15,763원** vs SHORT 123건 건당 **+1,271원**.
      시스템은 SHORT 편향(72%)인데 정작 SHORT 성과가 나쁘다 ⇒ **"하락 드리프트에 편승해서 벌었다"는
      가설이 지지되지 않는다.** 흑자(+912,938원)의 진짜 출처를 규명할 것.
      ⚠ 반드시 **포지션 단위**로 셀 것(계측 4원칙 ①, 417차 레그↔포지션 혼동 재발 금지).
- [ ] **B-7** `scripts/backfill_features.py:_BACKFILL_COMPUTED_KEYS` **26키 전체**의
      백필↔라이브 **동치성 일괄 검증**.
      🔴 **근거**: `vwap_momentum`이 전체창 L1에서 Bonferroni를 통과(|t|=12.3~14.4)했는데
      **라이브 전용에서는 어느 호라이즌도 통과하지 못한다**(h=30 |t|=3.3 < 6.05).
      즉 그 통과는 **백필이 만든 것**이었다. 490차 후속2 B-2는 이제 조사 안건이 아니라
      **확인된 사실**에 가깝다. ⚠ 같은 목록에 `vwap_position`·`cvd_delta_norm`·`atr`·
      `bb_position`·`poc_distance` 등 **현행 피처셋 다수**가 들어 있다 — 1건이 아니라
      **26키 전체가 같은 위험**이다.
      ⇒ B-2(단건)를 이 항목으로 **승격·흡수**한다.

### 490차 후속2 항목 중 이번 조사로 갱신된 것

- **A-1 (백필 제외 옵션)** — 우선순위 **상향**. "있으면 좋은 것"이 아니다:
  같은 데이터·같은 합격선에서 손익이 **3.2~4.8배** 달라졌다(전체창 −0.153 → 라이브 −0.736pt/일 @h=30).
  원인은 알파 감소가 아니라 **거래 4배 증가**(백필 행은 상수 0.0이라 신호가 발생하지 않아
  전체창 수치가 **무거래일에 희석**돼 있었다).
- **B-3 (`3_vwap` 섀도 계측)** — 순서가 옳았음이 재확인됐다. 라이브 전용 L2에서는
  **현행 배선 방향(+1)이 오히려 양수**(h=5 +4.44pt/일)이고 역방향(−1)이 −13.61pt/일이다
  (둘 다 t<1 미유의). 전체창에서는 +1도 음수였다 ⇒ **부호 반전 근거는 더 약해졌다.**
  L1의 음의 IC(라이브에서도 −0.129/−0.238/−0.335로 유지)와 손익 축이 **엇갈린다**는 것이
  현재까지의 정확한 상태다. 게이트는 건드리지 않는다.
- **B-2 (`vwap_momentum` 동치성)** — **B-7로 흡수**(단건 → 26키 전체).

---

## MW0601 491차 후속2 — 배터리 통계 취약점 3종 후속 (팻테일·동률·점질량)

> 근거: `docs/정기점검/26주WFA_피처셋재검증_L1L2L3_MW0601-20260824.md` **§13**,
> `DECISION_LOG.md` 2026-08-24 491차 후속2.
> 🔑 **한 줄 요약**: 거래수/일 vs net/일 순위상관 **−0.6384** — L2 합격선이 "알파가 큰 피처"가
> 아니라 **"거의 거래하지 않는 피처"** 를 뽑고 있다. ①과 ③이 같은 방향으로 결합한 결과다.
> **판정 무변경**(세 통제 모두 통과 0/9). 아래는 전부 **"함께 출력"**이지 문턱 변경이 아니다.

- [ ] **A-8** L2에 **잭나이프 t** 병기 + `drop_worst_days` 적용 옵션.
      🔴 **근거**: 최대영향 하루 1개만 빼면 합격 **9셀 중 5셀 탈락**(t 2.06→1.74 등).
      `macro_nasdaq_chg`는 총 79거래인데 **한 날이 전체 |손익|의 26.8%**다.
      🔑 `drop_worst_days`는 **이미 `VALIDATION_CAMPAIGN["spread_extreme_watch"]`에 있다** —
      새로 만들 필요 없이 **전파**만 하면 된다.
      ⚠ **합격선에 잭나이프 임계를 넣는 것은 별개 안건**이다(§9-4 사전등록 절차 필수).
      조사 결과를 보고 나서 문턱을 올리는 것도 **사후 변경**이다.
- [ ] **A-9** L1·L2에 **동률 탈락 건수·비율** 출력 (계측 4원칙 ③).
      **근거**: L2가 `값 == 러닝중앙값`이면 조용히 `continue` — `macro_nasdaq_chg` **99.3%**
      (10,758봉) · `cvd` **97.0%**(9,723봉) · `toxicity_queue_stress` **61.1%**가 사라지는데
      리포트 어디에도 없다. L1 `hit`의 중앙값 동률 마스크도 동일.
- [ ] **A-10** **층간 게이트** — L1 유효일 0 또는 `MIN_DAYS` 미만인 피처를 L2 합격자로 올리지 않는다
      (제외 또는 `⚠L1미평가` 태그).
      🔴 **근거**: `macro_nasdaq_chg`는 L1에서 **31일 전부 `unique<3` 탈락 → 유효일 0**인데
      **L2에서는 79거래로 합격**했다. 층간 정합성 검사가 없다.
- [ ] **A-11** **점질량·거래수 정규화 지표** 병기 — 피처별 최빈값 질량, 질량 제외 유효표본,
      `net/거래`, `gross/비용` 배수.
      🔴 **근거**: 질량 구간별 평균 거래수/일(h=5)이 0~10% **67.8** → 90%+ **4.7**로 단조 감소하고,
      거래수 vs net 상관이 **−0.6384**다. **일당 손익만 보면 저빈도 피처가 자동으로 유리하다.**
      최빈값 질량: `bull_exhaustion_shadow` 99.3% · `cvd` **90.0%**(합격) ·
      `macro_nasdaq_chg` 42.6%(합격) · `vwap_momentum` **32.4%** · `vwap_position` 21.1%.
- [ ] **B-8** rank 구현 **사본 8벌 → 정본 1벌 통합**
      (`_spearman` 5개 + `rankdata` 2개 + `_spearman_sign` 1개).
      **근거**: 456차 미해결 권고. 구 `argsort(argsort())` 서수 순위가 관계없는 데이터에
      **rho=+0.17**을 만들었고 456·460차가 **독립적으로 같은 버그를 발견**했다.
      이번 L1/L2 경로는 정본만 쓰지만 **타 채널은 미확인**이다.
- [ ] **B-9** **clip 경계 인공 점질량** 조사.
      🔑 **근거**: `vwap_position`의 최빈값이 **−2(= clip 하한)이고 질량 21.1%**다.
      즉 **①(팻테일) 방어인 clip이 ③(점질량)을 만든다** — 두 문제는 독립이 아니다.
      대상: `vwap_position` · `spread_ticks`(clip 0~20) · `mlofi_slope`(clip ±300).
      clip 폭을 데이터로 정하고 **경계 질량을 함께 계측**할 것.

### 참고 — 이번 점검에서 "이미 정상"으로 확인된 것 (재조사 불필요)

- **Rank IC 동률 처리 자체는 옳다.** `core_feature_discovery.rankdata()`는 평균순위(midrank),
  `corr()`는 한쪽이 상수면 `nan` 반환(**0 아님**), `analyze()`는 일별 `unique<3`이면 그날을 nan.
  456차·460차 수정분이 정상 동작 중 — **다시 파헤치지 말 것**. 남은 건 A-9·A-10·B-8뿐이다.
- **L1의 팻테일 방어도 옳다.** Spearman 순위 IC + 부호 기반 hit + 일별 유계 집계라
  28σ 급등(실측 max 57.1pt)이 t를 폭파시키지 못한다. 무방비인 것은 **L2뿐**이다.

### 신규 분석 스크립트 작성 시 상시 주의

- [ ] 새 분석 스크립트 스캐폴드에 **`utils.dll_bootstrap.ensure_conda_dll_path()` 기본 포함**.
      이번에도 `corr.py`가 `np.corrcoef` 첫 호출에서 **exit 127로 즉사**했다
      (448차/455차가 기록한 MKL 지연로드 0xC06D007F). **새 스크립트마다 재발한다.**

---

## 492차 (2026-08-25) — 피처 수명 분석 후속 · 주간회의 안건

> 이번 세션은 **읽기 전용 진단 + 판정 무영향 계측**만 했다. 아래는 전부 **승인 전 집행 없음**.
> 근거: `docs/정기점검/26주WFA_피처셋재검증_L1L2L3_MW0601-20260824.md` §14~§16,
> 규약: `docs/Spec for feature/피처_재검증_및_호라이즌배정_원칙.md`.

### 주간회의 안건 (신규)

- [ ] **A-12** 수명 기반 호라이즌 분리는 **추진하지 않는다** — §14 여섯 갈래 무의미 ·
      §15 대역 일치 4/25(p=0.83) · §16 배정 재현 7/25(p=0.69, 재현분 전부 h=30). 비용 0.
- [ ] **A-13** `vwap_position` **h=30 최강**(t=−9.12, 유일한 다중비교 생존)을 §7-1 부호
      안건과 **한 건으로 묶어** 심의. 현행 배정은 중기(15m)까지다. 비용 소.
- [ ] **A-14** `opt_pcr_extreme_signed` / `opt_pcr_norm` **완전 중복** 처분
      (11,304 관측 전부 동일). 비용 소.
...
- [ ] **A-18** L1 **노이즈 하한선을 통과 조건에 포함**할지 결정.
      ⚠ **기준 강화 = 사전등록 변경**이라 주간회의 전용. 실측 재료: h=5 하한 |t|=2.36에
      대해 **81개 중 69개(85%)가 미달**. 지금은 병기(▽ 표시)만 하고 판정과 무관하다.
...
## 493차 (2026-08-25, MW0602) — 장전 점검 후속

> 장전 국면이라 **코드 무변경**. 아래는 전부 **승인 전 집행 없음 · 15:10 이후 적용**.
> 근거: `docs/정기점검/매일점검/MW0602-20260825-점검리포트.md`.

### Fix (장후 적용)

- [ ] **F-1(493차)** `ofi_norm` 축퇴 **원인축 분리 계측** — P1 · 이상점 `1-1`.
      `features/technical/ofi.py` 에서 refit 창 원값 요약 4종(`n_nonzero`/`n_total`/
      `frac_zero`/`raw_range`) 산출 → `[ScalerRefresh]` 로그에 **기존 문구 뒤에 덧붙이기만**.
      🔴 **앞부분 한 글자도 바꾸지 말 것** — §12 파서가 정규식으로 읽어 문구 변경이 계측을
      조용히 끊는다(468차 G-2 경고 형태). 수집기 `stuck_indicators.patterns` 에
      `ofi_frac_zero` 추가(`measured_since` 병기). 판정 무영향. `py37_32` — walrus·`dict|` 금지.
      판정 기준: `frac_zero ≥ 0.90` 3일 연속 → 계산 결함 / `frac_zero < 0.5` 인데
      `raw_std≈0` → 실제 무변동. 커밋 `[MW0602] 493차: ofi_norm 축퇴 원인축 분리 계측`.
- [ ] **F-2(493차)** 런처 GUARD **판정 기준 사전등록** — P2 · 이상점 `1-4` (`O-3` 기준 부재 해소).
      `config/dailycheck_targets.json`(+ 스킬 사본)에 `launcher_guard` 지표 추가
      (원천 `logs/Mireuk_batch/launcher_*.log`, 값 `발화`/`무발화`,
      `measured_since: "2026-08-13"`). 사전등록 임계: **5거래일 연속 → P2 / 10거래일 연속 → P1**.
      🔴 임계는 관측값 역산이 아니라 달력 기준(1주/2주)이다 — 사후 완화 금지(313차 ④).
      ⚠ **`main.py` 종료 루틴은 건드리지 않는다** — 원인 규명 전이다.
- [ ] **F-3(493차)** 리포 루트 런타임 산출물 5종 → `data/` 이관 — P2 · 이상점 `1-5`.
      `calibration_metrics.json` · `effect_monitor_history.json` ·
      `meta_gate_tuning_metrics.json` · `microstructure_ab_metrics.json`(1.6MB) ·
      `rollout_readiness_metrics.json`. ⚠ **소비부를 함께 옮긴다** — 생산부만 옮기면 조용히
      `무기록`이 된다(invariants §0-C 거울상). 변경 전 `grep -rn "<파일명>"` 전수 확인 필수.
      우선순위 최하 — F-1·F-2 뒤.

### 고도화 (이번 주 — 2026-08-28 주간회의 전 표본 확보)

- [ ] **G-1(493차)** 파생 지표 **`CORE축퇴_개장해소`** 신설(값 `해소`/`미해소`/`무축퇴`).
      산출식 고정: *"당일 마지막 `trigger=A_WARMUP` 축퇴>0 이고 첫 `trigger=B_OPEN` 축퇴==0
      이면 `해소`"*. `measured_since: "2026-08-25"`.
      **왜**: 0824 `1-10`이 「장전 축퇴는 개장하면 풀린다」는 암묵 전제를 뒤집는 정정이었는데,
      **오늘 그 정정이 다시 필요했다**(0824 `B_OPEN` 0/6 vs 0825 1/6 — 사람이 두 날 로그를
      나란히 놓고서야 알았다). 474차·471차·1-9 가 전부 이 방식으로 뒤늦게 발견됐다.
      ⚠ `A_WARMUP`/`B_OPEN` 문자열이 바뀌면 조용히 죽는다 → **26주 WFA 「고착 지표
      감시목록(468차 G-2)」에 함께 등재할 것.** 판정 무영향(섀도).
- [ ] **G-2(493차)** `[Calibration]` 폴백 사유 **2축 분해** — `Calib_축퇴`(auc<0.53∧span<0.020)
      / `Calib_하한불가`(out_max<conf_floor)를 **병렬 신설**.
      ⚠ 기존 합산 지표를 **대체하지 않는다**(476차 G-2 `CORE축퇴_피처` 가 `CORE준비도`를
      대체하지 않고 병렬로 간 방식). 판정 무영향(섀도).
      🔴 **`conf_floor=0.3300` 값 변경은 포함하지 않는다** — 매매 정책 변경이라 주간회의 소관.
      지배 축이 `하한 도달불가` 면 **주간회의 안건 ①(앙상블 등급 임계 재설계)에 병합** 제안.

### 관측 (판정 예정)

- [ ] **O-16(493차)** `[CORE준비도] trigger=B_OPEN` 축퇴 개수 — 개장 직후 0/6 으로 풀리는가.
      오늘 **1/6 미해소**(전일 0/6 해소). **3거래일 중 2일 이상 미해소**면 `1-1` P1 → **P0
      승격 검토**(단기 CORE 상시 무력화). ⚠ 확정 결론 금지. 판정 **2026-08-27**.
- [ ] **O-17(493차)** 런처 `이미 실행 중인 main.py` 발화 연속일 — 현재 **8거래일 연속**.
      F-2 사전등록 임계 10 도달 시 P1 승격. 판정 **2026-08-27**.
- [ ] **O-18(493차)** `[Calibration]` 폴백 2축 비율 — 오늘은 합산 161건뿐이라 분해 불가.
      G-2 배포 다음 거래일 첫 값. 어느 축이 지배적인지가 `1-2` 처방을 가른다.

### 오늘(2026-08-25) 장중·장후에 반드시 닫을 것

- [ ] `O-12`(492차) `[DriftAdjuster]` `DRIFT_UP_SATURATED` 출현 — **F-7 배포 다음 거래일이
      오늘이다.** 장전 시점 `drift_action` 표본 0은 **정상**(조절기는 장중에 돈다).
- [ ] `O-30~O-32`(490차) `[2-A]` AUC 표 · `[2-B]` 재개 게이트 · 사이드카 `history` —
      원 등록대로 **오늘 장후**. **앞당기지 않는다.**
- [ ] `1-2`(오늘) 확률 보정기 폴백 — `O-30` 과 동시 판정. 오늘 26건 / 전일 30건.
- [ ] `O-17`(0821) `entry_horizon` 소비처 전수 — 원 등록 판정일이 **오늘 장후**다. 미착수.
- [ ] `O-2`·`O-6`·`O-10`·`O-11`(491차) · `O-13~O-15`(492차) · `O-23`(0821) — 전부 오늘 장후.

### 주간회의 2026-08-28 추가 안건

- [ ] **F-2 임계 5/10거래일 적정성** — 런처 GUARD 연속일 판정 기준.
- [ ] **`conf_floor=0.3300` 축을 안건 ①(앙상블 등급 임계 재설계)에 병합** — G-2 근거로.
      §12b 는 앙상블 A/B 임계가 **10일 연속 미도달**(max 0.5862)이라 보고했고, 보정기
      `out_max` 0.3126~0.3367 과 `conf_floor` 0.3300 의 겹침은 **같은 뿌리**다.

### 상시 주의 (이번 세션에서 확인한 환경 조건)

- [ ] **리눅스 샌드박스에서 평문 `git status` 를 근거로 쓰지 말 것** — CRLF 차이로 600파일이
      `M` 으로 뜬다. `git diff --stat -w --ignore-cr-at-eol` 이 공집합임을 실측 확인했다.
      수집기는 `git -c core.autocrlf=true status --porcelain` 을 쓴다(환경 중립). 정본은 그쪽이다.

---

## 494차 (2026-08-25, MW0602) — 장후 점검 후속 + 493차 장중 유보분 이관

> 리포트: `docs/정기점검/매일점검/MW0602-20260825-점검리포트.md` 제3부(장후).
> 🔴 **이 세션은 코드를 변경하지 않았다** — 아래는 전부 사용자 지시 대기.
> 🔴 **절대원칙 6종은 변경 대상이 아니다.** 커밋 첫 단어 `[MW0602]`.
> 🔴 재학습 코드를 건드리면 **`py310_64` 전용**(191차) — F-8이 해당.

### 🔴 착수 전 필수 (0순위)

- [ ] **`logs/crash_fault.log.3` 수동 백업** — 44.7MB · **다음 로테이션 대상** ·
      `Windows fatal exception: access violation` **5건**(2026-08-06 이전)이 그 안에만 있다.
      **F-4·F-10 착수 전에 반드시.** 구현 도중 로테이션이 돌면 원본이 사라진다.

### Fix (494차 신규)

- [ ] **F-8 (P1)** GuardFair 공정홀드아웃의 **측정 가능성 복구** — 대응 `1-11`
  - `learning/batch_retrainer.py:1090 _fair_validity()` 반환에 진단 3값 추가:
    `holdout_start_ts` · `train_end_ts` · `gap_bars`(= holdout_start 이후 현행 학습 침범 봉 수).
    🔴 **판정 로직 무변경**(여전히 False 반환).
  - `learning/batch_retrainer.py:1033 save_guard_shadow(...)` → `guard_shadow_log`에
    `fair_gap_bars INTEGER` 컬럼(`ALTER TABLE`, 기존 행 NULL — 464차 선례).
  - `.claude/skills/mireuk-daily-check/scripts/collect_evidence.py` §12에
    `guardfair_gap_bars` 지표(**매 EOD 무조건 1행**). `benign` 아님.
  - 🔴 **`config/settings.py:739 EOD_GUARD_FAIR_HOLDOUT["holdout_bars"]`(1850)는 이번 범위 밖.**
    값 선택은 **주간회의 2026-08-28 안건 ⑦**. 오늘 관측으로 고르면 313차 ④ 위반.
  - 소요 30분(회귀 포함 45분). 검증 **`O-29`(494차)**
- [ ] **F-9 (P2)** 크래시 서명 계수의 **분자 확정** — 대응 `1-12` · 장중 `G-4`의 **선행조건**
  - 분자 = `logs/crash_fault.log*`의 `Windows fatal exception:` 행 수
  - 보조 = 런처 `[AUTO-RESTART] main.py 재시작` · 🔴 **`일시적 크래시` 줄 제외**
    (정상 종료에도 찍힌다 — 그걸 분자로 쓰면 100% 고착이 구조적으로 보장)
  - 정상 종료 판별 = `[AUTO-RESTART] 정상 종료 감지 -- 이유: auto_shutdown`
  - 변경: `collect_evidence.py`(§12 정의) · `references/evidence_map.md`(「오탐 주의」 행)
  - 소요 10분. 검증: 내일 장전 §12에 `crash_signature = 0(무사고)`
- [ ] **F-10 (P2)** 크래시 서명 **별도 영속화** — 대응 `1-10` · **F-4와 한 커밋으로 묶는다**
  - 로테이션 직전에 `Windows fatal exception` 블록만 추출 →
    `logs/crash_signatures.log`(append 전용, 회전 없음). 시각·예외명·최상단 3프레임.
  - 변경: `main.py:16941` faulthandler 설정부 인근 · 로테이션 핸들러(구현 시 실측 확인)
  - 🔴 **절대원칙 ④** — 추출 로직을 COM 콜백에 넣지 말 것(파일 I/O만, `dynamicCall`·`emit` 없음)
  - 소요 25분. 검증: 다음 로테이션에 `.log.3`의 5건 보존

### Fix (493차 장중 이관 — 장후 적용 대상이었으나 오늘 미구현)

- [ ] **F-4 (P0)** 장중 크래시 **원인 축 계측** — 대응 `1-6`. **F-10과 한 커밋.** 소요 20분(회귀 30분)
- [ ] **F-5 (P1)** 재기동 시 **Hurst 워밍업 버퍼 당일 분봉 복원** — 대응 `1-7`.
      `_close_history`가 371차 `raw_features` 워밍업과 같은 처방을 못 받은 잔여.
      ⚠ `hurst_gate_shadow` 확정 결정(2026-08-01)과 **무관** — 그쪽은 임계, 이쪽은 버퍼. 소요 25분(회귀 40분)
- [ ] **F-6 (P2)** `[Engine] run_shadow` 경고 **WARN 채널 라우팅** + §12 등록 — 대응 `1-8`.
      종가 실측 **369건 · max 412.0ms**. 소요 15분
- [ ] **F-7 (P2)** 관측 ID **채번 규칙** + `O-16`~`O-18`(493차) **재부여** — 대응 `1-9`.
      ⚠ 규칙은 493차 장중 `6i`가 세웠으나 **기존 충돌 3건 재부여는 미실시**. 소요 15분

### Fix (493차 장전 이관)

- [ ] **F-1 (P1)** `ofi_norm` 축퇴의 **원인 축 구분 기록** — 대응 `1-1`.
      ⚠ **2026-08-27 전에 배포할 것** — `O-16` 3일차 판정이 그날이며, 그 전에 원인 구분이
      있어야 「P0 승격 검토」가 의미를 갖는다. 소요 15분(회귀 25분)
- [ ] **F-2 (P2)** 런처 GUARD 발화 **판정 기준 등록** — 대응 `1-4`. 소요 10분
- [ ] **F-3 (P2)** 리포 루트 런타임 산출물 5종 **출력 경로 `data/` 이전** — 대응 `1-5`

### 고도화 (494차 신규)

- [ ] **G-5** **「구조적 도달 불가」 지표군** — 수집기 §12b를 **§12d 도달 불가 레지스트리**로 확장.
      불변식을 매일 1행: GuardFair `train_end < holdout_start` / CORE 중기·장기 그룹
      `entry_horizon ∈ {10m,15m,30m}` / 앙상블 A `max(conf|dir≠0) ≥ 0.70` / 15:10 `[TimeExit]` 실집행.
      **오늘 4개 전부 False 또는 미발생.**
      **왜**: 471차 F-1·474차·476차·494차 `1-11`이 전부 같은 형태였고 **평균 3~6개월** 뒤에
      사람이 우연히 발견했다. 테스트는 *"누가 바꾸면 깨진다"* 를 잡지만 *"안 바뀌어서 계속
      죽어 있다"* 는 못 잡는다.
      **26주 WFA 편입 제안**: CLAUDE.md 「관측 계층 5종」(470차)의 **여섯 번째** 항목.
      ⚠ 섀도 관측 — 임계·정책 무변경.
- [ ] **G-6** **재기동을 「사건」이 아니라 「상태」로** — 매분 파이프라인 로그에
      `restart_age_min=N`을 **무조건 1값**(재기동 없으면 `-1`). 468차 G-1의 이벤트→상태 처방과 동일.
      **G-3(차단 로그에 `cold_start_window` 플래그)의 상위 개념** — G-3은 차단이 있을 때만
      보이고, G-6은 매분 보인다. `§5 규약(조건부 로그 금지)`을 따르는 쪽이 G-6.
      **효과**: `O-25`(콜드스타트 창 차단)의 **분모**가 생긴다. 지금은 분자만 있다.

### 고도화 (493차 이관)

- [ ] **G-1** 「개장 시점 CORE 축퇴 해소」를 **개장 전/후 대비 축**으로 계측 (장전)
- [ ] **G-2** `[Calibration]` 폴백 사유 **2축 분해 매일 집계** (장전)
      ℹ️ **목적의 절반은 오늘 달성됐다** — 종가 로그 전량 집계로 `축퇴 감지` **253** :
      `하한 도달불가` **90** ≒ **2.8:1** 분해 완료(`O-18` 첫 실측). 잔여 목적은 **매일 자동 집계**.
- [ ] **G-3** 「재기동 직후 콜드스타트 창」을 1급 관측 대상으로 (장중) — **G-6과 함께 설계**
- [ ] **G-4** 크래시 서명 **일일 하트비트** (장중) — 🔴 **F-9 선행 필수**(분자 오탐)

### 관측 (494차 신규 · 판정 예정)

- [ ] **O-27(494차)** — 내일 09:00 `session_state.json:auto_shutdown_done_date` 값.
      `'2026-08-25'` 유지 vs 빈 값. **빈 값이면** 기동 경로가 지운다는 뜻이고
      `1-4` 영향(기술) 서술을 정정해야 한다. → **2026-08-26 장전**
- [ ] **O-28(494차)** — `[OnlineLearner] 30m SGD UP붕괴 자동 복구` 일별 건수.
      08-24 **12** · 08-25 **12**. 3거래일 연속 ±2 이내면 §12 `benign` 등록. → **2026-08-27**
- [ ] **O-29(494차)** — `[GuardFair] … gap_bars=N`(F-8 산출). 배포 후 첫 EOD **6/6 비-NULL**.
      2주(10 EOD) 누적 분포를 주간회의 안건 ⑦ 입력으로.
      🔴 **이 값으로 `holdout_bars`를 즉석에서 고르지 말 것**(313차 ④). → **F-8 배포 다음 EOD / 2026-09-08**

### ✅ 오늘(2026-08-25 장후) 닫은 것

- [x] **`O-12`(492차)** ✅ 종결 — 492차 F-7 라이브 검증 완료.
      `DRIFT_UP_SATURATED` · `n=22` · `src=online_learner.recent_accuracy` **3요소 전부 출현**.
      판정: `sgd_alpha` **양방향 손잡이 상실 확정**(상향 무연산 · 하향 `RECOVERY_THRESHOLD=0.58`
      vs acc 36.3% 도달 불가). 🔴 **benign 아님** → 주간회의 안건 ⑤
- [x] **`O-32`(490차)** ✅ 종결 — **PASS**. `ensemble_decisions.meta_entry_quality_prob`
      NULL **0건 / 369행**, 6거래일 연속 0. `MetaGate.evaluate()` 경유 라이브 경로 검증됨
- [x] **`O-18`(493차)** ✅ 첫 실측 — `축퇴 감지` 253 : `하한 도달불가` 90. G-2 없이 분해
- [x] **`O-2`·`O-6`·`O-10`·`O-11`·`O-23`(491·0821차)** 종가 확정 —
      `O-6` max(n)=**7**(요구 15, 미도달) · `O-10` 축퇴율 **26/42=61.9%**(전일 80.6%, 개선) ·
      `O-11` **6/6 LONG**(7일 중 5일이 단일 방향 — 표본 4~24, **확정 금지**) ·
      `O-23` `sizing_trace` **12행 = `[Sizer]` 12건 일치** ✅
- [x] **장전 「확인 필요」 4건 전량 처분** — `GuardFair_유효`→`1-11` 판정 / `sgd_alpha`→고착 확정 /
      `drift_action`→해소 / 보정기 `fitted=False`→`3p-정정1` 병합
- [x] **`1-11` 규명** — GuardFair `fair_valid=1` **전 기간 0행**, 구조적 영구 무효

### 🔄 이월 (판정일 미도래)

- [ ] `O-16`(493차) `B_OPEN` 축퇴 — **2/3일차 미해소**(오늘 3표본 전부 1/6) → **2026-08-27**
- [ ] `O-17`(493차) 런처 GUARD 연속일 — 8거래일 유지 → **2026-08-27**
- [ ] `O-13`(492차) DB pruning 재현 — **1/3일차**(오늘 0건) → **2026-08-27**
- [ ] `O-14`(492차) `[Bias⚠] 5m` UP 건수 — 오늘 `UP=0` 21 / `UP=1` 30 / `UP=2` 1 / `UP=3` 5.
      **전일의 「UP=0 고정」 미재현**. 편향 축 DN→**FL**(77%) 이동. 표본 30 — **확정 금지**
- [ ] `O-15`(492차) TP1 코호트 — 오늘 **미도달 포지션 0건**이라 대비 불가. 누적
- [ ] `O-24`(493차) `[AUTO-RESTART]` 재발 — 1일차 재발 0 → **2026-09-08**
- [ ] `O-25`(493차) 콜드스타트 창 차단 — F-5 대기 → 다음 재기동일
- [ ] `O-26`(493차) `run_shadow` 지연 — 참고치 max **412.0ms**(승격선 1,000ms의 41.2%) → F-6 후 3거래일
- [ ] `O-4`(0821) `[Capital]` 괴리율 — **−40.5%**(장전 −41.3% → 장중 −41.2%). 관측 계속
- [ ] `O-17`(0821) `entry_horizon` 소비처 전수 — **미착수** → **2026-08-28(금)**. ⚠ 라벨 충돌(F-7)
- [ ] `O-30`·`O-31`(490차) `[2-A]`·`[2-B]`·사이드카 — ⬅️ **금요일 이월(등록일 오류 정정)**.
      캠페인 리포트는 **금요일 전용 체인**(`phases.md` C-5 · 476차)인데 원 등록이
      *"O-30 (08-25 EOD)"* 로 그 제약을 반영하지 않았다 → **2026-08-28(금)**
- [ ] `O-20`·`O-21`(0821) · `O-26~O-29`(489차) — **2026-08-28(금)** 전용 체인

### 확인 필요 (장후 등록 · 확정 아님)

- [ ] **보정기 학습표본 오염이 늘었고, 오늘 처음 「오염이 축퇴를 만드는」 방향이 나왔다**
      `[Calibration][CleanShadow]` 종가: `오염행=0` 369건 · **오염행>0 37건**, 그중
      `live≠clean` **36건**(전일 15건/13건). 방향 — 35건은 `live=False clean=True`
      (**오염이 축퇴를 가린다**), 마지막 1건 `15:06:00 오염행=117건 live=True clean=False`
      (**오염이 축퇴를 만든다** — 첫 관측).
      **무엇을 더 보면 닫히는가**: 채널 **[46] 보정기 학습표본 오염 게이지**(461차 P-D) 주간 집계.
      **언제**: **2026-08-28(금)**. 🔴 **확정 결론 금지**(313차 — 방향 전환 관측 1건).
      ⚠ **[46]은 게이지**이며 조치 트리거가 아니다. **[45]**(뒤집힘 횟수)와 혼동 금지
- [ ] **실전 전환 기준 ①의 부호가 뒤집혔다 — 「충족」으로 인용 금지**
      20거래일(07-28~08-25) 184포지션 통산 **+455,484원**(489차 D6: −178,004원) ·
      양수일 **10/20** · 표준편차 **425,214원 = 일평균의 18.7배**(④ **여전히 미충족**) ·
      **상위 2일 제외 −1,371,287원** · 상하위 2일씩 제외 −186,098원.
      **부호는 바뀌었으나 성격은 그대로** — 소수 대형 승리일 의존.
      **무엇을 더 보면 닫히는가**: 상하위 제외 후 통산 부호 + 표준편차/평균 비율 추세.
      **언제**: **2026-08-28(금) 주간회의**
- [ ] **재기동 원인이 `1-4`(런처 GUARD)와 같은 뿌리인가** — ① `auto_shutdown_done_date`
      **채워짐 ✅** ② 내일 GUARD 발화 여부 **미도래** ③ 새 세대 `crash_fault.log`
      `access violation` **0건 ✅**. **인과 미확정 — 확정 금지**(313차) → `O-27`
- [ ] `O-28` 30m SGD UP붕괴 복구 12건(2일 동수) — 위 관측 참조

### 주간회의 2026-08-28 안건 (494차 추가분)

- [ ] **⑦ 🆕 GuardFair `holdout_bars` 처분** — 대응 `1-11`.
      선택지 (A) `EOD_GUARD_FAIR_HOLDOUT.holdout_bars`를 줄여 공정비교를 성립시킨다
      (B) [23] 채널의 `fair_valid` 필터를 폐기하고 다른 축으로 간다.
      🔴 **양자택일이지 「둘 다 미룸」은 선택지가 아니다** — 지금은 **판정이 살아 있는 것처럼
      보이는 죽은 채널**이다(입력이 영구 공집합인데 `✅ PASS` 렌더).
      입력: F-8의 `gap_bars` 분포(2주). ⚠ **관측값 역산 금지**(313차 ④)
- [ ] **⑧ 🆕 절대원칙 ④ 적용 범위** — 493차 장중 제안 + 494차 `1-10` 보강
      (같은 서명이 08-06 이전 세대에 **5건**). *"COM `BlockRequest`가 GUI 렌더링 스레드와
      교차하는 경로"* 까지 포함할지. 🔴 **절대원칙 문언 변경이므로 주간회의 소관**
- [ ] **⑤ 보강** — 「상수 고착」 묶음에 `sgd_alpha` **양방향 손잡이 상실 확정** 추가
- [ ] **① 입력 갱신** — 앙상블 등급 임계 재설계: 오늘 `direction≠0` **max conf 0.5289** ·
      `≥0.60` 0건 · `≥0.70` 0건 · **11일 연속 미도달** · `ensemble_decisions.grade` C 46 / X 323 / **A 0**
- [ ] **실전 전환 기준 ①·④ 문구 갱신 제안** — 판정(「미충족」)은 **유지**하되 489차 D6의
      근거 수치가 낡았다(부호 반전). 낡은 수치를 두면 다음 세션이 혼란한다

### 상시 주의 (494차 확인)

- **캠페인 리포트 미생성은 금요일 아닌 날에 정상이다**(`phases.md` C-5 · 476차).
  적신호로 올리지 말 것. 최신본 `validation_campaign_report_20260821.md`
- **`data/_exit_normally` 부재는 정상** — `start_mireuk.bat:556-557`이 읽은 직후 삭제한다.
  로그의 `[Shutdown] 정상 종료 플래그 기록`과 교차확인할 것
- **`crash_fault.log`의 `Timeout (0:00:30)!`은 행(hang)이 아니다** —
  `dump_traceback_later(30, repeat=True)`의 무조건 주기 덤프
- **`A급(원시C)`는 앙상블 A가 아니다** — 체크리스트 상향분이며 `trades.raw_grade`는 `C`다.
  앙상블 `'A'`는 476차 이래 **0건**

### 🔴 494차 커밋 미완 (사용자 조치 필요)

- [ ] **`.git/HEAD.lock` 삭제 후 커밋** — 대응 이상점 `1-13`(494차 신규 P2)
  - 증상: 3파일 **스테이징 완료**(`git diff --cached --stat` → 1,933 insertions)했으나
    `fatal: cannot lock ref 'HEAD': Unable to create '.git/HEAD.lock': File exists`
  - `.git/HEAD.lock` = **0바이트 · 2026-08-25 09:14** = 오늘 장전 세션이 `51dbb8c`를
    커밋한 직후 남긴 잔존물. **커밋 자체는 성공**했고 잠금만 정리되지 않았다.
  - 🔴 **예약 점검(리눅스 샌드박스)은 윈도우 쪽 `.git/` 내부 파일을 `unlink` 하지 못한다**
    (`Operation not permitted` — `git add` 중 `tmp_obj_*` 정리에서도 같은 경고 9건).
    **샌드박스에서 구조적으로 해제 불가**다.
  - ⚠ 재시도 과정에서 **`index.lock`도 함께 남았다** — **잠금 2개를 모두 삭제**할 것
  - 조치: `.git/HEAD.lock` · `.git/index.lock` 삭제 후
    `git add -A dev_memory docs/정기점검/매일점검` →
    `git commit -m "[MW0602] 494차: 장후 점검 — GuardFair 구조적 영구 무효 규명 + access violation 전례 5건"`
  - ⚠ **스테이징 스냅샷은 `1-13` 추가 이전 판본**이다 — 반드시 `git add -A`를 먼저 다시 돌릴 것
  - ⚠ **파일 내용 유실 없음** — 리포트·`DECISION_LOG.md`·`NEXT_TODO.md` 전부 디스크에 저장됨
  - ⚠ 반복되면 **매일 점검이 커밋 없이 끝난다** — 멀티PC 이력 연속성(`[MW0602]` 태그)이 끊긴다.
    재발 시 장전 세션의 커밋 경로(잠금 정리)를 점검할 것
