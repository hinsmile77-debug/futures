...
### [V22] opt50008 필드명 KOA Studio 확인
- **내용**: opt50008(투자자별매도수금액요청)가 `콜순매수`/`풋순매수` 필드를 실제로 제공하는지 확인
- **방법**: KOA Studio에서 opt50008 TR 응답 필드명 직접 확인. DATA.log `[TR-DISCOVER]` 항목도 참조
- **기준**: rows > 0 + 콜/풋 순매수 값 정상 수신 확인
- **fallback**: 필드 없으면 옵션 수급 수집 방식 재검토 필요

### [V23] 프로그램매매 실시간 FID 캡처 (장중)
- **내용**: code=`P00101` type=`프로그램매매` FID 스캔 결과 — 차익/비차익 순매수 FID 번호 확인
- **방법**: 다음 장중 PROBE.log에서 `[PROBE-ALLRT-FIDS] type='프로그램매매'` 항목 확인
- **기준**: 차익순매수·비차익순매수 FID 번호 특정 → opt10060 TR 폴링 대체 가능 여부 판단

### [V24] 투자자ticker 실서버 지원 확인
- **내용**: 실서버 전환 후 `투자자ticker` 실시간 타입 동작 여부 확인
- **방법**: 실서버 연결 후 PROBE.log에서 `[PROBE-TICKER]` 또는 `[PROBE-ALLRT] type='투자자ticker'` 수신 확인
- **배경**: 모의투자 서버에서는 8가지 코드 조합 전부 ret=0이나 데이터 없음 — 실서버 전용으로 추정
