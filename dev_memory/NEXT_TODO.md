...
- [ ] **dev 이식 보류 — 기계적 체리픽으로는 런타임에 죽는다(2026-09-07 실측)**
      dev 는 같은 5건을 `91fb5a9`(MW0602 538차)로 **독립 구현**했고 API 가 다르다:
      `rhat_trace` ↔ **`rhat_diag`** · `structure_diagnostics`/`structure_note`
      **부재** · 대시보드 `_fmt(items)` 는 diag 를 안 받는 534차 원형 ·
      `prepare_params(…, db_path, excluded)` (freshness 경고 추가분).
      실제로 `a21e270` 체리픽을 떠 보면 **자동병합 4파일 + 충돌 3곳**인데, 자동병합된
      쪽이 dev 에 없는 함수를 부른다 ⇒ **빌드는 되고 런타임에 NameError** —
      오늘 고친 GP 교차 결함과 같은 유형이다. 그래서 중단했다.
      가져가려면 **이식이 아니라 적응**이다(dev API 에 맞춰 재작성 + dev 쪽 테스트).
      ⚠ 그러면 같은 기능의 **변종이 두 갈래에 생긴다** — 489차 B-3(채널 번호는 갈래마다
      독립) 과 같은 상황이므로, 가져갈지 자체를 먼저 정할 것.
      · dev 에 이미 반영된 것: `76d1a9b` GP 교차 NameError 수정(542차 곁가지)뿐이다.
