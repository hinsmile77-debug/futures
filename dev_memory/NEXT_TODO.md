...
**DailyConsolidator(zone_penalty) 개선 — 세션 초반 딥다이브, 미착수**
(`learning/self_learning/daily_consolidator.py`, 07-10 로그의 zone별 정확도
극심한 변동 딥다이브에서 도출. P0/P1/P2는 우선순위, 번호는 서로 독립):

- [ ] **[P0] 좀비 패널티 차단** — 당일 표본(`MIN_SAMPLES_PER_ZONE=5`) 미달 zone은
  `_history` 갱신이 스킵되는데, 패널티 판정(`consolidate()`)은 갱신 여부와 무관하게
  과거 이력 전체를 재판정해 표본 미달 zone도 계속 패널티 유지("좀비 패널티").
  EXIT_ONLY(하루 1건)·GAP_OPEN은 애초 집계 대상에서 제외하거나, 표본 미달 날은
  history에 갱신 표식을 남겨 "최근 2일 연속" 판정이 낡은 이력으로 성립하지 않게 수정.
- [ ] **[P0] 패널티 기준선 교정** — `PENALTY_THRESHOLD=0.45`는 2클래스(랜덤 50%) 발상인데
  실제 채점은 3클래스(UP/FLAT/DOWN, 랜덤 33%)라 스킬 0인 모델이 상시 패널티를 받는
  구조. 3-클래스 기준(33%)으로 재설정하거나, CB③처럼 방향예측만(direction≠0) 집계해
  사실상 2-클래스 지표로 만든 뒤 45~50% 기준 유지 — 지표와 임계가 다른 게임을 하는
  현재 상태 해소.
- [ ] **[P1] 일단위 → 풀링 판정 전환** — "하루 acc<45% 2일 연속"(n=12/일) 대신 최근
  3~5일 표본을 합산(n=40~85)한 뒤 Wilson score 하한이 기준선 미만일 때만 패널티.
  n=12의 단일 하루 acc는 통계적으로 패널티 판정 근거가 되기엔 노이즈가 너무 큼.
  (참고: Wilson 하한 개념은 `docs/Ref/Wilson 하한.txt` 참조)
- [ ] **[P1] 표본 확충** — 배포 정책과 무관하게 shadow 기록 방식으로 단기그룹(1m·3m·5m)
  검증 결과를 합산하면 zone당 하루 60~100건으로 노이즈가 1/3 수준으로 감소. 1m은
  이미 매분 370건 저장 중이라 코드 변경 최소.
- [ ] **[P2] 지표를 acc 대신 기대손익(realized_move) 기반으로 전환** — 현재는 "5m 방향
  3-클래스 적중률"인데 시스템이 실제로 버는 건 진입~청산 손익. 260704 감사 지적과
  같은 맥락으로 zone 패널티 근거를 `meta_labels.realized_move` 기반 기대손익(그
  zone에서 진입했다면 벌었을 금액)으로 바꾸는 게 장기적으로 더 정직한 지표.
- [ ] **[P2] Consolidator 대표 호라이즌 5m → 3m(또는 3m+5m) 교체 검토** — conf-층화
  분석 결과 5m은 방향 지표로 부적격(무스킬 확정), 3m이 유일하게 방향 편향 전 구간
  일관 양(+)·일별 안정성 최고·표본도 66% 많음(하루 123건 vs 5m 74건). 10m 고신뢰
  FLAT은 방향 지표가 아닌 진입회피 필터로만 가치. 단, 근본적으로 현재 상태에서는
  "zone별 우열"을 논할 스킬을 가진 호라이즌 자체가 없어 아래 항목(고신뢰 역보정
  근본원인 규명)이 선행 과제.
