...
## 2026-07-19 (358차, MW0601 세션 — 357차 결론 MW0601 독립 재현 + [2]/[3] 재학습·체인 최초 소급)

> 상세: `DECISION_LOG.md` 2026-07-19(358차) 항목 +
> `docs/정기점검/금요일점검/0719_주간회의_검토보고_MW0601.md`.

- [DONE 2026-07-19] MW0601 [2]/[3] 스코어러 재학습 + 주간 체인(ablation·TB·분위·
  메타라벨) 최초 소급 실행 + py37_32 크로스 로드 ALL_OK 검증.
- [ ] **다음 장중(7/20) 최우선** — `ensemble_decisions` 5개 컬럼 non-null 적재
  시작 확인(357차와 동일 쿼리, MW0601 DB 기준).
- [ ] **[MW0601 최대 리스크] 7/20 = 341~355차·340차 첫 라이브** — git reflog 실측
  7/17 00:29 pull, 7/16까지 전 거래 구코드. 특히 340차(사이징 5천만/3% 상향)와
  341·348차(틱스톱/유령하드스톱 수정)가 동시 첫 라이브 — W1 대형손실(-654만/3건)
  재발 여부 최우선 관찰. **조건부 롤백 기준 제안**(하드스톱 슬리피지 >스톱폭+2pt
  재발 시 사이징 임시 롤백) 사용자 승인 대기.
- [ ] **7/24 재판정**: A등급 exit_reason 재집계(357차 P1과 병합), MW0601 표본 기아
  사다리 2단계 여부(현재 보류 권고 — 340·353차 자연회복 관찰 우선).
- [ ] 342차 kelly_advised_skip 컬럼·355차 open_gap_shadow 테이블 — MW0601도
  342차 이후 재기동 없어 미생성 실측. 7/20 기동 시 생성 확인(357차와 공통 항목).
...
- [DONE 2026-07-19] **MW0601(v9-dev) — 커밋 14b7ca9 cherry-pick 완료** —
  `learning/batch_retrainer.py` 스케일러 저장경로 격리 버그(346차 통합테스트가
  라이브 `model/scaler/*.pkl`을 10피처 합성테스트 스케일러로 덮어쓴 사고의 원인)
  + `main.py` WarmupRetrain 자기스킵 버그 수정 반영됨.
- [DONE 2026-07-19] **MW0601(v9-dev) — `model/scaler/*.pkl` 6개 손상 여부 직접 확인
  완료 — 손상 없음, 단 105가 아니라 97피처** — 6개 전부 `n_features_in_=97`로
  서로 일치, `model/horizons/feature_names.pkl`도 동일하게 97개(대조군 일치,
  미스매치 없음) — 346차 통합테스트의 10피처 오염 패턴과 무관. mtime도
  2026-07-18 10:30로 손상 창(07-16 17:29)과 무관. 105 vs 97 차이는 손상이 아니라
  이 브랜치(v9-dev)가 MW0602의 모든 피처 추가 커밋을 아직 cherry-pick하지 않아
  활성 피처 수 자체가 다른 것으로 추정(둘 다 내부적으로 일관돼 있어 즉시 조치
  불필요) — 두 브랜치 피처 개수 차이 자체를 추적할 필요가 있으면 별도 항목으로
  등록할 것.
- [ ] **MW0601 — 손상 확인 시 복구(참고용, 이번엔 불필요 — 손상 없음 확정)** —
  다음 중 하나:
...
- [ ] **다음 금요일(2026-07-24) 주간 체인 자동실행 확인** — 333차 후속2~3 배선의 첫
  ...
  `[검증 캠페인] 요약:` 로그 확인.
- [DONE 2026-07-19] **MW0601에서 [2]/[3] 스코어러 재학습** — 358차(MW0601 세션)에서
  완료. MW0601은 MW0602와 달리 "버전 불일치"가 아니라 **모델 디렉토리 자체가 없던
  상태**(학습이 한 번도 실행된 적 없음, `ensemble_decisions` 16,976행 non-null 0건
  실측). `py310_64` 전체경로로 주간 체인 순서 그대로 수동 소급(ablation → 캠페인
  리포트 → 섀도우 TB → 분위회귀 → 메타라벨) + py37_32 크로스 로드 스모크 통과.
  상세: `docs/정기점검/금요일점검/0719_주간회의_검토보고_MW0601.md`. 잔여: 7/20
  장중 5개 컬럼 적재 확인(위 357차 항목과 동일).
...
> 없어 `self._fp_last_logged_level` 초기화가 빠져있던 것을 cherry-pick 시 함께 보강.
...
## 2026-07-15 (dev 브랜치 333차 후속4, cherry-pick — 캠페인 체인 중복 제거: scripts/campaign_steps.py 분리)
...
## 2026-07-15 (333차 후속3 — MW0601 섀도우 TB 재학습 완료 + §4-1 자동화 미배선 [주간회의 안건])

> 상세: `DECISION_LOG.md` 2026-07-15(333차 후속3) 항목.

- [DONE 2026-07-15] **MW0601 섀도우 TB 최초 재학습** — 이전엔 `pc1`(추정 MW0602)
  머신 얘기로 착각했으나 이 세션 자체가 MW0601(호스트명 확인)이었고 여기엔 섀도우
  TB가 한 번도 없었음. `py310_64` 전체경로로 6/6 호라이즌 재학습 성공(champion 대비
  전부 우위), `generate_validation_campaign_report.py` [1]이 "디렉토리 없음"→
  "OOS 표본 0건"으로 정상 전환 확인.
- [ ] **[주간회의 안건] §4-1 금요일 자동화 체인이 MW0601에 배선 안 됨** — 실제
  Windows 작업 스케줄러(`MireukiEODRetrain`, 매일 15:45)는 프로젝트 루트
  `retrain_eod.py`를 호출하는데, 이 스크립트엔 `_run_campaign_steps()`(섀도우 TB
  재학습·게이트 ablation·분위회귀 재학습 등, `scripts/eod_retrain.py`에만 존재)가
  없음. `logs/`에 `[검증 캠페인]` 로그 0건 확인 — 이 라이브 머신에서 §4-1 자동화가
  자동으로는 단 한 번도 실행된 적 없었음. 3안 중 결정 필요: ①`_run_campaign_steps()`
  를 스케줄된 루트 `retrain_eod.py`에 병합 ②Task Scheduler에 금요일 전용 항목
...
## 2026-07-15 (dev 브랜치 333차 후속3, cherry-pick — 검증캠페인 금요일 자동실행, 휴장 시 그 주 마지막 거래일로 이월)
...
- [DONE 2026-07-12] **[P0] trades.db 유령 트레이드 오염 정리** — 07-09 Chejan echo 루프(306차에서
  ...
  → [DONE 2026-07-15, 333차 후속2] `scripts/generate_validation_campaign_report.py`
  가 이 필터 컨벤션을 상속 못 받아 [0]/[5]/[6]/[7] 집계가 계속 오염되고 있던 것
  추가 발견·수정(`_NOT_GHOST_SQL` 적용). 상세: `DECISION_LOG.md` 동일 날짜 항목.
