...
- [ ] **A1 장전 예약 08:50 → 09:02** — 0814에 08:50 실행이라 08:55 매크로·PreRetrain·
      **O-1의 F-1 해제 로그**(`08:55:02 [EntryGate] ×0.6 → ×1.0`)가 다이제스트에 하나도
      안 잡혀 사람이 원본 로그로 메웠다. `phases.md` A-2(08:55 매크로)는 장전 점검 필수
      항목인데 구조적으로 관측 불가였다.
      09:02면 08:58 레짐 확정 + 09:00 첫 파이프라인 1사이클까지 포함되고,
      *첫 봉이 실제로 돌았는가*까지 볼 수 있어 오히려 강해진다
- [ ] **A2 장후 예약을 EOD 완료 발화로** (차선: **16:30** 고정) — 0814 장후 점검이 15:51에
      돌았는데 EOD 체인은 16:02:32에 끝나 캠페인 판정 리포트(165KB)를 1차가 못 열었고,
      [44] 판정 조건 도달을 늦게 발견했으며 점검이 1·2차로 쪼개졌다.
      정공법: `data/eod_retrain_done_{d}.txt` + 캠페인 요약 라인 폴링(17:00 타임아웃).
      ⚠ **16:10이 아니라 16:30을 권한다** — 0814 EOD 총 소요 17분 30초(15:45→16:02),
      인계문서 §5도 16:30 권장. 16:10은 최소선이라 재학습이 조금만 길어지면 또 놓친다
