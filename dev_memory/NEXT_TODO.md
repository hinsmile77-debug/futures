...
- [ ] **🔴 최우선: `quantile_tp_shadow`[3-B]를 `exit_replay`로 이관.**
      그 자체 시뮬은 **TP1에서 종료**하며 TP1 목표가를 확정이익으로 계상한다 —
      440차가 [23-B]에서 "TP1을 넓히면 장부가 좋아지는 순수 인공물"로 지목한 구조
      그대로다(v1 `tp1_x2` 최선 +37.96 → v2 최악 −30.68). **[3-B]의 질문 자체가
      "TP1 거리를 무엇으로 정하나"라 인공물이 답을 직접 오염시킨다.**
      ⚠ **`q_raw` 우위(+20.22pt)를 이관 전 수치로 인용하지 말 것** — 0801 결산이
      "유일한 승격 후보"로 올린 그 근거가 여기 걸려 있다.
- [ ] **`tp1_protect_offset_shadow`[25]/[25-B] 이관** — 자체 시뮬이 **TP2에서 종료**.
      [25-B]는 축이 **TP2 거리** 자체라 직격 대상이다.
