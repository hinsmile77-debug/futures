...
- [ ] **D7** — 선로드 워커가 메인 파이프라인을 막는 문제. 실측: 선로드 분 S0
      1,714~1,904ms + **S1·S4·S5·S6 동시 3~8배 상승**(자원 경합 패턴).
      권고 순서 **②먼저**: 선로드 시각을 분 경계 밖(예: 매분 :30초)으로 이동 —
      싸고 되돌리기 쉽다. 실패 시 ① 별도 프로세스(GIL 회피).
      함께 고칠 것: `(S0 비블로킹)` 로그가 사실과 다르다(계측 4원칙 ④).
