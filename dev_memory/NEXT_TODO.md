...
- [ ] **06-29~30 SHAP 이름↔값 정렬 어긋남 검증** — 그 이틀 1m SHAP이 121개 이름 전부에
      값을 남겼는데 0이 아닌 14개가 **알파벳 순 첫 14개**와 정확히 일치한다(모델은
      14피처). 길이 14 중요도 벡터를 길이 121 이름 목록에 zip한 정황 — 사실이면 그
      기간 SHAP 기록의 이름-값 대응이 전부 틀렸다. 07-27은 8이름/8값으로 일치해 현재
      경로엔 증상이 없다. **SHAP writer 코드를 아직 읽지 않은 정황 증거**이므로
      확정 전 코드 확인 필요. 과거 SHAP 기반 판단(주간 심사 후보 등)이 그 기간에
      내려졌다면 재검토 대상이 된다.
