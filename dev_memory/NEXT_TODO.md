...
### 🔵 458차 — 손익 원천 이전 제안 (MW0602, 2026-08-10, **승인 대기**)

> 제안서: `docs/미륵이고도화3/손익원천이전_제안_2026-08-10.md`
> 배경: `DECISION_LOG.md` 458차. **라이브 무변경 — 채널 신설은 승인 후 별도 커밋.**

- [ ] **사용자 승인 여부 결정** — 3-A([40-B] 역방향 팔 사전등록) / 3-B([49] 조기청산
      반사실, TP1 도달 전 익절성 틱스톱 58건) / 3-C(기대값 지도 1회) 각각 개별 승인 가능.
- [ ] **승인 시 구현 순서**: 3-B가 최우선(표본 이미 58건 확보, 정본 재생기 재사용) →
      3-A(스크립트 확장 + `VALIDATION_CAMPAIGN` 등록) → 3-C(오프라인 1회).
- [ ] (읽는 법) **방향 축 "해롭다" 인용 금지** — 3팔 점추정이 전부 음이지만
      t≈1.2/12퍼센타일로 비유의. [40] 공식 판정("무작위와 구분 안 됨")이 정본이다.
- [ ] (읽는 법) **§1-7 "보유+30분 MFE" 표는 상한**이다(스톱아웃 무시) — 이 표로
      "기다리면 55.9%가 TP2 간다"를 주장하지 말 것. 정확한 반사실은 3-B가 잰다.
