...
- [ ] **하한가 필드가 단계 확대를 반영하는지 미검증** — 실측이 상한 +20% / 하한 −8%로
      비대칭이었다. 하한이 −8% 고정이면 폭락일에 SHORT 오차단 위험이 있으나, `초과 방어`
      가드가 그 경우 자동으로 차단을 해제하므로 **안전은 확보**돼 있다. 다음 하한가
      근접일에 [18]이 실제로 확대되는지 확인하면 가드 없이도 정확해진다.
