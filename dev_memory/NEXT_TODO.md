...
**파생 — 라이브 데이터에 고정된 테스트 4건 (488차 발견, 미조치)**

- [ ] 🟠 **`test_427_conf_discrimination.py`(FAIL 5) · `test_429_entry_timing_and_vol_estimator.py`(4) ·
      `test_428_direction_value_and_exit_model.py`(2) · `test_425_loss_tier1_qty1.py`(1)** —
      전부 *"N차 실측"* 값을 **실 DB 산출값 그대로** 단언해, 표본이 늘면 판정식이
      멀쩡해도 깨진다. 424 와 **같은 유형**이며 현재 **FAIL 상태로 방치** 중이다.
      (`test_430_conf_scale_break.py` 는 상수 기반이라 면역 — 통과)
- [ ] **처방은 424 의 4축 분리를 그대로 쓴다**: ① 공식은 합성 입력으로 고정
      ② 실 DB 는 내부 정합성(재계산 일치) ③ 단조성(기준선 이상) ④ 판정 매핑.
- [ ] ⚠ **일괄 삭제·완화로 닫지 말 것** — 이 테스트들이 지키려던 "판정식 표류 감지"는
      실재하는 가치다. 없애면 §9-4(판정기준 사후 변경 금지)를 지킬 자동 수단이 사라진다.
- [ ] ⚠ **채널별 의미를 먼저 읽을 것** — 424 는 캠페인 시작이 고정값이라 단조성이
      보장됐다. 창(window)이 롤오프하는 채널이면 ③이 성립하지 않으므로 다른 불변식을
      찾아야 한다(창 semantics 확인이 선행).
