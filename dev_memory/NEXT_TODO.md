...
- [ ] **🔴 다음 차례: `tp1_protect_offset_shadow`[25]/[25-B] 이관** — 자체 시뮬이
      **TP2에서 종료**. [25-B]는 축이 **TP2 거리** 자체라 직격 대상이다.
      442차가 [3-B]에서 밟은 절차를 그대로 쓸 것: ① 이관 전 변형별 "폭"과 개선폭의
      단조성 확인(인공물 지문) → ② `engine` 스위치로 v1 보존 → ③ v1 롤백 비트단위
      재현 검증 → ④ `replay_engine_targets` 승격 → ⑤ 확정결정 레지스트리 갱신.
      ⚠ [25]는 `lock_offset_pts`(절대 pt 오버라이드)가 이미 `exit_replay`에 있어
      [3-B]보다 이관이 쉽다. §25-C 비례 lock 축도 함께 깨어난다.
      ⚠ **이관 후 판정이 뒤집힐 것을 예상할 것** — [3-B]가 FAIL→PASS였고 [23-B]도
      기준선 부호가 뒤집혔다. 현재 [25]의 "현행 초과 대안 4개"는 미검증 수치다.
