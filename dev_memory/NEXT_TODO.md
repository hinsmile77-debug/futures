...
- [ ] **④ [실증 확인: `dscbo1.StockMst`는 개별종목 전용 — TR 자체를 교체해야 함, `CpSysDib.MarketEye` 검증 대기 중]** KOSPI200/VKOSPI 폴링 100% 실패.
  - **실측 결과(관리자 권한 셸, 사용자 실행)**: `probe_cp_stock_mst.py`로 `U180`/`K2G01P`/`O2901P` 3종 모두 `dib_msg="71103 조회결과가 없습니다.(niis.stk.new.mst)"`(완전 미인식, 헤더 전부 빈값/0). 대조군 `A005930`(삼성전자)은 `dib_msg="71101 조회가 완료되었습니다"` + 헤더 정상(종목명="삼성전자", 현재가 등 실값) — **`dscbo1.StockMst` 자체는 정상 작동하나 지수는 아예 지원 안 함**이 확정됨.
  - **문서 확인**: `cybosplus.github.io/cpdib_rtf_1_/stockmst.htm`(비공식이나 이 프로젝트가 이미 신뢰해 쓰던 소스, `api_connector.py:55` 참조) 공식 헤더 목록에 종목코드 예시가 "A000660"뿐이고 지수 언급 자체가 없음 — U180 가설은 틀렸고, "코드 문제"가 아니라 애초에 **TR 선택이 틀렸던 것**으로 결론.
  - **신규 후보**: `CpSysDib.MarketEye`(주식/지수/선물옵션 통합 조회 TR, 문서상 "해외지수는 심볼코드 입력" 명시 — 국내지수 지원 가능성 높음) — 단, StockMst와 인터페이스가 다름(`SetInputValue(0, field_id_list)` + `SetInputValue(1, code_list)` + `GetDataValue(pos, row)` 행 단위 조회, 최대 200종목/요청). 신규 진단 스크립트 `scripts/probe_cp_market_eye.py` 작성 완료(K2G01P/O2901P/A005930 기본 조회, 필드 0~20 덤프).
  - **다음 액션(사용자 필요)**: 관리자 권한 셸에서 `python scripts\probe_cp_market_eye.py` 실행 후 결과 공유. K2G01P/O2901P 행에서 종목명·현재가가 정상 나오면 `get_index_price()`를 `dscbo1.StockMst` → `CpSysDib.MarketEye` 기반으로 재작성(코드 자체는 그대로 K2G01P/O2901P 유지 가능성 높음, TR·필드 인덱스만 교체).
