...
- [ ] **하드닝(코드 변경 — 사용자 판단 대기)** — 현재 정상인 이유가 가드가 아니라
      "우연히 일치"다. `_prep_shap_xy()`는 열 순서를 `h_names` 순서에 맞추는데
      `n_features_in_` 검사가 개수만 보므로 **길이가 같고 순서만 다르면 무방비**다.
      → `save_shap_scores()` 직전 `len(score_map) == model.n_features_in_` 어서션,
      또는 `h_names`와 `feature_names_{hz}.pkl` 순서 동일성 명시 검증을 권고.
