...
- [ ] **entry_horizon(ATR 기반) 선택 로직에 호라이즌 스킬 정보 반영 검토** — 최초
  ...
  별도 검토 가치 있음.
- [ ] **체크리스트 8_time 항목 이상 신호 원인 확인** (저비용, 빠르게 끝남) — 체크리스트
  grade 재구성 backtest(06-15~, +15m forward move)에서 8_time 체크가 설계 의도와
  반대로 나옴: 통과(정상 시간대) +0.46pt(t=2.04) vs 실패(EXIT_ONLY/OTHER 시간대,
  주로 점심 11:50~13:00 추정) **+2.74pt(t=5.11)로 오히려 더 좋음**. 체크리스트가
  차단하는 시간대가 실제로는 더 나은 성과 구간일 가능성 — zone 세분화(OTHER를
  LUNCH_RECOVERY처럼 별도 취급) 또는 8_time 기준 자체 재검토 필요.
- [ ] **사이징의 꼬리위험 — A급 내 저conf 구간까지 풀사이즈 적용 재검토** — 체크리스트
  A급 내에서도 ens_conf 층화 시 conf<0.35(하한 근처) 구간은 +0.40pt(t=0.92, 비유의)인
  반면 conf 0.35~0.45는 +3.07pt(t=6.56, 유의)로 격차가 큼. 그런데 현재 사이징은
  conf 크기와 무관하게 A급이면 grade_mult×1.5를 동일 적용(신뢰도배수 자체도 conf
  0.25~0.45 구간에서 0.6 고정, 위 "모의투자 사이징 파라미터" 항목과 연결).
  grade_mult에 ens_conf를 추가로 곱하는 식의 보정(예: conf가 하한에 가까울수록
  사이즈 축소)을 검토해 하한 근처 A급의 꼬리위험을 완화할 가치 있음.
