...
- [ ] **④ [조사 완료, 실증 검증 필요] KOSPI200/VKOSPI 폴링 100% 실패 — 코드 후보 U180 확보, 라이브 검증만 남음** — `main.py:2701` `_poll_kospi200_index()` / `collection/cybos/api_connector.py:1032` `get_index_price()`가 7/6 09:01~12:52 매분 `[CybosIndex] 종목명 검증 실패`(code=K2G01P/O2901P, `GetHeaderValue(1)` 빈 문자열)로 100% 실패.
  - **조사 결과**: 웹 검색(2건 독립 소스, 대신증권 사이보스플러스 Q&A 게시판 포함)에서 "대신증권 API/CYBOS Plus에서 코스피200(KP200) 지수 코드는 **U180**" 확인. `K2G01P`/`O2901P`는 KRX 표준 업종지수 코드(증권플러스 stockplus.com에서 `KOREA-K2G01P`="코스피 200", `KOREA-O2901P`="코스피200 변동성지수"로 정확히 매칭 확인 — 즉 이 코드 자체는 진짜 존재하는 유효한 KRX 코드)이지만, **Cybos Plus의 COM TR(`dscbo1.StockMst`)은 KRX 표준 코드가 아니라 대신 자체 내부 "U"-prefix 지수코드 체계를 기대하는 것으로 추정** — 종목명 필드가 계속 빈 문자열로 오는 현상과 정합.
  - **VKOSPI의 U-코드는 못 찾음** — KOSPI200(U180)만 확인됨, O2901P를 대체할 U-코드 후보 없음.
  - **막힌 지점**: 라이브 검증 시도 → 실패. 신규 진단 스크립트 `scripts/probe_cp_stock_mst.py` 작성(dscbo1.StockMst에 임의 코드로 BlockRequest 후 헤더 전체 덤프, probe_cp_option_mst.py와 동일 패턴, main.py 라이브 프로세스 무관)해서 `python scripts/probe_cp_stock_mst.py --code U180` / `--code K2G01P` / `--code O2901P` / `--code A0567`(대조군)로 실행 시도했으나, 이 세션의 셸이 비관리자 권한(`IsAdminRole: False`)이라 `CpUtil.CpCybos.IsConnect`가 False로 나와 COM 연결 자체가 안 됨 — Cybos Plus(`CpStart.exe`, SessionId 1)와 라이브 main.py는 관리자 권한으로 실행 중인 것으로 추정, DCOM 무결성 수준 불일치로 비관리자 프로세스는 접속 불가.
  - **다음 액션(사용자 필요)**: 관리자 권한 PowerShell/cmd에서 `& "C:\Users\pc1\anaconda3\envs\py37_32\python.exe" scripts\probe_cp_stock_mst.py --code U180` 실행 후 `dib_status`·headers[1](종목명) 확인. U180이 "코스피200" 등을 포함한 이름을 반환하면 `api_connector.py:48`의 `KOSPI200_INDEX_CODE`를 "U180"으로 교체 — 기존 자체 검증 로직(`name_contains not in data.get("name","")`)이 이미 걸려 있어 잘못된 코드를 넣어도 현재와 동일하게 안전하게 실패(None 반환)하므로 시도 자체의 리스크는 낮음.
