...
### 다음 거래일 장후 확인 (여기서 안 보면 계측이 죽은 채로 굳는다)

- [ ] **559-1** `unk_vol` 첫 적재 확인 — `raw_candles`·`session_bars` 양쪽에
      NULL 이 아닌 행이 있는가. 앵커 리포트 **I4** 행이 「0봉」에서 실수치로 바뀌는가.
      ⚠ 452차 섀도가 「죽은 섀도」로 한 달 방치됐던 것과 같은 자리다.
- [ ] **559-2** EOD 로그에 `[FeatureEpoch]` · `[InvestorMeasured]` · `[UnitMismatch]`
      3종이 찍히는가. 특히 `[UnitMismatch]` 는 **제외 행수 > 0** 이어야 정상이다
      (4거래일이 26주 창 안에 있다). 0 이면 필터가 배선에서 빠진 것이다.
- [ ] **559-3** 수급 `*_measured` 3종이 프리장(08:45~08:59) **0.0** → 09:00 이후
      **1.0** 으로 바뀌는가. 하루 종일 0.0 이면 `_futures_seen` 갱신 경로가 안 도는 것이다.
