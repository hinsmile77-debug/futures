...
재생기를 v2(라이브 청산 사다리 미러링 + 체제 인식)로 교체해 개별 정확도를
MAE 0.408R → **0.233R(−43%)** 로 올렸으나 **사전등록 합격선(부호 일치)은 미충족**
→ [47]은 SUSPEND 유지, 기하 A/B 4채널도 정지 그대로다.

- [ ] **남은 오차원 ①(틱 경로) 판단** — 격차는 틱청산 80건(+9.20pt)이 아니라
      **TP 도달 29건(−35.75pt)** 에 몰려 있다. 봉 단위 재생의 구조적 한계인지,
      TP 트리거 모델(envelope)이 여전히 라이브 틱 TP와 다른지 분해할 것.
- [ ] **주간회의 안건 ①: `require_sign_match` 기준의 불안정성.** |Σ실제|=**0.95 SD**로
      실제 총합이 분산 대비 0 근처라 작은 편향에도 부호가 뒤집힌다. post 세대만 보면
      틱/비틱 **둘 다 부호가 맞는데** 합치면 상쇄돼 뒤집힌다.
      ⚠ **사후 완화 금지** — 바꾼다면 *앞으로*에 대해 사전등록할 것.
- [ ] **주간회의 안건 ②: 판정식 단위 불일치.** `SUM(pnl_pts)`는 레그별 계약당 pt의
      단순합이라 계약가중이 아니다. 리포트가 양쪽을 병기 중이며 교체는 수동 결정.
- [ ] **[25] §25-C 비례 lock은 등록만 됐다** — 게이트가 열리면 자동 판정된다.
      현재 관측(전건 SUSPENDED): lock_prop 0.35/0.50/0.75 = −2.99/−1.83/+3.30 Δ현행.
      ⚠ 이 수치를 근거로 쓰지 말 것 — 재현 미회복 상태의 값이다.
