...
- [ ] **1m 두 피처가 07-27 pkl에는 어떻게 들어갔는가** — 395차가 `feature_names_1m.pkl`
      직접 로드로 배포를 확인(10피처, SHAP 스코어링까지 관측)했는데, 07-31 본은
      8피처이고 `active_features`(97)에도 없다. `get_available_feature_set()` 교집합
      구조상 레지스트리에 없으면 들어갈 수 없으므로 **07-27에 다른 경로가 있었다는
      뜻**이다(임시 registry 상태 / 다른 재학습 스크립트 추정). 재발하면 배포 스펙이
      또 흔들리므로 원인을 찾을 것. 지금은 결론에 영향 없음(둘 다 미배포 확정).
