...
- [ ] **MW0602에서 자기 TRADE 로그로 `tick_era_start`를 재도출할 것.** 그 PC는
      원래 atr_profit이라 경계가 다를 수 있고, 다르면 [25]/[3-B]/[23-B] 전부가
      **잘못된 체제로 재생되고 있다**. 지금은 MW0601 값을 그대로 쓰는 중이다.
- [ ] **PC별 경계를 어떻게 둘지 결정** — settings에 PC별 dict를 두는 방식이 자연스러우나
      **사전등록 값이라 §9상 주간회의 결정이 필요**하다. 일방 변경 금지.
