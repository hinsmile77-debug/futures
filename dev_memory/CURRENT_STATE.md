...
> 마지막 업데이트: 2026-08-17 (MW0601 474차) — D9 딥다이브: §3 정합화 + 라우팅 밴드
> 채널 + 30m 역필터 기각. **매매 정책 변경 0건.** 전체 **447 passed / 0 failed**.
>
> 🔴 **진단 정정 — "10m·15m 예측이 불가능"은 사실이 아니다.** 예측·채점·앙상블 투표는
> 정상이다(채점 10m 12,281건 · 15m 11,379건). 막힌 것은 셋뿐 — ① mid/long CORE
> 체크리스트 발동 ② 그 호라이즌 TP1 배수 ③ `trades.entry_horizon` 귀속.
> 그리고 **회귀가 아니라 태생**이다: 124차(06-08) 라우터가 1m/3m/5m만 반환하도록 태어났고
> **8일 뒤** 179차가 그 위에 mid/long 그룹을 정의했다. 방증 — `ATR_HORIZON_TP1_MULT`에
> **10m 이상 항목이 존재한 적이 없다**(라우터만 고쳐선 열리지 않는 문).
>
> 🔴 **30m 학습을 중단하면 CB③이 죽는다 — 초안 권고의 절반을 철회했다.**
> 가중 0.0이라 "매 EOD 25초 낭비"로 보이지만 아니다. `main.py:5539`가
> `if v["horizon"]=="30m"`으로 걸러 `circuit_breaker.record_accuracy()`에 먹이고
> `main.py:6203`이 Contrarian 판단을 만든다 — **CB③(절대원칙 §2)의 유일한 입력원**이다.
> 전환기준 ⑥이 *"30m 재도입 또는 CB③ 기준 호라이즌 교체"* 인 이유가 이것이다.
> **30m 처분과 CB③ 기준 교체는 한 묶음으로만 가능하다.**
>
> 🔴 **`entry_band_watch` 첫 실행이 내 딥다이브 관측을 반증했다.**
> 계기였던 4.4–6.0 밴드(승률 38.1%, n=21)가 **일자단위에서 소멸**했다 —
> `일평균 격차 −90,251원(문턱 200,000 미달) · 부호검정 p=1.0000(우세 8일/열위 9일) ·
> drop-worst 1일 제거 시 −31,284원(65% 축소)`. 승률격차 24.7%p는 문턱을 넘었으나
> **판정이 AND 결합**이라 발화하지 않았다 → **BAND_UNIFORM**.
> 372차의 "신호단위 유의 → 일자단위 소멸" 재현이며 **사전등록이 그걸 잡았다.**
> ⚠ n=21은 문턱(20)을 겨우 넘긴 값이라 **저검정력 주석** 동반 —
> `BAND_UNIFORM`은 "이상 없음이 입증됐다"가 아니라 "사전등록 기준에서 안 잡혔다"이다.
>
> 🟡 **라우터 전제가 비단조다.** tf 밴드별 방향 적중률에서 "고변동→장호라이즌 우위"가
> 성립하지 않는다(6.0~8.0: 5m 46.8%≈15m 47.6% / **≥8.0: 1m 42.2%로 단기 우위**).
> mid 개통(B3 신설)의 근거가 약하다 — 처분 3안 중 C의 전제.
>
> 🟡 **30m 역필터 기각 등록.** 탐색 집계의 *30m 동의 53건 45.3%/−219,596원 vs 불일치
> 50건 68.0%/+49,917원* 은 **채택 금지**다(6×3 칸 중 최극단 1칸의 사후 선택, 다중비교·
> 일자단위·drop-worst 미분해, 방향 반전은 457·458차 기왕 기각).
> `VALIDATION_CAMPAIGN_DECISIONS["reverse_30m_entry_filter"]`에 기록 —
> 관측은 296차 퇴역 판정의 **독립 방증**으로만 보존한다.
>
> **[코드버그 2건 — 둘 다 표본이 차야 터지는 지연 폭발]**
> ① `focus_band` 문자열 불일치(`"4.4-6.0"` vs `%g` `"4.4-6"`) → 감시밴드가 **조용히 0**,
>    "안 쌓였다"와 "이름이 안 맞는다"가 같은 출력으로 보였다 → 매칭 실패는 이제 판정 불가.
> ② **`"%,.0f" % v`는 py3.7 ValueError**(콤마 플래그는 `str.format` 전용) —
>    `entry_band_watch` 5곳 + **473차에 심어둔 `spread_extreme_watch` 4곳**.
>    473차 `math.comb`(py3.8+)·431차 대시보드 `%` 사고와 같은 클래스.
> 상세: `DECISION_LOG.md` 2026-08-17(474차), `NEXT_TODO.md` 474차 G1~G4.
>
> 이전 업데이트: 2026-08-17 (MW0601 473차) — 구조적 교착 2건(F-8·D8) 해소 시도 +
...
> 재학습 등 §4-1 주간 체인(`_run_campaign_steps()`)이 없음(그건 `scripts/
> eod_retrain.py`에만 있고 아무 스케줄도 그걸 호출 안 함) — `logs/`에 `[검증
> 캠페인]` 로그 0건으로 확인. 이 계정(MW0601)엔 섀도우 TB 모델이 없어 [1] 채널이
> "디렉토리 없음"이었던 것도 이 공백의 결과 — 수동으로 6/6 호라이즌 재학습
> 완료(champion 대비 전부 우위), 리포트는 정상 데이터 생성 확인. 3안(스케줄 병합/
> 별도추가/수동관행 공식화) 중 결정 필요. 상세: `DECISION_LOG.md`/`NEXT_TODO.md`
> 2026-07-15(333차 후속3) 항목.
>
> 이전 업데이트: 2026-07-14 (332차) — 장후(18:28) 디버그 재기동 크래시 2건
