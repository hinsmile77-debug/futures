...
ENTRY_HORIZON_B1        = 3.2   # 1m/3m 경계 [374차: 1.5→3.5, 387차: 3.5→3.2, 483차 동결]
ENTRY_HORIZON_B2        = 4.4   # 3m/5m 경계 [374차: 2.5→4.0, 387차: 4.0→4.4, 483차 동결]

# 🔴 [2026-08-23 주간회의 확정 — 동결] 위 두 경계를 **바꾸지 않는다.**
#
# `EntryHorizonRecalibrator`가 5주 연속(07-24·07-31·08-07·08-14·08-21) `UPDATE`를
# 찍었으나 **주간회의가 명시적으로 현행 유지를 결정**했다. 경보가 계속 뜨는 것은
# 미조치가 아니라 **결정된 상태**다 — 다음 점검이 이것을 새 안건으로 올리지 말 것.
#
# 왜 따르지 않는가:
#  ① **재보정기는 손익을 보지 않는다.** 최근 분포의 33/67 분위수를 제안하고 세 버킷이
#     3~74% 밖이면 UPDATE를 낸다 — 판정 기준은 오직 "1/3씩 안 나뉜다"이고,
#     **"1/3씩이어야 좋다"는 손익으로 검증된 적이 없는 가정**이다. 이 장치의 원래
#     목적은 374차 사고(경계가 분포와 어긋나 61건 전부 5m 고착)의 **재발 감시**다.
#  ② **사전등록 판정은 반대쪽이다.** `entry_band_watch`(474차 D9-B) 2026-08-23 실행:
#     `BAND_UNIFORM` — 감시밴드(4.4~6.0) 일평균 격차 −81,713원(문턱 200,000 미달),
#     부호검정 p=1.0000. 08-17 대비 그 밴드는 승률 38.1%→45.8% / −19,414→−6,548원으로
#     **개선**됐고 후반부는 흑자(전반 −190,853 / 후반 +27,427)다.
#  ③ **권고를 적용하면 79.5%가 재배정된다.** 08-21 권고(5.72/8.57)를 실제 진입
#     220 포지션에 대입한 실측(2026-08-23):
#         3m → 1m  142건 (승률 63.4% · 계약당 **+17,538원**)   ← 유일한 흑자 구간
#         5m → 1m   23건 (43.5% · −17,839원)
#         5m → 3m   10건 (70.0% · +99,978원)
#         변경 없음 45건
#     즉 **지금 흑자를 만드는 구간을 통째로 1m으로 내리는** 변경이고, 현행 1m 구간은
#     계약당 −20,018원 적자다. TP1 폭도 ATR×0.5 → ×0.3으로 줄어든다
#     (`ATR_HORIZON_TP1_MULT`). ⚠ 이 표는 **이동 대상**만 말한다 — 이동 후 손익
#     재시뮬은 하지 않았다(카운터팩추얼 아님).
#
# **경계를 다시 열 조건**(이 중 하나라도 성립하면 재상정):
#   · `entry_band_watch`가 `BAND_ANOMALY`를 낸다(사전등록 문턱은 고정 — 낮추지 말 것)
#   · 26주 WFA 재검증에서 변동성 체제 전환이 확인된다
#   · 374차형 고착 재발 — 어느 한 버킷이 **실제 진입 기준**으로 95% 이상을 점유
#     (⚠ 재보정기의 버킷비중은 **전 분봉** 기준이라 진입 분포와 다른 것을 잰다)
#
# 근거: `dev_memory/DECISION_LOG.md` 2026-08-23 483차 후속5,
...
# [MW0601 533차 / 풀타임 수집 Phase 2 선행] 2026-09-06 사용자가 작업 스케줄러
# `MireukiEODRetrain` 트리거를 15:45 → **15:50** 으로 옮겼다(StartBoundary 실측
# 확인). 이유: 메인 프로세스가 15:45 마감 동시호가 체결까지 수집하려면 15:46까지
# 살아 있어야 하고, 그동안 py310 재학습이 같은 raw_data.db 를 읽기 시작하면 안 된다.
# 근거: docs/미륵이고도화3/풀타임수집_검토및구현계획_2026-09-06.md §4 Phase 2.
EOD_RETRAIN_SCHEDULE_HM: str = "1550"

# ── [MW0601 533차] 풀타임 세션 봉 적재 (Phase 1) ──────────────────────────
# `raw_candles` 는 08:45~15:08 봉만 담는다 — 15:09 이후 봉은 수신·집계까지 끝난
# 뒤 `_on_candle_closed` 의 force-exit 가드가 저장 전에 버린다(10거래일 실측 매일
# 정확히 384행). 소비처 46파일이 "최근 N행" 조회를 하므로 raw_candles 에 행을 더하지
# 않고, **별도 테이블 `session_bars`** 에 세션 태그를 달아 전량 적재한다.
# 읽는 코드는 없다(판단 무영향). 소비 전환은 Phase 4에서 채널별로 따로 결정한다.
SESSION_BARS_ENABLED: bool = True
...
# [MW0601 457차 / C2] 유령 기준선(GuardGhost) 처리 정책 — 456차 계획서 §9-2 미결 결론.
#
# 배경: 장중(intraday) 재학습이 gbm_{h}.pkl을 덮어쓰면 acc.txt는 **다른 모델의 성적**이
# 된다. 456차 F7이 이 상황을 탐지해 로그(`[GuardGhost] … 존재하지 않는 모델과의 비교다`)
# 는 남겼지만 판정에는 반영하지 않아, 시스템이 스스로 무효라고 선언한 판정을 그대로
# 집행했다. 실측 결과 3m EOD 교체가 **7거래일 연속 보류**(2026-08-03~08-11)됐고,
# 그동안 배포본은 계속 CV 미검증 장중 모델이었다. 그 모델은 같은 기간 하루 6회
# ConstOut(상수 출력) 붕괴를 일으켰고, 그 복구 재학습이 CB⑤·진입차단으로 이어졌다.
#
#   "replace" (⑤안, 2026-08-11 사용자 결정) — 유령 기준선이면 가드를 적용하지 않고
#                CV 검증된 신규 모델로 교체한다. 사유를 판정문에 명시하고 WARNING 로그.
#   "hold"    (⑥안 / 456차 이전 동작)      — 유령이어도 acc.txt로 판정(= 보류 지속).
#
# ⚠ 이 정책은 **guard_fair가 불가능할 때만** 작동한다. EOD_GUARD_VERDICT_SOURCE가
#   "guard_fair"이고 오염 0봉이 확인되면 그쪽이 우선한다 — 배포본을 직접 채점하므로
#   유령 문제 자체가 성립하지 않는다. 즉 457차 C1(F6 정상화)이 진행될수록 이 분기가
#   타는 빈도는 자연히 줄어드는 것이 정상이다.
EOD_GUARD_GHOST_POLICY = "replace"   # "replace"(⑤안) | "hold"(⑥안)
...
# ── [MW0601 458차 / P0] 모델 선로드 조용한 창(quiet window) ───────────────────
#
# 457차 B1은 **절반만 성공했다.** 참조 스왑은 559ms → 41~84ms로 해결됐지만,
# 워커가 **파이프라인이 도는 그 분에** 떠서 언피클(2.1~2.5초)이 GIL을 물고 있는 동안
# 메인 스레드가 굶는다. 2026-08-12 실측 — 선로드 분에 **전 단계가 동시에** 뛴다:
#
#     평상시   total= 233ms | S0=   1 S1= 19 S2=11 S4= 56 S5=111 S6= 29
#     선로드분 total=2394ms | S0=1714 S1=109 S2=77 S4=161 S5=203 S6=123   (3~8배)
#
# S0만 느린 게 아니라 S1·S4·S5·S6가 함께 느려지는 것이 결정적 단서다 — 특정 단계의
# 문제가 아니라 **자원 경합**이라는 뜻이다(456차 장중 DB 분석 사고와 같은 형태이나,
# 거기서는 원인이 외부였고 여기서는 **자기 자신의 워커**다).
# 그 결과 CB⑤ 경고 5건 + Degraded 선제차단 3건(cause=S0)으로 진입이 막혔다.
#
# 대책: 워커를 **띄우는 시점**은 그대로 두되(교체 유실 방지 로직 무변경), 무거운
# 언피클을 분 안의 조용한 구간으로 **미룬다**. 파이프라인은 매분 :59~:02에 돌고
# 평상시 233ms에 끝나므로, :15에 시작해 2.1초를 써도 양쪽 파이프라인과 겹치지 않는다.
#
# ⚠ **스왑 지연은 0이다.** 종전에도 워커 완료는 *다음 분* S0 drain이 받아갔다
#   (11:02:03 선로드 완료 → 11:02:59 스왑 적용). :15에 끝내도 같은 분 :59 drain이
#   받으므로 교체 시점이 밀리지 않는다.
#
# 457차 G9 판정 규칙("발생하는데 cause=S0 → B1이 부족한 것 → 재조사")이 이 경로를
# 이미 지정했고, 딥다이브가 ①별도 프로세스(GIL 회피)보다 ②시각 이동이 싸고
# 되돌리기 쉽다고 권고했다 — 이것이 그 ②안이다.
# 롤백: enabled=False → 457차 동작(즉시 로드)으로 즉시 복귀.
MODEL_RELOAD_QUIET_WINDOW = {
    "enabled": True,
    # 분 안에서 언피클을 시작할 목표 초. 직전 파이프라인(~:02 종료)과 다음
    # 파이프라인(~:59 시작) 사이의 한가운데.
    "target_sec": 15,
    # 이 초를 지나 워커가 떴으면 이번 분 목표는 이미 지났다 — 다음 분 :15로 미룬다.
    # (여기서 그냥 시작하면 :59 파이프라인과 겹칠 위험이 있다)
    "latest_start_sec": 45,
    # 안전 상한 — 어떤 계산 오류가 나도 이보다 오래 스왑을 미루지 않는다.
    "max_wait_sec": 45,
}

# ── [MW0601 458차 / P1-A] 깨끗한 꼬리 매치드 대조 — 전환조건 재정의 ──────────
#
# **456차 전환조건 ①("fair_contaminated_bars == 0 인 EOD 행이 5거래일 연속")은
# 원리적으로 달성 불가였다.** 오염은 "홀드아웃 봉 ts ≤ 현행 cutoff"로 정의되는데,
# 장중 재학습이 매일 5회 pkl을 덮어써 cutoff를 당일 12:00 근방으로 밀어올린다.
# 5거래일 폭(1,850봉) 홀드아웃의 앞 91%는 **항상** 그 cutoff보다 앞이다
# (2026-08-12 실측 1,691/1,850 = 91%). 즉 기다림의 대상이 존재하지 않았고,
# 그동안 EOD 교체 6/6이 ghost_bypass로 무검증 통과했다.
#
# 대체 설계: 홀드아웃 중 **현행 cutoff 이후 꼬리**만 떼면 현행도 도전자도 학습하지
# 않은 구간이므로 매치드 페어가 성립한다(도전자는 홀드아웃 전체가 OOS).
# 오염일에도 살아남는 유일한 공정 표본이다.
#
# **하루치로 판정하지 않는 이유는 수학이다** (p≈0.33 이항 CI, 실측 159봉/일):
#     1거래일   n=  159 → 95% CI ±7.3pp   (가드 허용폭 2.5pp의 3배 — 무의미)
#     5거래일   n=  795 → 95% CI ±3.3pp   (여전히 부족)
#    10거래일   n=1,590 → 95% CI ±2.3pp   ← 여기서부터 2.5pp를 분해한다
# 필요표본 ≈ 1,359봉 ≈ 8.5거래일 → 여유를 둬 **10거래일**로 사전등록한다.
# ⚠ 결과가 답답하다고 이 값을 낮추지 말 것 — 사전등록 위반이다(457차 G4와 동일 규율).
#
# 이 값은 **아직 어떤 판정에도 쓰이지 않는다.** 지금은 guard_shadow_log의
# clean_new/clean_old/clean_n 3열에 누적만 하며, 도달 후 주간회의에서
# EOD_GUARD_VERDICT_SOURCE 전환 여부를 결정한다(§9 사전등록 원칙).
EOD_GUARD_CLEAN_TAIL = {
    "min_days": 10,        # 롤링 최소 거래일 (위 검정력 계산 근거)
    "min_bars": 1350,      # 롤링 최소 누적 봉 수 (2.5pp 해상도 하한)
    "alpha": 0.05,
}
...
# ── 포지션 복원 가드 [MW0601 554차] ────────────────────────────
# 2026-09-10 08:45, 08:02에 돌린 pytest(`test_493_exit_stage_all_builders._opened()`)가
# 운영 상태파일 `data/position_state.json`에 `LONG 2계약 @ 1040.0`을 써 넣었고,
# 08:40 기동이 그것을 "오늘 저장분"으로 복원했다. 프리장 첫 틱 1109.60 이 들어오자
# TP1/TP2 가 즉시 발동해 실제 매도 2계약이 체결되고 허구 이익 +6,921,594원이 기록됐다.
#
# 저장 진입가가 직전 거래일 종가에서 이 비율을 넘게 떨어져 있으면 **복원을 거부**한다.
# 5.0%는 KOSPI200 선물 일간 갭의 현실적 상한 쪽에 둔 값이다(그날 괴리는 6.6%).
# ⚠ 거부는 "포지션이 없다"는 뜻이 아니다 — 진짜 포지션이면 BrokerSync 가 복구할
#   기회가 남아 있고, 상태파일은 `.rejected_*` 로 보존한다.
POSITION_RESTORE_MAX_DEVIATION_PCT: float = 5.0
...
# [MW0601 529차] 스윙 위치 피처 — "레그가 얼마나 달렸고, 극단에 얼마나 가까운가".
# 배경: 2026-09-03 C1~C4가 전부 60분 레그 끝(스윙 저/고점에서 6.6~10 ATR 달린 뒤, 극단
#   0.6~1.3 ATR 안쪽)의 순방향 진입이었는데, 기존 연장폭 피처 둘(price_extension_atr ·
#   price_extension_atr_60m)은 "N분 전 **종가** 한 점" 대비라 V자 되돌림 뒤에는 0 근방으로
#   나온다(C1: 60분 연장 0.28 ATR vs 스윙 저점 대비 7.1 ATR). 극단까지의 거리는 피처 자체가
#   없었다(528차 §4-1). 이 피처는 **기록 전용**이다 — 진입 판정·사이징 어디에도 소비되지 않고
#   `raw_features`·`ensemble_decisions.features`에 남아 채널(`leg_exhaustion_entry_watch`) 판정의
#   원천이 된다. 판정 전에는 소비자를 붙이지 않는다(§9 사전등록 원칙).
# 키(방향 무관, 진입 방향은 소비자가 곱한다):
#   swing_high_{N}m / swing_low_{N}m        — 최근 N봉(현재 봉 포함) 최고가/최저가
#   dist_to_high_{N}m_atr                   — (최고가 − 종가) / ATR  (≥0, 매수라면 "천장까지")
#   dist_to_low_{N}m_atr                    — (종가 − 최저가) / ATR  (≥0, 매수라면 "달린 거리")
#   bars_since_high_{N}m / bars_since_low_{N}m — 극단 이후 경과 봉 수(현재 봉이면 0)
#   swing_ready_{N}m                        — 버퍼가 N봉을 채웠는가(False면 위 값은 부분 창 기준)
# ⚠ 워밍업(버퍼 < N)에는 값을 0으로 채우지 않고 **부분 창으로 계산하되 ready=False**를 함께
#   남긴다(계측 4원칙 ②·④ — "측정 안 됨"과 "0"을 구분). 거리 값은 ATR<=0이면 0.0 + ready=False.
SWING_FEATURE_LOOKBACKS_MIN = (60,)      # 튜플 — 다른 창(예: 20)을 추가하면 키가 그만큼 늘어난다
SWING_FEATURE_DIST_CLIP_ATR = 20.0       # 거리 상한(ATR 배수) — 극단 이상치 방어

# ── [MW0601 540차] GOLDEN POWER 교차 피처 — 기록 전용 ────────────────────────────
# 대신 사이보스 내장지표 GOLDEN POWER 복원 산식(`docs/미륵이고도화3/Golden power/
# GOLDEN_POWER_구현명세.md`, 2거래일 47개 지점 오차 ≤0.01 검증):
#     Golden Buy(n)  = (종가 − n봉 최저종가) / 종가 × 200
#     Golden Sell(n) = (n봉 최고종가 − 종가) / 종가 × 200      1.0 = 이격 0.5%
#
# 기록 키 8종(`features/feature_builder.py:compute_gp_cross_features`):
#   gp_buy_20 · gp_sell_20            원값
#   gp_cross_up_20 · gp_cross_dn_20   이번 봉에 그 선이 0.5 를 상향 통과했는가 (0/1)
#   gp_cross_tight_20                 그 교차가 「반대편 ≤ eps」를 함께 만족했는가 (0/1)
#   gp_ready_20                       버퍼가 period+1 봉을 채웠는가  ※ 계측 4원칙 ②·④
#   atr_bp · atr_bp_measured          상대 ATR = atr/종가×10000 (채널 게이트 축)
#
# 🔴 **소비자를 붙이지 말 것.** 진입 판정·사이징·체크리스트 어디에도 이 키를 읽는 곳이
#    없어야 한다(`tests/test_540_gp_cross_features.py` 가 전수 검사한다). 아래 두 채널이
#    판정할 때까지 매매 정책은 무변경이다.
# ⚠ 기간은 **20**이다 — 사용자 차트(`Golden Buy_20`) 기준. 명세서 기본값 25 와 다르다.
#    바꾸면 아래 두 채널의 사전등록 컷이 함께 무효가 된다.
# ⚠ `atr_bp` 는 26주 재검증 항목 `ATR_MIN_ENTRY`(절대 pt 고정 임계의 드리프트)의
#    **시기중립 대조축**과 같은 값이다. 둘을 함께 본다(CLAUDE.md 539차 항목).
...
# ── [MW0601 482차 / F-3] 메인 스레드 정지 전용 계측 (섀도 — 차단 없음) ────────
#
# 왜 필요한가: CB⑤(`CB_PIPE_PAUSE_MS`=5,000ms)와 FZ-1 워치독
# (`FREEZE_WATCHDOG_STALL_SEC`=180초) 사이 구간에 **주인이 없다.**
# 두 장치는 서로 다른 것을 재는데 그 사실이 계측에 드러나지 않았다:
#
#   CB⑤        : 파이프라인 경과시간 = `[PipePerf] total`(S0~S8 구간)
#   _tick_header: **메인 스레드 전체 정지시간**(Qt 타이머가 못 돈 시간)
#
# 2026-08-20 실측 — 5초 초과 정지 4건 전량이 CB⑤ 사정권 밖이었다:
#
#   시각      메인정지    같은 분 PipePerf   CB⑤ 미계상 잔차
#   12:38:05  5,297ms        315ms          4,982ms (94%)
#   12:40:08  8,375ms      2,838ms          5,537ms (66%)
#   13:16:04  5,141ms        329ms          4,812ms (94%)
#   13:54:04  5,047ms        390ms          4,657ms (92%)
#
# ⚠ 종전 해석("`pipe_elapsed=0` = 파이프라인 밖 정지")은 틀렸다 — `_pipe_elapsed_s`는
#   **마지막 파이프라인 완료 후 경과 초**이고 `notify_pipeline_ran()`이 0으로 리셋한다.
#   즉 0은 "직전에 파이프라인이 막 끝났다"는 뜻이며, 정지는 그 분에 **걸쳐** 있다.
#   그런데 파이프라인이 315ms밖에 안 걸린 분에도 메인은 5.3초 멈췄다 — 정지의 대부분이
#   S0~S8 밖(COM 콜백·Qt 페인트·다른 타이머)에서 났고 CB⑤는 그것을 볼 수 없다.
#
# 2026-08-19 동결(480차)이 정확히 "프로세스는 살아 있는데 메인 스레드가 멈춤"이었고,
# 그날 실손해가 0이었던 것도 우연히 FLAT이었기 때문이다. 같은 무관측 구간이 남아 있다.
#
...
#    2주 섀도로 잔차 분포를 본 뒤 승격 여부를 주간회의에서 정한다.
MAIN_THREAD_STALL_WARN_MS = 5_000     # 이 이상이면 `[MainStall]` WARNING 한 줄
MAIN_THREAD_STALL_ALERT_MS = 15_000   # 이 이상이면 ERROR — FZ-1(180초) 전 단계 경보
# `_tick_header` 자체의 블로킹 검출 하한(종전 하드코딩 2,000ms). 이 아래는 안 찍는다.
MAIN_THREAD_STALL_DETECT_MS = 2_000

# ── [MW0601 519차 / 사용자 지시] ALERT 밴드를 대시보드 「2 경보」 탭에 띄운다 ──
#
# 종전에는 ALERT(≥15초)도 모듈 로거(`logging.getLogger("SYSTEM")`)로만 나가
# **파일에만** 남았다 — 경보 탭은 `log_manager` 를 타는 것만 받으므로 화면에는
# 아무것도 뜨지 않았다. 2026-09-02 11:28:57 의 56,875ms 정지가 그렇게 조용히
# 지나갔고 사람이 안 것은 그날 장중 점검(12:26)이었다.
#
# **상한을 두는 이유**: ALERT 는 전 로그 기간 **5건/5거래일**(08-24 20,985ms ·
# 08-25 21,781ms · 08-28 17,390ms · 08-31 15,157ms · 09-02 56,875ms)이라
# 폭주 위험이 사실상 없다. 그래도 계측이 새 장애원이 되지 않도록 상한을 둔다 —
# 상한에 닿으면 **닿았다는 사실 자체를 한 줄 남기고** 그친다(계측 4원칙 ③).
# ⚠ 임계(15초) 자체는 **바꾸지 않았다** — 재보정은 표본이 더 필요하며
...
# ── [MW0601 552-10] 브로커 net 판독 세션 경계 ─────────────────────────────────
# 이 시각 **이후**의 첫 예탁현금 판독은 「당일 시가」인지 「장후 정산 롤오버」인지
# 판별할 기준점이 없다 — 그 판독으로는 그날 net 을 쓰지 않는다.
#
# 🔴 왜 필요한가: 2026-09-07 21:58 에 **새 세션**이 떴고, 그 프로세스에겐 롤오버
#    판독이 「그날 첫 판독」이라 501차 불변식 가드가 성립해 버렸다. 결과로 그날
#    net 이 -1,432,630 → -386,630(= -수수료)으로 덮어써졌다. 전환기준 ① 판정
#    원천이 오염된 것이다.
# 값 근거: 정규 마감 15:35(만기일 15:20) — CLAUDE.md 「장중 라이브 DB 분석 금지」
#    구간의 상한과 같은 경계를 쓴다. 이보다 이르게 잡으면 정상 장중 판독을 버린다.
BROKER_NET_SESSION_END = "15:35"
...
# ── [MW0601 519차 / 사용자 지시] F-1: 마감(15:40) 진입 시 잔여 포지션 자동청산 ──
#
# **무엇을 메우는가.** 절대원칙 §1(오버나이트 금지)의 집행 단계는 15:10 강제청산 →
# 15:18 FINAL_CLOSE 안전망까지다. **그 뒤에 생긴 포지션은 아무도 닫지 않는다.**
# 2026-09-01 15:34:46 에 원인불명 외부 매수 3계약이 들어왔고, 15:40:08 마감은 그
# 포지션을 보지도 않고 통계·리셋을 진행한 뒤 프로그램이 종료됐다. 밤을 넘겼고,
# 다음날 08:45:11 하드스톱으로 **-5,299,668원**이 확정됐다.
# 514차가 마감 진입부에 **탐지**(`[DailyCloseResidual]`)를 넣었으나 경보뿐이었다 —
# 사람이 그 시각 화면 앞에 없으면 아무 일도 일어나지 않는다. 이 플래그가 그
# 경보를 **집행**으로 승격한다.
#
# 🔴 **왜 위험한가 — 반드시 상한이 필요하다.** 마감 절차 안에서 주문을 내므로,
#    체결이 안 되거나 브로커가 거부하면 마감이 늦어지거나 무한 대기한다. 그래서
#    ⓐ 시도 횟수 ⓑ 전체 벽시계 시한 을 **둘 다** 건다. 시한을 넘기면 청산을
#    포기하고 **경보를 남긴 뒤 마감을 계속 진행한다** — 마감이 멈추면 EOD 재학습·
#    P8·세션 상태 저장이 통째로 날아가 피해가 더 크다.
#
# **시한을 15초 미만으로 잡은 이유**: 마감은 Qt 메인 스레드에서 돈다. 여기서
# 오래 블로킹하면 `[MainStall]` ALERT(`MAIN_THREAD_STALL_ALERT_MS` = 15,000ms)가
# **자기 자신 때문에** 발화한다. 안전장치가 새 경보를 만드는 모양을 피한다.
#
# ⚠ **이중 청산 위험은 없다.** 같은 시간대에 주문을 낼 수 있는 다른 경로는
#   `scripts/force_flat_guard.py` 뿐인데 그쪽은 `FORCE_FLAT_GUARD_ORDER_ENABLED
#   = False`(위)라 **알림 전용·미구현**이다. 그 플래그를 켤 때는 이 항목과 함께
#   재검토할 것 — 둘 다 켜면 15:39 가드와 15:40 마감이 같은 잔량에 두 번 주문한다.
# ⚠ 청산 함수는 `_ts_broker_direct_force_exit()` 를 그대로 재사용한다. 그 함수는
#   **브로커 잔고를 매번 다시 조회**하므로(엔진 상태가 아니라 계좌가 권위) 이미
#   닫힌 포지션에 중복 주문을 내지 않는다 — 잔고가 없으면 False 를 돌려준다.
DAILY_CLOSE_FORCE_EXIT_ENABLED = True   # False = 514차와 같은 경보 전용으로 되돌림
DAILY_CLOSE_FORCE_EXIT_MAX_ATTEMPTS = 3     # 주문 시도 상한
DAILY_CLOSE_FORCE_EXIT_TIMEOUT_SEC = 12.0   # 전체 벽시계 시한(< MainStall ALERT 15초)
DAILY_CLOSE_FORCE_EXIT_SETTLE_SEC = 2.0     # 시도 사이 체결 대기
...
VALIDATION_CAMPAIGN = {
    # ── [80] [MW0601 611차 후속4 / 2026-09-22] 개인 옵션흐름 → 선물 예측력 ────────
    # 🔴 **데이터를 보기 전에 고정한다**(§9 사전등록). 표본이 쌓인 뒤 기준을 정하면
    #    문턱을 낮추고 싶어진다 — 458차 D6 이 경계하는 형태다.
    #
    # 왜 재는가: 611차가 위클리 옵션 투자자 수급을 수집하기 시작했다. 첫날(9/21)
    #   하루치 분석에서 개인 Δ(콜-풋)은 선물에 **1분 후행**했고(k=-1 r=-0.531),
    #   예측 방향은 유의하지 않았다(조건부 검정 p=0.10~0.58). 하루 표본이라 결론이
    #   아니다 — 표본을 쌓아 **예측력이 있는지 없는지**를 사전등록 기준으로 가른다.
    #
    # 주 계열을 하나로 못박는 이유: 7상품 x 3주체를 다 재면 다중검정으로 우연이
    #   섞인다. 9/21 실측상 개인 옵션거래의 92%가 (월)위클리이므로 그것만 주 계열로
    #   삼고, 나머지는 **참고 출력**에 그친다(판정에 관여하지 않는다).
    #
    # 일자 단위로 집계하는 이유: 분 단위 관측은 하루 안에서 서로 독립이 아니다.
    #   372차가 같은 함정을 실증했다 — 신호단위로는 유의했는데 일자단위로 보니
    #   상관이 사실상 0이었다. 여기서는 **일자별 IC 를 구하고 일자를 표본으로 센다.**
    #
    # 만기일을 나누는 이유: 9/21 은 (월)위클리 만기일이었고 "당일 만기물만 순매수"
    #   라는 뚜렷한 패턴이 나왔다. 그것이 만기일 고유 현상인지 상시 패턴인지
    #   구분하지 않으면 둘을 섞어 평균낸 값으로 판정하게 된다.
    #
    # ⚠ **판정 배선은 표본 도달 후에 한다** — 지금 이 키는 리포트 생성기에서 inert
    #   다([2] atb_v2 와 같은 취지). 사전등록이 목적이지 주간 판정이 목적이 아니다.
    #   그래서 `CHANNEL_REGISTRY` 에도 **아직 등록하지 않았다** — 레지스트리는
    #   "리포트가 실제로 출력하는 채널"의 목록이라, 출력하지 않는 채널을 넣으면
    #   `assert_channel_registry_consistent()` 가 리포트 생성을 막는다(실측 확인).
    #   ⇒ 판정을 배선하는 커밋이 번호 **80**(MW0601 대역 80~159 첫 배정)을
    #     레지스트리에 함께 등록한다.
    "option_flow_predictive_watch": {
        "start_date": "2026-09-22",
        "min_trading_days": 20,          # 미달 → INSUFFICIENT (월/목 만기 각 4회 확보)
        # 주 계열: (월)위클리 개인 (콜 - 풋) 순매수의 분당 증감
        "primary_signal": "wk_mon_call_minus_put_delta",
        "primary_investor": "individual",
        # 예측 방향만 본다. k<=0(동시·후행)은 참고 출력이며 판정에 쓰지 않는다.
        "forward_minutes": [1, 3, 5],
        "ic_method": "spearman",         # 이상치에 강건
        "ic_abs_min": 0.05,              # 일자별 IC 평균의 하한 ([1] tb 와 동일 축)
        "sign_consistency_min": 0.60,    # IC 부호가 같은 날의 비율
        "p_max": 0.01,                   # 일자 IC 에 대한 t검정 (다중검정 여유)
        "split_by_expiry": True,         # 만기일/비만기일 각각 판정
        "pass_requires": "forward_minutes 중 1개 이상에서 위 3조건 동시 충족",
    },
    ...
    #       ③ [MW0601 489차 재등록] **OOS 기간 2분할 재현** — 전반/후반으로 갈라
    #          같은 후보가 양쪽에서 현행을 초과하는가(분리도 부호 유지).
    #          ⚠ 종전 ③은 *"MW0601 × MW0602 교차"* 였으나 2026-08-23 멀티PC 정책
    #            폐기로 **대조 상대가 사라져 충족 불가능한 관문**이 됐다. 방치하면
    #            "표본 축적 후 재검토"를 무기한 반복하게 되므로(0808 D2 경고),
    #            교차검증의 취지(*우연한 표본 구성에 기댄 우위를 걸러낸다*)를
    #            단일 갈래에서 살리는 형태로 **수치를 더 보기 전에** 갈아끼웠다.
    #          ⚠ 2분할은 표본을 절반으로 쪼갠다 — 통과해도 "약한 증거"다(313차).
    ...
    # ── [MW0601 436차 신설] §47 진입 맥락별 손익 — 사전등록 (관측 전 확정) ────────
    # 계기: 0805 일일점검이 사후 슬라이스로 발견한 "나빠 보이는 진입 맥락" 3종.
    #   S1 09시대 진입 / S2 반대방향 재진입 / S3 동일방향 5분 이내 재진입
    #
    # ⚠⚠ **이 채널의 존재 이유가 "그 관찰을 믿지 말자"다.** 등록 근거는 아래 4가지다.
    #
    # ① **사후 슬라이스 + 다중비교.** 0805에 진입 맥락을 여러 축(재진입 간격 4구간,
    #    동일/반대 방향, 직전 승/패, 시간대 6구간)으로 갈라본 뒤 **나빠 보이는 것만
    #    골라** 보고했다. 가지 수가 최소 15갈래 이상이므로 개별 p-value를 그대로 읽으면
    #    안 된다(garden of forking paths).
    # ② **관측 창을 바꾸면 부호가 뒤집힌다** — 실측(포지션합 pt):
    #        슬라이스      2026-07-01~(원 보고)   2026-06-10~
    #        S1 09시대          −23.90pt          **+31.99pt**
    #        S2 반대방향        −45.18pt            −34.35pt
    #        S3 동일<=5분       −15.78pt          **+37.29pt**
    # ③ **지표를 계약당으로 바꾸면 크기가 달라진다.** 원 보고는 포지션합 pt였고 그건
    #    계약수가 섞인 값이다(417차 교훈 — 대형 계약수 구간 06-10~07-10이 통째로 들어간다).
    #    계약당 정규화 시 S2는 −0.000(보완 +0.743)으로 **효과가 사실상 사라진다.**
    # ④ **원 보고의 해석 하나가 창 의존이었다.** "S2는 승률이 평균 이상인데 손실만 크다
    #    — 방향이 아니라 페이오프 문제"라고 적었으나 06-10~ 기준으로는 승률 51.5% vs
    #    보완 57.3%로 **오히려 낮다.** 그 해석은 07-01~ 창에서만 성립한다.
    #
    # **왜 3개 채널이 아니라 1개 채널의 3 슬라이스인가**: 363차 후속이 정한 원칙 —
    # "제안 하나당 새 검증 트랙 하나"가 늘면 §9의 관리 포인트 억제 효과가 역전된다.
    # 게다가 세 관찰은 전부 "진입 맥락 조건별 손익"이라 같은 질문이고, 한 채널로 묶어야
    # **다중비교 보정이 자연스럽게 강제**된다(따로 등록하면 각자 α=0.05를 쓰게 된다).
    #
    # 통계 설계:
    #   - 주 지표 = **계약당 pt**(`sum(pnl_pts)/entry_qty`) — 사이징 무관(417차).
    #   - 검정 = 슬라이스 vs 보완집합 평균차의 **순열검정**. 표본이 얇고 꼬리가 두꺼워
    #     정규 근사가 위험하고, scipy 의존도 피한다. ⚠ numpy BLAS가 이 PC에서 파손
    #     상태라(432차 후속) 행렬연산을 쓰지 않는다 — 순수 파이썬 구현.
    #   - 다중비교 = **Bonferroni**(α/슬라이스수). 등록 슬라이스는 3개지만 실제 탐색
    #     가지는 그보다 많았으므로 **이 보정은 하한**이다 — 통과해도 "약한 증거"다.
    #   - 방향 축(승률)과 페이오프 축을 **분리 계측**한다 — ④의 가설이 어느 축인지
    #     OOS에서 다시 확인할 수 있게.
    #
    # 판정 (관측 전 고정 — 관측 후 기준을 고치면 검증이 무의미해진다, §9):
    #   PASS(조치 불요): OOS에서 Bonferroni 보정 후 유의한 슬라이스가 없다.
    #   FAIL(처방 검토): 보정 후 유의(p<=α/k) **그리고** 평균차 음수 **그리고**
    #     drop-worst 후에도 음수 유지 **그리고** 일자단위 과반이 음수 — 4조건 동시 충족.
    #     → 즉시 조치 아님, 주간회의 수동 결정.
    #
    # ⚠ **FAIL이 나와도 "진입 차단"은 처방 후보가 아니다.** 두 가지 이유다:
    #   (a) 427·428차가 방향 축이 무작위와 구분되지 않음을 확정했으므로 진입 필터의
    #       기대값이 보장되지 않는다(430차 [44] 주석과 동일 논리).
    #   (b) 감사 §2-2 ③ "차단형 게이트는 이미 충분히 많다".
    #   처방은 **사이징 또는 집행(청산) 축**에서 찾을 것 — 417차(손절 준수율)·425차와
    #   같은 방향이다. 특히 S2가 페이오프 축에서만 유의하다면 그건 P2(집행) 안건이다.
    #
    # 구현은 라이브가 아니라 오프라인 스크립트다(scripts/entry_context_shadow.py).
    # trades만으로 전부 재구성되므로 진입 경로에 계측을 심을 필요가 없다 — 회귀 위험 0.
    "entry_context_shadow": {
        "min_samples": 25,   # OOS **슬라이스별** 진입 건수. 미달 슬라이스는 판정 보류
        "min_days": 10,      # 313차 원칙 — 맥락 효과는 장세에 민감하므로 넉넉히
        "oos_start": "2026-08-06",   # 가설 수립(0805) 다음 거래일
        "alpha": 0.05,       # Bonferroni 전 유의수준 (보정 후 = alpha / len(slices))
        "perm_iters": 20000, # 순열검정 반복
        "perm_seed": 20260805,  # 재현성 고정 — 실행마다 p가 흔들리면 판정이 흔들린다
        "slices": [
            "S1_hour09",           # 09시대 진입
            "S2_reverse_reentry",  # 직전 청산 후 반대방향 재진입
            "S3_same_dir_le5min",  # 직전 청산 후 동일방향 5분 이내 재진입
        ],
        # 창 민감도 표시용 — 이 채널의 등록 사유 ②를 리포트가 항상 보여준다.
        "sensitivity_windows": ["2026-06-10", "2026-07-01", "2026-07-14"],
    },
    "hurst_threshold_shadow": {
        ...
        # [MW0601 489차] 판정보조 ③ 대체물 — OOS 기간 2분할 재현(위 주석 참조).
        # 사용자 승인 2026-08-23(B-1). 관측 후 기준 수립을 피하려 **수치를 더 보기
        # 전에** 등록했다(313차). 표시 전용이며 verdict 합격선은 무변경이다(§9-4).
        "split_half_check": True,
        "split_half_min_per_half": 20,   # 반쪽당 최소 진입 — 미달이면 판정 보류 표기
    },
    ...
    "kelly_skip": {
        ...
        # [MW0601 489차 / B-2 (a)재등록] 종전 3관문은 *"**양 PC 각각** 전부 충족 시에만
        # 상정"* 이었다 → 멀티PC 폐기로 **이 갈래에서 3관문 충족**으로 재등록.
        # 임계는 관측 전 확정값이라 무변경이다(§9-4).
        # 🔴 같은 회의가 선행 작업으로 지목한 ②의 **계측이 489차에 이식됐다** —
        #    그 전에는 어떤 n에서도 상정 금지였고, 실제로 재는 코드가 없었다.
        ...
        "block_gate_scope": "this_branch",  # 종전 "both_pcs"
    ...
    # ── [MW0601 458차 / P3] 경과시간 축 사전등록 — **관측만, 문턱 없음** ──────
    # 2026-08-12 조기축소 3건이 전부 진입 후 11~47초에 발동했고 그중 2건은 잘라낸
    # 직후 반등해 잔여 레그가 TP2까지 갔다(#3은 30초 뒤 반등, 근사 −2.46pt 손해).
    # "진입 직후의 일시적 역행을 손절 신호로 오독하는가"라는 가설이 생겼으나
    # **표본 3건으로는 아무것도 결론지을 수 없다**(313차). 그래서 지금은 축만 만든다
    # (`elapsed_sec` 컬럼, tier1·tier2 양쪽).
    #
    # ⚠ **판정 문턱을 여기 적지 않는 것이 의도다.** 데이터를 본 뒤 문턱을 정하면
    #   사후 적합이다. 아래 min_samples/min_days에 도달했을 때 주간회의에서
    #   ① 구간 분할 방식 ② 합격선을 **먼저** 정하고 그 다음 집계한다.
    # ⚠ 기존 행의 elapsed_sec은 NULL이다 — 이 축의 집계에서 **제외**할 것.
    #   NULL을 0으로 읽으면 "즉시 발동" 구간이 통째로 오염된다(계측 4원칙 ②).
    "loss_tier1_elapsed_watch": {
        "min_samples": 25,   # elapsed_sec NOT NULL 인 tier1 터치 건
        "min_days": 6,       # 425차가 이 채널 계열에 세운 일자 규율과 동일
        "data_start": "2026-08-13",  # 컬럼 배포 다음 거래일부터 적립
    },
    ...
    "tp1_trail_shadow": {
        ...
        # ⚠ [MW0601 477차 후속3] 이 채널은 **qty=1 전용**이고 431차 사이징 수렴
        #   (2계약)으로 2026-08-13부터 신규 훅이 0이다 — 리포트가 "표본 부족"이
        #   아니라 **모집단 소멸**로 표기한다(기다려도 차지 않는다). 문턱을 낮추지
        #   말 것(458차 D6과 같은 사전등록 위반). qty≥2 축은 [25]가 맡는다.
    },
    # ── [MW0601 477차 후속3 신설 / 476차 G-1] §55 포지션 MFE/MAE 감시 (관찰 전용) ──
    #
    # 계기: 2026-08-18 진입 7건이 **전부 방향 적중**인데 포지션당 평균 보유 1.4분,
    #   실현 6.85pt/계약이었다. 같은 진입들의 +10분 MFE 합은 43.2pt —
    #   realized/MFE10 = 0.158. 그날 장중 레인지는 73.1pt(5일평균 32.3pt)였다.
    #   "청산 규칙이 이겼다"와 "청산 규칙이 절단했다"가 같은 사실의 양면인지를
    #   재는 축이 없었다.
    #
    # 계측: `scripts/position_mfe_shadow.py` — trades + raw_candles **소급 계산**이라
    #   라이브 코드를 건드리지 않는다([25]와 같은 설계, 과거 표본 즉시 적용).
    #   포지션 단위(entry_ts)로 집계한다 — 레그 단위 금지(계측 4원칙 ①, 417차).
    #
    # 판정 (관측 전 고정 — 사후 조정 금지, §9):
    #   PASS  : 추세일 부분집합의 realized/MFE10 중앙값 ≥ 0.35
    #   FAIL  : < 0.35 → TP1 배수(ATR_HORIZON_TP1_MULT)·보호전환 폭 재설계를
    #           **주간회의 안건으로 올린다**(자동 변경 금지)
    #   INSUFFICIENT : 추세일 표본 30건 또는 15거래일 미달
    #
    # 🔴 **MFE는 사후 최적값이라 도달 불가능한 상한이다.** 0.35라는 값은 "이만큼
    #   가져갈 수 있었다"는 뜻이 **아니다** — 완벽한 청산도 MFE를 100% 취할 수
    #   없다(스톱 없이 최고점에 파는 것과 같다). 이 비율은 **레짐 간 상대 비교**로만
    #   읽는다. 0.35는 0818 관측치(0.158)의 2배를 조금 넘는 지점으로, 단일일
    #   관측에 맞춘 값이 아니라 그 위에서 넉넉히 잡은 경계다(313차).
    "position_mfe_shadow": {
        "min_samples": 30,          # 추세일 진입 포지션 수
        "min_days": 15,             # 313차 원칙 — 일자 다양성
        "horizons_min": (5, 10, 20),
        "trend_day_range_mult": 1.5,   # 추세일 = 당일 레인지 ≥ 직전 5일평균 × 1.5
        "trend_day_lookback": 5,       # ⚠ 당일 제외(사전 정보만) — 룩어헤드 방지
        "realized_over_mfe10_median_max": 0.35,
    },
    ...
    # ── [MW0601 456차 / Wave 4 · G2-B] 수량 분해 하위 축 — 사전등록 ────────────
    # 417차가 열어두고 **미검증**으로 남긴 가설을 여기에 붙인다:
    #   "미니선물 호가 깊이를 여러 계약이 한 번에 훑어, 계약수가 클수록 손절 체결이
    #    의도가보다 나빠진다."
    # 417차는 '손절폭 초과 비율' vs 진입수량(rho=+0.318)만 봤고 **체결 슬리피지
    # 자체는 재지 않았다.** 이 채널이 이미 슬리피지를 쌓고 있으므로 **새 채널을
    # 만들지 않고** 수량 축만 추가한다(54개 채널 중복 방지).
    #
    ...
    #   [47]·[48]의 고지와 같은 취급이다. 이 축의 가치는 **앞으로의 표본에서
    #   재현되는가**에 있다.
    #
    # 판정 (관측 전 고정):
    #   SUPPORTS_HYP : rho(수량, |slippage|) > 0 AND 일자 부트스트랩 95% CI 하한 > 0
    #                  AND 전·후반 부호 일치 AND **상위 2건 제거 후에도 부호 유지**
    #   REJECTS_HYP  : 위 미충족(부호가 반대이고 CI가 0을 배제)
    #   PASS         : 구분 안 됨
    #   INSUFFICIENT : qty2p 표본 미달
    #
    # ⚠ 이상치 제거를 필수로 둔 이유 — 372차에서 "구간별 손익이 극적으로 나빠
    #   보였으나 손실의 91%가 단 2건에서 나온 착시"였다. 같은 실수를 막는다.
    # ⚠ qty>=2가 병목이다: 실측 1.3건/일(MAX_CONTRACTS=3 체제) → 20건까지 약 3주.
    #   **3주 뒤 미달이어도 min_samples를 낮추지 말 것** — 기준 완화는 사후 조작이다.
    #   그때는 채널 설계 자체를 재검토한다([8] KellySkip과 같은 상태).
    "exit_fill_slippage_qty": {
        "enabled": True,
        "min_samples": 60,          # 하드스톱 청산 레그
        "min_samples_qty2p": 20,    # 그중 qty>=2 — 실질 병목
        "min_days": 10,
        "alpha": 0.05,
        "outlier_drop_top": 2,      # 상위 2건 제거 후 부호 유지 확인
        "data_start": "2026-08-11", # 수량 기록 개시일(이전 행은 trades 조인 폴백)
        "pre_observed": True,       # 위 정직성 고지 — 리포트가 표기한다
    },
    ...
    "tp1_protect_offset_shadow": {
        ...
        # [MW0601 489차 / B-2 (a)재등록] 키 이름의 `_per_pc` 는 이제 **이 갈래 기준**
        # 으로 읽는다(멀티PC 폐기). 수치 70/13은 관측 전 확정값이라 무변경이며,
        # 키 이름은 이력 식별자라 바꾸지 않는다(461차 `mdd_pct` 교훈 — 번호·이름은
        # 표기 층이고 정본은 값이다). 도달해도 자동 적용이 아니다.
        "watch_scope": "this_branch",   # 종전 "each_pc"
        "watch_min_hooks_per_pc": 70,   # 보호전환 훅 수 (이 갈래 기준)
    ...
    # ── [MW0601 456차 / Wave 4 · F10-B] C→A 승격 정렬강도 하위 축 — 사전등록 ────
    # [36]이 이미 **경로별**(X→A / C→A / C→C) 우열을 판정 중이다. 여기서 더하는 것은
    # **C→A 내부의 pass_count 축** 하나뿐이다 — "승격에 정렬강도 하한을 둘 것인가".
    #
    # ⚠ **원안 폐기 기록** — 311차 방법론(`pass=6` vs `pass>=7`)을 그대로 옮기려
    #   했으나 실측 6거래일 42건이 **전부 pass>=7**이었다(7:2 / 8:7 / 9:16 / 10:12 / 11:5).
    #   `ENTRY_GRADE["A"]["min_pass"]=6`이라 pass=6 A급이 구조적으로 불가능하진
    #   않지만 42건 중 0건이다. 그대로 등록하면 처리군이 영원히 비어 **판정 불가
    #   채널**이 된다([8] KellySkip 함정). → **연속형 rho를 주지표로** 바꿨다.
    #
    # 연속형이 통계적으로 효율적이고 **임의 분할점을 만들지 않는다.** 정책 질문
    # ("하한을 둘 것인가")은 *"pass_count가 정보를 담는가"* 로 먼저 답해야 하며,
    # rho ≈ 0이면 **어떤 하한도 정당화되지 않는다.**
    #
    # 판정 (관측 전 고정):
    #   SUPPORTS_HYP : rho(pass_count, +15m 방향부호수익) > 0 AND 일자단위 t > 2.26
    #                  AND 전·후반 부호 일치 → 승격 하한 설계를 주간회의 안건으로
    #   REJECTS_HYP  : rho < 0 이고 유의 → 하한이 오히려 해롭다
    #   PASS         : 유의하지 않음 → **311차 가드를 C로 확장하지 않는다**
    #   INSUFFICIENT : 표본 미달
    #
    # +15m 호라이즌은 311차와 **동일하게 유지**한다(비교 가능성).
    # t>2.26 = 일자단위 df=9 양측 5%. 신호단위 풀링은 쓰지 않는다(372차 — 하루 수백
    # 개 상관된 분봉이 표본수를 부풀린다).
    "grade_promotion_passcount": {
        "enabled": True,
        "min_samples": 40,             # C→A 승격 포지션 (실측 7건/일 → 약 2주)
        "min_days": 10,
        "min_samples_per_bucket": 20,  # 보조 버킷(pass<=8 vs >=9) 판정 조건
        "bucket_split": 9,             # pass>=9 를 '높음'으로 — 관측 분포의 중앙 근방
        "t_threshold": 2.26,
        "data_start": "2026-08-11",    # checklist_pass_count 기록 개시일
    },
    "grade_x_promotion": {
        "entry_source": "SYSTEM_AUTO",
    ...
    "cal_guard_flap_watch": {
        ...
        # [MW0601 489차 / B-2 (a)재등록] 종전 트리거는 *"**양 PC 각각** 아래 두 조건을
        # 2주 연속 충족"* 이었다. 2026-08-23 멀티PC 정책 폐기로 대조 상대가 사라져
        # **충족 불가능한 트리거**가 됐으므로 **이 갈래 단독 기준**으로 재등록한다.
        # 임계값(0.10 / 4.0 / 2주)은 **관측 전 확정된 값이라 바꾸지 않는다** —
        # 바뀐 것은 "누가 충족해야 하는가"뿐이다(§9-4 합격선 무변경).
        # ⚠ 'PC별 분기 배포 금지' 조항은 배포 단위가 하나가 됐으므로 자동 소멸.
        "action_trigger_conf_jump_median": 0.10,
        "action_trigger_flip_per_day": 4.0,
        "action_trigger_consecutive_weeks": 2,
        "action_trigger_scope": "this_branch",   # 종전 "both_pcs"
    ...
    # ── [MW0601 456차 / Wave 4 · F4-A] TP1 보호전환 조기절단 하위 축 — 사전등록 ──
    # 가설: 현행 보호스톱 `TP1_PROTECT_ATR_LOCK_MULT=0.25`가 TP1 도달 직후 이익을
    # 조기 절단한다. 2026-08-10 관측 — TP1 터치 8건 중 **7건이 4~25초(평균 13초)**
    # 내 보호스톱 청산(실현 +0.07~+0.86pt). 살아남은 1건이 TP2 +4.58pt로
    # **스크래치 7건 합계와 맞먹었다.** 표본 1일 — 확정 결론 아님(313차).
    #
    # ⚠ **`would_be_tp2` 재구성을 쓰지 않는다** — 설계 중 두 가지가 막았다.
    #   ① 틱이 영속화되지 않는다(틱 테이블 없음, 로그뿐).
    #   ② 봉 재생으로 대체하면 [47] 게이트가 밝힌 결함을 탄다 — 그 재생기는
    #      TP3·4단계 트레일링 미모델링으로 **러너를 건당 2.61pt 과소평가**하며,
    #      이는 *"러너를 자르는 대안을 유리하게 보이게"* 한다. 이 축의 가설은
    #      정반대(**자르지 말았어야 했다**)라 **가설에 불리한 계통 편향**이 된다.
    #   → 청산정책을 모델링하지 않는 **MFE 관측**으로 대체한다. raw_candles의
    #     H/L만 쓰므로 그 결함 경로를 타지 않는다. **[47] SUSPEND 대상이 아니다.**
    #
    # ⚠ **MFE는 상한이다.** SUPPORTS_HYP가 나와도 "그만큼 벌 수 있었다"가 **아니라**
    #   "포착할 여지가 있었다"는 **탐지 신호**다. **기회 크기 추정에 쓰지 말 것** —
    #   실제 회수 가능액은 Phase B의 A/B에서만 답할 수 있다.
    #
    # 모집단: `synthetic_partial_exits`(arm_ts 기록, 436차 이후) ⋈ `trades`.
    #   즉 **1계약 보호전환 경로**다. 다계약 TP1 부분청산은 [12] tp1_trail_shadow가
    #   담당하므로 여기서 다루지 않는다(범위를 명시해 중복을 피한다).
    #
    # 판정 (관측 전 고정):
    #   SUPPORTS_HYP : 일자단위 평균 forgone_mfe_pts > 0.5 AND t > 2.26
    #                  AND 전·후반 부호 일치 → offset 재설계를 주간회의 안건으로
    #   PASS         : 그 외 → 현행 유지
    #   INSUFFICIENT : 표본 미달
    #
    # 0.5pt 근거: 08-10 스크래치 실현 평균이 **+0.44pt**였다. 0.5pt는 *"남긴 것이 번
    # 것만큼 크다"* 가 되는 지점이며, 왕복비용(0.0716×2=0.143pt) 대비 3.5배라
    # 비용에 묻히지 않는다. 사후에 고른 값이 아니라 **현행 실현액과의 동등선**이다.
    "tp1_protect_forgone_mfe": {
        "enabled": True,
        "min_samples": 40,        # TP1 보호전환 포지션 (실측 ~9.5건/일 → 약 2주)
        "min_days": 10,
        "forgone_pt_threshold": 0.5,
        "t_threshold": 2.26,      # 일자단위 df=9 양측 5%
        "data_start": "2026-08-07",   # arm_ts 신설일(436차) — 이전은 소급 불가
    },
    ...
    # ── [MW0601 457차 / G7] 라우터 × 모델 건강도 — 사전등록 (섀도 전용) ─────────
    #
    # 가설: `select_entry_horizon()`이 **ATR 밴드 단독**으로 호라이즌을 고르기 때문에,
    #   그 호라이즌의 모델이 병들어 있어도(ConstOut 활성 / CV 미검증 배포본) 그대로
    #   물량을 보낸다. 2026-08-11 실측 — 라우터가 **3m을 82%** 선택했는데 그 3m이
    #   하루 6회 ConstOut으로 앙상블에서 퇴출됐고 EOD 교체는 7거래일 막혀 있었다.
    #
    # ⚠ **그런데 그날은 아무것도 안 물렸다.** ConstOut 활성 구간과 진입 시도 시각의
    #   교집합이 **0건**이었다(13:24 진입은 13:23:01 해소 59초 뒤). 그래서 조치가
    #   아니라 섀도로 시작한다 — "라우터가 병든 호라이즌을 고르는 일이 실제로 얼마나
    #   자주 일어나는가"를 먼저 센다. 효과 미확인 상태로 진입 분포를 바꾸지 않는다(313차).
    #
    # ⚠ ①(ConstOut 제외)은 사실상 무료다 — ConstOut은 **이미 그 호라이즌을 앙상블에서
    #   빼고 있는데** 라우터만 그 사실을 모른다. 그래도 섀도를 먼저 두는 이유는
    #   "무해함"과 "이로움"이 다르기 때문이다. 빈도가 0에 가까우면 코드를 늘릴 이유가 없다.
    #
    # ⚠ ②(cv_acc=null 감점)는 **457차 C1 이후에만 신뢰할 수 있다.** F6 사이드카가
    #   프로덕션에서 처음으로 정상 동작하기 시작했으므로, 그 값이 안정되는 것을 본 뒤에
    #   판정한다. 그전 구간의 chosen_cv_null은 참고치일 뿐이다.
    #
    # 판정 (관측 전 고정):
    #   SUPPORTS_HYP : 진입까지 간 건 중 chosen_degraded 비율 > 10% AND 그 건들의
    #                  포지션 단위 평균 손익이 나머지보다 낮음 → 라우터 결합을 주간회의로
    #   REJECTS_HYP  : chosen_degraded 비율 < 2% → 빈도가 없다. 코드 추가 불필요
    #   PASS/INSUFFICIENT : 그 외 / 표본 미달
    #
    # 임계 근거(사후 조정 금지):
    #   · 10% — 진입 10건 중 1건. 이보다 드물면 정책을 바꿔도 손익에 안 보인다.
    #   · 2%  — 08-11 실측 0/5건(0%)의 상한 근방. 이 아래면 "실질 0"으로 읽는다.
    #   · min_samples 60 — 진입 실측 ~5건/일 → 약 12거래일
    "router_health_shadow": {
        "enabled": True,
        "min_samples": 60,          # 진입까지 간 건수
        "min_days": 10,
        "degraded_rate_supports": 0.10,
        "degraded_rate_rejects": 0.02,
        "data_start": "2026-08-12",
    },
    # ── [MW0601 457차 / G4] 방향 편향 상시 감시 — 사전등록 ──────────────────────
    #
    # 계기: 2026-08-11에 시장이 +2.9% 오르는 동안 앙상블이 SHORT를 LONG의 **8배**
    #   냈는데(152:19), **그 편향을 잡아낸 감시자가 하나도 없었다.**
    ...
    #
    # 이 채널은 A1 진단 하네스(`scripts/direction_bias_diagnosis.py`)의 상시화 버전이다.
    # 관측치 = (예측 UP비 − DN비) − (실현 UP비 − DN비), 일자단위.
    #   0이면 시장을 그대로 따라간 것. 음수로 유의하면 시장 대비 SHORT 초과 발행.
    #
    # ⚠ **일자단위로만 본다.** 372차가 확인했듯 신호단위 n은 독립이 아니다 —
    #   같은 분의 6개 호라이즌, 인접 분끼리 전부 상관돼 있다. 신호단위로 세면
    #   p값이 무한정 작아져 아무 편향이나 "유의"해진다.
    #
    # ⚠ **역방향 적중률을 함께 낸다.** A1 실측에서 예측을 뒤집으면 적중률이
    #   **떨어졌다**(3m 0.347→0.337, 5m 0.348→0.340). 즉 편향은 있는데 역방향
    #   알파는 없다. 이 값을 판정문에 같이 찍어, 다음 세션이 "SHORT 편향이니
    #   뒤집자"는 잘못된 처방으로 가는 것을 구조적으로 막는다.
    #
    # 판정 (관측 전 고정):
    #   ALERT_BIAS : Bonferroni 생존 호라이즌이 2개 이상 AND 부호가 전부 같음
    #                → 절편 보정 축을 주간회의 안건으로 (방향 반전 아님)
    #   PASS       : 그 외
    #   INSUFFICIENT : 거래일 미달
    #
    # 임계 근거(사후 조정 금지):
    #   · min_days 20  — 일자단위 t검정의 최소 표본. 313차 "단일일·소수일 판정 금지"
    #   · t 2.09       — df=19 양측 5%
    #   · 2개 호라이즌 — 1개는 단일 호라이즌 특이일 수 있다. 2개 이상이 같은 부호로
    #                    생존해야 "체계적"이라 부를 수 있다. A1 실측에서 3m·5m가
    #                    정확히 그 상태였다(t=−5.32 / −4.90).
    #   · Bonferroni m=호라이즌 수 — 455차 L1 스크리닝과 동일 관례
    "direction_bias_watch": {
        "enabled": True,
        "min_days": 20,
        "min_day_rows": 10,        # 그날 그 호라이즌 채점 표본 하한 (재기동일 방어)
        "t_threshold": 2.09,
        "alert_min_horizons": 2,
        "alpha": 0.05,
    ...
    #
    # 채널 번호 **[51]**. ⚠ dev 브랜치는 이 번호가 462차 저변동성 채널과 충돌해
    #   2026-08-23 MW0602 487차(F-9)가 [54]로 재배정했지만, v9-dev에는 그 462차
    #   채널이 없어 충돌이 없다 — 번호 체계는 각자 관리한다(멀티PC 정책 폐기, 487차).
    # [2026-08-23 487차 F-8(B)] 리포트 생성기가 이 채널의 생산부
    #   (`scaler_daily.const_out_by_horizon` 컬럼, 457차 G5) 존재를 감지해, 없는
    #   브랜치에서는 **NOT_AVAILABLE_ON_THIS_BRANCH**로 표기한다(dev 쪽 0821
    #   이상점 1-17 대응). v9-dev는 컬럼이 있으므로 상시 정상 경로를 탄다.
    #
    ...
    #   그 문서는 *"TP1 33% 부분청산이 레그 수를 2배로 만들어 수수료를 2배화한다"* 고
    #   썼으나 **거짓이다.** 수수료 공식은
    #     `entry_price × quantity × pt_value × RATE × 2`  (utils/db_utils.py:101)
    #   로 **레그 수가 아니라 계약수에만 비례**한다. 2026-08-11 실측 대조:
    #     실제 6레그 = 8,760원 / 부분청산 없이 3레그(qty=2) 가정 = 8,760원 → **차이 0원**
    #   → 부분청산은 비용 축에서 무해하다. 이 채널은 부분청산을 재지 않는다.
    #
    # **진짜 문제는 회전율 대비 엣지 부족이다.**
    #   왕복 손익분기 = 진입가 × RATE × 2 ≈ 966 × 0.000015 × 2 = **0.0290 pt/계약**.
    #   2026-08-11: gross +0.10pt를 6계약-청산으로 나누면 **0.017pt/계약** — 손익분기
    #   미만이다. 그래서 승률 66.7%(2승1패)인데도 순손익이 **-3,760원**이 됐다.
    #   같은 패턴이 반복된다: 08-10 8승2패 -244,067원 / 08-03 15승9패 -730,619원.
    #
    # 가설: **현행 게이트 어디에도 "이 진입의 기대 이동폭이 왕복 비용을 넘는가"를
    #   묻는 축이 없다.** MetaGate·ToxicityGate·EntryChecklist는 방향과 품질만 본다.
    #   그런데 `quantile_expected_pt`(MetaGate 분위회귀)가 **이미 기대 이동폭을 pt로
    #   내고 있다** — 계산은 하는데 비용과 대조하지 않는다.
    #
    # 이 채널이 재는 것 (둘 다 실체결가만 사용 — 재생기·시뮬레이터 금지):
    #   축1 **비용 커버율** : 포지션별 gross_pt/계약이 왕복비용을 넘는 비율
    #   축2 **사전 예측력** : 진입 시점 `quantile_expected_pt`가 축1을 예측하는가
    #                        (일자단위 스피어만 + 상·하위 3분위 커버율 격차)
    #
    # ⚠ 축2가 핵심이다. 축1만으로는 "비용이 크다"는 사후 서술에 그친다. 게이트로
    #   승격하려면 **진입 전에 알 수 있어야** 하고, 그것을 재는 것이 축2다.
    #   축2가 무정보(rho≈0)면 처방은 게이트 신설이 아니라 **분위회귀 재설계**다.
    #
    # 판정 (관측 전 고정):
    #   SUPPORTS_HYP : 축2 일자단위 rho > 0.20 AND |t| > 2.26 AND 상·하위 3분위
    #                  커버율 격차 > 15%p → cost_edge 게이트를 주간회의 안건으로
    #   REJECTS_HYP  : rho 95% CI가 0을 포함 AND 격차 < 5%p → 분위회귀 재설계로 전환
    #   PASS/INSUFFICIENT : 그 외 / 표본 미달
    #
    # 임계 근거(사후 조정 금지):
    #   · rho 0.20 — 이 프로젝트의 IC 스크리닝(455차 L1)이 쓰는 "약한 신호" 하한과 동일
    #   · t 2.26   — 일자단위 df=9 양측 5% ([1] tp1_protect_forgone_mfe와 동일 관례)
    #   · 15%p     — 왕복비용 0.029pt는 실측 포지션 gross 중앙값의 약 1/3 규모다.
    #                상·하위군이 그보다 작게 갈리면 게이트를 세워도 컷이 서지 않는다.
    #   · 5%p      — 위의 1/3. 이보다 작으면 표본을 더 모아도 실무 의미가 없다.
    #
    # ⚠ **부분청산 손익 기하는 이 채널의 범위가 아니다.** "TP1을 안 받았다면"은
    #   보호스톱 전환 여부까지 바뀌므로 실체결가만으로 재구성할 수 없고, 봉 재생을
    #   쓰면 [47]이 밝힌 러너 과소평가(건당 2.61pt) 편향을 탄다. 그쪽은
    #   [1] tp1_protect_forgone_mfe(456차)가 MFE 관측으로 이미 담당한다.
    "cost_edge_watch": {
        "enabled": True,
        "entry_source": "SYSTEM_AUTO",   # 수동 진입 제외
        "min_samples": 40,               # 포지션 단위 (실측 ~3~10건/일 → 약 2주)
        "min_days": 10,                  # 313차 — 단일일·소수일 판정 금지
        "rho_threshold": 0.20,
        "t_threshold": 2.26,
        "tercile_gap_pp_supports": 15.0,
        "tercile_gap_pp_rejects": 5.0,
        "alpha": 0.05,
        # 왕복 수수료율 — 여기 복사하지 말 것.
        # ⚠ [493차 / F-3] 캠페인·섀도 계측은 **COST_MODEL_COMMISSION_RATE**를
        #   import한다(라이브 FUTURES_COMMISSION_RATE 아님). 현재 구 요율에 핀돼
        #   있으며 실제 대비 6.54배 과소다 — 해제는 주간회의 승인 사항.
        "data_start": "2026-08-12",      # 사전등록일 다음 거래일부터 수집
    },
    ...
    # ── [MW0602 502차 U-2 체리픽] [57] trend_efficiency 진입 게이트 — 사전등록 ─────
    # 🔴 **결과를 보기 전에 박는다.** 임계 0.32는 MW0602 501차 후속2가 시간분할·LODO로
    # 고정한 값이며, **판정이 불리하게 나와도 사후에 완화·강화하지 말 것**(§9-4 검증
    # 시계 리셋 대상). v9-dev 자체 데이터로는 아직 이 임계를 검증한 적이 없다 —
    # 섀도로 독립 관측을 쌓고, 필요하면 v9-dev 자체 재검증을 별도로 수행할 것.
    #
    # **무엇을 묻나**: "최근 10분 이동거리 중 방향으로 누적된 비율(Kaufman ER)이 낮은
    # 분(minute)에 진입을 막으면 이득인가." MW0602 501차 후속2 §3의 4중 검증(LODO 31폴드
    # 전 폴드 임계 0.32 동일 · 시간분할 검증구간 +1,518,929원 · 부트스트랩 95%CI
    # [+333,161, +6,416,854] · 임계 민감도 0.26~0.36 전 구간 양수) — 전부 MW0602
    # 자기 데이터 기준이며 v9-dev에는 참고값일 뿐이다.
    #
    # ⚠ **판정 시 병기 의무 — 숨기지 말 것**: MW0602 사전 실측에서 이 규칙이 손해인
    #   날이 7일, 합계 −1,937,953원이었다. 평균은 양수지만 분산이 크다.
    # ⚠ **"손실일 탐지기"가 아니다** — 나쁜 *날*이 아니라 나쁜 *분*을 거른다.
    # 🔴 **승격은 `faststop_discovery`(2026-08-03) 부분 철회를 요구한다** —
    #   "신규 진입 차단 게이트 개발 중단"과 양립하지 않는다. 섀도 기록은 계측이라
    #   충돌하지 않는다.
    "trend_efficiency_entry_gate": {
        # 배선 다음 거래일부터가 판정 창. 그 이전은 **가설을 만든 구간**이라
        # 승격 근거로 쓰지 않는다.
        "start_date": "2026-09-01",
        "min_samples": 20,       # 타 채널과 동일 — 관측에서 역산하지 않았다
        "threshold": 0.32,       # 🔴 고정. 사후 변경 금지 (MW0602 501차 후속2 산출값)
        ...
        "require_ready": True,
        # 합격선 앵커 — hurst_gate_shadow와 동일 규약:
    ...
    # ── [MW0601 529차 / A·B] 레그 위치(탈진 위치) 순방향 진입 — 사전등록 ─────────────
    #
    # 무엇을 묻는가: "이미 크게 달린 레그의 끝(극단 근처)에서 그 방향으로 들어가는 진입"이
    #   손익을 깎는가. 2026-09-03 C1~C4가 전부 이 형태였고(528차), 사용자가 "탈진 위치에서
    #   추세 방향 진입"으로 지목했다.
    #
    # 왜 새 축인가: 기존 장치는 이 축을 못 본다 —
    #   · `10_chase`는 **10분 전 종가 한 점** 대비라 V자 되돌림 뒤 연장을 놓친다
    #     (C1: ext_10m 1.33 / ext_60m 0.28 인데 스윙 저점 대비는 7.07 ATR).
    #   · `RegimeExhaustionGate`(60분)는 `hurst<0.45` 전제에 걸린다(C2가 0.537로 탈락).
    #   · `3_vwap`은 VWAP 대비 위치만, `TrendGate`는 지속 봉 수만 본다.
    #   529차가 `swing_*`/`dist_to_*`/`bars_since_*` 피처를 신설해 이 축을 처음 계측한다.
    #
    # 🔴 두 장치는 건드리지 않는다(529차 실측):
    #   · `3_vwap` 순방향 이탈 ≥1.5σ 44건은 통산 −9k(중립)인데, 그중 **레그 초입 15건이
    #     +675,319원(12승 3패)** 이고 레그 끝 16건만 −96,093원이다. 이탈 크기가 아니라
    #     레그 위치가 축이므로, 3_vwap에 상한 감점을 붙이면 최상위 승리군을 함께 깎는다.
    #   · `TrendGate` 완화는 실제 인하 547회 중 대부분이 점심 진입금지(0.62)를 0.44로 여는
    #     것이었고, 완화 없이는 불가했던 진입 5건이 **5승 +279,000원**. streak ON 진입 88건
    #     통산 +800,120원. 완화 자체는 이익이다 → 결합 조건만 아래 `streak_leg_end_watch`로 분리.
    #
    # 모집단: SYSTEM_AUTO 포지션(entry_ts 병합, 계측 4원칙 ①) 중 **순방향** 진입.
    ...
    #   처리군 = run >= run_atr_min AND dist <= dist_atr_max AND swing_ready
    # 값의 원천: 529차 배선 이후 분은 `ensemble_decisions.features`의 스윙 키,
    #   그 이전은 `raw_candles` 재계산 프록시(`source="replay_proxy"`로 표기, 판정에는 함께 쓴다 —
    #   같은 산식이고 봉 데이터가 원본이라 사본 오차가 없다).
    #
    # ⚠ 컷(5.0 / 1.0)은 **관측 전에 고정**한다. 소급 스캔에서 run>=8·dist<=1.5가 더 좋아
    #   보였으나(11/5 p=0.21) 그것을 채택하면 데이터를 본 뒤 컷을 고르는 것이다(313차 ④).
    # ⚠ 소급 실측(179건 / 7주): 처리군 48건(17일) 25승23패 −1,232,787원, 사이즈 ½ 가정 시
    #   +616,394원. 그러나 **일자 부호검정 9/8 p=1.00** — 손실이 며칠에 집중돼 있어 채택 근거가
    #   아니다. 이 채널은 그 판정을 자동화하려고 만든다.
    "leg_exhaustion_entry_watch": {
        "enabled": True,
        "entry_source": "SYSTEM_AUTO",
        # 배선일 — 이후 분은 DB 스윙 키, 이전은 replay_proxy
        "feature_wired_date": "2026-09-04",
        "data_start": "2026-07-14",     # 사이징 체제 변경(431차) 이후 구간만
        "lookback_min": 60,             # SWING_FEATURE_LOOKBACKS_MIN 과 일치해야 한다
        # 🔴 처리군 컷 — 고정. 사후 변경 금지.
        "run_atr_min": 5.0,
        "dist_atr_max": 1.0,
        # 사전등록 합격선
        "min_samples": 40,              # 처리군 포지션 수 (소급 48 — 이미 충족이나 min_days가 남는다)
        "min_days": 25,                 # 처리군 발생 거래일 (소급 17 → 약 8거래일 더 필요)
        "alpha": 0.05,                  # 일자단위 부호검정(양측)
        "drop_worst_days": 3,           # 313차 ③ — 최악 3일 제거 후에도 처리군 열세 유지 요구
        # 🔴 거울상 보존 조건 — 이 채널의 고유 안전장치.
        #   레그 **초입**(run < run_atr_min) 순방향 진입은 소급 실측에서 승리군이다
        #   (초입 12건 10승2패 +532,588 / VWAP 이탈≥1.5σ 且 초입 15건 12승3패 +675,319).
        #   처리군을 누르는 조치가 초입군까지 함께 누르면 순이익이 사라진다. 그래서
        #   "초입군 평균 손익이 판정 시점에도 처리군보다 높다"를 통과 조건에 넣는다.
        "require_early_group_better": True,
        # 승격 형태 — [B] 판정 **전에** 확정한다(데이터를 본 뒤 형태를 고르지 않기 위해).
        #   1순위 `checklist_demote`: 체크리스트 `12_leg_position` 신설, pass_count-1 (10_chase 동형).
        #   2순위 `size_half`: 사이저 품질군 배수 ×0.5 (min 합성에 편입, 431차 방식).
        # 🔴 `block`(하드차단)은 **금지**한다 — 317차 FalseBlock 교훈(Hurst 하드차단이 진짜 추세
        #   분봉의 72.3%를 오판)이 그대로 적용되고, 처리군도 승률 52%(25승23패)로 절반은 이긴다
        #   (2026-09-03 C3가 그 절반 — 같은 위치에서 +160,424원).
        "promotion_order": ["checklist_demote", "size_half"],
        "hard_block_forbidden": True,
        "size_half_mult": 0.5,          # size_half 채택 시 배수
    },
    # ── [MW0601 540차 / GP-1·GP-2] GOLDEN POWER 교차 — 사전등록 2채널 ──────────────
    #
    # 사용자 관찰(2026-09-07 차트): 「GB 가 0.5 를 상향 돌파하고 GS 는 0 에 수렴하는 순간에
    # 진입하면 수익 청산 확률이 높다」. 실측으로 **관찰은 사실**이다 — TP 선도달 75.6% vs
    # 무작위 73.2% (z=+2.01, p=0.044).
    #
    # 🔴 **그러나 그 격자의 손익분기 승률이 정확히 75.0%다**(스톱 1.5ATR·TP 0.5ATR →
    #    1.5/(1.5+0.5)). 승률이 높아 보이는 것의 대부분은 스톱이 TP 의 3배라는 **격자 산술**이고,
    #    신호가 손익분기를 넘는 몫은 +0.6%p 뿐이다. 청산 격자 19개(스톱 0.5~3.0 × TP 0.5~2.0)를
    #    전수 탐색해도 **net 흑자는 0개**다(최선 −6,141원/건). 「승률」과 「기댓값」을 반드시
    #    나눠 볼 것 — 승률 75%는 이 격자에서 **본전**이다.
    #
    # 신호에 정보는 있다: 무작위 대비 gross 엣지 **+3,026원/건**. 문제는 **왕복 비용이 그
    # 2.5배**(7,596원)라는 것이다. 비용은 원화 고정(종가×9.81원)인데 TP 는 ATR 비례라
    # **상대 변동성이 곧 수수료 부담률**이다 — 6bp 면 수수료가 TP 의 65%, 16bp 면 24.5%.
    # 그래서 게이트 축을 `atr_bp`(= atr/종가×10000)로 잡는다.
    #
    # 두 채널을 **함께** 등록하는 이유: 「반대편이 0 에 수렴」 조건이 실제로 일하는지를
    # 대조로 가르기 위해서다. 소급상 그 조건이 사실상 전부를 설명한다(아래 수치).
    # ⚠ **컷(0.5 교차 · eps 0.05 · 16.0 bp)은 관측 전에 고정한다. 사후 변경 금지**(313차 ④).
    # ⚠ 판정에는 **전향 표본만** 쓴다. 소급 116건은 가설의 출처이지 근거가 아니다(SOP §11 K-1).
    #    그래서 `min_samples` 를 소급값보다 낮은 60 으로 잡아도 「소급으로 이미 충족」이 되지 않는다.
    #
    # 근거 문서: `docs/미륵이고도화3/Golden power/GP교차신호_검증_MW0601-20260907.md`
    #   (§3 승률·손익분기 · §4 청산격자 전수 · §6 상대ATR · §8-A 확증필터 불가)
    #
    # ── GP-1: 좁은 교차(반대편 ≤ eps) × 고변동 ─────────────────────────────────────
    # 소급(2025-08-19~2026-09-07, 257거래일, 스톱1.5/TP0.5, 실측요율):
    #   n=116 / 72일 · 건당 **+6,513원** · 일자 54/18 **p=0.0000** · TP선도달 83.6%
    #   최선 3일 제거 후 **+286,951원**(부호 유지, 313차 ③ 통과)
    #   같은 고변동 구간 대조: 순수교차 −8,548 / **무작위 −14,815** → 레짐 효과가 아니다
    # ⚠ **전·후반 분해가 경계에 민감하다** — 5/1 경계면 전반 +10,685(17/3) 인데
    #    5/15 경계면 전반 **−13,343**(35건, 20/6)로 뒤집힌다. 2026-05 한 달이 −607,521 이다.
    #    「매달 번다」가 아니며, 이 불안정성이 이 채널을 승격 전에 전향 검증해야 하는 이유다.
    "gp_cross_highvol_watch": {
        "enabled": True,
        "entry_source": "SIGNAL",        # 실체결이 아니라 **신호 단위**(미륵이 진입과 0.4%만 겹친다)
        "feature_wired_date": "2026-09-07",
        "data_start": "2026-09-08",      # 🔴 전향 전용 — 배선 다음 거래일부터 센다
        # 🔴 신호 정의 — 고정
        "period": 20,                    # GP_CROSS_PERIOD 와 일치해야 한다
        "cross_level": 0.5,              # GP_CROSS_LEVEL
        "tight_eps": 0.05,               # GP_CROSS_TIGHT_EPS — 반대편 「0 수렴」
        "require_tight": True,           # ← GP-2 와 갈리는 유일한 조건
        # 🔴 게이트 — 고정
        ...
        "min_days": 40,
        "alpha": 0.05,                   # 일자단위 부호검정(양측)
        "drop_best_days": 3,             # 🔴 최선 3일 제거 후에도 흑자 유지 요구(313차 ③ 역방향)
        # 🔴 대조군 동반 요구 — 이 채널 고유 안전장치.
        #   같은 고변동 구간의 무작위 진입이 함께 흑자면 그것은 레짐 효과이지 신호가 아니다.
        "require_beat_random_control": True,
        "control_min_samples": 200,
        # 승격 형태 — 판정 **전에** 확정한다.
        "promotion_order": ["record_only", "shadow", "size_quarter"],
        "hard_block_forbidden": True,
        "size_quarter_mult": 0.25,
        # 비용 — 핀값 금지(493차)
        "cost_source": "BROKER_CHANNEL_SPECS",
    },
    # ── GP-2: 순수 교차(반대편 무관) × 고변동 — **대조 채널** ──────────────────────
    # 같은 게이트·같은 청산으로 「반대편 ≤0.05」 조건만 뺀다.
    # 소급: n=2,849 / 172일 · 건당 **−8,548원** · 일자 75/97 p=0.109 · 최선3일제거 −29,896,243
    #   그중 GP-1 이 아닌 것(반대편 >0.05)만: n=2,755 · 건당 **−8,992원**
    #   ⇒ 소급상 **「반대편 0 수렴」 조건이 사실상 전부를 설명한다.**
    # 이 채널의 역할은 그 대조를 **전향으로도** 확인하는 것이다. 단독 승격 후보가 아니다.
    # ⚠ 빈도가 GP-1 의 25배(11.09건/일 vs 0.45건/일)라 표본은 훨씬 빨리 찬다 —
    #    먼저 차는 쪽이 먼저 판정된다고 해서 그쪽을 채택하지 말 것.
    "gp_cross_any_watch": {
        "enabled": True,
        "entry_source": "SIGNAL",
        "feature_wired_date": "2026-09-07",
        "data_start": "2026-09-08",
        "period": 20,
        "cross_level": 0.5,
        "tight_eps": 0.05,
        ...
        "require_beat_random_control": True,
        "control_min_samples": 200,
        # 🔴 승격 후보가 아니다 — 대조 전용. GP-1 판정의 해석에만 쓴다.
        "promotion_order": ["record_only"],
        "hard_block_forbidden": True,
        "role": "control_for_gp_cross_highvol_watch",
        "cost_source": "BROKER_CHANNEL_SPECS",
    },
    # ── [MW0601 553차 / GP-R1·GP-R2] GOLDEN POWER **규칙 병행운용(섀도)** — 사전등록 2채널 ──
    #
    # 🔴 **위 [GP-1·GP-2] `gp_cross_*` 와 다른 물건이다. 헷갈리면 안 된다.**
    #   · `gp_cross_*`  = **신호 단위**. 교차 시점의 ATR 격자(스톱1.5/TP0.5·60봉) 가상시뮬.
    #   · 이 두 채널   = **규칙 단위**. 진입·보유·청산이 전부 다른 **완결된 매매규칙**을
    #                    ChallengerEngine 섀도로 돌려 포지션 손익을 적립한다.
    #   같은 GP 피처를 쓰지만 **묻는 질문이 다르다** — 교차에 정보가 있는가(전자) vs
    #   이 규칙이 돈을 버는가(후자). 표본도 판정도 섞지 말 것.
    #
    # 출처: `docs/미륵이고도화3/Golden power/검증완료_진입청산규칙_정리.md` (2026-09-09)
    # 검토: `docs/미륵이고도화3/Golden power/GP병행운용_구현가능성_검토_MW0601-20260910.md`
    #
    # ── 🔴 원문서 수치를 그대로 인용하지 말 것 — 정정 4건 ────────────────────────────
    # ① **원화가 5배 과대**다. 원문서는 `1pt=250,000원`(정규선물)으로 환산했으나 운영
    #    종목은 **A0569000 미니선물 = 50,000원/pt**(`MINI_FUTURES_PT_VALUE`).
    #    롱 108,124,994원 → **21,625,000원**, 숏 18,762,534원 → **3,755,000원**.
    #    ※ 같은 폴더의 `analysis/gp_lib.py:PT_VALUE = 50_000` 은 맞다 — 요약문서만 틀렸다.
    # ② **비용 가정 0.15pt 는 채널을 안 나눈 값**이다 → 아래 `cost` 블록에서 분리한다.
    # ③ **데이터 원천 표기가 틀렸다.** 원문서 "session_bars 211거래일"인데 그 테이블은
    #    533차 풀타임 수집 시작(2026-09-07)이라 **3거래일**뿐이다. 실제 원천은
    #    `raw_candles`(2025-08-19~, 259거래일)이고 추출기도 그렇다(`analysis/gp_extract.py:64`).
    # ④ **기간 파라미터는 n=20**이다. `GOLDEN_POWER_구현명세.md`가 HTS 대조한 것은 n=25이며
    #    `golden_power.py` 기본값도 25다. n=20 은 HTS 대조를 거치지 않았다 —
    #    라이브는 `GP_CROSS_PERIOD=20` 을 쓰므로 일치하나, 25로 착각하면 다른 규칙이 된다.
    #
    # ── 🔴 왕복비용을 CYBOS·CREON 으로 분리한다 [2026-09-10 사용자 지시] ───────────────
    # 대신증권은 로그인 매체별로 요율을 고시한다(495차). 이 규칙들은 **거래 빈도가 높아
    # 비용이 판정을 뒤집는다** — 롱 587거래/211일, 숏 149거래/211일.
    #
    #   왕복비용(참조가 1,050 · 미니 1계약 · 슬리피지 1틱/편도 포함)
    #     CYBOS  0.246018pt = 12,301원   (편도 0.0098104%)
    #     CREON  0.079900pt =  3,995원   (편도 0.0019%)
    #     → **3.08배**. 원문서 가정 0.15pt 는 CYBOS 기준으로 **싸고**, CREON 기준으로 **비싸다**.
    #
    #   소급 t 재산출(211거래일 · 일자단위) — 채널을 나누면 결론이 갈린다:
    #     롱  원문서 2.91 → CYBOS **2.53** / CREON **3.19**
    #     숏  원문서 2.02 → CYBOS **1.64** / CREON **2.30**
    #   ⇒ 🔴 **숏은 CYBOS 에서 단일검정 문턱 1.96 을 못 넘고 CREON 에서는 넘는다.**
    #      같은 규칙의 유의성이 **PC 마다 다르다.** 그래서 판정은 반드시 **감지된 채널의
    #      요율로** 하고, 리포트는 두 채널 값을 **함께** 찍는다(`cost.report_both`).
    #   ⚠ 위 재산출은 **소급**이며 가설의 출처다. 판정 근거가 아니다(SOP §11 K-1).
    #
    # ── 데이터 배선 상태 ─────────────────────────────────────────────────────────────
    # GB/GS 와 교차 플래그는 **이미 라이브**다(540차, 2026-09-07):
    #     `gp_buy_20`(=GB) · `gp_sell_20`(=GS) · `gp_cross_up_20`(GB 0.5 상향) ·
    #     `gp_cross_dn_20`(GS 0.5 상향) · `gp_ready_20`
    #     — `features/feature_builder.py:compute_gp_cross_features`, 세션 리셋 종가 버퍼.
    # ⚠ 원문서의 `gp_bull_20`/`gp_bear_20` 은 산문 별칭이다. **실소비 키는 위 이름**이다.
    # Phase 1 에서 **추가로 배선할 것은 `ma20`·`ma60`·`ma_ready` 뿐**이다(숏 국면필터).
    # ⚠ 교차 경계가 원문서와 미세하게 다르다 — 원문서 「직전 ≤0.5 & 현재 >0.5」,
    #   코드 `pgb < 0.5 and gb >= 0.5`. 정확히 0.5 인 봉에서만 갈리며(실질 측도 0),
    #   **코드 정의를 정본으로 고정**한다. 이후 변경 금지.
    #
    # ── 관측 규약 ────────────────────────────────────────────────────────────────────
    # ⚠ 판정에는 **전향 표본만** 쓴다. 소급 211거래일은 가설의 출처다.
    # ⚠ 재기동 직후 `gp_ready_20=False`/`ma_ready=False` 인 분은 **미측정**이지 「신호 없음」이
    #   아니다(계측 4원칙 ②). 세션 리셋 버퍼가 재기동으로 비므로 그 구간이 실제로 생긴다.
    #   `record_unready_minutes` 로 세어 리포트에 분모와 함께 남긴다.
    # ⚠ 관측 60거래일 동안 **규칙 파라미터를 건드리지 않는다**(원문서 §운용권고 1).
    #   중간 조정은 사전등록 무효다(458차 D6).
    "gp_rule_cost": {
        # 두 채널이 공유하는 비용 규약. **핀값 금지**(493차) — 채널 스펙에서 파생한다.
        "cost_source": "BROKER_CHANNEL_SPECS",
        "channel_detect": "config.constants:detect_broker_channel",
        "split_by_channel": True,        # 🔴 [사용자 지시 2026-09-10] CYBOS·CREON 분리
        "report_both": True,             # 판정은 감지 채널, 표기는 두 채널 병기
        "slip_ticks_per_side": 1.0,      # 캠페인 공통 가정과 동일(미니 1틱=0.02pt)
        "pt_value_krw": 50_000,          # MINI_FUTURES_PT_VALUE — 정규선물 250,000 아님
        "contracts": 1,                  # 사이징 없음. 규칙 자체의 성질만 잰다
        # 사전등록 시점 참조표(참조가 1,050). 판정은 **체결가로 매건 재계산**하며 이 표는
        # 대조용이다 — 값이 크게 어긋나면 요율 세대가 바뀐 것이다(26주 항목과 연동).
        "reference": {
            "price_ref": 1050.0,
            "CYBOS": {"one_way_rate": 9.8104e-05, "comm_pt": 0.206018,
                      "roundtrip_pt": 0.246018, "roundtrip_krw": 12301},
            "CREON": {"one_way_rate": 1.9e-05, "comm_pt": 0.039900,
                      "roundtrip_pt": 0.079900, "roundtrip_krw": 3995},
        },
        # 🔴 challenger 엔진의 `0.000015` 하드코딩은 **키움 잔재**다(실제의 1/6.54).
        #   Phase 2 에서 이 블록 기준으로 교체한다. 교체 전 산출된 가상손익은 무효다.
        "legacy_rate_to_replace": 1.5e-05,
    },
    # ── GP-R1: 롱 — GB 단순돌파 · 90분 보유 ─────────────────────────────────────────
    # 소급(211거래일): 587거래 · +432.5pt · 일평균 +2.050 · 승일 59.2% · 노출 61.9% · MDD 83.4pt
    #   (미니 1계약 환산 +21,625,000원 / MDD 4,170,000원 = 목표자본 5천만원 대비 8.3%)
    # 🔴 **이건 알파 전략이 아니다.** 원문서 자체 검증: α=+0.840 **t=1.46(비유의)** ·
    #   β=+0.463 **t=10.87** · 손익 432.5 = **베타 255.4** + 알파 177.1 → **59%가 시장 드리프트**.
    #   211일 시장이 +551.3pt 오른 상승장 단일 국면이고 4분할이 전부 상승 구간이다.
    #   ⇒ **총손익으로 판정하면 상승장이 자동으로 통과시킨다.** 그래서 이 채널의 1차
    #      합격선은 총손익이 아니라 **동일노출 단순보유 대조군을 이기는가**다
    #      (소급: 규칙 +432.5 vs 동일노출 무비용 보유 +341.2 → 비용 88pt 내고 91pt 앞섬).
    # ⚠ α 유의성은 **요구하지 않는다** — 소급 211일에도 t=1.46 이라 60거래일에 t≥1.96 은
    #   구조적으로 불가능하다. 달성 불가능한 문턱을 등록하는 것은 사전등록이 아니라 알리바이다.
    #   대신 **α 점추정 > 0** 과 **대조군 우위**를 건다.
    "gp_rule_long_watch": {
        "enabled": True,
        ...
        # 🔴 규칙 정의 — 고정. 관측 중 변경 금지
        "period": 20,                    # GP_CROSS_PERIOD 와 일치해야 한다
        "trigger": "gp_cross_up_20",     # GB 가 0.5 를 상향 통과
        "cross_level": 0.5,
        "entry_delay_bars": 0,           # 즉시 — 돌파 확인 다음 봉
        "hold_minutes": 90,              # 주 청산
        "session_start": "09:20",        # = 09:00 + 20봉 워밍업
        "session_end": "14:50",          # 진입 마감
        "force_exit": "15:05",           # 절대원칙 §1(15:10)보다 보수적
        "stop_loss": None,               # 별도 손절 없음 — 90분 시간손절이 대신한다
        "non_overlap": True,             # 보유 중 재신호 무시
        "max_per_day": None,             # 횟수 제한 없음
        # 🔴 **추가 필터 금지.** 원문서 기각 목록: 직전압축(GB·GS≤0.5 & 스프레드≤0.5)과
        #   반대선 0 수렴을 붙이면 신호 2,620→1,099 로 줄면서 **300pt 를 파괴**한다
        #   (90분 +432 → +131). 「개선」하고 싶어지면 그건 다른 규칙이므로 새 채널을 판다.
        "forbidden_additions": ["squeeze_prefilter", "opposite_line_convergence"],
        # 🔴 합격선 — 사전등록
        "min_samples": 100,              # 전향 포지션 수(소급 2.78건/일 → 60일 약 167건)
        "min_days": 40,
        "unit": "position",              # 계측 4원칙 ① — 레그 단위 금지
        "alpha": 0.05,
        "test": "daily_sign_test",       # 일자단위 부호검정(313차)
        "drop_best_days": 3,             # 최선 3일 제거 후에도 부호 유지(313차 ③)
        # 🔴 이 채널의 존재 이유 — 베타를 걷어낸 뒤에도 남는가
        "require_beat_exposure_matched_control": True,
        "control_kind": "exposure_matched_buy_and_hold",  # 동일노출 · 비용 0
        "require_alpha_positive": True,  # 점추정만. 유의성은 요구하지 않는다(위 주석)
        "require_beat_random_control": True,
        "control_min_samples": 200,
        # 승격 형태 — 판정 **전에** 확정한다
        "promotion_order": ["record_only", "report_only"],
        "live_order_forbidden": True,    # 실주문 승격은 주간회의 안건(절대원칙 §6)
    },
    # ── GP-R2: 숏 — 압축돌파 · 하락추세 한정 · 60분 ──────────────────────────────────
    # 소급(211거래일): 149거래 · +75.1pt · 일평균 +0.356 · 승률 55.0% · 노출 8.5% · MDD 21.7pt
    #   (미니 1계약 환산 +3,755,000원) · 평균 보유 45.3분 · 1.00회/일
    # ✅ 성격이 롱과 정반대다: α=+0.452 **t=2.56(유의)** · β=**−0.037 t=−2.81(시장과 반대)** ·
    #    무작위 800회 대비 3.02σ(백분위 100%) · 4분할 전부 + · 격자 24조합 중 22개 양수 · 14개월 중 11개월 +
    # 🔴 **그래도 다중검정 문턱을 못 넘었다** — 탐색 70개 기준 필요 t=3.38, 귀무하 기대 최대
    #    t=√(2·ln70)=2.91 인데 실측 2.02 다. 무작위 3.02σ(p≈0.0011)를 70 보정하면 0.077.
    # 🔴 **그리고 비용을 채널로 나누면 CYBOS 에서 t=1.64 로 단일검정마저 못 넘는다**(위 참조).
    #    ⇒ 이 채널은 「유의한 규칙의 확인」이 아니라 **「관측 가치가 있는 후보의 전향 관찰」**이다.
    #      리포트에 그렇게 쓸 것. PASS 가 나와도 그것은 승격 근거가 아니라 재검토 트리거다.
    # 🔴 **국면 필터(MA20<MA60)는 선택이 아니다.** MA20>MA60 구간만 떼면 −186.7pt **t=−2.71
    #    (유의한 손실)** — 필터를 빼는 순간 전략이 마이너스로 뒤집힌다. 그래서 필터 미적용
    #    대조군을 **함께 기록**해 그 비대칭이 전향으로도 재현되는지 본다.
    "gp_rule_short_watch": {
        "enabled": True,
        ...
        # 🔵 [2026-09-10 사용자 결정] **"cont"(연속) 확정.**
        #   근거 ① 규칙이 발견된 화면이 사이보스 차트이고 HTS 이동평균은 연속이다.
        #        ② 진입창 시작 09:20 이 GP 20봉 워밍업과 정확히 일치한다 — MA60 워밍업으로
        #           40분을 더 기다릴 의도였다면 10:00 이라고 썼을 것이다.
        #        ③ sess 를 고르면 진입창의 약 12%(하루 39~43분)가 구조적으로 사라진다.
        #           숏은 하루 최대 1회라 표본에 직접 영향한다.
        #   ⚠ 미확인으로 남는 것: 211거래일 백테스트 t=2.02 가 어느 쪽 기준이었는지.
        #     최종 스크립트가 repo 에 없다. 전향 표본이 소급과 갈리면 이 항목을 먼저 의심할 것.
        #   근거: dev_memory/DECISION_LOG.md 2026-09-10(553차 후속2)
        "ma_basis": "cont",
        ...
        "squeeze_gb_max": 0.5,
        "squeeze_gs_max": 0.5,
        "squeeze_spread_max": 0.5,       # |GB − GS|
        "squeeze_valid_bars": 10,        # 압축 발생 후 10봉 이내에만 트리거 유효
        # ② 트리거
        "trigger": "gp_cross_dn_20",     # GS 가 0.5 를 상향 통과
        "cross_level": 0.5,
        "trigger_gb_max": 0.10,          # 반대선 수렴 — 원문서 GB ≤ 0.10
        # ③ 국면 필터 — **필수**
        "regime_filter": "ma20_lt_ma60",
        "ma_fast": 20,
        "ma_slow": 60,
        "entry_delay_bars": 0,
        "session_start": "09:20",
        "session_end": "14:50",
        "max_per_day": 1,                # 당일 최초 1회만
        # 청산
        "exit_gs_target": 1.2,           # 주 청산. 1.5 는 GS 분포 91백분위라 도달 불가였다
        "hold_minutes": 60,              # 시간 손절
        "force_exit": "15:05",
        "opposite_line_stop": None,      # 반대선(GB) 손절 미사용 — 1.5 는 손실만 키웠다
        # 🔴 합격선 — 사전등록
        "min_samples": 30,               # 소급 0.706건/일 → 60일 약 42건
        "min_days": 25,                  # 하루 최대 1회라 표본수 ≈ 일수
        "unit": "position",
        "alpha": 0.05,
        "test": "daily_sign_test",
        "drop_best_days": 3,
        "require_beta_nonpositive": True,   # 🔴 이 규칙의 존재 이유. 점추정 β ≤ 0
        "require_beat_random_control": True,
        "control_min_samples": 200,
        "record_no_filter_control": True,   # MA20>MA60 구간 대조군 동반 기록
        "promotion_order": ["record_only", "report_only"],
        "live_order_forbidden": True,
    },
    # ── [GP-R 공통] 다중검정 회계 ────────────────────────────────────────────────────
    # 원문서 §운용권고 5: 「다중검정 카운트는 리셋하지 말고 누적하라」.
    # 다만 **탐색과 확증은 분모가 다르다** — 위 두 규칙은 데이터를 보기 전에 고정된
    # 확증 가설 2개이고, 전향 표본은 탐색에 쓰인 적이 없다. 그래서 이번 판정의 분모는 2다.
    # 🔴 그러나 탐색 이력 70 을 **지우지 않는다.** 관측 중 새 규칙을 발견해 채널을 추가하면
    #    그것은 확증이 아니라 탐색이므로 `exploration_tests_prior` 에 누적한다.
    # ── [GP-R 공통] 섀도 도전자 ID — Phase 3 구현과 Phase 0 사전등록을 묶는 이름 ────────
    # 🔴 이름을 바꾸면 challenger.db 의 과거 행과 끊긴다. 차트·패널·리포트가 전부 이 값을
    #   보고 GP 행을 고른다(`tests/test_553_gp_rule_shadow_preregistration.py` 가 고정).
    "gp_rule_challenger_ids": {
        "long": "GP_LONG_GB90",
        "short": "GP_SHORT_SQZ60",
    },
    "gp_rule_multiplicity": {
        "exploration_tests_prior": 70,   # 211거래일 탐색에서 시도된 규칙 수(원문서)
        "confirmatory_tests_n": 2,       # 이번 전향 판정의 분모 → Bonferroni α = 0.025
        "reset_forbidden": True,
        "note": "탐색 70 은 소급 근거의 신뢰도를 깎는 값이고, 확증 2 는 전향 판정의 분모다. "
                "둘을 바꿔 쓰지 말 것.",
    },
    # ── [MW0601 529차 / C] TrendGate 완화 × 레그 끝 결합 — 사전등록 ────────────────
    #
    # 완화 자체는 이익이므로(위 주석) 묻는 것은 **결합**이다: "이미 크게 달린 레그의 끝인데
    # 지속 모드라고 문턱까지 낮춰 주는 것"이 나쁜가. 2026-09-03 C4가 이 형태다
    # (직전 10봉 연속 하락 → DN 지속 모드 ON → min_conf 완화 → 바닥 1.24pt 위 SHORT, −114,209원).
    #
    # ⚠ 모집단 원천이 **로그**다 — TrendGate 활성 상태는 DB 어디에도 없다(529차 확인:
    #   `ensemble_decisions`에 trend/streak 컬럼 0개, `gate_signals`에도 없음). `[TrendGate] … 지속
    #   모드 ON/OFF` 전이를 파싱해 재구성한다(`chase_foreign_combo_watch`의 로그 재파싱과 동형).
    #   ⇒ 로그 보관 기간이 표본 상한이다. `monthly_cleanup.py:LOG_KEEP_DAYS` 축소 시 이 채널이 먼저 죽는다.
    # ⚠ 소급: 처리군 35건(16일) 21승14패 −243,379원, 일자 9/7 p=0.80. 완화-필수 진입(원래 min_conf
    #   미달) 5건은 **5승 +279,000원**이라 "완화를 끄자"가 아니라 "결합만 보자"가 맞다.
    "streak_leg_end_watch": {
        "enabled": True,
        "entry_source": "SYSTEM_AUTO",
        "data_start": "2026-07-14",
        "population_source": "log",     # [TrendGate] ON/OFF 전이 파싱
        "run_atr_min": 5.0,             # leg_exhaustion_entry_watch 와 동일 컷(축 일치)
        "dist_atr_max": 1.0,
        "min_samples": 20,
        "min_days": 10,
        "alpha": 0.05,
        "drop_worst_days": 1,
        # 승격 형태: 완화 자체는 유지하고 **결합일 때만 완화 미적용**(min_conf override 스킵).
        # 하드차단 아님 — 원래 문턱으로 되돌릴 뿐이다.
        "promotion_order": ["skip_relax_when_leg_end"],
        "hard_block_forbidden": True,
    },
    # ── [MW0601 529차 / E] 레그 초입 진입 — 관측 전용(거울상) ──────────────────────
    #
    # 위 두 채널이 "레그 끝이 나쁜가"를 묻는다면 이쪽은 "레그 **초입**이 좋은가"를 기록한다.
    # 소급: 초입(run <= 2.0) 12건 10승2패 +532,588원 · VWAP 이탈≥1.5σ 且 초입 15건 12승3패 +675,319원.
    # 🔴 **판정도 승격도 하지 않는다** — 가점(등급 상향·사이즈 확대)은 진입을 늘리는 방향이라
    #   `faststop_discovery`(2026-08-03 사용자 결정) 및 절대원칙 §2 계열 게이트 원칙과 충돌한다.
    #   표본만 쌓고, 위 A 채널의 `require_early_group_better` 판정에 입력으로 쓴다.
    "leg_entry_early_watch": {
        "enabled": True,
        "entry_source": "SYSTEM_AUTO",
        "data_start": "2026-07-14",
        "run_atr_max": 2.0,             # 초입 정의 — 고정
        "observe_only": True,           # 판정문 없음(verdict 항상 OBSERVE)
        "min_samples": 20,              # 리포트에 "표본 도달" 표기용일 뿐 판정 아님
    },
...
VALIDATION_CAMPAIGN_DECISIONS = {
    # ── [MW0601 529차 / D] 3_vwap 무변경 확정 ─────────────────────────────────────
    "core_vwap_directional_requirement": {
        "date": "2026-09-04",
        "decision": "무변경 — 순방향 VWAP 이탈에 상한 감점을 붙이지 않는다",
        "note": "✅ **[2026-09-04 MW0601 529차 / 사용자 지시 조사 결과]** 체크리스트 `3_vwap`이 "
                "매수 시 VWAP **위**를 요구해 '멀리 벗어날수록 잘 통과'하는 구조라 "
                "「많이 올라왔으니 조심」이 아니라 「많이 올라왔으니 들어가기 쉽게」로 작동한다는 "
                "지적이 있었고, 구조 자체는 사실이다. **그러나 손익은 반대로 말한다.** "
                "SYSTEM_AUTO 179포지션(2026-07-14~09-03) 실측: 순방향 이탈 ≥1.5σ 44건은 통산 "
                "**−9,395원(중립)** 인데, 이를 레그 위치로 가르면 **초입(60분 스윙 대비 <5 ATR) "
                "15건이 +675,319원(12승 3패)** 이고 **레그 끝(≥5 ATR 且 극단 ≤1 ATR) 16건만 "
                "−96,093원**이다. 반대로 이탈이 작아도(0~1.5σ) 레그 끝이면 8건 −536,430원이다. "
                "⇒ 손익을 가르는 축은 **VWAP 이탈 크기가 아니라 레그 위치**이며, 3_vwap에 상한 "
                "감점을 붙이면 시스템 최상위 승리군을 먼저 깎는다. "
                "🔴 대신 **레그 위치를 독립 축으로 신설**한다 — 529차가 `swing_*`·`dist_to_*` 피처를 "
                "배선했고 `leg_exhaustion_entry_watch` 채널이 판정한다. "
                "⚠ 재론 시에도 3_vwap의 방향 요구 자체(매수=VWAP 위)는 CORE 규칙(절대원칙 §3)이라 "
                "변경 대상이 아니다. 논점은 '상한을 둘 것인가'였고, 그 답이 위 실측이다. "
                "근거: docs/정기점검/매일점검/MW0601-20260904-스윙피처도입과-3vwap-TrendGate-손익최적안.md",
    },
    # ── [MW0601 529차 / C 부속] TrendGate 완화 유지 확정 ──────────────────────────
    "trend_gate_relax_keep": {
        "date": "2026-09-04",
        "decision": "무변경 — 지속 모드 min_conf 완화를 끄지 않는다",
        "note": "✅ **[2026-09-04 MW0601 529차]** `TrendGate` 지속 모드(streak≥10)가 그 방향 "
                "min_conf를 0.44로 낮추는 것이 '연장될수록 진입을 쉽게 만든다'는 지적의 대상이었다. "
                "실측: 로그 기준 **실제 인하 547회** 중 대부분이 `0.62→0.44`(333회)·`0.65→0.44`(104회)로, "
                "주 효과는 **TimeRouter 점심 진입금지(0.62)·고문턱 구간을 여는 것**이다. "
                "streak ON 중 순방향 진입 88건 통산 **+800,120원**, 그중 **완화가 없었으면 문턱 미달로 "
                "불가했던 5건이 5승 0패 +279,000원**이다. 완화를 끄면 이 이익을 먼저 잃는다. "
                "🔴 나쁜 것은 완화가 아니라 **결합**이다 — streak ON 진입 중 레그 끝 35건이 −243,379원, "
                "레그 초입 19건은 +382,362원. 그래서 완화는 유지하고 `streak_leg_end_watch` 채널이 "
                "**결합일 때만 완화 미적용**을 판정한다(하드차단 아님 — 원래 문턱으로 되돌릴 뿐). "
                "⚠ 완화-필수 5건은 표본이 작아 이 결정의 근거는 88건 통산 쪽이다. "
                "근거: 위 문서 §2-2.",
    },
    # ══ [2026-08-23 주간회의 / B-2] 멀티PC 정책 폐기 후속 — 좀비 결정 7건 분류 확정 ══
    #
    # 2026-08-23부로 MW0601·MW0602 상호 조율이 폐기됐는데(CLAUDE.md 「멀티PC 작업
    # 컨벤션」), 확정 결정 레지스트리 18건 중 **7건이 해제조건·관문에 "양 PC"를
    # 전제**하고 있었다. 대조 상대가 사라졌으므로 그 조건들은 **영구히 충족 불가능**
    # 하다 — 방치하면 매주 리포트가 같은 조건을 재출력하면서 아무도 닫지 못한다.
    # 이는 0808 D2가 이미 경고한 함정이다(*"표본이 쌓여도 수렴하지 않을 수 있다 —
    # 기다림의 기대값이 달라졌다"*).
    #
    # 사용자 승인(2026-08-23) — 7건을 세 갈래로 분류 확정한다:
    #   (a) 재등록  : "양 PC" 조건을 **단일 갈래 기준**으로 갈아끼운다 (3건)
    #   (b) 영구확정: 결정을 유지하되 **해제조건을 삭제**한다 — 재론하려면 새 근거가
    #                 필요하고, "표본이 쌓이면 다시 본다"는 기대는 버린다 (4건)
    #   (c) 폐기    : 해당 없음
    #
    # ⚠ **판정(verdict)은 하나도 바꾸지 않는다.** 채널은 계속 매주 재계산·재출력되고
    #   합격선도 무변경이다(§9-4). 바뀐 것은 "이 결정을 언제 다시 여는가"뿐이다.
    "multi_pc_zombie_reclass_20260823": {
        "date": "2026-08-23",
        "decision": "**멀티PC 전제 해제조건 7건 분류 확정** — (a)재등록 3건 / "
                    "(b)영구확정 4건. 「양 PC 수렴/합의」를 해제조건으로 쓰지 않는다",
        "note": (
            "(b) **영구확정 4건 — 해제조건 삭제, 재론에는 새 근거가 필요하다**: "
            "`direction_ev_watch`[21](LONG 부호 PC간 역전 → 방향 축 처방 설계 금지 유지) · "
            "`signal_decay`[4](PC간 부호 역전 → 판정보류 유지. 🔴 2026-08-23 주에 "
            "**자체적으로** PASS(+0.18pt) → FAIL(−0.85pt)로 뒤집혀, 불일치의 실체가 "
            "PC 차이가 아니라 **n=15 자체 불안정**임이 드러났다 — 대조 상대가 있었어도 "
            "닫히지 않았을 항목이다) · `entry_timing_value_watch`[42](판정 정면 충돌 → "
            "'타점에 가치가 있다'를 확정 근거로 인용 금지 유지) · "
            "`chase_foreign_combo_watch`[16](A등급 누적 부호 역전 → "
            "`CHASE_FOREIGN_COMBO_GUARD_ENABLED` 활성화 금지 유지). "
            "네 건 모두 **결정 내용은 무변경**이고, 삭제된 것은 *'양 PC가 같은 방향으로 "
            "수렴하면 재검토'*라는 도달 불가능한 해제조건뿐이다. "
            "──── "
            "(a) **재등록 3건 — 단일 갈래 기준으로 갈아끼움**: "
            "`cal_guard_flap_watch`[45](트리거의 *'양 PC 각각 2주 연속'* → **이 갈래 "
            "2주 연속**. 'PC별 분기 배포 금지' 조항은 배포 단위가 하나가 됐으므로 소멸. "
            "2026-08-21 실측은 점프 중앙값 0.0335 < 0.10 으로 **여전히 미충족**이라 "
            "보류 상태는 그대로다) · "
            "`kelly_skip`[8](3관문의 *'양 PC 각각 충족'* → **이 갈래에서 3관문 충족**. "
            "⚠ 같은 회의가 선행 작업으로 지목한 **②(일자단위 짝지은 부호검정) 계측을 "
            "489차가 이식**했다 — 그 전에는 어떤 n에서도 상정 금지였다. 이식 후 첫 "
            "실측: ①충족(n=23) / ②**미충족**(p=0.4531, 평균차 **+86,733원**으로 누적과 "
            "**부호가 반대**) / ③**미충족**(2026-07-23 하루를 빼면 +143,110원으로 역전). "
            "즉 누적 −262,228원은 **단일일이 만든 값**이고 차단 도입은 상정 금지다) · "
            "`tp1_protect_offset_shadow`[25](`watch_min_hooks_per_pc=70` / "
            "`watch_min_days_per_pc=13` → **이 갈래 기준 동일 수치 유지**. 수치는 "
            "관측 전 확정된 값이라 바꾸지 않는다 — 바뀐 것은 'per_pc'의 해석뿐이다. "
            "2026-08-21 실측 52훅/9일로 미달이며 도달해도 자동 적용이 아니다). "
            "──── "
            "⚠ **판정·합격선은 전부 무변경**이다(§9-4). 이 항목은 거버넌스 결정이며 "
            "채널의 계산·판정문은 하나도 손대지 않았다. "
            "⚠ **과거 PC 대조 기록을 지우지 않는다** — 0802·0808 대조가 남긴 근거는 "
            "그 시점에 유효했던 사료다. 앞으로 그 형식의 대조를 **새로 만들지 않을** 뿐이다. "
            "근거: DECISION_LOG 2026-08-23(MW0601 489차 B-2), "
            "docs/정기점검/MW0601_이관_점검사항_20260823.md §5."
        ),
    },
    # [2026-08-23 주간회의] 진입 호라이즌 경계 동결 + 경보의 역할 재정의.
    # 이 항목이 있는 한 `EntryHorizonRecalibrator`의 주간 `UPDATE`는 **미조치가 아니다.**
    "entry_band_watch": {
        "date": "2026-08-23",
        "decision": "**동결** — `ENTRY_HORIZON_B1=3.2` / `_B2=4.4` 현행 유지. "
                    "재보정기 `UPDATE`는 **참고용 경보**이고 판정은 이 채널이 한다",
        "note": "5주 연속 UPDATE(07-24~08-21)에 대한 명시적 현행유지 결정이다. "
                "① 재보정기는 손익을 보지 않고 33/67 분위수만 본다 — '1/3씩이어야 "
                "좋다'는 검증된 적 없는 가정이며, 그 장치의 목적은 374차 고착 사고의 "
                "재발 감시다. ② 이 채널의 2026-08-23 판정은 BAND_UNIFORM(일평균 격차 "
                "−81,713원 < 문턱 200,000, 부호검정 p=1.0000)이고 감시밴드는 08-17 대비 "
                "승률 38.1%→45.8%로 개선됐다. ③ 08-21 권고(5.72/8.57)를 실제 진입 220 "
                "포지션에 대입하면 **79.5%(175건)가 재배정**되며, 그중 142건은 유일한 "
                "흑자 구간(3m, 계약당 +17,538원)이 적자 구간(현행 1m, −20,018원)의 "
                "설정으로 내려가는 이동이다(TP1 폭 ATR×0.5→×0.3). "
                "재개 조건: BAND_ANOMALY 발생 / 26주 WFA 체제 전환 확인 / **실제 진입** "
                "기준 한 버킷 95% 이상 고착. ⚠ 문턱을 낮춰 판정하지 말 것(458차 D6). "
                "근거: DECISION_LOG 483차 후속5, config/settings.py ENTRY_HORIZON_B1 주석.",
    },
...
PHASE5_GATE_DECISIONS = {
    ...
    5: {
        "due": "2026-08-29",
        "note": "2026-08-01 W4 결산에서 복원 4주 연기 → **재검토 2026-08-29**. "
...
# 🔴 [MW0602 495차 후속 체리픽 / 2026-08-26] **수수료율은 로그인 채널이 정한다.**
#
# 대신증권은 같은 계좌라도 **로그인 매체(트레이딩 채널)별로** 요율을 고시한다.
# 두 운영 PC의 역산이 각자의 고시와 일치하며 서로 **5.16배** 다르다:
#   · CYBOS 사이버 트레이딩 (이 PC = MW0601) — **0.0098104%** (39거래일 역산 일치)
#   · CREON 트레이딩        (MW0602)        — **0.0019%**    (33거래일 역산 일치)
# ⇒ 후속8의 *"요율은 상품과 무관하다"* 는 **채널 안에서만 참**이었다.
#   하나를 하드코딩하면 다른 채널 PC에서 5.16배 틀린다(495차에 실제로 발생).
# ⇒ 채널→요율 매핑과 감지는 `config/constants.py:BROKER_CHANNEL_SPECS` /
#   `detect_broker_channel()` 이 **단일 원천**이고, 아래 값은 전부 그로부터 파생된다.
...
if BROKER_CHANNEL is None:
    # ── 폴백 가시화 (계측 4원칙 ④) ─────────────────────────────────────────
    # 🔴 **v9-dev 는 CYBOS 로 폴백한다** — dev(MW0602) 원본은 CREON 이다.
    #   이 분기는 의도적 차이다: 폴백을 틀리면 **5.16배** 어긋나므로 각 브랜치가
    #   자기 PC의 채널로 떨어져야 한다. 이 PC(MW0601)는 CYBOS 로그인이다.
    # ⚠ 이 PC에는 **CREON·CYBOS 스타터가 둘 다 설치돼 있다**(실측:
    #   `C:\CREON\STARTER\ncstarter.log` 2026-07-09 /
    #   `C:\Daishin\STARTER\ncstarter.log` 2026-08-26). 감지는 **mtime 최신**을 쓰므로
    #   평소에는 CYBOS 가 이기지만, **누군가 CREON 을 열면 그날부터 CREON 으로
    #   뒤집힌다.** 그렇게 되면 요율이 1/5.16 이 되어 493차 F-1 결함이 재발한다.
    #   → 확실히 못박으려면 런처에서 `MIREUK_BROKER_CHANNEL=CYBOS` 를 세울 것
    #     (NEXT_TODO 495-1). 지금은 감지+폴백 양쪽이 CYBOS 라 이중 안전이다.
    import sys as _sys
    _sys.stderr.write("[BrokerChannel] 감지 실패 — CYBOS 폴백 적용 "
                      "(MIREUK_BROKER_CHANNEL 환경변수로 강제 가능)\n")
    BROKER_CHANNEL = "CYBOS"
...
#
# 1계약 1040pt 기준(미니·CYBOS 채널): 편도 ≈ 5,101원 / 왕복 ≈ 10,202원
#
# 🔴 **하드코딩하지 않는다 — 감지된 로그인 채널에서 파생한다**(위 절 참조).
#   493차 후속8까지는 `0.000098104` 가 박혀 있었고, 그것이 MW0602(CREON)에서
#   5.16배 틀리는 원인이 됐다(495차). 값 자체는 이 PC에서 동일하다 —
#   `detect_broker_channel()` → `CYBOS` → 0.0098104%.
FUTURES_COMMISSION_RATE = _BROKER_SPEC["one_way_commission_rate"]

# ✅ [2026-08-26 493차 후속8 / 사용자 제공] **CYBOS 채널 요율은 공식 고시로 확정됐다.**
#   대신증권 「투자대상종목 및 수수료율」: KOSPI200 선물 **거래금액에 관계없이
#   0.0098104%**. 493차 후속5의 39거래일 역산치 **0.0098103%** 와 5자리 일치 —
#   역산이 옳았음이 독립 확인됐다(상대오차 0.004%).
#   ⇒ 이 값의 출처는 「실측 역산」이 아니라 **고시 요율**이다.
#   ⇒ 후속5가 남긴 CR-7(공식 요율 교차확인)은 이것으로 닫힌다.
#   ⚠ **「거래금액에 관계없이」는 채널 안에서만 참이다** — 후속8은 이것을 상품 축으로만
#     읽어 *"요율은 상품과 무관"* 이라 썼는데, 495차가 **채널 축**을 찾아냈다.
#     미니/정규 구분이 없는 것은 맞고, **채널 구분은 있다**(CREON 0.0019%).

# 종전(키움) 요율 — **소급 재계산·세대 판별 전용**. 라이브 계산에 쓰지 말 것.
# 2026-08-25 이전 trades 행의 commission_krw는 이 요율로 계산돼 있다
# (trades.commission_rate_used 컬럼이 행마다 명시 — 계측 4원칙 ④).
FUTURES_COMMISSION_RATE_LEGACY_KIWOOM = 0.000015
# 신 요율이 라이브 계산에 적용되기 시작한 날짜(불연속 경계).
# ⚠ 이 날짜 앞뒤의 **엔진 net 시계열을 직접 비교하지 말 것** — 461차 mdd_pct와
#   같은 유형의 불연속이다. 마커: strategy_events METRIC_REDEFINITION.
# 🔴 **채널마다 전환일이 다르다** — 그래서 이것도 채널 스펙에서 파생한다
#   (CYBOS 2026-08-25 = MW0601 적용일 / CREON 2026-08-26 = MW0602 적용일).
...
# 핀 상태인 동안 캠페인 리포트가 상단 배너로 "비용요율 핀(과소 6.54배)"을 경고한다.
COST_MODEL_COMMISSION_RATE = 0.000015          # ← 승인 시 0.0000981로 교체
COST_MODEL_COMMISSION_RATE_PINNED = True       # ← 교체 시 False

# ── Circuit Breaker 설정 ───────────────────────────────────────
CB_SIGNAL_FLIP_LIMIT = 5  # 1분 내 신호 반전 횟수
CB_SIGNAL_FLIP_PAUSE = 15  # 진입 정지 (분)
CB_CONSEC_STOP_LIMIT = 3  # 연속 손절 횟수 — [2026-09-02 복원. 절대원칙 ② 문구와 일치]
# ── [MW0601 519차] 모의투자 한정 예외 **해제** — 9999 → 3 ────────────────────
#
# 🔴 **사용자 지시로 복원했다**(2026-09-02). 재검토 기한 2026-08-29를 5일 초과한
#    상태였고, 3일 연속 리포트가 같은 권고를 냈다.
#
# **왜 3인가**: 절대원칙 ②의 문구가 *"5분 내 손절 3연속 → 당일 정지"* 이고,
# 실전 전환 게이트 판정기(`strategy/ops/phase5_gate_status.py:_CB2_RESTORED_MAX`)도
# 3 이하를 「복원됨」으로 본다. 허용 범위는 2~3이며 **3은 그중 보수적인 쪽**이다
# (2로 하면 같은 5분 안에 두 포지션만 잃어도 그날이 끝난다).
#
# **선행조건은 이미 끝나 있었다 — 489차 A-1.** 그때 계측이 문구와 어긋나 있었다:
#   ① 시간창이 없어 하루 종일 흩어진 손절도 "연속"으로 쌓였다
#   ② 호출부가 청산 **레그** 단위 4곳이라 한 포지션의 계단식 손절이 2카운트였다
# 그 상태로 한도를 되돌렸다면 **포지션 하나로 당일 정지**가 성립했다 — 문구가
# 뜻한 바가 아니다. 489차가 둘 다 고쳤고(300초 창 + `entry_ts` 포지션 중복제거),
# 호출부 4곳이 전부 키를 넘기는 것을 2026-09-02에 재확인했다.
#
# **복원 시점 실측(2026-09-02)**: 그날 손절은 08:45:10 · 10:21:18 두 건인데 로그가
# 둘 다 `[CB] 연속 손절 1회` 다 — 96분 간격이라 300초 창이 사이에서 비웠다.
# 연속 카운터 최고치는 **1**이었고, 한도가 2였어도 정지는 없었다. 즉 이 복원은
# 「오늘부터 갑자기 자주 멈춘다」는 뜻이 아니다.
# 같은 날 08:45의 3레그 청산(-5,299,668원)이 **1회로만** 세어진 것이 489차
# 중복제거가 살아 있다는 직접 증거다.
#
# ⚠ **이제부터 CB②가 실제로 당일 정지를 낸다.** 발동하면 그날 신규 진입이 전부
#   사라지므로, 발동 로그(`[CB] 연속 손절 N회` → `_trigger_halt`)를 장후 점검에서
#   반드시 확인할 것. 실전 전환 기준 ⑤는 **값 복원만으로 충족되지 않는다** —
#   v9 계획 §0-1이 「복원 후 정상 발동 1회 확인」을 함께 요구한다(게이트 판정기가
#   그래서 MET 이 아니라 UNMEASURED 를 돌려준다).
# ⚠ 진입 표본이 줄 수 있다. 그것이 2026-07-05에 이 값을 9999로 둔 이유였다 —
#   min_samples 미달 채널의 적립 속도를 장후 점검에서 함께 볼 것.
# ── [MW0601 489차 / A-1 스테이지1] 카운트 창 ─────────────────────────────
# CLAUDE.md 절대원칙 ②는 *"**5분 내** 손절 3연속"* 인데 코드에는 **시간창이 없었다**
# (승리 레그로만 리셋). 게다가 `record_stop_loss()` 호출부가 청산 **레그** 단위 4곳
# (main.py `_post_partial_exit`/`_post_loss_tier1_exit`/`_post_exit`/
# `_ts_record_nonfinal_exit`)이라 **한 포지션의 계단식 손절이 2카운트**를 만들었다
# (2026-08-19 11:11 SHORT 3계약이 실례). 한도를 2~3으로 되돌리면 **단일 포지션
# 하나로 당일 정지**가 성립한다 — 문서가 뜻한 바가 아니다. 계측 4원칙 ①의 CB② 판.
#
# 그래서 복원 **전에** 계측을 문구에 맞춘다(489차 당시 라이브 상수는 무변경이었다):
#   · 시간창 300초 안의 손절만 센다
#   · 같은 포지션(entry_ts)의 여러 청산 레그는 **1회**로 센다
# 근거: dev_memory/DECISION_LOG.md 2026-08-23(MW0601 489차 A-1), NEXT_TODO 08-29 기한.
# → 그 선행조건 위에서 **2026-09-02(519차) 한도를 3으로 복원했다**(위 상수 참조).
CB_CONSEC_STOP_WINDOW_SEC = 300  # 5분 — 절대원칙 ② 문구와 정합
#
# ── 아래는 9999 유예기(2026-07-05 ~ 2026-09-02)의 기록이다 — **사료로 보존한다.**
#    현행 값은 3이며, 이 문단을 현재 상태로 인용하지 말 것.
#
# [2026-07-05 도입] CLAUDE.md 절대원칙 ②는 "5분 내 손절 3연속 → 당일 정지"이나,
# 모의투자 단계에서는 거래 기회 확보·데이터 축적(레이블/SGD/SHAP 표본)이 우선이라
# 9999(사실상 비활성)로 완화했다.
...
# [2026-09-02 종결] 기한 5일 초과 상태에서 사용자 지시로 복원 — 유예기 종료.
...
# ── 🔴 [MW0601 546차 / 사용자 지시] ProfitGuard 판정 손익을 **시스템 자동매매
#      한정**으로 좁힌다 ────────────────────────────────────────────────────
#
# 사용자 지시 「판정에서 외부매매 손익 분리하고 시스템 자동 매매에만 적용하도록
# 코드개선해」(2026-09-08).
#
# **계기 — 시스템이 벌지 않은 돈으로 시스템이 멈췄다.**
# 2026-09-08 실측: 09:04:58 장 시작 전인데 브로커 실현손익이 이미 +446,000원
# (엔진 기준 0원)이었고, 09:18~09:20 **외부(수동) 매매** 청산이 634,000원까지
# 밀어올려 09:21 에 `L2-Tier4`(중단 임계 500,000원)가 **당일 영구 중단**으로
# 래치됐다. 그날 시스템 자동진입은 **0건**이다. 09:54 실측 —
#     used=+513,967원  engine=+67,967원  broker=+513,967원
# 종전 판정 원천 `broker_net_est`(= CpTd6197 실현손익 gross − 엔진 당일 수수료)는
# **계좌 전체**라 외부 매매·야간세션·이월분이 그대로 섞인다.
#
# **바뀌는 것**: ProfitGuard(L1 트레일링·L2 티어·L3 오후모드·L4 수익CB) 4개
# 레이어가 보는 `daily_pnl_krw` 가 `trades.entry_source='SYSTEM_AUTO'` 레그의
# **실현 net 합계**(원)가 된다. 판정식·임계·티어 구성은 **한 줄도 안 바뀐다** —
# 같은 자에 다른 눈금을 대는 것이 아니라, 같은 눈금을 **재야 할 대상**에만 댄다.
#
# **바뀌지 않는 것**: 손익 리포트·대시보드 잔고·EOD 대사·전환기준 ① 판정은
# 전부 종전 원천 그대로다. 이 플래그는 **ProfitGuard 판정 한 곳**에만 걸린다
# (db_utils.fetch_ev_by_grade(system_only=…) 가 423차에 같은 이유로 도입된 것과
# 같은 성격 — 「얼마 벌었나」는 전체, 「시스템을 평가/보호하려면」은 SYSTEM_AUTO).
#
# ⚠ **불연속이다.** ProfitGuard 가 보는 일일손익 시계열은 2026-09-08 에 끊긴다.
#   앞뒤로 「그날 ProfitGuard 가 본 손익」을 직접 비교하지 말 것(461차 `mdd_pct`,
#   493차 수수료율과 같은 유형). 모든 차단 로그에 원천 토큰이 함께 찍히므로
#   (`src=engine(net·시스템진입만)`) 행별 세대는 로그로 구분된다.
#
# ⚠ **축이 gross→net, 계좌→엔진, 미실현포함→실현전용 으로 동시에 바뀐다.**
#   · 종전 broker gross 는 **보유 중 미실현이 섞였다**(main.py 의 497차 P3 주석).
#     새 축은 **실현 net 전용**이라 보유 중에는 움직이지 않는다.
#   · 그래서 L1 피크(`trail_activation_krw`)·L2 티어 임계는 **같은 값이라도 더
#     늦게 닿는다.** 이는 의도된 결과다 — 종전이 외부 손익까지 세어 **과도하게
#     빨리** 발동했다.
#   · 임계 자체의 재보정 여부는 **별건**이다(NEXT_TODO 543-1). 여기서 임계를
#     같이 건드리면 어느 쪽 효과인지 사후에 분리할 수 없다(313차 원칙).
#
# ⚠ **`entry_source` 시계열도 2026-09-02 에 불연속이다**(518차 F-3 — 라벨이
#   한 번 비-SYSTEM_AUTO 로 바뀌면 세션 끝까지 안 돌아오던 결함을 그날 고쳤다).
#   그 이전 구간을 이 축으로 소급 재현하면 SYSTEM_AUTO 가 과소 집계된다.
#
# ⚠ **미측정 ≠ 0**(계측 4원칙 ②). 당일 SYSTEM_AUTO 청산 레그가 **아직 0건**이면
#   합계 0원이 아니라 **"측정 안 됨"** 이다. 다만 ProfitGuard 는 "수익을 지키는"
#   가드라 손익 0 과 미측정의 **판정 결과가 같다**(어떤 레이어도 발동하지 않음).
#   그래서 값은 0.0 을 쓰되 레그 수(`legs`)를 함께 실어 배지·로그에서 구분한다.
#
# False 로 되돌리면 종전 `broker_net_est` 경로로 즉시 복귀한다(코드 잔존).
PROFIT_GUARD_SYSTEM_ONLY_PNL = True

# 위 축에서 **시스템 자동매매로 인정하는** `trades.entry_source` 값.
# ⚠ 여기 없는 값은 전부 제외다 — 실측 기준 제외 대상:
#     OPERATOR_MANUAL      대시보드 수동 진입 버튼
#     GHOST_PENDING_MISS   pending 미등록 외부 체결(= 2026-09-08 그 경로)
...
# [MW0601 457차 / B2] SHAP 계산을 워커 스레드로 이관 — 파이프라인 임계경로에서 제거.
#
# 435차 스로틀(위)은 **꼬리만** 막는다. 2026-08-11 실측은 꼬리가 아니라 **상시 중앙값**이
# 문제임을 보여줬다: `[S6Detail] shap=460~521ms`가 매분 나오는데 임계 900ms 미만이라
# 스로틀이 한 번도 걸리지 않았고, 그 상태로 S6가 679~1,048ms까지 자라
# **CB⑤ 경고 9건 중 4건(12:22·12:26·13:46·14:31)을 단독으로 유발**했다.
#
# 위 435차 주석이 남긴 "⚠ CB⑤ 입력에서 SHAP 시간을 빼는 것이 더 근본적"이라는 메모의
# 답이 이것이다 — **CB⑤ 입력 정의를 건드리지 않고**(절대원칙 §2 안전장치 불변)
# 파이프라인이 실제로 그 시간을 쓰지 않게 만든다. 계측 정의가 아니라 사실을 바꾼다.
#
# 설계:
#   · 워커 스레드가 permutation importance 계산 + DB 저장(save_shap_scores)까지 수행
#   · Qt 위젯 갱신(_update_shap_dashboard)은 절대 워커에서 하지 않는다 —
#     _deferred_callbacks 큐에 ("shap_ui",)를 넣고 다음 분봉 S0-B drain이 메인
#     스레드에서 소비한다(ConstOut 재적합이 쓰는 것과 동일한 기존 경로)
#   · 동시에 두 개가 돌지 않도록 _shap_worker_running으로 상호배제
#   · 모델 교체 대기 중(_pending_model_reload)에는 시작하지 않는다 — 교체 직후
#     새 모델로 다시 계산되게 한다
#
# 롤백: False로 되돌리면 종전(동기) 동작. 435차 스로틀은 두 모드 모두에서 유지된다.
SHAP_ASYNC_ENABLED: bool = True

# [MW0601 457차 / B1] 재학습 완료 모델 리로드를 워커 스레드로 이관.
#
# 2026-08-11 실측 — 장중 재학습 5회 **직후** 파이프라인 S0(모델 교체) 구간:
#   09:38:04 S0=4,381ms   10:21:03 S0=3,585ms   11:13:03 S0=3,197ms
#   12:44:03 S0=3,401ms   13:14:02 S0=2,979ms          ← 재학습 종료시각과 5/5 일치
# 최대 4,661ms(총 파이프라인)는 **CB⑤ 즉시청산 임계 5,000ms의 93%** 다. 포지션
# 보유 중 이 스파이크가 나면 의도치 않은 강제 청산으로 이어진다. 실제 피해도 이미
# 발생했다 — HealthPolicy Degraded 선제차단 5회로 신규 진입이 5개 구간에서 봉쇄됐다.
#
# 원인은 joblib.load × 12(GBM 6 + 호라이즌 피처명 6) + RF pkl 1을 Qt 메인 스레드에서
# 동기 실행한 것. 계산이 아니라 **디스크 읽기 + 언피클**이라 워커로 온전히 옮길 수 있다.
#
# 설계(ConstOut 재적합 워커와 동일한 기존 패턴):
#   · 워커: model.stage_reload() / rf_model.stage_load() — self 상태 변경 없음
#   · 큐:   _deferred_callbacks.put(("model_staged", gbm, rf))
#   · 메인: S0-B drain → _commit_model_reload() — 참조 대입 + 인덱스 재계산(수 ms)
#   · 스왑 지점이 predict_proba(S5) 이전이라 193차 "파이프라인 실행 중 교체 금지" 유지
#   · 워커 실패 시 _pending_model_reload를 내리지 않아 다음 분에 재시도(교체 유실 방지)
#
# 롤백: False → 종전 동기 동작(단, 내부적으로 stage/apply 2단계를 순차 호출하므로
#       파일 읽기 순서·부작용은 457차 이전과 동일하다).
MODEL_RELOAD_ASYNC_ENABLED: bool = True
...
# 이 시각 이후부터 수집한다.
# [2026-09-22] 종전 값 "09:02" 를 낮췄다. 이유는 **여유**다 — 한 번의 조회가 18행만
#   주므로, 09:02 첫 수집이면 옵션(1분 간격) 기준 08:44~09:02 로 하한이 딱 맞물린다.
#   첫 수집이 조금만 밀려도 이른 구간이 잘린다. 08:50 이면 수급 타이머 첫 호출
#   (실측 `[CybosInvestor] fetch#1` 08:58:13)에 걸려 마진이 생긴다.
#   장 개시 직후 서버 피크는 시작을 미루는 대신 아래 PEAK_SKIP 으로 건너뛴다.
#
# 🔴 **08:45 는 이 설정으로 해결되지 않는다 — 원천에 없다.**
#   2026-09-22 실측: type 3 을 어떤 값으로 주든 Cybos 의 첫 행은 **08:46** 이고
#   `t3=0846` 은 0행을 준다. 키움 화면에는 08:45 가 있으므로 **브로커 간 차이**다.
#   ⇒ DB 에 08:45 가 없는 것은 미륵이 결손이 아니다. 결손으로 세지 말 것
#     (계측 4원칙 ②: 미측정과 원천 부재는 다르다).
WEEKLY_OPTION_FLOW_START_AFTER = "08:50"
