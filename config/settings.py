...
# ⚠ [MW0602 495차] 아래 출처·날짜는 MW0601에서 확인된 것이다. 이 PC 계좌의 실측
#   요율(0.0019%)은 그 고시 수치(0.0098104%)와 다르므로, **이 계좌 기준 고시/설정
#   화면 확인은 아직 안 됐다** — 확인되면 SOURCE·VERIFIED_ON을 이 PC 기준으로 갱신할 것.
BROKER_FEE_SOURCE = "대신증권 CYBOS 모의투자 「투자대상종목 및 수수료율」 (MW0601 확인분 — MW0602 계좌 요율은 33거래일 역산)"
BROKER_FEE_VERIFIED_ON = "2026-08-26"   # MW0601 사용자가 고시 화면으로 확인한 날
...
#
# 🔴 [MW0602 495차 / 2026-08-26] **위 MW0601 수치(0.0098104%)는 이 PC 계좌와 다르다 —
#    dev 브랜치(MW0602 단독 운영, 487차)는 MW0602 실측값을 쓴다.**
#
# 493차 후속5·후속8 체리픽 검증에서 같은 스크립트(`commission_rate_recon.py --verify`)를
# **이 PC의 로그·trades.db**로 돌린 결과:
#   · 2026-07-07 ~ 08-25 **33거래일 / 약정대금 339.3억원** 전수 (오염일 1일 제외):
#       실효 편도 요율 = **0.0019000%**,  1변수 회귀 R² = 1.000000,  최대 잔차 1.5원
#       고정비 성분(레그당) = +0.15원 ≈ 0  → 순수 비례 요율, 고정비 없음
#   · 일자별 실효 요율이 33일 내내 0.001900%로 동일(우연이 아니다).
#   · pt_value 교차확인: ui_prefs.json = A0569000(미니) → 50,000 정상.
#     승수 착오(250,000)였다면 5.0배가 나와야 하는데 실측 격차는 5.16배 —
#     승수 원인이 아니라 **계좌의 요율 자체가 다르다.**
# ⇒ MW0601 0.0098104% vs MW0602 0.0019000% — 후속8의 "요율은 상품과 무관하다(고시)"
#   서술과 이 계좌 실측이 **모순**된다. 모의투자 계좌별 요율 설정 차이로 추정.
#   🔴 사용자 조치 필요: 이 계좌(CYBOS 모의)의 고시/설정 요율 화면을 확인해
#   0.0019%의 출처를 확정할 것(NEXT_TODO 495차). 확인 전까지 출처는 「33거래일 역산」.
# ⚠ 라이브 브로커 net 대사(F-2)가 매일 이 값을 검증한다 — 틀리면 MISMATCH가 뜬다.
FUTURES_COMMISSION_RATE = 0.000019  # 0.0019% 편도 (거래대금 기준) — MW0602 실측

# [사료 — MW0601 v9-dev의 값. 이 브랜치에서는 쓰지 않는다]
# ✅ [2026-08-26 493차 후속8 / 사용자 제공] MW0601 계좌는 브로커 공식 고시로 확정.
#   대신증권 「투자대상종목 및 수수료율」: KOSPI200 선물 **거래금액에 관계없이
#   0.0098104%**. 493차 후속5의 39거래일 역산치 **0.0098103%** 와 5자리 일치.
#   ⚠ 「거래금액에 관계없이」 = 정액 구간이 없다 — 이 성질은 MW0602 실측(고정비 ≈0)
#     에서도 동일하게 확인된다. 다른 것은 비례 요율의 크기뿐이다.

# 종전(키움) 요율 — **소급 재계산·세대 판별 전용**. 라이브 계산에 쓰지 말 것.
# 이 PC(MW0602)에서는 2026-08-26 이전 trades 행의 commission_krw가 이 요율로
# 계산돼 있다(trades.commission_rate_used 컬럼이 행마다 명시 — 계측 4원칙 ④).
...
# (MW0601은 2026-08-25 — PC마다 전환일이 다르다)
FUTURES_COMMISSION_RATE_EFFECTIVE_FROM = "2026-08-26"
