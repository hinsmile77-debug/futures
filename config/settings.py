...
BROKER_FEE_SOURCE = "대신증권 CYBOS 모의투자 「투자대상종목 및 수수료율」"
BROKER_FEE_VERIFIED_ON = "2026-08-26"   # 사용자가 고시 화면으로 확인한 날
...
FUTURES_COMMISSION_RATE = 0.000098104  # 0.0098104% 편도 (거래대금 기준)

# ✅ [2026-08-26 493차 후속8 / 사용자 제공] **브로커 공식 고시로 확정됐다.**
#   대신증권 「투자대상종목 및 수수료율」: KOSPI200 선물 **거래금액에 관계없이
#   0.0098104%**. 493차 후속5의 39거래일 역산치 **0.0098103%** 와 5자리 일치 —
#   역산이 옳았음이 독립 확인됐다(상대오차 0.004%).
#   ⇒ 이제 이 상수의 출처는 「실측 역산」이 아니라 **고시 요율**이다.
#   ⇒ 후속5가 남긴 CR-7(공식 요율 교차확인)은 이것으로 닫힌다.
#   ⚠ 「거래금액에 관계없이」 = **정액 구간이 없다**. 미니/정규 구분도 없이 같은 요율이며
#     승수 차이는 거래금액에 이미 반영되므로 요율은 상품과 무관하다.
