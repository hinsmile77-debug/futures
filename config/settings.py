...
# ── [MW0601 533차 체리픽] 풀타임 세션 봉 적재 (Phase 1) ──────────────────────
# `raw_candles` 는 08:45~15:08 봉만 담는다 — 15:09 이후 봉은 수신·집계까지 끝난 뒤
# `_on_candle_closed` 의 force-exit 가드가 **저장 전에** 버린다.
# 🔴 **MW0602 실측(2026-08-24~09-04, 10거래일)**: 매일 373~385행 · `MAX(ts)` 가
#   **15:08(6일) 또는 15:09(4일)** — 15:10~15:45 봉이 통째로 없다(마감 단일가 포함).
#   MW0601(매일 정확히 384행·15:08)과 값은 조금 다르지만 **결손 구조는 동일**하다.
# 소비처가 "최근 N행" 조회를 하므로 `raw_candles` 에 행을 더하지 않고, **별도 테이블
# `session_bars`** 에 세션 태그를 달아 전량 적재한다.
# **읽는 코드는 0이다**(판단 무영향). 소비 전환은 Phase 4에서 채널별로 따로 결정한다.
#
# ⚠ **원본의 `EOD_RETRAIN_SCHEDULE_HM = "1550"` 은 가져오지 않았다.**
#   그 상수는 MW0601 435차의 15:40 EOD 마커 경보용인데 **이 브랜치에는 그 상수도
#   소비 코드도 없다**(전수 확인). 없는 경보의 임계를 새로 만들 이유가 없다.
#   참고로 이 PC 예약작업 `Maitreya_EODretrain` 은 이미 **15:50** 이다(실측).
...
# 🔴 **이 게이트는 미시레짐 「탈진」·챌린저 탈진 전문가 풀과 별개다** — 이름만 같다.
#   ⓐ 미시레짐 `REGIME_EXHAUSTION="탈진"`(collection/macro/micro_regime.py) — 시장 상태
#      5분류의 하나. **전 기간 발동 0건**(입력 `bear/bull_exhaustion`이 상수 0).
#   ⓑ 이 게이트 — "탈진 반전 자리로 진입하려는가"를 보는 **진입 섀도 게이트**(379차).
#      로그의 `탈진` 문자열은 사실상 전부 이쪽이다.
#   ⓒ 챌린저 `REGIME_POOLS["탈진"]` — ⓐ가 켜졌을 때 쓸 대체 전략 풀(챔피언 None).
#   셋을 섞어 인용하면 "탈진이 N번 작동했다"는 틀린 문장이 만들어진다.
#   상세: docs/정기점검/매일점검/MW0602-20260903-탈진레짐-조사보고.md §1
...
# ── [MW0601 552-10 체리픽] 브로커 net 판독 세션 종료 시각 ──────────────────
# 이 시각 이후에 **기준점 없이** 들어온 예탁금 판독은 저장하지 않는다(SKIP_NO_BASE).
# 🔴 MW0602 실측으로 필요성이 확인됐다: 2026-09-07 22:02 재기동 세션이
#   `[BrokerNet] state=OK date=2026-09-07 net=-28948` 을 썼는데 그날 수수료가
#   **28,947원** 이다 — 예탁금이 이미 롤오버돼 「익일가 − 예탁 = −수수료」가 된
#   전형적 오염이다. 501차 가드가 뚫린 게 아니라 **기준점을 잃었다**:
#   `_broker_dep_base_today` 가 프로세스 상태라 재기동이 지웠고, 그 세션에겐
#   22:02 판독이 「그날 첫 판독」이라 불변식이 성립해 버렸다.
#   ⚠ 현재 DB 값(-164,948)이 맞는 것은 나중에 백필이 다시 계산했기 때문이지
#     구조가 막아준 것이 아니다.
...
VALIDATION_CAMPAIGN = {
    ...
    "profit_geometry_lowvol_watch": {
        ...
        "alpha": 0.05,
        # ── [MW0602 526차 후속6 / P-1] 부속 관측 [51-R] — 시간대 × ATR 층 잔여 ──
        # 🔴 **판정 무영향이다.** 위 profit_atr_ratio_gap_min·atr_split_pt·alpha 는
        #   한 글자도 바뀌지 않았다([51] verdict 는 종전 그대로 계산된다). 이 키는
        #   같은 모집단을 **시간대 × ATR 층**으로 교차해 보여주는 OBSERVE 절 전용이다.
        #
        # 계기 — O-69(13시대 진입 손실 집중)의 조기 판정(2026-09-03, 사용자 지시).
        #   20거래일 실측에서 13시대 44포지션 −1,298,472원 · 12거래일 중 11일 음수
        #   (일자 부호검정 p=0.0063)였는데, **ATR 층화 후 그 효과가 사라졌다**
        #   (저변동 층 내 일자내 순열 p=0.57). 13시는 하루 중 진입시점 ATR 최저
        #   (평균 1.44) · ATR<1.5 진입 비중 최고(27/44=61%, 09~10h는 0/48)여서
        #   **"13시"가 "저변동 진입"의 대리변수**였다. 모델 적중률(3m 13h 52% vs
        #   전 시간대 47~52%)·시장 효율(0.114 vs 0.101~0.126)에는 차이가 없었다.
        #
        # 왜 채널로 만드나 — 그 분해를 단발 딥다이브로 하면 다음에 "X시대가 나쁘다"가
        #   또 올라올 때 처음부터 다시 손으로 센다. 292·303·371·468차가 반복 확인한
        #   *"감시 대상 목록 자체가 낡는 것은 일일 점검이 못 잡는다"* 와 같은 계열이라
        #   **매주 자동 재계산되는 표**로 고정한다.
        #
        # 🔴 **임계는 관측에서 역산하지 않았다.**
        #   · strata_edges 2.6 = 위 atr_split_pt(462차 기존 분할선) 그대로
        #   · 1.5 · 2.0 = ATR_MIN_ENTRY(1.0)의 1.5배·2.0배
        #   · 1.25 = 진입 하한(1.0)과 1.5의 중간점
        #   0903 실측에서 가장 나빴던 구간은 [1.25,1.50)(−1,138,338)이지만 그 경계를
        #   컷으로 승격하지 않았다 — ATR 4분위가 **단조가 아니고**(Q1 −1,135,536 /
        #   Q2 +265,483 / Q3 −959,055 / Q4 +871,070) ATR<1.5 vs ≥1.5 일자내 순열이
        #   p=0.39라 어떤 경계도 근거가 없다.
        #
        # ⚠ 셀이 얇은 것이 **기본 상태**다(20거래일 185포지션 ÷ 6시간대 × 4층 ≈ 9건).
        #   미달 셀은 0이 아니라 `INSUFFICIENT(n=…)` 로 찍는다(계측 4원칙 ②).
        # ⚠ ATR 원천은 [51] 본판정과 **같은 정의**(_atr14_at = raw_candles 14봉)를 쓴다.
        #   0903 딥다이브는 엔진 ATR(`ensemble_decisions.features`)로 셌으므로 값이
        #   소수점에서 다를 수 있다 — 같은 채널 안에 두 정의를 섞지 않는 쪽을 택했다
        #   (461차 mdd_pct 분모 혼동과 같은 계열의 사고 예방).
        "hour_residual": {
            "strata_edges": (1.25, 1.5, 2.0, 2.6),
            "alpha": 0.05,
            "min_positions_per_cell": 10,
            "min_days_per_cell": 5,
            "permutations": 5000,
            "seed": 462,          # [51] 채널 차수 — 재현 가능한 고정 시드
            # 승격(주간회의 상정) 조건 — 사전등록. 하나라도 미달이면 관측 지속.
            #   ① ATR 층화 후 잔여 일자내 순열 p < alpha
            #   ② 그 시간대 일별 순손익 부호검정 p < alpha
            #   ③ 해당 시간대 셀이 min_positions_per_cell·min_days_per_cell 충족
            #   ④ ①~③을 **2주 연속** 날짜본에서 확인(리포트는 4주 보관 — 사람이 대조)
    ...
    #
    # ── [MW0602 526차 후속6] [58] 13:00~13:30 진입 금지 — 섀도 (사용자 지시) ──
    # 🔴 **집행하지 않는다.** 이 채널은 계측이며 라이브 진입 로직·TIME_ZONES·
    #   ZONE_ENTRY_BAN_ENFORCE 를 건드리지 않는다. "만약 13:00~13:30 진입을 막았다면
    #   무엇을 포기했나"를 매주 재계산해 **집행 여부를 사람이 판정**하게 한다.
    #   (2026-09-03 사용자 지시: "13:00~13:30 금지를 그래도 섀도로 올린다")
    #
    # 왜 [53] zone_ban_breach_watch 에 넣지 않았나 — **그쪽 모집단을 중간에 바꾸면
    #   사전등록이 깨진다.** [53]은 `banned_zones=("OTHER","EXIT_ONLY","PRE_MARKET")`
    #   로 등록된 위반 코호트(현재 n=9)를 재는 채널이고, 여기에 새 존을 끼우면 판정
    #   대상이 달라져 §9-4 검증 시계 리셋 사유가 된다(461차 mdd_pct·493차 요율·
    #   507차 PSI 피처집합과 같은 「같은 이름, 다른 지표」 사고). 그래서 별 채널이다.
    #
    # 🔴 **이 창(13:00~13:30)은 관측에서 골랐다 — 사후탐색이다.**
    #   0903 실측 15분 버킷: 13:00-13:15 −633,537(n=14) · 13:15-13:30 −322,993(n=7) ·
    #   13:30-13:45 **+4,428**(n=11) · 13:45-14:00 −346,370(n=12). 앞 30분만 묶은 것은
    #   그 표를 보고 자른 것이므로, **판정 창을 배선 다음 거래일부터**로 두고 그 이전
    #   실측은 「가설을 만든 구간」으로만 병기한다([46]·[55]·[57]과 같은 규약).
    #   사전(hypothesis-generating) 구간 실측 — 🔴 **창이 다르면 값도 다르다. 둘 다 적는다.**
    #     ⓐ 캠페인 전 구간(2026-07-09~09-03) = **채널의 `pre_window`가 인쇄하는 값**:
    #        코호트 38포지션 −510,721원 · 20거래일 중 13일 음수 ·
    #        최악일(07-21) 제외 −319,821원 / 잔여 264포지션 건당 +419원.
    #     ⓑ 최근 20거래일(2026-08-06~09-03) = 0903 딥다이브 §10이 인용한 값:
    #        코호트 21포지션 −956,530원 · 11거래일 중 9일 음수 / 잔여 164포지션 건당 −1,590원.
    #   ⚠ ⓐ와 ⓑ를 섞어 인용하지 말 것(461차 `mdd_pct`·493차 요율과 같은 계열의 사고).
    #   🔴 어느 쪽도 집행 근거가 아니다 — 창을 관측에서 골랐기 때문이다.
    #
    # 🔴 **④ ATR 잔여 조건이 핵심이다.** O-69 판정에서 13시대 열위는 ATR 층화 후
    #   소멸했다(p=0.57). 그래서 이 채널의 FAIL 조건에 [51-R]과 같은 층화 잔여 검정을
    #   **넣었다** — 조건을 느슨하게 만든 것이 아니라 **더 엄격하게** 만든 것이다.
    #   시간 기반 금지는 "시간대 고유 효과"가 있을 때만 정당하고, 저변동 진입 문제라면
    #   처방은 시간이 아니라 [51]·[25]·[10] 축이다.
    #
    # ⚠ 반사실 계산이 필요 없다 — 이 코호트는 **실제로 체결된 진입**이므로 그 실현
    #   손익이 곧 금지의 대가/이득이다(hurst_gate_shadow류 CF 재구성과 다르다).
    #   단 **대체 진입 효과는 반영하지 않는다**(막았다면 다른 분에 들어갔을 수 있다).
    # ⚠ 승격 시 필요한 배선은 이 채널이 아니라 라우터다 — TIME_ZONES 에
    #   `LUNCH_RECOVERY_EARLY(13:00~13:30)` 를 신설하고 ZONE_MIN_CONF ·
    #   C_AUTO_EXP_ZONES · main.py:_trend_mc_zones **세 표에 키를 함께** 넣어야
    #   현행 LUNCH_RECOVERY 와 동일 동작이 유지된다(하나만 빠지면 조용한 정책 변경).
    "lunch_early_entry_ban_shadow": {
        "entry_source": "SYSTEM_AUTO",
        # 판정 창 — 배선 다음 거래일. 그 이전은 가설 생성 구간(위 주석).
        "start_date": "2026-09-04",
        "window": ("13:00", "13:30"),   # 🔴 관측에서 고른 창 — 사후 변경 금지
        "min_samples": 20,              # 타 채널과 동일 — 관측에서 역산하지 않았다
        "min_days": 10,                 # 313차 일자단위 검정 최소치
        "alpha": 0.05,
        # FAIL(=금지 집행을 주간회의에 상정) 조건 — 넷 다 충족해야 한다:
        #   ① 코호트 n≥min_samples · 관측일≥min_days
        #   ② 짝지은 일자 검정에서 코호트가 열위(부호검정 p<alpha)
        #   ③ 최악일 제외(drop-worst) 후에도 코호트 누적이 음수
        #   ④ ATR 층화 후 잔여 일자내 순열 p<alpha  ← 시간대 고유 효과 확인
        "require_atr_residual": True,
        "drop_worst_day": True,
    },
    #
    # ── [MW0602 526차 후속8 / P-3] [59] 소진 피처 live 전환 조건 ⓐ 자동판정 ──────
    # 🔴 **자동 전환은 없다.** 이 채널은 판정만 낸다. `EXHAUSTION_RESTORE_MODE` 는
    #   사람이 주간회의에서 바꾼다(§9 사전등록 원칙).
    #
    # 왜 만드나 — 394차(2026-07-27)가 live 전환 조건 ⓐⓑⓒ를 사전등록했는데
    #   **ⓐ를 계산하는 코드가 어디에도 없다.** settings 주석과 피처 레지스트리에만
    #   문장으로 적혀 있어, CB②·CB③-P4·FP-CRITICAL과 똑같이 "재검토하기로 했는데
    #   안 함" 상태다(2026-09-03 조사보고 §8-3). 조건을 채널로 옮겨 매주 자동으로
    #   다시 묻게 한다.
    #
    # 무엇을 묻나 — ⓐ "교정 섀도(`bear/bull_exhaustion_shadow>0`)가 발화한 시점의
    #   **기대방향** 사후수익률이 **일자단위로** 유의하게 양(+)인가."
    #   기대방향: `bull_exhaustion`(상승압력 소진) → **SHORT** /
    #             `bear_exhaustion`(하락압력 소진) → **LONG**
    #   (`strategy/entry/checklist.py` MR 분기와 같은 대응이다)
    #
    # 🔴 **판정 모집단은 「MR 자격」 코호트다** — 전체 발화가 아니다.
    #   live 전환이 실제로 여는 문은 checklist 3_vwap MR 분기이고 그 분기는
    #   `|vwap_position|>1.5 ∧ 강도>=MR_EXHAUSTION_MIN_WEAK(0.60)` 를 함께 요구한다.
    #   전체 발화로 판정하면 **거래되지 않을 분까지 세어** ⓐ를 잘못 통과시킨다.
    # 🔴 **판정 호라이즌은 10분 하나다** — 다중검정을 피하려 단일로 고정한다.
    #   30분은 참고로만 병기하며 판정에 관여하지 않는다.
    #
    # ⚠ **판정 창은 배선 다음 거래일부터다.** 그 이전 구간은 이 채널의 설계를
    #   정하면서 이미 본 구간이라 **가설을 만든 구간**이다([46]·[55]·[57]·[58] 규약).
    #   사전 실측(2026-07-28~09-03, 23거래일 · 판정 무영향 · 집행 근거 아님):
    #     · 발화 62분 = bull 62 / **bear 0** (교정 후에도 bear측은 0이다)
    #     · MR 자격(vwap 동반) **21분** → 10분 기대방향 수익 평균 **−0.38pt**,
    #       적중 57%, 일자 부호 +6/−4  ⇒ 현 시점 ⓐ는 **미충족**
    #     · 강도 >=0.70 층 **4분**만 +3.90pt(4/4) — n=4라 확정 금지(313차)
    #   🔴 이 수치로 live 전환을 정당화하지 말 것.
    #
    # ⚠ ⓐ가 충족돼도 live 가 아니다 — ⓑ(교정값 포함 EOD 재학습 1회, 학습/서빙 분포
    #   정합) · ⓒ(탈진 레짐 발생률 + 챔피언 승격 정책 동시 결정)가 남는다.
    #   그리고 전환 시 **SHORT 편향이 커진다**(bear=0이라 LONG MR은 열리지 않는다) —
    #   `O-11`·`O-19` 방향 편향 채널과 함께 볼 것.
    "exhaustion_restore_watch": {
        "start_date": "2026-09-04",     # 배선 다음 거래일 — 그 이전은 가설 생성 구간
        "min_samples": 20,              # 타 채널과 동일 — 관측에서 역산하지 않았다
        "min_days": 10,                 # 313차 일자단위 검정 최소치
        "alpha": 0.05,
        "judgment_horizon_min": 10,     # 🔴 단일 고정(다중검정 회피)
        "aux_horizons_min": (30,),      # 참고 병기 — 판정 무관
        "strength_tiers": (0.60, 0.70), # 약한 MR / 강한 MR 층 (MR_EXHAUSTION_MIN 계열)
        "require_mr_eligible": True,    # 🔴 vwap 조건 동반분만 판정 모집단
        "drop_worst_day": True,         # 이상치 1일로 통과하지 못하게
        "consecutive_weeks_required": 2,
    },
    # ⚠ [MW0602 체리픽 주] v9-dev 원 커밋은 이 두 채널을 529차 채널군 앞에 두었으나
    #    이 브랜치에는 529차 채널이 없어 마지막 채널 뒤에 붙였다. 내용은 동일하다.
...
# ⚠ [MW0602 495차 / 554차 단위 정정] 이 PC의 실측 요율은 0.0019%다.
#   🔴 **두 배수를 섞어 쓰지 말 것**(계측 4원칙 ① — 단위 명시). 495차 원문이
#      「핀 과소 배수는 1.27배다(왕복 0.0712pt → 0.0795pt)」라 적었는데, 1.27은
#      **요율** 배수이고 괄호 안 두 값의 비는 **1.12**다. 슬리피지 0.04pt가
#      양쪽 동일해 배수를 희석하기 때문이다. 판정에 영향을 주는 것은 뒤쪽이다.
#
#        | PC              | 요율 배수 | **왕복비용 배수** |
#        | MW0602 (CREON)  |   1.27    |      **1.12**     |  0.0712 → 0.0795 pt
#        | MW0601 (CYBOS)  |   6.54    |      **3.43**     |  0.0712 → 0.2441 pt
#
#   그래도 핀 원칙은 동일하게 유지한다 — 사전등록 측정값의 재정의는 배수 크기와
#   무관하게 주간회의 승인 사항이다.
#   ⚠ 해제 시 **어느 채널의 verdict가 뒤집히는지**는 `--impact`가 답하지 않는다.
#     `scripts/cost_pin_release_preview.py`(554차)가 핀/해제 두 값으로 각 채널을
#     평가해 나란히 보여준다. 2026-09-09 실측: **14채널 전부 동일, 뒤집힘 0건**.
...
# [MW0602 526차 후속8 / P-4] `REGIME_EXHAUSTION_PARAMS` 사본 제거.
#   여기와 `collection/macro/micro_regime.py:26` 두 곳에 같은 dict 가 있었는데
#   **전수 grep 결과 양쪽 다 읽히지 않았다**(2026-09-03 조사보고 §2-5).
#   실동작은 `config/strategy_params.py` 의 `(macro, "EXHAUSTION")` 3종이 담당하고,
#   `hurst_override` 만 `main.py:8199` 가 `current_micro_regime == REGIME_EXHAUSTION`
#   상수 비교로 직접 구현한다. 정의는 `micro_regime.py` 쪽 하나만 남긴다
#   (그쪽이 레짐 상수와 같은 파일이라 원본이다).
