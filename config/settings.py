...
VALIDATION_CAMPAIGN = {
    ...
    "tp1_geometry_shadow": {
        ...
        #
        # [MW0601 421차 후속4, 2026-08-03 실측] **이 장치가 실제로 발동했다.**
        # OOS(n=22·MW0601)에서 tp1_x2가 current 대비 **−10.65pt로 열위**
        # (current −16.46 / tp1_x2 −27.11), 승률 63.6%→31.8%. 전체표본(n=77)의
        # +22.13pt 우위와 **부호가 반대**다 — 405차가 우려한 in-sample 오염이
        # 실측으로 확인됐다. PC간 부호 역전에 이어 **두 번째 반증**이며, 확정 결정
        # 레지스트리 note에 기록했다. 재론 시 이 두 반증을 함께 읽을 것.
...
VALIDATION_CAMPAIGN_DECISIONS = {
    ...
    "tp1_geometry_shadow": {
        ...
        "note": "MW0601에서 tp1_x2가 +32.78pt(현행 초과), MW0602에서 −19.04pt로 **PC간 부호가 "
                ...
                "⚠ **[MW0601 421차 후속4, 2026-08-03] OOS에서도 역전 — 반증이 두 겹이 됐다.** "
                "405차가 넣은 `oos_start=2026-07-31`(403차 배포 다음 거래일) 이후 신규분만 "
                "보면 tp1_x2가 현행 대비 **−10.65pt로 열위**다(current −16.46 vs tp1_x2 −27.11, "
                "n=22·MW0601). 승률도 63.6%→31.8%로 무너진다. 전체표본 우위(+22.13pt, n=77)는 "
                "403차가 **가설을 세운 그 기간의 in-sample**이라는 405차 우려가 실측으로 확인된 "
                "것이다. → 이 결정은 유지가 아니라 **강화**됐다. 완화·적용을 재론하려면 OOS "
                "표본이 min_samples를 넘긴 뒤 방향이 되돌아오는지를 먼저 볼 것. "
