...
### ⚠ 오탐 주의 — 크래시를 셀 때 분자를 잘못 잡는 법 [MW0602 494차 F-9]

`OOM` 사례와 **같은 계열**이다. 「무엇을 세는가」를 못박지 않으면 계측이 태어나자마자 죽는다.

| 문자열 | 쓸 수 있나 | 왜 |
|---|---|---|
| `logs/crash_signatures.log` 의 `[SIG]` 줄 | ✅ **분자** | 494차 F-10 이 회전 직전에 추출한 실제 크래시 블록. 일자 귀속은 각 줄의 `session=<ISO>` |
| `logs/crash_fault.log*` 의 `Windows fatal exception:` 행 | ✅ 당일 교차검증용 | ⚠ 4세대 FIFO 라 **과거는 사라진다**. 그리고 이 행은 **찢어질 수 있다** — 2026-08-25 실측 `Thread 0xWindows fatal exception: access violation`(주기 덤프와 크래시 덤프가 같은 fd 에 동시 기록). `grep -c` 는 잡지만 `^` 앵커는 놓친다 |
| 런처 `[AUTO-RESTART] main.py 재시작...` | ✅ **보조 판별자** | 재시작이 실제로 실행된 것. 실측(11 거래일 전수): **08-25 단 1건** — 그날이 유일한 실크래시일이다. 판별력이 있다 |
| 런처 `[AUTO-RESTART] … 일시적 크래시, 카운터 초기화` | 🔴 **분자로 쓰지 말 것** | **정상 종료에도 찍힌다.** 실측(11 거래일 전수, 2026-08-11~25): 이 줄이 있는 **10일 전부**에 `정상 종료 감지`가 같이 있고, 그중 실제 재시작은 **1일뿐**이다. 이것을 세면 `crash_signature` 가 **100% 고착이 구조적으로 보장**되어 지표가 아니라 상수가 된다 |
| 런처 `[AUTO-RESTART] 정상 종료 감지 -- 이유: auto_shutdown` | ✅ 정상 종료 판별 | 이 줄이 있는 날의 재시작은 크래시가 아니다 |

> 🔴 **`crash_signatures.log` 가 아예 없으면 `0(무사고)` 가 아니라 「미측정」이다.**
> 수집기는 그렇게 구현돼 있다(파일 부재 → 표본 없음). 계측이 배선되기도 전에 안심을
> 만드는 것이 FP-CRITICAL(2개월간 PSI=0.0)이 실제로 저지른 일이다 — 계측 4원칙 ②.

### ⚠ 오탐 주의 — 「예약」 로그를 「실행」으로 읽는 법 [MW0602 570차 후속 / G-1]

`[WarmupRetrain] 세션 재시작 감지 → GBM 즉시 재학습 예약` (`main.py:3778`) 은
**그 자리에서 재학습을 돌리지 않는다.** 「즉시」는 예약 플래그를 세운다는 뜻이고, 실제
소비는 **08:55 `[PreRetrain]` 단계**(`main.py:5289~5354`)에서 일어난다 — 전날 EOD
재학습이 성공했으면 `[PreRetrain] 08:55 사전 재학습 스킵 — … EOD 재학습 성공` 으로
**스킵되는 것이 정상**이다.

> 즉 「예약」 줄만 있고 그날 `[GBM] 배치 재학습` 이 없다고 해서 재학습이 누락된 것이
> 아니다. 판정하려면 **두 줄을 짝지어라** — 예약(`[WarmupRetrain]`) → 소비
> (`[PreRetrain]` 스킵 또는 실행). 2026-09-15 장전 점검이 예약 줄만 보고 누락을
> 의심해 `main.py` 를 직접 재추적해야 했다.

### 🔴 「동결」과 「종료」를 구분하는 법 — 하트비트 공백만으로는 못 가른다 [MW0602 570차 후속]

**프로세스가 죽으면 하트비트도 멈춘다.** 그래서 `[TS]` · 하트비트 파일 · SYSTEM.log 가
동시에 정체한 구간을 보고 **곧바로 「동결」이라 부르면 안 된다** — 「그 시간에 프로세스가
아예 없었다」와 구분이 되지 않는다. FZ-2 센티넬(`scripts/freeze_sentinel.py`)의 3신호
CRITICAL 도 마찬가지다: 센티넬은 **신호의 나이**를 재지 프로세스 생사를 재지 않는다.

가르는 증거는 `logs/crash_fault.log` 의 생명주기 줄이다:

| 관측 | 뜻 |
|---|---|
| 공백 구간 **앞**에 `[CLEAN EXIT] <ISO> PID=<pid>` 가 있다 | **종료**다. 동결이 아니다 |
| 공백 구간에 `[CLEAN EXIT]` 도 `[START]` 도 없다 | 프로세스는 살아 있는데 스레드가 멈췄다 = **동결** |
| 공백 **뒤** 첫 줄이 `[START] … PID=<새 pid>` | 재기동 시점. 앞뒤 PID 가 다르면 같은 프로세스가 아니다 |

> **실측 (2026-09-15)**: `[TS] 15:27:01` 다음 줄이 `[CLEAN EXIT] 15:27:18 PID=13544`,
> 그다음이 `[START] 15:32:47 PID=6644` 였다. 즉 15:27–15:32 는 동결이 아니라
> **프로세스 부재**였고, 런처 로그가 이유까지 적고 있었다 —
> `[AUTO-RESTART] 15:10 이후 종료 -- 재시작 안 함 (오버나이트 금지)`. **설계된 동작이다.**
> 그날 장후 리포트 1-9 가 이것을 「6분 38초 완전 동결」로 적었다가 자동조치에서 정정됐다.

> ⚠ FZ-1(`utils/freeze_watchdog.py`)이 발화하지 않은 것도 정상이다 — 감시 대상
> 프로세스가 이미 없었다. **발화 마커를 추가해도 이 구분에는 쓸 수 없다**(죽은 프로세스는
> 마커도 쓰지 못한다). 구분은 위 표의 `[CLEAN EXIT]`/`[START]` 로 한다.


### ⚠ 오탐 주의 — `[OptionFlow]` INFO 가 하루 한두 줄뿐인 것은 **정상이다** [MW0602 582차 후속 / G-3B]

주간옵션 수급 수집기(`collection/cybos/weekly_option_flow.py`)는 **매분 돈다.** 그런데
성공 로그는 **오늘 첫 성공 1회 + 그 뒤 30분마다**만 INFO 로 남는다
(`_info_every_sec = 1800.0`, 같은 파일 `:140`). 나머지 분은 `logger.debug` 인데
**DATA 로그에 DEBUG 레벨이 기록되지 않는다**(실측 0건) — 즉 아예 안 보인다.

> 그래서 하루치 `[OptionFlow]` INFO 는 **최대 13줄 안팎**이고, 늦게 떴거나 장중에
> 재기동했으면 **1줄일 수도 있다.** 줄 수가 적다고 「한 번 돌고 멈췄다」로 읽지 마라.

**판정은 줄 수가 아니라 그 줄이 들고 있는 값으로 한다.**

| 관측 | 뜻 |
|---|---|
| `[OptionFlow] stored=N rows … 최신봉=HH:MM` 의 **최신봉이 전진**한다 | 살아 있다. 두 줄만 있어도 판정된다 |
| INFO 가 1줄뿐이고 그 뒤 `[CLEAN EXIT]`/`[START]` 가 있다 | 재기동이다. 위 「동결과 종료」 절로 갈라라 |
| `[OptionFlow] 부분/전체 실패 streak=N` (WARNING) | 실패는 **별도 경로**다 — 연속 1회째 + 이후 10회마다만 찍힌다. `streak` 값을 읽어라 |
| INFO·WARNING 이 **둘 다** 없다 | 그때만 수집기 부재를 의심한다 |

> **DB 파일 mtime 을 1차 증거로 쓰지 마라.** 2026-09-21 O-83 판정이 그렇게 했는데,
> 그것은 간접 증거이고 `최신봉=` 이 이미 같은 사실을 직접 말하고 있었다 —
> 493차 §⑤(이미 받고 있는데 안 보고 있는 것) 계열이다.

⚠ **이 스로틀은 사고가 아니라 결정이다.** 2026-09-21 MW0601 `b35d363` 이 의도적으로
넣었다(매분 INFO 면 하루 390줄). **줄이 적다는 이유로 스로틀을 되돌리자고 제안하지 말 것** —
필요한 것은 더 잦은 로그가 아니라 위 표대로 읽는 것이다.

⚠ **오전 구간이 DB 에 비어 있는 것도 수집 중단이 아니다.** 라이브 수집기는 매분
**최근 18행**만 받으므로, 늦게 뜨거나 재기동하면 그 이전이 없다(같은 커밋 실측:
재기동 직후 봉 범위 `11:42~12:26`). 메우는 수단은 `scripts/backfill_option_flow.py`
(`type 3` 페이징)이며, **백필은 장후에 한다.**
