...
DEFAULT_CONFIG = {
    ...
    "threshold_reachability": {
        ...
        "pairs": {
            ...
            # ── [MW0602 500차 / G-5] 다섯 번째 쌍 — 「임계 좌초」 계통 감시 ────
            # 🔴 임계를 바꾸자는 것이 아니다. 0826 이상점 1-13 이 임계 좌초의
            #    **세 번째 사례**였고, 앞의 둘은 각각 다른 세션이 우연히 발견했으며
            #    그 사이 걸린 시간이 3개월·6개월이었다. 매일 보이게 하는 것까지가
            #    이 쌍의 몫이고, 완화 여부는 주간회의 소관이다(main.py:5849 주석).
            # ⚠ 위 `DriftRetrain조건B(n≥15)` 는 **결과 축**(n)이고 이것은 **원인 축**
            #   (conf)이다. 나란히 두면 "표본이 안 쌓인다"와 "왜 안 쌓이는가"가
            #   한 표에서 읽힌다.
            "SGD표본게이트(5m conf≥0.52)": {
                "db": "data/db/predictions.db",
                "sql": "select substr(ts,1,10) d, max(confidence) from predictions "
                       " where horizon = '5m' and direction != 0 "
                       "   and ts >= ? and ts <= ? || ' 23:59:59' group by 1",
                "threshold": 0.52,
                "known": "1-13 등록(0826) — 주간회의 2026-08-28 안건 ②. "
                         "🔴 그 전에 임계·표본 게이트 완화 금지"
                         "(490차 P0 학습 위생과 얽힘 · 사전등록 458차 D6)",
                "why": "SGD 온라인 학습의 고신뢰 레이블 필터(main.py `_min_conf_sgd=0.52`). "
                       "STEP 3 장중 재학습 조건 A·B 가 세는 표본 `n` 의 **유일한 입력 "
                       "경로**라, 여기서 좌초하면 그 아래 두 분기가 통째로 도달 불가가 "
                       "된다. 2026-08-26 실측 5m max(conf)=0.4580 → 당일 통과 0건. "
                       "⚠ 판정 기준 무변경 — 관측 전용이며 어떤 합격선도 건드리지 않는다",
            },
        },
    },
    # 텍스트로 훑을 기준 문서
