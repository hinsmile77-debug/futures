...
DEFAULT_CONFIG = {
    ...
    "day_summary_patterns": {
        ...
        # [MW0601 482차 / F-3] CB⑤가 재는 값 — 메인 스레드 정지와 **단위가 다르다**.
    ...
    "invariants": [
        ...
        # --- [MW0601 498차 / F-3] 비용 모델 3행 (2026-08-26 추가) -------------------
        # 왜 감시하는가: 2026-05-11 브로커 전환(키움→Cybos) 때 이 요율이 갱신되지
        # 않아 **6개월간 실제의 1/6.54**만 차감됐고, 그 사이 어떤 계측에도 걸리지
        # 않았다. 조용히 바뀌고 · 틀려도 크래시가 없고 · 틀린 방향이 항상 낙관이다.
        # 고친 뒤에도 감시 목록에 없으면 FP-CRITICAL(죽은 게이트)·TOX(죽은 섀도)와
        # 같은 계열의 재발이 된다. CLAUDE.md 26주 WFA 재검증 항목(493차 등록).
        #
        # ⚠ **기대값이 숫자가 아니다.** 495차 후속이 이 상수를 **로그인 채널 감지로
        #   파생**시켰다(CYBOS 0.0098104% / CREON 0.0019%, 5.16배 차이). 그래서
        #   지켜야 할 불변식은 "값이 얼마인가"가 아니라 **"파생을 유지하는가"** 다 —
        #   여기에 숫자 리터럴이 다시 나타나면 그것이 회귀 신호다.
        #   해석된 실제 요율은 `trades.commission_rate_used`(행별 세대 표기)로 확인한다.
        {"name": "FUTURES_COMMISSION_RATE",
         "expect": '_BROKER_SPEC["one_way_commission_rate"]',
         "why": "495차 후속 — 로그인 채널 감지로 **파생**. 숫자 리터럴로 되돌아가면 회귀"
                "(2026-05-11~08-25 6개월간 1/6.54 사고). 실제 요율은 채널별로 다르다"},
        {"name": "FUTURES_COMMISSION_RATE_EFFECTIVE_FROM",
         "expect": '_BROKER_SPEC["effective_from"]',
         "why": "시계열 불연속 경계 — 이 날짜 앞뒤 손익 직접 비교 금지의 근거(461차 mdd_pct 유형)"},
        {"name": "COST_MODEL_COMMISSION_RATE", "expect": "0.000015",
         "why": "캠페인·섀도 계측 전용 요율. 라이브와 **의도적으로 갈라져 있다**(493차 F-3 핀). "
                "주간회의 승인 시 라이브와 같은 값으로 교체 — 그때 이 기대값도 함께 바꿀 것"},
        {"name": "COST_MODEL_COMMISSION_RATE_PINNED", "expect": "True",
         "why": "라이브와 계측이 갈린 상태임을 매일 명시. 승인 교체 후에도 True면 그것이 이상"},
...
def day_summary(digests, cfg, out):
    ...
    if ex or pe:
        ...
        if len(_by_src) > 1 or "엔진" not in _by_src:
            ...
            A("")
        # ── [MW0601 518차 / G-4] 이월 포지션(전일 이전 진입)의 손익을 드러낸다 ──
        #
        # **증상**: 2026-09-02 다이제스트가 실재하는 **-5,299,668원**을 손익 표
        # 어디에도 싣지 않고, 정합성 줄에 `귀속 실패 레그 N행 ⚠` 한 마디로만 적었다.
        # 그날 헤드라인은 그 손실을 **뺀** 값이라, 다이제스트만 본 사람은 오늘을
        # 소액 흑자로 오독한다(이 리포트 작성자는 DB 를 직접 대조해 바로잡았다).
        #
        # **원인**: `assemble_positions()` 는 여는 이벤트(`[Position] 진입` /
        # FLAT→보유 체결)가 **오늘 로그 안에 있어야** 포지션을 연다. 전일 진입분은
        # 오늘 로그에 여는 이벤트가 없으므로 청산 레그가 `orphans` 로 떨어지고,
        # `tot_won` 은 `closed` 만 더하므로 그 금액이 통째로 사라진다.
        #
        # ⚠ **09-02 리포트의 제안(`entry_ts` 가 오늘이 아니면 이월로 표시)은 이
        #    도구에 적용할 수 없다** — 수집기는 **DB 를 읽지 않는다**(로그·설정·git
        #    전용, SKILL.md §2). `entry_ts` 라는 필드 자체가 여기 없다. 로그만으로
        #    같은 것을 판정하는 방법이 **"오늘 여는 이벤트가 없는 청산 레그"**, 즉
        #    이미 계산돼 있는 `orphans` 다. 제안의 의도를 로그 축으로 옮겨 구현한다.
        #
        # 🔴 **합산하지 않고 병기한다.** 이월분을 헤드라인에 그냥 더하면 이번에는
        #    「오늘 엔진 성적」이 전일 사고로 오염된다(507차 F-8 이 출처별 소계를
        #    따로 둔 것과 같은 이유). 세 값을 **전부** 보여주고 무엇이 무엇인지
        #    적는다 — 미측정을 0 으로도, 합계로도 뭉개지 않는다(계측 4원칙 ②·③).
        if orphans:
            _orph_won = sum(_won(r) for r in orphans)
            _orph_pt = sum(_pt(r) for r in orphans)
            A("**이월 포지션(전일 이전 진입) 추정 — 청산 레그 %d행 · %+.2fpt · %s원**"
              % (len(orphans), _orph_pt, format(_orph_won, "+,d")))
            A("")
            A("> 🔴 **위 포지션 표 합계(%s원)에 이 금액은 들어 있지 않다.**"
              " 오늘 로그에 여는 이벤트(`[Position] 진입` 또는 FLAT→보유 체결)가"
              " 없는 청산 레그라 포지션으로 조립되지 않는다 — **데이터 손실이"
              " 아니라 귀속 불가**이며, 금액 자체는 체결 실측이라 정확하다."
              % format(tot_won, "+,d"))
            A(">")
            A("> **오늘 계좌에서 실제로 오간 돈 = %s원**"
              " (오늘 연 포지션 %s원 + 이월분 %s원). 판정 원천은 여전히 브로커"
              " 실측 net 이다(493차 F-1) — 이 줄은 로그 축의 교차확인용이다."
              % (format(tot_won + _orph_won, "+,d"),
                 format(tot_won, "+,d"), format(_orph_won, "+,d")))
            A(">")
            A("> ⚠ 이월분이 있다는 것은 **전 거래일이 FLAT 으로 끝나지 않았다는 뜻**이다"
              " — 절대원칙 §1(15:10 강제청산) 위반 여부를 전일 리포트로 확인할 것.")
            A("")
            A("| 시각 | 종류 | 계약 | PnL(pt) | PnL(원) | 사유 |")
            A("|---|---|---|---|---|---|")
            for r in orphans:
                try:
                    _oq = int(r.get("qty") or 1)
                except (TypeError, ValueError):
                    _oq = 1
                A("| %s | %s | %d | %+.2f | %s | %s |" % (
                    r.get("hhmm"), "이월", _oq, _pt(r), format(_won(r), "+,d"),
        ...
        if orphans:
            # [518차 / G-4] 건수만 적으면 "정합성 경고" 로만 읽힌다 — 금액을 함께
            # 싣고 위 이월 블록을 가리킨다. 그 금액이 헤드라인에서 빠져 있다는
            # 사실이 이 줄에서도 보여야 한다.
            _bits.append("**귀속 실패(이월 추정) 레그 %d행 · %s원 ⚠** — 위 「이월 포지션」"
                         " 블록 참조, **헤드라인 합계에 미포함**"
                         % (len(orphans), format(sum(_won(r) for r in orphans), "+,d")))
        A("**정합성**: " + " · ".join(_bits))
        A("")
