...
"""
[2026-07-05 확인] 이 클래스는 main.py 어디서도 인스턴스화되지 않는 미사용 레거시다.
실제 실거래 청산 로직은 `main.py:_check_exit_triggers()`에 인라인으로 구현되어 있으며
우선순위도 다르다(1순위 하드스톱[인트라바 포함]→3순위 TP1/TP2 부분청산→3.5순위 신호
소멸청산→4순위 15:10 시간강제청산). 이 파일의 P1~P6 문서는 설계 의도의 기록일 뿐
실제 코드 경로가 아니므로, 청산 로직을 변경할 때는 반드시 main.py 쪽을 수정할 것.
(근거: docs/260704_SYSTEM_AUDIT_UPGRADE_PROPOSAL.md 로드맵 작업 중 발견)
