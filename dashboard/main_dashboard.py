...
class MinuteChartCanvas(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # [MW0601 598차] 가로 전일정 격자 보류 사유. `__init__` 에서 명시
        #   초기화한다 — `getattr(self, "_x", None)` 로만 읽으면 미설정이
        #   조용히 「없음」이 된다(계측 4원칙 ④).
    ...
    # ⚠ [체리픽] 시그니처는 **이 브랜치 것**을 지킨다 — GP 섀도(553차)는 v9-dev 전용이라
    #   `gp_trades`/`gp_wired` 를 들여오면 호출부가 전부 깨진다.
    ...
    def _draw_live_overlay(self, painter):
        ...
        if li is not None and self._live_candle:
            ...
            for _x1, _y1, _col, _w, _st in m.get("live_links") or ():
                ...
                # [dev 이식] dev 에는 `_draw_link_line` 이 없다(v9-dev 555차 후속2 전용) — 펜으로 직접
                _pen = QPen(_col)
                _pen.setWidthF(float(_w))
                _pen.setStyle(_st)
                painter.setPen(_pen)
    ...
    def _draw_trade_spans(self, painter: QPainter, plot: QRectF, candles, index_map, lo: float, hi: float, padded_count: int):
        ...
        for trade in (self._completed_trades if _show else []):
            ...
            # [오버레이 P4a·P7 / dev 이식] 면 + 세로 경계 + 칩을 깔고 그 위에 기존 선.
            # ⚠ v9-dev 는 여기서 대각 연결선(_draw_link_line, 555차 후속2)으로 바꾸지만
            #   dev 는 진입가 수평선이다. 그 시각 변경은 555차 몫이라 가져오지 않고,
            #   569·571차가 더한 **면·경계·칩만** 얹는다. 축밖 캐럿도 555차 몫이라 제외.
            _col = QColor(pen.color())
            _ep = float(trade.get("exit_price") or 0.0)
            _y2 = self._price_to_y(_ep, plot, lo, hi) if _ep > 0 else y
            self._draw_trade_span_area(painter, x1, y, x2, _y2, _col, step, plot)
            ...
            if len(self._completed_trades) <= self.TRADE_CHIP_MAX:
                try:
                    painter.setFont(QFont("Consolas", 8))
                    _pl = "%+.2fp 미륵이" % pnl_pts
                    self._draw_label_chip(painter, min(x1, x2), min(y, _y2) - S.p(20),
                                          _pl, QColor(13, 17, 23, 225), _col, True)
                    _wh = "%s %s→%s · %s" % (
                        str(trade.get("direction") or ""), entry_dt.strftime("%H:%M"),
                        exit_dt.strftime("%H:%M"), str(trade.get("exit_reason") or "")[:14])
                    painter.setFont(QFont("Consolas", 7))
                    self._draw_label_chip(painter, min(x1, x2), max(y, _y2) + S.p(4),
                                          _wh, QColor(13, 17, 23, 205), _col, False)
                except Exception as _e:
...
class MinuteChartDialog(QDialog):
    ...
    # [날짜선택 A단계 / 567차] 날짜 목록(배경 조회) → 메인 스레드 전달.
