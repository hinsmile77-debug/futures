...
class TradingSystem:
    ...
    def __init__(self):
        ...
        # [MW0602 497차 / P3] 당일 엔진 수수료 합 캐시 (ts, date, krw) — ProfitGuard
        # net 추정용. 60초 캐시로 매분 파이프라인의 반복 SQL을 막는다.
        ...
        # [MW0601 542차 이식] 수동 맥점 산출 — 관측 전용(진입·청산·사이징 무관)
        ...
        execute(
            ...
            """INSERT INTO trades
               (entry_ts, exit_ts, direction, raw_direction, executed_direction,
                ...
                exit_trigger, exit_outcome, commission_rate_used)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                       ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                ...
                _exit_trigger,
                _exit_outcome,
                # [MW0602 497차 / P2] 이 행의 수수료를 계산한 편도 요율.
                # 493차가 컬럼·백필을 만들었지만 **라이브 INSERT에는 빠져 있어**
                # 마이그레이션 이후 행이 전부 NULL이었다(2026-08-26 실측 8행).
    ...
    def _on_manual_levels_requested(self) -> None:
        """[MW0601 542차 이식] 수동 맥점 산출 — 대시보드 버튼 → 클릭 시각 기준 재산출.
    ...
    def run_minute_pipeline(self, bar: dict):
        ...
        if str(getattr(getattr(self, "broker", None), "name", "") or "").strip().lower() == "cybos":
            ...
            if _cached_date == _today_key and _cached_realized is not None:
                try:
                    # [MW0602 497차 / P3] 축 정합 — 종전에는 CpTd6197 실현손익
                    # (**gross**)을 그대로 썼고 엔진 폴백은 net이라 분기에 따라
                    # 축이 갈렸다. gross는 손실 시 net보다 덜 음수라 **손실 한도가
                    # 늦게 발동**한다(낙관 방향 — 493차 결함과 같은 방향).
                    # gross − 당일 엔진 수수료 합 = net 추정(브로커 실측 대비
                    # 오차 ~10원 수준, 2026-08-26 실증 429,644 vs 429,636).
    ...
    def run(self):
        ...
        # 종전에는 300초 주기라 장중 재기동 시 옵션 체인 카드(PCR·GEX·ATM OI)가
        # 최대 **5분간 「미수집」** 이었다. 2026-09-21 에는 재기동이 8회여서 그
        # 구멍이 실제로 여러 번 화면에 보였다.
        # `is_due()` 가 경과 시간으로만 판단하므로 중복 기동 위험은 없다.
        # ⚠ v9-dev 는 여기에 17초 오프셋(478차 FZ-1)이 더 있고 heartbeat 타이머도
        #   이 자리에 있다. dev 는 heartbeat 를 다른 곳에 두고 오프셋이 없으므로
        #   **이 브랜치 구조를 유지**한 채 612차 한 줄만 얹는다(체리픽 충돌 해소).
...
def _ts_engine_commission_today(self, today_key: str) -> float:
    """[MW0602 497차 / P3] 당일 엔진 수수료 합(원) — ProfitGuard net 추정용.
...
def _ts_push_balance_to_dashboard(self, result: dict, *, quiet: bool = False) -> None:
    ...
    # ── [MW0602 497차 / P1] 실시간잔고 스트립 축 정합 — 표시 전용 키 ──────────
