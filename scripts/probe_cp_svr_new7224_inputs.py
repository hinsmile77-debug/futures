...
def main() -> int:
    parser = argparse.ArgumentParser(description=f"Sweep input candidates for {PROGID} SetInputValue(0, ...)")
    ...
    header_indexes = [int(p.strip()) for p in args.header_indexes.split(",") if p.strip()]
