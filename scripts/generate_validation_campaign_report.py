...
def _spearman(x, y) -> float:
    ...
    [MW0601 456차 / Wave 4, MW0602 460차 — 두 세션이 독립적으로 발견·수정한 동일 버그]
    ...
    두 축의 행 순서가 같으면(같은 쿼리 결과) 동률군 내부 순위가 양쪽 다 "시간 순서"를
    대리하게 되고, 그 공유된 위치 성분이 상관으로 읽히는 계통편향(부호는 항상 +)이
    있었다. **영향** — 동률이 많은 축에서 없는 상관이 만들어진다. 특히 이산 소범위
    변수(청산수량 {1,2,3}, pass_count, 이진 플래그)에서 심하다. 456차 [17-Q] 설계 중
    합성데이터 검증으로 드러났다(관계 없는 데이터에 rho=+0.17, CI가 0을 배제).

    평균순위(midrank)로 교체한다 — Spearman의 표준 정의다.

    ⚠ **계측 불연속 경계 2026-08-10** — DECISION_LOG 456차 Wave 4 참조. 이 헬퍼를 쓰는
    기존 채널의 rho 값이 바뀐다(정정 방향). 동률이 없는 연속형 축은 사실상 무변화이고,
    **양쪽 축에 동시에 동률군이 있고 그 동률군이 시간에 뭉쳐 있을수록** 크게 바뀐다
    (수량·pass_count·플래그 축). 독립 데이터 몬테카를로(참값 rho=0, MW0602 460차 실측):
    연속×연속 편향 -0.003 → 정정 후 동일 / 수량×승패플래그 n=60 +0.023 → +0.001 /
    수량군이 시간에 뭉친 경우 n=60 **+0.110 → -0.001**. MW0602의 2026-08-10 시점
    호출부 4곳은 실측 |Δ| ≤ 0.0045로 판정 무변화였다([1] ic_3class 전 호라이즌
    ≤0.0004, [3] unc_corr -0.0001, [12] edge_corr -0.0045).
    """
...
# ──────────────────────────────────────────────────────────────
# [57] trend_efficiency 진입 게이트 — resolve + 누적 판정 (MW0602 502차 U-2 체리픽)
# ──────────────────────────────────────────────────────────────
...
def build_report(days: int) -> tuple:
    ...
    # [MW0602 502차 U-2 체리픽] [57] te 진입 게이트 — 요약행에 **손해일**을 함께 띄운다.
    ...
    L.append("")
    # [MW0601 444차] 🔴 번호 충돌 고지 — 이 리포트에는 **[47]이 둘** 있다.
    # MW0602 435차가 §47로 이 게이트를, MW0601 436차가 §47로 "진입 맥락별 손익"을
    # 각각 등록했다(두 PC가 같은 주에 같은 번호를 쓴 사고). 파생 라벨 [47-B]는
    # **이 게이트 쪽**에 속한다 — 아래 고지가 없으면 오독된다.
    L.append("> ⚠ **번호 충돌 주의 — 이 리포트에는 `[47]`이 둘 있다.** 이 절(재현충실도")
    L.append("> 게이트, MW0602 435차)과 **[47] 진입 맥락별 손익**(MW0601 436차)은 **완전히**")
    L.append("> **다른 채널**이다. 두 PC가 같은 주에 §47을 각자 등록해 생긴 충돌이며,")
    L.append("> 파생 라벨 **`[47-B]`(TP 체결 오버슈트)는 이 게이트 쪽**에 속한다.")
    L.append("> 번호 재정리는 두 PC·과거 문서가 공유하는 식별자라 **주간회의 안건**이다.")
    L.append("")
    _gate = off.get("_sim_fidelity_gate") or {}
    ...
    L.append("")
    L.append("> ⚠ **번호 충돌 주의** — 위쪽 **[47] 시뮬레이터 재현충실도 게이트**")
    L.append("> (MW0602 435차)와 **다른 채널**이다. 두 PC가 같은 주에 §47을 각자 등록해")
    L.append("> 생긴 충돌이며, `[47-B]`는 **그쪽(게이트)** 파생이지 이 절의 파생이 아니다.")
    L.append("")
    if not _g47:
        L.append("> ⚠ 미산출 — 스크립트 실행 실패 또는 사전등록 누락.")
    elif _g47.get("error"):
    ...
    # ── [MW0602 502차 U-2 체리픽] [57] trend_efficiency 진입 게이트 (섀도) ──────────
    ...
    L.append("> 다른 정의로 가리킨다. 원 근거(MW0602): `docs/정기점검/손실6일_사전간파체계_개선방안_20260830.md` §5"
              "(이 문서는 v9-dev에 없음 — MW0602 자기 데이터 분석).")
    L.append("")
    L.append("---")
    L.append("")
