...
def eval_gp_cross_channels() -> dict:
    ...
    2. **비용이 핀값이 아니라 실측 요율이다.** 사전등록이 `cost_source=
       "BROKER_CHANNEL_SPECS"`로 명시했다(493차 — 핀값은 왕복비용을 낙관한다).
       ⚠ [554차 단위 정정] 원문의 「6.5배」는 **요율** 배수이고, 실제 **왕복비용**
       배수는 MW0601 3.43배 · **이 PC(CREON) 1.12배**다(슬리피지 0.04pt가 양쪽
       동일해 희석된다). 판정에 영향을 주는 것은 왕복비용 배수다 — 계측 4원칙 ①.
