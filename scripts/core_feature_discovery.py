...
def main():
    ...
    try:
        ...
        # 🔴 [MW0602 체리픽 조정] 원본은 `config.constants.CORE_FEATURES` 를 읽는다 —
        #   MW0601 은 그 상수를 실집행 정의 파생으로 바꿨기 때문이다. **이 브랜치의
        #   그 상수는 일부러 레거시**(468차 F-3 SHAP 보호축)라, 그대로 읽으면 이 절이
        #   실집행 CORE 가 아니라 레거시 이름을 스크린하게 된다 — A-6 의 취지
        #   ("체크리스트 게이트가 오염되면 전 호라이즌에 번진다")와 어긋난다.
        #   그래서 실집행 정의를 직접 읽는다. 레거시 이름은 어차피 관찰군에 있다.
        from config.settings import CORE_FEATURES_BY_GROUP as _CFBG
        _CORE = sorted({v for v in _CFBG["short"].values() if isinstance(v, str) and v})
        _watch = list(_CORE) + ["cvd_divergence", "cvd_slope", "cvd", "ofi_norm"]
