...
def _ensure_runtime() -> None:
    ...
    if struct.calcsize("P") != 4:
        raise RuntimeError("OptionCurrentRq COM requires 32-bit Python")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Probe Cybos option current request payload")
    parser.add_argument("--code", required=True, help="Option code to query, e.g. B0165A01")
    parser.add_argument(
        "--progid",
        default="Dscbo1.OptionCurrentRq",
        help="COM ProgID to dispatch (default: Dscbo1.OptionCurrentRq)",
    )
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Common cause: invalid ProgID. Known candidate: Dscbo1.OptionCurrentRq"
            ) from exc
