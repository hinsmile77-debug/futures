...
def _column_crosscheck(cfg, json_map):
    ...
    이 대조를 할 수 없었다. 여기는 원본이 살아 있으므로 매 실행마다 대조한다.

    ⚠ **[2026-08-20 정정]** 473차는 `sizing_trace`의 "배포 후 적재 0행"을 배선 결함
      가능성으로 읽었으나 **틀렸다** — 배포일(08-14)이 장 마감 후였고 다음이 휴장
      (08-17)이라 **거래일이 없었을 뿐**이다. 08-18부터 정상 적재(59행/8행).
      "배포 후 N행"을 볼 때는 **그 사이 거래일 수**를 먼저 확인할 것.
    ...
    if not rows:
        return {"column_present": True, "n_column_rows": 0,
                "note": "컬럼은 있으나 적재 0행 — **그 사이에 거래일이 있었는지 먼저 "
                        "확인할 것**(휴장/장마감 후 배포면 0이 정상이다. 473차가 "
                        "sizing_trace에서 이 구분을 놓쳐 배선 결함으로 오독했다). "
                        "거래일이 지났는데도 0이면 배선이 죽은 것이다."}
