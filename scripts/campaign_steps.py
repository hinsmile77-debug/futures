...
def run_campaign_steps(logger, base_dir: str) -> None:
    ...
    steps = [
        ...
        # [MW0601 483차 후속5 / 2026-08-23 주간회의 결정 3] D9-B 라우팅 밴드 성과.
        #
        # 🔴 **474차에 사전등록까지 해놓고 발화 지점이 없었다.** `VALIDATION_CAMPAIGN
        # ["entry_band_watch"]`는 켜져 있는데 어떤 스케줄에도 연결돼 있지 않아,
        # 2026-08-23에 사람이 손으로 돌리기 전까지 **한 번도 판정이 나온 적이 없다.**
        # `monthly_cleanup.py`(2026-05 신설, 발화 지점 없어 미실행)와 같은 결함이다.
        # 하필 이 채널은 진입 호라이즌 경계 동결 결정(2026-08-23)의 **재개 조건**을
        # 판정하는 자리라, 안 돌면 그 결정을 되돌아볼 창구가 없다.
        #
        # 읽기 전용 stdout 판정이라 순서에 민감하지 않다. 판정 리포트 **앞**(읽기 전용
        # 채널 공통 규약). 장중이면 `guard_intraday`가 rc=2로 나가 SKIP(장중차단)으로
        # 집계된다 — 정규 EOD(15:45)는 걸리지 않는다.
        #
        # ⚠ [MW0602 484차] 이 PC는 **체인 등록만** 가져왔다 — 원 커밋 `b2f02db` 가
        #   함께 담은 「진입 호라이즌 경계 동결」 결정 3건(`config/settings.py`·`main.py`)은
        #   **매매 정책 변경**이라 주간회의 소관이며 미반영이다. 따라서 이 채널이 내는
        #   `BAND_ANOMALY` 를 "동결 재개 조건"으로 읽지 말 것 — 이 PC엔 동결 자체가 없다.
        ("라우팅 밴드 성과 [D9-B]", ["entry_band_watch.py"]),
