    ...
    for name in TARGET_FEATURES:
        ...
        for hz, h_min in HORIZONS.items():
            ...
            if len(idx_valid) >= 60:
                ...
                _, rho1 = stats.spearmanr(col[first_half], y[first_half]) if first_half.sum() > 20 else (None, None)
                _, rho2 = stats.spearmanr(col[second_half], y[second_half]) if second_half.sum() > 20 else (None, None)
