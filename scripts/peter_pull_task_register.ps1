...
#     BOM 은 메타데이터가 아니라 내용의 첫 3바이트라 git 이 그대로 나른다 --
#     받는 쪽에서 손으로 붙여도 다음 체크아웃이 덮는다. **보내는 쪽 블롭이 고쳐져야 한다.**
#
...
# ── BOM 자가진단 ────────────────────────────────────────────────────────
#  🔴 아래 「등록 검증」과 **겹치지 않는다.** 검증은 $Name 으로 되조회하는데,
#    BOM 이 없으면 $Name 자체가 똑같이 깨져 있다. 깨진 이름이 우연히 *유효한*
#    이름이면 등록이 성공하고 되조회도 그 깨진 이름을 찾아내 **검증을 통과한다** --
#    엉뚱한 이름의 작업이 조용히 등록된 채로. 2026-09-17 에 걸린 것은 깨진 이름이
#    마침 0x8007007b(잘못된 이름)였기 때문이지 검증이 이름을 본 것이 아니다.
#    그 경우를 잡는 것은 이 자가진단뿐이다. 등록 **전에** 멈춘다.
$canary = '피터'
if ($canary.Length -ne 2) {
    Write-Warning "이 .ps1 이 UTF-8 로 읽히지 않았다 - 파일 맨 앞 BOM(EF BB BF)이 사라진 것으로 보인다."
    Write-Warning "그대로 두면 작업 이름이 깨진 채 등록되거나 0x8007007b 로 죽는다."
