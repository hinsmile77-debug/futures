...
def _ensure_runtime() -> None:
    ...
    if struct.calcsize("P") != 4:
        raise RuntimeError(f"{PROGID} COM requires 32-bit Python")
...
def main() -> int:
    parser = argparse.ArgumentParser(description=f"Probe Cybos {PROGID} payload")
    parser.add_argument("--code", help="Optional input code (string or integer)")
    parser.add_argument("--progid", default=PROGID, help=f"COM ProgID (default: {PROGID})")
    ...
    parser.add_argument("--ensure-login", action="store_true", help="Run Cybos autologin if not connected")
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(f"Dispatch failed for progid={args.progid}") from exc
        ...
        if args.code:
            ...
            print("[INFO] no SetInputValue() applied (no-input probe)")
        ...
        data_fields = [int(p.strip()) for p in args.data_fields.split(",") if p.strip()]
