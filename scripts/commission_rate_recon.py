...
def _scan_lines(lines):
    ...
    🔴 **[MW0602 501차] 단, 「롤오버(장후 정산) 판독」은 제외한다.**
    ...
    마지막 줄을 무조건 쓰면 그런 날의 net이 −수수료로 기록된다. 실측 4일
    (2026-06-30 · 07-01 · 07-14 · 08-06)이 그렇게 오염돼 있었고, 08-06은
    −360,142가 −43,142로 기록돼 8월 손실 6위가 순위에서 사라졌다.

    **판별은 「예탁현금은 당일 시가 고정」이라는 불변식으로 한다** — 그날 첫
    판독과 예탁현금이 달라지면 롤오버다. 전 로그(49거래일 · 2,164줄) 검증에서
    **오탐 0 · 미탐 0**으로 정확히 그 4일만 잡는다.

    ⚠ 「실현손익 0 && 예탁≠익일가」로는 안 된다 — 미실현이 익일가를 움직이는
    장중 초반 줄이 걸려 **33일이 오탐**된다(설계 중 실제로 한 번 밟았다).

    ⚠ 요율 추정에는 영향이 없다 — 어느 판독을 쓰든 `gross − net = 수수료`가
    같은 값으로 성립한다(4일 모두 실효요율 0.001900%로 동일).
    """
    dep = nxt = gross = None
