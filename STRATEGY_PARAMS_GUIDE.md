...
> **[296차 갱신, 2026-07-06]** 30m 호라이즌은 앙상블/CoherenceGate/CascadeCoherence에서 전면 퇴역했다
> (`full_cv acc=0.3052`, 구조적으로 랜덤 이하 — 재도입 전까지 확정). 아래 `ensemble_w_30m`,
> `cb_accuracy_min_30m`, `threshold_30m`은 코드상 값은 유지되나 진입 판단에는 사용되지 않는다.
> **[모의투자 한정 한시 예외]** `cb_consec_stop_limit`은 표에 3으로 적혀 있으나 현재
> `CB_CONSEC_STOP_LIMIT=9999`로 사실상 비활성 — 실투 전환 전 2~5 범위로 복원 필수
> (사유·복원 조건은 CLAUDE.md §절대 원칙 참조).
