# 호라이즌별 피처셋 주간 건강 리포트 — MW0602 · 2026-09-04
...
- 건강도 관찰창: **2026-08-07 ~ 2026-09-04** (20거래일, 7,673행)
- 후보 축적 관찰창: 2026-06-09 ~ 2026-09-04 (60거래일)
...
### 1m — 배포 10개 (pkl 2026-09-01 15:45)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| WARN | `is_open_volatile` | 7673 | 88.5% | 88.5% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 3m — 배포 12개 (pkl 2026-09-01 14:49)

전 피처 OK.

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 5m — 배포 13개 (pkl 2026-09-04 15:45)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| CRITICAL | `is_close_volatile` | 7673 | 97.8% | 97.8% | 2 | 상태플래그(상수 정상) |
| WARN | `is_open_volatile` | 7673 | 88.5% | 88.5% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 10m — 배포 13개 (pkl 2026-09-04 15:46)

전 피처 OK.

CORE(mid): `vwap_position`=OK

### 15m — 배포 13개 (pkl 2026-09-04 15:46)
...
### 30m — 배포 17개 (pkl 2026-09-04 15:46)
...
| 계단형 | `cvd_direction_debias` | 93.1% | 0.8314 | 6.0분 | 계측만 |
| 계단형 | `multi_timeframe_5m` | 90.3% | 0.8719 | 10.0분 | 후보: POOL |
| 계단형 | `opt_atm_call_oi` | 91.2% | 0.9051 | 10.0분 | **배포: 30m** |
| 계단형 | `opt_atm_pcr` | 90.2% | 0.9154 | 10.0분 | **배포: 30m** |
| 계단형 | `opt_atm_put_oi` | 91.9% | 0.9124 | 10.0분 | 후보: pending:15m |
| 계단형 | `opt_chain_pcr` | 87.3% | 0.6320 | 10.0분 | **배포: 5m, 30m** |
| 계단형 | `opt_gex_bn` | 87.0% | 0.9371 | 10.0분 | **배포: 30m** |
| 상수형 | `above_vwap` | 95.0% | 0.8168 | 2.8분 | **배포: 5m, 30m** |
| 상수형 | `bull_exhaustion_shadow` | 99.0% | -0.0026 | 1.0분 | 계측만 |
| 상수형 | `bull_reversal_signal` | 95.2% | -0.0241 | 1.0분 | 계측만 |
| 상수형 | `feature_quality_score` | 99.2% | 0.9146 | 1.0분 | 계측만 |
...
| 상수형 | `multi_timeframe_15m` | 96.2% | 0.9286 | 15.0분 | 후보: POOL |
| 상수형 | `opt_pcr_bearish` | 97.8% | 0.9078 | 5.5분 | **배포: 30m** |
| 상수형 | `opt_pcr_bullish` | 98.4% | 0.8812 | 4.8분 | 계측만 |
| 상수형 | `opt_pcr_extreme` | 98.6% | 0.9072 | 8.5분 | **배포: 10m** |
| 상수형 | `opt_pcr_extreme_bearish` | 98.6% | 0.9072 | 8.5분 | 계측만 |
| 상수형 | `opt_pcr_extreme_bullish` | 98.5% | 0.8634 | 4.0분 | 계측만 |
...
|---|---|---|---|---|---|
| 10m | 0.527 (584) | 0.600 (10) | 0.46 | 0.6484 | INSUFFICIENT |
| 15m | 0.465 (187) | 0.625 (16) | 1.23 | 0.2195 | INSUFFICIENT |
| 3m | 0.504 (2188) | 0.429 (14) | -0.56 | 0.5731 | INSUFFICIENT |
| 5m | 0.483 (1121) | 0.333 (6) | -0.73 | 0.4629 | INSUFFICIENT |

## 4. 후보 파이프라인 현황판

| 후보 | 출처 | 축적(최근 60거래일) | 건강 | 상태 |
|---|---|---|---|---|
| `basis_change_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 43일 | OK | 📌 승격 보류 — basis_pt와 동일 검정에서 함께 재현 실패 |
| `basis_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 43일 | OK | 📌 승격 보류 — 표본 확대 후 신호 소멸(재현 실패) |
...
| `hurst_ready` | POOL | 48일 | WARN | 검증가능 (48일) |
| `imbalance_slope` | POOL | 60일 | OK | 검증가능 (60일) |
| `is_month_end_rebalance` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 43일 | CRITICAL | 검증가능 (43일) |
...
| `opt_atm_put_oi` | pending:15m | 55일 | OK | 검증가능 (55일) · 📌 강등(include → pending) — magnitude 계열 재현 실패 |
...
| `swing20_high_age` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing20_high_dist_atr` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing20_low_age` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing20_low_dist_atr` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing20_range_pos` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing60_high_age` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing60_high_dist_atr` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing60_low_age` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing60_low_dist_atr` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing60_range_pos` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing_day_high_age` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing_day_high_dist_atr` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing_day_low_age` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing_day_low_dist_atr` | POOL | 1일 | OK | 축적중 1/20일 |
| `swing_day_range_pos` | POOL | 1일 | OK | 축적중 1/20일 |
| `trend_efficiency` | POOL | 37일 | OK | 검증가능 (37일) |
| `vkospi` | pending:10m, pending:15m, pending:30m | 43일 | OK | 검증가능 (43일) |
