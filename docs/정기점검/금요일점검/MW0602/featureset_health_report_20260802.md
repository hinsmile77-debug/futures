# 호라이즌별 피처셋 주간 건강 리포트 — MW0602 · 2026-08-02
...
- 건강도 관찰창: **2026-07-03 ~ 2026-07-31** (20거래일, 7,542행)
- 후보 축적 관찰창: 2026-04-30 ~ 2026-07-31 (60거래일)
...
### 1m — 배포 10개 (pkl 2026-08-01 12:36)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| WARN | `is_open_volatile` | 7542 | 88.1% | 88.1% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 3m — 배포 12개 (pkl 2026-08-01 12:36)

전 피처 OK.

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 5m — 배포 13개 (pkl 2026-08-01 12:36)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| CRITICAL | `is_close_volatile` | 7542 | 97.8% | 97.8% | 2 | 상태플래그(상수 정상) |
| WARN | `is_open_volatile` | 7542 | 88.1% | 88.1% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 10m — 배포 13개 (pkl 2026-08-01 12:36)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| WARN | `opt_gex_sign` | 7247 | 1.2% | 91.9% | 3 |  |

CORE(mid): `vwap_position`=OK

### 15m — 배포 13개 (pkl 2026-08-01 12:36)
...
### 30m — 배포 17개 (pkl 2026-08-01 12:36)
...
|---|---|---|---|---|
| `basis_change_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 19일 | OK | 📌 승격 보류 — basis_pt와 동일 검정에서 함께 재현 실패 |
| `basis_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 19일 | OK | 📌 승격 보류 — 표본 확대 후 신호 소멸(재현 실패) |
...
| `hurst_ready` | POOL | 24일 | WARN | 검증가능 (24일) |
| `imbalance_slope` | POOL | 60일 | OK | 검증가능 (60일) |
| `is_month_end_rebalance` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 19일 | WARN | 축적중 19/20일 |
| `is_monthly_expiry_week` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 19일 | OK | 축적중 19/20일 |
| `is_monthly_witching` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 19일 | WARN | 축적중 19/20일 |
...
| `opt_atm_put_oi` | pending:15m | 31일 | OK | 검증가능 (31일) · 📌 강등(include → pending) — magnitude 계열 재현 실패 |
...
| `vkospi` | pending:10m, pending:15m, pending:30m | 19일 | OK | 축적중 19/20일 |
