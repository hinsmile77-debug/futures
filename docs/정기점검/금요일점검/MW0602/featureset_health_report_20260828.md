# 호라이즌별 피처셋 주간 건강 리포트 — MW0602 · 2026-08-28
...
- 건강도 관찰창: **2026-07-31 ~ 2026-08-28** (20거래일, 7,669행)
- 후보 축적 관찰창: 2026-06-02 ~ 2026-08-28 (59거래일)
...
### 1m — 배포 10개 (pkl 2026-08-28 15:45)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| WARN | `is_open_volatile` | 7669 | 88.3% | 88.3% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 3m — 배포 12개 (pkl 2026-08-28 15:45)

전 피처 OK.

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 5m — 배포 13개 (pkl 2026-08-28 15:45)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| CRITICAL | `is_close_volatile` | 7669 | 97.9% | 97.9% | 2 | 상태플래그(상수 정상) |
| WARN | `is_open_volatile` | 7669 | 88.3% | 88.3% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 10m — 배포 13개 (pkl 2026-08-28 15:46)

전 피처 OK.

CORE(mid): `vwap_position`=OK

### 15m — 배포 13개 (pkl 2026-08-28 15:46)
...
### 30m — 배포 17개 (pkl 2026-08-28 15:46)
...
| 계단형 | `multi_timeframe_5m` | 90.4% | 0.8737 | 10.0분 | 후보: POOL |
| 계단형 | `opt_atm_call_oi` | 90.9% | 0.8993 | 10.0분 | **배포: 30m** |
| 계단형 | `opt_atm_pcr` | 90.9% | 0.8875 | 10.0분 | **배포: 30m** |
| 계단형 | `opt_atm_put_oi` | 91.2% | 0.8943 | 10.0분 | 후보: pending:15m |
| 계단형 | `opt_chain_pcr` | 87.4% | 0.6663 | 10.0분 | **배포: 5m, 30m** |
| 계단형 | `opt_gex_bn` | 87.0% | 0.9253 | 10.0분 | **배포: 30m** |
...
| 상수형 | `multi_timeframe_15m` | 96.3% | 0.9284 | 15.0분 | 후보: POOL |
| 상수형 | `opt_pcr_bearish` | 98.2% | 0.9152 | 6.5분 | **배포: 30m** |
| 상수형 | `opt_pcr_bullish` | 98.6% | 0.8767 | 4.8분 | 계측만 |
| 상수형 | `opt_pcr_extreme` | 99.2% | 0.9072 | 7.2분 | **배포: 10m** |
| 상수형 | `opt_pcr_extreme_bearish` | 99.2% | 0.9072 | 7.2분 | 계측만 |
| 상수형 | `opt_pcr_extreme_bullish` | 98.8% | 0.8713 | 4.0분 | 계측만 |
...
|---|---|---|---|---|---|
| 10m | 0.541 (564) | 0.483 (29) | -0.61 | 0.5411 | INSUFFICIENT |
| 15m | 0.512 (215) | 0.650 (20) | 1.19 | 0.2360 | INSUFFICIENT |
| 3m | 0.512 (2213) | 0.409 (22) | -0.96 | 0.3368 | INSUFFICIENT |
| 5m | 0.486 (1175) | 0.333 (6) | -0.75 | 0.4556 | INSUFFICIENT |

## 4. 후보 파이프라인 현황판

| 후보 | 출처 | 축적(최근 60거래일) | 건강 | 상태 |
|---|---|---|---|---|
| `basis_change_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 38일 | OK | 📌 승격 보류 — basis_pt와 동일 검정에서 함께 재현 실패 |
| `basis_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 38일 | OK | 📌 승격 보류 — 표본 확대 후 신호 소멸(재현 실패) |
| `cancel_ratio` | POOL | 0일 | raw부재 | 📌 구현불가 확정 — 데이터 원천 없음 (완전 사장) |
| `cvd_direction` | pending:10m | 59일 | CRITICAL | 📌 교체 완료 — cvd_delta_norm이 대체 (후보 목록의 잔존 항목) |
| `foreign_futures_net` | pending:10m | 59일 | OK | 검증가능 (59일) |
| `hurst_ready` | POOL | 43일 | WARN | 검증가능 (43일) |
| `imbalance_slope` | POOL | 59일 | OK | 검증가능 (59일) |
...
| `microprice_slope` | POOL | 59일 | OK | 검증가능 (59일) |
| `mlofi_norm` | POOL | 59일 | OK | 검증가능 (59일) |
| `multi_timeframe_15m` | POOL | 32일 | OK | 검증가능 (32일) |
| `multi_timeframe_5m` | POOL | 32일 | OK | 검증가능 (32일) |
| `ofi_reversal_speed` | pending:1m | 59일 | OK | 검증가능 (59일) |
| `opt_atm_put_oi` | pending:15m | 50일 | OK | 검증가능 (50일) · 📌 강등(include → pending) — magnitude 계열 재현 실패 |
| `program_arb_net` | pending:5m, pending:10m, pending:15m, pending:30m | 59일 | OK | 검증가능 (59일) |
| `program_non_arb_net` | pending:5m, pending:10m, pending:15m, pending:30m | 59일 | OK | 검증가능 (59일) |
| `queue_depletion_speed` | POOL | 59일 | OK | 검증가능 (59일) |
| `queue_momentum` | POOL | 59일 | OK | 검증가능 (59일) |
| `queue_refill_rate` | POOL | 59일 | OK | 검증가능 (59일) |
| `queue_signal` | POOL | 59일 | OK | 검증가능 (59일) |
| `round_number_distance` | POOL | 32일 | OK | 검증가능 (32일) |
| `rv_iv_spread` | POOL | 31일 | OK | 검증가능 (31일) |
| `trend_efficiency` | POOL | 32일 | OK | 검증가능 (32일) |
| `vkospi` | pending:10m, pending:15m, pending:30m | 38일 | OK | 검증가능 (38일) |
