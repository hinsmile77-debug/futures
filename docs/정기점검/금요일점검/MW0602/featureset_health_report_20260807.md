# 호라이즌별 피처셋 주간 건강 리포트 — MW0602 · 2026-08-07
...
- 건강도 관찰창: **2026-07-10 ~ 2026-08-07** (20거래일, 7,583행)
- 후보 축적 관찰창: 2026-05-11 ~ 2026-08-07 (60거래일)
...
### 1m — 배포 10개 (pkl 2026-08-07 15:45)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| WARN | `is_open_volatile` | 7583 | 88.2% | 88.2% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 3m — 배포 12개 (pkl 2026-08-07 14:55)

전 피처 OK.

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 5m — 배포 13개 (pkl 2026-08-07 14:55)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| CRITICAL | `is_close_volatile` | 7583 | 97.8% | 97.8% | 2 | 상태플래그(상수 정상) |
| WARN | `is_open_volatile` | 7583 | 88.2% | 88.2% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 10m — 배포 13개 (pkl 2026-08-07 15:46)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| WARN | `opt_gex_sign` | 7288 | 0.6% | 92.5% | 3 |  |
| WARN | `opt_pcr_extreme` | 7288 | 80.6% | 80.6% | 2 |  |

CORE(mid): `vwap_position`=OK

### 15m — 배포 13개 (pkl 2026-08-07 15:46)
...
### 30m — 배포 17개 (pkl 2026-08-07 15:46)
...
| CRITICAL | `bull_reversal_signal` | 98.2% | 98.2% | 계측만 (배포·후보 아님) |
...
|---|---|---|---|---|
| `basis_change_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 24일 | OK | 📌 승격 보류 — basis_pt와 동일 검정에서 함께 재현 실패 |
| `basis_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 24일 | OK | 📌 승격 보류 — 표본 확대 후 신호 소멸(재현 실패) |
...
| `hurst_ready` | POOL | 29일 | WARN | 검증가능 (29일) |
...
| `is_monthly_expiry_week` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 24일 | WARN | 검증가능 (24일) |
...
| `opt_atm_put_oi` | pending:15m | 36일 | OK | 검증가능 (36일) · 📌 강등(include → pending) — magnitude 계열 재현 실패 |
...
| `vkospi` | pending:10m, pending:15m, pending:30m | 24일 | OK | 검증가능 (24일) |
