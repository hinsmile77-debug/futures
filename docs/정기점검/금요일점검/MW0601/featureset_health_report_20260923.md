# 호라이즌별 피처셋 주간 건강 리포트 — MW0601 · 2026-09-23
...
- 건강도 관찰창: **2026-08-27 ~ 2026-09-23** (20거래일, 7,622행)
- 후보 축적 관찰창: 2026-06-30 ~ 2026-09-23 (60거래일)
- ℹ 관찰창 범위에서 백필 생성 행 3,649행(10일)을 **제외**했다 (418차 기본 동작). 미시구조가 0.0인 소급 행이라 포함하면 §2-b·§4가 왜곡된다. 포함하려면 `--include-backfill`.
...
### 1m — 배포 8개 (pkl 2026-09-23 15:50)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| WARN | `is_open_volatile` | 7622 | 88.6% | 88.6% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 3m — 배포 12개 (pkl 2026-09-23 15:50)

전 피처 OK.

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 5m — 배포 12개 (pkl 2026-09-23 15:51)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| CRITICAL | `is_close_volatile` | 7622 | 97.9% | 97.9% | 2 | 상태플래그(상수 정상) |
| WARN | `is_open_volatile` | 7622 | 88.6% | 88.6% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 10m — 배포 11개 (pkl 2026-09-23 15:51)

전 피처 OK.

CORE(mid): `vwap_position`=OK

### 15m — 배포 13개 (pkl 2026-09-23 15:52)
...
### 30m — 배포 11개 (pkl 2026-09-23 15:52)
...
| CRITICAL | `bull_reversal_signal` | 97.5% | 97.5% | 계측만 (배포·후보 아님) |
| DEAD | `cvd_flow_fallback` | 100.0% | 100.0% | 계측만 (배포·후보 아님) |
| DEAD | `cvd_flow_source` | 100.0% | 100.0% | 계측만 (배포·후보 아님) |
| CRITICAL | `feature_quality_score` | 0.0% | 95.1% | 계측만 (배포·후보 아님) |
| CRITICAL | `gp_cross_dn_20` | 96.4% | 96.4% | 계측만 (배포·후보 아님) |
| CRITICAL | `gp_cross_tight_20` | 96.7% | 96.7% | 계측만 (배포·후보 아님) |
| CRITICAL | `gp_cross_up_20` | 96.1% | 96.1% | 계측만 (배포·후보 아님) |
| DEAD | `ma_cont_ready` | 0.0% | 100.0% | 계측만 (배포·후보 아님) |
...
| 결정론형 | `feature_quality_score` | 99.2% | 0.9057 | 1.5분 | 계측만 |
| 결정론형 | `time_cos` | 3.7% | 0.9947 | 1.0분 | **배포: 30m** |
| 결정론형 | `time_sin` | 4.2% | 0.9996 | 1.0분 | 계측만 |
| 계단형 | `cvd_direction_debias` | 92.6% | 0.8370 | 6.2분 | 계측만 |
| 계단형 | `multi_timeframe_5m` | 90.9% | 0.8723 | 10.0분 | 후보: POOL |
| 계단형 | `opt_atm_call_oi` | 91.7% | 0.9478 | 10.0분 | 후보: pending:15m |
| 계단형 | `opt_atm_pcr` | 90.8% | 0.9155 | 10.0분 | 후보: POOL, pending:15m |
| 계단형 | `opt_atm_put_oi` | 92.3% | 0.9416 | 10.0분 | 후보: pending:15m |
| 계단형 | `opt_chain_pcr` | 89.4% | 0.8116 | 10.0분 | 후보: pending:15m |
| 계단형 | `opt_gex_bn` | 89.2% | 0.9198 | 10.0분 | 후보: pending:15m |
| 누적형 | `swing_high_60m` | 84.0% | 0.9901 | 1.0분 | 계측만 |
| 누적형 | `swing_low_60m` | 83.4% | 0.9928 | 1.0분 | 계측만 |
| 상수형 | `bull_exhaustion_shadow` | 98.4% | -0.0026 | 1.0분 | 계측만 |
| 상수형 | `gp_ready_20` | 99.7% | 0.9472 | 24.5분 | 계측만 |
| 상수형 | `ma_cont_only` | 99.7% | 0.9824 | 61.0분 | 계측만 |
| 상수형 | `ma_regime_agree` | 99.7% | 0.9824 | 61.0분 | 계측만 |
| 상수형 | `ma_regime_down_cont` | 98.4% | 0.9579 | 52.5분 | 계측만 |
| 상수형 | `ma_regime_down_sess` | 98.9% | 0.9666 | 51.0분 | 계측만 |
| 상수형 | `ma_sess_ready` | 99.7% | 0.9824 | 61.0분 | 계측만 |
| 상수형 | `macro_nasdaq_chg` | 100.0% | 0.9973 | 3.0분 | 계측만 |
| 상수형 | `macro_risk_on` | 100.0% | 0.7239 | 4.5분 | 계측만 |
| 상수형 | `macro_sp500_chg` | 100.0% | 0.9973 | 3.0분 | **배포: 10m** |
| 상수형 | `macro_us10y_chg` | 100.0% | 0.9973 | 3.0분 | **배포: 3m, 10m** |
| 상수형 | `macro_vix` | 99.7% | 0.9703 | — | **배포: 10m, 30m** |
| 상수형 | `multi_timeframe_15m` | 96.2% | 0.9348 | 15.0분 | 후보: POOL |
| 상수형 | `opt_pcr_bearish` | 97.6% | 0.8688 | 5.8분 | **배포: 30m** |
| 상수형 | `opt_pcr_bullish` | 98.5% | 0.8735 | 5.5분 | 계측만 |
| 상수형 | `opt_pcr_extreme` | 97.4% | 0.8733 | 5.5분 | **배포: 10m** |
| 상수형 | `opt_pcr_extreme_bearish` | 97.4% | 0.8733 | 5.5분 | 계측만 |
| 상수형 | `opt_pcr_extreme_bullish` | 97.9% | 0.8618 | 3.0분 | 계측만 |
| 상수형 | `swing_ready_60m` | 99.7% | 0.9824 | 61.0분 | 계측만 |
| 상수형 | `trend_efficiency_ready` | 99.7% | 0.8999 | 22.2분 | 계측만 |
...
**10건** — 등재됐지만 등급이 아직 나쁜 항목 (최장 방치 57일). 등재 자체에는 만료·재검토 기한이 없으므로 여기서만 드러난다.

| 등급 | 피처 | 등재일 | 방치 | 소재 | 등재 사유 |
|---|---|---|---|---|---|
| DEAD | `bear_exhaustion` | 2026-07-28 | 57일 | 계측만 | 394차 섀도 복구 중 — EXHAUSTION_RESTORE_MODE 참조 |
| DEAD | `bear_exhaustion_signal` | 2026-07-28 | 57일 | 계측만 | 394차 섀도 복구 중 |
| DEAD | `bull_exhaustion` | 2026-07-28 | 57일 | 계측만 | 394차 섀도 복구 중 — EXHAUSTION_RESTORE_MODE 참조 |
| DEAD | `bull_exhaustion_signal` | 2026-07-28 | 57일 | 계측만 | 394차 섀도 복구 중 |
| DEAD | `cvd_exhaustion` | 2026-07-28 | 57일 | 계측만 | bear_exhaustion 별칭(deprecated) |
| DEAD | `cvd_exhaustion_signal` | 2026-07-28 | 57일 | 계측만 | bear_exhaustion_signal 별칭(deprecated) |
| DEAD | `macro_event_flag` | 2026-07-28 | 57일 | 계측만 | EVENT_DATES 빈 dict — 캘린더 미입력(Phase 0 §2-3) |
| CRITICAL | `cvd` | 2026-07-28 | 57일 | 계측만 | CVD 포화(+1.0 고착) — buy_vol 편향 파생, 미해결 등록 |
| CRITICAL | `cvd_direction` | 2026-07-28 | 57일 | 후보: pending:10m | CVD 포화 파생 — 미해결 등록 |
| WARN | `opt_gex_sign` | 2026-08-09 | 45일 | 후보: POOL, pending:15m | 시장구조(감마 음수 고착) — 계측 정상, 정보 중복. 451차 종결 |
...
**KNOWN 정리 가능**: 데이터 없음 3건(`program_foreign_net_krw`, `program_individual_net_krw`, `program_institution_net_krw`) — 원인이 해소됐는데 등재가 남아 있으면 그 항목이 앞으로의 진짜 이상을 조용히 삼킨다.

## 3. L4 — confidence 층화 검정 요약

출처: `data/horizon_conf_stratified_latest.json` (생성 2026-09-23, 창 20일)

| 호라이즌 | conf<0.55 acc(n) | conf≥0.55 acc(n) | z | p | 판정 |
|---|---|---|---|---|---|
| 10m | 0.461 (414) | 0.407 (27) | -0.55 | 0.5857 | INSUFFICIENT |
| 15m | 0.422 (147) | 0.500 (6) | 0.38 | 0.7040 | INSUFFICIENT |
| 3m | 0.500 (1481) | 0.737 (38) | 2.89 | 0.0039 | INSUFFICIENT |
| 5m | 0.522 (980) | 0.273 (11) | -1.65 | 0.0992 | INSUFFICIENT |

## 4. 후보 파이프라인 현황판

| 후보 | 출처 | 축적(최근 60거래일) | 건강 | 상태 |
|---|---|---|---|---|
| `basis_change_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 50일 | OK | 📌 승격 보류 — basis_pt와 동일 검정에서 함께 재현 실패 |
| `basis_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 50일 | OK | 📌 승격 보류 — 표본 확대 후 신호 소멸(재현 실패) |
...
| `foreign_futures_net` | pending:10m | 50일 | OK | 검증가능 (50일) |
| `hurst_ready` | POOL | 60일 | WARN | 검증가능 (60일) |
| `imbalance_slope` | POOL | 60일 | OK | 검증가능 (60일) |
| `is_month_end_rebalance` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 56일 | WARN | 검증가능 (56일) |
| `is_monthly_expiry_week` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 56일 | OK | 검증가능 (56일) |
| `is_monthly_witching` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 56일 | WARN | 검증가능 (56일) |
| `is_weekly_witching` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 56일 | OK | 검증가능 (56일) |
| `kyle_lambda` | POOL | 50일 | OK | 검증가능 (50일) |
| `micro_regime_code` | POOL | 60일 | OK | 검증가능 (60일) · 📌 1m 편입만 하지 않음 — 타 호라이즌 후보 자격은 유지 |
| `microprice_slope` | POOL | 60일 | OK | 검증가능 (60일) |
| `mlofi_norm` | POOL | 60일 | OK | 검증가능 (60일) |
| `multi_timeframe_15m` | POOL | 50일 | OK | 검증가능 (50일) |
| `multi_timeframe_5m` | POOL | 50일 | OK | 검증가능 (50일) |
| `ofi_reversal_speed` | pending:1m | 60일 | OK | 검증가능 (60일) |
| `opt_atm_call_oi` | pending:15m | 50일 | OK | 검증가능 (50일) · 📌 강등(include → pending) — magnitude 계열 재현 실패 |
| `opt_atm_pcr` | POOL, pending:15m | 50일 | OK | 검증가능 (50일) |
| `opt_atm_put_oi` | pending:15m | 50일 | OK | 검증가능 (50일) · 📌 강등(include → pending) — magnitude 계열 재현 실패 |
| `opt_chain_pcr` | pending:15m | 50일 | OK | 검증가능 (50일) · 📌 ⚠ CORE — 15m 승격만 보류. 30m CORE 지위는 그대로 유지 |
| `opt_gex_bn` | pending:15m | 50일 | OK | 검증가능 (50일) · 📌 강등(include → pending) — magnitude 계열 재현 실패, sign 계열로 대체 |
| `opt_gex_sign` | POOL, pending:15m | 50일 | WARN | 검증가능 (50일) |
| `program_arb_net` | pending:5m, pending:10m, pending:15m, pending:30m | 50일 | OK | 검증가능 (50일) |
| `program_non_arb_net` | pending:5m, pending:10m, pending:15m, pending:30m | 50일 | OK | 검증가능 (50일) |
| `queue_depletion_speed` | POOL | 60일 | OK | 검증가능 (60일) |
| `queue_directional_depletion` | POOL | 60일 | OK | 📌 1m 편입 종결 — 개선 대상 아님 (후보 재고에서 제외) |
| `queue_momentum` | POOL | 60일 | OK | 검증가능 (60일) |
| `queue_refill_rate` | POOL | 60일 | OK | 검증가능 (60일) |
| `queue_signal` | POOL | 60일 | OK | 검증가능 (60일) |
| `round_number_distance` | POOL | 50일 | OK | 검증가능 (50일) |
| `rv_iv_spread` | POOL | 49일 | OK | 검증가능 (49일) |
| `threshold_feasibility` | pending:15m | 60일 | OK | 📌 기각 — F4 재검증에서 부호 반전 (재현 실패) |
| `trend_efficiency` | POOL | 50일 | OK | 검증가능 (50일) |
| `vkospi` | pending:10m, pending:15m, pending:30m | 50일 | OK | 검증가능 (50일) |
| `vpin` | POOL | 50일 | OK | 검증가능 (50일) |
