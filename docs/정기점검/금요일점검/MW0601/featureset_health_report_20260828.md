# 호라이즌별 피처셋 주간 건강 리포트 — MW0601 · 2026-08-28
...
- 건강도 관찰창: **2026-07-31 ~ 2026-08-28** (20거래일, 7,527행)
- 후보 축적 관찰창: 2026-06-02 ~ 2026-08-28 (51거래일)
...
### 1m — 배포 8개 (pkl 2026-08-28 15:45)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| WARN | `is_open_volatile` | 7527 | 88.6% | 88.6% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 3m — 배포 12개 (pkl 2026-08-28 15:46)

전 피처 OK.

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 5m — 배포 12개 (pkl 2026-08-28 15:46)

| 등급 | 피처 | n | zero% | 최빈비중 | 고유값 | 비고 |
|---|---|---|---|---|---|---|
| CRITICAL | `is_close_volatile` | 7527 | 98.0% | 98.0% | 2 | 상태플래그(상수 정상) |
| WARN | `is_open_volatile` | 7527 | 88.6% | 88.6% | 2 | 상태플래그(상수 정상) |

CORE(short): `cvd_delta_norm`=OK / `vwap_position`=OK / `ofi_pressure`=OK

### 10m — 배포 11개 (pkl 2026-08-28 15:46)

전 피처 OK.

CORE(mid): `vwap_position`=OK

### 15m — 배포 13개 (pkl 2026-08-27 15:47)
...
### 30m — 배포 11개 (pkl 2026-08-28 15:47)
...
| CRITICAL | `bull_reversal_signal` | 97.7% | 97.7% | 계측만 (배포·후보 아님) |
...
| 계단형 | `multi_timeframe_5m` | 90.4% | 0.8749 | 8.2분 | 후보: POOL |
| 계단형 | `opt_atm_call_oi` | 91.5% | 0.8978 | 10.0분 | 후보: pending:15m |
| 계단형 | `opt_atm_pcr` | 90.8% | 0.8880 | 10.0분 | 후보: POOL, pending:15m |
| 계단형 | `opt_atm_put_oi` | 91.3% | 0.8953 | 10.0분 | 후보: pending:15m |
| 계단형 | `opt_chain_pcr` | 87.8% | 0.7439 | 10.0분 | 후보: pending:15m |
| 계단형 | `opt_gex_bn` | 87.5% | 0.9269 | 10.0분 | 후보: pending:15m |
| 상수형 | `bull_exhaustion_shadow` | 99.0% | -0.0026 | 1.0분 | 계측만 |
| 상수형 | `bull_reversal_signal` | 95.6% | -0.0227 | 1.0분 | 계측만 |
| 상수형 | `feature_quality_score` | 99.2% | 0.9115 | 1.5분 | 계측만 |
| 상수형 | `macro_nasdaq_chg` | 100.0% | 0.9973 | 3.0분 | 계측만 |
| 상수형 | `macro_risk_on` | 100.0% | 0.9694 | — | 계측만 |
...
| 상수형 | `multi_timeframe_15m` | 96.2% | 0.9284 | 15.0분 | 후보: POOL |
| 상수형 | `opt_pcr_bearish` | 98.0% | 0.9133 | 6.5분 | **배포: 30m** |
| 상수형 | `opt_pcr_bullish` | 98.5% | 0.8931 | 5.5분 | 계측만 |
| 상수형 | `opt_pcr_extreme` | 99.2% | 0.9076 | 7.2분 | **배포: 10m** |
| 상수형 | `opt_pcr_extreme_bearish` | 99.2% | 0.9076 | 7.2분 | 계측만 |
| 상수형 | `opt_pcr_extreme_bullish` | 98.8% | 0.8817 | 4.0분 | 계측만 |
...
출처: `data/horizon_conf_stratified_latest.json` (생성 2026-08-28, 창 20일)

| 호라이즌 | conf<0.55 acc(n) | conf≥0.55 acc(n) | z | p | 판정 |
|---|---|---|---|---|---|
| 10m | 0.553 (559) | 0.519 (52) | -0.47 | 0.6419 | INSUFFICIENT |
| 15m | 0.459 (231) | 0.469 (32) | 0.10 | 0.9163 | INSUFFICIENT |
| 3m | 0.513 (1898) | 0.435 (46) | -1.05 | 0.2933 | INSUFFICIENT |
| 5m | 0.492 (1216) | 0.500 (16) | 0.07 | 0.9479 | INSUFFICIENT |

## 4. 후보 파이프라인 현황판

| 후보 | 출처 | 축적(최근 60거래일) | 건강 | 상태 |
|---|---|---|---|---|
| `basis_change_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 32일 | OK | 📌 승격 보류 — basis_pt와 동일 검정에서 함께 재현 실패 |
| `basis_pt` | pending:1m, pending:3m, pending:5m, pending:10m | 32일 | OK | 📌 승격 보류 — 표본 확대 후 신호 소멸(재현 실패) |
| `cancel_ratio` | POOL | 0일 | raw부재 | 📌 구현불가 확정 — 데이터 원천 없음 (완전 사장) |
| `cvd_direction` | pending:10m | 51일 | CRITICAL | 📌 교체 완료 — cvd_delta_norm이 대체 (후보 목록의 잔존 항목) |
| `foreign_futures_net` | pending:10m | 36일 | OK | 검증가능 (36일) |
| `hurst_ready` | POOL | 44일 | WARN | 검증가능 (44일) |
| `imbalance_slope` | POOL | 51일 | OK | 검증가능 (51일) |
| `is_month_end_rebalance` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 38일 | CRITICAL | 검증가능 (38일) |
| `is_monthly_expiry_week` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 38일 | OK | 검증가능 (38일) |
| `is_monthly_witching` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 38일 | WARN | 검증가능 (38일) |
| `is_weekly_witching` | pending:1m, pending:3m, pending:5m, pending:10m, pending:15m, pending:30m | 38일 | OK | 검증가능 (38일) |
| `kyle_lambda` | POOL | 32일 | OK | 검증가능 (32일) |
| `micro_regime_code` | POOL | 51일 | OK | 검증가능 (51일) · 📌 1m 편입만 하지 않음 — 타 호라이즌 후보 자격은 유지 |
| `microprice_slope` | POOL | 51일 | OK | 검증가능 (51일) |
| `mlofi_norm` | POOL | 51일 | OK | 검증가능 (51일) |
| `multi_timeframe_15m` | POOL | 32일 | OK | 검증가능 (32일) |
| `multi_timeframe_5m` | POOL | 32일 | OK | 검증가능 (32일) |
| `ofi_reversal_speed` | pending:1m | 51일 | OK | 검증가능 (51일) |
| `opt_atm_call_oi` | pending:15m | 32일 | OK | 검증가능 (32일) · 📌 강등(include → pending) — magnitude 계열 재현 실패 |
| `opt_atm_pcr` | POOL, pending:15m | 32일 | OK | 검증가능 (32일) |
| `opt_atm_put_oi` | pending:15m | 32일 | OK | 검증가능 (32일) · 📌 강등(include → pending) — magnitude 계열 재현 실패 |
| `opt_chain_pcr` | pending:15m | 32일 | OK | 검증가능 (32일) · 📌 ⚠ CORE — 15m 승격만 보류. 30m CORE 지위는 그대로 유지 |
| `opt_gex_bn` | pending:15m | 32일 | OK | 검증가능 (32일) · 📌 강등(include → pending) — magnitude 계열 재현 실패, sign 계열로 대체 |
| `opt_gex_sign` | POOL, pending:15m | 32일 | OK | 검증가능 (32일) |
| `program_arb_net` | pending:5m, pending:10m, pending:15m, pending:30m | 36일 | OK | 검증가능 (36일) |
| `program_non_arb_net` | pending:5m, pending:10m, pending:15m, pending:30m | 36일 | OK | 검증가능 (36일) |
| `queue_depletion_speed` | POOL | 51일 | OK | 검증가능 (51일) |
| `queue_directional_depletion` | POOL | 48일 | OK | 📌 1m 편입 종결 — 개선 대상 아님 (후보 재고에서 제외) |
| `queue_momentum` | POOL | 51일 | OK | 검증가능 (51일) |
| `queue_refill_rate` | POOL | 51일 | OK | 검증가능 (51일) |
| `queue_signal` | POOL | 51일 | OK | 검증가능 (51일) |
| `round_number_distance` | POOL | 32일 | OK | 검증가능 (32일) |
| `rv_iv_spread` | POOL | 31일 | OK | 검증가능 (31일) |
| `threshold_feasibility` | pending:15m | 51일 | OK | 📌 기각 — F4 재검증에서 부호 반전 (재현 실패) |
| `trend_efficiency` | POOL | 32일 | OK | 검증가능 (32일) |
| `vkospi` | pending:10m, pending:15m, pending:30m | 32일 | OK | 검증가능 (32일) |
| `vpin` | POOL | 32일 | OK | 검증가능 (32일) |
