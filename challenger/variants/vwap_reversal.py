...
"""
VWAP 2σ 이탈 후 하락 압력 소진 상태 = 역추세 매수 신호.

조건:
  vwap_position < -1.5  (VWAP 하방 1.5σ 초과)
  bear_exhaustion > 0   (하락 압력 소진 감지 — 구: cvd_exhaustion)
...
class VwapReversalChallenger(BaseChallenger):
    ...
    BEAR_EXHAUSTION_MIN = 0.0    # bear_exhaustion 최소값 (존재만 해도 OK)
    ...
    def generate_signal(self, features, context):
        ...
        cond_exh  = bear_exhaustion >= self.BEAR_EXHAUSTION_MIN
        cond_pos  = close_price < vwap_val  # 아직 VWAP 아래

        if cond_vwap and cond_exh and cond_pos:
            # 이탈 깊이에 비례한 신뢰도 (최대 0.72)
            depth      = abs(vwap_position) - 1.5   # 1.5σ 초과분
            confidence = min(0.55 + depth * 0.05 + bear_exhaustion * 0.05, 0.72)
            direction  = 1
            grade      = self._grade_from_confidence(confidence)
            meta = {
                "vwap_position":   round(vwap_position, 3),
                "bear_exhaustion": round(bear_exhaustion, 3),
