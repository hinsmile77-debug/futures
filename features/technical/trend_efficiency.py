...
def calculate_trend_efficiency(
    ...
    Args:
        ...
        return_ready: True면 `(value, ready)` 튜플 반환 (502차 U-1)

    Returns:
        0.0~1.0 (표본 부족 시 중립값 0.5, 총이동거리가 0에 가까우면 0.0).
        `return_ready=True`면 `(값, ready)` — ready=False는 **표본 부족 폴백**이다.

    [502차 U-1] ready 플래그를 도입한 이유 — 계측 4원칙 ④("폴백이 쓰였으면 그 사실을
    남긴다"):
      표본 부족 폴백값 **0.5**는 진입 게이트 후보 임계 0.32보다 **크다**. 즉 게이트를
      배선하면 *"아직 계산할 수 없다"* 가 *"효율적인 구간이다"* 로 읽혀 **폴백이 게이트를
      통과한다.** 전 분봉의 0.49%(57/11,739)로 빈도는 낮지만 **장 초반에 몰리고**,
      하필 그 구간이 손실 집중 구간이다(501차 딥다이브 §7: 개장 1시간이 일 손실의
      46~74%). FP-CRITICAL(2개월 PSI=0.0)·TOX 죽은 섀도와 같은 계열의 씨앗이다.
      → 소비자는 `ready=False`를 **별도 취급**할 것. 0.5를 그대로 비교하지 말 것.
      근거: `docs/정기점검/손실6일_진입전인식방안_종합_20260830.md` §8-0.
