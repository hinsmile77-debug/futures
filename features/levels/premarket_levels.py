...
# ── [MW0601 542차 이식] 시점 격자 — 수동(임의 시각) 산출용 ────────────────
...
def fit_stage2(summaries, end):
    ...
    """P1(09:30) — `fit_stage2_at` 의 key=None 경로. 동작은 542차 이식 전과 같다."""
...
def compute_manual(at, today_bars, prev, atr, p1, p_at, candidates,
    ...
    """[MW0601 542차 이식] **임의 시각** 산출 — 수동 버튼 전용.
    ...
    반환 dict 은 `compute_stage()` 와 같은 모양(`rhat` 포함)에 `at`·`model` 두 키가
    더 붙는다 — 그래서 `levels_store.stage_warnings()` 를 그대로 태울 수 있다.
